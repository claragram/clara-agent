...
def _(rid, params: dict) -> dict:
    from clara_cli.input_sanitize import sanitize_user_prompt_text
                ...
                try:
                    ...
                    if hosted or peer:
                        return _err(
                            ...
                            + "Update Clara Desktop to continue it.",
                        )
    ...
    # over another app when they are back in Clara.
        ...
        if (
            ...
            if not is_truthy_value(params.get("confirm_truncate")):
                ...
                return _err(
                    ...
                    "(update your Clara client if a rewind was intended)",
                )
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from clara_cli.clipboard import has_clipboard_image, save_clipboard_image
...
def _(rid, params: dict) -> dict:
    ...
    def run():
        ...
        try:
            ...
            # CLARA_HOME override (the ContextVar set on the session-create
            ...
            home_token = (
                set_clara_home_override(_profile_home_str)
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "Restart exactly the app intended for the Preview URL, not Clara Desktop itself.",
            ...
            "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the Clara/Desktop process cwd.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not Clara Desktop. If it serves Clara/Desktop UI or another unrelated app, stop that process and report failure.",
    ...
    def run():
        ...
        try:
            ...
            # the session's CLARA_HOME override (the ContextVar set on the
...
def _(rid, params: dict) -> dict:
    # `text` is a JSON string describing the OS window underneath the Clara
