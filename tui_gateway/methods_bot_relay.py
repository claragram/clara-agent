...
(local, remote URL, SSH, Clara Cloud, docker) must be able to find every
...
def _(rid, params: dict) -> dict:
    ...
    ``clara -p <profile> chat -c "Bot Chat"`` transport local DMs use and
    ...
    try:
        ...
        resolved = "default" if profile.lower() == "clara" else profile
        ...
        fd, tmp = tempfile.mkstemp(prefix="clara-relay-dm-", suffix=".txt", text=True)
        try:
