...
handlers delegate to the same `clara_cli.profiles` primitives the REST
endpoints use.
...
def _(rid, params: dict) -> dict:
    """List Clara profiles (name, path, model, description, skill count).
    ...
    def _canonical_session_row(db, profile_path):
        ...
        describe the same session (clara-agent#88200) with no client-side
        pointer involved.
    ...
    def _latest_profile_session_rows(db):
        ...
        sessions never surface in conversation lists (clara-agent#90268).
    ...
    try:
        ...
        # append the protocol to SOUL.md (the desktop's clara-bots plugin)
...
def _(rid, params: dict) -> dict:
    ...
    ``clara setup`` in that flow to recover. A profile spawned as an
    ...
    # instead (clara_cli.auth: profile reads fall back to the global
    ...
    def _mirror_voice_sections() -> bool:
        ...
        profile via the context-local CLARA_HOME override — the same
        ...
        try:
            ...
            token = set_clara_home_override(str(path))
            try:
    ...
    if model and provider:
        try:
            from clara_cli.web_routers.profiles import _write_profile_model
        ...
        try:
            from clara_cli.config import load_config_readonly, read_user_config_raw
            from clara_cli.web_routers.profiles import _write_profile_model
            from clara_constants import (
                reset_clara_home_override,
                set_clara_home_override,
...
def _(rid, params: dict) -> dict:
    ...
    All reads are scoped to the profile via the CLARA_HOME override.
    """
    name = str(params.get("name") or "").strip()
    if not name:
    ...
    try:
        ...
        try:
            ...
            from clara_cli.skills_config import get_disabled_skills
            ...
            # Toolsets: the same filtered universe the `clara tools`
            ...
            # composites (clara-discord, feishu_drive, ...) and reports
            ...
            try:
                ...
                platform_enabled = set()
            try:
                from clara_cli.tools_config import _DEFAULT_OFF_TOOLSETS
            except Exception:
            ...
            for ts_name, ts_label, ts_desc in _get_effective_configurable_toolsets():
                ...
                # a per-profile editor — `clara tools` / Settings is where
            ...
            description = ""
            try:
                from clara_cli.profiles import read_profile_meta
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if isinstance(params.get("description"), str):
            try:
                from clara_cli.profiles import write_profile_meta
        ...
        if needs_cfg:
            ...
            try:
                ...
                if isinstance(params.get("disabled_skills"), list):
                    try:
                        from clara_cli.skills_config import save_disabled_skills
