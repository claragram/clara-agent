...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.clara/.env`` into the gateway process via
    ``clara_cli.config.reload_env``, matching classic CLI's ``/reload``
    ...
    """
    try:
        from clara_cli.config import reload_env
...
@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m clara_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        ...
        r = subprocess.run(
            [sys.executable, "-m", "clara_cli.main", *argv],
            capture_output=True,
            text=True,
            ...
            cwd=os.getcwd(),
            # cli.exec runs `python -m clara_cli.main` (can drive the agent) →
...
def _(rid, params: dict) -> dict:
    ...
    if name == "plan":
        ...
        # .clara/plans/ via write_file. Works on any backend.
    ...
    if name == "moa":
        ...
        try:
            from clara_cli.moa_config import moa_usage, normalize_moa_config
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # Re-bind CLARA_HOME to the session's profile so get_skill_commands()
        ...
        _home_token = (
            set_clara_home_override(_profile_home) if _profile_home else None
        )
        try:
...
def _(rid, params: dict) -> dict:
    try:
        ...
        api_key = get_secret("CLARA_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("CLARA_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_clara_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    # Optional profile scoping: cronjob() keys off CLARA_HOME, so scoping the
    ...
    if profile:
        try:
            ...
            from clara_constants import set_clara_home_override
    ...
    try:
        ...
        if token is not None:
            try:
                from clara_constants import reset_clara_home_override
...
def _(rid, params: dict) -> dict:
    ...
    gateway. Shares its renderer with the ``clara journey`` CLI.
    """
    try:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from clara_cli.skills_hub import do_install
        ...
        if action == "browse":
            from clara_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from clara_cli.skills_hub import inspect_skill
...
def _(rid, params: dict) -> dict:
    ...
    keys], transport}]}`` — the same catalog `clara mcp` offers, so
    ...
    try:
        ...
        from clara_cli import mcp_catalog
...
# (clara_cli/web_routers/mcp.py) so a desktop plugin can manage MCP servers for
# ANY profile, not just the launch profile. Each accepts an optional ``profile``
# param that scopes CLARA_HOME via set_clara_home_override (omitted/None = the
# launch profile) in a try/finally, exactly like ``skills.manage`` / ``mcp.catalog``.
# All persistence reuses clara_cli/mcp_config.py helpers — no logic is duplicated.
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from clara_cli.config import load_config, save_config, save_env_value
        from clara_cli.mcp_config import (
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from clara_cli.mcp_config import _remove_mcp_server
...
def _(rid, params: dict) -> dict:
    ...
    (``window.claraDesktop.openExternal``) and then polls
    ...
    the SAME interactive MCP OAuth machinery ``clara mcp login`` uses
    ...
    try:
        ...
        clara_home = str(get_clara_home().expanduser().resolve(strict=False))
        result = mcp_oauth_sessions.start_flow(
            clara_home, name, cfg, client_redirect_uri=client_redirect_uri
        )
        return _ok(
            rid,
            {
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``clara plugins`` / the dashboard, so the three surfaces
    ...
    Actions:
      ...
      - ``install`` → git-clone into ``~/.clara/plugins/`` (non-interactive).
    ...
    plugins live under each profile's CLARA_HOME, so a client can list or
                ...
                out.append(
                    {
                        ...
                        # skills/MCP format) vs a native Clara plugin.
