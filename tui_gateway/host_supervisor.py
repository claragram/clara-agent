...
def _default_registry_path() -> Path:
    return get_clara_home() / "state" / _REGISTRY_NAME
...
class HostSupervisor:
    ...
    def __init__(
        ...
        *,
        ...
        expected_clara_home: str | None = None,
        autostart: bool = True,
    ) -> None:
        ...
        self.expected_clara_home = expected_clara_home if expected_clara_home is not None else str(get_clara_home())
    ...
    def _spawn_locked(self, *, reason: str) -> None:
        ...
        if self.env:
            env.update(self.env)
        env["CLARA_COMPUTE_HOST_HEARTBEAT_SECS"] = str(self.heartbeat_secs)
    ...
    def _validate_hello(self) -> None:
        ...
        got_home = str(hello.get("clara_home") or "")
        if got_home and got_home != self.expected_clara_home:
            raise RuntimeError(f"compute host CLARA_HOME mismatch: {got_home} != {self.expected_clara_home}")
