...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.clara/.env ] && grep -q "^GITHUB_TOKEN=" ~/.clara/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.clara/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
    elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
      GITHUB_TOKEN=$(uv run python3 "${CLARA_HOME:-$HOME/.clara}/skills/github/github-auth/scripts/git-credential-token.py")
    fi
  fi
fi
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Clara Agent\",
    \"comments\": [
...
```bash
source "${CLARA_HOME:-$HOME/.clara}/skills/github/github-auth/scripts/gh-env.sh"
...
```bash
# 若无问题——批准
gh pr review $PR_NUMBER --approve --body "Reviewed by Clara Agent. Code looks clean — good test coverage, no security concerns."
