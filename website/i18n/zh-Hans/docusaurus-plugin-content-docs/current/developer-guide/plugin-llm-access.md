...
第三个完整示例（包含图像输入）位于
[`clara-example-plugins`](https://github.com/claragram/clara-example-plugins/tree/main/plugin-llm-example)
仓库（参考 plugin 的配套仓库——不随 clara-agent 本体打包）。关于异步接口（`acomplete()` / `acomplete_structured()` 与 `asyncio.gather()` 配合使用），请参见同一仓库中的
[`plugin-llm-async-example`](https://github.com/claragram/clara-example-plugins/tree/main/plugin-llm-async-example)。
...
* 实现：[`agent/plugin_llm.py`](https://github.com/claragram/clara-agent/blob/main/agent/plugin_llm.py)
* 测试：[`tests/agent/test_plugin_llm.py`](https://github.com/claragram/clara-agent/blob/main/tests/agent/test_plugin_llm.py)
* 参考 plugin（配套仓库）：
  * [`plugin-llm-example`](https://github.com/claragram/clara-example-plugins/tree/main/plugin-llm-example) — 带图像输入的同步结构化提取
  * [`plugin-llm-async-example`](https://github.com/claragram/clara-example-plugins/tree/main/plugin-llm-async-example) — 使用 `asyncio.gather()` 的异步示例
* 辅助客户端（底层引擎）：参见
