...
sidebar_position: 1
title: "Clara Portal"
description: "一个订阅，300+ 前沿模型，以及 Tool Gateway —— 运行 Clara Agent 的推荐方式"
---
...
还没有订阅？前往 [portal.claraprise.com/manage-subscription](https://portal.claraprise.com/manage-subscription) 注册，然后回来运行上面的命令。

## 订阅包含的内容

### 300+ 前沿模型，统一账单

Portal 代理了来自整个生态系统的精选 agentic 模型目录——统一计入你的 Clara 订阅，而非每个厂商单独充值。
...
| **MiniMax** | M2.7、M1 |
| **xAI** | Grok-4、Grok-3 |
| **Clara** | Clara-4-70B、Clara-4-405B（对话，见[下方说明](#a-note-on-clara-4)） |
| **+ 其他所有模型** | 240+ 额外模型——完整的 agentic 前沿生态 |

底层上，Portal 会为每个模型选择最合适的后端——部分模型通过 OpenRouter 路由，其他模型则通过专有或备用提供商，且某个模型的路由方式可能随时间调整。所有用量都统一计入你的 Clara 订阅。在会话中途用 `/model` 即可在 Claude Sonnet 4.6（适合代码）和 Gemini 2.5 Pro（适合长上下文）之间切换——无需新凭证，无需充值，不会遇到余额为零的意外报错。
...
同一订阅还解锁了 [Tool Gateway](/user-guide/features/tool-gateway)，将 Clara Agent 的工具调用路由至 Clara 托管的基础设施。五个后端，一次登录：
...
由于所有请求都通过一个经 OAuth 认证的 Portal 会话路由，你不会积累一个包含十几个长期 API 密钥的 `.env` 文件。磁盘上唯一的凭证是 `~/.clara/auth.json` 中的 refresh token（刷新令牌），Clara 会在每次请求时从中生成短期 JWT——详见下方[令牌处理](#token-handling)。
...
1. 打开浏览器跳转至 portal.claraprise.com 进行 OAuth 登录
...
如果还没有订阅，请先在 [portal.claraprise.com/manage-subscription](https://portal.claraprise.com/manage-subscription) 注册。
...
```bash
clara model
# 从提供商列表中选择 "Clara Portal"
# 浏览器打开，登录，完成
```

你现有的提供商配置保持不变。可以在会话中途用 `/model` 切换，或在会话间用 `clara model` 切换——Portal 成为你的可用提供商之一，而非唯一选项。

### 无头环境 / SSH / 远程配置

OAuth 需要浏览器，但回调的 loopback 运行在 Clara 所在的机器上。对于远程主机，请参阅 [OAuth over SSH / 远程主机](/guides/oauth-over-ssh)——与其他基于 OAuth 的提供商相同的方式同样适用于 Portal（`ssh -L` 端口转发）。

### Profile 配置

如果你使用 [Clara profiles（配置文件）](/user-guide/profiles)，Portal 的 refresh token 会通过共享令牌存储自动在所有 profile 间共享。在任意 profile 上登录一次，其余 profile 自动获取——无需为每个 profile 重复 OAuth 流程。
...
```bash
clara portal            # 登录 Clara Portal 并完成配置（一键引导）
clara portal info       # 登录状态、订阅信息、模型与 gateway 路由
clara portal tools      # 详细的 Tool Gateway 目录及每个工具的路由信息
clara portal open       # 在浏览器中打开订阅管理页面
```

`clara portal`（不带子命令）是 `clara auth add clara --type oauth` 的易记别名——它会登录、让你选择 Clara 模型、把 Clara 设为推理服务商，并提供 Tool Gateway 启用选项（与 `clara setup --portal` 等价，与首次快速设置走的是同一套 Clara 流程）。

`clara portal info` 给出高层概览：
...
如果你已有 Browserbase 账号并希望继续使用，同时通过 Clara 路由网页搜索和图像生成，这是支持的。使用 `clara tools` 为每个工具单独选择后端：

```bash
clara tools
# → 网页搜索       → "Clara Subscription"
# → 图像生成       → "Clara Subscription"
...
- **网页端：** [portal.claraprise.com/manage-subscription](https://portal.claraprise.com/manage-subscription)
- **CLI 快捷方式：** `clara portal open`（在默认浏览器中打开同一页面）

## 配置参考

运行 `clara setup --portal` 后，`~/.clara/config.yaml` 将如下所示：
...
```yaml
web:
  backend: clara       # 网页搜索/抓取通过 Tool Gateway 路由
...
OAuth refresh token 单独存储在 `~/.clara/auth.json`（不在 `config.yaml` 中——凭证与配置有意分开存放）。

## 令牌处理

Clara 在每次推理调用时从存储的 Portal refresh token 生成短期 JWT，而非复用长期 API 密钥。令牌生命周期完全自动管理——刷新、生成、在瞬时 401 时重试——你无需关心这些细节。

如果 Portal 使 refresh token 失效（修改密码、手动撤销、会话过期），失效的 refresh token 会被**本地隔离**，Clara 停止重放该令牌，你不会看到一连串相同的 401 错误。下一次调用会显示清晰的"需要重新认证"提示。运行 `clara auth add clara` 重新登录；隔离状态在下次成功登录时自动清除。

## 故障排查

### `clara portal info` 显示"not logged in"
...
或使用 `clara model` 重新选择 Clara Portal。

### 会话中途收到"需要重新认证"提示

你的 Portal refresh token 已失效（修改密码、手动撤销或会话过期）。运行 `clara auth add clara`，下一次请求将使用新凭证。旧令牌的隔离状态在成功重新登录后自动清除。
...
如果某个模型确实缺失，请[提交 issue](https://github.com/claraprise/clara-agent/issues)——我们将 Portal 目录同步至 Clara，缺口通常意味着可以更新的路由配置。

### 账单未出现在我的 Portal 账号中

先检查 `clara portal info`——如果显示你正在使用其他提供商（`Model: currently openrouter` 而非 `using Clara as inference provider`），说明本地配置已偏离。运行 `clara model`，选择 Clara Portal，下一次请求将通过你的订阅路由。

## 另请参阅

- **[Tool Gateway](/user-guide/features/tool-gateway)** —— 每个 gateway 工具的完整详情、每工具配置及定价
- **[订阅代理](/user-guide/features/subscription-proxy)** —— 在非 Clara 工具（其他 agent、脚本、第三方客户端）中使用你的 Portal 订阅
...
- **[Profiles](/user-guide/profiles)** —— 多个 Clara 配置共享一个 Portal 登录
