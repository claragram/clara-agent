...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/claraprise/clara-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    clara_meta = (fm.get("metadata") or {}).get("clara") or {}
    tags = clara_meta.get("tags") or []
    related = clara_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `clara skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Clara Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Clara ships with a large built-in skill library copied into `~/.clara/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Clara also syncs bundled skills on `clara update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.clara/skills/` tree, it is still shipped with Clara; restore it with `clara skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with clara-agent — install via clara skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with clara-agent under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
