...
This page covers all commands related to [Clara profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `clara profile` without a subcommand shows help.
...
```bash
clara profile use <name>
```

Sets `<name>` as the active profile. All subsequent `clara` commands (without `-p`) will use this profile.
...
clara profile use default
```

## `clara profile create`

```bash
clara profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `clara profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `clara update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.clara`), use `clara skills opt-out` / `clara skills opt-in`. |
...
clara profile create work2 --clone-from work

# Clone everything from a specific profile
clara profile create work2-backup --clone-from work --clone-all
```

## `clara profile describe`

```bash
clara profile describe [<name>] [options]
```
...
clara profile describe researcher

# Set it explicitly
clara profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
clara profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
clara profile describe --all --auto
```

## `clara profile delete`

```bash
clara profile delete <name> [options]
```
...
```bash
clara profile delete mybot
clara profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.clara`) cannot be deleted — use `clara uninstall` to remove everything.
:::

## `clara profile show`

```bash
clara profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Clara home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ clara profile show work
Profile: work
Path:    ~/.clara/profiles/work
...
```bash
clara profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Clara installation.
...
```bash
clara profile alias work
# Creates/updates ~/.local/bin/work

clara profile alias work --name mywork
# Creates ~/.local/bin/mywork

clara profile alias work --remove
# Removes the wrapper script
```

## `clara profile rename`

```bash
clara profile rename <old-name> <new-name>
```
...
```bash
clara profile rename mybot assistant
# ~/.clara/profiles/mybot → ~/.clara/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `clara profile export`

```bash
clara profile export <name> [options]
```
...
```bash
clara profile export work
# Creates work.tar.gz in the current directory

clara profile export work -o ./work-2026-03-29.tar.gz
```
...
## `clara profile import`

```bash
clara profile import <archive> [options]
```
...
```bash
clara profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

clara profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `clara -p telemetry`). |
...
clara profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
clara profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
clara profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
clara profile install ./telemetry/
```

### `clara profile update`

```bash
clara profile update <name> [--force-config] [--yes]
```
...
```bash
clara profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Clara version, author, env var requirements, the source URL/path, and
...
`clara profile list` also shows the distribution name and version in a
`Distribution` column, and `clara profile show <name>` / `delete <name>`
...
clara profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
clara profile install https://github.com/your-org/internal-assistant.git
```
...
`clara_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Clara version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Clara can clone from.
3. Tell recipients to run `clara profile install <your-repo-url>`.
...
## `clara -p` / `clara --profile`

```bash
clara -p <name> <command> [options]
clara --profile <name> <command> [options]
```

Global flag to run any Clara command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
clara -p work chat -q "Check the server status"
clara --profile dev gateway start
clara -p personal skills list
clara -p work config edit
```

## `clara completion`

```bash
clara completion <shell>
```
...
- `clara profile <TAB>` — subcommands (list, use, create, etc.)
- `clara profile use <TAB>` — profile names
- `clara -p <TAB>` — profile names
