...
description: "Run Clara Agent directly on an Android phone with Termux"
---

# Clara on Android with Termux
...
Clara Agent can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Clara from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
This APT repository is **community-maintained by `@adybag14-cyber` and is not an official Workprise distribution**. Workprise does not build, sign, host, or audit these packages. Enabling the repository means trusting the contributor-operated repository and its signing key. Termux itself remains a Tier 2 / best-effort platform.
:::
...
APT-managed Clara installs are marked with install method `apt`. Clara therefore does not run its Git self-updater against package-owned files; use the package manager instead:

```bash
pkg update
pkg upgrade clara-agent
```

Packaging/repository/signing problems for this option should be reported to the community packaging repositories above. Clara runtime bugs can still be reported here, keeping in mind that Android/Termux support is best-effort.
...
- links `clara` into `$PREFIX/bin` so it stays on your Termux PATH
...
Clara requires **Python >=3.11,&lt;3.14**. Current Termux ships `python`
...
### 5. Put `clara` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/clara" "$PREFIX/bin/clara"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `clara` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.clara/.env`.
...
### `clara doctor` says ripgrep or Node is missing
