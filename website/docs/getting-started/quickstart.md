---
sidebar_position: 1
title: "Clara Agent Quickstart"
description: "Your first conversation with Clara Agent — from install to chatting in under 5 minutes"
---

# Clara Agent Quickstart

This guide gets you from zero to a working Clara setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Clara Agent Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Clara Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Clara for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Clara working on my machine | `clara setup` | Run a real chat and verify it responds |
| I already know my provider | `clara model` | Save the config, then start chatting |
| I want a bot or always-on setup | `clara gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `clara model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `clara model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Clara cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.
...
The single most important setup step. Use `clara model` to walk through the choice interactively:

```bash
clara model
```

:::tip Easiest path: Clara Portal
...
:::info Setup modes
On a fresh install, `clara setup` offers three modes:

- **Quick Setup (Clara Portal)** — free OAuth login, no API keys; sets up a model plus the Tool Gateway tools. The recommended fast path.
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `clara update`. Re-enable anything later with `clara tools`, seed skills with `clara skills opt-in --sync`, or tune settings with `clara setup agent`.
:::
...
|----------|-----------|---------------|
| **Clara Portal** | Subscription-based, zero-config | OAuth login via `clara model` |
| **OpenAI Codex** | ChatGPT or Codex subscription, uses Codex models | Device code auth via `clara model` → **ChatGPT or Codex Subscription** |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `clara model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `clara_cli/models.py` may change between releases) | `clara model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `clara model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `clara model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `clara model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `clara model` (requires `copilot` CLI + `copilot login`) |
...
Clara Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `clara model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Clara separates secrets from normal config:

- **Secrets and tokens** → `~/.clara/.env`
- **Non-secret settings** → `~/.clara/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
clara config set model anthropic/claude-opus-4.6
clara config set terminal.backend docker
clara config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
clara            # classic CLI
clara --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Clara ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `clara` vs `clara --tui`.
:::
...
```bash
clara --continue    # Resume the most recent session
clara -c            # Short form
```
...
```bash
clara gateway setup    # Interactive platform configuration
```
...
- `clara tools` — tune tool access per platform
- `clara skills` — browse and install reusable workflows
...
```bash
clara config set terminal.backend docker    # Docker isolation
clara config set terminal.backend ssh       # Remote server
```

For Docker sandboxes, you can also enable the **egress credential-injection proxy** so the sandbox never sees your real API keys — only opaque proxy tokens that work exclusively from behind a local TLS-intercepting daemon. See [Egress proxy](../user-guide/egress/iron-proxy.md). Setup is `clara egress setup && clara egress start`; `clara setup terminal` also points Docker users at it. Modal, SSH, Daytona, and Singularity are not wired yet.

### Voice mode

```bash
# From the Clara install directory (the curl installer placed it at
# ~/.clara/clara-agent on Linux/macOS or %LOCALAPPDATA%\clara\clara-agent on Windows):
cd ~/.clara/clara-agent
...
Skills are on-demand instruction documents that teach Clara how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

Clara ships with a catalog of bundled skills already installed in `~/.clara/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
clara skills browse                      # list everything available
clara skills search kubernetes           # find skills by keyword
clara skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `clara skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let Clara ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.clara/clara-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Clara opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `clara model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `clara gateway setup` and check `clara gateway status` |
| `clara --continue` can't find old session | Switched profiles or session never saved | Check `clara sessions list` and confirm you're in the right profile |
...
| `clara doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `clara model`
3. `clara setup`
4. `clara sessions list`
5. `clara --continue`
6. `clara gateway status`
...
| `clara tools` | Configure which tools are enabled per platform |
| `clara setup` | Full setup wizard (configures everything at once) |
| `clara doctor` | Diagnose issues |
| `clara update` | Update to latest version |
| `clara gateway` | Start the messaging gateway |
| `clara --continue` | Resume last session |
...
- **[Moving to another machine](/reference/faq#exporting-clara-to-another-machine)** — `clara backup` migrates your whole setup (or [a single profile](/reference/faq#moving-a-single-profile-to-another-machine)); no need to rebuild from scratch
