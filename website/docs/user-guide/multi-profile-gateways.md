...
If you only run one Clara agent, you don't need this page — see
...
[Connecting Desktop to Many Clara Instances](./multi-connection-desktop.md).

## When to use this

You want this setup when you have two or more Clara agents that should all
...
(`ai.clara.gateway-<name>.plist`) or systemd user service
(`clara-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
clara profile create coder
clara profile create personal-bot
clara profile create research
...
```bash
clara config set gateway.multiplex_profiles true
clara gateway restart
```

Equivalently, in the default profile's `~/.clara/config.yaml`:
...
You do **not** run `clara gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `clara gateway start` / `run` is a
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.clara/profiles/coder/.env`; the default listener key is rejected.
...
home). `clara status` reports the multiplexer and the profiles it serves;
`clara status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/clara-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: clara-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    clara gateway "$action"
  else
    clara -p "$profile" gateway "$action"
  fi
}
...
```bash
clara-gateways start      # start every configured profile
clara-gateways stop       # stop every configured profile
clara-gateways restart    # restart all
clara-gateways status     # status across all
clara-gateways list       # delegates to `clara gateway list`
```

:::tip
The `default` profile is targeted with `clara gateway <action>` (no `-p`),
not `clara -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `clara -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.clara.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/clara-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.clara.gateway.plist` /
`clara-gateway.service`.
...
```bash
# Default profile
tail -f ~/.clara/logs/gateway.log
tail -f ~/.clara/logs/gateway.error.log

# Named profile
tail -f ~/.clara/profiles/<name>/logs/gateway.log
tail -f ~/.clara/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.clara/logs/gateway.log ~/.clara/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
clara logs -f                  # follow default profile
clara -p coder logs -f         # follow one profile
clara logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
clara profile list             # profiles + model + gateway state
clara-gateways status          # full status across every profile
launchctl list | grep clara    # macOS — PIDs and labels
systemctl --user list-units 'clara-gateway-*'   # Linux — units
```
...
The default profile uses `~/.clara/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
clara config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.clara/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=clara --why="gateways running" \
  sleep infinity &
...
`clara-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.clara/.env ~/.clara/profiles/*/.env
```

## Updating the code

`clara update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `clara gateway start` after a previous `clara gateway stop`. The
...
```bash
ps -ef | grep "clara_cli.*-p <profile>"
cat ~/.clara/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.clara.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.clara.gateway-<profile>.plist

# Linux
systemctl --user restart clara-gateway-<profile>.service
```

### Health check

```bash
clara doctor                  # default profile
clara -p <profile> doctor     # one profile
```
