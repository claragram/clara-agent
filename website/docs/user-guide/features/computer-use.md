...
Clara Agent can drive your desktop — clicking, typing, scrolling,
...
The built-in `computer_use` toolset is the recommended Clara integration. It
...
**Fresh installs already have the driver.** The Clara installer
...
- **`clara tools`** → pick `🖱️  Computer Use` — installs the driver
...
```
clara computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `clara computer-use
status` to verify the install.

Already have cua-driver? Clara reuses it when it supports the 0.20 runtime
contract. During setup, toolset enablement, `clara update`, and the first
`computer_use` call of a session, Clara checks the local version and
...
selected with `CLARA_CUA_DRIVER_CMD` stays
under your control, so Clara reports the incompatibility and leaves it
unchanged.

If you install Cua Driver first, `cua-driver skills install` installs Cua's
skill pack under `~/.cua-driver/skills/cua-driver`. Clara autodetection is a
planned cua-driver follow-up, so currently point Clara at that directory or
...
interface. The built-in toolset provides Clara actions, configuration,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording**. Grant the identity named by `clara computer-use doctor`. Standard mode uses CuaDriver.app; bounded and unrestricted modes use the Clara host identity. |
...
```
clara -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.clara/config.yaml`.

## Permission modes and logged-in browser profiles

Clara maps its existing approval UX onto cua-driver's immutable runtime
modes. Permission mode and capability manifest approval are launch settings.
They cannot change after the runtime starts:

| Clara session | cua-driver mode | Human intervention |
|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal Clara approvals; Cua stops at its protected boundary |
...
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit Clara risk acceptance; no runtime Cua prompts |
...
computer_use:
  ...
  capability_manifest: ~/.clara/cua-manifest.yaml
```
...
for the format). Clara launches a private runtime with
...
identity instead of resetting with every Clara build), and Clara verifies
...
resetting or closing the Clara session, cancellation cleanup, or process exit
closes that transport session. Clara also stops private runtimes that it
launched for bounded or unrestricted access. One Clara
conversation cannot change another runtime's mode or grants. Bounded and
unrestricted modes use a private
embedded service under the Clara host identity.
...
## `clara computer-use doctor` — your first triage stop

`clara computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ clara computer-use doctor
...
Clara run declares a public cua-driver **session name** (something like
`clara-3a7b9c14d2e8`). The name labels cursor identity and related state, so
...
without it. Clara disables it automatically where it is a known failure
...
Clara keeps its wrapper skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the Clara-side `computer_use` workflow and action vocabulary. For
...
The command installs the pack under `~/.cua-driver/skills/cua-driver`. Clara
autodetection is a planned cua-driver follow-up, so currently point Clara at
...
These are **platform deep dives, not duplicates of the Clara skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Clara
...
image capture also saves a bounded, shareable copy under Clara' image cache
...
Clara applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.clara/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Clara applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Clara agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Clara agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Clara warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Clara from inside the RDP / console
...
```
CLARA_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
CLARA_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Clara disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Clara sets
...
When it's on, `clara computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Clara at a binary you built from source instead
of the published release. Clara resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`CLARA_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Clara testing (see notes).
...
### Option B — point Clara straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`CLARA_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
CLARA_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
CLARA_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Clara is using your build

- `clara computer-use status` prints the resolved binary path and
  version.
- `clara computer-use doctor` confirms the binary is reachable and
...
- **Clara spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  Clara YOLO, Clara instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Clara
  change needed either way.
- **Local builds skip the version check.** Clara warns when the
...
**First action when anything's off: run `clara computer-use doctor`.**
...
Run `clara computer-use install` to fetch the cua-driver binary, or
run `clara tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `clara computer-use doctor` will
...
- **Clara-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  Clara `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  `LINUX.md` / `RECORDING.md` / `WEB_APPS.md`. Clara autodetection is a
  planned follow-up; currently point Clara at the installed pack directory
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Clara among them)
