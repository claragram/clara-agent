...
Clara runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Clara
...
1. Clara captures a baseline of current diagnostics for the file.
...
…). Clara auto-detects the binary on PATH or in
`<CLARA_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Clara at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<CLARA_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`clara lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Clara installs both packages together when you
run `clara lsp install typescript` or auto-install fires on first
use.

## CLI

```
clara lsp status          # service state + per-server install status
clara lsp list            # registry, optionally --installed-only
clara lsp install <id>    # eagerly install one server
clara lsp install-all     # try every server with a known recipe
clara lsp restart         # tear down running clients
clara lsp which <id>      # print resolved binary path
```

`clara lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <CLARA_HOME>/lsp/bin
...
When `install_strategy: auto`, Clara installs binaries into
`<CLARA_HOME>/lsp/bin/`. NPM packages land in
`<CLARA_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Clara-owned and is
...
**`clara lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<CLARA_HOME>/lsp/bin/`. Run
`clara lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `clara lsp status`**
...
When `clara lsp status` shows a `Backend warnings` section, install
...
`~/.clara/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.clara/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `clara lsp restart` to clear the set;
