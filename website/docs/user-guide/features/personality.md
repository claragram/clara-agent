...
description: "Customize Clara Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Clara Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `CLARA_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Clara is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Clara now seeds a default `SOUL.md` automatically in:

```text
~/.clara/SOUL.md
```

More precisely, it uses the current instance's `CLARA_HOME`, so if you run Clara with a custom home directory, it will use:
...
- Clara creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Clara loads `SOUL.md` only from `CLARA_HOME`
- Clara does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Clara falls back to a built-in default identity
...
If Clara loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `CLARA_HOME`, the personality belongs to the Clara instance itself.

That also makes it easier to teach users:
- "Edit `~/.clara/SOUL.md` to change Clara' default personality."
...
- how Clara should handle uncertainty, disagreement, or ambiguity
...
## What Clara injects into the prompt
...
Clara ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Clara, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Clara its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

Built-in personalities are always available on every surface (CLI, messaging platforms, TUI, and the desktop app). You can add your own — or override a built-in by reusing its name — in `~/.clara/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.clara/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-clara)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Clara speaks
- `display.skin` and `/skin` affect how Clara looks in the terminal
