...
When Clara Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.clara/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.clara/config.yaml` (switchable per session
via `/personality`).
...
If you use Clara' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.clara/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Clara Agent's deliverable
