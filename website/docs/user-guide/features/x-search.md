...
Clara can expose two different X surfaces:
...
:::tip
If you're paying Portal for an xAI model anyway, Live Search calls bill against the same xAI key configured for chat. See [Clara Portal](/integrations/clara-portal).
:::
...
|------------|--------|-------|
| **SuperGrok / X Premium+ OAuth** | Browser login at `accounts.x.ai`, refreshed automatically | `clara auth add xai-oauth` — see [xAI Grok OAuth (SuperGrok / X Premium+)](../../guides/xai-grok-oauth.md) |
| **`XAI_API_KEY`** (preferred) | Paid xAI API key | Set in `~/.clara/.env` |
...
Auto-enables when xAI credentials (OAuth token or `XAI_API_KEY`) are present. Disable explicitly via `clara tools` → Search → x_search if you don't want this.
...
The tool surfaces this when both auth paths fail. Either set `XAI_API_KEY` in `~/.clara/.env` or run `clara auth add xai-oauth` and complete the browser login. Then restart your session so the agent re-reads the tool registry.
...
1. **Toolset not enabled.** Run `clara tools` and confirm `🐦 X (Twitter) Search` is checked.
2. **No xAI credentials.** The check_fn returns False, so the schema stays hidden. Run `clara auth status` to confirm xai-oauth login state, and check that `XAI_API_KEY` is set (if you're using the API-key path).
