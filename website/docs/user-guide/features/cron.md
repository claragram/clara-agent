...
Schedule tasks to run automatically with natural language or cron expressions. Clara exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
All of this is available to Clara itself through the `cronjob` tool, so you can create, pause, edit, and remove jobs by asking in plain language — no CLI required.

:::tip
**Which model does a cron job run on?** Resolution at fire time is: per-job pin → `cron.model` in `config.yaml` → the global default from `clara model`.

- **Per-job pin** — set by *you* via the dashboard, `clara cron create/edit --model … --provider …`, or by editing `~/.clara/cron/jobs.json`. Once set, it sticks until you change it. The agent's `cronjob` tool cannot set or change per-job models — inference pins are user-owned.
- **`cron.model` / `cron.model_provider`** — a cron-fleet default: every unpinned job runs on this model, independent of your chat model. Set it once (`clara config set cron.model <name>`) and switching your chat model with `clara model` or `/model` never touches your cron fleet.
- **Global default** — only when neither of the above is set does a job follow `clara model`. In this case Clara **snapshots** the provider and model at creation, and if the global default later changes the job **fails closed**: it skips the run, makes no inference call, and alerts you **once** — the job stays skipped (and silent) on subsequent ticks until you act or the config is restored (#44585). For recurring or otherwise repeatable jobs, pin the provider/model explicitly (`clara cron edit <job_id> --provider <provider> --model <model>`) to proceed. A consumed finite one-shot cannot be updated; create a new future one-shot with an explicit provider and model instead. This prevents an unattended job from silently inheriting a switch to a paid provider/model. Setting `cron.model` (or a per-job pin) is the deliberate way to route cron spend, and the drift guard does not engage for an axis covered by it. Operators who instead want unpinned jobs to track the changing global default can [disable the drift guard](#letting-unpinned-jobs-track-global-defaults).
...
`clara setup --portal` is the lowest-friction option for unattended runs since OAuth refresh is automatic. See [Clara Portal](/integrations/clara-portal).
:::

:::tip
**Per-job reasoning effort.** A job can pin its own thinking level, independent of the model pin: one of `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`, `ultra`. When set, it overrides both the global `agent.reasoning_effort` and per-model `agent.reasoning_overrides` for that job's runs (`none` disables thinking). Set it via `clara cron create/edit --reasoning-effort high`; pass an empty string on edit to clear the pin and follow config again. (It is deliberately not exposed on the agent's `cronjob` tool — model configuration stays a user decision.) Levels a model doesn't support are clamped or omitted by the provider at request time — pinning `xhigh` on a model that caps at `high` runs at `high`. The pin has no effect on `no_agent` jobs (there is no LLM call to tune). Use it to run heavy scheduled analyses at `high` while cheap recurring jobs run at `minimal`, without touching your global default.
:::

:::warning
Cron-run sessions cannot recursively create more cron jobs. Clara disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
clara cron create "every 2h" "Check server status"
clara cron create "every 1h" "Summarize new feed items" --skill blogwatcher
clara cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill maps \
  --name "Skill combo"
```
...
Clara will use the unified `cronjob` tool internally.
...
Or: `clara config set cron.preflight false`
...
```bash
clara config set cron.model_drift_guard false
```
...
clara cron create "every 1d at 09:00" \
...
```bash
clara cron edit <job_id> --schedule "every 4h"
clara cron edit <job_id> --prompt "Use the revised task"
clara cron edit <job_id> --skill blogwatcher --skill maps
clara cron edit <job_id> --add-skill maps
clara cron edit <job_id> --remove-skill blogwatcher
clara cron edit <job_id> --clear-skills
```
...
```bash
clara cron list
clara cron pause <job_id_or_name>
clara cron resume <job_id_or_name>
clara cron run <job_id_or_name>
clara cron remove <job_id_or_name>
clara cron edit <job_id_or_name> [...flags]
clara cron status
clara cron tick
```
...
```bash
clara gateway install     # Install as a user service
sudo clara gateway install --system   # Linux: boot-time system service for servers
clara gateway             # Or run in foreground
...
1. loads jobs from `~/.clara/cron/jobs.json`
...
A file lock at `~/.clara/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.

### Execution history

Clara records each claimed cron attempt in the profile-local
`~/.clara/cron/executions.db` before executor or provider dispatch. Attempts
move through `claimed`, `running`, and one immutable terminal state:
`completed`, `failed`, or `unknown`. After restart, Clara marks an abandoned
...
Inspect recent attempts with `clara cron runs [job-id] --limit 20` (alias:
...
in a row and suggesting you fix, pause (`clara cron pause <job>`), or remove
it. Any successful run resets the streak, and `clara cron list` shows the
...
```bash
clara cron incidents                 # list incidents (newest activity first)
clara cron incidents --state alerted # filter: detected | alerted | closed
clara cron incidents ack <id>        # acknowledge — stop re-pinging
```
...
### Fleet health check: `clara cron doctor`

`clara cron doctor` is a read-only health check over every active job. It
...
- script missing, not a file, or resolving outside `CLARA_HOME/scripts`,
...
`clara cron incidents` (durable failure records) and `clara cron runs`
...
| `"local"` | Save to local files only (`~/.clara/cron/output/`) | Default on CLI |
...
`last_delivery_error`. `clara cron list` shows it in yellow as
`delivery_failed: <reason>`, `clara cron doctor` reports it as a delivery
...
- `bot-chat:<profile>` targets another profile **on the same machine**. Names are validated against `clara profile list` when the job is created; profiles on other gateways or machines can never be targeted, so same-named profiles across machines are unambiguous.
...
and in `clara cron doctor` as `last delivery unverified (...)`. The marker is
...
If the agent's final response contains `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.clara/cron/output/`), but no message is sent to the delivery target.
...
Or set the `CLARA_CRON_SCRIPT_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 3600s default.
...
Or set the `CLARA_CRON_MEDIA_SEND_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 300s default. A timed-out attachment is recorded in the job's run status as a partial delivery failure (the text still delivers).
...
`.sh` / `.bash` files run under `bash` from `PATH` when available, otherwise `/bin/bash` (important on Windows Git Bash). Anything else runs under the current Python interpreter (`sys.executable`). Scripts must resolve inside `$CLARA_HOME/scripts/` — relative names, absolute paths, and `~`-prefixed paths are accepted when the resolved target stays in that directory; paths that escape it are rejected. Subprocess env is sanitized (`_sanitize_subprocess_env`): provider API credentials and other Clara-managed secrets are **not** inherited by cron scripts.

### The agent sets these up for you

The `cronjob` tool's schema exposes `no_agent` to Clara directly, so you can describe a watchdog in chat and let the agent wire it up:

```text
Ping me on Telegram if RAM is over 85%, every 5 minutes.
```

Clara will write the check script to `~/.clara/scripts/` via `write_file`, then call:
...
cronjob(
    action="create",
    prompt="Fetch the top 10 AI/ML stories from Hacker News. Save them to ~/.clara/data/briefs/raw.md in markdown format with title, URL, and score.",
    schedule="0 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.clara/data/briefs/raw.md. Score each story 1–10 for engagement potential and novelty. Output the top 5 to ~/.clara/data/briefs/ranked.md.",
    schedule="30 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.clara/data/briefs/ranked.md. Write 3 tweet drafts (hook + body + hashtags). Deliver to telegram:7976161601.",
    schedule="0 8 * * *",
...
- When Job 2 fires, Clara reads Job 1's most recent output from `~/.clara/cron/output/{job1_id}/*.md`
...
From the CLI: `clara cron create "every 6h" "Scan for news" --continuity`, and `clara cron edit <job_id> --continuity` / `--no-continuity` to toggle it on an existing job. The same toggle appears in the dashboard's cron editor and the desktop Bot Mode routine dialog.
...
- `clara cron list` — a red `⚠ Missed scheduled fire:` line under the job
- The dashboard job view

The stamp always reflects **current** auto-fire health: it is overwritten by newer misses and cleared automatically by the next successful run. If you see it, the job and its schedule are fine — the gateway side of the fire path needs attention (most commonly, restart the gateway through its supervisor so it loads the full profile environment: `clara gateway restart`).
...
Runtimes that can't receive detached results (one-shot `clara -z`, `clara
...
Cron runs each job in a fresh agent session with no chat platform attached. By default the cron agent gets **the toolset you configured for the `cron` platform in `clara tools`** — not the CLI default, not everything under the sun.
...
When `enabled_toolsets` is set on a job it wins; otherwise the `clara tools` cron-platform config wins; otherwise Clara falls back to the built-in defaults. This matters for cost control: carrying `browser`, `delegation` into every tiny "fetch news" job bloats the tool-schema prompt on every LLM call.

### Skipping the agent entirely: `wakeAgent`

If your cron job attaches a pre-check script (via `script=`), the script can decide at runtime whether Clara should even invoke the agent. Emit a final stdout line of the form:
...
```bash
#!/bin/bash
# ~/.clara/scripts/feed-changed.sh
FEED="$HOME/data/feed.json"
STATE="$HOME/.clara/scripts/.feed-changed.last"
...
:::tip
Clara's own `~/.clara/state.db` is an internal schema that changes between releases. Don't query it from a pre-run gate — point at your own database or feed instead.
:::

Credit: this recipe set was prompted by @iankar8's exploration in [#2654](https://github.com/claraprise/clara-agent/pull/2654), which proposed adding sql/file/command triggers as a parallel mechanism. The `script` + `wakeAgent` gate already covers all three cases at $0, so the work landed as documentation instead.
...
Jobs are stored in `~/.clara/cron/jobs.json`. Output from job runs is saved to `~/.clara/cron/output/{job_id}/{timestamp}.md`.

Job definitions are plain JSON on disk: they survive `clara update`, gateway restarts, and machine reboots. A job that was mid-run during a restart is marked `unknown` in the execution ledger — it is not automatically retried, but the job's next scheduled tick fires normally. See [Execution history](#execution-history) for details.

:::tip
Ask the agent to manage jobs through the `cronjob` tool, `clara cron edit`, or `/cron` — not by patching `jobs.json` directly. Direct edits can fail silently when [file write safety](../security.md#file-write-safety) blocks the path (for example when `CLARA_WRITE_SAFE_ROOT` is set), and the [file-mutation verifier](../configuration.md#file-mutation-verifier) footer is the authoritative signal that nothing was saved.
:::

Jobs may store `model` and `provider` as `null`. When those fields are omitted, Clara resolves them at execution time from the global configuration. They only appear in the job record when a per-job override is set.
