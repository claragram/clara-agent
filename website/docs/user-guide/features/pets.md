...
Clara can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<CLARA_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Clara renders the sprite at full fidelity when your
...
clara pets install boba --select
...
| Goal | Command |
| --- | --- |
| Browse the gallery | `clara pets list [query] [--limit N]` |
| List installed pets | `clara pets list --installed` |
| Install a pet | `clara pets install <slug> [--select] [--force]` |
| Set the active pet | `clara pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `clara pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `clara pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `clara pets off` |
| Remove an installed pet | `clara pets remove <slug>` |
| Diagnose setup | `clara pets doctor` |

`clara pets show` flags:
...
Beyond installing pre-made pets from the gallery, Clara can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `clara pets` → the generate flow.
...
2. **Hatch** — the chosen base is used as a reference image to generate one grounded animation row per Clara state (idle, thinking, tool use, etc.), which are deterministically sliced into frames and packed into a standard petdex/Codex atlas (8×9 grid of 192×208 cells). The result is a valid spritesheet you keep — and could `petdex submit`.

### Image backend

Generation uses the active [image-generation provider](/user-guide/features/image-generation), but it requires **reference-image grounding** so each animation row stays the same character as the base. Reference-capable backends: **Clara Portal**, **OpenRouter**, **OpenAI** (`gpt-image-2`), and **Krea**. OpenRouter/Clara run a quality-first model chain by default.

- Resolution order prefers Clara Portal → OpenAI → OpenRouter.
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `clara tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `CLARA_PET_IMAGE_PROVIDER` env var (e.g. `CLARA_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Clara is
...
Run `clara pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; Clara uses its own
  profile-scoped `<CLARA_HOME>/pets/` instead — install through `clara pets`.

## See also

- The [`clara-agent` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-clara-agent.md)
