...
The web dashboard is a browser-based UI for managing your Clara Agent installation. Instead of editing YAML files or running CLI commands, you can configure settings, manage API keys, and monitor sessions from a clean web interface.

:::tip
Hosted-mode auth uses Clara Portal OAuth; if you also want the dashboard to talk to a real backend, `clara setup --portal` wires up the model and tool gateway too. See [Clara Portal](/integrations/clara-portal).
:::
...
```bash
# Custom port
clara dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
clara dashboard --host 0.0.0.0

# Start without opening browser
clara dashboard --no-open
```
...
child with the selected profile's `CLARA_HOME`, so the conversation runs
...
processes (manage them via `clara -p <name> gateway …`), each profile's
session database, and cron schedulers (the Cron page already aggregates
across profiles with its own filter).

## Prerequisites

The default `clara-agent` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
cd ~/.clara/clara-agent && uv pip install -e ".[web,pty]"
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `cd ~/.clara/clara-agent && uv pip install -e ".[all]"` includes both extras and is the easiest path if you also want messaging/voice/etc.

When you run `clara dashboard` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.

The Chat tab is part of every `clara dashboard` launch — the embedded browser chat pane (running the TUI over PTY/WebSocket) is always available, with no extra flag required.
...
- **Disk warnings** — the volume holding `~/.clara` is nearly full
...
- Node.js (same requirement as `clara --tui`; the TUI bundle is built on first launch)
- `ptyprocess` — installed by the `pty` extra (`cd ~/.clara/clara-agent && uv pip install -e ".[web,pty]"`, or `[all]` covers both)
...
```ini
[Service]
EnvironmentFile=%h/.clara/.env
ExecStart=/path/to/venv/bin/python -m clara_cli.main dashboard \
    --host 0.0.0.0 --port 9119 --no-open
```

with `~/.clara/.env` containing:

```bash
CLARA_DASHBOARD_BASIC_AUTH_USERNAME=admin
CLARA_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
CLARA_DASHBOARD_BASIC_AUTH_SECRET=<32+ random bytes; openssl rand -base64 32>
```
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `clara config set` and the gateway read from.
:::
...
Create and manage [profiles](../profiles.md) — isolated Clara instances with their own config, skills, and sessions.
...
- **Set as active** — flips the sticky default that **future CLI/gateway runs** pick up (same as `clara profile use`). This does *not* change what the dashboard manages — that's the profile switcher's job
- **Edit model / description / SOUL** — inline editors writing into that profile
- **Rename / Delete** — named profiles only

### Skills

Browse, search, and toggle installed skills and toolsets, and install new ones from the hub. Skills are loaded from `~/.clara/skills/` and grouped by category.
...
- **Browse hub** — a third view searches the skill hub across all sources (the same as `clara skills search`), installs any result by identifier with a live install log, and offers an "Update all" button to refresh installed skills.
...
block in `config.yaml` that `clara mcp` reads from.
...
**Catalog:** browse the Clara-approved MCP servers (the bundled `optional-mcps/`
...
`clara mcp catalog` / `clara mcp install` use.
...
Connect Clara to any messaging platform from the browser — full parity with
`clara setup gateway`. The page lists every supported channel (Telegram,
...
- **Restart gateway** — credentials are written to `~/.clara/.env` and the enabled flag to `config.yaml`; the gateway connects each enabled channel on its next restart, which you can trigger right from the page.
...
- **Host** — live system stats: OS / kernel, architecture, hostname, Python and Clara versions, CPU core count + utilization, memory, disk usage of the Clara home, uptime, and load average. (CPU/memory/disk come from `psutil` when installed; identity fields are always shown.) The Clara version shows an **update-status badge** (up to date / N commits behind) and a **Check for updates** button. When an update is available on a git install, an **Update now** button opens a confirmation dialog — showing how many commits you'll pull — before running `clara update` in the background. On Docker/Nix installs the dashboard can't apply the update in place, so it shows the correct out-of-band command instead.
- **Clara Portal** — login status, the active inference provider, and the Tool Gateway routing table (which tools run via the Portal vs. locally), with a link to manage your subscription. Read-only mirror of `clara portal`.
- **Skill curator** — the background skill-maintenance status (active / paused, interval, last run) with pause/resume and a run-now button. Mirrors `clara curator`.
...
![System admin page — host stats and Clara Portal status](/img/dashboard/admin-system-top.png)
...
This re-reads `~/.clara/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
`CLARA_HOME`. Omitted = the dashboard's own profile. Unknown profile names
...
- **`disk`** — a live `shutil.disk_usage()` sample of the volume holding
  `~/.clara`. Fields: `pressure`, `free_mb`, `total_mb`, `used_percent`,
...
| `GET /api/mcp/catalog` | Browse the Clara-approved MCP catalog |
...
| `GET /api/clara/update/check` | Report update availability (commits behind, install method) without applying. For git installs that are behind, also returns a `commits` list (`sha`, `summary`, `author`, `at`) of what's changed. `?force=1` busts the 6h cache |
| `GET /api/curator` · `PUT .../paused` · `POST .../run` | Skill-curator status + pause/resume + run |
| `GET /api/portal` | Clara Portal auth + Tool Gateway routing (read-only) |
...
|-------|-----------|----------|
| `clara dashboard` (default — binds to `127.0.0.1`) | OFF | Local development |
| `clara dashboard --host 0.0.0.0` | **ON** | Remote / production — protect with the username/password provider or OAuth |
...
If the gate would engage but **no** `DashboardAuthProvider` is registered (no Clara plugin, no custom plugin), `clara dashboard` refuses to bind with an explicit error message. There is no "default-deny but accept everything" fallback — a misconfigured gated dashboard never starts.
...
Because every login is verified against Clara Portal and protected by your Clara account, **the Clara provider is the one suitable for exposing a dashboard to the public internet.**

#### Registering a dashboard

To use the Clara provider you need an OAuth client ID (shape `agent:{id}`). There are two ways to get one:

- **CLI — `clara dashboard register`.** Run it on the host where the dashboard lives. It resolves your existing Clara login (run `clara setup` first if you're not logged in), registers a self-hosted OAuth client with the Portal, and writes `CLARA_DASHBOARD_OAUTH_CLIENT_ID` into `~/.clara/.env` for you. Optional flags: `--name` (a human-readable label, otherwise auto-generated) and `--redirect-uri` (a public HTTPS callback URL for an internet-facing host).
...
|---------|-----------|--------|----------------|
| `CLARA_DASHBOARD_OAUTH_CLIENT_ID` | `dashboard.oauth.client_id` | `agent:{instance_id}` | `clara dashboard register` |

Per the Clara Agent convention (`~/.clara/.env` is for API keys / secrets only), **`config.yaml` is the recommended place to set these values** for local dev, on-prem, and any deployment you control directly. The environment-variable path exists so a hosting platform's secret injection can push per-deploy `client_id`s without anyone having to edit `config.yaml` inside the image — that's its primary purpose.
...
Bundled providers reported these issues:
  • clara: CLARA_DASHBOARD_OAUTH_CLIENT_ID is not set (and
...
Configure an auth provider before exposing the dashboard:
  ...
  • OAuth: run `clara dashboard register` (Clara Portal) or install a
...
#### Worked example: Claraship

From a logged-in Clara install to a Clara-gated dashboard in three steps.

**1. Log in and register the dashboard.** `clara dashboard register` uses your existing Clara login to provision an OAuth client and writes `CLARA_DASHBOARD_OAUTH_CLIENT_ID` into `~/.clara/.env` for you:

```bash
clara setup            # if you're not already logged into Clara Portal
clara dashboard register
# ✓ Registered dashboard "swift_falcon"
# …writes CLARA_DASHBOARD_OAUTH_CLIENT_ID to ~/.clara/.env
```

**2. Run the dashboard on a reachable address.** A non-loopback bind engages the OAuth gate, and the `client_id` just written activates the `clara` provider:

```bash
clara dashboard --host 0.0.0.0 --port 9119 --no-open
```

**3. Log in.** Open `http://<host>:9119/`, you'll be bounced to `/login`. Click **Sign in with Claraship** → authenticate at the Portal → land back on the authenticated dashboard. Verify the gate from any machine:
...
`GET /api/auth/me` then returns the verified session (`provider: clara`). For an internet-facing host, register with `--redirect-uri https://clara.example.com/auth/callback` and set `CLARA_DASHBOARD_PUBLIC_URL` so the OAuth callback resolves to your public URL (see [Public URL override](#public-url-override)).
...
The username/password provider is intended for self-hosted / on-prem / homelab dashboards on a **trusted network**, or reachable only over a **VPN**. It protects a single shared credential with no external identity provider, MFA, or per-user accounts behind it, so it is **not suitable for exposing a dashboard directly to the public internet**. For an internet-facing dashboard, use the [Claraship provider](#default-provider-claraship) (or your own [self-hosted OIDC](#self-hosted-oidc-provider) / [custom OAuth](#custom-providers) provider) instead.
:::

#### Configuration

Like the Clara provider, it reads from `config.yaml` (canonical) with environment variables winning when set non-empty. It activates only when `username` plus either `password_hash` (preferred) or `password` are configured — otherwise it's a no-op, so OAuth users and loopback operators are unaffected.
...
|---------|-----------|-------|
| `CLARA_DASHBOARD_BASIC_AUTH_USERNAME` | `dashboard.basic_auth.username` | required to activate |
| `CLARA_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` | `dashboard.basic_auth.password_hash` | preferred (no plaintext at rest) |
| `CLARA_DASHBOARD_BASIC_AUTH_PASSWORD` | `dashboard.basic_auth.password` | plaintext; **wins over a config `password_hash`** so you can rotate via env |
| `CLARA_DASHBOARD_BASIC_AUTH_SECRET` | `dashboard.basic_auth.secret` | token-signing key |
| `CLARA_DASHBOARD_BASIC_AUTH_TTL_SECONDS` | `dashboard.basic_auth.session_ttl_seconds` | access-token lifetime |
...
**1. Set credentials in `~/.clara/.env`.** Hash the password so no plaintext sits at rest, and set a stable signing secret so sessions survive restarts:
...
cat >> ~/.clara/.env <<EOF
CLARA_DASHBOARD_BASIC_AUTH_USERNAME=admin
CLARA_DASHBOARD_BASIC_AUTH_PASSWORD_HASH=$HASH
CLARA_DASHBOARD_BASIC_AUTH_SECRET=$(openssl rand -base64 32)
EOF
chmod 600 ~/.clara/.env
```
...
`GET /api/auth/me` then returns the verified session (`provider: basic`). Keep this behind a VPN — see the warning above; for a public host use the [Claraship](#default-provider-claraship) or [self-hosted OIDC](#self-hosted-oidc-provider) provider instead.
...
If you run your own identity provider, the bundled `plugins/dashboard_auth/self_hosted` plugin authenticates the dashboard against it using **standard OpenID Connect** — no per-IDP code, no Clara Portal involved. It's verified against and works with any conformant OIDC server:

> **Authentik · Keycloak · Zitadel · Authelia · Auth0 · Okta · Google · …**

Like the Clara provider, it auto-loads and only registers itself once it's configured, so it's a no-op for loopback dashboards.
...
dashboard:
  oauth:
    ...
    self_hosted:
      issuer: https://auth.example.com/application/o/clara/   # required
      client_id: clara-dashboard                              # required
...
| Env var | Overrides | Notes |
|---------|-----------|-------|
| `CLARA_DASHBOARD_OIDC_ISSUER` | `dashboard.oauth.self_hosted.issuer` | OIDC issuer URL — required |
| `CLARA_DASHBOARD_OIDC_CLIENT_ID` | `dashboard.oauth.self_hosted.client_id` | Public client id — required |
| `CLARA_DASHBOARD_OIDC_SCOPES` | `dashboard.oauth.self_hosted.scopes` | Defaults to `openid profile email` |
...
**1. Run Keycloak with a pre-configured realm.** Save this realm export as `realm-clara.json` — it defines a `clara` realm, a **public PKCE client** (`clara-dashboard`), and a test user, all imported on boot so there's nothing to click in the admin UI:
...
```json
{
  ...
  "clients": [
    {
      "clientId": "clara-dashboard",
      "name": "Clara Agent Dashboard",
      "enabled": true,
      "publicClient": true,
...
docker run --rm -p 8080:8080 \
  ...
  -v "$PWD/realm-clara.json:/opt/keycloak/data/import/realm-clara.json:ro" \
  quay.io/keycloak/keycloak:26.0 \
  start-dev --import-realm
```

Once it's up, the realm advertises standard OIDC discovery at
`http://localhost:8080/realms/clara/.well-known/openid-configuration` (issuer
`http://localhost:8080/realms/clara`). The admin console is at
...
```bash
export CLARA_DASHBOARD_OIDC_ISSUER="http://localhost:8080/realms/clara"
export CLARA_DASHBOARD_OIDC_CLIENT_ID="clara-dashboard"
export CLARA_DASHBOARD_PUBLIC_URL="http://localhost:9119"
clara dashboard --host 0.0.0.0 --port 9119 --no-open
```

`CLARA_DASHBOARD_PUBLIC_URL` tells the dashboard its OAuth callback is
...
> admin console (Clients → clara-dashboard → Settings). The same pattern works
...
For deploys behind reverse proxies that don't reliably forward those headers (manual nginx setups, on-prem ingresses, custom-domain deploys with partial proxy chains), set `dashboard.public_url` (or `CLARA_DASHBOARD_PUBLIC_URL`) to the **complete public URL** the dashboard is reached at:

```yaml
dashboard:
  public_url: "https://dashboard.example.com/clara"
  trusted_proxies:
    - "172.20.0.5"
```
...
first; without one, Clara fails closed at startup. This prevents the local SPA
...
Clara always preserves loopback trust and rejects `*`, `0.0.0.0/0`, and
...
clara dashboard --host 127.0.0.1 --port 9119 --no-open
```
...
|---------|---------------|-------------|
| `dashboard.public_url` in `config.yaml` | `CLARA_DASHBOARD_PUBLIC_URL` | Local dev / on-prem (canonical) |
| `CLARA_DASHBOARD_PUBLIC_URL` env var | — | Hosting-platform secrets / CI |
...
The provider implements the [Clara Portal OAuth contract v1](https://github.com/claraship/clara-account-service/blob/main/docs/agent-dashboard-oauth-contract.md) — authorization-code grant with PKCE (S256):

1. User hits `/` without a session cookie → gate redirects to `/login`.
2. Login page shows a "Continue with Claraship" button → `/auth/login?provider=clara`.
3. Server stashes PKCE state in a short-lived cookie, redirects user to `https://portal.claraship.com/oauth/authorize?…`.
...
| Name | Lifetime | Notes |
|------|----------|-------|
| `clara_session_at` | Token TTL (15 min) | HttpOnly, SameSite=Lax, Secure-when-HTTPS |
| `clara_session_pkce` | 10 min | HttpOnly; holds the PKCE verifier + provider hint during the round trip. SameSite=None + Secure over HTTPS (must survive the cross-site IDP redirect chain — Chromium drops SameSite=Lax cookies set on a 302 in a cross-site chain); SameSite=Lax on loopback HTTP |
| `clara_session_rt` | unused in v1 | Reserved for forward-compat; not written when `refresh_token` is empty |
...
The sidebar widget shows `Logged in as <user_id…> via clara` with a logout icon. Clicking it POSTs `/auth/logout`, which clears all dashboard-auth cookies and redirects back to `/login`.

### Audit log

Every login start, success, failure, and session-verify failure is written as a JSON line to `$CLARA_HOME/logs/dashboard-auth.log`. Sensitive fields (`access_token`, `refresh_token`, `code`, `code_verifier`, `state`, `Authorization` header) are redacted before logging.

### Custom providers

To plug a non-Clara OAuth provider (e.g. Google, GitHub, custom OIDC), create a plugin that registers a `DashboardAuthProvider`:

```python
# ~/.clara/plugins/dashboard-auth-myidp/__init__.py
from clara_cli.dashboard_auth import DashboardAuthProvider, Session, LoginStart
...
```bash
# Quick env-var path.
CLARA_DASHBOARD_OAUTH_CLIENT_ID=agent:test \
  clara dashboard --host 0.0.0.0
...
You protect the remote dashboard with one of the bundled auth providers, and the desktop app signs in against whichever one the backend advertises. For a backend reachable beyond your own machine — a VPS, a public host, anything internet-facing — the recommended provider is **OAuth (Clara Portal)** (register it with [`clara dashboard register`](#registering-a-dashboard) and sign in with *Sign in with Claraship*). The bundled [username/password provider](#usernamepassword-provider-no-oauth-idp) is the quickest option when the backend is on a trusted LAN or reachable only over a VPN, but is **not suitable for direct public-internet exposure**. Binding the dashboard to a non-loopback address engages its auth gate; once signed in, Desktop reuses the session for the chat WebSocket automatically — there is no token to copy or paste.

The recipe below uses the username/password path because it's the quickest to stand up on a trusted network; for the OAuth path see [Default provider: Claraship](#default-provider-claraship).

### On the backend (the remote machine)

```bash
# 1. Set the dashboard login credentials in ~/.clara/.env (secrets file, 0600).
cat >> ~/.clara/.env <<'EOF'
CLARA_DASHBOARD_BASIC_AUTH_USERNAME=admin
CLARA_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
...
- **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/clara` are supported if you front it with a reverse proxy)
...
The session refreshes automatically and survives restarts when `CLARA_DASHBOARD_BASIC_AUTH_SECRET` is set on the backend.

### Environment-variable override

Instead of the in-app setting, you can point the desktop at a backend with an env var before launching it. When `CLARA_DESKTOP_REMOTE_URL` is set, it overrides the saved in-app URL (the Gateway settings panel shows an "env override" badge and disables editing); you still **Sign in** with your username and password from the panel.

| Env var | Value |
|---------|-------|
| `CLARA_DESKTOP_REMOTE_URL` | `http://<backend-host>:9119` |

### Troubleshooting

- **"Remote gateway incomplete"** — you haven't entered a remote URL.
- **Sign-in fails with 401 / "Invalid credentials"** — the username or password doesn't match the backend's `CLARA_DASHBOARD_BASIC_AUTH_USERNAME` / `CLARA_DASHBOARD_BASIC_AUTH_PASSWORD`. The backend returns the same generic error for unknown user and wrong password, so check both. Confirm the gate with `curl -s http://<host>:9119/api/status | jq '.auth_required, .auth_providers'` — it should report `true` and include `"basic"`.
...
- **Signed out on every restart** — set `CLARA_DASHBOARD_BASIC_AUTH_SECRET` to a stable value; otherwise the signing key is regenerated per boot.
...
The frontend is built with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui-style components. Production builds output to `clara_cli/web_dist/` which the FastAPI server serves as a static SPA.

## Automatic Build on Update

When you run `clara update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `clara dashboard` will build it on first launch.
...
| Theme | Character |
|-------|-----------|
| **Clara Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **Clara Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |
| **Clara Blue** (`clara-blue`) | Clara-branded blue accents with airy spacing |
