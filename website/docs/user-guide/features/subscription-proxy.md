...
description: "Use your Clara Portal subscription (or other OAuth provider) as an OpenAI-compatible endpoint for external apps"
---
...
chat completions — use your Clara-managed provider subscription as their
...
| Use case | "Use Clara as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Clara Portal OAuth flow. Clara stores
the refresh token in `~/.clara/auth.json` — the same place all Clara
...
Model:      Clara-4-70B    # or Clara-4.3-36B, Clara-4-405B
```
...
Currently shipped: `clara` (Clara Portal) and `xai` (xAI / Grok). More
OAuth providers can be added by implementing the `UpstreamAdapter`
interface in `clara_cli/proxy/adapters/`.
...
If you see `not logged in`, run `clara portal`. If you see
...
The proxy only forwards paths the upstream actually serves. For Clara
Portal:
...
supports; check `portal.claraprise.com/models`.
...
[portal.claraprise.com](https://portal.claraprise.com).
...
`clara_cli/proxy/adapters/<provider>.py` and registering it in
