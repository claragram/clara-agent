...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Clara's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Clara instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
clara memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `clara` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Clara syncs them back at session init.
...
The `clara honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `clara memory setup honcho` (or run `clara memory setup` and pick it from the list); the `clara honcho` subcommand then appears on the next invocation.

```bash
clara memory setup honcho    # Configure Honcho directly (works before activation)
clara honcho status          # Connection status, config, and key settings
clara honcho setup           # Redirects to `clara memory setup` (post-activation alias)
clara honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
clara honcho peer            # Show or update peer names + dialectic reasoning level
clara honcho mode            # Show or set recall mode (hybrid/context/tools)
clara honcho tokens          # Show or set token budget for context and dialectic
clara honcho identity        # Seed or show the AI peer's Honcho identity
clara honcho sync            # Sync Honcho config to all existing profiles
clara honcho peers           # Show peer identities across all profiles
clara honcho sessions        # List known Honcho session mappings
clara honcho map             # Map current directory to a Honcho session name
clara honcho enable          # Enable Honcho for the active profile
clara honcho disable         # Disable Honcho for the active profile
clara honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `clara honcho`

If you previously used the standalone `clara honcho setup`:
...
No re-login or re-setup needed. Run `clara memory setup` and select "honcho" — the wizard detects your existing config.
