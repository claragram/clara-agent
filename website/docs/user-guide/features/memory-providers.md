...
Clara Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
clara memory setup      # interactive picker + configuration
clara memory status     # check what's active
clara memory off        # disable external provider
```

You can also select the active memory provider via `clara plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.clara/config.yaml`:
...
When a memory provider is active, Clara automatically:
...
**Setup Wizard:**
```bash
clara memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `clara honcho setup` command still works (it now redirects to `clara memory setup`), but is only registered after Honcho is selected as the active memory provider.
...
**Config:** `$CLARA_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$CLARA_HOME/honcho.json` > `~/.clara/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/claraprise/clara-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/clara).
...
:::tip Migrating from `clara honcho`
If you previously used `clara honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Clara profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Clara profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Clara profile. Host key `clara` → default; `clara.<profile>` for others. |
...
```bash
clara profile create coder --clone
```

`--clone` creates a `clara.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Clara profile, creates host blocks for any profile without one, inherits settings from the default `clara` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `clara memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
See the [config reference](https://github.com/claraprise/clara-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/clara).
...
clara memory setup    # select "openviking"
# Or manually:
clara config set memory.provider openviking
```

`clara memory setup` can reuse or copy connection values from
`~/.openviking/ovcli.conf`. Manual setup uses the active profile's `.env` file;
for the default profile that is `~/.clara/.env`, and for named profiles use
`~/.clara/profiles/<profile>/.env`.
...
Peer identity is optional. By default, Clara sends no peer ID and writes
...
Set `memory.openviking.agent: clara` to restore the old peer-scoped writes.
...
Clara sends `User-Agent: openviking-memory-clara/<version>` on OpenViking
...
**Setup (Platform):**
```bash
clara memory setup    # select "mem0" → "Platform"
# Or manually:
clara config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.clara/.env
```

**Setup (OSS):**
```bash
clara memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
clara memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
clara memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Setup (Self-Hosted Dashboard):** connect to a Mem0 server you run via Docker (the dashboard's REST API):

```bash
clara memory setup    # select "mem0" → "Self-hosted server"
# Or via flags:
clara memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
```

Or configure manually — either as env vars:

```bash
echo "MEM0_HOST=http://localhost:8888" >> ~/.clara/.env
echo "MEM0_API_KEY=your-admin-api-key" >> ~/.clara/.env
```
...
**Config:** `$CLARA_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.clara/.env`.
...
| `user_id` | `clara-user` | User identifier |
| `agent_id` | `clara` | Agent identifier |
...
**Switching modes:** Re-run `clara memory setup mem0 --mode <platform|selfhosted|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
clara memory setup    # select "hindsight"
# Or manually:
clara config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.clara/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p clara ui start`

**Config:** `$CLARA_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `clara` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Clara Agent and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/claraprise/clara-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
clara memory setup    # select "holographic"
# Or manually:
clara config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.clara-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$CLARA_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
clara memory setup    # select "retaindb"
# Or manually:
clara config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.clara/.env
```
...
# Then configure Clara
clara memory setup    # select "byterover"
# Or manually:
clara config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$CLARA_HOME/byterover/` (profile-scoped)
...
| **Requires** | `pip install supermemory` + [cloud API key](http://app.supermemory.ai/integrations?connect=clara), or a [self-hosted server](https://supermemory.ai/docs/self-hosting/overview) |
...
**Setup:**
```bash
clara memory setup    # select "supermemory"
# Or manually:
clara config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.clara/.env
```
...
Before running `clara memory setup`, set `base_url` in
`$CLARA_HOME/supermemory.json`:
...
Then run `clara memory setup` and enter the API key printed by the local
server. Configuring the endpoint first ensures the setup connection probe also
stays local.

**Config:** `$CLARA_HOME/supermemory.json`
...
| `container_tag` | `clara` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `clara-{identity}` → `clara-coder`) to isolate memories per Clara profile
...
| **Requires** | `pip install clara-memori` + `clara-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install clara-memori
clara-memori install
clara config set memory.provider memori
clara memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `clara-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$CLARA_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$CLARA_HOME/` so each profile has its own credentials
