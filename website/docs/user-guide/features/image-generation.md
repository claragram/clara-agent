...
description: Generate images via FAL.ai — 11 models including FLUX 2, GPT Image (1.5 & 2), Nano Banana Pro, Ideogram, Recraft V4 Pro, Krea 2, and more, selectable via `clara tools`.
...
Clara Agent generates images from text prompts via FAL.ai. Eleven models are supported out of the box, each with different speed, quality, and cost tradeoffs. The active model is user-configurable via `clara tools` and persists in `config.yaml`.
...
Navigate to **🎨 Image Generation**, pick your backend (Clara Subscription or FAL.ai), then the picker shows all supported models in a column-aligned table — arrow keys to navigate, Enter to select:
...
```yaml
image_gen:
  provider: fal                 # `clara` if you picked Clara Subscription
  model: fal-ai/flux-2/klein/9b
  max_parallel_requests: 4      # concurrent images in one tool-call batch
```

`image_gen.provider` is the single selection key: `clara` routes through the managed Tool Gateway; a vendor name (`fal`, `openai`, `xai`, `krea`, ...) goes direct with your own key. The runtime always follows this stored selection — a `FAL_KEY` in `.env` is ignored while `provider: clara`, and `provider: fal` without `FAL_KEY` errors with `image_gen is configured to use fal (set via clara tools), but FAL_KEY is not set. Run 'clara tools' to change it.` rather than silently rerouting. Change providers via `clara tools`, not by adding/removing keys. (The old `use_gateway` boolean is legacy — still read as `clara` when `true`, but never written anymore.)

`max_parallel_requests` defaults to `4`. Clara clamps it to at least one and
...
new models appear in the picker as soon as OpenRouter serves them; no Clara
...
Clara Portal proxies the chat-completions protocol only, so its picker offers
...
only for now; responses are saved to `$CLARA_HOME/cache/images/`.
...
as a tool the chat model may call, and Clara cannot force the call — the
...
3. **Submission** — `_submit_fal_request()` routes via direct FAL credentials or the managed Clara gateway, according to the stored `image_gen.provider` selection.
...
- **Requires credentials** for the active backend (FAL `FAL_KEY` / Clara Subscription, `OPENAI_API_KEY`, xAI OAuth, `KREA_API_KEY`)
...
- **Temporary URLs** — backends return hosted URLs that expire after hours/days; Clara materializes them to the local cache so delivery still works after expiry
