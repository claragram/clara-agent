...
Clara Agent includes two model-callable web tools backed by multiple providers:

- **`web_search`** — search the web and return ranked results
- **`web_extract`** — fetch and extract readable content from one or more URLs

Both are configured through a single backend selection. Providers are chosen via `clara tools` or set directly in `config.yaml`.
...
| **xAI (Grok)** | `XAI_API_KEY` or `clara auth add xai-oauth` | ✔ | — | Paid (SuperGrok or per-token) |

Brave Search, DDGS, and xAI are **search-only** — pair any of them with Firecrawl/Tavily/Keenable/Exa/Parallel when you also need `web_extract`. DDGS uses the [`ddgs` Python package](https://pypi.org/project/ddgs/) under the hood; if it isn't already installed, run `pip install ddgs` (or let Clara lazy-install it on first use). xAI runs Grok's server-side `web_search` tool on the Responses API — results are LLM-generated rather than index-backed, so titles, descriptions, and URL choice are all model output (see the [trust-model caveat](#xai-grok) below).
...
| `web_extract` — same URL, same format, same provider | Full text stored under `~/.clara/cache/web/` | Shared across CLI, gateway, cron, and subagent processes |
...
### Quick setup via `clara tools`

Run `clara tools`, navigate to **Web Search & Extract**, and pick a provider. The wizard prompts for the required URL or API key and writes it to your config.
...
SearXNG is a privacy-respecting, open-source metasearch engine that aggregates results from 70+ search engines. **No API key required** — just point Clara at a running SearXNG instance.
...
To enable JSON responses for Clara, add the following override:
...
Then select SearXNG as the search backend in `~/.clara/config.yaml`:
...
Or set via `clara tools` → Web Search & Extract → SearXNG.
...
With this config, Clara uses SearXNG for all search queries and Firecrawl for URL extraction — combining free search with high-quality extraction.

---

### Tavily

AI-optimised search and extract. Select Tavily in `clara tools` (or set `web.backend: tavily`) to use it **keyless** with no account (rate-limited). Set an API key when you want higher limits.
...
2. `web.backend` (shared fallback; `clara` = managed Tool Gateway)
3. Auto-detect from environment variables (never-configured setups only)

### Auto-detection

If no backend has **ever** been selected (no `web.backend` / per-capability key written by you or `clara tools`), Clara picks the first available one based on which credentials are set:
...
| `FIRECRAWL_API_KEY` or `FIRECRAWL_API_URL` (or the Clara Tool Gateway is ready) | firecrawl |
| `SEARXNG_URL` | searxng |
...
Run `clara setup` to see which web backend is detected:
...
source ~/.clara/clara-agent/.venv/bin/activate
...
```bash
clara skills install official/research/searxng-search
```
