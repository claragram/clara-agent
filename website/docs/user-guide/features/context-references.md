...
Type `@` followed by a reference to inject content directly into your message. Clara expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Clara env: `$CLARA_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$CLARA_HOME/skills/.hub/`
