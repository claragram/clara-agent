...
`~/.clara/config.yaml`, `~/.clara/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/clara`:

```text
/etc/clara/
├── config.yaml     # managed config layer (wins over ~/.clara/config.yaml)
└── .env            # managed env layer (wins over ~/.clara/.env + shell)
```
...
The location can be relocated with the `CLARA_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `CLARA_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Clara.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export CLARA_MANAGED_DIR=/opt/org/clara-policy
```

:::warning
A user who can set `CLARA_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `clara doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/clara/config.yaml` (managed) | `/etc/clara/.env` (managed) |
| 2 | `~/.clara/config.yaml` (user) | `~/.clara/.env` (user) |
...
```bash
clara config        # shows a header naming the managed source + the pinned keys
clara doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Clara refuses and names the source:

```bash
$ clara config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/clara/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `clara config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/clara

# Pin some config values for every user on this machine
sudo tee /etc/clara/config.yaml >/dev/null <<'YAML'
model:
  provider: clara
security:
  redact_secrets: true
...
sudo tee /etc/clara/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/clara
sudo chmod 0644 /etc/clara/config.yaml /etc/clara/.env
```

Changes take effect on the next Clara start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`clara doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Clara as `root`), managed scope is advisory.
