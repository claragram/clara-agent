...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Clara Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Clara Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `clara setup --portal` — and you're ready to `clara chat`. See [Clara Portal](/integrations/clara-portal).
:::

:::tip
Clara also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `clara --tui` — see the [TUI](tui.md) guide.
:::
...
clara chat --query-file prompt.txt
clara chat --query-file - < prompt.txt

# With a specific model
clara chat --model "anthropic/claude-sonnet-4"

# With a specific provider
clara chat --provider clara        # Use Clara Portal
clara chat --provider openrouter  # Force OpenRouter

# With specific toolsets
clara chat --toolsets "web,terminal,skills"

# Start with one or more skills preloaded
clara -s clara-agent-dev,github-auth
clara chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
clara --continue             # Resume the most recent CLI session (-c)
clara --resume <session_id>  # Resume a specific session by ID (-r)
clara --resume latest        # Resume the most recent session (same as -c)
clara --resume latest --in ./dir  # Resume ./dir's latest session, staying in ./dir

# Verbose mode (debug output)
clara chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
clara -w                         # Interactive mode in worktree
clara -w -z "Fix issue #123"     # Single query in worktree
```

### Worktree cleanup

`clara -w` sessions create disposable worktrees under `<repo>/.worktrees/`.
...
```bash
clara worktree list              # audit: age, size, verdict, reason per tree
clara worktree prune             # remove safe trees + delete merged branches
clara worktree prune --dry-run   # show the plan without changing anything
clara worktree prune --trees-only     # leave local branches alone
clara worktree prune --branches-only  # leave worktrees alone
```
...
- Trees **in use by a running clara session** are never touched.
- **Untracked-only scratch** (PR body drafts, notes) is archived to
  `~/.clara/archive/worktree-prune/` before its tree is removed — never
  destroyed.
...
launches `clara -w` for days — no longer accumulate merged scratch trees
...
The `clara plugins` commands manage native Clara plugins and portable Agent
...
clara plugins disable <plugin-name>
clara plugins update <plugin-name>
clara plugins remove <plugin-name>
```

Portable packages remain disabled until explicitly enabled. Clara currently
...
<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Clara CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Clara CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `CLARA_YOLO_MODE` is on (either `clara --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Clara to background (Unix only). Run `fg` in the shell to resume. |
...
- **Approvals still apply.** A dangerous command (`rm -rf`, writes to `~/.clara/config.yaml`, etc.) goes through the same approval prompt the agent's `terminal` tool uses. `!` is a cost/latency shortcut, not a security bypass.
...
Type `/` to see the autocomplete dropdown. Clara supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Clara replies |
...
```bash
clara -s clara-agent-dev,github-auth
clara chat -s github-pr-workflow -s github-auth
```

Clara loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.clara/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.clara/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Clara..."
  # Add your own!
```
...
`Ctrl+J` and backslash continuation are enabled by default, matching Claude Code / Codex / OpenCode multiline shortcuts. On supported terminals such as iTerm2, Clara also requests extended key reporting so `Shift+Enter` arrives as a distinct newline key. If your terminal sends LF for plain `Enter` and you need the legacy `Ctrl+J`-as-submit fallback, opt out:
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Clara recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work by default. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Clara — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The first time you press Enter while Clara is working, Clara prints a one-line reminder explaining the `/busy` knob. It only fires once per install; `onboarding.seen.busy_input_prompt` in `config.yaml` records that it was shown. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Clara to the background — just like any terminal process. The shell prints a confirmation:

```
Clara Agent has been suspended. Run `fg` to bring Clara Agent back.
```
...
Resume this session with:
  clara --resume 20260225_143052_a1b2c3
...
```bash
clara --continue                          # Resume the most recent CLI session
clara -c                                  # Short form
clara -c "my project"                     # Resume a named session (latest in lineage)
clara --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
clara --resume "refactoring auth"         # Resume by title
clara --resume latest                     # Resume the most recent session (same as -c)
clara --resume latest --in ./my-project   # Latest session for ./my-project's workspace
clara -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `clara sessions rename <id> <title>` from the command line. Use `clara sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Clara's SQLite state database under `~/.clara/state.db`. The database keeps:
...
Clara immediately confirms the task and gives you back the prompt:
