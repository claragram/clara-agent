...
Connect Clara to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Clara supports two WeCom integration modes:
...
> Run `clara gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Clara decrypts the message, queues it for the agent
...
(Use `clara gateway start` only after `clara gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Clara and the token the
...
in `~/.clara/.env` values around `=` will also break signature checks. After
fixing, restart `clara gateway run`.
...
Check `clara gateway run` logs for the bound host/port. If the adapter bound to
