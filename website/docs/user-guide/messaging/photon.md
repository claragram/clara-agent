...
Connect Clara to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Clara — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Clara runs it in a
...
clara photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Clara Agent` project on your account.
...
6. **Runs `npm install`** inside the plugin's sidecar directory. On
   ...
   `~/.clara/photon/sidecar`; set `PHOTON_SIDECAR_DIR` to pin an
   explicit location.

Runtime credentials are written to `~/.clara/.env`
...
(device token, dashboard project id) lives in `~/.clara/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Clara
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Clara replies with a pairing code. Approve it with:

```bash
clara pairing approve photon <CODE>
```

Use `clara pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.clara/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.clara/.env`):
...
By default Clara responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Clara` and
`@Clara agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Clara will reply.
...
assigned iMessage line Clara uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run clara photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `clara photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `clara photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `clara photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Clara sends images, voice
  ...
  media.
- **Native polls are supported.** Clara sends poll content through
...
- **Read receipts are supported.** The sidecar marks an inbound iMessage
  read after forwarding it to Clara, so the sender sees `Read` without
  waiting for a model/tool turn. Inbound receipts for Clara-sent messages
...
- **Message effects are supported.** Clara sends text with native iMessage
...
- **Cron and standalone sends need the gateway running.** Out-of-process
  senders (cron jobs, `clara send`, the dashboard) reuse the sidecar the
  gateway spawned — they read its port/token from
  `<clara-home>/runtime/photon-sidecar.json`, written once the sidecar
...
- **Shared/free-tier lines can't initiate conversations with new
  ...
  brand-new recipient will be rejected by Photon even when Clara is set
...
| `PHOTON_MENTION_PATTERNS` | Clara wake words  | JSON list / comma / newline regex patterns for group mentions |
