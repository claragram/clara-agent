...
sidebar_position: 30
title: "Clara Relay"
description: "Connect Clara to messaging platforms through a relay connector that owns the platform credentials — enrollment, capabilities, config, and troubleshooting"
---

# Clara Relay (Connector)
...
Clara Relay is not a chat platform itself — it is a **connector system** that
...
1. Resolves a fresh Clara Portal access token from your existing login
   (`~/.clara/auth.json`) — this proves which Clara org (tenant) you own. If
   `gateway.idp.token_url` is configured, your own IdP is used instead (the
   air-gapped / self-hosted-IdP path, no Clara Portal involved): with
...
3. The connector verifies the token (signature, single-use, tenant match),
   ...
   once.
4. Persists the credentials into `~/.clara/.env`:
...
`clara gateway enroll` refuses to run in managed/hosted installs — there the
...
|---------|-------|---------|
| `GATEWAY_RELAY_URL` | env (`~/.clara/.env`) | Connector relay WebSocket URL. Presence enables the relay platform. |
...
| `gateway.idp.token_url` | `config.yaml` | When set, enrollment/provisioning authenticates against your own IdP instead of Clara Portal: OAuth2 client-credentials when `gateway.idp.client_id`/`client_secret` are also set; otherwise an ambient token endpoint (plain GET returning the token, raw or `{"access_token": …}`). |
...
token. Re-login with `clara auth add clara` (or `clara setup`) and retry.
...
startup. Restart it (`clara gateway restart`).
