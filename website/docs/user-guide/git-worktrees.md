...
title: "Git Worktrees"
description: "Run multiple Clara agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Clara Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Clara so each session has a clean, isolated working directory.

## Why Use Worktrees with Clara?

Clara treats the **current working directory** as the project root:

- CLI: the directory where you run `clara` or `clara chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.clara/config.yaml`
...
Clara creates `.worktrees/my-experiment/` inside the repo (branch
`clara/my-experiment`, based on the freshly-fetched remote tip unless
`worktree_sync: false`), and retargets the session's terminal and file tools
into it — no restart needed. Omit the name to get a random `clara-<id>`
...
git worktree add ../repo-feature feature/clara-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/clara-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Clara there:

```bash
cd ../repo-feature

# Start Clara in the worktree
clara
```
...
git worktree add ../repo-experiment-a feature/clara-a
git worktree add ../repo-experiment-b feature/clara-b
```
...
- Works on its own branch (`feature/clara-a` vs `feature/clara-b`).
...
- Clara checkpoint data under `~/.clara/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Clara experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/clara-checkpoints-docs`, `feature/clara-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Clara from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `clara -w` (Automatic Worktree Mode)

Clara has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `clara/clara-<hash>`).
...
```bash
clara -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `clara -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Clara session its own clean checkout.
