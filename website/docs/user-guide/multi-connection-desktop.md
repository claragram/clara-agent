...
# Connecting Desktop to Many Clara Instances

Register every Clara backend you own — the local runtime, remote gateways on
your LAN or VPS, SSH hosts, and Clara Cloud instances — in one desktop app,
...
- **The sidebar profile rail** — the plug button at the right end of the rail
  (tooltip: **"Connect another Clara gateway…"**) deep-links straight to
...
list of Clara gateways. Its intro says it plainly: *"Manage this device and
every Clara gateway it can reach through remote, SSH, or Cloud connections."*
Each entry is a *connection*:

| Kind | What it is | Auth |
|---|---|---|
| **Local** | "The Clara runtime managed by this app." | automatic |
| **Remote gateway** | "A Clara gateway reachable over HTTP(S) — LAN, Tailscale, or the internet." | session token or OAuth |
| **SSH** | "A Clara install reached over SSH." The app opens the tunnel and starts the dashboard for you | SSH key + adopted token |
| **Clara Cloud** | "A hosted instance discovered through your Clara Cloud account." | portal sign-in |
...
- **Duplicates are rejected when you save**: there is only ever one **local**
  ...
  **SSH** entries are deduplicated on the normalized `user@host:port` plus
  remote profile.
- Cloud entries normally come from the Clara Cloud sign-in/discovery flow at
  the top of the Gateways page — the **Clara Cloud** kind in the add-connection
...
3. Pick the kind: **Local**, **Clara Cloud**, **Remote gateway**, or **SSH**.
   (**Local** is disabled while the app-managed local entry exists — which is
   almost always; **Clara Cloud** directs you to the cloud sign-in/discovery
   flow above.)
4. Fill the fields:
   - **Name** — required, unique; the "device name" shown everywhere this
     instance appears (placeholder: `Homelab`). Max 64 characters.
   - *Remote gateway only:*
     - **Gateway URL** — the base URL of a running `clara serve` backend,
     ...
     - **Authentication** — choose **Session token** or **OAuth**:
       - **Session token** — paste the dashboard session token from the
         ...
         token."*
       - **OAuth** — sign in through the Clara Portal browser flow; no token
         to paste.
   - *SSH only:*
...
Gateways are machines or hosted backends; profiles are isolated Clara agents
...
connection is registered) dispatches `clara update` to every eligible
...
- **Clara Cloud** instances are skipped with a *"Managed by Clara Cloud"*
...
panel, ⌘K **Update Clara**, the update-ready toast) run the same fan-out
...
- **"Connection test failed"** — the backend isn't reachable at that URL from
  this machine. Check that `clara serve` is running on the remote host, the
...
- **"Update Clara Desktop to chat with agents on other connections"** — the
