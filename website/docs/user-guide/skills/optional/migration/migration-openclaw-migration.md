---
title: "Openclaw Migration — Import an OpenClaw setup (memories, skills) into Clara"
...
Import an OpenClaw setup (memories, skills) into Clara.
...
# OpenClaw -> Clara Migration

Use this skill when a user wants to move their OpenClaw setup into Clara Agent with minimal manual cleanup.
...
```bash
clara claw migrate              # Full interactive migration
...
clara claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `clara setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_clara.py` to:

- import `SOUL.md` into the Clara home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Clara memory entries
- merge OpenClaw command approval patterns into Clara `command_allowlist`
- migrate Clara-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Clara working-directory configuration
- copy OpenClaw skills into `~/.clara/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Clara workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.clara/tts/`
- archive non-secret docs that do not have a direct Clara destination
...
- `scripts/openclaw_to_clara.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.clara/skills/migration/openclaw-migration/scripts/openclaw_to_clara.py`

Do not guess a shorter path like `~/.clara/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.clara/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Clara-compatible secrets, currently:
...
Clara CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Clara. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Clara workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Clara skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python ~/.clara/skills/migration/openclaw-migration/scripts/openclaw_to_clara.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python /home/USER/.clara/skills/migration/openclaw-migration/scripts/openclaw_to_clara.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python ~/.clara/skills/migration/openclaw-migration/scripts/openclaw_to_clara.py --preset user-data
```

Execute a user-data migration:

```bash
python ~/.clara/skills/migration/openclaw-migration/scripts/openclaw_to_clara.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python ~/.clara/skills/migration/openclaw-migration/scripts/openclaw_to_clara.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python ~/.clara/skills/migration/openclaw-migration/scripts/openclaw_to_clara.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Clara unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Clara targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Clara destination. Unsupported auth blobs must still be reported as skipped.
...
- Clara persona state imported
- Clara memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.clara/skills/openclaw-imports/`
