...
| | |
|---|---|
| Source | Optional — install with `clara skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the Clara MCP catalog. The
...
Two halves, in this order: the editor side must be up before Clara connects.
...
### One-time, Clara side
    ...
    clara mcp install unreal-engine
...
`~/.clara/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Clara — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Clara connects
...
2. Start the Clara session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Clara
they appear as:
...
then happens at `clara mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Clara runs batched calls
...
- **Start order matters.** Editor + server up first, then the Clara
...
- **Timeouts on long operations.** Clara' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.clara/config.yaml` for
