...
| | |
|---|---|
| Source | Optional — install with `clara skills install official/creative/touchdesigner-mcp` |
...
```
Clara Agent -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${CLARA_HOME:-$HOME/.clara}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to Clara config (if missing)
...
3. **Restart Clara session** to pick up the new MCP server
