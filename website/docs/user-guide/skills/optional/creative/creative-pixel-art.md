...
| | |
|---|---|
| Source | Optional — install with `clara skills install official/creative/pixel-art` |
...
import sys
import os
sys.path.insert(0, os.path.expanduser("~/.clara/skills/creative/pixel-art/scripts"))
...
```bash
cd ~/.clara/skills/creative/pixel-art/scripts
...
- ffmpeg on PATH (only needed for video — Clara installs package this)
