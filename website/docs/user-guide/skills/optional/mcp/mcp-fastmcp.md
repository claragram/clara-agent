...
| | |
|---|---|
| Source | Optional — install with `clara skills install official/mcp/fastmcp` |
...
| Related skills | [`clara-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-clara-agent), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- smoke-test a server with the FastMCP CLI before wiring it into Clara or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Clara. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.clara/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.clara/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Clara integration, either:

- configure the server in `~/.clara/config.yaml` using the `native-mcp` skill, or
...
### Clara cannot see the deployed server

The server-building part may be correct while the Clara config is not. Load the `native-mcp` skill and configure the server in `~/.clara/config.yaml`, then restart Clara.
