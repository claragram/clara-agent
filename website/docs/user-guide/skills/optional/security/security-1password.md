...
| | |
|---|---|
| Source | Optional — install with `clara skills install official/security/1password` |
| Path | `optional-skills/security\1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Clara Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Clara terminal calls (desktop app flow only)
...
### Service Account (recommended for Clara)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${CLARA_HOME:-~/.clara}/.env` (the skill will prompt for this on first load).
...
## Clara Execution Pattern (desktop app flow)

Clara terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/clara-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/clara-op.sock"
