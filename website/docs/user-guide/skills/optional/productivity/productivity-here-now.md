...
| | |
|---|---|
| Source | Optional — install with `clara skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${CLARA_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${CLARA_SKILL_DIR}/scripts/drive.sh` for private Drive storage
...
| `--client {name}`      | Agent name for attribution (e.g. `clara`)    |
