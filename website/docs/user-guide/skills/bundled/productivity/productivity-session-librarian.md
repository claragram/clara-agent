...
| List/filter by metadata (age, source, cost, tokens, workspace) | `clara sessions list` / `stats` via terminal |
| Rename | `clara sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `clara sessions archive <filters>` |
| Delete (destructive) | `clara sessions delete` / `clara sessions prune <filters>` |
| Export before deleting anything valuable | `clara sessions export --session-id <id> --format md` |
...
`clara sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `clara sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `clara sessions list`
