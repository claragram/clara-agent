...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _clara_env="${CLARA_HOME:-$HOME/.clara}/.env"; [ -f "$_clara_env" ] && grep -q "^GITHUB_TOKEN=" "$_clara_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_clara_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
