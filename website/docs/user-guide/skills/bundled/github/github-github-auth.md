...
- Give it a name like "clara-agent"
- Select scopes:
...
- Give it a title like "clara-agent-&lt;machine-name>"
...
uv run python3 "${CLARA_HOME:-$HOME/.clara}/skills/github/github-auth/scripts/git-credential-token.py"
```
...
elif _clara_env="${CLARA_HOME:-$HOME/.clara}/.env"; [ -f "$_clara_env" ] && grep -q "^GITHUB_TOKEN=" "$_clara_env"; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_clara_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
  export GITHUB_TOKEN=$(uv run python3 "${CLARA_HOME:-$HOME/.clara}/skills/github/github-auth/scripts/git-credential-token.py")
  echo "AUTH_METHOD=curl"
else
