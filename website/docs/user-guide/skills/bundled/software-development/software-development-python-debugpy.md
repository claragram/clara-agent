...
- A long-running process (clara gateway, tui_gateway) misbehaves and you can't restart it
...
The clara test runner and pytest both support this:
...
For long-lived processes: Clara gateway, tui_gateway, a daemon, a process that's already misbehaving and can't be restarted clean.

### Setup

```bash
source <clara-agent-repo>/.venv/bin/activate
pip install debugpy
```
...
The easiest terminal-side DAP client is VS Code CLI or a small script. From inside Clara you have two practical options:
...
```json
{
  "name": "Attach to Clara",
  "type": "debugpy",
  "request": "attach",
  "connect": { "host": "127.0.0.1", "port": 5678 },
  "justMyCode": false,
  "pathMappings": [
    { "localRoot": "${workspaceFolder}", "remoteRoot": "<clara-agent-repo>" }
  ]
}
```
...
## Debugging Clara-specific Processes

### Tests
See Recipe 3. The wrapper captures subprocess output, so run pytest directly for interactive pdb.

### `run_agent.py` / CLI — one-shot
Easiest: add `breakpoint()` near the suspect line, then run `clara` normally. Control returns to your terminal at the pause point.

### `tui_gateway` subprocess (spawned by `clara --tui`)
...
debugpy.wait_for_client()
```
Start `clara --tui`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
...
9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For Clara subagents, debug one process at a time.
