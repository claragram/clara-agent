...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`clara-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-clara-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Clara terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Clara itself, `model.provider: openai-codex` uses Clara-managed Codex
OAuth from `~/.clara/auth.json` after `clara auth add openai-codex`. For the
...
## Clara Gateway Caveat

When invoking the Codex CLI from a Clara gateway/service context (for example,
