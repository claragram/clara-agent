...
description: "How Clara detects and responds to native JS dialogs and interacts with cross-origin iframes via a persistent CDP connection."
---

# Browser CDP Supervisor

The CDP supervisor closes two long-standing gaps in Clara' browser tooling:
...
(`clara-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs before
...
One `asyncio.Task` running in a background daemon thread per Clara `task_id`.
...
- `toolsets.py` — registers `browser_dialog` in `browser`, `clara-acp`, `clara-api-server`, and core toolsets (gated on CDP reachability)
- `clara_cli/config.py` — `browser.dialog_policy` and `browser.dialog_timeout_s` defaults
