...
description: "How to build a memory provider plugin for Clara Agent"
---

# Building a Memory Provider Plugin

Memory provider plugins give Clara Agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.

:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](/developer-guide/context-engine-plugin), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `clara plugins`.
:::

## Installation Layouts

Clara discovers memory providers from four sources, in this precedence order:

| Source | Location | Notes |
|---|---|---|
| Bundled | `plugins/memory/<name>/` | Ships with Clara. Closed to new providers — see [CONTRIBUTING](https://github.com/claraship/clara-agent/blob/main/CONTRIBUTING.md). |
...
Clara, in `$CLARA_HOME/plugins/<name>/` when installed by a user, or in
`./.clara/plugins/<name>/` for a project-local one:
...
`clara_agent.memory_providers` group. The entry-point name is the provider
name users select in `memory.provider`; its value points to the provider's
`register(ctx)` function:

```toml title="pyproject.toml"
[project.entry-points."clara_agent.memory_providers"]
...
package layout. No copy under `$CLARA_HOME/plugins/` is required.

A package entry point gets everything a directory install does, including the
two files Clara reads from disk rather than importing — `config_schema.py`
(the dashboard config panel) and `cli.py` (your `clara <provider>`
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          clara_home (str): Active CLARA_HOME path. Use for storage.
        """
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `clara memory setup` | **Yes** |
| `save_config(values, clara_home)` | Write non-secret config to native location | **Yes** (unless env-var-only) |
...
The gate binds to every compaction authority, not just the Clara
...
required checkpoint cannot be guaranteed there. The checkpoint-aware Clara
...
`get_config_schema()` returns a list of field descriptors used by `clara memory setup`:
...
Every field in `get_config_schema()` is prompted during `clara memory setup`. Providers with many options should keep the schema minimal — only include fields the user **must** configure (API key, required credentials). Document optional settings in a config file reference (e.g. `$CLARA_HOME/myprovider.json`) rather than prompting for them all during setup. This keeps the setup wizard fast while still supporting advanced configuration. See the Supermemory provider for an example — it only prompts for the API key; all other options live in `supermemory.json`.
:::

## Save Config

```python
def save_config(self, values: dict, clara_home: str) -> None:
    """Write non-secret config to your native location."""
    import json
    from pathlib import Path
    config_path = Path(clara_home) / "my-provider.json"
...
the `messages` parameter; Clara will continue calling them with the legacy
signature.
...
All storage paths **must** use the `clara_home` kwarg from `initialize()`, not hardcoded `~/.clara`:

```python
# CORRECT — profile-scoped
from clara_constants import get_clara_home
data_dir = get_clara_home() / "my-provider"

# WRONG — shared across all profiles
data_dir = Path("~/.clara/my-provider").expanduser()
```
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `clara my-provider status`, `clara my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `clara <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `clara --help`.
...
def my_command(args):
    ...
    if sub == "status":
        ...
        print("Usage: clara my-provider <status|config>")

def register_cli(subparser) -> None:
    """Build the clara my-provider argparse tree.
