...
Plugins can launch and supervise fresh Clara child sessions without importing
...
active agent turn fails closed with `No active Clara parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until Clara can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; Clara's existing unsafe-tool block remains enforced.
