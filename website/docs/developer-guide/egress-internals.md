...
description: "How the iron-proxy egress firewall integrates with Clara — module layout, lifecycle, security invariants, and extension points"
---

# Egress proxy internals

This page covers the architecture of the egress credential-injection firewall (`clara egress` / iron-proxy) from a contributor / plugin author's perspective. End-user setup + usage docs live at [Egress proxy](../user-guide/egress/iron-proxy.md).
...
clara_cli/proxy_cli.py               Wizard + slash command handlers.
                                       `clara egress {install,setup,start,stop,
                                       status,disable,config}`.  Wires the
                                       core module into argparse.

clara_cli/main.py:_dispatch_egress   Top-level subparser dispatcher.
                                       dest='egress_command' (intentionally
                                       disjoint from the inbound OAuth
                                       `clara proxy` subparser, which uses
                                       dest='proxy_command').

clara_cli/config.py: proxy schema    The `proxy:` block in DEFAULT_CONFIG.
...
tools/environments/docker.py
  ...
  DockerEnvironment.__init__          Docker-side merge logic: collision
                                       ...
                                       _CLARA_EGRESS_NODE_OPTIONS_APPEND
...
tests/test_iron_proxy_e2e.py          Live E2E (gated on CLARA_RUN_E2E=1).
...
clara egress install
  -> agent.proxy_sources.iron_proxy.install_iron_proxy(force=...)
       ...
       Stage into ~/.clara/bin/.iron-proxy_XXXX, chmod 755, os.replace
         to ~/.clara/bin/iron-proxy (atomic).
       _VERSION_CACHE.pop(target) so a forced reinstall re-probes
         --version on next call.

clara egress setup [--from-bitwarden | --no-bitwarden] [--rotate-tokens]
  -> proxy_cli.cmd_setup
...
| Path | Mode | Test |
|---|---|---|
| `~/.clara/proxy/` (dir) | `0o700` | `test_proxy_state_dir_is_0o700` |
...
`merge_mappings(existing, discovered, rotate=False)` MUST return prior tokens for providers that overlap.  Re-running `clara egress setup` cannot silently 401 running sandboxes.  `--rotate-tokens` is the explicit opt-in.
...
`cmd_setup` MUST NOT downgrade `credential_source: bitwarden` to `env` on re-run without an explicit `--no-bitwarden` flag.  Running `clara egress setup` (no flag) preserves whatever was previously configured.
...
If the provider uses SigV4 / SDK-minted OAuth / request signatures, a static header swap cannot cover it.  Add the env var to `_NON_BEARER_PROVIDERS` so the wizard and `clara egress status` warn about it:
...
4. Sets the seven env vars (HTTPS_PROXY, NO_PROXY, REQUESTS_CA_BUNDLE, SSL_CERT_FILE, CURL_CA_BUNDLE, NODE_EXTRA_CA_CERTS, CLARA_EGRESS_PROXY) and the per-mapping `CLARA_PROXY_TOKEN_<NAME>` vars.
5. Distributes the CA cert into the sandbox at a path the runtime will trust (typically `/etc/ssl/certs/clara-egress-ca.crt`).
...
CLARA_RUN_E2E=1 scripts/run_tests.sh tests/test_iron_proxy_e2e.py

# Live PTY smoke against `clara egress`
CLARA_HOME=/tmp/clara-egress-test python3 -m clara_cli.main egress --help
CLARA_HOME=/tmp/clara-egress-test python3 -m clara_cli.main egress setup --help
```
...
- User-facing setup + troubleshooting: [Egress proxy](https://agent.claraprise.com/docs/user-guide/egress/iron-proxy)
- Docker backend internals: [Docker](https://agent.claraprise.com/docs/user-guide/docker)
- Bitwarden Secrets Manager integration: [`clara secrets bitwarden`](https://agent.claraprise.com/docs/user-guide/secrets/bitwarden)
- CLI command reference: [`clara egress`](https://agent.claraprise.com/docs/reference/cli-commands#clara-egress)
- Sandbox-injected environment variables: [Egress proxy (sandbox-injected)](https://agent.claraprise.com/docs/reference/environment-variables#egress-proxy-sandbox-injected)
