...
| Variable | Meaning |
|----------|---------|
| `CLARA_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `CLARA_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `CLARA_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by Clara itself — they're private to these helpers. The variables Clara *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `clara --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:

```bash
htui() {
  local root
  root="$(_clara_root)" || { echo "htui: not in a Clara checkout" >&2; return 1; }
  ( cd "$root" && PYTHONPATH="$root" \
      "$CLARA_MAIN_CHECKOUT/.venv/bin/python" -m clara_cli.main --tui --dev "$@" )
}
```

`--dev` compiles from source, so it links `ui-tui/node_modules` from `CLARA_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_clara_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `CLARA_TUI_DIR` are mutually exclusive
`CLARA_TUI_DIR` points Clara at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `clara --tui --dev` exits with an error. Run `unset CLARA_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_clara_root)" || { echo "hgui: not in a Clara checkout" >&2; return 1; }
  deps="${CLARA_GUI_DEPS_CHECKOUT:-$CLARA_MAIN_CHECKOUT}"
  ...
  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _clara_link_deps "$desktop" "$deps/apps/desktop"
    _clara_link_deps "$root" "$deps"
  else
  ...
  trap '_clara_gui_cleanup "$root"' INT TERM EXIT

  ( cd "$desktop"
    export PATH="$root/node_modules/.bin:$PATH"
    CLARA_DESKTOP_CLARA_ROOT="$root" \
    CLARA_DESKTOP_PYTHON="$CLARA_MAIN_CHECKOUT/.venv/bin/python" \
    CLARA_DESKTOP_IGNORE_EXISTING=1 \
    CLARA_DESKTOP_CWD="$root" \
    npm run dev )
}
```
...
|----------|----------------|
| `CLARA_DESKTOP_CLARA_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `clara`. |
| `CLARA_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `CLARA_DESKTOP_IGNORE_EXISTING` | Ignores any `clara` on `PATH` so it can't shadow the worktree. |
| `CLARA_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
```bash
# The enclosing worktree, verified as a real Clara checkout.
_clara_root() {
  local root
  root="$(git rev-parse --show-toplevel 2>/dev/null)" || return 1
  [[ -f "$root/clara_cli/main.py" && -d "$root/ui-tui" ]] && print -r "$root"
}
...
_clara_gui_cleanup() {
  ...
  lsof -t -i:5174 >/dev/null 2>&1 && killport 5174
  pgrep -f 'clara_cli\.main.*dashboard.*--port 0' 2>/dev/null | xargs -r kill -TERM 2>/dev/null
}
```
...
- [TUI](../user-guide/tui.md) — `clara --tui --dev` and the `CLARA_TUI_DIR` prebuild path
- [Desktop App](../user-guide/desktop.md) — building from source and the backend resolution ladder
- [`apps/desktop/README.md`](https://github.com/claraprise/clara-agent/blob/main/apps/desktop/README.md) — dev server, sandbox script, and packaging
- [Environment Variables](../reference/environment-variables.md) — every `CLARA_*` variable Clara reads
