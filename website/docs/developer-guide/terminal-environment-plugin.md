...
Clara runs shell commands through a pluggable set of **terminal backends**.
...
installed under `~/.clara/plugins/`, registering a backend the user selects
...
| `clara setup` backend picker | `display_name`, `description`, `setup_instructions()`, `post_setup()` |
| Dashboard terminal-backend picker (probe status) | `probe()` |
| `clara status` / `clara doctor` | `doctor_checks()` |
...
```python title="~/.clara/plugins/acmebox/__init__.py"
...
class AcmeBoxProvider(TerminalEnvironmentProvider):
    ...
    def cache_path_base(self):
        return "~/.clara"    # where synced cache files land, or None
...
```yaml title="~/.clara/plugins/acmebox/plugin.yaml"
name: acmebox
version: 0.1.0
...
```bash
clara plugins enable acmebox
clara config set terminal.backend acmebox
```
