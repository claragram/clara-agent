...
title: "Desktop Plugin SDK (@clara/plugin-sdk)"
description: "Extend the native Clara Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [Clara Desktop](/user-guide/desktop) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `ClaraPlugin`.
It imports one module — `@clara/plugin-sdk` — and gets everything: the app's
...
`$CLARA_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across Clara. This page is the **native
desktop app** (`clara desktop`) SDK — the `@clara/plugin-sdk` module and
`$CLARA_HOME/desktop-plugins/`. The **web dashboard** (`clara dashboard`) has
its own, unrelated plugin system on `window.__CLARA_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](/user-guide/features/extending-the-dashboard). Python
CLI/gateway plugins are documented at [Build a Clara Plugin](/developer-guide/plugins).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$CLARA_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Unified package** | `$CLARA_HOME/plugins/<id>/desktop/plugin.js` | plugins that also ship agent-side code | none — same disk pipeline |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

All three take the same `ClaraPlugin` contract, appear in **Settings → Plugins**,
...
Create `$CLARA_HOME/desktop-plugins/hello/plugin.js` (that's `~/.clara/...`
by default, or `~/.clara/profiles/<name>/...` under a named profile). The folder
name must equal the plugin `id`.

```javascript
// ~/.clara/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@clara/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, Clara' }),
      jsx('div', {
...
The only importable specifiers are `@clara/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `ClaraPlugin`:

```ts
interface ClaraPlugin {
...
interface PluginContext {
  ...
  os: PluginOs
  /** Plugin-scoped JSON persistence (keys live under `clara.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@clara/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@clara/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@clara/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@clara/plugin-sdk'
...
```javascript
import { Button, useTheme } from '@clara/plugin-sdk'
...
```javascript
import { host, requestTheme } from '@clara/plugin-sdk'
...
```javascript
import { TRANSCRIPT_DIRECTIVE_AREA } from '@clara/plugin-sdk'
...
`window.clara.send('get-price eth')` (or a declarative
`<button data-clara-send="get-price eth">` — no script needed) hands that
...
```javascript
import { Contribute, TITLEBAR_AREAS } from '@clara/plugin-sdk'
...
for requests; `targetProfile` is the backend Clara profile served by that route.
...
approval/turn alerts use. It fires only while the user is away from Clara
...
ctx.os.notify({
  ...
  // Body click → focus Clara + navigate. Same vocabulary as OS deep links:
...
`activate` is deeplink-compatible: `clara://index-network/intent/1` and the
hash path `/index-network/intent/1` resolve to the same in-app route (and the
same `clara://…` URL works as an OS deep link). Action buttons only render on
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@clara/plugin-sdk'
...
```javascript
import { queryClient } from '@clara/plugin-sdk'
...
a package into `~/.clara/plugins` is inert on every surface until the user
...
backend, the remote box's `~/.clara/plugins` is not reachable as a filesystem —
...
the `clara://` scheme — a plain anchor on your website or README:
...
`dashboard/` subfolder of a regular Clara plugin and declare it in a
`manifest.json`:
...
clara-agent codebase directly (`clara_state`, `clara_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.clara/`)
...
(`clara.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `ClaraPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@clara/plugin-sdk` alias.
2. It's still lint-fenced to `@clara/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@clara/plugin-sdk`, `react`,
...
| Plugin contract | `ClaraPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginOs`, `PluginRestOptions`, `PluginNativeNotificationInput`, `PluginNotificationAction`, `ClaraOpenTarget`, `Contribution` |
...
### Agents: the `clara-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`clara-desktop-plugins`** skill — it carries the same contract as this page in
...
`$CLARA_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `clara logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@clara/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.clara/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.clara/logs/errors.log` for
