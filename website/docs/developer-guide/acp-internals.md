...
The ACP adapter wraps Clara' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
clara acp / clara-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.clara/.env
  -> configure stderr logging
  -> construct ClaraACPAgent
...
- `allow_once` -> Clara `once`
- `allow_always` -> Clara `always`
- reject options -> Clara `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Clara tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["clara-acp"])
...
Instead it reuses Clara' runtime resolver:

- `acp_adapter/auth.py`
- `clara_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Clara provider/credentials. It also always advertises a terminal setup auth method (`clara-setup`, args `--setup`) so first-run ACP clients can open Clara' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.clara/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `clara-acp` toolset definition
- `clara_cli/main.py` — `clara acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `clara-acp` script
