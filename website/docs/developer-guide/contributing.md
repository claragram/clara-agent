...
sidebar_position: 4
title: "Contributing"
description: "How to contribute to Clara Agent — dev setup, code style, PR process"
---

# Contributing

Thank you for contributing to Clara Agent! This guide covers setting up your dev environment, understanding the codebase, and getting your PR merged.
...
- Building a custom/local tool without modifying Clara core? Start with [Build a Clara Plugin](../developer-guide/plugins/index.md)
- Building a new built-in core tool for Clara itself? Start with [Adding Tools](./adding-tools.md)
...
You can also run a fully isolated Clara instance (throwaway CLARA_HOME, separate Electron
userData, distinct Electron app name to avoid the single-instance lock):

```bash
scripts/dev-sandbox.sh python -m clara_cli.main
scripts/dev-sandbox.sh --persistent python -m clara_cli.main desktop  # state survives restarts, but lives in the worktree :)
```
...
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.clara/.env
```
...
- **Profile-safe paths**: Never hardcode `~/.clara` — use `get_clara_home()` from `clara_constants` for code paths and `display_clara_home()` for user-facing messages. See [AGENTS.md](https://github.com/claraship/clara-agent/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.
...
Projects built on (or reviewed by) Clara can keep reviewer checklists inside the repository under `.agents/checks/`. Each file is a focused, plain-markdown checklist that an agent loads before reviewing a change touching the matching area:
...
When you ask Clara to review a PR in a repository that has `.agents/checks/`, tell it (or teach it via a skill) to read the relevant checklists first and report against them. This gives review agents the project-specific bar that generic review prompts miss.
...
- **Discord**: [discord.gg/j9vzaxnsg](https://discord.gg/j9vzaxnsg)
- **GitHub Discussions**: For design proposals and architecture discussions
- **Skills Hub**: Upload specialized skills and share with the community

## License

By contributing, you agree that your contributions will be licensed under the [MIT License](https://github.com/claraship/clara-agent/blob/main/LICENSE).
