---
sidebar_position: 8
title: "Use Voice Mode with Clara"
description: "A practical guide to setting up and using Clara voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Clara
...
:::tip
[Clara Portal](/integrations/clara-portal) bundles both the LLM and TTS through one OAuth — voice mode works end-to-end with no extra credentials.
:::
...
- you want Clara sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Clara.
...
## Step 1: make sure normal Clara works first
...
Clara supports both local and cloud speech stacks.
...
### If you use `clara setup`

If you choose NeuTTS in the setup wizard, Clara checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Clara transcribes and responds
...
- asking Clara to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Clara loop.

## Tuning CLI behavior

### Silence threshold

If Clara starts/stops too aggressively, tune:
...
Clara stays a normal chat bot, but can speak replies.
...
- you want Clara to function like a portable research or ops assistant
...
Clara joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Clara detects speech boundaries
- transcripts are posted in the associated text channel
- Clara responds in text and audio
...
1. get text Clara working
2. run `clara setup tts` to enable voice support
