...
```bash
clara teams-pipeline token-health
clara teams-pipeline token-health --force-refresh
```
...
```bash
clara teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
clara teams-pipeline maintain-subscriptions
clara teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Clara cron (recommended if you already run the Clara gateway)

Clara ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.clara/scripts/`. First create the script:

```bash
mkdir -p ~/.clara/scripts
cat > ~/.clara/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec clara teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.clara/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
clara cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/clara-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Clara Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=clara
EnvironmentFile=/etc/clara/env
ExecStart=/usr/local/bin/clara teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/clara-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Clara Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now clara-teams-pipeline-maintain.timer
systemctl list-timers clara-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/clara teams-pipeline maintain-subscriptions >> /var/log/clara/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.clara/.env` at the top of a wrapper script that crontab calls.
...
```bash
clara teams-pipeline subscriptions   # should show expirationDateTime advanced
clara teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
clara teams-pipeline list
clara teams-pipeline list --status failed
clara teams-pipeline show <job-id>
```

### Replay a stored job

```bash
clara teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
clara teams-pipeline fetch --meeting-id <meeting-id>
clara teams-pipeline fetch --join-web-url "<join-url>"
clara teams-pipeline fetch --join-web-url "<join-url>" --organizer-user-id <entra-user-id>
```
...
- run `clara teams-pipeline maintain-subscriptions --dry-run`
- inspect `clara teams-pipeline list --status failed`
...
- run `clara teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `clara teams-pipeline run`
...
- [ ] `clara teams-pipeline validate` returns an OK snapshot
- [ ] `clara teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Clara cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
