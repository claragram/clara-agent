...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `clara send`."
---

# Pipe Script Output to Messaging Platforms

`clara send` is a small, scriptable CLI that pushes a message to any
messaging platform Clara is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | clara send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `clara
...
clara send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
clara send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
clara send --list

# Filter by platform
clara send --list telegram
```
...
Any platform Clara ships adapters for works as a target:
...
`clara send` resolves the message body in this order:

1. **Positional argument** — `clara send --to telegram "hi"`
2. **`--file PATH`** — `clara send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | clara send --to telegram`

When stdin is a TTY (no pipe), Clara does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  clara send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `clara send` reuses your Clara config, the same script works on
any host where Clara is installed — no need to export bot tokens into
...
`clara send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  clara send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | clara send \
...
```bash
./train.py --epochs 200 && \
  clara send --to telegram "training done" || \
  clara send --to telegram "training failed (exit $?)"
```
...
clara send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(clara send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `clara send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `clara send` calls
the platform's REST endpoint directly using credentials from
`~/.clara/.env` and `~/.clara/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `clara gateway start` and retry.
...
# Machine-readable
clara send --list --json
```

The listing is built from `~/.clara/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`clara gateway
...
| Approach | Multi-platform | Reuses Clara creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `clara send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`clara send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `clara send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `clara send` shares with cron delivery.
