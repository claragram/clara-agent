...
description: "Complete guide to migrating your OpenClaw / Clawdbot setup to Clara Agent — what gets migrated, how config maps, and what to check after."
---

# Migrate from OpenClaw

`clara claw migrate` imports your OpenClaw (or legacy Clawdbot/Moldbot) setup into Clara. This guide covers exactly what gets migrated, the config key mappings, and what to verify after migration.

:::note
Coming from **Claude Code** or **OpenAI Codex CLI** instead? Use [`clara import-agent`](../user-guide/import-from-other-agents.md).
:::

:::tip
If your OpenClaw setup was multi-provider, `clara setup --portal` collapses it to one OAuth — 300+ models plus the Tool Gateway in a single login. See [Clara Portal](/integrations/clara-portal).
:::
...
clara claw migrate --preset full --migrate-secrets --yes
```
...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.clara/` (by default a single restore-point archive is written before apply, under `~/.clara/backups/pre-migration-*.zip`; restorable with `clara import`). |
...
| What | OpenClaw source | Clara destination | Notes |
|------|----------------|-------------------|-------|
| Persona | `workspace/SOUL.md` | `~/.clara/SOUL.md` | Direct copy |
| Workspace instructions | `workspace/AGENTS.md` | `AGENTS.md` in `--workspace-target` | Requires `--workspace-target` flag |
| Long-term memory | `workspace/MEMORY.md` | `~/.clara/memories/MEMORY.md` | Parsed into entries, merged with existing, deduped. Uses `§` delimiter. |
| User profile | `workspace/USER.md` | `~/.clara/memories/USER.md` | Same entry-merge logic as memory. |
| Daily memory files | `workspace/memory/*.md` | `~/.clara/memories/MEMORY.md` | All daily files merged into main memory. |
...
| Source | OpenClaw location | Clara destination |
|--------|------------------|-------------------|
| Workspace skills | `workspace/skills/` | `~/.clara/skills/openclaw-imports/` |
| Managed/shared skills | `~/.openclaw/skills/` | `~/.clara/skills/openclaw-imports/` |
| Personal cross-project | `~/.agents/skills/` | `~/.clara/skills/openclaw-imports/` |
| Project-level shared | `workspace/.agents/skills/` | `~/.clara/skills/openclaw-imports/` |

Skill conflicts are handled by `--skill-conflict`: `skip` leaves the existing Clara skill, `overwrite` replaces it, `rename` creates a `-imported` copy.

### Model and provider configuration

| What | OpenClaw config path | Clara destination | Notes |
|------|---------------------|-------------------|-------|
| Default model | `agents.defaults.model` | `config.yaml` → `model` | Can be a string or `{primary, fallbacks}` object |
| Custom providers | `models.providers.*` | `config.yaml` → `custom_providers` (auto-migrated to the canonical `providers:` dict on the next `clara update` config migration) | Maps `baseUrl`, `apiType`/`api` — handles both short ("openai", "anthropic") and hyphenated ("openai-completions", "anthropic-messages", "google-generative-ai") values |
| Provider API keys | `models.providers.*.apiKey` | `~/.clara/.env` | Requires `--migrate-secrets`. See [API key resolution](#api-key-resolution) below. |

### Agent behavior

| What | OpenClaw config path | Clara config path | Mapping |
|------|---------------------|-------------------|---------|
...
| OpenClaw config path | Clara config path | Notes |
|---------------------|-------------------|-------|
...
| OpenClaw field | Clara field | Notes |
|----------------|-------------|-------|
...
| TTS assets | `~/.clara/tts/` (file copy) |

### Messaging platforms

| Platform | OpenClaw config path | Clara `.env` variable | Notes |
|----------|---------------------|----------------------|-------|
...
| What | OpenClaw path | Clara path | Notes |
|------|-------------|-------------|-------|
...
| Gateway auth token | `gateway.auth.token` | `.env` → `CLARA_GATEWAY_TOKEN` | Requires `--migrate-secrets` |
...
### Archived (no direct Clara equivalent)

These are saved to `~/.clara/migration/openclaw/<timestamp>/archive/` for manual review:

| What | Archive file | How to recreate in Clara |
|------|-------------|--------------------------|
| `IDENTITY.md` | `archive/workspace/IDENTITY.md` | Merge into `SOUL.md` |
| `TOOLS.md` | `archive/workspace/TOOLS.md` | Clara has built-in tool instructions |
...
| Cron jobs | `archive/cron-config.json` | Recreate with `clara cron create` |
| Plugins | `archive/plugins-config.json` | See [plugins guide](/user-guide/features/hooks) |
| Hooks/webhooks | `archive/hooks-config.json` | Use `clara webhook` or gateway hooks |
| Memory backend | `archive/memory-backend-config.json` | Configure via `clara honcho` |
| Skills registry | `archive/skills-registry-config.json` | Use `clara skills config` |
...
| Multi-agent list | `archive/agents-list.json` | Use Clara profiles |
...
The migration resolves all three formats. For env templates and SecretRef objects with `source: "env"`, it looks up the value in `~/.openclaw/.env` and the `openclaw.json` env sub-object. SecretRef objects with `source: "file"` or `source: "exec"` can't be resolved automatically — the migration warns about these, and those values must be added to Clara manually via `clara config set`.

## After migration

1. **Check the migration report** — printed on completion with counts of migrated, skipped, and conflicting items.

2. **Review archived files** — anything in `~/.clara/migration/openclaw/<timestamp>/archive/` needs manual attention.

3. **Start a new session** — imported skills and memory entries take effect in new sessions, not the current one.

4. **Verify API keys** — run `clara status` to check provider authentication.

5. **Test messaging** — if you migrated platform tokens, restart the gateway: `systemctl --user restart clara-gateway`

6. **Check session policies** — run `clara config show` and verify the `session_reset` value matches your expectations.

7. **Re-pair WhatsApp** — WhatsApp uses QR code pairing (Baileys), not token migration. Run `clara whatsapp` to pair.

8. **Archive cleanup** — after confirming everything works, run `clara claw cleanup` to rename leftover OpenClaw directories to `.pre-migration/` (prevents state confusion).
...
Keys might be stored in several places depending on your OpenClaw version: inline in `openclaw.json` under `models.providers.*.apiKey`, in `~/.openclaw/.env`, in the `openclaw.json` `"env"` sub-object, or in `agents/main/agent/auth-profiles.json`. The migration checks all four. If keys use `source: "file"` or `source: "exec"` SecretRefs, they can't be resolved automatically — add them via `clara config set`.

### Skills not appearing after migration

Imported skills land in `~/.clara/skills/openclaw-imports/`. Start a new session for them to take effect, or run `/skills` to verify they're loaded.

### TTS voice not migrated

OpenClaw stores TTS settings in two places: `messages.tts.providers.*` and the top-level `talk` config. The migration checks both. If your voice ID was set via the OpenClaw UI (stored in a different path), you may need to set it manually: `clara config set tts.elevenlabs.voice_id YOUR_VOICE_ID`.
