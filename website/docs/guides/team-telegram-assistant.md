...
This tutorial walks you through setting up a Telegram bot powered by Clara Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Clara Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.clara/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Clara itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Clara Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_clara_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Clara Agent. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Clara Gateway...
...
# View live logs
journalctl --user -u clara-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `clara gateway install` to update the plist.
:::
...
```bash
clara gateway stop && clara gateway start
```
...
clara pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.clara/.env`:
...
Control how much detail the bot shows when using tools. In `~/.clara/config.yaml`:
...
Customize how the bot communicates by editing `~/.clara/SOUL.md`:

For a full guide, see [Use SOUL.md with Clara](/guides/use-soul-with-clara).
...
```bash
# From the CLI
clara cron list          # View all scheduled jobs
clara cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u clara-gateway` (Linux) or `~/.clara/logs/gateway.log` (macOS) |
| Cron job output | `~/.clara/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.clara/cron/jobs.json` |
| Pairing data | `~/.clara/pairing/` |
| Session history | `~/.clara/sessions/` |
