...
description: "Diagnose and fix common Clara cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`clara gateway` for foreground, or `clara gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `clara cron tick`.
...
```bash
date
clara cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.clara/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.clara/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.clara/.env` |
...
| `local` | Write access to `~/.clara/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `clara cron list` for updated `last_error` field (if available).
...
### Check 5: Relay-fronted platforms (Clara Cloud / Team Gateway)
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `clara skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `clara skills list`.
...
2. `~/.clara/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `clara cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Clara config directory). Verify:
```bash
ls ~/.clara/scripts/your-script.py   # Must exist
clara cron edit <job_id> --script ~/.clara/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `clara skills install <skill-name>`.
...
Jobs are stored in `~/.clara/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.clara/cron/jobs.json
chmod 600 ~/.clara/cron/jobs.json   # Your user should own it
```
...
```bash
clara cron list                    # Show all jobs, states, next_run times
clara cron run <job_id>            # Schedule for next tick (for testing)
clara cron edit <job_id>           # Fix configuration issues
clara logs                         # View recent Clara logs
clara skills list                  # Verify installed skills
```
...
3. Open an issue at [github.com/claraship/clara-agent](https://github.com/claraship/clara-agent) with:
