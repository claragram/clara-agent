---
sidebar_position: 16
title: "Manage Clara Cloud with MCP"
description: "Connect Clara Agent to the Clara Portal MCP server so your local agent can list, start, stop, and manage your Clara Cloud instances conversationally"
---

# Manage Clara Cloud with MCP

[Clara Cloud](https://portal.claraship.com/cloud) runs hosted Clara Agent instances for you. Normally you manage them from the `/agents` page in the [Clara Portal](/integrations/clara-portal). This guide connects your **local** Clara Agent to the Portal's MCP server so you can manage those cloud instances by just asking — "list my cloud agents", "restart the stopped one", "what's it costing me" — without leaving your terminal.

It's a standard [MCP](/user-guide/features/mcp) server hosted by Claraship, gated by the same OAuth login you already use for the Portal. Once connected, Clara gets two tools it can call on your behalf.

## What you can do with it

Once connected, the model can call these on your Clara Cloud org:
...
- A [Clara Portal](/integrations/clara-portal) account with [Clara Cloud](https://portal.claraship.com/cloud) access (at least one instance, or the ability to create one).
...
```bash
clara mcp add --url https://portal.claraship.com/mcp --auth oauth clara-cloud
```
...
4. Stores the resulting token under `~/.clara/mcp-tokens/` and reuses it (refresh is automatic).
...
If you ever need to point the connection at a different org, remove and re-add the server (`clara mcp remove clara-cloud`, then the `add` command again) and pick the other org in the browser.

## Step 2: verify it connected

```bash
clara mcp test clara-cloud
```
...
```text
List my Clara Cloud agents and their current status.
```
...
```text
Create a new Clara Cloud instance named scratch, then tell me when it's ready.
```

Clara reports what each tool returned — the instance list, the new status, the created instance's details — so you can confirm the action landed.

## Configuration

After `clara mcp add`, the server lives in `~/.clara/config.yaml`:

```yaml
mcp_servers:
  clara-cloud:
    url: "https://portal.claraship.com/mcp"
    auth: oauth
```

No credentials go in `config.yaml` — the OAuth token is kept separately under `~/.clara/mcp-tokens/`, the same way the Portal refresh token stays out of your config.
...
Run `/reload-mcp` after changing the config. See [Use MCP with Clara](/guides/use-mcp-with-clara) for the full filtering model (`include`/`exclude`, `prompts`, `resources`).

## Troubleshooting

### The browser shows an org picker and I'm not sure which to choose

You belong to multiple Portal organizations. Pick the org whose Clara Cloud instances you want to manage from this connection. If you're unsure, it's the org that owns the instances you see on the Portal `/agents` page. You can re-choose later by removing and re-adding the server.
...
If they're still missing, run `clara mcp test clara-cloud` to see the connection error directly.

### It asks me to log in again

OAuth tokens refresh automatically, but if the Portal invalidates your session (password change, revoke, expiry) the next call asks you to re-authorize. Re-run the `clara mcp add` command — the browser flow re-mints a token.

### Headless / SSH / remote host

The OAuth browser callback runs on the machine where Clara is running. On a remote host, forward the loopback port over SSH — the same pattern as any other OAuth login. See [OAuth over SSH / Remote Hosts](/guides/oauth-over-ssh).

## See also

- **[Clara Portal](/integrations/clara-portal)** — the subscription, models, and Tool Gateway behind the same login
- **[Use MCP with Clara](/guides/use-mcp-with-clara)** — connecting and filtering MCP servers in general
- **[MCP feature overview](/user-guide/features/mcp)** — what MCP is and how Clara uses it
