---
sidebar_position: 7
title: "Use SOUL.md with Clara"
description: "How to use SOUL.md to shape Clara Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Clara

`SOUL.md` is the **primary identity** for your Clara instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Clara to feel like the same assistant every time you talk to it — or if you want to replace the Clara persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Clara should be
- what Clara should avoid stylistically
- how Clara should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Clara is and how Clara speaks
...
Clara now uses only the global SOUL file for the current instance:

```text
~/.clara/SOUL.md
```

If you run Clara with a custom home directory, it becomes:
...
Clara automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Clara does not overwrite it
- if the file exists but is empty, Clara adds nothing from it to the prompt

## How Clara uses it

When Clara starts a session, it reads `SOUL.md` from `CLARA_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Clara feels.
...
Clara already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Clara should sound.

# Avoid
What Clara should not do.

# Defaults
How Clara should behave when ambiguity appears.
```
...
Then restart Clara or start a new session.
...
### I edited SOUL.md but Clara still sounds the same

Check:
- you edited `~/.clara/SOUL.md` or `$CLARA_HOME/SOUL.md`
...
### Clara is ignoring parts of my SOUL.md
