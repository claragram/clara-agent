...
clara cron create "0 2 * * *" \
  "You are a project manager triaging the claragram/clara-agent GitHub repo.

1. Run: gh issue list --repo claragram/clara-agent --state open --json number,title,labels,author,createdAt --limit 30
...
clara cron create "0 9 * * 1" \
  "Scan the claragram/clara-agent repo for documentation drift.

1. Run: gh pr list --repo claragram/clara-agent --state merged --json number,title,files,mergedAt --limit 30
