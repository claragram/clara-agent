...
clara cron create "0 2 * * *" \
  "You are a project manager triaging the claraship/clara-agent GitHub repo.

1. Run: gh issue list --repo claraship/clara-agent --state open --json number,title,labels,author,createdAt --limit 30
...
clara cron create "0 9 * * 1" \
  "Scan the claraship/clara-agent repo for documentation drift.

1. Run: gh pr list --repo claraship/clara-agent --state merged --json number,title,files,mergedAt --limit 30
