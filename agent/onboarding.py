...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Clara starts and finds ``~/.openclaw/``.

    Points users at ``clara claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``clara claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Clara, run "
        "`clara claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `clara claw cleanup` (renames it to ~/.openclaw.pre-migration — "
