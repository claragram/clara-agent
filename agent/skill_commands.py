...
#
# When a user invokes a /skill (or /bundle), Clara expands the turn into a
...
# tests/openviking_plugin/test_openviking.py::test_skill_markers_match_clara_scaffolding.
# ---------------------------------------------------------------------------
...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``CLARA_PLATFORM`` env var and
    ``CLARA_SESSION_PLATFORM`` from the gateway session context. Returns
    ...
    try:
        ...
        resolved_platform = (
            os.getenv("CLARA_PLATFORM")
            or get_session_env("CLARA_SESSION_PLATFORM")
        )
    except Exception:
        resolved_platform = os.getenv("CLARA_PLATFORM")
    return resolved_platform or None


def _resolve_skill_commands_home() -> str:
    """Return the effective Clara home the skill scan should be scoped to.

    A gateway session can switch between profiles that each carry their own
    ``skills.external_dirs`` (via ``set_clara_home_override``), but the
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.clara.config``
    ...
    try:
        ...
        lines = ["", f"[Skill config (from {display_clara_home()}/config.yaml):"]
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.clara/skills/ and return a mapping of /command -> skill info.
    ...
    try:
        ...
        # launch home, so a multiplexed profile scope (set_clara_home_override)
        ...
        for scan_dir in dirs_to_scan:
            ...
            for skill_md in _iter:
                ...
                try:
                    ...
                    # with a core Clara slash command (name or alias). The
                    ...
                    if resolve_command(cmd_name) is not None:
                        logger.warning(
                            ...
                            "collides with a core Clara command; skipping "
...
def get_skill_commands() -> Dict[str, Dict[str, Any]]:
    ...
    active profile's Clara home changes (e.g. Desktop switching profiles
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.clara/skills/`` and any ``skills.external_dirs`` so the
...
def build_preloaded_skills_prompt(
    ...
    bundle-invocation gate (#59156). Without this, ``clara -s <skill>`` or
    a deployment's ``CLARA_TUI_SKILLS`` env var could force-load a skill an
