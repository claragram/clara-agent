"""OpenAI-shape bridge shared by Clara' ACP clients.

An ACP agent (``copilot --acp``, and the ACP CLIs that reach Clara as
...
the agent's *own* tool notifications. Clara' agentic surface — ``memory``,
...
* :func:`completion_to_stream_chunks` — re-shape a one-shot ACP response as
  ...
  is inherently one-shot from Clara' perspective).
...
of its own (Copilot) forwards everything Clara offers; a CLI that is an
autonomous agent with its own read/edit/execute tools must forward only Clara'
agent-level tools, because re-offering the overlapping ones makes Clara re-run
...
class StreamChunks(list):
    ...
    Clara reads provider-level extras off the object returned by
    ``chat.completions.create`` (e.g. ``clara_projected_messages``, consumed by
...
def tool_specs_from_openai_tools(
    ...
    *,
    ...
    name is in it survive — that is how a client forwards just Clara'
