...
Plugins built on Clara Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Clara
...
:class:`~clara_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Clara' main config
...
handles every provider, fallback chain, and per-task override Clara
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`clara_cli.plugins.PluginContext`
