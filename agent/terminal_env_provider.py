...
Providers live in ``~/.clara/plugins/<name>/`` (user, opt-in via
...
* ``cache_path_base`` — where auto-synced ``~/.clara/cache`` files land
  inside the backend (e.g. ``"~/.clara"`` for home-synced backends,
  ``"/root/.clara"`` for root-homed containers), or ``None`` when host
...
The factory stamps ``_clara_backend_name`` on the returned object so
...
class TerminalEnvironmentProvider(abc.ABC):
    ...
    def cache_path_base(self) -> Optional[str]:
        """Base dir for synced Clara cache files inside the backend."""
        return None
    ...
    def setup_instructions(self) -> List[str]:
        """Lines printed by ``clara setup`` after this backend is selected.
