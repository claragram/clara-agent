...
3. ``~/.clara/.anthropic_oauth.json`` (Clara PKCE login)
...
def _getenv(name: str, default: str = "") -> str:
    ...
    read) when on. Mirrors the same wrapper in clara_cli/runtime_provider.py.
    """
...
class CredentialPersistError(RuntimeError):
    ...
    ``~/.clara/.anthropic_oauth.json`` for ``clara_pkce``).
...
#     authority boundary of ``claude_code``/``clara_pkce`` is the shared
#     singleton file, which other Clara processes/profiles read with fresh
...
def _spent_rotation_sidecar_path(source_path: Path) -> Path:
    ...
    return source_path.with_name(source_path.name + ".clara-spent-rotations.json")
...
def spent_rotation_source_path(source: Any) -> Optional[Path]:
    ...
    if source == "clara_pkce":
        return _get_clara_oauth_file()
    return None
        ...
        payload = json.dumps(
            {
                ...
                "comment": (
                    ...
                    "never durably committed. Written by Clara so sibling "
...
def claude_code_credentials_path() -> Path:
    ...
    This file is not profile-owned: every Clara profile's credential pool
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    Clara notices an expired token, Claude Code may have already rotated it.
    ...
    try:
        from clara_cli.auth import AUTH_LOCK_TIMEOUT_SECONDS, _auth_store_lock, env_float

        refresh_timeout_seconds = env_float(
            "CLARA_ANTHROPIC_REFRESH_TIMEOUT_SECONDS", 20
        )
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Clara historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``clara models``) never mutates ``~/.clara/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Clara)
      ...
      5. Anthropic credential_pool OAuth entry (~/.clara/auth.json)
    ...
    # 1. Clara-managed OAuth/setup token env var
    ...
    # 5. Clara credential_pool OAuth entry.
...
# ── Clara-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.clara/.anthropic_oauth.json (our own file).
...
def _get_clara_oauth_file() -> Path:
    return get_clara_home() / ".anthropic_oauth.json"


def _root_clara_oauth_file() -> Optional[Path]:
    ...
    try:
        ...
        if root.resolve(strict=False) == get_clara_home().resolve(strict=False):
            return None
...
def run_clara_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Clara-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Clara with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from clara_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_clara_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Clara-managed OAuth credentials from ~/.clara/.anthropic_oauth.json."""
    ...
    if oauth_file.exists():
        try:
            ...
            logger.debug("Failed to read Clara OAuth credentials: %s", e)
    return None


def _write_clara_oauth_credentials(
    access_token: str,
    ...
    *,
    ...
    """Write refreshed clara_pkce tokens back to ~/.clara/.anthropic_oauth.json.
    ...
    its own CLARA_HOME (#100339).

    Without this, a successful pool-level refresh of a ``clara_pkce``-sourced
    ...
    """
    oauth_file = target if target is not None else _get_clara_oauth_file()
    try:
        oauth_data = {
            "accessToken": access_token,
            "refreshToken": refresh_token,
        ...
        logger.error(
            "Failed to write refreshed Clara OAuth credentials to %s: %s", oauth_file, e
        )
