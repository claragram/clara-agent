...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `clara backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - clara_home (str): The active CLARA_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.clara``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "clara").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'clara memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], clara_home: str) -> None:
        ...
        Called by 'clara memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``clara_home`` is the active CLARA_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE CLARA_HOME.

        ``clara backup`` only walks CLARA_HOME, so any provider state kept
        ...
        of the archive, and ``clara import`` restores them to their original
