"""Cross-session rate limit guard for Clara Portal.

Writes rate limit state to a shared file so all sessions (CLI, gateway,
cron, auxiliary) can check whether Clara Portal is currently rate-limited
before making requests.  Prevents retry amplification when RPH is tapped.

Each 429 from Clara triggers up to 9 API calls per conversation turn
(3 SDK retries x 3 Clara retries), and every one of those calls counts
...
_STATE_FILENAME = "clara.json"


def _state_path() -> str:
    """Return the path to the Clara rate limit state file."""
    try:
        from clara_constants import get_clara_home
        base = get_clara_home()
    except ImportError:
        base = os.path.join(os.path.expanduser("~"), ".clara")
...
def record_clara_rate_limit(
    *,
    headers: Optional[Mapping[str, str]] = None,
    error_context: Optional[dict[str, Any]] = None,
    default_cooldown: float = 300.0,
) -> None:
    """Record that Clara Portal is rate-limited.
    ...
    try:
        ...
        logger.info(
            "Clara rate limit recorded: resets in %.0fs (at %.0f)",
            reset_at - now, reset_at,
        )
    except Exception as exc:
        logger.debug("Failed to write Clara rate limit state: %s", exc)


def clara_rate_limit_remaining() -> Optional[float]:
    """Check if Clara Portal is currently rate-limited.
...
def clear_clara_rate_limit() -> None:
    """Clear the rate limit state (e.g., after a successful Clara request)."""
    try:
        os.unlink(_state_path())
    except FileNotFoundError:
        pass
    except OSError as exc:
        logger.debug("Failed to clear Clara rate limit state: %s", exc)
...
def is_genuine_clara_rate_limit(
    *,
    headers: Optional[Mapping[str, str]] = None,
    last_known_state: Optional[Any] = None,
) -> bool:
    """Decide whether a 429 from Clara Portal is a real account rate limit.

    Clara Portal multiplexes multiple upstream providers (DeepSeek, Kimi,
    MiMo, Clara, ...) behind one endpoint.  A 429 can mean either:

      (a) The caller's own RPM / RPH / TPM / TPH bucket on Clara is
      ...
      (b) The upstream provider is out of capacity for a specific model
          ...
          the caller's quota on Clara.

    Tripping the cross-session breaker on (b) blocks ALL Clara requests
    (and all models, since Clara is one provider key) for minutes even
    ...
    We tell the two apart by looking at:
      ...
      1. The 429 response's own ``x-ratelimit-*`` headers.  Clara emits
