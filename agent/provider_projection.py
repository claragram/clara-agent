"""Fold an agent-as-provider's own activity back into Clara' turn state.

Most providers are models: they ask Clara to run a tool and Clara runs it, so
...
time Clara sees the response that work is already done.

Those calls must never come back as pending ``tool_calls`` — Clara would re-run
...
* the **skill-review nudge**, whose counter (``_iters_since_skill``) only moves
  on Clara tool iterations, of which there are none.

So the provider client hands both back on the completion object and this helper
applies them: ``clara_projected_messages`` (already-completed
``assistant(tool_calls=[…])`` + ``tool(result)`` history rows) and
``clara_provider_tool_iterations`` (how many tool iterations happened inside
...
def splice_provider_projection(
    ...
    """
    projected = getattr(response, "clara_projected_messages", None)
    ...
    raw_iters = getattr(response, "clara_provider_tool_iterations", 0)
    try:
