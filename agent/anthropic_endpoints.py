...
def _is_clara_portal_endpoint(base_url: str | None) -> bool:
    ...
    ``https://inference-api.claraship.com/v1/messages``.  Portal-specific
    behaviours key off this: Bearer JWT auth, verbatim catalog model ids,
    and native thinking-signature replay.

    Trusted hosts only:

    1. Prod hostname ``inference-api.claraship.com``
    2. The operator-set ``CLARA_INFERENCE_BASE_URL`` hostname (staging/preview)

    Lookalikes such as ``inference-api.claraship.com.attacker.test`` are
    rejected (hostname match, not substring).
    """
    if base_url_host_matches(base_url or "", "inference-api.claraship.com"):
        return True
    try:
