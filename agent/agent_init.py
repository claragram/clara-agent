...
def _warn_memory_provider_unavailable(name: str, reason: str = "") -> None:
    ...
    not inheriting ``~/.clara/.env``. See Workprise/clara-agent#2765.
    ...
    logger.warning(
        ...
        "provider's credentials/config with 'clara memory status'. Note: "
        "systemd/gateway services do not inherit ~/.clara/.env automatically; set "
...
def _provider_default_routes(provider: str) -> set[str]:
    ...
    routes: set[str] = set()
    try:
        from clara_cli.providers import CLARA_OVERLAYS, get_provider

        overlay = CLARA_OVERLAYS.get(provider)
    ...
    try:
        ...
        from clara_cli.models import normalize_provider as normalize_model_provider
        from clara_cli.providers import normalize_provider as normalize_registry_provider
...
def _build_codex_gpt5_autoraise_notice(
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: clara config set compression.codex_gpt55_autoraise false"
    )
...
def _codex_gpt55_autoraise_notice_marker():
    ...
    Lives under ``$CLARA_HOME`` (which is profile-scoped) alongside the other
    ...
    """
    return get_clara_home() / ".codex_gpt55_autoraise_notice"
...
def _record_codex_gpt55_autoraise_notice(autoraise: Dict[str, Any]) -> None:
    ...
    Best-effort: a read-only or missing ``$CLARA_HOME`` just means the notice
...
def _refuse_checkpoint_required_on_codex_app_server(
    ...
    the default — Clara never even initiates the compaction), so no
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .clara.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / CLARA_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.clara/SOUL.md as the primary
    ...
    if api_mode in {"chat_completions", "codex_responses", "anthropic_messages", "bedrock_converse", "codex_app_server"}:
    ...
    elif agent.provider in {"clara", "clara-portal", "workprise"}:
        ...
        agent.api_mode = clara_api_mode(agent.model)
    else:
        ...
        try:
            from clara_cli.providers import host_mandated_api_mode as _host_mandated_api_mode
    ...
    agent._cache_ttl = "5m"
    try:
        from clara_cli.config import load_config_readonly as _load_pc_cfg
    ...
    # x-clara-credits-* response headers after each API call.  Session-start
    # remaining is latched the first time a header is ever seen so we can
    # report cumulative micros spent.  Surfaced behind CLARA_DEV_CREDITS.
    ...
    # both live under ~/.clara/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from clara_logging import setup_logging, setup_verbose_logging
    setup_logging(clara_home=_ra()._clara_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that clara_logging.setup_logging() does not
        ...
        # handler level inside clara_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        try:
            from clara_cli.config import load_config_readonly as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if base_url_host_matches(effective_base, "openrouter.ai"):
                ...
                client_kwargs["default_headers"] = clara_xai_default_headers()
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `clara model` to "
                        "select a provider, or run `clara setup` for first-time "
                        "configuration."
                    )
    ...
    # A multiplexed gateway may enter a different CLARA_HOME after
    ...
    # worker (kanban_show tool is present iff CLARA_KANBAN_TASK is set).
    ...
    try:
        ...
        if not delegated_child:
            os.environ["CLARA_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.clara/sessions/ alongside gateway sessions
    clara_home = get_clara_home()
    agent.logs_dir = clara_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.clara/sessions/session_{sid}.json)
    ...
    try:
        from clara_cli.config import load_config_readonly as _load_sess_cfg
    ...
    # `clara --resume <id>` can restore the bypass (CLI resume paths read
    ...
    try:
        from clara_cli.config import load_config_readonly as _load_agent_config
    ...
    # management API (the historical Clara behavior) or left to LM Studio's
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "clara"
                    except Exception:
                        pass
    ...
    # Lets an enterprise admin append to or replace Clara' built-in
    ...
    if codex_app_server_auto_compaction not in {"native", "clara", "off"}:
        _ra().logger.warning(
            "Invalid compression.codex_app_server_auto=%r; using 'native'. "
            "Valid values are: native, clara, off.",
    ...
    if _config_context_length is not None and isinstance(_model_cfg, dict):
        ...
        if _configured_provider_norm in _runtime_first_provider_ids:
            ...
            try:
                from clara_cli.auth import resolve_provider as resolve_auth_provider
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            _candidate = None
            try:
                from clara_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH and not _allow_lmstudio_explicit_below_floor:
        raise ValueError(
            ...
            f"by Clara Agent.  Choose a model with at least "
    ...
    # Clara Clara 3/4 are chat models, not tool-call-tuned. The interactive
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            from clara_cli.model_switch import _check_clara_model_warning
            ...
            if _clara_warn:
                ...
                if hasattr(agent, "_emit_warning"):
                    ...
                    print(f"\n{_user_msg}\n", file=sys.stderr)
                _ra().logger.warning(_clara_warn)
        except Exception:
            pass
