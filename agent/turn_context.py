...
def build_turn_context(
    ...
    *,
    ...
    # Tag log records on this thread with the session ID for ``clara logs``.
    ...
    if (
        ...
        # Clara only initiates compaction in "clara" mode (#36801).
        _codex_native_auto = (
        ...
        if _preflight_deferred:
            ...
            logger.info(
                "Skipping Clara preflight compression for codex app-server "
                "(mode=%s); Clara will not start thread compaction here.",
    ...
    if agent._memory_manager:
        ...
        # to surface it. Rendered by Clara (via _emit_status), so it always
