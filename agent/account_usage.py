...
from clara_cli.auth import AuthError, _read_codex_tokens, resolve_codex_runtime_credentials
...
def build_clara_credits_snapshot(account_info) -> Optional[AccountUsageSnapshot]:
    """Map a ClaraPortalAccountInfo into an AccountUsageSnapshot for /usage.
    ...
    """
    try:
        from clara_cli.clara_account import clara_portal_topup_url
        ...
        details.append(f"Top up: {clara_portal_topup_url(account_info)}")
...
def clara_credits_lines(*, markdown: bool = False, timeout: float = 10.0) -> list[str]:
    """Return rendered Clara-credits /usage lines, or [] when there's nothing to show.

    Account-independent of any live agent: gated on "a Clara account is logged in"
    ...
    Dev override: when CLARA_DEV_CREDITS_FIXTURE selects a fixture state, /usage
    ...
    try:
        ...
        tok = (get_provider_auth_state("clara") or {}).get("access_token")
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
            account = pool.submit(
                get_clara_portal_account_info, force_fresh=True
            ).result(timeout=timeout)
        snapshot = build_clara_credits_snapshot(account)
        return render_account_usage_lines(snapshot, markdown=markdown)
    except Exception:
...
def _snapshot_from_credits_state(state) -> Optional[AccountUsageSnapshot]:
    ...
    so CLARA_DEV_CREDITS_FIXTURE can exercise /usage without a live account. The
    *_usd strings are mock display values here (not server balance to compute on);
    ...
    try:
        ...
        details.append("(dev fixture — CLARA_DEV_CREDITS_FIXTURE)")
        return AccountUsageSnapshot(
            provider="clara",
            source="dev-fixture",
            fetched_at=_utc_now(),
...
def build_credits_view(*, markdown: bool = False, timeout: float = 10.0) -> CreditsView:
    ...
    try:
        ...
        from clara_cli.clara_account import (
            ...
            clara_portal_topup_url,
        )

        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
            account = pool.submit(get_clara_portal_account_info, force_fresh=True).result(
                timeout=timeout
            )
    except Exception:
    ...
    # ("Top up: <url>" + "(or run /topup)") that build_clara_credits_snapshot
    ...
    return CreditsView(
        ...
        topup_url=clara_portal_topup_url(account),
...
def _resolve_codex_usage_credentials(
    ...
    state, then the credential pool.  Clara's native OAuth setup now stores
    ...
    *,
    ...
    try:
        ...
        return CodexResetRedeemResult(
            status="unavailable",
            message="No Codex credentials available. Run `clara auth` to sign in with your ChatGPT account.",
        )
        ...
        if code in (401, 403):
            return CodexResetRedeemResult(
                ...
                message=(
                    ...
                    "run `clara auth` and sign in with your ChatGPT account."
                ),
            )
        return CodexResetRedeemResult(
            status="unavailable",
    ...
    if code == "reset":
        ...
        # persisted pool cooldowns so Clara doesn't keep the credential
        # frozen behind the now-stale ``last_error_reset_at`` (issue #43747).
        try:
            from clara_cli.auth import clear_codex_pool_quota_cooldowns
