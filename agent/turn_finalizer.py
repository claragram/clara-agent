...
decomposition campaign (``~/.clara/plans/god-file-decomposition.md``, Phase 1
...
to the caller. The function is __PROT_0_synchroclara__ with a single return — mirroring the
...
def finalize_turn(
    ...
    *,
    ...
    if iteration_limit_fallback:
        ...
        _kanban_task = os.environ.get("CLARA_KANBAN_TASK")
        if _kanban_task:
    ...
    result = {
        ...
        # billing audits by callers like `clara -z --usage-file`.
        "service_tier": (
    ...
    if failed and str(_turn_exit_reason) == "session_persistence_failed":
        result["error"] = final_response or (
            ...
            "health (`clara doctor`), then send your message again"
        )
