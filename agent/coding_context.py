"""Coding-context awareness — base Clara, every interactive surface.

When the user runs Clara inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), Clara shifts into a **coding posture**. This module is the
...
from clara_cli._subprocess_compat import bounded_git_probe

logger = logging.getLogger("clara.coding_context")
...
_EDIT_FORMAT_GUIDANCE: dict[str, tuple[tuple[str, ...], str]] = {
    ...
    "replace": (
        ("claude", "sonnet", "opus", "haiku",
         ...
         "clara", "llama", "mistral", "devstral", "minimax"),
        "- Edit format: author new files with `write_file`; for edits to "
...
# in _CLARA_CORE_TOOLS, so they're present on every surface this fires on.
...
class RuntimeMode:
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        selection (``--toolsets``, ``CLARA_TUI_TOOLSETS``, …); they never
...
def is_coding_context(
    *,
    ...
    """Whether Clara should operate in its coding posture right now."""
