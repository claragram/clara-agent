"""Upload a Clara session transcript to Hugging Face as an agent trace.

Clara stores sessions in its own SQLite store (``clara_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``clara trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Clara' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "clara-traces"
_CLARA_VERSION = "clara-agent"
...
# ---------------------------------------------------------------------------
# Conversion: Clara OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Clara' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Clara conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.clara/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `clara trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/clara-traces`` dataset. Returns a
