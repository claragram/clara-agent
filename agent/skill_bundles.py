...
Storage
-------
Bundles live in ``~/.clara/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``CLARA_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``clara bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under CLARA_HOME.

    Honors ``CLARA_BUNDLES_DIR`` for tests; falls back to
    ``<CLARA_HOME>/skill-bundles``.
    """
    override = os.environ.get("CLARA_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_clara_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `clara bundles` CLI subcommand.
# ---------------------------------------------------------------------------
