"""Public, plugin-safe lifecycle API for delegated Clara subagents.
...
_EXECUTOR = _DaemonExecutor(max_workers=8, thread_name_prefix="clara-lifecycle")
_SECRET = secrets.token_bytes(32)
_ACTIVE_PARENT_AGENT: contextvars.ContextVar[Any] = contextvars.ContextVar(
    "clara_subagent_lifecycle_parent", default=None
)
...
class SubagentLifecycleService:
    ...
    def launch(self, request: SubagentLaunchRequest) -> SubagentHandle:
        ...
        if parent is None:
            raise SubagentLifecycleError(
                "No active Clara parent session is available."
            )
        ...
        if not subagent_id:
            raise SubagentLifecycleError("Clara failed to assign a child identity.")
        created = time.time()
        handle = SubagentHandle(
    ...
    def _validate_request(request: SubagentLaunchRequest, parent: Any) -> None:
        ...
        if request.working_directory is not None:
            raise SubagentLifecycleError(
                "working_directory is not supported because Clara delegates use isolated task environments."
            )
        if request.blocked_tools:
            raise SubagentLifecycleError(
                "Per-tool blocking is not supported; use allowed_toolsets. Clara always blocks unsafe child tools."
            )
        try:
            metadata_bytes = len(
