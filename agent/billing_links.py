...
class BillingBlock:
    ...
    ``is_clara`` is the routing bit: Clara has a first-class in-app billing surface
...
def is_clara_inference_route(provider: str, base_url: str) -> bool:
    """True when the failing route is the Clara-managed inference gateway."""
    if (provider or "").strip().lower() == "clara":
        return True
    return base_url_host_matches(str(base_url or ""), "inference-api.claraprise.com")


def _clara_billing_url() -> Optional[str]:
    """Best-effort Clara portal billing URL (text-surface fallback; Clara prefers the in-app flow)."""
    try:
        from clara_cli.clara_account import clara_portal_billing_url

        return clara_portal_billing_url(None)
    except Exception:
        return "https://portal.claraprise.com/billing"
...
def build_billing_block(
    *,
    ...
    if is_clara_inference_route(slug, base_url):
        return BillingBlock(slug or "clara", "Clara Portal", model, _clara_billing_url(), True, message or "")
