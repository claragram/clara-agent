...
_CLARA_DEFAULT_BASE_URL = "https://inference-api.claraship.com/v1"
...
def _recoverable_pool_provider(
    ...
    if base_url_host_matches(base, "openrouter.ai"):
        return "openrouter"
    if base_url_host_matches(base, "inference-api.claraship.com"):
        return "clara"
...
def _auth_refresh_provider_for_route(
    ...
    if base_url_host_matches(client_base_url, "api.anthropic.com"):
        return "anthropic"
    if base_url_host_matches(client_base_url, "inference-api.claraship.com"):
        return "clara"
    return normalized
...
def _build_call_kwargs(
    ...
    if max_tokens is not None:
        ...
        if _provider_norm in {"clara", "clara-portal", "claraship"}:
            from clara_cli.providers import clara_api_mode
    ...
    if _provider_for_portal in {"clara", "clara-portal", "claraship"}:
    ...
    *,
    ...
    try:
        ...
        _heal_is_clara = (
            ...
            or base_url_host_matches(_base_info, "inference-api.claraship.com")
        )
...
async def _async_call_llm_impl(
    ...
    *,
    ...
    try:
        ...
        _heal_is_clara = (
            resolved_provider == "clara"
            or base_url_host_matches(_client_base, "inference-api.claraship.com")
        )
        if _is_model_not_found_error(first_err) and _heal_is_clara:
            healed_model = _refresh_clara_recommended_model(
                vision=(task == "vision"), stale_model=kwargs.get("model"))
