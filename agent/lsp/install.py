...
appropriate.  All installs go to a Clara-owned bin staging dir,
``<CLARA_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``clara lsp
  status``.
- ``off`` — same as ``manual`` for now (kept distinct so we can
  evolve behavior later, e.g. logging differently).

The actual installs happen __PROT_0_synchroclaraly__ the first time a server is
...
# ``<CLARA_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # `clara lsp status` reports the host's presence.
...
def clara_lsp_bin_dir() -> Path:
    """Return the Clara-owned bin staging dir for LSP servers."""
    from clara_constants import get_clara_home

    p = get_clara_home() / "lsp" / "bin"
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    for staged in _native_binary_candidates(clara_lsp_bin_dir() / name):
...
def _install_npm(
    ...
    """
    # Managed npm first: $CLARA_HOME/node is not on an arbitrary process's
    # PATH, so a bare which() misses the Node that Clara installed and
    ...
    if npm is None:
        ...
        return None
    staging = clara_lsp_bin_dir().parent  # <CLARA_HOME>/lsp/
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a clara-owned target dir.
    ...
    console script.
    """
    pip_target = clara_lsp_bin_dir().parent / "python-packages"
    ...
    for script_dir in script_dirs:
        for bin_path in _native_binary_candidates(script_dir / bin_name):
            if bin_path.exists():
                link = clara_lsp_bin_dir() / bin_path.name
                if not link.exists():
                    try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``clara lsp status`` CLI to give users a quick
