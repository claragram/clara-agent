...
The :class:`LSPService` is the bridge between the __PROT_0_synchroclara__
...
- A **single asyncio event loop** runs in a background thread.  All
  client work happens on that loop.  __PROT_1_Synchroclara__ callers from
...
class _BackgroundLoop:
    ...
    Provides :meth:`run` for __PROT_2_synchroclara__ callers — submits a coroutine
...
class LSPService:
    ...
    def create_from_config(cls) -> Optional["LSPService"]:
        """Build a service from ``clara_cli.config`` settings.
    ...
    def enabled_for(self, file_path: str) -> bool:
        ...
        timeout cost — until the service is restarted (``clara lsp
    ...
    def get_diagnostics_sync(
        ...
        *,
        ...
        """__PROT_3_Synchroclaraly__ open ``file_path`` in the right server, wait for
    ...
    # ------------------------------------------------------------------
    # status / introspection (used by ``clara lsp status``)
    # ------------------------------------------------------------------
