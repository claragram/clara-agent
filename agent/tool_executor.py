...
def _ensure_file_checkpoint(
    ...
    # which can differ from the Clara process cwd (notably in Docker).  Resolve
...
def _resolve_concurrent_tool_timeout() -> float | None:
    ...
    in config.yaml wins, the legacy ``CLARA_CONCURRENT_TOOL_TIMEOUT_S`` env var
    ...
    return resolve_timeout(
        ...
        env_var="CLARA_CONCURRENT_TOOL_TIMEOUT_S",
    )
...
def _flush_session_db_after_tool_progress(
    ...
    *,
    ...
    current Clara process before the normal turn-end persistence path runs.
...
def _run_agent_tool_execution_middleware(
    ...
    *,
    ...
    """Run Relay rewrites before Clara policy and dispatch exactly once."""
    ...
    def _clara_pipeline(relay_args: dict[str, Any]) -> Any:
...
def _resolve_sequential_tool_timeout() -> float | None:
    ...
    ``CLARA_CONCURRENT_TOOL_TIMEOUT_S`` legacy bridge) so the two executor
