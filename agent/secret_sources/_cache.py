...
cache problem must never block Clara startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Clara home used for cache paths.

    ``home_path`` is whatever ``load_clara_dotenv()`` already resolved;
    falling back to ``$CLARA_HOME`` / ``~/.clara`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        from clara_constants import get_clara_home

        home_path = get_clara_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<clara_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if CLARA_HOME is ever made traversable.
            try:
