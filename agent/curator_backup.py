...
A pre-run snapshot of ``~/.clara/skills/`` (excluding ``.curator_backups/``
itself) is taken before any mutating curator pass. Snapshots are tar.gz
files under ``~/.clara/skills/.curator_backups/<utc-iso>/`` with a
...
``~/.clara/cron/jobs.json`` as ``cron-jobs.json`` when it exists. Cron
...
def _backups_dir() -> Path:
    return get_clara_home() / "skills" / ".curator_backups"
...
def _cron_jobs_file() -> Path:
    """Source path for the live cron jobs store (``~/.clara/cron/jobs.json``)."""
    return get_clara_home() / "cron" / "jobs.json"
...
def snapshot_skills(reason: str = "manual", *, protect_ids: Optional[Set[str]] = None) -> Optional[Path]:
    """Create a tar.gz snapshot of ``~/.clara/skills/`` and prune old ones.
    ...
    if not skills.exists():
        logger.debug("No ~/.clara/skills/ directory — nothing to back up")
        return None
...
def rollback(backup_id: Optional[str] = None) -> Tuple[bool, str, Optional[Path]]:
    """Restore ``~/.clara/skills/`` from a snapshot.
    ...
    Strategy:
      ...
      4. Extract the chosen snapshot into ``~/.clara/skills/``.
    ...
    if target is None:
        return (
            ...
            + " (use `clara curator rollback --list` to see available snapshots)",
            None,
        )
