...
``auth.json``.  They intentionally have no dependency on ``clara_cli.auth`` so
...
# Sources Clara owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Clara-owned OAuth/device-code state)
