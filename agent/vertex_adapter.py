"""Vertex AI (Google Cloud) adapter for Clara Agent.

Provides authentication and configuration for Vertex AI's OpenAI-compatible
endpoint. This allows Clara to use Gemini models via Google Cloud with
...
# who installed plain `clara-agent` and only later selected a Gemini model.
try:
...
def has_explicit_vertex_config() -> bool:
    """True only when the user deliberately pointed Clara at Vertex.
    ...
    Clara up for Vertex would suddenly see it (and could spend against those
    credentials). So this checks only Clara-scoped signals:
      ...
      * a resolvable ``VERTEX_CREDENTIALS_PATH`` service-account file
        (the Clara-specific path var — NOT ``GOOGLE_APPLICATION_CREDENTIALS``).
    """
    if _resolve_project_override():
        return True
