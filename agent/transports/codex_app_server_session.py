...
Owns one Codex thread per Clara session. Drives `turn/start`, consumes
...
call is __PROT_0_synchroclara__ and behaves like AIAgent's existing chat_completions loop.
"""
...
# Clara' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_CLARA_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Clara/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Clara session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _CLARA_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("CLARA_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="clara",
            client_title="Clara Agent",
            client_version=_get_clara_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Clara' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Clara' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own clara-tools callback we
            # auto-accept — the user already approved Clara' tools
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO Clara
        approval-mode/timeout logic. The Clara-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; Clara approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Clara approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Clara returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the Clara approval mode/timeout resolution
    ...
    # "prompt expired"; the Clara-side messaging already distinguishes them.
    return "decline"
...
def _get_clara_version() -> str:
    """Best-effort Clara version string for codex's userAgent line."""
    try:
