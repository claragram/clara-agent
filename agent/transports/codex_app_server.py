...
projection into Clara' display, approval bridging, transcript projection into
...
"codex_app_server"`. Clara' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    Threading model:
      - Spawning thread (caller) drives request/response pairs __PROT_1_synchroclaraly__.
    ...
    Intentionally NOT async. AIAgent.run_conversation() is __PROT_0_synchroclara__ and
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Clara secret — gateway
        ...
        spawn_env = clara_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("CLARA_KANBAN_TASK"):
            kanban_db = spawn_env.get("CLARA_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "CLARA_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("CLARA_HOME", os.path.expanduser("~/.clara")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "clara",
        client_title: str = "Clara Agent",
