...
row taxonomy is inferred from the atlas shape so Clara can render both legacy
...
# rows (1536x1872), while older Clara/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Clara' activity state names. They are not always identical to the
...
# Legacy Clara/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Clara activity names -> accepted row-name aliases in descending
