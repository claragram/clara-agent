...
def build_subscription_state(*, timeout: float = 15.0) -> SubscriptionState:
    ...
    Dev override: when ``CLARA_DEV_SUBSCRIPTION_FIXTURE`` names a fixture state,
...
_DEV_FIXTURE_PORTAL = "https://portal.claraprise.com/billing"
...
def dev_fixture_subscription_state() -> Optional[SubscriptionState]:
    """Return a fixture :class:`SubscriptionState` for ``CLARA_DEV_SUBSCRIPTION_FIXTURE``.
    ...
    Throwaway scaffolding — mirrors ``CLARA_DEV_CREDITS_FIXTURE``.
    """
    name = (os.getenv("CLARA_DEV_SUBSCRIPTION_FIXTURE") or "").strip().lower()
    if not name:
        return None
    ...
    return SubscriptionState(logged_in=False, error=f"unknown CLARA_DEV_SUBSCRIPTION_FIXTURE: {name}")
