...
Data resolution order:
  ...
  2. Disk cache (~/.clara/models_dev_cache.json — any age; stale data is
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Clara ↔ models.dev
# ---------------------------------------------------------------------------

# Clara provider names → models.dev provider IDs
PROVIDER_TO_MODELS_DEV: Dict[str, str] = {
    ...
    # under the "meta" provider id; Clara' provider is "meta-ai" (and the
...
# Reverse mapping: models.dev id → Clara ids (built lazily; many-to-one,
# e.g. both "meta" and "meta-ai" may map to the same models.dev id).
_MODELS_DEV_TO_PROVIDER: Optional[Dict[str, List[str]]] = None


def _models_dev_to_clara_ids(mdev_id: str) -> List[str]:
    """Return the Clara provider ids that map to *mdev_id* (may be [])."""
    global _MODELS_DEV_TO_PROVIDER
    if _MODELS_DEV_TO_PROVIDER is None:
        reverse: Dict[str, List[str]] = {}
        for clara_id, mapped in PROVIDER_TO_MODELS_DEV.items():
            reverse.setdefault(mapped, []).append(clara_id)
...
def _get_cache_path() -> Path:
    ...
    return get_clara_home() / "models_dev_cache.json"


def _get_etag_path() -> Path:
    """Return path to the ETag sidecar file for conditional GET."""
    from clara_constants import get_clara_home
    return get_clara_home() / "models_dev_cache.etag"
...
def fetch_models_dev(
    ...
    When ``force_refresh=True`` (used by ``clara config refresh``, the
...
def _provider_override_section(provider: str) -> Optional[Dict[str, Any]]:
    ...
    Accepts either the Clara provider id or the models.dev provider id as
    ...
    # Reverse: caller passed a models.dev id, config keyed by Clara id.
    for clara_id in _models_dev_to_clara_ids(provider_key):
        if clara_id != provider_key:
            candidates.append(clara_id)
...
def _get_provider_models(
    ...
    """Resolve a Clara provider ID to its models dict from models.dev.
...
def list_provider_models(
    ...
    picker (``clara model``), which is an interactive user-facing flow
...
# Gemma models whose TPM quotas are too small for normal Clara agent traffic.
...
def get_provider_info(
    ...
    Accepts either a Clara provider ID (e.g. "kilocode") or a models.dev
    ...
    ``allow_network=False``.
    """
    # Resolve Clara ID → models.dev ID
...
def get_model_info(
    ...
    Accepts Clara or models.dev provider ID.  Tries exact match then
