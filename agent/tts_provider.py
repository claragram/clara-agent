...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Clara with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.clara/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Clara
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``clara tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``clara setup`` for
