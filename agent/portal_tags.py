"""Centralized Clara Portal request tags.

Every Clara request that hits the Clara Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Clara can attribute usage to Clara Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=clara-agent",
        "client=clara-client-v<__version__>",
    ]

The version is sourced live from ``clara_cli.__version__`` so it auto-aligns
...
``clara_cli.__version__`` is the canonical source of truth.
"""
...
# active conversation id here and ``clara_portal_tags()`` picks it up as a
...
_conversation_id: ContextVar[Optional[str]] = ContextVar(
    "clara_portal_conversation_id", default=None
)
...
# sticky ``session_id``, Clara Portal's sticky key and xAI's ``x-grok-conv-id``
...
_affinity_scope: ContextVar[Optional[str]] = ContextVar(
    "clara_affinity_scope", default=None
)
...
def _clara_version() -> str:
    """Return the current Clara release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``clara_cli`` cannot be imported (should
...
def clara_client_tag() -> str:
    """Return the ``client=...`` tag for Clara Portal requests.

    Format: ``client=clara-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=clara-client-v{_clara_version()}"


def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a Clara session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    Clara conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.clara/sessions/`` storage, session logs, and lineage.
...
def clara_portal_tags(session_id: str | None = None) -> List[str]:
    """Return the canonical list of Clara Portal product tags.

    Always returns a fresh list so callers can mutate it freely
    (e.g. ``merged_extra.setdefault("tags", []).extend(clara_portal_tags())``).

    When ``session_id`` is provided, a ``conversation=<session_id>`` tag is
    appended so Portal usage can be attributed to a specific Clara
    ...
    """
    tags = ["product=clara-agent", clara_client_tag()]
