"""Unified removal contract for every credential source Clara reads from.

Clara seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.clara/.env
    claude_code   — ~/.claude/.credentials.json
    clara_pkce   — ~/.clara/.anthropic_oauth.json
    device_code   — auth.json providers.<provider> (clara, openai-codex, ...)
    qwen-cli      — ~/.qwen/oauth_creds.json
    gh_cli        — gh auth token
    config:<name> — custom_providers config entry
    model_config  — model.api_key when model.provider == "custom"
    manual        — user ran `clara auth add`
...
unify here is **removal**:
    ...
    ``clara auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, clara device_code (partial), clara_pkce, copilot gh_cli, and
...
class RemovalStep:
    ...
    Attributes:
        provider: Provider pool key (``"xai"``, ``"anthropic"``, ``"clara"``, ...).
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.clara/.env  → clear it
    ...
    """
    from clara_cli.config import get_env_path, remove_env_value
    ...
    try:
        ...
        if env_path.exists():
            ...
            # canonical reader in clara_cli/config.py. read_text() with no
    ...
    if shell_exported:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to Clara.",
            f"  The pool entry is now suppressed — Clara will ignore "
            f"{env_var} until you run `clara auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Clara stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `clara auth add anthropic` to re-enable if needed.",
    ])


def _remove_clara_pkce(provider: str, removed) -> RemovalResult:
    """~/.clara/.anthropic_oauth.json is ours — delete it outright."""
    from clara_constants import get_clara_home

    result = RemovalResult()
    oauth_file = get_clara_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Clara Anthropic OAuth credentials")
...
def _remove_clara_device_code(provider: str, removed) -> RemovalResult:
    """Clara OAuth lives in auth.json providers.clara — clear it and suppress.

    We suppress in addition to clearing because nothing else stops a future
    `clara auth add clara` (or any other path that writes providers.clara)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `clara auth add clara` to re-engage, which is the
...
def _remove_minimax_oauth(provider: str, removed) -> RemovalResult:
    ...
    Same pattern as Clara: single-source OAuth state with refresh tokens.
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``clara auth remove xai-oauth <N>`` silently undoes
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `clara model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Clara auth store is not enough — _seed_from_singletons() would
    ...
    via ``clara auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `clara auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `clara auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Clara stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `clara auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="clara_pkce",
        remove_fn=_remove_clara_pkce,
        description="~/.clara/.anthropic_oauth.json",
    ))
    register(RemovalStep(
        provider="clara", source_id="device_code",
        remove_fn=_remove_clara_device_code,
        description="auth.json providers.clara",
    ))
    register(RemovalStep(
