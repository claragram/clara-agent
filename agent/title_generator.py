...
# Clara' own machine-authored openers. A compaction handoff or a resumed
...
def _auto_title_enabled() -> bool:
    ...
    try:
        ...
        # from agent code paths where a module-level clara_cli import risks
...
def apply_instant_title(
    ...
    """Write the derived title __PROT_0_synchroclaraly__. Cheap enough to run inline.
...
def auto_title_session(
    ...
    post-``clara update`` stale-module window, where this function's lazy
    ...
    try:
        ...
        logger.warning(
            ...
            "restart the running Clara process): %s",
            e,
        )
...
def _is_real_user_turn(message: Any) -> bool:
    ...
    Clara persists a lot of machinery under ``role="user"`` — compaction
