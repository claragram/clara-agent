...
as ``kind: backend``) or ``~/.clara/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        backend can't honor are not advertised), and by ``clara tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$CLARA_HOME/cache/videos/``, creating parents as needed."""
    from clara_constants import get_clara_home

    path = get_clara_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$CLARA_HOME/cache/videos/``.
...
def save_url_video(
    ...
    *,
    ...
    """Download a video URL and write it under ``$CLARA_HOME/cache/videos/``.
