...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the CLARA_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Clara runtime / deployment
    "CLARA_HOME", "CLARA_PROFILE", "CLARA_GATEWAY_LOCK_DIR",
    "CLARA_MAX_ITERATIONS", "CLARA_MAX_TOKENS", "CLARA_API_TIMEOUT",
    "CLARA_REDACT_SECRETS", "CLARA_CLARA_TIMEOUT_SECONDS",
    "_CLARA_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "CLARA_KANBAN_DB", "CLARA_KANBAN_WORKSPACES_ROOT", "CLARA_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "CLARA_KANBAN_",
    "CLARA_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    ``set_secret_scope``. Parses the small KEY=VALUE subset Clara writes
    ...
    ``clara_cli.config._parse_env_value``) but never mutates the process
    ...
    # Parse values with the canonical Clara parser: save_env_value
    ...
    from clara_cli.config import _parse_env_value
...
def build_profile_secret_scope(clara_home: Path) -> Dict[str, str]:
    ...
    try:
        from clara_cli.env_loader import get_secret_source_values
