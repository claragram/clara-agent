...
CLARA_HOME_DIR="${CLARA_HOME:-$HOME/.clara}"
CLARA_CFG="${CLARA_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Clara config has twozero_td MCP entry ──
if [[ ! -f "$CLARA_CFG" ]]; then
    echo -e " ${FAIL} Clara config not found at ${CLARA_CFG}"
    manual_steps+=("Create ${CLARA_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$CLARA_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Clara config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Clara config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${CLARA_CFG} manually"); }
    manual_steps+=("Restart Clara session to pick up config change")
fi
