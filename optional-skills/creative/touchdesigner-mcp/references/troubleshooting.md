...
## 5. Clara Configuration

### Config location

`$CLARA_HOME/config.yaml` (defaults to `~/.clara/config.yaml` when `CLARA_HOME` is unset)
...
Restart the Clara session for changes to take effect. The MCP connection is
