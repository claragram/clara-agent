...
Maps each role archetype to the Clara skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public clara-agent
...
## Clara skills relevant to video production

### Visual / rendering skills (`clara-agent/skills/creative/`)
...
### Design / pre-production skills (`clara-agent/skills/creative/`)
...
### Audio / media skills (`clara-agent/skills/creative/` + `skills/media/`)
...
These are **not** Clara skills but external CLIs / APIs that profiles invoke.
...
## Built-in Clara tools for media review

These are native Clara tools — not invoked via terminal but through their own
...
key is present in `${CLARA_HOME:-~/.clara}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Clara profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${CLARA_HOME:-~/.clara}/.env` → environment variable.
...
When a new Clara-public video skill ships:
