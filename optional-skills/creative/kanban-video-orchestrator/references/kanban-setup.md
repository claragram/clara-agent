...
the actual `setup.sh` that creates the project workspace, configures Clara
...
> [Workprise/kanban-video-pipeline](https://github.com/claraprise/kanban-video-pipeline).
...
2. **Create profiles** — `clara profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.clara/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `clara kanban create` assigned to the director
...
```bash
clara profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.clara/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.clara/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Clara
...
Each profile gets a `SOUL.md` at `~/.clara/profiles/<name>/SOUL.md` that
...
```bash
clara kanban create "Direct production of <video title>" \
    --assignee director \
...
the Clara `.env` (`${CLARA_HOME:-$HOME/.clara}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _clara_env="${CLARA_HOME:-$HOME/.clara}/.env"
    if grep -q "^${var}=" "$_clara_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_clara_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_clara_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY clara ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY clara OPENROUTER_API_KEY || exit 1
# ...
```
