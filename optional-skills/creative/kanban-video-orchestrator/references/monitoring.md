...
clara kanban watch --tenant <project-slug>

# Snapshot of the board
clara kanban list --tenant <project-slug>
clara kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
clara kanban stats --tenant <project-slug>

# Visual dashboard (browser)
clara dashboard

# Inspect a specific task (includes comments + events)
clara kanban show <task-id>

# Follow a single task's event stream
clara kanban tail <task-id>
```

Verify available subcommands with `clara kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `clara kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `clara kanban block <id>` then `clara kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `clara kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `clara kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `clara profile list` (profile exists?), `clara status` (gateway/dispatcher up?), and verify tenant. |
...
clara kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
clara kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(clara kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
clara kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
clara kanban block <task-id>
clara kanban archive <task-id>

# Diagnose what happened
clara kanban show <task-id>      # task body, comments, recent events
clara kanban tail <task-id>      # follow the live event stream
clara kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `clara kanban create` for the director
...
while true; do
    clear
    clara kanban list --tenant <slug>
    echo "---"
    clara kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `clara kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `clara profile list` and start
