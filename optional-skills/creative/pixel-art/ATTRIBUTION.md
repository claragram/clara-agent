...
- `generate_video()` split: the Pollinations text-to-image call was removed
  (Clara uses its own `image_generate` + `pixel_art()` pipeline for base
...
- Pollinations text-to-image generation (`pixelart_image.py`,
  `generate_base()` in `pixelart_video.py`) — Clara has `image_generate`
...
- License: MIT (inherits from clara-agent repo)
- Original author of the skill shell: dodo-reach
- Expansion with palettes + video: Clara Agent contributors
