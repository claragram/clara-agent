...
### 1. Start order: editor first, Clara second

Clara probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Clara session.
...
restart the editor, restart the Clara session.
...
server fails to bind (Output Log) or Clara' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.clara/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Clara

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Clara' connection
lives in `~/.clara/config.yaml` via `clara mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Clara — just don't
mistake it for the Clara setup step.
...
explicitly warns against overlapping calls. Clara executes same-turn tool
...
### 9. Timeouts: Clara gives up before Unreal does

Clara' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Clara runs elsewhere (SSH backend,
