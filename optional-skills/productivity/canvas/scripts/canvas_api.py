#!/usr/bin/env python3
"""Canvas LMS API CLI for Clara Agent.
...
def _check_config():
    ...
    if missing:
        clara_env = os.path.join(
            os.environ.get("CLARA_HOME", os.path.expanduser("~/.clara")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {clara_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Clara Agent"
    )
