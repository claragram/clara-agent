...
Usage (via cron with --no-agent):
    ...
    clara cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$CLARA_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Clara-Watcher/1.0"})
