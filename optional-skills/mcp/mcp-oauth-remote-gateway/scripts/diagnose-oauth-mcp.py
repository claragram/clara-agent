...
Usage:
  ...
  <server_name> matches the files in $CLARA_HOME/mcp-tokens/<server>.json etc.
...
def _clara_home():
    # Prefer Clara' own resolver (profile-safe); fall back to env then ~/.clara.
    try:
        from clara_constants import get_clara_home
        return str(get_clara_home())
    except Exception:
        return os.environ.get("CLARA_HOME") or os.path.expanduser("~/.clara")


def _tokens_dir():
    return os.path.join(_clara_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "clara-diag", "version": "1.0"}},
        },
        headers={
