...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("CLARA_HOME", os.path.expanduser("~/.clara")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("CLARA_HOME", Path.home() / ".clara")) / "skills" / "red-teaming" / "godmode"
...
CLARA_HOME = Path(os.getenv("CLARA_HOME", Path.home() / ".clara"))
CONFIG_PATH = CLARA_HOME / "config.yaml"
PREFILL_PATH = CLARA_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Clara/Clara models — already uncensored, just needs clean prompt
    "clara": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "clara" in model_lower or "clara" in model_lower:
        return "clara"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Clara config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.clara/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Clara for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Clara for changes to take effect.")
