...
The design is **Clara-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
clara skills install official/security/unbroker
```

Then start a new Clara session and drive it (below). The skill works zero-config; a few optional
...
- **PII handling.** Dossiers live under the Clara home directory (`0600`, optionally
