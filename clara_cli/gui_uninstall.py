"""
Clara Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``clara desktop`` / ``clara gui``)
     Built inside the agent checkout under ``$CLARA_HOME/clara-agent/``:
       ...
       - ``$CLARA_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Clara.app`` or ``~/Applications/Clara.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Clara`` (NSIS per-user)
...
the app name ("Clara"), separate from ``$CLARA_HOME``:
  - macOS:   ``~/Library/Application Support/Clara``
  - Windows: ``%APPDATA%\\Clara``
  - Linux:   ``$XDG_CONFIG_HOME/Clara`` (default ``~/.config/Clara``)
...
``clara uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(clara_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return clara_home / "clara-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Clara"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Clara"
...
def source_built_gui_artifacts(clara_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``clara desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``clara-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(clara_home)
    ...
    return [
        ...
        clara_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Clara" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Clara.app"),
            home / "Applications" / "Clara.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Clara).
            local_base / "Programs" / "Clara",
            # Older / alternate layout some builds used.
            local_base / "clara-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Clara")
    else:
        ...
        # ``uninstall_gui``.
        from clara_cli.linux_desktop_entry import desktop_entry_path

        data = os.environ.get("XDG_DATA_HOME")
        data_base = Path(data) if data else (home / ".local" / "share")
        paths += [
            # The launcher entry `clara desktop` installs. Its icon is
            ...
            data_base / "applications" / "Clara.desktop",
            data_base / "icons" / "hicolor" / "scalable" / "apps" / "clara.png",
        ]
        ...
        for size in ("24x24", "32x32", "48x48", "256x256", "512x512", "1024x1024"):
            paths.append(data_base / "icons" / "hicolor" / size / "apps" / "clara.png")
    return paths


def agent_is_installed(clara_home: Path) -> bool:
    """Return True when a usable Python agent install exists under CLARA_HOME.
    ...
    if (agent_root / "clara_cli").is_dir():
        return True
...
def gui_is_installed(clara_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(clara_home):
        if p.exists():
            return True
...
def gui_install_summary(clara_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = clara_home if clara_home is not None else get_clara_home()
...
def uninstall_gui(
    clara_home: "Path | None" = None, *, remove_userdata: bool = True
) -> "list[Path]":
    ...
    Never touches ``clara-agent/clara_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$CLARA_HOME``.
    ...
    if sys.platform.startswith("linux"):
        ...
        # Reindex so Clara disappears from the launcher.
        try:
            from clara_cli.linux_desktop_entry import (
                desktop_entry_path,
        ...
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove clara' or "
            "'sudo dnf remove clara'). AppImage builds are a single file you "
