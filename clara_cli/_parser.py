"""
Top-level argparse construction for the clara CLI.
...
# argparse runs (it sets ``CLARA_HOME`` and strips itself from ``sys.argv``),
...
# tests/clara_cli/test_top_level_value_flags_parity.py fails CI if the parser
...
def top_level_value_flag_sets() -> tuple[frozenset[str], frozenset[str]]:
    ...
    the exact drift that made ``clara --reasoning high chat …`` misread
...
def _inherited_flag(parser, *args, **kwargs):
    """Register a flag that ``clara_cli.relaunch`` should carry over when
...
_EPILOGUE = """
Examples:
    clara                        Start interactive chat
    clara chat -q "Hello"        Single query mode
    clara --tui                  Launch the modern TUI (or set display.interface: tui)
    clara --cli                  Force the classic REPL (overrides display.interface: tui)
    clara -c                     Resume the most recent session
    clara -c "my project"        Resume a session by name (latest in lineage)
    clara --resume <session_id>  Resume a specific session by ID
    clara --resume latest        Resume the most recent session (same as -c)
    clara --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    clara setup                  Run setup wizard
    clara logout                 Clear stored authentication
    clara auth add <provider>    Add a pooled credential
    clara auth list              List pooled credentials
    clara auth remove <p> <t>    Remove pooled credential by index, id, or label
    clara auth reset <provider>  Clear exhaustion status for a provider
    clara model                  Select default model
    clara fallback [list]        Show fallback provider chain
    clara fallback add           Add a fallback provider (same picker as `clara model`)
    clara fallback remove        Remove a fallback provider from the chain
    clara config                 View configuration
    clara config edit            Edit config in $EDITOR
    clara config set model gpt-4 Set a config value
    clara gateway                Run messaging gateway
    clara -s clara-agent-dev,github-auth
    clara -w                     Start in isolated git worktree
    clara gateway install        Install gateway background service
    clara sessions list          List past sessions
    clara sessions browse        Interactive session picker
    clara sessions rename ID T   Rename/title a session
    clara logs                   View agent.log (last 50 lines)
    clara logs -f                Follow agent.log in real time
    clara logs errors            View errors.log
    clara logs --since 1h        Lines from the last hour
    clara debug share             Upload debug report for support
    clara console                Open the safe Clara command console
    clara update                 Update to latest version
    clara dashboard              Start web UI dashboard (port 9119)
    clara dashboard --stop       Stop running dashboard processes
    clara dashboard --status     List running dashboard processes
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="clara",
        description="Clara Agent - AI assistant with tool-calling capabilities",
    ...
    # Mirrors `clara chat --model ... --provider ...` semantics.
    _inherited_flag(
        parser,
        "-m",
        "--model",
        ...
        help=(
            ...
            "Applies to -z/--oneshot and --tui. Also settable via CLARA_INFERENCE_MODEL env var."
        ),
    )
    _inherited_flag(
        parser,
        ...
        help=(
            ...
            "under model.provider — use `clara setup` or edit the file to change it."
        ),
    )
    _inherited_flag(
        parser,
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "without a TTY prompt.  Equivalent to CLARA_ACCEPT_HOOKS=1 or "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=False,
        help="Ignore ~/.clara/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    _inherited_flag(
        parser,
        "--ignore-rules",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Clara Agent",
    )
    ...
    # top-level parser: when the user writes `clara -m foo chat`, argparse
    ...
    # `-w/--worktree`, `--yolo`, etc. (see tests/clara_cli/
    ...
    _inherited_flag(
        ...
        help=(
            "Auto-approve any unseen shell hooks declared in config.yaml "
            "without a TTY prompt (see also CLARA_ACCEPT_HOOKS env var and "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore ~/.clara/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    _inherited_flag(
        chat_parser,
        "--ignore-rules",
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Clara itself.",
    )
    chat_parser.add_argument(
        "--source",
        default=None,
