"""Client for uploading ``clara debug share`` bundles to Clara-internal S3.

This is the opt-in (``--clara``) destination for ``clara debug share``.
Unlike the public paste.rs path, bundles uploaded here go to a Clara-owned
S3 bucket via a short-lived signed URL minted by the Clara account service
(NAS).  The bucket auto-expires objects after 14 days, and the contents are
only viewable by Clara staff (and allowlisted Discord mods) through a
...
# Base URL of the Clara account service that mints the signed upload URL.
...
NAS_BASE = os.environ.get(
    "CLARA_DIAGNOSTICS_BASE_URL", "https://portal.claraprise.com"
)
...
_USER_AGENT = "clara-agent/debug-share"
...
def share_to_clara(report_bundle: bytes) -> dict:
    """Orchestrate the full Clara-S3 upload of a gzipped *report_bundle*.
