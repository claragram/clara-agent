...
``clara update`` mutation (CLI apply, CLI --check, dashboard update
endpoint). The decision layers:

1. **Baked provenance marker** (``/etc/clara/image-provenance.json``,
   written by the image build — see :mod:`clara_cli.image_provenance`):
