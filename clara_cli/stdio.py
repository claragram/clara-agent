...
on Japanese locales, etc.).  Clara's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``clara_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Clara is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``CLARA_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("CLARA_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Clara-managed Git install directories so
    ...
    # already-running shells don't see the change — which means clara
    ...
    # install without Clara-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Clara's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Clara's
    ``clara config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Clara (User env var in Windows Settings, or
    export in a PowerShell profile) and Clara picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Clara-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\clara\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``clara`` from right after install) retain their old PATH.

    Any subprocess Clara spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Clara-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "clara", "git", "cmd"),
        os.path.join(local_appdata, "clara", "git", "bin"),
        os.path.join(local_appdata, "clara", "git", "usr", "bin"),
        # Clara venv Scripts directory — host of the clara.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes clara, but harmless to include.
        os.path.join(local_appdata, "clara", "clara-agent", "venv", "Scripts"),
