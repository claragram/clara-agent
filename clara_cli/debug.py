"""``clara debug`` debug tools for Clara Agent.

Currently supports:
    clara debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.clara/logs/*.log`` are not leaked into
                          the public paste service. Pass ``--no-redact``
                          to disable.
                          Pass ``--clara`` to upload instead to Clara-internal
                          storage (AWS S3) via a signed URL minted by the
                          Clara account service: the bundle is private
                          (viewable only by Clara staff / allowlisted mods via
...
_REDACTION_BANNER = (
    "[clara debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.clara/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``clara debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    """__PROT_0_Synchroclaraly__ DELETE any pending pastes whose ``expire_at`` has passed.
    ...
    intended to be called from every ``clara debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `clara debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "clara-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``clara debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.clara/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``clara debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "clara-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----ClaraDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from clara_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_clara_home() / "logs" / filename) if filename else None


# Logs written by a client process rather than by this backend. When the
# desktop app talks to a remote/docker/SSH backend, `clara debug share` runs
...
_CLIENT_SIDE_LOGS = {
    "desktop": (
        "written by Clara Desktop on the machine running the app, not by this "
...
def _capture_dump() -> str:
    """Run ``clara dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``clara dump`` is run
        internally.
...
# ---------------------------------------------------------------------------
# Shared bundle collection (used by both the paste.rs and Clara-S3 paths)
# ---------------------------------------------------------------------------

# Bundle format identifier embedded in the Clara-S3 JSON envelope. The
# discord-support viewer keys off this string to parse the bundle.
_CLARA_BUNDLE_FORMAT = "clara-debug-share/1"
...
def collect_share_bundle(
    ...
    Clara-S3 path (``--clara``).  Centralising it guarantees the Clara bundle is
    built from the *same* force-redacted snapshots as the public paste path —
    redaction is the safety boundary, so the Clara path must never see raw
    logs.
...
def build_clara_bundle(bundle: dict[str, str], redact: bool = True) -> bytes:
    """Gzip-compress a :func:`collect_share_bundle` mapping into the Clara envelope.

    The JSON shape is what the discord-support viewer (Repo 3) parses::

        {"format": "clara-debug-share/1",
         "redacted": <bool>,
         "created": <iso8601>,
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``clara debug share`` (CLI) and the
    ...
    # shared collector so the paste.rs and Clara-S3 paths build identical,
    ...
    if redact:
        logger.info(
            "clara debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    clara = getattr(args, "clara", False)
    ...
    if clara:
        _run_debug_share_clara(args, log_lines=log_lines, redact=redact)
        return
    ...
    try:
        ...
        print("\nRun `clara debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print("To delete now:  clara debug delete <url>")

    print("\nShare these links with the Clara team for support.")


_CLARA_PRIVACY_NOTICE = """\
⚠️  --clara: This uploads your debug bundle to Clara-INTERNAL storage (AWS S3),
    NOT a public paste service. The following is included:
  • System info (OS, Python/Clara version, provider, which API keys are
  ...
  • The bundle is viewable only by Clara staff (and allowlisted Discord mods)
...
def _run_debug_share_clara(args, *, log_lines: int, redact: bool) -> None:
    """Handle ``clara debug share --clara``: upload the bundle to Clara-S3.

    Collects the same force-redacted bundle as the paste path, gzips it into
    the Clara envelope, requests a signed URL from NAS, uploads, and prints the
    ...
    if redact:
        logger.info(
            "clara debug share --clara: applied force-mode redaction before upload"
        )
    blob = build_clara_bundle(bundle, redact=redact)

    print("Uploading to Clara diagnostics storage...")
    try:
        res = share_to_clara(blob)
    except Exception as exc:
        print(
            f"\nClara upload failed: {exc}\n"
            "\nThe Clara diagnostics service may be unavailable or not yet "
            "provisioned.\n"
            "Run `clara debug share --local` to print the report instead, "
            "or `clara debug share` to upload to a public paste service.\n",
            file=sys.stderr,
        )
        sys.exit(1)

    view_url = res.get("viewUrl") or res.get("view_url")
    print("\nDebug bundle uploaded to Clara (private):")
    if view_url:
    ...
    print(
        "\nShare this private link with the Clara team — only Clara staff "
        "(via Google login) can open it."
    )
    print(
        "\nPick up the discussion in:\n"
        "  GitHub Issues        https://github.com/claraprise/clara-agent/issues\n"
        "  Clara Portal Support  https://portal.claraprise.com/help\n"
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: clara debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'clara debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``clara debug`` call.
    ...
    # best-effort — any failure is swallowed so ``clara debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: clara debug <command>")
        print()
        print("Commands:")
        ...
        print("  --clara       Upload to Clara-internal storage (private, staff-only,")
