"""
Clara Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m clara_cli.main web          # Start on http://127.0.0.1:9119
    python -m clara_cli.main web --port 8080
"""
...
from clara_cli._subprocess_compat import windows_detach_flags, windows_hide_flags
from clara_cli.install_identity import get_install_id as _shared_get_install_id
...
try:
    ...
    # running `clara dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["CLARA_WEB_DIST"]) if "CLARA_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``clara gateway run`` — but the
    desktop app spawns a ``clara dashboard`` backend, not a gateway, so a cron
    ...
    if isinstance(provider, InProcessCronScheduler):
        try:
            ...
            if len(profile_homes) > 1:
                ...
                from clara_cli.profiles import _check_gateway_running
...
def _warm_gateway_module() -> None:
    ...
    ``clara_cli.gateway``. But the first WS connection and its initial
...
def _eager_reconcile_own_session_db() -> None:
    ...
    bringing a store left behind by `clara update` current before the
    ...
    """
    try:
        from clara_state import SessionDB, _default_db_path
...
async def _lifespan(app: "FastAPI"):
    ...
    # so a store left behind by `clara update` kept 500ing every
    ...
    # Import clara_cli.gateway eagerly *before* the lifespan yield so the
    ...
    # model picker) can detect when `clara update` replaced the code
    ...
    # Desktop-spawned backends (CLARA_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `clara
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("CLARA_DESKTOP") == "1":
        ...
        # a healthy standalone gateway (launched via `clara gateway run`,
        ...
        try:
            from clara_cli.gateway import _reap_unsupervised_gateway_orphans
    ...
    def _boot_local_runtime():
        try:
            ...
            from clara_cli.local_runtime.bootstrap import ensure_local_runtime
    ...
    try:
        ...
        try:
            from clara_cli.local_runtime.bootstrap import shutdown_local_runtime
...
app = FastAPI(title="Clara Agent", version=__version__, lifespan=_lifespan)


# Memory-provider OAuth connect routes live in the memory layer, not here.
from clara_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
# CLARA_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
def _resolve_session_token() -> str:
    return os.environ.get("CLARA_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)


_SESSION_TOKEN = _resolve_session_token()
_SESSION_HEADER_NAME = "X-Clara-Session-Token"
...
def _apply_ssh_owner_nonce(nonce: Optional[str]) -> None:
    ...
    if nonce:
        ...
        try:
            marker = os.path.join(purelib, f".clara-ssh-runtime-{nonce}")
...
#
# This list is defined in ``clara_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from clara_cli.dashboard_auth.public_paths import (
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Clara-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
def _dashboard_public_hosts() -> frozenset[str]:
    ...
    ``public_url`` is already Clara' canonical browser-facing URL behind a
    ...
    """
    from clara_cli.dashboard_auth.prefix import resolve_public_url
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    unauthenticated-public-dashboard hole behind the June 2026 ``clara-0day``
...
def _desktop_loopback_auth_exempt(
    ...
    for EVERY ``clara serve`` on the machine — including the private loopback
    backends the Desktop app spawns for itself. Those backends authenticate
    with the per-spawn session token (injected via
    ``CLARA_DASHBOARD_SESSION_TOKEN`` for local spawns, ``--ssh-session-token
    ...
    Exemption requires ALL of: loopback bind, ``CLARA_DESKTOP=1`` (set by
    ...
    ``clara serve`` with ``CLARA_DESKTOP=1`` exported but no credential is
    NOT exempt.
    """
    ...
    if os.environ.get("CLARA_DESKTOP") != "1":
        return False
    return bool(
        os.environ.get("CLARA_DASHBOARD_SESSION_TOKEN")
    ...
    if path.startswith("/api/plugins/"):
        ...
        if _authed:
            ...
            if len(parts) >= 4:
                ...
                if plugin_name:
                    try:
                        from clara_cli.plugins_cmd import (
                            _get_enabled_set,
                            _get_disabled_set,
                        )
...
async def _dashboard_auth_gate(request: Request, call_next):
    from clara_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from clara_cli.dashboard_auth.token_auth import token_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "proxy.enabled": {
        "type": "boolean",
        "description": (
            "Docker-only egress credential firewall. Requires `clara egress setup` "
            "and `clara egress start`; Modal/SSH/Daytona are not wired yet."
        ),
        "category": "security",
    },
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Clara (no terminal prompt), "
    ...
    "updates.refresh_cua_driver": {
        "type": "boolean",
        "description": (
            "Refresh an already-installed cua-driver during clara update. "
...
_CATEGORY_MERGE: Dict[str, str] = {
    ...
    # fold it into general rather than spawning a one-field orphan category.
    "session": "general",
    # `clara.keepalive_interval_seconds` is the only schema-surfaced clara field
...
from clara_cli.web_models import (  # noqa: F401
    ConfigUpdate,
    EnvVarUpdate,
    EnvVarDelete,
    EnvVarReveal,
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Clara provider slug — fine.
    ...
    1. Vendor-name → Clara-provider mapping: when the provider string is
       not a known Clara provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    from clara_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from clara_cli.model_normalize import normalize_model_for_provider
    from clara_cli.providers import resolve_custom_provider, resolve_user_provider
    ...
    # not be treated as a stray vendor prefix -- it isn't a known Clara
    ...
    if (
        ...
        try:
            ...
            cur_provider = ""
        from clara_cli.models import _AGGREGATOR_PROVIDERS
...
_MANAGED_FILES_ROOT_ENV = "CLARA_DASHBOARD_FILES_ROOT"
...
# CLARA_HOME itself, at which point every one of these basenames is a live
...
# without needing to resolve them relative to CLARA_HOME.
...
def _is_sensitive_filename(name: str) -> bool:
    ...
    canonical Clara credential-store basenames (see
...
def _is_sensitive_path(path: Path) -> bool:
    ...
    and needs no CLARA_HOME resolution, so it blocks these trees wherever
...
def _default_clara_root_is_opt_data() -> bool:
    raw = os.environ.get("CLARA_HOME", "").strip()
    if not raw:
        return False
    try:
        from clara_constants import get_default_clara_root

        root = get_default_clara_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``clara update``.
    ...
    a container — e.g. the clara-webui image sharing the Clara source tree),
    the dashboard's ``clara update`` button is the correct update path and
    ...
    """
    if _default_clara_root_is_opt_data():
        return True
    try:
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's Clara root is actually /opt/data
    # (the container/hosted layout) or when CLARA_DASHBOARD_FILES_ROOT is set.
    if _default_clara_root_is_opt_data():
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str] = None):
    ...
    The dashboard /chat page runs Clara inside an xterm.js PTY. Browser
...
async def fs_write_text(payload: FsWriteText):
    ...
    Mirrors the local Electron ``clara:fs:writeText`` hardening: the path is
    ...
    tmp = target.with_name(f".{target.name}.clara-tmp-{os.getpid()}")
    try:
...
# gate + path hardening as /api/fs). Logic lives in ``clara_cli.web_git``;
# these are thin, executor-offloaded wrappers (git/gh can block).
# ---------------------------------------------------------------------------

from clara_cli import web_git as _web_git  # noqa: E402
...
from clara_cli.web_routers import git as _git_routes  # noqa: E402

app.include_router(_git_routes.router)

from clara_cli.web_routers import local_models as _local_models_routes  # noqa: E402

app.include_router(_local_models_routes.router)
from clara_cli.web_routers.git import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    git_status_route,
    git_worktrees_route,
    git_branches_route,
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    * ``profile_platforms`` — ``{profile: platforms}`` runtime platform maps
    ...
    """
    try:
        from clara_cli.profiles import _check_gateway_running, profiles_to_serve
...
# install, minted on first read and persisted under the ROOT Clara home
# (get_default_clara_root()) — NOT the profile-scoped CLARA_HOME — so every
...
def _load_configured_gateway_platforms() -> set[str]:
    ...
    helper is __PROT_0_synchroclara__ by design; ``get_status`` runs it in Starlette's
...
_PROFILE_PLATFORM_STATUS_KEY_RE = re.compile(
    # Profile segment mirrors clara_cli.profiles._PROFILE_ID_RE.  Platform
...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_clara_home() at call time
    ...
    try:
        ...
        # CLARA_HOME (see issue #69143). Plain /api/status keeps the exact
        ...
        # Windows install the first import of clara_cli.gateway blocks the
        ...
        # SPA's StatusPage can show "OAuth gate ON via Claraship" or
        ...
        auth_flows: list[str] = []
        try:
            from clara_cli.dashboard_auth import (
        ...
        # Clara bootstrap-session validity for the NAS health sweep. A hosted
        # agent whose Clara auth dies terminally (invalid_grant / quarantine)
        ...
        clara_session_valid = "unknown"
        try:
            from clara_cli.auth import get_clara_session_validity
            clara_session_valid = get_clara_session_validity()
        except Exception:
            clara_session_valid = "unknown"
        ...
        status = {
            ...
            "can_update_clara": not _dashboard_local_update_managed_externally(),
            "gateway_running": gateway_running,
            "gateway_state": gateway_state,
            ...
            "clara_session_valid": clara_session_valid,
        }
        ...
        try:
            ...
            storage_check = await run_in_threadpool(_probe_state_db, get_clara_home())
        ...
        # clara_session_valid above. Deliberately NOT folded into
        ...
        try:
            ...
            status["memory"] = await run_in_threadpool(
                ...
                profile_dir if profile_dir else get_clara_home(),
            )
        except Exception:
            status["memory"] = {"pressure": "unknown"}

        # Disk-usage rollup (NS-656, same lineage as OOF-2/OOF-107 fleet
        # disk-exhaustion incidents). One statvfs call on CLARA_HOME's
        ...
        try:
            from clara_state import SessionDB as _SDB
            from clara_constants import get_clara_home as _ghh
        ...
        # ``gateway_mode`` are low-sensitivity PRODUCT surface — Clara Cloud
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the clara home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_clara_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def run_curator():
    ...
    try:
        proc = _spawn_clara_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
# ---------------------------------------------------------------------------
# Portal endpoint — Clara Portal auth + Tool Gateway routing status (read-only).
# ---------------------------------------------------------------------------
...
def _get_portal_status_sync():
    ...
    auth: Dict[str, Any] = {}
    try:
        from clara_cli.auth import get_clara_auth_status_local

        # Read-only dashboard endpoint: refresh-free snapshot so polling
        # never performs an OAuth refresh or burns a refresh token.
        auth = get_clara_auth_status_local() or {}
    except Exception:
        auth = {}

    features = []
    try:
        from clara_cli.clara_subscription import get_clara_subscription_features

        feats = get_clara_subscription_features(cfg)
        if feats is not None:
            for feat in feats.items():
                if getattr(feat, "managed_by_clara", False):
                    state = "via Clara Portal"
...
async def run_prompt_size():
    try:
        proc = _spawn_clara_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_clara_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_clara_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *__PROT_1_synchroclara__*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from clara_cli.debug import build_debug_share
...
# streamed to a per-action log file under ``~/.clara/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_clara_home() / "logs"
...
_UPDATE_ACTION_COMPLETED_RE = re.compile(
    r"^=== clara-update completed ([0-9a-f]{32}) ===$"
)
...
def _dashboard_spawn_executable() -> str:
    ...
    runpy.run_module('clara_cli.main', ...)"``) — so ``sys.executable`` is
...
def _spawn_clara_action(
    subcommand: List[str],
    name: str,
    *,
    env_overrides: Optional[Dict[str, str]] = None,
) -> subprocess.Popen:
    """Spawn ``clara <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``clara_cli.main`` module so the action
    ...
    cmd = [_dashboard_spawn_executable(), "-m", "clara_cli.main", *subcommand]

    # The dashboard runs *inside* the gateway process, so os.environ carries
    # _CLARA_GATEWAY=1. Inheriting it makes a spawned `clara gateway restart`
    ...
    action_env = {**os.environ, "CLARA_NONINTERACTIVE": "1"}
    action_env.pop("_CLARA_GATEWAY", None)
    ...
    action_id = (env_overrides or {}).get("CLARA_ACTION_ID")
    if action_id:
...
def _durable_completed_update_action_id(lines: List[str]) -> Optional[str]:
    ...
    for index, line in enumerate(lines):
        if line.startswith("=== clara update started "):
            last_start = index
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join(["clara", *_gateway_subcommand(profile, verb)])
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``clara gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``clara gateway restart`` children race each other on the
    ...
    proc = _spawn_clara_action(subcommand, "gateway-restart")
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``clara gateway restart`` in the background."""
    try:
...
async def gateway_drain(request: Request):
    ...
    active (no ``CLARA_DASHBOARD_DRAIN_SECRET``), the route is NOT a token
...
@app.post("/api/clara/update")
async def update_clara():
    """Kick off ``clara update`` in the background."""
    if _dashboard_local_update_managed_externally():
        message = (
            "Clara updates are managed outside this dashboard in "
            "containerized environments. The built-in local updater is "
            "disabled here."
        )
        _record_completed_action("clara-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "clara-update",
    ...
    if refusal is not None:
        _record_completed_action("clara-update", refusal.message, exit_code=1)
    ...
    existing = _ACTION_PROCS.get("clara-update")
    if existing is not None and existing.poll() is None:
        response = {
            "ok": True,
            "pid": existing.pid,
        ...
        action_id = _ACTION_IDS.get("clara-update")
        if action_id:
    ...
    try:
        proc = _spawn_clara_action(
            ["update"],
            "clara-update",
            env_overrides={"CLARA_ACTION_ID": action_id},
        )
    except Exception as exc:
        _log.exception("Failed to spawn clara update")
...
@app.get("/api/clara/update/check")
async def check_clara_update(force: bool = False):
    """Report whether a Clara update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/clara/update`` actually runs ``clara update``.

    Returns:
        install_method: 'apt' | 'git' | 'docker' | 'nix' | 'nixos' | 'unknown'
        current_version: installed Clara version string
    ...
    if install_method == "apt":
        payload["message"] = (
            "Clara is managed by Termux APT; run `pkg upgrade clara-agent`."
        )
        return payload
    ...
    try:
        from clara_cli.banner import check_for_updates

        if force:
            try:
                (get_clara_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def speak_text(payload: TTSSpeakRequest, profile: Optional[str] = None):
    ...
    configured in ``~/.clara/config.yaml`` under ``tts.``.
    """
...
async def get_action_status(name: str, lines: int = 200):
    ...
    if name == "clara-update":
        ...
        if durable_update_action_id:
            marker = f"=== clara-update completed {durable_update_action_id} ==="
...
def _latest_update_receipt_summary() -> Optional[Dict[str, Any]]:
    ...
    Phase-1 bullet 3 (#91277): the receipt (written by EVERY ``clara
    ...
    """
    try:
        from clara_cli.update_receipt import read_latest_receipt
...
@app.get("/api/clara/update/receipt")
async def get_update_receipt():
    ...
    if not receipt:
        raise HTTPException(
            status_code=404,
            detail="No update receipt found (no `clara update` run recorded).",
        )
...
from clara_cli.web_routers import sessions as _sessions_routes  # noqa: E402

app.include_router(_sessions_routes.list_router)
from clara_cli.web_routers.sessions import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_sessions,
)


from clara_cli.web_routers import profiles as _profiles_routes  # noqa: E402

app.include_router(_profiles_routes.sessions_router)
from clara_cli.web_routers.profiles import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Clara supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def _flat_json_path(provider: ProviderConfigSchema) -> Path:
    return get_clara_home() / provider.name / "config.json"
...
def _install_memory_provider_pip_dependencies(dependencies: List[str]) -> List[Dict[str, Any]]:
    ...
    # under /opt/clara is sealed read-only, and installs must be redirected
    # to the writable durable target on the data volume
    # (CLARA_LAZY_INSTALL_TARGET, e.g. /opt/data/lazy-packages) — the same
...
def _read_memory_provider_existing_values(name: str) -> Dict[str, Any]:
    ...
    for path in (
        clara_home / f"{name}.json",
        clara_home / name / "config.json",
    ):
...
def _save_memory_provider_native_config(name: str, provider: Any, values: Dict[str, Any]) -> None:
    if provider is not None and hasattr(provider, "save_config"):
        try:
            ...
            provider.save_config(values, str(get_clara_home()))
            return
...
# in clara_cli/config.py — listed here for deterministic ordering in the UI.
...
def _dashboard_code_skew_guard() -> Optional[str]:
    ...
    The dashboard and Desktop-owned ``clara serve`` are long-lived; their
    ``sys.modules`` is frozen at boot.  When ``clara update`` (or a manual
...
def _dashboard_skew_restart_hint() -> str:
    ...
    ``clara serve --isolated`` (local or SSH). Hardcoding a systemd unit
    misleads macOS/launchd hosts and Desktop SSH backends, which have no
    ``clara-dashboard`` unit (#97046).
    """
    if os.environ.get("CLARA_SERVE_HEADLESS") == "1":
        return (
            "restart the Desktop-owned backend to load the new code "
            "(use Restart backend in Clara Desktop, or quit and reopen the app)"
        )
    return (
        "restart this Clara process to load the new code "
        "(clara dashboard --port <port>, or the equivalent service restart for this install)"
    )
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `clara model` does so GUI onboarding lands on a
    sensible default instead of blindly taking the first curated entry. For
    Clara this honors the user's free/paid tier: free users get a free model,
    ...
    where free_tier is True/False for Clara and None otherwise. `model` may be
    ...
    if slug == "clara":
        try:
            from clara_cli.models import (
                get_curated_clara_model_ids,
                get_pricing_for_provider,
                check_clara_free_tier,
                clara_policy_allowed_ids,
                partition_clara_models_by_tier,
            ...
            model_ids = get_curated_clara_model_ids()
            pricing = get_pricing_for_provider("clara") or {}
            free_tier = check_clara_free_tier(force_fresh=True)
            ...
            # predicate.
            _policy_allowed = clara_policy_allowed_ids()

            if free_tier:
                model_ids, pricing = union_with_portal_free_recommendations(
                    model_ids, pricing, portal_url
                )
                model_ids = restrict_to_clara_policy(
                    model_ids, _policy_allowed, rescue_empty=True,
                )
                model_ids, _unavailable = partition_clara_models_by_tier(
            ...
            model = pick_silent_default_model(model_ids, provider="clara")
            return {"provider": "clara", "model": model, "free_tier": bool(free_tier)}
        except Exception:
            _log.exception("GET /api/model/recommended-default (clara) failed")
            return {"provider": "clara", "model": "", "free_tier": None}

    # Non-Clara: preferred silent default when the provider's curated list
    ...
    try:
        from clara_cli.inventory import build_models_payload, load_picker_context
        from clara_cli.models import pick_silent_default_model
...
def set_moa_models(body: MoaConfigPayload, profile: Optional[str] = None):
    ...
    try:
        from clara_cli.moa_config import normalize_moa_config, validate_moa_payload
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.clara/config.yaml`` — applies to **new** sessions only.
...
def _apply_model_assignment_sync(
    ...
    """__PROT_2_Synchroclara__ body of POST /api/model/set.
    ...
    if scope == "main":
        ...
        # When switching the main provider to Clara, mirror the CLI's
        # post-model-selection behaviour (clara_cli/main.py
        # prompt_enable_tool_gateway / tools_config apply_clara_managed_defaults):
        # auto-route any *unconfigured* tools through the Clara Tool Gateway.
        # This is purely additive — apply_clara_managed_defaults skips every
        ...
        if provider.strip().lower() == "clara":
            try:
                from clara_cli.clara_subscription import apply_clara_managed_defaults
                from clara_cli.tools_config import _get_platform_tools

                enabled = _get_platform_tools(
                    cfg, "cli", include_default_mcp_servers=False
                )
                changed = apply_clara_managed_defaults(
                    cfg,
                ...
                # Portal lookup hiccups / non-subscriber / non-clara gating
                # must never block saving the model assignment.
                _log.debug("apply_clara_managed_defaults skipped", exc_info=True)

        save_config(cfg)

        # Register a named ``custom_providers`` entry for a custom/local
        # endpoint, mirroring the ``clara model`` custom flow
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from clara_cli.main import _auto_provider_name, _save_custom_provider
        ...
        # a now-unpaid provider (e.g. clara with $0 balance) keeps paying 402s
...
async def update_config(body: ConfigUpdate, profile: Optional[str] = None):
    def _run():
        ...
        # different CLARA_HOME than this process's gateway sessions.
...
def _is_other_profile(profile: Optional[str]) -> bool:
    ...
    try:
        ...
        return True
    return target.resolve() != get_process_clara_home().resolve()
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``clara model`` universe). This is what lets the desktop Keys tab
    ...
    """
    try:
        from clara_cli.provider_catalog import provider_catalog
    except Exception:
        return {}
    ...
    try:
        from clara_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `clara model` provider — would be
        ...
        # Vertex — a `clara model` provider — would be invisible in the desktop
...
def _get_env_vars_sync(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict, *, custom: bool = False) -> dict:
        ...
        return {
            ...
            # CLI `clara model` picker uses (not desktop-only prefix guesses).
...
async def set_env_var(body: EnvVarUpdate, profile: Optional[str] = None):
    def _run():
        with _profile_scope(body.profile or profile):
            ...
            from clara_cli.credential_lifecycle import save_provider_env_credential
...
def list_custom_endpoints(profile: Optional[str] = None):
    ...
    the process-level CLARA_HOME. Mirrors ``/api/config``'s profile scoping.
    """
    try:
        with _config_profile_scope(profile):
...
async def remove_env_var(body: EnvVarDelete, profile: Optional[str] = None):
    def _run():
        with _profile_scope(body.profile or profile):
            ...
            from clara_cli.credential_lifecycle import remove_provider_env_credential
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Clara from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Clara to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Clara from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Clara to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Clara in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Clara through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Clara via Home Assistant.",
    ...
    "email": {
        ...
        "docs_url": "https://agent.claraship.com/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Clara to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Clara inside Feishu / Lark.",
    ...
    "google_chat": {
        ...
        "docs_url": "https://agent.claraship.com/docs/user-guide/messaging/google_chat",
    },
    "wecom": {
    ...
    "weixin": {
        ...
        "docs_url": "https://agent.claraship.com/docs/user-guide/messaging/weixin/",
        "env_vars": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN", "WEIXIN_BASE_URL"),
        "required_env": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN"),
    },
    "bluebubbles": {
        "name": "BlueBubbles (iMessage)",
        "description": "Use Clara through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Clara to a QQ Bot from the QQ Open Platform.",
    ...
    "teams": {
        ...
        "docs_url": "https://agent.claraship.com/docs/user-guide/messaging/teams",
    },
    ...
    "irc": {
        ...
        "docs_url": "https://agent.claraship.com/docs/user-guide/messaging/irc",
    },
    "line": {
        "description": "Use Clara from LINE via the LINE Messaging API webhook.",
        "docs_url": "https://agent.claraship.com/docs/user-guide/messaging/line",
    },
    "ntfy": {
        "description": "Chat with Clara over ntfy push topics (ntfy.sh or self-hosted).",
        "docs_url": "https://agent.claraship.com/docs/user-guide/messaging/ntfy",
    },
    "photon": {
        "description": "Use Clara through iMessage via Photon's managed Spectrum platform.",
        "docs_url": "https://agent.claraship.com/docs/user-guide/messaging/photon",
    },
    "raft": {
        "description": "Join a Raft workspace as an external agent.",
        "docs_url": "https://agent.claraship.com/docs/user-guide/messaging/raft",
    },
    "simplex": {
        "description": "Talk to Clara over SimpleX Chat via a local simplex-chat daemon.",
        "docs_url": "https://agent.claraship.com/docs/user-guide/messaging/simplex",
    },
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Clara to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
    ...
    "webhook": {
        ...
        "docs_url": "https://agent.claraship.com/docs/user-guide/messaging/webhooks/",
    ...
    "msgraph_webhook": {
        ...
        "docs_url": "https://agent.claraship.com/docs/user-guide/messaging/msgraph-webhook",
        "required_env": (),
    },
    "whatsapp_cloud": {
        "name": "WhatsApp Cloud API",
        "description": "Use Clara via Meta's hosted WhatsApp Cloud API (no local bridge).",
        "docs_url": "https://agent.claraship.com/docs/user-guide/messaging/whatsapp-cloud",
    },
    "relay": {
        "name": "Relay (experimental)",
        "description": "Generic relay adapter fronted by the Clara Relay connector.",
        "docs_url": "",
        "required_env": (),
    },
}
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in clara gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in clara gateway setup",
...
# clara_cli/setup_hidden_env.py. Shared with the `clara setup gateway`
# wizard so the surfaces ask for the same things.
from clara_cli.setup_hidden_env import (  # noqa: E402
...
def _messaging_platform_payload(
    ...
    # CLARA_HOME contextvar override (#56986 / #69143), so the profile's
...
def _whatsapp_session_path() -> Path:
    ...
    return get_clara_dir("platforms/whatsapp/session", "whatsapp/session")
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.agent.claraship.com"
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Clara Agent").strip() or "Clara Agent"
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Clara appears
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # load_env() honors the CLARA_HOME contextvar override; the gateway
...
def _multiplex_port_binding_conflict(
    ...
    *enabling* is blocked; disabling/clearing stays allowed so users can
    ...
    if not requested or requested.lower() == "current":
        ...
        # The dashboard's own profile. "custom" (an unrecognized CLARA_HOME)
...
# the canonical ``clara auth add <provider>`` command so the dashboard can
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.clara/.anthropic_oauth.json`` — Clara-managed terminal PKCE
    ...
    2. ``ANTHROPIC_API_KEY`` → ``ANTHROPIC_TOKEN`` → ``CLAUDE_CODE_OAUTH_TOKEN``
       ...
       env during ``load_clara_dotenv()``, so the same check covers them)
    ...
    try:
        ...
        read_clara_oauth_credentials = None  # type: ignore
        _get_clara_oauth_file = None  # type: ignore

    clara_creds = None
    if read_clara_oauth_credentials:
        try:
            clara_creds = read_clara_oauth_credentials()
        except Exception:
            clara_creds = None
    if clara_creds and clara_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "clara_pkce",
            "source_label": f"Clara PKCE ({_get_clara_oauth_file() if _get_clara_oauth_file else None})",
            "token_preview": _truncate_token(clara_creds.get("accessToken")),
            "expires_at": clara_creds.get("expiresAt"),
            "has_refresh_token": bool(clara_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Clara even
    when they also have a separate Clara-managed PKCE login.
    """
    try:
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    hold its session in an OS keychain Clara can't read, so the unverified
    state is presented as "managed by the Copilot CLI" — never as signed out.
    """
    try:
        from clara_cli.auth import get_external_process_provider_status
...
def _external_process_cli_command(provider_id: str, default: str) -> str:
    ...
    Clara at a custom binary (``CLARA_COPILOT_ACP_COMMAND`` /
    ``COPILOT_CLI_PATH``) would otherwise be told to run a command that isn't
    the one Clara spawns. Non-external-process providers get ``default`` back
    untouched.
    """
    try:
        from clara_cli.auth import PROVIDER_REGISTRY, get_external_process_provider_status
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "docs_url": "https://portal.claraship.com",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "clara auth add openai-codex",
    ...
    },
    {
        ...
        # as Clara's device-code flow; the PKCE bit is a security
        # extension that doesn't change the operator experience.
        "flow": "device_code",
        "cli_command": "clara auth add minimax-oauth",
    ...
    },
    {
        ...
        "docs_url": "https://agent.claraship.com/docs/guides/xai-grok-oauth",
    ...
    # server). Clara previously reimplemented that subscription-OAuth PKCE
    ...
    # credentials. Login still works via the terminal (`clara auth add
    # anthropic`, unaffected by this change) or a plain API key below.
    {
        "id": "anthropic",
        "name": "Anthropic API Key",
        "flow": "external",
        "cli_command": "clara auth add anthropic",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from clara_cli import auth as hauth
        if provider_id == "clara":
            # Read-only accounts-tab card: refresh-free snapshot so listing
            # providers never performs an OAuth refresh.
            raw = hauth.get_clara_auth_status_local()
            return {
                "logged_in": bool(raw.get("logged_in")),
                "source": "clara_portal",
                "source_label": raw.get("portal_base_url") or "Clara Portal",
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside Clara, so the disconnect
    ...
    user sees exactly what executes, and Clara then stops resolving the token.
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    # catalog entry) but, unlike other external providers, Clara still OWNS
    # the credential it can show here: the Clara-managed PKCE file
    # (~/.clara/.anthropic_oauth.json) and its credential-pool entry, both
    # written by `clara auth add anthropic` in the terminal. Those are ours
    ...
    if provider.get("flow") == "external" and provider.get("id") != "anthropic":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside Clara — run the disconnect command to remove it."
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``clara model`` universe) — so any OAuth/external provider added as a
    ...
    #    in lockstep with the `clara model` universe (zero-edit for new plugins).
    try:
        from clara_cli.provider_catalog import provider_catalog
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"clara auth add {d.slug}",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("clara_pkce", "claude_code", ...)
    ...
    sync with the `clara model` picker; _OAUTH_OVERRIDES supplies per-provider
...
async def disconnect_oauth_provider(
    ...
    def _run():
        with _profile_scope(profile):
            ...
            # Anthropic clears only the Clara-managed PKCE file and auth-store entry.
            ...
            if provider_id == "anthropic":
                cleared = False
                try:
                    from agent.anthropic_adapter import _get_clara_oauth_file
                    oauth_file = _get_clara_oauth_file()
                ...
                try:
                    from clara_cli.auth import clear_provider_auth
            ...
            try:
                from clara_cli.auth import clear_provider_auth, invalidate_clara_auth_status_cache
                cleared = clear_provider_auth(provider_id)
                if provider_id == "clara":
                    invalidate_clara_auth_status_cache()
...
# points at `clara auth add anthropic` (terminal PKCE, unaffected) instead.
#
#   Device code (Clara, OpenAI Codex):
#     1. POST /api/providers/oauth/{clara|openai-codex}/start
...
async def _start_device_code_flow(
    ...
    """Initiate a device-code flow (Clara, OpenAI Codex, MiniMax, or xAI).
    ...
    if provider_id == "clara":
        ...
        import httpx
        pconfig = PROVIDER_REGISTRY["clara"]
        portal_base_url = (
            os.getenv("CLARA_PORTAL_BASE_URL")
            or os.getenv("CLARA_PORTAL_BASE_URL")
            or pconfig.portal_base_url
        ).rstrip("/")
        client_id = pconfig.client_id
        scope = pconfig.scope

        def _do_clara_device_request():
            with httpx.Client(
        ...
        device_data, effective_scope = await asyncio.get_running_loop().run_in_executor(
            None, _do_clara_device_request
        )
        sid, sess = _new_oauth_session("clara", "device_code", profile=profile)
        ...
        threading.Thread(
            target=_clara_poller, args=(sid,), daemon=True, name=f"oauth-poll-{sid[:6]}"
        ).start()
        return {
            "session_id": sid,
            "flow": "device_code",
    ...
    if provider_id == "minimax-oauth":
        ...
        # operator's perspective it's identical to Clara's device-code
    ...
    if provider_id == "xai-oauth":
        from clara_cli.auth import _xai_oauth_request_device_code
        import httpx
...
def _clara_poller(session_id: str) -> None:
    """Background poller that drives a Clara device-code flow to completion."""
    from clara_cli.auth import (
        _poll_for_token,
        refresh_clara_oauth_from_state,
    )
    from datetime import datetime, timezone
    import httpx
    with _oauth_sessions_lock:
    ...
    try:
        ...
        # Same post-processing as _clara_device_code_login (validate/refresh JWT)
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            full_state = refresh_clara_oauth_from_state(
                auth_state,
                timeout_seconds=15.0,
                force_refresh=False,
            )
            from clara_cli.auth import persist_clara_credentials
            persist_clara_credentials(full_state)
        with _oauth_sessions_lock:
            sess["status"] = "approved"
        _log.info("oauth/device: clara login completed (session=%s)", session_id)
    except Exception as e:
        _log.warning("clara device-code poll failed (session=%s): %s", session_id, e)
        with _oauth_sessions_lock:
            sess["status"] = "error"
...
def _minimax_poller(session_id: str) -> None:
    ...
    Mirrors `_clara_poller` but calls the MiniMax-specific token endpoint,
    which uses a PKCE-style ``code_verifier`` + ``user_code`` rather than
    the ``device_code`` field used by Clara. On success, builds the same
    auth_state dict that ``_minimax_oauth_login`` (the CLI flow) builds
    and persists via ``_minimax_save_auth_state`` — so the dashboard
    path leaves the system in the same state as
    ``clara auth add minimax-oauth``.
    """
    from clara_cli.auth import (
        _minimax_poll_token,
...
def _xai_device_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            # Mirror `clara auth add xai-oauth`: first credential may become
            ...
            # prior ``clara auth remove xai-oauth`` (mirrors auth_add_command
            # and the ``clara model`` re-login path in _login_xai_oauth).
...
def _codex_device_code_start_error(resp: Any) -> str:
    ...
    if "device" in lower and ("authori" in lower or "enable" in lower):
        message = (
            ...
            "account may need device-code authorization enabled before Clara "
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from clara_cli.auth import _save_codex_tokens
...
async def poll_oauth_session(
    ...
    Shared by the device-code flows (Clara, OpenAI Codex, MiniMax, xAI).
...
def _session_db_read_probe_statements() -> tuple:
    ...
    after `clara update` until the first message forced a writable open.
    """
    from clara_state_schema import schema_read_probe_statements
...
def _open_session_db_at_path(db_path: Path, *, read_only: bool):
    ...
    from clara_state import SessionDB, is_malformed_schema_error
...
def _maybe_auto_archive_for_profile(profile: Optional[str]) -> None:
    ...
    The Desktop backend is spawned as ``clara serve`` — it runs neither the
...
def _prune_sessions(body: SessionPrune):
    """Delete ended sessions matching filters (mirrors `clara sessions prune`)."""
    has_window = (
    ...
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_clara_home()
...
async def get_logs(
    ...
    from clara_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_clara_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from clara_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_default_profile() -> str:
    ...
    A desktop pool backend runs one process per profile (CLARA_HOME already
    ...
    fallback would write a non-default profile's job into ``~/.clara``.
    Resolve the process's own profile instead. ``custom`` (an unrecognized
    CLARA_HOME outside the profiles tree) has no profile-dir equivalent, so
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, CLARA_HOME)."""
    from clara_cli import profiles as profiles_mod
...
from clara_cli.web_routers import cron as _cron_routes  # noqa: E402

app.include_router(_cron_routes.router)
from clara_cli.web_routers.cron import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_cron_jobs,
    get_cron_job,
    list_cron_job_runs,
    create_cron_job,
...
def _gateway_fire_endpoint(profile: str, home: Path) -> str:
    ...
    if multiplex and profile != "default":
        ...
        listener_profile, listener_home = "default", get_default_clara_root()
        _log.info(
    ...
    try:
        ...
        # yaml.safe_load of config.yaml (tests/clara_cli/
        # test_config_read_guard.py). The CLARA_HOME override scopes
        ...
        token = set_clara_home_override(str(listener_home))
        try:
...
def _gateway_intentionally_stopped(profile: Optional[str]) -> bool:
    ...
    commands (``clara gateway stop`` persists ``"stopped"``; start and
...
#
# Wraps the same config data layer the CLI uses (clara_cli.mcp_config), so
# servers managed here show up under `clara mcp list` and vice versa.  Secrets
...
def _normalize_mcp_server_create(
    ...
    """
    from clara_cli.mcp_config import (
        _bearer_auth_headers,
        _strip_bearer_prefix,
    )
    from clara_cli.mcp_security import validate_mcp_server_entry
...
from clara_cli.web_routers import mcp as _mcp_routes  # noqa: E402

app.include_router(_mcp_routes.router)
from clara_cli.web_routers.mcp import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_mcp_servers,
    add_mcp_server,
    replace_mcp_servers,
    remove_mcp_server,
...
def _mcp_oauth_callback_url(request: Request, server_name: str) -> str:
    ...
    from clara_cli.dashboard_auth.prefix import prefix_from_request, resolve_public_url
...
def _mcp_oauth_transaction(flow) -> threading.Lock:
    key = (flow.clara_home, flow.server_name)
...
def _run_dashboard_mcp_oauth(flow, cfg: dict) -> None:
    ...
    try:
        ...
        from tools.mcp_oauth import ClaraTokenStorage, force_interactive_oauth
        from tools.mcp_oauth_manager import get_manager

        home_token = set_clara_home_override(flow.clara_home)
        secret_token = set_secret_scope(build_profile_secret_scope(Path(flow.clara_home)))
        try:
            transaction = _mcp_oauth_transaction(flow)
            with transaction, force_interactive_oauth(), dashboard_oauth_flow(flow):
                manager = get_manager()
                storage = ClaraTokenStorage(flow.server_name)
                backup = storage.snapshot()
                previous_entry = None
                try:
                    previous_entry = manager.remove(
                        flow.server_name,
                        clara_home=flow.clara_home,
                    )
...
#
# Wraps the same JSON store the CLI uses (clara_cli.webhook); the webhook
...
async def list_webhooks():
    import clara_cli.webhook as wh
...
# spawn the real `clara gateway <verb>` so behaviour matches the CLI exactly.
...
async def start_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_clara_action(_gateway_subcommand(profile, "start"), "gateway-start")
    except HTTPException:
        raise
    except Exception as exc:
...
async def stop_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_clara_action(_gateway_subcommand(profile, "stop"), "gateway-stop")
    except HTTPException:
        raise
    except Exception as exc:
...
async def list_credential_pool():
    ...
    from clara_cli.auth import read_credential_pool
...
async def add_credential_pool_entry(body: CredentialPoolAdd):
    ...
    try:
        ...
        # (via DELETE below or `clara auth remove`) can seed again.
        # Mirrors the `clara auth add` behaviour in auth_commands.py.
...
async def remove_credential_pool_entry(provider: str, index: int):
    ...
    registry the CLI ``clara auth remove`` uses: each source cleans up its
    ...
    from clara_cli.auth import suppress_credential_source
...
async def run_doctor():
    try:
        proc = _spawn_clara_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_clara_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
def _dashboard_backup_dir() -> Path:
    return get_clara_home() / "backups"


def _new_dashboard_backup_path() -> Path:
    stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
    return _dashboard_backup_dir() / f"clara-backup-{stamp}-{secrets.token_hex(4)}.zip"
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_clara_action(args, "backup")
    except Exception as exc:
...
async def run_import(body: ImportRequest):
    ...
    if body.force:
        args.append("--force")
    try:
        proc = _spawn_clara_action(args, "import")
    except Exception as exc:
...
async def list_hooks():
    ...
    def _run():
        ...
        try:
            from clara_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    # Checkpoints live under <clara_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_clara_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def prune_checkpoints():
    try:
        proc = _spawn_clara_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``clara`` subprocess, and
...
def _hub_action_name(verb: str, key: str) -> str:
    ...
    ``_spawn_clara_action`` tracks one process/log per name, so a shared
...
from clara_cli.web_routers import skills as _skills_routes  # noqa: E402

app.include_router(_skills_routes.hub_router)
from clara_cli.web_routers.skills import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    install_skill_hub,
    uninstall_skill_hub,
    update_skills_hub,
...
# Human-readable labels for each hub source id (matches `clara skills search`
# provenance).  Keep in sync with create_source_router()'s source list.
_SKILL_HUB_SOURCE_LABELS = {
    "official": "Official (Clara)",
    "clara-index": "Clara Index",
    "skills-sh": "skills.sh",
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_clara_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "clara setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local CLARA_HOME override so the write lands in the target
    ...
    token = set_clara_home_override(str(profile_dir))
    try:
        provider, model = _normalize_main_model_assignment(provider, model)
        cfg = load_config()
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local CLARA_HOME override (same mechanism as
    ...
    """
    from clara_constants import set_clara_home_override, reset_clara_home_override
    from clara_cli.mcp_config import _save_bearer_auth_token
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the CLARA_HOME override. Returns the
    number of skills newly disabled.
    """
    from clara_constants import set_clara_home_override, reset_clara_home_override
    from clara_cli.skills_config import get_disabled_skills, save_disabled_skills
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_clara_home()`` at call
       time — the context-local override from ``set_clara_home_override``
    ...
    retargeted to the *current* ``get_clara_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a CLARA_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
def _config_profile_scope(profile: Optional[str]):
    ...
    ``set_clara_home_override`` contextvar — it does NOT swap the
    ...
    which is all endpoints that resolve ``get_clara_home()`` at call time
...
from clara_cli.web_routers import tools as _tools_routes  # noqa: E402

app.include_router(_tools_routes.router)
from clara_cli.web_routers.tools import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_toolsets,
    toggle_toolset,
    get_toolset_config,
    get_toolset_models,
...
def _resolve_toolset_model_plugin(ts_key: str, provider_row: dict) -> Optional[str]:
    ...
    the managed "Clara Subscription" image row instead carries the legacy
...
# (which attach to cua-driver's OWN identity, com.trycua.driver — not Clara,
...
#
# The endpoint spawns the same ``clara --tui`` binary the CLI uses, behind
...
if sys.platform.startswith("win"):
    try:
        from clara_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from clara_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
from clara_cli.pty_session import PtySessionRegistry, RegistryFull, run_reaper  # noqa: E402
...
_GATEWAY_WS_PROTOCOL = "clara-gateway-v1"
_GATEWAY_WS_TICKET_PROTOCOL_PREFIX = "clara-gateway-ticket."
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from clara_cli.dashboard_auth.audit import AuditEvent, audit_log
        from clara_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
        ...
        try:
            ...
            if protocol_ticket:
                ...
                ws._clara_ws_subprotocol = _GATEWAY_WS_PROTOCOL
...
def _resolve_chat_argv(
    ...
    Default: whatever ``clara --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``CLARA_TUI_RESUME`` env var —
    matching what ``clara_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``CLARA_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``CLARA_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).

    `active_session_file` (when set) is forwarded as
    ``CLARA_TUI_ACTIVE_SESSION_FILE``. The TUI writes the current session id
    ...
    ``CLARA_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_clara_home()`` from that env var at its own import, so the child
    binds the profile's config, skills, memory, and state.db from the start
    — the same propagation ``clara -p <name>`` performs. The in-process
    ``CLARA_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    """
    from clara_cli.main import PROJECT_ROOT, _apply_tui_python_env, _make_tui_argv
    ...
    # Clara TUI child: build via the single spawn-env factory (profile-home
    # contract applied; secrets kept — the spawned agent needs provider creds).
    # An explicit profile scope still overrides CLARA_HOME before config is
    # bridged into the child environment.
    from tools.environments.local import build_subprocess_env
    env = build_subprocess_env(scrub_secrets=False, inherit_profile_home=True)
    if profile_dir is not None:
        env["CLARA_HOME"] = str(profile_dir)
    try:
        from clara_cli.config import (
    ...
    env.setdefault("CLARA_TUI_DISABLE_MOUSE", "1")
    env.setdefault("CLARA_TUI_INLINE", "1")
    ...
    if resume:
        ...
        env["CLARA_TUI_RESUME"] = resume

    if sidecar_url:
        env["CLARA_TUI_SIDECAR_URL"] = sidecar_url
    ...
    # inherits the profile CLARA_HOME set above.
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            env["CLARA_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _resolve_client_ws_host() -> Optional[str]:
    ...
    1. Explicit ``CLARA_DASHBOARD_WS_HOST`` env var — wins always. Operators
    ...
    """
    explicit = os.environ.get("CLARA_DASHBOARD_WS_HOST", "").strip()
    if explicit:
        return explicit
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from clara_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
async def _resolve_chat_argv_async(
    ...
    ``_make_tui_argv``.  Keep that __PROT_3_synchroclara__ work off the WebSocket event
...
def _active_session_file_for_channel(app: "FastAPI", channel: str) -> Path:
    ...
    fd, raw_path = tempfile.mkstemp(prefix="clara-pty-active-", suffix=".json")
    os.close(fd)
    path = Path(raw_path)
    files[channel] = path
    return path
...
# ---------------------------------------------------------------------------
# /api/console — safe Clara Console command WebSocket.
#
# Unlike /api/pty, this endpoint never spawns a PTY, shell, or full Clara CLI
        ...
        with _console_executor_lock:
            if _console_executor is None:
                _console_executor = concurrent.futures.ThreadPoolExecutor(
                    ...
                    thread_name_prefix="clara-console",
                )
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from clara_cli.console_engine import ClaraConsoleEngine

        engine = ClaraConsoleEngine(output_limit=_CONSOLE_OUTPUT_LIMIT)
            ...
            if command_id == command_generation:
                ...
                await _console_send(
                    ...
                    send_lock,
                    {
                        ...
                        "id": command_id,
                        "message": (
                            "Command timed out. Clara Console returned to the prompt."
                        ),
                        "command": line,
                    },
                )
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Clara inside WSL2 to use the dashboard's /chat "
    ...
    if raw_resume and env:
        registry_resume = env.get("CLARA_TUI_RESUME") or raw_resume
...
async def gateway_ws(ws: WebSocket) -> None:
    ...
    await handle_ws(
        ws,
        auth_identity=getattr(ws, "_clara_auth_identity", None),
        subprotocol=getattr(ws, "_clara_ws_subprotocol", None),
    )
...
# CLARA_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`clara_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from clara_cli.dashboard_auth.prefix import normalise_prefix
...
def _render_active_theme_bootstrap_css() -> str:
    ...
    default Clara Teal canvas before the SPA flips the configured user
    theme into place.
    ...
    try:
        ...
        for theme in _discover_user_themes():
            ...
            return (
                '<style id="clara-theme-bootstrap">'
                ":root{"
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/clara/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /clara`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__CLARA_BASE_PATH__`` honour that prefix
    without rebuilding the bundle.
    """
    # `clara serve` is the headless backend: it must NEVER serve the browser
    ...
    _headless = os.environ.get("CLARA_SERVE_HEADLESS") == "1"
    if _headless:
        _msg = (
            "Headless backend (clara serve): web UI disabled — use "
            "`clara dashboard` for the browser UI."
        )

        @application.get("/{full_path:path}")
        async def no_frontend(full_path: str):
            # Desktop token handshake (#94227): the Electron shell boots by
            # fetching `/` and extracting ``window.__CLARA_SESSION_TOKEN__``
            ...
            # `clara update` replaced the backend) had no way to adopt the
            ...
            if full_path == "" and not gated:
                ...
                return HTMLResponse(
                    ...
                    f"window.__CLARA_SESSION_TOKEN__={token_js};"
                    "window.__CLARA_AUTH_REQUIRED__=false;"
                    "</script></head><body>"
                    "Headless backend (clara serve): web UI disabled — use "
                    "`clara dashboard` for the browser UI."
                    "</body></html>",
    ...
    # (#82614): a long-lived `clara dashboard --skip-build` process that
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/clara``)
        ...
        ``__CLARA_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__CLARA_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__CLARA_BASE_PATH__="{prefix}";'
                f"window.__CLARA_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__CLARA_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__CLARA_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__CLARA_BASE_PATH__="{prefix}";'
                f"window.__CLARA_AUTH_REQUIRED__={gated_js};"
                f"</script>"
        ...
        # (``CLARA_HOME/dashboard-themes/<name>.yaml``), inject a minimal
        # critical-CSS block so the first paint uses the target palette.
        # Without this the SPA paints the default Clara Teal canvas, then
    ...
    # under ``/clara`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Clara backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Clara Teal",         "description": "Classic dark teal — the canonical Clara look"},
    {"name": "default-large", "label": "Clara Teal (Large)", "description": "Clara Teal with bigger fonts and roomier spacing"},
    {"name": "clara-blue",     "label": "Clara Blue",           "description": "Light mode — vivid Clara-blue accents on cream canvas"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.clara/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.clara/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.

    Uses the dashboard process launch home, not ``get_clara_home()``, so a
    transient profile override from embedded chat does not hide themes that
    live under the server's own ``CLARA_HOME``.
    """
    themes_dir = get_process_clara_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.clara/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as clara_cli.plugins):
    1. User plugins:    ~/.clara/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.clara/plugins/  (only if CLARA_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from clara_cli.plugins import get_bundled_plugins_dir
    ...
    # CLARA_HOME override (e.g. embedded /chat under --open-profile).
    #
    # #87197: when the process itself is profile-scoped (``--profile <name>``
    # sets ``CLARA_HOME=<root>/profiles/<name>``), the launch home is the
    # profile directory, which has no ``plugins/`` — user plugins are
    # installed in the clara root (``~/.clara/plugins``). Scan the default
    # root as well (``get_default_clara_root()`` unwraps
    # ``<root>/profiles/<name>`` → ``<root>`` and returns a custom
    # ``CLARA_HOME`` unchanged when it *is* the root), mirroring how
    # ``clara_cli.plugins`` resolves plugin install locations. The
    # ``seen_names`` dedupe below keeps profile-local plugins (if any)
    # authoritative over same-named root plugins.
    from clara_constants import get_default_clara_root

    user_plugin_roots = [get_process_clara_home() / "plugins"]
    root_plugins = get_default_clara_root() / "plugins"
    ...
    # ``clara_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("CLARA_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".clara" / "plugins", "project"))
...
async def get_dashboard_plugins():
    ...
    def _run():
        ...
        try:
            from clara_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
            enabled_set = _get_enabled_set()
            disabled_set = _get_disabled_set()
        except Exception:
            enabled_set = set()
...
def _merged_plugins_hub(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    those can trigger imports, auth/network checks, and other __PROT_4_synchroclara__ work
    ...
    plugins_root_resolved = (get_clara_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if cached_result is False:
                        auth_required = True
                        auth_command = f"clara auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from clara_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from clara_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from clara_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from clara_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.clara/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"clara_dashboard_plugin_{plugin['name']}"
...
from clara_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    ...
    so Electron passes ``CLARA_DESKTOP_READY_FILE`` and waits for this JSON.
    Normal CLI/dashboard launches still use the stdout READY line below.
    """
    target = os.environ.get("CLARA_DESKTOP_READY_FILE")
    if not target:
        return
...
def _is_serve_orphaned(
    ...
    *,
    ...
    ``CLARA_PARENT_PID`` is the Electron Desktop PID, not necessarily this
    Python process's immediate PPID. On Windows the venv ``clara.exe`` launcher
...
def _start_parent_death_watchdog() -> None:
    ...
    Desktop versions that provide only ``CLARA_PARENT_PID`` retain PID-only
    tracking.
    """
    raw_pid = os.environ.get("CLARA_PARENT_PID")
    start_marker = os.environ.get("CLARA_PARENT_START_MARKER")
    nonce = os.environ.get("CLARA_PARENT_NONCE")
    ...
    try:
        poll = max(0.5, float(os.environ.get("CLARA_SERVE_WATCHDOG_POLL_S", "2.0")))
    except (TypeError, ValueError):
        poll = 2.0
...
# spawn (and any script wrapping ``clara serve``) cannot tell "port occupied"
...
# CLARA_BACKEND_READY sentinel (which is NOT changed by any of this).
...
def _write_machine_sentinel_line(line: str) -> None:
    ...
    CLARA_DESKTOP_READY_FILE channel) for port discovery instead. Never
...
def _report_port_in_use(host: str, port: int) -> None:
    ...
    print(
        ...
        "'clara serve' / 'clara dashboard' backend or the Clara gateway. "
...
def start_server(
    ...
    build and no SPA mount (mount_spa() honours ``CLARA_SERVE_HEADLESS``), so
    ...
    try:
        ...
        _log.debug("Clara auth keepalive did not start: %s", exc)
    ...
    # the clara-0day MCP-persistence campaign abused unauthenticated public
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # to register (e.g. missing CLARA_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with Clara Agent exposes a
            # module-level ``LAST_SKIP_REASON`` string for this purpose;
            # without it the operator would only see "no providers" which
            # is misleading when the provider IS installed but unconfigured.
            skip_reasons: list[str] = []
            try:
                from plugins.dashboard_auth import clara as _clara_plugin

                if _clara_plugin.LAST_SKIP_REASON:
                    skip_reasons.append(
                        f"  • clara: {_clara_plugin.LAST_SKIP_REASON}"
                    )
            except Exception:
                pass
            ...
            if host in _LOOPBACK_HOST_VALUES:
                _public_url_for_msg = ""
                try:
                    from clara_cli.dashboard_auth.prefix import (
                ...
                _local_only_hint = (
                    ...
                    "(and unset CLARA_DASHBOARD_PUBLIC_URL) to restore the "
            ...
            _fix_hint = (
                ...
                "  • OAuth: run `clara dashboard register` (Clara Portal) or "
            ...
            try:
                ...
                from clara_cli.plugins_cmd import _BASIC_AUTH_PLUGIN_KEYS
                ...
                if _has_creds and (set(_disabled) & _BASIC_AUTH_PLUGIN_KEYS):
                    _fix_hint = (
                        ...
                        "`clara plugins enable basic`), then restart the "
                        "dashboard.\n\n"
                    ) + _fix_hint
            except Exception:
                pass
    ...
    # CLARA_DASHBOARD_READY, open the browser, *then* serve.
    #
    ...
    # as agent turns, and a single __PROT_5_synchroclara__ GIL-holding call on a worker
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            if os.getenv("CLARA_DESKTOP") == "1":
                try:
                    from clara_cli.dashboard_procs import (
            ...
            try:
                from clara_cli.process_identity import reap_orphaned_mcp_helpers
            ...
            # for standalone `clara serve` (no CLARA_PARENT_PID env).
            ...
            # is what lets `clara update` relaunch a manually-started serve
            ...
            ready_token = "CLARA_BACKEND_READY" if headless else "CLARA_DASHBOARD_READY"
            ...
            if headless:
                ...
                print(f"  Clara backend listening on {host}:{actual_port}", flush=True)
            else:
                print(f"  Clara Web UI → http://{host}:{actual_port}")
            ...
            if start_mcp_discovery_after_bind:
                ...
                try:
                    from clara_cli.mcp_startup import defer_background_mcp_discovery
