"""Background keepalive for long-lived Clara Portal sessions."""
...
from clara_cli.auth import (
    ...
    CLARA_INVOKE_JWT_MIN_TTL_SECONDS,
    AuthError,
...
#    Clara credential lifetimes are not fixed: this varies by account, and
...
CLARA_AUTH_KEEPALIVE_INTERVAL_SECONDS = 15 * 60
CLARA_AUTH_KEEPALIVE_MIN_INTERVAL_SECONDS = 60
# Ticks per credential lifetime. Four keeps the refresh comfortably ahead of
# expiry without making the thread chatty.
CLARA_AUTH_KEEPALIVE_TICKS_PER_LIFETIME = 4
CLARA_AUTH_KEEPALIVE_INITIAL_DELAY_SECONDS = 60
CLARA_AUTH_KEEPALIVE_INTERVAL_CONFIG_KEY = "keepalive_interval_seconds"
...
def _timeout_seconds(value: Optional[float]) -> float:
    if value is not None:
        return float(value)
    try:
        return float(os.getenv("CLARA_CLARA_TIMEOUT_SECONDS", "15"))
    except (TypeError, ValueError):
        return 15.0


def _clara_config() -> dict:
    """Return the ``clara:`` section of config.yaml, or {} on any failure.
...
def _interval_seconds(value: Optional[int]) -> int:
    ...
    Explicit argument wins, then ``clara.keepalive_interval_seconds`` in
    ...
    if value is not None:
        try:
            ...
            return CLARA_AUTH_KEEPALIVE_INTERVAL_SECONDS

    raw = _clara_config().get(CLARA_AUTH_KEEPALIVE_INTERVAL_CONFIG_KEY)
    ...
    try:
        ...
        logger.warning(
            "Ignoring invalid clara.%s=%r; using %ds",
            CLARA_AUTH_KEEPALIVE_INTERVAL_CONFIG_KEY,
            raw,
            CLARA_AUTH_KEEPALIVE_INTERVAL_SECONDS,
        )
        return CLARA_AUTH_KEEPALIVE_INTERVAL_SECONDS


def _observed_lifetime_seconds() -> Optional[int]:
    """Lifetime the server issued for the current Clara credentials, in seconds.
...
def _tick_seconds(configured_interval: int, lifetime: Optional[int]) -> int:
    ...
    derived = lifetime // CLARA_AUTH_KEEPALIVE_TICKS_PER_LIFETIME
    return max(
        CLARA_AUTH_KEEPALIVE_MIN_INTERVAL_SECONDS,
...
def _refresh_selected_pool_entry(
    *,
    ...
    """Refresh the current Clara credential pool entry when it is stale.

    Returns True when a pool entry exists and is usable/refreshed, False when a
    pool exists but no entry can be used, and None when no Clara pool exists.
    """
    try:
        ...
        logger.debug("Clara auth keepalive: credential pool unavailable: %s", exc)
        return None
    ...
    try:
        ...
        logger.debug("Clara auth keepalive: credential pool selection failed: %s", exc)
        return False
    ...
    if access_expiring or not key_usable:
        ...
        if refreshed is None:
            return False
        logger.debug("Clara auth keepalive: refreshed credential pool entry")
        return True

    return True


def refresh_clara_auth_keepalive_once(
    *,
    min_key_ttl_seconds: int = CLARA_INVOKE_JWT_MIN_TTL_SECONDS,
    min_access_ttl_seconds: Optional[int] = None,
    timeout_seconds: Optional[float] = None,
) -> bool:
    """Refresh Clara auth once if credentials are configured."""
    ...
    try:
        ...
        logger.debug("Clara auth keepalive: refreshed singleton auth state")
        return True
    except AuthError as exc:
        if exc.relogin_required:
            logger.info("Clara auth keepalive requires re-login: %s", exc)
        else:
            logger.debug("Clara auth keepalive failed: %s", exc)
        return False
    except Exception as exc:
        logger.debug("Clara auth keepalive failed: %s", exc)
        return False
...
def _keepalive_loop(
    ...
    *,
    ...
    while not stop_event.is_set():
        ...
        refresh_clara_auth_keepalive_once(
...
def start_clara_auth_keepalive(
    *,
    interval_seconds: Optional[int] = None,
    initial_delay_seconds: int = CLARA_AUTH_KEEPALIVE_INITIAL_DELAY_SECONDS,
    min_key_ttl_seconds: int = CLARA_INVOKE_JWT_MIN_TTL_SECONDS,
    timeout_seconds: Optional[float] = None,
) -> Optional[threading.Thread]:
    """Start the process-wide Clara auth keepalive thread."""
    ...
    with _keepalive_lock:
        ...
        _keepalive_thread = threading.Thread(
            ...
            daemon=True,
            name="clara-auth-keepalive",
        )
        _keepalive_thread.start()
        logger.debug("Clara auth keepalive started")
        return _keepalive_thread


def stop_clara_auth_keepalive(timeout: float = 5.0) -> None:
