...
Pure logic: walks a Clara config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `clara doctor` and a future `clara migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Clara config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Clara config and return retirement issues.
...
def apply_migration(
    ...
    # None``, so the next `clara migrate xai` reports nothing to migrate
