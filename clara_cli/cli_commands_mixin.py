...
``cli.py``'s ``ClaraCLI`` class. ``ClaraCLI`` inherits ``CLICommandsMixin`` so
...
from clara_constants import display_clara_home, is_termux as _is_termux_environment
...
class CLICommandsMixin:
    ...
    def _handle_rollback_command(self, command: str):
        ...
        Syntax:
            ...
            /rollback <N> --all       — classic full restore (may overwrite
                                        files you edited after Clara did)
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: clara --checkpoints")
    ...
    def _handle_diff_command(self, command: str):
        ...
        Syntax:
            ...
            /diff session          — everything Clara changed (checkpoint baseline)
    ...
    def _print_session_diff(self, cwd: str, stat_only: bool):
        ...
        if result.get("empty") or (not stat and not diff):
            print("  No changes — Clara hasn't edited any files here yet.")
            return
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for Clara config/state.
        ...
        if subcmd in {"list", "ls"}:
            ...
            if not snaps:
                ...
                return
            print(f"  State snapshots ({display_clara_home()}/state-snapshots/):\n")
    ...
    def _handle_export_command(self, command: str):
        ...
        try:
            ...
            print("  Share it: the other user runs /import or `clara profile import <archive>`.")
    ...
    def _handle_import_command(self, command: str):
        ...
        try:
            ...
            pass
        print(f"  Use it: clara -p {imported}")
    ...
    def _handle_journey_command(self, cmd_original: str) -> None:
        """Handle /journey — the learning timeline (see `clara journey`).
        ...
        from cli import _cprint
        from clara_cli.journey import register_cli
    ...
    def _handle_paste_command(self):
        ...
        from clara_cli.clipboard import has_clipboard_image
    ...
    def _handle_copy_command(self, cmd_original: str) -> None:
        ...
        try:
            from clara_cli.clipboard import (
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f"  {_DIM}Tip: type your next message, or run clara chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in clara_cli/colors.py
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        # "Is `clara gateway` running?" is the correct diagnosis, and the
        ...
        # CLI printed "Is `clara gateway` running?" while the gateway was
        ...
        try:
            ...
            pass
        _cprint("  Timed out waiting for the gateway. Is `clara gateway` running?")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                ...
                return
            _cprint("  Tip:   Use /history or `clara sessions list` to find sessions.")
            return
        ...
        if target.isdigit():
            ...
            from clara_cli.main import _resolve_session_by_name_or_id
        ...
        if not session_meta:
            ...
            _cprint("  Use /sessions or `clara sessions list` to see available sessions.")
            return
        ...
        # same directory as a startup `clara -c`/`--resume`. The startup resume
    ...
    def _handle_worktree_command(self, cmd_original: str) -> None:
        ...
        commits, same as ``clara -w``).

        ``prune`` is the same attended reclaim as ``clara worktree prune``
        (clara_cli/worktree_gc.py): never deletes tracked changes, unique
        ...
        if sub in {"new", "add", "create"}:
            ...
            # same way `clara -w` and session-resume cwd restore do.
            try:
                os.chdir(wt_info["path"])
            except OSError as e:
                print(f"  ⚠ Created worktree but could not enter it: {e}")
            os.environ["TERMINAL_CWD"] = wt_info["path"]
            # Register for the same keep-if-unpushed cleanup as `clara -w`.
    ...
    def _handle_branch_command(self, cmd_original: str) -> None:
        ...
        try:
            self._session_db.create_session(
                ...
                source=os.environ.get("CLARA_SESSION_SOURCE", "cli"),
                model=self.model,
                model_config={
    ...
    def _handle_pet_command(self, cmd: str):
        ...
        from clara_cli.pets import _set_active, _set_enabled, print_pet_gallery, set_pet_scale, toggle_pet_display
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to clara_cli.curator so the CLI and the `clara curator`
        ...
        try:
            from clara_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to clara_cli.skills_hub."""
        ...
        if args and args[0].lower() in {"pending", "approve", "apply", "reject",
            ...
            if out is not None:
                print(out)
                return
        from clara_cli.skills_hub import handle_skills_slash
    ...
    def _handle_plan_command(self, cmd: str):
        ...
        ``.clara/plans/`` via ``write_file``. No engine, no model-tool
    ...
    def _handle_background_command(self, cmd: str):
        ...
        def run_background():
            ...
            try:
                ...
                if response:
                    try:
                        from clara_cli.skin_engine import get_active_skin
                        _skin = get_active_skin()
                        label = _skin.get_branding("response_label", "⚕ Clara")
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        Mirrors ``clara bundles list`` but renders inside the running
        ...
        if not bundles:
            ...
            _cprint(
                f"  {_DIM}Create one with: clara bundles create "
        ...
        _cprint(
            ...
            f"Manage with `clara bundles`.{_RST}"
        )
        ...
        if sub.startswith("connect"):
            ...
            if hasattr(self, '_pending_input'):
                self._pending_input.put(
                    ...
                    "just because CDP is connected. This is typically a Clara-managed isolated debug "
    ...
    def _handle_heartbeat_command(self, cmd: str) -> None:
        ...
        cross-process schedules use `clara cron`.
        """
        from cli import _DIM, _RST, _cprint
        from clara_cli.heartbeat import parse_interval, format_interval
        ...
        if interval is None:
            ...
            return
        if interval < 0:
            from clara_cli.heartbeat import MIN_INTERVAL_SECONDS
        ...
        _cprint(
            ...
            f"lives only while this Clara process runs — use `clara cron` for "
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"Clara keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_loop_command(self, cmd: str) -> None:
        ...
        from clara_cli.loops import dispatch_loop_command
        ...
        if result.get("created"):
            try:
                from clara_cli.loops import goal_blocks_loop_tick
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from clara_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print("\n  Usage: /skin <name>")
            print(f"  Custom skins: drop a YAML file in {display_clara_home()}/skins/\n")
            return
    ...
    def _compose_in_editor(self, initial_text: str = "") -> str:
        ...
        fd, path = tempfile.mkstemp(suffix=".md", prefix="clara_prompt_")
        try:
    ...
    def _handle_focus_command(self, cmd_original: str) -> None:
        ...
        from clara_cli.colors import Colors as _Colors
    ...
    def _set_tool_progress_mode(self, mode: str) -> None:
        ...
        """
        from clara_cli.focus_view import normalize_tool_progress_mode
    ...
    def _note_focus_hidden_line(self, function_name: str) -> None:
        ...
        if not getattr(self, "_focus_view_enabled", False):
            return
        from clara_cli.focus_view import would_display_tool_line
    ...
    def _emit_focus_recovery_line(self) -> None:
        ...
        if not getattr(self, "_focus_view_enabled", False):
            return
        from clara_cli.focus_view import format_hidden_line
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while Clara is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while Clara is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will redirect the current run while Clara is busy; /stop still cancels it."
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from clara_cli.models import _is_anthropic_fast_model
            agent = getattr(self, "agent", None)
    ...
    def _handle_debug_command(self, cmd_original: str = ""):
        ...
        - ``/debug clara``   → upload to Clara-internal storage (private, staff-only)
        - ``/debug local``  → render the report to stdout, no upload

        ``clara`` and ``local`` are mutually exclusive; if both are given,
        ...
        clara = "clara" in words and not local
        ...
        args = SimpleNamespace(
            lines=200, expire=7, local=local, clara=clara, yes=True
        )
        run_debug_share(args)

    def _handle_update_command(self) -> bool:
        """Handle /update — update Clara Agent to the latest version.

        In the classic CLI this exits the session and relaunches as
        ``clara update`` so the user sees update output directly and gets
        ...
        if is_managed():
            print(f"  ✗ {format_managed_message('update Clara Agent')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update Clara Agent"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title="⚕  Update Clara Agent",
            detail="This will exit the current session and run `clara update`.",
            choices=choices,
        )
        if raw is None:
    ...
    def _handle_wake_command(self, command: str):
        """Handle /wake [on|off|status] — the 'Hey Clara' hotword listener.
