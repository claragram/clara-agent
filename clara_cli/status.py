"""
Status command for clara CLI.

Shows the status of all Clara Agent components.
"""
...
from clara_cli.auth import AuthError, resolve_provider
from clara_cli.colors import Colors, color
from clara_cli.config import get_env_path, get_env_value, get_clara_home, load_config
from clara_cli.models import provider_label
...
from clara_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``clara config``'s
...
def _estop_status_line():
    """One-line pause banner for `clara status`, or None when not paused.

    Cheap: a single stat on $CLARA_HOME/ESTOP via agent.estop.
    """
    try:
    ...
    return f"⏸️  PAUSED (global emergency stop{suffix}; `clara resume` to lift)"


def show_status(args):
    """Show status of all Clara Agent components."""
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Clara Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        ...
        # Read-only display: use the refresh-free snapshot so `clara status`
    ...
    clara_account_info = None
    if (
        clara_status.get("logged_in")
        or clara_status.get("access_token")
        or clara_status.get("portal_base_url")
        or clara_status.get("inference_credential_present")
        or clara_status.get("error_code")
    ):
        try:
            clara_account_info = get_clara_portal_account_info()
        except Exception:
            clara_account_info = None

    clara_logged_in = bool(
        clara_status.get("logged_in")
        or (clara_account_info and clara_account_info.logged_in)
    )
    clara_inference_present = bool(
        clara_status.get("inference_credential_present")
        or (clara_account_info and clara_account_info.inference_credential_present)
    )
    clara_error = clara_status.get("error")
    if clara_logged_in:
        clara_label = "logged in"
    elif clara_inference_present:
        clara_label = "not logged in (Clara inference key configured)"
    else:
        clara_label = "not logged in (run: clara portal)"
    print(
        f"  {'Clara Portal':<12}  {check_mark(clara_logged_in)} "
        f"{clara_label}"
    )
    portal_url = clara_status.get("portal_base_url") or "(unknown)"
    inference_url = (
        clara_status.get("inference_base_url")
        or (clara_account_info.inference_base_url if clara_account_info else None)
    )
    access_exp = _format_iso_timestamp(clara_status.get("access_expires_at"))
    key_exp = _format_iso_timestamp(clara_status.get("agent_key_expires_at"))
    refresh_label = "yes" if clara_status.get("has_refresh_token") else "no"
    if clara_logged_in or portal_url != "(unknown)" or clara_error:
        print(f"    Portal URL: {portal_url}")
    if clara_inference_present and inference_url:
        print(f"    Inference:  {inference_url}")
    if clara_logged_in or clara_status.get("access_expires_at"):
        print(f"    Access exp: {access_exp}")
    if clara_logged_in or clara_inference_present or clara_status.get("agent_key_expires_at"):
        print(f"    Key exp:    {key_exp}")
    if clara_logged_in or clara_status.get("has_refresh_token"):
        print(f"    Refresh:    {refresh_label}")
    if clara_error:
        print(f"    Error:      {clara_error}")

    codex_logged_in = bool(codex_status.get("logged_in"))
    print(
        f"  {'OpenAI Codex':<12}  {check_mark(codex_logged_in)} "
        f"{'logged in' if codex_logged_in else 'not logged in (run: clara model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: clara auth add minimax-oauth)'}"
    )
    ...
    # disrupt the already-printed Clara/Codex/Qwen/MiniMax rows above.
    try:
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: clara auth add xai-oauth)'}"
    )
    ...
    # =========================================================================
    # Clara Subscription Features
    # =========================================================================
    if managed_clara_tools_enabled():
        features = get_clara_subscription_features(config)
        print()
        print(color("◆ Clara Tool Gateway", Colors.CYAN, Colors.BOLD))
        if not features.clara_auth_present:
            print("  Clara Portal   ✗ not logged in")
        else:
            print("  Clara Portal   ✓ managed tools available")
        for feature in features.items():
            if feature.managed_by_clara:
                state = "active via Clara subscription"
            elif feature.active:
                current = feature.current_provider or "configured provider"
                state = f"active via {current}"
            elif feature.included_by_default and features.clara_auth_present:
                state = "included by subscription, not currently selected"
            elif feature.key == "modal" and features.clara_auth_present:
                state = "available via subscription (optional)"
            else:
                state = "not configured"
            print(f"  {feature.label:<15} {check_mark(feature.available or feature.active or feature.managed_by_clara)} {state}")
    elif clara_logged_in or clara_inference_present:
        # Clara OAuth without entitlement, or an opaque inference key without
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: clara model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from clara_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'clara-agent[vercel]')"
        ...
        # provider's doctor rows (fail-soft — never break `clara status`).
        try:
    ...
    try:
        from clara_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   clara gateway")
    ...
    jobs_file = get_clara_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    if _session_count is not None and _session_count > 0:
        ...
        sessions_file = get_clara_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'clara doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'clara setup' to configure", Colors.DIM))
    print()
