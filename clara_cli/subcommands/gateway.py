"""``clara gateway`` and ``clara proxy`` subcommand parsers.

Extracted verbatim from ``clara_cli/main.py:main()`` (god-file Phase 2).
...
from clara_cli.subcommands._shared import add_accept_hooks_flag
...
def build_gateway_parser(
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if CLARA_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy clara.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Clara gateway unit files "
            "(e.g. clara.service) left over from older installs. Profile "
            "units (clara-gateway-<profile>.service) and unrelated "
    ...
    gateway_enroll = gateway_subparsers.add_parser(
        ...
        description=(
            ...
            "Authenticates as your Clara Portal account (the connector derives the "
            ...
            "GATEWAY_RELAY_SECRET / GATEWAY_RELAY_DELIVERY_KEY into ~/.clara/.env. "
            "Requires being logged in (clara setup). Not available in managed installs."
        ),
    )
    ...
    gateway_enroll.add_argument(
        ...
        help=(
            ...
            "GATEWAY_RELAY_WAKE_URL in ~/.clara/.env and forwarded at provision. "
    ...
    proxy_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "to an OAuth-authenticated provider (e.g. Clara Portal). External "
    ...
    proxy_start.add_argument(
        "--provider",
        default="clara",
        help="Upstream provider: clara or xai (default: clara). See `clara proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
