...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Clara Agent. Use 'clara debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    clara debug share              Upload debug report (asks for confirmation)
    clara debug share --yes        Skip confirmation (for scripts/CI)
    clara debug share --lines 500  Include more log lines
    clara debug share --expire 30  Keep paste for 30 days
    clara debug share --local      Print report locally (no upload)
    clara debug share --no-redact  Disable upload-time secret redaction
    clara debug share --clara       Upload to Clara-internal storage (private)
    clara debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    share_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Upload the debug bundle to Clara-internal storage (AWS S3) instead "
            "of a public paste service. The bundle is private — viewable only "
            "by Clara staff (and allowlisted Discord mods) via a Google-login-"
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'clara debug share'",
    )
