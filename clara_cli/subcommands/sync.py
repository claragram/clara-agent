"""``clara sync`` subcommand parser — Skill Sync.

Cloned from ``clara_cli/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    clara sync status                 show gate/opt-in/head state
    clara sync pull                   pull and materialize opted-in skills
    clara sync push                   push opted-in skills
    clara sync now                    reconcile: pull then push
    clara sync enable <skill>         opt a skill into sync
    clara sync disable <skill>        opt a skill out of sync
    clara sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    clara sync propose <skill>        share a skill with your organisation

Sync is INERT unless the resolved Clara token carries the access-gate claim
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  clara sync status            what is synced, and from where\n"
            "  clara sync enable my-skill   include a skill in your sync\n"
            "  clara sync now               pull, then push\n"
            "  clara sync propose my-skill  share a skill with your team\n"
        ),
