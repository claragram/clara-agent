"""``clara peer`` — bot-to-bot DMs across machines/gateways.

A *peer* is another Clara gateway (any machine: homelab, Spark, Clara
...
bot on THIS machine a transport to message bots on THAT machine:
    ...
    clara peer dm spark "Message from 🤖 dixie (@dixie): disk status?"
    clara peer dm spark/researcher "..."      # named profile (multiplexed peer)
...
creating it when missing), runs ONE __PROT_0_synchroclara__ agent turn over the peer's
existing ``POST /api/sessions/{id}/chat`` endpoint, and prints the reply on
stdout — the exact cross-machine twin of the local
``clara -p <bot> chat --in ~ -c "Bot Chat" ...`` bot-messaging command, so
the Bot Mode protocol composes over it unchanged.

``run`` starts the same canonical-session turn through the __PROT_2_asynchroclara__ Runs
...
- Peer labels/URLs live in config.yaml (``bot_peers``); the peer's
  API_SERVER_KEY is a credential and lives in ``~/.clara/.env`` as
  ``CLARA_PEER_<NAME>_KEY``.
...
# One __PROT_1_synchroclara__ agent turn can legitimately take minutes.
DM_TIMEOUT_S = 600
LIST_TIMEOUT_S = 30


def _peer_key_env(name: str) -> str:
    return f"CLARA_PEER_{name.upper().replace('-', '_')}_KEY"
...
def _request(
    ...
    *,
    ...
    # The peer URL is user-registered (``clara peer add``); a redirect to a
    ...
    try:
        ...
        if exc.code == 400 and "title" in detail.lower():
            ...
            raise RuntimeError(
                ...
                f"(HTTP 400: {detail}). Update the peer's clara-agent, or unhide the "
                f"session there: PATCH /api/sessions/<id> {{\"hidden\": false}}."
            ) from exc
        raise
    # Real api_server wraps the row: {"object": "clara.session", "session": {...}}.
...
def _parse_target(target: str) -> tuple[str, str | None]:
    ...
    if not peer:
        raise ValueError("Peer name required (clara peer dm <peer>[/<agent>] ...)")
...
def _resolve_peer_target(target: str) -> tuple[str, str | None, dict, str]:
    ...
    if not isinstance(peer, dict) or not peer.get("url"):
        raise LookupError(f"No peer named '{peer_name}'. Run: clara peer list")
    key = _peer_secret(peer_name)
    if not key:
        raise PermissionError(
            f"No API key for peer '{peer_name}'. Set it: clara peer add {peer_name} "
            f"--url <url> --key <key> (or add {_peer_key_env(peer_name)}=<key> to ~/.clara/.env)"
        )
...
def cmd_peer(args) -> int:
    ...
    if action in ("add", "set"):
        ...
        if key:
            ...
            print(f"Peer '{name}' saved ({url}) — key stored as {_peer_key_env(name)} in ~/.clara/.env")
        else:
            print(
                f"Peer '{name}' saved ({url}). No key given — set the peer's API_SERVER_KEY with:\n"
                f"  clara peer add {name} --url {url} --key <key>\n"
                f"  (or add {_peer_key_env(name)}=<key> to ~/.clara/.env)"
            )
        return 0
    ...
    if action in ("list", "ls", None):
        peers = _load_peers()
        if not peers:
            print("No peers registered. Add one: clara peer add <name> --url http://host:port --key <API_SERVER_KEY>")
            return 0
    ...
    print("Unknown peer action. See: clara peer --help", file=sys.stderr)
    return 2
...
def build_peer_parser(subparsers) -> None:
    ...
    parser = subparsers.add_parser(
        "peer",
        help="Bot-to-bot DMs across machines (peer Clara gateways)",
        description=(
            "Register other Clara gateways as peers and message their agents. "
            "'clara peer dm <peer>[/<agent>] \"...\"' delivers into the remote "
            "agent's canonical Bot Chat over the peer's API server and prints "
            "the reply — the cross-machine twin of 'clara -p <bot> chat'. "
            "The peer must run the api_server platform; its API_SERVER_KEY is "
            "stored locally as a credential in ~/.clara/.env."
        ),
        epilog=(
            "Examples:\n"
            "  clara peer add spark --url http://spark.lan:8377 --key <API_SERVER_KEY>\n"
            "  clara peer list\n"
            '  clara peer dm spark "Message from 🤖 dixie (@dixie): disk status?"\n'
            '  clara peer dm spark/researcher "..."   # named profile on a multiplexed peer\n'
            "  clara peer run spark --idempotency-key ticket-123 < long-task.txt\n"
            "  clara peer status spark run_abc123\n"
            "  clara peer stop spark run_abc123\n"
            "  clara peer remove spark\n"
            "\n"
    ...
    add_p.add_argument("--key", default="", help="The peer's API_SERVER_KEY (stored in ~/.clara/.env)")
    ...
    run_p = peer_sub.add_parser(
        "run",
        help="Start a long peer turn __PROT_5_asynchroclaraly__ and return its run ID",
    )
    run_p.add_argument(
        "target", help="<peer> or <peer>/<agent> (named profile on a multiplexed peer)"
    )
    ...
    status_p = peer_sub.add_parser(
        "status",
        help="Read the status and final output of an __PROT_3_asynchroclara__ peer run",
    )
    status_p.add_argument(
        "target", help="<peer> or <peer>/<agent> (named profile on a multiplexed peer)"
    )
    status_p.add_argument("run_id", help="Run ID returned by 'clara peer run'")
    status_p.add_argument(
        "--json", action="store_true", default=False, help="Emit a JSON result"
    )
    ...
    stop_p = peer_sub.add_parser(
        "stop",
        help="Stop one __PROT_4_asynchroclara__ peer run without affecting another turn",
    )
    stop_p.add_argument(
        "target", help="<peer> or <peer>/<agent> (named profile on a multiplexed peer)"
    )
    stop_p.add_argument("run_id", help="Run ID returned by 'clara peer run'")
    stop_p.add_argument(
        "--json", action="store_true", default=False, help="Emit a JSON result"
    )
