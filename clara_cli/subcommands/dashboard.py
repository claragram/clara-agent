"""``clara dashboard`` / ``clara serve`` subcommand parsers.
...
``clara_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `clara dashboard` / `clara serve` cmdlines and SIGTERM them directly —
    # the same path `clara update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Clara web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Clara web server processes and exit",
    )
...
def build_serve_parser(
    *,
    ...
    parser = argparse.ArgumentParser(
        prog="clara serve",
        description=(
            "Run the Clara backend server - the JSON-RPC/WebSocket gateway the "
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Clara Agent web dashboard for managing config, API keys, and sessions",
    )
    _add_server_runtime_args(dashboard_parser)
    dashboard_parser.add_argument(
        "--no-open", action="store_true", help="Don't open browser automatically"
    )
    # Backward-compat shim: older Clara desktop app shells (<= 0.15.x) spawn the
    # backend as `clara dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Clara couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the Clara Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Clara backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Clara backend server — the JSON-RPC/WebSocket gateway the "
            "desktop app and remote clients connect to. Headless: it never opens "
            "a browser UI."
    ...
    # `clara dashboard register` — register a self-hosted dashboard OAuth
    # client with Clara Portal and write the client_id into ~/.clara/.env.
    # Nested subparser so bare `clara dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        "register",
        help="Register a self-hosted dashboard with Clara Portal (writes the OAuth client ID to .env)",
        description=(
            "Register this install as a self-hosted dashboard with your Clara "
            "Portal account. Creates an OAuth client, writes "
            "CLARA_DASHBOARD_OAUTH_CLIENT_ID into ~/.clara/.env, and prints "
            "how to engage the login gate. Requires being logged in (clara setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://clara.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        default=None,
        help=(
            "Override the Clara Portal base URL for registration (default: the "
            "portal you logged into). The access token must be valid at this "
            "portal. Also settable via CLARA_DASHBOARD_PORTAL_URL. Mainly for "
