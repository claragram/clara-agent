...
def build_gui_parser(subparsers, *, cmd_gui: Callable) -> None:
    ...
    gui_parser = subparsers.add_parser(
        ...
        description=(
            "Launch the Clara Electron desktop app. By default this installs "
    ...
    gui_parser.add_argument(
        "--ignore-existing",
        action="store_true",
        help="Force Desktop to ignore any clara CLI already on PATH during backend resolution",
    )
    gui_parser.add_argument(
        "--clara-root",
        help="Override the Clara source root used by Desktop (sets CLARA_DESKTOP_CLARA_ROOT)",
    )
    gui_parser.add_argument(
        "--cwd",
        help="Initial project directory for Desktop chat sessions (sets CLARA_DESKTOP_CWD)",
    )
    gui_parser.add_argument(
        "--skip-build",
        action="store_true",
    ...
    gui_parser.add_argument(
        "--identity",
        default="Clara Local Signing",
        help="Certificate name to create/use for --setup-tcc-identity (default: Clara Local Signing)",
    )
