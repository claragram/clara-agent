"""``clara mcp`` subcommand parser.

Extracted from ``clara_cli/main.py:main()`` (god-file Phase 2 follow-up).
...
def build_mcp_parser(subparsers, *, cmd_mcp: Callable) -> None:
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Clara as an MCP server",
        description=(
            "Manage MCP server connections and run Clara as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'clara mcp add' to connect to a new server, or\n"
            "'clara mcp serve' to expose Clara conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Clara as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `clara mcp add ...` to fall through to
    # interactive chat.
    ...
    # ── Catalog (Clara-approved MCPs shipped with the repo) ─────────────────
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `clara mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
        help="List Clara-approved MCPs available for one-click install",
    )
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `clara mcp install n8n`)",
    )
