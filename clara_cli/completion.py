"""Shell completion script generation for clara CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_clara_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Clara Agent bash completion
# Add to ~/.bashrc:
#   eval "$(clara completion bash)"

_clara_profiles() {{
    local profiles_dir="$HOME/.clara/profiles"
...
_clara_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_clara_profiles)" -- "$cur"))
        return
    fi
...
complete -F _clara_completion clara
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef clara
# Clara Agent zsh completion
# Add to ~/.zshrc:
...
_clara_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.clara/profiles" ]]; then
        profiles+=($HOME/.clara/profiles/*(N/:t))
    fi
...
_clara() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_clara_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'clara command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Clara Agent fish completion",
        "# Add to your config:",
        "#   clara completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __clara_profiles",
        "    echo default",
        "    if test -d $HOME/.clara/profiles",
        "        for d in $HOME/.clara/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c clara -f -s p -l profile"
        " -d 'Profile name' -xa '(__clara_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__clara_profiles)' -d 'Profile name'"
                )
