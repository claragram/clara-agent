...
One read-only pass that answers, BEFORE any mutation: what Clara runtimes
...
`detect_install_method`) — so `clara update --plan` can run on a live
...
Deployment kinds (the concept most fleet-update bugs were missing):
    ...
    git      — source checkout; updatable in place via `clara update`
...
Supervisors (how a runtime is restarted after code changes):
    ...
    desktop           — Desktop app supervises `clara serve`; it respawns
...
@dataclass
class RuntimeRecord:
    """One running (or expected) Clara runtime on this machine."""
...
class UpdatePlan:
    ...
    update_mechanism: str = "clara update"
...
def _detect_supervisor_for_pid(
    ...
    if pid in service_pids:
        try:
            from clara_cli.gateway import is_macos, supports_systemd_services
...
def describe_restart_mechanism(mechanism: str, profile: str) -> str:
    ...
    if profile != "default":
        return f"clara -p {profile} gateway restart"
...
def collect_runtime_inventory() -> UpdatePlan:
    ...
    try:
        ...
        try:
            from clara_cli.image_provenance import read_image_provenance
    ...
    try:
        from clara_cli.gateway import _get_service_pids
    ...
    try:
        from clara_cli.gateway import find_windows_gateway_services
    ...
    try:
        from clara_cli.gateway import find_profile_gateway_processes
    ...
    # launched `clara serve --host <ip>` for a remote Desktop, or a
    # long-lived `clara dashboard`. Every serve/dashboard registers itself
...
def print_update_plan(plan: UpdatePlan) -> None:
    ...
    if not plan.runtimes:
        print("  Running Clara services: none detected — code swap only.")
        return
...
def _serve_unit_matches_profile(profile: str, unit: object) -> bool:
    """Does *unit* name a ``clara-serve*``/``clara-dashboard*`` unit for *profile*?

    Serve/dashboard runtimes have their OWN unit vocabulary; the gateway's
    ``clara-gateway*`` names never cover them (#100479). Exact names only —
    ``work`` must not claim ``clara-serve-workbench`` — and a scope prefix
    (``user/clara-serve``) is tolerated because the restart phase records
    ...
    if profile == "default":
        return name in {"clara-serve", "clara-dashboard"}
    return name in {f"clara-serve-{profile}", f"clara-dashboard-{profile}"}
...
def match_runtime_outcomes(
    ...
    *,
    ...
    (#100479): a ``clara-serve*``/``clara-dashboard*`` unit, a killed
    PID, or — when the caller passes ``stale_serve_pids`` (the
    ``(pid, create_time)``-verified survivor probe,
    :func:`clara_cli.update_abort_recovery._surviving_pre_update_serve_runtimes`)
    ...
    gateway's outcome: ``relaunched_profiles`` and ``clara-gateway*`` name a
    ...
    try:
        ...
        for runtime in plan.runtimes:
            ...
            if r.kind in _SERVE_KINDS:
                ...
                continue
            outcome = "unaccounted"
            # The bare "clara-gateway" unit name is gateway-specific: a
...
def report_unaccounted_runtimes(outcomes: list[dict[str, Any]]) -> bool:
    ...
    if any(o.get("kind") not in _SERVE_KINDS for o in missed):
        print("      clara gateway restart                # active profile")
        print("      clara -p <profile> gateway restart   # named profile")
    if any(o.get("kind") in _SERVE_KINDS for o in missed):
        ...
        print("      systemctl --user restart clara-serve.service   # unit-managed serve")
        print("      relaunch `clara serve` / `clara dashboard` / the Desktop app")
    return True
...
def record_plan_in_receipt(plan: UpdatePlan) -> None:
    ...
    try:
        import clara_cli.update_receipt as ur
