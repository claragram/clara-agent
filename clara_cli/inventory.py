...
- ``list_authenticated_providers`` already populates each row's
  ...
  curation and pulls in non-agentic models (Clara /models returns ~400
...
def build_models_payload(
    ...
    *,
    ...
    - ``pricing``: enrich each row with formatted per-model pricing and,
      for Clara, ``free_tier``/``unavailable_models`` so the GUI picker can
      show $/Mtok columns and gate paid models on free accounts —
      mirroring the ``clara model`` CLI picker. Adds network calls
      (pricing fetch + Clara tier check); only set for interactive pickers.
    ...
    - ``force_fresh_clara_tier``: bypass the short Clara free-tier cache when
      selecting Portal-recommended Clara models and applying tier gating. Keep
    ...
    rows = list_authenticated_providers(
        ...
        force_fresh_clara_tier=force_fresh_clara_tier,
    ...
    try:
        from clara_cli.providers import is_routing_aggregator as _is_routing_aggregator
    except Exception:
    ...
    if pricing:
        _apply_pricing(rows, force_fresh_clara_tier=force_fresh_clara_tier)
    if capabilities:
...
def build_aux_picker_rows(
    *,
    ...
    back. Mirrors the same filter in ``clara_cli/moa_cmd.py``.
...
def _reasoning_catalog_reader(slug: str):
    ...
    try:
        from clara_cli.models import (
            clara_model_reasoning_capabilities,
            openrouter_model_reasoning_capabilities,
            warm_clara_reasoning_caps_async,
    ...
    if slug == "clara":
        warm_clara_reasoning_caps_async()
        return clara_model_reasoning_capabilities
    if slug == "openrouter":
        warm_openrouter_reasoning_caps_async()
...
def _apply_featured(rows: list[dict]) -> None:
    ...
    Aggregator providers (clara, openrouter) serve dozens of models across many
...
def _apply_custom_aliases(rows: list[dict]) -> None:
    ...
    :func:`clara_cli.providers.custom_provider_aliases` — lets the frontend
...
def _append_unconfigured_rows(
    ...
    *,
    ...
    from clara_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    for entry in CANONICAL_PROVIDERS:
        ...
        if entry.slug.lower() == cur:
            ...
            warning = (
                ...
                else "Configured provider is not authenticated; run `clara model` to reactivate. "
                "Showing the saved model only."
            )
            extras.append(
                {
...
def _anthropic_oauth_credentials_present() -> bool:
    ...
    model.provider / API-key env vars: Clara' own Anthropic device flow
    ...
    try:
        ...
        clara_creds = read_clara_oauth_credentials() or {}
        if clara_creds.get("accessToken"):
            return True
...
def _filter_explicit_provider_rows(rows: list[dict], ctx: ConfigContext) -> list[dict]:
    ...
    pickers want the narrower subset the user explicitly configured for Clara.
    """
    ...
    for row in rows:
        ...
        if slug == "anthropic" and _anthropic_oauth_credentials_present():
            # Anthropic OAuth logins (Clara device flow / Claude Code) are
...
def _provider_is_keyless(slug: str) -> bool:
    """True when the provider's Clara overlay declares it keyless."""
    try:
        from clara_cli.providers import CLARA_OVERLAYS
        overlay = CLARA_OVERLAYS.get(slug)
...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        row["warning"] = (
            ...
            if auth_type == "api_key" and key_env
            else f"run `clara model` to configure ({auth_type})"
        )
...
def _apply_pricing(
    ...
    *,
    ...
    # Resolve Clara free-tier once (cached in models.py for the TTL window).
    clara_free_tier: Optional[bool] = None
    ...
    for row in rows:
        ...
        for mid in models:
            ...
            # Sale chrome is Clara Portal-only. Other providers (OpenRouter,
        ...
        if slug == "clara":
            try:
                if clara_free_tier is None:
                    clara_free_tier = check_clara_free_tier(
                        force_fresh=force_fresh_clara_tier
                    )
                row["free_tier"] = bool(clara_free_tier)
                if clara_free_tier:
                    _selectable, unavailable = partition_clara_models_by_tier(
...
def _local_runtime_row(ctx: "ConfigContext") -> dict | None:
    ...
    """
    try:
        from clara_cli.local_runtime.bootstrap import staged_model_ids
        ...
        if not current:
            ...
            # selection.
            try:
                from clara_cli.local_runtime.endpoint import _state_endpoint
...
def _moa_provider_row(current_provider: str = "") -> dict | None:
    ...
    picker path (:func:`clara_cli.model_switch.list_picker_providers`) so the
