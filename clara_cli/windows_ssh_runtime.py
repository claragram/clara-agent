...
def _root() -> Path:
    ...
    # helper's token, including custom CLARA_HOME layouts.
    return get_default_clara_root() / "desktop-ssh"
...
def process_state(pid: int, creation_time_ns: int, clara_path: str, spawn_nonce: str) -> dict[str, Any]:
    import psutil
    _nonce(spawn_nonce)
    try:
    ...
    expected = os.path.normcase(os.path.abspath(clara_path))
    arg0 = os.path.normcase(os.path.abspath(argv[0]))
    # argv[0] is either the clara exe directly, or (normal case) the base Python
    ...
    # `-c <bootstrap that runs clara_cli.main>`; also accept a plain `-m` launch.
    ...
    launches_module = (
        argv[1:3] == ["-m", "clara_cli.main"]
        or (len(argv) > 2 and argv[1] == "-c" and "clara_cli.main" in argv[2])
    )
...
def terminate_owned(pid: int, creation_time_ns: int, clara_path: str, spawn_nonce: str) -> bool:
    state = process_state(pid, creation_time_ns, clara_path, spawn_nonce)
...
def _resolve_direct_interpreter(python_entry: str) -> tuple[str, list[str]]:
    ...
    Also resolves clara_cli's own location and prepends its parent, because the
    ...
    # the clara_cli package parent must be explicit -- the launcher resolves it via
    ...
    if not root or not os.path.isdir(root):
        raise ValueError("could not locate the clara_cli package")
    return base, py_path
...
def spawn_backend(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    configured_path = str(payload["claraPath"])
    if not os.path.isabs(configured_path):
        raise ValueError("Clara path must be absolute")
    clara_path = os.path.abspath(configured_path)
    ...
    venv_dir = os.path.dirname(clara_path)
    python_entry = os.path.join(venv_dir, "python.exe")
    if not os.path.isfile(python_entry):
        raise ValueError("Clara Python runtime was not found")
    ...
    bootstrap = (
        ...
        "runpy.run_module('clara_cli.main',run_name='__main__',alter_sys=True)"
    )
...
def inspect_clara(clara_path: str) -> dict[str, Any]:
    path = os.path.abspath(clara_path)
    if not os.path.isabs(clara_path) or not os.path.isfile(path):
        raise ValueError("Clara path is not an executable file")
...
def dispatch(argv: list[str]) -> Any:
    ...
    if operation == "probe":
        import platform
        return {"os": "Windows", "arch": platform.machine(), "claraHome": str(get_default_clara_root()), "python": sys.executable}
