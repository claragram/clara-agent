...
1. **Update receipt** — a machine-readable JSON record of what one
   ``clara update`` run discovered, did, skipped (and why), written to
   ``<CLARA_HOME>/logs/update_receipts/``. Silent-failure classes this
...
``clara_cli.build_info.get_code_identity()``: an image build reports
...
# Module-level current receipt. ``clara update`` is a single-threaded CLI
...
class UpdateReceipt:
    """Collects the observable facts of one ``clara update`` run."""
    ...
    def gateway_restart_result(
        ...
        *,
        ...
        if fresh_recovery is not None:
            ...
            # Serve/dashboard coverage (#92145). ``clara serve`` hosts
...
def _receipt_dir() -> Path:
    ...
    return get_clara_home() / "logs" / _RECEIPT_DIR_NAME
...
def finalize_pending_update_receipt(
    ...
    ``clara update`` has many early-termination paths (Windows
