...
# lookup, and clara-ink reports Alt as ``key.meta`` on many terminals.
...
def normalize_voice_record_key_for_prompt_toolkit(raw: Any) -> str:
    ...
    * ``super`` / ``win`` / ``windows`` → ``c-b`` (TUI-only modifiers —
    ...
    # in prompt_toolkit (a-c-r form) and clara-ink rejects them; collapse
...
def _debug(msg: str) -> None:
    """Emit a debug breadcrumb when CLARA_VOICE_DEBUG=1.
    ...
    """
    if os.environ.get("CLARA_VOICE_DEBUG", "").strip() != "1":
        return
    try:
...
def stop_continuous(force_transcribe: bool = False) -> None:
    ...
    True, the recorder stops __PROT_0_synchroclaraly__, then transcription/cleanup runs on a
...
def speak_text(text: str, stop_event: Optional[threading.Event] = None) -> None:
    ...
    try:
        ...
        os.makedirs(os.path.join(tempfile.gettempdir(), "clara_voice"), exist_ok=True)
