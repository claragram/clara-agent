...
from clara_constants import get_clara_home  # noqa: F401 — config paths

from clara_cli.local_runtime.binaries import runtimes_root

logger = logging.getLogger(__name__)

_SUPERVISOR = None  # process-wide singleton; one router per Clara process
...
def models_dir() -> Path:
    ...
    return get_default_clara_root() / "models"
...
def _stop_state_server(state: dict) -> None:
    ...
    from clara_cli.local_runtime.endpoint import _pid_alive
...
def ensure_local_runtime(config: dict, force: bool = False) -> "object | None":
    ...
    # Another Clara process may already be supervising — reuse via state,
    ...
    try:
        ...
        from clara_cli.local_runtime.presets import generate_presets
        ...
        # Clara-release default when unpinned) is preferred; when it isn't
