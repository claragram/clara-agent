"""Clara Portal Remote Spending HTTP client (Phase 2b).

Thin, fail-loud client for the four ``/api/billing/*`` endpoints the terminal
billing screens drive. Companion to ``clara_cli/clara_account.py`` (which owns
...
- **Auth** = the OAuth bearer JWT Clara already holds for inference
  (``get_provider_auth_state("clara")["access_token"]``). No API-key auth on these.
- **Portal base URL** resolves with the same precedence as the device-flow login
  (``auth.py``): ``CLARA_PORTAL_BASE_URL`` → ``CLARA_PORTAL_BASE_URL`` → the
...
DEFAULT_PORTAL_BASE_URL = "https://portal.claraprise.com"
...
# clara_cli.auth.CLARA_BILLING_MANAGE_SCOPE (kept here too so this module has no
...
def resolve_portal_base_url(state: Optional[dict[str, Any]] = None) -> str:
    ...
    ``CLARA_PORTAL_BASE_URL`` → ``CLARA_PORTAL_BASE_URL`` → stored auth-state
    ``portal_base_url`` → registry default. Trailing slash stripped.
    """
    env = os.getenv("CLARA_PORTAL_BASE_URL") or os.getenv("CLARA_PORTAL_BASE_URL")
...
# Short-lived cache for the resolved (token, base). `resolve_clara_access_token`
...
def _billing_not_logged_in(exc: Optional[BaseException] = None) -> "BillingAuthError":
    ...
    err = BillingAuthError(
        "Not logged into Clara Portal — run `clara portal` to log in.",
        status=401,
        error="invalid_token",
    )
    if exc is not None:
...
def _resolve_token_and_base(*, use_cache: bool = True) -> tuple[str, str]:
    ...
    (``resolve_clara_access_token``), so a short-lived (~15 min) access token that
    ...
    when there is no usable Clara session at all.
    ...
    try:
        token = resolve_clara_access_token()
...
def _request(
    ...
    *,
    ...
    try:
        ...
        raise BillingError(
            f"Could not reach Clara Portal: {exc.reason}", error="network_error"
        ) from exc
        ...
        raise BillingError(
            "Could not reach Clara Portal: timed out", error="network_error"
        ) from exc
