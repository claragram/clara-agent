"""Helpers for Clara subscription managed-tool capabilities."""
...
from clara_cli.config import get_env_value, load_config
from clara_cli.clara_account import (
    ClaraPortalAccountInfo,
...
# Maps a tools_config provider's ``managed_clara_feature`` to the tool-pool
# coverage category (clara_cli.clara_account.TOOL_COVERAGE_CATEGORIES). Lets the
# `clara tools` picker scope its entitlement gate to the selected backend, so a
# free-tool-pool user is allowed image gen but denied video gen at select time —
# consistent with the per-category feature gates in get_clara_subscription_features.
...
def _selected_provider(section: object, name_key: str = "provider") -> Optional[str]:
    ...
    an in-memory section dict: ``"clara"`` for the managed selection (stored
    ``clara`` value or legacy ``use_gateway: true``), a vendor name for BYOK
    picks, or ``None`` when no selection is stored. Keeping this in lockstep
    with the runtime resolver is what stops ``clara status`` from lying.
    """
    if not isinstance(section, dict):
        return None
...
class ClaraFeatureState:
    key: str
    label: str
    included_by_default: bool
    available: bool
    active: bool
    managed_by_clara: bool
...
@dataclass(frozen=True)
class ClaraSubscriptionFeatures:
    subscribed: bool
    clara_auth_present: bool
    provider_is_clara: bool
    features: Dict[str, ClaraFeatureState]
    account_info: Optional[ClaraPortalAccountInfo] = None

    @property
    def web(self) -> ClaraFeatureState:
        return self.features["web"]

    @property
    def image_gen(self) -> ClaraFeatureState:
        return self.features["image_gen"]

    @property
    def tts(self) -> ClaraFeatureState:
        return self.features["tts"]

    @property
    def stt(self) -> ClaraFeatureState:
        return self.features["stt"]

    @property
    def browser(self) -> ClaraFeatureState:
        return self.features["browser"]

    @property
    def video_gen(self) -> ClaraFeatureState:
        return self.features["video_gen"]

    @property
    def modal(self) -> ClaraFeatureState:
        return self.features["modal"]

    def items(self) -> Iterable[ClaraFeatureState]:
...
def _has_agent_browser() -> bool:
    ...
    try:
        ...
        # Clara-managed Node dirs (Windows installer / POSIX $CLARA_HOME/node)
        ...
        managed_path = with_clara_node_path().get("PATH", "")
        if managed_path:
...
def _local_stt_backend_available() -> bool:
    ...
    ``apply_clara_managed_defaults`` from flipping a working local setup
    to the managed gateway.
    """
    if get_env_value("CLARA_LOCAL_STT_COMMAND"):
        return True
    try:
...
def get_clara_subscription_features(
    ...
    *,
    ...
    provider_is_clara = str(model_cfg.get("provider") or "").strip().lower() == "clara"
    ...
    clara_auth_present = bool(account_info and account_info.logged_in)

    def _entitled_for(category: str) -> bool:
        return bool(account_info and account_info.tool_gateway_entitled_for(category))
    subscribed = provider_is_clara or clara_auth_present
    ...
    # requires `pip install faster-whisper`. For Clara subscribers we'd
    # rather route through the managed OpenAI audio gateway — see
    # apply_clara_managed_defaults below.
    ...
    # "clara" (stored value or legacy use_gateway: true) = managed gateway;
    ...
    web_use_gateway = web_selected == "clara"
    tts_use_gateway = tts_selected == "clara"
    stt_use_gateway = stt_selected == "clara"
    browser_use_gateway = browser_selected == "clara"
    image_use_gateway = image_selected == "clara"
    video_use_gateway = video_selected == "clara"

    # The "clara" selection is serviced by a concrete vendor implementation —
    # normalize the current-provider labels so downstream vendor checks hold.
    if web_backend == "clara" or web_use_gateway:
        web_backend = "firecrawl"
    if tts_provider == "clara" or tts_use_gateway:
        tts_provider = "openai"
    if stt_provider == "clara" or stt_use_gateway:
        stt_provider = "openai"
    if browser_provider == "clara" or browser_use_gateway:
    ...
    # Keyless Tavily is opt-in: selecting it in `clara tools` / setup writes
    ...
    try:
        ...
        local_stt_available = bool(_HAS_FASTER_WHISPER) or bool(
            get_env_value("CLARA_LOCAL_STT_COMMAND")
        )
    except Exception:
        local_stt_available = bool(get_env_value("CLARA_LOCAL_STT_COMMAND"))
    ...
    features = {
        "web": ClaraFeatureState(
            key="web",
            label="Web tools",
            included_by_default=True,
            available=web_available,
            active=web_active,
            managed_by_clara=web_managed,
        ...
        "image_gen": ClaraFeatureState(
            key="image_gen",
            label="Image generation",
            included_by_default=True,
            available=image_available,
            active=image_active,
            managed_by_clara=image_managed,
            direct_override=image_active and not image_managed,
            toolset_enabled=image_tool_enabled,
            current_provider="FAL" if (image_selected not in (None, "clara") or (image_selected is None and direct_fal)) else ("Clara Subscription" if (image_managed or image_use_gateway) else ""),
            explicit_configured=image_selected is not None or direct_fal,
        ),
        "video_gen": ClaraFeatureState(
            key="video_gen",
            label="Video generation",
            included_by_default=False,
            available=video_available,
            active=video_active,
            managed_by_clara=video_managed,
            direct_override=video_active and not video_managed,
            toolset_enabled=video_tool_enabled,
            current_provider="FAL" if (video_selected not in (None, "clara") or (video_selected is None and direct_fal_video)) else ("Clara Subscription" if (video_managed or video_use_gateway) else ""),
            explicit_configured=video_selected is not None or direct_fal_video,
        ),
        "tts": ClaraFeatureState(
            key="tts",
            label="OpenAI TTS",
            included_by_default=True,
            available=tts_available,
            active=tts_active,
            managed_by_clara=tts_managed,
        ...
        "stt": ClaraFeatureState(
            key="stt",
            label="Speech-to-text",
            included_by_default=True,
            available=stt_available,
            active=stt_active,
            managed_by_clara=stt_managed,
        ...
        "browser": ClaraFeatureState(
            key="browser",
            label="Browser automation",
            included_by_default=True,
            available=browser_available,
            active=browser_active,
            managed_by_clara=browser_managed,
        ...
        "modal": ClaraFeatureState(
            key="modal",
            label="Modal execution",
            included_by_default=False,
            available=modal_available,
            active=modal_active,
            managed_by_clara=modal_managed,
    ...
    return ClaraSubscriptionFeatures(
        subscribed=subscribed,
        clara_auth_present=clara_auth_present,
        provider_is_clara=provider_is_clara,
        features=features,
...
def apply_clara_managed_defaults(
    ...
    *,
    ...
    if not (
        ...
        return set()
    if not features.provider_is_clara:
        return set()
    ...
    if "browser" in selected_toolsets and not features.browser.explicit_configured and not (
        ...
        browser_cfg["cloud_provider"] = "clara"
...
# ``get_clara_subscription_features`` reads (web uses "backend", browser
...
def get_gateway_eligible_tools(
    ...
    *,
    ...
    - unconfigured: tools with no direct credentials and no explicit
      non-clara selection (easy switch, safe to pre-check)
    - has_direct: tools where the user has their own API keys
    - explicit_configured: tools with an explicit non-clara selection stored
    ...
    All lists are empty when the user is not a paid Clara subscriber or
    is not using Clara as their provider.
    """
    ...
    # Quick provider check without the heavy get_clara_subscription_features call
    model_cfg = config.get("model")
    if not isinstance(model_cfg, dict) or str(model_cfg.get("provider") or "").strip().lower() != "clara":
        return [], [], [], []

    direct = _get_gateway_direct_credentials()

    # Check which tools the user has explicitly opted into the gateway for.
    # This is distinct from managed_by_clara which fires implicitly when
    ...
    for key in _ALL_GATEWAY_KEYS:
        ...
        elif selected is not None and selected != "clara":
            # An explicit non-clara selection (e.g. a keyless local backend
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    # already_managed, just for a non-clara vendor.
    ...
    try:
        from clara_cli.setup import prompt_checklist
    except Exception:
        return set()
    ...
    try:
        account_info = get_clara_portal_account_info(force_fresh=False)
    except Exception:
        account_info = None
    pool_only = bool(
        account_info
    ...
    source_label = "free tool pool" if pool_only else "Clara subscription"
    ...
    # pre-checked checklist re-fired on every Clara model swap.
    ...
    if pool_only:
        title = "Your free Clara tool pool — pick the tools to enable:"
    else:
        title = (
    ...
    # so the next Clara model swap doesn't pre-check it again. Cancel paths
...
# ---------------------------------------------------------------------------
# Inline Clara Portal login for the Tool Gateway picker (`clara tools`)
# ---------------------------------------------------------------------------


def ensure_clara_portal_access(
    *,
    capability: str = "the Clara Tool Gateway",
    coverage_category: Optional[str] = None,
) -> bool:
    """Make sure the user is entitled to the Clara Tool Gateway, logging in if
    needed.

    Used by ``clara tools`` when a user selects a Clara-managed Tool Gateway
    backend (e.g. "Firecrawl (Clara Portal)").  Unlike ``clara model``'s Clara
    login, this:
    ...
    It only performs the Clara Portal device-code OAuth (when the user isn't
    ...
    if info is None or not info.logged_in:
        if not _run_clara_portal_login_only(capability=capability):
            return False
        try:
            info = get_clara_portal_account_info(force_fresh=True)
        except Exception:
...
def _run_clara_portal_login_only(*, capability: str) -> bool:
    """Run the Clara Portal device-code OAuth and persist credentials only.
    ...
    try:
        from clara_cli.auth import (
            _auth_store_lock,
            _load_auth_store,
            _clara_device_code_login,
            _read_shared_clara_state,
            _save_auth_store,
            _save_provider_state,
            _sync_clara_pool_from_auth_store,
            _try_import_shared_clara_state,
            _write_shared_clara_state,
        )
    except Exception as exc:  # pragma: no cover - defensive
        print(f"  Could not start Clara Portal login: {exc}")
        return False

    print()
    print(f"  {capability} requires a Clara Portal login.")
    try:
        proceed = input("  Log in to Clara Portal now? [Y/n]: ").strip().lower()
    except (EOFError, KeyboardInterrupt):
        print()
        return False
    if proceed not in {"", "y", "yes"}:
        print("  Skipped Clara Portal login.")
        return False

    try:
        # Snapshot the active_provider so a tool-config login never silently
        # switches the user's inference provider to Clara.
        ...
        if shared:
            try:
                do_import = input(
                    "  Found existing Clara OAuth credentials. Import them? [Y/n]: "
                ).strip().lower()
            except (EOFError, KeyboardInterrupt):
                do_import = "y"
            if do_import in {"", "y", "yes"}:
                auth_state = _try_import_shared_clara_state(timeout_seconds=15.0)
        ...
        _sync_clara_pool_from_auth_store()
        print("  Clara Portal login successful.")
        return True
    except KeyboardInterrupt:
        print("\n  Login cancelled.")
        return False
    except SystemExit:
        # _clara_device_code_login raises SystemExit on subscription_required;
        # it already printed billing guidance.
        return False
    except Exception as exc:
        print(f"  Clara Portal login failed: {exc}")
        return False
