"""Unified provider-credential lifecycle across every store Clara reads.

A provider API key can live in up to THREE stores at once:

    1. ``~/.clara/.env``                     — the canonical secret store
    2. ``~/.clara/auth.json`` →
       ``credential_pool.<provider>[*]``      — env-seeded pool entries
       (``source == "env:<VAR>"``) persisted by the pool loader
    3. ``~/.clara/config.yaml``              — inline mirrors written by the
...
def purge_env_credential_references(
    ...
    # Make the removal sticky the same way `clara auth remove` does: a
...
def save_provider_env_credential(env_var: str, value: str) -> Dict[str, Any]:
    ...
    re-add through the UI behaves like ``clara auth add``.
    ...
    ran ``clara auth add <provider> --type api-key`` separately. This makes
    the Desktop save's effect on disk match what ``clara auth add`` does.
    """
    from clara_cli.config import load_env, save_env_value
    ...
    # that reads this var.
    try:
        from clara_cli.auth import unsuppress_credential_source
...
def remove_provider_env_credential(env_var: str) -> Dict[str, Any]:
    ...
    """
    from clara_cli.config import load_env, remove_env_value
