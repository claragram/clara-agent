"""``clara gateway enroll`` — enroll a self-hosted gateway with a relay connector.
...
``docs/connector-gateway-auth-design.md``:
  ...
  1. Resolve a fresh Clara Portal access token from the existing login
     (``~/.clara/auth.json``) — the same path ``clara dashboard register``
     uses (``resolve_clara_access_token``). This proves *which Clara org (tenant)*
  ...
  4. Persist ``GATEWAY_RELAY_ID`` / ``GATEWAY_RELAY_SECRET`` /
     ...
     ``~/.clara/.env``. The per-gateway secret authenticates the WS upgrade;
...
def _resolve_identity_token() -> str:
    """Resolve the caller-identity bearer token (generic-OIDC or Clara Portal).
    ...
    self-hosted-IdP path; otherwise Clara Portal). Raises RuntimeError on failure.
    """
...
def _post_enroll(
    *,
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                "Connector rejected the caller identity (401). Your Clara Portal "
                "token could not be verified — try `clara auth add clara` and retry."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def cmd_gateway_enroll(args) -> None:
    ...
    from clara_cli.config import is_managed, save_env_value
    ...
    if is_managed():
        print(
            "✗ `clara gateway enroll` is not available in a managed/hosted install.\n"
    ...
    #    gapped / self-hosted-IdP, NO Clara Portal); otherwise the Clara Portal token.
    try:
    ...
    #    save_env_value writes them to ~/.clara/.env (0600 dir) and never logs.
    to_write = {
    ...
    # process is launched under (load_clara_dotenv exports that .env), so
...
def _warn_if_secondary_multiplex_profile() -> bool:
    ...
    in clara_cli/gateway.py. Best-effort: any failure to determine the
    ...
    try:
        ...
        default_root = Path(get_default_clara_root()).resolve()
        home = Path(get_clara_home()).resolve()
        try:
