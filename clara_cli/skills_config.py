"""
Skills configuration for Clara Agent.
`clara skills` enters this module.

Toggle individual skills or categories on/off, globally or per-platform.
Config stored in ~/.clara/config.yaml under:
...
from clara_cli.config import cfg_get, load_config, save_config
from clara_cli.colors import Colors, color
from clara_cli.platforms import PLATFORMS as _PLATFORMS
...
def save_disabled_skills(config: dict, disabled: Set[str], platform: Optional[str] = None):
    ...
    Essential skills (e.g. ``clara-agent``) are silently dropped from the
