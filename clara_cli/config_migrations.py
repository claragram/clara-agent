...
``clara_cli.config.migrate_config``. Each step is a function
...
``clara_cli.config`` imports :func:`run_migrations` lazily (inside
``migrate_config``), and every step function here resolves its helpers
(``read_raw_config``, ``_persist_migration``, ``get_env_value``, …) lazily
through the live ``clara_cli.config`` module object at call time via
:func:`_cfg`. There is deliberately NO module-level import of
``clara_cli.config`` here, so no circular import can form — and, just as
importantly, tests that monkeypatch helpers on ``clara_cli.config`` (e.g.
``patch("clara_cli.config.read_raw_config", ...)``) keep working, because
...
#: CLARA_TOOL_PROGRESS*) forever is not worth it. Below-floor configs are
...
def support_floor_message() -> str:
    ...
    return (
        ...
        f"{display_clara_home()}/config.yaml and run `clara setup` to "
...
def _cfg():
    """Return the live ``clara_cli.config`` module (lazy, cycle-free)."""
    from clara_cli import config
...
def _migrate_to_21(results: Dict[str, Any], quiet: bool) -> None:
    ...
    read_raw_config = _c.read_raw_config
    _persist_migration = _c._persist_migration
    get_clara_home = _c.get_clara_home
    ...
    if "enabled" not in plugins_cfg:
        ...
        # Scan ``$CLARA_HOME/plugins/`` for currently installed user plugins.
        ...
        if not quiet:
            if grandfathered:
                ...
                print(
                    ...
                    "Use `clara plugins enable <name>` to activate."
                )
...
def _migrate_to_23(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # settings in their `config.yaml`, and `clara curator status` has no
    ...
    #   3. Creates `~/.clara/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_clara_home() — old profiles that predate this
    ...
    try:
        curator_dir = get_clara_home() / "logs" / "curator"
        ...
        if added_curator:
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_curator)}) — edit via `clara config set`"
                )
        if added_aux:
            results["config_added"].append(
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_aux)}) — edit via `clara config set`"
                )
...
# behavior set it to true explicitly via `clara config set`.
...
def _migrate_to_38(results: Dict[str, Any], quiet: bool) -> None:
    ...
    from clara_cli.relay_plugin_cutover import legacy_relay_plugin_keys
...
def _migrate_to_39(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # period that has since ended server-side, leaving every Clara-signed-in
    ...
    # provider surface (`video_generate`, `clara tools` → Video Generation).
    ...
    if changed:
        ...
        if not quiet:
            print(
                ...
                "lists — video generation now lives under `clara tools` → "
                "Video Generation (Clara Subscription or FAL)."
            )
