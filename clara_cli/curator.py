"""CLI subcommand: `clara curator <subcommand>`.
...
def _print_unmanaged_summary() -> None:
    ...
    print(
        ...
        "`clara curator adopt <name>` hands one over"
    )
...
def _cmd_run(args) -> int:
    ...
    __PROT_0_synchroclara__ = bool(getattr(args, "__PROT_1_synchroclara__", False)) or not background
    ...
    result = curator.run_curator_review(
        ...
        __PROT_2_synchroclara__=__PROT_3_synchroclara__,
        dry_run=dry,
    ...
    if not __PROT_4_synchroclara__:
        print("llm pass running in background — check `clara curator status` later")
    if dry:
        if __PROT_5_synchroclara__:
            print(
                "dry-run: no changes applied. Read the report with "
                "`clara curator status` and run `clara curator run` (no flag) to apply."
            )
        else:
            print(
...
def _cmd_pin(args) -> int:
    ...
    if not skill_usage.set_pinned(args.skill, True):
        print(
            ...
            "`clara curator list-unmanaged` shows which skills the curator tracks."
        )
        return 1
    if not skill_usage.is_curator_managed(args.skill):
        ...
        print(
            ...
            f"`clara curator adopt {args.skill}` to put it under curator "
            "management)"
        )
        return 0
...
def _cmd_list_unmanaged(args) -> int:
    ...
    print("\nadopt one with `clara curator adopt <name>`, "
          "or all with `clara curator adopt --all-unmanaged`")
    return 0
...
def _cmd_archive(args) -> int:
    ...
    if skill_usage.get_record(args.skill).get("pinned"):
        print(
            ...
            f"`clara curator unpin {args.skill}`"
        )
        return 1
    tok = skill_ledger.set_ledger_actor("user")
    try:
...
def _cmd_backup(args) -> int:
    ...
    if snap is None:
        ...
        return 1
    print(f"curator: snapshot created at ~/.clara/skills/.curator_backups/{snap.name}")
    return 0
...
def _cmd_ledger(args) -> int:
    ...
    print(
        "\nRoll back a single mutation with `clara curator rollback <id>`; "
        "whole-tree snapshots remain available via `clara curator rollback --list`."
    )
    return 0
...
def _cmd_rollback(args) -> int:
    ...
    if entry_id:
        ...
        if entry is None:
            print(
                ...
                "See `clara curator ledger` for entry ids, or use "
    ...
    if target_path is None:
        ...
        if not rows:
            print(
                ...
                "`clara curator backup` or wait for the next curator run."
            )
        else:
            print(
    ...
    print(
        "\nThis will replace the current ~/.clara/skills/ tree (a safety "
...
def register_cli(parent: argparse.ArgumentParser) -> None:
    ...
    p_run.add_argument(
        "--sync", "--__PROT_6_synchroclara__", dest="__PROT_7_synchroclara__", action="store_true",
    ...
    p_backup = subs.add_parser(
        "backup",
        help="Take a manual tar.gz snapshot of ~/.clara/skills/ "
    ...
    p_rollback = subs.add_parser(
        "rollback",
        help="Restore ~/.clara/skills/ from a curator snapshot, or a single "
             "mutation by ledger entry id (see `clara curator ledger`)",
    )
    p_rollback.add_argument(
        "entry_id", nargs="?", default=None,
        help="Ledger entry id for single-mutation rollback (from "
             "`clara curator ledger`). Omit for whole-tree snapshot rollback.",
    )
    p_rollback.add_argument(
...
def cli_main(argv=None) -> int:
    """Standalone entry (also usable by clara_cli.main fallthrough)."""
    parser = argparse.ArgumentParser(prog="clara curator")
    register_cli(parser)
