"""
Dump command for clara CLI.

Outputs a compact, plain-text summary of the user's Clara setup
...
from clara_cli.config import get_clara_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.clara/.env.
    ...
    exports. ``clara debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.clara_build_sha`` by the Dockerfile's
    ``CLARA_GIT_SHA`` build-arg (see ``clara_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``clara dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from clara_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(clara_home: Path) -> int:
...
def _cron_summary(clara_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = clara_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"clara_home:      {display_clara_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.clara/.env: a managed
        ...
        # A credential added via `clara auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `clara auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["clara-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(clara_home)}")
    lines.append(f"  skills:             {_count_skills(clara_home)}")
