"""`clara checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.clara/checkpoints/``.  Actions:

    clara checkpoints               # same as `status`
    clara checkpoints status        # total size, project count, breakdown
    clara checkpoints list          # per-project checkpoint counts + workdir
    clara checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    clara checkpoints clear [-f]    # nuke the entire base (asks first)
    clara checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    clara checkpoints
    clara checkpoints prune --retention-days 3 --max-size-mb 200
    clara checkpoints clear -f
...
from clara_cli.sizefmt import format_bytes as _fmt_bytes
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: clara checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``clara checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `clara checkpoints` → status
