"""Cross-process mutual exclusion for in-flight Clara updates.

Three different surfaces can start an update of the same install tree:

* ``clara update`` from a terminal,
* the dashboard's Update button (``POST /api/clara/update`` →
  ``_spawn_clara_action(["update"])``, detached),
* the desktop's Update button, which hands off to the Tauri
  ``clara-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``clara update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <CLARA_HOME>/.clara-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``clara update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("Clara is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``clara-setup`` binary under ``~/.clara`` is only refreshed by a full
  installer run (``copy_self_to_clara_home`` deliberately no-ops during
  ...
  ancestry check those users get exit 2 ("Clara is still running") on every
  GUI update forever, with no Clara process actually running.
"""
...
MARKER_NAME = ".clara-update-in-progress"

# Set by an orchestrating updater (the Tauri `clara-setup --update` flow) to
# its own pid before spawning `clara update` as a child stage. The parent
...
HANDOFF_PID_ENV = "CLARA_UPDATE_HANDOFF_PID"
...
# to show "Clara is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Clara home (never the context-local profile override):
    the Rust updater resolves ``$CLARA_HOME`` or the platform default, and the
    ...
    return get_process_clara_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``clara update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``clara-setup`` binaries
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Clara update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `clara update` as a stage): we run under ITS claim
