"""``clara portal`` — the human-readable entry point for Clara Portal.

Running ``clara portal`` with no subcommand performs the one-shot Portal
onboarding: OAuth login, pick a Clara model, switch the inference provider to
Clara, and offer to enable the Tool Gateway. It is the friendly alias for
``clara auth add clara --type oauth`` (which still works), is identical to
``clara setup --portal``, and runs the same Clara flow as the first-time quick
setup.

Subcommands:
  (none)   Log in to Clara Portal + set it up (one-shot onboarding).
...
already in ``clara auth`` or ``clara tools``. It's the onboarding + discovery
...
DEFAULT_PORTAL_URL = "https://portal.claraprise.com"
SUBSCRIPTION_URL = "https://portal.claraprise.com/manage-subscription"
DOCS_URL = "https://agent.claraprise.com/docs/user-guide/features/tool-gateway"
...
def _cmd_status(args) -> int:
    ...
    print()
    print(color("  Clara Portal", Colors.MAGENTA))
    ...
    if logged_in:
        ...
        print("  Login:   clara portal")

    # Provider selection (independent of auth)
    model_cfg = config.get("model") if isinstance(config.get("model"), dict) else {}
    provider = str(model_cfg.get("provider") or "").strip().lower()
    if provider == "clara":
        print(f"  Model:   {color('✓ using Clara as inference provider', Colors.GREEN)}")
    elif provider:
        print(f"  Model:   currently {provider} (switch with `clara model`)")
    ...
    for feat in features.items():
        if feat.managed_by_clara:
            state = color("via Clara Portal", Colors.GREEN)
...
def _cmd_tools(args) -> int:
    ...
    if not features.clara_auth_present:
        print(color("  Not logged into Clara Portal — sign in with `clara portal`.", Colors.YELLOW))
        print()
...
def _cmd_login(args) -> int:
    """Run the one-shot Clara Portal onboarding (login + model + provider + tools).

    This is the human-readable front door for `clara auth add clara --type
    oauth`. It reuses the exact wiring behind `clara setup --portal` (which in
    turn runs the same Clara flow as the first-time quick setup), so the
    commands stay in lockstep: device-code login, pick a Clara model, switch the
    inference provider to Clara, then offer the Tool Gateway opt-in.
    """
    from clara_cli.setup import _run_portal_one_shot
...
def portal_command(args) -> int:
    """Top-level dispatch for `clara portal <subcommand>`."""
    sub = getattr(args, "portal_command", None)
    if sub in {None, "", "login"}:
        # Default to the one-shot onboarding — `clara portal` is the
        # human-readable alias for `clara auth add clara --type oauth` /
        # `clara setup --portal`.
    ...
    print("Run `clara portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `clara portal` on the given argparse subparsers object."""
    portal_parser = subparsers.add_parser(
        "portal",
        help="Set up Clara Portal (login, model pick, Tool Gateway); see also `portal info`",
        description=(
            "Run `clara portal` with no subcommand to log in to Clara Portal "
            "and set it up — pick a model, set Clara as your provider, and offer "
            "the Tool Gateway (the human-readable alias for `clara auth add "
            "clara --type oauth`, identical to `clara setup --portal`). "
    ...
    portal_sub.add_parser(
        "login",
        help="Log in to Clara Portal + set it up (default; one-shot onboarding)",
    )
    portal_sub.add_parser(
        "info",
    ...
    portal_sub.add_parser(
        "tools",
        help="List Tool Gateway tools and which are routed via Clara",
    )
