...
A system-level directory (default ``/etc/clara``, root-owned and not
user-writable) supplies ``config.yaml`` and ``.env`` values that WIN over the
user's ``~/.clara/config.yaml`` and ``~/.clara/.env`` on a per-leaf-key basis.

This is DISTINCT from ``clara_cli.config.is_managed()`` / ``CLARA_MANAGED``,
...
_DEFAULT_MANAGED_DIR = Path("/etc/clara")
...
def _under_pytest() -> bool:
    ...
    Used to ignore the system default ``/etc/clara`` during tests so a real
    managed scope on a developer/CI box can't leak policy into the suite. Tests
    that exercise managed scope set ``CLARA_MANAGED_DIR`` explicitly, which is
...
def get_managed_dir() -> Optional[Path]:
    ...
    Resolution (highest priority first):
      1. ``$CLARA_MANAGED_DIR`` — deployment/bootstrap path override (IT-only;
         never persisted to any .env). Honored only when set to a non-empty value
         AND the directory exists.
      2. ``/etc/clara`` — POSIX default, when it exists. Ignored under pytest so
    ...
    """
    override = os.environ.get("CLARA_MANAGED_DIR", "").strip()
    if override:
        p = Path(override)
...
def apply_managed_overlay(config: dict) -> dict:
    ...
    (rather than going through clara_cli.config.load_config) to honor managed
    scope. Mirrors clara_cli.config._load_config_impl's managed merge exactly:
    ...
    try:
        ...
        from clara_cli.config import _deep_merge, _expand_env_vars, _normalize_root_model_keys
