"""Install and remove the Linux desktop entry (``clara.desktop``).

``clara desktop`` builds and launches the Electron app. On Linux, a
...
DESKTOP_ENTRY_NAME = "clara.desktop"
...
def desktop_entry_path() -> Path:
    """Where the ``clara.desktop`` entry lives."""
...
def _can_import_clara_cli(interpreter: Path) -> bool:
    """Whether *interpreter* can import ``clara_cli.main`` unaided.
...
def _running_interpreter_fallback() -> str:
    ...
    The RUNNING interpreter by definition has ``clara_cli`` importable
...
def resolve_exec_command(project_root: Optional[Path] = None) -> str:
    """Build the absolute ``Exec=`` command line for ``clara desktop``.

    Prefer the real ``clara`` executable (argv[0] or PATH). When Clara
    ...
    bytes. ``resolve_clara_bin()`` prefers ``sys.argv[0]``, which differs
    ...
    """
    from clara_cli.relaunch import resolve_clara_bin

    bin_path = _resolve_clara_bin_for_desktop_entry(
        resolve_clara_bin, checkout_root=project_root
    )
    interpreter = _running_interpreter()
    if not _can_import_clara_cli(Path(interpreter)):
        # The candidate interpreter cannot actually import clara_cli.main
    ...
    if bin_path:
        ...
        if _needs_interpreter(resolved):
            ...
            # a NON-venv interpreter (e.g. the repo's `clara` script with
            ...
            # sys.executable is the interpreter actually running Clara (the
...
def _resolve_clara_bin_for_desktop_entry(
    resolve_fn=None,
    checkout_root: Optional[Path] = None,
) -> Optional[str]:
    """Resolve the launcher binary for the persisted ``.desktop`` entry.

    Wraps :func:`clara_cli.relaunch.resolve_clara_bin` with one
    desktop-entry-specific rule: an ``argv[0]`` that points inside this
    checkout is a launch-context artifact (the repo ``clara`` script the
    ...
    if resolve_fn is None:
        from clara_cli.relaunch import resolve_clara_bin as resolve_fn
    ...
    def _inside_checkout(candidate: str) -> bool:
        try:
            ...
            return False
        # The repo `clara` script and anything else shipped in the tree is
        ...
        for root in {checkout_root, resolved_root}:
            if root is not None and (path == root or root in path.parents):
                return True
        # The `python -m clara_cli.main` relaunch context surfaces the
    ...
    # interpreter is an external launcher (e.g. /opt/.../bin/clara from
    ...
    if rerouted is None and primary:
        # argv[0] was checkout-internal AND PATH had no `clara` — common
...
def _wrapper_targets_checkout(wrapper: Path, checkout_root: Path) -> bool:
    ...
    ``<checkout>/venv/bin/python <checkout>/clara``; a venv console
    ...
    for root in roots:
        ...
        # <root>/clara …): a path-continuation boundary is also a match.
        if root + "/" in text:
            return True
    return False
...
def _known_wrapper_candidates():
    ...
    (``$PREFIX/bin``). The wrapper is always named ``clara``.
    """
    candidates = []
    home = Path.home()
    prefix = os.environ.get("PREFIX")
    if prefix:
        candidates.append(Path(prefix) / "bin" / "clara")
    if hasattr(os, "geteuid") and os.geteuid() == 0:
        candidates.append(Path("/usr/local/bin/clara"))
    candidates.append(home / ".local" / "bin" / "clara")
    return candidates


def _project_root() -> Path:
    """This file lives at ``<checkout>/clara_cli/linux_desktop_entry.py``.
...
def _needs_interpreter(bin_path: Path) -> bool:
    ...
    ``sys.executable`` to see Clara' venv (rather than its own shebang)."""
    try:
...
def render_desktop_entry(exec_command: str, icon: str) -> str:
    return (
        ...
        "Type=Application\n"
        "Name=Clara\n"
        "GenericName=Clara Desktop\n"
        "Comment=Launch Clara Desktop\n"
        f"Exec={exec_command}\n"
        f"Icon={icon}\n"
        "Terminal=false\n"
        "Categories=Utility;\n"
        "StartupNotify=true\n"
        "StartupWMClass=Clara\n"
    )
...
def _hicolor_icon_dest(subdir: str) -> Path:
    return _xdg_data_home() / "icons" / "hicolor" / subdir / "apps" / "clara.png"
...
def _install_icon_to_hicolor(icon: Path) -> bool:
    ...
    The freedesktop icon lookup finds an installed ``apps/clara.png``
    by the unqualified name ``clara``, so the entry can reference the
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    """Write (or refresh) the Clara desktop entry. Return its path.
    ...
    icon_value = str(icon) if icon.is_file() else "clara"
