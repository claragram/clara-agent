"""``clara worktree`` — audit and reclaim accumulated git worktrees/branches.

Attended counterpart of the silent startup pruner (see
``clara_cli/worktree_gc.py`` for the policy and shared invariants). Usage:

    clara worktree list             # audit: verdict + reason per tree
    clara worktree prune            # reap safe trees + merged branches
    clara worktree prune --dry-run  # show the plan, change nothing
    clara worktree prune --trees-only / --branches-only
"""
...
def cmd_worktree(args) -> int:
    ...
    if action == "list":
        ...
        print(
            ...
            f"{_fmt_size(reapable_mb)} reclaimable now via `clara worktree prune`."
        )
