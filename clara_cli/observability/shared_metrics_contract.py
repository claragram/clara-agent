"""Bounded product contract for the first Clara shared-metrics slice."""
...
SCHEMA_KEY = "clara.metrics.schema_version"
SCHEMA_VERSION = "clara.metrics.event.v2"
MODEL_CALL_SCOPE = "clara.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "clara.task_run"
TOOL_CALL_SCOPE = "clara.tool_call"
CLIENT_ACTIVE_MARK = "clara.client.active"
TOOL_APPROVAL_MARK = "clara.tool_approval"
SKILL_LIFECYCLE_MARK = "clara.skill.lifecycle"
SKILL_LOAD_MARK = "clara.skill.load"
SUBSCRIBER_NAME = "clara.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "clara.client.active"
LEGACY_MODEL_CALL_METRIC = "clara.model_call.count"
MODEL_ROUTE_METRIC = "clara.model_route.count"
TASK_STARTED_METRIC = "clara.task_run.started"
TASK_FINISHED_METRIC = "clara.task_run.finished"
TOOL_CALL_METRIC = "clara.tool_call.count"
TOOL_APPROVAL_METRIC = "clara.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "clara.skill.lifecycle.count"
SKILL_LOAD_METRIC = "clara.skill.load.count"
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted Clara installation method."""
    normalized = str(value or "").strip().lower()
    if normalized == "nix":
        return "nixos"
...
def client_resource(
    ...
    *,
    ...
    normalized_version = str(clara_version or "").strip()
    ...
    return {
        ...
        "clara_version": normalized_version,
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("clara_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its Clara logical scope."""
    metadata = getattr(event, "metadata", None)
    if (
    ...
    if relay_metadata - {"otel.status_code"} or metadata.get(
        ...
        return None
    call_role = metadata.get("clara.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from clara_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Clara terminal state to bounded task outcome dimensions."""
...
def tool_category(kwargs: dict[str, Any]) -> str:
    """Map Clara registry toolset metadata to a low-cardinality category."""
    ...
    if toolset.startswith(
        ("discord", "email", "feishu", "clara-yuanbao", "slack", "sms")
    ):
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal Clara tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to Clara."""
