"""Durable aggregation and local export for Clara shared metrics."""
...
_PACKAGE_SCHEMA_VERSION = "clara.shared_metrics.v2"
...
class SharedMetricsStore:
    ...
    def __init__(
        ...
        root = get_clara_home() / "telemetry" / "shared_metrics"
        ...
        return [
            {
                ...
                "resource": {
                    "clara_version": row["clara_version"],
    ...
    def _ensure_schema(self) -> None:
        with self._connection(busy_timeout_ms=_SCHEMA_BUSY_TIMEOUT_MS) as connection:
            # Serialize first-run creation and upgrades across Clara processes.
            with write_txn(connection):
    ...
    def _add_send_columns(connection: sqlite3.Connection) -> None:
        ...
        branch, so bumping would make an older Clara — a second profile on an
    ...
    def _create_package_in_transaction(
        ...
        resource = {
            "clara_version": period_row["clara_version"],
