"""Direct NeMo Relay integration for Clara shared client metrics."""
...
class _Runtime:
    """Own shared-metrics state layered on the Clara core Relay host."""

    def __init__(self, host: relay_runtime.RelayRuntime | None = None) -> None:
        resolved_host = host or relay_runtime.get_runtime()
        if resolved_host is None:
            raise RuntimeError("Clara core Relay runtime is unavailable")
    ...
    def start_task(self, event: dict[str, Any]) -> _TaskRun | None:
        """Open one Relay function scope for a Clara task run."""
        ...
        with session.lock:
            ...
            if existing is not None:
                ...
                if task is not None:
                    ...
                    # physical attempt. Provider fallback resets Clara's
            ...
            if task is not None:
                ...
                if retry_ordinal is not None and retry_ordinal > 0:
                    # A real Clara retry can advance api_request_id while
    ...
    def finish_task(self, event: dict[str, Any]) -> None:
        ...
        if finished:
            try:
                ...
                logger.warning(
                    "Clara shared-metrics task flush failed",
                    exc_info=True,
                )
            else:
                self._export()
    ...
    def close_session(self, event: dict[str, Any]) -> None:
        ...
        if failures:
            logger.warning(
                "Clara shared-metrics session %s closed with errors: %s",
                session.session_id,
                "; ".join(failures),
            )
    ...
    def shutdown(self) -> None:
        ...
        try:
            ...
            logger.warning(
                "Clara shared-metrics shutdown flush failed",
                exc_info=True,
            )
        else:
            self._export()
    ...
    def _finish_tool_call(
        ...
        try:
            ...
            logger.warning(
                "Clara shared-metrics tool call close failed",
                exc_info=True,
            )
    ...
    def _finish_model_call(
        ...
        try:
            ...
            logger.warning(
                "Clara shared-metrics model call close failed", exc_info=True
            )
    ...
    def _finish_task(
        ...
        try:
            ...
            logger.warning("Clara shared-metrics task close failed", exc_info=True)
        finally:
    ...
    def _send_exported_packages(self) -> None:
        from clara_cli.observability.shared_metrics_send_config import (
            resolve_send_config,
        )
        ...
        with self._send_lock:
            ...
            thread = threading.Thread(
                ...
                name="clara-shared-metrics-send",
                daemon=True,
            )
    ...
    def _safe(callback: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
        try:
            ...
            logger.warning("Clara shared metrics operation failed", exc_info=True)
            return None


def enabled() -> bool:
    """Return the shared-metrics policy for the active Clara profile."""
    ...
    try:
        ...
        logger.debug("Unable to read Clara shared-metrics policy", exc_info=True)
        value = False
    else:
...
def _reconcile_send_consent_once() -> None:
    ...
    and creating ``~/.clara/telemetry`` for every fully-disabled user would
    ...
    try:
        ...
        # fully-disabled user was getting a ~/.clara/telemetry directory.
        default_path = (
            get_clara_home() / "telemetry" / "shared_metrics" / "metrics.sqlite3"
        )
...
def observe_lifecycle(hook_name: str, **kwargs: Any) -> None:
    """Project one Clara lifecycle event into the core Relay integration."""
    ...
    try:
        ...
        logger.warning(
            "Clara shared metrics hook failed: %s", hook_name, exc_info=True
        )


def _with_runtime_toolset(event: dict[str, Any]) -> dict[str, Any]:
    """Attach the toolset already declared by Clara's runtime registry."""
...
def start_task_run(
    *,
    ...
    """Start task metrics at the outer Clara execution boundary."""
    if not enabled():
        return
    ...
    *,
    ...
    with _RUNTIME_LOCK:
        ...
        try:
            ...
            logger.warning("Clara shared metrics initialization failed", exc_info=True)
