"""
Configuration management for Clara Agent.

Config files are stored in ~/.clara/ for easy access:
- ~/.clara/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.clara/.env         - API keys and secrets

This module provides:
- clara config          - Show current configuration
- clara config edit     - Open config in editor
- clara config get      - Print a resolved configuration value
- clara config set      - Set a specific value
- clara config unset    - Remove a user configuration value
- clara config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``clara config set`` (which rewrites
    ...
    ``config.yaml`` in place: clara never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.clara/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``clara logs``
    ...
    if fallback == "last-known-good":
        ...
        msg = (
            ...
            f"Fix the YAML (clara config edit) and retry."
        )
    else:
        msg = (
            f"Failed to parse {config_path}: {exc}. "
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  clara config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
def get_active_config_parse_failure() -> Optional[str]:
    ...
    Consumers: ``clara_cli.auth.resolve_provider`` uses this to refuse
...
#   the next ``subprocess.run([...])`` Clara makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Clara itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``clara update``, the TUI build.
...
#   on every plugin install / ``clara update``.
...
# * ``CLARA_HOME`` / ``CLARA_PROFILE`` / ``CLARA_CONFIG`` /
#   ``CLARA_ENV`` — Clara runtime location flags. Writing these into
...
#
# IMPORTANT: ``CLARA_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (CLARA_LANGFUSE_PUBLIC_KEY,
# CLARA_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    "PYTHONEXECUTABLE", "PYTHONCLARAERSITE",
    # Node
    ...
    # Clara runtime location — never via dashboard env writer.
    # NOT a CLARA_* blanket: integration credentials (CLARA_GEMINI_*,
    # CLARA_LANGFUSE_*, CLARA_SPOTIFY_*, ...) ARE allowed.
    "CLARA_HOME", "CLARA_PROFILE", "CLARA_CONFIG", "CLARA_ENV",
    "CLARA_CONFIG_PATH", "CLARA_ENV_PATH",
    ...
    "CLARA_COPILOT_ACP_COMMAND", "CLARA_COPILOT_ACP_ARGS",
    # Clara security policy / approval-routing context. These remain available
    ...
    "CLARA_YOLO_MODE", "CLARA_ACCEPT_HOOKS", "CLARA_REDACT_SECRETS",
    "CLARA_INTERACTIVE", "CLARA_EXEC_ASK", "CLARA_GATEWAY_SESSION",
    "CLARA_CRON_SESSION", "CLARA_SINGLE_QUERY_SESSION",
    "CLARA_SESSION_KEY", "CLARA_SESSION_PLATFORM",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if _env_var_policy_name(key) in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Clara runtime location "
            "and security policy (CLARA_HOME, CLARA_YOLO_MODE, ...) "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # CLARA_TOOL_PROGRESS_MODE is deprecated (replaced by display.tool_progress
    ...
    # Activation is via plugins.enabled (opt-in through `clara plugins enable
    # observability/langfuse` or `clara tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "CLARA_LANGFUSE_ENV",
    "CLARA_LANGFUSE_RELEASE",
    "CLARA_LANGFUSE_SAMPLE_RATE",
    "CLARA_LANGFUSE_MAX_CHARS",
    "CLARA_LANGFUSE_CAPTURE",
    "CLARA_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so different
    # profiles can use different ACP backends without cross-leak.
    "CLARA_ACP_AUTH_METHOD",
    "CLARA_ACP_AUTO_APPROVE",
    "CLARA_COPILOT_ACP_COMMAND",
    "CLARA_COPILOT_ACP_ARGS",
    "COPILOT_CLI_PATH",
...
from clara_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
# from `nix run` / `nix profile install` (which don't set CLARA_MANAGED).
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("CLARA_MANAGED", "").strip()
    marker = None
    if raw:
        marker = raw.lower()
    else:
        managed_marker = get_clara_home() / ".managed"
        # An interactive shell reads the marker, because it does not see the
        # CLARA_MANAGED variable of the service. A marker with content
...
def is_managed() -> bool:
    """Check if Clara is running in package-manager-managed mode.

    Two signals: the CLARA_MANAGED env var (set by the systemd service),
    or a .managed marker file in CLARA_HOME (set by the NixOS activation
...
_NIX_UPDATE_MSG = (
    "Update Clara through the Nix source that installed it "
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``clara_cli/`` — i.e. the git checkout for source
    installs, ``/opt/clara`` inside the published image. It is a property of
    the running interpreter, NOT of ``$CLARA_HOME``, which is why a
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Clara was installed: 'apt', 'docker', 'nix', 'nixos',
    ...
    2. Legacy home-scoped stamp ``$CLARA_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. CLARA_MANAGED env / .managed marker (NixOS managed mode)
    4. /nix/store/ path detection -> 'nix' (nix run / nix profile install)
    5. .git directory presence -> 'git'
    6. Fallback -> 'unknown'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.clara``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$CLARA_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.clara:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$CLARA_HOME``, a
    ...
    ``docker`` and ``clara update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``workprise/clara-agent`` image bakes a ``docker``
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $CLARA_HOME.
    try:
    ...
    #    host install whose shared $CLARA_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``clara update``.
    try:
        method = (
            (get_clara_home() / ".install_method")
    ...
    # detect Nix installs that don't set CLARA_MANAGED (e.g. ``nix run``,
...
def _running_in_container() -> bool:
    """Thin wrapper around ``clara_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$CLARA_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/clara`` in the published image, which instead bakes the stamp at
...
# Long-form text for ``clara update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# clara_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Clara, it doesn't update
...
Clara Agent runs as a published image (workprise/clara-agent), not a
...
Verify the new version after restart:
  docker run --rm workprise/clara-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/workprise/clara-agent/tags
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``clara update`` inside Docker.
...
def format_managed_message(action: str = "modify this Clara installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    return (
        f"Cannot {action}: this Clara installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Clara."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from CLARA_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, clara_bin
    or None if container mode is not active, we're already inside the
    container, or CLARA_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("CLARA_DEV") == "1":
        return None
    ...
    container_mode_file = get_clara_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "clara-agent")
    exec_user = info.get("exec_user", "clara")
    clara_bin = info.get("clara_bin", "/data/current-package/bin/clara")
...
# Re-export from clara_constants — canonical definition lives there.
from clara_constants import get_clara_home, get_process_clara_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_clara_home() / "config.yaml"
...
def require_parseable_user_config(*, ignore_user_config: bool = False) -> None:
    ...
    """
    if ignore_user_config or os.environ.get("CLARA_IGNORE_USER_CONFIG") == "1":
        return
...
def get_env_path() -> Path:
    ...
    return get_clara_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_clara_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the CLARA_UID / CLARA_GID env vars set by Docker deployments.

    Docker containers running Clara commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level CLARA_HOME once, but
    subdirectories created at runtime by ``ensure_clara_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("CLARA_UID", "").strip()
    gid_str = os.environ.get("CLARA_GID", "").strip()
    try:
...
def _chown_to_clara_uid(path) -> None:
    """Chown ``path`` to ``CLARA_UID:CLARA_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_clara_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_clara_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the clara group can
    share state with the gateway service.

    The mode can be overridden via the CLARA_HOME_MODE environment variable
    (e.g. CLARA_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse CLARA_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``CLARA_UID``/``CLARA_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("CLARA_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_clara_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Clara runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("CLARA_CONTAINER") or os.environ.get("CLARA_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set CLARA_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into CLARA_HOME, upgrading legacy empty templates.
...
# ensure_clara_home(). Only successful passes are recorded, so a raised
# managed-mode/missing-profile error keeps re-checking on later loads.
_CLARA_HOME_ENSURED: set = set()


def ensure_clara_home():
    """Ensure ~/.clara directory structure exists with secure permissions.
    ...
    After the first successful pass for a given ``CLARA_HOME`` we only re-run
    ...
    ``get_clara_home()`` and therefore re-run for the new path.
    """
    home = get_clara_home()
    key = str(home)

    # Named profiles must be created explicitly (e.g. ``clara profile create``).
    # Check tombstones before the memo so a stale empty shell cannot skip
    # the deleted-profile guard.
    from clara_constants import assert_named_profile_home_live
    assert_named_profile_home_live(home)
    if key in _CLARA_HOME_ENSURED and home.is_dir():
        return
    if is_managed():
        old_umask = os.umask(0o007)
        try:
            _ensure_clara_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
    ...
    _CLARA_HOME_ENSURED.add(key)


def _ensure_clara_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"CLARA_HOME {home} does not exist."
        )
...
from clara_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: F401
...
# selection step (Clara Portal / OpenRouter / Custom endpoint), so this
...
def _split_key_path(key: str) -> list[str]:
    ...
    ``clara config set`` uses ``.`` as the nesting separator, so a key that
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `clara config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.clara.config`` entries, then checks
    ...
    try:
        ...
        # should never break `clara update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Clara' own config writer has historically emitted it
    ...
    # Clara (``_save_discovered_models_to_config``), not hand-curated.
    # Older Clara versions wrote an in-mapping ``__discovered_model_catalog__``
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Clara versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``clara_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (clara_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (clara_cli/setup.py)
    "platform_toolsets",     # written by the setup wizard (clara_cli/setup.py)
    "known_plugin_toolsets", # written/read by clara_cli/tools_config.py toolset-save flow
    "known_builtin_toolsets",  # ditto — which builtin toolsets a platform's checklist has offered
    "tool_gateway_declined_tools",  # per-tool Tool Gateway offer declines (clara_cli/clara_subscription.py, #92647)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'clara setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Clara won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
    ...
    # scalars are bridged into os.environ (gateway/run.py, clara send) so
            ...
            for _key in ("backend", "search_backend", "extract_backend"):
                ...
                if note:
                    issues.append(ConfigIssue(
                        ...
                        "Run 'clara tools' and pick a different Web Search & Extract provider",
                    ))
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'clara doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "clara update / clara -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from clara_cli.config_migrations import (
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ clara config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
        # ── Versioned migration ladder (table-driven) ──
        # The per-version steps live in clara_cli.config_migrations as a
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from clara_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    try:
        ...
        from clara_cli.toolset_validation import validate_platform_toolsets
        from clara_cli.toolset_scope import toolset_allowed_for_platform
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: clara config set <key> <value>")
    ...
    # `clara update`, but only the version bump is persisted.
    ...
    # metadata.clara.config in their SKILL.md frontmatter.
    ...
    * ``${env:VAR}`` — Cursor-style SecretRef, same resolution after the
    ...
    if inner.startswith("env:"):
        ...
        if val is not None:
            return val
        logger.warning(
            "Config ref %r: %s is not set (check ~/.clara/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_clara_dotenv()`` populated the process env, or an env var
...
def split_model_config_default(raw_default: Any) -> tuple[str, str]:
    ...
    together through ``ClaraCLI`` construction: the model becomes a plain
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``clara config set``
    ...
    backends) — while display paths (``clara status``/``dump``) read ``name``
...
#   - survives the ``str() → int()`` round-trip through the CLARA_MAX_ITERATIONS
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _clara_home / "config.yaml"`` local variable
...
# Back-compat alias — canonical set lives in clara_cli.personality.
from clara_cli.personality import NEUTRAL_PERSONALITY_NAMES as _NEUTRAL_PERSONALITY_NAMES  # noqa: F401


def _prompt_text(value: Any) -> str:
    """Normalize config prompt values from YAML before handing them to AIAgent.

    Delegates to :mod:`clara_cli.personality` — the single owner of
...
def render_personality_prompt(value: Any) -> str:
    ...
    from clara_cli.personality import render_personality_prompt as _render
...
def resolve_ephemeral_system_prompt_from_config(cfg: Optional[Dict[str, Any]]) -> str:
    ...
    still prefer ``CLARA_EPHEMERAL_SYSTEM_PROMPT`` when that env var is set.

    Delegates to :mod:`clara_cli.personality` (single owner).
    """
    from clara_cli.personality import resolve_ephemeral_system_prompt

    return resolve_ephemeral_system_prompt(cfg)


def read_raw_config() -> Dict[str, Any]:
    """Read ~/.clara/config.yaml as-is, without merging defaults or migrating.
...
def read_user_config_raw(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    ``_clara_home``, tui profile override, multi-profile probes).
    """
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.clara/config.yaml.
    ...
    (which change ``CLARA_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    ...
    Expanding ``~`` on the Clara host rewrites it to the host or container
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/clara/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_clara_dotenv()
...
#   openai-codex (OAuth — clara auth) — OpenAI Codex
#   clara         (OAuth — clara auth) — Clara Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.clara/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Clara writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.clara/.env.
    ...
    interactive menu render (`clara tools`, `clara setup`, status
    panels). Sanitisation is O(lines), so re-parsing the
    same file on every call was burning ~300ms of CPU per `clara tools`
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.clara/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.clara/.env."""
    if is_managed():
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    - The fixed ``CLARA_CUSTOM_`` prefix keeps the result a valid POSIX name
    ...
    return f"CLARA_CUSTOM_{slug}_API_KEY" if slug else "CLARA_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.clara/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.clara/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Clara — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Clara vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` or ``~/.clara/.env``, scope-aware.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.clara/.env`` over ``os.environ``.

    Used for Clara-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ⚕ Clara Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from clara_cli.auth import get_anthropic_key
    ...
    # Warn on stale CLARA_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("CLARA_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale CLARA_MAX_ITERATIONS={_env_ghost} "
                f"(run 'clara doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  clara config edit     # Edit config file", Colors.DIM))
    print(color("  clara config set <key> <value>", Colors.DIM))
    print(color("  clara setup           # Run setup wizard", Colors.DIM))
    print()
...
def resolve_cron_model_drift_defaults(
    ...
    *,
    ...
    wins ``CLARA_MODEL``; the environment is only a fallback. Per-job and cron
    ...
    provider = ""
    model = _model_assignment_text(env.get("CLARA_MODEL", ""))
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "changed model/provider. Inspect with `clara cron list`, then pin the "
        "intended values with `clara cron edit <job_id> --provider <provider> "
        "--model <model>`."
    )
...
_SCHEMA_DEFINED_DICT_KEYS = frozenset({
    ...
    # (clara_cli/plugins_cmd.py, clara_cli/plugin_index.py). Absent from
    # DEFAULT_CONFIG (written only when used), so listed here for
    # `clara config set plugins.index_url ...` validation.
    "plugins",
})
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``clara config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``clara config set``
...
def _redirect_platform_display_key(key: str) -> tuple[str, Optional[str]]:
    ...
    ``clara config set platforms.telegram.streaming false`` landed on a key
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            CLI exposes this via ``clara config set --force``.
    """
    if is_managed():
    ...
    if not isinstance(_default_value_for_key(key), str):
        ...
        if _lower in {'true', 'yes', 'on'}:
            ...
            #   clara config set platform_toolsets.line '["file","web"]'
            # or a multi-line YAML block:
            #   clara config set custom_providers '- name: foo
            #     base_url: https://...'
    ...
    # ``clara config set model.provider openai`` doesn't silently
    ...
    # (e.g. ``clara config set model gpt-5.6-sol`` when model already
    ...
    if "." not in key:
        ...
        if isinstance(_existing, dict):
            if key == "model":
                ...
                print(
                    f"    clara config set {key}.<sub-key> <value>",
                    file=sys.stderr,
                )
                print(
                ...
                print(
                    f"    clara config set --force {key} {value!r}",
                    file=sys.stderr,
                )
                sys.exit(1)
    try:
    ...
    # so a fresh `clara config set model.api_base ...` lands on the canonical
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_clara_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
    ...
    # — e.g. `clara config set model.api_key cfut_...` routes to config.yaml
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # model-cache rows too, so `clara config unset <KEY>` fully removes
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: clara config get <key> [--json]")
            print()
            print("Examples:")
            print("  clara config get model")
            print("  clara config get terminal.backend")
            print("  clara config get skills.config --json")
            sys.exit(1)
        ...
        if not key or value is None:
            print("Usage: clara config set [--force] <key> <value>")
            print()
            print("Examples:")
            print("  clara config set model anthropic/claude-sonnet-4")
            print("  clara config set terminal.backend docker")
            print("  clara config set OPENROUTER_API_KEY sk-or-...")
            print()
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: clara config unset <key>")
            print()
            print("Examples:")
            print("  clara config unset model")
            print("  clara config unset terminal.backend")
            print("  clara config unset OPENROUTER_API_KEY")
            sys.exit(1)
        try:
        ...
        if missing_config:
            ...
            print("    Run 'clara config migrate' to add them")
        ...
        print("  clara config           Show current configuration")
        print("  clara config edit      Open config in editor")
        print("  clara config get <key>          Print a resolved config value")
        print("  clara config set <key> <value>   Set a config value")
        print("  clara config unset <key>        Remove a config value")
        print("  clara config check     Check for missing/outdated config")
        print("  clara config migrate   Update config with new options")
        print("  clara config path      Show config file path")
        print("  clara config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``clara config`` UI so users
