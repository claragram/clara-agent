...
``CommandDef.execute`` (clara_cli/commands.py) names a key in
...
for a fixed context (enforced by tests/clara_cli/test_commands_execute.py).

Import discipline: this module imports nothing heavy at module level and
``clara_cli.commands`` does NOT import this module (the ``execute`` field is
...
def _exec_profile(ctx: CommandContext) -> CommandReply:
    ...
    if not home_display:
        ...
        home_display = display_clara_home()
...
def _exec_bundles(ctx: CommandContext) -> CommandReply:
    ...
    if not bundles:
        return CommandReply(
            ...
            "Create one with: clara bundles create <name> --skill <s1> --skill <s2>\n"
...
def _exec_help(ctx: CommandContext) -> CommandReply:
    ...
    from clara_cli.commands import gateway_help_lines
