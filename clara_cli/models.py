...
Add, remove, or reorder entries here — both `clara setup` and
`clara` provider-selection will pick up the change automatically.
"""
...
from clara_cli.urllib_security import open_credentialed_url, url_origin
...
_CLARA_USER_AGENT = f"clara-cli/{_CLARA_VERSION}"
...
def _custom_provider_ssl_context(base_url: str):
    ...
    if not base_url:
        return None
    try:
        from clara_cli.config import get_custom_provider_tls_settings
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `clara model`
    flow without maintaining a separate static list.
    """
    from clara_cli.codex_models import DEFAULT_CODEX_MODELS, _finalize_codex_models
...
# $CLARA_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Clara-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $CLARA_HOME/models_dev_cache.json directly (no network). Falls
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    # endpoint expects (see clara_cli/model_setup_flows.py).
...
# ---------------------------------------------------------------------------
# Clara Portal free-model helper
# ---------------------------------------------------------------------------
# The Clara Portal models endpoint is the source of truth for which models
...
# ---------------------------------------------------------------------------
# Clara Portal account tier detection
# ---------------------------------------------------------------------------
def is_clara_free_tier(account_info: dict[str, Any]) -> bool:
...
def partition_clara_models_by_tier(
    model_ids: list[str],
    pricing: dict[str, dict[str, str]],
    free_tier: bool,
) -> tuple[list[str], list[str]]:
    """Split Clara models into (selectable, unavailable) based on user tier.
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    The Portal's ``/api/clara/recommended-models`` endpoint advertises which
    models are free *right now* — independent of what the in-repo
    ``_PROVIDER_MODELS["clara"]`` list happens to contain or whether the
    ...
    older Clara that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    users. The Portal's ``/api/clara/recommended-models`` endpoint advertises
    which paid models are blessed *right now* — independent of what the
    in-repo ``_PROVIDER_MODELS["clara"]`` list happens to contain or whether
    ...
    picker even if the user is running an older Clara that doesn't ship
    ...
    * ``pricing`` is left untouched — we deliberately do NOT synthesize
      ...
      :func:`partition_clara_models_by_tier` keeps free models selectable;
...
def check_clara_free_tier(*, force_fresh: bool = False) -> bool:
    """Check if the current Clara Portal user is on a free (unpaid) tier.
...
# ---------------------------------------------------------------------------
# Clara Portal recommended models
#
...
CLARA_RECOMMENDED_MODELS_PATH = "/api/clara/recommended-models"
_CLARA_RECOMMENDED_CACHE_TTL: int = 600  # seconds (10 minutes)
# (result_dict, timestamp) keyed by portal_base_url so staging vs prod don't collide.
_clara_recommended_cache: dict[str, tuple[dict[str, Any], float]] = {}


def _clara_recommended_disk_path() -> "Path":
    """Disk path for the persisted recommended-models cache."""
    from clara_constants import get_clara_home
    return get_clara_home() / "cache" / "clara_recommended_cache.json"


def _read_clara_recommended_disk(base: str) -> dict[str, Any] | None:
    ...
    """
    try:
        with open(_clara_recommended_disk_path(), encoding="utf-8") as fh:
            blob = json.load(fh)
...
def _write_clara_recommended_disk(base: str, data: dict[str, Any]) -> None:
    ...
    if not data:
        return
    path = _clara_recommended_disk_path()
    try:
        try:
        ...
        logging.getLogger(__name__).debug(
            "clara recommended-models disk cache write failed: %s", exc
        )


def fetch_clara_recommended_models(
    portal_base_url: str = "",
    timeout: float = 5.0,
    *,
    force_refresh: bool = False,
) -> dict[str, Any]:
    """Fetch the Clara Portal's curated recommended-models payload.

    Hits ``<portal>/api/clara/recommended-models``. The endpoint is public —
    no auth is required. Results are cached per portal URL for
    ``_CLARA_RECOMMENDED_CACHE_TTL`` seconds in process; pass
    ``force_refresh=True`` to bypass the in-process cache.

    A successful live fetch is also persisted to a per-base disk cache
    (``$CLARA_HOME/cache/clara_recommended_cache.json``) as last-known-good.
    ...
    if not force_refresh and cached is not None:
        ...
        if now - cached_at < _CLARA_RECOMMENDED_CACHE_TTL:
            return payload

    url = f"{base}{CLARA_RECOMMENDED_MODELS_PATH}"
    try:
    ...
    if data:
        ...
        _clara_recommended_cache[base] = (data, now)
        _write_clara_recommended_disk(base, data)
        return data
    ...
    disk = _read_clara_recommended_disk(base)
    if disk:
        _clara_recommended_cache[base] = (disk, now)
        return disk
...
def _resolve_clara_portal_url() -> str:
    """Best-effort lookup of the Portal base URL the user is authed against."""
    try:
        from clara_cli.auth import (
            DEFAULT_CLARA_PORTAL_URL,
        ...
        return "https://portal.claraprise.com"
...
def get_clara_recommended_aux_model(
    *,
    vision: bool = False,
    ...
    * ``vision=False`` → ``paidRecommendedCompactionModel`` or
    ...
    via :func:`check_clara_free_tier`. Pass an explicit bool to bypass the
    ...
    ``google/gemini-3-flash-preview``).
    """
    base = portal_base_url or _resolve_clara_portal_url()
    payload = fetch_clara_recommended_models(base, force_refresh=force_refresh)
    if not payload:
        return None

    if free_tier is None:
        try:
            free_tier = check_clara_free_tier()
        except Exception:
...
# this list:  clara model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `clara model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `clara model` TUI

CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ProviderEntry("clara",           "Clara Portal",              "Clara Portal (Everything your agent needs, 300+ models with bundled tool use)"),
...
#
# Some vendors expose several Clara provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `clara model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``clara model``, the
...
# In-repo fallback for the model Clara silently lands on when the user never
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    """
    try:
        from clara_cli.model_catalog import get_default_model_from_cache
...
# curated-list entry [0]. Metered aggregators (Clara Portal, OpenRouter) order
...
# ``clara model``) uses the richer free/paid-tier-aware resolver — see
# ``get_recommended_default_model`` in clara_cli/web_server.py and
# ``partition_clara_models_by_tier`` — which can hit the Portal.
_SILENT_DEFAULT_PROVIDERS: frozenset[str] = frozenset({"clara", "openrouter"})
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``clara auth add openai-codex`` without
    ``clara model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``clara model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    clara-agent is tool-calling-first — every provider path assumes the model
    ...
    **Permissive when the field is missing.** Some OpenRouter-compatible gateways
    (Clara Portal, private mirrors, older catalog snapshots) don't populate
...
# copy, `clara -p`, a cron job, or a freshly booted gateway answers
...
# the Clara Portal list different models, and a staging Portal must not answer
# for production.
...
def _reasoning_caps_disk_path() -> Path:
    from clara_constants import get_clara_home
    return get_clara_home() / "cache" / "reasoning_caps.json"
...
def _fetch_reasoning_caps_catalog(
    ...
    (OpenRouter itself, Clara Portal). Returns None when the catalog is
    ...
    """
    headers = {"Accept": "application/json", "User-Agent": _CLARA_USER_AGENT}
    try:
...
# Clara Portal serves OpenRouter's catalog schema, so the same parser and
...
_clara_reasoning_caps_cache: dict[str, Optional[dict[str, Any]]] | None = None
_clara_reasoning_caps_failed_at: float | None = None


def clara_catalog_url() -> str:
    ...
    ``CLARA_INFERENCE_BASE_URL`` → resolved credential base → prod — so a
    ...
    """
    return f"{_resolve_clara_pricing_credentials()[1]}/v1/models"


def _fetch_clara_reasoning_caps(
    timeout: float = 6.0, *, force: bool = False
) -> Optional[dict[str, Optional[dict[str, Any]]]]:
    """Clara Portal counterpart of :func:`_fetch_openrouter_reasoning_caps`."""
    global _clara_reasoning_caps_cache, _clara_reasoning_caps_failed_at
    if _clara_reasoning_caps_cache is not None and not force:
        return _clara_reasoning_caps_cache
    if (
        _clara_reasoning_caps_failed_at is not None
        and (time.monotonic() - _clara_reasoning_caps_failed_at) < 60
    ):
        return None
    caps_by_id = _fetch_reasoning_caps_catalog(clara_catalog_url(), timeout)
    if caps_by_id is None:
        _clara_reasoning_caps_failed_at = time.monotonic()
        return None
    _clara_reasoning_caps_cache = caps_by_id
    return caps_by_id


def _refresh_clara_reasoning_caps() -> None:
    _fetch_clara_reasoning_caps(force=True)


def clara_model_reasoning_capabilities(
    model_id: Optional[str],
    *,
    timeout: float = 6.0,
    allow_fetch: bool = False,
) -> Optional[dict[str, Any]]:
    """Return live-catalog reasoning capabilities for a Clara Portal model.

    Same tri-state contract and cache-only default as
    :func:`openrouter_model_reasoning_capabilities`; warm the cache with
    :func:`warm_clara_reasoning_caps_async` from hot paths.
    """
    model = str(model_id or "").strip()
    if not model:
        return None
    caps_by_id = _clara_caps_cached()
    if caps_by_id is None and allow_fetch:
        caps_by_id = _fetch_clara_reasoning_caps(timeout=timeout)
    if caps_by_id is None:
        return None
    return caps_by_id.get(model)


_clara_caps_disk_checked = False
_clara_caps_warm_started = False


def _clara_caps_cached() -> Optional[dict[str, Optional[dict[str, Any]]]]:
    ...
    """
    global _clara_reasoning_caps_cache, _clara_caps_disk_checked
    if _clara_reasoning_caps_cache is None and not _clara_caps_disk_checked:
        _clara_caps_disk_checked = True
        _clara_reasoning_caps_cache = _hydrate_reasoning_caps_from_disk(
            clara_catalog_url(), _refresh_clara_reasoning_caps
        )
    return _clara_reasoning_caps_cache


def warm_clara_reasoning_caps_async() -> None:
    """Clara Portal counterpart of :func:`warm_openrouter_reasoning_caps_async`."""
    global _clara_caps_warm_started
    if _clara_caps_warm_started or _clara_caps_cached() is not None:
        return
    _clara_caps_warm_started = True
    _warm_reasoning_caps_async(_refresh_clara_reasoning_caps)
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from clara_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — clara-agent
        ...
        if preferred_id == silent_default:
            ...
            # picker shows which model Clara lands on when none is selected.
            desc = "default"
        else:
...
def get_curated_clara_model_ids() -> list[str]:
    """Return the curated Clara Portal model-id list.

    Prefers the remotely-hosted catalog manifest (published under
    ``website/static/api/model-catalog.json``); falls back to the in-repo
    snapshot in ``_PROVIDER_MODELS["clara"]`` when the manifest is
    unreachable. Always returns a list (never None).
    """
    try:
        from clara_cli.model_catalog import get_curated_clara_models
        remote = get_curated_clara_models()
    except Exception:
        remote = None
    if remote:
        return list(remote)
    return list(_PROVIDER_MODELS.get("clara", []))
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from clara_constants import AI_GATEWAY_BASE_URL
...
def compute_sale_discount(
    ...
    Clara Portal-only feature: callers gate on the provider; this helper only
    sees ``original`` because the Clara fetch path opted in via
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    Works with any OpenRouter-compatible endpoint (OpenRouter, Clara Portal).

    When *include_sale_original* is true (Clara Portal only) and the gateway
    ...
    for item in payload.get("data", []):
        ...
        if mid and isinstance(pricing, dict):
            ...
            # Sale chrome is Clara Portal-only. Never copy pricing.original for
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return clara-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; clara's picker expects
...
_DEFAULT_CLARA_INFERENCE_BASE = "https://inference-api.claraprise.com"


def _resolve_clara_pricing_credentials() -> tuple[str, str]:
    """Return ``(api_key, base_url)`` for Clara Portal pricing.

    The Clara inference ``/v1/models`` endpoint exposes pricing without
    ...
    1. ``CLARA_INFERENCE_BASE_URL`` env override (staging / preview)
    ...
    try:
        ...
        env_base = _clara_inference_env_override()
    except Exception:
        env_base = None
    ...
    base_url = (env_base or creds_base or _DEFAULT_CLARA_INFERENCE_BASE).rstrip("/")
...
def clara_policy_allowed_ids(*, force_refresh: bool = False) -> Optional[set[str]]:
    """The Clara model ids the caller's org may reach, or ``None`` to not filter.
    ...
    """
    try:
        from clara_cli.clara_account import clara_policy_present

        if clara_policy_present() is not True:
            return None
    except Exception:
        return None
    ...
    # Same arguments as get_pricing_for_provider's clara branch, so a caller
...
_CLARA_POLICY_APPEND_MAX = 64

# How long a Clara catalog stays trusted. Its contents depend on the org's
...
_CLARA_CATALOG_TTL_SECONDS = 300.0


def restrict_to_clara_policy(
    model_ids: list[str],
    ...
    *,
    ...
    if rescue_empty and not kept and len(allowed) <= _CLARA_POLICY_APPEND_MAX:
        return sorted(allowed)
    return kept


def get_pricing_for_provider(provider: str, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    """Return live pricing for providers that support it (openrouter, clara, ai-gateway, novita)."""
    ...
    if normalized == "clara":
        ...
        if base_url:
            return fetch_models_with_pricing(
                ...
                base_url=base_url,
                force_refresh=force_refresh,
                # Sale chrome (pricing.original) is Clara Portal-only.
                include_sale_original=True,
                cache_ttl_seconds=_CLARA_CATALOG_TTL_SECONDS,
            )
    return {}
...
def _configured_custom_provider_ids() -> set[str]:
    ...
    try:
        ...
        from clara_cli.providers import custom_provider_slug
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``clara model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from clara_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _get_ollama_request_headers() -> dict[str, str]:
    ...
    try:
        from clara_cli.config import normalize_extra_headers
...
def probe_ollama_local_models(
    ...
    try:
        ...
        request_headers = {"User-Agent": _CLARA_USER_AGENT}
...
def curated_models_for_provider(
    ...
    *,
    ...
    # Try live API first (Codex, Clara, etc. all support /models)
...
_AGGREGATOR_PROVIDERS = frozenset(
    {"clara", "openrouter", "ai-gateway", "copilot", "kilocode"}
)
...
def _resolve_static_model_alias(
    ...
    try:
        from clara_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def detect_static_provider_for_model(
    ...
    # If someone types `/model clara` or `/model anthropic`, treat it as a
    ...
    if resolved_provider not in {"custom", "openrouter"}:
        ...
        if (
            ...
            # curated list is ordered most-capable-first (e.g. Clara Portal),
            # entry [0] is the priciest flagship, and typing ``/model clara``
...
def _configured_provider_ids() -> set[str]:
    ...
    Includes both top-level ids (``ollama``, ``clara``) and ``custom:*``
...
def _resolve_provider_prefix(model_name: str) -> Optional[tuple[str, str]]:
    ...
    ``clara/deepseek-v4-pro`` or ``ollama/qwen3.5:4b`` should route to the
    ...
    ``TestDenormalizeProviderSwitch`` in tests/clara_cli/test_web_server.py).
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Clara' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``clara_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Clara should expose the /fast toggle for this model."""
...
def _fast_mode_route_supported(
    ...
    OpenRouter, Clara, Copilot, Azure, Bedrock, and custom base_urls either
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``clara auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.clara/.env``.
...
#   - "clara": curated list and Portal /models endpoint are the source of
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from clara_cli.model_search import model_alias_canonical
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    Tries live API endpoints for providers that support them (Codex, Clara),
    falling back to static lists. For providers in ``_MODELS_DEV_PREFERRED``
    (opencode-go/zen, xiaomi, deepseek, smaller inference providers, etc.),
    models.dev entries are merged on top of curated so new models released
    on the platform appear in ``/model`` without a Clara release.
    """
    requested = str(provider or "").strip().lower()
    if requested == "ollama":
        if force_refresh:
    ...
    if normalized == "openai-codex":
        from clara_cli.codex_models import get_codex_model_ids
        ...
        # a Clara release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "clara":
        # Try live Clara Portal /models endpoint
        try:
            from clara_cli.auth import fetch_clara_models, resolve_clara_runtime_credentials
            creds = resolve_clara_runtime_credentials()
            if creds:
                live = fetch_clara_models(api_key=creds.get("api_key", ""), inference_base_url=creds.get("base_url", ""))
                if live:
                    return live
        except Exception:
            pass
        # Live failed (or no creds). Fall back to the docs-hosted manifest
        # — NOT the in-repo _PROVIDER_MODELS["clara"] snapshot — so newly
        # added Portal models still surface without a Clara release.
        manifest_ids = get_curated_clara_model_ids()
        if manifest_ids:
            return manifest_ids
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # ``clara model`` shows.
            from clara_cli.providers import is_official_openai_host
...
#
# Cache strategy:
#   - One JSON file at $CLARA_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    from clara_constants import get_clara_home
    return get_clara_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    (codex, copilot, anthropic-via-claude-code, clara portal), the
    relevant tokens live in ``$CLARA_HOME/auth.json`` and external
    ...
    # env var, so it must change the fingerprint too or `clara config set
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _CLARA_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Clara' default Claude IDs elsewhere use
...
def copilot_model_api_mode(
    ...
    *,
    ...
    # endpoint, not through Clara' native Anthropic adapter. The live catalog may
...
def opencode_zen_free_headers() -> dict:
    ...
    """
    try:
        from clara_cli import __version__ as _v
    except Exception:
        _v = "0"
    return {
        "Authorization": "",
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _CLARA_USER_AGENT}
...
_DEEPINFRA_MODELS_QUERY = "filter=true&sort_by=clara"
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from clara_constants import get_clara_home
    return get_clara_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "moa":
        try:
            ...
            return {
                ...
                "message": f"MoA preset `{requested}` was not found. Run `clara moa list`.",
            }
        except Exception as exc:
            return {
                "accepted": False, "persist": False, "recognized": False,
        ...
        if ollama_models is None:
            return {
                ...
                "message": (
                    ...
                    "Clara will save the model name, but local Ollama model discovery could not verify it."
                ),
            }
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Clara will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
    ...
    if normalized in {"openai-codex", "xai-oauth"}:
        ...
        # BEFORE the hidden-slug soft-accept below: the suffix is a Clara
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Clara cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if _variant_base is not None and _variant_base in set(api_models):
            ...
            # fetch) so we only accept models Clara actually ships.  (#46850)
            #
            ...
            # Clara provider: also check the Portal's live
            # /api/clara/recommended-models feed. That feed can list a model
            # (e.g. a newly-promoted free/paid recommendation) before it's
            # been added to the hardcoded _PROVIDER_MODELS["clara"] curated
            # list or the docs-hosted catalog manifest has been rebuilt.
            # `clara chat` already accepts these models via
            ...
            if normalized == "clara":
                try:
                    portal_payload = fetch_clara_recommended_models(
                        _resolve_clara_portal_url()
                    )
                    portal_model_names = {
                        name.lower()
                ...
                if requested_for_lookup.lower() in portal_model_names:
                    return {
                        ...
                        "message": (
                            ...
                            f"listing but is a current Clara Portal recommendation — accepted."
                        ),
                    }
