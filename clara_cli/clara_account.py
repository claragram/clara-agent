"""Normalized Clara Portal account entitlement helpers."""
...
ClaraAccountInfoSource = Literal["jwt", "account_api", "inference_key", "none", "error"]

# Free tool-pool coverage categories. Kept byte-for-byte aligned with the
# Portal's TOOL_COVERAGE_CATEGORIES (clara-account-service
...
_account_info_cache: tuple[str, float, "ClaraPortalAccountInfo"] | None = None
_ACCOUNT_INFO_CACHE_LOCK = threading.Lock()


@dataclass(frozen=True)
class ClaraPortalSubscriptionInfo:
...
@dataclass(frozen=True)
class ClaraPaidServiceAccessInfo:
...
@dataclass(frozen=True)
class ClaraPortalAccountInfo:
    logged_in: bool
    source: ClaraAccountInfoSource
    fresh: bool
    ...
    clara_client: Optional[str] = None
    ...
    subscription: Optional[ClaraPortalSubscriptionInfo] = None
    paid_service_access: Optional[bool] = None
    paid_service_access_info: Optional[ClaraPaidServiceAccessInfo] = None
    tool_access: Optional[ClaraToolAccessInfo] = None
...
def clara_portal_billing_url(account_info: Optional[ClaraPortalAccountInfo] = None) -> str:
    """Return the billing URL for a normalized Clara account snapshot."""
    try:
    ...
    if not isinstance(base, str) or not base.strip():
        base = DEFAULT_CLARA_PORTAL_URL
    return f"{base.rstrip('/')}/billing"


def clara_portal_topup_url(account_info: Optional[ClaraPortalAccountInfo] = None) -> str:
    ...
    """
    base_billing = clara_portal_billing_url(account_info)  # {base}/billing
...
def format_clara_portal_entitlement_message(
    account_info: Optional[ClaraPortalAccountInfo],
    *,
    capability: str = "this feature",
    include_refresh_hint: bool = True,
    coverage_category: Optional[str] = None,
) -> Optional[str]:
    """Return user-facing guidance for a missing Clara tool-gateway entitlement.
    ...
    """
    billing_url = clara_portal_billing_url(account_info)
    ...
    if account_info is not None:
        if coverage_category is not None:
            ...
            if account_info.tool_gateway_entitled:
                ...
                return (
                    f"{capability} isn't included with your current Clara Portal "
    ...
    if account_info is None:
        return (
            f"Clara could not verify your Clara Portal entitlement, so {capability} "
            f"is unavailable. Run `clara model` to refresh your login, or check "
    ...
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"Clara inference credentials are configured, but Clara cannot verify "
                f"your Clara Portal paid access for {capability}. Log in with "
                f"`clara model` to enable Portal-managed features. Billing and "
                f"credits are managed at {billing_url}."
            )
        return (
            f"Log in to Clara Portal to use {capability}: run `clara model`. "
            f"Billing and credits are managed at {billing_url}."
        )

    if account_info.paid_service_access is None:
        detail = (
            f"Clara could not verify your Clara Portal paid access, so {capability} "
            f"is unavailable."
        )
        if account_info.error:
            detail += f" Account lookup failed: {account_info.error}."
        if include_refresh_hint:
            detail += " Run `clara model` to refresh your session."
    ...
    if reason == "account_missing":
        return (
            f"Clara could not find a Clara Portal account or organisation for this "
            f"login, so {capability} is unavailable. Run `clara model` to "
            f"authenticate again; if the problem persists, contact Clara support."
        )
    ...
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and not account_info.fresh:
            message += " If you recently bought credits, run `clara model` to refresh Clara."
        return message

    return (
        f"Your Clara Portal account does not currently have paid service access, "
        f"so {capability} is unavailable. Add credits or update billing at {billing_url}."
    )


def _no_paid_access_message(
    account_info: ClaraPortalAccountInfo,
    capability: str,
    billing_url: str,
) -> str:
    access = account_info.paid_service_access_info
    ...
    if access and access.member_spend_cap_exceeded:
        ...
        return (
            f"Your Clara Portal access is paused because you've exceeded the"
            f" per-member spend cap set by your organisation.{cap_detail}"
            f"{credit_detail} Ask your organisation admin to raise the"
            f" member spend cap at {billing_url}, then run `clara model`"
            f" to refresh."
        )
    ...
    if has_active_subscription and active_subscription_is_paid:
        credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
        return (
            f"Your Clara Portal credits are exhausted{credit_detail}, so {capability} "
    ...
    if has_active_subscription and active_subscription_is_paid is False:
        return (
            f"Your current Clara Portal plan does not include paid service access, "
    ...
    if has_active_subscription is False:
        credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
        return (
            f"Your Clara Portal account has no active subscription or usable credits"
    ...
    credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
    return (
        f"Your Clara Portal account has no usable paid credits{credit_detail}, so "
...
def clara_policy_present() -> Optional[bool]:
    ...
    """
    try:
        from clara_cli.auth import get_provider_auth_state, _decode_jwt_claims
...
def clara_policy_notice(*, removed: bool) -> str:
    """A one-line notice for a list the org's policy narrowed, else ``""``.

    A blocked model is omitted rather than marked, which reads as "Clara does
    ...
    *removed* is whether the filter actually dropped anything. The catalog read
    ...
    """
    if not removed or clara_policy_present() is not True:
        return ""
    return (
...
def _fresh_account_info(
    *,
    ...
    try:
        from clara_cli.auth import get_provider_auth_state, resolve_clara_access_token

        access_token = resolve_clara_access_token()
        ...
        payload = _fetch_clara_account_info(access_token, portal_base_url)
        if not payload:
            return _error_info(
...
def _info_from_inference_key_pool(
    portal_base_url: Optional[str],
) -> Optional[ClaraPortalAccountInfo]:
    """Return an explicit unknown-entitlement snapshot for opaque Clara keys."""
    try:
        entry = _select_clara_pool_entry()
        if entry is None:
            return None
...
def _info_from_oauth_pool(
    *,
    ...
    try:
        payload = _fetch_clara_account_info(access_token, entry_portal_url)
    except Exception as exc:
        return _error_info(
            error=exc,
            logged_in=True,
...
def _select_clara_pool_entry() -> Optional[Any]:
...
def _info_from_valid_jwt(
    ...
    *,
    ...
    try:
        from clara_cli.auth import _decode_jwt_claims
    except Exception:
        return None
    ...
    access_info = ClaraPaidServiceAccessInfo(
        allowed=paid_access,
    ...
    return ClaraPortalAccountInfo(
        ...
        clara_client=_coerce_str(claims.get("clara_client")),
        portal_base_url=portal_base_url,
...
def _tool_access_from_value(value: Any) -> Optional[ClaraToolAccessInfo]:
    """Parse a Portal ``tool_access`` object (from the JWT claim or the account
    API) into :class:`ClaraToolAccessInfo`. Fails closed: a non-object value
    ...
    return ClaraToolAccessInfo(enabled=enabled, coverage=coverage)


def _subscription_from_payload(value: Any) -> Optional[ClaraPortalSubscriptionInfo]:
    if not isinstance(value, dict):
        return None
    return ClaraPortalSubscriptionInfo(
...
def _paid_service_access_from_payload(value: Any) -> Optional[ClaraPaidServiceAccessInfo]:
    if not isinstance(value, dict):
        return None
    allowed = _coerce_bool(value.get("allowed"))
    paid_access = _coerce_bool(value.get("paid_access"))
    return ClaraPaidServiceAccessInfo(
        allowed=allowed,
        paid_access=paid_access,
