"""clara memory setup|status — configure memory provider plugins.
...
def _provider_pip_dependencies(provider_name: str, declared: list) -> list:
    ...
    ``clara memory setup`` — if the update-time refresh only reinstalled
    ...
    if provider_name == "hindsight":
        try:
            import json
            cfg_path = get_clara_home() / "hindsight" / "config.json"
...
# ---------------------------------------------------------------------------
# Curses-based interactive picker (same pattern as clara tools)
# ---------------------------------------------------------------------------
...
def _install_dependencies(provider_name: str, *, force: bool = False) -> None:
    ...
    ranges. This is how ``clara update`` heals the active memory provider
    ...
    # data volume (CLARA_LAZY_INSTALL_TARGET). install_specs handles the
...
def cmd_setup_provider(provider_name: str) -> None:
    ...
    if not match:
        ...
        print("  Run 'clara memory setup' to see available providers.\n")
        return
    ...
    if hasattr(provider, "post_setup"):
        ...
        provider.post_setup(clara_home, config)
        return
...
def cmd_setup(args) -> None:
    ...
    if not providers:
        ...
        print("  Install a plugin to ~/.clara/plugins/ and try again.\n")
        return
...
def _write_env_vars(
    env_writes: dict,
    clara_home: str | os.PathLike[str] | None = None,
) -> None:
    """Persist memory-provider env vars through the canonical ``.env`` writer.

    Delegates to ``clara_cli.config.save_env_value`` so every key flows
    ...
    ``CLARA_HOME`` / etc.), CR/LF stripping on the value, and the atomic
    ...
    ``.env`` -> ``os.environ`` chain on the next Clara startup, and the
    ...
    ``clara_home`` may be supplied by plugin ``post_setup`` hooks that
    already received an explicit home directory (e.g. a non-default
    profile). It is applied through the context-local Clara home override
    ...
    token = set_clara_home_override(clara_home) if clara_home is not None else None
    try:
...
def cmd_status(args) -> None:
    ...
    if provider_name:
        ...
        if provider:
            ...
            if provider.is_available():
                ...
                if required_fields:
                    ...
                    for f in required_fields:
                        ...
                        print(line)
                print(
                    "  Note: systemd/gateway services do not inherit ~/.clara/.env —"
                )
                print(
            ...
            print(f"  Install the '{provider_name}' memory plugin to ~/.clara/plugins/")
