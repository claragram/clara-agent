...
Clara is developed on Linux / macOS and tested natively on Windows too.
...
_WINDOWS_GATEWAY_BREAKAWAY_ENV = "_CLARA_GATEWAY_BREAKAWAY"
...
# Electron exits, so the gateway never gets respawned after a `clara
...
def windows_hide_flags() -> int:
    ...
    flash but also want to collect stdout/exit code __PROT_0_synchroclaraly__.
...
def suppress_platform_ver_console() -> None:
    ...
    *visible* console: one flashing ``cmd`` window per process, triggered by
    ...
    during a dependency's import, not from Clara' own code.
    """
    if not IS_WINDOWS:
        return
    try:
...
def noninteractive_git_env(
    ...
    Clara shells out to git from many non-interactive contexts — MCP catalog
...
def _text_names_clara(text: str) -> bool:
    """True when *text* names Clara at a path-segment / token boundary.

    A bare ``"clara" in text`` substring test would also match unrelated
    processes whose paths merely contain the letters (``...\\sclaraa\\...``),
    ...
    that *starts with* ``clara`` (``clara``, ``clara.exe``, ``clara_cli``,
    ``clara-agent``, ``clara-runtime``) or the hidden-dir form
    ``.clara``/``.clara-runtime``.
    """
    for token in re.split(r"[\\/\s=,;\"']+", text.lower()):
        if token.startswith("clara") or token.startswith(".clara"):
            return True
    return False


def _process_command_is_clara(pid: int) -> bool:
    """Best-effort check that *pid* currently runs Clara code."""
    try:
        import psutil
        ...
        return _text_names_clara(f"{command} {executable}")
    except Exception:
        return False
...
def pid_is_clara(
    ...
    *,
    ...
    The PID must be valid, currently exist, and identify a Clara process. When
    ...
    if (
        ...
        return False
    try:
        return _process_command_is_clara(pid)
    except Exception:
        return False
...
def _legacy_kill_process_tree(proc: "subprocess.Popen") -> None:
    ...
    if IS_WINDOWS:
        ...
        # ``pid_is_clara`` guard is for BARE pids from state files or process
...
def bounded_probe_run(
    ...
    *,
    ...
    scan hit exactly this during ``clara update`` on slow-WMI machines
