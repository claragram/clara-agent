"""Runtime-backed validation behind ``clara plugins doctor``.
...
def _doctor_runtime(plugin_path: Path):
    ...
    test framework. Registration code executes under a temporary CLARA_HOME
    ...
    try:
        ...
        # removes it instead of stranding a clara-plugin-doctor-* directory
        # until interpreter GC (or never, on a hard exit).
        temporary_home = stack.enter_context(
            tempfile.TemporaryDirectory(prefix="clara-plugin-doctor-")
        )
        ...
        stack.enter_context(
            patch.dict(
                ...
                {
                    "CLARA_HOME": str(home),
                    "CLARA_BUNDLED_PLUGINS": str(bundled),
                    "CLARA_ENABLE_PROJECT_PLUGINS": "0",
                },
                clear=False,
            )
        )
    ...
    modules_before = {
        ...
        if name == "clara_plugins" or name.startswith("clara_plugins.")
    }
    ...
    try:
        ...
        if not manifests:
            raise _DoctorLoadError(
                f"Clara discovery found no valid plugin manifest under {copied}"
            )
        if len(manifests) != 1:
            raise _DoctorLoadError(
        ...
        for name in list(sys.modules):
            if (
                ...
                and (name == "clara_plugins" or name.startswith("clara_plugins."))
            ):
...
def _holds_plugin(path: Path) -> bool:
    ...
    confusing error: ``clara plugins doctor`` with no argument defaults to
...
def resolve_plugin_path(target: str | os.PathLike[str] | None = None) -> Path:
    ...
    if _is_plugin_id(raw):
        user_root = get_clara_home() / "plugins"
        ...
        try:
            ...
            pass
        candidates.append(Path.cwd() / ".clara" / "plugins" / raw)
...
def _check_manifest_v2(report: "DoctorReport", manifest: Any) -> None:
    ...
    from clara_cli.plugins import SUPPORTED_MANIFEST_VERSION

    mv = getattr(manifest, "manifest_version", 1)
    if mv > SUPPORTED_MANIFEST_VERSION:
        report.warning(
            f"manifest_version {mv} is newer than this Clara supports "
    ...
    if missing:
        report.warning(
            ...
            + ", ".join(missing)
            + " — Clara never auto-installs plugin dependencies; "
    ...
    if schema:
        from clara_cli.plugins import _CONFIG_SCHEMA_TYPES
...
def doctor_plugin(target: str | os.PathLike[str] | None = None) -> DoctorReport:
    """Validate one plugin through Clara' real scanner and registration path."""
    try:
