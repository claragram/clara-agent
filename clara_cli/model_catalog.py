...
The Clara docs site hosts a JSON manifest of curated models for providers
we want to update without shipping a release (currently OpenRouter and
Clara Portal). This module fetches, validates, and caches that manifest,
...
DEFAULT_CATALOG_FALLBACK_URLS: tuple[str, ...] = (
    "https://raw.githubusercontent.com/claragram/clara-agent/main/website/static/api/model-catalog.json",
)
...
def _cache_path() -> Path:
    ...
    return get_clara_home() / "cache" / "model_catalog.json"
...
def get_catalog(*, force_refresh: bool = False) -> dict[str, Any]:
    ...
    # this via get_curated_clara_model_ids on every open) never block on the
...
def refresh_catalogs() -> bool:
    ...
    free-pricing filter) and the Clara Portal recommendations, writing each
    ...
    try:
        from clara_cli.models import fetch_clara_recommended_models, fetch_openrouter_models

        fetch_openrouter_models(force_refresh=True)
        fetch_clara_recommended_models(force_refresh=True)
    except Exception:
...
def get_curated_clara_models() -> list[str] | None:
    """Return Clara Portal's curated list of model ids from the manifest.

    Returns ``None`` when the manifest is unavailable.
    """
    block = _get_provider_block("clara")
    if not block:
        return None
    out: list[str] = []
    for m in block.get("models", []):
...
def get_default_model_from_cache(provider: str) -> str | None:
    ...
    ``"default": true``; that entry is the model Clara silently lands on when
    ...
    network-free. The cache is kept fresh by the picker/`clara update` paths;
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    ``clara update`` pulls the latest repo, so the freshly-pulled
    ...
    to ``~/.clara/cache/model_catalog.json`` via the same atomic writer the
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``clara model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
