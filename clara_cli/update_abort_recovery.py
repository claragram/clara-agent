...
``clara update`` performs its fleet restart in the interpreter that started
...
itself lives in :mod:`clara_cli.update_restart_recovery`.
"""
...
def _serve_unit_recovery_available() -> bool:
    """Can a fresh process restart ``clara-serve*`` units on this host?"""
...
def _qualified_serve_skips(skip_units) -> list[dict]:
    ...
    ``clara-serve.service`` can exist in BOTH the user and the system
...
def _recover_gateway_restart_after_abort(
    ...
    *,
    ...
    ``clara update`` normally performs the fleet restart in the interpreter
    ...
    The same child additionally restarts active ``clara-serve*`` systemd
    units (#92145).  ``clara serve`` hosts ``tui_gateway.server`` and is
    ...
    """
    from clara_cli.update_cmd import _gateway_recovery_partition
    ...
    command = [
        sys.executable,
        "-m",
        "clara_cli.update_restart_recovery",
        "--stdin",
    ]
    env = os.environ.copy()
    env["CLARA_UPDATE_RESTART_RECOVERY"] = "1"
    for marker in ("_CLARA_GATEWAY", "CLARA_GATEWAY", "CLARA_GATEWAY_MODE"):
        env.pop(marker, None)
...
def _warn_stale_serve_runtimes(rows) -> None:
    ...
    terminal naming the responsible process. ``clara serve`` hosts
    ...
    print(
        "    Restart them before using Clara again, e.g."
        " `systemctl --user restart clara-serve.service`"
        " or by relaunching `clara serve` / the Desktop app."
    )
...
def _abort_recovery_is_complete(
    *,
    ...
    ``gateway_state.json`` / control socket — so a ``clara serve`` still
    ...
    * no ``clara-serve*`` unit failed to produce a verified replacement; and
