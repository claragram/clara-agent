...
class Session:
    ...
    ``refresh_token`` are opaque to Clara — provider-specific.
    """
...
class LoginStart:
    ...
    :func:`clara_cli.dashboard_auth.cookies.set_pkce_cookie`.
    """
...
def classify_jwks_lookup_error(exc: BaseException) -> Exception:
    ...
    * ``jwt.DecodeError`` — the bearer is not a JWT at all (an opaque peer
      ...
      every non-JWT bearer with a fast 503 ``Auth provider 'clara'
