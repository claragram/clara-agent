...
Three cookies in play:
  - clara_session_at:   the OAuth access token
                         (HttpOnly, lifetime = token TTL, ~15 min)
  - clara_session_rt:   the OAuth refresh token
                         (HttpOnly, lifetime = 24h, ROTATING + reuse-detected)
                         Clara Portal issues a rotating refresh token for the
                         dashboard auth-code grant (Portal NAS #293 / clara
  ...
  - clara_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
https://datatracker.ietf.org/doc/html/draft-west-cookie-prefixes):
  ...
  * Gated HTTPS, direct deploy (Path=/) — ``__Host-`` prefix. Binds the
    ...
    guarantee.
  * Gated HTTPS, behind a reverse-proxy prefix (Path=/clara) —
    ``__Secure-`` prefix. ``__Host-`` is disallowed when Path != "/";
    ``__Secure-`` keeps the Secure-required hardening without the
    Path constraint, and the explicit ``Path=/clara`` covers
...
setter wrote ``__Secure-clara_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "clara_session_at"
SESSION_RT_COOKIE = "clara_session_rt"
...
# auth plugins are enabled (for example Basic + Clara OAuth).
SESSION_PROVIDER_COOKIE = "clara_session_provider"
PKCE_COOKIE = "clara_session_pkce"
...
SSO_ATTEMPT_COOKIE = "clara_sso_attempt"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /clara`` we want ``Path=/clara`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    ``refresh_token`` is written as the RT cookie when non-empty. Clara Portal
    issues a 24h rotating refresh token (clara #37247); a provider that
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/clara``)
