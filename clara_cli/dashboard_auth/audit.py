...
from ``clara_constants`` or other clara_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$CLARA_HOME/logs/dashboard-auth.log``.

    Uses ``clara_constants.get_clara_home()`` (a leaf module — no import
    ...
    return get_clara_home() / "logs" / "dashboard-auth.log"
