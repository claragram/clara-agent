...
def _redirect_uri(request: Request) -> str:
    ...
    Three resolution tiers:
      ...
      1. ``CLARA_DASHBOARD_PUBLIC_URL`` env var or
      ...
      2. ``X-Forwarded-Prefix: /clara`` (Mission Control deploys) — we
...
def _provider_pkce_segments(cookie_payload: dict[str, str]) -> dict[str, str]:
    ...
    :class:`~clara_cli.dashboard_auth.base.LoginStart` (values are
    ...
    wire encoding.
    """
    flat = cookie_payload.get("clara_session_pkce", "")
    return dict(
...
def _prefix(request: Request) -> str:
    ...
    ``clara_cli.dashboard_auth.prefix`` for the normalisation rules.
    """
...
async def auth_native_authorize(
    ...
    from clara_cli.dashboard_auth import native_flow
...
async def api_auth_ws_ticket(request: Request):
    ...
    from clara_cli.dashboard_auth.ws_tickets import TTL_SECONDS, mint_ticket
...
async def auth_native_refresh(request: Request, body: _NativeRefreshBody):
    ...
    from clara_cli.dashboard_auth.base import RefreshExpiredError
