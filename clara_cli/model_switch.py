...
- ``clara_cli.providers``        -- canonical provider identity + overlays
- ``clara_cli.model_normalize``  -- per-provider name formatting
...
def _declared_model_ids(value: Any) -> list[str]:
    ...
    if isinstance(value, dict):
        for model_id in value:
            # Backward compat: pre-fix Clara wrote sentinel keys inside the
...
def _entry_models_discovered(entry: Any) -> bool:
    """True when the entry's ``models`` mapping was auto-discovered by Clara.

    The current shape is an entry-level ``models_discovered: true`` sibling of
    ``models``. Older Clara versions wrote an in-mapping
...
def _models_config_is_allowlist(value: Any, discovered: bool = False) -> bool:
    ...
    written by ``_save_custom_provider`` / the ``clara model`` wizard — not a
    ...
    while ``clara model`` still live-probed the full ``/v1/models`` list.
    ...
    ``_entry_models_discovered``): a catalog Clara itself persisted after a
    ...
    *,
    ...
    try:
        ...
        for entry in providers:
            ...
            # A mapping Clara itself discovered (``models_discovered: true``
...
_CLARA_MODEL_WARNING = (
    "Claraship Clara 3 & 4 models are NOT agentic and are not designed "
    "for use with Clara Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Claraship Clara 3 / Clara 4 chat families.
...
#   Claraship/Clara-3-Llama-3.1-70B, clara-4-405b, openrouter/clara3:70b
...
# ---------------------------------------------------------------------------
def is_clara_clara_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Clara Clara 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_CLARA_CLARA_NON_AGENTIC_RE.search(model_name))


def _check_clara_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Clara Clara 3/4 chat model."""
    if is_clara_clara_non_agentic(model_name):
        return _CLARA_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``clara config set model.aliases.xxx``
...
# switch (CLARA_HOME moves, so the path moves) and a config/key rotation
...
def _direct_alias_source_identity() -> Optional[tuple]:
    ...
    """
    try:
        from clara_constants import get_config_path
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from clara_cli.model_switch import
...
def resolve_startup_model_route(
    ...
    *,
    ...
    ``ClaraCLI`` is constructed before the interactive ``/model`` pipeline
    ...
    if current_provider:
        try:
            ...
            if _is_routing_agg(_norm_prov(current_provider)):
                from clara_cli.models import _find_openrouter_slug
    ...
    try:
        from clara_cli.models import normalize_provider
...
def _resolve_alias_fallback(
    ...
    Falls back to ``("openrouter", "clara")`` only when no authenticated
    providers are supplied (backwards compat for non-interactive callers).
    """
    providers = authenticated_providers or ("openrouter", "clara")
...
def resolve_display_context_length(
    ...
    about Codex OAuth, Copilot, Clara, and falls back to models.dev for the
    rest.
...
async def resolve_display_context_length_async(
    ...
    ``/v1/models``, Copilot, Clara, Codex, GMI, Ollama, models.dev and
    ...
    ``clara_cli.route_identity.should_clear_context_pin_async``, which
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'clara model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'clara doctor' — config issues detected:"
        ...
        from clara_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from clara_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
        ...
        if not new_model:
            if pdef.base_url:
                from clara_cli.runtime_provider import _auto_detect_local_model
    ...
    # =================================================================
        ...
        try:
            from clara_cli.config import load_config
            from clara_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from clara_cli.providers import resolve_user_provider as _ruser
        ...
        if current_provider == "custom" and current_base_url:
            try:
                from clara_cli.models import should_use_ollama_native_catalog
    ...
    # --- Clara Portal dual-wire override ---
    ...
    if target_provider in {"clara", "clara-portal", "claraship"}:
    ...
    # clara_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    # kimi) the same way.
    from clara_cli.models import opencode_provider_family as _oc_family_fn
    if _oc_family_fn(target_provider) is not None and isinstance(base_url, str):
        from clara_cli.models import normalize_opencode_base_url
    ...
    clara_warn = _check_clara_model_warning(new_model)
    if clara_warn:
        warnings.append(clara_warn)
...
def _prefetch_provider_models_parallel(provider_slugs: list[str]) -> None:
    ...
    :param provider_slugs: Clara provider IDs to prefetch (e.g. ``["openrouter",
        "anthropic", "deepseek"]``).  Unknown providers are silently skipped.
    """
    from clara_cli.models import cached_provider_model_ids
    ...
    def _fetch_one(slug: str) -> None:
        try:
            ...
            if models:
                from clara_cli.models import update_provider_cache_entry
...
def _collect_authed_provider_slugs(
    ...
    from clara_cli.auth import PROVIDER_REGISTRY, _load_auth_store
    from clara_cli.providers import CLARA_OVERLAYS, ALIASES as _PROVIDER_ALIAS_TABLE
    from clara_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS, CANONICAL_PROVIDERS

    _excluded_set = {str(p).strip().lower() for p in excluded if p}
    slugs: list[str] = []
    seen: set[str] = set()

    # --- Section 1: Clara-mapped providers (PROVIDER_TO_MODELS_DEV) ---
    for clara_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        _alias_target = _PROVIDER_ALIAS_TABLE.get(clara_id)
        if (
            _alias_target
            and _alias_target != clara_id
        ...
        try:
            ...
            pass
        if _canonical != clara_id:
            continue
        if clara_id.lower() in seen:
            continue
        if clara_id.lower() in _excluded_set or mdev_id.lower() in _excluded_set:
            continue
        pdata = models_dev_data.get(mdev_id)
        if not isinstance(pdata, dict):
            continue
        pconfig = PROVIDER_REGISTRY.get(clara_id)
        if pconfig and pconfig.auth_type != "api_key":
            continue
        from clara_cli.auth import is_runtime_provider_routable
        if not is_runtime_provider_routable(clara_id):
            continue
        ...
        if not has_creds:
            try:
                ...
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(clara_id)
                )
                if raw_pool_present:
                    has_creds = _credential_pool_is_usable(
                        clara_id, raw_pool_present=True
                    )
            except Exception:
                pass
        if has_creds:
            slugs.append(clara_id)
            seen.add(clara_id.lower())

    # --- Section 2: Clara-only providers (CLARA_OVERLAYS) ---
    _mdev_to_clara = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    for pid, overlay in CLARA_OVERLAYS.items():
        if pid.lower() in seen:
            continue
        clara_slug = _mdev_to_clara.get(pid, pid)
        if clara_slug.lower() in seen:
            continue
        if pid.lower() in _excluded_set or clara_slug.lower() in _excluded_set:
            continue
        has_creds = False
        ...
        if not has_creds:
            try:
                ...
                if pid in providers_store or clara_slug in providers_store:
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
            try:
                if _credential_pool_is_usable(clara_slug):
                    has_creds = True
            except Exception:
                pass
        if has_creds:
            slugs.append(clara_slug)
            seen.add(pid.lower())
            seen.add(clara_slug.lower())
    ...
    # Clara excluded: its picker branch builds from the curated list and it
    # cannot reach the api_key-only pathway, so a prefetched entry is written
    # and never read.
    return [s for s in slugs if s != "clara"]
...
def list_authenticated_providers(
    ...
    custom_providers: list | None = None,
    *,
    force_fresh_clara_tier: bool = False,
    ...
    Uses the curated model lists from clara_cli/models.py (OPENROUTER_MODELS,
    ...
    ``force_fresh_clara_tier`` bypasses the short Clara tier cache for explicit
    ...
    from clara_cli.models import (
        ...
        clear_provider_models_cache, get_curated_clara_model_ids,
    )
    ...
    from clara_cli.config import coerce_provider_id, stringify_provider_map
    ...
    # Compared against clara_id / mdev_id (section 1), pid / clara_slug
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from clara_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by clara provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "clara" pulls from the remote model-catalog manifest published at
    # https://agent.claraship.com/docs/api/model-catalog.json so
    ...
    curated["clara"] = get_curated_clara_model_ids()
    # Ollama Cloud uses dynamic discovery (no static curated list)
    if "ollama-cloud" not in curated:
        from clara_cli.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (
        ...
        from clara_cli.models import fetch_lmstudio_models
    ...
    # --- 1. Check Clara-mapped providers ---
    from clara_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from clara_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for clara_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if (
            ...
            continue
        # Resolve the canonical provider profile name.  Skip clara_ids
        # that are mere aliases resolving to a different canonical profile
        # (e.g. "kimi" and "moonshot" both → "kimi-coding").  Only process
        # entries whose clara_id matches the canonical profile name so
        ...
        # same clara_id).  Distinct canonical profiles that share a
        ...
        if slug.lower() in seen_slugs:
            continue
        if clara_id.lower() in _excluded or mdev_id.lower() in _excluded:
            continue
        ...
        # section 2 (CLARA_OVERLAYS) with proper auth store checking.
        if pconfig and pconfig.auth_type != "api_key":
            continue
        # models.dev catalogs include providers Clara may not route yet.
        ...
        if not has_creds:
            try:
                from clara_cli.auth import _load_auth_store
                store = _load_auth_store()
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(clara_id)
                )
        ...
        # /model picker sees the SAME list `clara model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(clara_id)
        if not model_ids:
            model_ids = curated.get(clara_id, [])
            if clara_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(clara_id, model_ids)
    ...
    # --- 2. Check Clara-only providers (clara, openai-codex, copilot, opencode-go) ---
    from clara_cli.providers import CLARA_OVERLAYS
    from clara_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Clara provider ID.
    # CLARA_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Clara IDs ("copilot").
    _mdev_to_clara = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    ...
    for pid, overlay in CLARA_OVERLAYS.items():
        ...
        # Resolve Clara slug — e.g. "github-copilot" → "copilot"
        clara_slug = _mdev_to_clara.get(pid, pid)
        if clara_slug.lower() in seen_slugs:
            continue
        if pid.lower() in _excluded or clara_slug.lower() in _excluded:
            continue
        ...
        if getattr(overlay, "keyless", False):
            ...
            has_creds = _has_aws_sdk_creds_for_listing(clara_slug)
        elif overlay.auth_type == "vertex":
        ...
        if not has_creds and overlay.auth_type == "external_process":
            try:
                ...
                _ext_status = get_auth_status(clara_slug) or {}
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or clara_slug in providers_store):
                    has_creds = True
            except Exception as exc:
        ...
        if not has_creds:
            try:
                if _credential_pool_is_usable(clara_slug):
                    ...
                    try:
                        from agent.credential_pool import load_pool
                        _pool = load_pool(clara_slug)
                        if _pool.has_credentials():
                            has_creds = True
                    except Exception:
                        pass
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", clara_slug, exc)
        ...
        # configured.
        if not has_creds and clara_slug == "anthropic":
            try:
                ...
                if (clara_creds and clara_creds.get("accessToken")) or \
        ...
        if clara_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(clara_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(clara_slug)
                model_ids = _ids if _ids else (curated.get(clara_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(clara_slug, []) or curated.get(pid, [])
        elif clara_slug == "clara":
            # Clara serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `clara model` picker deliberately shows ONLY the curated agentic
            ...
            # order. Mirror that exactly (see _model_flow_clara in main.py) so
            ...
            model_ids = curated.get("clara", [])
            try:
                from clara_cli.models import (
                    get_pricing_for_provider as _clara_pricing,
                    check_clara_free_tier as _clara_free,
                    union_with_portal_free_recommendations as _union_free,
                    union_with_portal_paid_recommendations as _union_paid,
                )
                from clara_cli.auth import get_provider_auth_state as _clara_state

                _pricing = _clara_pricing("clara") or {}
                _portal = ""
                try:
                    _st = _clara_state("clara") or {}
                    _portal = _st.get("portal_base_url", "") or ""
                except Exception:
                    _portal = ""
                if _clara_free(force_fresh=force_fresh_clara_tier):
            ...
            try:
                from clara_cli.models import (
                    clara_policy_allowed_ids as _clara_policy,
                    restrict_to_clara_policy as _clara_restrict,
                )

                model_ids = _clara_restrict(
                    model_ids, _clara_policy(), rescue_empty=True,
                )
            except Exception:
                pass
        else:
            ...
            if not model_ids:
                model_ids = curated.get(clara_slug, []) or curated.get(pid, [])
                if clara_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(clara_slug, model_ids)
        total = len(model_ids)
        if clara_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
        ...
        results.append({
            "slug": clara_slug,
            "name": get_label(clara_slug),
            "is_current": clara_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "clara",
        ...
        seen_slugs.add(clara_slug.lower())
        _record_builtin_endpoint(clara_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or CLARA_OVERLAYS (keeps /model in sync
    # with `clara model`).
    try:
        from clara_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            # Clara writes ``models:`` as a dict keyed by model id, but older
            ...
            if group_key not in ep_groups:
                ...
                # Clara's own writer uses (mirrors section-4 grouping).
            ...
            # ``clara model``, not an allowlist — see
        ...
        for grp in ep_groups.values():
            ...
            #   Ollama/llama.cpp match ``clara model``. Pin with
            ...
            if _probe_live:
                ...
                try:
                    from clara_cli.models import cached_fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # active model. Clara's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, clara_cli/config.py) already consume that dict.
        ...
        for grp in groups.values():
            ...
            #   ``clara model``. Pin a dict catalog with
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from clara_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`clara_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from clara_cli.models import fetch_openrouter_models
