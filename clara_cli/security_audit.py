"""On-demand supply-chain audit for Clara Agent installs.

Scans three surfaces a Clara user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Clara venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.clara/plugins``
...
def _discover_plugins(clara_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.clara/plugins``.
...
def _discover_components(
    *,
    ...
    home = clara_home or Path(get_clara_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `clara security audit`."""
    ...
    components = _discover_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, clara_home=home
    )
