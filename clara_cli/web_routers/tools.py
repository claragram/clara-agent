...
late-binding seam in :mod:`clara_cli.web_deps` (``late`` for callables,
...
from clara_cli.web_deps import late, LateState
from clara_cli.web_models import (
...
_log = logging.getLogger("clara_cli.web_server")
...
_spawn_clara_action = late("_spawn_clara_action")
...
async def get_toolsets(profile: Optional[str] = None):
    ...
    from clara_cli.platforms import platform_label
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    # same background install `clara tools` runs interactively. Without this,
    ...
    # dead-end that forced users to discover `clara computer-use install`
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``clara tools`` picker shows
    ...
    def _read():
        with _profile_scope(profile):
            ...
            if cat:
                ...
                # re-probing per row.
                features = get_clara_subscription_features(config, force_fresh=True)
                for prov in _visible_providers(cat, config, force_fresh=True):
                    ...
                    row = {
                        ...
                        "requires_clara_auth": bool(prov.get("requires_clara_auth")),
                        "is_active": is_active,
                        # Honest server-side readiness. The GUI's old client-side
                        # heuristic showed "Ready" for every zero-env-var row —
                        # including logged-out Clara Subscription rows and never-run
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `clara tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``clara tools``
    ...
    Managed Clara rows (``managed_clara_feature``) additionally report the
    Portal entitlement state: the CLI flow gates these selections on
    ``ensure_clara_portal_access`` (inline login), but the GUI has no inline
    ...
    ``managed_by_clara``). The response now carries an additive
    ``needs_clara_auth: true`` + ``feature`` so the client can drive the
    existing Clara Portal OAuth flow (``POST /api/providers/oauth/clara/start``)
    and refetch.
    """
    from clara_cli.tools_config import (
        TOOL_CATEGORIES,
    ...
    def _run():
        with _profile_scope(body.profile or profile):
            ...
            # Entitlement check for managed Clara rows — mirrors the gate the CLI
            # applies via ensure_clara_portal_access at selection time. This hits
            ...
            managed_feature = (row or {}).get("managed_clara_feature")
            if managed_feature:
                features = get_clara_subscription_features(config, force_fresh=True)
                ...
                if not entitled:
                    response["needs_clara_auth"] = True
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.clara/.env`` via ``save_env_value`` —
    the same store ``clara tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``clara tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``clara -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same CLARA_HOME the rest of the
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``clara computer-use permissions grant`` as a background action.
