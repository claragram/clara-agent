"""Implementation of the ``clara bundles`` CLI subcommand.

Mirrors the structure of ``clara_cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `clara bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  clara bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``clara bundles`` argparse tree.

    Called from ``clara_cli/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `clara bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``clara bundles <subcommand>`` to the right handler."""
