"""Image-authored deployment provenance for immutable Clara runtimes.

The published image bakes ``/etc/clara/image-provenance.json`` outside both
``$CLARA_HOME`` and the mutable checkout.  A bind-mounted checkout (including
...
IMAGE_PROVENANCE_PATH = Path("/etc/clara/image-provenance.json")
