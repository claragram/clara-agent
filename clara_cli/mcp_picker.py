"""MCP picker — interactive `clara mcp picker` (also the default `clara mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``clara mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `clara plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from clara_cli.cli_output import prompt_yes_no
from clara_cli.curses_ui import curses_single_select
from clara_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Clara session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from clara_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `clara mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: clara mcp install <name>    Picker: clara mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Clara is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Clara — run `clara update` "
...
def show_catalog() -> None:
    """`clara mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`clara mcp picker` (and default `clara mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`clara mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `clara mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
