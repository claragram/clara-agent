...
to the resolved absolute path of the client binary.  Clara' interpreter is
...
* Dynamically-linked builds look up ``libpython`` via
  ...
  no dylib — every clara command, including update/doctor, died in dyld
  (#95425).
...
from clara_cli.managed_uv import _RUNTIME_DIR_NAME
...
def _install_anchor(venv_dir: Path, source_file: Path) -> None:
    ...
    try:
        ...
        try:
            from clara_cli.managed_uv import _macos_sign_managed_python
...
def tcc_anchor_state(project_root: Path | None = None) -> tuple[str, str]:
    """Report the anchor state for ``clara doctor``.
