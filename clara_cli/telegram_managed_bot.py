...
manual BotFather token copy-paste. Clara talks only to the Clara onboarding
...
DEFAULT_API_URL = "https://setup.agent.claraprise.com"
...
DEFAULT_MANAGER_BOT = "ClaraSetupBot"

DEFAULT_BOT_NAME = "Clara Agent"
...
def generate_username_slug(length: int = 16) -> str:
    ...
    keeping ``clara_<slug>_bot`` under Telegram's 32-character username limit.
    """
...
def generate_bot_username(profile_name: Optional[str] = None) -> str:
    """Generate a secure suggested bot username like ``clara_<slug>_bot``.
    ...
    """
    _ = profile_name
    return f"clara_{generate_username_slug()}_bot"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Clara Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Clara Telegram onboarding service.")
