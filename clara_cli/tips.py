...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or clara -c.",
    ...
    "Ctrl+Z suspends Clara to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "clara -c resumes your most recent CLI session. clara -c \"project name\" resumes by title.",
    "clara -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "clara -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "clara chat -t web,terminal enables only specific toolsets for a focused session.",
    "clara chat -s github-pr-workflow preloads a skill at launch.",
    "clara chat -q \"query\" runs a single non-interactive query and exits.",
    "clara chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "clara chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "clara --yolo bypasses all dangerous command approval prompts for the entire session.",
    "clara chat --source telegram tags the session for filtering in clara sessions list.",
    "clara -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "clara doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "clara dump outputs a compact setup summary — great for bug reports.",
    "clara config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "clara config edit opens config.yaml in your default editor.",
    "clara config check scans for missing or stale configuration options.",
    "clara sessions browse opens an interactive session picker with search.",
    "clara sessions stats shows session counts by platform and database size.",
    "clara sessions prune --older-than 30 cleans up old sessions.",
    "clara skills search react --source skills-sh searches the skills.sh public directory.",
    "clara skills check scans installed hub skills for upstream updates.",
    "clara skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "clara skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "clara mcp add github --command npx adds MCP servers from the command line.",
    "clara mcp serve runs Clara itself as an MCP server for other agents.",
    "clara auth add lets you add multiple API keys for credential pool rotation.",
    "clara completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "clara logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "clara backup creates a zip backup of your entire Clara home directory.",
    "clara profile create coder creates an isolated profile that becomes its own command.",
    "clara profile create work --clone copies your current config and keys to a new profile.",
    "clara update syncs new bundled skills to ALL profiles automatically.",
    "clara gateway install sets up Clara as a system service (systemd/launchd).",
    "clara memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "clara webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: clara tools disables unused tools, clara skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "clara models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as clara -w).",
    ...
    "CLARA_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Clara tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'clara profile create coder' creates the 'coder' command.",
    "clara profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Clara prints a resume command with session ID and stats.",
    "clara sessions export backup.jsonl exports all sessions for backup or analysis.",
    "clara -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "clara skills install official/security/1password installs optional skills from the repo.",
    ...
    "/plan writes a markdown implementation plan to .clara/plans/ without executing anything.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: clara cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Clara runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "clara gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Clara via a local macOS server.",
    ...
    "Context files (.clara.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Clara loads project context from .clara.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "clara mcp opens an interactive picker of Clara-approved MCPs you can install in one keystroke.",
    "clara mcp catalog lists Clara-approved MCP servers shipped with the repo.",
    "clara mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — clara tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.clara/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "clara chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "clara plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "clara acp runs Clara as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "CLARA_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "clara auth add clara or clara auth add openai-codex sets up OAuth-based providers.",
    ...
    "clara status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.clara/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Clara your own.",
    ...
    "Before a gateway session reset, Clara auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.clara/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Profiles scope Clara state via CLARA_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "CLARA_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in CLARA_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.clara/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'clara profile create ls' would be rejected.",
    "clara profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "clara claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "CLARA_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via clara tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.clara/.env.",
    "Skills from trusted repos (Claragram) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.clara/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Clara auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Clara config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Clara is working.',
    ...
    '/reload reloads ~/.clara/.env into the running session — pick up new API keys without restarting.',
    ...
    "CLARA_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.clara/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.clara/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'clara curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "clara curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'clara curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'clara auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    'credential_pool_strategies.<provider>: round_robin cycles keys evenly instead of the fill_first default.',
    'use_gateway: true per-tool routes web, image, tts, or browser through your Clara subscription — no extra keys.',
    ...
    # --- TUI & Dashboard ---
    'CLARA_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "CLARA_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'clara dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'clara dashboard embeds the full Clara TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.clara/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.clara/dashboard-plugins/ — no npm build required.',
    ...
    'CLARA_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs concise/all/error/off).',
    'CLARA_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'CLARA_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'CLARA_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.clara/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.clara/skills/ so drop-in skills appear without restarting the session.',
    ...
    'clara -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'clara chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'clara chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'clara chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "clara chat --source tool tags programmatic chats so they don't clutter clara sessions list.",
    'clara dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'clara sessions rename <ID> "new title" renames any past session; clara sessions delete <ID> removes one.',
    'clara import restores a full Clara backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'clara fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'clara pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'clara setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'clara status --deep runs the full health sweep across every component; plain clara status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'CLARA_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'CLARA_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.clara/plugins/ — trust-gated by design.',
    "CLARA_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'CLARA_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'CLARA_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'CLARA_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'CLARA_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'CLARA_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'CLARA_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'CLARA_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'CLARA_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'CLARA_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'CLARA_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Clara block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from clara sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.clara/dashboard-plugins/.',
]
...
# any project or none — Clara is not a coding-only agent.
# ---------------------------------------------------------------------------
