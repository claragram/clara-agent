"""
clara fallback — manage the fallback provider chain.
...
Subcommands:
  clara fallback [list]   Show the current fallback chain (default when no subcommand)
  clara fallback add      Pick provider + model via the same picker as `clara model`,
                           then append the selection to the chain
  clara fallback remove   Pick an entry to delete from the chain
  clara fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.clara/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from clara_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `clara model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  clara fallback add")
        print()
        return
    ...
    print("  Docs: https://agent.claraship.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `clara model`, then append the selection to the chain."""
    from clara_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `clara model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `clara fallback list` to view, or `clara fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``clara fallback [subcommand]``."""
