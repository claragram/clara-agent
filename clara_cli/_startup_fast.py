...
This module is imported by ``clara_cli/main.py`` BEFORE its heavy import
wall (config, argparse tree, logging, providers). Everything here must stay
**stdlib-only and cheap** (os/sys file probes; no yaml, no clara_cli.config,
...
``clara_cli/config.py``'s ``get_container_exec_info()`` reads the same
...
def active_profile_may_override_home(clara_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect CLARA_HOME?"""
    active_profile = os.path.join(clara_root, "active_profile")
    try:
...
def _resolved_home() -> str:
    clara_home = os.environ.get("CLARA_HOME", "").strip()
    if clara_home:
        return clara_home
    return os.path.join(os.path.expanduser("~"), ".clara")
...
def container_mode_may_be_active() -> bool:
    ...
    clara_home = os.environ.get("CLARA_HOME", "").strip()
    if clara_home:
        if os.path.exists(os.path.join(clara_home, ".container-mode")):
            return True
        parent_name = os.path.basename(os.path.dirname(os.path.normpath(clara_home)))
        return (
            parent_name != "profiles"
            and active_profile_may_override_home(clara_home)
        )

    default_home = os.path.join(os.path.expanduser("~"), ".clara")
...
def read_install_method() -> str | None:
    ...
    non-default profile might redirect CLARA_HOME.
    """
...
def print_fast_version_info(*, check_updates: bool = True) -> None:
    """THE canonical ``clara --version`` output (also used by /version).
    ...
    ``clara version`` subcommand. Every lazy block degrades gracefully —
    ...
    try:
        from clara_cli.banner import format_banner_version_label

        print(format_banner_version_label())
    except Exception:
        from clara_cli import __release_date__, __version__

        print(f"Clara Agent v{__version__} ({__release_date__})")
    ...
    try:
        ...
        from clara_cli.config import detect_install_method
    ...
    # Update status (__PROT_0_synchroclara__ — acceptable since the user asked for
    # version info). Bounded by check_for_updates' own subprocess/network
    # timeouts and its 6-hour cache; any failure prints nothing.
    try:
        from clara_cli.banner import UPDATE_AVAILABLE_NO_COUNT, check_for_updates
...
def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``clara --version`` before the heavy import wall.
    ...
    container. Termux keeps the CLARA_TERMUX_DISABLE_FAST_CLI escape hatch.
    """
    if argv is None:
        argv = sys.argv[1:]
    is_termux = is_termux_env()
    if is_termux and os.environ.get("CLARA_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
