"""Clara update pipeline — extracted from ``clara_cli/main.py``.
...
change is that references to helpers/constants that STAY in ``clara_cli.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``clara_cli.main`` reference — so existing call sites and test monkeypatches
that target ``clara_cli.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``clara_cli.main``.
...
Imports are one-way: ``clara_cli.main`` imports this module, never the reverse
...
from clara_constants import get_default_clara_root, venv_python_path

logger = logging.getLogger(__name__)


def _m():
    """Lazy ``clara_cli.main`` reference.

    Lets callers keep patching ``clara_cli.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``clara_cli.main`` -> ``clara_cli.update_cmd`` stays one-way
    at import time.
    """
    from clara_cli import main
...
#: ``_purge_stale_clara_modules`` so any LATER import chain resolves against
...
_STALE_PURGE_PROTECTED = frozenset(
    {
        ...
        "clara_cli.clara_logging",
    }
)


def _purge_stale_clara_modules() -> None:
    """Evict every cached Clara module after the checkout changed in-place.

    ``clara update`` keeps running in the pre-pull Python process. The
    gateway auto-restart phase that follows does function-level
    ``from clara_cli.gateway import ...`` — executing NEW source inside an
    ...
    field failure: freshly-pulled ``clara_cli.gateway`` does
    ``from clara_cli.cli_output import line_input``, but ``cli_output`` was
    ...
    export. This is the class fix: drop EVERY cached module under the Clara
    ...
    try:
        ...
        if purged:
            logger.debug(
                "Purged %d stale Clara module(s) after checkout update", len(purged)
            )
    except Exception as exc:
        logger.debug("Could not purge stale Clara modules: %s", exc)


def _reload_updated_runtime_modules() -> None:
    """Reload update-sensitive modules after the checkout changes in-place.

    ``clara update`` keeps running in the pre-pull Python process. After a
...
def _reload_config_modules() -> None:
    ...
    ``clara update`` runs in the PRE-pull Python process. After ``git pull``
    ...
    This function force-reloads ``clara_cli.config_defaults``,
    ``clara_cli.config``, and ``clara_cli.config_migrations`` from disk
    so subsequent imports read the UPDATED code.

    It also reloads ``clara_cli._subprocess_compat`` and
    ``clara_cli.dashboard_procs`` so that post-update dashboard cleanup
    ...
    importlib.invalidate_caches()
    for mod_name in (
        "clara_cli.config_defaults",
        "clara_cli.config",
        "clara_cli.config_migrations",
        "clara_cli._subprocess_compat",
...
def _run_config_check_fresh() -> tuple:
    ...
    from clara_cli.config import check_config_version
...
def _run_migrate_config_fresh(*, interactive: bool = False, quiet: bool = False) -> dict:
    ...
    """
    _reload_config_modules()
    from clara_cli.config import migrate_config
...
def _migrate_sibling_profile_configs() -> list[tuple[str, int, int]]:
    ...
    shared checkout serves every profile, but ``clara update`` historically
    ...
    caller): scope config reads/writes via the context-local CLARA_HOME
    ...
    try:
        ...
        from clara_cli.profiles import _get_profiles_root, _PROFILE_ID_RE

        active_home = get_process_clara_home()
        ...
        for entry in sorted(root.iterdir()):
            ...
            token = set_clara_home_override(entry)
            try:
...
def _check_and_apply_config_migration(
    *,
    ...
    try:
        ...
        print("     Run 'clara config migrate' to check manually.")
        return
    ...
    if version_bump_only:
        ...
        try:
            ...
            print("     Run 'clara config migrate' to retry.")
        ...
        if assume_yes:
            ...
            try:
                ...
                print(
                    ...
                    "Run 'clara config migrate' manually to configure."
                )
                response = "n"
        ...
        if response in {"", "y", "yes", "auto"}:
            ...
            if (gateway_mode or assume_yes or response == "auto") and missing_env:
                print("  ℹ API keys require manual entry: clara config migrate")
        else:
            print()
            print("Skipped. Run 'clara config migrate' later to configure.")
    else:
    ...
    # context-local CLARA_HOME override (never os.environ — other
    ...
    # it and warn loudly.
    try:
        from clara_cli.backup import restore_cron_jobs_if_emptied
    ...
    try:
        from clara_cli.backup import restore_config_model_settings_if_rewritten
    ...
    try:
        from clara_cli.backup import restore_cron_jobs_all_profiles
    ...
    try:
        from clara_cli.backup import restore_config_model_settings_all_profiles
...
# Critical files that Clara must be able to import immediately after an
...
_UPDATE_CRITICAL_FILES = (
    "clara_cli/main.py",
    "clara_cli/config.py",
    "clara_cli/__init__.py",
    "clara_cli/web_server.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _prune_orphan_rescue_refs(
    ...
    ``refs/clara-update-backups/orphan-<branch>-<ts>-<sha>``. A rescue ref
    ...
    try:
        list_result = subprocess.run(
            git_cmd + [
                ...
                "--format=%(refname)",
                "--sort=refname",
                f"refs/clara-update-backups/orphan-{branch}-*",
            ],
            cwd=cwd,
            capture_output=True,
            text=True, encoding="utf-8", errors="replace",
        ...
        if max_age_days > 0:
            ...
            prefix = f"refs/clara-update-backups/orphan-{branch}-"
            for ref in refs:
...
def _editable_install_is_current(git_cmd, cwd, pre_pull_sha: str | None) -> bool:
    ...
    On Windows that rewrite is the only reason the running ``clara.exe`` has
    ...
    Skipping is safe because Clara pins its editable finder to a *static*
...
def _validate_python_files_syntax(
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="clara-syntax-check-") as tmpdir:
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``clara`` startup; if any of them
...
def _critical_module_import_failures(
    ...
    ``_venv_core_imports_healthy``): ``clara update`` can be driven by a
    ...
    a later one.
    """
    from clara_constants import FIRST_PARTY_MODULE_ROOTS

    import secrets

    marker = f"__CLARA_IMPORT_HEALTH_{secrets.token_hex(16)}__"
    probe = (
        ...
        # The root set is injected from clara_constants so this can't drift
        # from the hint the user is shown (they disagreed once already).
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('clara_') or %r:\n"
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``clara update --gateway`` so interactive prompts (stash restore,
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `clara update`.
...
def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `clara update`.
    ...
    try:
        ...
        return
    db_path = get_clara_home() / "state.db"
    ...
    if interrupted:
        ...
        print(
            "  A previous `clara sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    clara sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  clara sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``clara update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``clara update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: clara curator status)"
    )
...
def _reload_process_scan_modules() -> None:
    ...
    ``from clara_cli._subprocess_compat import bounded_probe_run``. If the
...
def _finish_dashboard_update_cleanup(
    ...
    *already_restarted_units* forwards the systemd unit names (no
    ...
    print("    clara dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    ``clara_cli.main`` re-export surface and guards the #49145 regression.
    """
...
def _stage_replacement(src: str, dst: str) -> str:
    ...
    untouched.
    """
    staging = f"{dst}.clara-update-staging"
    backup = f"{dst}.clara-update-old"
...
def _discard_staged(staged) -> None:
    ...
    the tree. The user then follows the "re-run `clara update`" advice with
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``clara_constants.py``
...
def _assess_parked_branch_switch(
    ...
    stale feature branch left behind by earlier tooling; ``clara update``
...
def _print_parked_branch_skip_warning(
    ...
    print(f"    git -C {cwd} checkout {target_branch} && clara update")
    print(
...
def _print_update_completion(message: str) -> None:
    ...
    action_id = os.environ.get("CLARA_ACTION_ID", "")
    if len(action_id) == 32 and all(char in "0123456789abcdef" for char in action_id):
        print(f"=== clara-update completed {action_id} ===")
...
def _refuse_update_for_contended_shims(exc: BaseException) -> None:
    ...
    """
    print("✗ Cannot continue the update: live Clara launcher(s) could not be")
    print("  moved aside:")
    for name in getattr(exc, "failed_shims", []) or ["clara.exe"]:
        print(f"    {name}")
    print("  Another process is holding this install's venv — typically Clara")
    print("  Desktop, a gateway, or another clara REPL — and mutating the venv")
    print("  now would strand it half-updated.")
    print("  The dependency install has been deferred: close the process(es)")
    print("  above, then run any `clara` command to finish it automatically.")
...
def _should_zip_fallback_on_update_error(exc: BaseException) -> bool:
    ...
    A dependency-install failure (locked ``clara.exe`` / ``uv pip install``
...
def _zip_overlay_block_reason(
    ...
    two-phase replace creates ``*.clara-update-staging`` siblings inside the
...
_ZIP_STAGING_ARTIFACT_SUFFIXES = (".clara-update-staging", ".clara-update-old")
...
def _abort_zip_update_if_dirty_tree() -> None:
    ...
    print("  Stash or commit your changes, then rerun `clara update`.")
...
def _post_update_sqlite_runtime_status():
    ...
    from clara_cli.sqlite_runtime import probe_sqlite_runtime
...
def _print_verified_update_completion(message: str) -> bool:
    """Print a success completion only after probing the next Clara runtime."""
    ...
    print(
        "  Rebuild the Clara venv with a uv-managed Python, restart Clara, "
        "then verify with `clara doctor`."
    )
    return False
...
def _clear_stale_sqlite_sidecars(db_path: Path) -> None:
    ...
    database (a crashed writer, or a second Clara process the updater's drain
...
def _print_update_summary(
    *,
    ...
    if node_failures or not desktop_build_ok or not sqlite_runtime_ok:
        ...
        if not desktop_build_ok:
            print("  Run `clara desktop` to retry the desktop rebuild.")
        if not sqlite_runtime_ok:
            print(
                "  The Python runtime remediation did not complete. Run `clara "
                "update` again; if SQLite is unchanged, rebuild the Clara venv "
                "with a uv-managed Python, restart Clara, then verify with "
                "`clara doctor`."
            )
    else:
...
def _write_gateway_update_exit_code(ok: bool) -> None:
    path = get_clara_home() / ".update_exit_code"
    try:
...
def _restore_state_db_from_snapshot(state_path: Path, snap_state: Path) -> bool:
    ...
    """
    from clara_cli.backup import _foreign_db_holder_pids, verify_sqlite_integrity
    from clara_cli.sqlite_safe_read import LiveConnectionError, offline_file_access

    holders = _foreign_db_holder_pids(state_path)
    if holders:
        print(
            f"  ✗ Auto-restore refused: process(es) {holders} still hold "
            "state.db or its WAL open. Stop them (clara gateway stop), "
    ...
    try:
        ...
        print(
            ...
            "handles (or restart Clara) and retry."
        )
        return False
...
def _verify_and_restore_one_state_db(home: Path, *, label: str) -> None:
    ...
    """
    try:
        from clara_cli.backup import _quick_snapshot_root, verify_sqlite_integrity
...
def _verify_and_restore_state_dbs_post_update() -> None:
    ...
    try:
        from clara_cli.backup import _sibling_profile_homes
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False) -> bool:
    """Update Clara Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `clara update "
            f"--branch {branch}`, or update against main with `clara update`."
        )
        _m().sys.exit(1)
    ...
    tmp_dir = tempfile.mkdtemp(prefix="clara-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"clara-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for clara-agent itself
        ...
        # GitHub ZIPs extract to clara-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"clara-agent-{branch}")
        ...
        try:
            for item in entries:
                ...
                # Clara.exe) exists only in the LIVE tree. Swapping `apps`
    ...
    from clara_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    if uv_bin:
        ...
        from clara_cli.managed_uv import managed_python_env
    ...
    if not import_ok:
        ...
        print("  Re-run `clara update` to complete it.")
        _m().sys.exit(1)
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: clara skills list-modified  "
    ...
    try:
        from clara_cli.model_catalog import seed_cache_from_checkout
    ...
    try:
        from clara_cli.update_receipt import finalize_update_receipt
...
_AUTOSTASH_NAME_PREFIX = "clara-update-autostash-"

#: Age past which a leftover ``clara-update-autostash-*`` entry is called out
...
def _warn_orphaned_update_autostashes(git_cmd: list[str], cwd: Path) -> int:
    ...
    Clara never drops one automatically.
...
def _park_stashed_changes(stash_ref: str) -> None:
    ...
    Used by ``clara update --keep-stash`` (the desktop updater's mode): the
...
def _reject_unsafe_stash_restore(
    ...
    print()
    print("✗ Restored local changes made the Clara agent unexecutable.")
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Clara behaves unexpectedly.")
    ...
    if restore.returncode != 0 and not has_conflicts and (
        ...
        # files makes clara completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Clara couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Clara couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Clara behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Clara couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Clara couldn't drop "
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_clara_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_clara_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _invalidate_update_cache():
    ...
    ``clara update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from clara_constants import get_default_clara_root

    default_home = get_default_clara_root()
...
# ``fleet_restart_pending`` lives under CLARA_HOME (not next to the venv).
...
def _fleet_restart_pending_marker_path() -> Path:
    """CLARA_HOME breadcrumb for a pull that has not yet restarted the fleet."""
    return get_clara_home() / _FLEET_RESTART_PENDING_NAME
...
def _current_checkout_sha() -> str | None:
    ...
    try:
        from clara_cli.build_info import get_code_identity
...
def _warn_pending_fleet_restart(*, startup: bool = False) -> None:
    ...
    print(
        "⚠ A previous `clara update` pulled new code but did not "
    ...
    if startup:
        print(
            "  Run `clara update` or `clara gateway restart`.",
            file=stream,
        )
...
def _restart_systemd_gateway_units_best_effort(failed: list) -> None:
    """Best-effort ``systemctl restart`` of every clara-gateway/serve unit."""
...
def _run_pending_fleet_restart() -> bool:
    ...
    try:
        _m()._purge_stale_clara_modules()
    except Exception:
        pass
    try:
        from clara_cli.gateway import (
            find_gateway_pids,
            is_macos,
            is_windows,
    ...
    try:
        ...
        if is_windows():
            try:
                from clara_cli import gateway_windows
...
def _apply_pending_fleet_restart_catchup() -> None:
    """On an already-up-to-date ``clara update``, finish a skipped restart.
    ...
    if _run_pending_fleet_restart():
        ...
        return
    print("  ⚠ Fleet restart incomplete. Recover with: clara gateway restart")
    sys.exit(1)
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "clara.exe"
    lines = ["✗ Another clara.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Clara Desktop, exit any open `clara` REPLs, and")
    lines.append("  stop the gateway (`clara gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `clara update --force` if you've already")
...
def _classify_concurrent_instance(pid: int) -> str:
    ...
    from clara_cli._scan_venv_blockers import _is_pausable_gateway  # noqa: PLC0415
...
def _filter_non_gateway_concurrent_instances(
    ...
    ``clara update``. If every concurrent instance is a gateway, the pause
    ...
    TUI shell, a Clara Desktop backend child, an unrelated ``clara`` REPL)
...
def _capture_active_tool_dependencies() -> list[str]:
    """Snapshot Python dependencies installed explicitly through ``clara tools``."""
    try:
        from clara_cli import tools_config

        return tools_config.active_restorable_python_tool_dependencies()
    except Exception as exc:
        logger.debug("Could not snapshot active Clara Tools dependencies: %s", exc)
        return []
...
def _restore_active_tool_dependencies(
    ...
    *,
    ...
    """Restore allowlisted ``clara tools`` dependencies into a rebuilt venv.
    ...
    try:
        ...
        logger.debug("Clara Tools dependency restore skipped (import failed): %s", exc)
        return
    ...
    print()
    print(f"→ Restoring {len(missing)} Clara Tools dependency set(s)...")
    restored: list[str] = []
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    first use. ``clara update`` runs ``uv pip install -e .[all]`` which
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `clara update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `clara update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``hindsight-all``), NOT in Clara' editable-install extras or
    ...
    try:
        from clara_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from clara_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$CLARA_HOME/bin/uv``, but on Termux the official
    ...
    """
    from clara_cli.managed_uv import resolve_uv
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `clara update` is exactly the step
...
def _npm_lockfile_changed(clara_root: Path) -> bool:
    ...
    # landed must NOT skip the reinstall — otherwise every later `clara
    ...
    try:
        ...
        cache_file = clara_root / f".npm_lock_hash_{cache_key}"
...
def _record_npm_lockfile_hash(clara_root: Path) -> None:
...
def _repair_node_deps_on_current_checkout(
    ...
    *,
    ...
    clara update" — but the early return never reached the Node refresh,
    ...
    if node_failures:
        ...
        print("    Fix npm and re-run `clara update`.")
        print_completion(
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        from clara_constants import is_wsl
        ...
        if is_wsl() and path_npm and _m()._is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `clara update`.")
            failed = []
            if any(
    ...
    # Clara profile using this checkout. Keep one per-checkout cache under the
    # shared Clara root rather than rerunning npm once per named profile.
    shared_clara_root = get_default_clara_root()
    ...
    # `clara update` case. __PROT_0_Synchroclara__ and can block ~11s on a true cold
    ...
    if not _m()._npm_lockfile_changed(shared_clara_root):
    ...
    def _partial_update_failure(*labels: str) -> list[str]:
        ...
        print("    deps). Fix npm and re-run `clara update`.")
        return list(labels)
    ...
    nixos_env = with_clara_node_path(_m()._nixos_build_env())
    ...
    # `clara update` comes from the *desktop* build, not this step; that
    ...
    if result.returncode == 0:
        _record_npm_lockfile_hash(shared_clara_root)
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.clara/logs/update.log`` only, never the terminal.

    During ``clara update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``clara update --check``: fetch and report without installing.
    ...
    from clara_cli.gitlock import clear_stale_git_locks, clear_stale_tmp_packs
    ...
    if is_shallow:
        ...
        if head_sha and target_sha and head_sha == target_sha:
            ...
            from clara_cli.banner import _github_compare_behind
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``clara update`` without requiring a reinstall.  The
    ...
    ``clara`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/clara.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v clara'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/clara, code at /usr/local/lib/clara-agent).
    fhs_link = Path("/usr/local/bin/clara")
    ...
    path_comment = (
        "# Clara Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _ensure_acp_launcher() -> None:
    r"""Self-heal: install a ``clara-acp`` launcher next to the ``clara`` one.

    Mirrors the launcher block in ``scripts/install.sh`` so existing installs
    gain the ACP command on ``clara update`` without a reinstall.  ACP hosts
    (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving the
    ``clara-acp`` command name against the login-shell PATH; the console
    script of that name lives inside the install's venv, which is not on that
    PATH, so those hosts report Clara as not installed even when it is.

    The shim simply delegates to the sibling ``clara`` launcher with the
    ...
    No-op on Windows (install.ps1 stages the ``clara`` / ``clara-acp``
    launchers into the managed binary dir ``$ClaraHome\bin`` and puts THAT
    ...
    ``clara_cli._install_repair.ensure_windows_bin_launchers`` re-stages
    them) and wherever a ``clara-acp`` is already present next to the
    ``clara`` command.  Unwritable directories (e.g. ``/usr/local/bin`` as
    ...
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        clara_cmd = bin_dir / "clara"
        acp_cmd = bin_dir / "clara-acp"
        try:
            if not (clara_cmd.is_file() or clara_cmd.is_symlink()):
                continue
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of CLARA_HOME under
      ``backups/`` (restorable via ``clara import``; the #48200 wrong-path
    ...
    snapshot_id = None
    try:
        from clara_cli.backup import (
            _quick_snapshot_root,
            create_quick_snapshot,
            verify_sqlite_integrity,
        )

        # NOTE: this function later does `from clara_constants import
        # get_clara_home`, which makes the name function-local — the
        # module-level import is shadowed and unbound here. Alias explicitly.
        from clara_cli.config import get_clara_home as _get_home
        ...
        try:
            from clara_cli.backup import create_pre_update_snapshots_all_profiles
            ...
            if _sibling_snaps:
                ...
                try:
                    from clara_cli.update_receipt import record_step
    ...
    try:
        from clara_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Human-readable size
    from clara_cli.sizefmt import format_bytes

    size_str = format_bytes(size_bytes)

    # Render path using display_clara_home so the user sees ~/.clara/...
    try:
        from clara_constants import get_clara_home, display_clara_home

        home = get_clara_home()
        try:
            display_path = f"{display_clara_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  clara import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``clara update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``clara update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run clara from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.clara-bootstrap-complete`,
        ...
        managed_markers = (
            _m().PROJECT_ROOT / ".clara-bootstrap-complete",
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The clara.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m clara_cli.main
    ...
    calling process and its ancestors are always excluded (a CLI ``clara
    ...
    try:
        ...
        # fetch cmdline/cwd lazily for plausible Python/uv/Clara candidates.
    ...
    for proc in proc_iter:
        ...
        if not is_holder and not (
            ...
            or name_low in {"uv.exe", "uvx.exe", "clara.exe"}
        ):
            continue
        ...
        # the `-m clara_cli.main gateway run` argv, so autostarted gateways
...
# ``cryptography`` is the canonical case: ``clara_cli.main`` used to import
...
def _abort_dependency_sync_if_self_locked(gateway_resume=None) -> None:
    ...
    - The ``clara.exe`` console shim we were launched from (#88838, #89599).
      The marker cannot help here — every future ``clara`` launch is also the
...
def _defer_update_for_self_lock(loaded: list[str]) -> None:
    ...
    print("  sync has been deferred: the next `clara` launch will complete it")
...
def _clara_holder_subcommand(cmdline: str) -> str | None:
    """The actual Clara SUBCOMMAND a venv-holder argv runs, or None.

    Token-based, never substring (#90778: ``kanban --preserve-cache``
    contained \"serve\" and got labeled as the Desktop backend). Finds the
    ``clara_cli.main`` / ``clara(.exe)`` entry token, then returns the
    ...
    for i, token in enumerate(tokens):
        ...
        if low.endswith("clara_cli.main") and i > 0 and tokens[i - 1] == "-m":
            entry_idx = i
            break
        base = low.rsplit("\\", 1)[-1].rsplit("/", 1)[-1]
        if base in ("clara", "clara.exe"):
            entry_idx = i
            break
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    (#90778): a standalone ``clara dashboard`` must not be labeled as the
    ...
    """
    lines = [
        "✗ Other Clara processes are running from this install's venv:",
    ]
    hint_by_subcommand = {
        "serve": "  ← Clara backend (if the Desktop app is open, close it)",
        "dashboard": "  ← clara dashboard (stop it: clara dashboard stop, or close that terminal)",
        "gateway": "  ← gateway",
    }
    for pid, name, cmdline in matches[:6]:
        sub = _clara_holder_subcommand(cmdline)
    ...
    lines.append(
        "  Close the Clara desktop app / other Clara terminals, then re-run:"
    )
    lines.append("    clara update")
    lines.append("  (or use `clara update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _venv_launcher_ancestors(pids: list[int]) -> list[int]:
    ...
    # Never return ourselves or our own ancestry: a CLI ``clara update``
...
def _leftover_pausable_gateway_pids(
    ...
    """
    from clara_cli._scan_venv_blockers import _is_pausable_gateway
...
def _refuse_gateway_ancestor_tree_kill(
    ...
    A chat agent can launch plain ``clara update`` through its terminal tool.
    ...
    try:
        from clara_cli.gateway import _is_pid_ancestor_of_current_process
    ...
    print("  Otherwise, run `clara update` from a separate terminal.")
    return True
...
def _ledger_manual_serve_holders(
    ...
    Clara spawner). Returns the full ledger entries so the relauncher can
    rebuild the launch command from structured host/port/profile instead of
    parsing argv.
    """
    try:
        from clara_cli.process_identity import ledger_entries, spawner_is_dead
    except Exception:
        return []
...
def _relaunch_stopped_serves(token: dict) -> None:
    ...
    if skipped or failed:
        print(
            ...
            "restart them manually (clara serve --host <ip> --port <port>)."
        )
    try:
        from clara_cli.update_receipt import record_step
...
def _orphaned_desktop_backend_pids(
    ...
    shim before spawning clara-setup, and the update-in-progress marker
    ...
    the update with "Clara is still running" while the user stares at zero
    open windows (ryanc's 2026-08-09 01:59/02:17 failures).

    A holder qualifies only when BOTH hold:

    - its cmdline is a Clara backend (``clara_cli.main`` + ``serve`` /
      ``dashboard``), and
    ...
    managed-runtime descendants (the ``.clara-runtime`` interpreter child)
    ...
    def _is_backend(argv_low: str) -> bool:
        return "clara_cli.main" in argv_low and (
    ...
    for pid, _name, cmdline in matches:
        ...
        try:
            ...
            # value round-trips through pid_is_clara at kill time. (Reading
...
def _ledger_reapable_backend_pids(
    ...
    (``clara_cli.process_identity``). A holder qualifies when ALL of:
...
def _handoff_reapable_backend_pids(
    ...
    """PIDs of Clara ``serve``/``dashboard`` backends safe to reap during a
    ...
    run AND no live Desktop shim (``clara.exe``) is open, NOTHING legitimate
    ...
    - Only Clara backends (``clara_cli.main`` + ``serve``/``dashboard``)
    ...
    - Only runs when the CALLER has confirmed the hand-off context
      ...
      ``clara.exe`` shim) — outside that gate this function is never called
...
def _stop_process_trees(
    ...
    ``.clara-runtime`` interpreter child alive and holding the install open
    (#70026). Best effort; never raises.
    """
    from gateway.status import get_process_start_time
    from clara_cli._subprocess_compat import pid_is_clara, windows_hide_flags
    ...
    for entry in pids:
        ...
        try:
            ...
            if not pid_is_clara(
                ...
                logger.debug(
                    "Skipping taskkill of non-Clara or changed PID %s",
                    pid,
                )
                continue
            subprocess.run(
...
def _looks_like_desktop_control_plane(cmdline: str) -> bool:
    """True for this-install ``clara serve`` / ``clara dashboard`` argv.
    ...
    """
    if "clara_cli.main" not in (cmdline or "").lower():
        return False
    return _clara_holder_subcommand(cmdline) in ("serve", "dashboard")
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    clara.exe concurrent-instance guard does not see them. They still import
    ...
    print("→ Stopping Windows gateway process(es) before updating Clara...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m clara_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: clara gateway run")
...
def _cold_start_windows_gateway_after_update() -> bool:
    ...
    ``clara gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from clara_cli import gateway_windows
        from clara_cli.gateway import find_gateway_pids
    except Exception as exc:
        raise RuntimeError(
...
def _for_each_systemd_gateway_unit(
    ...
    *,
    ...
    """Process each ``clara-gateway*.service``/``clara-serve*.service`` unit
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        # ``unit.startswith("clara-serve")`` alone would also accept the
        # unrelated ``clara-server.service`` — require the exact base unit
        # or the hyphenated profile family instead (review on #83595).
        if not (
            unit == "clara-gateway.service"
            or unit.startswith("clara-gateway-")
            or unit == "clara-serve.service"
            or unit.startswith("clara-serve-")
        ):
            continue
...
def _service_unit_supports_graceful_sigusr1_restart(svc_name: str) -> bool:
    ...
    Only ``clara-gateway*`` units run ``gateway/run.py``, which installs the
    SIGUSR1 handler. ``clara-serve*`` units (#83438) don't, so sending them
    ...
    (``clara-gateway-helper`` is fine — profile units are
    ``clara-gateway-<profile>`` — but ``clara-gatewayd``-style names are
    not) can't be sent a SIGUSR1 it doesn't handle.
    """
    return svc_name == "clara-gateway" or svc_name.startswith("clara-gateway-")
...
def _warn_incomplete_gateway_fleet_restart(failed_units: list) -> None:
    ...
    from clara_cli.gateway import is_macos
    ...
    print("    clara gateway status")
    if any(not name.startswith("ai.clara.") for name in ordered):
        print("    systemctl --user restart <unit>   # user-scope")
        print("    sudo systemctl restart <unit>     # system-scope")
    if any(name.startswith("ai.clara.") for name in ordered):
...
def _restart_launchd_gateway_after_update(
    *, supervision_verify: bool = True
    ...
    try:
        ...
        try:
            ...
            print(
                ...
                "Recover manually: clara gateway restart"
            )
    ...
    # to a detached helper; both __PROT_1_asynchroclara__. A helper that dies before its
    ...
    print(
        ...
        "    Check logs, then: clara gateway restart"
    )
    return [], [current_label]
...
def _restart_macos_launchd_gateways(
    ...
    ``ai.clara.gateway*`` LaunchAgent must reload it — restarting only the
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    when ``clara_cli.gateway`` itself no longer imports, which is one of the
...
def _gateway_service_matches_profile(profile: str, service: object) -> bool:
    ...
    that ``clara-gateway-foobar.service`` was already restarted.  These are
    ...
    return name in {
        f"clara-gateway-{profile}",
        f"ai.clara.gateway-{profile}",
        f"gateway-{profile}",
    }


# Abort recovery lives in its own bounded module (review on #96235). Re-exported
# here because `clara_cli.main` and the update flow below address these names
# through `update_cmd`.
from clara_cli.update_abort_recovery import (  # noqa: E402
...
def _gateway_recovery_partition(
    ...
    only safe source here: re-importing ``clara_cli.gateway`` in the failing
    ...
    unrecoverable.  The fresh child runs a separate ``clara-serve*`` systemd
    ...
    ``clara serve``: it records no spawner, so it reads as ``manual-serve``.
        ...
        for runtime in getattr(plan, "runtimes", ()) or ():
            ...
            if kind == "gateway":
                ...
                if supervisor == "desktop":
                    ...
                    # `clara serve` sets neither CLARA_SPAWN nor
                    # CLARA_PARENT_PID, so it lands here as "manual-serve".
                    # Unit-backed serve runtimes are recovered by the
                    # fresh child's systemd pass, which enumerates
                    # `clara-serve*` from systemd instead of trusting this
                    ...
                    reason = (
                        ...
                        " unit pass when it owns a clara-serve* unit, else"
...
def _warn_gateway_restart_phase_aborted(exc: BaseException, pids) -> None:
    ...
    (e.g. importing ``clara_cli.gateway`` from the freshly pulled checkout)
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    ``clara update`` never touched them, so installs created before the
...
def _refresh_bootstrap_cache_scripts(branch: str = "main") -> None:
    ...
    The Desktop GUI updater (``clara-setup.exe``) executes
    ``$CLARA_HOME/bootstrap-cache/install-<ref>.ps1`` (or ``.sh``) for its
    ...
    try:
        ...
        cache_dir = Path(_m().get_clara_home()) / "bootstrap-cache"
...
def _resume_windows_gateways_after_update(token: dict | None) -> None:
    ...
    if relaunched or unmapped_relaunched:
        ...
        if not ready_pids:
            ...
            print("    Recover with: clara gateway restart")
            raise RuntimeError(
...
def _portable_git_candidates() -> list:
    ...
    The Clara-managed PortableGit tree lives under the SHARED root
    (``<root>/git/...``), not the profile-scoped CLARA_HOME
    (``<root>/profiles/<name>``), so a profile-scoped ``clara update`` must
    ...
    candidates = []
    try:
        for root in (get_default_clara_root(), Path(get_clara_home())):
            candidates.append(
...
def _locate_real_git() -> Optional[Path]:
    ...
    Clara-managed PortableGit copy; accept the first candidate that runs and
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``clara update`` sees a clean tree instead of autostashing every run.
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    ``install.ps1`` forever — so ``clara update``, which ships with the checkout
...
def _rebuild_desktop_after_update(
    ...
    # `clara desktop --build-only` subprocess re-imports the whole CLI stack
    ...
    desktop_build_cmd = [sys.executable, "-m", "clara_cli.main", "desktop", "--build-only"]
    ...
    #
    # Start the build subprocess with the Clara-managed Node on PATH: when
    # `clara update` runs inside the desktop updater chain (Desktop →
    # clara-setup → clara update), the shell PATH customizations are lost,
    # so a bare-PATH child would fail with `node: not found` before cmd_gui can
    # self-heal.
    from clara_constants import with_clara_node_path

    build_env = with_clara_node_path()
    ...
    if build_result.returncode != 0:
        print("  ⚠ Desktop build failed (run `clara desktop` to retry)")
...
def _venv_foreign_owned_paths(venv_root, limit: int = 5) -> list:
    ...
    A venv that was ever touched by ``sudo pip`` / ``sudo clara`` contains
    root-owned files (classically ``*.dist-info/INSTALLER``). A later normal
    ``clara update`` then dies mid-mutation inside ``uv pip install -e .``
    ("Permission denied (os error 13)") with ``venv/bin/clara`` already
...
def _refuse_update_if_venv_foreign_owned(project_root) -> None:
    ...
    print("  Updating now would fail midway (Permission denied) and leave Clara broken.")
    print("  This usually happens after running clara or pip with sudo. Offending paths:")
...
def _drain_or_signal_gateway_for_update(
    ...
    """Decide how ``clara update`` hands a running gateway over to new code.
    ...
    1. **Gateway is an ancestor of this process** — THREE-WAY DEADLOCK BREAK
       (#100179). When ``clara update`` runs INSIDE the gateway's own
       process tree (the clara-auto-update cron job is the canonical case),
       waiting for that gateway to exit is a circular wait::

           gateway  waits on all in-flight work units (#77184)
             └─ cron agent session waits on the `clara update` process
                  └─ `clara update` waits on the gateway to exit  ← back to A
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        from clara_cli.update_receipt import begin_update_receipt
    ...
    # running Clara runtime, its supervisor, and its running code version —
    ...
    # On Windows, abort early if another clara.exe is holding the venv shim
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        scripts_dir = _m()._venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _m()._detect_concurrent_clara_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `clara serve` backend) will keep .pyd
    ...
    # the clara.exe shim guard above, but its lock probe only checks the shim
    ...
    if _m()._is_windows() and not getattr(args, "force_venv", False):
        ...
        if _venv_holders:
            ...
            # the spawn ledger proves are orphaned Clara backends — the
            ...
            if _ledger_backends:
                print(
                    ...
                    "Clara backend process(es) hold the venv; stopping their trees"
                )
        ...
        if _venv_holders:
            ...
            if _orphan_backends:
                ...
                # its backend (and any .clara-runtime child) holding the
                # venv. Nothing will respawn an orphan, so reap the tree and
                # re-check instead of dead-ending with "Clara is still
        ...
        if _venv_holders:
            ...
            # `clara serve --host <ip>` powering a REMOTE Desktop holds the
        ...
        if _venv_holders:
            ...
            # the hand-off context those surviving Clara backends are leaks,
            ...
            try:
                ...
                if _scripts_dir is not None:
                    _no_live_shim = not _m()._detect_concurrent_clara_instances(_scripts_dir)
            except Exception:
                _no_live_shim = False
            if _handoff and _no_live_shim:
                ...
                if _handoff_backends:
                    print(
                        f"  ⚠ {len(_handoff_backends)} Clara backend process(es) "
    ...
    # process by design (a CLI `clara update` IS the venv python), and an
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if handed_off_sync:
                print("→ Finishing the dependency install handed off by clara.exe...")
            elif not healthy:
            ...
            if handed_off_sync or not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all Clara windows/gateways and re-run: clara update")
            else:
            ...
            if runtime_repaired is not None and not _m()._is_windows():
                ...
                print(
                    "  Any running Clara gateways, Desktop backends, or other "
        ...
        # orphan merge-conflict markers in clara_cli/config.py bricked
        # every user who ran ``clara update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``clara
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``clara update`` again later once a fix lands.")
                    else:
        ...
        # Before this guard, ``clara update`` printed "✓ Code updated!" and
        # reinstalled deps + rebuilt the desktop app against the stale tree —
        # no error, no warning, ``clara doctor`` healthy. Compare pre-pull
        ...
        if pre_pull_sha and post_pull_sha == pre_pull_sha:
            ...
            print(
                ...
                f"git -C {_m().PROJECT_ROOT} checkout {branch} && clara update"
            )
            _m()._resume_windows_gateways_after_update(_windows_gateway_resume)
            sys.exit(1)
        ...
        # ``clara update`` can catch up even when git is already up to date.
        ...
        # the old bytecode (e.g. get_clara_home added to clara_constants).
        removed = _m()._clear_bytecode_cache(_m().PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # marker survives and the next ``clara`` launch finishes the install
        ...
        # symbols in clara_constants or lazy_deps. The dependency install
        ...
        if lazy_ok:
            ...
            print(
                "  ⚠ Lazy-refresh recovery incomplete — run `clara` again "
        ...
        try:
            from clara_cli.macos_tcc_anchor import ensure_tcc_anchor
        ...
        # current — so copy it straight over ~/.clara/cache/model_catalog.json
        ...
        # seed_profile_skills() uses subprocess with an explicit CLARA_HOME so
        # it is not affected by sync_skills()'s module-level CLARA_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's CLARA_HOME env var points at the default or a named profile.
        try:
        ...
        try:
            from clara_cli.profiles import backfill_profile_envs
        ...
        # check `clara curator status`. Self-stamps after printing so it
        ...
        # Self-heal the clara-acp launcher for installs that predate it, so
        # ACP hosts (Zed, JetBrains, Buzz) can resolve Clara on PATH without
        # a reinstall.  No-op on Windows (the launcher migration below owns
        # that) and when already present.
        try:
            _ensure_acp_launcher()
        except Exception as e:
            logger.debug("clara-acp launcher self-heal failed: %s", e)

        # Migrate the Windows clara launchers to the managed binary dir
        # (the default Clara root's bin, next to the managed uv) and repair
        # them if they are missing. Earlier layouts put them inside the git
        # checkout (clara-agent\bin) or put venv\Scripts itself on PATH; the
        # in-checkout copies were swept by this command's own pre-update
        # autostash (git stash push --include-untracked) and, with
        # --keep-stash (the desktop updater), never restored — `clara`
        ...
        # the managed clone under the default Clara root).
        try:
            from clara_cli._install_repair import migrate_windows_bin_path
        ...
        # don't have it. Tying the refresh to ``clara update`` gives users a
        ...
        try:
            ...
            if (
                ...
                from clara_cli.tools_config import install_cua_driver
                ...
                # driver) keeps the installed version — `clara update`
                # must stay fast; `clara computer-use install --upgrade`
        ...
        # When running as ``clara update --gateway`` (spawned by the gateway's
        ...
        # ``user/clara-serve`` and ``system/clara-serve`` are two different
        ...
        #
        # Purge stale cached Clara modules FIRST: the import below pulls
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... clara-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... clara-gateway*)
            ...
            try:
                from clara_cli.gateway import _get_restart_exit_wait_budget
            ...
            # Discover all clara-gateway* units (default + profiles) plus
            # clara-serve* units (the Desktop app's backend, #83438).
            if supports_systemd_services():
                try:
                    _ensure_user_systemd_env()
                except Exception:
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        print(
                            ...
                            f"Check the gateway with: clara gateway status"
                        )
                        continue
                    ...
                    def _restart_one_systemd_gateway_unit(svc_name: str) -> None:
                        ...
                        # clara-serve has no such handler (it isn't
                        ...
                        if _manage_cmd is None:
                            ...
                            print(
                                ...
                                f"    To let `clara update` restart it automatically, allow\n"
            ...
            # Restart EVERY ai.clara.gateway* LaunchAgent, not only the
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: clara gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: clara -p <profile> gateway run  for each profile)"
                        )

            if failed_or_stale_units:
                gateway_fleet_restart_incomplete = True
                if gateway_mode:
                    _exit_code_path = get_clara_home() / ".update_exit_code"
                    try:
            ...
            try:
                from clara_cli.update_receipt import record_gateway_restart
            ...
            # the reporter hit through tui_gateway (hosted by `clara serve`,
        ...
        # Warn if legacy Clara gateway unit files are still installed.
        # When both clara.service (from a pre-rename install) and the
        # current clara-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `clara update` surfaces the issue until the user migrates.
        try:
            from clara_cli.gateway import (
                has_legacy_clara_units,
                _find_legacy_clara_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_clara_units():
                print()
                print("⚠ Legacy Clara gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_clara_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (clara.service) fight the current")
                print("  clara-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    clara gateway migrate-legacy")
                print()
        ...
        #
        # Forward the systemd units restarted above (includes clara-serve*,
        ...
        # `clara serve` (no unit) is left running its pre-update
        ...
        print("  clara model              # Select provider and model")
        ...
        try:
            from clara_cli.update_receipt import (
        ...
        try:
            if _pre_update_plan is not None and _pre_update_plan.runtimes:
                ...
                if report_unaccounted_runtimes(_runtime_outcomes):
                    gateway_fleet_restart_incomplete = True
                try:
                    import clara_cli.update_receipt as _ur
        ...
        if gateway_fleet_restart_incomplete:
            ...
            # ``clara update`` still runs the catch-up restart.
            sys.exit(1)
        _clear_fleet_restart_pending_marker()
            ...
            if _called_process_error_is_python_dep_install(e):
                ...
                if _m()._is_windows():
                    ...
                    print(
                        ...
                        '"from clara_cli.main import main; main()" update --yes'
                    )
            try:
                from clara_cli.update_receipt import finalize_update_receipt
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state
      (typically the freshly-pulled ``clara_cli.gateway`` no longer imports,
