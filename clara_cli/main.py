#!/usr/bin/env python3
"""
Clara CLI - Main entry point.

Usage:
    clara                     # Interactive chat (default)
    clara chat                # Interactive chat
    clara gateway             # Run gateway in foreground
    clara gateway start       # Start gateway as service
    clara gateway stop        # Stop gateway service
    clara gateway status      # Show gateway status
    clara gateway install     # Install gateway service
    clara gateway uninstall   # Uninstall gateway service
    clara setup               # Interactive setup wizard
    clara logout              # Clear stored authentication
    clara status              # Show status of all components
    clara cron                # Manage cron jobs
    clara cron list           # List cron jobs
    clara cron status         # Check if cron scheduler is running
    clara doctor              # Check configuration and dependencies
    clara honcho setup                    # Configure Honcho AI memory integration
    clara honcho status                   # Show Honcho config and connection status
    clara honcho sessions                 # List directory → session name mappings
    clara honcho map <name>               # Map current directory to a session name
    clara honcho peer                     # Show peer names and dialectic settings
    clara honcho peer --user NAME         # Set user peer name
    clara honcho peer --ai NAME           # Set AI peer name
    clara honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    clara honcho mode                     # Show current memory mode
    clara honcho mode [hybrid|honcho|local]  # Set memory mode
    clara honcho tokens                   # Show token budget settings
    clara honcho tokens --context N       # Set session.context() token cap
    clara honcho tokens --dialectic N     # Set dialectic result char cap
    clara honcho identity                 # Show AI peer identity representation
    clara honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    clara honcho migrate                  # Step-by-step migration guide: OpenClaw native → Clara + Honcho
    clara --version           Show version and update status
    clara update              Update to latest version
    clara uninstall           Uninstall Clara Agent
    clara acp                 Run as an ACP server for editor integration
    clara sessions browse     Interactive session picker with search

    clara claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: clara_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``clara_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``clara update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``clara_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, clara crashes on import and the user can't run
# ``clara update`` to recover.  Missing the bootstrap means UTF-8 stdio
...
from clara_cli._subprocess_compat import suppress_platform_ver_console
from clara_cli.cli_output import line_input
...
# Two lines of inline path math so ``python clara_cli/main.py`` (script
# mode — sys.path[0] is clara_cli/, not the repo root) can import the
# canonical helpers; everything else lives in clara_cli._startup_fast.
_bootstrap_root = os.path.realpath(os.path.join(os.path.dirname(__file__), os.pardir))
if _bootstrap_root not in sys.path:
    sys.path.insert(0, _bootstrap_root)
from clara_cli import _startup_fast  # noqa: E402

# Early venv self-heal — MUST run before any third-party import below.  When
# a prior ``clara update`` left a recovery marker and a core package's import
# files were wiped (#57828 — failed lazy backend refresh), the module-level
# ``from clara_cli.env_loader import ...`` / ``from clara_cli.config import
...
# package directory, so if IT can't import, nothing else in clara_cli can
# either. It is also the canonical home of the probe/repair tables reused by
# the full recovery path below.
from clara_cli import _early_recovery as _early_recovery_mod
...
# no logs, live PID" class this watchdog exists for. ``clara_startup_watchdog``
...
# do — under-arming recreates OOF-298. Foreground `clara gateway run`
...
def _run_and_exit_oneshot(
    ...
    *,
    ...
    try:
        from clara_cli.oneshot import run_oneshot
...
def _set_process_title() -> None:
    """Set the process title to 'clara' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``clara tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``clara.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("clara")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"clara", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"clara")
        # Windows: the .exe name is already ``clara.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* clara_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("CLARA_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".clara", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    explicit ``--tui``/``CLARA_TUI=1``, then a real-TTY gate (a
    ...
    cron jobs, pipes run ``clara … chat -q`` with stdio on a pipe. This
    ...
    if "--cli" in argv:
        return False
    if os.environ.get("CLARA_TUI") == "1" or "--tui" in argv:
        return True
    try:
...
# `entry.tsx`; this is just the earlier cousin. ``CLARA_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("CLARA_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`clara --tui … >log`, CI capture):
...
def _active_profile_may_override_home_fast(clara_root: str) -> bool:
    return _startup_fast.active_profile_may_override_home(clara_root)
...
def _try_ultrafast_version() -> bool:
    """Handle ``clara --version`` before config/logging imports."""
...
from clara_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from clara_cli.subcommands.cron import build_cron_parser
from clara_cli.subcommands.sync import build_sync_parser
from clara_cli.subcommands.gateway import build_gateway_parser
from clara_cli.subcommands.profile import build_profile_parser
from clara_cli.subcommands.model import build_model_parser
from clara_cli.subcommands.setup import build_setup_parser

from clara_cli.subcommands.whatsapp import build_whatsapp_parser
from clara_cli.subcommands.slack import build_slack_parser
from clara_cli.subcommands.login import build_login_parser
from clara_cli.subcommands.logout import build_logout_parser
from clara_cli.subcommands.auth import build_auth_parser
from clara_cli.subcommands.status import build_status_parser
from clara_cli.subcommands.pause import build_pause_parser
from clara_cli.subcommands.webhook import build_webhook_parser
from clara_cli.subcommands.hooks import build_hooks_parser
from clara_cli.subcommands.doctor import build_doctor_parser
from clara_cli.subcommands.verify import build_verify_parser
from clara_cli.subcommands.security import build_security_parser
from clara_cli.subcommands.approvals import build_approvals_parser
from clara_cli.subcommands.dump import build_dump_parser
from clara_cli.subcommands.debug import build_debug_parser
from clara_cli.subcommands.backup import build_backup_parser
from clara_cli.subcommands.import_cmd import build_import_cmd_parser
from clara_cli.subcommands.import_agent import build_import_agent_parser
from clara_cli.subcommands.config import build_config_parser
from clara_cli.subcommands.skin import build_skin_parser
from clara_cli.subcommands.console import build_console_parser
from clara_cli.subcommands.update import build_update_parser
from clara_cli.subcommands.uninstall import build_uninstall_parser
from clara_cli.subcommands.dashboard import build_dashboard_parser, build_serve_parser
from clara_cli.subcommands.gui import build_gui_parser
from clara_cli.subcommands.logs import build_logs_parser
from clara_cli.subcommands.prompt_size import build_prompt_size_parser
from clara_cli.subcommands.memory import build_memory_parser
from clara_cli.subcommands.acp import build_acp_parser
from clara_cli.subcommands.tools import build_tools_parser
from clara_cli.subcommands.insights import build_insights_parser
from clara_cli.subcommands.monitoring import build_monitoring_parser
from clara_cli.subcommands.skills import build_skills_parser
from clara_cli.subcommands.pairing import build_pairing_parser
from clara_cli.subcommands.plugins import build_plugins_parser
from clara_cli.subcommands.mcp import build_mcp_parser
from clara_cli.subcommands.claw import build_claw_parser


def _require_tty(command_name: str) -> None:
    """Exit with a clear error if stdin is not a terminal.

    Interactive TUI commands (clara tools, clara setup, clara model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'clara {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any clara module import.
#
# Many modules cache CLARA_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("CLARA_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.clara/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set CLARA_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `clara mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to Clara' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo clara -p <name>` against the invoking user's home.
        ...
        candidate = home / ".clara" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # after the subcommand (`clara chat -p coder`), so keep scanning broadly.
    # The exception is command-argv passthrough regions such as `mcp add --args`.
    from clara_cli._parser import top_level_value_flag_sets
    ...
    # Mirrors clara_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If CLARA_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.clara/profiles/coder or
    # /opt/data/profiles/coder).  If CLARA_HOME points to the clara root
    # instead (e.g. systemd hardcodes CLARA_HOME=/root/.clara), we must
    # still read active_profile — the user may have switched profiles via
    # `clara profile use` and the gateway should honour that choice.
    # See issue #22502.
    clara_home_env = os.environ.get("CLARA_HOME", "")
    if profile_name is None and clara_home_env:
        if Path(clara_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the clara root.
    #
    ...
    # above) or pin ``CLARA_HOME`` to the profile directory (step 1.5), and
    # a bare invocation means "the root CLARA_HOME profile". If a supervised
    # default-profile child read active_profile here, switching the active
    # profile (e.g. via the dashboard or ``clara profile use``) would
    ...
    # restart routing):
    #   - CLARA_SUPERVISED_CHILD: generalized marker exported by the
    #     generated systemd unit, launchd plist, and Windows Scheduled-Task
    #     launchers (#74872).
    #   - CLARA_S6_SUPERVISED_CHILD: legacy s6 container marker (back-compat;
    ...
    #     predate the CLARA_SUPERVISED_CHILD marker. Consulted ONLY for
    ...
    #     running unrelated clara commands), so honoring it globally would
    #     silently disable the sticky active_profile for those.
    #   - CLARA_GATEWAY_EXTERNAL_SUPERVISOR: explicit external-supervisor
    #     opt-in (``clara gateway run --external-supervisor``).
    #
    ...
    def _under_gateway_supervisor() -> bool:
        if os.environ.get("CLARA_SUPERVISED_CHILD"):
            return True
        if os.environ.get("CLARA_S6_SUPERVISED_CHILD"):
            return True
        is_gateway_cmd = next(
        ...
        return os.environ.get(
            "CLARA_GATEWAY_EXTERNAL_SUPERVISOR", ""
    ...
    if profile_name is None and not _under_gateway_supervisor():
        try:
            ...
            active_path = get_default_clara_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set CLARA_HOME
    if profile_name is not None:
        try:
            from clara_cli.profiles import resolve_profile_env

            clara_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            clara_home = _resolve_sudo_user_profile_env(profile_name)
            if not clara_home:
            ...
            # A bug in profiles.py must NEVER prevent clara from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["CLARA_HOME"] = clara_home
...
# Windows launcher self-heal — the ``clara`` command users run is a COPY of
# the venv console script, staged into the managed binary dir (the default
# Clara root's ``bin``, next to the managed uv) by install.ps1. That dir
# lives OUTSIDE the git checkout precisely because an earlier layout staged
# the copies at ``<checkout>\bin``, where ``clara update``'s autostash
# (``git stash push --include-untracked``) swept them off disk; with the
# desktop updater's ``--keep-stash`` nothing restored them and ``clara``
...
# ``python -m clara_cli.main``. Costs a few stat calls when healthy; gates
# fail toward inaction so source checkouts are untouched. Sits AFTER the
# profile override on purpose — no clara module may be imported before
# profiles resolve. The launcher dir itself is per-machine (the helper
# anchors on the DEFAULT root, not CLARA_HOME), so profile sessions heal
# the same shared dir.
if sys.platform == "win32":
    try:
        from clara_cli import _install_repair as _install_repair_mod

        _install_repair_mod.ensure_windows_bin_launchers(_bootstrap_root)
    except Exception:
        pass

# Load .env from ~/.clara/.env first, then project root as dev fallback.
...
# Bridge security.redact_secrets from config.yaml → CLARA_REDACT_SECRETS env
# var BEFORE clara_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    # yaml.load — the SAME parse then serves clara_logging's
    # _read_logging_config and any later raw reads in this process, collapsing
    # 3-4 config.yaml parses per invocation into one.
    from clara_cli.config import read_raw_config as _read_raw_early

    _cfg_path = get_clara_home() / "config.yaml"
    if _cfg_path.exists():
        ...
        try:
            ...
            pass
        if "CLARA_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["CLARA_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `clara` subcommands
...
try:
    from clara_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from clara_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if clara_constants not importable yet
...
# Provider model-selection wizard flows extracted to clara_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (clara_cli.main._model_flow_*) keep resolving unchanged.
from clara_cli.model_setup_flows import (
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_clara_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("CLARA_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("CLARA_TERMUX_PREFETCH_UPDATES") == "1"
...
def _relative_time(ts) -> str:
    ...
    in :mod:`clara_cli.timefmt` so lightweight consumers don't have to
    import the whole CLI surface.
    """
    from clara_cli.timefmt import relative_time
...
def _has_any_provider_configured(*, strict_profile_scope: bool = False) -> bool:
    ...
    """
    from clara_cli.config import get_env_path, get_clara_home, load_config
    from clara_cli.auth import get_auth_status

    # Determine whether Clara itself has been explicitly configured (model
    ...
    if isinstance(model_cfg, dict):
        ...
        if isinstance(_default, dict):
            from clara_cli.config import split_model_config_default
        ...
        _model_name = ""
    _has_clara_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    # Check for Clara Portal OAuth credentials
    auth_file = get_clara_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Clara has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Clara to use their tokens.
    if _has_clara_config and not strict_profile_scope:
        try:
...
def _confirm_startup_expensive_model_override(args) -> None:
    ...
    try:
        ...
        from clara_cli.model_selection_guards import (
            combined_message,
            selection_warnings,
        )
    except Exception as exc:
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    ``clara -c`` from repo A continues repo A's last session rather than the
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, clara_bin
        cli_args: the original CLI arguments (everything after 'clara')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    clara_bin = container_info["clara_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo clara {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo clara {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print(
                    "Use 'clara sessions list' to see available sessions, or "
            ...
            try:
                from clara_cli.terminal_breadcrumbs import resolve_breadcrumb_session
...
def _print_tui_exit_summary(
    ...
    print(f"  clara --tui --resume {target}")
    if title:
        print(f'  clara --tui -c "{title}"')
    print()
    ...
    *starts* is the set of workspace keys the launch install explicitly scopes
    ...
    while stack:
        ...
        if not isinstance(entry, dict):
            continue
        # Workspace symlink (e.g. node_modules/@clara/ink → ui-tui/packages/…):
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @clara/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@clara" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/clara-ink/package.json",
    "packages/clara-ink/index.js",
    "packages/clara-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``CLARA_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("CLARA_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``CLARA_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("CLARA_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "CLARA_HOME": clara_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(clara_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(clara_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if clara_cli_dir is None:
        clara_cli_dir = Path(__file__).parent
    bundled = clara_cli_dir / "tui_dist" / "entry.js"
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``clara update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("CLARA_QUIET"):
            print(f"Restored missing TUI workspace: {tui_dir}")
        return

    print(
        "Error: the TUI workspace is missing from this Clara checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `clara update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Clara checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `clara --tui`\n"
        "If the checkout is still inconsistent, run `clara update --force`.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (CLARA_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("CLARA_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        # find_node_executable() prefers the managed $CLARA_HOME/node tree,
        # which is not on PATH — a bare which() would declare "node not found"
        # and exit on an install whose only Node is the one Clara installed,
        # and would pick a system Node over the managed one when both exist.
        from clara_constants import find_node_executable

        path = find_node_executable(bin)
        if not path and bin == "node":
            try:
                from clara_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("CLARA_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with CLARA_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset CLARA_TUI_DIR (e.g. `unset CLARA_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # clara_cli/tui_dist/entry.js but never ships ui-tui/ at all (that
    ...
    if (
        ...
        def _run_tui_install() -> subprocess.CompletedProcess:
            from clara_constants import with_clara_node_path
            ...
            return subprocess.run(
                ...
                encoding="utf-8",
                errors="replace",
                env=_npm_lifecycle_env(with_clara_node_path()),
            )

        result = _run_tui_install()
        if result.returncode != 0:
            # An npm outside the root package.json's `engines.npm` range fails
            # here before doing any work; repair once (upgrade a Clara-managed
            ...
            from clara_cli.npm_engine import maybe_repair_npm_engine
    ...
    if tui_dev:
        # Keep the local @clara/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @clara/ink resolves through
        # packages/clara-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "clara-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from clara_cli.oneshot import _normalize_toolsets
...
def _apply_tui_python_env(env: dict) -> None:
    ...
    src_root = str(env.get("CLARA_PYTHON_SRC_ROOT") or "").strip()
    if not src_root or not Path(src_root).is_dir():
        env["CLARA_PYTHON_SRC_ROOT"] = str(PROJECT_ROOT)

    cwd = str(env.get("CLARA_CWD") or "").strip()
    if not cwd or not Path(cwd).is_dir():
        env["CLARA_CWD"] = _safe_tui_cwd(env)

    python = str(env.get("CLARA_PYTHON") or "").strip()
    if os.path.dirname(python):
        python_path = Path(python)
        if not python_path.is_absolute():
            python_path = Path(env["CLARA_CWD"]) / python_path
    ...
    if not python_is_executable:
        env["CLARA_PYTHON"] = sys.executable
...
def _launch_tui(
    ...
    # TUI child is a clara process: propagate the profile-home contract via
    # the single factory; keep secrets (the TUI/agent needs provider creds).
    from tools.environments.local import build_subprocess_env
    env = build_subprocess_env(scrub_secrets=False, inherit_profile_home=True)
    try:
        from clara_cli.config import apply_terminal_config_to_env
        apply_terminal_config_to_env(env=env)
    except Exception:
        logger.debug("Failed to apply terminal config bridge for TUI launch", exc_info=True)
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="clara-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["CLARA_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    ...
    if worktree:
        try:
            ...
            if repo:
                ...
                # pruner __PROT_2_synchroclaraly__, and a repack must not block launch.
        ...
        if not wt_info:
            sys.exit(1)
        env["CLARA_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    _apply_tui_python_env(env)

    if model:
        env["CLARA_MODEL"] = model
        env["CLARA_INFERENCE_MODEL"] = model
    if provider:
        env["CLARA_TUI_PROVIDER"] = provider
        env["CLARA_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["CLARA_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["CLARA_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["CLARA_TUI_SKILLS"] = value
    if query:
        env["CLARA_TUI_QUERY"] = query
    if image:
        env["CLARA_TUI_IMAGE"] = image
    if checkpoints:
        env["CLARA_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["CLARA_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["CLARA_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["CLARA_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["CLARA_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["CLARA_ACCEPT_HOOKS"] = "1"
    ...
    # CLARA_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from a full os.environ snapshot (via
    # build_subprocess_env), an exported/stale value
    # in the user's shell would otherwise make a plain `clara --tui` try to
    ...
    # still set CLARA_TUI_RESUME themselves.
    env.pop("CLARA_TUI_RESUME", None)
    if resume_session_id:
        env["CLARA_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `clara update` so
    ...
    if code == 42:
        from clara_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``CLARA_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``clara kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``clara kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("CLARA_KANBAN_BOARD"):
        return
    try:
        from clara_cli.kanban_db import get_current_board

        os.environ["CLARA_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.clara/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Clara — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Clara still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    workers, cron jobs, and pipelines run ``clara … chat -q`` with stdout
...
def cmd_chat(args):
    ...
    if isinstance(_resume_raw, str) and _resume_raw.strip().lower() == "latest":
        ...
        if _last_id:
            ...
            print("Use 'clara sessions list' to see available sessions.")
            sys.exit(1)
    ...
    if isinstance(_resume_foreign, str) and _resume_foreign.strip().lower() in (
        ...
        print(f"  (later: clara --resume {_imported_id})")
    ...
    try:
        from clara_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from clara_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'clara doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Clara isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  clara setup")
        print()
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'clara setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from clara_cli.banner import prefetch_banner_data, prefetch_update_check
    ...
    #
    # FIRST RUN is the exception: with an empty ~/.clara/skills the banner
    ...
    def _skills_dir_is_unseeded() -> bool:
        try:
            from clara_cli.config import get_clara_home
            skills_dir = Path(get_clara_home()) / "skills"
    ...
    if _skills_dir_is_unseeded():
        ...
        try:
            import clara_cli.banner as _banner_mod
    ...
    if getattr(args, "ignore_rules", False):
        os.environ["CLARA_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["CLARA_SESSION_SOURCE"] = args.source
    ...
    try:
        ...
        # Mixed-version installs (new cli.py, older clara_cli.config) crash
        # here — e.g. missing resolve_turn_limit / split_model_config_default
        # (#96900). The agent-setup mixin prints this hint too late: ClaraCLI
        # construction already failed. Fast-chat launch also goes through
        # cmd_chat, so this one catch covers `clara` / `clara chat`.
        from clara_constants import emit_partial_update_hint
...
def cmd_gateway(args):
    ...
    from clara_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from clara_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from clara_cli.config import get_env_value, save_env_value
    from clara_constants import find_node_executable, with_clara_node_path
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Clara?")
        print()
    ...
    # has no creds.json.  Every subsequent `clara gateway` then paid a 30s
    ...
    session_dir = get_clara_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: clara gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `clara gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  clara gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Clara Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  clara gateway")
        ...
        print()
        print("  Or install as a service: clara gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'clara whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``clara whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``clara_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from clara_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_setup(args):
    ...
    from clara_cli.setup import run_setup_wizard
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from clara_cli.models import clear_provider_models_cache
            clear_provider_models_cache()
            print("  Cleared model picker cache.")
        except Exception:
            pass
    from clara_cli.setup import run_setup_action_with_navigation
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``clara model``) and the setup wizard
    ...
    from clara_cli.providers import (
    ...
    effective_provider = (
        config_provider or os.getenv("CLARA_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'clara model' for "
                "available providers, or run 'clara doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in clara_cli/models.py). A multi-member group shows one
    ...
    # Honor ``model_catalog.excluded_providers`` so the CLI ``clara model``
    ...
    # matching against clara_id / alias / canonical slug.
    _cli_excluded = {
        str(p).strip().lower()
    ...
    elif selected_provider == "clara":
        _model_flow_clara(config, current_model, args=args)
    ...
    # a leftover OPENAI_BASE_URL in ~/.clara/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.clara/.env if the active provider is not 'custom'.
    ...
    """
    from clara_cli.config import get_env_value, save_env_value, load_config
...
#
# Clara uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `clara model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `clara model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`clara_cli.plugins.PluginContext.register_auxiliary_task`) appear
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from clara_cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Clara only falls back to a lightweight backend (OpenRouter,")
        print("  Clara Portal) if the main model is unavailable.  Override a")
...
def _aux_select_for_task(task: str) -> None:
    ...
    up new ones through the normal ``clara model`` flow, then route aux tasks
    to them here.
    """
    from clara_cli.config import load_config
    from clara_cli.inventory import build_aux_picker_rows, format_aux_picker_entries
...
def _aux_flow_provider_model(
    ...
    from clara_cli.auth import _prompt_model_selection
    from clara_cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from clara_cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from clara_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Clara URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _remove_custom_provider(config):
    ...
    try:
        from clara_cli.curses_ui import curses_radiolist
...
# downstream call sites read `clara_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `clara` invocation — including
# fast paths like `clara --version` and slash-command dispatch that never
...
# surface so argparse wiring and test monkeypatches on clara_cli.main.<name>
# keep resolving unchanged. Importing those modules eagerly costs ~50ms on
# every `clara` invocation, including fast paths like `clara --version`
...
_LAZY_COMMAND_EXPORTS = {
    "clara_cli.sessions_cmd": (
        "cmd_sessions",
    ),
    "clara_cli.dashboard_procs": (
        "_detect_concurrent_clara_instances",
        "_filter_dashboard_respawn_candidates",
        "_kill_stale_dashboard_processes",
        "_scan_dashboard_processes",
    ),
    "clara_cli.update_cmd": (
...
def _self():
    ...
    keeps monkeypatches on clara_cli.main.<name> working, exactly like a
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from clara_cli.models import _PROVIDER_MODELS
...
def _prompt_api_key(
    ...
    """Shared API-key entry point for ``clara setup`` / ``clara model``.
    ...
    recover from a malformed paste without editing ``~/.clara/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from clara_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from clara_cli.config import save_env_value
    from clara_cli.secret_prompt import masked_secret_prompt
    ...
    from clara_cli.env_loader import format_secret_source_suffix
    ...
    if choice.startswith("c") and not pool_backed:
        ...
        print(
            f"  API key cleared.  Re-run `clara setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from clara_constants import display_clara_home as _dhh_fn

            print(
                f"    Clara will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate Clara CLI with a provider."""
    from clara_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from clara_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from clara_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from clara_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from clara_cli.cron import cron_command
...
def cmd_sync(args):
    ...
    if sub in {"enable", "disable"}:
        ...
        if not is_curation_eligible(skill):
            print(
                ...
                f"skills under ~/.clara/skills/ can sync.",
                file=sys.stderr,
            )
            return 1
    ...
    if sub == "status":
        ...
        if status.get("org_available"):
            ...
            if modified:
                print(
                    ...
                    f"{', '.join(modified)}\n"
                    f"  Share them back with `clara sync propose <skill>`. "
        ...
        if not status.get("logged_in"):
            print("\nNot logged into Clara Portal — sync is inert.", file=sys.stderr)
        elif not status.get("clara_admin"):
            print(
            ...
            print(
                "\nSync feature is off for this instance (set CLARA_SYNC_ENABLED=1 "
            ...
            print(
                ...
                "CLARA_SYNC_BASE_URL). Sync is inert.",
                file=sys.stderr,
            )
        return 0
    ...
    try:
        ...
        return 1
    if not identity.get("clara_admin"):
        print(
    ...
    try:
        if sub == "pull":
            ...
            result = ssc.push_skills(identity=identity, message="clara sync push")
        elif sub == "now":
            pull_res = ssc.pull_skills(identity=identity)
            push_res = ssc.push_skills(identity=identity, message="clara sync now")
...
def cmd_webhook(args):
    ...
    from clara_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``clara slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: clara slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `clara slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from clara_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from clara_cli.kanban import kanban_command
...
def cmd_project(args):
    ...
    from clara_cli.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from clara_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from clara_cli.doctor import run_doctor
...
def cmd_verify(args):
    ...
    from clara_cli.verify_cmd import run_verify_command

    sys.exit(run_verify_command(args))


def cmd_security(args):
    """Dispatch `clara security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from clara_cli.security_audit import cmd_security_audit
...
def cmd_approvals(args):
    """Dispatch `clara approvals <subcmd>`."""
    from clara_cli.approvals_suggest import approvals_command
...
def cmd_dump(args):
    ...
    from clara_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from clara_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from clara_cli.config import config_command
...
def cmd_skin(args):
    ...
    from clara_cli.skin_cmd import skin_command

    skin_command(args)


def cmd_backup(args):
    """Back up Clara home directory to a zip file."""
    if getattr(args, "quick", False):
        from clara_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from clara_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Clara backup from a zip file."""
    from clara_cli.backup import run_import
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    # `clara --version` pre-import fast path (the `version` subcommand
...
def cmd_uninstall(args):
    """Uninstall Clara Agent (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from clara_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from clara_cli.uninstall import run_gui_uninstall
    ...
    from clara_cli.uninstall import run_uninstall
...
# Update pipeline lives in clara_cli/update_cmd.py (main.py decomposition,
...
# monkeypatches on clara_cli.main.<name> keep resolving unchanged without
...
# validated against. Lives next to the checkout (NOT in CLARA_HOME) because
...
def _sweep_stale_bytecode_if_checkout_changed() -> None:
    ...
    ``clara update``, a manual ``git pull``, a ZIP update, a file-sync
    ...
    Update-time clears alone can never close this class: ``clara update``
    ...
    ``clara`` entry point compares the checkout fingerprint (cheap file
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    mtime comparison. ``git checkout`` / ``git pull`` / ``clara update``
    ...
    ``clara_cli/web_dist/`` (per vite.config.ts outDir), NOT ``web/dist/``,
    ...
    dist_dir = project_root / "clara_cli" / "web_dist"
...
def _web_ui_stamp_path() -> Path:
    """Return the path to the web UI build stamp file under $CLARA_HOME."""
    from clara_constants import get_clara_home
    return get_clara_home() / "web-ui-build-stamp.json"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``clara`` launcher present but ``clara_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the clara venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — clara venv python3, no nix-shell overhead
    ...
    # a clara venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``clara update`` to stash the
    lockfile — repeatedly.
    ...
    # of a Clara-managed install, or a freshly provisioned managed npm when
    ...
    run_env["PATH"] = with_clara_node_path(run_env)["PATH"]
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    project_root = web_dir.parent.parent if web_dir.parent.name == "apps" else web_dir.parent
    dist_index = project_root / "clara_cli" / "web_dist" / "index.html"
    try:
...
def _do_build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``clara web``).
    ...
    # build path usable on a stock `py -m clara_cli.main web` invocation.
    ...
    if not npm:
        ...
        return not fatal
    build_env = _npm_lifecycle_env(with_clara_node_path())
    ...
    # as `clara update`'s _update_node_dependencies() (ui-tui + web +
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (clara web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``clara update`` can unconditionally call ``clara desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``clara desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $CLARA_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $CLARA_HOME."""
    from clara_constants import get_clara_home
    return get_clara_home() / "desktop-build-stamp.json"
...
def _renderer_bundle_dir(desktop_dir: Path, *, source_mode: bool) -> Optional[Path]:
    ...
    # macOS: …/Clara.app/Contents/MacOS/Clara → …/Contents/Resources
    resources = (
...
def _renderer_bundle_torn(dist_dir: Path) -> bool:
    ...
    the intact SOURCE tree, ``clara desktop`` skips the rebuild that would fix
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``clara update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable_in(release_dir: Path) -> Optional[Path]:
    ...
    *release_dir* is electron-builder's ``directories.output`` — the live
    ...
    """
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Clara.app/Contents/MacOS/Clara"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Clara.exe",
            release_dir / "win-ia32-unpacked" / "Clara.exe",
            release_dir / "win-arm64-unpacked" / "Clara.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "clara",
            release_dir / "linux-unpacked" / "Clara",
            release_dir / "linux-arm64-unpacked" / "clara",
            release_dir / "linux-arm64-unpacked" / "Clara",
        ]
    ...
    if sys.platform == "win32" and len(existing) > 1:
        ...
        # Picking purely by mtime can then hand a wrong-architecture Clara.exe
...
# unpacked`` (or the mac ``Clara.app``) and the Electron unpack + asar + rename
...
# ``clara update`` used to report "partially complete" over an empty
...
#
# The desktop self-update chain (Desktop → clara-setup --update →
# `clara update` → `clara desktop --build-only` → relaunch) rebuilds
# Clara.exe on the end user's machine and used to verify only that the file
...
# wrong-architecture Clara.exe is caught BEFORE the updater replaces the
...
def _windows_native_machine_from_iswow64() -> Optional[str]:
    ...
    process arch) and rejected a correctly-built ARM64 ``Clara.exe``.
...
def _windows_native_machine() -> str:
    ...
    emulation: the desktop update chain runs an x64 clara-setup.exe (and thus
...
def _ensure_desktop_exe_launchable(
    ...
    print(f"✗ The built Clara.exe failed its integrity check: {error}")
    ...
    if restored is not None:
        print("  ↩ Update aborted — restored the previous working Clara.exe from backup.")
        print("    Your existing version was kept and still works. Run `clara desktop`")
        print("    (or the in-app update) again to retry with a fresh Electron download.")
        return restored, True

    print("  ✗ No usable backup was found to restore.")
    print("    Run `clara desktop --force-build` to rebuild, or re-run the Clara")
...
def _purge_electron_build_cache(
    ...
    '…/linux-unpacked/Clara'`` desktop build failure: a corrupt zip in the
...
def _redownload_electron_dist(
    ...
    *,
    ...
    dl_env = with_clara_node_path(env)
    if mirror:
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Clara.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Clara.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Clara.exe: Access is
    ...
    an unrelated "Clara" process is never touched. Best-effort: never raises.
...
def _desktop_macos_local_codesign(
    ...
    #    contains the outer Clara.app component, so an absolute-parts check
...
def _desktop_macos_relaunchable_fixup(
    ...
    *,
    ...
    "Clara is damaged and can't be opened" — and macOS TCC forgets every
    ...
    if exe is None:
        return True
    # exe = .../Clara.app/Contents/MacOS/Clara  ->  app bundle = .../Clara.app
    app = exe.parents[2]
...
def _desktop_macos_setup_tcc_identity(identity: str = "Clara Local Signing") -> bool:
    """Create/import a self-signed code-signing cert and configure Clara to use it.

    One-shot setup for ``clara desktop --setup-tcc-identity``. Creates a
    ...
    if not already_imported:
        ...
        tmp_dir = Path(tempfile.mkdtemp(prefix="clara-tcc-"))
        try:
            ...
            def _export_p12(extra_args: list) -> None:
                subprocess.run(
                    [
                        ...
                        "-out", str(p12), "-passout", "pass:claralocal",
                    ],
                    capture_output=True, check=True,
                )
    ...
    # Point Clara at the identity (config.yaml, not .env — it's not a secret).
    try:
        from clara_cli.config import set_config_value
    ...
    print(
        ...
        "stuck, reset it with:  tccutil reset All com.claraship.clara"
    )
    return True
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``clara desktop --build-only`` → electron-builder ``--dir``).
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Clara Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Clara Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def _detect_linux_password_store() -> str | None:
    ...
    fails under `clara desktop` because the launcher environment doesn't look
...
def _desktop_launch_options() -> tuple[list[str], str, str, str]:
    ...
    is one of "auto"/"1"/"0" (normalized for the CLARA_DESKTOP_DISABLE_GPU
...
def _register_linux_desktop_entry() -> None:
    """Install the XDG desktop entry for Clara Desktop (Linux only, best-effort).
    ...
    shell. ``clara uninstall --gui`` removes it.
    """
    try:
        from clara_cli.linux_desktop_entry import install_desktop_entry, is_supported
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from clara_logging import setup_logging as _setup_logging_gui
    ...
    # with_clara_node_path() copies os.environ when called with no arg.
    env = with_clara_node_path()
    if getattr(args, "fake_boot", False):
        env["CLARA_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["CLARA_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "clara_root", None):
        env["CLARA_DESKTOP_CLARA_ROOT"] = str(Path(args.clara_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["CLARA_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    else:
        env["CLARA_DESKTOP_CWD"] = os.getcwd()
    ...
    # `CLARA_DESKTOP_DISABLE_GPU=... clara desktop` and
    # `ELECTRON_OZONE_PLATFORM_HINT=... clara desktop` keep working.
    config_electron_flags, config_disable_gpu, config_password_store, config_ozone_hint = (
        _desktop_launch_options()
    )
    if config_disable_gpu != "auto" and "CLARA_DESKTOP_DISABLE_GPU" not in os.environ:
        env["CLARA_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    # `CLARA_DESKTOP_PASSWORD_STORE=... clara desktop` keeps working.
    if sys.platform == "linux" and "CLARA_DESKTOP_PASSWORD_STORE" not in os.environ:
        password_store = (
        ...
        if password_store:
            env["CLARA_DESKTOP_PASSWORD_STORE"] = password_store
    ...
    if getattr(args, "setup_tcc_identity", False):
        identity = getattr(args, "identity", None) or "Clara Local Signing"
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  clara gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            # Put the Clara-managed Node on PATH so npm's child scripts (which
            # shell out to bare `node`, e.g. electron-winstaller's
            # select-7z-arch.js) resolve it even when the parent PATH is
            # stripped — the desktop updater chain (Desktop → clara-setup →
            # clara update) loses shell PATH customizations. Wrapping the
            # NixOS build env keeps its PYTHON hint while restoring managed Node
            # ahead of a bare PATH (same idiom as the `clara update` path).
            nixos_env = with_clara_node_path(_nixos_build_env())
            ...
            if not source_mode:
                ...
                # holds Clara.exe locked on Windows, so the pack can't replace
            ...
            if (
                ...
                if restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Clara.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Clara.exe, close any")
                    print("  running Clara desktop window and retry.")
                print("  If the log shows Electron download retries, rebuild via a mirror:")
                print("    ELECTRON_MIRROR=<mirror-base-url> clara desktop --force-build")
                sys.exit(build_result.returncode or 1)
            if not source_mode:
                assert staging_dir is not None
                staged_executable = _staged_exe()
                # Locally-built apps are ad-hoc signed; make them relaunchable after
                # an in-place self-update (otherwise macOS reports "Clara is
                ...
                # success on a Clara.exe Windows cannot load (truncated PE from
    ...
    # Linux: register the app in the desktop launcher, so Clara shows up
    ...
    if source_mode:
        print("→ Launching Clara Desktop from source build...")
    ...
    print(f"→ Launching packaged Clara Desktop: {' '.join(launch_command)}")
    launch_result = subprocess.run(launch_command, cwd=desktop_dir, env=env, check=False)
    sys.exit(launch_result.returncode)


# Dashboard process-hygiene helpers live in clara_cli/dashboard_procs.py
...
# clara_cli.main.<name> keep resolving unchanged.
...
def _parse_dashboard_runtime(command: str) -> tuple[str, str, int] | None:
    ...
    if any(
        ...
        for pattern in (
            "clara dashboard",
            "clara_cli.main dashboard",
            "clara_cli/main.py dashboard",
        )
    ):
        mode = "dashboard"
    elif any(
        ...
        for pattern in (
            "clara serve",
            "clara_cli.main serve",
            "clara_cli/main.py serve",
        )
    ):
        mode = "serve"
    if mode is None:
...
_DASHBOARD_SYSTEMD_UNIT = "clara-dashboard.service"
...
def _restart_managed_dashboard_service(
    ...
    # Probe the user manager first: Clara installs Linux services in the
...
def _get_systemd_service_for_pid(pid: int) -> str | None:
    ...
    ``clara-serve.service``).  Returns ``None`` when the PID is not
    ...
    try:
        ...
        for line in text.splitlines():
            line = line.strip()
            # Format: 0::/system.slice/clara-serve.service
...
def _dashboard_cmdline_for_pid(pid: int) -> list[str] | None:
    ...
    is reconstructed, but clara launch commands don't embed exotic args).
...
def _respawn_dashboard_processes(commands: list[list[str]]) -> list[list[str]]:
    """Best-effort respawn of manually-started dashboards after ``clara update``.
    ...
    log_path = get_clara_home() / "logs" / "dashboard-restart.log"
    try:
...
# =========================================================================
# Fork detection and upstream management for `clara update`
# =========================================================================
...
# Install-scoped breadcrumbs live next to the venv (not under $CLARA_HOME)
...
def _recover_from_interrupted_install() -> None:
    """Finish update work left half-done by a prior ``clara update``.
    ...
    protocol stream (``clara acp`` speaks JSON-RPC on stdout) must never get
...
def _recover_core_update_marker_locked() -> None:
    ...
    """
    print(
        "⚠ A previous `clara update` was interrupted mid-install — "
        "finishing dependency installation now..."
    )

    # Windows: a normal ``clara.exe`` launch always has the launcher as an
    ...
    self_locked = _windows_running_clara_launcher_locked()
    if self_locked:
        install_prefix, install_env = _default_venv_install_target()
        print(
            "  → Running from clara.exe; applying package-only first aid, "
    ...
    try:
        from clara_cli import _install_repair as _ir
        ...
        if self_locked:
            print(
                "  Clara is still running from the launcher that needs "
                "replacing. Close other Clara windows, restart from a "
...
def _windows_shim_in_process_chain() -> Path | None:
    ...
    ``venv\\Scripts\\clara.exe`` is a launcher that runs the interpreter with
    ...
    ``clara ...`` command holds its own shim, and an editable install run
    ...
    ``clara.exe`` belonging to some other install never matches.
    """
    if not _is_windows():
        return None
    ...
    if scripts_dir is None:
        return None
    shims = {_norm_exe_path(shim): shim for shim in _clara_exe_shims(scripts_dir)}
    if not shims:
        return None
...
def _windows_running_clara_launcher_locked() -> bool:
    """True when a venv ``clara*.exe`` shim is this process or an ancestor.
...
_UPDATE_REEXEC_ENV = "CLARA_UPDATE_REEXEC"
...
def _reexec_dependency_sync_off_windows_shim() -> bool:
    ...
    The child re-runs ``clara update``, so the whole remaining flow — the
    ...
    from clara_constants import venv_python_path

    python_exe = venv_python_path(shim.parent.parent, windows=True)
    cmd = [str(python_exe), "-m", "clara_cli.main", *sys.argv[1:]]
...
def _default_venv_install_target() -> tuple[list[str], dict[str, str] | None]:
    ...
    try:
        ...
        uv_bin = None
    if uv_bin:
        from clara_constants import project_venv_dir
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``clara update`` is still progressing even if pip/uv itself is silent.
    """
...
def _venv_scripts_dir() -> Path | None:
    ...
    from clara_constants import project_venv_dir, venv_bin_dir
...
def _clara_exe_shims(scripts_dir: Path) -> list[Path]:
...
def _quarantine_running_clara_exe(
    ...
    when ``clara update`` runs, ``clara.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``clara.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next clara invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Clara Desktop backend child process
    ...
    2. If all retries fail, print a clear warning naming the most likely
       culprit (running Clara Desktop / gateway / REPL).

    The updater's own launcher is no longer one of those culprits: an update
    started from ``clara.exe`` re-runs itself under the venv Python before
    ...
    for shim in _clara_exe_shims(scripts_dir):
        if not shim.exists():
            continue
        ...
        print(
            "    Close Clara Desktop, exit other `clara` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `clara update`."
        )
...
def _cleanup_pending_shim_renames(scripts_dir: Path) -> int:
    """Drop reboot renames older Clara versions queued for our shims.

    Clara used to fall back to ``MoveFileExW(MOVEFILE_DELAY_UNTIL_REBOOT)``
    ...
    try:
        ...
        with winreg.OpenKey(
            ...
            kept, removed = _filter_pending_shim_renames(
                entries, _clara_exe_shims(scripts_dir)
            )
            if not removed:
                return 0
            if kept:
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_clara_exe`` if uv didn't write replacements.

    This is the safety-critical direction. A failed *quarantine* only aborts an
    update; a failed *restore* leaves the install with no ``clara`` on PATH,
...
class ShimQuarantineError(RuntimeError):
    """A live ``clara*.exe`` shim could not be renamed aside (#87331).
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``clara.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``clara.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``clara`` drops off PATH. This wraps
    ...
    if scripts_dir is not None:
        moved = _quarantine_running_clara_exe(scripts_dir, failed_out=failed)
    ...
    try:
        ...
        # quarantined aside and `clara` missing from PATH after a green
...
def _quarantine_stamp_ms(stale: Path) -> int | None:
    ...
    :func:`_quarantine_running_clara_exe`. We neither rescue nor delete those:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep — and where necessary RESCUE — ``clara.exe.old.*`` from updates.

    Called early on every clara invocation. Two cases the old unconditional
    ``unlink()`` got wrong, both ending with ``clara`` gone from PATH:

    1. **Orphan rescue.** If ``clara.exe`` is missing while
       ``clara.exe.old.*`` is present, that .old file is the ONLY surviving
    ...
    2. **Concurrency.** A fresh quarantine file may belong to an update in
       ...
       ``clara update`` does exactly this). Leave anything inside the grace
...
def _run_package_only_install(
    ...
    *,
    ...
    rewrite ``clara.exe``. The editable-install quarantine path would rename
...
def _repair_venv_via_import_probes(
    ...
    *,
    ...
    detected (#57828). Package-only reinstall — never rewrites ``clara.exe``.
    ...
    print("  ⚠ Venv repair incomplete. Run manually, then `clara update`:")
    print(
...
def _interpreter_scripts_dir() -> Path | None:
    ...
    """
    from clara_constants import venv_bin_dir
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``clara.exe`` / ``clara-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_clara_exe`` for the rationale.
    ...
    if (
        ...
        # Quarantining the wrong dir means the running clara.exe stays locked
    ...
    # hours later inside ``clara update``'s desktop-rebuild or skill-sync
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    On Windows, ``uv pip install -e .`` can register ``clara.exe`` in the
    wheel RECORD while the file never lands on disk — typically when the live
    ``clara.exe`` shim is locked during ``clara update``, or when uv/distlib
    skips a launcher write. The symptom is ``clara-agent.exe`` and
    ``clara-acp.exe`` present but ``clara.exe`` missing, so ``clara`` drops
    ...
    try:
        ...
        print(
            "  ⚠ Entry point repair failed; try `clara update --force` after "
            "closing other clara processes."
        )
        return
    ...
    if still_missing:
        print(
            ...
            "Workaround: python -m clara_cli.main <command>"
        )
    else:
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``clara update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``clara.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``clara`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `clara update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `clara update --force` after closing other clara processes."
        )
        return
...
class _UpdateOutputStream:
    """Stream wrapper used during ``clara update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.clara/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``clara update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``clara update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.clara/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``clara update --gateway``) the update is already
    ...
    try:
        ...
        # clara_cli.config.get_clara_home to simulate setup failure.
        from clara_cli.config import get_clara_home as _get_clara_home

        logs_dir = _get_clara_home() / "logs"
...
def cmd_update(args):
    """Update Clara Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Clara Agent")
        return
    ...
    if getattr(args, "plan", False):
        ...
        # Clara runtime across profiles, its supervisor, and its running
        # code version — without mutating anything. Safe on a live fleet.
        from clara_cli.update_inventory import (
    ...
    # exit 1 errors).
    from clara_cli.update_contract import (
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `clara update --branch=<x>` would actually pull.
    ...
    from clara_cli.update_lock import (
    ...
    try:
        ...
        try:
            from clara_cli.update_receipt import finalize_pending_update_receipt
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``clara -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `clara profile` — show current profile status
        from clara_cli.profiles import format_profile_label

        profile_name = get_active_profile_name()
        dhh = display_clara_home()
        ...
        if current is not None:
            ...
            if p.alias_path:
                ...
                print(f"Alias:          {alias_display} → clara -p {p.name}")
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print("Switched to: default (~/.clara)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  clara profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     clara -p {name} chat")
                else:
        ...
        from clara_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from clara_constants import get_clara_home as _hh
        ...
        from clara_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `clara profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → clara -p {name}  ({wrapper})")
        print()
        ...
        from clara_cli.profiles import profile_exists, validate_alias_name
        ...
        from clara_cli.profiles import normalize_profile_name, rename_profile
        ...
        from clara_cli.profiles import export_profile, get_profile_export_path
        ...
        from clara_cli.profiles import import_profile
        ...
        import tempfile
        from clara_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="clara_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  clara -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      clara -p {plan.manifest.name} chat")
        ...
        from clara_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `clara profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"clara -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from clara_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("clara_requires"):
            print(f"Requires:     Clara {data['clara_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from clara_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.clara_requires:
        print(f"  Requires: Clara {mf.clara_requires}")
...
def _report_dashboard_status() -> int:
    ...
    if not live:
        print("No clara dashboard or serve processes running.")
        return 0

    print(f"{len(live)} clara dashboard/serve process(es) running:")
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    provider on the spot — or point them at ``clara dashboard register`` for
    OAuth.
    ...
    try:
        from clara_cli.web_server import should_require_dashboard_auth
    ...
    try:
        from clara_cli.dashboard_auth import list_providers
        if list_providers():
    ...
    print("    [2] OAuth via Clara Portal (run `clara dashboard register`)")
    print("    [3] Cancel")
    print()
    ...
    try:
        from clara_cli.config import load_config, save_config
        from clara_cli.plugins_cmd import ensure_basic_auth_plugin_enabled_in_config
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    if sys.platform == "win32":
        from clara_cli.windows_ssh_runtime import read_token
    ...
    # The Desktop client writes the token under $HOME/.clara/desktop-ssh: a
    # literal "~/.clara/desktop-ssh" in apps/desktop/electron/remote-lifecycle.ts
    # expanded against the account's $HOME, independent of CLARA_HOME and the
    # active profile. Anchor validation to that same OS-home path, NOT to
    # get_clara_home(): a non-default sticky profile (or any CLARA_HOME pointing
    # elsewhere, e.g. a Docker /opt/data root) re-homes get_clara_home() and
    # would otherwise reject every token the client legitimately wrote (#69551).
    token_root = _Path.home() / ".clara" / "desktop-ssh"
    try:
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    Packaged Desktop sets ``CLARA_WEB_DIST`` to ``.../app.asar/dist`` or
    ``.../app.asar.unpacked/dist``. A standalone ``clara dashboard`` that
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No clara dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `clara update`.
    ...
    # `clara serve` is headless/non-interactive: fail closed on a corrupt
    ...
    if _token_file and not _headless_backend:
        raise SystemExit("--ssh-session-token-file is only valid with clara serve")

    # ── Sanitize Desktop-inherited env that hijacks a standalone launch ─
    # Desktop Electron spawns its backend with CLARA_DESKTOP=1 plus
    # CLARA_WEB_DIST=<packaged app.asar[/unpacked]/dist> (and often
    # CLARA_SERVE_HEADLESS=1 on the serve path). A shell that inherits
    # those vars then runs `clara dashboard` would otherwise:
    #   - serve the desktop renderer → "Desktop IPC bridge is unavailable"
    #     (issue #52945), or
    #   - disable the SPA via inherited CLARA_SERVE_HEADLESS.
    # Only strip Electron-packaged WEB_DIST contamination — caller-managed
    # CLARA_WEB_DIST overrides (dev / custom builds) must still work.
    # The desktop-spawned backend itself (CLARA_DESKTOP=1) keeps its dist.
    # Intentionally headless `serve` re-sets CLARA_SERVE_HEADLESS below.
    if os.environ.get("CLARA_DESKTOP") != "1":
        _inherited_web_dist = os.environ.get("CLARA_WEB_DIST", "")
        if _is_electron_packaged_web_dist(_inherited_web_dist):
            os.environ.pop("CLARA_WEB_DIST", None)
    if not _headless_backend:
        os.environ.pop("CLARA_SERVE_HEADLESS", None)
    ...
    # launches the dashboard (`worker dashboard` → CLARA_HOME points into
    ...
    if (
        ...
        and os.environ.get("CLARA_DESKTOP") != "1"
    ):
        ...
        reexec_argv = [
            sys.executable, "-m", "clara_cli.main",
            "-p", "default",
        ...
        from tools.environments.local import build_subprocess_env
        # Exact env preservation: CLARA_HOME is explicitly pinned to the
        ...
        # CLARA_HOME.  We must resolve the root explicitly instead of just
        # dropping CLARA_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV CLARA_HOME=/opt/data`), so an unset
        # CLARA_HOME falls back to $HOME/.clara = /opt/data/.clara — an
        ...
        # update-button guard also misfires).  get_default_clara_root()
        # returns the root for both layouts: ~/.clara for a standard install
        ...
        try:
            from clara_constants import get_default_clara_root
            env["CLARA_HOME"] = str(get_default_clara_root())
        except Exception:
            # Best-effort: if root resolution fails, fall back to the prior
            # behaviour (drop CLARA_HOME) rather than block the reroute.
    ...
    # the same logs directory as every other Clara surface.
    try:
        from clara_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if _headless_backend:
        ...
        os.environ["CLARA_SERVE_HEADLESS"] = "1"
    elif "CLARA_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["CLARA_WEB_DIST"])
            if "CLARA_WEB_DIST" in os.environ
            else PROJECT_ROOT / "clara_cli" / "web_dist"
        )
        if not (_dist_root / "index.html").exists():
            ...
            # default dist location is recoverable: a custom CLARA_WEB_DIST
            # points at a caller-managed directory the build cannot populate.
            _recoverable = "CLARA_WEB_DIST" not in os.environ
            if _recoverable:
        ...
        # CLARA_WEB_DIST is set without --skip-build: the build is skipped
        ...
        _dist_root = Path(os.environ["CLARA_WEB_DIST"]).expanduser()
        if not (_dist_root / "index.html").exists():
            print(f"✗ CLARA_WEB_DIST is set but no web dist found at: {_dist_root}")
            print("  Pre-build first:  npm install --workspace web && npm run build -w web")
            print("  Or unset CLARA_WEB_DIST to build and use the default web UI dist.")
            sys.exit(1)
        # Write the expanded path back: web_server reads CLARA_WEB_DIST raw
        # at import (no expanduser), so a validated "~/dist" would otherwise
        # pass here and still 404 there.
        os.environ["CLARA_WEB_DIST"] = str(_dist_root)
        print(f"→ Using web dist from CLARA_WEB_DIST: {_dist_root}")

    # Discover and load plugins so any DashboardAuthProvider plugin
    # (e.g. plugins/dashboard_auth/clara) registers BEFORE start_server's
    ...
    _mcp_discovery_after_bind = _headless_backend and os.environ.get("CLARA_DESKTOP") == "1"
    if not _mcp_discovery_after_bind:
        try:
            from clara_cli.mcp_startup import start_background_mcp_discovery
    ...
    from clara_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    """Register a self-hosted dashboard OAuth client with Clara Portal."""
    from clara_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from clara_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from clara_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from clara_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Clara log files."""
    from clara_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    """Open the safe Clara command console."""
    from clara_cli.console_engine import run_console_repl
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from clara_cli.models import CANONICAL_PROVIDERS as _cp
        return ["auto"] + [p.slug for p in _cp]
    except Exception:
        # Fallback: static list guarantees the CLI always works
        return [
            "auto", "openrouter", "clara", "openai-codex", "xai-oauth", "copilot-acp", "copilot",
...
def _first_positional_argv() -> str | None:
    ...
    ``clara -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``clara`` or only flags → defaults to ``chat``.
        return False
...
def _resolve_deferred_platform_cli_command(command_name: str | None) -> None:
    ...
    registers a top-level ``clara <name>`` command (e.g. Photon ->
    ...
    import it, so the CLI registration never happens and ``clara photon``
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("CLARA_TUI") == "1")
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo: chokepoint guarantee that CLARA_YOLO_MODE is set before ANY
    ...
    if not _is_tui_chat_launch(args):
        ...
        try:
            from clara_cli.plugins import start_background_plugin_discovery
            ...
            # unchanged: every __PROT_0_synchroclara__ reader goes through
    ...
    if _run_inline_mcp_discovery:
        try:
            # MCP tool discovery remains __PROT_1_synchroclara__ for entrypoints that do
...
def _apply_safe_mode(args) -> None:
    if not getattr(args, "safe_mode", False):
        return
    os.environ["CLARA_SAFE_MODE"] = "1"
    os.environ["CLARA_IGNORE_USER_CONFIG"] = "1"
    os.environ["CLARA_IGNORE_RULES"] = "1"
...
def _try_fast_serve_launch() -> bool:
    ...
    for unrelated Clara commands performs thousands of filesystem-backed
    ...
    """
    if os.environ.get("CLARA_DISABLE_FAST_SERVE_LAUNCH") == "1":
        return False
    ...
    try:
        from clara_cli.config import get_container_exec_info
...
def _try_fast_chat_launch() -> bool:
    ...
    ``clara`` / ``clara -w -s foo --yolo`` / ``clara chat`` don't need the
    ...
    """
    if os.environ.get("CLARA_DISABLE_FAST_CHAT_LAUNCH") == "1":
        return False
    argv = sys.argv[1:]
    ...
    from clara_cli._parser import build_top_level_parser
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if not _is_termux_startup_environment():
        return False
    if os.environ.get("CLARA_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["CLARA_DEFER_AGENT_STARTUP"] = "1"
            os.environ["CLARA_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["CLARA_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `clara --tui` is the hot path on phones. The full parser setup imports
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        mem_dir = get_clara_home() / "memories"
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_clara_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_clara_home()}/memories/\n")
    else:
        from clara_cli.memory_setup import memory_command

        memory_command(args)


def cmd_acp(args):
    """Launch Clara Agent as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from clara_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from clara_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from clara_cli.tools_config import tools_command
...
def cmd_monitoring(args):
    ...
    if action == "status":
        ...
        print(f"  OTel SDK:       {'installed' if otlp_exporter.is_available() else 'not installed'} "
              f"(optional extra: clara-agent[otlp])")
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from clara_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    elif getattr(args, "skills_action", None) in ("trust", "untrust"):
        _cmd_skills_trust(args)
    else:
        from clara_cli.skills_hub import skills_command

        skills_command(args)


def _cmd_skills_trust(args):
    """``clara skills trust [path]`` / ``clara skills untrust [path]``.
...
def cmd_pairing(args):
    from clara_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from clara_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from clara_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from clara_cli.claw import claw_command
...
def _advertise_agent_env() -> None:
    ...
    (``clara-agent`` in huggingface.js ``agent-harnesses.ts``): standard-var
    matching is exact, so any other value is counted as "unknown".
    ``CLARA_AGENT`` is the Clara-specific marker. setdefault: never
    clobber an outer harness (e.g. Clara running inside another agent's
    terminal).
    """
...
def main():
    """Main entry point for clara CLI."""
    # Cosmetic: make the process show up as 'clara' instead of 'python3.11'
    ...
    # Sweep stale ``clara.exe.old.*`` quarantine files left by previous
    # ``clara update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_clara_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    # If the checkout changed since the last launch (clara update, manual
    ...
    # Self-heal a venv left half-built by an interrupted ``clara update``
    ...
    # ``clara skills install update``) merely defers recovery one launch;
    # under-matching (missing ``clara -p work update``) would race a recovery
    ...
    # never restarted the fleet. Do NOT restart here — that is ``clara
    ...
    # =========================================================================
    # model command  (parser built in clara_cli/subcommands/model.py)
    # =========================================================================
    build_model_parser(subparsers, cmd_model=cmd_model)

    from clara_cli.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from clara_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://agent.claraship.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `clara model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    worktree_parser = subparsers.add_parser(
        ...
        description=(
            "Attended reclaim for the .worktrees/ directory clara -w sessions "
            ...
            "https://agent.claraship.com/docs/user-guide/cli#worktree-cleanup"
        ),
    )
    ...
    def _dispatch_worktree(_args):
        from clara_cli.worktree_cmd import cmd_worktree
    ...
    browser_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "default profile so Clara can copy it — DESTRUCTIVE (unsaved tabs "
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://agent.claraship.com/docs/user-guide/secrets/"
        ),
    )
    ...
    from clara_cli import secrets_cli as _secrets_cli
    from clara_cli import onepassword_secrets_cli as _op_secrets_cli
    ...
    # `clara proxy` (defined elsewhere in this file) is a separate INBOUND
    ...
    egress_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://agent.claraship.com/docs/user-guide/egress/iron-proxy"
        ),
    )

    from clara_cli import proxy_cli as _proxy_cli
    ...
    def _dispatch_egress(args):  # noqa: ANN001
        ...
        # from the inbound OAuth ``clara proxy`` subparser (dest='proxy_command').
    ...
    # migrate command
    # =========================================================================
    from clara_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in clara_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in clara_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)


    # =========================================================================
    # whatsapp command  (parser built in clara_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `clara whatsapp` which sets up "
    ...
    # =========================================================================
    # slack command  (parser built in clara_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from clara_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in clara_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in clara_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in clara_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in clara_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # pause / resume commands  (parser built in clara_cli/subcommands/pause.py)
    # =========================================================================
    build_pause_parser(subparsers)

    # =========================================================================
    # cron command  (parser built in clara_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)
    build_sync_parser(subparsers, cmd_sync=cmd_sync)

    # =========================================================================
    # webhook command  (parser built in clara_cli/subcommands/webhook.py)
    # =========================================================================
    build_webhook_parser(subparsers, cmd_webhook=cmd_webhook)

    # =========================================================================
    # peer command — bot-to-bot DMs across machines (peer Clara gateways)
    # =========================================================================
    from clara_cli.subcommands.peer import build_peer_parser

    build_peer_parser(subparsers)

    # =========================================================================
    # portal command — Clara Portal status + Tool Gateway routing
    # =========================================================================
    from clara_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from clara_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    from clara_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # =========================================================================
    # hooks command  (parser built in clara_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in clara_cli/subcommands/doctor.py)
    # =========================================================================
    build_doctor_parser(subparsers, cmd_doctor=cmd_doctor)

    # =========================================================================
    # verify command  (parser built in clara_cli/subcommands/verify.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in clara_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # approvals command  (parser built in clara_cli/subcommands/approvals.py)
    # =========================================================================
    build_approvals_parser(subparsers, cmd_approvals=cmd_approvals)

    # =========================================================================
    # dump command  (parser built in clara_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in clara_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in clara_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.clara/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo clara uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from clara_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in clara_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # import-agent command  (parser: clara_cli/subcommands/import_agent.py)
    # =========================================================================
    def cmd_import_agent(args):
        from clara_cli.agent_import import import_agent_command
        import_agent_command(args)

    build_import_agent_parser(subparsers, cmd_import_agent=cmd_import_agent)

    # =========================================================================
    # config command  (parser built in clara_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # skin command  (parser built in clara_cli/subcommands/skin.py)
    # =========================================================================
    build_skin_parser(subparsers, cmd_skin=cmd_skin)

    # =========================================================================
    # console command  (parser built in clara_cli/subcommands/console.py)
    # =========================================================================
    build_console_parser(subparsers, cmd_console=cmd_console)

    # =========================================================================
    # pairing command  (parser built in clara_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in clara_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from clara_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in clara_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``clara --help``, ``clara logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from clara_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from clara_cli.curator import register_cli as _register_curator_cli
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and Clara shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
        from clara_cli.pets import register_cli as _register_pets_cli
    ...
    try:
        from clara_cli.journey import register_cli as _register_journey_cli
    ...
    # =========================================================================
    # memory command  (parser built in clara_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in clara_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Linux.\n\n"
            "Use `clara computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `clara tools` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `clara computer-use doctor` to run cua-driver's\n"
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the Clara app. `status` reports the driver's grant\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            import os as _os
            import subprocess
            from clara_cli.tools_config import _cua_driver_contract_status
            ...
            override = _os.environ.get("CLARA_CUA_DRIVER_CMD", "").strip()
            if path:
                version = ""
                try:
                    from clara_cli.tools_config import _cua_driver_env
                ...
                if not contract.get("ready"):
                    ...
                    if override:
                        print(
                            "    Update the binary selected by CLARA_CUA_DRIVER_CMD, or unset "
                            "the override and run: clara computer-use install --upgrade"
                        )
                    else:
                        print("    Run: clara computer-use install")
                    return 1
                try:
                    ...
                    if st and st.get("update_available"):
                        ...
                        print("    Run: clara computer-use install --upgrade")
                    elif st:
                        print("  ✓ Up to date.")
                    else:
                        # Older driver (no check-update verb) or offline.
                        print("  Refresh to latest: clara computer-use install --upgrade")
                except Exception:
                    print("  Refresh to latest: clara computer-use install --upgrade")
                return 0
        ...
        if action == "permissions":
            ...
            if perms_action == "status":
                ...
                if not st["platform_supported"]:
                    ...
                    sys.exit(1)
                if not st["installed"]:
                    print("cua-driver: not installed. Run: clara computer-use install")
                    sys.exit(1)
                ...
                if st["can_grant"]:  # macOS TCC permissions
                    ...
                    if not st["ready"]:
                        print("  Grant: clara computer-use permissions grant")
    ...
    # =========================================================================
    # mcp command  (parser built in clara_cli/subcommands/mcp.py)
    # =========================================================================
    ...
    sessions_export.add_argument(
        ...
        help=(
            ...
            "md/qmd: output directory (default: <clara home>/session-exports)"
        ),
    )
    sessions_export.add_argument(
        "--format",
    ...
    sessions_import = sessions_subparsers.add_parser(
        "import",
        help="Import a Claude Code or Codex CLI session into Clara",
        description=(
            "Pull a conversation started in Claude Code (~/.claude/projects) "
            "or Codex CLI (~/.codex/sessions) into the Clara session store "
            "so it can be resumed with 'clara --resume <id>'. The foreign "
    ...
    # cmd_sessions lives in clara_cli/sessions_cmd.py (main.py decomposition).
    ...
    # and lets monkeypatches on clara_cli.main.cmd_sessions keep working.
    ...
    # =========================================================================
    # insights command  (parser built in clara_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)
    build_monitoring_parser(subparsers, cmd_monitoring=cmd_monitoring)

    # =========================================================================
    # claw command  (parser built in clara_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # NOTE: the `clara version` subcommand was removed — `clara --version`
    # / `-V` now carries the full output including update status.

    # =========================================================================
    # update command  (parser built in clara_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in clara_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in clara_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in clara_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in clara_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # for one release. The Clara-Setup.exe success screen tells users to
    # run `clara desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in clara_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in clara_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in clara_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``clara -c Pokemon Agent Dev`` → ``clara -c 'Pokemon Agent Dev'``
    ...
    # fails (e.g. 'clara -c model' where 'model' is consumed as the
    ...
    # --yolo: set CLARA_YOLO_MODE *before* plugin discovery.  The call to
    ...
    # so introspection/management commands (clara hooks list, cron
    ...
    # ``clara egress start`` refusing when credential_source=bitwarden
