"""Clara-managed uv and Python runtime repair.

Clara owns its own uv binary at ``$CLARA_HOME/bin/uv`` (or ``uv.exe`` on
Windows).  Every code path that needs uv resolves it from that single location.
If the binary is missing, ``ensure_uv()`` bootstraps it via the official
standalone installer with ``UV_UNMANAGED_INSTALL`` / ``UV_INSTALL_DIR`` pointed
at ``$CLARA_HOME/bin`` so the installer writes directly there — no PATH
probing, no conda guards, no multi-location resolution chains.

The Python backing the install is different: it is shared by every Clara
profile because the checkout's ``venv`` is shared.  Runtime repair therefore
uses an install-scoped store under ``<checkout>/.clara-runtime/python``. A
...
__PROT_0_synchroclara__ rollback and is parked for cleanup after the updating process
releases it.
"""
...
from clara_cli.sqlite_runtime import SQLiteRuntimeInfo, probe_sqlite_runtime

logger = logging.getLogger(__name__)

_PROJECT_ROOT = Path(__file__).resolve().parents[1]
_RUNTIME_DIR_NAME = ".clara-runtime"
_VENV_NAME = "venv"
_ALT_VENV_NAME = ".venv"
_REPAIR_LOCK_NAME = "runtime-repair.lock"
_MACOS_MANAGED_PYTHON_IDENTIFIER = "com.claraship.clara.managed-python"
...
def managed_uv_path() -> Path:
    """Return the path where Clara keeps *its* uv binary.

    ``$CLARA_HOME/bin/uv`` on POSIX, ``$CLARA_HOME\\bin\\uv.exe`` on
...
def managed_python_env(
    ...
    *,
    ...
    """Return a sanitized environment for Clara-private uv Python commands."""
    target = (
        Path(install_dir)
...
def _macos_sign_managed_python(python: Path) -> bool:
    ...
    TCC with a cdhash-only identity that changes whenever Clara provisions a
...
def _report_runtime_repair_failure(repair: RuntimeRepairResult) -> None:
    if repair.backup_venv is None:
        ...
        print(
            "    Sessions stay protected meanwhile: Clara keeps databases "
            "out of WAL mode on this SQLite build. The next `clara update` "
            "will retry."
        )
        return
...
class _UvResult(str):
    ...
    ``(path, fresh_bootstrap)`` tuple across releases. ``clara update`` runs
    the call site from the *old*, already-imported ``clara_cli.main`` against
...
def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    ...
    uv releases roughly weekly while many users run ``clara update`` daily;
    re-running a blocking network self-update on every invocation is waste
    and, offline, an unbounded hang risk. A stamp file under CLARA_HOME
    caches the last successful self-update time.
    """
    try:
        from clara_constants import get_clara_home

        stamp = get_clara_home() / "cache" / ".uv_self_update_stamp"
...
def update_managed_uv(
    *,
    ...
    Call this during ``clara update`` so the managed copy stays current.
...
def _reload_clara_constants():
    """Re-execute ``clara_constants`` from disk and return the fresh module.

    ``clara update`` imports ``clara_constants`` from the OLD checkout,
    ...
    return importlib.reload(clara_constants)
...
def _attempt_install_generation(
    ...
    *,
    ...
    try:
        ...
        logger.warning("uv resolved Python outside the Clara generation: %s", python)
        _remove_tree(generation, boundary=python_root)
        return None
...
def _install_safe_python_generation(
    ...
    *,
    ...
    # user isn't stuck on every `clara update` with no path to a fixed
...
def _windows_runtime_holders() -> tuple[bool, str]:
    if platform.system() != "Windows":
        return False, ""
    main_module = sys.modules.get("clara_cli.main")
    ...
    if holders:
        ...
        return True, f"other Clara processes still hold the venv (PID {pids})"
    return False, ""
...
def _windows_runtime_self_lock(live: Path) -> tuple[bool, str]:
    ...
    ancestors on purpose — a CLI ``clara update`` itself runs from the
    ...
    updater's own ``python.exe`` (or a waiting ``clara.exe`` launcher
    ...
    # Belt-and-braces: the venv\Scripts\clara.exe launcher stays mapped
...
def _refresh_managed_uv_catalog(uv_bin: str) -> bool:
    ...
    unmanaged installs.  Only the Clara-managed binary is ever refreshed;
...
def _default_live_venv(root: Path) -> Path:
    ...
    ``not-applicable`` on every ``clara update`` and stayed on
    journal_mode=DELETE forever — even though the WAL fallback warning
    promises that ``clara update`` repairs the runtime (issue class:
...
def repair_vulnerable_runtime(
    ...
    *,
    ...
    post-cutover smoke failures restore the parked venv __PROT_1_synchroclaraly__.
    """
    root = Path(project_root) if project_root is not None else _PROJECT_ROOT
    ...
    if self_locked:
        ...
        print(
            "    Retrying `clara update` from inside this venv cannot help: "
        ...
        print(f"      cd {root}")
        print("      <system Python> -m clara_cli.main update")
        print(
            "    Sessions stay protected meanwhile: Clara keeps databases "
...
def _install_uv(target: Path) -> None:
    ...
    ``$CLARA_HOME/bin/`` instead of ``~/.local/bin/``.
    """
