...
Used by ``clara kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``clara_cli/goals.py`` — same aux
...
CLARA_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    env_int("CLARA_KANBAN_SPECIFY_MAX_TOKENS", 6000),
)

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for the Clara Agent board.
...
def _profile_author() -> str:
    """Mirror of ``clara_cli.kanban._profile_author``. Kept local to
    avoid a circular import when kanban.py imports this module."""
    return (
        os.environ.get("CLARA_PROFILE")
    ...
    *,
    ...
    try:
        ...
        resp = call_llm(
            ...
            temperature=0.3,
            max_tokens=CLARA_KANBAN_SPECIFY_MAX_TOKENS,
