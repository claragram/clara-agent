...
This module hosts the Clara billing/subscription methods lifted out of
``cli.py``'s ``ClaraCLI`` class. ``ClaraCLI`` inherits
...
Import discipline mirrors ``clara_cli.cli_commands_mixin``:
...
class CLIBillingMixin:
    ...
    def _print_clara_credits_block(self) -> bool:
        """Print the Clara dollar balance block (two-bar view) when a Clara account
        ...
        truth). Falls back to the legacy ``clara_credits_lines`` text only when the
        model is unavailable. Agent-independent (a portal fetch gated on "a Clara
        ...
        wall-clock-bounded; honors CLARA_DEV_CREDITS_FIXTURE for offline testing.
        """
    ...
    def _print_usage_cta(self) -> None:
        ...
        usage read with the same nudge. Only called when a Clara account is logged
        in (the balance block printed), since both commands are Clara-account only.
        """
    ...
    def _show_subscription(self):
        """`/subscription` (alias `/upgrade`) — view the Clara plan + browser hand-off.
        ...
        if not state.logged_in:
            ...
            if state.error:
                ...
                _cprint(f"  💳 {_d('Not logged into Clara Portal.')}")
                print("  Run `clara portal` to log in, then /subscription.")
            return
    ...
    def _open_url_in_browser(self, url: str) -> bool:
        ...
        use (``clara_cli.auth``): ``webbrowser.open()`` returns ``True`` even when
    ...
    def _subscription_preview_and_confirm(self, state, tier_id, *, allow_stepup=True):
        ...
        from clara_cli.clara_billing import BillingError, BillingScopeRequired, post_subscription_preview
    ...
    def _subscription_handle_scope_required(self, state, *, retry, idempotency_key=None):
        ...
        step_up_clara_billing_scope directly (it opens the browser + blocks), then
        ...
        if not getattr(self, "_app", None):
            print("  Run `clara portal` and allow Remote Spending, then re-run /subscription.")
            return
        confirm_choices = [
        ...
        try:
            ...
            granted = step_up_clara_billing_scope(open_browser=True)
        ...
        try:
            from clara_cli import clara_billing as _nb
    ...
    def _show_billing(self, command: str = "/topup"):
        """`/topup` — Remote Spending for Clara (one interactive modal).
        ...
        if not state.logged_in:
            ...
            if state.error:
                ...
                _cprint(f"  💳 {_d('Not logged into Clara Portal.')}")
                print("  Run `clara portal` to log in, then /topup.")
            return
    ...
    def _billing_overview(self, state):
        ...
        if not state.cli_billing_enabled:
            ...
            self._billing_portal_hint(
                state,
                reason="A billing admin can turn it on from the portal's Clara Agent page to add funds here.",
            )
            return
    ...
    def _billing_require_admin(self, state) -> bool:
        ...
        if not state.cli_billing_enabled:
            ...
            self._billing_portal_hint(
                state,
                reason="A billing admin can turn it on from the portal's Clara Agent page before adding funds.",
            )
            return False
        return True
    ...
    def _billing_confirm_and_charge(self, state, amount):
        ...
        print(f"  {'─' * 41}")
        _consent = (
            "By confirming, you allow Claragram to charge your card."
        )
    ...
    def _billing_render_charge_error(self, state, exc):
        ...
        if isinstance(exc, BillingRemoteSpendingRevoked) or code == "remote_spending_revoked":
            ...
            print(f"  🔴 {who} Reconnect to restore — run `clara portal` to re-authorize.")
        elif isinstance(exc, BillingSessionRevoked) or code == "session_revoked":
            print("  🔴 Your session was logged out. Run `clara portal` to log in again.")
            ...
            print("  Remote spending is off for this account — a billing admin can turn it on from the portal's Clara Agent page.")
    ...
    def _billing_handle_scope_required(self, state, *, amount=None, idempotency_key=None):
        ...
        if not getattr(self, "_app", None):
            print("  Run `clara portal` and allow Remote Spending, then retry.")
            return
        confirm_choices = [
            ("yes", "Allow Remote Spending", "open your browser to authorize"),
            ("no", "Not now", "cancel"),
        ...
        if not (fresh.logged_in and fresh.cli_billing_enabled):
            print("  Remote Spending is allowed for this terminal, but it's still off for this org. A billing admin can turn it on from the portal's Clara Agent page, then run /topup again.")
    ...
    def _billing_auto_reload_flow(self, state):
        ...
        print()
        _ar_consent = (
            f"By confirming, you authorize Claragram to charge {card.masked} "
