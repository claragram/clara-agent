"""
Interactive setup wizard for Clara Agent.
...
Config files are stored in ~/.clara/ for easy access.
"""
...
from clara_cli.curses_ui import MenuNavigationEvent, MenuNavigationStart
...
_DOCS_BASE = "https://agent.claraprise.com/docs"
...
# display_clara_home imported lazily at call sites (stale-module safety during clara update)
...
from clara_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from clara_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Clara Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Clara using environment variables or config commands:")
    print_info("  clara config set model.provider custom")
    print_info("  clara config set model.base_url http://localhost:8080/v1")
    print_info("  clara config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'clara setup' in an interactive terminal to use the full wizard.")
    print()
...
class _SetupNavigationState:
    """Per-invocation navigation state for the __PROT_0_synchroclara__ setup wizard."""
...
_SETUP_NAVIGATION: ContextVar[_SetupNavigationState | None] = ContextVar(
    "clara_setup_navigation", default=None
)
...
def is_noninteractive() -> bool:
    ...
    ``CLARA_NONINTERACTIVE=1`` (see ``clara_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("CLARA_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``CLARA_NONINTERACTIVE=1`` or a closed/redirected
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from clara_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'clara setup')")


def _print_setup_summary(config: dict, clara_home):
    ...
    try:
        from clara_cli.auth import resolve_provider
    ...
    if not _provider_ready:
        print()
        print_warning("No inference provider is configured — Clara cannot chat yet.")
        print_info("  Finish this one step with either of:")
        print_info("    clara model            (pick any provider/model)")
        print_info("    clara setup --portal   (Clara Portal OAuth, no API key)")
    ...
    tool_status = []
    subscription_features = get_clara_subscription_features(config)
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'clara setup' to configure"))


    # Web tools (Exa, Parallel, Firecrawl, Tavily, or Keenable)
    if subscription_features.web.managed_by_clara:
        tool_status.append(("Web Search & Extract (Clara subscription)", True, None))
    ...
    if subscription_features.browser.managed_by_clara:
        tool_status.append(("Browser Automation (Clara Browser Use)", True, None))
    ...
    # Image generation — FAL (direct or via Clara), or any plugin-registered
    # provider (OpenAI, etc.)
    if subscription_features.image_gen.managed_by_clara:
        tool_status.append(("Image Generation (Clara subscription)", True, None))
    ...
    # Video generation — opt-in via `clara tools` → Video Generation.
    # Only show the row when a plugin reports available so we don't badger
    # users who don't care about video gen with a "missing" status line.
    if subscription_features.video_gen.managed_by_clara:
        tool_status.append(("Video Generation (FAL via Clara subscription)", True, None))
    else:
        try:
            from agent.video_gen_registry import list_providers as _list_video_providers
            from clara_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_clara:
        tool_status.append(("Text-to-Speech (OpenAI via Clara subscription)", True, None))
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'clara setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'clara setup tts'"))
    else:
    ...
    if _stt_feature is not None and _stt_feature.managed_by_clara:
        tool_status.append(("Speech-to-Text (OpenAI via Clara subscription)", True, None))
        ...
        if fw_ok:
            ...
            tool_status.append(
                ("Speech-to-Text (Local Whisper — not installed)", False, "run 'clara tools' → Speech-to-Text")
            )

    if subscription_features.modal.managed_by_clara:
        tool_status.append(("Modal Execution (Clara subscription)", True, None))
    elif cfg_get(config, "terminal", "backend") == "modal":
        if subscription_features.modal.direct_override:
            tool_status.append(("Modal Execution (direct Modal)", True, None))
        else:
            tool_status.append(("Modal Execution", False, "run 'clara setup terminal'"))
    elif managed_clara_tools_enabled() and subscription_features.clara_auth_present:
        tool_status.append(("Modal Execution (optional via Clara subscription)", True, None))

    # Home Assistant
    if get_env_value("HASS_TOKEN"):
        tool_status.append(("Smart Home (Home Assistant)", True, None))

    # Spotify (OAuth via clara auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'clara setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {clara_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('clara setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('clara setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('clara setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('clara setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('clara setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('clara config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('clara config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('clara config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('clara', Colors.GREEN)}              Start chatting")
    print(f"   {color('clara gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('clara doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `clara tools` and `clara setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``clara model``)
    ...
    provider added to ``clara model`` is automatically available here.
    ...
    # Delegate to the shared clara model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from clara_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: clara model")
    ...
    # on demand via `clara auth add`, `clara setup` vision, and
    # `clara setup tts`. This keeps both quick and full setup thin.


    # Tool Gateway prompt is already shown by _model_flow_clara() above.
    save_config(config)
...
def _install_neutts_deps() -> bool:
    ...
    from clara_cli.tools_config import _pip_install
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``clara model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from clara_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    choices = []
    providers = []
    if managed_clara_tools_enabled() and subscription_features.clara_auth_present:
        choices.append("Clara Subscription (managed OpenAI TTS, billed to your subscription)")
        providers.append("clara-openai")
    choices.extend(
        [
    ...
    selected_via_clara = selected == "clara-openai"
    if selected == "clara-openai":
        selected = "openai"
        print_info("OpenAI TTS will use the managed Clara gateway and bill to your subscription.")
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.clara/.env."
            )
    ...
    if selected == "neutts":
    ...
    elif selected == "openai" and not selected_via_clara:
        ...
        # via the Clara auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via clara setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'clara setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Clara runs shell commands and code.")
    ...
    # under ~/.clara/plugins/). Fail-soft: a broken plugin must not take the
    # setup wizard down.
    ...
    if selected_backend == "local":
        ...
        # configurable later via `clara setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `clara setup terminal`.
        ...
        if prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
            ...
            print_info(
                "Run `clara egress setup` then `clara egress start` to mint "
                "tokens and launch the proxy."
            )
        else:
            print_info(
                "Skipping egress firewall. You can enable it later with `clara egress setup`."
            )
        ...
        managed_modal_available = bool(
            ...
            and
            get_clara_subscription_features(config).clara_auth_present
        ...
        if use_managed_modal:
            ...
            print_info("Modal execution will use the managed Clara gateway and bill to your subscription.")
        ...
        print_info("Requires the optional SDK: pip install 'clara-agent[vercel]'")
        ...
        try:
            ...
            # Managed uv first: $CLARA_HOME/bin is never on PATH, so a bare
            # which() misses the uv Clara installed. Bootstrapping one is
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'clara-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into CLARA_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("CLARA_MAX_ITERATIONS")
    ...
    print_info("  Run `clara setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # CLARA_MAX_ITERATIONS from agent.max_turns at startup.
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.clara/logs/tool_calls.log (gateway only)")
...
def _setup_telegram_auto_result():
    ...
    try:
        from clara_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_clara_home(Path(get_clara_home()))
    except Exception:
        pass
...
def _profile_name_from_clara_home(clara_home) -> str | None:
    """Return the active profile name when CLARA_HOME is a profile dir."""
    if clara_home.parent.name == "profiles":
        return clara_home.name
    return None
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Clara delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Clara to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from clara_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://agent.claraprise.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://agent.claraprise.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  clara config edit")
    print_info("   Open config in your editor:  clara config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from clara_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Clara from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'clara setup gateway' later to configure.")
    else:
        for idx in selected:
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     clara config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
    ...
    # via `clara import` or `clara setup gateway`). Gating this on
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `clara setup tools` and `clara tools` use the same flow:
...
def setup_telemetry(config: dict):
    ...
    print_info("Collection is local. Sending them to Clara is a separate opt-in.")
    ...
    print_info("")
    print_info("Sending uploads each daily package to the Clara telemetry")
...
def _record_send_consent_change(*, enabled: bool) -> None:
    ...
    """
    try:
        from clara_cli.observability.shared_metrics import SharedMetricsStore
        from clara_cli.observability.shared_metrics_sender import (
            reconcile_send_consent,
        )
        from clara_cli.sqlite_util import write_txn
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from clara_cli.config
    ...
    if section_key == "model":
        ...
        from clara_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_clara.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_clara migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_clara", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Clara.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Clara to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Clara at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Clara at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Clara at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Clara at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Clara equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Clara config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Clara.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Clara's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(clara_home: Path) -> bool:
    ...
    print_info("Clara can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: clara claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=clara_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: clara claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Clara configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Clara config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Clara (use clara claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    """One-shot Clara Portal setup — OAuth + model pick + provider + Tool Gateway.

    Wired into ``clara setup --portal`` and ``clara portal``. This is the
    Clara-Portal slice of the first-time quick setup, collapsed into a single
    shareable command so a brand-new user goes from zero to a fully working
    Clara session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``clara setup`` and hunt for the quick-setup option.

    The login + model selection + provider switch + Tool Gateway opt-in are all
    delegated to ``_model_flow_clara`` — the exact same flow quick setup uses
    (``_run_first_time_quick_setup``) and the same one ``clara model`` runs
    when you pick Clara. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``clara portal`` always offers a model picker,
    and there is a single source of truth for the Clara onboarding steps.
    """
    ...
    print(color("│     ⚕ Clara Setup — Clara Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    print_info("    — all routed through your Clara Portal sub.")
    print()
    print_info("  Sign up: https://portal.claraprise.com/manage-subscription")
    print()

    # _model_flow_clara handles BOTH the logged-out path (device-code OAuth,
    # which selects a model internally) and the already-logged-in path (curated
    # Clara model picker), then offers the Tool Gateway opt-in and sets
    # provider=clara via the login/model save. This is the same routine quick
    # setup calls, so `clara portal` == quick setup's Clara step.
    try:
        from clara_cli.main import _model_flow_clara

        _model_flow_clara(config)
    except (KeyboardInterrupt, EOFError, SystemExit):
        # _login_clara raises SystemExit(130)/(1) on cancel/failure; the
        # logged-out path inside _model_flow_clara catches it, but the
        ...
        print_info("  You can retry later with `clara portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_clara error during `clara portal`: %s", exc)
        print()
        print_error(f"  Clara Portal setup encountered an error: {exc}")
        print_info("  You can retry later with `clara portal`.")
        return

    # Re-sync the in-memory config from disk — _model_flow_clara (and the
    ...
    print_info("  Run `clara portal info` to inspect routing.")
    print_info("  Run `clara` to start chatting.")


@contextmanager
def _setup_navigation_scope():
    """Install and reliably restore the setup menu navigation context."""
    from clara_cli.curses_ui import (
...
def run_setup_action_with_navigation(
    ...
    *,
    ...
    Shared commands such as ``clara model`` use the same provider/model
...
def _run_setup_wizard_impl(args):
    ...
    Supports full, quick, and section-specific setup:
      clara setup           — full or quick (auto-detected)
      clara setup model     — just model/provider
      clara setup tts       — just text-to-speech
      clara setup terminal  — just terminal backend
      clara setup gateway   — just messaging platforms
      clara setup tools     — just tool configuration
      clara setup telemetry — just local shared metrics
      clara setup agent     — just agent settings
    """
    from clara_cli.config import is_managed, managed_error
    if is_managed():
    ...
    # --portal: one-shot Clara Portal setup. Skips the rest of the wizard.
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Clara Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Clara Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Clara Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_setup_steps(
                [("Quick Setup", lambda: _run_quick_setup(config, clara_home))]
            )
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Clara configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'clara setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(clara_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up Clara?",
            [
                "Quick Setup (Clara Portal) — free OAuth login, no API keys, model + tools (recommended)",
    ...
    print_info(f"Data folder:  {clara_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'clara config edit'")
    ...
    # Tune later with `clara setup agent`.
    if not is_existing:
    ...
    def _gateway_step() -> None:
        ...
        from clara_cli.gateway import ensure_gateway_service
    ...
    _print_setup_summary(config, clara_home)


def _run_first_time_quick_setup(config: dict, clara_home, is_existing: bool):
    """Streamlined first-time setup via Clara Portal: OAuth, model, terminal & messaging.

    Routes straight to the Clara Portal provider — runs the device-code OAuth
    login, picks a Clara model, then configures the terminal backend and (optionally)
    a messaging platform. Applies sensible defaults for everything else (agent
    settings, tools); the user can customize later via ``clara setup <section>``
    or switch providers with ``clara model``.
    """
    from clara_cli.config import load_config

    # Step 1: Clara Portal — OAuth login + model selection.
    # _model_flow_clara() handles both the logged-out path (device-code OAuth,
    # which selects a model internally) and the already-logged-in path (curated
    # Clara model picker). Provider is set to "clara" by the login/model save.
    print()
    print_header("Clara Portal")
    print_info("One subscription, 300+ models, plus the Tool Gateway:")
    print_info("  web search, image generation, TTS, browser automation.")
    print_info("Sign up: https://portal.claraprise.com/manage-subscription")
    print()
    try:
        ...
        print()
        print_info("Clara Portal setup cancelled.")
    except Exception as exc:
        logger.debug("_model_flow_clara error during quick setup: %s", exc)
        print_warning(f"Clara Portal setup encountered an error: {exc}")
        print_info("You can try again later with: clara model")

    # Re-sync the wizard's config dict from disk — _model_flow_clara (and the
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'clara setup gateway'",
        ],
        0,
    )
    ...
    if gateway_choice == 0:
        ...
        # (e.g. via `clara import` from another machine).
        from clara_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
    ...
    print()
    print_info("  Configure all settings:    clara setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  clara setup gateway")
...
def _print_macos_fda_tip() -> None:
    ...
    print_info("  your terminal (and Clara.app if you use Desktop), or run:")
    print_info("    open \"x-apple.systempreferences:com.apple.preference"
               ".security?Privacy_AllFiles\"")
    print_info("  The grant is permanent — it survives every Clara update.")
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    ``clara-agent`` skill (the agent's operating manual for driving,
    configuring, and troubleshooting Clara) is always seeded — without
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``clara tools`` (which
    ...
    try:
        ...
        from clara_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``clara setup agent`` /
    ``clara config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, clara_home, is_existing: bool):
    ...
    print_info("essential `clara-agent` skill is always kept so the agent")
    print_info("can help you drive and configure Clara itself.")
    print()
    ...
    if path == 0:
        ...
        # `clara update` runs don't re-inject them. Essential skills (the
        # `clara-agent` operating manual) are still seeded by the sync.
        try:
        ...
        print_info("  Enable tools:        clara tools")
        print_info("  Seed skills:         clara skills opt-in --sync")
        print_info("  Add MCP servers:     clara mcp add")
        print_info("  Enable plugins:      clara plugins")
        print_info("  Tune agent settings: clara setup agent")
        print()
        _print_setup_summary(config, clara_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, clara_home)


def _blank_slate_walkthrough(config: dict, clara_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            set_bundled_skills_opt_out(True)
            # Essential skills (the `clara-agent` operating manual) are
            # still seeded even for an opted-out profile.
            sync_skills(quiet=True)
            print_info("No skills seeded (except the essential `clara-agent`")
            print_info("skill). A .no-bundled-skills marker keeps future")
            print_info("`clara update` runs from re-injecting them. Opt back in any")
            print_info("time with `clara skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `clara tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `clara plugins list` / `clara plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `clara plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `clara mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `clara mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   clara tools")
    print_info("  Seed skills:         clara skills opt-in --sync")
    print_info("  Add MCP servers:     clara mcp add")
    print_info("  Tune agent settings: clara setup agent")
    print()
...
def _run_quick_setup(config: dict, clara_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'clara setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Clara to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'clara setup gateway'.")
