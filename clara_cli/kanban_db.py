...
``<root>`` is the **shared Clara root** (the parent of any active
...
``clara -p <profile>`` joins the same board as the dispatcher that
...
the profile's Clara home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``CLARA_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``CLARA_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``clara kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.clara``. In Docker / custom
deployments where ``CLARA_HOME`` points outside ``~/.clara`` (e.g.
``/opt/clara``), ``<root>`` is ``CLARA_HOME``. Legacy env-var
overrides still work:

* ``CLARA_KANBAN_DB`` — pin the database file path directly.
* ``CLARA_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``CLARA_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``CLARA_KANBAN_DB``,
``CLARA_KANBAN_WORKSPACES_ROOT``, and ``CLARA_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/clara-kanban-v1-spec.pdf`` for the full
...
from clara_cli.sqlite_util import add_column_if_missing as _add_column_if_missing
...
def normalize_reasoning_effort(effort: Optional[str]) -> Optional[str]:
    ...
    Accepts any level in ``clara_constants.VALID_REASONING_EFFORTS`` plus
    ...
    """
    from clara_constants import VALID_REASONING_EFFORTS
...
def _assert_not_delegated_child_mutation() -> None:
    ...
    try:
        ...
        delegated = bool(os.environ.get("CLARA_DELEGATED_CHILD_CONTEXT"))
    if delegated:
...
def _fire_kanban_lifecycle_hook(event: str, task_id: str, **fields: Any) -> None:
    ...
    ``profile_name`` is resolved from the active CLARA_HOME so dispatcher- and
...
def _kanban_observer_consumed(event: str) -> bool:
    ...
    """
    try:
        from clara_cli.lifecycle import has_hook
...
# ``CLARA_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``CLARA_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("CLARA_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``CLARA_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("CLARA_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``CLARA_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get(
        "CLARA_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", ""
    ).strip()
    if raw:
        try:
            parsed = int(raw)
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "clara_kanban_current_board_override",
    default=None,
)
...
# enough that kebab-case names like ``atm10-server`` or ``clara-agent``
...
def kanban_home() -> Path:
    """Return the shared Clara root that anchors the kanban board.

    Resolution order:

    1. ``CLARA_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_clara_root()``, which already returns ``<root>``
       when ``CLARA_HOME`` is ``<root>/profiles/<name>``, and returns
       ``CLARA_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``CLARA_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("CLARA_KANBAN_HOME", "").strip()
    if override:
        return Path(override).expanduser()
    from clara_constants import get_default_clara_root
    return get_default_clara_root()
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``clara kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``CLARA_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``clara kanban boards
    ...
    env = os.environ.get("CLARA_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``clara kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``CLARA_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("CLARA_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``CLARA_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("CLARA_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``CLARA_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
    ...
    see the kanban docs.
    """
    override = os.environ.get("CLARA_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``clara kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # ``clara_constants.VALID_REASONING_EFFORTS``, or ``"none"`` for thinking
    ...
    # within an agent loop that propagated ``CLARA_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    workspace_path       TEXT,
    branch_name          TEXT,
    -- Optional link to a first-class Project (clara_cli/projects_db). When set,
    ...
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``CLARA_SESSION_ID``. NULL
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("CLARA_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _sqlite_connect(path: Path) -> sqlite3.Connection:
    ...
    advisory locks on the database (see ``clara_cli.sqlite_safe_read``).
    The registration is released automatically when the connection closes.
    """
    from clara_cli.sqlite_safe_read import connect_tracked
...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``clara gateway run --replace`` /
...
def _validate_sqlite_header(path: Path) -> None:
    ...
    from clara_cli.sqlite_safe_read import read_header_bytes_preopen
...
def _backup_corrupt_db(path: Path) -> Optional[Path]:
    ...
    # see clara_cli.sqlite_safe_read), so it must only run once the board has
    ...
    # rescue.
    from clara_cli.sqlite_safe_read import has_live_connection
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    **Narrow auto-repair:** when the integrity failure consists *only* of
    ...
    all sources Clara treats as user-controlled-but-trusted on the
...
def repair_db(
    ...
    *,
    ...
    :class:`RepairResult` so ``clara kanban repair`` can report and choose
    its own exit code.
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``CLARA_KANBAN_DB`` env → ``CLARA_KANBAN_BOARD`` env →
    ...
    # holder (e.g. an external `clara kanban list` mid-integrity-probe) block
    ...
    with _cross_process_init_lock(path):
        ...
        from clara_state import preflight_db_writability
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See clara_state._WAL_INCOMPAT_MARKERS for detection logic.
                from clara_state import apply_wal_with_fallback
                apply_wal_with_fallback(conn, db_label=f"kanban.db ({path.name})")
                # FULL (was NORMAL): fsync before each checkpoint to narrow the
                # crash window that can leave a b-tree page header torn.
                conn.execute("PRAGMA __PROT_1_synchroclara__=FULL")
                conn.execute("PRAGMA wal_autocheckpoint=100")
                # Bound the WAL file size now that the periodic explicit
                # checkpoint is PASSIVE (never truncates): on the writer's
                # natural post-checkpoint reset SQLite trims the -wal file
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``clara kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``CLARA_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _check_file_length_invariant(conn: sqlite3.Connection) -> None:
    ...
    """
    from clara_cli.sqlite_safe_read import file_length_matches_header
...
def write_txn(conn: sqlite3.Connection, *, allow_nested: bool = False):
    ...
    if getattr(conn, "in_transaction", False):
        ...
        savepoint = f"clara_nested_{secrets.token_hex(8)}"
...
def create_task(
    ...
    *,
    ...
    each name to ``clara --skills ...``. Use this to pin a task to a
    ...
    if project_id:
        from clara_cli import projects_db as _pdb
    ...
    # `clara --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``clara kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `clara kanban recompute` (issue #22459).
...
def store_attachment_bytes(
    ...
    *,
    ...
    (``clara kanban attach``) so name-sanitisation, the size cap, and the
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``clara kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``clara kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``clara kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``clara kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``CLARA_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Clara' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Clara-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``clara kanban show <id>`` and the dashboard.
...
def _resolve_worktree_workspace(
    ...
    launched from, e.g. the Clara checkout). If no anchor is configured
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    - ``worktree``: a real linked git worktree. If ``workspace_path`` names
      a repo root, Clara treats it as an anchor and materializes a linked
      worktree at ``<repo>/.worktrees/<task-id>``. If ``workspace_path`` names
      a concrete target path, Clara creates/reuses that linked worktree. With
      no ``workspace_path``, Clara anchors on the board's ``default_workdir``
      ...
      dispatcher's CWD. When ``branch_name`` is empty, Clara uses
      ``wt/<task-id>``.
...
# without thrashing. Overridable via ``CLARA_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Clara
...
def _defer_reclaim_for_live_worker(
    ...
    *,
    ...
    event so the hold is visible in ``clara kanban tail``. The next dispatch
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``clara kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Clara profile.
    ...
    if not rows:
        return False
    try:
        from clara_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def review_dispatch_enabled() -> bool:
    ...
    The default is true because Clara ships the ``sdlc-review`` skill and the
...
# Assumed per-worker memory footprint for the derived default cap. Clara
...
def resolve_max_in_progress(configured: Optional[int]) -> Optional[int]:
    ...
    Callers that parse config (gateway dispatcher, ``clara kanban dispatch``)
...
def count_running_tasks_other_boards(board: Optional[str] = None) -> int:
    ...
    Boards are matched by resolved DB path, so the ``CLARA_KANBAN_DB``
    ...
    *,
    ...
    def _any_spawnable_review() -> bool:
        if not review_rows:
            return False
        try:
            from clara_cli.profiles import profile_exists as _rpe
        except Exception:
    ...
    if _default_assignee:
        try:
            from clara_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real Clara profile.
        # `_default_spawn` invokes ``clara -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Clara
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `clara kanban tail` — without
    ...
    # Auto-dispatch is enabled by default because Clara bundles the
    ...
    for row in review_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        try:
            from clara_cli.profiles import profile_exists
        except Exception:
            profile_exists = None  # type: ignore[assignment]
...
def _module_clara_argv() -> list[str]:
    """Return the interpreter-bound Clara CLI invocation."""
    # ``clara_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``clara`` package — there is no
    # ``clara`` package to import.
    return [sys.executable, "-m", "clara_cli.main"]


def _absolute_clara_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Clara shim."""
...
def _clara_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Clara executable path.
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_clara_argv()
    return [_absolute_clara_path(path)]


def _resolve_clara_argv() -> list[str]:
    """Resolve the ``clara`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$CLARA_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("clara")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\clara.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m clara_cli.main`` — fallback for setups where
       Clara is launched from a venv and the ``clara`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_clara_bin`` for the same reason. Kept
    local (not imported from gateway) because ``clara_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("CLARA_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _clara_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _clara_path_argv(resolved_env_bin)
        return _module_clara_argv()

    clara_bin = _safe_which_no_cwd("clara") if _IS_WINDOWS else shutil.which("clara")
    if clara_bin:
        return _clara_path_argv(clara_bin)
    return _module_clara_argv()
...
def _resolve_worker_cli_toolsets(clara_home: Optional[str]) -> Optional[list[str]]:
    ...
    task-scoped kanban lifecycle tools when ``CLARA_KANBAN_TASK`` is set.
    """
    if not clara_home:
        return None
    try:
        ...
        token = set_clara_home_override(clara_home)
        try:
            cfg = load_config()
        ...
        _log.debug(
            "kanban worker: could not resolve CLI toolsets for CLARA_HOME=%r (%s)",
            clara_home,
            exc,
        )
        return None
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``clara -p <profile> chat -q ...`` subprocess.
    ...
    ``CLARA_KANBAN_DB`` / ``CLARA_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject CLARA_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `clara -p <name>` the
    # _apply_profile_override() runs *before* clara_constants is imported.
    # If CLARA_HOME is absent from the child's env, get_clara_home() falls
    # back to Path.home() / ".clara" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from clara_cli.profiles import resolve_profile_env
    try:
        env["CLARA_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via CLARA_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # CLARA_HOME never had profiles created.
        pass
    if task.tenant:
        env["CLARA_TENANT"] = task.tenant
    env["CLARA_KANBAN_TASK"] = task.id
    env["CLARA_KANBAN_WORKSPACE"] = workspace
    # Tag the worker's session so it lands in state.db as `kanban`, not as an
    # untitled `cli` row. A worker is a dispatcher-owned run whose transcript is
    # read on the board and in `clara kanban log` — it is not a conversation
    ...
    # ("work kanban task t_…").
    env["CLARA_SESSION_SOURCE"] = "kanban"
    ...
    if task.branch_name:
        env["CLARA_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["CLARA_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["CLARA_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Goal-loop mode: the worker reads these and wraps its run in the
    # Ralph-style /goal judge loop (see cli.py quiet-mode path). Only set
    # when enabled so non-goal tasks keep a clean env.
    if task.goal_mode:
        env["CLARA_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["CLARA_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # that even when the worker activates a profile (`clara -p <name>`
    # rewrites CLARA_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_clara_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["CLARA_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["CLARA_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    _retag_legacy_worker_sessions(env["CLARA_KANBAN_WORKSPACES_ROOT"])
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["CLARA_KANBAN_BOARD"] = resolved_board
    # CLARA_PROFILE is the author the kanban_comment tool defaults to.
    # `clara -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["CLARA_PROFILE"] = profile_arg

    # A worker must NEVER boot the interactive TUI: an inherited CLARA_TUI=1
    ...
    # older clara builds on PATH that predate the flag's precedence.
    env.pop("CLARA_TUI", None)

    cmd = [
        *_resolve_clara_argv(),
        "-p", profile_arg,
        "--cli",
        # Worker subprocesses switch to a profile-scoped CLARA_HOME above,
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(env.get("CLARA_HOME"))
    if worker_toolsets:
    ...
    # `clara kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`clara` executable not found on PATH. "
            "Install Clara Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``clara kanban daemon`` is systemd-friendly.
    ...
    dispatcher and ``clara kanban dispatch`` — the standalone daemon must
    ...
    while not stop_event.is_set():
        try:
            ...
            # dispatcher and `clara kanban dispatch` do (OOF-30): explicit
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled CLARA_PROFILE values like "clara-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Clara root exists

    Reads profile paths directly so this module has no import dependency on
    ``clara_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from clara_constants import get_default_clara_root
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``clara kanban assignees`` for the terminal.
