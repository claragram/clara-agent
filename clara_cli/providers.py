"""
Single source of truth for provider identity in Clara Agent.
...
2. **Clara overlays** — transport type, auth patterns, aggregator flags,
...
# -- Clara overlay ----------------------------------------------------------
# Clara-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class ClaraOverlay:
    """Clara-specific provider metadata layered on top of models.dev."""
...
CLARA_OVERLAYS: Dict[str, ClaraOverlay] = {
    "moa": ClaraOverlay(
        transport="openai_chat",
        auth_type="virtual",
        base_url_override="moa://local",
    ),
    "openrouter": ClaraOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "clara": ClaraOverlay(
        ...
        base_url_override="https://inference-api.claraship.com/v1",
    ),
    ...
    "openai-api": ClaraOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": ClaraOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": ClaraOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="CLARA_QWEN_BASE_URL",
    ),
    "lmstudio": ClaraOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": ClaraOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": ClaraOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": ClaraOverlay(
    ...
    "kimi-for-coding": ClaraOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": ClaraOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": ClaraOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": ClaraOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": ClaraOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": ClaraOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": ClaraOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": ClaraOverlay(
        transport="openai_chat",
    ...
    "opencode": ClaraOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": ClaraOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_GO_BASE_URL",
    ),
    "opencode-free": ClaraOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "tencent-tokenhub": ClaraOverlay(
        transport="openai_chat",
        base_url_env_var="TOKENHUB_BASE_URL",
    ),
    "tencent-tokenplan": ClaraOverlay(
        transport="anthropic_messages",
    ...
    "azure-foundry": ClaraOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": ClaraOverlay(
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "clara", "user-config"
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Clara overlays (for providers not in models.dev: clara, openai-codex, etc.)
      2. models.dev catalog + Clara overlay
    ...
    overlay = CLARA_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + clara extra
    ...
    if overlay is not None:
        # Clara-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
    ...
    # tencent-tokenhub) are absent from models.dev and CLARA_OVERLAYS, so
...
def clara_api_mode(model: str = "") -> str:
    """Resolve the wire protocol for a Clara Portal model.
    ...
    historical Clara transport — so callers that don't yet know the model
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    ...
    Resolution order:
      ...
      2. Clara Portal dual-wire (model-derived; overlay alone is openai_chat).
      3. Known provider → transport → TRANSPORT_TO_API_MODE.
      4. Direct provider checks (bedrock).
      5. Default: 'chat_completions'.

    *model* is optional but required for dual-wire providers (Clara) whose
    ...
    # Clara is dual-wire: anthropic/* → Messages, everything else →
    # chat_completions. The Clara overlay still advertises openai_chat
    ...
    if provider_norm in {"clara", "clara-portal", "claraship"}:
    ...
    # Direct provider checks for providers not in CLARA_OVERLAYS
    if provider == "bedrock":
        return "bedrock_converse"
...
def resolve_provider_full(
    ...
    # 0.5 Exact Clara provider IDs must win over LOSSY alias collapsing.
    ...
    if canonical != raw:
        try:
            from clara_cli.auth import PROVIDER_REGISTRY as _AUTH_PROVIDER_REGISTRY
