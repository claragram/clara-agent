...
Backs ``clara kanban export|import``, the matching ``/boards/{slug}/export``
...
ARCHIVE_FORMAT = "clara-kanban-board"
    ...
    *,
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        manifest = {
            ...
            "clara_version": str(clara_version),
            "includes": {
...
def _read_manifest(root: Path) -> dict[str, Any]:
    ...
    if not path.exists():
        raise ValueError(
            "archive is not a Clara kanban board export (no manifest.json)"
        )
    try:
    ...
    if not isinstance(manifest, dict) or manifest.get("format") != ARCHIVE_FORMAT:
        raise ValueError(
            "archive is not a Clara kanban board export "
    ...
    if not isinstance(version, int) or version > ARCHIVE_FORMAT_VERSION:
        raise ValueError(
            f"archive format version {version!r} is newer than this Clara "
            f"understands (max {ARCHIVE_FORMAT_VERSION}) — update Clara and retry"
        )
    return manifest
    ...
    *,
    ...
    return {
        ...
        "source": {
            ...
            "clara_version": manifest.get("clara_version"),
        },
        "counts": counts,
