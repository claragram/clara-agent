...
from clara_constants import agent_browser_runnable, find_node_executable
from tools.environments.local import clara_subprocess_env

_IS_WINDOWS = platform.system() == "Windows"

_DEP_CHECKS = {
    # find_node_executable() rather than a bare which(): $CLARA_HOME/node is
...
def _has_clara_agent_browser() -> bool:
    ...
    # install.sh installs globally into $CLARA_HOME/node/bin/ via npm -g --prefix
...
def ensure_dependency(
    ...
    if shell == "powershell":
        ...
        cmd = [
            ...
            "-File", str(script),
            "-Ensure", dep,
            "-ClaraHome", str(get_clara_home()),
        ]
    else:
        cmd = ["bash", str(script), "--ensure", dep]

    run_env = clara_subprocess_env(inherit_credentials=False)
