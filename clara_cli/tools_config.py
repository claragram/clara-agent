"""
Unified tool configuration for Clara Agent.

`clara tools` and `clara setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.clara/config.yaml under
...
from clara_cli.clara_subscription import (
    MANAGED_FEATURE_COVERAGE_CATEGORY,
    ClaraSubscriptionFeatures,
    apply_clara_managed_defaults,
    get_clara_subscription_features,
)
from clara_cli.clara_account import format_clara_portal_entitlement_message
from clara_cli.toolset_scope import (
    _TOOLSET_PLATFORM_RESTRICTIONS,
    toolset_allowed_for_platform as _toolset_allowed_for_platform,
)
from tools.tool_backend_helpers import CLARA_MANAGED_PROVIDER, fal_key_is_configured
...
def _post_setup_no_window_flags(*, streams_to_console: bool = False) -> int:
    ...
    ``clara tools post-setup <key>`` child. On Windows, every console child
    ...
    :func:`clara_cli._subprocess_compat.windows_hide_flags`) suppresses it
...
from clara_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _CLARA_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `clara tools` → Video Generation, which walks
...
# `clara tools` → X (Twitter) Search setup walks users through credential
# setup. The tool's check_fn means the schema still won't appear to the
# model if the credential later goes missing or expires.
_DEFAULT_OFF_TOOLSETS = {"homeassistant", "spotify", "discord", "discord_admin", "video", "video_gen", "x_search", "a2a"}


# Config-only capabilities: they appear in `clara tools` for provider/API-key
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from clara_cli.auth import _read_xai_oauth_tokens
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``clara tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from clara_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _get_plugin_toolset_keys() -> set:
    ...
    try:
        from clara_cli.plugins import get_plugin_toolset_keys_nowait
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``clara tools`` checklist actually offers
    for ``platform``.
    ...
    so ``clara tools`` never claims to add or remove a toolset the user was
...
from clara_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "web": {
        ...
        #   - "Clara Subscription" — managed Firecrawl billed via Clara
        #     subscription (requires_clara_auth + override_env_vars).
    ...
    "image_gen": {
        ...
        #   - "Clara Subscription" — managed FAL billed via the Clara
        #     subscription (requires_clara_auth + override_env_vars).
        ...
        "providers": [
            {
                ...
                "env_vars": [],
                "requires_clara_auth": True,
                "managed_clara_feature": "image_gen",
    ...
    "video_gen": {
        ...
        "icon": "🎬",
        # "Clara Subscription" row mirrors the image_gen pattern — managed
        # FAL video generation billed via the Clara Portal.  Plugin-backed
        ...
        "providers": [
            {
                ...
                "env_vars": [],
                "requires_clara_auth": True,
                "managed_clara_feature": "video_gen",
                "override_env_vars": ["FAL_KEY"],
                # The underlying plugin backend — when the user picks
                # "Clara Subscription" we set video_gen.provider = "fal"
    ...
    "x_search": {
        ...
        "setup_note": (
            "Clara routes X searches through xAI's built-in x_search "
    ...
    "browser": {
        ...
        # backend, never on the paid Clara Subscription gateway row:
        ...
        #   - "Clara Subscription (Browser Use cloud)" — managed Browser Use
        #     billed via Clara subscription (requires_clara_auth +
        ...
        "providers": [
            ...
            },
            {
                ...
                "tag": "Zig headless browser spawned by Clara, text-only (no screenshots)",
                "env_vars": [],
                "browser_provider": "local",
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set CLARA_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "CLARA_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "CLARA_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "CLARA_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "CLARA_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "CLARA_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
# provider+model picker like `clara model` — NOT this single-key prompt, so
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("CLARA_CUA_DRIVER_CMD", "").strip() or "cua-driver"


def _cua_version_summary(raw: str, *, limit: int = 120) -> str:
    """Reduce a driver's ``--version`` output to one short status line.

    A binary selected by ``CLARA_CUA_DRIVER_CMD`` is not obliged to answer
...
def _cua_driver_env() -> dict:
    """cua-driver child env with the Clara telemetry policy applied.
...
def _cua_driver_contract_status(binary: Optional[str] = None) -> dict:
    """Inspect whether an installed driver supports Clara' runtime contract."""
    import time
...
def _pip_install(
    ...
    *,
    ...
    # Managed uv first: $CLARA_HOME/bin is never on PATH, so a bare which()
    # misses the uv Clara installed and prefers a system one when both exist.
...
def install_cua_driver(
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``clara update`` and
      by ``clara computer-use install --upgrade``.
    ...
    falling through to the full upstream installer. ``clara update`` sets
    ...
    a further ~600s wait on Windows). ``clara computer-use install
    ...
    ``clara update`` already prints a contextual line before its update
    ...
    ``clara update`` and must not inherit the explicit install command's
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `clara update` calls this
    ...
    override = os.environ.get("CLARA_CUA_DRIVER_CMD", "").strip()
    if override and not binary:
        _print_warning(
            "    CLARA_CUA_DRIVER_CMD does not resolve to an executable: "
            f"{override}"
        )
        _print_info(
    ...
    # An installed driver that fails Clara' runtime contract (version floor,
    # missing manifest verbs) is repaired regardless of the caller's mode.
    # Clara' own minimum requirement IS the confirmation that an upgrade is
    ...
    # contract landed, with `clara update` declining to refresh).
    ...
    if repair_existing:
        ...
        _print_warning(
            f"    Found cua-driver {version}, but Clara cannot use its current "
            f"runtime contract: {reason}."
        )
        if os.environ.get("CLARA_CUA_DRIVER_CMD", "").strip():
            _print_info(
        ...
        if is_windows and require_confirmed_update:
            ...
            _print_info(
                ...
                "clara computer-use install --upgrade"
            )
            return False
    ...
    if not _cua_install_target_writable():
        ...
        _print_info(
            "    Run `clara computer-use install --upgrade` from an admin account to update it."
        )
        return bool(binary)
    ...
    # caller: `clara update` (require_confirmed_update=True) keeps the
    ...
    # `clara computer-use install --upgrade` falls through and re-runs the
    # installer as before.
    ...
    if binary and not repair_existing:
        ...
        if _state is None and require_confirmed_update:
            ...
            _print_info(
                "    Force a refresh with: clara computer-use install --upgrade"
            )
            return True
    ...
    if ok and repair_existing:
        ...
        if not repaired.get("ready"):
            ...
            _print_info("    Run: clara computer-use doctor")
            return False
    if ok and before:
        try:
...
# Optional refreshes launched by ``clara update`` are quiet and unattended.
...
def _run_cua_driver_installer(
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    # Unattended refreshes (installer_timeout set by `clara update`) fail
    ...
    def _reap_after_timeout(proc):
        ...
        ceiling stops bounding anything and ``clara update`` hangs past its
    ...
    try:
        # When not verbose (e.g. `clara update`'s refresh), capture the
        ...
        if verbose:
            ...
            # Preserve the full installer output. During `clara update`,
            # sys.stdout is the mirroring _UpdateOutputStream whose `_log`
            # handle is ~/.clara/logs/update.log — write straight to it so
        ...
        if result.returncode == 0 and installed_binary:
            ...
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / Clara process.")
            return True
...
def _ensure_browser_use_cli(*, verbose_hints: bool = False) -> None:
    ...
    Firecrawl, and the Clara-managed cloud rows all execute through
    ...
    check — only the Clara-managed ``$CLARA_HOME/bin`` copy does.
    ``install_cli()`` short-circuits on the managed copy and otherwise
    provisions it, so resolution always lands on a binary Clara installs
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key == "lightpanda":
        # Browser Use mode drives Lightpanda directly (Clara spawns
        ...
        if lightpanda_bin:
            ...
            if os.name == "nt":
                _print_info("    Lightpanda has no native Windows build; run Clara under WSL2.")
        return
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        # — it resolves lazily via npx (or a global/Clara-managed install)
        ...
        # (PATH -> Homebrew/Clara-managed node -> npx) rather than a bare
        # shutil.which — Clara-managed-Node-only setups resolve agent-browser
        ...
        # Clara-managed-node -> npx cascade _find_agent_browser uses at
        ...
        if _is_npx_agent_browser_sentinel(browser_cmd):
            ...
            # — Clara-managed-Node-only setups resolve npx only through the
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                _print_info("    Change via stt.local.model in ~/.clara/config.yaml")
            else:
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.clara/config.yaml")
        ...
        # Run the full `clara auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.clara/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from clara_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: clara auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `clara auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: clara auth spotify")
        except Exception as exc:
        ...
        try:
            from clara_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: clara plugins enable observability/langfuse")
        _print_info("    Restart Clara for tracing to take effect.")
        _print_info("    Verify: clara plugins list")
        ...
        try:
            ...
            _print_info("    Run later: clara auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `clara auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: clara auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: clara auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``clara tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``clara tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: clara tools post-setup <key>")
        return 2
...
def _exempt_explicit_platform_native(
    ...
    the composite they chose (e.g. ``clara-discord``, which contains those
...
#:
#: Saving ``clara tools`` (or one toggle in the desktop Toolsets UI) replaces
...
#: has touched the picker, while everyone still on ``[clara-cli]`` inherits it
...
def _enable_recently_shipped_toolsets(
    ...
    Either way of saying no outlives this: unchecking in ``clara tools``
    ...
    default_ts = plat_info["default_toolset"] if plat_info else f"clara-{platform}"
    ...
    for ts_key in sorted(_RECENTLY_SHIPPED_TOOLSETS):
        ...
        # narrow composites (clara-acp, clara-webhook) stay narrow.
...
def _get_platform_tools(
    ...
    *,
    ...
    # ``clara-discord``) is an opt-in to the platform's native default-off
    ...
    # explicit config like ``[clara-cli, a2a]`` must survive filtering just
    ...
    # "clara-cli" (which include all _CLARA_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[clara-cli, spotify]`` after enabling Spotify via ``clara
        ...
        # (e.g. "clara-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``clara tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `clara tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"clara-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("clara-")}
    ...
    # shipped as a bundled plugin but user must opt in via `clara tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `clara tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by clara tools — default enabled
                enabled_toolsets.add(pts)
    ...
    # JSON-array string (e.g. "['memory']") from `clara config set` or a
    ...
    # is invalid (e.g. a migration or hand-edit left `clara` instead of
    # `clara-cli`), resolve_toolset() returns [] for each and the platform ends
    ...
    # at runtime, not only during the next `clara update`/`clara doctor`.
    ...
    if isinstance(_explicit, list) and _explicit:
        ...
        if (
            ...
            logger.warning(
                ...
                "name(s): %s) - tools will be unavailable. Run `clara tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (clara-cli, clara-telegram, etc.)
    ...
    # Opening `clara tools` is the user's opt-in to reconfigure tools, so treat
...
def _toolset_has_keys(
    ...
    *,
    ...
    features: Optional[ClaraSubscriptionFeatures] = None,
) -> bool:
    ...
    if ts_key in {"web", "image_gen", "video_gen", "tts", "stt", "browser"}:
        if features is None:
            features = get_clara_subscription_features(
                config, force_fresh=force_fresh
            )
        feature = features.features.get(ts_key)
        if feature and (feature.available or feature.managed_by_clara):
            return True
...
# ("Clara Subscription" managed-gateway entry, "Firecrawl Self-Hosted")
...
# ("Clara Subscription", "Local Browser", "Camofox") — see the comment block
...
def _visible_providers(
    ...
    *,
    ...
    Clara-managed Tool Gateway rows (``managed_clara_feature``) are always
    shown — even to logged-out / unentitled users — so the picker advertises
    that the capability exists.  Selecting one drives an inline Clara Portal
    login + entitlement check (see ``_configure_provider``); the row only
    *activates* the gateway once paid access is confirmed.
    """
    if features is None:
        features = get_clara_subscription_features(config, force_fresh=force_fresh)
    ...
    for provider in cat.get("providers", []):
        # Clara-managed Tool Gateway rows stay visible regardless of auth —
        # selecting one drives an inline Portal login. A `requires_clara_auth`
        # row that is NOT a managed gateway feature (pure pre-auth UX) is
        # still hidden until the user is logged in.
        if (
            provider.get("requires_clara_auth")
            and not provider.get("managed_clara_feature")
            and not features.clara_auth_present
        ):
            continue
        ...
        if (
            pool_only
            and provider.get("managed_clara_feature") == "video_gen"
    ...
    # later) so the picker lists them alongside FAL / Clara Subscription.
    ...
    # remaining hardcoded rows ("Clara Subscription", "Firecrawl
    ...
    # the hardcoded "Clara Subscription" / "Local Browser" / "Camofox" rows
...
def _hidden_clara_gateway_message(
    cat: dict,
    config: dict,
    capability: str,
    *,
    ...
    """Deprecated: Clara Tool Gateway rows are no longer hidden.

    Previously this returned a "log in / upgrade" banner shown above a
    category when its Clara-managed rows were filtered out for unentitled
    ...
    (``ensure_clara_portal_access``). Kept as a no-op so call sites stay simple;
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `clara tools` gets a silent no-op
...
# Python dependencies installed explicitly through ``clara tools`` are not
...
def active_restorable_python_tool_dependencies() -> list[str]:
    """Return ``clara tools`` Python dependencies present in this runtime."""
    return [
        name
...
def _agent_browser_installed() -> bool:
    ...
    from clara_cli.clara_subscription import _local_browser_runnable

    # The install hook runs in a spawned ``clara tools post-setup`` process,
...
def _cloud_agent_browser_installed() -> bool:
    ...
    from clara_cli.clara_subscription import _has_agent_browser
...
def provider_readiness_status(
    ...
    *,
    ...
    - ``"needs_auth"``  — needs a sign-in: Clara Portal login/entitlement for
    ...
    zero-env-var row, including logged-out Clara Subscription rows).

    ``features`` (a ``ClaraSubscriptionFeatures``) can be passed to avoid
    ...
    managed_feature = provider.get("managed_clara_feature")
    if provider.get("requires_clara_auth") or managed_feature:
        if features is None:
            features = get_clara_subscription_features(config)
        if not features.clara_auth_present:
            return "needs_auth"
        if managed_feature:
...
def _configure_tool_category(
    ...
    *,
    ...
    hidden_clara_message = _hidden_clara_gateway_message(
        cat,
        config,
        f"the Clara Subscription provider for {name}",
        force_fresh=force_fresh,
    )
    ...
    if len(providers) == 1:
        ...
        if hidden_clara_message:
            for line in hidden_clara_message.splitlines():
        ...
        # When the user is logged into Clara, surface a marker on providers
        # whose access is included in their subscription so it's visually
        # obvious which options cost extra vs. cost nothing on top of Clara.
        try:
            _clara_logged_in = bool(
                get_clara_subscription_features(
                    config,
                    force_fresh=force_fresh,
                ).clara_auth_present
        ...
        for p in providers:
            ...
            # Mark Clara-managed entries. Logged-in paid subscribers get the
            # "included" star; everyone else gets a "via Clara Portal" hint so
            ...
            sub_marker = ""
            if p.get("managed_clara_feature"):
                if _clara_logged_in:
                    sub_marker = "  ★ Included with your Clara subscription"
                else:
                    sub_marker = "  ★ via Clara Portal (login on select)"
...
def _is_provider_active(
    ...
    *,
    ...
    if plugin_name and not provider.get("managed_clara_feature"):
        # Managed (Clara-subscription) entries fall through to the
    ...
    if video_plugin_name and not provider.get("managed_clara_feature"):
    ...
    if managed_feature:
        ...
        if managed_feature == "image_gen":
            ...
            if isinstance(image_cfg, dict):
                ...
                if configured_provider not in {None, "", "fal", CLARA_MANAGED_PROVIDER}:
                    return False
                if (
                    configured_provider != CLARA_MANAGED_PROVIDER
                    and image_cfg.get("use_gateway") is not None
                    and not is_truthy_value(image_cfg.get("use_gateway"), default=False)
                ):
                    return False
            return feature.managed_by_clara
        ...
        if provider.get("tts_provider"):
            return (
                feature.managed_by_clara
                and cfg_get(config, "tts", "provider")
                in {provider["tts_provider"], CLARA_MANAGED_PROVIDER}
            )
        if provider.get("stt_provider"):
            return (
                feature.managed_by_clara
                and cfg_get(config, "stt", "provider")
                in {provider["stt_provider"], CLARA_MANAGED_PROVIDER}
            )
        if "browser_provider" in provider:
            ...
            return feature.managed_by_clara and current in {
        ...
        if provider.get("web_backend"):
            ...
            return (
                feature.managed_by_clara
                and current in {provider["web_backend"], CLARA_MANAGED_PROVIDER}
...
def _select_plugin_image_gen_provider(plugin_name: str, config: dict, *, use_gateway: bool = False) -> None:
    ...
    ``use_gateway=True`` marks a provider picked through the Clara-managed
    flow: the stored selection becomes ``image_gen.provider: clara`` (the
    ...
    img_cfg["provider"] = CLARA_MANAGED_PROVIDER if use_gateway else plugin_name
...
# Kept in sync with the dashboard selects (clara_cli/web_server.py
...
def _select_plugin_video_gen_provider(plugin_name: str, config: dict, *, use_gateway: bool = False) -> None:
    ...
    ``video_gen.provider: clara``; BYOK picks store the plugin name; any
    ...
    vid_cfg["provider"] = CLARA_MANAGED_PROVIDER if use_gateway else plugin_name
...
def _write_provider_config(provider: dict, config: dict, *, managed_feature) -> None:
    ...
    post-setup hooks, gate on Clara auth, or run interactive model pickers.
    ...
    category. Managed "Clara Subscription" rows write ``clara``; BYOK rows
    ...
    legacy shim (use_gateway: true ⇒ clara) cannot override the new choice.
    """
    def _set_selection(section_key: str, name_key: str, vendor_value) -> None:
        ...
        section[name_key] = (
            CLARA_MANAGED_PROVIDER if managed_feature else vendor_value
        )
    ...
    # image_gen/video_gen "Clara Subscription" rows carry only
    # managed_clara_feature) still persist the "clara" selection.
    if managed_feature and managed_feature not in {"web", "tts", "stt", "browser"}:
        section = config.setdefault(managed_feature, {})
        if isinstance(section, dict):
            section["provider"] = CLARA_MANAGED_PROVIDER
            section.pop("use_gateway", None)
...
def apply_provider_selection(ts_key: str, provider_name: str, config: dict) -> None:
    ...
    post-setup hooks, gate on Clara Portal auth, or run interactive model
    ...
    # flow). Managed picks store the "clara" selection.
    plugin_name = provider.get("image_gen_plugin_name")
    if plugin_name:
        img_cfg = config.setdefault("image_gen", {})
        if not isinstance(img_cfg, dict):
            ...
            config["image_gen"] = img_cfg
        img_cfg["provider"] = (
            CLARA_MANAGED_PROVIDER if managed_feature else plugin_name
        )
        img_cfg.pop("use_gateway", None)
    ...
    if video_plugin:
        ...
        if not isinstance(vid_cfg, dict):
            ...
            config["video_gen"] = vid_cfg
        vid_cfg["provider"] = (
            CLARA_MANAGED_PROVIDER if managed_feature else video_plugin
        )
        vid_cfg.pop("use_gateway", None)
...
def _configure_provider(
    ...
    *,
    ...
    # Clara-managed Tool Gateway backends are always listed (see
    # _visible_providers), but only *activate* once the user has paid Clara
    ...
    # "enable all tools" prompt (that lives in `clara model`).
    if managed_feature:
        from clara_cli.clara_subscription import (
            MANAGED_FEATURE_COVERAGE_CATEGORY,
            ensure_clara_portal_access,
        )

        if not ensure_clara_portal_access(
            capability=f"{provider.get('name', 'the Clara Tool Gateway')}",
            coverage_category=MANAGED_FEATURE_COVERAGE_CATEGORY.get(managed_feature),
        ):
            _print_warning(
                "  Not enabled — Clara Portal access is required for this backend."
            )
            return

    # Pure pre-auth UX rows (requires_clara_auth without a managed gateway
    # feature) keep the old gate. Managed rows are handled by the inline
    # login above, so don't double-check them here.
    if provider.get("requires_clara_auth") and not managed_feature:
        features = get_clara_subscription_features(config, force_fresh=force_fresh)
        entitled = bool(
            features.account_info and features.account_info.paid_service_access is True
        )
        if not features.clara_auth_present or not entitled:
            message = format_clara_portal_entitlement_message(
                features.account_info,
                capability=f"{provider.get('name', 'Clara Subscription')}",
            )
            _print_warning(
                f"  {message or 'Clara Subscription is only available after logging into Clara Portal.'}"
            )
            return
    ...
    if provider.get("tts_provider"):
        ...
        tts_cfg["provider"] = (
            CLARA_MANAGED_PROVIDER if managed_feature else provider["tts_provider"]
        )
    ...
    if not env_vars:
        ...
        if managed_feature:
            _print_info("  Requests for this tool will be billed to your Clara subscription.")
        ...
        if backend:
            ...
            # explicit selection: "clara" for a managed row, "fal" for BYOK.
            img_cfg = config.setdefault("image_gen", {})
            if isinstance(img_cfg, dict):
                img_cfg["provider"] = (
                    CLARA_MANAGED_PROVIDER if managed_feature else "fal"
                )
                img_cfg.pop("use_gateway", None)
    ...
    # Clara-managed sibling, show a single dim hint so users know
    # they can avoid the key entirely via a Portal subscription.
    # Suppressed when the user is already authed to Clara.
    _show_portal_hint = False
    if env_vars and not managed_feature and not provider.get("requires_clara_auth"):
        try:
            ...
            for _cat_key, _cat in TOOL_CATEGORIES.items():
                ...
                if provider in _providers and any(
                    sib.get("managed_clara_feature") for sib in _providers
                ):
                    _has_managed_sibling = True
                    break
            if _has_managed_sibling:
                _features = get_clara_subscription_features(
                    config,
                    force_fresh=force_fresh,
                )
                _show_portal_hint = not _features.clara_auth_present
        except Exception:
            _show_portal_hint = False

    if _show_portal_hint:
        _print_info("  Available through Clara Portal subscription.")
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``clara model`` — or point at a
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    substrate — so this picker lists exactly what the ``clara model`` aux-task
    ...
    """
    try:
        from clara_cli.inventory import (
    ...
    if not providers:
        _print_warning(
            ...
            "with `clara model`, then re-run this."
        )
        return
...
def _reconfigure_provider(
    ...
    *,
    ...
    # Same inline Clara Portal login + entitlement gate as _configure_provider:
    ...
    # provider string per category — "clara" for managed rows, the vendor name
    # for BYOK rows — and drops any legacy use_gateway key so the read-time
    # shim (use_gateway: true ⇒ clara) cannot override the fresh pick.
    if provider.get("tts_provider"):
        tts_cfg = config.setdefault("tts", {})
        tts_cfg["provider"] = (
            CLARA_MANAGED_PROVIDER if managed_feature else provider["tts_provider"]
    ...
    if provider.get("stt_provider"):
        stt_cfg = config.setdefault("stt", {})
        stt_cfg["provider"] = (
            CLARA_MANAGED_PROVIDER if managed_feature else provider["stt_provider"]
        )
    ...
    if "browser_provider" in provider:
        ...
        browser_cfg = config.setdefault("browser", {})
        if managed_feature:
            browser_cfg["cloud_provider"] = CLARA_MANAGED_PROVIDER
            _print_success(f"  Browser cloud provider set to: {bp or 'clara'}")
        elif bp == "local":
    ...
    if provider.get("web_backend"):
        ...
        web_cfg["backend"] = (
            CLARA_MANAGED_PROVIDER if managed_feature else provider["web_backend"]
        )
    ...
    if not env_vars:
        ...
        if backend:
            ...
            if backend == "fal":
                img_cfg = config.setdefault("image_gen", {})
                if isinstance(img_cfg, dict):
                    # A managed (Clara Subscription) row also carries
                    # imagegen_backend="fal" — store the "clara" selection
    ...
    if backend:
        ...
        if backend == "fal":
            ...
            if isinstance(img_cfg, dict):
                ...
                # never clobber a Clara-managed pick back onto direct keys.
                img_cfg["provider"] = (
                    CLARA_MANAGED_PROVIDER if managed_feature else "fal"
                )
                img_cfg.pop("use_gateway", None)
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `clara model`). Run it directly so reconfigure doesn't fall back to
...
def _shared_metrics_menu_label(config: dict) -> str:
    ...
    if not enabled:
        ...
        state = "collecting + sending to Clara"
    else:
...
def _configure_shared_metrics_interactive(config: dict) -> None:
    """Toggle shared-metrics collection and sending from `clara tools`.
    ...
    sending off.
    """
    from clara_cli.setup import setup_telemetry
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `clara tools` and `clara setup tools`.
    ...
    print(color("  Guide: https://agent.claraprise.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    if first_install:
        for pkey in enabled_platforms:
            ...
            auto_configured = apply_clara_managed_defaults(
                config,
            ...
            for ts_key in sorted(auto_configured):
                ...
                print(color(f"  ✓ {label}: using your Clara subscription defaults", Colors.GREEN))
    ...
    print(color(f"  Tool configuration saved to {display_clara_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'clara' or gateway restart.", Colors.DIM))
    print()
