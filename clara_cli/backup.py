"""
Backup and import commands for clara CLI.

`clara backup` creates a zip archive of the entire ~/.clara/ directory
(excluding the clara-agent repo and transient files).

`clara import` restores from a backup zip, overlaying onto the current
CLARA_HOME root.
"""
...
from clara_cli.sizefmt import format_bytes as _format_size
...
# Where ``clara backup --quick`` / ``/snapshot`` / the pre-update safety net
...
# ``clara-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/clara-agent/``
...
# CLARA_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "clara-agent",     # the codebase repo — re-clone instead
    ...
    "browser-profile",
    # Python dependency trees (plugin / MCP-server venvs under CLARA_HOME) —
...
# Clara-managed runtime downloads that only exist at the top of a profile
...
# ONLY at the root of CLARA_HOME and at ``profiles/<name>/`` — a deeper
...
def _in_excluded_root_dir(rel_path: Path) -> bool:
    """True when *rel_path* (relative to CLARA_HOME) is, or sits inside, a
    Clara-managed runtime tree at the top of a profile home."""
...
# ``state.db.pre-update-emergency-<timestamp>.bak`` at the CLARA_HOME root
...
# File names that ``clara import`` must never overwrite, matched by basename so
...
#     stuck "starting"/"cooking", disconnecting it from the Clara portal
...
# Reserved archive subtree for provider state that lives OUTSIDE CLARA_HOME
...
class BackupInProgressError(RuntimeError):
    """Raised when another process already owns the Clara backup slot."""
...
@contextmanager
def _backup_operation_lock(clara_home: Path, timeout_seconds: float = 0.25):
    """Acquire one cross-process backup slot for full and quick snapshots."""
    lock_path = clara_home / ".backup.lock"
            ...
            while True:
                try:
                    ...
                    if time.monotonic() >= deadline:
                        raise BackupInProgressError("another Clara backup is already running")
                    time.sleep(0.05)
        else:
            import fcntl
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside CLARA_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to clara root) should be skipped."""
    parts = rel_path.parts
    ...
    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``clara-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/clara-agent/`` — must be preserved.
        if part == "clara-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside CLARA_HOME.
...
def _iter_backup_files(
    ...
    never descends a multi-GB excluded tree), the root-only ``clara-agent``
    carve-out, profile-home-root runtime trees, and the per-file exclusion
    rules — shared by the manual ``clara backup`` path and the automatic
    ...
    """
    for dirpath, dirnames, filenames in os.walk(clara_root, followlinks=False):
        rel_dir = Path(dirpath).relative_to(clara_root)

        # ``clara-agent`` is only pruned at the root level; nested dirs
        ...
        dirnames[:] = [
            d for d in dirnames
            if (d not in _EXCLUDED_DIRS or (d == "clara-agent" and not is_root))
...
# to the user as a hung `clara update` (#70553 follow-up). Sessions databases
...
def verify_sqlite_integrity(
    ...
    *,
    ...
    if check_header:
        ...
        # clara_cli.sqlite_safe_read). Verification targets snapshots and
    ...
    try:
        ...
        try:
            ...
            logger.error(
                ...
                "database handles (or restart Clara) and retry.",
                dst, exc2,
            )
            return False
...
def run_backup(args) -> None:
    """Create a zip backup of the Clara home directory."""
    clara_root = get_default_clara_root()

    if not clara_root.is_dir():
        print(f"Error: Clara home directory not found at {clara_root}")
        sys.exit(1)

    try:
        with _backup_operation_lock(clara_root):
            _run_backup_locked(args, clara_root)
    except BackupInProgressError as exc:
        print(f"Error: {exc}")
        raise SystemExit(2) from exc


def _run_backup_locked(args, clara_root: Path) -> None:
    ...
    try:
        if args.output:
            ...
            if out_path.is_dir():
                ...
                out_path = out_path / f"clara-backup-{stamp}.zip"
        else:
            stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
            out_path = Path.home() / f"clara-backup-{stamp}.zip"
    ...
    print(f"Scanning {display_clara_home()} ...")
    skipped_dirs: set = set()
    files_to_add: list[tuple[Path, Path]] = list(
        _iter_backup_files(clara_root, out_path, skipped_dirs)
    )

    # External memory-provider state (e.g. ~/.honcho, ~/.hindsight) lives
    # outside CLARA_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_clara_home()}."
        )
    ...
    if not errors:
        print(f"\nRestore with: clara import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Clara backup.
    ...
    # Look for telltale files that a clara home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Clara backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.clara/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a clara dir name
        if prefix in {".clara", "clara"}:
            return prefix + "/"
...
def _extract_member_atomically(
    ...
    with nothing behind it — during ``clara import``, which is the
    ...
    so without the chown a ``sudo clara import`` would silently re-own every
    ...
    if mode is None:
        ...
        # take over an existing setuid/setgid file, so ``clara import`` would
        # hand whoever produced the zip the identity that file runs as.  Nothing
        # constrains that to Clara' own state either: the ``_external/`` branch
...
def run_import(args) -> None:
    ...
    # same path printed as "Target:" via display_clara_home(). Resolving
    # through get_default_clara_root() instead maps a profile home
    # (<root>/profiles/<name>) back to <root>, silently retargeting the
    # restore at the live root while the profile directory stays empty.
    clara_root = get_clara_home()
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_clara_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (clara_root / "config.yaml").exists()
        has_env = (clara_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Clara configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under CLARA_HOME.
            ...
            # Clara portal. Matched by basename so both the root profile and
            # named profiles (profiles/<name>/gateway_state.json) are covered.
            if Path(rel).name in _IMPORT_SKIP_NAMES:
                skipped_runtime.append(rel)
                continue

            target = clara_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(clara_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_clara_home()}."
            )
        ...
        profiles_dir = clara_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # clara_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print("\n  Profiles detected but aliases could not be created.")
                    print("  Run: clara profile list  (after installing clara)")

        # Guidance
        print()
        if not (clara_root / "clara-agent").is_dir():
            print("Note: The clara-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: clara update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  clara -p {pname} gateway install")
        ...
        native_default = _get_platform_default_clara_home()
        ...
        if clara_root != native_default and default_has_install:
            print(
                "\nRestored into a non-default home; leaving the gateway service "
                "alone to avoid clashing with the install at "
                f"{native_default}."
            )
            print("To start a gateway for this home, run:  clara gateway install")
        else:
            try:
                from clara_cli.gateway import ensure_gateway_service, _is_service_running
        ...
        print("Done. Your Clara configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and clara backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to CLARA_HOME).
...
# — `clara update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $CLARA_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside CLARA_HOME and the entry is silently
...
def _quick_snapshot_root(clara_home: Optional[Path] = None) -> Path:
...
def _create_quick_snapshot_locked(
    ...
    if failed_dbs:
        ...
        print(
            ...
            f"{root} and run: clara snapshot list"
        )
        logger.error(
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(clara_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside CLARA_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``clara update``.
    ...
    Args:
        ...
        clara_home: Override for the Clara home directory (tests).
...
def create_pre_update_snapshots_all_profiles(
    ...
    home = invoking_home or get_clara_home()
...
def restore_config_model_settings_if_rewritten(
    ...
    """Safety net for silent config.yaml model/MoA loss across ``clara update``.
    ...
    Args:
        snapshot_id: The pre-update quick-snapshot id (from
            :func:`create_quick_snapshot`).
        clara_home: Override for the Clara home directory (tests/siblings).
    ...
    for name, snap_id in profile_snapshots.items():
        ...
        try:
            result = restore_config_model_settings_if_rewritten(
                snap_id, clara_home=profile_home
            )
        except Exception as exc:
            logger.debug(
...
def restore_cron_jobs_all_profiles(
    ...
    for name, snap_id in profile_snapshots.items():
        ...
        if profile_home is None:
            continue
        try:
            result = restore_cron_jobs_if_emptied(snap_id, clara_home=profile_home)
        except Exception as exc:
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(clara_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for clara backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_clara_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, clara_root: Path) -> Optional[Path]:
    """Single-flight wrapper for automatic full zip backups."""
    try:
        with _backup_operation_lock(clara_root):
            return _write_full_zip_backup_locked(out_path, clara_root)
    except BackupInProgressError as exc:
        logger.warning("Full-zip backup skipped: %s", exc)
        return None


def _write_full_zip_backup_locked(out_path: Path, clara_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``clara_root`` to ``out_path``.
    ...
    try:
        files_to_add = list(_iter_backup_files(clara_root, out_path))
    except OSError as exc:
...
def _pre_update_backup_dir(clara_home: Optional[Path] = None) -> Path:
    home = clara_home or get_clara_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of CLARA_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<CLARA_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``clara update``) should continue even if the backup fails.
    """
    clara_root = clara_home or get_default_clara_root()
    if not clara_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(clara_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, clara_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `clara claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of CLARA_HOME under ``backups/`` before a
    ``clara claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``clara import <archive>``.  Writes to
    ``<CLARA_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `clara import` and the
