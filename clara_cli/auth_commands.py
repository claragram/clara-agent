...
_OAUTH_CAPABLE_PROVIDERS = {"anthropic", "clara", "openai-codex", "xai-oauth", "qwen-oauth", "minimax-oauth"}
...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_clara_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:clara_pkce",
    ...
    if provider == "clara":
        # Codex-style auto-import: if a shared Clara credential lives at
        # <clara-root>/shared/clara_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `clara --profile <name>
        # auth add clara --type oauth` a one-tap operation for users who
        # run multiple profiles.
        shared = auth_mod._read_shared_clara_state()
        if shared:
            try:
                path = auth_mod._clara_shared_store_path()
            except RuntimeError:
                path = None
            print()
            if path:
                print(f"Found existing Clara OAuth credentials at {path}")
            else:
            ...
            if do_import in {"", "y", "yes"}:
                ...
                rehydrated = auth_mod._try_import_shared_clara_state(
                    timeout_seconds=getattr(args, "timeout", None) or 15.0,
                )
                if rehydrated is not None:
                    custom_label = (getattr(args, "label", None) or "").strip() or None
                    entry = auth_mod.persist_clara_credentials(rehydrated, label=custom_label)
        ...
        creds = auth_mod._clara_device_code_login(
        ...
        # Honor `--label <name>` so clara matches other providers' UX.  The
        # helper embeds this into providers.clara so that label_from_token
        # doesn't overwrite it on every subsequent load_pool("clara").
        custom_label = (getattr(args, "label", None) or "").strip() or None
        entry = auth_mod.persist_clara_credentials(creds, label=custom_label)
        shown_label = entry.label if entry is not None else label_from_token(
    ...
    if provider == "openai-codex":
        ...
        # latest login: a second ``clara auth add openai-codex`` overwrote
    ...
    if provider == "xai-oauth":
        ...
        # The singleton round-trip collapsed every added account into the
        # latest login: a second ``clara auth add xai-oauth`` overwrote the
    ...
    raise SystemExit(f"`clara auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Clara reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `clara auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `clara auth` is called bare."""
