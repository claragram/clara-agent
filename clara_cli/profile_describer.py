...
Used by ``clara profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``clara_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Clara Agent kanban board.
...
Rules:
  ...
  - Never write "Clara Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_clara_home())
        else:
