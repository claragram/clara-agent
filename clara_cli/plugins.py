...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with clara-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.clara/plugins/<name>/``
3. **Project plugins** – ``./.clara/plugins/<name>/`` (opt-in via
   ``CLARA_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``clara_agent.plugins``
   entry-point group.
...
from clara_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE
from clara_cli.plugin_capabilities import (  # noqa: F401 — re-exported
    CAPABILITY_REGISTRY,
    VALID_CAPABILITY_IDS,
    plugin_capability_granted,
)
from clara_cli.plugin_capabilities import (
...
# ---------------------------------------------------------------------------
#
# Set ``CLARA_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.clara/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("CLARA_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When CLARA_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Clara loggers.
    """
    ...
    logger.debug(
        "CLARA_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    "pre_llm_call",
    "post_llm_call",
    # Streaming LLM output observer hooks. Fired __PROT_6_asynchroclaraly__ off the token
    ...
    # finish. Clara' shipped guidance lives in the evidence-based
    ...
    "pre_transcription",
    # Kanban task lifecycle hooks. Fired by clara_cli.kanban_db when a task
    ...
    # separate `clara -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `clara kanban dispatch`),
    ...
    # ``clara kanban dispatch``); on_kanban_task_updated fires in whichever
    ...
    #   result: clara_cli.kanban_db.DispatchResult (spawned, reclaimed,
...
# __PROT_3_synchroclaraly__ to completion.
#
...
ENTRY_POINTS_GROUP = "clara_agent.plugins"
ENTRY_POINT_CAPABILITIES_GROUP = "clara_agent.plugin_capabilities"
...
def discover_entrypoint_manifests() -> List["PluginManifest"]:
    ...
    * **Capability declarations** — read from the companion
      ``clara_agent.plugin_capabilities`` entry-point group (declarations
...
PLUGIN_SECTIONS_START = "<!-- clara-plugin-sections:start -->"
PLUGIN_SECTIONS_END = "<!-- clara-plugin-sections:end -->"
...
def format_system_prompt_section(section_id: str, content: str) -> str:
    ...
    return (
        ...
        f"<!-- clara-plugin-section-chars:{len(content)} -->\n\n"
        f"{content}"
    )
...
# Reserved event namespace prefix — only core may publish ``clara:<event>``.
CLARA_EVENT_NAMESPACE = "clara"
...
def _plugin_home_scope(home: Path):
    """Bind discovery and loading to the manager's immutable Clara home."""
...
_KNOWN_MANIFEST_FIELDS: Set[str] = {
    ...
    "capabilities", "emits", "listens", "clara", "depends",
}

# Highest manifest schema version this Clara understands.
...
def _parse_manifest_v2_fields(data: Mapping, key: str) -> Dict[str, Any]:
    ...
    if mv > SUPPORTED_MANIFEST_VERSION:
        logger.warning(
            "Plugin %s: manifest_version %d is newer than this Clara "
        ...
        for dep in manifests[k].requires_plugins:
            ...
            if resolved is None:
                logger.warning(
                    ...
                    "via ctx.has_plugin). Run `clara plugins enable %s` if "
                    "it is installed.",
                    k, dep_id, dep_id,
                )
                continue
...
class PluginManifest:
    ...
    #              in ~/.clara/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``clara plugins list``. For a flat plugin at
    ...
    # ``clara_cli.plugin_capabilities.CAPABILITY_REGISTRY``. Declaration is
    ...
    # Declared pip dependencies. VALIDATED AND SURFACED ONLY — Clara never
    ...
    # (``clara plugins show``); a plugin may emit/subscribe without declaring.
...
def _plugin_relative_segments(key: str) -> tuple[str, ...]:
    ...
    ``retry.policy``).  Full Clara paths, traversal syntax, and the security-
...
class PluginState:
    ...
    def data_dir(self) -> Path:
        ...
        return get_clara_home() / "plugin-data" / self._data_namespace
...
class PluginContext:
    ...
    def set_config(self, key: str, value: Any) -> None:
        ...
        try:
            ...
            raise
        from clara_cli import config as config_mod
        ...
        # threads. Other Clara config writers still retain their existing
    ...
    def platform_actions(self):
        ...
        if self._platform_actions is None:
            from clara_cli.platform_actions import PlatformActions
    ...
    def profile_name(self) -> str:
        """Return the active Clara profile name (e.g. ``"default"``).

        Derived from ``CLARA_HOME`` via
        :func:`clara_cli.profiles.get_active_profile_name`, so it works in
        ...
        running under ``~/.clara/profiles/<name>``, or ``"custom"`` when
        ``CLARA_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def register_approval_transport(self, name: str, present_fn: Callable) -> None:
        ...
        ``present_fn`` may be __PROT_0_synchroclara__ or async.
        """
    ...
    def call_mcp(
        ...
        __PROT_1_Synchroclara__; safe to call from plugin hooks and tools. Routes through
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Clara core) are trusted by default —
    ...
    def inject_message(
        ...
        *,
        ...
        __PROT_5_asynchroclara__ dispatch, not that platform delivery has completed.
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``clara honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``clara <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> Optional[PluginRegistration]:
        ...
        ``provider`` must be an instance of
        :class:`clara_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        """
        from clara_cli.dashboard_auth import DashboardAuthProvider
        from clara_cli.dashboard_auth.registry import (
    ...
    def register_secret_source(self, source) -> Optional[PluginRegistration]:
        ...
        sources run during ``load_clara_dotenv()`` startup — after
        ``~/.clara/.env`` loads, before Clara reads credentials — when
        ...
        NOTE ON TIMING: ``load_clara_dotenv()`` usually runs at import
        *before* plugin discovery.  After discovery completes, the plugin
        ...
        + ``load_clara_dotenv``) so the first process sees them (#64177).
    ...
    def register_slack_action_handler(
        ...
        Clara' Slack adapter wires registered handlers into its
    ...
    def register_telegram_handler(self, factory: Callable) -> None:
        ...
        Clara' Telegram adapter invokes registered factories at ``connect()``
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``clara model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: clara_cli.main imports plugins indirectly
        from clara_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def emit(self, event: str, payload: Optional[dict] = None) -> int:
        ...
        including ``clara:x`` or a foreign ``other:x``) is rejected with a
        ``ValueError`` and a logged warning — fail-closed. The ``clara:``
        ...
        if ":" in event:
            logger.warning(
                ...
                plugin_key, event, plugin_key, CLARA_EVENT_NAMESPACE,
            )
            raise ValueError(
                f"Plugin '{plugin_key}' may not emit '{event}': emit only the "
                f"bare event name; the namespace is forced to '{plugin_key}:' "
                f"and the '{CLARA_EVENT_NAMESPACE}:' prefix is reserved for core"
            )
    ...
    def subscribe(self, event: str, callback: Callable) -> None:
        ...
        *event* is the full ``<plugin_key>:<event>`` name (or ``clara:<event>``
    ...
    def register_skill(
        ...
        ``~/.clara/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def __init__(self, scope_key: Optional[str] = None) -> None:
        ...
        # original scope.
        self.scope_key = scope_key or clara_home_key()
        ...
        # by resolved clara home). The ledger is therefore keyed per manager
        # — i.e. per (clara_home, plugin_id) — and every release/restore
        ...
        # contributed, so `clara plugins list` still attributes them once the
        # full plugin loads.
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        with self._discovery_lock, _plugin_home_scope(self.home_path):
            ...
            if env_var_enabled("CLARA_SAFE_MODE"):
                logger.info("CLARA_SAFE_MODE=1 — plugin discovery skipped")
            ...
            try:
                ...
                # load_clara_dotenv() already ran at import time. Re-pull so the
    ...
    def _refresh_secret_sources_after_discovery(self) -> None:
        ...
        try:
            ...
            from clara_cli.env_loader import load_clara_dotenv, reset_secret_source_cache
        except Exception:
            return
        try:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        if stale_relay_keys:
            logger.warning(
                "Removed Clara plugin %s is still listed in plugins.enabled; "
        ...
        for manifest in winners.values():
            ...
            # Relay lifecycle ownership now lives in the Clara core. Loading
            ...
            if (
                ...
                loaded.error = (
                    "removed — Relay lifecycle is owned by Clara core; configure "
                    f"{RELAY_PLUGINS_CONFIG_ENV} instead"
                )
                self._plugins[lookup_key] = loaded
                logger.warning(
                    "Refusing to load removed Clara Relay plugin '%s'; %s",
                    lookup_key,
                    loaded.error,
                )
                continue
            ...
            # Built-in backends auto-load — they ship with clara and must
            ...
            # all ~20 of them added several seconds to every `clara`
            # invocation — including plain `clara chat`, which never touches a
            ...
            # path actually asks for that platform. Every platform Clara ships
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `clara plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def register_approval_transport(
        ...
        *,
        ...
        from clara_cli.approval_transport import RegisteredApprovalTransport
        ...
        self._approval_transports[clean] = RegisteredApprovalTransport(
            ...
            present=present_fn,
            plugin_id=plugin_id,
            profile_home=str(get_clara_home().resolve()),
        )
    ...
    def get_approval_transport(self, name: str):
        ...
        if registered is None:
            return None
        if registered.profile_home != str(get_clara_home().resolve()):
            return None
        return registered
    ...
    def _collect_directory_manifests(self) -> List[PluginManifest]:
        ...
        # 2. User plugins (~/.clara/plugins/)
        user_dir = get_clara_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.clara/plugins/), only when explicitly opted
        # in. This must match the full discovery gate exactly.
        if _env_enabled("CLARA_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".clara" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set CLARA_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
    ...
    def has_enabled_portable_mcp(self, raw_config: Mapping[str, Any]) -> bool:
        ...
        """
        if _env_enabled("CLARA_SAFE_MODE"):
            return False
        ...
        for manifest in winners.values():
            ...
            if lookup_key not in enabled and manifest.name not in enabled:
                continue
            try:
                from clara_cli.agent_plugins import _discover_mcp
        ...
        *,
        ...
        for child in sorted(path.iterdir()):
            ...
            if portable_file.exists() or portable_file.is_symlink():
                try:
                    from clara_cli.agent_plugins import read_agent_plugin_manifest
    ...
    def _classify_entrypoint_kind(self, ep) -> str:
        ...
        module's import cost in every Clara process (e.g. a pip
    ...
    def _scan_entry_points(self) -> List[PluginManifest]:
        ...
        declarations from the ``clara_agent.plugin_capabilities`` group.
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``clara plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`clara plugins
    ...
    def _register_deferred_platform_tools(
        ...
        ``clara tools`` checklist, and even an explicit ``platform_toolsets``
        ...
        try:
            ...
            # `clara plugins list` under-reports what the process is actually
    ...
    def _warn_python_dependencies(self, manifest: PluginManifest) -> None:
        ...
        python_dependencies is a declaration seam ONLY: Clara validates and
        ...
        if missing:
            logger.warning(
                ...
                "installed: %s. Clara does not install plugin dependencies "
    ...
    def _load_plugin_scoped(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # above cannot see them and `clara plugins list` would
    ...
    def _load_portable_plugin(
        ...
        lookup_key = manifest.key or manifest.name
        try:
            from clara_cli.agent_plugins import load_agent_plugin

            package = load_agent_plugin(
                Path(manifest.path),
                get_clara_home() / "plugin-data" / manifest.skill_namespace,
            )
            ctx = PluginContext(manifest, self)
    ...
    def _load_directory_module(
        ...
        *,
        ...
        """Import a directory-based plugin as ``clara_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``clara_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
        ...
        # from a different Clara home (profile switch reusing a slug
        # like "clara-lcm") or from an earlier force=True reload in the
            ...
            try:
                if use_timeout:
                    ...
                    thread = threading.Thread(
                        target=_runner,
                        name=f"clara-hook-{callback_name}"[:40],
                        daemon=True,
                    )
                    thread.start()
...
# Keyed cache: resolved Clara home -> PluginManager. Clara supports
# multiple profiles via different CLARA_HOME directories, and a single
# long-lived process (gateway multiplexer, test session, embedder) can
# switch between them via ``set_clara_home_override()`` — which is a
# ContextVar and deliberately does NOT touch os.environ (see
# clara_constants.set_clara_home_override). A process-wide single-slot
...
def _plugin_home_key() -> Path:
    ...
    Plugins are discovered from ``get_clara_home() / "plugins"`` and some
    plugins (notably context engines such as clara-lcm) capture that home
    at registration time for profile-scoped storage. A long-lived process
    can temporarily switch Clara home (env var *or* the context-local
    ``set_clara_home_override()``) while serving another profile, so the
    plugin manager must be scoped to the active Clara home instead of
    being one process-wide singleton.
    """
    try:
        return get_clara_home().expanduser().resolve()
    except Exception:
        return get_clara_home().expanduser()
...
def _clear_plugin_submodules(manager: Optional[PluginManager]) -> None:
    ...
    ``clara_plugins.<slug>`` and registers that top-level module in
    ...
    *relative* import (``from . import foo``, ``from .sub import bar``)
    ...
    ``clara_plugins.<slug>.<submodule>``. When we swap in a fresh manager
...
def get_plugin_manager() -> PluginManager:
    """Return the plugin manager for the active Clara profile/home.
    ...
    profile switch — via ``CLARA_HOME`` or the context-local
    ``set_clara_home_override()`` — gets its own manager with its own
    ...
    with _plugin_managers_lock:
        ...
        if manager is None:
            manager = PluginManager(scope_key=clara_home_key(current_home))
...
def start_background_plugin_discovery() -> None:
    ...
    overlap the CPU/subprocess-heavy rest of startup. Every __PROT_2_synchroclara__
...
def _plugin_toolset_keys_cache_path():
    ...
    return get_clara_home() / "cache" / "plugin_toolset_keys.json"
...
def _get_pre_tool_call_directive_details(
    ...
    from clara_cli.lifecycle import invoke_hook as invoke_lifecycle_hook
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    own loop so the caller still gets a concrete result __PROT_4_synchroclaraly__. The
    ...
    thread = threading.Thread(
        target=_runner,
        name="clara-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_subscriptions() -> Dict[str, List[Callable]]:
    ...
    (``<plugin_key>:<event>`` or ``clara:<event>``) to subscriber callbacks in
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``clara tools`` TUI so plugin-provided toolsets appear
