"""Windows ConPTY bridge for the `clara dashboard` chat tab.

Drop-in counterpart to ``clara_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``clara_cli.web_server`` consumes: ``spawn``, ``read``,
