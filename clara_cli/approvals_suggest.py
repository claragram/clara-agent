"""``clara approvals suggest`` — mine approval history into allowlist proposals.

Clara has no dedicated approval-decision ledger: ``always`` answers land in
...
is the session DB (``~/.clara/state.db``): every assistant ``terminal`` tool
...
* **Destructive / privilege / credential / obfuscation classes are never
  ...
  restarts, clara self-management, …) are eligible.
...
def normalize_command(command: str) -> str:
    """Fold user/clara home prefixes and collapse whitespace.

    Reuses tools.approval's home-folding machinery so proposals are portable
    across machines/users (``/home/alice/x`` -> ``~/x``).
    """
    from tools.approval import (
        _rewrite_resolved_clara_home,
        _rewrite_resolved_user_home,
    )

    folded = _rewrite_resolved_user_home(_rewrite_resolved_clara_home(command))
...
def _render_text(proposals: list[Proposal], days: int) -> None:
    ...
    print(
        ...
        "  clara approvals suggest --apply 1,3\n"
        "Entries are merged into command_allowlist in ~/.clara/config.yaml."
    )


def suggest_command(args) -> int:
    """Entry point for ``clara approvals suggest``."""
...
def approvals_command(args) -> int:
    """Dispatch ``clara approvals <subcommand>``."""
    sub = getattr(args, "approvals_command", None)
    if sub == "suggest":
        return suggest_command(args)
    if sub == "test":
        from clara_cli.approvals_test import approvals_test_command
        return approvals_test_command(args)
    print(
        "usage: clara approvals <subcommand>\n"
        "\n"
        "subcommands:\n"
        ...
        "\n"
        "Run `clara approvals <subcommand> -h` for details."
    )
    return 1
