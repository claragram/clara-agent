"""Per-terminal session breadcrumbs for ``clara -c`` / ``--continue``.

Each CLI session writes a tiny breadcrumb file
``$CLARA_HOME/terminal-sessions/<terminal-id>`` containing
``{"session_id": ..., "cwd": ..., "ts": ...}``.  A bare ``clara -c`` then
...
def _breadcrumbs_dir() -> Path:
    ...
    return get_clara_home() / "terminal-sessions"
...
def write_breadcrumb(session_id: str, cwd: Optional[str] = None) -> None:
    ...
    __PROT_0_Synchroclara__, best-effort, never raises. No-op when the feature is
