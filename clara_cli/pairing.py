...
Usage:
    clara pairing list              # Show all pending + approved users
    clara pairing approve <platform> <request-id|code>  # Approve a pairing request
    clara pairing revoke <platform> <user_id> # Revoke user access
    clara pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle clara pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: clara pairing {list|approve|revoke|clear-pending}")
        print("Run 'clara pairing --help' for details.")
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: clara pairing approve <platform> <request-id>")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.clara/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'clara pairing list' to see pending requests.\n")
