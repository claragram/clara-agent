"""Dashboard process-hygiene helpers — extracted from ``clara_cli/main.py``.
...
``_detect_concurrent_clara_instances``) are lifted verbatim. References to
helpers that STAY in ``clara_cli.main`` (``_find_stale_dashboard_pids``,
``_respawn_dashboard_processes``, ``_is_windows``, ...) are routed through a
lazy ``_m()`` main reference so existing test monkeypatches on
``clara_cli.main.<name>`` keep reaching this code path, and imports stay
...
patches on ``clara_cli.main`` resolve unchanged.
"""
...
def _m():
    """Lazy ``clara_cli.main`` reference (call-time; keeps patches working)."""
...
def _scan_dashboard_processes(
    *,
    ...
    ``clara dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``clara update`` replaces files on disk, the running
    ...
    ``clara-dashboard.service`` systemd unit.  Managed units are restarted
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Clara Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``clara serve`` as
    ...
    environment variable ``CLARA_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        ...
        # The headless backend (`clara serve`) is the same long-lived server
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "clara.*dashboard"`) keeps us consistent
            # with `clara_cli.gateway._scan_gateway_pids` and avoids the
    ...
    # miss profiled launches — `clara --profile p serve --host <ip>` contains
    # neither "clara serve" nor "clara_cli.main serve". Every serve/
    ...
    # truncated view.
    try:
        from clara_cli.process_identity import ledger_entries
...
def _clara_home_for_pid(pid: int) -> str | None:
    """Best-effort ``CLARA_HOME`` from *pid*'s environment."""
    try:
        import psutil

        home = psutil.Process(pid).environ().get("CLARA_HOME")
        if home:
            return home
    except Exception:
        pass
    ...
    for part in raw.split(b"\x00"):
        if part.startswith(b"CLARA_HOME="):
...
def _is_ephemeral_port_zero_backend(argv: list[str]) -> bool:
    ...
    Ephemeral-port backends are owned by Clara Desktop (or become PPID-1
    orphans after a prior update respawn).  Replaying them after
    ``clara update`` multiplies listening backends because ``--port 0``
...
def _profile_key_for_respawn(
    argv: list[str], clara_home: str | None = None
) -> str:
    """Stable owner key: ``CLARA_HOME`` when known, else ``--profile`` / ``-p``.

    ``CLARA_HOME`` ending in ``profiles/<name>`` is normalized to
    ``profile:<name>`` so it shares a cap with an explicit ``--profile``
    flag for the same profile (#78821).  Non-profile homes (including
    distinct ``…/.clara`` roots) keep a resolved ``home:`` key so
    ...
    if clara_home:
        try:
            home_path = Path(clara_home).resolve()
        except (OSError, RuntimeError, ValueError):
            home_path = Path(clara_home)
        ...
        try:
            ...
            return f"home:{os.path.normcase(clara_home)}"
...
def _filter_dashboard_respawn_candidates(
    ...
    *,
    ...
    """Select which killed manual backends to respawn after ``clara update``.

    Each candidate is ``(pid, argv, clara_home)``.  *own_home* is the
    updating install's home; it defaults to this process's
    ``get_clara_home()`` and exists as a parameter so tests can pin it.

    Rules (#78821, #94030):
    1. Never resurrect Desktop ephemeral ``serve|dashboard --port 0``
       backends — Desktop (``CLARA_DESKTOP_CHILD_PID``) owns their
       lifecycle.  These are also the PPID-1 orphans that previously
       multiplied across updates because ``--port 0`` always binds a
       fresh free port.
    2. Never replay a backend from a **foreign** ``CLARA_HOME``.  The
    ...
    4. Cap at most one managed backend per profile / ``CLARA_HOME``.

    Intentionally does **not** blanket-skip every PPID-1 process: a prior
    ``clara update`` respawn detaches with ``start_new_session=True``, so
    ...
    for _pid, argv, clara_home in candidates:
        if not argv:
            continue
        if _is_ephemeral_port_zero_backend(argv):
            continue
        if own_key and clara_home and _normalized_home_for_compare(clara_home) != own_key:
            continue
        norm = _normalize_dashboard_cmdline(argv)
        if norm in seen_cmdlines:
            continue
        profile_key = _profile_key_for_respawn(argv, clara_home)
...
def _kill_stale_dashboard_processes(
    ...
    *,
    ...
    """Kill running ``clara dashboard`` / ``clara serve`` processes.

    Called at the end of ``clara update`` (default ``reason``) and also
    from ``clara dashboard --stop`` (which overrides ``reason``).  The
    ...
    When ``restart_managed`` is true (the ``clara update`` path), a detected
    ``clara-dashboard.service`` is restarted through systemd; any OTHER
    killed PID that was supervised by a systemd unit (custom unit names —
    e.g. a remote backend's ``clara-serve.service``) has its owning unit
    ...
    *already_restarted_units* names units (no ``.service`` suffix) the
    caller already restarted directly — e.g. ``clara update``'s systemd
    fleet-restart loop, which restarts ``clara-serve*`` units before this
    ...
    if restart_managed and _m()._restart_managed_dashboard_service(reason):
        ...
        # ``clara-dashboard.service`` and ``clara-serve.service`` -- the
        ...
        _dash_unit = getattr(
            _m(), "_DASHBOARD_SYSTEMD_UNIT", "clara-dashboard.service"
        )
        already_restarted_units = set(already_restarted_units or ()) | {
            str(_dash_unit).removesuffix(".service")
        }

    # When the Clara Desktop Electron app spawns this dashboard as a
    # backend child, it sets CLARA_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] = set()
    raw_pid = os.environ.get("CLARA_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    # caller asked for restarts (the `clara update` path) — `--stop` must
    ...
    if restart_managed and sys.platform != "win32":
        for pid in pids:
            ...
            if not pid_service[pid]:
                ...
                # Snapshot CLARA_HOME before the kill so per-profile caps
                # still work after the process is gone (#78821).
                cmdline = _m()._dashboard_cmdline_for_pid(pid)
                if cmdline:
                    pid_cmdline[pid] = cmdline
                    pid_home[pid] = _clara_home_for_pid(pid)

        if already_restarted_units:
            # Already handled directly by the caller (e.g. clara update's
        ...
        for pid in pids:
            try:
                ...
                if not pid_is_clara(
                    ...
                    failed.append((pid, "not clara-owned or process identity changed"))
                    continue
                result = subprocess.run(
    ...
    #    remote backend (clara serve) under Restart=on-failure never comes
...
def _detect_concurrent_clara_instances(
    ...
    The Clara Desktop Electron app spawns ``clara.EXE`` as a backend child,
    so during ``clara update`` the user-invoked process and the desktop's
    ...
    shims (``clara.exe`` / ``clara-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``clara update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``clara.exe``) is a separate process from the Python
    ...
    for shim in _m()._clara_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated clara.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``clara update``
    ...
    #      clara.exe sitting *under* a non-Clara parent (e.g. a Clara
...
def _is_desktop_local_serve_cmdline(command: str) -> bool:
    ...
    Desktop primary/pool backends launch as::
        ...
        clara serve --host 127.0.0.1 --port 0
        clara serve --isolated --host 127.0.0.1 --port 0 ...
    ...
    if "serve" not in cmd:
        return False
    if "clara" not in cmd and "clara_cli" not in cmd:
        return False
...
def _exclude_pids_from_env() -> set[int]:
    """PIDs Desktop marks as live backends (CLARA_DESKTOP_CHILD_PID)."""
    raw = os.environ.get("CLARA_DESKTOP_CHILD_PID", "")
    out: set[int] = set()
...
# on the *remote* host for every ``clara serve`` backend it spawns over SSH
# (see apps/desktop/electron/remote-lifecycle.ts). A backend started from
# another client/machine — e.g. a MacBook driving a ``clara serve`` on a Mac
...
def _clara_home_dir() -> Path:
    """Resolved Clara home (CLARA_HOME override or ~/.clara)."""
    override = os.environ.get("CLARA_HOME", "").strip()
    if override:
...
def _valid_lockfile_payload(parsed: object, ownership_id: str) -> bool:
    ...
    *adds* lock-owned PIDs to its spare-set.
    ...
    for field in ("profile", "claraPath", "claraHome", "logPath", "startedAt"):
        value = parsed.get(field)
        if not isinstance(value, str) or len(value) > 1024:
            return False
    # logPath is written as ``{lock_root}/{ownershipId}/{spawnNonce}.log``. We
    # only check the suffix so a relocated CLARA_HOME (different leading path)
...
def _lock_owned_serve_pids(base_dir: Path | None = None) -> set[int]:
    ...
    Scans ``{clara_home}/desktop-ssh/<ownershipId>/backend.lock.json`` (the
    ...
    root = base_dir if base_dir is not None else (
        _clara_home_dir() / _REMOTE_LOCK_SUBDIR
    )
    owned: set[int] = set()
    if not root.is_dir():
        return owned
...
def _reap_orphaned_desktop_local_serves(
    *,
    reason: str = "orphaned desktop-local clara serve",
    signal_term=None,
    signal_kill=None,
    sleep_fn=None,
    ...
    """Kill leftover Desktop-local ``clara serve`` backends with no parent.
    ...
    running under CLARA_PARENT_PID; this helper clears *already* orphaned
    ...
    - never self / never CLARA_DESKTOP_CHILD_PID entries
    ...
    - never a candidate younger than ``_REAP_MIN_AGE_SECONDS`` (or whose age
      ...
      only after the backend reports CLARA_BACKEND_READY, so during
    ...
    for pid, cmd in scanned:
        ...
        # CLARA_BACKEND_READY, so a sibling spawned seconds ago is not yet
