"""Profile distributions — shareable, packaged Clara profiles via git.

A distribution is a Clara profile published as a git repository (or
...
* ``clara profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``clara skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``clara profile``, not a parallel tree):

    clara profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "clara-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            clara_requires=str(data.get("clara_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.clara_requires:
            out["clara_requires"] = self.clara_requires
        if self.author:
...
def check_clara_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Clara {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``clara profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Clara profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Clara distribution."
        )

    # Version check up-front so we fail fast
    check_clara_requires(manifest.clara_requires, clara_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.clara).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="clara_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `clara profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`clara profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="clara_dist_update_") as tmp:
        plan = plan_install(
