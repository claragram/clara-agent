"""
Multi-provider authentication system for Clara Agent.

Supports OAuth device code flows (Clara Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.clara/auth.json with cross-process file locking.
...
# httpx is imported lazily: it costs ~30ms at import time and clara_cli.auth
...
if TYPE_CHECKING:
    ...
    class _LazyHttpx:
        ...
        # ("clara_cli.auth.httpx.Client", ...) keeps working in tests.
...
from clara_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
DEFAULT_CLARA_INFERENCE_URL = "https://inference-api.claraprise.com/v1"
DEFAULT_CLARA_CLIENT_ID = "clara-cli"
CLARA_INFERENCE_INVOKE_SCOPE = "inference:invoke"
CLARA_BILLING_MANAGE_SCOPE = "billing:manage"
DEFAULT_CLARA_SCOPE = CLARA_INFERENCE_INVOKE_SCOPE
CLARA_DEVICE_CODE_SOURCE = "device_code"
CLARA_AUTH_PATH_INVOKE_JWT = "invoke_jwt"
ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120       # refresh 2 min before expiry
CLARA_INVOKE_JWT_MIN_TTL_SECONDS = ACCESS_TOKEN_REFRESH_SKEW_SECONDS
...
try:  # Version tag for the Codex token-endpoint User-Agent; fall back if unavailable.
    from clara_cli import __version__ as _CLARA_CLI_VERSION
except Exception:  # pragma: no cover - version import should always succeed
    _CLARA_CLI_VERSION = "unknown"
CODEX_OAUTH_USER_AGENT = f"clara-cli/{_CLARA_CLI_VERSION}"
...
SPOTIFY_DOCS_URL = "https://agent.claraprise.com/docs/user-guide/features/spotify"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

OAUTH_OVER_SSH_DOCS_URL = "https://agent.claraprise.com/docs/guides/oauth-over-ssh"
...
def normalize_actual_base_url(base_url: str) -> str:
    ...
    surface for Clara' Responses transport.
    """
...
PROVIDER_REGISTRY: Dict[str, ProviderConfig] = {
    "clara": ProviderConfig(
        id="clara",
        name="Clara Portal",
        auth_type="oauth_device_code",
        portal_base_url=DEFAULT_CLARA_PORTAL_URL,
        inference_base_url=DEFAULT_CLARA_INFERENCE_URL,
        client_id=DEFAULT_CLARA_CLIENT_ID,
        scope=DEFAULT_CLARA_SCOPE,
    ),
    ...
    "anthropic": ProviderConfig(
        ...
        # from it), so removing it would stop Clara finding a setup-token
    ...
    "opencode-free": ProviderConfig(
        ...
        # secret to configure. Select via `clara model` / `/model free`.
        api_key_env_vars=(),
    ),
...
try:
    ...
    for _pp in _list_providers_for_registry():
        ...
        if _pp.auth_type == "external_process":
            ...
            # without it, `clara -m <that provider>` dies with
...
def get_anthropic_key() -> str:
    ...
    ``~/.clara/.env`` so a deliberate key rotation isn't shadowed by a stale
    ...
    """
    from clara_cli.config import get_env_value_prefer_dotenv
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from clara_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Prefer ~/.clara/.env over os.environ so a deliberate key rotation
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    instead of prompting the operator to run ``clara auth``.
    """
    return (
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `clara model` to re-authenticate."

    if error.code == "subscription_required":
        if error.provider == "clara":
            return _format_clara_entitlement_auth_error(error)
...
def _format_clara_entitlement_auth_error(error: AuthError) -> str:
    try:
        ...
        message = format_clara_portal_entitlement_message(
            account_info,
            capability="Clara model access",
        )
        if message:
            return message
    except Exception:
        pass
    return f"{error} Check credits or billing in Clara Portal, then retry."
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("CLARA_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.clara/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_clara_home() / "auth.json"
    # Seat belt: if pytest is running and CLARA_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch CLARA_HOME, tests invoked without the
    # hermetic conftest, or sandbox escapes via threads/subprocesses. In
    # production (no PYTEST_CURRENT_TEST) this is a single dict lookup.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".clara" / "auth.json").resolve(strict=False)
        try:
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set CLARA_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom CLARA_HOME that is not a profile).
    ...
    try:
        ...
        global_root = get_default_clara_root()
    except Exception:
        return None
    profile_home = get_clara_home()
    try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``_clara_auth_status_cache``): read_credential_pool() -> load_pool() runs
    this once per provider row in the /model picker, and the path resolution
    (``_global_auth_file_path()`` -> ``get_default_clara_root()``) + JSON
...
def _file_lock(
    ...
    (e.g. profile auth.json vs shared Clara store) don't share reentrancy
    ...
    # A stress test with 20 concurrent Clara processes reproduced this
...
def _auth_store_lock(
    ...
    *,
    ...
    ``_clara_shared_store_lock``, acquire ``_auth_store_lock`` FIRST
    (outer) and the shared Clara lock SECOND (inner). All runtime
...
def _load_auth_store(auth_file: Optional[Path] = None) -> Dict[str, Any]:
    ...
    if isinstance(raw, dict) and (
        ...
        if isinstance(raw.get("providers"), dict):
            _migrate_stale_clara_portal_url(raw["providers"])
        return raw
    ...
    if isinstance(raw, dict) and isinstance(raw.get("systems"), dict):
        systems = raw["systems"]
        providers = {}
        if "clara_portal" in systems:
            providers["clara"] = systems["clara_portal"]
        return {"version": AUTH_STORE_VERSION, "providers": providers,
                "active_provider": "clara" if providers else None}
...
def _save_auth_store(auth_store: Dict[str, Any], target_path: Optional[Path] = None) -> Path:
    ...
    # clara-agent install tree (#25821, #93050).
...
def _load_provider_state_with_source(
    ...
    global-root fallback state; persisting a rotated Clara refresh token only to
...
def _load_provider_state(auth_store: Dict[str, Any], provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    profile can see providers (e.g. ``clara``) that were only authenticated at
    global scope. Once the user runs ``clara auth login <provider>`` inside
...
def mark_provider_active_if_unset(provider_id: str) -> None:
    ...
    Used by ``clara auth add`` OAuth paths that create credential-pool
...
def strip_cloned_single_use_oauth_grants(profile_dir: Path) -> Dict[str, Any]:
    ...
    into another (``clara profile create --clone-all``, the dashboard/TUI
...
# (an independent ``clara -p <p> auth add`` grant, or the only surviving
...
def consume_oauth_heal_notices() -> List[str]:
    ...
    ``clara auth list`` / ``clara auth status`` print them so the user sees
...
def _find_root_counterpart(
    ...
    Anthropic ``clara_pkce`` grant uses one client id and carries no claims,
    ...
    (an independent ``clara -p <p> auth add`` login for another account).
    """
...
def _heal_forked_single_use_oauth_grants(provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        # user's ~/.clara/auth.json from a test that forgot to isolate HOME.
        real_home_env = os.environ.get("HOME", "")
        if real_home_env and _same_path(root_path, Path(real_home_env) / ".clara" / "auth.json"):
            return None
    ...
    with _auth_store_lock():
        ...
        with _auth_store_lock(target_path=root_path):
            ...
            for row in p_rows:
                ...
                # .anthropic_oauth.json (the ``clara auth`` PKCE shape); a
                # profile clara_pkce-family row is that grant's copy.
                is_pkce = str(row.get("source") or "").endswith("clara_pkce")
                    ...
                    if root_singleton_row is not None:
                        ...
                        idx = next(
                            (i for i, r in enumerate(r_rows)
                             ...
                             and str(r.get("source") or "").endswith("clara_pkce")),
                            None,
                        )
            ...
            if summary["adopted"] and root_singleton is not None and root_singleton_row is not None:
                # Keep root's singleton and its ``clara_pkce``-seeded pool row
                ...
                pkce_idx = next(
                    (i for i, r in enumerate(r_rows)
                     if _is_oauth_pool_payload(r) and r.get("source") == "clara_pkce"),
                    None,
                )
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``clara auth add <provider>`` inside the profile, profile entries
...
def get_provider_auth_state(provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    OAuth or Clara device-code session). See issue #18594 follow-up.
    """
    auth_store = _load_auth_store()
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    try:
        ...
        # ``provider: anthropic`` as a MoA advisor/aggregator has opted Clara
    ...
    # not by the user explicitly configuring anthropic in Clara.
    ...
    # the user initiated inside Clara (manual add / device-code / PKCE), plus
    ...
    try:
        for entry in read_credential_pool(normalized):
            ...
            if source.startswith("env:"):
                ...
                continue
            if (
                source in {"device_code", "loopback_pkce", "clara_pkce", "manual"}
    ...
    # explicit for users who never set Clara up for it. Only Clara-scoped
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `clara logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from clara_cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""

        lines = ["Config issue detected — run 'clara doctor' for full diagnostics:"]
        for ci in issues:
...
def _refuse_env_adoption_if_config_corrupt() -> None:
    ...
    When ``~/.clara/config.yaml`` EXISTS but fails to parse, ``load_config()``
    ...
    """
    try:
        from clara_cli.config import get_active_config_parse_failure, get_config_path
    ...
    raise AuthError(
        ...
        f"was saved next to it) or run clara setup.",
        code="corrupt_config",
    )
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'clara model' for available providers, or run 'clara doctor' to diagnose config issues."
    ...
    # Auto-detect an OpenRouter credential added via `clara auth add openrouter`
    ...
    # `clara auth list` showing the credential while requests go out with no
    ...
    for pid, pconfig in PROVIDER_REGISTRY.items():
        ...
        for env_var in pconfig.api_key_env_vars:
            if has_usable_secret(_scoped_key_env(env_var)):
                ...
                # OAuth but has a stale key in ~/.clara/.env isn't silently
    ...
    raise AuthError(
        "No inference provider configured. Run 'clara model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.clara/.env.",
...
_CLARA_PORTAL_ALLOWED_HOSTS: FrozenSet[str] = frozenset({
    "portal.claraprise.com",
    "localhost",
    "127.0.0.1",
})


def _migrate_stale_clara_portal_url(providers: Dict[str, Any]) -> None:
    clara = providers.get("clara")
    if not isinstance(clara, dict):
        return
    stored = (clara.get("portal_base_url") or "").strip()
    if stored:
        parsed = urlparse(stored)
        if parsed.hostname in _CLARA_STALE_PORTAL_HOSTS:
            logger.warning(
                "auth: migrating stale clara portal_base_url %s -> %s",
                stored, DEFAULT_CLARA_PORTAL_URL,
            )
            clara["portal_base_url"] = DEFAULT_CLARA_PORTAL_URL


# Allowlist of hosts the Clara Portal proxy is willing to forward inference
...
# (CLARA_INFERENCE_BASE_URL) bypass validation — that's the documented
...
_ALLOWED_CLARA_INFERENCE_HOSTS: FrozenSet[str] = frozenset({
    "inference-api.claraprise.com",
})


def _validate_clara_inference_url_from_network(url: Optional[str]) -> Optional[str]:
    ...
    The env-var override path (``CLARA_INFERENCE_BASE_URL``) bypasses
    ...
    if parsed.scheme != "https":
        logger.warning(
            "clara: refusing non-https inference URL scheme %r from Portal response",
            parsed.scheme,
        )
        return None
    if parsed.hostname not in _ALLOWED_CLARA_INFERENCE_HOSTS:
        logger.warning(
            "clara: refusing inference URL host %r from Portal response "
...
def _clara_inference_env_override() -> Optional[str]:
    """Return the user-set ``CLARA_INFERENCE_BASE_URL`` override, if any.
    ...
    """
    return _optional_base_url(os.getenv("CLARA_INFERENCE_BASE_URL"))


def _clara_portal_env_override() -> Optional[str]:
    """Return the user/deployment-set Portal base URL override, if any.

    Mirrors ``_clara_inference_env_override()``: ``CLARA_PORTAL_BASE_URL`` /
    ...
    return _optional_base_url(
        os.getenv("CLARA_PORTAL_BASE_URL") or os.getenv("CLARA_PORTAL_BASE_URL")
    )
...
def _clara_invoke_jwt_status(
    token: Any,
    *,
    scope: Any = None,
    expires_at: Any = None,
    min_ttl_seconds: int = CLARA_INVOKE_JWT_MIN_TTL_SECONDS,
) -> Optional[str]:
    ...
    if CLARA_INFERENCE_INVOKE_SCOPE not in scopes:
...
def _clara_invoke_jwt_is_usable(
    token: Any,
    *,
    scope: Any = None,
    expires_at: Any = None,
...
def _assert_clara_inference_jwt_usable(
    ...
    *,
    ...
    if reason is None:
        return
    raise AuthError(
        "Clara Portal access token is not a usable inference JWT "
        f"({reason}). Re-authenticate with: clara auth add clara",
        provider="clara",
        code=reason,
...
def _log_clara_invoke_jwt_selected(
    *,
    access_token: Any,
    sequence_id: Optional[str] = None,
) -> None:
    logger.debug("Clara inference auth: using NAS invoke JWT")
    _oauth_trace(
        "clara_invoke_jwt_selected",
        sequence_id=sequence_id,
        access_token_fp=_token_fingerprint(access_token),
    )


def _clara_jwt_expires_at(token: Any, fallback_expires_at: Any = None) -> Optional[str]:
    claims = _decode_jwt_claims(token)
    exp = claims.get("exp")
    if isinstance(exp, (int, float)):
        try:
...
def _set_clara_agent_key_from_invoke_jwt(
    state: Dict[str, Any],
    *,
    ...
    expires_at = _clara_jwt_expires_at(access_token, state.get("expires_at"))
...
def _select_clara_invoke_jwt(
    ...
    *,
    ...
    _set_clara_agent_key_from_invoke_jwt(state)
    _log_clara_invoke_jwt_selected(
        access_token=state.get("access_token"),
        sequence_id=sequence_id,
    )


_CLARA_EFFECTIVE_STATE_IGNORED_KEYS = frozenset({
...
def _clara_effective_provider_state(state: Dict[str, Any]) -> Dict[str, Any]:
    return {
        key: value
        for key, value in state.items()
        if key not in _CLARA_EFFECTIVE_STATE_IGNORED_KEYS
    }
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("CLARA_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
def get_qwen_auth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        # and `clara model` walks users into a broken Qwen setup flow.
...
# =============================================================================
# Spotify auth — PKCE tokens stored in ~/.clara/auth.json
# =============================================================================
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("CLARA_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set CLARA_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("CLARA_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("CLARA_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("CLARA_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `clara auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `clara auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `clara auth spotify` first.",
                provider="spotify",
        ...
        if should_refresh:
            try:
                ...
                if exc.relogin_required and state.get("refresh_token"):
                    ...
                    # Mirrors the Clara / xAI-OAuth / Codex-OAuth / MiniMax pattern.
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `clara auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.clara/.env, and return it.
    ...
    print("       App name:     anything (e.g. clara-agent)")
    ...
    # Persist so subsequent `clara auth spotify` runs skip the wizard.
    save_env_value("CLARA_SPOTIFY_CLIENT_ID", raw)
    ...
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        save_env_value("CLARA_SPOTIFY_REDIRECT_URI", redirect_uri_hint)

    print()
    print("Saved CLARA_SPOTIFY_CLIENT_ID to ~/.clara/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    # with "CLARA_SPOTIFY_CLIENT_ID is required".
    ...
    print()
    print("Open this URL to authorize Clara:")
    print(authorize_url)
    print()
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(divider)
    print(f"Clara is waiting for the OAuth callback on {redirect_uri}")
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.clara/auth.json (not ~/.codex/)
#
# Clara maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Clara auth store (~/.clara/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `clara auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `clara auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `clara auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `clara auth` to re-authenticate.",
            provider="openai-codex",
...
def _sync_codex_pool_entries(
    ...
    * ``device_code`` — the singleton-seeded entry written by the device-code
      OAuth flow when the user logged in via ``clara setup`` / the model
      picker.  Always synced with the fresh tokens.
    * ``manual:device_code`` — entries created by ``clara auth add openai-codex``
      ...
      ``clara auth add openai-codex`` and must not be overwritten by a
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    """Save Codex OAuth tokens to Clara auth store (~/.clara/auth.json)."""
    ...
    with _auth_store_lock():
        ...
        # ``clara auth add openai-codex`` created (which must not be
...
def _recover_codex_tokens_from_cli(reason: str) -> Optional[Dict[str, str]]:
    """Adopt a valid Codex CLI token pair into Clara auth, if available."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating Clara auth state."""
    ...
    if response.status_code == 429:
        ...
        # "run clara auth" prompt (see issue #32790).
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `clara auth` to re-authenticate."
            )
            relogin_required = True
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Clara auth store automatically.
    """
    try:
        ...
        # Self-heal cross-store refresh_token rotation. Clara keeps its OWN
        ...
        # the Codex CLI (or another Clara process) rotates the shared token,
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Clara's own Codex token store.
    ...
    if data is None:
        ...
        if pool_token:
            base_url = (
                os.getenv("CLARA_CODEX_BASE_URL", "").strip().rstrip("/")
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("CLARA_CODEX_REFRESH_TIMEOUT_SECONDS", 20)

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other Clara processes
...
def _probe_codex_quota_restored(
    ...
    *,
    ...
    Clara persists a Codex 429's ``reset_at`` locally and freezes the
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.clara/auth.json
# =============================================================================
...
def _read_xai_oauth_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No xAI OAuth credentials stored. Select xAI Grok OAuth (SuperGrok / Premium+) in `clara model`.",
            provider="xai-oauth",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "xAI OAuth state is missing tokens. Re-authenticate with `clara model`.",
            provider="xai-oauth",
    ...
    if not access_token:
        raise AuthError(
            "xAI OAuth state is missing access_token. Re-authenticate with `clara model`.",
            provider="xai-oauth",
    ...
    if not refresh_token:
        raise AuthError(
            "xAI OAuth state is missing refresh_token. Re-authenticate with `clara model`.",
            provider="xai-oauth",
...
def _xai_proactive_refresh_skew_seconds(access_token: str) -> int:
    ...
    resolution (chat turn, Imagine tool call, ``clara auth status``, …),
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.clara/auth.json``. A single MITM during initial
    ...
    if host != "x.ai" and not host.endswith(".x.ai"):
        raise AuthError(
            ...
            f"discovery; re-authenticate with `clara model` to re-fetch.",
            provider="xai-oauth",
            code="xai_discovery_invalid",
        )
    return url
...
def _xai_validate_inference_base_url(value: str, *, fallback: str) -> str:
    ...
    the user's SuperGrok subscription. ``XAI_BASE_URL`` / ``CLARA_XAI_BASE_URL``
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "xAI OAuth is missing refresh_token. Re-authenticate with `clara model`.",
            provider="xai-oauth",
            code="xai_auth_missing_refresh_token",
            relogin_required=True,
        )
    ...
    # written by an older Clara (or hand-edited) may carry a non-xAI
    ...
    # with a clear error so the user can re-run `clara model` to refetch.
    ...
    if response.status_code != 200:
        ...
        # on the allowlist for API access).  Re-running ``clara model``
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("CLARA_XAI_REFRESH_TIMEOUT_SECONDS", 20)
...
def _clara_device_auth_timeout_message(portal_base_url: str) -> str:
    """Actionable timeout text for Clara device-code login failures.
    ...
    """
    portal = (portal_base_url or DEFAULT_CLARA_PORTAL_URL).rstrip("/")
    return (
...
def _poll_for_token(
    ...
    # the CLI login (_clara_device_code_login) and the dashboard/desktop
    # poller (web_server._clara_poller, which surfaces str(e) to the UI).
    raise TimeoutError(_clara_device_auth_timeout_message(portal_base_url))


# =============================================================================
# Clara Portal — token refresh and model discovery
# =============================================================================

# -----------------------------------------------------------------------------
# Shared Clara token store — lets OAuth credentials persist across profiles
# so a new `clara --profile <name> auth add clara --type oauth` can one-tap
# import instead of running the full device-code flow every time.
#
# File lives at ${CLARA_SHARED_AUTH_DIR}/clara_auth.json, defaulting to
# ``<clara-root>/shared/clara_auth.json`` where ``<clara-root>`` is what
# ``get_default_clara_root()`` returns — ``~/.clara`` on Linux/macOS,
# ``%LOCALAPPDATA%\clara`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's CLARA_HOME so named profiles (which
# typically live under ``<clara-root>/profiles/<name>/``) all see the
# same file.
#
...
CLARA_SHARED_STORE_FILENAME = "clara_auth.json"
_clara_shared_lock_holder = threading.local()


def _clara_shared_auth_dir() -> Path:
    """Resolve the directory that holds the shared Clara token store.

    Honors ``CLARA_SHARED_AUTH_DIR`` so tests can redirect it to a tmp
    path without touching the real user's home. Defaults to
    ``<clara-root>/shared/``, where ``<clara-root>`` is what
    :func:`clara_constants.get_default_clara_root` returns — so
    Linux/macOS classic installs land at ``~/.clara/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\clara\\shared\\``, and
    Docker / custom ``CLARA_HOME`` deployments at
    ``<CLARA_HOME>/shared/``. Sits outside any named profile so all
    profiles under the same root share the store.
    """
    override = os.getenv("CLARA_SHARED_AUTH_DIR", "").strip()
    if override:
        return Path(override).expanduser()
    from clara_constants import get_default_clara_root
    return get_default_clara_root() / "shared"


def _clara_shared_store_path() -> Path:
    path = _clara_shared_auth_dir() / CLARA_SHARED_STORE_FILENAME
    # Seat belt: if pytest is running and this resolves to a path under the
    # real user's Clara root, refuse rather than silently corrupt cross-profile
    # state. Tests must set CLARA_SHARED_AUTH_DIR to a tmp_path (conftest
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from clara_constants import get_default_clara_root
        real_home_shared = (
            get_default_clara_root() / "shared" / CLARA_SHARED_STORE_FILENAME
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                f"Refusing to touch real user shared Clara auth store during test run: "
                f"{path}. Set CLARA_SHARED_AUTH_DIR to a tmp_path in your test fixture."
            )
    return path


@contextmanager
def _clara_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    """Cross-profile lock for the shared Clara OAuth store.
    ...
    ``_try_import_shared_clara_state``, which holds this lock alone for
    ...
    """
    try:
        lock_path = _clara_shared_store_path().with_suffix(".lock")
    except RuntimeError:
        # No CLARA_HOME yet (pre-setup): fall through without locking.
        yield
        return

    with _file_lock(
        lock_path,
        _clara_shared_lock_holder,
        timeout_seconds,
        "Timed out waiting for shared Clara auth lock",
    ):
        yield


def _merge_shared_clara_oauth_state(state: Dict[str, Any]) -> bool:
    """Copy fresher shared OAuth tokens into a profile-local Clara state."""
    shared = _read_shared_clara_state()
    if not shared:
        return False
...
def _write_shared_clara_state(state: Dict[str, Any]) -> None:
    """Persist a minimal copy of the Clara OAuth state to the shared store.
    ...
    shared = {
        ...
        "scope": state.get("scope") or DEFAULT_CLARA_SCOPE,
        "client_id": state.get("client_id") or DEFAULT_CLARA_CLIENT_ID,
        "portal_base_url": state.get("portal_base_url") or DEFAULT_CLARA_PORTAL_URL,
        "inference_base_url": state.get("inference_base_url") or DEFAULT_CLARA_INFERENCE_URL,
        "obtained_at": state.get("obtained_at"),
        "expires_at": state.get("expires_at"),
        "updated_at": datetime.now(timezone.utc).isoformat(),
    }
    try:
        with _clara_shared_store_lock():
            path = _clara_shared_store_path()
            ...
            # Create with 0o600 atomically via os.open(O_EXCL) — closes the TOCTOU
            # window where write_text() + post-write chmod briefly exposed Clara
            ...
            try:
                ...
                try:
                    ...
                    pass
        _oauth_trace(
            "clara_shared_store_written",
            path=str(path),
            refresh_token_fp=_token_fingerprint(refresh_token),
        )
    except Exception as exc:
        logger.debug("Failed to write shared Clara auth store: %s", exc)


def _read_shared_clara_state() -> Optional[Dict[str, Any]]:
    """Return the shared Clara OAuth state if present and well-formed.
    ...
    try:
        ...
        logger.debug("Shared Clara auth store at %s is unreadable: %s", path, exc)
        return None
    if not isinstance(payload, dict):
        return None
...
def _clear_shared_clara_state(reason: str) -> None:
    """Remove the shared Clara OAuth store after a terminal token failure."""
    try:
        with _clara_shared_store_lock():
            path = _clara_shared_store_path()
            try:
                ...
                pass
        _oauth_trace("clara_shared_store_cleared", reason=reason)
    except Exception as exc:
        logger.debug("Failed to clear shared Clara auth store: %s", exc)


def _is_terminal_clara_refresh_error(exc: Exception) -> bool:
    """True when retrying the same Clara refresh token cannot succeed."""
    return (
        isinstance(exc, AuthError)
        and exc.provider == "clara"
...
def _quarantine_clara_oauth_state(
    state: Dict[str, Any],
    error: AuthError,
    *,
    reason: str,
    ...
    # sizes/booleans. NEVER pass a raw token/agent_key into the log call — Clara
    # has a known bug class where credential-shaped literals get corrupted in logs.
    forensic: Dict[str, Any] = {
        "reason": reason,
        "error_code": error.code,
        # No session_id field exists on Clara state; provenance is client_id +
    ...
    logger.warning(
        "Clara OAuth state quarantined (terminal auth death): %s",
    ...
    _clear_shared_clara_state(reason)
    invalidate_clara_auth_status_cache()


def _quarantine_clara_pool_entries(
    auth_store: Dict[str, Any],
    error: AuthError,
    *,
    reason: str,
) -> bool:
    """Remove singleton-seeded Clara pool entries that contain dead OAuth state."""
    pool = auth_store.get("credential_pool")
    if not isinstance(pool, dict):
        return False
    entries = pool.get("clara")
    if not isinstance(entries, list):
        return False

    retained = []
    removed = False
    singleton_sources = {CLARA_DEVICE_CODE_SOURCE, f"manual:{CLARA_DEVICE_CODE_SOURCE}"}
    for entry in entries:
    ...
    if removed:
        pool["clara"] = retained
        _oauth_trace(
            "clara_pool_device_code_quarantined",
            reason=reason,
            error_code=error.code,
        )
    return removed
...
def _try_import_shared_clara_state(
    *,
    timeout_seconds: float = 15.0,
) -> Optional[Dict[str, Any]]:
    """Attempt to rehydrate Clara OAuth state from the shared store.
    ...
    """
    try:
        with _clara_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            shared = _read_shared_clara_state()
            if not shared:
                return None

            # Build a full state dict so refresh_clara_oauth_from_state has every
            ...
            state: Dict[str, Any] = {
                ...
                "client_id": shared.get("client_id") or DEFAULT_CLARA_CLIENT_ID,
                "portal_base_url": shared.get("portal_base_url") or DEFAULT_CLARA_PORTAL_URL,
                "inference_base_url": shared.get("inference_base_url") or DEFAULT_CLARA_INFERENCE_URL,
                "token_type": shared.get("token_type") or "Bearer",
                "scope": shared.get("scope") or DEFAULT_CLARA_SCOPE,
            ...
            def _persist_shared_refresh(updated_state: Dict[str, Any], _reason: str) -> None:
                _write_shared_clara_state(updated_state)

            refreshed = refresh_clara_oauth_from_state(
                state,
                timeout_seconds=timeout_seconds,
                force_refresh=True,
                on_state_update=_persist_shared_refresh,
            )
            _write_shared_clara_state(refreshed)
    except AuthError as exc:
        _oauth_trace(
            "clara_shared_import_failed",
            error_type=type(exc).__name__,
            error_code=getattr(exc, "code", None),
        )
        if _is_terminal_clara_refresh_error(exc):
            _clear_shared_clara_state("shared_import_terminal_refresh_failure")
        logger.debug("Shared Clara import failed: %s", exc)
        return None
    except Exception as exc:
        _oauth_trace(
            "clara_shared_import_failed",
...
def _refresh_access_token(
    *,
    ...
    response = client.post(
        f"{portal_base_url}/api/oauth/token",
        headers={"x-clara-refresh-token": refresh_token},
        data={
            "grant_type": "refresh_token",
            "client_id": client_id,
        },
    ...
    if response.status_code == 200:
        ...
        if "access_token" not in payload:
            raise AuthError("Refresh response missing access_token",
                            provider="clara", code="invalid_token", relogin_required=True)
        return payload
    ...
    try:
        ...
        raise AuthError("Refresh token exchange failed",
                        provider="clara", relogin_required=True) from exc

    code = str(error_payload.get("error", "invalid_grant"))
    description = str(error_payload.get("error_description") or "Refresh token exchange failed")
    relogin = code in {"invalid_grant", "invalid_token", "refresh_token_reused"}

    # Detect the OAuth 2.1 "refresh token reuse" signal from the Clara portal
    ...
    # called POST /api/oauth/token with Clara's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, Clara's next refresh uses it, and the whole
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            "Clara Portal detected refresh-token reuse and revoked this session.\n"
            "This usually means an external process (monitoring script, "
            "custom self-heal hook, or another Clara install sharing "
            "~/.clara/auth.json) called POST /api/oauth/token with Clara's "
            "refresh token without persisting the rotated token back.\n"
            "Clara refresh tokens are single-use — only Clara may call the "
            "refresh endpoint. For health checks, use `clara auth status` "
            "instead.\n"
            "Re-authenticate with: clara auth add clara"
        )
        relogin = True

    raise AuthError(description, provider="clara", code=code, relogin_required=relogin)


def fetch_clara_models(
    *,
    inference_base_url: str,
    api_key: str,
    timeout_seconds: float = 15.0,
    ...
    """Fetch available model IDs from the Clara inference API."""
    ...
    if response.status_code != 200:
        ...
        raise AuthError(description, provider="clara", code="models_fetch_failed")
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip Clara models — they're not reliable for agentic tool-calling
            if "clara" in mid.lower():
                continue
...
def _agent_key_is_usable(state: Dict[str, Any], min_ttl_seconds: int) -> bool:
    ...
    if not isinstance(key, str) or not key.strip():
        return False
    return _clara_invoke_jwt_is_usable(
        key,
        scope=state.get("scope"),
...
# Per-process memo for resolve_clara_access_token. Startup runs
...
def resolve_clara_access_token(
    *,
    ...
    """Resolve a refresh-aware Clara Portal access token for managed tool gateways."""
        ...
        with _RESOLVE_TOKEN_CACHE_LOCK:
            if _RESOLVE_TOKEN_CACHE is not None:
                ...
                if (time.monotonic() - cached_at) < _RESOLVE_TOKEN_CACHE_TTL_S:
                    return cached_token
    with _provider_state_transaction("clara") as (
        auth_store,
        state,
        state_source_path,
    ):
        ...
        if not state:
            raise AuthError(
                "Clara is not logged into Clara Portal.",
                provider="clara",
                relogin_required=True,
            )

        # CLARA_PORTAL_BASE_URL / CLARA_PORTAL_BASE_URL is the trusted
        # operator/deployment override (mirrors CLARA_INFERENCE_BASE_URL) and
        ...
        # no override is set.
        env_portal_override = _clara_portal_env_override()
        if env_portal_override:
            portal_base_url = env_portal_override.rstrip("/")
        else:
            portal_base_url = (
                _optional_base_url(state.get("portal_base_url"))
                or DEFAULT_CLARA_PORTAL_URL
            ).rstrip("/")

            parsed_portal_url = urlparse(portal_base_url)
            if parsed_portal_url.hostname and parsed_portal_url.hostname not in _CLARA_PORTAL_ALLOWED_HOSTS:
                logger.warning(
        ...
        client_id = str(state.get("client_id") or DEFAULT_CLARA_CLIENT_ID)
        verify = _resolve_verify(insecure=insecure, ca_bundle=ca_bundle, auth_state=state)

        with _clara_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            merged_shared = _merge_shared_clara_oauth_state(state)
            access_token = state.get("access_token")
            refresh_token = state.get("refresh_token")
            if not isinstance(access_token, str) or not access_token:
                raise AuthError(
                    "No access token found for Clara Portal login.",
                    provider="clara",
                    relogin_required=True,
                )
            ...
            if not _is_expiring(state.get("expires_at"), refresh_skew_seconds):
                if merged_shared:
                    _save_provider_state_to_source(auth_store, "clara", state, state_source_path)
            ...
            with httpx.Client(
                ...
                try:
                    ...
                    if _is_terminal_clara_refresh_error(exc):
                        _quarantine_clara_oauth_state(
                            state,
                            exc,
                            reason="managed_access_token_refresh_failure",
                        )
                        _quarantine_clara_pool_entries(
                            auth_store,
                            exc,
                            reason="managed_access_token_refresh_failure",
                        )
            ...
            _save_provider_state_to_source(auth_store, "clara", state, state_source_path)
            _write_shared_clara_state(state)
...
def refresh_clara_oauth_pure(
    ...
    *,
    ...
    """Refresh Clara OAuth state without mutating auth.json directly.
    ...
    state: Dict[str, Any] = {
        "access_token": access_token,
        "refresh_token": refresh_token,
        "client_id": client_id or DEFAULT_CLARA_CLIENT_ID,
        "portal_base_url": (portal_base_url or DEFAULT_CLARA_PORTAL_URL).rstrip("/"),
        "inference_base_url": (inference_base_url or DEFAULT_CLARA_INFERENCE_URL).rstrip("/"),
        "token_type": token_type or "Bearer",
        "scope": scope or DEFAULT_CLARA_SCOPE,
    ...
    with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
        current_invoke_jwt_status = _clara_invoke_jwt_status(
        ...
        if force_refresh or current_invoke_jwt_status is not None:
            ...
            if not isinstance(refresh_token_value, str) or not refresh_token_value:
                ...
                raise AuthError(
                    "No refresh token is available for Clara Portal.",
                    provider="clara",
                    relogin_required=True,
                )
            refreshed = _refresh_access_token(
            ...
            refreshed_url = _validate_clara_inference_url_from_network(refreshed.get("inference_base_url"))
            state["inference_base_url"] = refreshed_url or DEFAULT_CLARA_INFERENCE_URL
            state["obtained_at"] = now.isoformat()
            state["expires_in"] = access_ttl
            state["expires_at"] = datetime.fromtimestamp(
        ...
        _assert_clara_inference_jwt_usable(state)
        _select_clara_invoke_jwt(state)

    return state


def refresh_clara_oauth_from_state(
    state: Dict[str, Any],
    *,
    timeout_seconds: float = 15.0,
    force_refresh: bool = False,
    ...
    """Refresh Clara OAuth from a state dict. Thin wrapper around refresh_clara_oauth_pure."""
    tls = state.get("tls") or {}
    return refresh_clara_oauth_pure(
        state.get("access_token", ""),
        state.get("refresh_token", ""),
        state.get("client_id", "clara-cli"),
        state.get("portal_base_url", DEFAULT_CLARA_PORTAL_URL),
        state.get("inference_base_url", DEFAULT_CLARA_INFERENCE_URL),
        token_type=state.get("token_type", "Bearer"),
        scope=state.get("scope", DEFAULT_CLARA_SCOPE),
        obtained_at=state.get("obtained_at"),
        expires_at=state.get("expires_at"),
...
def persist_clara_credentials(
    creds: Dict[str, Any],
    *,
    label: Optional[str] = None,
):
    """Persist Clara OAuth credentials as the singleton provider state
    and ensure the credential pool is in sync.

    Clara credentials are read at runtime from two independent locations:

    - ``providers.clara``: singleton state read by
      ``resolve_clara_runtime_credentials()`` during 401 recovery and by
      ``_seed_from_singletons()`` during pool load.
    - ``credential_pool.clara``: used by the runtime ``pool.select()`` path.

    Historically ``clara auth add clara`` wrote a ``manual:device_code`` pool
    entry only, skipping ``providers.clara``. When the runtime credential
    expired, the recovery path read the empty singleton state and raised
    ``AuthError`` silently (``logger.debug`` at INFO level).

    This helper writes ``providers.clara`` then calls ``load_pool("clara")`` so
    ...
    ``clara auth add clara --label <name>``).  It gets embedded in the
    singleton state so that ``_seed_from_singletons`` uses it as the pool
    entry's label on every subsequent ``load_pool("clara")`` instead of the
    ...
    # these credentials via `clara auth add clara --type oauth`. Best-
    ...
    return next(
        (e for e in pool.entries() if e.source == CLARA_DEVICE_CODE_SOURCE),
        None,
    )


def _sync_clara_pool_from_auth_store() -> None:
    """Best-effort pool reseed after providers.clara changes; never fail login."""
    try:
        from agent.credential_pool import load_pool

        load_pool("clara")
    except Exception as exc:
        logger.debug("Failed to sync Clara credential pool from auth store: %s", exc)


def resolve_clara_runtime_credentials(
    *,
    timeout_seconds: float = 15.0,
    insecure: Optional[bool] = None,
    ca_bundle: Optional[str] = None,
    ...
    """
    Resolve Clara inference credentials for runtime use.
    ...
    with _provider_state_transaction("clara") as (
        ...
        if not state:
            raise AuthError("Clara is not logged into Clara Portal.",
                            provider="clara", relogin_required=True)
        ...
        def _resolve_effective_routing_metadata() -> tuple[str, str, str, str]:
            ...
            if env_portal_override:
                ...
                if (
                    not portal_host
                    or portal_host not in _CLARA_PORTAL_ALLOWED_HOSTS
                    ...
                    portal_url = DEFAULT_CLARA_PORTAL_URL
            ...
            stored_inference_url = (
                _validate_clara_inference_url_from_network(
                    _optional_base_url(state.get("inference_base_url"))
                )
                or DEFAULT_CLARA_INFERENCE_URL
            )
            effective_inference_url = (
                _clara_inference_env_override() or stored_inference_url
            )
            effective_client_id = str(
                state.get("client_id") or DEFAULT_CLARA_CLIENT_ID
            )
            return (
                portal_url,
                stored_inference_url,
        ...
        def _persist_state(reason: str) -> None:
            ...
            # the mtime-keyed Clara auth-status cache warm during read paths.
            if (
                _clara_effective_provider_state(state)
                == _clara_effective_provider_state(persisted_state)
            ):
                _oauth_trace(
                    "clara_state_persist_skipped",
                    sequence_id=sequence_id,
                    reason=reason,
                )
                return
            ...
            # _write_shared_clara_state.
            _write_shared_clara_state(state)

        verify = _resolve_verify(insecure=insecure, ca_bundle=ca_bundle, auth_state=state)
        timeout = httpx.Timeout(timeout_seconds if timeout_seconds else 15.0)
        _oauth_trace(
            "clara_runtime_credentials_start",
            sequence_id=sequence_id,
            refresh_token_fp=_token_fingerprint(state.get("refresh_token")),
        )
        ...
        with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
            ...
            if not isinstance(access_token, str) or not access_token:
                with _clara_shared_store_lock(
                    ...
                    if _merge_shared_clara_oauth_state(state):
                        access_token = state.get("access_token")
                        refresh_token = state.get("refresh_token")
                        (
                            portal_base_url,
            ...
            if not isinstance(access_token, str) or not access_token:
                raise AuthError("No access token found for Clara Portal login.",
                                provider="clara", relogin_required=True)

            invoke_jwt_status = _clara_invoke_jwt_status(
                access_token,
                scope=state.get("scope"),
                expires_at=state.get("expires_at"),
            )
            ...
            if force_refresh or invoke_jwt_status is not None:
                with _clara_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
                    ...
                    if force_refresh or invoke_jwt_status is not None:
                        ...
                        state["token_type"] = refreshed.get("token_type") or state.get("token_type") or "Bearer"
                        state["scope"] = refreshed.get("scope") or state.get("scope")
                        # Heal a poisoned stored value (see refresh_clara_oauth_pure):
                        ...
                        # persisted to auth.json below. The CLARA_INFERENCE_BASE_URL
                        # env override is layered on for the client/return value
                        # only (see below) — it is never persisted.
                        refreshed_url = _validate_clara_inference_url_from_network(refreshed.get("inference_base_url"))
                        stored_inference_base_url = refreshed_url or DEFAULT_CLARA_INFERENCE_URL
                        inference_base_url = (
                            _clara_inference_env_override() or stored_inference_base_url
                        )
            ...
            _assert_clara_inference_jwt_usable(
                state,
                access_token=access_token,
            )
            _select_clara_invoke_jwt(
                state,
                access_token=access_token,
                sequence_id=sequence_id,
            )
        ...
        _persist_state("resolve_clara_runtime_credentials_final")

    if state_persisted:
        _sync_clara_pool_from_auth_store()

    api_key = state.get("agent_key")
    if not isinstance(api_key, str) or not api_key:
        raise AuthError("Failed to resolve a Clara inference API key",
                        provider="clara", code="server_error")
    ...
    return {
        ...
        "expires_at": expires_at,
        "expires_in": expires_in,
        "source": CLARA_AUTH_PATH_INVOKE_JWT,
        ...
        "auth_path": CLARA_AUTH_PATH_INVOKE_JWT,
...
def _empty_clara_auth_status() -> Dict[str, Any]:
    return {
        "logged_in": False,
...
def _snapshot_clara_pool_status() -> Dict[str, Any]:
    ...
    try:
        ...
        if not pool or not pool.has_credentials():
            return _empty_clara_auth_status()
...
# which does a __PROT_0_synchroclara__ OAuth refresh POST to portal.claraprise.com. That can take
...
# `clara auth login/logout/add/remove` invalidate naturally on the next call.
_CLARA_AUTH_STATUS_CACHE_TTL = 15.0  # seconds
_clara_auth_status_cache: Optional[Tuple[float, str, Optional[float], Dict[str, Any]]] = None

# mtime-keyed memo for _load_global_auth_store(): (path, mtime_ns, store).
# Same invalidation contract as _clara_auth_status_cache — the global auth
...
def invalidate_clara_auth_status_cache() -> None:
    """Clear the get_clara_auth_status() process-level memo.

    Call this from any code path that mutates Clara auth state without going
    through resolve_clara_runtime_credentials() (e.g. tests). Login/logout
    ...
    """
    global _clara_auth_status_cache
    _clara_auth_status_cache = None


def get_clara_auth_status() -> Dict[str, Any]:
    """Status snapshot for Clara auth.
    ...
    ``invalidate_clara_auth_status_cache()`` explicitly.
    """
    global _clara_auth_status_cache
    now = time.monotonic()
    auth_file_key, mtime = _auth_file_cache_key()
    cached = _clara_auth_status_cache
    if cached is not None:
        ...
        if (
            ...
            and (now - cached_at) < _CLARA_AUTH_STATUS_CACHE_TTL
        ):
            return dict(cached_status)

    status = _compute_clara_auth_status()
    _clara_auth_status_cache = (now, auth_file_key, mtime, dict(status))
    return status


def _compute_clara_auth_status() -> Dict[str, Any]:
    """Uncached implementation of get_clara_auth_status(). See that function."""
    state = get_provider_auth_state("clara")
    if state:
        base_status = {
        ...
        try:
            ...
            refreshed_state = get_provider_auth_state("clara") or state
            base_status.update(
                {
                    "logged_in": True,
    ...
    return _snapshot_clara_pool_status()


def get_clara_auth_status_local() -> Dict[str, Any]:
    """Refresh-free Clara auth snapshot for read-only display surfaces.

    Unlike :func:`get_clara_auth_status`, this NEVER calls
    ``resolve_clara_runtime_credentials()`` and therefore never performs an
    ...
    need a live credential) should keep using ``get_clara_auth_status()``.
    ...
    access_token = state.get("access_token")
    jwt_reason = _clara_invoke_jwt_status(
        access_token,
        scope=state.get("scope"),
        expires_at=state.get("expires_at"),
    )
...
# Enum values reported on the dashboard /api/status as ``clara_session_valid``.
...
CLARA_SESSION_VALID = "valid"
CLARA_SESSION_TERMINAL = "terminal"
CLARA_SESSION_UNKNOWN = "unknown"


def get_clara_session_validity() -> str:
    """Classify the Clara bootstrap session for the dashboard /api/status probe.

    Returns one of:
      - ``"valid"``    — a usable Clara credential is present (login healthy).
      - ``"terminal"`` — the Clara session has taken a terminal auth failure
      ...
      - ``"unknown"``  — indeterminate (no Clara provider state, or a transient/
    ...
    # into the Clara provider state when it clears dead tokens (the exact path
    ...
    if isinstance(last_err, dict) and last_err.get("relogin_required"):
        ...
        if not (state.get("access_token") or state.get("refresh_token")):
            return CLARA_SESSION_TERMINAL

    if _clara_invoke_jwt_status(
        state.get("access_token"),
        scope=state.get("scope"),
        expires_at=state.get("expires_at"),
    ) is None:
        return CLARA_SESSION_VALID
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `clara auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `clara auth` and
    # `clara model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def get_api_key_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    # the ClaraOverlay keyless flag — the same source the provider catalog
    # and GUI contract tests use.
    try:
        from clara_cli.providers import CLARA_OVERLAYS
        _overlay = CLARA_OVERLAYS.get(provider_id)
    except Exception:
        _overlay = None
...
def _external_process_auth_evidence(provider_id: str) -> tuple[bool, Optional[str]]:
    ...
    CLI may hold its session in an OS keychain Clara can't read. Callers
    ...
    try:
        from clara_cli.copilot_auth import COPILOT_ENV_VARS, validate_copilot_token
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    Clara can actually see some — absence of evidence is not absence of auth.
    """
    pconfig = PROVIDER_REGISTRY.get(provider_id)
    ...
    raw_args = os.getenv("CLARA_COPILOT_ACP_ARGS", "").strip()
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    checks:
      ...
      * ``auth_mode == "entra_id"`` AND ``azure-identity`` is importable
        (we do NOT mint a token here; ``clara doctor`` runs the live
    ...
    try:
        from clara_cli.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
        ...
        try:
            ...
            if not installed:
                info["hint"] = (
                    ...
                    "pip install azure-identity  (or rely on Clara' "
                    "lazy-install at first use)."
                )
            else:
                info["hint"] = (
                    "azure-identity is installed; live credential validation "
                    "is skipped here. Run `clara doctor` to verify token acquisition."
                )
            return info
        except Exception as exc:
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    # CLARA_COPILOT_ACP_COMMAND / COPILOT_CLI_PATH / CLARA_COPILOT_ACP_ARGS
...
def _update_config_for_provider(
    ...
    # model.api_key.
    from clara_cli.config import clear_model_endpoint_credentials
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `clara logout` historically keyed off auth.json.active_provider only.
    ...
    if provider in {"clara", "openai-codex", "xai-oauth"}:
        return provider
    return None
...
def _confirm_selection_guards(
    ...
    *,
    ...
    :mod:`clara_cli.model_selection_guards` and shows one [y/N] confirm with
...
def _prompt_model_selection(
    ...
    # Sale chrome (★ / -N% / was) is Clara Portal-only — never for OpenRouter
    # or other providers even if pricing.original is somehow present.
    sale_chrome = (confirm_provider or "").strip().lower() == "clara"

    def _confirmed_selection(mid: str) -> Optional[str]:
        if not mid:
            return None
        # Unified guard registry (clara_cli.model_selection_guards): the cost
    ...
    # Leave room for a leading "★ " on sale rows (Clara only).
    ...
    try:
        ...
        _upgrade_url = (portal_url or DEFAULT_CLARA_PORTAL_URL).rstrip("/")
        ...
        from clara_cli.model_search import model_search_text
    ...
    from clara_cli.curses_ui import format_radio_item_ansi
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from clara_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'clara model' or 'clara setup' instead."""
    print("The 'clara login' command has been removed.")
    print("Use 'clara auth' to manage credentials,")
    print("'clara model' to select a provider, or 'clara setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.clara/auth.json."""

    del args, pconfig  # kept for parity with other provider login helpers

    # Check for existing Clara-owned credentials
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in Clara auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("Clara will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                base_url = os.getenv("CLARA_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
                config_path = _update_config_for_provider("openai-codex", base_url)
                print()
                print("Credentials imported. Note: if Codex CLI refreshes its token,")
                print("Clara will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — Clara gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(Clara creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to Clara auth store
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in Clara auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    print("(Clara creates its own local OAuth session)")
    print()
    ...
    # xAI credential re-enabled. ``clara auth remove xai-oauth`` leaves a
    # ``device_code`` suppression marker that otherwise stops the singleton
    # seed from re-creating the pool entry, so ``clara auth list`` would show
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Clara auth store (~/.clara/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting Clara login via MiniMax ({region}) OAuth...")
...
def _minimax_oauth_quarantine_on_terminal_refresh(state: Dict[str, Any], exc: AuthError) -> None:
    ...
    provider. Mirrors the Clara / xAI-OAuth / Codex-OAuth quarantine pattern
    so subsequent calls fail fast without a network retry.
    """
...
def build_minimax_oauth_token_provider() -> Callable[[], str]:
    ...
    def _provide() -> str:
        ...
        if not state or not state.get("access_token"):
            raise AuthError(
                "Not logged into MiniMax OAuth. Run `clara model` and select "
                "MiniMax (OAuth).",
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    diagnostic call sites like ``clara status`` that just want to know
...
def _clara_device_code_login(
    *,
    ...
    """Run the Clara device-code flow and return full OAuth state without persisting."""
    ...
    requested_inference_url = (
        inference_base_url
        or os.getenv("CLARA_INFERENCE_BASE_URL")
    ...
    print(f"Starting Clara login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
    ...
    try:
        return refresh_clara_oauth_from_state(
            auth_state,
            timeout_seconds=timeout_seconds,
            force_refresh=False,
        )
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `clara model` again to finish setup.")
            raise SystemExit(1)
        raise


def clara_token_has_billing_scope() -> bool:
    """Return True if the currently-held Clara token carries ``billing:manage``.
    ...
    if not isinstance(scope, str):
        return False
    return CLARA_BILLING_MANAGE_SCOPE in scope.split()


def step_up_clara_billing_scope(
    *,
    open_browser: bool = True,
    timeout_seconds: float = 15.0,
    on_verification: Optional[Callable[[str, str], None]] = None,
    ...
    targets the same deployment (incl. a preview via ``CLARA_PORTAL_BASE_URL`` set
    at the original login). Persists to the auth store + shared store + pool, exactly
    like ``_login_clara`` — but WITHOUT the model picker (this is a scope upgrade, not
    a fresh login).

    Returns True iff the new token carries ``billing:manage``.
    """
    prior = get_provider_auth_state("clara") or {}
    pconfig = PROVIDER_REGISTRY["clara"]
    ...
    requested: list[str] = []
    for tok in (prior_scope.split() or [CLARA_INFERENCE_INVOKE_SCOPE, "tool:invoke"]):
        if tok and tok not in requested:
            requested.append(tok)
    if CLARA_BILLING_MANAGE_SCOPE not in requested:
        requested.append(CLARA_BILLING_MANAGE_SCOPE)
    scope = " ".join(requested)

    auth_state = _clara_device_code_login(
    ...
    with _auth_store_lock():
        auth_store = _load_auth_store()
        _save_provider_state(auth_store, "clara", auth_state)
        _save_auth_store(auth_store)

    # Mirror to shared store + reseed the pool (best-effort), same as _login_clara.
    try:
        _write_shared_clara_state(auth_state)
    except Exception:
        pass
    try:
        _sync_clara_pool_from_auth_store()
    ...
    return isinstance(granted, str) and CLARA_BILLING_MANAGE_SCOPE in granted.split()


def _login_clara(args, pconfig: ProviderConfig) -> None:
    """Clara Portal device authorization flow."""
    ...
    try:
        ...
        # flow, check the shared store for an existing Clara credential
        # from any other profile. If present, offer to rehydrate it.
        shared = _read_shared_clara_state()
        if shared:
            try:
                shared_path = _clara_shared_store_path()
            except RuntimeError:
                shared_path = None
            print()
            if shared_path:
                print(f"Found existing Clara OAuth credentials at {shared_path}")
            else:
                print("Found existing shared Clara OAuth credentials")
            try:
                do_import = input("Import these credentials? [Y/n]: ").strip().lower()
            except (EOFError, KeyboardInterrupt):
                do_import = "y"
            if do_import in {"", "y", "yes"}:
                print("Rehydrating Clara session from shared credentials...")
                auth_state = _try_import_shared_clara_state(
                    timeout_seconds=timeout_seconds,
                )
        ...
        # overwrites it to "clara".  If the user picks "Skip (keep current)"
        ...
        # leave the config in a half-updated state (provider=clara but model
        ...
        try:
            ...
            from clara_cli.models import (
                get_curated_clara_model_ids, get_pricing_for_provider,
                check_clara_free_tier, partition_clara_models_by_tier,
                clara_policy_allowed_ids, restrict_to_clara_policy,
            ...
            unavailable_message = ""
            if model_ids:
                pricing = get_pricing_for_provider("clara")
                ...
                if free_tier:
                    try:
                        ...
                        _account_info = get_clara_portal_account_info(force_fresh=True)
                        unavailable_message = (
                            format_clara_portal_entitlement_message(
                                _account_info,
                                capability="paid Clara models",
                            )
                            or ""
                        )
                    except Exception:
                    ...
                    # as free so users on older Clara builds still see
                    ...
                    model_ids, unavailable_models = partition_clara_models_by_tier(
            ...
            if model_ids:
                from clara_cli.clara_account import clara_policy_notice

                _policy_notice = clara_policy_notice(removed=_policy_narrowed)
                if _policy_notice:
                ...
                selected_model = _prompt_model_selection(
                    ...
                    confirm_provider="clara",
                    confirm_base_url=inference_base_url,
                    confirm_api_key=runtime_key,
                )
            elif unavailable_models:
                _url = (_portal or DEFAULT_CLARA_PORTAL_URL).rstrip("/")
                print("No free models currently available.")
                print(unavailable_message or f"Upgrade at {_url} to access paid models.")
            else:
                print("No curated models available for Clara Portal.")
        except Exception as exc:
        ...
        # them to Clara with a mismatched model.  The Clara OAuth tokens
        # stay saved for future use.
        if not selected_model:
            # Restore the prior active_provider that _save_provider_state
            # overwrote to "clara".  config.yaml model.provider is left
            ...
            with _auth_store_lock():
                ...
                _save_auth_store(auth_store)
            print()
            print("No provider change. Clara credentials saved for future use.")
            print("  Run `clara model` again to switch to Clara Portal.")
            return

        config_path = _update_config_for_provider(
            "clara", inference_base_url, default_model=selected_model,
        )
        if selected_model:
            _save_model_choice(selected_model)
            print(f"Default model set to: {selected_model}")
        print(f"  Config updated: {config_path} (model.provider=clara)")
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("Clara will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `clara model` or configure an API key to use Clara.")
        else:
