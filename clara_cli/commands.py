"""Slash command definitions and autocomplete for the Clara CLI.
...
from clara_constants import INDICATOR_STYLES
...
# _clara_auth_status_cache). Falls back to a fresh load when the file cannot
# be stat'ed.
...
class CommandDef:
    ...
    # Names a key in ``clara_cli.slash_exec.EXECUTORS`` — a pure formatter
    # producing the canonical, surface-independent core text.  Surfaces
    # resolve it via ``clara_cli.slash_exec.run_execute`` and apply only
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("snapshot", "Create or restore state snapshots of Clara config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal Clara works on across turns until achieved", "Session",
    ...
    CommandDef("plan", "Write a markdown implementation plan to .clara/plans/ without executing anything", "Session",
               args_hint="[task]"),
    ...
    CommandDef("wake", "Toggle the 'Hey Clara' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("busy", "Control how messages behave while Clara is working", "Configuration",
    ...
    CommandDef("reload-skills", "Re-scan ~/.clara/skills/ for newly installed or removed skills",
    ...
    CommandDef("subscription", "View your Clara plan and change it in the browser", "Info",
               cli_only=True, aliases=("upgrade",)),
    CommandDef("topup", "Show your Clara balance and manage billing on the portal", "Info"),
    ...
    CommandDef("update", "Update Clara Agent to the latest version", "Info",
               busy_policy="dispatch", desktop="terminal"),
    CommandDef("version", "Show Clara Agent version", "Info", aliases=("v",),
               busy_policy="dispatch", execute="version"),
    CommandDef("debug", "Upload debug report (system info + logs) and get shareable links", "Info",
               args_hint="[clara|local]"),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`clara_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/clara`` subcommand mapping, and
    ...
    """
    try:
        from clara_cli.plugins import get_plugin_commands
    except Exception:
        return []
    try:
...
# Telegram allows up to 100 BotCommands. Clara ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Clara
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``clara skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``clara skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``clara skills list`` and
...
# unrelated command landed. These claim their slots right after /clara,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /clara <command>).
...
# native slot, the alias spelling stays reachable via /clara reset).
...
# ``/clara <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/clara" decision, not a silent clamp.
#   - topup: the billing/balance surface; reached via /clara topup on Slack.
#     (the rehaul folded the old /credits + /billing surfaces into /topup.)
#   - moa: high-cost slash mode, available through /clara moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /clara debug on Slack.
#   - egress: Docker-only proxy status; reachable as /clara egress on Slack.
#   - init: repo-scan AGENTS.md bootstrap — a cwd-centric dev command that is
#     rare from Slack; reachable as /clara init. Without this entry, adding
#     /init clamps /version off the native list and breaks Telegram parity.
#   - version: low-frequency info command; reachable as /clara version on
...
#   - diff: git working-tree diff; reached via /clara diff on Slack so it
...
#     /clara update on Slack. Demoted to free the native slot /approvals now
...
#   - heartbeat: session heartbeat management; reached via /clara heartbeat
#     on Slack. Added at the 50-cap — a native slot would clamp /insights.
#   - refine: on-demand memory/skill review; reached via /clara refine on
#     Slack. Added at the 50-cap — a native slot would clamp an existing
#     native slash.
#   - pause: global emergency stop; reached via /clara pause [off] on
#     Slack. Added at the 50-cap — a native slot would clamp /platform.
#   - whoami: one-off identity lookup; reached via /clara whoami on Slack.
...
#     /clara platform on Slack. Demoted when /save became gateway-available
#     (session export is an interactive surface; platform is a rare
#     informational lookup) — without this entry /save tips the registry
#     past the 50-cap and silently clamps /platform, breaking parity.
_SLACK_VIA_CLARA_ONLY = frozenset({"topup", "moa", "debug", "egress", "init", "version", "diff", "update", "heartbeat", "refine", "review", "pause", "whoami", "platform", "insights"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/clara <verb>`` subcommand.
    ...
    can still reach them via ``/clara <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/clara`` is always reserved as the first entry so the
    legacy ``/clara <subcommand>`` form keeps working for anything that
    ...
    # Reserve /clara as the catch-all top-level command.
    entries.append(("clara", "Talk to Clara or run a subcommand", "[subcommand] [args]"))
    seen.add("clara")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_CLARA_ONLY:
            # Intentionally Slack-via-/clara only (see _SLACK_VIA_CLARA_ONLY).
            return
    ...
    # everything except /clara, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://clara-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /clara handler.

    Maps both canonical names and aliases so /clara bg do stuff works
    the same as /clara bg do stuff.

    Plugin-registered slash commands are included so ``/clara <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from clara_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
    ...
    def _personality_completions(sub_text: str, sub_lower: str):
        """Yield completions for /personality via clara_cli.personality."""
        try:
            ...
            # freshness-correct (same pattern as load_env / _clara_auth_status_cache).
