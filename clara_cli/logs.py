"""``clara logs`` — view and filter Clara log files.
...
Usage examples::
    ...
    clara logs                    # last 50 lines of agent.log
    clara logs -f                 # follow agent.log in real time
    clara logs errors             # last 50 lines of errors.log
    clara logs gateway -n 100    # last 100 lines of gateway.log
    clara logs gui -f            # follow gui.log (dashboard/pty/ws)
    clara logs desktop -f        # follow desktop.log (Electron app boot/backend)
    clara logs --level WARNING    # only WARNING+ lines
    clara logs --session abc123   # filter by session ID substring
    clara logs --component tools  # only tool-related lines
    clara logs --since 1h         # lines from the last hour
    clara logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_clara_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Clara runs — try 'clara chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_clara_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_clara_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_clara_home()}/logs/")
        return

    print(f"Log files in {display_clara_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'clara chat' to generate logs)")
