"""Offline, non-destructive recovery for a damaged Clara session database.
...
def _copy_source_bundle(source: Path, snapshot_dir: Path) -> tuple[Path, list[str]]:
    ...
    locks -- the failure class ``clara_cli.sqlite_safe_read`` exists to
    ...
    keeps this path consistent with ``clara_state._backup_db_file``.
    """
    ...
    try:
        ...
        if before != after:
            raise SessionRecoverySafetyError(
                ...
                "Stop every Clara process using this profile and retry. "
                "This includes the interactive `clara` CLI session this "
                ...
                "Clara process even after the gateway is stopped. Run the "
                "recovery from a fresh shell with no `clara` session open, "
...
def _recover_via_lost_and_found(
    *,
    ...
    from clara_cli.session_lost_and_found import (
