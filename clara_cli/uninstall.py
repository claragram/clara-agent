"""
Clara Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.clara/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Clara PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing clara-agent or clara PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Clara Agent" comment and following line
                if '# Clara Agent' in line or '# clara-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('clara' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing clara
                if 'clara' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
            ...
            if new_content != original_content:
                ...
                # This is the user's own shell rc, not a Clara-owned file, and
...
def remove_wrapper_script():
    """Remove the clara wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "clara",
        Path.home() / ".local" / "bin" / "clara-acp",
        Path.home() / ".local" / "bin" / "clara-agent",
        Path("/usr/local/bin/clara"),
        Path("/usr/local/bin/clara-acp"),
        Path("/usr/local/bin/clara-agent"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains clara_cli reference)
                content = wrapper.read_text(encoding="utf-8")
                if 'clara_cli' in content or 'clara-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(clara_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``clara`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Clara home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (clara_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``clara gateway run`` processes
    ...
    try:
        from clara_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from clara_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``CLARA_HOME`` and ``CLARA_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\clara\git\cmd``, ``%LOCALAPPDATA%\clara\git\bin``,
#      ``%LOCALAPPDATA%\clara\git\usr\bin``, ``%LOCALAPPDATA%\clara\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\clara\git\`` and Node to
#      ``%LOCALAPPDATA%\clara\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``clara dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\clara\gateway-service\`` and sometimes
...
def _clara_path_markers(clara_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Path-entry substrings that identify Clara-owned User-PATH entries.

    ``include_managed_bin`` adds the managed binary dir (``<root>\\bin``,
    holding the clara launchers and the managed uv) — only wanted when
    ...
    """
    root = str(clara_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare clara-agent install dir.
    markers = [root + "\\clara-agent", root + "\\git", root + "\\node", root + "\\venv"]
    if include_managed_bin:
        markers.append(root + "\\bin")
    # Also match if CLARA_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "clara".  We don't want to catch
    # unrelated entries like "cclara-foo" or "ephermeral", so we look for
    # backslash-clara as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(clara_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Strip Clara-owned entries from User-scope PATH in the registry.
    ...
    ``include_managed_bin`` adds ``<clara_home>\\bin`` (the managed binary
    dir holding the clara launchers and the managed uv) to the sweep. Only
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _clara_path_markers(clara_home, include_managed_bin=include_managed_bin)
            kept: list[str] = []
...
def remove_clara_env_vars_windows() -> list[str]:
    """Delete CLARA_HOME and CLARA_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("CLARA_HOME", "CLARA_GIT_BASH_PATH"):
                try:
...
def remove_windows_bin_launchers(*, windows: bool | None = None) -> list[Path]:
    """Delete the ``clara`` launchers install.ps1 staged in the managed
    binary dir (the default Clara root's ``bin``, next to the managed uv).

    Every uninstall mode deletes the code checkout, so the launchers —
    which invoke ``<checkout>\\venv\\Scripts`` — would otherwise dangle:
    ``clara`` in a new terminal resolves to a launcher whose target is
    ...
    ``_install_repair._quarantine_running_clara_exe`` relies on), so
    ...
    *windows* is an injectable platform verdict for tests (same pattern as
    ...
    try:
        ...
        bin_dir = get_default_clara_root() / "bin"
    except Exception as e:
...
def _is_default_clara_home(clara_home: Path) -> bool:
    """Return True when ``clara_home`` points at the default (non-profile) root."""
    try:
        from clara_constants import get_default_clara_root
        return clara_home.resolve() == get_default_clara_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its CLARA_HOME directory.

    We shell out to ``clara -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current CLARA_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m clara_cli.main` so we don't depend on a `clara`
    #    wrapper that may be half-removed mid-uninstall.
    clara_invocation = [_sys.executable, "-m", "clara_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                clara_invocation + ["gateway", subcmd],
                capture_output=True,
    ...
    # 3. Wipe the profile's CLARA_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``clara uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$CLARA_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from clara_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(clara_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Clara Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Clara Chat GUI installation was found.")
        print(f"  Checked: {clara_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Clara agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(clara_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Clara agent at {clara_home / 'clara-agent'}")
        print(f"  • Your config, sessions, and secrets under {clara_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Clara agent is still installed. Run 'clara' to use the CLI,")
    print("or 'clara uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.clara/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.clara/ for future reinstall
    """
    ...
    # offer to clean them up too instead of leaving zombie CLARA_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_clara_home(clara_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.clara data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Clara Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {clara_home / 'config.yaml'}")
    print(f"  Secrets: {clara_home / '.env'}")
    print(f"  Data:    {clara_home / 'cron/'}, {clara_home / 'sessions/'}, {clara_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their CLARA_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Clara data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Clara code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, clara_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Clara PATH entries from shell configs / Windows User PATH")
    print("  • Clara wrapper scripts and Clara-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • Clara config/data: {clara_home}")
        if _is_default_clara_home(clara_home):
        ...
        print(f"  • Keep Clara config/data: {clara_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``clara`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$CLARA_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in clara_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\clara\git\cmd, not %LOCALAPPDATA%.
        # The managed binary dir (clara\bin: launchers + managed uv) leaves
        ...
        sweep_managed_bin = full_uninstall and _is_default_clara_home(clara_home)
        removed_path_entries = remove_path_from_windows_registry(
            Path(os.path.expandvars(str(clara_home))),
        ...
        if removed_path_entries:
            ...
            log_info("No Clara-owned PATH entries in User environment")

        log_info("Removing CLARA_HOME / CLARA_GIT_BASH_PATH User env vars...")
        removed_env = remove_clara_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Clara-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing clara command...")
    ...
    #     would dangle — `clara` in a new terminal would resolve and then
    #     error on its missing venv target, worse than command-not-found.
    if _is_windows():
        log_info("Removing Windows clara launchers...")
        ...
        if removed_launchers:
            ...
            log_info("No Windows clara launchers found")

    # 3b. Remove node/npm/npx symlinks the installer left in ~/.local/bin
    #     (only when they still point into this Clara home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Clara-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(clara_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Clara-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(clara_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE CLARA_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from clara_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(clara_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.clara/, just remove the clara-agent subdir
            if clara_home in project_root.parents or project_root.parent == clara_home:
    ...
    #     under CLARA_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(clara_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(clara_home)
    ...
    # 5. Optionally remove ~/.clara/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile CLARA_HOME dirs live under
        ...
        try:
            if clara_home.exists():
                shutil.rmtree(clara_home)
                log_success(f"Removed {clara_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {clara_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {clara_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://agent.claraprise.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://agent.claraprise.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Clara Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m clara_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import clara_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``clara_constants`` + ``clara_cli.colors``
    (and lazily ``clara_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m clara_cli.uninstall")
    parser.add_argument(
        "--mode",
