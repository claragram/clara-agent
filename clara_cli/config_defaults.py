"""Default configuration data for Clara Agent.

Pure-data leaf module: DEFAULT_CONFIG and OPTIONAL_ENV_VARS, extracted
verbatim from clara_cli/config.py. Must not import from clara_cli.config.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["clara-cli"],
    # SQLite journal mode used by every Clara database opener. WAL is the
    ...
    # Soft file-descriptor limit for long-running Clara server processes.
    ...
    "session": {
        # Per-terminal `clara -c`: each CLI session drops a breadcrumb file
        # under $CLARA_HOME/terminal-sessions/<terminal-id>, and a bare
    ...
    "agent": {
        ...
        # all normalized by clara_cli.config.resolve_turn_limit.
        "max_turns": None,
        ...
        # budget. CLI one-shot equivalent: `clara chat --run-budget N`.
        ...
        # an interactive `clara gateway restart` must never block for hours
        ...
        # the Clara-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Clara
        ...
        # identity slot (SOUL.md). Empty by default. The CLARA_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Clara adds a coding operating brief
        ...
        # blocks indefinitely (input() is __PROT_0_synchroclara__) and ignores this.
        ...
        # `clara doctor`. Delivered via the status channel only —
        ...
        # `clara status` / fleet monitoring). Retries never stop — this is a
        ...
        # __PROT_2_synchroclaraly__ before the gate runs.  Set to 0 to disable the bound
        ...
        # ``CLARA_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        # Root directory for Clara' terminal session temp files (background
        # logs/pid/exit files, code-execution sandboxes, etc.). When empty,
        # Clara uses TMPDIR/TMP/TEMP if set, otherwise a managed dir on real
        # storage at CLARA_HOME/cache/terminal (auto-pruned after 72h) — NOT
        ...
        # bot_relay from a short-lived `clara -p <bot> chat -Q` recipient
        ...
        #             CLARA_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force CLARA_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Clara auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Clara sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Clara
        ...
        # bundled Clara image, which drops to the `clara` user via
    ...
    "web": {
        ...
        # `clara tools`, not a ring member). Set by the `clara tools`
    ...
    "browser": {
        ...
        #   Browser Use mode (default) — "lightpanda" makes Clara spawn
        ...
        # `clara doctor` say so. Also settable via AGENT_BROWSER_ENGINE.
        "engine": "auto",
        ...
        # browser tools) runs on a Clara-managed SNAPSHOT of the user's
        ...
        # fresh session launches — driven by Clara' packaged Chromium. Only
        ...
        # deletes the snapshot store (~/.clara/browser-profile/) so copied
        ...
        # approval does it run `clara browser close-profile` (terminates the
        ...
        "camofox": {
            # When true, Clara sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Clara should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``clara chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.clara/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: clara sweeps the checkpoint base at startup
        ...
        # ``clara checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # AGENTS.md, CLAUDE.md, .clara.md, or .cursorrules before Clara applies
    ...
    # Single-query (``clara -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    # single read_file page exceeds these limits, Clara truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Clara' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); clara =
                                      # Clara' compression threshold triggers
        ...
        "codex_responses_native": False,  # Opt in to OpenAI's server-side compaction
                                      ...
                                      # route/model is unaffected. Clara' local
    ...
    "auxiliary": {
        ...
        "vision": {
            "provider": "auto",    # auto | openrouter | clara | codex | custom
        ...
        "review": {
            "provider": "auto",    # auto (= inherit main model) | openrouter | clara | anthropic | ...
        ...
        # Invoked by ``clara kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``clara kanban decompose`` and the kanban
        ...
        # ``clara profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `clara model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `clara` (and `clara chat`) launches by default:
        ...
        # REPL and `--tui` (or CLARA_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `clara --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `clara -c` muscle memory.  Default off so existing
        # users aren't surprised.  CLARA_TUI_RESUME=<id> always wins.
        ...
        # When true (default), `clara --tui` drops a one-time hint
        ...
        # Clara credits status-bar notices (usage bands, grant-spent, depleted /
        ...
        # model — see clara_cli/focus_view.py.
        "focus_view": False,
        ...
        # can set `path`/`paths` to CLARA_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/clara`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `clara pets`. Disabled until
        ...
        "pet": {
            ...
            # get_clara_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # protocol ping (see clara_cli/web_server.py — an event-loop stall
        ...
        # CLARA_TUI_WS_ORPHAN_REAP_GRACE_S env var remains an internal
        ...
        # (CLARA_TUI_SESSION_TTL_S, default 6h) are closed with
        ...
        # ``--insecure`` is not). The bundled Clara Portal plugin reads
        # both keys at startup; they are the canonical surface for these
        # settings. Each can be overridden by an environment variable —
        # ``CLARA_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``CLARA_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # ``portal_url`` defaults to https://portal.claraship.com).
        "oauth": {
        ...
        # (``CLARA_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # clara-account-service at deploy time via the
        # ``CLARA_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``CLARA_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/clara``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Clara appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.clara/cache/piper-voices/
    ...
    # "Hey Clara" hands-free wake word. Always-on, on-device hotword
    ...
    "wake_word": {
        ...
        "phrase": "hey clara",       # for "sherpa" this IS the detected phrase (any text works); for other engines it's a cosmetic label — detection is keyed by the model/keyword below
        ...
        "openwakeword": {
            # "hey_clara" (the bundled, works-out-of-the-box default) OR a
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.clara/plugins/.
    "context": {
        "engine": "compressor",
    ...
    # configured providers (OpenRouter, Clara, Z.ai, Kimi, etc.) are supported.
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.clara/cache/delegation/
        ...
        "max_concurrent_children": 10,  # unified concurrency cap: max parallel children per batch
                                       ...
                                       # fall back to __PROT_1_synchroclara__ execution. Floor of 1, no ceiling.
    ...
    # response. If not, Clara feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Clara auto-pauses the goal and
    ...
    "moa": {
        ...
        # <clara_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # goes to ~/.clara/skills/ unless create_dir (below) redirects it.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Where agent-created skills (skill_manage action=create) are written.
        # Empty = the profile-local skills dir (~/.clara/skills/). When set,
        ...
        # against CLARA_HOME. The directory is scanned for skills alongside
        # the local dir. e.g. "/opt/brain/skills"
        "create_dir": "",
        # Project-local skill discovery: when a session starts inside a git
        # checkout, ``<root>/.clara/skills/`` and ``<root>/.agents/skills/``
        ...
        # with ``clara skills trust`` (run from inside it). Set to false to
        ...
        # Managed by ``clara skills trust`` / ``clara skills untrust``.
        "trusted_project_dirs": [],
        # Substitute ${CLARA_SKILL_DIR} and ${CLARA_SESSION_ID} in SKILL.md
        ...
        # (`clara skills install`). Runs ALONGSIDE the built-in skills
        ...
        # ~/.clara/skills/.curator_ledger.jsonl with before/after file
        # hashes; file contents are stored content-addressed (deduped) under
        # ~/.clara/.curator_backups/blobs/. Enables `clara curator ledger`
        # and single-mutation `clara curator rollback <entry-id>`.
    ...
    # cron daemon.
    #
    # See `clara curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `clara curator run --consolidate` overrides
        ...
        # normally restored on every `clara update`, so pruning them only
        ...
        # skills are kept forever. When > 0, `clara curator purge` deletes
        ...
        # skipped), snapshot ~/.clara/skills/ into
        # ~/.clara/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `clara curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for clara-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Clara Agent*" header.
    ...
    # dangerous command. -q runs export CLARA_INTERACTIVE=1 (for interactive
    ...
    "approvals": {
        ...
        # modal; CLARA_TUI_NO_CONFIRM=1 force-skips that modal regardless of
    ...
    # replace Clara' built-in platform hint for a single messaging platform
    ...
    # ``clara plugins enable|disable`` and intentionally omitted here so an
    ...
    # stored approval from ~/.clara/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or CLARA_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # .cursorrules, project-local .clara config) always require human
        ...
        # longer trigger the startup banner. Add via `clara doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``clara_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Clara to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $CLARA_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # agent's existing Clara Portal token — there is no token key here.
        "chronos": {
        ...
        # Also overridable via CLARA_CRON_MAX_PARALLEL env var.
        ...
        # CLARA_CRON_SCRIPT_TIMEOUT. Keep this in sync with
        ...
        # SessionDB opens/migrates state.db __PROT_3_synchroclaraly__ and has no timeout
        ...
        # CLARA_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
        ...
        # overridable via CLARA_CRON_MEDIA_SEND_TIMEOUT env var. Keep in
    ...
    # todos to ready, and fires `clara -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `clara` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `clara kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     clara-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Clara tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.clara/logs/.
    ...
    # curated model lists for OpenRouter and Clara Portal from this URL,
    # falling back to the in-repo snapshot on network failure.  Lets us
    # update model picker lists without shipping a clara-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://agent.claraship.com/docs/api/model-catalog.json",
    ...
    # #8731).
    #
    # Provider keys accept the Clara provider id (as used elsewhere in
    ...
    "gateway": {
        ...
        # internal CLARA_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # bridges these keys to the internal CLARA_STARTUP_WATCHDOG /
        # CLARA_STARTUP_WATCHDOG_TIMEOUT_S env vars AND applies them to the
        ...
        # producing ~/.clara/sessions/sessions.json entirely.
        ...
        # the CLARA_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.clara.gateway` that defenses 1-2 don't
        ...
        # `clara update` hang because it can never drain the gateway.
        "restart_loop_guard": {
            "max_restarts": 3,
            "window_seconds": 60,
            "max_gap_seconds": 300,
        ...
        # ``CLARA_GATEWAY_MAX_STARTS`` / ``CLARA_GATEWAY_START_WINDOW_S``
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.clara/.env,
        ...
        # behavior: files must live under the Clara cache, under
        ...
        # user. Bridged to CLARA_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Clara
        # cache (~/.clara/cache/{documents,images,audio,video,screenshots})
        ...
        # to CLARA_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # CLARA_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to CLARA_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.clara/state.db.
    ...
    "sessions": {
        ...
        # the default of ``clara sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.clara/sessions/session_{sid}.json`` on every turn
        ...
        # user runs `clara sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `clara update` prints a one-line notice with
        ...
        # ~/.clara/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to CLARA_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # CLARA_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
        ...
        # (non-streaming) export such as `clara sessions export`. Checked
    ...
    # directory. Collection is opt-in (``enabled``). Transmission to the Clara
    ...
    "telemetry": {
        "shared_metrics": {
            "enabled": False,
            # Transmit exported packages to the Clara telemetry service.
            ...
            "endpoint": "https://telemetry.claraship.com/v1/telemetry",
        },
    },

    # ``clara doctor`` behaviour.
    "doctor": {
        # Per-probe timeout (seconds) for the opt-in `clara doctor --live`
        # real-call backend probes (Firecrawl/FAL/browser/MCP/TTS/STT).
        "live_probe_timeout": 10,
    },

    # ``clara update`` behaviour.
    "updates": {
        ...
        #     DBs) into <CLARA_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``clara backup``-style zip
        #     of CLARA_HOME into <CLARA_HOME>/backups/, restorable with
        #     ``clara import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `clara update` does with uncommitted local changes to the
        ...
        # When `clara update` finds the source checkout parked on a feature
        ...
        #                         merge. `clara update --switch-branch`
    ...
    "lsp": {
        ...
        #              ``<CLARA_HOME>/lsp/bin/`` on first use.
    ...
    # `clara tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.clara/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.clara/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.clara/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, Clara may use this encrypted
            ...
            # ~/.clara/bin/ on first use.  When False you must install
        ...
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.clara/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Clara disables it for our users unless they
        # explicitly opt in here. When false (default), Clara sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; Clara also calls
        ...
        "no_overlay": None,
        # cua-driver permission mode for each Clara computer-use runtime.
        ...
        # manifest used when permission_mode is "bounded". Clara passes the
    ...
    #
    # Configure with `clara egress setup`.  Disabled by default — the rest of
    # Clara works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.clara/bin/ on
        ...
        # Together, DeepSeek, Clara).  Wildcards (`*.foo.com`) are supported.
        "extra_allowed_hosts": [],
    },

    # Clara Desktop (Electron app) launch options. These only affect
    # `clara desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the CLARA_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
        ...
        # Ignored on macOS/Windows. Bridged to the CLARA_DESKTOP_PASSWORD_STORE
    ...
    "clara": {
        # Upper bound on the Clara auth keepalive tick, in seconds. The tick
    ...
    # Clara downloads official llama.cpp release binaries, then spawns and
    ...
    "local_runtime": {
        ...
        # (Clara still finds an external llama-server you run yourself).
        "enabled": False,
        # Pinned llama.cpp release tag (rolling bNNNN). Bumped by Clara
...
OPTIONAL_ENV_VARS = {
    ...
    "CLARA_BASE_URL": {
        "description": "Clara Portal base URL override",
        "prompt": "Clara Portal base URL (leave empty for default)",
        "url": None,
        "password": False,
        "category": "provider",
        "advanced": True,
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Clara mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'clara model' for endpoint-specific config)",
    ...
    "FIRECRAWL_GATEWAY_URL": {
        "description": "Exact Firecrawl tool-gateway origin override for Clara Subscribers only (optional)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for Clara Subscribers only, used to derive vendor hosts, e.g. claraship.com -> firecrawl-gateway.claraship.com",
    ...
    "TOOL_GATEWAY_SCHEME": {
        "description": "Shared tool-gateway URL scheme for Clara Subscribers only, used to derive vendor hosts (`https` by default, set `http` for local gateway testing)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Clara Subscriber access token for tool-gateway requests (optional; otherwise read from the Clara auth store)",
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey Clara' wake word engine (optional; openWakeWord is the free default)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Clara, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @clara:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. CLARA_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #clara)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: clara-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'clara-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Clara API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Clara API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Clara API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # CLARA_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to CLARA_TOOL_PROGRESS_MODE for backward
    ...
    # CLARA_TOOL_PROGRESS is fully unsupported since the v12 config support
    # floor retired its only consumer (the v3→4 migration).
    "CLARA_PREFILL_MESSAGES_FILE": {
