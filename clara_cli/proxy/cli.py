"""CLI handlers for the ``clara proxy`` subcommand."""
...
from clara_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "clara proxy requires aiohttp. Run `clara setup` to install it.",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    provider = getattr(args, "provider", None) or "clara"
    try:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"clara auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Clara proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Clara proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: clara proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``clara proxy <subcommand>``."""
    ...
    print(
        "clara proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  clara proxy start [--provider clara|xai] [--host 127.0.0.1] [--port 8645]\n"
