...
# xAI's public API is OpenAI-compatible for the endpoints Clara commonly
# uses. The Responses endpoint is included because Clara' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Clara-managed OAuth credentials."""

    auth_hint = "clara auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`clara auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`clara auth reset xai-oauth` or re-authenticate with "
                    "`clara auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `clara auth add xai-oauth --type oauth`."
            )
