"""Migrate Clara' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Clara' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Clara mcp_servers.<n>.command/args/env  → codex stdio transport
  Clara mcp_servers.<n>.url/headers       → codex streamable_http transport
  Clara mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Clara mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Clara-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Clara' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Clara config.")
...
# Clara keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Clara' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, clara_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Clara MCP server config to the codex inline-table dict
    ...
    if not isinstance(clara_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(clara_cfg.get("command"))
    has_url = bool(clara_cfg.get("url"))
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (CLARA_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Clara)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Clara' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Clara' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Clara
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="clara-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a CLARA_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed clara-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.clara`` symlink that happens to live under ``/private/var/folders``
...
def _build_clara_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Clara' own
    ...
    so a clara installed under /opt/, /usr/local/, or a venv all work.
    CLARA_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # CLARA_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_clara_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever CLARA_HOME its launcher sets
    ...
    # — that would override the launcher's CLARA_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("CLARA_HOME", tmp_path) would
    ...
    clara_home = os.environ.get("CLARA_HOME") or ""
    if clara_home and _looks_like_test_tempdir(clara_home):
        clara_home = ""
    if clara_home:
        env["CLARA_HOME"] = clara_home
    # PYTHONPATH passes through so a worktree-launched clara finds the
    ...
    env["CLARA_QUIET"] = "1"
    env["CLARA_REDACT_SECRETS"] = env.get("CLARA_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    expose_clara_tools: bool = True,
) -> MigrationReport:
    """Translate Clara mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        clara_config: full ~/.clara/config.yaml dict
        ...
        expose_clara_tools: when True (default), register Clara' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Clara for tools
    ...
    clara_servers = (clara_config or {}).get("mcp_servers") or {}
    if not isinstance(clara_servers, dict):
        report.errors.append(
            "mcp_servers in Clara config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in clara_servers.items():
    ...
    # Inject Clara' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Clara for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/clara_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_clara_tools:
        translated["clara-tools"] = _build_clara_tools_mcp_entry()
        if "clara-tools" not in report.migrated:
            report.migrated.append("clara-tools")
