...
def _stat_db_file_identity(path: Path) -> Optional[Tuple[int, int]]:
    ...
    Mirrors the clara_state helper of the same name; kept local so this
    module has no import-time dependency on clara_state (which imports
...
def acquire(db_path: Optional[Path] = None) -> "SessionDB":
    ...
    shared generation was opened — e.g. by ``clara sessions recover`` or
...
# ── Backwards-compatible aliases (clara_state re-exports) ──
# Kept so call sites and tests can import either from clara_state
