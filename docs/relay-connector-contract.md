...
This document is the formal interface between the **Clara gateway** (Python,
`gateway/relay/`) and the **connector** (Node/TypeScript,
`Workprise/gateway-gateway`). The connector implementer's first action is to
read this file.
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `clara gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`clara gateway enroll` → connector `/relay/enroll`, or
...
(`Workprise/gateway-gateway`); this section is the gateway-facing summary.
...
driven by the managed Portal / `clara` CLI.
