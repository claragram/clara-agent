...
(`clara-account-service`) and anyone debugging the managed-cron path.

Chronos lets a hosted Clara gateway **scale to zero** while idle and still
...
ChronosCronScheduler.reconcile()        ── agent computes next_run_at
  │  POST {portal}/api/agent-cron/provision   (auth: agent's Clara access token)
  ▼
...
|---|---|---|---|
| 1 | agent → NAS (`provision`/`cancel`/`list`) | the agent's existing **Clara Portal access token** (Bearer) — for a hosted agent this is the **bootstrap-session token** NAS planted in `auth.json` (client `clara-cli-vps`), NOT an `agent:*` client token | NAS (its normal agent-token path) |
...
> agent uses the **bootstrap-session access token** (`resolve_clara_access_token`),
> minted under the bootstrap-only client `clara-cli-vps` and seeded into the
...
- **Auth:** `Authorization: Bearer <agent Clara access token>`. NAS validates via
...
1. **Dashboard app** (`clara_cli/web_server.py`) — the agent's only public
...
- **Multi-replica agents:** the store CAS makes the fire at-most-once across N
  gateway replicas sharing one `CLARA_HOME` — exactly one replica runs each
  fire.
...
If `callback_url` / `portal_url` is blank or the agent has no Clara login,
