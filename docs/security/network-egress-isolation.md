...
When running Clara inside Docker, the default `network_mode: host` gives the
...
The Clara [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=clara,clara-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://clara-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Clara trust model and vulnerability reporting
