...
has to be thrown away or summarized. Clara has always done this in one batch:
...
Clara folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Clara's normal session flush is
...
With `micro_compact` unset or `false` Clara behaves exactly as it always has:
...
__PROT_0_synchroclara__ `/models` probe and telemetry must never be what blocks a turn.
...
Defaults to `$CLARA_HOME/logs/agent.log`. It reports passes, outcome mix, net
