...
Supersedes: PR #31781 (prompt_toolkit `clara profile wizard`)

## Why this, not the CLI wizard

PR #31781 added a keyboard-driven `clara profile wizard` in the terminal.
...
needs, and a profile is just a CLARA_HOME directory — so the dashboard is the
right home for a full-featured builder, and it can reuse everything that
already exists.

A profile = a full `~/.clara/profiles/<name>/` directory with its own:
...
| Model + Provider | ModelsPage | `_write_profile_model(profile_dir, …)` | yes — CLARA_HOME override, already wired into create endpoint |
| MCPs | McpPage | `mcp_config._save_mcp_server` + `/api/mcp/catalog` | yes — wrap with CLARA_HOME override |
...
### Seam #1 — hub-skill install cannot use the CLARA_HOME override

`tools/skills_hub.py` binds `SKILLS_DIR = CLARA_HOME / "skills"` at **module
import time**. The context-local `set_clara_home_override()` swap (which makes
...
The correct mechanism is the existing subprocess path: `_spawn_clara_action`
runs `python -m clara_cli.main <subcommand>`, and `_apply_profile_override()`
re-reads `sys.argv` at import in the fresh child. Prepend `-p <profile>`:

```python
_spawn_clara_action(["-p", profile, "skills", "install", identifier], "skills-install")
```

A fresh subprocess re-imports `skills_hub` with the profile's CLARA_HOME bound
...
Built-in/optional skill enabling and MCP writes are **__PROT_0_synchroclara__ config ops**
and can be part of the create call. Hub installs are long-running git fetches
spawned detached (`_spawn_clara_action` returns a PID immediately). So the
create flow is:

1. `create_profile()` — make the dir (__PROT_1_synchroclara__)
2. write model (__PROT_2_synchroclara__, CLARA_HOME override)
3. write selected MCP servers (__PROT_3_synchroclara__, CLARA_HOME override)
4. seed/enable selected built-in + optional skills (__PROT_4_synchroclara__)
5. spawn `clara -p <profile> skills install <id>` per hub skill (async, returns PIDs)
...
class ProfileCreate(BaseModel):
    ...
    mcp_servers: List[MCPServerCreate] = []      # __PROT_5_synchroclara__, CLARA_HOME override
    builtin_skills: List[str] = []               # __PROT_6_synchroclara__ enable/seed
...
page afterward. Mirror `_write_profile_model`'s CLARA_HOME-override helper for
...
- Backend E2E with isolated CLARA_HOME: POST a full create body
