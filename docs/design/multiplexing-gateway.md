...
_profile_runtime_scope(profile_home)           (gateway/run.py)
   ├── set_clara_home_override(home)          config / state.db / skills /
...
- `get_secret(name)` resolves: global allowlist → active scope → fallback.
  ...
  - multiplexing **on**, no scope installed: **raises `UnscopedSecretError`**
    ...
    value.
- A small allowlist (`CLARA_HOME`, `CLARA_PROFILE`, proxy settings,
...
reload helper: `clara_cli.env_loader.load_clara_dotenv` skips the
...
## The CLARA_HOME override

`clara_constants.py` holds a context-local override consulted by
`get_clara_home()` before the `CLARA_HOME` env var. Everything that resolves
...
automatically. `get_process_clara_home()` exists for the few machine-level
assets that must not follow the override. `clara_home_key()` gives
...
`profiles_to_serve(multiplex, profile_allowlist)` in `clara_cli/profiles.py`
...
DB handles are resolved at call time through the active CLARA_HOME override —
...
CLARA_HOME override. Asset writes are atomic, type- and size-capped.
...
| Built-in tool registry | Built-ins are process-global; plugin-registered tools are overlaid per profile via `clara_home_key()`. |
...
- `agent/secret_scope.py`, `clara_constants.py`, `gateway/profile_routing.py`,
  `gateway/run.py` (`_profile_runtime_scope`), `clara_cli/profiles.py`
