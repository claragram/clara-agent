...
The canonical transcript store is SQLite via `SessionDB` (from `clara_state`). The
...
Counts consecutive restarts via a JSON file (`{CLARA_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `clara update`, `clara gateway restart`,
or `/restart`.
