...
`clara doctor` reports the explicit repair command.
...
repaired or restored file. Do not run `clara doctor --fix` while the gateway
is still up. Next steps:

```bash
clara gateway stop
CLARA_HOME="$HOME/.clara" clara sessions recover --source "$HOME/.clara/state.db" --inspect-only
# if recoverable:
CLARA_HOME="$HOME/.clara" clara sessions recover --source "$HOME/.clara/state.db" --output "$HOME/recovered-state.db"
```
...
```bash
clara gateway stop
CLARA_HOME="$HOME/.clara" clara sessions repair --check-only
CLARA_HOME="$HOME/.clara" clara sessions repair
```
