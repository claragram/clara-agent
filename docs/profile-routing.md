...
`clara config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.clara/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `CLARA_HOME`, secret scope, and
