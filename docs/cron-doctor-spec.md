...
a provider run fails, or delivery starts failing. `clara cron list` shows some of
...
Add `clara cron doctor` as a read-only diagnostic command that summarizes cron
...
- `clara cron doctor` returns `0` and prints a healthy message when active jobs
...
- It returns `1` and prints grouped job-level issues when any active job has:
  ...
  - script path missing/outside `CLARA_HOME/scripts`, or
