# Clara Observer Hooks

Clara observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Clara' planner, model providers, memory, tool registry,
...
Clara also has a first-party NeMo Relay shared-metrics path. It uses these
...
```text
telemetry_schema_version = "clara.observer.v1"
```

Hook callbacks are fail-open. Clara catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Clara only builds
...
The native NeMo Relay SDK integration maps Clara session, turn, LLM, and tool
