...
Clara gateway daemon, exported over OTLP/HTTP to an operator-configured
...
capture is a separate plane served by Clara's native NeMo Relay SDK
...
| --- | --- | --- |
| Gateway gauges | `/v1/metrics` | `clara.gateway.up/state/busy/drainable/active_agents/background_work/background_delegations/restart_requested`, `clara.platform.up/degraded` with bounded `error_code` attributes |
...
`clara.gateway.active_agents`, `clara.gateway.background_work`, and
`clara.gateway.background_delegations` are complementary. `active_agents`
...
```bash
clara monitoring status
```

The OpenTelemetry SDK is an optional extra (`pip install 'clara-agent[otlp]'`),
...
a plain `clara gateway run`: the exporter lives in the gateway process, so
...
`clara.gateway.up`, `clara.platform.up`, and `clara.platform.degraded`.
...
absent_over_time(clara_gateway_up[5m])

# Locally owned bridge is explicitly down.
clara_platform_up == 0

# Scheduler thread is stale even though the gateway may still be alive.
clara_cron_scheduler_heartbeat_age_seconds > 180

# Ticker loops but has not completed a successful tick recently.
clara_cron_scheduler_last_success_age_seconds > 300

# One or more jobs are beyond their existing scheduler grace window.
clara_cron_jobs_overdue > 0

# Catch-up counter increased, proving at least one stale occurrence was
# collapsed and run once after a delay.
increase(clara_cron_scheduler_catch_up_occurrences[15m]) > 0
```

Cron execution lifecycle records arrive as `clara.cron_execution` spans.
Alert or derive events from bounded attributes such as:

```text
clara.status = failed|unknown
clara.delivery_outcome = failed|not_configured
clara.error_class = auth_failed|rate_limited|timeout|network_error|
...
3. a cron lifecycle feed keyed only by opaque `clara.job_key`;
...
Clara Agent-owned Relay transport health remains in scope. A separate gateway
...
python scripts/observability/otel_capture_collector.py \
  --host 127.0.0.1 --port 4318 --log /tmp/clara_otel_capture.jsonl
...
python scripts/observability/gateway_health_export_probe.py \
  ...
  --log /tmp/clara_otel_capture.jsonl --wait 8
...
`clara.error_class = ...` list in this file's alert section and the enum's unit
...
```bash
clara monitoring status                 # posture
python scripts/observability/gateway_health_export_probe.py \
  --endpoint http://127.0.0.1:4318/v1/traces \
...
The `clara monitoring` CLI intentionally exposes `status` only. This first
release covers only Clara Agent-owned service-health and operational-diagnostic
signals, including Clara Agent-owned Relay transport health. Team Gateway's
