...
Clara includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into Clara and does not require a Clara observability plugin. Clara remains
importable without Relay on other native targets. Those targets use an
explicit reduced-capability no-op host:
Clara execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `clara-agent[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

> [!WARNING]
> This removes the Clara `observability/nemo_relay` plugin. Existing users
...
> `plugins.toml` selected with `CLARA_NEMO_RELAY_PLUGINS_TOML`. The legacy
> `CLARA_NEMO_RELAY_ATOF_*` and `CLARA_NEMO_RELAY_ATIF_*` variables no
> longer activate exporters. Without the new variable, Clara does not run
> Relay plugin discovery, configuration layering, middleware, or exporters.

Clara requires NeMo Relay 0.7.1 or later within the 0.7 release line. That
...
Clara installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Clara process so configured interceptors can
...
Collection remains off unless Clara policy enables it:
...
opt-in. Set `CLARA_NEMO_RELAY_PLUGINS_TOML` to a selected `plugins.toml` to
activate configured middleware, exporters, or dynamic plugins. When the
variable is unset, Clara does not invoke Relay's plugin initializer, so Relay
...
from the selected file only. If the selected file cannot be loaded, Clara
...
retain the same `session_id` and add `clara.session.segment` plus
`clara.session.segment_reason` (`compaction` or `max_turns`).

## Process-Wide Plugin Policy and Profile Isolation

Relay plugin configuration is a process-level deployment choice, not a Clara
profile setting. The first hosted profile triggers lazy initialization, and
every additional profile hosted by that Clara process shares the resulting
static middleware, dynamic plugins, subscribers, exporters, and guardrail
policy. After initialization succeeds, Clara logs:

```text
Relay plugins are active process-wide and apply to all profiles hosted by this Clara process.
```
...
profile's Relay scope stack. Native dynamic plugins are loaded into the Clara
process and share the same policy boundary.

Run profiles in separate Clara processes when they require different trust
...
Clara core owns one Relay host and one isolated Relay session scope per Clara
session. Core lifecycle producers use
`clara_cli.observability.relay_runtime` to obtain the shared session handle or
run Relay scope, LLM, tool, and mark APIs in that session context. New product
marks do not require Clara plugin registration. Shared-metrics marks must
...
```text
Clara turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> Clara shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Clara sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that Clara used for the logical call, such as
...
New calls use `clara.model_route.count`. The previous
`clara.model_call.count` contract remains readable only so pending local
counters created by older builds can be exported without losing data.

The first consented session start emits an empty `clara.client.active` Relay
...
counter in any rolling 24-hour window. The metric has no dimensions; Clara
version, OS family, architecture, and install method remain bounded package
resources. Concurrent Clara processes share the SQLite latch, so simultaneous
...
Each task run is a Relay `Function` scope named `clara.task_run`, parented to
the owning Clara session. The start counter contains only bounded execution
...
provider attempts for the same Clara API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Clara tool-call
...
cancellations. Active task ownership follows the task ID if Clara rotates its
conversation session during context compression.

Each tool invocation is represented by a Relay tool lifecycle named
`clara.tool_call`. The terminal counter contains only bounded tool category,
outcome, approval outcome, latency, and explicit retry-count buckets. Clara
...
tool or plugin names. Clara does not infer retries from repeated tool names or
adjacent calls; when the
hook does not provide an explicit retry relationship, the retry bucket is
`unknown`. Approval decisions are emitted as `clara.tool_approval` marks and
...
Successful skill mutations emit `clara.skill.lifecycle` marks with only a
bounded action and provenance. Successful loads emit `clara.skill.load`
marks with bounded provenance, first-use or reuse state, reuse-after-patch
state, and a use-count bucket. Clara derives reuse and patch-generation
...
```text
$CLARA_HOME/telemetry/shared_metrics/metrics.sqlite3
$CLARA_HOME/telemetry/shared_metrics/outbox/*.json
```
...
are written with atomic replacement. Each package records the Clara version,
...
schema field name, its current scope is one `CLARA_HOME`, so it is more
...
Deleting `$CLARA_HOME/telemetry/shared_metrics` resets the identifier together
...
The install identity is scoped to one `CLARA_HOME`. To reset it, stop Clara
processes and remove `$CLARA_HOME/telemetry/shared_metrics`. This deliberately
...
Run a real Clara CLI turn against the deterministic local model server:
...
`$CLARA_HOME/telemetry/shared_metrics/outbox/` to the Clara telemetry ingest
...
telemetry:
  shared_metrics:
    ...
    send: false      # NEW: transmit to the Clara telemetry service
```
...
Removing `$CLARA_HOME/telemetry/shared_metrics` still resets local identity,
