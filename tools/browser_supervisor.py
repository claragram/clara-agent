...
One ``CDPSupervisor`` runs per Clara ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Clara logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "clara-dialog-bridge.invalid"
...
(() => {
  if (window.__claraDialogBridgeInstalled) return;
  window.__claraDialogBridgeInstalled = true;
  const ENDPOINT = "http://clara-dialog-bridge.invalid/";
