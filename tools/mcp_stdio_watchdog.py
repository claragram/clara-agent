...
<url>``) is spawned as a direct child of the Clara process. Clara's own
...
Clara process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next Clara startup's opt-in
