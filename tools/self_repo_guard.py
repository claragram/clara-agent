...
def _block_message(operation: str, root: Path) -> str:
    ...
    return (
        f"Blocked: `{operation}` would rewrite Clara's live source checkout "
        ...
        "checkout, stop Clara, run the command externally, then restart "
        "Clara."
    )
...
def _scratch_dir_hint() -> str:
    ...
    base = Path(clara_home).expanduser() if clara_home else Path.home() / ".clara"
