"""
Cron job management tools for Clara Agent.
...
# uses 30s) — comfortably below the 1800s default CLARA_AGENT_TIMEOUT.
_CRON_RUN_HEARTBEAT_INTERVAL = 10.0

# Hard ceiling on how long the heartbeat keeps the parent watchdog at bay.
# The child cron run has its own inactivity watchdog (CLARA_CRON_TIMEOUT,
# default 600s) that bounds a wedged job, but with CLARA_CRON_TIMEOUT=0
...
#      saying "cat ~/.clara/.env" or "rm -rf /". `_scan_cron_prompt()` runs
...
#      `clara-agent-dev` skill contains a security postmortem mentioning
#      `cat ~/.clara/.env`, which tripped `read_secrets` and silently
...
def _origin_from_env() -> Optional[Dict[str, str]]:
    ...
    origin_platform = get_session_env("CLARA_SESSION_PLATFORM")
    origin_chat_id = get_session_env("CLARA_SESSION_CHAT_ID")
    if origin_platform and origin_chat_id:
        ...
        if thread_id and origin_platform == "slack":
            message_id = get_session_env("CLARA_SESSION_MESSAGE_ID") or None
        ...
        return {
            ...
            "chat_name": get_session_env("CLARA_SESSION_CHAT_NAME") or None,
            "thread_id": thread_id,
            ...
            # send_message, which passes CLARA_SESSION_USER_ID to
            ...
            "scope_id": get_session_env("CLARA_SESSION_SCOPE_ID") or None,
        }
    return None
...
def _local_delivery_notice(job: Dict[str, Any], user_deliver: Optional[str]) -> Optional[str]:
    ...
    ``CLARA_SESSION_PLATFORM``/``CHAT_ID`` is set for them), so a
    ...
    for part in str(deliver).split(","):
        ...
        if not profile_exists(canon):
            return (
                ...
                "profile that exists here (clara profile list), or omit the "
...
def _resolve_cron_context_deliver(deliver: Optional[str]) -> Optional[str]:
    ...
    delivery target — the ``CLARA_CRON_AUTO_DELIVER_*`` contextvars that
    ...
    * Every other element (``local``, ``all``, explicit ``platform:...``)
    ...
    if not is_truthy_value(get_session_env("CLARA_CRON_SESSION", "")):
        return deliver

    def _creator_target() -> str:
        platform = get_session_env("CLARA_CRON_AUTO_DELIVER_PLATFORM", "").strip()
        chat_id = get_session_env("CLARA_CRON_AUTO_DELIVER_CHAT_ID", "").strip()
        if not platform or not chat_id:
            return "local"
        thread_id = get_session_env("CLARA_CRON_AUTO_DELIVER_THREAD_ID", "").strip()
        if thread_id:
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    ...
    Scripts must be relative paths that resolve within CLARA_HOME/scripts/.
    ...
    # Only relative paths within ~/.clara/scripts/ are allowed.
    if raw.startswith(("/", "~")) or (len(raw) >= 2 and raw[1] == ":"):
        return (
            f"Script path must be relative to ~/.clara/scripts/. "
            f"Got absolute or home-relative path: {raw!r}. "
            f"Place scripts in ~/.clara/scripts/ and use just the filename."
        )

    # Validate containment after resolution
    from tools.path_security import validate_within_dir

    scripts_dir = get_clara_home() / "scripts"
...
def _latest_job_output_excerpt(job_id: str, max_chars: int = 2000) -> Optional[str]:
    ...
    ``~/.clara/cron/output/``. Never raises.
    """
    try:
...
def _try_dispatch_background_run(
    ...
    None
        ...
        ``clara -z``, stateless HTTP, Kanban worker, nested cron run).
    ...
    # 'running' by a dead owner process -- e.g. a PRIOR one-shot `clara
    ...
    if not session_key:
        # Direct Python callers (`clara cron run`, tests) have no agent
    ...
    try:
        ...
        origin_ui_session_id = get_session_env("CLARA_UI_SESSION_ID", "") or ""
    except Exception:
        pass
...
def _gateway_liveness_notice(plural: bool = False) -> dict:
    ...
    Thin adapter over the shared CLI helper ``clara_cli.cron._builtin_gateway_liveness``
    ...
    """
    try:
        from clara_cli.cron import _builtin_gateway_liveness
    ...
    if _gw is False:
        return {
            ...
            "warning": (
                f"The Clara gateway is not running — {subject} "
                "but will NOT fire until the gateway is started "
                "(clara gateway install / clara gateway start). "
        ...
        if normalized == "create":
            ...
            try:
                job = create_job_with_scheduler_registration(
                    ...
                    # (clara cron create --reasoning-effort) ONLY — it is
...
Jobs run in a fresh session with no current-chat context, so prompts must be self-contained, and the agent's FINAL RESPONSE is what gets delivered — cron runs are autonomous and cannot ask questions. Prefer updating an existing job over creating near-duplicates.""",
    "parameters": {
        ...
        "properties": {
            ...
            "script": {
                "type": "string",
                "description": f"Optional script run each tick; stdout is injected into the agent's prompt as context (with no_agent=True the script IS the job). Relative paths resolve under {display_clara_home()}/scripts/; .sh/.bash via bash, else Python. On update, '' clears."
            },
            "monitor": {
                "type": "string",
...
def check_cronjob_requirements() -> bool:
    ...
    return (
        env_var_enabled("CLARA_INTERACTIVE")
        or env_var_enabled("CLARA_GATEWAY_SESSION")
        or env_var_enabled("CLARA_EXEC_ASK")
    )
...
def _cronjob_handler(args, **kw):
    ...
    return cronjob(
        ...
        # `clara cron create/edit --model`, or hand-edited jobs). The agent
