#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Clara Skills Hub.
...
Used by clara_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _skills_dir() -> Path:
    ...
    return Path(forced) if forced is not None else _clara_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            clara_meta = metadata.get("clara", {})
            if isinstance(clara_meta, dict):
                tags = clara_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Workprise) but not activated
    ...
    OFFICIAL_REPO = "Workprise/clara-agent"
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        # upstream project maintains (e.g. impeccable's Clara-native bundle
        # under pbakaus/impeccable:.clara/skills/impeccable). Install pulls
    ...
    def _fetch_from_live_repo(self, rel: str) -> Optional[SkillBundle]:
        ...
        ``clara update``. Rather than telling them to update first, resolve
    ...
    def _upstream_pointer(self, skill_dir: Path) -> Optional[Dict[str, str]]:
        ...
        A stub declares ``metadata.clara.upstream`` in its SKILL.md
        frontmatter:
    ...
    def _upstream_pointer_from_content(self, content: Union[str, bytes]) -> Optional[Dict[str, str]]:
        """Parse ``metadata.clara.upstream`` out of SKILL.md content."""
        ...
        if not isinstance(meta_block, dict):
            return None
        clara_meta = meta_block.get("clara")
        if not isinstance(clara_meta, dict):
            return None
        upstream = clara_meta.get("upstream")
...
def _category_skill_dirs(directory: Path) -> List[str]:
    ...
    ``clara_cli.skills_hub._existing_categories``.
    """
    skill_dirs: List[str] = []
...
def _clara_index_cache_file() -> Path:
    return _index_cache_dir() / "clara-index.json"


def _load_clara_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for CLARA_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    clara_index_cache_file = _clara_index_cache_file()
    if clara_index_cache_file.exists():
        try:
            age = time.time() - clara_index_cache_file.stat().st_mtime
            if age < CLARA_INDEX_TTL:
                return json.loads(clara_index_cache_file.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("Clara index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "Clara index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Clara index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        clara_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        clara_index_cache_file.write_text(json.dumps(data), encoding="utf-8")
    except OSError:
        pass
...
class ClaraIndexSource(SkillSource):
    """Skill source backed by the centralized Clara Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "clara-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "clara-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        ClaraIndexSource(auth=auth), # Centralized index (search + resolved install paths)
