...
it is automatically downloaded from GitHub releases to $CLARA_HOME/bin/tirith.
...
def _get_clara_home() -> str:
    """Return the Clara home directory, respecting CLARA_HOME env var."""
    return str(get_clara_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_clara_home(), ".tirith-install-failed")
...
def _clara_bin_dir() -> str:
    """Return $CLARA_HOME/bin, creating it if needed."""
    d = os.path.join(_get_clara_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $CLARA_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_clara_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $CLARA_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $CLARA_HOME/bin/tirith
    ...
    clara_bin = os.path.join(_clara_bin_dir(), "tirith")
    if os.path.isfile(clara_bin) and os.access(clara_bin, os.X_OK):
        _resolved_path = clara_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return clara_bin
