...
409), and is driven by:
  ...
  * the ``clara sync status|pull|push|now`` CLI.

It lives beside ``tools/skills_sync.py`` (NOT under ``clara_cli/``) so the
...
**Clara admin**. We read that off the access token, which rides on the same
bearer ``resolve_clara_runtime_credentials()`` returns; we decode the JWT
...
status conflates "may administer Clara" with "has Skill Sync enabled", and has
...
is toggled via ``clara sync enable/disable`` (a ``sync`` flag on the skill's
...
user-authored skills under ``~/.clara/skills/`` are eligible; bundled and
...
# only as the local *intent* the user toggles via ``clara sync enable`` — it is
...
# namespace from clara-agent's local ``content_hash`` (skills_guard.py:846),
...
#
# We reuse resolve_clara_runtime_credentials() for the bearer (it honors the
...
def _decode_jwt_payload_unverified(token: str) -> Dict[str, Any]:
    ...
    plugins/dashboard_auth/clara/__init__.py:463.
    """
    try:
...
def resolve_identity() -> Dict[str, Any]:
    """Resolve the Clara bearer + owner + dev-gate flag.

    Returns a dict: ``{api_key, base_url, owner, clara_admin, claims}``.
    ...
    try:
        ...
        raise SyncInertError(f"no Clara credentials: {e}") from e
    ...
    clara_admin = claims.get(CLARA_ADMIN_CLAIM) is True
    return {
        "api_key": api_key,
        "base_url": (creds or {}).get("base_url"),
        "owner": str(owner),
        "clara_admin": clara_admin,
        "claims": claims,
    }
...
def dev_gate_open() -> bool:
    ...
    try:
        return bool(resolve_identity().get("clara_admin"))
...
# sync.base_url (or the CLARA_SYNC_BASE_URL bridge env) overrides it to point
...
DEFAULT_SYNC_BASE_URL = "https://gateway-gateway.claraship.com"

def resolve_sync_base_url() -> Optional[str]:
    """Resolve the sync-plane base URL.

    Order: CLARA_SYNC_BASE_URL env bridge -> config.yaml ``sync.base_url`` ->
    ...
    """
    env = os.getenv("CLARA_SYNC_BASE_URL")
...
# ---------------------------------------------------------------------------
# Sync feature configuration — env-first, so a Clara Cloud instance can be set
...
# precedence as base_url: the CLARA_SYNC_* env var wins, else config.yaml
# ``sync.*``, else a built-in default.
#
#   CLARA_SYNC_BASE_URL        -> sync.base_url        (the sync plane URL)
#   CLARA_SYNC_ENABLED         -> sync.enabled         (master on/off; default off)
#   CLARA_SYNC_DEFAULT_OPT_IN  -> sync.default_opt_in  (personal sync policy; default false
...
def sync_feature_enabled() -> bool:
    ...
    ``CLARA_SYNC_ENABLED`` -> ``sync.enabled`` -> False. This is the master
    switch a Clara Cloud deployment sets to opt its instances into sync by
    default. It is checked by the gate-and-swallow entrypoints IN ADDITION to
    the Clara-admin token gate and a configured base URL — all three must hold for
    background sync to run.
    """
    return _sync_config_bool("CLARA_SYNC_ENABLED", "enabled", default=False)


def sync_org_auto_propose() -> bool:
    """Whether an agent/user edit to an org skill is proposed automatically.

    ``CLARA_SYNC_ORG_AUTO_PROPOSE`` -> ``sync.org_auto_propose`` -> False.

    False (default): edits to an org-shared skill stay LOCAL until the user
    runs ``clara sync propose <skill>``. The skill keeps working with the
    ...
    return _sync_config_bool(
        "CLARA_SYNC_ORG_AUTO_PROPOSE", "org_auto_propose", default=False
    )
...
def sync_default_opt_in() -> bool:
    ...
    ``CLARA_SYNC_DEFAULT_OPT_IN`` -> ``sync.default_opt_in`` -> False.

    False (default): opt-IN — a skill syncs only after an explicit
    ``clara sync enable`` (or a plane manifest that opted it in). True: opt-OUT
    ...
    Clara Cloud deployment wants. Per the design notes, this default is
    ...
    """
    return _sync_config_bool("CLARA_SYNC_DEFAULT_OPT_IN", "default_opt_in", default=False)


# ---------------------------------------------------------------------------
# Local skill eligibility + the personal sync opt-in "sync" flag
#
# Only agent-created + user-authored skills under ~/.clara/skills/ sync.
...
def is_sync_eligible(skill_name: str) -> bool:
    ...
    Eligible = present locally under ~/.clara/skills/, NOT bundled, NOT
...
def list_synced_skill_names() -> List[str]:
    ...
    - **opt-out (Clara Cloud "on by default"):** every *eligible* skill syncs
      UNLESS its usage record explicitly carries ``sync: false``. This is what a
      deployment sets (via ``CLARA_SYNC_DEFAULT_OPT_IN``) so a user's skills
...
def _all_local_skill_names() -> List[str]:
    ...
    opt-out policy). A skill is any directory under ~/.clara/skills/ containing
...
def stable_device_id() -> str:
    ...
    Use ``set_device_name()`` / ``clara sync device --name`` to set an explicit
    label."""
    ...
    # Clara Cloud (and any templated deployment) can seed the label
    # declaratively via CLARA_SYNC_DEVICE_NAME, so a hosted instance shows a
    # recognizable name with no CLI call. Env seeds the FIRST-USE value only; it
    # is then persisted, so a later `clara sync device --name` (or editing the
    # file) still wins on that device. An explicit file (above) always wins over
    # the env.
    import os

    env_name = (os.environ.get("CLARA_SYNC_DEVICE_NAME") or "").strip()
...
def set_device_name(name: str) -> str:
    ...
    Writes the (trimmed) name to ~/.clara/skills/.sync_device_id, overwriting
...
# the caller's three-way merge. Auth is the Clara bearer resolved above.
# ---------------------------------------------------------------------------
...
# ~/.clara/skills/.sync_state as JSON.
#
...
# path under ~/.clara/skills/ (the sync contract: "the profile root is a tree
...
def _skill_rel_path(skill_name: str) -> Optional[PurePosixPath]:
    """Return the skill's path relative to ~/.clara/skills/ (posix), or None."""
    try:
...
def _check_version(caps: Dict[str, Any]) -> None:
    ...
    if major != WIRE_VERSION:
        raise SyncError(
            f"this server speaks sync version {ver!r}, but this Clara speaks "
            f"{WIRE_VERSION} — update Clara to sync with it"
        )
    ...
    if overlaps:
        ...
        return {
            ...
            "message": (
                ...
                f"{conflict_ref}. Resolve out-of-band (clara sync / NAS UI)."
            ),
        }
...
def pull_skills(
    ...
    *,
    ...
    ~/.clara/skills/. Only paths the user has opted into (``sync: true``) are
...
# (no push, no pull, no-op) unless the signed-in user is a Clara admin.
# ---------------------------------------------------------------------------

def maybe_push_skills(*, message: str = "clara skill sync") -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if not identity.get("clara_admin"):
            return None  # access gate: inert unless the user is a Clara admin
        if not sync_feature_enabled():
            return None  # feature off for this instance (CLARA_SYNC_ENABLED)
...
def sync_status() -> Dict[str, Any]:
    """Return a status snapshot for ``clara sync status``. Never raises."""
    status: Dict[str, Any] = {
        "clara_admin": False,
        "logged_in": False,
    ...
    try:
        ...
        status["clara_admin"] = bool(identity.get("clara_admin"))
    except SyncInertError:
        pass
    except Exception as e:
...
#
# Org skills live under a DISTINCT local namespace, ~/.clara/skills/_org/
...
#
# `clara sync propose` is the org sharing surface; proposal is
...
def pull_org_skills(
    ...
    *,
    ...
    """Pull the org canonical set into ``~/.clara/skills/_org/<org_id>/``.
