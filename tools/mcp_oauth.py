...
URL of Clara' published Client ID Metadata Document (CIMD) as the
...
This module provides the glue:
    - ``ClaraTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Clara Agent"
...
from clara_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__clara_user_skipped__"
...
def _get_token_dir(clara_home: str | Path | None = None) -> Path:
    """Return the directory for MCP OAuth token files.

    Uses CLARA_HOME so each profile gets its own OAuth tokens.
    Layout: ``CLARA_HOME/mcp-tokens/``
    """
    from clara_constants import get_clara_home

    base = Path(clara_home) if clara_home is not None else Path(get_clara_home())
...
def _cached_redirect_port(storage: "ClaraTokenStorage | None") -> int | None:
    ...
    redirect URI that was registered with it. If Clara restarts and chooses a
...
def _cached_redirect_uri(storage: "ClaraTokenStorage | None") -> str | None:
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    shared, actionable ``clara mcp login`` next-step so the guidance wording
    lives in one place across every non-interactive OAuth boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} "
            "Run `clara mcp login <server>` interactively to (re)authorize, "
...
# ---------------------------------------------------------------------------
# ClaraTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class ClaraTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        CLARA_HOME/mcp-tokens/<server_name>.json         -- tokens
        CLARA_HOME/mcp-tokens/<server_name>.client.json   -- client info
        CLARA_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
        CLARA_HOME/mcp-tokens/<server_name>.cimd-off      -- CIMD refused here
    """

    def __init__(self, server_name: str, *, clara_home: str | Path | None = None):
        self._server_name = _safe_filename(server_name)
        self._clara_home = Path(clara_home) if clara_home is not None else None

    def _tokens_path(self) -> Path:
        return _get_token_dir(self._clara_home) / f"{self._server_name}.json"

    def _client_info_path(self) -> Path:
        return _get_token_dir(self._clara_home) / f"{self._server_name}.client.json"

    def _meta_path(self) -> Path:
        return _get_token_dir(self._clara_home) / f"{self._server_name}.meta.json"

    def _cimd_rejected_path(self) -> Path:
        return _get_token_dir(self._clara_home) / f"{self._server_name}.cimd-off"
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if OAuthToken is None and not _ensure_sdk_loaded():
            return None
        # Clara records an absolute wall-clock ``expires_at`` alongside the
    ...
    def mark_cimd_rejected(self) -> None:
        ...
        refused. Cleared by ``remove()``, i.e. by ``clara mcp login`` /
        ``clara mcp remove``, so a fixed document gets another chance.
        """
    ...
    def restore(self, snapshot: dict[str, bytes], *, only_if_absent: bool = False) -> None:
        ...
        if not snapshot:
            return
        token_dir = _get_token_dir(self._clara_home)
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Clara.</p></body></html>"
            ) if code else (
    ...
    async def _redirect_handler(authorization_url: str) -> None:
        ...
        if on_ssh and redirect_uri:
            ...
            print(
                ...
                f"\n"
                f"  See: https://agent.claraprise.com/docs/guides/oauth-over-ssh\n",
                file=sys.stderr,
            )
        ...
        if result["auth_code"] is None:
            ...
            if cimd_url:
                hint = (
                    ...
                    f"Clara' Client ID Metadata Document ({cimd_url}); set "
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `clara mcp login <server>` later to "
...
ClaraOAuthClientProvider: Any = None


def _get_clara_oauth_provider_class() -> type | None:
    global ClaraOAuthClientProvider
    if ClaraOAuthClientProvider is not None:
        return ClaraOAuthClientProvider
    if not _ensure_sdk_loaded():
        return None

    class _ClaraOAuthClientProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, token_user_agent: "str | None" = None, **kwargs: Any):
            ...
            self._clara_token_user_agent = token_user_agent

        def _stamp_token_user_agent(self, request):
            ua = getattr(self, "_clara_token_user_agent", None)
            if ua:
    ...
    _ClaraOAuthClientProvider.__name__ = "ClaraOAuthClientProvider"
    _ClaraOAuthClientProvider.__qualname__ = "ClaraOAuthClientProvider"
    ClaraOAuthClientProvider = _ClaraOAuthClientProvider
    return ClaraOAuthClientProvider
...
def remove_oauth_tokens(
    ...
    *,
    ...
    storage = ClaraTokenStorage(server_name, clara_home=clara_home)
    storage.remove()
...
# 2026-07-28. The SDK does the protocol work; Clara only decides whether a
...
# agent.claraprise.com/docs/* 301s here.
_CIMD_CLIENT_METADATA_URL = (
    "https://workprise.github.io/clara-agent/docs/oauth/client-metadata.json"
)
...
# (section 4.2), so a CIMD flow cannot use the ephemeral port Clara picks
...
def _has_cached_client_info(storage: "ClaraTokenStorage | None") -> bool:
...
def _server_declined_cimd(storage: "ClaraTokenStorage | None") -> bool:
    ...
    Clara has to fix the redirect URI. Cached authorization-server metadata
...
def _maybe_use_cimd(
    cfg: dict,
    storage: "ClaraTokenStorage | None" = None,
) -> "tuple[str, int] | None":
    """Return ``(client_id URL, pinned callback port)``, or None to use DCR.

    Every early return below is a case where the redirect URI Clara would
...
#   "Claude Code" → 200
#   "Codex"       → 200
#   "Clara Agent" / "Clara" / "Cursor" / "VS Code" / … → 403
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    if OAuthClientMetadata is None:
        _ensure_sdk_loaded()
    client_name = cfg.get("client_name", "Clara Agent")
    scope = cfg.get("scope")
    ...
    metadata_kwargs: dict[str, Any] = {
        ...
        # rejecting loopback redirect_uris. Clara is a CLI/desktop app
...
def _invalidate_tokens_on_client_change(
    ...
    if removed:
        logger.warning(
            ...
            "Re-authorize with: clara mcp login %s",
...
def humanize_oauth_registration_error(
    ...
    *,
    ...
    Figma's remote MCP gates DCR on exact ``client_name``. Clara auto-sets
    ``Claude Code`` (known-good); this message fires when the user overrode
    that with something Figma still rejects, or an older Clara is running.
    """
    msg = str(exc)
    lowered = msg.lower()
...
def build_oauth_auth(
    ...
    storage = ClaraTokenStorage(server_name)
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`clara mcp login {server_name}` interactively first to complete "
    ...
    provider_class = _get_clara_oauth_provider_class()
