...
Design contract (matches the Clara "no lazy-reading escape hatches / don't
...
def _check_metadata_block(frontmatter: Dict[str, Any]) -> List[LintFinding]:
    ...
    clara_meta = meta.get("clara") if isinstance(meta, dict) else None
    if not isinstance(clara_meta, dict):
        findings.append(
            LintFinding(
                WARNING,
                "missing-metadata",
                "frontmatter is missing metadata.clara.{tags, related_skills}.",
            )
        )
    else:
        if "tags" not in clara_meta:
            findings.append(
                LintFinding(
                    WARNING, "missing-metadata", "metadata.clara.tags is missing."
                )
            )
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("clara", "agent", "clara agent") and (
        author != "Clara Agent"
    ):
        findings.append(
            LintFinding(
                WARNING,
                "author-caps",
                f"author '{author}' should be 'Clara Agent' (proper caps) "
