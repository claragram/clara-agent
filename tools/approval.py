...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("CLARA_YOLO_MODE", ""))
...
# Clara session id (observability identity, distinct from the gateway
...
# os.environ["CLARA_INTERACTIVE"] races: one session's restore in `finally`
...
# single-threaded CLI callers that still export CLARA_INTERACTIVE.
_clara_interactive_ctx: contextvars.ContextVar[Optional[str]] = contextvars.ContextVar(
    "clara_interactive",
    default=None,
)


def set_clara_interactive_context(interactive: bool) -> contextvars.Token:
    """Bind interactive mode for the current context (thread or asyncio task).

    Use this instead of mutating ``os.environ["CLARA_INTERACTIVE"]`` from
    concurrent executor threads. When unset (default), interactive detection
    falls back to the ``CLARA_INTERACTIVE`` env var for legacy callers.
    """
    return _clara_interactive_ctx.set("1" if interactive else "")


def reset_clara_interactive_context(token: contextvars.Token) -> None:
    """Restore the prior value from :func:`set_clara_interactive_context`."""
    _clara_interactive_ctx.reset(token)
...
def _is_interactive_cli() -> bool:
    ...
    back to the ``CLARA_INTERACTIVE`` env var for single-threaded callers.
    """
    ctx_val = _clara_interactive_ctx.get()
    if ctx_val is not None:
        return is_truthy_value(ctx_val)
    return env_var_enabled("CLARA_INTERACTIVE")
...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    try:
        ...
        # Forward the Clara session id so observer plugins parent approval
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("CLARA_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return os.getenv("CLARA_SESSION_PLATFORM", "") or ""
...
def _is_cron_approval_context() -> bool:
    ...
    try:
        ...
        return is_truthy_value(get_session_env("CLARA_CRON_SESSION", ""))
    except Exception:
        return env_var_enabled("CLARA_CRON_SESSION")
...
def _is_unattended_platform_approval_context() -> bool:
    ...
    ``CLARA_SESSION_PLATFORM`` like chat gateways do, but there is no human
...
def _is_single_query_approval_context() -> bool:
    ...
    ``clara chat -q "..."`` runs one turn and exits with no user waiting to
    answer approval prompts, but it still exports ``CLARA_INTERACTIVE=1`` so
    ...
    try:
        ...
        return is_truthy_value(get_session_env("CLARA_SINGLE_QUERY_SESSION", ""))
    except Exception:
        return env_var_enabled("CLARA_SINGLE_QUERY_SESSION")


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set CLARA_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind CLARA_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds CLARA_SESSION_PLATFORM via
    ...
    if _is_unattended_platform_approval_context():
        return False
    if env_var_enabled("CLARA_GATEWAY_SESSION"):
        return True
...
def _should_fall_through_to_cli_approval(
    *,
    ...
    ``CLARA_EXEC_ASK`` (and sometimes a session platform marker) can leak into
...
# via shell expansions like $HOME or $CLARA_HOME, or by the resolved absolute
# active profile home path such as /home/clara/.clara/config.yaml. The
# resolved-absolute form is folded into the ~/.clara/ patterns at detection
...
# go stale when CLARA_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_CLARA_ENV_PATH = (
    r'(?:~\/\.clara/|'
    r'(?:\$home|\$\{home\})/\.clara/|'
    r'(?:\$clara_home|\$\{clara_home\})/)'
    r'\.env\b'
)
# ~/.clara/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _CLARA_ENV_PATH; matches the CLARA_HOME override form as
# well as ~/.clara/.
_CLARA_CONFIG_PATH = (
    r'(?:~\/\.clara/|'
    r'(?:\$home|\$\{home\})/\.clara/|'
...
def _save_blocked_payload(command: str) -> Optional[str]:
    ...
    try:
        ...
        script_dir = get_clara_home() / "cache" / "blocked-scripts"
        ...
        path.write_text(
            "#!/bin/bash\n"
            "# Auto-saved by Clara: this command exceeded the inline command\n"
...
DANGEROUS_PATTERNS = [
    ...
    # These are native Windows EXEs / cmdlets reachable from ANY Clara
    ...
    (r'\bappdata[\\/](?:local|roaming)[\\/]clara[^\n]*\.env\b', "access to Clara secrets (Windows path)"),
    # ─────────────────────────────────────────────────────────────────────
    ...
    # `clara` and `gateway` (e.g. `clara -p ade gateway restart`) so a
    # profile flag can't slip the agent past the guard.
    (r'\bclara\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart clara gateway (kills running agents)"),
    (r'\bclara\s+update\b', "clara update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `clara gateway restart`
    ...
    # a lifecycle command past the guard — same treatment as the `clara ...
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart clara-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart clara-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(clara|gateway|cli\.py)\b', "kill clara/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill clara` but not
    # `kill -9 $(pgrep -f clara)` because the substitution is opaque
    ...
    # the `clara gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.clara.gateway`).
    # Catch the operations that stop, restart, or unload it.
    #
    # Order-independent (2026-08-02 incident): the previous version required
    # "clara"/"ai.clara" to appear AFTER the launchctl verb in the same
    ...
    # 'ai.clara.gateway-apollo:...' ...; do label=${item%%:*}; launchctl
    # bootout "$label"; done` — never has the literal text "clara" appear
    ...
    # an unrelated "clara" mention now also matches) — for an approval gate
    # that's the correct direction to err: an extra approval prompt is
    # cheap, a missed one took down the whole gateway fleet.
    (r'(?=[\s\S]*\blaunchctl\s+(?:stop|kickstart|bootout|unload|kill|disable|remove)\b)(?=[\s\S]*\b(?:clara|ai\.clara)\b)', "stop/restart clara launchd service (kills running agents)"),
    # File copy/move/edit into sensitive system paths (/etc/ and macOS
    # /private/etc/ mirror).
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Clara file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.clara/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a Clara-managed security file (~/.clara/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_CLARA_CONFIG_PATH}|{_CLARA_ENV_PATH})', "in-place edit of Clara config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_CLARA_CONFIG_PATH}|{_CLARA_ENV_PATH})', "in-place edit of Clara config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_CLARA_CONFIG_PATH}|{_CLARA_ENV_PATH})', "in-place edit of Clara config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/ and ~/.clara/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / CLARA_HOME even when those are set after this module is
    ...
    #
    # Fold the (more specific) Clara home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\clara), so folding the user home
    # first would eat the prefix the Clara-home fold needs.
    command = _rewrite_resolved_clara_home(command)
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / CLARA_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.clara``); the matched
...
def _rewrite_resolved_clara_home(command: str) -> str:
    """Rewrite the resolved absolute Clara home prefix to ``~/.clara/``.

    Resolves the active ``CLARA_HOME`` at call time (and its symlink-resolved
    form) and folds an occurrence of ``<home>/`` in *command* into
    ``~/.clara/`` so the static ``_CLARA_CONFIG_PATH`` / ``_CLARA_ENV_PATH``
    patterns match. In Docker and gateway deployments the agent often references
    the resolved absolute path directly (e.g. ``sed -i ...
    /home/clara/.clara/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$CLARA_HOME``. Matches both POSIX and Windows separators. No-op when the
    path can't be resolved or doesn't appear.
    """
    try:
        from clara_constants import get_clara_home
        home = get_clara_home().expanduser()
        candidates = [
            str(home),
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.clara")
...
def _mask_quoted_newlines(command: str) -> str:
    ...
    start. That made any multi-line quoted argument (``clara send`` message
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one Clara ad-hoc temp script."""
    try:
    ...
    if os.path.dirname(target) != temp_dir:
        return False
    return re.fullmatch(r"clara-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None


_GATEWAY_LIFECYCLE_SPLICE_DESCRIPTION = (
    "stop/restart clara gateway via shell-spliced verb (kills running agents)"
)
...
def _is_shell_token_spliced_gateway_lifecycle(command: str) -> bool:
    ...
    ``launchctl kick"start" -k gui/501/ai.clara.gateway`` auto-approved
    ...
    anchored on a clara-gateway identifier — reusing its prose
...
def _release_permission_mode_dependents(session_key: str) -> None:
    """Drop resources whose immutable mode is derived from Clara YOLO.
...
def is_approval_bypass_active_for_session(session_key: str) -> bool:
    """Return whether one exact session bypasses Clara approval prompts.

    Collapses the canonical three-source bypass check used across the codebase
    into one place:
      - process-scoped ``--yolo`` / ``CLARA_YOLO_MODE`` (frozen at import time
    ...
    *,
    ...
    Args:
        ...
        fail_closed_when_no_human: When True, a non-interactive non-gateway
            ...
            CLARA_INTERACTIVE unset) BLOCKS instead of auto-approving. The
    ...
    # Single-query (-q) sessions export CLARA_INTERACTIVE=1 but have no user
    ...
    if not is_cli and not is_gateway:
        ...
        if _is_cron_approval_context():
            ...
            logger.warning(
                ...
                "BLOCKED (fail-closed). Set CLARA_INTERACTIVE or "
                "CLARA_GATEWAY_SESSION to answer the prompt.",
                autoapprove_log_prefix, pattern_key, description,
            )
            return {
                "approved": False,
        ...
        logger.warning(
            "%s (pattern: %s): %s — set CLARA_INTERACTIVE or "
            "CLARA_GATEWAY_SESSION to require approval.",
            autoapprove_log_prefix, pattern_key, description,
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("CLARA_EXEC_ASK"):
        ...
        # can see (CLARA_EXEC_ASK / platform-marker leaks into CLI).
...
def request_tool_approval(
    ...
    *,
    ...
    (a bare script with no ``CLARA_INTERACTIVE``) fails CLOSED — a plugin-
...
def get_plugin_manager():
    ...
    from clara_cli.plugins import discover_plugins, get_plugin_manager as _get_manager
...
def _present_with_selected_transport(
    *,
    ...
    try:
        from agent.redact import redact_sensitive_text
        from clara_cli.approval_transport import ApprovalRequest, invoke_approval_transport
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = _is_interactive_cli()
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("CLARA_EXEC_ASK")
    ...
    if _is_single_query_approval_context():
        is_cli = False
        is_gateway = False
        # CLARA_EXEC_ASK routes through the gateway decision loop (no human
        ...
        if _is_single_query_approval_context():
            if _get_single_query_approval_mode() == "deny":
                ...
                try:
                    ...
                    try:
                        from clara_cli.config import load_config_readonly as _load_cfg
...
def check_execute_code_guard(code: str, env_type: str,
    ...
    # Ask-mode (CLARA_EXEC_ASK) still takes this path even when INTERACTIVE
    ...
    if notify_cb is None:
        # CLARA_EXEC_ASK (and sometimes a session platform marker) can leak
