...
teammate agent (another Clara profile on this install, or an agent on a
...
DM each other via a prompt-injected ``clara -p <bot> chat ...`` shellout.
...
- Everything here is additive. The legacy protocol transports
  (``clara -p`` / ``clara peer dm``) keep working for older prompts.

The transports themselves are unchanged and proven:
- local teammate  → ``clara -p <name> chat --in ~ -c "Bot Chat"
  --create-if-missing -Q --query-file <tmp>`` (one turn, reply on stdout)
- peer teammate   → ``clara peer dm <peer>[/<name>] < <tmp>``
                ...
                "properties": {
                    "target": {
                        ...
                        "description": (
                            ...
                            "('researcher', 'clara' for the default agent), or "
...
def _resolve_local_name(target: str, roster: list[str]) -> Optional[str]:
    """Map a target handle to a profile name ('clara' → 'default')."""
    want = target.strip()
    if not want:
        return None
    if want.lower() == "clara":
...
def message_agent_tool(
    ...
    if peer_match or bare_peer:
        ...
        # Pin the registry-owning profile (#93935): `clara peer` resolves
...
def cleanup_bot_dm_cache(
    ...
    locations: list[tuple[Path, str]] = [
        (temp_root, "clara-dm-*.txt"),
        (temp_root, "clara-relay-dm-*.txt"),
    ]
    try:
...
def _delivery_lock(argv: list[str], *, stdin_file: bool):
    ...
    Local deliveries (``clara -p <profile> chat …``) collide with relay
    ...
    # resolves the venv-relative clara next to this gateway's interpreter
    ...
    if (
        stdin_file
        or len(argv) < 3
        or cli not in ("clara", "clara.exe")
        or argv[1] != "-p"
    ):
        return contextlib.nullcontext()
    from tools.bot_relay import acquire_turn_lock

    home = Path(os.getenv("CLARA_HOME") or os.path.expanduser("~/.clara"))
    return acquire_turn_lock(_clara_root(home), argv[2])
...
def _agent_home(agent: Any) -> str:
    ...
    try:
        ...
        pass
    return os.getenv("CLARA_HOME") or os.path.expanduser("~/.clara")
