...
Clara Cloud, docker) is a persistent line. This module is the gateway-side
...
file plumbing on the gateway's own CLARA root — no network. The public
...
# ``bot_mode.envelope_ttl_seconds`` default in clara_cli/config_defaults.py.
...
def _normalize_roster_row(row: Any) -> Optional[dict]:
    ...
    if not profile or not connection_id:
        return None
    if not handle:
        handle = "clara" if profile == "default" else profile
    if (
...
def _clara_cli() -> str:
    """Resolve the clara CLI beside this gateway's own interpreter.

    The deliver RPC runs on the target gateway, whose process is the venv
    python — its bin/Scripts directory holds the matching ``clara``
    entrypoint. A bare ``"clara"`` relies on PATH, which is exactly what
    ...
    sibling = exe.parent / ("clara.exe" if sys.platform == "win32" else "clara")
    if sibling.is_file():
        return str(sibling)
    found = shutil.which("clara")
    if found:
        return found
    return "clara"
...
# turns concurrently: deliveries spawn separate ``clara`` subprocesses, so
