...
_FORCE_SYNC_ENV = "CLARA_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.clara") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.clara to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``CLARA_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, clara_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.clara/`` directory as a tar archive,
        ...
        """
        with self._transaction_lock:
            self._sync_back_transaction(clara_home=clara_home)

    def _sync_back_transaction(self, clara_home: Path | None = None) -> None:
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="clara-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.clara/skills/a.md`` →
        ``~/.clara/skills/a.md``, a new remote file at
        ``/root/.clara/skills/b.md`` maps to ``~/.clara/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
