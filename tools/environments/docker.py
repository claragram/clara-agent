...
_EGRESS_LABEL_KEY = "clara-egress"
...
def _load_clara_env_vars() -> dict[str, str]:
    """Load ~/.clara/.env values without failing Docker command execution."""
    try:
...
def _get_active_profile_name() -> str:
    """Return the active Clara profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale clara-tagged containers left behind by prior processes.

    Targets containers that match all of:

    * ``label=clara-agent=1`` (created by this codebase)
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling Clara process whose reuse path will pick them
      up; killing them would crash the sibling mid-command)
    * (optional) ``label=clara-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a clara process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Clara
    process leaves its container behind permanently because there's no
    subsequent Clara process scheduled to reuse that exact (task, profile)
    pair.
    """
    docker = docker_exe or find_docker() or "docker"
    filters = ["--filter", "label=clara-agent=1", "--filter", "status=exited"]
    if profile_filter:
        filters.extend(["--filter", f"label=clara-profile={_sanitize_label_value(profile_filter)}"])
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``CLARA_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("CLARA_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using CLARA_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image's init drops from root to the 'clara'
...
def _egress_proxy_args_for_docker() -> tuple[list[str], dict[str, str], list[str]]:
    ...
    * ``env_overrides`` — env vars to set on container creation: ``HTTPS_PROXY``,
      ...
      paths, and one ``CLARA_PROXY_TOKEN_<NAME>`` per minted mapping
    * ``host_args`` — extra ``--add-host`` flags so the container can reach the
    ...
    if not status.configured:
        msg = (
            ...
            "Run `clara egress setup` to mint tokens and write proxy.yaml."
        )
        if enforce:
    ...
    if not (status.pid and status.listening):
        msg = (
            ...
            "Start it with `clara egress start`."
        )
        if enforce:
            raise RuntimeError(msg)
        logger.warning("%s — continuing without proxy (enforce_on_docker=false).", msg)
    ...
    if status.ca_cert_path is None or not status.ca_cert_path.exists():
        ...
        msg = (
            ...
            "Re-run `clara egress setup` to regenerate it."
        )
        if enforce:
            raise RuntimeError(msg)
        logger.warning("%s — continuing without proxy (enforce_on_docker=false).", msg)
    ...
    if not mappings:
        msg = (
            ...
            "corrupt.  Re-run `clara egress setup` to mint provider "
    ...
    container_ca = "/etc/ssl/certs/clara-egress-ca.crt"
    ...
    # through the proxy.  Keep the CLARA_PROXY_TOKEN_* aliases for
    # diagnostics.
    for m in mappings:
        env_overrides[m.real_env_name] = m.proxy_token
        env_overrides[f"CLARA_PROXY_TOKEN_{m.real_env_name}"] = m.proxy_token
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``clara-agent:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.clara/sandboxes/). Non-persistent
        ...
        if egress_env_overrides:
            try:
                from clara_cli.config import load_config as _load_cfg_for_collision
        ...
        try:
            from clara_cli.config import load_config as _load_cfg_for_precedence
        ...
        # ``_CLARA_EGRESS_NODE_OPTIONS_APPEND`` and merge here.
        _egress_node_append = merged_env.pop(
            "_CLARA_EGRESS_NODE_OPTIONS_APPEND", None,
        )
        ...
        # s6-overlay images (e.g. clara-agent:latest) already use /init as PID 1
        ...
        container_name = f"clara-{uuid.uuid4().hex[:8]}"
        # Labels make clara-created containers identifiable to:
        #   * the orphan reaper (`clara-agent=1` for the global sweep filter)
        #   * future cross-process reuse (`clara-task-id`, `clara-profile`)
        #   * operators running `docker ps --filter label=clara-agent=1`
        ...
        label_args = [
            "--label", "clara-agent=1",
            "--label", f"clara-task-id={task_label}",
            "--label", f"clara-profile={profile_name}",
        ...
        self._labels = {
            "clara-agent": "1",
            "clara-task-id": task_label,
            "clara-profile": profile_name,
            _EGRESS_LABEL_KEY: egress_label,
        }

        # Cross-process container reuse (issue #20561 — docs claim "ONE long-lived
        # container shared across sessions").  If a prior Clara process
    ...
    def _resolve_passthrough_env(self) -> tuple[dict[str, str], set[str]]:
        ...
        # win over the generic Clara secret blocklist. Only implicit passthrough
        # keys are filtered. Also strip Clara-internal dynamic secrets
        # (AUXILIARY_*_API_KEY / _BASE_URL, GATEWAY_RELAY_* auth) that the
        # name-based blocklist doesn't cover — see _is_clara_internal_secret.
        _implicit_forward = {
            k for k in passthrough_keys if not _is_clara_internal_secret(k)
        }
        forward_keys = explicit_forward_keys | (_implicit_forward - _CLARA_PROVIDER_ENV_BLOCKLIST)
        clara_env = _load_clara_env_vars() if forward_keys else {}
        unset_names: set[str] = set()
        for key in sorted(forward_keys):
            value = os.getenv(key) or clara_env.get(key)
    ...
    def _recreate_container(self) -> bool:
        ...
        task_label = self._labels.get("clara-task-id", "")
        profile_label = self._labels.get("clara-profile", "")
        existing = self._find_reusable_container(
        ...
        if not self._container_id:
            ...
            try:
                ...
                new_name = f"clara-{_uuid.uuid4().hex[:8]}"
    ...
    def _find_reusable_container(
        ...
        matches containers that happened to be named ``clara-*`` but were
        started by some other tool.
        """
        try:
            filters = [
                "--filter", "label=clara-agent=1",
                "--filter", f"label=clara-task-id={task_label}",
                "--filter", f"label=clara-profile={profile_label}",
            ]
            if egress_label != "off":
                ...
                # clara-egress label is present and not "off".  Without
                # this, a container created with egress=on can be silently
                # reused after the operator runs "clara egress disable",
        ...
        # container) but can happen if a previous Clara process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Clara exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Clara process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next Clara startup. That covers the
        ...
        # outlives Clara processes, processes inside it stay alive, and
        ...
        import threading
        t = threading.Thread(target=_do_cleanup, daemon=True, name=f"clara-cleanup-{log_id}")
        t.start()
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``clara /quit``
