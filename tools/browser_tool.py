...
supports multiple backends — **Browser Use** (cloud, default for Clara
...
from clara_cli.config import DEFAULT_CONFIG, cfg_get
...
# means a compromised transitive dependency could read every Clara secret
...
def _build_browser_env() -> dict:
    ...
    Strips Clara-managed secrets (provider keys, gateway tokens, GitHub auth,
    infra secrets) then re-adds only the browser-backend keys the worker needs.
    The ``clara_subprocess_env`` import is deferred to keep ``browser_tool``
    ...
    env = clara_subprocess_env(inherit_credentials=False)
...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    clara_node_bin = str(clara_home / "node" / "bin")
    clara_node_root = str(clara_home / "node")
    clara_nm_bin = str(clara_home / "node_modules" / ".bin")
    return [clara_node_bin, clara_node_root, clara_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
def _get_cdp_override_raw() -> str:
    ...
    *whether* a CDP override is configured (tool ``check_fn`` gates,
    ...
    ``clara doctor``) MUST use this instead of :func:`_get_cdp_override`.
...
# plugins drop in under ``~/.clara/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    """Return the provider cached for the active Clara profile."""
...
def _resolve_cloud_provider_uncached() -> Optional[CloudBrowserProvider]:
    ...
    fallback. If unset, fall back to Browser Use (managed Clara gateway or
    ...
    browser plugins (``~/.clara/plugins/browser/<vendor>/``) participate
    ...
    try:
        ...
        if isinstance(browser_cfg, dict) and "cloud_provider" in browser_cfg:
            ...
            if provider_key in ("local", "camofox"):
                ...
                _cloud_provider_resolved = True
                return None
            if provider_key == "clara":
                # Managed "Clara Subscription" selection is serviced by the
    ...
    if resolved is None and provider_key is None:
        ...
        # was ever written: Browser Use first (managed Clara gateway or
...
from clara_constants import is_termux as _is_termux_environment
...
# comparison sites (four here, plus clara_cli/tools_config.py and
# clara_cli/doctor.py) from drifting if the sentinel's exact spelling ever
# changes.
...
def lightpanda_engine_status() -> Tuple[bool, str]:
    ...
    if bu_mode:
        return True, "Browser Use mode: Clara spawns `lightpanda serve` per session"
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Clara silently
...
def _run_chrome_fallback_command(
    ...
    # uses, not a bare shutil.which("npx") — Clara-managed-Node-only setups
    ...
    # Claim the dir before using it: another clara process's orphan reaper
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the Clara CLI where sys.stdout and sys.stderr both report
...
_REAL_PROFILE_SESSION = "clara-real-profile"
...
def _real_profile_cdp() -> tuple:
    ...
    Snapshots the user's default-Chromium profile into a clara-owned copy
    ...
    if not _use_real_profile():
        ...
        try:
            from clara_cli.browser_connect import cleanup_real_profile_snapshots
    ...
    with _real_profile_cdp_lock:
        ...
        # from a previous clara process; if it is driving OUR copy dir, hand it
        ...
        if err or not snap_dir:
            from clara_cli.browser_connect import _PROFILE_LOCKED_PREFIX
            ...
            if err and err.startswith(_PROFILE_LOCKED_PREFIX):
                ...
                return None, (
                    ...
                    "`clara browser close-profile`, then retry."
                )
        ...
        from clara_cli.browser_connect import chromium_executable
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-clara_…`` the resulting socket path exceeds the
...
# run exactly once, before the cleanup loop started, which meant a clara
...
# Hard ceiling for a daemon whose owning clara process is still alive but
...
# Owner Clara home per session (#86402).  The inactivity janitor is one
...
def _emergency_cleanup_all_sessions():
    ...
    crashed clara processes — this way every clean clara exit sweeps
    ...
    # Sweep orphans from other crashed clara processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live clara processes.
    try:
...
def _session_owner_scope(task_id: str):
    """Run under the Clara home + secret scope that owns ``task_id``'s
    ...
    home_token = set_clara_home_override(owner_home)
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current clara PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live clara process
...
def _socket_dir_idle_seconds(socket_dir: str) -> Optional[float]:
    ...
    clara restarts and does not depend on in-memory bookkeeping surviving an
    exception path.
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning clara process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced clara PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent clara instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-clara_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            ...
            # live clara process and must not be touched — but "owner alive"
        ...
        if not os.path.isfile(pid_file):
            ...
            # agent-browser writes its PID/owner files. Another Clara process
...
def _update_session_activity(task_id: str):
    ...
    Also records the owning Clara home on first sight so the process-global
    ...
    with _cleanup_lock:
        ...
        _session_owner_homes.setdefault(task_id, str(get_clara_home()))
...
def _create_local_session(task_id: str, allow_real_profile: bool = True) -> Dict[str, str]:
    ...
    # browser running on a clara-owned SNAPSHOT of their real profile —
    ...
    # ``browser.engine: lightpanda`` that endpoint is a Clara-spawned
...
def _resolve_npx_bin() -> Optional[str]:
    """Resolve a runnable npx binary, preferring the Clara-managed/Homebrew
    extended search over a bare ambient PATH lookup.

    Checking bare PATH first would let a broken or unrelated system npx
    shadow a healthy Clara-managed one with no recovery — every candidate
    is therefore validated with ``node_tool_runnable`` (the same check
    ``find_clara_node_executable`` uses to self-heal a managed Node tree)
...
def _find_agent_browser(*, validate: bool = True) -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Clara-managed
    ...
    # node_modules binary, which disappears on the next ``clara update`` and
    ...
    # Build an extended search PATH including Clara-managed Node, macOS
    ...
    try:
        ...
        if ensure_dependency("browser"):
            candidates = [
                ...
                shutil.which("agent-browser", path=str(get_clara_home() / "node_modules" / ".bin")),
                shutil.which("agent-browser", path=str(get_clara_home() / "node" / "bin")),
                shutil.which("agent-browser", path=str(get_clara_home() / "node")),
            ]
...
def warm_agent_browser_npx_cache(timeout: float = 60.0) -> bool:
    ...
    ``clara update`` (or ``clara doctor --fix``) warms npx's own cache
    ...
    provider/gateway credential Clara holds, while running registry-fetched
    npm code on every ``clara update`` (the GHSA-m4m8-xjp4-5rmm class of
    ...
    cmd = [
        ...
        "--ignore-scripts",
        # --prefer-offline: once cached, repeat `clara update`/`doctor
...
def _run_browser_command(
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this clara PID as the session owner (cross-process safe
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = clara_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise Clara
    ...
    from clara_constants import get_clara_dir
    screenshots_dir = get_clara_dir("cache/screenshots", "browser_screenshots")
...
def _cleanup_single_browser_session(task_id: str) -> None:
    ...
    if session_info:
        ...
        # A Lightpanda session is a process Clara spawned itself (Browser
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/claragram/clara-agent:latest")
                else:
