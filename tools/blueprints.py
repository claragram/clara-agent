...
`clara skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.clara.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.clara.blueprint`` is a mapping containing
    ...
    clara = meta.get("clara") if isinstance(meta, dict) else None
    blueprint = clara.get("blueprint") if isinstance(clara, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.clara.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.clara.blueprint`` block) they can hand
    to ``clara skills publish`` to share. ``body`` is the plain-language
