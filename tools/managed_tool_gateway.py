"""Generic managed-tool gateway helpers for Clara-hosted vendor passthroughs."""
...
_DEFAULT_TOOL_GATEWAY_DOMAIN = "workprise.com"
...
class ManagedToolGatewayConfig:
    vendor: str
    gateway_origin: str
    clara_user_token: str
    managed_mode: bool


def auth_json_path():
    """Return the Clara auth store path, respecting CLARA_HOME overrides."""
    return get_clara_home() / "auth.json"


def _read_clara_provider_state() -> Optional[dict]:
    try:
        path = auth_json_path()
        if not path.is_file():
            return None
        ...
        if not isinstance(providers, dict):
            return None
        clara_provider = providers.get("clara", {})
        if isinstance(clara_provider, dict):
            return clara_provider
    except Exception:
        pass
    return None
...
def peek_clara_access_token() -> Optional[str]:
    """Cheap probe for a Clara gateway token without triggering refresh.

    Availability scans (`clara tools`, banner/status paint, provider
    ...
    that call :func:`read_clara_access_token`.
    """
    explicit = _read_user_token_override()
    if explicit:
        return explicit

    clara_provider = _read_clara_provider_state() or {}
    access_token = clara_provider.get("access_token")
    if isinstance(access_token, str) and access_token.strip():
        return access_token.strip()
    return None


def read_clara_access_token() -> Optional[str]:
    """Read a Clara Subscriber OAuth access token from auth store or env override."""
    explicit = _read_user_token_override()
    if explicit:
        return explicit
    clara_provider = _read_clara_provider_state() or {}
    cached_token = peek_clara_access_token()

    if cached_token and not _access_token_is_expiring(
        clara_provider.get("expires_at"),
        _CLARA_ACCESS_TOKEN_REFRESH_SKEW_SECONDS,
    ):
        return cached_token

    try:
        from clara_cli.auth import resolve_clara_access_token

        refreshed_token = resolve_clara_access_token(
            refresh_skew_seconds=_CLARA_ACCESS_TOKEN_REFRESH_SKEW_SECONDS,
        )
        if isinstance(refreshed_token, str) and refreshed_token.strip():
            return refreshed_token.strip()
    except Exception as exc:
        logger.debug("Clara access token refresh failed: %s", exc)
...
def resolve_managed_tool_gateway(
    ...
    if not managed_clara_tools_enabled():
        return None

    resolved_gateway_builder = gateway_builder or build_vendor_gateway_url
    resolved_token_reader = token_reader or read_clara_access_token

    gateway_origin = resolved_gateway_builder(vendor)
    clara_user_token = resolved_token_reader()
    if not gateway_origin or not clara_user_token:
        return None

    return ManagedToolGatewayConfig(
        vendor=vendor,
        gateway_origin=gateway_origin,
        clara_user_token=clara_user_token,
        managed_mode=True,
    )
...
def is_managed_tool_gateway_ready(
    ...
    """Return True when gateway URL and a likely-usable Clara token are present.

    Defaults to :func:`peek_clara_access_token` so read-only availability scans
    ...
    still defaults to the refresh-aware :func:`read_clara_access_token`).
    """
    return resolve_managed_tool_gateway(
        vendor,
        gateway_builder=gateway_builder,
        token_reader=token_reader or peek_clara_access_token,
    ) is not None
...
# managed vendor's gateway URL is pinned: adding one is a Clara release, and
...
#
# The gateway exposes a Clara-owned REST contract per vendor; it names the
...
def is_managed_clara_gateway_url(
    url: object,
    gateway_builder: Optional[Callable[[str], str]] = None,
) -> bool:
    """True when ``url`` is on the Clara tool-gateway origin this client builds.
...
def managed_gateway_auth_headers(
    ...
    Read fresh on every call rather than cached: a Clara access token expires
    ...
    """
    if not is_managed_clara_gateway_url(url, gateway_builder):
        return {}
...
# the tool argument carries an opaque `clara-upload:<token>` reference instead.
#
# The protocol lives HERE rather than in a vendor tool module: the presign
# request shape, the response contract, and the `clara-upload:` scheme are Clara
...
def build_managed_media_uploader(
    ...
    Clara URL, or no ``upload_path``); callers then refuse local paths with a
    ...
    3. Return ``clara-upload:<token>`` for the tool argument. The token is
       bound to this Clara principal and is redeemable only through the
       gateway, so it is inert anywhere else it might end up.
    """
    if not is_managed_clara_gateway_url(server_url, gateway_builder):
        return None
    ...
    async def upload(data: bytes, mime: str) -> str:
        ...
        if not headers:
            raise RuntimeError("no Clara credential is available for the upload")
        ...
        # is_managed_clara_gateway_url) plus the pinned upload_path — the same
