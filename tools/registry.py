"""Central registry for all clara-agent tools.
...
def _discovery_cache_path() -> Optional[Path]:
    ...
    try:
        ...
        # import time (clara_constants itself is stdlib-only, so no cycle).
        from clara_constants import get_clara_home

        return Path(get_clara_home()) / "cache" / "tool_discovery_cache.json"
    except Exception:
        return None
...
# timescales. Cache results for ~30 s so env-var flips via ``clara tools``
...
def check_fn_cache_scope() -> Optional[str]:
    ...
    cache. A multiplex gateway installs a Clara-home override for every
    ...
    try:
        ...
        browser_identity = (
            get_session_env("CLARA_SESSION_ID", ""),
            get_session_env("CLARA_BROWSER_CONTROL_PRINCIPAL", ""),
            get_session_env("CLARA_BROWSER_CONTROL_TRANSPORT_FAMILY", ""),
        )
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``clara tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def __init__(self):
        ...
        # Plugin registrations are overlays keyed by resolved CLARA_HOME. A
    ...
    def _plugin_namespace_of_module(
        ...
        if module_namespace.startswith("clara_plugins."):
    ...
    def deregister(self, name: str, *, scope: Optional[str] = None) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # (``clara_plugins.{name}``), not the exact module string.
                # A handler defined in ``clara_plugins.pkg.handlers`` is
                # still owned by the ``clara_plugins.pkg`` package — exact
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``clara tools enable foo``)
