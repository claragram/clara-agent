...
Small Node helper that bridges Clara Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Clara is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Clara-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Clara can receive inbound messages
...
The Clara plugin's `clara photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Clara is Python, so replies go through this sidecar
