...
// `message.content.text` before Clara can see it. We rewrite the two inbound
...
const MARKER = "Clara patch: Preserve mixed text + attachment iMessage payloads";
...
export function patchSpectrumTs(root = scriptDir()) {
  ...
  for (const file of files) {
    ...
    // Clara to patch; keep the legacy v8 path below for older pinned installs.
    if (
