// Clara Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Clara): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Clara -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Clara-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `clara photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Clara can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "clara-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply Clara'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Clara allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Clara, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-clara-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Clara supplied them so a known-good
