# A2A — Agent-to-Agent protocol for Clara
...
peer (another Clara, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
clara gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Clara serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.clara/a2a_audit.jsonl`.
- Conversations persist to `~/.clara/a2a_conversations/` — they survive context
