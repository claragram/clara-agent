...
def _resolve_matrix_bang_command(name: str) -> str | None:
    """Resolve a ``!command`` token to a dispatchable Clara command token.

    Matrix clients often reserve leading ``/`` for local client commands.
    Clara accepts ``!command`` as a Matrix-friendly alias, but only for
...
def _normalize_matrix_bang_command(text: str) -> str:
    """Convert Matrix ``!command`` aliases to normal Clara ``/command`` text."""
...
# would pin the root CLARA_HOME for every profile and all bots' Olm
# identities would collide in one crypto.db (#89168). Mirrors the
# pairing-store fix (a6397c379).
from clara_constants import get_clara_dir as _get_clara_dir
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    # the adapter accepts "!command" as the alias that always reaches Clara
    ...
    def _resolve_store_dir(self) -> Path:
        ...
        ``_profile_runtime_scope`` -- ``get_clara_dir`` honors that
        context-local CLARA_HOME, so each profile's adapter gets its own
        ...
        """
        self._store_dir = _get_clara_dir("platforms/matrix/store", "matrix/store")
        return self._store_dir

    @property
    def _crypto_db_path(self) -> Path:
        store_dir = self._store_dir or _get_clara_dir("platforms/matrix/store", "matrix/store")
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(
            os.getenv("CLARA_MATRIX_TEXT_BATCH_DELAY_SECONDS", "0.6")
        )
        self._text_batch_split_delay_seconds = float(
            os.getenv("CLARA_MATRIX_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0")
        )
        ...
        if self._encryption:
            ...
            if self._encryption:
                try:
                    ...
                    _acct_id = self._user_id or "clara"
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        otherwise normal phrases like "Clara Agent" become "Agent".
        """
        if not body:
            return ""
...
# + _PLATFORMS["matrix"] static dict in clara_cli/{setup,gateway}.py, and the
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        txn_id = f"clara_{int(time.time() * 1000)}_{os.urandom(4).hex()}"
...
def interactive_setup() -> None:
    """Configure Matrix credentials. Replaces clara_cli/setup.py::_setup_matrix
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print_info("📬 Home Room: where Clara delivers cron job results and notifications.")
...
def _is_connected(config) -> bool:
    ...
    configured. Read via clara_cli.gateway.get_env_value so setup-status
