"""ntfy platform adapter (Clara plugin).
...
``plugins/platforms/ntfy/``. The Clara plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "clara-in"              # subscribe topic (incoming)
          publish_topic: "clara-out"     # optional — defaults to topic
...
_ECHO_TAG = "clara-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``clara cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Clara dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `clara gateway status` without
