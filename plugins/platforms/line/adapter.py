"""
LINE Messaging API platform adapter for Clara Agent.
...
to Clara Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``CLARA_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            clara_home = Path(get_clara_home()).resolve()
        except Exception:
            clara_home = Path.home().joinpath(".clara").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``clara status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``clara status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``clara setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.clara/.env`` via ``clara_cli.config``.
    """
    print()
    ...
    try:
        from clara_cli.config import get_env_value as _get_env, save_env_value as _set_env
    except ImportError:
        print("clara_cli.config not available; set LINE_* vars manually in ~/.clara/.env")
        return
