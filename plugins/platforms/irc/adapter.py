"""
IRC Platform Adapter for Clara Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Clara agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = _get_scoped_secret("IRC_NICKNAME") or extra.get("nickname", "clara-bot")
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Clara Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Clara Agent shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: clara_, clara_1, clara_2...
...
def check_requirements() -> bool:
    ...
    # clara setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `clara gateway setup` flow for the IRC platform.
    ...
    print_info("Connect Clara to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #clara — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.clara/.env")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = _get_scoped_secret("IRC_NICKNAME") or extra.get("nickname", "clara-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "clara-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Clara Agent (cron)")
