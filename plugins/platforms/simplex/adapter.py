"""SimpleX Chat platform adapter (Clara plugin).
...
This adapter ships as a Clara platform plugin under
``plugins/platforms/simplex/``. The Clara plugin loader scans the
...
Optional environment variables:
    ...
    CLARA_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``clara setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Clara — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("CLARA_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``clara setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.clara/.env`` via
    ``clara_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "clara_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.clara/.env"
        )
        return
