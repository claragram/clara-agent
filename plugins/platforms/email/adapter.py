"""
Email platform adapter for the Clara gateway.

Allows users to interact with Clara by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "clara-agent" "version" "{_clara_version}" '
            '"vendor" "Workprise" '
            '"support-email" "noreply@workprise.com")',
        )
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `clara config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `clara gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Clara Agent")
        ...
        msg_id = f"<clara-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# clara_cli/gateway.py, and the _send_email dispatch in
