...
Auto-tracks and cleans up ephemeral files created during Clara Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$CLARA_HOME` and `/tmp/clara-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `CLARA_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `CLARA_HOME` or `/tmp/clara-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$CLARA_HOME/disk-cleanup/` is itself excluded
- `$CLARA_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
