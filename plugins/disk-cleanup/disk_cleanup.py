"""disk_cleanup — ephemeral file cleanup for Clara Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under CLARA_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly CLARA_HOME and /tmp/clara-*
Never touches: ~/.clara/logs/ or any system directory.
"""
...
try:
    ...
    def get_clara_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("CLARA_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".clara").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$CLARA_HOME/logs/``."""
    return get_clara_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$CLARA_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under CLARA_HOME or ``/tmp/clara-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    clara_home = get_clara_home()
    try:
        path.resolve().relative_to(clara_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/clara-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("clara-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "clara-agent", "backups", "profiles", ".worktrees",
...
# Paths under $CLARA_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so CLARA_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        clara_home = get_clara_home()
        for parent in ("cron", "cronjobs"):
            base = clara_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside CLARA_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under CLARA_HOME, but never recurse into known
    # durable state trees.  Some installs place the Clara checkout, venv,
    # and desktop build under CLARA_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    clara_home = get_clara_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in clara_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    clara_home = get_clara_home()
    try:
        rel = path.resolve().relative_to(clara_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "clara-agent",
        ...
        # Path isn't under CLARA_HOME (e.g. /tmp/clara-*) — fall through.
        pass
