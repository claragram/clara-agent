...
Chronos (the Greek god of time, alongside Clara) is the first non-default
...
NAS's ``agent-cron`` endpoints with the agent's existing Clara token.
...
class ChronosCronScheduler(CronScheduler):
    ...
    def is_available(self) -> bool:
        ...
        callback URL (for NAS→agent fires), and a usable Clara token (the agent
        ...
        if not (_cfg("cron", "chronos", "portal_url") and _cfg("cron", "chronos", "callback_url")):
            return False
        return self._have_clara_token()

    def _have_clara_token(self) -> bool:
        """True if the agent has a Clara Portal login (no network call).

        Checks the stored auth state for a Clara access token — does NOT refresh
