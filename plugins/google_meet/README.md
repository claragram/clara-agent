...
Let the clara agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where clara runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `clara meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `clara meet` CLI |
...
| `cli.py` | `clara meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$CLARA_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `clara meet node {run,list,approve,remove,status,ping}` |
...
clara meet install                                      # pip + Chromium
clara meet setup                                        # preflight
clara meet auth                                         # optional
clara meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
clara meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.clara/.env
clara meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
clara meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
clara meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.clara/.env
clara meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, clara will **not** switch your system audio input automatically — the
...
clara plugins enable google_meet
clara meet node run --display-name my-mac --host 0.0.0.0 --port 18789
