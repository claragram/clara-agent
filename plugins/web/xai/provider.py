...
``{title, url, description, position}`` rows every other Clara web
provider produces.
...
prefers Clara-managed xAI Grok OAuth (via ``clara auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.clara/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``clara tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `clara auth` to sign in with "
