...
_DEFAULT_AGENT = ""
_OPENVIKING_USER_AGENT = f"openviking-memory-clara/{_CLARA_VERSION}"
...
_SYNC_TRACE_ENV = "CLARA_OPENVIKING_SYNC_TRACE"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip Clara slash-skill scaffolding before sending content to OpenViking.
...
def _normalize_openviking_url(url: str) -> str:
    ...
    # could attach Clara to an unrelated deployment and forward credentials to
    ...
    try:
        ...
        raise _OpenVikingEndpointError(
            "OpenViking endpoint safety validation failed; Clara refused the connection."
        ) from exc
...
def _load_clara_openviking_config() -> dict:
    try:
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("CLARA_HOME", "")).expanduser() if os.environ.get("CLARA_HOME") else Path.home() / ".clara"
...
def _start_local_openviking_server(endpoint: str) -> tuple[str, str]:
    ...
    if _local_openviking_port_is_open(host, port):
        ...
        return (
            ...
            f"Port {host}:{port} is occupied by {listener}. Clara did not start "
    ...
    try:
        ...
        # The Clara Desktop backend can include the Clara venv's
        # site-packages in PYTHONPATH. If inherited, openviking-server would
        # import aiohttp and friends from the Clara venv instead of its own
        # (its venv's site-packages are shadowed because PYTHONPATH precedes
        # them) —
        # and on Windows the loaded DLLs then lock the Clara venv,
        # aborting `clara update` with access-denied on .pyd files.
...
def _runtime_openviking_timeout_message(endpoint: str) -> str:
    return (
        ...
        "OpenViking memory is temporarily unavailable; Clara will retry on a later access or when "
        "the config changes."
    )
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Clara session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in Clara only", "write values only to Clara .env"),
    ...
    _save_clara_only_config(
        config=config,
        provider_config=provider_config,
        env_path=env_path,
        values=values,
    )
    _print_openviking_ready("Connection saved to Clara .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See clara-agent#28296 review.
    ...
    def is_available(self) -> bool:
        ...
        if os.environ.get("OPENVIKING_ENDPOINT"):
            return True
        provider_config = _load_clara_openviking_config()
    ...
    def save_config(self, values: Dict[str, Any], clara_home: str) -> None:
    ...
    def post_setup(self, clara_home: str, config: dict) -> None:
        ...
        from clara_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

        clara_home_path = Path(clara_home)
        env_path = clara_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        connection_error = ""
        try:
            settings = _resolve_connection_settings(_load_clara_openviking_config())
        except _OpenVikingEndpointError as exc:
        ...
        self._turn_count = 0
        clara_home = str(kwargs.get("clara_home") or "").strip()
        if not clara_home:
            try:
                from clara_constants import get_clara_home
                clara_home = str(get_clara_home())
            except Exception:
                clara_home = str(Path.home() / ".clara")
        self._clara_home = clara_home
            ...
            try:
                ...
                if health_state == "unreachable":
                    ...
                    _emit_runtime_warning(
                        ...
                        "Clara will retry on a later access or when the config changes.",
                        warning_callback,
                    )
                    self._client = None
            except ImportError:
    ...
    def _ensure_client(self) -> Optional["_VikingClient"]:
        ...
        ``~/.clara/.env`` after startup never reach the live client and tools
        keep running against stale auth until the user restarts clara (#21130).
    ...
    def _ensure_client_locked(self) -> Optional["_VikingClient"]:
        ...
        if health_state == "responded":
            logger.warning(
                "%s OpenViking memory is temporarily unavailable; Clara will retry on a "
    ...
    def _pending_session_dir(self) -> Optional[Path]:
        if not self._clara_home:
            return None
        return Path(self._clara_home) / _PENDING_SESSIONS_RELATIVE_DIR
    ...
    def _run_lock_dir(self) -> Optional[Path]:
        if not self._clara_home:
            return None
        return Path(self._clara_home) / _RUN_LOCKS_RELATIVE_DIR
    ...
    def _profile_token_budget(self) -> int:
        cfg = _load_clara_openviking_config()
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of Clara' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert Clara canonical messages into OpenViking batch payloads."""
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See clara-agent#28296.
    ...
    def _read_uri_payload(
        ...
        *,
        ...
        # (e.g. viking://user/clara/.overview.md).
    ...
    def _tool_remember(self, args: dict) -> str:
        ...
        session_id = f"clara-remember-{uuid.uuid4().hex[:12]}"
        ...
        recovery_note = (
            ...
            "Clara. Otherwise, do not resubmit automatically; report the uncertain "
            "state to the user."
        )
        ...
        # otherwise alter the live Clara conversation session.
        try:
