...
- OpenViking server running and reachable from Clara

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
Clara can identify older servers that expose the legacy status-only health
...
connection values into Clara, or create a minimal `ovcli.conf` when one does
not exist.
...
default profile that is `~/.clara/.env`; for a named profile use
...
OpenViking's server config is separate from Clara:
...
Clara-side provider config is read from environment variables in the active
profile's `.env`:
...
When `OPENVIKING_API_KEY` is set, Clara lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
Clara sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
Clara also sends `User-Agent: openviking-memory-clara/<version>` on
OpenViking requests. This standard harness identifier contains the Clara
...
does not ask for a peer ID. Without a configured peer, Clara sends neither
...
order remains environment, linked OpenViking config, then Clara YAML. To use
...
on the old implicit `clara` peer now use user memory for new writes. Without
...
Set `agent: clara` to restore peer-scoped writes. Memories written at user
...
`viking_remember` creates a one-shot `clara-remember-<random>` OpenViking
...
not advertised or used. The one-shot session is separate from the live Clara
...
Use the same OpenViking profile and credentials as Clara for manual recovery.
...
Clara built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| Clara action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under user memory, or the configured peer memory directory |

Built-in `replace` and `remove` operations are not mirrored because Clara
...
file URIs, such as
`viking://user/default/peers/clara/memories/preferences/mem_abc123.md` (any
