...
def _write_env(env_path: Path, env_writes: dict[str, str]) -> None:
    ...
    if env_path.exists():
        ...
        # clara_cli/config.py. read_text() with no encoding falls back to the
...
def _save_mem0_json(clara_home: str, data: dict) -> None:
    """Merge-write to mem0.json."""
    config_path = Path(clara_home) / "mem0.json"
    existing = {}
...
def _setup_platform(clara_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    schema = [
        ...
        {"key": "user_id", "description": "User identifier", "default": "clara-user"},
        {"key": "agent_id", "description": "Agent identifier", "default": "clara"},
    ...
    # docs tell self-hosted users to put MEM0_HOST in ~/.clara/.env. Warn
    ...
    if os.environ.get("MEM0_HOST", "").strip():
        print(
            ...
            "remove it from ~/.clara/.env (or unset it) or Clara will keep "
    ...
    provider.save_config(provider_config, clara_home)

    if env_writes:
        _write_env(Path(clara_home) / ".env", env_writes)
...
def _setup_selfhosted(clara_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    user_id = flags.get("user_id") or _prompt(
        "User identifier", default=provider_config.get("user_id") or "clara-user"
    )
    agent_id = _prompt("Agent identifier", default=provider_config.get("agent_id") or "clara")
...
def _setup_oss(clara_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    if not flags.get("_mode_from_flag"):
        _setup_oss_interactive(clara_home, config)
        return
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "clara-user")
    ...
    if env_writes:
        _write_env(Path(clara_home) / ".env", env_writes)
    _save_mem0_json(clara_home, {"mode": "oss", "user_id": user_id, "agent_id": "clara", "oss": oss_config})
...
def _prompt_api_key(label: str, env_var: str, clara_home: str) -> str:
    ...
    if not existing:
        ...
        if env_path.exists():
            ...
            # clara_cli/config.py; a Notepad BOM on the first line would
...
_PGVECTOR_CONTAINER = "clara-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "clara"
...
def _setup_oss_interactive(clara_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], clara_home)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(f"{embedder_def['label']} embedder", embedder_def["env_var"], clara_home)
        if key:
    ...
    user_id = input(f"  User ID [{os.getenv('USER', 'clara-user')}]: ").strip()
    user_id = user_id or os.getenv("USER", "clara-user")

    agent_id = input("  Agent ID [clara]: ").strip()
    agent_id = agent_id or "clara"
    ...
    if env_writes:
        _write_env(Path(clara_home) / ".env", env_writes)
    _save_mem0_json(clara_home, {"mode": "oss", "user_id": user_id, "agent_id": agent_id, "oss": oss_config})
...
def post_setup(clara_home: str, config: dict) -> None:
    """Entry point called by clara memory setup framework.
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(clara_home, config, flags)
        return

    if flags["mode"] in ("selfhosted", "self-hosted"):
        _setup_selfhosted(clara_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(clara_home, config, flags)
        return
