...
```bash
clara memory setup    # select "mem0"
```
...
Behavioral settings live in `$CLARA_HOME/mem0.json` (set them via `clara memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.clara/.env`.
...
| `user_id` | `clara-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Clara session and call `mem0_search` — it connects to your server.
...
```bash
clara memory setup mem0 --mode oss \
...
```bash
clara memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$CLARA_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.clara/mem0_qdrant"}}
  }
}
```
...
```bash
clara memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$CLARA_HOME/mem0.json`).
