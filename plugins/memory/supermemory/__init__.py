...
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=clara"
...
def _load_supermemory_config(clara_home: str) -> dict:
    config = _default_config()
    config_path = Path(clara_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, clara_home: str) -> None:
    config_path = Path(clara_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str,
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "clara"},
        )

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes Clara writes into the "Clara" Space in the Supermemory
        ...
        merged = {"sm_source": "clara", **(metadata or {})}
...
def _resolve_container_tag_for_setup(clara_home: str, *, identity: str = "default") -> str:
    config = _load_supermemory_config(clara_home)
...
def _probe_supermemory_connection(api_key: str, clara_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(clara_home)
    base_url = _resolve_base_url(config["base_url"])
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(clara_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `clara memory setup`.
        # All other options are documented for $CLARA_HOME/supermemory.json
    ...
    def save_config(self, values, clara_home):
        ...
        _save_supermemory_config(sanitized, clara_home)
    ...
    def get_status_config(self, provider_config: dict) -> dict:
        ...
        status = _probe_supermemory_connection(api_key, clara_home)
    ...
    def post_setup(self, clara_home: str, config: dict) -> None:
        ...
        from clara_cli.memory_setup import _prompt, _write_env_vars
        ...
        if env_writes:
            _write_env_vars(env_writes, clara_home=clara_home)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from clara_constants import get_clara_home
        self._clara_home = kwargs.get("clara_home") or str(get_clara_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._clara_home)
