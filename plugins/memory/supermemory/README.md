...
- Hosted: API key from [app.supermemory.ai/integrations?connect=clara](http://app.supermemory.ai/integrations?connect=clara)
...
Before running `clara memory setup`, add the local endpoint to
...
Then run `clara memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$CLARA_HOME/supermemory.json`
...
| `container_tag` | `clara` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `clara-{identity}` → `clara-coder`). |
...
`https://api.supermemory.ai`. Clara resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: clara`, and document writes stamp
`metadata.sm_source: clara`. This is a **functional routing key, not telemetry**:
it groups Clara-written memories into a dedicated "Clara" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Clara profile:

```json
{
  "container_tag": "clara-{identity}"
}
```

For a profile named `coder`, this resolves to `clara-coder`. The default profile resolves to `clara-default`. Without `{identity}`, all profiles share the same container.
