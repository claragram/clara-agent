"""clara-memory-store — holographic memory plugin using MemoryProvider interface.
...
Config in $CLARA_HOME/config.yaml (profile-scoped):
  plugins:
    clara-memory-store:
      db_path: $CLARA_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
def _load_plugin_config() -> dict:
    try:
        ...
        return cfg_get(all_config, "plugins", "clara-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, clara_home):
        """Write config to config.yaml under plugins.clara-memory-store."""
        from pathlib import Path
        config_path = Path(clara_home) / "config.yaml"
        try:
            import yaml
            ...
            existing["plugins"]["clara-memory-store"] = values
    ...
    def get_config_schema(self):
        ...
        _default_db = f"{display_clara_home()}/memory_store.db"
        return [
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _clara_home = str(get_clara_home())
        _default_db = _clara_home + "/memory_store.db"
        db_path = self._config.get("db_path", _default_db)
        # Expand $CLARA_HOME in user-supplied paths so config values like
        # "$CLARA_HOME/memory_store.db" or "~/.clara/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$CLARA_HOME", _clara_home)
            db_path = db_path.replace("${CLARA_HOME}", _clara_home)
