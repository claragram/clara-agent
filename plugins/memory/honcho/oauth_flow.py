...
``clara://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Clara OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "clara-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``clara-desktop`` / ``clara-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Clara.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local CLARA_HOME override can't reach.
