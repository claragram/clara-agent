...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/clara>
...
```bash
clara memory setup honcho   # configure Honcho directly (works on a fresh install)
clara memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
clara config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.clara/.env
```

> `clara honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `clara memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `clara model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$CLARA_HOME/honcho.json` | Profile-local (isolated Clara instances) |
| 2 | `~/.clara/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Clara profile: `clara` (default) or `clara_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `clara honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Clara session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`clara-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`clara`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-clara-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening clara in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Clara session ID (timestamp + hex). Every `clara` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Clara profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`clara`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Clara profile: `clara` (default) or `clara_<profile>` (e.g. `clara -p coder` -> host key `clara_coder`). Older `clara.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `CLARA_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `clara-agent`) |
...
|---------|-------------|
| `clara memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `clara honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `clara memory setup`) |
| `clara honcho status` | Show resolved config for active profile |
| `clara honcho enable` / `disable` | Toggle Honcho for active profile |
| `clara honcho mode <mode>` | Change recall or observation mode |
| `clara honcho peer --user <name>` | Update user peer name |
| `clara honcho peer --ai <name>` | Update AI peer name |
| `clara honcho tokens --context <N>` | Set context token budget |
| `clara honcho tokens --dialectic <N>` | Set dialectic max chars |
| `clara honcho map <name>` | Map current directory to a session name |
| `clara honcho sync` | Create host blocks for all Clara profiles |
