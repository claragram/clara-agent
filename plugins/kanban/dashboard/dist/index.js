/**
 * Clara Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.clara/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses (window.__CLARA_PLUGIN_SDK__ || window.__HERMES_PLUGIN_SDK__) for React +
...
(function () {
  ...
  const COLUMN_DOT = {
    triage: "clara-kanban-dot-triage",
    todo: "clara-kanban-dot-todo",
    ready: "clara-kanban-dot-ready",
    running: "clara-kanban-dot-running",
    blocked: "clara-kanban-dot-blocked",
    review: "clara-kanban-dot-review",
    done: "clara-kanban-dot-done",
    archived: "clara-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-clara-task";
  ...
  const DOCS_URL = "https://agent.claragram.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://agent.claragram.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "clara.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="clara-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])clara-kanban-md-code\1/i.test(attrs)) {
      return ' class="clara-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "clara-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "clara-kanban-md",
  ...
  // standard `drop` event and our `clara-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("clara-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("clara-kanban-column--drop");
          if (target) target.classList.add("clara-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("clara-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("clara-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("clara-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading (window.__CLARA_SESSION_TOKEN__ || window.__HERMES_SESSION_TOKEN__) directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "clara-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "clara-kanban-attention",
        "clara-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "clara-kanban-attention-bar" },
        h("span", { className: "clara-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "clara-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "clara-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "clara-kanban-attention-row",
                  "clara-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "clara-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "clara-kanban-attention-row-id" }, task.id),
                h("span", { className: "clara-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "clara-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "clara-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "clara-kanban-diag-action-btn",
      action.suggested ? "clara-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " clara-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".clara-kanban-drawer-comment-row input, .clara-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "clara-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("clara-kanban-diag", sevClass) },
      h("div", { className: "clara-kanban-diag-header" },
        h("span", { className: "clara-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "clara-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "clara-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "clara-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "clara-kanban-diag-data-row" },
                  h("span", { className: "clara-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "clara-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "clara-kanban-diag-data-row" },
                h("span", { className: "clara-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "clara-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "clara-kanban-diag-reassign-row" },
            h("span", { className: "clara-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "clara-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "clara-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "clara-kanban-diag-msg",
              msg.ok ? "clara-kanban-diag-msg--ok" : "clara-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "clara-kanban-section" },
      h("div", { className: "clara-kanban-section-head-row" },
        h("span", { className: "clara-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "clara-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "clara-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "clara-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "clara-kanban-docs-link",
      title: "Open Clara Kanban docs in a new tab",
      "aria-label": "Clara Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `clara kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "clara-kanban-msg-ok" : "clara-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "clara-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "clara-kanban-boardswitcher" },
      h("div", { className: "clara-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "clara-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "clara-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "clara-kanban-dialog-title" },
        ...
        h("div", { className: "clara-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Clara profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "clara-kanban-bulk" },
      h("span", { className: "clara-kanban-bulk-count" },
      ...
      h("div", { className: "clara-kanban-bulk-priority",
      ...
      h("div", { className: "clara-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Clara profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "clara-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("clara-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("clara-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "clara-kanban-trash",
        dragOver ? "clara-kanban-trash--drop" : "",
        props.draggingTaskId ? "clara-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "clara-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "clara-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".clara-kanban-card")) return true;
      if (target.closest && target.closest(".clara-kanban-column-add")) return true;
      if (target.closest && target.closest(".clara-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("clara-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("clara-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".clara-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "clara-kanban-columns",
        isScrollable ? "clara-kanban-columns--scrollable" : "",
        isPanning ? "clara-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("clara-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("clara-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "clara-kanban-column",
        dragOver ? "clara-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "clara-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "clara-kanban-col-check",
        ...
        h("span", { className: cn("clara-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "clara-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "clara-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "clara-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "clara-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "clara-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "clara-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "clara-kanban-lane" },
                  h("div", { className: "clara-kanban-lane-head" },
                    h("span", { className: "clara-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "clara-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "clara-kanban-card--stale-red";
    if (age >= tier.amber) return "clara-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".clara-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "clara-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "clara-kanban-card",
        props.selected ? "clara-kanban-card--selected" : "",
        props.failed ? "clara-kanban-card--failed" : "",
        props.draggingSource ? "clara-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "clara-kanban-card-content" },
          h("div", { className: "clara-kanban-card-row" },
            h("label", {
              className: "clara-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "clara-kanban-card-check",
            ...
            h("span", { className: "clara-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or clara kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "clara-kanban-warning-badge",
                    "clara-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "clara-kanban-needs-assignee",
          ...
          h("div", { className: "clara-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "clara-kanban-card-row clara-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "clara-kanban-assignee",
                            title: `Assigned to Clara profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "clara-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "clara-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "clara-kanban-ago",
  ...
  // centered modal reusing the clara-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "clara-kanban-dialog clara-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "Clara profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "Clara profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "clara-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "clara-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "clara-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "clara-kanban-drawer-close",
        ...
        data ? h("div", { className: "clara-kanban-drawer-comment-foot" },
          h("div", {
            className: "clara-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "clara-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "clara-kanban-section" },
      h("div", { className: "clara-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "clara-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "clara-kanban-drawer-body" },
      h("div", { className: "clara-kanban-drawer-title" },
        h("span", { className: cn("clara-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "clara-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "clara-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "clara-kanban-section" },
            h("div", { className: "clara-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "clara-kanban-section" },
            h("div", { className: "clara-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "clara-kanban-done-no-result clara-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "clara-kanban-section" },
            h("div", { className: "clara-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "clara-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "clara-kanban-section" },
        h("div", { className: "clara-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "clara-kanban-comment" },
            h("div", { className: "clara-kanban-comment-head" },
              h("span", { className: "clara-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "clara-kanban-diag-action-btn",
      ...
      h("div", { className: "clara-kanban-section" },
        h("div", { className: "clara-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "clara-kanban-comment" },
                h("div", { className: "clara-kanban-comment-head" },
                  h("span", { className: "clara-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "clara-kanban-comment-ago" },
      ...
      h("div", { className: "clara-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "clara-kanban-event",
              isDiag ? "clara-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "clara-kanban-event-header" },
                  h("span", { className: "clara-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "clara-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "clara-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "clara-kanban-event-header-plain" },
                  h("span", { className: "clara-kanban-event-kind" }, e.kind),
                  h("span", { className: "clara-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "clara-kanban-event-phantom-row" },
                  h("span", { className: "clara-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "clara-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "clara-kanban-event-payload" },
    ...
    return h("div", { className: "clara-kanban-section" },
      h("div", { className: "clara-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "clara-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `clara-kanban-run--${r.outcome || r.status || "ended"}`
          : "clara-kanban-run--active";
        return h("div", { key: r.id, className: cn("clara-kanban-run", outcomeClass) },
          h("div", { className: "clara-kanban-run-head" },
            h("span", { className: "clara-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "clara-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "clara-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "clara-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "clara-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "clara-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "clara-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "clara-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "clara-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "clara-kanban-pre clara-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "clara-kanban-meta-row" },
      h("span", { className: "clara-kanban-meta-label" }, props.label),
      h("span", { className: "clara-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "clara-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "clara-kanban-meta-row" },
        h("span", { className: "clara-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "clara-kanban-meta-value clara-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "clara-kanban-meta-row" },
        h("span", { className: "clara-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "clara-kanban-meta-value clara-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function ModelEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "clara-kanban-meta-row" },
        h("span", { className: "clara-kanban-meta-label" }, tx(t, "model", "Model")),
        h("span", {
          className: cn(
            "clara-kanban-meta-value clara-kanban-editable",
    ...
    return h("div", { className: "clara-kanban-meta-row" },
      h("span", { className: "clara-kanban-meta-label" }, tx(t, "model", "Model")),
      loading
        ? h("span", { className: "clara-kanban-meta-value text-muted-foreground" },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "clara-kanban-section" },
      h("div", { className: "clara-kanban-section-head-row" },
        h("span", { className: "clara-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "clara-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "clara-kanban-section" },
      h("div", { className: "clara-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "clara-kanban-deps-row" },
        h("span", { className: "clara-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "clara-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "clara-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "clara-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "clara-kanban-dep-chip-x",
      ...
      h("div", { className: "clara-kanban-deps-row" },
        h("span", { className: "clara-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "clara-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "clara-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "clara-kanban-section" },
      ...
      h("div", { className: "clara-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "clara-kanban-home-sub clara-kanban-home-sub--on"
              : "clara-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if ((window.__CLARA_PLUGINS__ || window.__HERMES_PLUGINS__) && typeof (window.__CLARA_PLUGINS__ || window.__HERMES_PLUGINS__).register === "function") {
    (window.__CLARA_PLUGINS__ || window.__HERMES_PLUGINS__).register("kanban", KanbanPage);
  }
})();
