...
class ClaraProfile(ProviderProfile):
    """Clara Portal — product tags, reasoning with Clara-specific omission."""

    def resolve_aux_model(self, *, vision: bool = False) -> str:
        """Ask the Portal which cheap model it currently recommends.

        ``/api/clara/recommended-models`` is the authoritative, tier-aware
        source (free vs paid), so the auxiliary fast tier tracks the live
        catalog instead of a hardcoded id that 404s the day Clara retires it.
        ...
        try:
            ...
            return get_clara_recommended_aux_model(vision=vision) or ""
        except Exception:
            return ""
    ...
    def build_extra_body(
        ...
        body: dict[str, Any] = {"tags": clara_portal_tags(session_id=session_id)}
        ...
        #
        # Resolve it exactly like ``clara_portal_tags`` resolves the
    ...
    def _cannot_disable_reasoning(model: str | None) -> bool:
        ...
        try:
            ...
            caps = clara_model_reasoning_capabilities(model)
            if caps is None:
                warm_clara_reasoning_caps_async()
                return True
        except Exception:
    ...
    def build_api_kwargs_extras(
        ...
        **context,
        ...
        """Clara: passes the full reasoning_config, disable included.
