"""Meta Model API (Muse Spark) provider plugin for Clara Agent.
...
Bundled from https://github.com/albertodepaola/clara-meta-provider. Clara'
...
* **Zero core edits.** Everything rides on ``ProviderProfile`` hooks. No changes
  to clara' ``model_metadata.py`` / ``models.py`` / ``run_agent.py`` are needed:
...
def _resolve_effort(reasoning_config: dict | None) -> str:
    """Map Clara' reasoning_config to a Meta-safe ``reasoning_effort`` value.
...
meta_ai = MetaAIProfile(
    ...
    # in clara_cli/providers.py and the retention hint in
