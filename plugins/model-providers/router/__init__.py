"""Ramp Router (router.com) provider plugin for Clara Agent.
...
* **Responses API is the native wire.** Router serves ``GET /v1/models``
  ...
  in ``clara_cli/providers.py`` keep every path on the native wire —
...
``clara_cli/models.py``: cache-only lookups on the per-request hot path
...
def _resolve_api_key() -> str:
    ...
    raw environment when the clara_cli helper is unavailable (e.g. stripped
    test environments).
    """
    resolvers = []
    try:
...
def _disk_path() -> Optional[Path]:
    try:
        ...
        return get_clara_home() / "cache" / "router_catalog.json"
    except Exception:
        return None
...
def _warm_efforts_async() -> None:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        # Match the canonical caps warmer (clara_cli/models.py): a mid-suite
...
router = RouterProfile(
    ...
    base_url=_base_url(),
    auth_type="api_key",
    # Identify Clara traffic to the gateway (Router attributes coding-agent
    ...
    default_headers={"User-Agent": f"Clara-Agent/{_CLARA_VERSION}"},
