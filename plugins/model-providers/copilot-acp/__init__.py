...
``~/.clara/plugins/model-providers/`` or a pip entry point uses the exact same
...
copilot_acp = CopilotACPProfile(
    ...
    # clara_cli/auth.py::resolve_external_process_provider_credentials; the env
    # var names are unchanged, so existing setups keep working.
    process_command="copilot",
    process_args=("--acp", "--stdio"),
    process_command_env_vars=("CLARA_COPILOT_ACP_COMMAND", "COPILOT_CLI_PATH"),
    process_args_env_var="CLARA_COPILOT_ACP_ARGS",
)
