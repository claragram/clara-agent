...
request omits ``thinking``.  Clara' ``reasoning_config = {"enabled": False}``
...
:meth:`ZaiProfile.build_api_kwargs_extras` translates the Clara reasoning
...
(per Z.AI / BigModel docs).  Clara' richer effort scale is collapsed onto
...
def _glm_5_2_reasoning_effort(
    ...
    """Map Clara reasoning effort onto GLM's native vocabulary.
