...
``<clara home>/plugins/<name>/``. That tree is the plugin *install* dir —
``clara plugins remove`` deletes it and ``clara plugins update`` git-pulls
...
``<clara home>/plugin-data/<name>/``, owned by the user, untouched by
...
through ``agent.secret_scope`` / ``.env`` like everywhere else in Clara.
...
# Mirrors the plugin-name shape `clara plugins install` accepts. Anything
...
def plugin_data_dir(name: str) -> Path:
    ...
    ``<clara home>/plugin-data/<name>/`` — survives plugin update and
    removal, and follows the active profile because it resolves through
    :func:`clara_constants.get_clara_home` on every call. Don't cache the
    result across profile switches.
    """
    from clara_constants import get_clara_home

    root = get_clara_home() / "plugin-data" / _validate_name(name)
