...
three virtual model IDs so the ``clara tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `clara tools` → Image "
                    "Generation → OpenAI to configure, or `clara setup` "
                    "to add the key."
                ),
