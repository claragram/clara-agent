"""langfuse — Clara plugin for Langfuse observability.

Traces Clara conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the Clara plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``clara plugins enable
observability/langfuse`` or ``clara tools → Langfuse Observability``). At
runtime the plugin also requires the ``langfuse`` SDK and credentials; if
either is missing the hooks are inert.

Required env vars (set via ``clara tools`` or ~/.clara/.env):
  CLARA_LANGFUSE_PUBLIC_KEY  - Langfuse project public key (pk-lf-...)
  CLARA_LANGFUSE_SECRET_KEY  - Langfuse project secret key (sk-lf-...)
  CLARA_LANGFUSE_BASE_URL    - Langfuse server URL (default: https://cloud.langfuse.com)

Optional env vars:
  CLARA_LANGFUSE_ENV         - environment tag (e.g. "production", "local")
  CLARA_LANGFUSE_RELEASE     - release/version tag
  CLARA_LANGFUSE_SAMPLE_RATE - sampling rate 0.0–1.0 (default: 1.0)
  CLARA_LANGFUSE_MAX_CHARS   - max chars per field (default: 12000)
  CLARA_LANGFUSE_CAPTURE     - content capture mode (default: "sanitized")
  ...
  CLARA_LANGFUSE_DEBUG       - set to "true" for verbose logging
"""
...
def _debug_enabled() -> bool:
    return _env_bool("CLARA_LANGFUSE_DEBUG")
...
def _capture_mode() -> str:
    ...
    value = _env("CLARA_LANGFUSE_CAPTURE").lower()
    if not value:
    ...
    if not _warned_invalid_capture:
        ...
        logger.warning(
            "Langfuse plugin: invalid CLARA_LANGFUSE_CAPTURE=%r, falling back "
            "to %r (valid: %s)",
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the Clara plugin system —
    ...
    with _LANGFUSE_CLIENT_LOCK:
        ...
        if Langfuse is None:
            logger.warning(
                ...
                "tracing is disabled. Run `clara tools` and configure Langfuse "
                "Observability to reinstall it."
            )
            _LANGFUSE_CLIENT = _INIT_FAILED
            return None

        public_key = _env("CLARA_LANGFUSE_PUBLIC_KEY") or _env("LANGFUSE_PUBLIC_KEY")
        secret_key = _env("CLARA_LANGFUSE_SECRET_KEY") or _env("LANGFUSE_SECRET_KEY")
        ...
        placeholder_issues = [
            msg
            for msg in (
                _validate_langfuse_key("CLARA_LANGFUSE_PUBLIC_KEY", public_key),
                _validate_langfuse_key("CLARA_LANGFUSE_SECRET_KEY", secret_key),
            )
            if msg
        ]
        if placeholder_issues:
            logger.warning(
                ...
                "or unset CLARA_LANGFUSE_PUBLIC_KEY / CLARA_LANGFUSE_SECRET_KEY to "
        ...
        base_url = _env("CLARA_LANGFUSE_BASE_URL") or _env("LANGFUSE_BASE_URL") or "https://cloud.langfuse.com"
        environment = _env("CLARA_LANGFUSE_ENV") or _env("LANGFUSE_ENV")
        release = _env("CLARA_LANGFUSE_RELEASE") or _env("LANGFUSE_RELEASE")
        sample_rate = _env("CLARA_LANGFUSE_SAMPLE_RATE")
        ...
        if sample_rate:
            try:
                ...
                logger.warning("Invalid CLARA_LANGFUSE_SAMPLE_RATE=%r", sample_rate)
        ...
        # short-lived-process gap (kanban workers / clara chat -q / cron): exit
...
def _trace_key(
    ...
    *,
    ...
    Older Clara paths only expose ``task_id``/``session_id``. Newer paths
...
def _safe_value(value: Any, *, max_chars: Optional[int] = None, depth: int = 0,
    ...
    max_chars = max_chars if max_chars is not None else int(_env("CLARA_LANGFUSE_MAX_CHARS", "12000") or "12000")
    if depth > 4:
        return "<max-depth>"
...
def _canonical_usage_and_cost(
    ...
    *,
    ...
    """Translate canonical Clara usage into Langfuse usage and cost maps."""
    ...
    # Clara estimate rather than summing components because it also includes
...
def _finalize_all_traces() -> None:
    ...
    kanban workers, ``clara chat -q`` one-shots, cron jobs — can exit while
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older Clara branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current Clara also has a turn-scoped
    ...
    # messages is a list only for legacy Clara branches that fired
    # pre_llm_call with API messages directly. Current Clara fires
...
def register(ctx) -> None:
    ...
    # Clara versions.  pre_api_request / post_api_request fire per API
