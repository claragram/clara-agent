...
Clara Agent (see website/docs/getting-started/platform-support.md). The
...
build sandbox. ``nix/python.nix`` sets ``CLARA_NIX_BUILD=1`` on the
Clara package derivation, so only that build may create an artifact.
...
_IN_NIX_BUILD = os.environ.get("CLARA_NIX_BUILD") == "1"
...
_BLOCK_MESSAGE = (
    ...
    "See: https://agent.claraprise.com/docs/getting-started/installation\n"
    "\n"
    "If you are developing, use an editable install instead:\n"
    "  uv sync          # or: uv pip install -e .\n"
    "\n"
    "If you are building with Nix (uv2nix), this error should not fire —\n"
    "the Clara Nix derivation sets CLARA_NIX_BUILD=1. If it does, file a bug."
)
