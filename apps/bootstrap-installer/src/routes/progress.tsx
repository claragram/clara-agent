...
export default function ProgressScreen({ bootstrap }: ProgressProps) {
  ...
  const title = bootstrap.status === 'completed' ? 'Done' : isUpdate ? 'Updating Clara' : 'Setting up Clara Agent'

  const description = isUpdate
    ? 'Clara is updating to the latest version — this only takes a moment.'
  ...
  return (
    <div className="clara-fade-in flex h-full flex-col">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {showLogs && (
          <div className="flex w-1/2 flex-col border-l border-(--stroke-clara)">
            <div className="flex shrink-0 items-center justify-between border-b border-(--stroke-clara) px-3 py-2 text-xs">
      ...
      <div className="flex shrink-0 items-center justify-between border-t border-(--stroke-clara) px-6 py-3">
        <button
