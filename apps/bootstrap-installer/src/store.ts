...
export const $claraHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.clara/logs/bootstrap-installer.log')
    $claraHome.set('~/.clara')
  ...
  try {
    const [logPath, claraHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_clara_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $claraHome.set(claraHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Clara" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Clara-Setup.exe --update);
...
export async function launchClaraDesktop(): Promise<void> {
  ...
  await invoke('launch_clara_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Clara repository' },
