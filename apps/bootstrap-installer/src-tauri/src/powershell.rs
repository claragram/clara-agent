...
/// read side does not see EOF until the last of them closes it. `clara update`
...
/// stranded with it.
///
/// Same bound `Invoke-ClaraStep` grew in `scripts/desktop-update/windows.ps1`
...
///
/// `clara_home_override` propagates to the child as $CLARA_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, clara_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = clara_home_override {
        cmd.env("CLARA_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, clara_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = clara_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_clara_home() {
    ...
    /// child. `clara update` is this shape -- the Electron build alone is
    /// megabytes.
    #[cfg(unix)]
    #[tokio::test]
