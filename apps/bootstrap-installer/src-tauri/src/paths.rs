...
//!
//! Mirrors `clara_constants.get_clara_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\clara
//!   macOS:   ~/.clara
//!   Linux:   ~/.clara  (override via $CLARA_HOME)
//!
//! NOTE (macOS): Python's get_clara_home(), scripts/install.sh, and the
//! Electron desktop's resolveClaraHome() ALL use ~/.clara on macOS — there
...
/// Returns the canonical Clara home directory, respecting $CLARA_HOME if set.
pub fn clara_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("CLARA_HOME") {
    ...
    {
        // %LOCALAPPDATA%\clara — matches scripts/install.ps1's $ClaraHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("clara");
        }
    }

    // macOS + Linux + fallback: ~/.clara (matches Python get_clara_home(),
    // install.sh, and the Electron desktop's resolveClaraHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".clara");
    }
...
pub fn bootstrap_cache_dir() -> PathBuf {
    clara_home().join("bootstrap-cache")
}
...
/// CLARA_HOME so it survives repo checkout deletion (unlike anything under
/// clara-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\clara\clara-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_clara`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under CLARA_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves CLARA_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    clara_home().join(".clara-update-in-progress")
}
...
pub fn copy_self_to_clara_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to CLARA_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_CLARA_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".clara-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under CLARA_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[clara-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("CLARA_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_clara_home() -> String {
    clara_home().to_string_lossy().into_owned()
}
