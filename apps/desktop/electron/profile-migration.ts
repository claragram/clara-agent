...
export interface MigrationDeps {
  ...
  /** Default profile home (`~/.clara`). Default's state.db and gateway.pid live here. */
  claraHome: string
  /** Named-profile root (`~/.clara/profiles`). Does not contain `default`. */
  profilesRoot: string
  ...
  isClaraProcess: (pid: number) => boolean
  now: () => number
...
/**
 * Production layout: default IS `claraHome`; named profiles are children of
 * `profilesRoot`. There is no `profiles/default` directory on a normal install.
 */
export function profileStateDbPath(name: string, claraHome: string, profilesRoot: string): string {
  return name === 'default' ? `${claraHome}/state.db` : `${profilesRoot}/${name}/state.db`
}

export function profileGatewayPidPath(name: string, claraHome: string, profilesRoot: string): string {
  return name === 'default' ? `${claraHome}/gateway.pid` : `${profilesRoot}/${name}/gateway.pid`
}

function resolveClaraHome(profilesRoot: string, claraHome?: string): string {
  if (claraHome) {
    return claraHome
  }

  // Tests that predate claraHome pass only profilesRoot.
...
/**
 * Return the profile names whose gateway.pid file points to a live clara process.
 ...
 *
 * `claraHome` is optional so existing call sites that only pass `profilesRoot`
 * still work: it is derived as the parent of `…/profiles`.
 */
export function findRunningGatewayProfiles(
  profilesRoot: string,
  allProfiles: string[],
  deps: Pick<MigrationDeps, 'existsSync' | 'readFileSync' | 'isClaraProcess'> & { claraHome?: string }
): string[] {
  const claraHome = resolveClaraHome(profilesRoot, deps.claraHome)
  const running: string[] = []

  for (const name of allProfiles) {
    const pidFile = profileGatewayPidPath(name, claraHome, profilesRoot)
...
export function decideMigration(
  ...
  for (const name of candidates) {
    const s = score(profileStateDbPath(name, deps.claraHome, deps.profilesRoot))
...
/** Default is always a candidate; it is `$CLARA_HOME`, not `$CLARA_HOME/profiles/default`. */
...
export function migrateActiveProfileIfMissing(desktopProfileConfigPath: string, deps: MigrationDeps): boolean {
  ...
  // launches `clara --profile default` and is worse than writing nothing
