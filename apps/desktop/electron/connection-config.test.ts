...
test('profileRemoteOverride treats a cloud entry as a remote override', () => {
  ...
  const config = {
    profiles: {
      coder: { mode: 'cloud', url: 'https://agent-1.agents.claraship.com', authMode: 'oauth' }
    }
  }

  assert.deepEqual(profileRemoteOverride(config, 'coder'), {
    url: 'https://agent-1.agents.claraship.com',
    authMode: 'oauth',
    token: undefined
  })
})
...
test('OAuth ticket-mint 503 surfaces the Cloud-down error (startup boundary)', () => {
  const baseUrl = 'https://ares-3009.agents.claraship.com'
  ...
  if (cloudError !== null) {
    ...
    assert.ok(cloudError.message.includes('Clara Cloud agent ares-3009.agents.claraship.com is down'))
