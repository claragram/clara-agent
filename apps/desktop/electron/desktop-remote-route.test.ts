...
test('environment URL without its token keeps the existing error', () => {
  assert.throws(
    ...
    /CLARA_DESKTOP_REMOTE_TOKEN is not/
  )
})
...
test('global SSH treats an omitted port as 22 and checks the primary route', () => {
  const route = resolveDesktopRemoteRoute({
    config: { mode: 'ssh', remote: { mode: 'ssh', host: 'box.test', user: 'clara' } },
    registry: registry('ssh-primary', [
      { id: 'ssh-primary', kind: 'ssh', label: 'SSH primary', host: 'box.test', user: 'clara', port: 22 }
    ])
  })
...
test('profile remote wins over a registry-backed global SSH route', () => {
  const route = resolveDesktopRemoteRoute({
    config: {
      mode: 'ssh',
      remote: { mode: 'ssh', host: 'global-box.test', user: 'clara' },
      profiles: {
        worker: { mode: 'remote', url: 'https://worker.test', authMode: 'token', token: tokenA }
      }
    },
    ...
    registry: registry('global-ssh', [
      { id: 'global-ssh', kind: 'ssh', label: 'Global SSH', host: 'global-box.test', user: 'clara' },
...
test('profile SSH wins over a different registry primary SSH route', () => {
  const route = resolveDesktopRemoteRoute({
    config: {
      ...
      remote: { mode: 'ssh', host: 'global-box.test', user: 'clara' },
      profiles: {
        worker: { mode: 'ssh', host: 'worker-box.test', user: 'clara' }
      }
    },
    profile: 'worker',
    registry: registry('global-ssh', [
      { id: 'global-ssh', kind: 'ssh', label: 'Global SSH', host: 'global-box.test', user: 'clara' },
      { id: 'worker-ssh', kind: 'ssh', label: 'Worker SSH', host: 'worker-box.test', user: 'clara' }
    ])
  })
...
test('local route does not inherit an unrelated registry SSH connection', () => {
  const route = resolveDesktopRemoteRoute({
    config: { mode: 'local' },
    registry: registry('local', [
      { id: 'unused-ssh', kind: 'ssh', label: 'Unused SSH', host: 'box.test', user: 'clara' }
    ])
  })
...
// remote transport, or startClara() spawns a loopback `clara serve` the
...
test('falls back to a CLOUD registry primary when the v1 mode is local', () => {
  ...
  assert.equal((route as any)?.org, 'clara')
})
