 ...
 * the three user-facing uninstall modes to the `clara uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m clara_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `clara_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'clara_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Clara.app/Contents/MacOS/Clara  → …/Clara.app
 *   Windows: …\Clara\Clara.exe                 → …\Clara  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Clara.app/Contents/MacOS/Clara → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Clara.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Clara.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Clara$/i.test(dir) || /[\\/]clara-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/clara
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import clara_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, claraHome }) {
 ...
 * modes + `pythonPath`=agentRoot so `import clara_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Clara install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "CLARA_HOME=${String(claraHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
