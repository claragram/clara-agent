...
export function registerPetOverlayIpc({
  ...
  ipcMain.handle('clara:pet-overlay:open', async (_event, request) => {
  ...
  ipcMain.handle('clara:pet-overlay:close', async () => {
    closePetOverlay()
  ...
  ipcMain.on('clara:pet-overlay:set-bounds', (_event, bounds) => {
  ...
  ipcMain.on('clara:pet-overlay:ignore-mouse', (_event, ignore) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('clara:pet-overlay:set-focusable', (_event, focusable) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('clara:pet-overlay:state', (_event, payload) => {
    const petOverlayWindow = getPetOverlayWindow()

    if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
      petOverlayWindow.webContents.send('clara:pet-overlay:state', payload)
    }
  })
  // Overlay → main renderer: control messages (pop back in, composer submit).
  ipcMain.on('clara:pet-overlay:control', (_event, payload) => {
    const mainWindow = getMainWindow()
    ...
    mainWindow.webContents.send('clara:pet-overlay:control', payload)
  })
}
