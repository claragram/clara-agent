...
export function resolveDesktopRemoteRoute({
  ...
  if (envUrl) {
    ...
    if (!envToken) {
      throw new Error(
        'CLARA_DESKTOP_REMOTE_URL is set but CLARA_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Clara backend.'
      )
    }
  ...
  if (!modeIsRemoteLike(config.mode)) {
    ...
    // resolves local and spawns a loopback `clara serve` the desktop never
