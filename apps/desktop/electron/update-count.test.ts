...
function createTempGitRepo() {
  const cwd = fs.mkdtempSync(path.join(os.tmpdir(), 'clara-update-count-'))
  ...
  try {
    ...
    git('config', 'user.name', 'Clara Test')
    git('config', 'user.email', 'clara@example.invalid')
...
test('compareApiUrl builds the GitHub compare URL for HTTPS origins', () => {
  assert.equal(
    compareApiUrl({
      currentSha: SHA_A,
      originUrl: 'https://github.com/claraprise/clara-agent.git',
      targetSha: SHA_B
    }),
    `https://api.github.com/repos/Workprise/clara-agent/compare/${SHA_A}...${SHA_B}`
  )
})
...
test('compareApiUrl handles SSH origin forms', () => {
  for (const originUrl of [
    ...
    'ssh://git@github.com/claraprise/clara-agent.git',
