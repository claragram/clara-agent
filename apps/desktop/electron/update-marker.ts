...
export function markerPath(claraHome) {
  return path.join(claraHome, '.clara-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(claraHome)
  let raw
 ...
 *
 * The Tauri-based clara-setup.exe takes several seconds to initialise its
...
export function writeUpdateMarker(
  ...
  const file = markerPath(claraHome)
  const nowMs = now()
  const owner = readLiveUpdateMarker(claraHome, { kill, maxAgeMs, now: () => nowMs })
 ...
 * Clara to exit…") clobbers that still-running updater's claim: the
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(claraHome, opts)
