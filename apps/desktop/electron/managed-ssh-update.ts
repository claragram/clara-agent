 ...
 * wire protocol:
 *
 *   gate dials -> drain every captured scope -> detached `clara update`
...
function buildPosixManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const home = validateRemoteValue(target.claraHome, 'Clara home')
  const claraPath = validateRemoteValue(target.claraPath, 'launcher path')
  ...
  const launcherWord = expandRemotePath(claraPath)

  const updateCommand =
    `env CLARA_HOME=${homeWord} ` +
    `CLARA_UPDATE_CORRELATION_ID=${shq(correlation)} ` +
    'CLARA_UPDATE_ORIGIN_PROFILE=default ' +
    `CLARA_UPDATE_ORIGIN_HOME=${homeWord} ` +
    `CLARA_UPDATE_OUTPUT_PATH=${outputWord} ` +
...
function buildWindowsManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const wrapper = [
    ...
    `$env:CLARA_HOME=${psLiteral(home)}`,
    `$env:CLARA_UPDATE_CORRELATION_ID=${psLiteral(correlation)}`,
    '$env:CLARA_UPDATE_ORIGIN_PROFILE="default"',
    `$env:CLARA_UPDATE_ORIGIN_HOME=${psLiteral(home)}`,
    `$env:CLARA_UPDATE_OUTPUT_PATH=${psLiteral(outputPath)}`,
    ...
    `$env:CLARA_UPDATE_WINDOWS_DETACHED=${psLiteral(correlation)}`,
    `$env:CLARA_UPDATE_TAURI_OUTCOME_PATH=${psLiteral(statusPath)}`,
    `$env:CLARA_UPDATE_TAURI_READY_PATH=${psLiteral(readyPath)}`,
    ...
    `& ${psLiteral(claraPath)} update --yes *>> ${psLiteral(outputPath)}`,
    '$rc=$LASTEXITCODE',
...
marker_path=install_root/'.clara-update-in-progress'
...
async function runManagedSshUpdate<TScope extends ManagedSshScope>(
  ...
  return {
    ...
    message:
      outcome === 'updated'
        ? 'Remote Clara updated and every managed SSH profile is ready.'
        : restoreOk
