...
// registry, and the clara:terminal:* IPC surface. Extracted from main.ts; the
...
export function registerTerminalIpc({
  ...
  function terminalShellCommand() {
    // CLARA_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
    ...
    const override = (process.env.CLARA_DESKTOP_SHELL || (isWindows ? '' : process.env.SHELL) || '').trim()
  ...
  function terminalShellEnv() {
    ...
    // /TERM=dumb; some terminals set COLORFGBG which would flip Clara' TUI into
    ...
    env.TERM_PROGRAM = 'Clara'
    env.TERM_PROGRAM_VERSION = app.getVersion()

    // Let a clara/--tui launched in this pane know it's embedded in the desktop
    // GUI (build_environment_hints surfaces this). Distinct from CLARA_DESKTOP,
    // which marks the agent *backend* and gates cron/gateway behavior.
    env.CLARA_DESKTOP_TERMINAL = '1'

    return env
  }

  function terminalChannel(id, suffix) {
    return `clara:terminal:${id}:${suffix}`
  }
  ...
  ipcMain.handle('clara:terminal:start', async (event, payload = {}) => {
  ...
  ipcMain.handle('clara:terminal:attach', (event, id) => {
  ...
  ipcMain.handle('clara:terminal:write', (_event, id, data) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('clara:terminal:resize', (_event, id, size = {}) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('clara:terminal:cwd', async (_event, id) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('clara:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))
