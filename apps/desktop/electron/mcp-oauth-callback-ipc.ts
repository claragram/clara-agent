 ...
 *   - the browser sees only a minimal "return to Clara" page.
 */
...
export function registerMcpOauthCallbackIpc() {
  ...
  ipcMain.handle('clara:mcp-oauth:listen', async () => {
  ...
  ipcMain.handle('clara:mcp-oauth:wait', async (_event, id, timeoutMs) => {
  ...
  ipcMain.handle('clara:mcp-oauth:cancel', (_event, id) => {
    dispose(String(id || ''))
