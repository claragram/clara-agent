...
import { buildDesktopBackendEnv, claraManagedNodePathEntries, normalizeClaraHomeRoot } from './backend-env'
import {
...
import { isClaraOwnedVenvDaemon } from './venv-holder-select'
...
import {
  ...
  resolveVenvClaraCommand
} from './windows-clara-path'
import {
...
const USER_DATA_OVERRIDE = process.env.CLARA_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.CLARA_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.CLARA_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with CLARA_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[clara] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (DEV_CDP.port) {
  ...
  console.log(
    `[clara] renderer debugging on http://127.0.0.1:${DEV_CDP.port} — anything that can reach it ` +
      'can run code in the renderer. CLARA_DESKTOP_CDP_PORT=off to disable.'
  )
} else {
  const why = describeDevCdpDecision(DEV_CDP)

  if (why) {
    console.warn(`[clara] ${why}`)
  }
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[clara] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// it). The value arrives via CLARA_DESKTOP_PASSWORD_STORE, bridged by the
// `clara desktop` launcher from detection or `desktop.password_store` in
...
if (PASSWORD_STORE.warning) {
  console.warn(`[clara] ${PASSWORD_STORE.warning}`)
}

if (PASSWORD_STORE.store) {
  app.commandLine.appendSwitch('password-store', PASSWORD_STORE.store)
  console.log(`[clara] using password-store backend: ${PASSWORD_STORE.store}`)
}
...
// never go through `clara desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose Clara sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[clara] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[clara] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[clara] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[clara] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[clara] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('clara:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
...
// always-on timers. A completely idle, minimized Clara burned ~20% CPU
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[clara] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[clara] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[clara] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[clara] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// CLARA_HOME — the user-facing root for everything Clara-related. Mirrors
// scripts/install.ps1's $ClaraHome and scripts/install.sh's $CLARA_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\clara (matches install.ps1)
//   macOS / Linux: ~/.clara (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.clara directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\clara yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// CLARA_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// CLARA_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.clara / %LOCALAPPDATA%\clara.
function resolveClaraHome() {
  if (process.env.CLARA_HOME) {
    return normalizeClaraHomeRoot(process.env.CLARA_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'clara-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a CLARA_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\clara and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('CLARA_HOME')

    if (fromRegistry) {
      return normalizeClaraHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'clara')
    const legacy = path.join(app.getPath('home'), '.clara')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.clara setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.clara')
}

const CLARA_HOME = resolveClaraHome()

function pathWithClaraManagedNode(...entries) {
  const managed = claraManagedNodePathEntries(CLARA_HOME).filter(directoryExists)

  return [...managed, ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_CLARA_ROOT — the canonical mutable Clara install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_CLARA_ROOT = path.join(CLARA_HOME, 'clara-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_CLARA_ROOT, 'venv')
...
// boots, going straight to `resolveClaraBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_CLARA_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_CLARA_ROOT, '.clara-bootstrap-complete')
...
// active-profile.json records which Clara profile the desktop launches its
// local backend as. When set, startClara() passes `clara --profile <name>
// dashboard …`, which deterministically pins CLARA_HOME (see
// _apply_profile_override in clara_cli/main.py) and bypasses the sticky
// ~/.clara/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors clara_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// claraDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under CLARA_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by clara_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(CLARA_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (clara_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.CLARA_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.CLARA_DESKTOP_BOOT_FAKE_ERROR || ''
...
const SKIP_QUIT_CONFIRM = process.env.CLARA_DESKTOP_SKIP_QUIT_CONFIRM === '1'

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.CLARA_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.CLARA_DESKTOP_APP_NAME || 'Clara'
...
// 'clara:native-theme' ('dark' | 'light' | 'system'); we pin
...
if (IS_WINDOWS) {
  app.setAppUserModelId('com.workprise.clara')
}

// Seed the native About panel with the live Clara version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `clara update` mid-session is reflected without an app restart;
...
// startClara(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.CLARA_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.CLARA_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
//   * WSL2 IPC stall  = the renderer's `clara:backend:touch` roundtrips
...
const POOL_KEEPALIVE_FRESH_MS = Math.max(
  120_000,
  Number(process.env.CLARA_DESKTOP_POOL_KEEPALIVE_FRESH_MS) || 4 * 60_000
)
...
// onto the error so subsequent startClara() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning clara serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Clara backend',
  phase: 'idle',
  progress: 0,
  retryable: false,
  running: false,
...
function rememberLog(chunk) {
  ...
  if (claraLog.length > 300) {
    claraLog.splice(0, claraLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-clara-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('clara:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the clara:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('clara:bootstrap:event', ev)
}
...
function promptFirstRunSetupChoice(backend) {
  broadcastBootstrapEvent({
    ...
    activeRoot: backend.activeRoot || ACTIVE_CLARA_ROOT
  })
}
...
// The Tauri updater writes CLARA_HOME/.clara-update-in-progress for the whole
...
// (`force_kill_other_clara`, taskkill /IM clara.exe) kills it, the launch
...
function updateGateDeps() {
  return {
    hasLiveMarker: () => Boolean(readLiveUpdateMarker(CLARA_HOME)),
...
const BUNDLE_SWAP_RELAUNCH_FLAG = '--clara-bundle-swap-relaunched'
...
// The detached updater swaps the packaged bundle on disk AFTER `clara update`
...
async function waitForUpdateToFinish() {
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — Clara will start automatically when it completes…',
        12
      )
    },
  ...
  // "nothing happened").
  try {
    const result = readAndConsumeHandoffResult(CLARA_HOME)
    ...
    if (result && result.ok && result.manual) {
      ...
      dialog.showErrorBox(
        'Clara update did not finish',
        `${result.message}\n\nDetails: ${path.join(CLARA_HOME, 'logs', 'desktop-update-handoff.log')}`
      )
    }
  } catch (err) {
  ...
  if (outcome === 'timeout') {
    ...
    await advanceBootProgress('backend.update-restart', 'Restarting Clara to load the updated app…', 14)
...
function findOnPath(command) {
  ...
  // shell-script shim named `clara` — must not shadow `clara.cmd`/`clara.exe`.
...
function unwrapWindowsVenvClaraCommand(command, backendArgs) {
  return resolveVenvClaraCommand(command, backendArgs, {
    isWindows: IS_WINDOWS,
    isCommandScript,
    fileExists,
    directoryExists,
...
// spawns `clara serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `clara` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'clara_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      // `clara --version` (~10.5s measured cold), and a false negative here
      ...
      execProbeSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, CLARA_HOME, ...(backend.env || {}) },
        timeout: PROBE_TIMEOUT_MS,
        stdio: 'ignore',
        // `.cmd`/`.bat` shim backends carry shell: true in their descriptor
        // (see resolveClaraBackend step 4); execFileSync of a .cmd without
...
function isClaraSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'clara_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.CLARA_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Clara deps (notably pywinpty's Rust-built
    ...
    for (const version of SUPPORTED_VERSIONS) {
      try {
        const out = execFileSync(
          ...
          // pathologically wedged reg.exe can't hang the __PROT_0_synchroclara__ boot
      ...
      try {
        const out = execFileSync(
          ...
          hiddenWindowsChildOptions({
            ...
            // Bare interpreter startup — much lighter than the clara-import
            // probes, but still python.exe under cold cache / AV scan, so
            // share the probe budget rather than running unbounded (this
            // __PROT_1_synchroclara__ exec previously had no timeout at all).
            timeout: PROBE_TIMEOUT_MS
          })
        )
...
// findGitBash — locate bash.exe on Windows. Resolves CLARA_GIT_BASH_PATH
...
// normal CLARA_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\clara\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'clara', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'clara', 'git', 'bin', 'git.exe'))
  }
...
function recentClaraLog() {
  return claraLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's clara root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_CLARA_ROOT.
// CLARA_DESKTOP_CLARA_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.CLARA_DESKTOP_CLARA_ROOT && path.resolve(process.env.CLARA_DESKTOP_CLARA_ROOT),
    !IS_PACKAGED && isClaraSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isClaraSourceRoot(ACTIVE_CLARA_ROOT) ? ACTIVE_CLARA_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_CLARA_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('clara:updates:progress', merged)
  }
}
      ...
      const req = https.get(
        url,
        {
          headers: {
            ...
            'User-Agent': 'clara-desktop-update-check'
          },
          timeout: 10_000
        },
        res => {
...
// Windows that binary owns ALL repo mutation — running `clara update` +
...
function resolveUpdaterBinary() {
  return resolveStagedUpdaterBinary(CLARA_HOME, { fileExists, isWindows: IS_WINDOWS })
}
...
// Path to the venv shim whose lock decides whether `clara update` can write
// fresh entry points. On Windows this is the file the running backend
// `clara.exe` holds open; on POSIX it's never mandatory-locked.
function venvClaraShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'clara.exe')
    : path.join(updateRoot, 'venv', 'bin', 'clara')
}
...
// Kill only Clara-OWNED venv daemons (the memory plugin's hindsight daemon:
...
// venv files mapped. External holders (a user terminal running `clara`,
...
function killClaraOwnedVenvDaemons(updateRoot) {
  if (!IS_WINDOWS) {
    return
  }
  ...
  try {
    ...
    holders = (Array.isArray(parsed) ? parsed : [parsed]).filter(p =>
      isClaraOwnedVenvDaemon(p?.ExecutablePath, p?.CommandLine, scriptsDir)
    )
  } catch {
  ...
  for (const holder of holders) {
    ...
    if (Number.isInteger(pid) && pid > 0) {
      rememberLog(`[updates] stopping Clara-owned venv daemon (hindsight) PID ${pid} before hand-off`)
...
// only signals the direct child, so on Windows a backend `clara.exe` that
// spawned its own grandchildren (a `clara` REPL, a pty terminal session, the
// gateway) would survive and keep the venv shim locked. taskkill /T /F reaps
// the whole tree __PROT_4_synchroclaraly__. Windows-only: this is called solely from the
...
async function claimBackendChild(child, command, profile, nonce, outputTail: BackendOutputTail | null = null) {
  ...
  if (decision.action === 'fail') {
    ...
    throw new Error(
      `Clara backend (PID ${child.pid}) died before its identity could be recorded: ${decision.reason}${outputTail?.describe() ?? ''}`
    )
  }
  ...
  if (decision.action === 'degrade') {
    ...
    rememberLog(
      `WARNING: process start marker probe failed for live Clara backend PID ${child.pid}; ` +
  ...
  try {
    ...
    child.claraBackendIdentity = identity
    ...
    await waitForBackendExit(child)
    throw new Error(
      `Could not persist ownership for the Clara backend: ${error.message}${outputTail?.describe() ?? ''}`
    )
  }
}

function releaseBackendChild(child) {
  const identity = child?.claraBackendIdentity
...
// `claraProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `clara.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a clara
// REPL, a pty terminal, the gateway) keep `clara.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const claraProcess = backendConnectionState.getProcess()
  ...
  // unlock — the shim probe only covers venv\Scripts\clara.exe, but the
  // backend is `python.exe -m clara_cli.main serve`, which need not hold
  // the shim at all (#74805 first-attempt race).
  const initialPids = []

  if (claraProcess && Number.isInteger(claraProcess.pid)) {
    initialPids.push(claraProcess.pid)
  }
  ...
  stopBackendTreesForUpdate(claraProcess, {
  ...
  // `clara gateway stop --all`: the CLI discovers every profile's gateway
  ...
  stopGatewayBeforeUpdate(venvClaraShimPath(updateRoot), CLARA_HOME)

  // Reap Clara-OWNED venv daemons the tree-kill above cannot reach: the
  ...
  killClaraOwnedVenvDaemons(updateRoot)

  const shim = venvClaraShimPath(updateRoot)
  ...
  const gate = await waitForBackendRelease(
    initialPids,
    {
      ...
      collectStragglerPids: () => {
        ...
        const currentClaraProcess = backendConnectionState.getProcess()

        if (currentClaraProcess && Number.isInteger(currentClaraProcess.pid)) {
          stragglers.push(currentClaraProcess.pid)
        }
...
// `clara update`). Instead we spawn the staged Clara-Setup binary with
// --update and quit, so it can run `clara update` (which refuses while we
...
async function applyUpdates(opts: { stopSafeBlockers?: boolean } = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      ...
      // Windows (quit → detached orchestrator → `clara update` → relaunch),
      ...
      // in: the CLARA_DESKTOP_CHILD_PID reaper-exclusion dance (#37532),
      ...
      // get the manual `clara update` card once; their next update pulls it.
      return await applyUpdatesPosixHandoff(opts)
    }

    if (!updater) {
      // No staged updater binary — this is a CLI-installed user (they ran
      // `clara desktop`, never the Tauri installer that self-copies
      // clara-setup.exe into CLARA_HOME). On Windows the repo hand-off
      ...
      if (!resolveUpdateScriptHandoff(updateRoot)) {
        // They DO have a working `clara` on PATH / in the venv, so the
        ...
        try {
          ...
          if (head.code === 0 && current && current !== 'HEAD') {
            ...
            if (branch !== 'main') {
              command = `clara update --branch ${branch}`
            }
          }
        } catch {
          // Best-effort: fall back to bare `clara update` if branch detection fails.
        }

        rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
        emitUpdateProgress({ stage: 'manual', message: command, percent: null })

        return { ok: true, manual: true, command, claraRoot: updateRoot }
      }

      rememberLog('[updates] no staged updater; using repo hand-off script for CLI install')
    }

    const handoffConflict = updateHandoffConflict(CLARA_HOME)
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Clara — this window will close and the updater will open. Don’t reopen Clara yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    preflightStateDb(CLARA_HOME, rememberLog)

    // Stop our own backend(s) and wait for the venv shim to unlock BEFORE we
    // spawn the updater. Without this the updater races a still-locked
    // clara.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running clara, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Clara install open ' +
        '(a second Clara window or a terminal running clara?). Close it and retry.'
      ...
      if (IS_WINDOWS) {
        ...
        startGatewaysAfterUpdateAbort(venvClaraShimPath(updateRoot))
      }
    ...
    // Clara processes running from this venv.  The updater normally refuses
    ...
    if (IS_WINDOWS) {
      ...
      // is __PROT_7_asynchroclara__ on Windows: even after releaseBackendLock's PID-exit
    ...
    // `clara update` will run (the venv shim is locked while we live).
    //
    ...
    // checkout, so each `clara update` refreshes the code that drives the
    ...
    if (scriptHandoff) {
      ...
      child = spawnUpdaterProcess(wrapped.command, wrapped.args, {
        ...
        env: {
          ...process.env,
          CLARA_HOME,
          CLARA_UPDATE_STARTED_AT: String(updateStartedAt),
          PATH: pathWithClaraManagedNode(venvBin)
        },
        detached: true,
        stdio: 'ignore'
      })
      ...
      // The `clara update` child adopts the SCRIPT's claim via
      // update_lock.py's process-ancestry rule; no mtime heuristics needed.
      if (Number.isInteger(child.pid)) {
        writeUpdateMarker(CLARA_HOME, child.pid, { startedAt: updateStartedAt })
      }
      ...
      // abort with "Another Clara update is already running (PID <itself>)" —
      ...
      if (Number.isInteger(child.pid) && stagedUpdaterSupportsPrewrittenMarker(updater)) {
        writeUpdateMarker(CLARA_HOME, child.pid)
    ...
    if (!handoffOutcome.ok) {
      const message = `Update failed to start: ${handoffOutcome.message}. Clara will keep running — try again, or run \`clara update\` from a terminal.`
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvClara = path.join(venvBin, IS_WINDOWS ? 'clara.exe' : 'clara')
  const venvPython = path.join(venvBin, IS_WINDOWS ? 'python.exe' : 'python')

  // The updater invokes the venv's Clara launcher, which in turn requires the
  ...
  // marker-only install through --update dead-ends at "Could not find the clara
  ...
  const updaterArgs = chooseUpdaterArgs(
    {
      hasBootstrapMarker: fileExists(path.join(updateRoot, '.clara-bootstrap-complete')),
      hasVenvClara: fileExists(venvClara),
...
// Python-level pre-update snapshot inside `clara update`.
function preflightStateDb(claraHome, rememberLog) {
  const stateDbPath = path.join(claraHome, 'state.db')
  ...
  try {
    ...
    if (stat.size > 100) {
      ...
      const emergencyPath = path.join(claraHome, `state.db.pre-update-emergency-${ts}.bak`)
      ...
      try {
        ...
        try {
          const homeDir = fs.readdirSync(claraHome)
          ...
          for (const old of backups.slice(2)) {
            try {
              fs.unlinkSync(path.join(claraHome, old))
            } catch {
              void 0
            }
          }
...
// out, runs `clara update`, swaps/relaunches the app bundle, and writes
// .clara-update-result.json for the relaunched Desktop to surface. It shows
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  if (!handoff) {
    emitUpdateProgress({ stage: 'manual', message: 'clara update', percent: null })

    return { ok: true, manual: true, command: 'clara update', claraRoot: updateRoot }
  }
  ...
  const child = spawnUpdaterProcess(handoff.command, args, {
    ...
    env: {
      ...
      CLARA_HOME,
      CLARA_UPDATE_STARTED_AT: String(updateStartedAt),
      PATH: pathWithClaraManagedNode(path.join(updateRoot, 'venv', 'bin'))
    },
    detached: true,
    stdio: 'ignore'
  })
  ...
  emitUpdateProgress({
    stage: 'restart',
    message:
      'Updating Clara — this window will close. Don’t reopen Clara yourself; it restarts automatically when the update finishes.',
    percent: 100
  })
...
// Marker-independent: is the canonical install at ACTIVE_CLARA_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isClaraSourceRoot(ACTIVE_CLARA_ROOT) &&
    fileExists(venvPython) &&
    canImportClaraCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_CLARA_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function activeRuntimeState() {
  ...
  // pinned commit -- users update via the in-app update path or `clara
...
function resolveWebDist() {
  const override = process.env.CLARA_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set CLARA_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  if (present.length > 0) {
    ...
    rememberLog(
      `[renderer] every renderer bundle is incomplete (${present.join(', ')}). ` +
        ...
        `Repair with: clara desktop --force-build`
    )
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: clara desktop --force-build`
  )
...
function resolveClaraCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Clara.app/Contents/...`
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveClaraCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, backendArgs, options: any = {}) {
  ...
  return {
    ...
    label,
    command,
    args: ['-m', 'clara_cli.main', ...backendArgs],
...
// createActiveBackend — build a backend pointing at ACTIVE_CLARA_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Clara at ${ACTIVE_CLARA_ROOT}`,
    command,
    args: ['-m', 'clara_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      claraHome: CLARA_HOME,
      pythonPathEntries: [ACTIVE_CLARA_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_CLARA_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveClaraBackend(backendArgs) {
  // 1. Explicit override -- CLARA_DESKTOP_CLARA_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.CLARA_DESKTOP_CLARA_ROOT && path.resolve(process.env.CLARA_DESKTOP_CLARA_ROOT)

  if (overrideRoot && isClaraSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Clara source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `clara` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isClaraSourceRoot.)
  if (!IS_PACKAGED && isClaraSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Clara source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. ACTIVE_CLARA_ROOT — the canonical install at
  //    %LOCALAPPDATA%\\clara\\clara-agent (Windows) or ~/.clara/clara-agent.
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active Clara runtime at ${ACTIVE_CLARA_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  // 4. Existing `clara` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    CLARA_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.CLARA_DESKTOP_IGNORE_EXISTING !== '1') {
    let claraCommand = null
    const claraOverride = process.env.CLARA_DESKTOP_CLARA

    if (claraOverride) {
      const resolvedOverride = findOnPath(claraOverride)

      if (resolvedOverride) {
        claraCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(claraOverride, { isWsl: IS_WSL })) {
        claraCommand = claraOverride
      } else {
        rememberLog(`Ignoring Windows Clara override under WSL: ${claraOverride}`)
      }
    } else {
      claraCommand = findOnPath('clara')
    }

    if (claraCommand) {
      if (looksLikeDesktopAppBinary(claraCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Clara CLI: ${claraCommand}`)
        claraCommand = null
      }
    }

    if (claraCommand) {
      const unwrapped = unwrapWindowsVenvClaraCommand(claraCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `clara` shim
      ...
      const shellForProbe = isCommandScript(claraCommand)

      // CLARA_DESKTOP_CLARA is an explicit deployment override (used by
      ...
      if (shouldTrustClaraOverride(claraOverride) || verifyClaraCli(claraCommand, { shell: shellForProbe })) {
        ...
        // unwrapWindowsVenvClaraCommand here: the second call repeats the
        // same un-memoized import probe, costing up to another full probe
        // timeout on the boot path for an answer we already have.
        return {
          label: `existing Clara CLI at ${claraCommand}`,
          command: claraCommand,
          args: backendArgs,
          bootstrap: false,
          env: {},
          kind: 'command',
      ...
      rememberLog(
        `Ignoring existing Clara CLI at ${claraCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed clara_cli module via system Python.
  ...
  if (python) {
    ...
    // having clara_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\clara\clara-agent\venv.
    if (canImportClaraCli(python)) {
      return {
        kind: 'python',
        label: `installed clara_cli module via ${python}`,
        command: python,
        args: ['-m', 'clara_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: clara_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveClaraBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Clara Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveClaraBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Clara install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Clara recovery was handed off to Clara Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      claraHome: CLARA_HOME,
      logRoot: path.join(CLARA_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Clara install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Clara bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(CLARA_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any

      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startClara() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // clara:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveClaraBackend(backend.args))
  }
  ...
  if (!isClaraSourceRoot(ACTIVE_CLARA_ROOT)) {
    throw new Error(
      `Clara install at ${ACTIVE_CLARA_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Clara' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\clara\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `clara` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Clara on Windows (provides Git Bash, ' +
  ...
  if (!fileExists(venvPython)) {
    ...
    // normal flow because activeRuntimeState() requires isClaraSourceRoot()
    // plus an importable clara_cli before it hands back the active runtime.
    // If we hit this, the user (or a deleted venv) broke the invariant; tell
    // them to re-run the install.
    throw new Error(
      `Clara venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Clara at ${ACTIVE_CLARA_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Clara runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function multipartBody(upload) {
  const boundary = `----clara-${crypto.randomBytes(12).toString('hex')}`
    ...
    (requestState: any) =>
      new Promise((resolve, reject) => {
        ...
        if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
          reject(new Error(`Unsupported Clara backend URL protocol: ${parsed.protocol}`))
                ...
                reject(
                  new Error(
                    `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                      'The endpoint is likely missing on the Clara backend.'
                  )
                )
        ...
        req.setTimeout(timeoutMs, () => {
          req.destroy(new Error(`Timed out connecting to Clara backend after ${timeoutMs}ms`))
        })
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = client.request(
      parsed,
      {
        agent,
        method: 'GET',
        headers: options.bearer ? { Authorization: `Bearer ${options.bearer}` } : { 'X-Clara-Session-Token': token }
      },
      res => {
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Clara-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('clara:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveClaraCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('clara:preview-file-changed', payload)
}
...
// waitForClaraReady how to read a 401 — rejected session vs gated route.
...
async function waitForClara(baseUrl, token, signal?, authMode?, headers = {}) {
  const { probeHealth, probeIsCredentialed } = await buildReadinessHealthProbe(baseUrl, authMode, token)

  return waitForClaraReady(baseUrl, {
    token,
    signal,
    fetchPublicJson,
...
function sendBackendExit(payload) {
  ...
  webContents.send('clara:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('clara:close-preview-requested')
}
...
function sendPreviewNavCommand(command: 'back' | 'forward' | 'reload') {
  ...
  webContents.send('clara:preview-nav', command)
}
...
function sendOpenFolderRequested() {
  ...
  webContents.send('clara:open-folder-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Clara...".
function sendPowerResume() {
  ...
  webContents.send('clara:power-resume')
}
...
// 'clara:power-battery' push below.
ipcMain.handle('clara:power-battery:get', () => onBatteryPower === true)
...
function broadcastBatteryState(next: boolean) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('clara:power-battery', next)
    }
  }
}
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('clara:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean, target = mainWindow) {
  ...
  webContents.send('clara:window-state-changed', state)
}
...
function installContextMenuBridge(window: BrowserWindow) {
  window.webContents.on('context-menu', (_event, params) => {
    ...
    if (params.isEditable && params.misspelledWord) {
      window.webContents.send('clara:context-menu-spellcheck', {
...
function installMediaPermissions() {
  ...
  // __PROT_2_Synchroclara__ check handler: Chromium consults this for getUserMedia on
...
//   * Clara Portal now issues a 24h ROTATING, reuse-detected refresh token
//     alongside the ~15-min access token (Portal NAS #293 / clara #37247).
//     Both are set as HttpOnly cookies (``clara_session_at`` ~15 min,
//     ``clara_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
// "not signed in", which on the initial boot path (startClara → the renderer's
// single-shot boot() with no retry) surfaced as the "Clara couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Clara
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Clara Cloud agent…' : 'Sign in to Clara gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Clara backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// feature; the server half lives in clara_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
// --- Clara Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
// The "cloud" connection mode lets a user sign in to the Clara portal ONCE in
...
// portal session cookie living in `persist:clara-remote-oauth`:
...
// Whether the OAuth partition currently holds a live Clara portal session — the
// credential that powers both discovery and the silent cascade. The portal
// authenticates via PRIVY, not the Clara gateway session cookies, so this
// checks for the `privy-token` cookie on the portal host (NOT
// hasLiveOauthSession, which looks for clara_session_at/rt that the portal
    ...
    return await new Promise<boolean>(resolve => {
      ...
      try {
        win = new BrowserWindow({
          ...
          height: 720,
          show: false,
          title: 'Renewing Clara Cloud session…',
          autoHideMenuBar: true,
          webPreferences: {
            contextIsolation: true,
            nodeIntegration: false,
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a Clara Cloud sign-in.'))
...
// Discover the hosted (Clara Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to Clara Cloud. Open Settings → Gateway, choose Clara Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (body === undefined) {
      ...
      if (error && error.statusCode === 401) {
        const err = new Error(
          'Your Clara Cloud session has expired. Open Settings → Gateway and sign in again.'
        ) as any
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Clara Cloud session has expired. Sign in to Clara Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Clara Cloud org tag on cloud-mode entries so Settings can
...
function readDesktopConnectionsRegistry() {
  ...
  if (mtime === null) {
    ...
    try {
      ...
      // hard-failing every clara:connections:* call.
 ...
 * editor doesn't carry — cloud `org`, ssh `remoteClaraPath`/`remoteProfile` —
...
// a valid stored value (pins the root CLARA_HOME explicitly); null means "no
...
// contains "clara", avoiding false positives from stale gateway.pid files
// whose PID was recycled by the OS to an unrelated process.
function isClaraProcess(pid) {
  try {
...
//   1. Legacy ~/.clara/active_profile (explicit CLI choice via clara profile use)
//   2. Running gateway (gateway.pid with verified liveness + clara identity)
...
function migrateActiveProfileIfMissing() {
  migrateActiveProfileIfMissingPure(DESKTOP_PROFILE_CONFIG_PATH, {
    legacyActivePath: path.join(CLARA_HOME, 'active_profile'),
    claraHome: CLARA_HOME,
    profilesRoot: path.join(CLARA_HOME, 'profiles'),
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.CLARA_DESKTOP_REMOTE_URL)
  ...
  const remoteUrl = envOverride ? String(process.env.CLARA_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    remoteUrl,
    // The persisted Clara Cloud org (slug/id) for a cloud connection, or '' for
    ...
    sshRemoteClaraPath: (ssh || savedSsh)?.remoteClaraPath || '',
    sshRemoteProfile: (ssh || savedSsh)?.remoteProfile || '',
    // The env override only forces the global/primary connection; a per-profile
    // scope is never overridden by CLARA_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the Clara Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Clara Cloud instance, NOT a user-owned remote gateway,
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remoteClaraPath: input.sshRemoteClaraPath ?? existingBlock.remoteClaraPath,
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    // refresh token (clara #37247), and the gateway middleware transparently
    ...
    try {
      ...
      // For a Clara-managed Cloud agent, a 502/503/504 from the WS-ticket mint
      ...
      // else (#85335).
      const cloudError = makeClaraCloudBackendDownError(baseUrl, error)
      ...
      throw gatewayTicketFailure(
        ...
        'Could not reach the remote Clara gateway while refreshing its WebSocket ticket. Try reconnecting.'
      )
    }
  ...
  if (!token) {
    throw new Error(
      'Remote Clara gateway is selected, but no session token is saved. ' +
...
function activeSshTerminalTarget(webContentsId?: number) {
  ...
  const route = resolveDesktopRemoteRoute({
    config,
    env: {
      token: process.env.CLARA_DESKTOP_REMOTE_TOKEN,
      url: process.env.CLARA_DESKTOP_REMOTE_URL
    },
    profile,
    registry: readDesktopConnectionsRegistry()
  })
...
async function rollbackSshBootstrapResult(ssh, result, profile, sshConfig, boundaryError) {
  ...
  try {
    const expected = {
      ...
      claraPath: result.claraPath,
      claraHome: result.claraHome,
    ...
    if (result.platform?.os === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        ssh,
        { claraPath: result.claraPath, claraHome: result.claraHome, python: result.pythonPath },
        expected
      )
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, metadata, fingerprint, lease) {
  ...
  try {
    ...
    const platform = await detectRemotePlatform(ssh, sshConfig.remoteClaraPath || '')
    const lifecycle = platform.os === 'Windows' ? connectWindowsRemote : remoteLifecycle.connect
    result = await lifecycle({
      ssh,
      profile: resolveRemoteSshDashboardProfile(sshConfig.remoteProfile, profile),
      remoteClaraPath: sshConfig.remoteClaraPath || '',
      ownershipId: sshOwnershipKey(profile),
      ...
      pickLocalPort,
      waitForClara: (baseUrl, token) => waitForClara(baseUrl, token, lease.signal, 'token'),
  ...
  await fenceManagedSshBootstrapPublication({
    ...
    publish: () => {
      ...
      sshConnections.set(scope, {
        ...
        host: sshConfig.host,
        hostLabel,
        claraVersion: result.claraVersion || '',
        ...
        // per-profile pools also use bare keys. Only startClara' explicit call
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.claraVersion || 'clara (version unknown)'} at ${result.claraPath || '?'}`
  )
  ...
  return {
    ...connection,
    remoteClaraVersion: result.claraVersion || '',
    ssh: {
      effectiveConfigFingerprint: sshConfig.effectiveConfigFingerprint,
      host: sshConfig.host,
...
//   2. env override (CLARA_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
function globalRemoteActive() {
  if (process.env.CLARA_DESKTOP_REMOTE_URL) {
    return true
  }
...
// env → global) so a startClara() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Clara
...
async function testDesktopConnectionConfig(input: any = {}) {
  if (input.mode === 'ssh') {
    const sshConfig = normalizeSshConfig({
      ...
      remoteClaraPath: input.sshRemoteClaraPath
    })
    ...
    try {
      ...
      for (;;) {
        try {
          await ssh.open()
          const platform: any = await detectRemotePlatform(ssh, sshConfig.remoteClaraPath || '')
          let claraPath
          let claraVersion
          let supported
          ...
          if (platform.os === 'Windows') {
            ...
            claraPath = runtime.claraPath
            const inspection = await helper(ssh, runtime, 'inspect', [runtime.claraPath])
            claraVersion = inspection.version
            supported = inspection.supported
          } else {
            claraPath = await remoteLifecycle.locateClara(ssh, sshConfig.remoteClaraPath || '')
            claraVersion = await remoteLifecycle.probeClaraVersion(ssh, claraPath)
            supported = await remoteLifecycle.remoteSupportsSshOwnership(ssh, claraPath)
          }
          ...
          if (!supported) {
            return {
              ...
              error: 'Update Clara on the remote host before connecting with Desktop SSH.'
            }
          }
          ...
          return {
            ...
            remoteClaraPath: claraPath,
            remoteClaraVersion: claraVersion,
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startClara())
  ...
  // connect to Clara gateway". Mirror the renderer's connect here so the test
...
function resetClaraConnection({ soft = false } = {}) {
  backendStartFailure = null
  remoteReauthFailure = null
  remoteLiveness.clear()
  const claraProcess = backendConnectionState.invalidate()
  stopBackendChild(claraProcess)
...
// startClara() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetClaraConnection() invalidates it.
  const claraProcess = backendConnectionState.getProcess()
  const dying = claraProcess && !claraProcess.killed ? claraProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('clara:connection:applied')
}
...
function broadcastConnectionsChanged(payload: { connectionId: string; reason: 'removed' | 'saved' | 'updated' }) {
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('clara:connections:changed', payload)
    }
  }
}
...
async function ensureBackend(profile) {
  ...
  if (route.backend === 'primary') {
    const connection = await startClara()
  ...
  // spawning, so two children never share one profile's CLARA_HOME.
  ...
  entry.connectionPromise = spawnPoolBackend(key, entry).catch(async error => {
    ...
    rememberLog(
      `Clara backend for profile "${key}" failed to start: ${error instanceof Error ? error.message : String(error)}`
    )
...
async function ensureRegistryBackend(connectionId, profile, managedUpdateCorrelation = '') {
  ...
  const resolveRegistrySshConfig = () => {
    ...
    if (!resolvedRegistrySshConfig) {
      resolvedRegistrySshConfig = normalizeSshConfig({
        ...
        remoteClaraPath: source.remoteClaraPath,
  ...
  if (source.kind === 'local') {
    ...
    localEntry.connectionPromise = spawnPoolBackend(profileKey, localEntry, {
      ...
      rememberLog(
        `Clara backend for profile "${profileKey}" (forced-local) failed to start: ${error instanceof Error ? error.message : String(error)}`
      )
...
async function connectRegistryBackend(
  ...
  if (source.kind === 'ssh') {
    ...
    return {
      ...
      // is only the routing label. clara:api uses it to translate explicit
      // self-profile query filters into the backend's namespace.
      remoteProfile: sshConfig.remoteProfile || '',
      logs: claraLog.slice(-80),
      ...getWindowState()
    }
  }
  ...
  await waitForClara(connection.baseUrl, connection.token, undefined, connection.authMode, connection.headers)
  poolEntry.remoteBaseUrl = connection.baseUrl
...
function remoteUpdateTargetFromState(state): RemoteUpdateTarget {
  if (!state?.ssh || !state?.claraPath || !state?.claraHome) {
  ...
  return {
    ...
    claraPath: state.claraPath,
    claraHome: state.claraHome,
...
async function openManagedSshUpdateTransport(
  ...
  try {
    const platform: any = await detectRemotePlatform(ssh, config.remoteClaraPath || '')

    if (platform.os === 'Windows') {
      const runtime = platform.claraPath ? platform : await probeWindowsRemote(ssh, config.remoteClaraPath || '')
      ...
      return {
        ...
        target: {
          ...
          platform: 'Windows',
          claraPath: runtime.claraPath,
          claraHome: runtime.claraHome,
          pythonPath: runtime.python
        }
      }
    }

    const claraPath = await remoteLifecycle.locateClara(ssh, config.remoteClaraPath || '')
    const claraHome = await remoteLifecycle.probeRemoteClaraHome(ssh)

    return {
      close: () => ssh.close(),
      target: { ssh, platform: platform.os, claraPath, claraHome }
    }
  } catch (error) {
    await ssh.close()
    throw error
...
async function drainManagedSshScope(scope) {
  ...
  try {
    ...
    if (state.remotePlatform === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        state.ssh,
        { claraPath: state.claraPath, claraHome: state.claraHome, python: state.pythonPath },
        expected
      )
    ...
    if (state && state.localPort && state.remotePort && scope.reuseToken) {
      try {
        ...
        if (!forwardClosed) {
          await waitForClara(`http://127.0.0.1:${state.localPort}`, scope.reuseToken, undefined, 'token')
        } else {
...
// local-spawn portion of startClara() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  if (remote) {
    await waitForClara(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
  ...
  // --profile wins over the inherited CLARA_HOME env (see _apply_profile_override
  // step 3 in clara_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveClaraBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const claraCwd = resolveClaraCwd()
  ...
  assertLocalProfileCanStart(profile, profileDeletionGate, key =>
    directoryExists(path.join(CLARA_HOME, 'profiles', key))
  )

  rememberLog(`Starting Clara backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: claraCwd,
        CLARA_DASHBOARD_SESSION_TOKEN: token,
        ...
        ...parentIdentityEnv,
        CLARA_WEB_DIST: webDist,
        ...(readyFile ? { CLARA_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Clara backend for profile "${profile}" failed to start: ${error.message}`)
    releaseBackendChild(child)
    backendPool.delete(poolKey)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Clara backend for profile "${profile}" exited (${signal || code})`)
    releaseBackendChild(child)
    backendPool.delete(poolKey)
    ...
    if (!ready) {
      rejectStart?.(
        new Error(
          `Clara backend for profile "${profile}" exited before it became ready (${signal || code}).${outputTail.describe()}`
        )
      )
    }
  })
  ...
  await Promise.race([waitForClara(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Clara backend for profile "${profile}"`,
    rememberLog
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `Clara backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
// backend whose ensure_clara_home() recreates the deleted profile directory.
//
...
async function startClara() {
  ...
  if (!isPrimaryInstance) {
    ...
    throw new Error('Clara Desktop is already running in another window.')
  }

  await reapOrphanedBackendsOnce()

  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startClara() call re-throws the same error
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving Clara backend', 8)
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      ...
      if (!backendConnectionState.isCurrentAttempt(connectionAttempt)) {
        throw new Error('Clara backend start was superseded by a newer connection attempt.')
      }

      await advanceBootProgress('backend.remote', `Connecting to remote Clara backend at ${remote.baseUrl}`, 24)
      await waitForClara(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
      ...
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Clara backend is ready',
        progress: 94,
        running: true,
        error: null
      })

      return createPrimaryRemoteConnection(remote, claraLog.slice(-80), getWindowState())
    }
    ...
    // deterministic (it wins over the sticky ~/.clara/active_profile file) and
    // resolves CLARA_HOME the same way `clara -p <name>` does on the CLI. An
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving Clara runtime', 28)

        return resolveClaraBackend(backendArgs)
      },
      resolveRemote: () => {
    ...
    await advanceBootProgress('backend.spawn', `Starting Clara backend via ${backend.label}`, 84)
    rememberLog(`Starting Clara backend via ${backend.label}`)
    ...
    const claraProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin CLARA_HOME for the child so Python's get_clara_home()
          // resolves to the SAME location our resolveClaraHome() picked. Without
          // this pin, Python falls back to ~/.clara on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\clara, which differs from C:\Users\<u>\.clara.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets CLARA_HOME via setx; the desktop
    ...
    primaryOutputTail.attach(claraProcess)
    ...
    const portAnnouncement = waitForDashboardPortAnnouncement(claraProcess, {
    ...
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, claraProcess)

    if (!processOwner) {
      stopBackendChild(claraProcess)
      await waitForBackendExit(claraProcess)
      releaseBackendChild(claraProcess)
      throw new Error('Clara backend start was superseded by a newer connection attempt.')
    }

    claraProcess.stdout.on('data', rememberLog)
    claraProcess.stderr.on('data', rememberLog)
    ...
    claraProcess.once('error', error => {
      releaseBackendChild(claraProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Clara backend error: ${error.message}`)
        rejectBackendStart?.(new Error('Clara backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Clara backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Clara backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    claraProcess.once('exit', (code, signal) => {
      releaseBackendChild(claraProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Clara backend exit (${signal || code})`)
      ...
      rememberLog(`Clara backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `Clara backend exited before it became ready (${signal || code}).${primaryOutputTail.describe()}`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Clara backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentClaraLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Clara backend to launch', 86)
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Clara backend to become ready', 90)
    await Promise.race([waitForClara(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => claraProcess.exitCode === null && !claraProcess.killed,
      rememberLog
    })
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local Clara backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Clara backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
function notifyBrowserPopoutClosed(tabId) {
  ...
  for (const other of BrowserWindow.getAllWindows()) {
    if (!other.isDestroyed()) {
      other.webContents.send('clara:browser-popout:closed', tabId)
    }
  }
}
...
// here so it can leave the app's bounds and stay visible while Clara is
...
// pushes pet state over IPC (clara:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via clara:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Clara app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Clara icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see clara:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Clara in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('clara:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
// window showing only the composer and its scrollback, so Clara can be driven
...
function startHudCursorFeed(win: BrowserWindow) {
  ...
  const timer = setInterval(() => {
    ...
    last = key
    win.webContents.send('clara:hud:cursor', point)
  }, HUD_CURSOR_POLL_MS)
...
function startHudGameOverlayFeed(win: BrowserWindow) {
  ...
  const push = (state: { active: boolean; app: string }) => {
    if (!win.isDestroyed()) {
      win.webContents.send('clara:hud:game-overlay', state)
    }
  }
  ...
  const enumerate = async () => {
    ...
    if (!reported) {
      reported = true
      console.warn(`[clara] HUD cannot enumerate windows: ${windows.reason}`)
    }
...
function broadcastHudState(open) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    if (!win.isDestroyed()) {
      win.webContents.send('clara:hud:changed', payload)
    }
  }
}
...
function spawnHudWindow(sessionId, profile) {
  const win = new BrowserWindow({
    ...
    // `clara:hud:set-bounds`, which flips resizable on for the call — the
  ...
  // grab, clara:hud:workspace-transfer temporarily makes the X11 window
...
function openHudWindow(sessionId, profile) {
  ...
  if (hudWindow && !hudWindow.isDestroyed()) {
    ...
    if (sessionId && sessionId !== hudSessionId) {
      ...
      hudWindow.webContents.send('clara:hud:goto', sessionId)
...
function spawnQuickEntryWindow() {
  ...
  win.webContents.on('did-finish-load', () => {
    if (!win.isDestroyed() && quickEntryLastState) {
      win.webContents.send('clara:quick-entry:state', quickEntryLastState)
    }
  })
...
function showQuickEntryWindow() {
  ...
  quickEntryWindow.webContents.send('clara:quick-entry:shown')
}
...
function createWindow() {
  ...
  // __PROT_5_synchroclaraly__ before the window is gone.
    ...
    callbacks: {
      ...
      onFailedLoadBudgetExhausted: details => {
        ...
        void loadRendererLoadErrorPage(mainWindow, {
          ...
          repairHint: 'clara desktop --force-build',
  ...
  startClara().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('clara:connection', async (_event, profile) => {
...
ipcMain.handle('clara:connection:for', async (_event, payload) => {
  const { connectionId, profile } = payload && typeof payload === 'object' ? (payload as any) : ({} as any)
  const registry = readDesktopConnectionsRegistry()
  const id = String(connectionId || '').trim() || registry.primary
  // Same single-owner claim as 'clara:connection', keyed by the composite
...
ipcMain.on('clara:connection:active-route', (event, route) => {
...
// "Starting Clara…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('clara:connection:revalidate', async () => {
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: () => resetClaraConnection({ soft: true }),
...
ipcMain.handle('clara:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)

  return { ok: true }
})
ipcMain.handle('clara:gateway:ws-url', async (_event, profile) => {
  return gatewayWsUrlIpcResult(() => freshGatewayWsUrl(profile))
})
ipcMain.handle('clara:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('clara:window:openInstance', async () => {
  createInstanceWindow()

  return { ok: true }
})
ipcMain.handle('clara:window:openBrowser', async (_event, tabId) => {
...
ipcMain.handle('clara:window:openInTerminal', async (_event, sessionId, opts) => {
  ...
  try {
    ...
    const backend = resolveClaraBackend(tuiResumeArgs(sessionId.trim(), profile || undefined))

    if (!backend.command) {
      return { ok: false, error: 'Clara is not installed yet' }
    }
    ...
    const scriptPath = path.join(
      scriptDir,
      `clara-${crypto.randomBytes(6).toString('hex')}${terminalScriptExtension()}`
    )
    ...
    fs.writeFileSync(
      ...
      buildTerminalScript({
        ...
        cwd,
        env: terminalScriptEnv(backend.env, CLARA_HOME)
      }),
      { mode: 0o700 }
    )
...
ipcMain.handle('clara:wake-indicator:get', () => wakeIndicatorController.getState())
ipcMain.on('clara:wake-indicator:set', (_event, state) => {
...
ipcMain.handle('clara:zoom:get', event => {
...
ipcMain.on('clara:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('clara:backend:recycle', async (_event, profile) => {
...
ipcMain.handle('clara:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startClara() call restarts the
...
ipcMain.handle('clara:bootstrap:repair', async () => {
  // Forceful repair: force the next startClara() through the full installer
  ...
  // "bypass active runtime" path inside startClara() and fall through
...
ipcMain.handle('clara:boot-progress:get', async () => bootProgressState)
ipcMain.handle('clara:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('clara:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('clara:plugin-profile-routes', async (_event, rawProfileNames) => {
...
ipcMain.handle('clara:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('clara:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('clara:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
...
ipcMain.handle('clara:secret-storage:get', async () => ({ on: secretStoragePolicy().on }))
ipcMain.handle('clara:secret-storage:set', async (_event: any, on: any) => applySecretStorageEncryption(on === true))
...
ipcMain.handle('clara:connections:list', async () => sanitizeConnectionsRegistry())
ipcMain.handle('clara:connections:save', async (_event, payload) => {
  const saved = await saveRegistryConnection(payload)

  return { ok: true, connection: saved, registry: sanitizeConnectionsRegistry() }
})
ipcMain.handle('clara:connections:remove', async (_event, id) => {
  const key = String(id || '')
...
ipcMain.handle('clara:connections:set-primary', async (_event, id) => {
...
ipcMain.handle('clara:connections:set-launch-mode', async (_event, mode) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('clara:connections:set-last-used', async (_event, id) => {
  const registry = setLastUsedConnection(readDesktopConnectionsRegistry(), String(id || ''))
  writeDesktopConnectionsRegistry(registry)

  return { ok: true, registry: sanitizeConnectionsRegistry(registry) }
})
ipcMain.handle('clara:connections:test', async (_event, id) => {
  const registry = readDesktopConnectionsRegistry()
  ...
  if (entry.kind === 'ssh') {
    const result = await testDesktopConnectionConfig({
      ...
      sshRemoteClaraPath: entry.remoteClaraPath
    })
...
async function probeSshProfileInventory(connection) {
  ...
  const sshConfig = normalizeSshConfig({
    ...
    remoteClaraPath: connection.remoteClaraPath
  })
  ...
  try {
    await ssh.open()
    const profiles = await remoteLifecycle.listRemoteClaraProfiles(ssh)
    ...
    registry.connections.map(async connection => {
      ...
      try {
        ...
        if (connection.kind === 'ssh') {
          ...
          // The root CLARA_HOME is an agent too; enumerations that omit it
...
ipcMain.handle('clara:agents:roster', async () => {
  const registry = readDesktopConnectionsRegistry()
  const enumerations = await enumerateRegistryAgentSources(registry)
...
// clara:gateway:ws-url. Same single-use-ticket discipline for OAuth sources.
...
ipcMain.handle('clara:gateway:ws-url-for', async (_event, payload) => {
...
ipcMain.handle('clara:connections:update-managed', async (_event, rawId) => requestManagedSshUpdate(rawId))

// Fan out `clara update` to every eligible registered connection at once.
...
ipcMain.handle('clara:connections:update-all', async (_event, payload) => {
  const registry = readDesktopConnectionsRegistry()
    ...
    registry.connections
      ...
      .map(async connection => {
        ...
        try {
          ...
          const body: any = await postJsonForBackend(descriptor, '/api/clara/update', {}, { timeoutMs: 15_000 })
...
// analogue of the clara:api handler's own auth split: OAuth backends prefer a
...
ipcMain.handle('clara:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('clara:connection-config:oauth-login', async (_event, rawUrl) => {
  ...
  if (strategy === 'native') {
    try {
      ...
      // startClara() re-dials instead of replaying the stale rejection.
      remoteReauthFailure = null
...
ipcMain.handle('clara:connection-config:oauth-logout', async (_event, rawUrl) => {
  const baseUrl = normalizeRemoteBaseUrl(rawUrl)
...
// --- Clara Cloud (cloud-auto-discovery Phase 3) ---
...
ipcMain.handle('clara:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('clara:connection-config:save', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('clara:connection-config:apply', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('clara:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
// Persistence-only sibling of clara:profile:set: records the profile the
...
// the gateway (#79886).
ipcMain.handle('clara:profile:remember', async (_event, name) => ({
  profile: writeActiveDesktopProfile(name)
}))
ipcMain.handle('clara:profile:set', async (_event, name) => {
  assertCanMutateManagedPrimaryRouting()
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new CLARA_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('clara:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('clara:requestMicrophoneAccess', async () => {
...
ipcMain.handle('clara:window:readBelow', async event => {
...
async function handleClaraApiRequest(request) {
  ...
  // backend calls ensure_clara_home() which recreates the profile directory,
...
ipcMain.handle('clara:api', async (_event, request) => {
  ...
  // respawn the old-name backend and recreate its CLARA_HOME (#45474).
  ...
  if (!mutatingProfile) {
    return handleClaraApiRequest(request)
  }

  const releaseProfileDeletion = profileDeletionGate.acquire(mutatingProfile)

  return handleClaraApiRequest(request).finally(releaseProfileDeletion)
})
...
ipcMain.handle('clara:ambient:claim', (_event, key) => !claimedAmbientCue(String(key ?? '')))

ipcMain.handle('clara:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('clara:focus-session', payload.sessionId)
    }

    // Plugin / session-less activation — serializable path (+ optional notifyId
    // for renderer callbacks). Same vocabulary as clara://index-network/….
    if (payload?.activate || payload?.notifyId) {
      mainWindow.webContents.send('clara:notification-activate', {
  ...
  notification.on('action', (_actionEvent, index) => {
    ...
    if (payload?.sessionId && !payload?.notifyId && !payload?.activate) {
      mainWindow.webContents.send('clara:notification-action', { sessionId: payload.sessionId, actionId: action.id })
...
ipcMain.handle('clara:data-url-read-max:get', () => ({
...
ipcMain.handle('clara:data-url-read-max:set', (_event, maxMb) => {
...
ipcMain.handle('clara:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('clara:readFileDataUrlForAttach', async (_event, filePath) => {
  return readFileDataUrlForIpc(filePath, {
...
ipcMain.handle('clara:readFileText', async (_event, filePath) => {
...
// `clara:readFileText` is the *preview* read and silently truncates at
...
ipcMain.handle('clara:readPluginSource', async (_event: unknown, filePath: unknown) => {
  const { resolvedPath, stat } = await resolveReadableFileForIpc(filePath, {
...
ipcMain.handle('clara:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('clara:writeClipboard', (_event, text) => {
...
ipcMain.handle('clara:selectSavePath', async (_event, options: any = {}) => {
  const result = await dialog.showSaveDialog(mainWindow, {
...
ipcMain.handle('clara:readClipboard', () => clipboard.readText())

ipcMain.handle('clara:saveGatewayFile', (_event, payload) => saveGatewayFile(payload))

ipcMain.handle('clara:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))
...
ipcMain.handle('clara:context-menu:edit', (event, command) => {
...
ipcMain.handle('clara:context-menu:copy-image', event => {
...
ipcMain.handle('clara:context-menu:spellcheck', (event, action) => {
...
ipcMain.handle('clara:context-menu:guest-add-word', (_event, payload) => {
...
ipcMain.handle('clara:capturePreview', async (_event, payload) => {
  const guest = electronWebContents.fromId(Number(payload?.webContentsId))

  return capturePreviewContents(guest, payload?.rect, payload?.viewport)
})

ipcMain.handle('clara:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('clara:saveClipboardImage', async () => {
...
ipcMain.handle('clara:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('clara:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('clara:watchDirectory', (_event, dir) => watchDirectory(String(dir || '')))

ipcMain.handle('clara:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))
...
ipcMain.on('clara:active-work', (event, payload) => {
  const id = event.sender.id
...
ipcMain.on('clara:titlebar-theme', (_event, payload) => {
...
ipcMain.on('clara:native-theme', (_event, mode) => {
...
// __PROT_3_synchroclara__ writeFileSync and doing one per tick blocks the main process
...
// Answered __PROT_6_synchroclaraly__ so preload can publish the verdict before the
...
ipcMain.on('clara:translucency:support', event => {
...
ipcMain.on('clara:keep-awake', (_event, on) => {
...
ipcMain.handle('clara:quick-entry:settings:get', async () => {
...
ipcMain.handle('clara:quick-entry:settings:set', async (_event, patch) => {
...
ipcMain.on('clara:quick-entry:submit', (_event, payload) => {
  hideQuickEntryWindow()
  ...
  mainWindow.webContents.send('clara:quick-entry:submit', {
...
ipcMain.on('clara:quick-entry:state', (_event, payload) => {
  quickEntryLastState = payload ?? null

  if (quickEntryWindow && !quickEntryWindow.isDestroyed()) {
    quickEntryWindow.webContents.send('clara:quick-entry:state', payload)
  }
})

ipcMain.on('clara:quick-entry:dismiss', () => hideQuickEntryWindow())
...
ipcMain.on('clara:devtools:disable-f12', (_event, on) => {
...
ipcMain.handle('clara:openExternal', (_event, url) => {
...
// per-session secondary windows spawned via `clara:window:openSession`).
...
ipcMain.handle('clara:find-in-page', async (event, query, options) => {
  const win = BrowserWindow.fromWebContents(event.sender)
  ...
  // The match count still arrives __PROT_8_asynchroclaraly__ via `found-in-page`; this
  // reply only acknowledges that Chromium has begun returning this request.
  return { count: 0 }
})

ipcMain.handle('clara:stop-find-in-page', event => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('clara:preview:reach', async (event, url) => reachablePreviewUrl(event.sender.id, String(url || '')))

ipcMain.handle('clara:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveClaraCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('clara:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveClaraCwd()
}))

ipcMain.handle('clara:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('clara:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('clara:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('clara:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('clara:resolveFavicon', (_event, url) => resolveFaviconCached(url))

ipcMain.handle('clara:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('clara:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: claraLog.slice(-200) }))
...
ipcMain.on('clara:logs:renderer-error', (_event, report) => {
...
// Embedded terminal PTY host (clara:terminal:*) — see terminal-ipc.ts.
...
ipcMain.handle('clara:updates:check', async () =>
...
ipcMain.handle('clara:updates:apply', async (_event, payload) =>
...
ipcMain.handle('clara:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('clara:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical Clara version (the one `release.py` bumps in
// clara_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Clara version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveClaraVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'clara_cli', '__init__.py')
...
// Renderer-bundle skew: `clara update` moves the SOURCE TREE, but the UI
...
// Re-resolve the live Clara version and push it into the native About panel
// just before showing it, so an in-place `clara update` is reflected without
...
ipcMain.handle('clara:version', async () => {
  const skew = await detectRendererSkew()

  return {
    appVersion: resolveClaraVersion(),
    electronVersion: process.versions.electron,
    nodeVersion: process.versions.node,
    platform: process.platform,
    claraRoot: resolveUpdateRoot(),
...
// The About page's "Restart Clara" button (shown when bundleSwapPending):
...
ipcMain.handle('clara:app:relaunch', async () => {
...
// the actual removal via `clara uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (clara_cli/uninstall.py + clara_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_CLARA_ROOT
  ...
  const fallback = () => ({
    clara_home: CLARA_HOME,
    agent_installed: isClaraSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'clara_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, CLARA_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Clara agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import clara_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_CLARA_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `clara-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `clara-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('clara:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('clara:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('clara:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('clara:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// clara:// deep links (e.g. clara://blueprint/morning-brief?time=08:00,
// clara://mcp/install?name=NAME&config=B64 — the vendor "Add to Clara"
// button, or clara://plugin/install?repo=owner/repo). Dev
// (`CLARA_DESKTOP_DEV_SERVER`) registers clara-dev:// instead — bare
// Electron or a stale OS handler often owns clara:// on dev machines.
...
// ---------------------------------------------------------------------------
const CLARA_PROTOCOL = DEV_SERVER ? 'clara-dev' : 'clara'
/** Schemes accepted when parsing inbound URLs (dev accepts both). */
const DEEPLINK_SCHEMES = DEV_SERVER ? ['clara-dev', 'clara'] : ['clara']
...
function handleDeepLink(url) {
  ...
  // clara://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('clara:deep-link', payload)
...
ipcMain.handle('clara:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${CLARA_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(CLARA_PROTOCOL, process.execPath, [entry])
    } else {
      app.setAsDefaultProtocolClient(CLARA_PROTOCOL)
    }

    rememberLog(`[deeplink] registered ${CLARA_PROTOCOL}:// handler`)
  } catch (err) {
...
// second-instance argv. Without the lock a second `clara://` launch spawns a
...
if (!isPrimaryInstance) {
  ...
  // startup (shortcut registration, createWindow → startClara), whose
...
app.whenReady().then(() => {
  ...
  // startClara() finishes resolving the configured backend.
  const primaryProfile = primaryProfileKey()
  ...
  // Win/Linux cold start: the launching clara:// URL is in our own argv.
...
app.on('before-quit', event => {
  ...
  // quitting Clara never keeps another app's chord hostage.
