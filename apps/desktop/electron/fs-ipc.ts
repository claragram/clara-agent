...
// hardening, CLARA_HOME resolution, and the git binary stay injected.
...
export function registerFsIpc({
  ...
  ipcMain.handle('clara:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

  ipcMain.handle('clara:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

  // Reveal a path in the OS file manager (Finder / Explorer / Files).
  ipcMain.handle('clara:fs:reveal', async (_event, targetPath) => {
  ...
  ipcMain.handle('clara:fs:openDir', async (_event, dirPath) => {
  ...
  // The LOCAL Desktop runtime-plugin root: `<CLARA_HOME>/desktop-plugins`,
  // resolved from the main-process CLARA_HOME (see resolveClaraHome) — NOT from
  // the connected backend. A remote backend reports its own `clara_home` over
  ...
  async function localPluginsRoot(dirName: string): Promise<string> {
    ...
    // profiles/<name>/, matching the profile-scoped clara_home the backend
    // reported before this resolver existed. 'default'/unset pins the global root.
    const profile = readActiveDesktopProfile()
    const base = profile && profile !== 'default' ? path.join(claraHome, 'profiles', profile) : claraHome
  ...
  ipcMain.handle('clara:fs:desktopPluginsRoot', async () => localPluginsRoot('desktop-plugins'))

  // The LOCAL logs root (`<CLARA_HOME>/logs`, profile-aware) — the error
  // card's "Open Logs" action reveals agent.log/gateway.log without the user
  // knowing where CLARA_HOME lives. Same Electron-local resolution as the
  // plugin roots: valid in every connection mode, created on demand.
  ipcMain.handle('clara:fs:logsRoot', async () => localPluginsRoot('logs'))

  // The LOCAL agent-plugin root (`<CLARA_HOME>/plugins`), same Electron-local
  ...
  ipcMain.handle('clara:fs:agentPluginsRoot', async () => localPluginsRoot('plugins'))

  ipcMain.handle('clara:plugin:probe', async (_event, payload) => {
  ...
  ipcMain.handle('clara:plugin:installDesktop', async (_event, payload) => {
    const identifier = String(payload?.identifier || payload?.repo || '').trim()
  ...
  ipcMain.handle('clara:fs:rename', async (_event, targetPath, newName) => {
  ...
  ipcMain.handle('clara:fs:writeText', async (_event, filePath, content) => {
  ...
  ipcMain.handle('clara:fs:trash', async (_event, targetPath) => {
    const target = String(targetPath || '').trim()
