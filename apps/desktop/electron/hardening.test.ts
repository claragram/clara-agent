...
function withTempDir(run: (dir: string) => void) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'clara-secret-file-'))
...
test('attachment data URL helper reads bytes above the preview default without changing that limit', async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'clara-desktop-large-attachment-'))
...
test('the written file is owner-only even where chmod does nothing', () => {
  ...
  withTempDir(dir => {
    ...
    // for free. __PROT_0_Synchroclara__ and restored in `finally`, and the electron project
...
test('resolveRequestedPathForIpc resolves relative paths from the trimmed base directory', () => {
  const baseDir = path.join(os.tmpdir(), 'clara-desktop-base')
...
test('resolveReadableFileForIpc validates existence type size and sensitivity', async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'clara-desktop-hardening-'))
...
test('resolveReadableFileForIpc blocks common sensitive files', async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'clara-desktop-sensitive-'))
...
test('resolveReadableFileForIpc blocks symlinks whose realpath is sensitive', async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'clara-desktop-realpath-'))
...
test('resolveDirectoryForIpc accepts directories and rejects invalid directory targets', async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'clara-desktop-dir-'))
...
test('resolveDirectoryForIpc accepts directory symlinks or junctions', async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'clara-desktop-dir-link-'))
...
// (see windows-clara-resolution.test.ts). These pin the propagation the PR
...
test('connection-config save and apply IPC handlers route payloads through coerceDesktopConnectionConfig', () => {
  ...
  for (const channel of ['clara:connection-config:save', 'clara:connection-config:apply']) {
