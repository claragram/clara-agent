...
export function registerHudIpc({
  ...
  ipcMain.on('clara:hud:native-drag', event => {
    event.returnValue = hudWindowing().move === 'native-drag'
  })

  ipcMain.on('clara:hud:windowing', event => {
  ...
  ipcMain.on('clara:hud:workspace-transfer', (event, transferring) => {
  ...
  ipcMain.handle('clara:hud:open', async (_event, request) => {
    openHudWindow(
  ...
  ipcMain.handle('clara:hud:frost', (_event, showing) => {
  ...
  // reaches the bar.
  ipcMain.on('clara:hud:ignore-mouse', (_event, ignore) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('clara:hud:begin-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('clara:hud:end-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('clara:hud:move-by', (event, delta) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('clara:hud:set-bounds', (event, bounds) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('clara:hud:reset-layout', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('clara:hud:session', (event, sessionId) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('clara:hud:close', async () => {
    closeHudWindow()
