...
// Relative, not `@clara/shared`: the electron bundle is built by esbuild with
...
function encryptDesktopSecret(value, safeStorageApi, options: { allowPlainText?: boolean } = {}) {
  ...
  if (!encryptionAvailable) {
    ...
    throw new Error(
      'Secure token storage is unavailable (no OS keyring service was found), so Clara Desktop cannot save remote gateway tokens. ' +
        ...
        'or set CLARA_DESKTOP_REMOTE_URL and CLARA_DESKTOP_REMOTE_TOKEN in your environment.'
    )
  }
  ...
  try {
    ...
    throw new Error(
      `Failed to encrypt the remote gateway token for secure storage${detail}. ` +
        'Set CLARA_DESKTOP_REMOTE_URL and CLARA_DESKTOP_REMOTE_TOKEN in your environment as a fallback.'
    )
  }
}
...
// CLARA_DESKTOP_REMOTE_* env vars.
