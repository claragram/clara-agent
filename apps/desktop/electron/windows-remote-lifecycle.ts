...
const READY_RE = /^CLARA_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitClaraPath = '') {
  const explicit = psLiteral(explicitClaraPath)
  ...
  const script = [
    ...
    '$claraHome=$env:CLARA_HOME',
    'if(-not $claraHome){$claraHome=Join-Path $env:LOCALAPPDATA "clara"}',
    'Assert-NoReparse $claraHome $true',
    '$candidate=[IO.Path]::Combine($claraHome, "clara-agent\\venv\\Scripts\\clara.exe")',
    ...
    '$profileCandidate=[IO.Path]::Combine($HOME, "clara-agent\\.venv\\Scripts\\clara.exe")',
    ...
    '$fallbackHomeCandidate=Join-Path $claraHome "clara-agent\\venv\\Scripts\\clara.exe"',
    '$fallbackProfileCandidate=Join-Path $HOME "clara-agent\\.venv\\Scripts\\clara.exe"',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command clara.exe -ErrorAction SilentlyContinue',
    ...
    '$clara=$null',
    'foreach($candidate in $candidates){Assert-NoReparse $candidate $true;$candidatePython=[IO.Path]::Combine([IO.Path]::GetDirectoryName($candidate), "python.exe");Assert-NoReparse $candidatePython $true;try{$item=Get-Item -LiteralPath $candidate -Force -ErrorAction Stop;if(($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -eq 0 -and -not $item.PSIsContainer){$clara=$item.FullName;break}}catch [Management.Automation.ItemNotFoundException]{continue}}',
    'if(-not $clara){throw "Clara is not installed on the remote Windows host."}',
    'Assert-NoReparse $clara $false',
    'if($explicit -and $clara -ne $explicit){throw "The configured Clara path is not an executable file."}',
    '$python=[IO.Path]::Combine([IO.Path]::GetDirectoryName($clara), "python.exe")',
    'Assert-NoReparse $python $false',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;claraHome=$claraHome;claraPath=$clara;python=$python}|ConvertTo-Json -Compress'
  ].join(';')

  return JSON.parse((await ssh.exec(powerShellCommand(script))).trim())
}

function windowsUpdateMarkerProbeCommand(claraHome) {
  const script = [
    '$ErrorActionPreference="Stop"',
...
public static class ClaraMarkerNoFollow {
...
`,
    ...
    '}',
    '}',
    `$home=${psLiteral(claraHome)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".clara-update-in-progress"',
    '$result="UNCERTAIN"',
    '$stream=$null;$memory=$null',
    'try{',
    'Assert-NoReparse $marker $true',
    'if(-not (Test-Path -LiteralPath $marker -PathType Leaf)){$result="CLEAR"}else{$stream=[ClaraMarkerNoFollow]::OpenRead($marker)',
 ...
 */
async function assertWindowsRemoteInstallUpdateClear(ssh, claraHome) {
  let observation = ''

  try {
    observation =
      String(await ssh.exec(windowsUpdateMarkerProbeCommand(claraHome)))
        .replace(/^\uFEFF/, '')
        .trim()
        .split(/\r?\n/)
        .pop() || ''
...
async function detectRemotePlatform(ssh, explicitClaraPath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitClaraPath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'clara_cli.windows_ssh_runtime', operation, ...args]
...
function atomicWindowsSpawnCommand(runtime, reservation: any = {}) {
  const argv = [runtime.python, '-m', 'clara_cli.windows_ssh_runtime', 'spawn']
  const helper = operation => [runtime.python, '-m', 'clara_cli.windows_ssh_runtime', operation]

  const script = [
    '$ErrorActionPreference="Stop"',
    `$home=${psLiteral(runtime.claraHome)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".clara-update-in-progress"',
    ...
    reservation.ownershipId
      ...
      : '  if($LASTEXITCODE -ne 0){exit $LASTEXITCODE}',
    reservation.ownershipId
      ? `  $spawned=$spawnLines[-1]|ConvertFrom-Json; $lock=[ordered]@{schemaVersion=2;protocolVersion=1;ownershipId=${psLiteral(reservation.ownershipId)};spawnNonce=${psLiteral(reservation.spawnNonce)};pid=[int]$spawned.pid;creationTimeNs=[string]$spawned.creationTimeNs;port=0;profile=${psLiteral(reservation.profile)};claraPath=${psLiteral(reservation.claraPath)};claraHome=${psLiteral(reservation.claraHome)};tokenFingerprint=${psLiteral(reservation.tokenFingerprint)};startedAt=${psLiteral(reservation.startedAt)}}|ConvertTo-Json -Compress; ` +
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.claraPath === 'string' &&
    typeof lock.claraHome === 'string'
  )
}
...
function reusableWindowsLock(lock, state, profile, reuseToken, runtime) {
  return Boolean(
    ...
    lock.claraPath === runtime.claraPath &&
    lock.claraHome === runtime.claraHome
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertBootstrapNotSuperseded(signal)
  const runtime = await probeWindowsRemote(ssh, remoteClaraPath)
  await assertWindowsRemoteInstallUpdateClear(ssh, runtime.claraHome)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.claraPath])

  if (!inspection.supported) {
    const error: any = new Error('Update Clara on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.claraPath = inspection.path
  const claraVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located clara at ${runtime.claraPath}`)
  ...
  try {
    ...
    spawned = await atomicWindowsSpawn(
      ssh,
      runtime,
      JSON.stringify({ ownershipId, spawnNonce, profile, claraPath: runtime.claraPath }),
      {
        ownershipId,
        spawnNonce,
        profile,
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForClara(baseUrl, token)
    assertBootstrapNotSuperseded(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="Clara SSH"', 'powershell.exe -NoLogo')
