 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote Clara dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the Clara install on the remote (login-shell probe),
 ...
 *     `CLARA_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForClara in.
 *
 * The minted CLARA_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const READY_RE = /^CLARA_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m
const REMOTE_LOCK_DIR = '~/.clara/desktop-ssh'
...
// Resolve the remote clara executable. An EXPLICIT path is honored strictly
...
async function locateClara(ssh, remoteClaraPath) {
  const resolveLauncher = async (candidate: string) => {
    // Return the candidate path directly. The clara binary or wrapper script
    ...
    // See https://github.com/claraprise/clara-agent/issues/74411
    return candidate
  }
  ...
  if (remoteClaraPath) {
    if (await isExecutable(remoteClaraPath)) {
      return resolveLauncher(remoteClaraPath)
    }

    const err: any = new Error(
      `The Clara path you set is not an executable on the remote host: "${remoteClaraPath}". ` +
        'Check the path (it must be the full path to the `clara` binary on the remote, e.g. ' +
        '~/clara-agent/.venv/bin/clara), or clear it to auto-detect.'
    )

    err.kind = 'clara-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v clara')}`)).trim()
  ...
  candidates.push('~/.local/bin/clara')
  candidates.push('/usr/local/bin/clara')
  candidates.push('~/.clara/clara-agent/venv/bin/clara')
  ...
  const err: any = new Error(
    'Clara is not installed on the remote host (could not find a `clara` executable). ' +
      'Install it on the remote with:  curl -fsSL https://agent.claraprise.com/install.sh | sh  ' +
...
// Probe the resolved binary's version string (first line of `<clara> --version`,
// e.g. "Clara Agent v0.18.2 ..."), or '' on failure. Surfaces WHICH clara a
// connection uses, so a stale/unexpected install is visible.
async function probeClaraVersion(ssh, claraPath) {
  try {
    const out = (await ssh.exec(`${expandRemotePath(claraPath)} --version 2>&1`)).trim()
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". Clara Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The CLARA_HOME the remote dashboard will use (explicit env wins, else
// ~/.clara). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemoteClaraHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${CLARA_HOME:-$HOME/.clara}"')).trim().split('\n').pop()

    return out || '~/.clara'
  } catch (cause) {
    const error: any = new Error('Could not resolve the remote Clara home.')
...
marker=home/'.clara-update-in-progress'
try:
 ...
 * bytes; it never imports or executes code from the changing Clara checkout.
 ...
 */
async function assertRemoteInstallUpdateClear(ssh, claraHome) {
  const home = assertSafeRemoteHome(claraHome)
  let observation = ''
  ...
  try {
    ...
    const error: any = new Error('Could not prove that the remote Clara install is clear for SSH startup.')
  ...
  const error: any = new Error(
    live
      ? `Remote Clara update process ${live[1]} is still running; SSH startup is paused.`
      : 'The remote Clara update marker is unreadable or malformed; refusing SSH startup.'
  )
...
async function listRemoteClaraProfiles(ssh) {
  const home = assertSafeRemoteHome(await probeRemoteClaraHome(ssh))
  ...
  try {
    ...
    const error: any = new Error('Could not list remote Clara profiles.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
function assertSafeRemoteHome(home) {
  ...
  if (!/^(\/|~\/)[A-Za-z0-9._/+-]+$/.test(value) || value.includes('..')) {
    const error: any = new Error('Unsafe remote Clara home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'claraPath', 'claraHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(
  ...
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !claraPath) {
    return false
  }
  ...
  try {
    const script =
      ...
      `pid=${Number(pid)}\n` +
      `expected=os.path.expanduser(${shq(claraPath)})\n` +
      // The installer-facing launcher is intentionally preserved for invocation
      // (#74411), but it may `exec python <install-dir>/clara`, leaving neither
      // launcher nor CLARA_HOME-derived entrypoint in argv. The ownership-scoped
      // token path + random nonce + exact profile below are the alternative proof.
      `clara_home=os.path.expanduser(${shq(claraHome)}) if ${shq(claraHome)} else ""\n` +
      'expected_entries={expected}\n' +
      'if clara_home:\n' +
      ' expected_entries.add(os.path.join(clara_home,"clara-agent","venv","bin","clara"))\n' +
...
function buildOwnedStaleTerminationCommand(lock, ownershipId) {
  ...
  const expectedPath = expandRemotePath(lock.claraPath)
  const expectedHome = lock.claraHome ? expandRemotePath(lock.claraHome) : "''"
  ...
  const executableMatch = lock.claraHome
...
function lockMatchesManagedUpdateScope(lock, expected) {
  return Boolean(
    ...
    lock.claraPath === expected.claraPath &&
    lock.claraHome === expected.claraHome
  )
}
...
expected_path=os.path.expanduser(${py(lock.claraPath)})
clara_home=os.path.expanduser(${py(lock.claraHome)})
expected_entries={expected_path,os.path.join(clara_home,"clara-agent","venv","bin","clara")}
...
result=None
try:
 result=subprocess.run(["sh","-c",payload,"clara-update-mutex",str(fd)],pass_fds=(fd,),check=False)
finally:
 os.close(fd)
...
function buildSpawnCommand(claraPath, profile, opts: any = {}) {
  const clara = expandRemotePath(claraPath)
  ...
  const marker = expandRemotePath(`${remoteInstallRoot(opts.claraHome || '~/.clara')}/.clara-update-in-progress`)

  const updateMutex = expandRemotePath(
    `${remoteInstallRoot(opts.claraHome || '~/.clara')}/.clara-update-in-progress.mutex`
  )
  ...
  const dashCmd =
    ...
    `exec env CLARA_DESKTOP=1 ${clara} ${profileArgs}${subCmd}`

  const detachedShell = `eval "exec $1>&-"; ${dashCmd} </dev/null >> ${logPath} 2>&1 & echo $!`
  const detachedSpawn = `child=$("$(command -v setsid || echo nohup)" sh -c ${shq(detachedShell)} clara-update-child "$1" & echo $!)`
...
async function remoteSupportsSshOwnership(ssh, claraPath) {
  const clara = expandRemotePath(claraPath)

  const out = await ssh.exec(
    `help="$(${clara} serve --help 2>&1)"; ` +
...
async function spawnRemoteDashboard(
  ssh,
  { claraPath, profile, token, ownershipId, claraHome = '~/.clara', assertInstallClear = async () => {} }
) {
  if (!(await remoteSupportsSshOwnership(ssh, claraPath))) {
    const err: any = new Error(
      'The remote Clara install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update Clara on the remote host to continue using Desktop SSH mode.'
    )
  ...
  try {
    ...
    out = await ssh.exec(
      buildSpawnCommand(claraPath, profile, {
        spawnNonce,
        tokenFilePath,
        logPath,
        claraHome,
 ...
 * opened SshConnection, forward/pickLocalPort/waitForClara, a token-gated
...
async function connect(deps) {
  ...
  const claraHome = await probeRemoteClaraHome(ssh)
  await assertRemoteInstallUpdateClear(ssh, claraHome)
  const claraPath = await locateClara(ssh, remoteClaraPath)
  log(`located clara at ${claraPath}`)
  const claraVersion = await probeClaraVersion(ssh, claraPath)

  if (claraVersion) {
    log(`remote clara version: ${claraVersion}`)
  }
  ...
  if (isLockfileSkew(lock)) {
    ...
    const error: any = new Error(
      `The remote ownership record ${lpath} does not match this Clara Desktop build (${lock.reason}). ` +
  ...
  if (lock) {
    ...
    const reusable =
      ...
      lock.claraPath === claraPath &&
      lock.claraHome === claraHome
  ...
  const spawned = await spawnRemoteDashboard(ssh, {
    ...
    ownershipId,
    claraHome,
    assertInstallClear: () => assertRemoteInstallUpdateClear(ssh, claraHome)
  })
  ...
  try {
    ...
    assertBootstrapNotSuperseded(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForClara(baseUrl, spawnToken)
    assertBootstrapNotSuperseded(signal)
