// Unit tests for the pure Windows `clara` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvClaraCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `clara` shim shadows
//      the real clara.cmd/clara.exe.
...
//   3. resolveVenvClaraCommand() — must probe the venv python via
//      canImportClaraCli() before trusting it, or a broken venv gets
...
test('chooseUpdaterArgs: gentle --update when both updater runtime files exist', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvClara: true, hasVenvPython: true }, 'main'), [
    '--update',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: marker-only install uses --repair when the venv is gone', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvClara: false, hasVenvPython: false }, 'main'),
    ['--repair', '--branch', 'main']
  )
})

test('chooseUpdaterArgs: partial updater runtimes use --repair', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvClara: false, hasVenvPython: true }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvClara: true, hasVenvPython: false }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: passes the branch through unchanged in both modes', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvClara: true, hasVenvPython: true }, 'release/1.2'),
    ['--update', '--branch', 'release/1.2']
  )
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvClara: false, hasVenvPython: false }, 'release/1.2'),
    ['--repair', '--branch', 'release/1.2']
  )
})

function makeDeps(overrides: Partial<Parameters<typeof resolveVenvClaraCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportClaraCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    claraHome: '/fake/clara-home',
...
test('resolveVenvClaraCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvClaraCommand('/root/venv/Scripts/clara.exe', [], deps), null)
})

test('resolveVenvClaraCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvClaraCommand('/root/venv/Scripts/clara.cmd', [], deps), null)
})

test('resolveVenvClaraCommand: returns null when the basename is not clara/clara.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvClaraCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvClaraCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvClaraCommand('/root/venv/bin/clara.exe', [], deps), null)
})

test('resolveVenvClaraCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvClaraCommand('/root/venv/Scripts/clara.exe', [], deps), null)
})

test('resolveVenvClaraCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportClaraCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvClaraCommand('/root/venv/Scripts/clara.exe', ['serve'], deps)
...
test('resolveVenvClaraCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvClaraCommand('/root/venv/Scripts/clara.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'clara_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvClaraCommand: is case-insensitive on clara.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvClaraCommand('/root/venv/Scripts/CLARA.EXE', [], deps))
  assert.ok(resolveVenvClaraCommand('/root/venv/SCRIPTS/clara.exe', [], deps))
})
