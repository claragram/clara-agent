...
//   CLARA_WIN_SSH_HOST   ssh alias/host of the Windows box
//   CLARA_WIN_SSH_USER   remote user
//   CLARA_WIN_SSH_CLARA absolute path to the remote clara.exe under test
const liveHost = process.env.CLARA_WIN_SSH_HOST || ''
const liveUser = process.env.CLARA_WIN_SSH_USER || ''
const configuredClara = process.env.CLARA_WIN_SSH_CLARA || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-Clara-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredClara)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredClara.replace('clara.exe', 'python.exe')}' -m clara_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredClara.replace('clara.exe', 'python.exe')
        const terminate = `& '${python}' -m clara_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.claraPath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m clara_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m clara_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
