// Profile-delete routing logic for the `clara:api` IPC handler.
//
...
// be rejected: spawning a fresh backend would call ensure_clara_home() and
...
/** Parse a `clara:api` request into the profile name a DELETE targets. */
 ...
 * still removing the profile and recreate its CLARA_HOME.
 *
...
/**
 * Route the next `clara:api` request away from the primary/window backend
 ...
 * ensure_clara_home() recreates the directory the delete just removed.
 */
