 ...
 *
 * The staged Tauri `clara-setup.exe` has no self-update path, so every
 ...
 * checkout instead: every `clara update` refreshes the code that drives the
 ...
 * and QUITS, the script waits it out, runs `clara update`, swaps/relaunches
 * the app, and writes .clara-update-result.json. With the app gone before
 * the update starts, the CLARA_DESKTOP_CHILD_PID reaper-exclusion dance is
 ...
 * `clara update` card — old checkouts pull the script on their next update).
 */
 ...
 *
 * The Tauri installer self-copies into CLARA_HOME on *every* platform
 * (`clara-setup.exe` on Windows, `clara-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_clara_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `clara update`,
...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(claraHome, 'clara-setup.exe')
 ...
 *
 * `copy_self_to_clara_home` deliberately no-ops during `--update`
 ...
 * "Another Clara update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `clara update`, which
 ...
 * the installer is written to CLARA_HOME at install/repair time, making mtime
 ...
 * Node reports exec failures __PROT_0_asynchroclaraly__) or an early `exit`. A failed
