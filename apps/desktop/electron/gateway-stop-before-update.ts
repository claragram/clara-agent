 ...
 *
 * So we delegate to `clara gateway stop --all`: the CLI discovers every
 ...
 * force-kills survivors — the same logic `clara update`'s
...
export function stopGatewayBeforeUpdate(
  ...
  return runGatewayLifecycleCommand(claraCliPath, ['gateway', 'stop', '--all'], deps)
}
 ...
 */
export function startGatewaysAfterUpdateAbort(claraCliPath: string, deps: StopGatewayBeforeUpdateDeps = {}): boolean {
  return runGatewayLifecycleCommand(claraCliPath, ['gateway', 'start', '--all'], deps)
}

function runGatewayLifecycleCommand(claraCliPath: string, args: string[], deps: StopGatewayBeforeUpdateDeps): boolean {
  ...
  if (deps.spy) {
    deps.spy(claraCliPath, args)
  }

  if (!existsSync(claraCliPath)) {
    return false
  }

  try {
    exec(claraCliPath, args, {
