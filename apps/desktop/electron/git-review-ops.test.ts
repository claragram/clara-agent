...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'clara-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'clara-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'Clara Test'], { cwd: dir })
