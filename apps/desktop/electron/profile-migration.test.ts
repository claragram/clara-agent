 ...
 *      top of startClara() in main.ts, before primaryProfileKey() is read); the
...
function baseDeps(overrides: Record<string, unknown> = {}) {
  ...
  return {
    legacyActivePath: '/home/u/.clara/active_profile',
    claraHome: '/home/u/.clara',
    profilesRoot: '/home/u/.clara/profiles',
...
test('findRunningGatewayProfiles returns [] when no pid files exist', () => {
  const fs = makeFs({ '/home/u/.clara/profiles/coder': { dir: true } })
  assert.deepEqual(
    findRunningGatewayProfiles('/home/u/.clara/profiles', ['coder'], {
      ...fs,
      isClaraProcess: () => true
    }),
    []
...
test('findRunningGatewayProfiles drops stale (non-clara) recycled PIDs', () => {
  // Two profiles have pid files, but only coder's pid is a live clara process.
  // The recycled PID at 5678 belongs to an unrelated process (e.g. Chrome).
  const fs = makeFs({
    '/home/u/.clara/profiles/coder/gateway.pid': { content: '{"pid":1234}' },
    '/home/u/.clara/profiles/writer/gateway.pid': { content: '{"pid":5678}' }
  })

  const deps = {
    ...fs,
    isClaraProcess: (pid: number) => pid === 1234
  }

  assert.deepEqual(findRunningGatewayProfiles('/home/u/.clara/profiles', ['coder', 'writer'], deps), ['coder'])
})

test('findRunningGatewayProfiles tolerates malformed pid files', () => {
  const fs = makeFs({
    '/home/u/.clara/profiles/coder/gateway.pid': { content: '{not json' }
  })
...
test('findRunningGatewayProfiles drops non-integer and non-positive PIDs', () => {
  const fs = makeFs({
    '/home/u/.clara/profiles/coder/gateway.pid': { content: '{"pid":-1}' },
    '/home/u/.clara/profiles/writer/gateway.pid': { content: '{"pid":1.5}' },
    '/home/u/.clara/profiles/extra/gateway.pid': { content: '{"pid":0}' }
  })

  assert.deepEqual(
    findRunningGatewayProfiles('/home/u/.clara/profiles', ['coder', 'writer', 'extra'], {
      ...fs,
      isClaraProcess: () => true
    }),
    []
...
test('findRunningGatewayProfiles preserves order of allProfiles', () => {
  const fs = makeFs({
    '/home/u/.clara/profiles/coder/gateway.pid': { content: '{"pid":1}' },
    '/home/u/.clara/profiles/writer/gateway.pid': { content: '{"pid":2}' }
  })

  const deps = { ...fs, isClaraProcess: () => true }
  assert.deepEqual(findRunningGatewayProfiles('/home/u/.clara/profiles', ['coder', 'writer'], deps), [
    'coder',
    'writer'
  ])
})
...
test('listProfileDirs includes default and any name passing the regex', () => {
  const fs = makeFs({
    '/home/u/.clara/profiles/default': { dir: true },
    '/home/u/.clara/profiles/coder': { dir: true },
    '/home/u/.clara/profiles/writer': { dir: true }
  })
...
test('listProfileDirs skips files (not directories) and invalid names', () => {
  const fs = makeFs({
    '/home/u/.clara/profiles/default': { dir: true },
    '/home/u/.clara/profiles/some-file.txt': { dir: false },
    '/home/u/.clara/profiles/UPPERCASE': { dir: true } // regex rejects
  })
...
test('decideMigration suppresses write when best is default (single-profile fallback)', () => {
  ...
  // default-primary and we leave it alone. Default's DB is $CLARA_HOME/state.db,
  // not profiles/default/state.db.
  const deps = baseDeps()

  const d = decideMigration(null, [], ['default', 'coder'], deps, p => (p.endsWith('/.clara/state.db') ? 99 : 50))
...
test('migrateActiveProfileIfMissing is a no-op when a user-selected preference file exists', () => {
  ...
  const fs = makeFs({
    ...
    '/home/u/.clara/profiles/coder': { dir: true },
    '/home/u/.clara/profiles/writer': { dir: true },
    '/home/u/.clara/profiles/writer/state.db': { size: 400 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing writes legacy choice with no _migrated flag', () => {
  ...
  const fs = makeFs({
    '/home/u/.clara/active_profile': { content: 'coder' },
    '/home/u/.clara/profiles/coder': { dir: true },
    '/home/u/.clara/profiles/writer': { dir: true }
  })
...
test('migrateActiveProfileIfMissing writes heuristic choice with _migrated=true', () => {
  ...
  const fs = makeFs({
    '/home/u/.clara/profiles/coder/state.db': { size: 50 * 1024 * 1024, mtime: NOW - 86_400_000 },
    '/home/u/.clara/profiles/writer/state.db': { size: 200 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing is a no-op for single-profile (default-only) installs', () => {
  ...
  // Production default DB is ~/.clara/state.db, not profiles/default/state.db.
  let written: unknown = null

  const fs = makeFs({
    '/home/u/.clara/state.db': { size: 10 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing prefers a single running gateway over heuristics', () => {
  ...
  const fs = makeFs({
    '/home/u/.clara/profiles/coder/gateway.pid': { content: '{"pid":42}' },
    '/home/u/.clara/profiles/writer/state.db': { size: 500 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })

  const deps = baseDeps({
    ...fs,
    isClaraProcess: (pid: number) => pid === 42,
    writeJson: (_p: string, payload: unknown) => {
      written = payload
    }
  })
...
// ---------------------------------------------------------------------------
// Production layout: default is ~/.clara, not ~/.clara/profiles/default
// ---------------------------------------------------------------------------

test('profileStateDbPath puts default at claraHome, named under profilesRoot', () => {
  assert.equal(profileStateDbPath('default', '/home/u/.clara', '/home/u/.clara/profiles'), '/home/u/.clara/state.db')
  assert.equal(
    profileStateDbPath('conduit', '/home/u/.clara', '/home/u/.clara/profiles'),
    '/home/u/.clara/profiles/conduit/state.db'
  )
})

test('profileGatewayPidPath puts default at claraHome', () => {
  assert.equal(
    profileGatewayPidPath('default', '/home/u/.clara', '/home/u/.clara/profiles'),
    '/home/u/.clara/gateway.pid'
  )
  assert.equal(
    profileGatewayPidPath('coder', '/home/u/.clara', '/home/u/.clara/profiles'),
    '/home/u/.clara/profiles/coder/gateway.pid'
  )
})
...
test('findRunningGatewayProfiles sees default gateway.pid at claraHome', () => {
  const fs = makeFs({
    '/home/u/.clara/gateway.pid': { content: '{"pid":99}' },
    '/home/u/.clara/profiles/coder/gateway.pid': { content: '{"pid":11}' }
  })

  assert.deepEqual(
    findRunningGatewayProfiles('/home/u/.clara/profiles', ['default', 'coder'], {
      ...fs,
      claraHome: '/home/u/.clara',
      isClaraProcess: pid => pid === 99
    }),
    ['default']
  )
})
...
test('migrateActiveProfileIfMissing does not pin a tiny named profile over a large default DB', () => {
  ...
  // ~/.clara/profiles/<name>, never scored ~/.clara/state.db, and wrote
  // { profile: named, _migrated: true }.
  let written: unknown = null

  const fs = makeFs({
    '/home/u/.clara/profiles/conduit': { dir: true },
    '/home/u/.clara/state.db': { size: 409 * 1024 * 1024, mtime: NOW - 86_400_000 },
    '/home/u/.clara/profiles/conduit/state.db': { size: 2 * 1024 * 1024, mtime: NOW - 60_000 }
  })
...
test('migrateActiveProfileIfMissing still pins a named profile that actually beats default', () => {
  ...
  const fs = makeFs({
    '/home/u/.clara/profiles/work': { dir: true },
    '/home/u/.clara/state.db': { size: 5 * 1024 * 1024, mtime: NOW - 86_400_000 },
    '/home/u/.clara/profiles/work/state.db': { size: 200 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing does not pin default when only default gateway is running', () => {
  ...
  const fs = makeFs({
    '/home/u/.clara/gateway.pid': { content: '{"pid":7}' },
    '/home/u/.clara/state.db': { size: 10 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })

  const deps = baseDeps({
    ...fs,
    isClaraProcess: (pid: number) => pid === 7,
    writeJson: (_p: string, payload: unknown) => {
      written = payload
    }
  })
