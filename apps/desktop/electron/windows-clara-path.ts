/**
 * windows-clara-path.ts
 *
 * Pure, dependency-injected pieces of Windows `clara` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvClaraCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `clara` shim shadowed the real
 *      clara.cmd/clara.exe; the shim then failed the --version probe and
 ...
 *      and Clara launcher. Marker-only or partial runtimes must use --repair;
 *      only a runnable pair can use --update.
 *   3. resolveVenvClaraCommand() — unwrapWindowsVenvClaraCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `clara`) must
 * not shadow `clara.cmd`/`clara.exe`. The empty entry is kept LAST so
 ...
 * venv Python interpreter and the Clara launcher that drives `clara update`.
...
export interface ResolveVenvClaraCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportClaraCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
...
/**
 * If `command` is a Windows venv `clara`/`clara.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/clara(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m clara_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportClaraCli(). A
 * venv whose update died mid-`pip install` still has python.exe + clara.exe
 ...
 *
 * Returns null when `command` is not a venv clara shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvClaraCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvClaraCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^clara(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Clara at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Clara Python at ${python}`,
    command: python,
