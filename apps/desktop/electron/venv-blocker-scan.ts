...
const SCAN_MODULE = 'clara_cli._scan_venv_blockers'
 ...
 * preview servers. Unknown Python/Clara processes are deliberately ignored.
 */
export async function stopSafeVenvBlockers(
  ...
  for (const process of result.processes) {
    ...
    try {
      await execFn(
        pythonPath,
        ['-m', 'clara_cli._scan_venv_blockers', '--terminate-safe', String(process.pid), String(process.createTime)],
...
export function formatBlockerMessage(result: VenvBlockerScanResult): string {
  const lines = [
    'Update aborted: another Clara process is using this installation.',
    '',
...
export function formatProbeFailedMessage(error?: string): string {
  ...
  return (
    'Update aborted: Desktop could not verify the Clara installation is free.' +
    timeoutDetail +
    '\n\n' +
    'Close other Clara windows and terminals, then retry.  If the problem\n' +
    'persists, run `clara update` in a terminal for detailed diagnostics.'
  )
}
