 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of Clara Agent by spawning
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.clara-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(claraHome) {
  return path.join(claraHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.clara/clara-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(claraHome) {
  if (!claraHome) {
    return null
  }

  const candidate = path.join(claraHome, 'clara-agent', 'scripts', installScriptName())
...
function cachedScriptPath(claraHome, commit) {
  return path.join(bootstrapCacheDir(claraHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/Workprise/clara-agent/${ref}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(claraHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(claraHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, claraHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass CLARA_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          CLARA_HOME: claraHome || process.env.CLARA_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, claraHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, claraHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--clara-home', claraHome]
...
async function fetchManifest({ scriptPath, installerKind, emit, claraHome, activeRoot, installStamp, pinCommit }) {
  const isPosix = installerKind === 'posix'

  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, claraHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, claraHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(claraHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, claraHome, emit })
