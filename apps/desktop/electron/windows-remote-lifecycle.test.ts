...
test('Windows relaunch gate refuses live and uncertain markers before executing the remote runtime', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = sshWith(async command => {
      ...
      if (script.includes('Get-Command clara.exe')) {
        return JSON.stringify({
          os: 'Windows',
          arch: 'AMD64',
          claraHome: 'C:\\Users\\alice\\.clara',
          claraPath: 'C:\\Clara\\clara.exe',
          python: 'C:\\Clara\\python.exe'
        })
      }

      if (script.includes('.clara-update-in-progress')) {
        return observation
      }
    ...
    assert.equal(
      scripts.some(script => script.includes('clara_cli.windows_ssh_runtime')),
      false
    )
  }
})
...
test('Windows relaunch gate uses strict install-wide marker parsing and fail-closed PID probing', async () => {
  ...
  await assertWindowsRemoteInstallUpdateClear(ssh, 'C:\\Users\\alice\\.clara\\profiles\\research')
  assert.match(script, /\.clara-update-in-progress/)
...
test('Windows probe validates Clara and Python topology before selection', async () => {
  let script = ''
  ...
  const fallbackJoin = script.indexOf('Join-Path $claraHome')
  ...
  await assert.rejects(
    detectRemotePlatform(
      sshWith(async command => {
        ...
        throw new Error('Clara is not installed on the remote Windows host.')
      })
    ),
    (err: any) => err.kind === 'unsupported-platform' && /Clara is not installed/.test(err.message)
  )
})

test('helper command uses the fixed remote Python entry point and quotes path data', () => {
  const command = helperCommand({ python: "C:\\Program Files\\Clara's\\python.exe" }, 'inspect', [
    'C:\\x y\\clara.exe'
  ])

  const encoded = command.split(' ').pop()!
  const script = Buffer.from(encoded, 'base64').toString('utf16le')
  assert.match(script, /-m' 'clara_cli\.windows_ssh_runtime' 'inspect'/)
  assert.match(script, /Clara''s/)
  assert.match(script, /C:\\x y\\clara\.exe/)
})
...
test('Windows SSH reuse requires the requested remote profile to match the lock', () => {
  ...
  const runtime = { claraPath: lock.claraPath, claraHome: lock.claraHome }
