 ...
 * CLARA_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS, clamped to a 45s floor.
 */
...
test('honors a valid CLARA_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS override', () => {
  const env = { CLARA_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS: '120000' }
  assert.equal(resolvePortAnnounceTimeoutMs(env), 120_000)
})

test('clamps an override below the floor up to the 45s minimum', () => {
  const env = { CLARA_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS: '1000' }
  assert.equal(resolvePortAnnounceTimeoutMs(env), MIN_PORT_ANNOUNCE_TIMEOUT_MS)
})

test('rounds a fractional override', () => {
  const env = { CLARA_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS: '60000.7' }
...
test('falls back to the default for malformed / non-positive overrides', () => {
  for (const bad of ['', 'abc', '0', '-5', 'NaN', undefined]) {
    const env = bad === undefined ? {} : { CLARA_DESKTOP_PORT_ANNOUNCE_TIMEOUT_MS: bad }
    assert.equal(
...
test('resolves with the announced port', async () => {
  ...
  child.stdout.emit('data', 'noise before\nCLARA_DASHBOARD_READY port=54321\n')
  assert.equal(await p, 54321)
})

test('resolves with a CLARA_BACKEND_READY port (headless `serve`)', async () => {
  const child = makeFakeChild()
  const p = waitForDashboardPort(child, 1000)
  child.stdout.emit('data', 'CLARA_BACKEND_READY port=43210\n')
...
test('parses the port even when the line arrives split across chunks', async () => {
  const child = makeFakeChild()
  const p = waitForDashboardPort(child, 1000)
  child.stdout.emit('data', 'CLARA_DASHBOARD_READY po')
...
test('rejects with the timeout message after the deadline', async () => {
  ...
  await assert.rejects(
    ...
    /Timed out waiting for Clara backend port announcement \(20ms\)/
  )
})
...
test('a late announcement after timeout does not throw (listeners torn down)', async () => {
  ...
  assert.doesNotThrow(() => {
    child.stdout.emit('data', 'CLARA_DASHBOARD_READY port=9999\n')
  })
})
...
function mkTmpReadyFile() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'clara-ready-test-'))
...
test('exit-before-announcement error carries the buffered output tail (stdout path)', async () => {
  ...
  const wait = waitForDashboardPortAnnouncement(child, {
    describeOutputTail: () => '\nRecent backend output:\nModuleNotFoundError: clara_cli'
  })

  child.emit('exit', 1, null)

  await assert.rejects(wait, /exited before port announcement \(1\)[\s\S]*ModuleNotFoundError: clara_cli/)
})

test('exit-before-announcement error carries the buffered output tail (ready-file path)', async () => {
  const child = makeFakeChild()
  const readyFile = path.join(os.tmpdir(), `clara-ready-${process.pid}-${Date.now()}.json`)
...
test('bufferedOutput accepts the legacy CLARA_DASHBOARD_READY sentinel too', async () => {
  const child = makeFakeChild()

  const port = await waitForDashboardPortAnnouncement(child, {
    bufferedOutput: () => 'CLARA_DASHBOARD_READY port=43212\n',
    timeoutMs: 500
  })
...
test('bufferedOutput without a sentinel still resolves from later live stdout', async () => {
  ...
  child.stdout.emit('data', Buffer.from('CLARA_BACKEND_READY port=43213\n'))
