 ...
 * console. It writes CLARA_HOME/.clara-update-result.json on every exit
 ...
 * ever surfaces, and the user may not reopen Clara within 30 minutes.
...
export function handoffResultPath(claraHome: string): string {
  return path.join(claraHome, '.clara-update-result.json')
}
...
export function readAndConsumeHandoffResult(
  ...
  const file = handoffResultPath(claraHome)
  let raw: string
