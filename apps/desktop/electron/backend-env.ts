 ...
 * `iter_clara_node_dirs()` in clara_constants.py, which the Electron main
 * process cannot import.
 */
function claraManagedNodePathEntries(
  claraHome,
  ...
  const root = pathModule.join(claraHome, 'node')
...
function buildDesktopBackendPath({
  ...
  const claraNodeDirs = claraManagedNodePathEntries(claraHome, { platform, pathModule })
  const venvBin = venvRoot ? pathModule.join(venvRoot, platform === 'win32' ? 'Scripts' : 'bin') : null
  const saneEntries = platform === 'win32' ? [] : POSIX_SANE_PATH_ENTRIES

  return appendUniquePathEntries([claraNodeDirs, venvBin, currentPath, saneEntries], { delimiter })
}

function normalizeClaraHomeRoot(claraHome, { pathModule = pathModuleForPlatform(process.platform) }: any = {}) {
  if (!claraHome) {
    return claraHome
  }

  const resolved = pathModule.resolve(String(claraHome))
...
function buildDesktopBackendEnv({
  ...
  return {
    ...
    // cp1252, ...). clara_bootstrap sets this inside the child too, but only
