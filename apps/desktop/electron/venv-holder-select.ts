 ...
 *
 * The pre-update handoff kills Clara-OWNED venv daemons (the memory plugin's
 * hindsight daemon) so the updater never races a mapped shim. External
 * holders (a user terminal running `clara`, unrelated scripts) must NOT be
...
/**
 * True when a process is a Clara-owned venv daemon: its exe lives under
 ...
 * spawns DETACHED — it outlives Clara and holds venv shims mapped).
 */
export function isClaraOwnedVenvDaemon(
