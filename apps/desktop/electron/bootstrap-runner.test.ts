...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'clara-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/clara-runner-test',
    sourceRepoRoot: null,
    claraHome: '/tmp/clara-runner-test',
    logRoot: '/tmp/clara-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'clara-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'clara-agent')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/clara-agent',
      claraHome: '/tmp/clara'
    }),
    ['--dir', '/tmp/clara-agent', '--clara-home', '/tmp/clara', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/clara-agent', '--clara-home', '/tmp/clara', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp: stamp,
      activeRoot: '/tmp/clara',
      claraHome: '/tmp/home'
    }),
    ['--dir', '/tmp/clara', '--clara-home', '/tmp/home', '--branch', 'main']
  )
})
