 ...
 *     `X-Clara-Session-Token` header; WS uses `?token=`.
...
// clara_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - clara_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - clara_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / clara #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-clara_session_at', '__Secure-clara_session_at', 'clara_session_at']
const RT_COOKIE_VARIANTS = ['__Host-clara_session_rt', '__Secure-clara_session_rt', 'clara_session_rt']

// The Clara portal (NAS) does NOT use Clara gateway session cookies — it is a
...
// Keep this aligned with clara_cli.profiles.validate_profile_name(). `default`
// is the built-in root alias; these names cannot be created as profiles.
const RESERVED_REMOTE_PROFILES = new Set(['clara', 'test', 'tmp', 'root', 'sudo'])
...
function gatewayTicketFailure(error, authMessage, transportMessage) {
  ...
  // classification (isServerSideHttpError / makeClaraCloudBackendDownError) and
...
/** Which Clara profile the remote SSH dashboard should actually run as.
...
// Clara Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remoteClaraPath = String(entry.remoteClaraPath || '').trim()

  if (remoteClaraPath) {
    out.remoteClaraPath = remoteClaraPath
  }

  // A Desktop profile can be a local routing label rather than the profile
  // name used by the remote Clara installation. Preserve an explicit mapping
  // when it is a valid Clara profile identifier; otherwise fall back to the
...
function localPrimaryRequestScope(opts: ProfileRouteOptions): boolean | null {
  ...
  // spawned them: `_spawn_clara_action` registers the (often dynamic, e.g.
 ...
 *     `--profile`, so its `CLARA_HOME` scopes it.
 *
...
function resolveProfileBackendRoute(profile, opts: ProfileRouteOptions = {}): ProfileBackendRoute {
  ...
  if (scopedProfile === primaryProfile) {
    ...
    // the wire: the dashboard's process CLARA_HOME can belong to a different
 ...
 * `clara:api` handler — both routes reach a backend whose namespace is the
...
function translateSelfProfileQuery(path, profile, backendProfile) {
  ...
  try {
    parsed = new URL(rawPath, 'http://clara.local')
  } catch {
    return path
  }
...
/**
 * Resolve the two decisions made by the `clara:api` IPC handler from the same
...
/**
 * True if any cookie in `cookies` is a clara session ACCESS-token cookie
...
/**
 * True if the cookie jar holds a live Clara PORTAL (Privy) session — a non-empty
 ...
 * the Clara gateway session cookies, so cloud sign-in / discovery liveness
