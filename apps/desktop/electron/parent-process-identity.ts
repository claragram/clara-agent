export type ParentWatchdogEnv = {
  CLARA_PARENT_PID: string
  CLARA_PARENT_START_MARKER?: string
  CLARA_PARENT_NONCE?: string
  /** Spawn tag consumed by clara_cli.process_identity (`v1:<install>:<purpose>:<spawner_pid>:<spawner_create_s>`).
...
export function parentWatchdogEnv(pid: number, startMarker: string | null, nonce: string): ParentWatchdogEnv {
  ...
  const env: ParentWatchdogEnv = { CLARA_PARENT_PID: String(pid) }
  env.CLARA_SPAWN = spawnTag(pid, startMarker)
  ...
  env.CLARA_PARENT_START_MARKER = startMarker
  env.CLARA_PARENT_NONCE = nonce

  return env
}

/** Build the `CLARA_SPAWN` tag mirrored by clara_cli/process_identity.py.
