...
export const ZOOM_STORAGE_KEY = 'clara:desktop:zoomLevel'
 ...
 * but forgot to emit 'clara:zoom:changed', leaving the control stuck at 100%.
...
export function applyZoomLevel(webContents, level) {
  ...
  webContents.send('clara:zoom:changed', { level: clamped, percent: zoomLevelToPercent(clamped) })
 ...
 * only learns of changes through `clara:zoom:changed`, which never fired —
