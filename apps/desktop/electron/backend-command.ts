// Backend subcommand routing for the desktop-managed Clara process.
//
// The desktop app launches its own headless backend via `clara serve` — it
...
// app hasn't updated yet, or an older `clara` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Clara profile to pin via `--profile`.
 */
 ...
 * `-m clara_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `clara_cli/subcommands/dashboard.py` source registers
