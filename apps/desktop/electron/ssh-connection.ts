...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(CLARA_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-Clara-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.clara/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'clara-desktop-ssh')
  }

  return path.join(os.homedir(), '.clara', 'desktop-ssh')
}
