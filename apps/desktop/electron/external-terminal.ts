// Launching the Clara TUI in the user's OWN terminal emulator.
//
...
// `clara --tui --resume <id>` there. Two problems have to be solved for that
// to work anywhere:
//
//  1. WHAT to run. The desktop's Clara runtime is often a venv Python invoked
//     as `python -m clara_cli.main`, not a `clara` on PATH — so the command
 ...
 * headless child) is the wrong answer for an interactive terminal. The Clara
