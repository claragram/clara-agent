 ...
 *
 * 1. TORN BUNDLE after an update (#95575): `clara update` replaces the app
...
export interface RendererLoadErrorDetails {
  ...
  /** Repair command hint, e.g. `clara desktop --force-build`. */
  repairHint?: string
  /**
...
export function buildRendererLoadErrorPage(details: RendererLoadErrorDetails = {}): string {
  ...
  const title = 'Clara couldn\u2019t start the desktop UI'
  ...
  const repair = details.repairHint ? `<p>Repair with: <code>clara desktop --force-build</code></p>` : ''
...
<main>
  ...
  <code>clara desktop --force-build</code>, then restart the app.</p>
