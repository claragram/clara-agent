 ...
 *   1. CLARA_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\clara\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect CLARA_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.CLARA_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'clara', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'clara', 'git', 'usr', 'bin', 'bash.exe'))
  }
