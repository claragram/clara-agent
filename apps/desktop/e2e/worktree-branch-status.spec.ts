...
function createGitRepo(root: string): string {
  ...
  execFileSync('git', ['config', 'user.name', 'Clara E2E'], { cwd: repo })
...
function configureRepoCwd(claraHome: string, mockUrl: string, repo: string): void {
  writeMockProviderConfig(claraHome, mockUrl)
  fs.appendFileSync(path.join(claraHome, 'config.yaml'), `\nterminal:\n  cwd: ${repo}\n`, 'utf8')
  writeEnvFile(claraHome)
}
...
test.beforeAll(async () => {
  ...
  configureRepoCwd(sandbox.claraHome, mock.url, repo)
