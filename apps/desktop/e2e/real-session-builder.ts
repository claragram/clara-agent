 ...
 * AIAgent loop, using the E2E mock provider configured in `claraHome`.
 *
...
export class RealSessionBuilder {
  ...
  private constructor(claraHome: string) {
  ...
  static async start(claraHome: string): Promise<RealSessionBuilder> {
    const builder = new RealSessionBuilder(claraHome)
