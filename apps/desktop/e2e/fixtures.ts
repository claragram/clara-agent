/**
 * Shared E2E fixtures for the Clara desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → clara serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `clara desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `clara-e2e-${prefix}-${Math.random()}`))
  const claraHome = path.join(root, 'clara-home')
  const userDataDir = path.join(root, 'electron-user-data')

  fs.mkdirSync(claraHome, { recursive: true })
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(claraHome, 'config.yaml')
 ...
 */
export function writeEnvFile(claraHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(claraHome, '.env')
 ...
 */
function writeEmptyConfig(claraHome: string): void {
  const configPath = path.join(claraHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - CLARA_HOME → sandbox clara-home (isolated config/sessions)
 *  - CLARA_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - CLARA_DESKTOP_IGNORE_EXISTING=1 → don't pick up `clara` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - CLARA_DESKTOP_CLARA_ROOT → REPO_ROOT (dev checkout resolution)
 *  - CLARA_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    CLARA_HOME: sandbox.claraHome,
    CLARA_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    CLARA_DESKTOP_IGNORE_EXISTING: '1',
    CLARA_DESKTOP_CLARA_ROOT: REPO_ROOT,
    CLARA_DESKTOP_APP_NAME: `ClaraE2E-${Date.now()}`,
    ...
    CLARA_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated CLARA_HOME + userData
 * @param env      - the process environment (already has CLARA_HOME etc.)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.claraHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via CLARA_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`clara serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.claraHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { CLARA_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Clara backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'Clara.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'Clara.app', 'Contents', 'MacOS', 'Clara')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'clara')
}
 ...
 * progress without spawning a real Clara backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * CLARA_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    CLARA_DESKTOP_BOOT_FAKE: '1',
    CLARA_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + clara-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).CLARA_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).CLARA_DESKTOP_CLARA
  delete (env as Record<string, string | undefined>).CLARA_DESKTOP_CLARA_ROOT
