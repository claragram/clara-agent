...
/**
 * E2E smoke tests for the packaged Clara desktop app.
 *
 ...
 * (credential stripping, isolated CLARA_HOME + userData, unique app name).
 *
...
test('window opens with the Clara title', async () => {
...
test('HUD composer remains fully inside the transparent window', async () => {
  ...
  await fixture!.page.evaluate(() =>
    (window as typeof window & {
      claraDesktop?: { hud?: { open: (options: { sessionId: null }) => Promise<void> } }
    }).claraDesktop?.hud?.open({ sessionId: null })
  )
