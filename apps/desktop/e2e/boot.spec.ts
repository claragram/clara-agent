 ...
 * packaged binary) with a real `clara serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → clara serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with Clara title', async () => {
  ...
  test('the preload bridge reaches the renderer', async () => {
    const bridge = await fixture!.page.evaluate(() => {
      const desktop = (window as unknown as { claraDesktop?: Record<string, unknown> }).claraDesktop
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → clara serve is spawned → WS connects → config
