...
const UPSTREAM_URL = 'https://github.com/claraprise/clara-agent.git'
...
async function verifyInteractiveSurfaces(cdp, timed, measure, label) {
  ...
  const version = await timed(`version.ipc.${label}`, () => cdp.eval('window.claraDesktop.getVersion()'))
...
async function withTemporarySandbox(label, body) {
  const sandbox = mkdtempSync(join(tmpdir(), `clara-short-session-${label}-`))
...
async function executeRunInSandboxAttempt(target, index, warmup, mock, output, sandbox, attempt) {
  ...
  const claraHome = join(sandbox, 'clara-home')
  const userData = join(sandbox, 'electron-user-data')
  const desktopLog = join(claraHome, 'logs', 'desktop.log')
  ...
  writeSandboxConfig(claraHome, mock.url)
  ...
  const child = spawn(
    ...
    [target.targetDesktop, `--user-data-dir=${userData}`, `--remote-debugging-port=${port}`],
    {
      ...
      env: sanitizedEnv({
        CLARA_DESKTOP_APP_NAME: `ClaraShortSession-${target.label}-${process.pid}-${index}-${warmup ? 'w' : 'm'}-${attempt}`,
        CLARA_DESKTOP_CLARA_ROOT: target.targetRoot,
        CLARA_DESKTOP_IGNORE_EXISTING: '1',
        CLARA_DESKTOP_USER_DATA_DIR: userData,
...
async function main() {
  ...
  const ephemeralRoot = mkdtempSync(join(tmpdir(), 'clara-short-session-ab-'))
  const prepared = []
