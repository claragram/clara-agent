 ...
 * product name (`Clara`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Clara'
 *
 * This is a hard failure with no obvious cause for the user — `clara desktop`
 ...
 * (the mac-unpacked Clara.app bundle) and Windows (win-unpacked), so we
 ...
 * Clara.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `clara desktop --build-only` (clara_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Clara.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Clara'}.exe`
