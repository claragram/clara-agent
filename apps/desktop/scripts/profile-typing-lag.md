...
cd apps/desktop
XCURSOR_SIZE=24 CLARA_DESKTOP_DEV_SERVER=http://127.0.0.1:5174 \
...
node apps/desktop/scripts/profile-typing.mjs \
  --chars=400 --cps=30 --out=/tmp/clara-typing
# → /tmp/clara-typing.cpuprofile  (open in Chrome DevTools Performance)
# → /tmp/clara-typing.before.heapsnapshot
# → /tmp/clara-typing.after.heapsnapshot
```
...
1. **Per-keystroke `scrollHeight` read removed.** The expansion useEffect
   ...
   (forces __PROT_0_synchroclara__ layout). Replaced with a `draft.length > 60`
