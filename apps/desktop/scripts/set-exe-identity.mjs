#!/usr/bin/env node
// set-exe-identity.mjs — stamp the Clara icon + version metadata onto the
// built Clara.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Clara.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `clara desktop`, the installer's
...
// path (which rebuilds via `clara desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.mjs <path-to-Clara.exe>
//
...
// Stamp the Clara icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ...
      CompanyName: 'Workprise',
      LegalCopyright: 'Copyright (c) 2026 Workprise'
    }
  })

  console.log('[set-exe-identity] done — Clara icon + identity stamped')
}
