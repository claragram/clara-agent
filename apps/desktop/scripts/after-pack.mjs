 ...
 *
 * Stamps the Clara icon + identity onto the packed Windows Clara.exe via
 * rcedit (delegated to set-exe-identity.mjs). This runs for EVERY packed build
 * — first install, `clara desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Clara')
 */
...
export default async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Clara'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Clara.exe keeps the stock Electron icon`)
  }
}
