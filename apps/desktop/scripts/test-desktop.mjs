...
// no longer bundles the Clara Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Clara.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Clara'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Clara.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Clara'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default CLARA_HOME for non-sandboxed runs -- matches main.ts's
// resolveClaraHome(). On Windows it's %LOCALAPPDATA%\clara; elsewhere
// it's ~/.clara. The fresh-install sandbox launchFresh() sets its own
// CLARA_HOME and never touches this.
const DEFAULT_CLARA_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'clara')
  }
  return path.join(os.homedir(), '.clara')
})()
const VENV_ROOT = path.join(DEFAULT_CLARA_HOME, 'clara-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'clara-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.CLARA_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Clara-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Clara-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Clara-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Clara-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.CLARA_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.CLARA_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const claraHome = path.join(sandbox, 'clara-home')
  ...
  env.CLARA_DESKTOP_CWD = cwd
  env.CLARA_DESKTOP_IGNORE_EXISTING = '1'
  env.CLARA_DESKTOP_TEST_MODE = 'fresh-install'
  env.CLARA_DESKTOP_USER_DATA_DIR = userDataDir
  env.CLARA_HOME = claraHome
  delete env.CLARA_DESKTOP_CLARA
  delete env.CLARA_DESKTOP_CLARA_ROOT
  ...
  console.log(`  CLARA_HOME: ${claraHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(claraHome, 'clara-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Clara Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of clara_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'clara-agent', 'clara_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Clara
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + CLARA_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  CLARA_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
