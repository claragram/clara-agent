...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `clara-dev-mock-${Date.now()}`))
  ...
  return { root, claraHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(claraHome, mockUrl) {
  fs.writeFileSync(
    path.join(claraHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(claraHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function main() {
  ...
  writeMockConfig(sandbox.claraHome, mock.url)
  console.log(`  CLARA_HOME: ${sandbox.claraHome}`)
  ...
  const env = {
    ...
    CLARA_DESKTOP_APP_NAME: `ClaraDevMock-${Date.now()}`,
  }
