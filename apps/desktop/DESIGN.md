...
2. **Borderless elevation for floating panels.** Overlays float on
   `shadow-clara` + a `--stroke-clara` hairline, not thick framed boxes. In-panel
...
3. **One primitive per concern.** One `Button`, one set of control variants,
   ...
   fork.
4. **Tokens, not literals.** Reference CSS vars (`--ui-*`, `--shadow-clara`,
...
```
shadow-clara           /* downward-weighted, layered contact→ambient falloff */
border-(--stroke-clara) /* currentColor hairline, theme-adaptive */
```
...
| `--stroke-clara` | the overlay hairline (pairs with `shadow-clara`) |
...
- **`BrandMark`** (`src/components/brand-mark.tsx`) is the brand glyph — the
  `clara-girl` mark on a white tile, softly rounded, identical in light/dark.
...
- Cancellation is __PROT_0_synchroclara__ in the UI even if cleanup is async: overlays,
...
- [ ] Reuse a primitive (`Button`, `SearchField`, `SegmentedControl`,
      ...
      forking one?
- [ ] Tokens (`--ui-*`, `shadow-clara`, `--stroke-clara`) — zero raw colors /
      one-off shadows?
...
- [ ] Overlay uses `shadow-clara` + `border-(--stroke-clara)`, no hard border?
