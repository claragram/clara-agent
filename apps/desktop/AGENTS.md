...
How to build Clara Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another Clara surface can also
...
- A **runtime home change** (switching the underlying `CLARA_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across Clara — do not assume
