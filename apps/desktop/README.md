...
<tr><td><b>Voice</b></td><td>Talk to Clara and hear it back.</td></tr>
...
### Install with Clara (recommended)

Already have the Clara CLI? Just run:

```bash
clara desktop
```

It builds and launches the GUI against your existing install — same config, keys, sessions, and skills. If Desktop cannot find a usable runtime or saved remote connection, first launch lets you connect to an existing Clara gateway or install Clara locally. Local onboarding then walks you through choosing a provider and model.
...
```bash
# throwaway CLARA_HOME, separate Electron userData, distinct app name to avoid the single-instance lock
...
first launch it can install the Clara Agent runtime into `CLARA_HOME`
(`~/.clara`, or `%LOCALAPPDATA%\clara` on Windows), using the same layout as a
CLI install.
...
- **Clara Agent** runs as a headless `clara serve` process and exposes the
...
1. `CLARA_DESKTOP_CLARA_ROOT`
2. the current source checkout during development
3. a completed managed install
4. `CLARA_DESKTOP_CLARA`, or `clara` on `PATH`
5. a system Python that can import the Clara runtime
...
Desktop supports a managed local backend, explicit remote gateways, and Clara
...
screen offers **Connect to existing Clara** before starting the local installer.
...
terminal commands, and file operations run against the remote Clara host, not
...
transport- or Clara-managed header names such as `Authorization`, `Cookie`,
`Host`, `Origin`, `Referer`, and `X-Clara-Session-Token`.
...
Boot logs land in `CLARA_HOME/logs/desktop.log` (includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure.
...
```powershell
# Force a clean first-launch setup
Remove-Item "$env:LOCALAPPDATA\clara\clara-agent\.clara-bootstrap-complete"
# Rebuild a broken Python venv
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\clara\clara-agent\venv"
```

> The default Clara home on Windows is `%LOCALAPPDATA%\clara`. Set the `CLARA_HOME` env var if you've relocated it.
