...
const seedAt = (hue: number) => withHue(claraTheme.colors.primary, hue)

const CLARA_BLUE = '#0053FD'

describe('themeHue', () => {
  it('reads the accent hue that ships', () => {
    // Clara blue. Both palettes sit at this hue — light seeds `#0053fd` and dark
    // `#4a84fe`, the same blue at two lightnesses, which is what lets one pick
    // serve both appearances.
    expect(themeHue(claraTheme)).toBe(263)
    expect(Math.round(hexToOklch(claraTheme.darkColors!.primary)!.h)).toBe(263)
  })
...
describe('the shipped clara accents', () => {
  const cases = [
    { appearance: 'light', colors: claraTheme.colors, seed: '#0053fd' },
    { appearance: 'dark', colors: claraTheme.darkColors!, seed: '#4a84fe' }
  ] as const
  ...
  it('is one blue at two lightnesses, not two blues', () => {
    const light = hexToOklch(claraTheme.colors.primary)!
    const dark = hexToOklch(claraTheme.darkColors!.primary)!
  ...
  it('leaves GitHub’s neutrals in place — only the accent family is forked', () => {
    for (const key of ['background', 'foreground', 'card', 'border', 'sidebarBackground'] as const) {
      expect(claraTheme.colors[key]).toBe(githubTheme.colors[key])
      expect(claraTheme.darkColors![key]).toBe(githubTheme.darkColors![key])
    }
  })
})
...
describe('retintTheme', () => {
  ...
  it('is an identity at the theme’s own accent', () => {
    const same = retintTheme(claraTheme, claraTheme.colors.primary)

    expect(same.colors).toEqual(claraTheme.colors)
    expect(same.darkColors).toEqual(claraTheme.darkColors)
  })

  it('moves every accent-family slot, in both modes', () => {
    const rose = retintTheme(claraTheme, seedAt(350))

    for (const mode of ['colors', 'darkColors'] as const) {
      const before = claraTheme[mode]!
  ...
  it('keeps the four seed slots locked together', () => {
    const teal = retintTheme(claraTheme, seedAt(195)).colors
  ...
  it('leaves the chrome alone', () => {
    ...
    const violet = retintTheme(claraTheme, seedAt(285))

    for (const key of ['background', 'foreground', 'card', 'border', 'muted', 'mutedForeground'] as const) {
      expect(violet.colors[key], key).toBe(claraTheme.colors[key])
      expect(violet.darkColors![key], `dark ${key}`).toBe(claraTheme.darkColors![key])
    }
  })

  it('holds perceived lightness and chroma while only the hue moves', () => {
    const base = hexToOklch(claraTheme.colors.primary)!

    for (const hue of HUES) {
      const seed = hexToOklch(retintTheme(claraTheme, seedAt(hue)).colors.primary)!
  ...
  it('keeps the accent readable on the sidebar at every hue', () => {
    for (const hue of HUES) {
      const t = retintTheme(claraTheme, seedAt(hue))
  ...
  it('re-picks the foreground that sits on the accent', () => {
    for (const hue of HUES) {
      const c = retintTheme(claraTheme, seedAt(hue)).colors
  ...
  it('accepts any hex form and ignores junk', () => {
    expect(retintTheme(claraTheme, '#0053FD').colors.primary).toBe(retintTheme(claraTheme, '0053fd').colors.primary)
    // A half-typed hex from a text input must not blow up the theme.
    expect(retintTheme(claraTheme, '#00').colors).toEqual(claraTheme.colors)
    expect(retintTheme(claraTheme, 'nonsense').colors).toEqual(claraTheme.colors)
  })

  // The real motivating case: Clara blue is legible on GitHub's light sidebar
  ...
  describe('a seed that only works in one mode', () => {
    const blue = retintTheme(claraTheme, CLARA_BLUE)

    it('keeps the picked color where it already passes', () => {
      expect(blue.colors.primary.toLowerCase()).toBe(CLARA_BLUE.toLowerCase())
    })

    it('lightens it for the mode where it does not', () => {
      const dark = blue.darkColors!.primary

      expect(dark.toLowerCase()).not.toBe(CLARA_BLUE.toLowerCase())
    ...
    it('adapts by lightness, holding the hue — so it still reads as the brand', () => {
      const picked = hexToOklch(CLARA_BLUE)!
  ...
  it('does not brand a slot that never tracked the accent', () => {
    ...
    const neutralRing = {
      ...claraTheme,
      colors: { ...claraTheme.colors, ring: '#9a9a9a' },
  ...
  describe('a theme whose accent slots are shades of each other', () => {
    const shaded = {
      ...claraTheme,
      colors: { ...claraTheme.colors, primary: '#ddd6ff', ring: '#8b80e8', midground: '#8b80e8' },
      darkColors: undefined
    }
