/**
 * Live skin sync from the Clara backend.
 *
 * The backend resolves the active skin (built-in or `$CLARA_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (Clara
 ...
 */
export function ingestBackendSkin(skin: ClaraSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  // (clara), so we never register a converted theme under `default`. It is still a
  // valid apply TARGET though: a runtime switch back to `default` must repaint the
  // desktop to its own default (setTheme normalizes `default` → clara). So we only
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as ClaraSkin)
