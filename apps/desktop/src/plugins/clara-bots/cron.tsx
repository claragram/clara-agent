/**
 * Routines: the Clara cron jobs scoped to the bot you're chatting with — the
...
// clara -p <bot> chat delegation wrapper so the run reaches that bot's
...
export function routinePrompt(
  ...
  return (
    ...
    `clara -p ${shellQuote(bot)} chat -c ${shellQuote(`Routine: ${title}`)} -q ${shellQuote(`[Scheduled routine] ${instruction}`)}\n\n` +
...
// Clara-native schedule string; Advanced exposes it raw.
...
/** Compose the Clara schedule string from picker state. */
