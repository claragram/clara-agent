...
describe('routing', () => {
  ...
  it('resolves @clara to the default member', async () => {
    const { rounds } = await loadRoom()
    ...
    const parsed = rounds.parseGroupChatMentions('@clara take a look', members)
...
describe('turn prompt', () => {
  it('addresses the default profile as @clara', async () => {
    const { rounds } = await loadRoom()
    ...
    expect(own).toMatch(/You are @clara,/)
    ...
    expect(peer).toMatch(/group chat with @clara/)
  })
