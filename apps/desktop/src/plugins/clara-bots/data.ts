...
import { atom, host, queryClient, useQuery, useValue } from '@clara/plugin-sdk'
...
export const BOT_ATTENTION_HINTS: Record<string, string> = {
  ...
  missing_config: 'Provider not configured — run clara model',
 ...
 *  protocol into the system prompt itself (clara-agent bot_mode_probe).
...
export function useRoster() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Multi-source desktops (clara-agent #86875) also expose the union
...
/** __PROT_0_Synchroclara__ union-roster read for the composer surfaces (autocomplete
 ...
 *  callable alias is 'clara' — the mention middleware resolves it back to
...
export function botHandle(name: string, bot?: Partial<RosterRow> | null): string {
  ...
  return (name || '').trim().toLowerCase() === 'default' ? 'clara' : name
}

/** Taggable @-forms derived from a bot's friendly names — the core profile
 *  display name (`clara profile rename`) and the Bot Mode title. Free text
 ...
 *  (researchbuddy). Reserved tokens are dropped so a bot renamed "Clara"
 *  can never hijack the primary profile's @clara alias. */
export function mentionNameForms(value: null | string | undefined): string[] {
  ...
  return [...new Set([slug, collapsed])].filter(
    form => /^[a-z0-9][a-z0-9_-]*$/.test(form) && !['all', 'everyone', 'user', 'default', 'clara'].includes(form)
  )
}
...
export function botFriendlyNames(bot: Partial<RosterRow> | null | undefined): Array<null | string | undefined> {
  ...
  return [bot?.ui_meta?.['clara-bots']?.title, localTitle, aliasTitle, bot?.title, bot?.display_name]
}
...
export function newBotChat(bot: RosterRow) {
  if (typeof host.newChat !== 'function') {
    host.notify?.({
      kind: 'error',
      message:
        getPluginCtx()?.i18n?.t('bot.openAnotherChatUnsupported') ?? 'Update Clara Desktop to open another Bot chat.'
    })
...
export function resolveRosterMentions(
  ...
  for (const match of prose.matchAll(/(^|\s)@([a-z0-9][a-z0-9_-]*)/gi)) {
    ...
    if (token === 'clara') {
      token = byForm.has('clara') ? 'clara' : token
    }
