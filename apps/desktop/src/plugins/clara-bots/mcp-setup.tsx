...
import { Button, host, Input, useI18n } from '@clara/plugin-sdk'
...
// and the row falls back to the "run clara mcp / Settings" hint. profile is
...
export function McpSetupButton({ profile, entry, onDone, ensureProfile }: McpSetupButtonProps) {
  ...
  const beginOAuth = async () => {
    ...
    const mcpOauthBridge =
      typeof window !== 'undefined' && window.claraDesktop && window.claraDesktop.mcpOauth
        ? window.claraDesktop.mcpOauth
        : null
    ...
    try {
      ...
      } else if (typeof window !== 'undefined' && window.claraDesktop && window.claraDesktop.openExternal) {
        window.claraDesktop.openExternal(authUrl)
      } else {
