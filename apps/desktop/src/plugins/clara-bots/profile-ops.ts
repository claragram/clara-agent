...
import { forgetSessionUnread, host, queryClient } from '@clara/plugin-sdk'
...
export function mergeServerMeta(roster: RosterRow[], fetchedAt = 0) {
  ...
  for (const bot of roster) {
    const server = bot.ui_meta?.['clara-bots']
...
/** Permanently delete a bot's Clara profile, then remove plugin-local state
 ...
 * directory (clara-agent#52279). That is the "can't delete a bot" error. */
