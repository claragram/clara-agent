 ...
 * `{type:'clara-skill-pick'}` back and the plugin installs via skills.manage.
 *
...
import type * as ClaraSdk from '@clara/plugin-sdk'
import { cleanup, fireEvent, render, screen } from '@testing-library/react'
import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest'

const HUB_ORIGIN = 'https://agent.claraship.com'
...
vi.mock('@clara/plugin-sdk', async importOriginal => {
  const original = await importOriginal<typeof ClaraSdk>()
...
describe('hub pick messages', () => {
  it('installs the picked skill when it comes from our own frame', () => {
    ...
    postPick(
      { identifier: 'clara/web-research', name: 'Web Research', type: 'clara-skill-pick' },
      {
        source: frame.contentWindow
      }
    )

    expect(installCalls()).toEqual([['skills.manage', { action: 'install', query: 'clara/web-research' }]])
  })
  ...
  it('regression: refuses identifiers outside the slug charset', () => {
    ...
    for (const identifier of ['../../etc/passwd', 'skill; rm -rf /', '-flag', 'name with spaces', '']) {
      postPick({ identifier, name: 'Web Research', type: 'clara-skill-pick' }, { source: frame.contentWindow })
    }
  ...
  it('ignores messages that are not a skill pick', () => {
    ...
    postPick({ identifier: 'clara/web-research', type: 'oauth-callback' }, { source: frame.contentWindow })
    postPick({ identifier: 'clara/web-research', type: 'clara-skill-pick' }, { source: frame.contentWindow })
