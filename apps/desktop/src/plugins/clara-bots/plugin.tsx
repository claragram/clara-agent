/**
 * Clara Bot Mode — a "one chat per agent" roster for the Clara desktop.
 *
 * Left pane "Bots": one row per Clara profile (a bot = an agent profile) with
 ...
 *
 * Right tile "Routines": scheduled tasks (Clara cron jobs) scoped to the
 ...
 * bot-initiated sends use `clara -p <bot> chat --in ~ -c "Bot Chat"`.
 */

import { CHAT_EMPTY_AREA, COMPOSER_AREAS, host, PALETTE_AREA, translateNow } from '@clara/plugin-sdk'
import type { ChatEmptyProps, PluginContext } from '@clara/plugin-sdk'
...
export default {
  ...
  register(ctx: PluginContext) {
    ...
    // __PROT_0_synchroclaraly__ per keystroke. Multi-source rosters contribute their
    // precomputed @name-device handles via botHandle. The active profile is
    // excluded (a bot doesn't @ itself); 'default' surfaces as @clara.
    ctx.register({
            ...
            try {
              const registry =
                typeof window !== 'undefined'
                  ? await Promise.resolve(window.claraDesktop?.connections?.list?.()).catch(() => null)
                  : null
    ...
    try {
      if (typeof window !== 'undefined') {
        unbindConnectionsChanged =
          window.claraDesktop?.connections?.onChanged?.(payload => {
    ...
    // A bot's chat before it has spoken: core's splash is Clara' wordmark and
      ...
      data: {
        handler: async (draft: ComposerDraftPayload): Promise<ComposerDraftPayload> => {
          ...
          const lines = mentionedBots.map(bot => {
            ...
            const title = String(
              botRosterMeta(bot, $botMeta.get())?.title || bot.ui_meta?.['clara-bots']?.title || bot.title || ''
            ).trim()
