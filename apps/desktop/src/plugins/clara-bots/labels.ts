...
export function displayName(bot: Partial<RosterRow>, meta?: BotMeta | null): string {
  ...
  // "Clara". Annotated active rows carry sourceScoped too, and keying this
  ...
  // Core-profile display name (profile.yaml, set via `clara profile rename
  ...
  // never generic "Clara" or a hostname-derived label.
  if (alias) {
  ...
  // that reads like nobody bothered. Present it as Clara (the agent it is)
