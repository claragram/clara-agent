...
import { atom, host } from '@clara/plugin-sdk'
...
const GROUP_CHAT_SYNC_META_KEY = 'clara-bots-groups'
...
/** The v3 envelope stored under the default profile's `clara-bots-groups`
 ...
 *  clients (Clara Go, headless backends) see rooms regardless of which
...
export function scheduleGroupChatServerSync(
  ...
  // Queue on the ACTIVE gateway __PROT_0_synchroclaraly__ (tests and older hosts have no
 ...
 *  renamed bot never shows up as its raw profile id or a stale "Clara"
 *  (community report, Aug 21 2026: renamed default still read "Clara is
 *  thinking…" in group rooms). The untitled primary profile is literally
 *  named "default" — render it as Clara (matching displayName and the
 *  @clara handle) so the main agent never loses its name in rooms. */
export function groupSpeakerLabel(name?: null | string) {
  ...
  // Core profile display_name (`clara profile rename …` / dashboard) from
  ...
  return trimmed.toLowerCase() === 'default' ? 'Clara' : trimmed
}
