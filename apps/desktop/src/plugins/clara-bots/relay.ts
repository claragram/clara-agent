...
import { host, LruCache } from '@clara/plugin-sdk'
...
// to (local, remote URL, SSH, Clara Cloud, docker) must be able to find
...
// clara_cli/config_defaults.py and tui_gateway/methods_bot_relay.py and fails
  ...
  try {
    ...
    return profiles
      .map(profile => ({
        ...
        title: String(profile?.ui_meta?.['clara-bots']?.title || profile?.display_name || ''),
