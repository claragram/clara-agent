...
import type { ClaraApiRequest } from '@/global'
...
// "Timed out connecting to Clara backend" that hangs the UI (#48504).
//
...
export class ClaraGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'Clara gateway connection closed',
      connectErrorMessage: 'Could not connect to Clara gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'Clara gateway is not connected',
...
// process to route them to that host (clara:api's registry branch). Null
...
export function claraApi<T>(request: ClaraApiRequest): Promise<T> {
  return window.claraDesktop.api<T>({ ...connectionScoped(), ...request })
}
