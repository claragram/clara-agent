...
import { capabilityScoped, getApiRequestConnection, claraApi, type ProfileScope, profileScoped } from './client'
...
export async function listSessions(
  ...
  const result = await claraApi<PaginatedSessions>({
    ...profileScoped(),
    path:
...
export async function listSidebarSessions(req: SidebarSessionsRequest): Promise<SidebarSessionsResponse> {
  ...
  try {
    result = await claraApi<SidebarSessionsResponse>({
      ...profileScoped(),
...
export function searchSessions(query: string): Promise<SessionSearchResponse> {
  return claraApi<SessionSearchResponse>({
...
export function getSessionMessages(
  ...
  return claraApi<SessionMessagesResponse>({
    ...sessionScope,
...
export function renameSession(
  ...
  return claraApi<{ ok: boolean; title: string }>({
    ...(profile ? { profile } : {}),
    path: `/api/sessions/${encodeURIComponent(id)}`,
    method: 'PATCH',
