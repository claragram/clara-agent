...
import { claraApi, connectionScoped, profileScoped, STARTUP_REQUEST_TIMEOUT_MS } from './client'
...
// __PROT_0_synchroclara__ long-operation endpoint rather than weakening all API timeouts.
const CRON_TRIGGER_REQUEST_TIMEOUT_MS = 24 * 60 * 60 * 1000

// Cron jobs are stored per-profile (<CLARA_HOME>/cron/jobs.json), and the
...
export async function getCronJobRuns(jobId: string, limit = 20): Promise<SessionInfo[]> {
  const { runs } = await claraApi<{ runs: SessionInfo[] }>({
    ...profileScoped(),
    ...connectionScoped(),
...
export async function getCronDeliveryTargets(): Promise<CronDeliveryTarget[]> {
  const { targets } = await claraApi<{ targets: CronDeliveryTarget[] }>({
    ...profileScoped(),
    ...connectionScoped(),
...
export function getAutomationBlueprints(): Promise<{ blueprints: AutomationBlueprint[] }> {
  return claraApi<{ blueprints: AutomationBlueprint[] }>({
    ...profileScoped(),
    ...connectionScoped(),
