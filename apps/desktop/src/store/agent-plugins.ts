...
/**
 * Feature store for backend (agent) plugins — the native Clara plugins plus
 ...
 * primitives `clara plugins` and the dashboard use, so all surfaces agree on
...
export interface AgentPluginRow {
  ...
  /** Agent Plugins v1 package (portable skills/MCP format) vs native Clara. */
  portable?: boolean
}
 ...
 *  CLARA_HOME. Always refetches (it's a cheap local disk scan on the
