 ...
 *  Working in `~/www/clara-agent`, the draft says "clara-agent" constantly
 *  — a "Use skill: clara-agent" pill on every mention is noise (the reported
 *  annoyance that prompted this guard). Boundary containment, not exact
 *  segment equality, so worktree dirs (`clara-agent-suggest`) suppress too. */
 ...
 *  segment of a qualified form (`github/clara-agent-dev`, `plugin:skill`). */
