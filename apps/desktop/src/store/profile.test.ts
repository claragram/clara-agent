...
const { getProfiles } = await import('@/clara')
...
const remoteConn = (over: Partial<ClaraConnection> = {}): ClaraConnection =>
  ({ baseUrl: 'https://clara-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as ClaraConnection
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { claraDesktop: { getConnection } })
