 ...
 *
 * The archive is the CLI's own `clara profile export` tar.gz (config, skills,
...
import { exportProfileArchive, importProfileArchive } from '@/clara'
...
import type { ProfileDesktopOverlay } from '@/types/clara'
...
const ARCHIVE_FILTERS = [{ extensions: ['tar.gz', 'tgz'], name: 'Clara profile' }]
...
export async function runExportProfileFlow(profile?: string): Promise<null | string> {
  ...
  const pick = window.claraDesktop?.selectSavePath
...
export async function runImportProfileFlow(): Promise<null | string> {
  const paths = await window.claraDesktop?.selectPaths?.({
