...
import { getClaraConfigRecord, saveClaraConfig } from '@/clara'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getClaraConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveClaraConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
