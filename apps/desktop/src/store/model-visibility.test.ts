...
describe('model visibility', () => {
  ...
  it('preserves hidden-provider sentinel without re-adding defaults', () => {
    // User explicitly hid all models for "clara" — sentinel marks this choice.
    const stored = new Set([emptyProviderSentinelKey('clara')])

    const visible = effectiveVisibleKeys(stored, [
      provider('clara', ['clara-3-llama-3.1-70b', 'clara-3-llama-3.1-8b']),
      provider('ollama', ['qwen3:latest'])
    ])

    expect(visible.has(modelVisibilityKey('clara', 'clara-3-llama-3.1-70b'))).toBe(false)
    expect(visible.has(modelVisibilityKey('clara', 'clara-3-llama-3.1-8b'))).toBe(false)
    // Sentinel itself is stripped from the result.
    expect(visible.has(emptyProviderSentinelKey('clara'))).toBe(false)
  ...
  it('restores model when toggling on after hiding all', () => {
    // Simulates: user hid all "clara" models, then toggles one back on.
    const stored = new Set([emptyProviderSentinelKey('clara'), modelVisibilityKey('ollama', 'qwen3:latest')])

    // After toggle: sentinel removed, one model added.
    const afterToggle = new Set(stored)
    afterToggle.delete(emptyProviderSentinelKey('clara'))
    afterToggle.add(modelVisibilityKey('clara', 'clara-3-llama-3.1-70b'))
    ...
    expect(visible.has(modelVisibilityKey('clara', 'clara-3-llama-3.1-70b'))).toBe(true)
    expect(visible.has(modelVisibilityKey('clara', 'clara-3-llama-3.1-8b'))).toBe(false)
  })
  ...
  it('resolveVisibleKeys preserves sentinels that effectiveVisibleKeys strips', () => {
    const stored = new Set([emptyProviderSentinelKey('clara')])
    const providers = [provider('clara', ['clara-x', 'clara-y']), provider('ollama', ['qwen3:latest'])]

    const resolved = resolveVisibleKeys(stored, providers)
    expect(resolved.has(emptyProviderSentinelKey('clara'))).toBe(true)
    expect(resolved.has(modelVisibilityKey('clara', 'clara-x'))).toBe(false)
    ...
    expect(effectiveVisibleKeys(stored, providers).has(emptyProviderSentinelKey('clara'))).toBe(false)
  })
})

describe('toggleModelVisibility', () => {
  const providers = [provider('openai', ['gpt-a', 'gpt-b']), provider('clara', ['clara-x', 'clara-y'])]
  ...
  it('keeps a hidden provider hidden when a different provider is toggled (regression for #43485)', () => {
    // Hide ALL of clara — its sentinel is now stored.
    let stored: Set<string> | null = null
    stored = apply(stored, 'clara', 'clara-x')
    stored = apply(stored, 'clara', 'clara-y')
    expect(stored.has(emptyProviderSentinelKey('clara'))).toBe(true)

    // Toggle a model in another provider. clara must NOT snap back on.
    stored = apply(stored, 'openai', 'gpt-a')

    expect(stored.has(emptyProviderSentinelKey('clara'))).toBe(true)
    const visible = effectiveVisibleKeys(stored, providers)
    expect(visible.has(modelVisibilityKey('clara', 'clara-x'))).toBe(false)
    expect(visible.has(modelVisibilityKey('clara', 'clara-y'))).toBe(false)
  })

  it('clears only the toggled provider sentinel when a model is re-enabled', () => {
    let stored: Set<string> | null = new Set([emptyProviderSentinelKey('openai'), emptyProviderSentinelKey('clara')])
  ...
  it('re-enabling one model of a hidden-all provider restores ONLY that model, not the curated defaults', () => {
    // openai hidden-all, clara untouched.
  ...
  it('toggling from an empty (non-null) stored set adds the model without expanding defaults', () => {
    ...
    expect(next.has(modelVisibilityKey('clara', 'clara-x'))).toBe(false)
  })
  ...
  it('toggling off one default model from null stored keeps the rest of the curated defaults', () => {
    ...
    expect(next.has(modelVisibilityKey('clara', 'clara-x'))).toBe(true)
...
describe('featured defaults', () => {
  ...
  it('defaults to the featured shortlist when a provider publishes one', () => {
    const clara = featuredProvider(
      'clara',
      ['anthropic/opus', 'anthropic/haiku', 'google/gemini', 'x-ai/grok'],
      ['anthropic/opus', 'google/gemini', 'x-ai/grok']
    )

    const visible = defaultVisibleKeys([clara])

    // Featured are visible; the non-featured model is hidden by default.
    expect(visible.has(modelVisibilityKey('clara', 'anthropic/opus'))).toBe(true)
    expect(visible.has(modelVisibilityKey('clara', 'google/gemini'))).toBe(true)
    expect(visible.has(modelVisibilityKey('clara', 'x-ai/grok'))).toBe(true)
    expect(visible.has(modelVisibilityKey('clara', 'anthropic/haiku'))).toBe(false)
  })
...
describe('setProviderVisibility', () => {
  ...
  it('leaves other providers untouched (their sentinels survive)', () => {
    ...
    // Turn openai fully on; clara must stay hidden.
    const next = setProviderVisibility(stored, providers, 'openai', true)

    expect(next.has(emptyProviderSentinelKey('clara'))).toBe(true)
    const visible = effectiveVisibleKeys(next, providers)
    expect(visible.has(modelVisibilityKey('clara', 'clara-x'))).toBe(false)
    expect(visible.has(modelVisibilityKey('openai', 'gpt-a'))).toBe(true)
  })
  ...
  it('collapses model families to one key per family when enabling', () => {
    ...
    const ps = [provider('clara', ['model', 'model-fast'])]

    const next = setProviderVisibility(null, ps, 'clara', true)

    expect(next.has(modelVisibilityKey('clara', 'model'))).toBe(true)
    // The -fast sibling is represented by its base family, not its own key.
    expect(next.has(modelVisibilityKey('clara', 'model-fast'))).toBe(false)
  })
})
