 ...
 * (`clara:data-url-read-max:*`). The default and the clamp are shared with
...
export async function refreshDataUrlReadMaxMb(): Promise<number> {
  const api = window.claraDesktop?.dataUrlReadMax
...
if (typeof window !== 'undefined' && window.claraDesktop?.dataUrlReadMax) {
