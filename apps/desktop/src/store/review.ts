...
import type { ClaraReviewFile, ClaraReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Clara' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'clara.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'clara.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'clara.desktop.reviewTreeMode'
const SELECTED_KEY = 'clara.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<ClaraReviewFile[]>([])
...
export const $reviewShipInfo = atom<ClaraReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['claraDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: ClaraReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly ClaraReviewFile[], path: string): ClaraReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.claraDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.claraDesktop?.openExternal?.(url)
    }
