...
export function setZoomPercent(percent: number): void {
  window.claraDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.claraDesktop?.zoom) {
  void window.claraDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.claraDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
