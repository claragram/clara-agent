...
import type { ProjectInfo, ProjectsPayload } from '@/types/clara'
...
const PROJECT_SCOPE_KEY = 'clara.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Clara gateway is not connected')
  }
...
async function activeProjectsContext(): Promise<ActiveProjectsContext> {
  ...
  if (!gateway || gateway !== activeGateway() || profile !== projectProfile()) {
    throw new Error('Active Clara profile changed while connecting')
  }
...
async function refreshProjectTreeAcrossProfiles(): Promise<void> {
  ...
  try {
    const res = await claraApi<ProjectTreePayload>({
...
const repoScanStates = new WeakMap<ClaraGateway, RepoScanState>()
const scanningGatewayGenerations = new WeakMap<ClaraGateway, number>()
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  if (isDesktopFsRemoteMode()) {
    ...
    // with `scan: true` — added in #81723) so repos with zero Clara
  ...
  try {
    const policy = repoDiscoveryPolicyFromConfig(await getClaraConfig(context.profile))
...
// repo, returning where Clara should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<ClaraGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<ClaraGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.claraDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.claraDesktop?.writeClipboard?.(path)
  }
}
