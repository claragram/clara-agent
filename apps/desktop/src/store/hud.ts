 ...
 * composer with the reply scrolling above it, so Clara can be driven while
...
export const canUseHud = (): boolean =>
  typeof window !== 'undefined' && typeof window.claraDesktop?.hud?.open === 'function'

export function openHud(sessionId?: null | string): void {
  const api = window.claraDesktop?.hud
...
export function resetHudLayout(): void {
  void window.claraDesktop?.hud?.resetLayout?.()
}
...
export const reportHudSession = (sessionId: null | string): void => window.claraDesktop?.hud?.setSession?.(sessionId)
...
export function watchHudState(onClosed?: (sessionId: null | string) => void): () => void {
  const off = window.claraDesktop?.hud?.onChanged?.(({ open, sessionId }) => {
    $hudActive.set(open)
