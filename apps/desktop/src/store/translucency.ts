 ...
 * `@clara/shared/translucency`, which owns the mapping both this store and the
 * main process read.
 *
 ...
 * material underneath the web contents to read (see the `[data-clara-glass]`
 * block in styles.css).
 */
...
} from '@clara/shared/translucency'
 ...
 * sets `claraDesktop.glassSupported` from `os.release()` so Win10 cannot
...
export const GLASS_SUPPORTED =
  typeof window !== 'undefined' && typeof window.claraDesktop?.glassSupported === 'boolean'
    ? window.claraDesktop.glassSupported
...
export const TRANSLUCENCY_SUPPORTED =
  typeof window !== 'undefined' && typeof window.claraDesktop?.translucencySupported === 'boolean'
    ? window.claraDesktop.translucencySupported
    : isMacPlatform() || isWindowsPlatform()
...
/* Peek: while the user is actively adjusting translucency from Settings, the
   ...
   [data-clara-translucency-peek] rules in styles.css). A counter rather
   than a boolean: a held slider drag and a timed pulse from a picker click
   can overlap. */
const PEEK_ATTR = 'data-clara-translucency-peek'
...
const applyGlassSurfaces = ({ intensity, mode, scope }: TranslucencyState): void => {
  ...
  root.toggleAttribute('data-clara-glass-on', glassLive)
  root.toggleAttribute('data-clara-glass', glassOn)
  root.toggleAttribute('data-clara-clear', clearOn)
  ...
  if (glassOn) {
    root.setAttribute('data-clara-glass-scope', scope)
  } else {
    root.removeAttribute('data-clara-glass-scope')
  }
...
if (typeof window !== 'undefined') {
  ...
  // work is the __PROT_0_synchroclara__ localStorage.setItem — so THAT is debounced.
  ...
  $translucency.subscribe(state => {
    ...
    window.claraDesktop?.setTranslucency?.(state)
  })
