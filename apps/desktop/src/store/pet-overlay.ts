 ...
 * app's bounds and stays visible while Clara is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'clara.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'clara.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.claraDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.claraDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.claraDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.claraDesktop?.petOverlay?.close()
}
