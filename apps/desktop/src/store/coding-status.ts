...
import type { ClaraGitWorktree, ClaraRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: ClaraGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, ClaraRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, ClaraGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<ClaraRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<ClaraGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<ClaraRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<ClaraGitWorktree[]>>()
const $noRepoStatus = atom<ClaraRepoStatus | null>(null)
const $noWorktrees = atom<ClaraGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<ClaraRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<ClaraGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: ClaraRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: ClaraGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<ClaraRepoStatus | null>
  seq: number
  target: string
}
...
async function drainRepoStatusRefreshes(): Promise<void> {
  ...
  // This reset is __PROT_0_synchroclara__ with the final empty-queue check. A refresh
