...
export const $pullRequestsByBranch = atom<Record<string, ClaraBranchPullRequest>>({})
...
export const $prBranchBySession = persistentAtom<Record<string, string>>(
  'clara.desktop.prBranchBySession',
  {},
  Codecs.stringRecord
)
...
const $prScannedSessions = persistentAtom<string[]>('clara.desktop.prScannedSessions', [], Codecs.stringArray)
...
export function pullRequestBucket(pr: ClaraBranchPullRequest | undefined): PullRequestBucket {
  if (!pr) {
    return 'none'
  }
