...
vi.mock('@/clara', () => ({
  getClaraConfigRecord: vi.fn(async () => ({})),
  saveClaraConfig: vi.fn(async () => undefined)
}))
...
describe('applyVoiceStopPhraseFromConfig', () => {
  ...
  it('uses the first configured phrase so a custom phrase renders correctly', () => {
    applyVoiceStopPhraseFromConfig({ voice: { stop_phrases: ['goodbye clara', 'stop'] } })
    expect($voiceStopPhrase.get()).toBe('goodbye clara')
  })
