...
const clara = await import('@/clara')
const getClaraConfig = vi.mocked(clara.getClaraConfig)
...
describe('project scope', () => {
  ...
  it('enterProject scopes the sidebar to the project id', () => {
    ...
    // swallowed); the __PROT_0_synchroclara__ scope change is what matters here.
    enterProject('p_123')
  ...
  it('persists the scope to localStorage', () => {
    enterProject('p_abc')
    expect(window.localStorage.getItem('clara.desktop.projectScope')).toBe('p_abc')
  })
})
  ...
  it('names the stale-backend remedy when a remote gateway lacks the worktree route', async () => {
    ...
    desktopGit.mockReturnValue({
      worktreeAdd: vi.fn(async () => {
        throw new Error(
          'Expected JSON from https://vps/api/git/worktree/add but got HTML (status 404). The endpoint is likely missing on the Clara backend.'
        )
      })
    } as never)
...
describe('repository discovery policy', () => {
  ...
  it('records disabled policy without invoking the filesystem scanner', async () => {
    ...
    getClaraConfig.mockResolvedValue({
      desktop: {
  ...
  it('passes custom roots and exclusions to Electron and records on the origin gateway', async () => {
    ...
    expect(getClaraConfig).toHaveBeenCalledWith('default')
  ...
  it('does not scan the local filesystem for remote connections but still refreshes the project tree', async () => {
    ...
    expect(scanRepos).not.toHaveBeenCalled()
    expect(getClaraConfig).not.toHaveBeenCalled()
    ...
    // `scan: true`) — repos with zero Clara sessions must still surface —
