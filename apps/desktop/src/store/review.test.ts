...
// reaches the gateway); everything else routes through window.claraDesktop.git,
...
function file(path: string, over: Partial<ClaraReviewFile> = {}): ClaraReviewFile {
  return { path, status: 'modified', staged: false, added: 1, removed: 0, ...over } as ClaraReviewFile
}

type ReviewStub = Record<string, ReturnType<typeof vi.fn>>

// Install a review bridge on window.claraDesktop. Any op not supplied defaults
...
describe('ship flow', () => {
  ...
  it('createOrOpenPr opens the existing PR without creating a new one', async () => {
    const review = stubReview()
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'https://example.com/pr/9' } } as ClaraReviewShipInfo)

    await createOrOpenPr()

    expect(review.createPr).not.toHaveBeenCalled()
    expect(
      (window.claraDesktop as unknown as { openExternal: ReturnType<typeof vi.fn> }).openExternal
...
describe('refreshShipInfo', () => {
  it('populates ship info from the bridge', async () => {
    const info: ClaraReviewShipInfo = {
      ghReady: true,
      pr: { url: 'https://example.com/pr/3' }
    } as ClaraReviewShipInfo
  ...
  it('resets ship info when there is no bridge', async () => {
    delete (window as unknown as { claraDesktop?: unknown }).claraDesktop
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'x' } } as ClaraReviewShipInfo)
