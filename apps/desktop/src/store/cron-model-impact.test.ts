...
import type { ModelAssignmentResponse } from '@/types/clara'
...
describe('setMainModelAssignment', () => {
  it('shows one consumer warning and routes via a read-only review action', async () => {
    ...
    await setMainModelAssignment({ provider: 'clara', model: 'new/model' })
  ...
  it('keeps an existing warning for an older backend but clears it on explicit zero impact', async () => {
    ...
    await setMainModelAssignment({ provider: 'clara', model: 'one' })
    expect($notifications.get()).toHaveLength(1)

    setModelAssignment.mockResolvedValueOnce(response(undefined))
    await setMainModelAssignment({ provider: 'clara', model: 'two' })
    expect($notifications.get()).toHaveLength(1)
    ...
    await setMainModelAssignment({ provider: 'clara', model: 'three' })
    expect($notifications.get()).toEqual([])
  })
  ...
  it('publishes only the latest same-profile assignment when responses reverse', async () => {
    ...
    const firstCall = setMainModelAssignment({ provider: 'clara', model: 'first' })
    const secondCall = setMainModelAssignment({ provider: 'clara', model: 'second' })
  ...
  it('invalidates pending responses and action closures on profile or connection changes', async () => {
    ...
    const call = setMainModelAssignment({ provider: 'clara', model: 'pending' })
    ...
    await setMainModelAssignment({ provider: 'clara', model: 'current' })
