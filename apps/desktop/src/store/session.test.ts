...
describe('composer model persistence scope', () => {
  ...
  it('keeps inferred local-primary connections on the historical bare keys', () => {
    ...
    window.localStorage.setItem('clara.desktop.composer.model', 'legacy-model')
    window.localStorage.setItem('clara.desktop.composer.provider', 'legacy-provider')
    ...
    expect(window.localStorage.getItem('clara.desktop.composer.model')).toBe('next-model')
    expect(window.localStorage.getItem('clara.desktop.composer.model.registry.local.default')).toBeNull()
  })
...
describe('session owner hints', () => {
  ...
  it('ignores malformed persisted entries and never throws on hydrate', () => {
    window.localStorage.setItem(
      'clara.desktop.sessionOwnerHints.v1',
      JSON.stringify([
        'junk',
        ['no-route', null],
...
describe('workspaceCwdForNewSession', () => {
  afterEach(() => {
    ...
    window.localStorage.removeItem('clara.desktop.workspace-cwd')
    window.localStorage.removeItem('clara.desktop.workspace-cwd.remote.http%3A%2F%2Fbackend-a.default')
    window.localStorage.removeItem('clara.desktop.workspace-cwd.remote.http%3A%2F%2Fbackend-b.default')
  ...
  it('does not publish a delayed configured default after ownership is lost', async () => {
    ...
    ;(window as { claraDesktop?: unknown }).claraDesktop = {
  ...
  it('prefers the configured default over the sticky remembered workspace', () => {
    window.localStorage.setItem('clara.desktop.workspace-cwd', '/home/user/sticky')
  ...
  it('keeps remote workspace memory separate from local and other remotes', () => {
    window.localStorage.setItem('clara.desktop.workspace-cwd', '/local/project')
    $currentCwd.set('/live/session/path')
...
describe('unread finished sessions', () => {
  ...
  it('clears a persisted unread row when the session is opened', async () => {
    ...
    // The optimistic flip is __PROT_0_synchroclara__; the PATCH is fire-and-forget.
...
describe('remembered session id (per profile)', () => {
  ...
  it('discards legacy unsuffixed keys on first read (zero-migration, refuse-to-guess)', () => {
    ...
    localStorage.setItem('clara.desktop.lastSessionId', 'legacy-session')
    ...
    expect(localStorage.getItem('clara.desktop.lastSessionId')).toBeNull()
  })
  ...
  it('uses encodeURIComponent so profile names with reserved chars are isolated', () => {
    ...
    expect(localStorage.getItem('clara.desktop.lastSessionId.profile.research%2Fops')).toBe('ops-session')
...
describe('remembered route (per profile)', () => {
  ...
  it('discards legacy unsuffixed keys on first read (zero-migration, refuse-to-guess)', () => {
    localStorage.setItem('clara.desktop.lastRoute', '/skills')
    ...
    expect(localStorage.getItem('clara.desktop.lastRoute')).toBeNull()
  })
  ...
  it('uses encodeURIComponent so profile names with reserved chars are isolated', () => {
    ...
    expect(localStorage.getItem('clara.desktop.lastRoute.profile.research%2Fops')).toBe('/cron')
    expect(getRememberedRoute('research')).toBeNull()
  })
