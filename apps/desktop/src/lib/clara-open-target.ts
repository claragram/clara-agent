 ...
 * `clara://` deep links and `host.navigate('/path?…')`.
 *
 ...
 *  - `clara://index-network/intent/1` → `/index-network/intent/1` (plugin-scoped)
 *  - `clara://open/my-page?item=x` → `/my-page?item=x` (generic open)
 *  - `/my-page?item=x` / `#/my-page?item=x` (hash-router paths)
 */

export type ClaraOpenTarget = string | { href: string } | { path: string; params?: Record<string, string> }

const CLARA_PROTOCOL = 'clara:'
...
export function normalizeClaraOpenString(raw: string): string | null {
  ...
  if (trimmed.startsWith('clara://') || trimmed.startsWith(`${CLARA_PROTOCOL}//`)) {
    try {
      const url = new URL(trimmed)
      const host = url.hostname || ''
      const rest = decodeURIComponent((url.pathname || '').replace(/^\//, ''))

      // clara://open/<path>?… → /<path>?…
      if (host === 'open') {
        if (!rest) {
          return null
        }
      ...
      // clara://index-network/intent/1 → /index-network/intent/1
...
export function resolveClaraOpenPath(target: ClaraOpenTarget | null | undefined): string | null {
  if (target == null) {
    return null
  }

  if (typeof target === 'string') {
    return normalizeClaraOpenString(target)
  }
  ...
  if ('href' in target && typeof target.href === 'string') {
    return normalizeClaraOpenString(target.href)
  }

  if ('path' in target && typeof target.path === 'string') {
    const base = normalizeClaraOpenString(target.path)
 ...
 * (`clara://<kind>/<name>?…` → kind/name/params).
 */
export function pathFromClaraDeepLink(kind: string, name: string, params: Record<string, string> = {}): string | null {
  if (!kind || !name) {
    return null
  }

  if (kind === 'open') {
    return resolveClaraOpenPath({ path: `/${name.replace(/^\//, '')}`, params })
  }
  ...
  return resolveClaraOpenPath({ path: `/${kind}/${name.replace(/^\//, '')}`, params })
}

/** Convenience for `clara://open/<name>?…` payloads. */
export function pathFromOpenDeepLink(name: string, params: Record<string, string> = {}): string | null {
  return pathFromClaraDeepLink('open', name, params)
}
