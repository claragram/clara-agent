...
export function watchInPage(doc: Document, holder: WatchHolder, stage: WatchStage, label?: string): void {
  const win = doc.defaultView as (Window & { __claraWatch?: Record<string, unknown> }) | null
  ...
  const busy = 25
  const state = win.__claraWatch || (win.__claraWatch = {})
  ...
  const build = () => {
    ...
    const stale = doc.querySelectorAll('clara-watch')

    for (let i = 0; i < stale.length; i++) {
      stale[i].remove()
    }

    const host = doc.createElement('clara-watch')
  ...
  // this up" in dev.
  const stamp = (win as unknown as { __claraWatchTag?: number }).__claraWatchTag
