...
afterEach(() => {
  ...
  document.querySelectorAll('clara-annotate').forEach(node => node.remove())
})
...
describe('annotateInPage overlay', () => {
  it('installs a host and tears it down completely', () => {
    ...
    expect(document.querySelector('clara-annotate')).toBeTruthy()

    api.teardown()

    expect(api.isInstalled()).toBe(false)
    expect(document.querySelector('clara-annotate')).toBeNull()
  })
  ...
  it('keeps a pin glued to its element after the element moves', () => {
    ...
    const pin = () =>
      document.querySelector('clara-annotate')!.shadowRoot!.querySelector('[data-annotate-pin="1"]') as HTMLElement
  ...
  it('keeps the picked element when the draft becomes a pin, even if the selector is wrong', () => {
    ...
    const api = annotateInPage(document)
    api.install()
    const host = document.querySelector('clara-annotate') as HTMLElement
