...
//
// When someone is in a hands-free "Hey Clara" voice chat, the natural way to
...
// phrase (optionally addressed to Clara), so a real turn that merely contains
...
// Optional address prefixes so "clara stop" / "ok stop" / "hey clara, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey clara', 'hey clara,', 'clara', 'clara,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("clara") is not a stop command on its own.
      continue
    }
 ...
 * to Clara). Returns false for anything that merely contains "stop" as part of
