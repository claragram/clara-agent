...
// Clara is rendering, and when the two are opposite schemes it takes over
...
function renderedMode(doc: Document): RenderedMode {
  const attr = doc.documentElement.dataset.claraMode
 ...
 * the only carriers of Clara' palette — are dropped so the paste target's
