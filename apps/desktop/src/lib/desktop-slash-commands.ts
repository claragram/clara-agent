...
/** Why a known Clara command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Clara profile', surface: action('profile') },
  {
    name: '/skin',
...
function isKnownClaraSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Clara' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownClaraSlashCommand(normalized)
}
 ...
 * activity — the ones that ship with Clara and were never asked for. It is
