...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { claraDesktop: { readDir: typeof readDir } }).claraDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): ClaraReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { claraDesktop: unknown }).claraDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: ClaraReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<ClaraReadDirResult>(resolve => {
  ...
  it('discards a stale live refresh after the active registered connection changes', async () => {
    let resolveRefreshFromA: ((result: ClaraReadDirResult) => void) | undefined
  ...
  it('discards a stale child read after the active registered connection changes', async () => {
    let resolveChildFromA: ((result: ClaraReadDirResult) => void) | undefined
  ...
  it('discards a stale root read after the active registered connection changes', async () => {
    let resolveFirst: ((result: ClaraReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<ClaraReadDirResult>(resolve => {
  ...
  it('returns no-bridge gracefully when window.claraDesktop is missing', async () => {
    delete (window as unknown as { claraDesktop?: unknown }).claraDesktop
  ...
  it('keeps an unreadable root on screen while it re-probes', async () => {
    ...
    let releaseProbe: ((value: ClaraReadDirResult) => void) | undefined
  ...
  it('keeps loaded rows on screen while the root refreshes', async () => {
    ...
    let releaseRefresh: ((value: ClaraReadDirResult) => void) | undefined
  ...
  it('clears the rows when the same path is re-read from another backend', async () => {
    ...
    let releaseFromB: ((value: ClaraReadDirResult) => void) | undefined
