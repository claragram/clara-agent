...
export type ProjectTreeEntry = ClaraReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: ClaraReadDirEntry) {
...
async function filterIgnored(entries: ClaraReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<ClaraReadDirResult> {
