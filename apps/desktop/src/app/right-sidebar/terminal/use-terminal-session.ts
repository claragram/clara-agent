...
const CLARA_PATHS_MIME = 'application/x-clara-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(CLARA_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(CLARA_PATHS_MIME)
...
export function useTerminalSession({
  ...
  useEffect(() => {
    ...
    const terminalApi = window.claraDesktop?.terminal
    ...
    const term = new Terminal({
      ...
      // Full-screen TUIs (clara --tui, vim) grab the mouse, so a plain drag
    ...
    // Capture the buffer on a leading-edge throttle and persist __PROT_0_synchroclaraly__ via
    ...
    term.attachCustomKeyEventHandler(event => {
      ...
      void (async () => {
        const text = (await window.claraDesktop?.readClipboard?.()) ?? ''
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[clara-terminal] WebGL unavailable; falling back to DOM', err)
      }
  ...
  useEffect(() => {
    ...
    return $terminalInjection.subscribe(command => {
      ...
      hasSessionActivityRef.current = true
      void window.claraDesktop?.terminal?.write(sessionId, `${command}\r`)
