...
vi.mock('@/clara', async importOriginal => {
  const actual = await importOriginal<typeof ClaraModule>()

  class FakeClaraGateway {
  ...
  return { ...actual, ClaraGateway: FakeClaraGateway }
})

import type * as ClaraModule from '@/clara'
import type { ClaraGateway } from '@/clara'
import {
  $gateway,
...
const fakeGateway = { connectionState: 'open' } as unknown as ClaraGateway
...
async function activateRemoteGateway() {
  ...
  setPrimaryGateway(primary as unknown as ClaraGateway, 'default')
  $gateway.set(primary as unknown as ClaraGateway)
...
describe('useGatewayRequest', () => {
  ...
  it('surfaces a missing optional scoped mint bridge without falling back to a stale ticket or local lookup', async () => {
    const { desktop, gateway } = await activateRemoteGateway()
    Reflect.deleteProperty(window.claraDesktop, 'getGatewayWsUrlFor')
  ...
  it('rejects instead of hanging forever when the reconnect getConnection() wedges (#93454)', async () => {
    ...
    } as unknown as ClaraGateway

    const getConnection = vi.fn(() => new Promise(() => undefined))

    ;(window as unknown as { claraDesktop: unknown }).claraDesktop = { getConnection }
    $gateway.set(dropped)
