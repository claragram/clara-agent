...
// bug that drives the REAL useGatewayBoot hook + REAL ClaraGateway through a
...
function Harness({
  ...
  refreshClaraConfig = async () => undefined,
  refreshSessions
}: {
...
beforeEach(() => {
  ...
  ;(window as { claraDesktop?: unknown }).claraDesktop = fakeDesktop()
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForClara) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startClara()'s remote branch awaits waitForClara() for 45s before it
    ...
    ;(window as { claraDesktop?: unknown }).claraDesktop = desktop
    ...
    // After ~45s waitForClara gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Clara backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
  ...
  it('reports a Settings switch setup failure and does not disarm a newer switch started by recovery UI', async () => {
    ...
    vi.mocked(notifyError).mockImplementationOnce((_error, fallback) => {
      // A notification/recovery callback may __PROT_0_synchroclaraly__ start another
  ...
  it('a superseded Settings switch cannot publish delayed cwd or config work after the winner', async () => {
    ...
    const refreshClaraConfig = async (_force = false, shouldPublish?: () => boolean) => {
      if (!shouldPublish) {
        return
      }
    ...
    render(<Harness refreshClaraConfig={refreshClaraConfig} />)
    await flushAsync()
    expect($gatewayState.get()).toBe('open')
  ...
  it('a getConnection() that hangs on INITIAL boot rejects on its own after the reconnect-attempt timeout, not only when main eventually gives up (#93454)', async () => {
    ...
    // eventual timeout (e.g. waitForClara, ~45s) ever settled it. A wedge
    // that main never resolves (not even a rejection) must not hang
    // "Starting Clara…" forever; the renderer needs to own its own bound
  ...
  it('FIX: post-boot ticket-mint boot-progress errors do not lock the UI', async () => {
    ...
    act(() => {
      desktop.emitBootProgress({
        error: 'Could not reach the remote Clara gateway while refreshing its WebSocket ticket. Try reconnecting.',
  ...
  it('FIX: a failed session-list fetch during boot is non-fatal — the app still boots', async () => {
    ...
    // failDesktopBoot's "Clara couldn't start" overlay — the socket is open
  ...
  it('seeds the configured default project dir pre-connect — no route-resume race (#71873)', async () => {
    ...
    expect(cwdAtConnect).toBe('C:\\Clara')
    expect($currentCwd.get()).toBe('C:\\Clara')
  })
  ...
  it('FIX #82679: a transient remote boot failure self-heals — the next attempt rebuilds the dropped connection', async () => {
    ...
    // SSH/HTTP remote whose transport dropped. startClara() rejects with a
