import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@clara/shared'
...
export function useGatewayRequest() {
  ...
  const gateway = useStore($gateway) as ClaraGateway | null
  const gatewayRef = useRef<ClaraGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.claraDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<ClaraGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as ClaraGateway | null
      }),
    []
  )
