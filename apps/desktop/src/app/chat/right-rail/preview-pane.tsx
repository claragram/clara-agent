        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.claraDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({ embedded = false, onRestartServer, reloadRequest = 0, tabId, target }: PreviewPaneProps) {
  ...
  const annotateGuest = useCallback((): null | PreviewAnnotateGuest => {
    ...
    return {
      capture: async rect => {
        ...
        const dataUrl = await window.claraDesktop.capturePreview?.({ rect, viewport, webContentsId })
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Clara to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.claraDesktop?.watchPreviewFile ||
      !window.claraDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.claraDesktop.onPreviewFileChanged(payload => {
    ...
    void window.claraDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.claraDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.claraDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:clara-preview')
    ...
    const onGuestContextMenu = (event: Event) => {
      ...
      const zoom = window.claraDesktop?.zoom?.factor?.() || 1
      ...
      openGuestContextMenu(
        ...
        },
        {
          addToDictionary: (word: string) => {
            ...
            if (typeof webContentsId === 'number') {
              void window.claraDesktop?.contextMenuGuestAddWord?.({ webContentsId, word })
            }
          },
          copyImage: () => void window.claraDesktop?.contextMenuCopyImage?.(),
          ...
          <PreviewBrowserBar
            ...
            onOpenExternal={
              !isBrowserWindow() && !canOpenBrowserWindow()
                ? () => void window.claraDesktop?.openExternal(currentUrl)
                : undefined
            }
