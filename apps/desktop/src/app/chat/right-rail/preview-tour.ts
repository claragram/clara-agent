...
function buildTourScript(action: TourAction): string {
  return `(function () {
  var w = window;
  if (!w.__claraTourEngine) {
    ${driverIife}
    w.__claraTourHolder = {};
    w.__claraTourCollect = (${collectTourTargets.toString()});
    w.__claraTourEngine = (${runTourEngine.toString()});
  }
  if (!document.getElementById('__clara-tour-style')) {
    var style = document.createElement('style');
    style.id = '__clara-tour-style';
    style.textContent = ${JSON.stringify(driverCss)};
    (document.head || document.documentElement).appendChild(style);
  }
  return JSON.stringify(w.__claraTourEngine(
    w.driver.js.driver,
    w.__claraTourHolder,
    ${JSON.stringify(action)},
    w.__claraTourCollect,
    document
  ));
})()`
}
