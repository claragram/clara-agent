...
const preamble = () => `  var w = window;
  ...
  w.__claraActHolder = w.__claraActHolder || {};
  w.__claraAct = ${actEngineSource()};
  w.__claraWatch_fn = (${watchInPage.toString()});
  w.__claraWatchTag = ${WATCH_TAG};
  var holder = w.__claraActHolder;
  var act = function (a) { return w.__claraAct(document, holder, a); };
  var watch = function (stage, label) {
    try { w.__claraWatch_fn(document, holder, stage, label); } catch (err) {}
  };
...
${preamble()}
  ...
  document.addEventListener('pointerdown', function (e) {
    w.__claraHit = { tag: e.target ? e.target.tagName : '?', trusted: e.isTrusted === true };
...
function hitNote(hit?: { tag: string; trusted: boolean } | null): string | undefined {
  return hit && hit.tag === 'CLARA-WATCH' ? 'The action overlay intercepted the click instead of the page.' : undefined
}
