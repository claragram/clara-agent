...
describe('wake-word ear visibility', () => {
  ...
  it('stays mounted during a busy agent turn', () => {
    applyWakeStatus({ available: true, enabled: true, listening: true, phrase: 'hey clara' })
    renderControls({ busy: true, busyAction: 'stop' })

    expect(screen.getByLabelText('Wake word: "hey clara" — listening')).toBeTruthy()
  })

  it('stays mounted (enabled in config) even when a start was refused', () => {
    applyWakeStatus({ available: true, enabled: true, listening: false, phrase: 'hey clara' })
    ...
    expect(screen.getByLabelText('Wake word: "hey clara" — off')).toBeTruthy()
  })

  it('stays visible (never hides) even when unavailable and not enabled', () => {
    applyWakeStatus({ available: false, enabled: false, listening: false, phrase: 'hey clara' })
    renderControls()
  ...
  it('surfaces the backend refusal reason in the tooltip, still visible', () => {
    applyWakeStatus({ available: false, enabled: false, listening: false, phrase: 'hey clara' })
    applyWakeStartResult({ hint: 'run `clara tools` (Voice section)', reason: 'unavailable', started: false })
    renderControls()

    const ear = screen.getByLabelText('Wake word: "hey clara" — off')
  ...
  it('shows a disabled paused ear inside the voice-conversation pill', () => {
    ...
    const ear = screen.getByLabelText('Wake word: "hey clara" — paused during voice chat')
