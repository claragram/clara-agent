...
import { getClaraConfig, getClaraConfigDefaults } from '@/clara'
...
interface ClaraConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function useClaraConfig({ activeSessionIdRef }: ClaraConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshClaraConfig = useCallback(
    async (force = false, shouldPublish: () => boolean = () => true) => {
      ...
      try {
        const [config, defaults] = await Promise.all([getClaraConfig(), getClaraConfigDefaults().catch(() => ({}))])
        ...
        // rendering/applying Clara' built-in medium over the user's config.
        if (!canPublish()) {
          return
        }
  ...
  return { refreshClaraConfig, sttEnabled, voiceMaxRecordingSeconds }
}
