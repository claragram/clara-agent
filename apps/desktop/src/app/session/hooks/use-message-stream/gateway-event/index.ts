...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  const { activeSessionIdRef, compactedTurnRef, refreshClaraConfig, sessionStateByRuntimeIdRef } = deps
  ...
  // refreshClaraConfig — two REST calls (config + defaults) per event, per
