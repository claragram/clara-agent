...
// internal state from usePromptActions) flushes __PROT_1_synchroclaraly__ instead of
...
// it, rather than reaching for Clara's own browser and panes.
...
describe('usePromptActions /compress', () => {
  ...
  it('does not clobber the foreground transcript when compression resolves after a session switch', async () => {
    ...
    // Start the request in a __PROT_0_synchroclara__ act. The async `submitText` harness
...
describe('usePromptActions submit / queue drain semantics', () => {
  ...
  it('pins prompt.submit to the active registry connection when the remote session row is untagged', async () => {
    $connection.set({ connectionId: 'clara01', mode: 'remote' } as never)
  ...
  it('uploads file bytes via file.attach on a remote gateway and submits the rewritten ref', async () => {
    ...
    const requestGateway = vi.fn(async (method: string, params?: Record<string, unknown>) => {
      ...
      if (method === 'file.attach') {
        return {
          attached: true,
          path: '/remote/work/.clara/desktop-attachments/report.txt',
          ref_text: '@file:.clara/desktop-attachments/report.txt',
          uploaded: true
        } as never
      }
    ...
    expect(calls[1]?.params).toEqual({
      session_id: RUNTIME_SESSION_ID,
      text: '@file:.clara/desktop-attachments/report.txt\n\nconvert this to epub'
    })
  })
  ...
  it('uploads Windows file bytes when local mode fronts a POSIX WSL/Docker backend', async () => {
    ...
    const requestGateway = vi.fn(async (method: string, params?: Record<string, unknown>) => {
      ...
      if (method === 'file.attach') {
        return {
          attached: true,
          path: '/root/.clara/desktop-attachments/report.txt',
          ref_text: '@file:.clara/desktop-attachments/report.txt',
          uploaded: true
        } as never
      }
    ...
    expect(calls[1]).toEqual({
      ...
      params: { session_id: RUNTIME_SESSION_ID, text: '@file:.clara/desktop-attachments/report.txt\n\nsummarize' }
    })
  })
  ...
  it('uploads file bytes when the terminal backend is a container (docker)', async () => {
    ...
    const requestGateway = vi.fn(async (method: string, params?: Record<string, unknown>) => {
      ...
      if (method === 'file.attach') {
        return {
          attached: true,
          path: '/root/.clara/attachments/report.txt',
          ref_text: '@file:/root/.clara/attachments/report.txt',
          uploaded: true
        } as never
      }
...
describe('usePromptActions eager-upload races', () => {
  ...
  it('joins an in-flight eager upload at submit instead of staging the file twice', async () => {
    ...
    // under .clara/desktop-attachments/. Submit must await the in-flight upload
    ...
    const requestGateway = vi.fn(async (method: string) => {
      ...
      if (method === 'file.attach') {
        ...
        return { attached: true, ref_text: '@file:.clara/desktop-attachments/doc.pdf', uploaded: true } as never
      }
...
describe('usePromptActions submit session-context isolation (#54527)', () => {
  ...
  it('aborts when the user switches sessions during the tail of a successful create', async () => {
    ...
    // tell: every switch path retargets it __PROT_2_synchroclaraly__, so it no longer
  ...
  it('still aborts when the user genuinely switches chats after create, during attachment sync (#62805)', async () => {
    ...
    const requestGateway = vi.fn(async (method: string, params?: Record<string, unknown>) => {
      ...
      if (method === 'file.attach') {
        ...
        return {
          attached: true,
          ref_text: '@file:.clara/desktop-attachments/test.pdf',
          uploaded: true
        } as never
      }
...
describe('usePromptActions busy-gateway churn tolerance (#64327)', () => {
  ...
  it('still aborts when the user genuinely moves to a different chat mid-submit', async () => {
    ...
    // selection and route __PROT_3_synchroclaraly__.
...
describe('usePromptActions eager attachment upload (drop-time)', () => {
  ...
  it('uploads a dropped file the moment it lands (active session) and rewrites the chip with the gateway ref', async () => {
    ...
    $connection.set({ mode: 'remote' } as never)
    const readFileDataUrl = vi.fn(async () => 'data:application/pdf;base64,JVBERi0=')
    Object.defineProperty(window, 'claraDesktop', { configurable: true, value: { readFileDataUrl } })
    ...
    const requestGateway = vi.fn(async (method: string) => {
      ...
      if (method === 'file.attach') {
        return {
          attached: true,
          ref_text: '@file:.clara/desktop-attachments/DEVIS_signed.pdf',
          uploaded: true
        } as never
      }
    ...
    expect(chip.refText).toBe('@file:.clara/desktop-attachments/DEVIS_signed.pdf')
