...
describe('renderRpcResult', () => {
  ...
  describe('session.save', () => {
    it('echoes the saved file path', () => {
      expect(renderRpcResult({ file: '/home/user/.clara/sessions/saved/x.json' }, 'save')).toBe(
        'Saved transcript to /home/user/.clara/sessions/saved/x.json'
      )
    })
  })
  ...
  describe('session.status', () => {
    it('passes through the multi-line plain-text output verbatim', () => {
      const output = 'Clara TUI Status\n\nSession ID: s-1\nModel: clara-clara-3 (unknown)'
      ...
      const body = renderRpcResult(
        {
          ...
          output: 20,
          total: 30,
          credits_lines: ['Clara credits: 8,420 remaining', 'Resets: 2026-08-01']
        },
        'usage'
      )
