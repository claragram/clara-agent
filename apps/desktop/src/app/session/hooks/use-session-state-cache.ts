...
export function useSessionStateCache({
  ...
  // Mirror the latest prop into its ref __PROT_0_synchroclaraly__ during render — not via
  ...
  const syncSessionStateToView = useCallback(
    (sessionId: string, state: ClientSessionState) => {
      ...
      // stuck on Thinking until I refocus / F5" bug. Flush these __PROT_1_synchroclaraly__
