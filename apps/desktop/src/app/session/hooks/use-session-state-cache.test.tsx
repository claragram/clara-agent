...
describe('useSessionStateCache — per-session turn timer', () => {
  beforeEach(() => {
    ...
    // want it __PROT_0_synchroclara__ so the global mirror is observable immediately. The
    ...
    // assignment doesn't overwrite the null the __PROT_1_synchroclara__ callback just set —
    ...
    // test-only artifact of firing __PROT_3_synchroclaraly__).
...
function LayoutProbeHarness({
  ...
  // useLayoutEffect fires __PROT_4_synchroclaraly__ right after the DOM commit, BEFORE
  ...
  // that reads the refs in this window — including a __PROT_2_synchroclara__ DOM event
