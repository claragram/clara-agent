...
export function useRouteResume({
  ...
  useEffect(() => {
    ...
    if (routedSessionId) {
      ...
      // stuck empty forever. selectedStoredSessionIdRef is set __PROT_0_synchroclaraly__ at
      ...
      if ((gatewayBecameOpen || explicitlyRequested || !alreadyActive) && shouldResume && !creatingSessionRef.current) {
        ...
        // resumeSession makes __PROT_1_synchroclaraly__ at entry.
  ...
  // sets it __PROT_2_synchroclaraly__ at entry) and the pathname/gateway are unchanged, so
