...
function setWorkspaceTransfer(transferring: boolean): void {
  window.claraDesktop?.hud?.setWorkspaceTransfer?.(transferring)
}

function moveHud(state: PressState): void {
  window.claraDesktop?.hud?.moveBy?.({
    width: state.originW,
...
function armGrab(state: PressState, workspaceTransfer: boolean): void {
  ...
  window.claraDesktop?.hud?.beginMove?.()
}
 ...
 * (see clara:hud:move-by — a transparent frameless window drifts wider on
...
export function useHudComposerDrag(
  ...
  const reset = useCallback(() => {
    ...
    if (state) {
      if (state.armed) {
        window.claraDesktop?.hud?.endMove?.()
      }
