...
import { getClaraConfigRecord, saveMcpServers } from '@/clara'
...
import { setClaraConfigCache } from '../hooks/use-config-record'

/**
 * Explicit-confirm gate for `clara://mcp/install` deep links. The payload is
 ...
 * confirming lets Clara spawn that local process. An existing server name is
...
export function McpInstallDeepLinkDialog() {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const current = getServers(await getClaraConfigRecord())
      ...
      setClaraConfigCache(previous => (previous ? { ...previous, mcp_servers: nextServers } : previous))
