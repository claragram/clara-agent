...
import { getLatestSessionMessages } from '@/clara'
...
import { useClaraConfigRecord } from '../hooks/use-config-record'
...
import { useClaraConfig } from '../session/hooks/use-clara-config'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  const { refreshClaraConfig, sttEnabled, voiceMaxRecordingSeconds } = useClaraConfig({ activeSessionIdRef })
  ...
  useEffect(() => {
    ...
    window.addEventListener('clara:open-keybinds', onOpenKeybinds)

    return () => window.removeEventListener('clara:open-keybinds', onOpenKeybinds)
  }, [navigate])
  ...
  useEffect(() => {
    ...
    void refreshClaraConfig(true)
    void refreshActiveProfile()
    resetProjectTreeState()
  }, [gatewayScope, refreshCurrentModel, refreshClaraConfig])
  ...
  const configRecord = useClaraConfigRecord()
