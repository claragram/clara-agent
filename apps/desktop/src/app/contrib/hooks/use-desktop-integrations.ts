...
import { pathFromClaraDeepLink, resolveClaraOpenPath } from '@/lib/clara-open-target'
...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    ...
    // "About Clara" — it is the OS-standard affordance for updating THIS app,
    ...
    const unsubscribe = window.claraDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow('client'))
  ...
  useEffect(() => {
    window.claraDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = window.claraDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.claraDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // clara://index-network/intent/1 vocabulary with deep links.
  useEffect(() => {
    const unsubscribe = window.claraDesktop?.onNotificationActivate?.(payload => {
      if (!payload) {
        return
      }
      ...
      if (payload.activate) {
        ...
        // the pre-IPC validation — any future claraDesktop.notify caller gets
        // funneled through the same resolver.
        const path = resolveClaraOpenPath(payload.activate)
  ...
  useEffect(() => {
    const unsubscribe = window.claraDesktop?.onDeepLink?.(payload => {
      if (!payload?.kind) {
        return
      }
      ...
      // link (clara://index-network/intent/1, clara://open/…). The resolver
      // rejects reserved kinds and unsafe paths.
      const path = pathFromClaraDeepLink(payload.kind, payload.name || '', payload.params || {})
    ...
    void window.claraDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    const unsubscribe = window.claraDesktop?.onClosePreviewRequested?.(
  ...
  useEffect(() => {
    const unsubscribe = window.claraDesktop?.onPreviewNav?.(command => {
  ...
  useEffect(() => {
    const unsubscribe = window.claraDesktop?.onOpenFolderRequested?.(() => void openFolderAsProject())
