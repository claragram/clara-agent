 ...
 * agent sources (local runtime + any number of remote gateways / Clara Cloud
...
export function ConnectionsRegistrySection() {
  ...
  useEffect(() => {
    if (!editorWantsOauth || !editorUrl || !window.claraDesktop?.probeConnectionConfig) {
      setAuthProbe(null)
  ...
  const signInOauth = useCallback(async () => {
    ...
    try {
      const result = await window.claraDesktop.oauthLoginConnectionConfig(editorUrl)
  ...
  // Fan out `clara update` to every eligible source; per-connection results
