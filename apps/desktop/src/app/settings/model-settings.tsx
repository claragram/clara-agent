      ...
      {moa && currentMoaPreset && (
        <section>
          ...
          <p className="mb-2 text-xs text-muted-foreground">
            {locale === 'fr'
              ? "Configurez des préréglages nommés apparaissant comme modèles sous le fournisseur Mélange d'agents. L'agrégateur est le modèle d'exécution."
          ...
          <div className="mb-2 flex flex-wrap items-center gap-2">
            <Select onValueChange={setSelectedMoaPreset} value={selectedMoaPreset || moa.default_preset}>
              <SelectTrigger className={cn('min-w-40', CONTROL_TEXT)}>
                <SelectValue placeholder={locale === 'fr' ? 'Préréglage' : 'Preset'} />
              </SelectTrigger>
              <SelectContent>
            ...
            <Button
              ...
              variant="text"
            >
              {locale === 'fr' ? 'Définir par défaut' : 'Set default'}
            </Button>
            <Button
            ...
            <Input
              ...
              placeholder={locale === 'fr' ? 'nouveau préréglage' : 'new preset'}
            ...
            <Button
              ...
              variant="textStrong"
            >
              {locale === 'fr' ? 'Ajouter un préréglage' : 'Add preset'}
            </Button>
          </div>
          <div className="mb-2 text-xs text-muted-foreground">
            {locale === 'fr' ? 'Par défaut\xa0:' : 'Default:'} <span className="font-mono">{moa.default_preset}</span>
          </div>
          <div className="grid gap-1">
            {currentMoaPreset.reference_models.map((slot, index) => (
              <ListRow
                ...
                title={locale === 'fr' ? `Modèle de référence ${index + 1}` : `Reference ${index + 1}`}
              />
            ))}
            <Button
              disabled={applying}
              ...
              variant="textStrong"
            >
              {locale === 'fr' ? 'Ajouter un modèle de référence' : 'Add reference model'}
            </Button>
            <ListRow
              below={
                <div className="mt-2 flex flex-wrap items-center gap-2 pt-1">
              ...
              title={locale === 'fr' ? 'Agrégateur' : 'Aggregator'}
            />
          </div>
        </section>
      )}
