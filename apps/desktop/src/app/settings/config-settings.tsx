...
import { getClaraConfigSchema, getElevenLabsVoices, saveClaraConfig } from '@/clara'
...
function ConfigSettingsInner({
  ...
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useClaraConfigRecord(scopeProfile)
  ...
  const writeConfigCache = useMemo(() => claraConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const {
    ...
    queryKey:
      scopeProfile == null ? ['clara-config-schema'] : ['clara-config-schema', normalizeProfileKey(scopeProfile)],
    queryFn: () => getClaraConfigSchema(scopeProfile),
    staleTime: 5 * 60 * 1000
  })
  ...
  // never touched — possibly changed out-of-band by `clara config set`
  // while this page sat open — is never resent with its stale value.
  const configBaselineRef = useRef<ClaraConfigRecord | null>(null)
    ...
    const t = window.setTimeout(() => {
      ...
      saveQueueRef.current = saveQueueRef.current.then(async () => {
        try {
          ...
          const result = await saveClaraConfig(patch, scopeProfile)
  ...
  const applyConfig = (next: ClaraConfigRecord) => {
    saveVersionRef.current += 1
    setConfig(next)
    setSaveVersion(saveVersionRef.current)
  }

  const updateConfig = (next: ClaraConfigRecord) => {
