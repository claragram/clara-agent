  ...
  it('subscribes for verification, opens the verification URL, cleans up, and invalidates on completion', async () => {
    ...
    act(() => {
      gatewayMock.emit('billing.step_up.verification', {
        payload: {
          user_code: 'ABCD-1234',
          verification_url: 'https://portal.claraship.com/device'
        },
    ...
    expect(result.current.verification).toEqual({
      code: 'ABCD-1234',
      url: 'https://portal.claraship.com/device'
    })

    result.current.openVerification()
    expect(window.claraDesktop?.openExternal).toHaveBeenCalledWith('https://portal.claraship.com/device')
