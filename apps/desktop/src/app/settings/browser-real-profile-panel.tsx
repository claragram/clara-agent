...
import { type ProfileScope, saveClaraConfigRecord } from '@/clara'
 ...
 * the default browser's profile (cookies/logins) into a Clara-owned copy;
...
export function BrowserRealProfilePanel({ profile }: BrowserRealProfilePanelProps) {
  ...
  const { data: config } = useClaraConfigRecord(profile)
  ...
  const toggle = useCallback(
    async (on: boolean) => {
      ...
      try {
        await saveClaraConfigRecord(next, profile)
        notify({
          kind: 'info',
