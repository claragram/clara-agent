...
import { disconnectOAuthProvider, listOAuthProviders } from '@/clara'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.claraDesktop?.terminal)
...
//      `clara model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Clara can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Clara can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // Clara never deletes creds another tool owns behind a silent API call.
