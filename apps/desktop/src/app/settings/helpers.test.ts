...
describe('settings helpers', () => {
  ...
  it('reads and writes nested config paths', () => {
    const config: ClaraConfigRecord = { display: { theme: 'mono' } }
  ...
  it('rejects prototype-polluting config paths', () => {
    const config: ClaraConfigRecord = {}
  ...
  describe('providerGroup', () => {
    it('maps a provider env var to its labeled group', () => {
      ...
      expect(providerGroup('CLARA_API_KEY')).toBe('Clara Portal')
    ...
    it('prefers the longest matching prefix so CN/regional buckets win', () => {
      ...
      // CLARA_QWEN_ shares the CLARA_ stem with other integrations.
      expect(providerGroup('CLARA_QWEN_BASE_URL')).toBe('DashScope (Qwen)')
  ...
  describe('enumOptionsFor — backend selector dropdowns', () => {
    ...
    it('surfaces user-defined command-type TTS providers (canonical providers nesting + legacy)', () => {
      const withCustom: ClaraConfigRecord = {
        tts: {
          provider: 'neutts',
    ...
    it('never offers a built-in name as a command provider, even one absent from the dropdown list', () => {
      const shadowing: ClaraConfigRecord = {
        tts: {
          provider: 'edge',
          providers: {
  ...
  describe('sectionFieldEntries', () => {
    it('renders memory.provider from config even when the backend schema omits it', () => {
      ...
      const config: ClaraConfigRecord = { memory: { memory_enabled: true, provider: '' } }
    ...
    it('infers the field type from the config value when the schema omits the key', () => {
      const config: ClaraConfigRecord = { memory: { provider: '', memory_enabled: true, memory_char_limit: 2200 } }
    ...
    it('prefers the backend schema entry over inference when both exist', () => {
      ...
      const config: ClaraConfigRecord = { memory: { provider: 'honcho' } }
  ...
  describe('clearsEnabledToolsets', () => {
    it('flags a non-empty → empty transition', () => {
      const prev: ClaraConfigRecord = { toolsets: ['memory', 'terminal', 'web_search'] }
      const next: ClaraConfigRecord = { toolsets: [] }
    ...
    it('does not flag a non-empty → missing transition (deep-merge preserves the key)', () => {
      ...
      const prev: ClaraConfigRecord = { toolsets: ['memory'] }
      const next: ClaraConfigRecord = {}

      expect(clearsEnabledToolsets(prev, next)).toBe(false)
    })

    it('does not flag when at least one toolset remains', () => {
      const prev: ClaraConfigRecord = { toolsets: ['memory', 'terminal'] }
      const next: ClaraConfigRecord = { toolsets: ['memory'] }

      expect(clearsEnabledToolsets(prev, next)).toBe(false)
    })

    it('does not flag when the list was already empty', () => {
      const prev: ClaraConfigRecord = { toolsets: [] }
      const next: ClaraConfigRecord = { toolsets: [] }
    ...
    it('does not flag an unrelated edit that never touched toolsets', () => {
      const prev: ClaraConfigRecord = { model: 'a', toolsets: ['memory'] }
      const next: ClaraConfigRecord = { model: 'b', toolsets: ['memory'] }
  ...
  describe('diffConfig', () => {
    it('omits a top-level key the draft never touched', () => {
      ...
      // an agent set via `clara config set` while the page sat open must not
      // come back in the patch just because it's still present in the draft.
      const baseline: ClaraConfigRecord = { fallback_providers: ['nara1'], timezone: 'UTC' }
      const draft: ClaraConfigRecord = { fallback_providers: ['nara1'], timezone: 'America/New_York' }
    ...
    it('includes a nested key only when it actually changed, leaving siblings out', () => {
      const baseline: ClaraConfigRecord = { display: { personality: 'default', show_reasoning: true } }
      const draft: ClaraConfigRecord = { display: { personality: 'default', show_reasoning: false } }
    ...
    it('sends a new key that was absent from the baseline', () => {
      const baseline: ClaraConfigRecord = {}
      const draft: ClaraConfigRecord = { timezone: 'UTC' }
    ...
    it('returns an empty object when the draft matches the baseline exactly', () => {
      const baseline: ClaraConfigRecord = { toolsets: ['memory'], display: { personality: 'default' } }
      const draft: ClaraConfigRecord = { toolsets: ['memory'], display: { personality: 'default' } }

      expect(diffConfig(baseline, draft)).toEqual({})
    })

    it('treats an array as a whole value, not diffed element by element', () => {
      const baseline: ClaraConfigRecord = { toolsets: ['memory', 'terminal'] }
      const draft: ClaraConfigRecord = { toolsets: ['memory'] }
