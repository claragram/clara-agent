...
import { getClaraConfigRecord, type ProfileScope, profileScopeKey } from '@/clara'
...
//
// Distinct from session/hooks/use-clara-config.ts, which is side-effecting —
// it pushes personality/cwd/voice/… into the session stores for live chat.
export const CLARA_CONFIG_KEY = ['clara-config-record'] as const
...
// share a cache row.
export const claraConfigKey = (profile?: ProfileScope) =>
  profile == null ? CLARA_CONFIG_KEY : ([...CLARA_CONFIG_KEY, profileScopeKey(profile)] as const)
...
export const useClaraConfigRecord = (profile?: ProfileScope) =>
  useQuery({
    queryKey: claraConfigKey(profile),
    ...
    queryFn: () => getClaraConfigRecord(profile ?? undefined),
    staleTime: 0
  })

// setClaraConfigCache writes the app-wide (base-key) record. Pass a profile to
...
export const setClaraConfigCache = writeCache<ClaraConfigRecord>(CLARA_CONFIG_KEY)
export const claraConfigCacheWriter = (profile?: ProfileScope) =>
  writeCache<ClaraConfigRecord>(claraConfigKey(profile))

export const invalidateClaraConfig = (profile?: ProfileScope) =>
  queryClient.invalidateQueries({ queryKey: claraConfigKey(profile) })
