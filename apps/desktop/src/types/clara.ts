...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Clara doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in clara_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `clara model` uses — instead of
...
export type ClaraConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `clara model` CLI / onboarding OAuth flow. */
  auth_type?: string
  ...
  /** Clara only: whether the current account is on the free tier. */
  free_tier?: boolean
  /** Clara only: paid models a free-tier user cannot select (shown disabled). */
...
// non-desktop) Clara simply carries none of it. See store/profile-share.ts.
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (clara.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors clara_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Clara, 'hub' = installed. */
  provenance?: 'agent' | 'bundled' | 'hub'
}

/** One entry of the built-in optional-skills catalog (optional-skills/ in the
 *  repo) — official skills that ship with Clara but install on demand. */
...
export interface ToolProvider {
   ...
   *  CLI `clara tools` active-provider detection). */
  is_active: boolean
  /** Honest readiness computed server-side (keys ∧ Clara entitlement ∧
 ...
 *  not Clara); elsewhere, driver health from `cua-driver doctor`. `null`
...
export interface ActionStatusResponse {
  ...
  pid: number | null
  running: boolean
  /** clara-update only: durable completion identity recovered from update.log. */
  action_id?: string
  /** clara-update only: summary of the durable update receipt (#91277 bullet 3) —
...
/** Shape of `GET /api/clara/update/check` — the backend's own update state.
...
/** One Clara-approved MCP catalog entry from `GET /api/mcp/catalog`. */
...
export interface ModelAssignmentResponse {
  ...
  base_url?: string
  /** Toolset keys auto-routed through the Clara Tool Gateway as a result of
   *  switching the main provider to Clara. Empty unless provider === 'clara'
