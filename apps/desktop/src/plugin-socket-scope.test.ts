...
// (#73044). Exercises the REAL clara + store/gateway + store/profile chain:
...
const { pluginSocket, setApiRequestConnection, setApiRequestProfile } = await import('@/clara')
...
const conn = (over: Partial<ClaraConnection> = {}): ClaraConnection =>
  ({
    authMode: 'oauth',
