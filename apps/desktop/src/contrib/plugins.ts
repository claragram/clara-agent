 ...
 *    a `ClaraPlugin` registers automatically (vite glob — drop a folder in).
 *    `clara-bots` (Bot Mode) ships in-tree and is ON by default; other
 *    reference/demo plugins live in the companion `clara-example-plugins`
 ...
 *  - RUNTIME: the on-disk doors (`<clara home>/desktop-plugins/<name>/plugin.js`
 *    and the unified-package half `<clara home>/plugins/<name>/desktop/plugin.js`)
...
const modules = import.meta.glob<{ default: ClaraPlugin }>('../plugins/*/plugin.{js,ts,tsx}', { eager: true })
...
export function discoverBundledPlugins(): void {
  ...
  for (const [path, mod] of Object.entries(modules)) {
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      console.warn(`[plugins] ${path} has no valid default ClaraPlugin export — skipped`)
