...
import type * as ClaraModule from '@/clara'
import type { ClaraReadDirResult } from '@/global'
...
// getStatus would supply the connected backend's clara_home — a REMOTE path in
// remote mode. The disk scanner must NOT derive the plugin root from it (#66899).
const getStatus = vi.fn(async () => ({ clara_home: '/remote/box/.clara' }))

vi.mock('@/clara', async importActual => ({
  ...(await importActual<typeof ClaraModule>()),
  getStatus: () => getStatus()
}))
...
describe('scanDiskPlugins (#66899)', () => {
  it('scans the Electron-resolved local roots, never the backend clara_home', async () => {
    desktopPluginsRoot.mockResolvedValue('/local/.clara/desktop-plugins')
    agentPluginsRoot.mockResolvedValue('/local/.clara/plugins')
    readDir.mockResolvedValue({ entries: [] })

    await discoverRuntimePlugins()

    expect(desktopPluginsRoot).toHaveBeenCalled()
    expect(readDir).toHaveBeenCalledWith('/local/.clara/desktop-plugins')
    expect(readDir).toHaveBeenCalledWith('/local/.clara/plugins')
    // The remote backend's clara_home must never feed the local plugin scan.
    expect(getStatus).not.toHaveBeenCalled()
    expect(readDir).not.toHaveBeenCalledWith('/remote/box/.clara/desktop-plugins')
  })
  ...
  it('treats a package without a Desktop half as metadata, not a throwing file read', async () => {
    ...
    readDir.mockImplementation(async dir => {
      if (dir === '/local/.clara/plugins') {
        return { entries: [{ isDirectory: true, name: 'my-feature', path: '/local/.clara/plugins/my-feature' }] }
      }

      if (dir === '/local/.clara/plugins/my-feature') {
        return {
          entries: [{ isDirectory: false, name: 'plugin.yaml', path: '/local/.clara/plugins/my-feature/plugin.yaml' }]
        }
      }
    ...
    expect(readDir).toHaveBeenCalledWith('/local/.clara/plugins/my-feature')
    expect(readDir).not.toHaveBeenCalledWith('/local/.clara/plugins/my-feature/desktop')
  ...
  it('a DIRECTORY named plugin.js is not a plugin entry (metadata walk rejects it)', async () => {
    ...
    agentPluginsRoot.mockResolvedValue('')
    readDir.mockImplementation(async dir => {
      if (dir === '/local/.clara/desktop-plugins') {
        return { entries: [{ isDirectory: true, name: 'odd', path: '/local/.clara/desktop-plugins/odd' }] }
      }

      if (dir === '/local/.clara/desktop-plugins/odd') {
        // A folder literally named plugin.js — must resolve to "no entry".
        return {
          entries: [{ isDirectory: true, name: 'plugin.js', path: '/local/.clara/desktop-plugins/odd/plugin.js' }]
        }
      }
  ...
  it('still scans the standalone root when agentPluginsRoot is absent (older shell)', async () => {
    delete (window.claraDesktop as unknown as { agentPluginsRoot?: unknown }).agentPluginsRoot
    desktopPluginsRoot.mockResolvedValue('/local/.clara/desktop-plugins')
    readDir.mockResolvedValue({ entries: [] })
  ...
  it('loads a unified desktop half OPT-IN: inventoried but not activated by default', async () => {
    ...
    readDir.mockImplementation(async dir => {
      if (dir === '/local/.clara/plugins') {
        return { entries: [{ isDirectory: true, name: 'uni', path: '/local/.clara/plugins/uni' }] }
      }

      if (dir === '/local/.clara/plugins/uni') {
        return {
          entries: [{ isDirectory: true, name: 'desktop', path: '/local/.clara/plugins/uni/desktop' }]
        }
      }

      if (dir === '/local/.clara/plugins/uni/desktop') {
        return {
            ...
            ? [
                {
                  isDirectory: false,
                  name: 'plugin.js',
                  path: '/local/.clara/plugins/uni/desktop/plugin.js'
                }
              ]
            : []
        }
    ...
    try {
      ...
      // ~/.clara/plugins stays installed-but-inert until the user toggles it.
...
describe('watchRuntimePlugins dir watch (#66899)', () => {
  it('watches both Electron-resolved local roots, never the backend clara_home', async () => {
    desktopPluginsRoot.mockResolvedValue('/local/.clara/desktop-plugins')
    agentPluginsRoot.mockResolvedValue('/local/.clara/plugins')
    readDir.mockResolvedValue({ entries: [] })
    ...
    expect(watchDirectory).toHaveBeenCalledWith('/local/.clara/desktop-plugins')
    expect(watchDirectory).toHaveBeenCalledWith('/local/.clara/plugins')
    expect(watchDirectory).not.toHaveBeenCalledWith('/remote/box/.clara/desktop-plugins')
    expect(getStatus).not.toHaveBeenCalled()
  })
})
...
describe('plugin source reads (512 KiB preview-cap bug)', () => {
  ...
  const standaloneRootWith = (name: string) => {
    const folder = `/local/.clara/desktop-plugins/${name}`
  ...
  it('loads the full source via readPluginSource when the shell offers it', async () => {
    ;(window.claraDesktop as unknown as { readPluginSource: unknown }).readPluginSource = readPluginSource
    desktopPluginsRoot.mockResolvedValue('/local/.clara/desktop-plugins')
    agentPluginsRoot.mockResolvedValue('')
    ...
    try {
      ...
      expect(readPluginSource).toHaveBeenCalledWith('/local/.clara/desktop-plugins/big/plugin.js')
      expect(register).toHaveBeenCalledTimes(1)
  ...
  it('older shell without readPluginSource: a truncated preview read fails LOUDLY, never evaluates', async () => {
    ...
    try {
      ...
      expect($pluginRecords.get().huge).toMatchObject({
        kind: 'disk',
        status: 'error',
        file: '/local/.clara/desktop-plugins/huge/plugin.js'
      })
...
describe('bundled-shadowed disk copies', () => {
  it('skips a disk copy of a bundled plugin but publishes a visible inventory row', async () => {
    ...
    publishPlugin({ id: 'clara-bots', name: 'Bot Mode', kind: 'bundled', status: 'loaded' })
    ...
    try {
      const id = await loadRuntimePlugin(
        'export default { id: "clara-bots", name: "Bot Mode", register() {} }',
        'clara-bots',
        { file: '/local/.clara/desktop-plugins/clara-bots/plugin.js' }
      )

      // Skipped — the bundled copy stays the only live registration...
      expect(id).toBeNull()
      expect($pluginRecords.get()['clara-bots']).toMatchObject({ kind: 'bundled', status: 'loaded' })
      ...
      expect($pluginRecords.get()['clara-bots:disk-shadowed']).toMatchObject({
        kind: 'disk',
        status: 'disabled',
        file: '/local/.clara/desktop-plugins/clara-bots/plugin.js'
      })
