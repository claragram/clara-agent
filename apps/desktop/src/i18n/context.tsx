...
export interface I18nConfigClient {
  getConfig: () => Promise<ClaraConfigRecord>
  saveConfig: (config: ClaraConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.claraDesktop?.api) {
      return Promise.resolve({})
    }

    return getClaraConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.claraDesktop?.api) {
...
export function getConfigDisplayLanguage(config: ClaraConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: ClaraConfigRecord, locale: Locale): ClaraConfigRecord {
