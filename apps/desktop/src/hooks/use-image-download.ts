...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'clara:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.claraDesktop?.saveImageFromUrl) {
        if (await window.claraDesktop.saveImageFromUrl(src)) {
