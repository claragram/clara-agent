...
function sessionsBotsTree() {
  ...
  registerPane('clara-bots:pane', { placement: 'left', hideOnly: true })
  registerPane('workspace', { placement: 'main', uncloseable: true })
  $layoutTree.set(
    split('row', [
      group(['sessions', 'clara-bots:pane'], { active: 'sessions', id: 'g-side' }),
...
describe('hide-only strip tabs', () => {
  it('hides and shows a chrome tab, keeping the pane in the tree', () => {
    ...
    expect(setStripTabHidden('clara-bots:pane', true)).toBe(true)
    expect($hiddenTreePanes.get()).toContain('clara-bots:pane')
    expect($hiddenStripTabs.get()).toContain('clara-bots:pane')
    // Hidden, not dismissed: the pane stays in the layout tree.
    expect(allPaneIds($layoutTree.get()!)).toContain('clara-bots:pane')

    expect(setStripTabHidden('clara-bots:pane', false)).toBe(true)
    expect($hiddenTreePanes.get()).not.toContain('clara-bots:pane')
    expect($hiddenStripTabs.get()).not.toContain('clara-bots:pane')
  })

  it('refuses to hide the zone last visible tab', () => {
    sessionsBotsTree()
    setStripTabHidden('clara-bots:pane', true)
  ...
  it('persists hides and clears them on reveal', () => {
    ...
    const persisted = JSON.parse(window.localStorage.getItem('clara.desktop.hiddenStripTabs.v1') ?? '[]')

    expect(persisted).toContain('clara-bots:pane')
    ...
    revealTreePane('clara-bots:pane')
    expect($hiddenTreePanes.get()).not.toContain('clara-bots:pane')
    expect(window.localStorage.getItem('clara.desktop.hiddenStripTabs.v1')).toBeNull()
  })
  ...
  it('lists the zone hide-only tabs with live hidden state for the menu', () => {
    ...
    expect(hideOnlyZoneTabs('g-side')).toEqual([
      ...
      { hidden: true, id: 'clara-bots:pane', title: 'clara-bots:pane' }
    ])
  ...
  it('excludes hide-only tabs from every close verb', () => {
    ...
    $layoutTree.set(group(['sessions', 'clara-bots:pane', 'session-tile:x'], { active: 'sessions', id: 'g-mixed' }))
