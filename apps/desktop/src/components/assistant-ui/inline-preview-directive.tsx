 ...
 * own design.
 *
 * WIDGETS TALK BACK OFF-SCREEN. `window.clara.send(prompt)` (or declarative
 * `data-clara-send` on any clickable element) routes the prompt through the
...
const SIZE_MESSAGE_TYPE = 'clara-inline-preview-size'
const INTENT_MESSAGE_TYPE = 'clara-inline-preview-intent'
...
/** The script that gives the widget its ONE voice: `clara.send(prompt)`.
 ...
 *  wires `data-clara-send` so declarative HTML works with zero script:
 *  `<button data-clara-send="get-price eth">ETH</button>`. */
export function intentScript(token: string): string {
  return (
    ...
    ')},"*");return true}' +
    'window.clara={send:send};' +
    'addEventListener("click",function(e){var el=e.target&&e.target.closest?' +
    'e.target.closest("[data-clara-send]"):null;' +
    'if(el)send(el.getAttribute("data-clara-send")||"")},true)})()</script>'
  )
}
...
export function InlinePreviewDirective({
  ...
  // URL connection — including a same-machine `clara serve` — renders live.)
