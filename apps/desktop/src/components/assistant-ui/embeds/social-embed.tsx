...
const SCRIPT: Record<string, { id: string; src: string }> = {
  instagram: { id: 'clara-ig-embed', src: 'https://www.instagram.com/embed.js' },
  tiktok: { id: 'clara-tt-embed', src: 'https://www.tiktok.com/embed.js' },
  twitter: { id: 'clara-tw-embed', src: 'https://platform.twitter.com/widgets.js' }
}
...
export default function SocialEmbedRenderer({ descriptor }: { descriptor: EmbedDescriptor }) {
  ...
  useEffect(() => {
    ...
    void loadScript(descriptor.provider).then(() => {
      // The script renders __PROT_0_asynchroclaraly__; nudge a few times so the embed
