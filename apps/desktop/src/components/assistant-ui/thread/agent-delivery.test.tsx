...
describe('delivery command detection', () => {
  it('matches the canonical delivery command', () => {
    const cmd = 'clara -p turqoise chat --in ~ -c "Bot Chat" -Q -q "Message from 🤖 Clara (@clara): hi there"'

    expect(deliveryTargetFromCommand(cmd)).toBe('turqoise')
  })

  it('matches with a cd prefix and timeout wrapper', () => {
    const cmd = 'cd ~ && timeout 240 clara -p mr-tester chat --in "~" -Q -q "Message from 🤖 Clara: hello"'
  ...
  it('ignores ordinary terminal commands', () => {
    ...
    expect(deliveryTargetFromCommand('clara -p turqoise chat -q "plain question"')).toBeNull()
    expect(deliveryTargetFromCommand('clara sessions list')).toBeNull()
  })
})

describe('reply extraction', () => {
  it('strips session_id bookkeeping and keeps the reply', () => {
    const output = 'session_id: 20260813_220347_f69ac6\nHi Clara! Good to hear from you.'

    expect(replyTextFromResult({ output })).toBe('Hi Clara! Good to hear from you.')
  })
