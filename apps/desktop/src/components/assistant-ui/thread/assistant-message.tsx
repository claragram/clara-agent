...
const ErrorRecoveryActions: FC = () => {
  ...
  // Open Logs reveals the LOCAL Electron profile's CLARA_HOME/logs. On a
  ...
  const openLogs = useCallback(async () => {
    try {
      const root = await window.claraDesktop?.logsRoot?.()
      ...
      const result = await window.claraDesktop?.openDir?.(root)
