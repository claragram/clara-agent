...
const CLARA_REF_TYPES = WIRE_REFERENCE_KINDS
type ClaraRefType = (typeof CLARA_REF_TYPES)[number]
...
const CLARA_DIRECTIVE_RE = referenceRe()
...
export const claraDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(CLARA_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...claraDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Clara directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.claraDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.claraDesktop?.readFileDataUrl(id)
