import type { GatewayWsUrlResult } from '@clara/shared'
import type { TranslucencyState } from '@clara/shared/translucency'
...
declare global {
  interface Window {
    claraDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<ClaraConnection>
      ...
      // Resume this session in the user's own terminal emulator (`clara --tui
      ...
      // Clara Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new CLARA_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: ClaraApiRequest) => Promise<T>
      notify: (payload: ClaraNotification) => Promise<boolean>
      ...
      readFileText: (filePath: string) => Promise<ClaraReadFileTextResult>
      ...
      readPluginSource?: (filePath: string) => Promise<ClaraReadFileTextResult>
      ...
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<ClaraPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<ClaraPreviewWatch>
      ...
      watchDirectory?: (dir: string) => Promise<ClaraPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setActiveWork?: (payload: ClaraActiveWork) => void
      setTitleBarTheme?: (payload: ClaraTitleBarTheme) => void
      ...
      zoom?: {
        ...
        /** __PROT_0_Synchroclara__ zoom factor of this window (1 = 100%). */
        factor?: () => number
      ...
      readDir: (path: string) => Promise<ClaraReadDirResult>
      ...
      // Local Desktop runtime-plugin root (<CLARA_HOME>/desktop-plugins),
      ...
      /** LOCAL `<CLARA_HOME>/logs` (profile-aware) — error card "Open Logs". */
      logsRoot?: () => Promise<string>
      // Local AGENT-plugin root (<CLARA_HOME>/plugins), same Electron-local
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<ClaraGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<ClaraGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<ClaraGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<ClaraRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: ClaraReviewScope, baseRef?: null | string) => Promise<ClaraReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: ClaraReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<ClaraReviewShipInfo>
          ...
          prList: (repoPath: string, branches: string[], numbers?: number[]) => Promise<ClaraRepoPullRequests>
          ...
          fetchPrComment: (repoPath: string, url: string) => Promise<ClaraPrComment | null>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: ClaraTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<ClaraTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: ClaraWindowState) => void) => () => void
      ...
      onPreviewFileChanged: (callback: (payload: ClaraPreviewFileChanged) => void) => () => void
...
export interface ClaraTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface ClaraTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `clara update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  claraRoot?: string
...
export interface ClaraConnection {
  baseUrl: string
  darwinMajor?: number
...
export interface ClaraTitleBarTheme {
  background: string
  foreground: string
}

/** Turns in flight, so the main process can confirm before a quit kills them. */
export interface ClaraActiveWork {
  count: number
  titles: string[]
}

export interface ClaraWindowState {
  darwinMajor?: number
  isFullscreen: boolean
  isMinimized?: boolean
  isVisible?: boolean
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Clara Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Clara Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteUrl?: string
  // For a 'cloud' connection: the selected Clara Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // Whether the OAuth partition holds a live Clara portal (Privy) session — the
  // portal authenticates via Privy, so this reflects the privy-token cookie, NOT
  // the clara gateway session cookies. See cookiesHavePrivySession.
  signedIn: boolean
}

// A discovered Clara Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface DesktopBootProgress {
  error: string | null
  fakeMode: boolean
  /** True when the boot failure is a Clara Cloud agent that is down (HTTP 502/503/504). */
...
export interface ClaraApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface ClaraPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface ClaraReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface ClaraPreviewWatch {
  id: string
  path: string
}
...
export interface ClaraGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface ClaraGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
  isRemote: boolean
...
// preselect it.
export interface ClaraGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface ClaraRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface ClaraRepoStatus {
  branch: null | string
...
// turn began.
export type ClaraReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface ClaraReviewFile {
  path: string
  added: number
  removed: number
...
export interface ClaraReviewList {
  files: ClaraReviewFile[]
...
export interface ClaraReviewPr {
  url: string
  state: string
  number: number
}
...
export interface ClaraBranchPullRequest {
  branch: string
  draft: boolean
  number: number
...
export interface ClaraRepoPullRequests {
  ghReady: boolean
  prs: ClaraBranchPullRequest[]
}
...
export interface ClaraPrComment {
  author: string
  body: string
  diffHunk: string
  kind: 'issue' | 'review'
...
export interface ClaraReviewShipInfo {
  ghReady: boolean
  pr: ClaraReviewPr | null
}

export interface ClaraReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface ClaraReadDirResult {
  entries: ClaraReadDirEntry[]
  error?: string
}

export interface ClaraPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface ClaraSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
