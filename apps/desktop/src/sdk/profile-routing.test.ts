...
vi.mock('@/clara', () => ({ deleteProfile: vi.fn(), getLogs: vi.fn(), getStatus: vi.fn(), claraApi: vi.fn() }))
...
const { deleteProfile, claraApi } = await import('@/clara')
...
describe('connection-aware plugin host APIs', () => {
  it('retires a profile gateway before deleting it', async () => {
    ...
    // profile's backend, re-creating its CLARA_HOME (#94235).
  ...
  it('refreshes the profile inventory before asking Electron for routes', async () => {
    ...
    ;(window as unknown as { claraDesktop: unknown }).claraDesktop = { getProfileRoutes }
  ...
  it('reads and hides persisted sessions through the source primary without activating the profile', async () => {
    ...
    expect(claraApi).toHaveBeenNthCalledWith(
      1,
    ...
    expect(claraApi).toHaveBeenNthCalledWith(2, {
      connectionId: 'source-a',
      path: '/api/sessions/bot-chat',
      method: 'PATCH',
      body: { hidden: true, profile: 'backend-worker' }
    })
    expect(vi.mocked(claraApi).mock.calls.every(([request]) => !('profile' in request))).toBe(true)
