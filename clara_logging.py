"""Centralized logging setup for Clara Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.clara/logs/`` (profile-aware via ``get_clara_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``clara_cli.web_server``,
    ``clara_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Clara (TUI, gateway, ``hy_memory`` server, MCP
...
from clara_constants import get_config_path, get_clara_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_clara_session_injector", False):
    ...
    def _session_record_factory(*args, **kwargs):
        ...
        try:
            record.clara_home = str(get_clara_home().resolve())  # type: ignore[attr-defined]
        except Exception:
            record.clara_home = ""  # type: ignore[attr-defined]
        return record

    _session_record_factory._clara_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``clara logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``clara logs --component gateway``.
    "gateway": ("gateway", "clara_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("clara_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "clara_cli.web_server",
        "clara_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    ...
    """Configure the Clara logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    clara_home
        Override for the Clara home directory.  Falls back to
        ``get_clara_home()`` (profile-aware).
    log_level
    ...
    home = clara_home or get_clara_home()
    from clara_constants import mkdir_under_clara_home
    log_dir = mkdir_under_clara_home(home / "logs")
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_clara_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._clara_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the clara group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from clara_cli.config import is_managed
...
class _ProfileRoutingFileHandler(logging.Handler):
    """Route queued records to the log file for their Clara home.
    ...
    def __init__(
        ...
        *,
        ...
        self._clara_routed_log_path = Path(self.baseFilename)
    ...
    def _home_for_record(self, record: logging.LogRecord) -> Path:
        raw_home = getattr(record, "clara_home", "")
        try:
    ...
    def _handler_for_home(self, home: Path) -> _ManagedRotatingFileHandler:
        with self._profile_handlers_lock:
            ...
            mkdir_under_clara_home(path.parent)
...
# the module header): when several Clara processes log to the same file, an
    ...
    *root* directly, so emitting threads never block on file I/O or the
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._clara_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="clara-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        ...
        for h in list(root.handlers):
            if getattr(h, "_clara_queue", False):
...
def _add_rotating_handler(
    ...
    *,
    ...
    for existing in _queued_file_handlers:
        ...
        if getattr(existing, "_clara_routed_log_path", None) == resolved:
...
def _read_logging_config():
    ...
    try:
        ...
        # reuses the parse clara_cli.main's early bridge already did (one
        # config.yaml parse per process instead of 3-4). Fall back to a
        # direct parse when clara_cli.config isn't importable (bare
        # clara_logging consumers).
        try:
            from clara_cli.config import read_raw_config as _rrc
            cfg = _rrc() or {}
        except Exception:
