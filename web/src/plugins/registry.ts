 ...
 *
 * Plugins call window.__CLARA_PLUGINS__.register(name, Component)
 ...
 * Exposed at runtime as ``window.__CLARA_PLUGIN_SDK__.sdkVersion`` so a
...
export function exposePluginSDK() {
  window.__CLARA_PLUGINS__ = {
    register: registerPlugin,
    registerSlot,
  };

  window.__CLARA_PLUGIN_SDK__ = {
    ...
    // window.__CLARA_SESSION_TOKEN__ directly.
    authedFetch,
    ...
    // UI components — Clara DS where available, shadcn/ui primitives elsewhere.
    components: {
      Card,
      CardHeader,
      CardTitle,
