/**
 * Clara Dashboard Plugin SDK — typed contract (SPIKE)
 * ====================================================
 *
 * This is the public type surface for ``window.__CLARA_PLUGIN_SDK__`` and
 * ``window.__CLARA_PLUGINS__``, the globals the dashboard host exposes to
 ...
 * window.__CLARA_PLUGIN_SDK__`` because:
 *   1. The runtime object is assembled from many internal modules
 *      (``@/lib/api``, ``@claraship/ui``, …). Deriving the type would
 ...
 *
 * Versioning: bump ``ClaraPluginSDK["sdkVersion"]`` (and the
 ...
 *   - Ship as a published ``@clara/dashboard-plugin-sdk`` types package, or
 ...
 *     (pulls @claraship/ui types into the contract) or is the loose
 ...
 * of calling ``fetch`` with a hand-read ``window.__CLARA_SESSION_TOKEN__``.
 */
...
// ---------------------------------------------------------------------------
// Registry surface (window.__CLARA_PLUGINS__)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// SDK surface (window.__CLARA_PLUGIN_SDK__)
// ---------------------------------------------------------------------------

export interface ClaraPluginSDK {
  ...
  /**
   * Shared UI primitives (Clara DS / shadcn). Typed permissively at the
