...
import { api, CLARA_BASE_PATH } from "@/lib/api";
...
export const MANIFEST_CACHE_KEY = "clara:plugin-manifests";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${CLARA_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${CLARA_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?clara_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-clara-plugin", manifest.name);
