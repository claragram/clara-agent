import { buildClaraWebSocketUrl } from "@clara/shared";

// The dashboard can be served either at the root of its host (e.g.
// https://kanban.tilos.com/) or under a URL prefix when reverse-proxied
// (e.g. https://mission-control.tilos.com/clara/). The Python backend
// injects ``window.__CLARA_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__CLARA_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const CLARA_BASE_PATH = readBasePath();
const BASE = CLARA_BASE_PATH;
...
declare global {
  interface Window {
    __CLARA_SESSION_TOKEN__?: string;
    __CLARA_BASE_PATH__?: string;
    ...
    __CLARA_AUTH_REQUIRED__?: boolean;
  }
}
const SESSION_HEADER = "X-Clara-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__CLARA_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``clara update``, ``clara gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__CLARA_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__CLARA_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
  ...
  if (res.ok) {
    ...
    // current ``window.__CLARA_SESSION_TOKEN__`` is valid, so the next
 ...
 *
 * The dashboard's gated-mode WS auth (``clara_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__CLARA_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Clara-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``clara_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__CLARA_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export const api = {
  ...
  updateClara: () =>
    fetchJSON<ActionResponse>("/api/clara/update", { method: "POST" }),
  checkClaraUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/clara/update/check${force ? "?force=true" : ""}`,
    ),
 ...
 * Clara Portal contract V1 (the access token has no email/name claims —
...
export interface SkillHubSource {
  ...
  /** clara-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
  ...
  /** Phase 7: registered ``DashboardAuthProvider`` names (e.g. ``["clara"]``).
   ...
   * updates are handled by the outer launcher instead of ``clara update``. */
  can_update_clara?: boolean;
  config_path: string;
  ...
  /** NS-656: disk-usage rollup for the CLARA_HOME volume. Absent on
   * older gateways. */
 ...
 * sample of the CLARA_HOME volume — no staleness dimension, so no
 * sampled_at. */
