...
interface ClaraConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function ClaraConsoleModal({ open, onClose }: ClaraConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("clara> ");
  const inputPromptRef = useRef("clara> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "clara> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Clara Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="clara-console-title"
    >
      <div
        className={cn(
          themedBody,
