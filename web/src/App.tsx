...
import { SelectionSwitcher } from "@claraship/ui/ui/components/selection-switcher";
...
const SIDEBAR_COLLAPSED_KEY = "clara-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="clara-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="clara-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateClara = status?.can_update_clara === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'clara update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "clara update";
    return (
      t.status.updateClaraConfirmMessage ??
      `This will run 'clara update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateClaraConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateClara) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateClara,
      runningLabel: t.status.updatingClara,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the Clara gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateClaraConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateClaraConfirmTitle ?? `${t.status.updateClara}?`}
    />
    </>
  );
}
