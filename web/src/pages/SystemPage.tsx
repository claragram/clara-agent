...
export default function SystemPage() {
  ...
  // spawned `clara import` runs non-interactively (stdin is /dev/null), so
  ...
  const checkForUpdate = useCallback(
    async (force = false) => {
      if (status?.can_update_clara === false) return;
      setCheckingUpdate(true);
      try {
        const info = await api.checkClaraUpdate(force);
        setUpdateInfo(info);
        if (force) {
    ...
    [showToast, status?.can_update_clara],
  );
  ...
  const applyUpdate = async () => {
    ...
    if (status?.can_update_clara === false) {
      showToast(
        "Clara updates are managed outside this dashboard.",
        "success",
      );
      return;
    }
    try {
      const resp = await api.updateClara();
      if (!resp.ok) {
        showToast(
          resp.message ??
      ...
      setActiveAction(resp.name ?? "clara-update");
  ...
  const canUpdateClara = status?.can_update_clara !== false;
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <ConfirmDialog
        open={canUpdateClara && updateConfirmOpen}
        onCancel={() => setUpdateConfirmOpen(false)}
        onConfirm={() => void applyUpdate()}
        title="Update Clara?"
        description={
          updateInfo && updateInfo.behind && updateInfo.behind > 0
            ? `This will run 'clara update' (${updateInfo.update_command}) and pull ${updateInfo.behind} new commit${updateInfo.behind === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`
            : `This will run 'clara update' (${updateInfo?.update_command ?? "clara update"}) and restart the gateway when it finishes.`
        }
        ...
        <Card>
          <CardContent className="py-4">
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-y-3 gap-x-6 text-sm">
              ...
              <div>
                <div className="text-xs uppercase tracking-wider text-muted-foreground">Clara</div>
                <div className="flex items-center gap-2">
                  <span>v{stats?.clara_version}</span>
                  {canUpdateClara &&
                    updateInfo &&
      ...
      <section className="flex flex-col gap-3">
        <H2 variant="sm" className="flex items-center gap-2 text-muted-foreground">
          <Globe className="h-4 w-4" /> Clara Portal
        </H2>
        <Card>
          <CardContent className="flex flex-col gap-3 py-4">
            <div className="flex items-center gap-3">
              ...
              <a
                href={portal?.subscription_url || "https://portal.claraship.com/manage-subscription"}
                target="_blank"
                rel="noreferrer"
            ...
            {!portal?.logged_in && (
              <p className="text-xs text-muted-foreground">
                Log in with <span className="font-mono">clara portal</span>.
              </p>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="flex flex-col gap-4 py-4">
            ...
            <div className="flex flex-col gap-3 border-t border-border pt-4 sm:flex-row sm:items-end">
              <div className="grid min-w-0 flex-1 gap-2">
                ...
                <Input
                  ...
                  placeholder="$CLARA_HOME/backups/clara-backup.zip"
                />
              </div>
              <Button
                size="sm"
            ...
            <ConfirmDialog
              ...
              title="Restore full Clara backup?"
              description={`This will overwrite your current Clara configuration, skills, sessions, and data with the contents of ${backupImportLabel(importConfirmTarget)}. This cannot be undone.`}
              destructive
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-muted-foreground max-w-prose">
                    ...
                    returns links to send the Clara team. Pastes auto-delete
                    after 6 hours.
                  </span>
                </div>
              </div>
