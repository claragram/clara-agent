...
const BACKEND = process.env.CLARA_DASHBOARD_URL ?? "http://127.0.0.1:9119";

/**
 * In production the Python `clara dashboard` server injects a one-shot
 * session token into `index.html` (see `clara_cli/web_server.py`). The
 * Vite dev server serves its own `index.html`, so unless we forward that
 * token, every protected `/api/*` call 401s.
 *
 * This plugin fetches the running dashboard's `index.html` on each dev page
 * load, scrapes the `window.__CLARA_SESSION_TOKEN__` assignment, and
 * re-injects it into the dev HTML. No-op in production builds.
 */
function claraDevToken(): Plugin {
  const TOKEN_RE = /window\.__CLARA_SESSION_TOKEN__\s*=\s*"([^"]+)"/;
  const EMBEDDED_RE =
    /window\.__CLARA_DASHBOARD_EMBEDDED_CHAT__\s*=\s*(true|false)/;

  return {
    name: "clara:dev-session-token",
    apply: "serve",
    async transformIndexHtml() {
      try {
        ...
        if (!match) {
          console.warn(
            `[clara] Could not find session token in ${BACKEND} — ` +
              `is \`clara dashboard\` running? /api calls will 401.`,
          );
          return;
        }
        ...
        return [
          {
            ...
            injectTo: "head",
            children:
              `window.__CLARA_SESSION_TOKEN__="${match[1]}";` +
              `window.__CLARA_DASHBOARD_EMBEDDED_CHAT__=${embeddedJs};`,
          },
        ];
      } catch (err) {
        console.warn(
          `[clara] Dashboard at ${BACKEND} unreachable — ` +
            `start it with \`clara dashboard\` or set CLARA_DASHBOARD_URL. ` +
...
export default defineConfig({
  ...
  resolve: {
    ...
    // When @workprise/ui is symlinked via `file:../../design-language`,
      ...
      output: {
        codeSplitting: {
          ...
          groups: [
            ...
            {
              name: "ui",
              test: /node_modules[\\/]@(nous-research|workprise)[\\/]ui([\\/]|$)/,
            },
            {
              name: "vendor",
              test: /node_modules[\\/]/,
  ...
  server: {
    proxy: {
      ...
      // Same host as `clara dashboard` must serve these; Vite has no
