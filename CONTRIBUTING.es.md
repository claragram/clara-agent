# Contribuir a Clara Agent

¡Gracias por contribuir a Clara Agent! Esta guía cubre todo lo que necesitas: configurar tu entorno de desarrollo, entender la arquitectura, decidir qué construir y conseguir que tu PR sea aceptado.
...
2. **Compatibilidad entre plataformas** — macOS, diferentes distribuciones de Linux y WSL2 en Windows. Queremos que Clara funcione en todas partes.
...
Las habilidades incluidas (en `skills/`) se envían con cada instalación de Clara. Deben ser **ampliamente útiles para la mayoría de los usuarios**:
...
Si tu habilidad es especializada, contribuida por la comunidad o de nicho, es mejor para un **Skills Hub** — súbela a un registro de habilidades y compártela en el [Discord de Workprise](https://discord.gg/Workprise). Los usuarios pueden instalarla con `clara skills install`.

---

## Proveedores de Memoria: Publicar como Plugin Independiente

**Ya no aceptamos nuevos proveedores de memoria en este repositorio.** El conjunto de proveedores integrados en `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) está cerrado. Si quieres añadir un nuevo backend de memoria, publícalo como un **repositorio de plugin independiente** que los usuarios instalen en `~/.clara/plugins/` (o a través de un entry point de pip).

Los plugins de memoria independientes:

- Implementan el mismo ABC `MemoryProvider` (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown` y opcionalmente `post_setup(clara_home, config)` para integración con el asistente de configuración
...
- Se integran con `clara memory setup` a través de `post_setup()` — sin necesidad de tocar el código base
...
├── cli.py                    # Clase ClaraCLI — TUI interactiva, integración prompt_toolkit
├── model_tools.py            # Orquestación de herramientas (capa delgada sobre tools/registry.py)
├── toolsets.py               # Agrupaciones y presets de herramientas (clara-cli, clara-telegram, etc.)
├── clara_state.py           # Base de datos de sesiones SQLite con búsqueda de texto completo FTS5, títulos de sesión
...
│
├── clara_cli/               # Implementaciones de comandos CLI
...
│   ├── auth.py                   # Resolución de proveedor, OAuth, Clara Portal
...
├── website/                  # Sitio de documentación (agent.claraprise.com)
│
...
### Configuración del usuario (almacenada en `~/.clara/`)

| Ruta | Propósito |
|------|-----------|
| `~/.clara/config.yaml` | Configuración (modelo, terminal, toolsets, compresión, etc.) |
| `~/.clara/.env` | Claves API y secretos |
| `~/.clara/auth.json` | Credenciales OAuth (Clara Portal) |
| `~/.clara/skills/` | Todas las habilidades activas (incluidas + instaladas desde hub + creadas por el agente) |
| `~/.clara/memories/` | Memoria persistente (MEMORY.md, USER.md) |
| `~/.clara/state.db` | Base de datos de sesiones SQLite |
| `~/.clara/sessions/` | Índice de enrutamiento del gateway (`sessions.json`), migas de pan de solicitudes, transcripciones `*.jsonl` del gateway y (opcionalmente) snapshots JSON por sesión cuando `sessions.write_json_snapshots: true` está configurado. Los snapshots por sesión están desactivados por defecto; state.db es canónica. |
| `~/.clara/cron/` | Datos de trabajos programados |
| `~/.clara/whatsapp/session/` | Credenciales del puente WhatsApp |
...
- **Persistencia de sesión**: Todas las conversaciones se almacenan en SQLite (`clara_state.py`) con búsqueda de texto completo y títulos de sesión únicos.
...
(por ejemplo `_CLARA_CORE_TOOLS` o un toolset dedicado); de lo contrario la herramienta
...
2. **Las herramientas referenciadas en el cuerpo de SKILL.md deben ser herramientas nativas de Clara o servidores MCP que la habilidad espere explícitamente.** Usa los nombres de herramientas en comillas invertidas: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, etc.
...
Clara usa un sistema de skins basado en datos — no se necesitan cambios de código para añadir una nueva skin.

**Opción A: Skin de usuario (archivo YAML)**

Crea `~/.clara/skins/<nombre>.yaml`:
...
Añade al dict `_BUILTIN_SKINS` en `clara_cli/skin_engine.py`. Usa el mismo esquema que arriba pero como dict de Python.
...
Clara se ejecuta en Linux, macOS y Windows nativo (además de WSL2). Al escribir código
...
Clara tiene acceso al terminal. La seguridad importa.
...
2. **Probar manualmente**: Ejecuta `clara` y ejercita la ruta de código que cambiaste
...
- Usa [GitHub Issues](https://github.com/claraprise/clara-agent/issues)
...
- **Discord**: [discord.gg/Workprise](https://discord.gg/Workprise) — para preguntas, mostrar proyectos y compartir habilidades
