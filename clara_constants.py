"""Shared constants for Clara Agent.
...
_UNSET = object()
_CLARA_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_CLARA_HOME_OVERRIDE", default=_UNSET
)
...
def set_clara_home_override(path: str | Path | None) -> Token:
    """Set a context-local Clara home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _CLARA_HOME_OVERRIDE.set(value)


def reset_clara_home_override(token: Token) -> None:
    """Restore the previous context-local Clara home override."""
    _CLARA_HOME_OVERRIDE.reset(token)


def get_clara_home_override() -> str | None:
    """Return the active context-local Clara home override, if any."""
    override = _CLARA_HOME_OVERRIDE.get()
    if override is _UNSET or not override:
        return None
    return str(override)


def _get_platform_default_clara_home() -> Path:
    """Return the platform-native default Clara home path."""
    if sys.platform == "win32":
        local_appdata = os.environ.get("LOCALAPPDATA", "").strip()
        base = Path(local_appdata) if local_appdata else Path.home() / "AppData" / "Local"
        return base / "clara"
    return Path.home() / ".clara"


def _clara_home_from_env() -> Path:
    """Resolve CLARA_HOME from the process environment only.

    Reads the ``CLARA_HOME`` env var, falling back to the platform-native
    default.  Deliberately ignores the context-local override installed by
    :func:`set_clara_home_override`, so this reflects the process/launch
    scope rather than a per-task profile.  Shared by :func:`get_clara_home`
    and :func:`get_process_clara_home` so the two never drift.
    """
    val = os.environ.get("CLARA_HOME", "").strip()
    if val:
        return Path(val)
    return _get_platform_default_clara_home()


def _warn_profile_fallback_once() -> None:
    """Warn once when falling back to the default home while a profile is active.

    Guard: if a non-default profile is sticky-active but ``CLARA_HOME`` is
    ...
    if _profile_fallback_warned:
        return
    try:
        fallback_home = _get_platform_default_clara_home()
    ...
    if active and active != "default":
        ...
        msg = (
            f"[CLARA_HOME fallback] CLARA_HOME is unset but active "
            f"profile is {active!r}. Falling back to {fallback_home}, which "
            f"is the DEFAULT profile — not {active!r}. Any data this "
            f"process writes will land in the wrong profile. The "
            f"subprocess spawner should pass CLARA_HOME explicitly "
            f"(see issue #18594)."
        )
        try:
...
def get_clara_home() -> Path:
    """Return the Clara home directory (default: platform-native path).

    Resolution order: context-local override (see
    :func:`set_clara_home_override`) → ``CLARA_HOME`` env var → the
    platform-native default.  This is the single source of truth — all other
    copies should import this.

    When ``CLARA_HOME`` is unset but an ``active_profile`` file indicates
    ...
    ``clara_cli/kanban_db.py``).  See https://github.com/claraship/clara-agent/issues/18594.
    """
    override = get_clara_home_override()
    if override:
        return Path(override)

    if not os.environ.get("CLARA_HOME", "").strip():
        _warn_profile_fallback_once()

    return _clara_home_from_env()


def clara_home_key(path: str | Path | None = None) -> str:
    """Return a stable key for a Clara home/profile directory.
    ...
    """
    candidate = Path(path) if path is not None else get_clara_home()
    resolved = candidate.expanduser().resolve(strict=False)
    return os.path.normcase(str(resolved))


def get_process_clara_home() -> Path:
    """Return the Clara home for the running process, ignoring task overrides.

    Unlike :func:`get_clara_home`, this never follows the context-local
    override set by :func:`set_clara_home_override`.  It resolves only the
    process ``CLARA_HOME`` env var (falling back to the platform default),
...
# Process-level memo for get_default_clara_root(). The function resolves
# CLARA_HOME against the native home on every call (~80us of path
...
# Its result depends only on (CLARA_HOME, platform native home), which are
# compared for free on each call, so the memo is freshness-correct even if a
# test or plugin mutates CLARA_HOME mid-process.
_default_clara_root_memo: "tuple[str, str, Path] | None" = None


def get_default_clara_root() -> Path:
    """Return the root Clara directory for profile-level operations.

    In standard deployments this is the platform-native Clara home
    (``~/.clara`` on POSIX, ``%LOCALAPPDATA%\\clara`` on native Windows).

    In Docker or custom deployments where ``CLARA_HOME`` points outside
    ``~/.clara`` (e.g. ``/opt/data``), returns ``CLARA_HOME`` directly
    — that IS the root.

    In profile mode where ``CLARA_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.clara/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    global _default_clara_root_memo
    native_home = _get_platform_default_clara_home()
    env_home = os.environ.get("CLARA_HOME", "")
    if _default_clara_root_memo is not None:
        memo_native, memo_env, memo_result = _default_clara_root_memo
    ...
    if not env_home:
        ...
        try:
            ...
            # CLARA_HOME is under ~/.clara (normal or profile mode)
            result = native_home
        except ValueError:
            ...
            if env_path.parent.name == "profiles":
                ...
                # Not a profile path — CLARA_HOME itself is the root
                result = env_path
    _default_clara_root_memo = (str(native_home), env_home, result)
    return result
...
# Files whose presence marks a directory as a real Clara home. A fresh home
...
_CLARA_HOME_MARKERS = ("config.yaml", ".env", "state.db")


def _is_clara_profiles_root(profiles_dir: Path) -> bool:
    """Return True when *profiles_dir* is a canonical ``<clara-home>/profiles``.

    Anchors named-profile recognition so it only fires for directories that
    provably live under a Clara home: the classic ``~/.clara`` layout, a
    root carrying Clara-home marker files (Docker/custom ``CLARA_HOME``
    like ``/opt/data``), a ``profiles/.deleted`` tombstone directory (only
    ever created by ``clara profile delete``), or the process's resolved
    default Clara root.
    """
    ...
    try:
        if (profiles_dir / _DELETED_PROFILES_DIR).is_dir():
            return True
        if any((root / marker).exists() for marker in _CLARA_HOME_MARKERS):
            return True
    except OSError:
        pass
    try:
        return root.resolve(strict=False) == get_default_clara_root().resolve(
            strict=False
        )
    except OSError:
        return False
...
def named_profile_home(path: str | Path) -> Path | None:
    ...
    Clara home (see :func:`_is_clara_profiles_root`). A default Clara home
    whose path merely contains a ``profiles`` segment
    (e.g. ``/tmp/foo/profiles/notahome/.clara``) is not a named profile,
    ...
    for candidate in (current, *current.parents):
        if (
            ...
            and _is_clara_profiles_root(candidate.parent)
        ):
            return candidate
        # Stop at a default Clara home so a coincidental ``profiles/``
        # ancestor is not treated as a named-profile root.
        if candidate.name == ".clara":
            return None
    return None
...
def assert_named_profile_home_live(path: str | Path) -> None:
    ...
    Default ``CLARA_HOME`` (not under ``profiles/``) is unchanged.
    """
...
def mkdir_under_clara_home(path: str | Path) -> Path:
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``CLARA_OPTIONAL_SKILLS``.
    """
    override = os.getenv("CLARA_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_clara_home() / "optional-skills"


def get_optional_mcps_dir(default: Path | None = None) -> Path:
    """Return the optional-mcps directory, honoring package-manager wrappers.

    Mirrors :func:`get_optional_skills_dir` for the MCP catalog (Clara-approved
    Model Context Protocol servers shipped with the repo but disabled by
    default). Packaged installs may ship ``optional-mcps`` outside the Python
    package tree and expose it via ``CLARA_OPTIONAL_MCPS``.
    """
    override = os.getenv("CLARA_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_clara_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``CLARA_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Caller-supplied ``default`` (typically the source-checkout path)
        3. ``<CLARA_HOME>/skills`` last-resort
    """
    override = os.getenv("CLARA_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
...
def get_clara_dir(
    ...
    *,
    ...
    """Resolve a Clara subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to CLARA_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to CLARA_HOME (e.g. ``"image_cache"``).
        home: Optional explicit Clara home. Profile-aware callers that manage
            more than one home in the same process use this instead of
            temporarily mutating the process or context-local CLARA_HOME.

    Returns:
        Absolute ``Path`` — legacy location if it exists with content,
        otherwise the new location.
    """
    home = home or get_clara_home()
...
def iter_clara_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Clara-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\clara\\node``. POSIX installs use
    ``$CLARA_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_clara_home()
    dirs = [root / "node"]
    bin_dir = root / "node" / "bin"
    # NOTE: keep this ordering in sync with claraManagedNodePathEntries() in
...
_CLARA_NODE_TARGET_MAJOR = int(os.environ.get("CLARA_NODE_TARGET_MAJOR", "22"))
_managed_node_heal_attempted = False
_NODE_BOOTSTRAP_SCRIPT = Path(__file__).resolve().parent / "scripts" / "lib" / "node-bootstrap.sh"

# Install tree root (this file lives at <install_root>/clara_constants.py).
# Used by secure_parent_dir() to skip chmod on the install dir — chmodding it
# 0700 breaks clara-user traversal in Docker (UID 10000). See #25821, #93050.
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Clara-managed Node trees live under ``$CLARA_HOME/node`` (or a profile's
    ``CLARA_HOME``). A partial upgrade or interrupted install can leave
    ``bin/npm`` behind while ``lib/cli.js`` is missing — the wrapper exists but
    immediately throws ``MODULE_NOT_FOUND``. ``find_clara_node_executable``
    used to trust file presence alone, so ``clara update`` would pick that
...
def clara_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Clara-managed node/npm/npx shim exists on disk."""
    names = set()
    for command in ("node", "npm", "npx"):
        names.update(_candidate_node_command_names(command))
    for directory in iter_clara_node_dirs(home):
        for name in names:
...
def _print_managed_node_in_use_notice() -> None:
    ...
    print(
        "→ Clara-managed Node.js is in use by a running app; deferring its "
        "upgrade until the app is closed (re-run `clara update` afterwards).",
        flush=True,
    )


def _heal_managed_node_windows(home: Path | None = None) -> bool | None:
    """Redownload the portable Node zip into ``%CLARA_HOME%\\node`` on Windows.
    ...
    ``FILE_SHARE_DELETE`` — the same mechanism as the clara.exe quarantine);
    ...
    index_url = f"https://nodejs.org/dist/latest-v{_CLARA_NODE_TARGET_MAJOR}.x/"
    try:
    ...
    match = re.search(
        rf"node-v{_CLARA_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def _bootstrap_managed_node_posix() -> bool:
    """Install a fresh managed Node under ``$CLARA_HOME/node`` on POSIX.
    ...
    ``CLARA_NODE_SKIP_LINKS=1`` so the user's own node/npm on PATH is not
...
def bootstrap_clara_managed_node() -> str | None:
    """Install a Clara-managed Node tree and return its npm path.
    ...
    Clara never modifies a toolchain it does not own, so instead it provisions
    its own tree under ``$CLARA_HOME/node`` (the same tree a fresh install
    ...
    present.
    """
    existing = find_clara_node_executable("npm")
    if existing:
        return existing
    ...
    for directory in iter_clara_node_dirs():
...
def heal_clara_managed_node() -> bool:
    """Redownload Clara-managed Node when the tree exists but is broken.
    ...
    if _managed_node_heal_attempted:
        return False
    if not clara_managed_node_tree_present():
        return False
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "CLARA_HOME": str(get_clara_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def _managed_node_tree_outdated(home: Path | None = None) -> bool:
    ...
    same way a broken one does: :func:`find_clara_node_executable` triggers
    the once-per-process heal, which redownloads
    ``latest-v{_CLARA_NODE_TARGET_MAJOR}.x`` — so existing users are upgraded
    ...
    for directory in iter_clara_node_dirs(home):
        for name in _candidate_node_command_names("node"):
            ...
            if "-" in version:
                return True
            return major < _CLARA_NODE_TARGET_MAJOR
    return False


def find_clara_node_executable(command: str) -> str | None:
    """Return a Clara-managed Node/npm executable path, healing broken trees.

    Outdated trees (node major below ``_CLARA_NODE_TARGET_MAJOR``) heal the
    ...
    if needs_heal and heal_clara_managed_node():
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Clara-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy Clara-managed installs.

    This is for Clara-owned subprocesses that should not be broken by a bad,
    ...
    """
    managed = find_clara_node_executable(command)
    if managed:
        return managed
    if clara_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_clara_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Clara-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    existing = merged.get("PATH", "")
    parts = [p for p in existing.split(os.pathsep) if p]
    managed = [str(path) for path in iter_clara_node_dirs() if path.is_dir()]
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``clara update`` — leaving a **dangling symlink** that ``which`` still
...
def display_clara_home() -> str:
    """Return a user-friendly display string for the current CLARA_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.clara``
        profile:  ``~/.clara/profiles/coder``
        custom:   ``/opt/clara-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.clara``.  For code that needs a real ``Path``, use
    :func:`get_clara_home` instead.
    """
    home = get_clara_home()
    try:
        # as_posix(): on Windows, str() of a relative Path renders
        # backslashes, producing mixed-separator chimeras like
        # ``~/AppData\Local\clara/skills/`` once callers append
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``CLARA_HOME`` or other path
    env vars resolve to an unexpected location.

    Also refuses to chmod the clara-agent install tree (the directory this
    ...
    See https://github.com/claraship/clara-agent/issues/25821 and
    https://github.com/claraship/clara-agent/pull/93050.
    """
    parent = path.parent.resolve()
    # Refuse root and its direct children (/usr, /home, /var, /tmp, …).
    if parent == Path("/") or len(parent.parts) < 3:
        return
    # Refuse the install tree root. chmodding it 0700 breaks clara-user
    ...
    if parent == _INSTALL_ROOT or _INSTALL_ROOT in parent.parents:
        # A credential file inside the install tree usually means CLARA_HOME
        ...
        logging.getLogger(__name__).warning(
            ...
            "clara-agent install directory (%s). Credential files are "
            "normally stored under the clara home directory instead.",
            parent,
            _INSTALL_ROOT,
        )
        return
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{CLARA_HOME}/home`` when the profile-home directory exists."""
    clara_home = get_clara_home_override() or (env or {}).get("CLARA_HOME") or os.getenv("CLARA_HOME")
    if not clara_home:
        return None
    profile_home = os.path.join(clara_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("CLARA_REAL_HOME") or os.getenv("CLARA_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Clara profile HOME.

    ``CLARA_HOME`` scopes Clara state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={CLARA_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{CLARA_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{CLARA_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Clara' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["CLARA_REAL_HOME"] = real_home
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Normalize a cross-boundary cwd when Clara itself runs inside WSL.
...
def is_container() -> bool:
    ...
    See: claraship/clara-agent#47111
    """
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under CLARA_HOME.

    Replaces the ``get_clara_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, clara_logging.py, clara_time.py, etc.).
    """
    return get_clara_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under CLARA_HOME."""
    return get_clara_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under CLARA_HOME."""
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_clara_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._clara_ipv4_patched = True  # type: ignore[attr-defined]
...
def venv_bin_dir(venv_dir, *, windows: bool | None = None) -> Path:
    ...
    across four ``clara_cli`` modules using three different Windows
    ...
    A few sites outside ``clara_cli`` (``tools/code_execution_tool.py``,
    ...
    *windows* lets a caller pass its own platform verdict. Several callers
    ...
    Windows paths on Linux CI (``clara_cli.main._is_windows`` and friends);
...
# Top-level packages/modules that ship as part of Clara itself. An ImportError
...
# `clara_cli.update_cmd`'s post-update probe consumes this same set so the
...
def is_first_party_module(name: str | None) -> bool:
    """True when *name* is a module that ships with Clara.
    ...
    if not root:
        return False
    return root in FIRST_PARTY_MODULE_ROOTS or root.startswith("clara_")
...
def partial_update_hint(exc: BaseException) -> list[str]:
    ...
    rather than their config, is the problem — and `clara update` is exactly
    ...
    return [
        ...
        "    clara update",
        "If that also fails, reinstall: https://agent.claraship.com",
    ]
