...
def _profile_user_agent() -> str:
    """Return a ``clara-cli/<version>`` UA string, with a stable fallback.
    ...
    """
    try:
        from clara_cli import __version__ as _ver  # lazy: avoid layer cycle at import time
        return f"clara-cli/{_ver}"
    except Exception:
        return "clara-cli"
...
class ProviderProfile:
    ...
    # describe how to launch it; clara_cli/auth.py's
    ...
    def create_client(self, **client_kwargs: Any) -> Any | None:
        ...
        a profile registered from ``~/.clara/plugins/model-providers/`` or a
    ...
    def fetch_models(
        ...
        *,
        ...
        # clara-cli UA so the catalog endpoint is reachable.
