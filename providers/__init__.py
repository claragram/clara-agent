...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with clara-agent)
2. User plugins: ``$CLARA_HOME/plugins/model-providers/<name>/``
3. Pip-installed plugins: distributions exposing a ``clara_agent.plugins``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$CLARA_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$CLARA_HOME/plugins/model-providers/`` if it exists."""
    try:
        from clara_constants import get_clara_home

        d = get_clara_home() / "plugins" / "model-providers"
...
def _installed_plugins_dir() -> Path | None:
    """Return ``$CLARA_HOME/plugins/`` if it exists.

    This is where ``clara plugins install`` clones a plugin — flat, one
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple CLARA_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_clara_user_provider_{safe_name}"
...
def _discover_entry_point_providers() -> None:
    """Import pip-installed provider plugins via the ``clara_agent.plugins``
    ...
    * **Provider targets only.** The ``clara_agent.plugins`` group is shared
    ...
    first, so filesystem plugins (bundled + ``$CLARA_HOME``) keep their
    ...
    try:
        from clara_cli.plugins import _get_disabled_plugins, _get_enabled_plugins
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$CLARA_HOME/plugins/model-providers/<name>/``
      2b. Plugins installed by ``clara plugins install`` at
          ``$CLARA_HOME/plugins/<name>/`` that declare ``kind: model-provider``
    ...
    # 0. Pip-installed plugins — entry points in the ``clara_agent.plugins``
    ...
    #    filesystem steps means a bundled or ``$CLARA_HOME`` profile of the
    ...
    # 1. Bundled plugins — shipped with clara-agent.
    ...
    # 2. User plugins — under $CLARA_HOME/plugins/model-providers/<name>/.
    ...
    # 2b. Plugins installed by ``clara plugins install`` / the plugin index.
    #     Those clone into $CLARA_HOME/plugins/<name>/ — flat, NOT under
