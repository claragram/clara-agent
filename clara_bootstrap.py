"""Windows UTF-8 bootstrap for Clara entry points.
...
should be imported at the very top of every Clara entry point
(``clara``, ``clara-agent``, ``clara-acp``, ``python -m gateway.run``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``clara_cli._subprocess_compat.suppress_platform_ver_console``
    ...
    ``clara_bootstrap``, never ``clara_cli.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Clara modules.

    Clara ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Clara source root
    ...
    """
    root = src_root or os.environ.get("CLARA_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``CLARA_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("CLARA_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import clara_bootstrap``
# (or ``from clara_bootstrap import apply_windows_utf8_bootstrap``) at
