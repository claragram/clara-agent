# nix/nixosModules.nix — the NixOS module for clara-agent
#
...
#
# Container mode: clara runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $CLARA_HOME/.env
# and read by clara at startup — no container recreation needed for env changes.
#
# Tool resolution: the clara wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }:
{
  flake.nixosModules.default =
    ...
    let
      cfg = config.services.clara-agent;
      common = import ./moduleCommon.nix { inherit lib; };

      effectivePackage = common.effectivePackage cfg;
      clara-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

      claraHome = "${cfg.stateDir}/.clara";
      ...
      # CLARA_HOME via addToSystemPackages, so they can save settings through the
      ...
      # clara user + sudo on first boot (writable layer persists), then
      # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
      containerEntrypoint = pkgs.writeShellScript "clara-container-entrypoint" ''
        set -eu

        CLARA_UID="''${CLARA_UID:?CLARA_UID must be set}"
        CLARA_GID="''${CLARA_GID:?CLARA_GID must be set}"

        # ── Group: ensure a group with GID=$CLARA_GID exists ──
        # Check by GID (not name) to avoid collisions with pre-existing groups
        # (e.g. GID 100 = "users" on Ubuntu)
        EXISTING_GROUP=$(getent group "$CLARA_GID" 2>/dev/null | cut -d: -f1 || true)
        if [ -n "$EXISTING_GROUP" ]; then
          GROUP_NAME="$EXISTING_GROUP"
        else
          GROUP_NAME="clara"
          if command -v groupadd >/dev/null 2>&1; then
            groupadd -g "$CLARA_GID" "$GROUP_NAME"
          elif command -v addgroup >/dev/null 2>&1; then
            addgroup -g "$CLARA_GID" "$GROUP_NAME" 2>/dev/null || true
          fi
        fi

        # ── User: ensure a user with UID=$CLARA_UID exists ──
        PASSWD_ENTRY=$(getent passwd "$CLARA_UID" 2>/dev/null || true)
        if [ -n "$PASSWD_ENTRY" ]; then
          TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
          TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
        else
          TARGET_USER="clara"
          TARGET_HOME="/home/clara"
          if command -v useradd >/dev/null 2>&1; then
            useradd -u "$CLARA_UID" -g "$CLARA_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
          elif command -v adduser >/dev/null 2>&1; then
            adduser -u "$CLARA_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
          fi
        fi
        mkdir -p "$TARGET_HOME"
        chown "$CLARA_UID:$CLARA_GID" "$TARGET_HOME"
        chmod 0750 "$TARGET_HOME"

        # Ensure CLARA_HOME is owned by the target user.
        ...
        if [ -n "''${CLARA_HOME:-}" ] && [ -d "$CLARA_HOME" ]; then
          find "$CLARA_HOME" \! -user "$CLARA_UID" -exec chown "$CLARA_UID:$CLARA_GID" {} +
        fi
        ...
        if [ ! -f /var/lib/clara-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
          ...
          touch /var/lib/clara-tools-provisioned
        fi

        if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/clara ]; then
          mkdir -p /etc/sudoers.d
          echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/clara
          chmod 0440 /etc/sudoers.d/clara
        fi
        ...
        if command -v setpriv >/dev/null 2>&1; then
          exec setpriv --reuid="$CLARA_UID" --regid="$CLARA_GID" --init-groups "$@"
      ...
      # so they can update without recreation. Env vars go through $CLARA_HOME/.env.
      ...
      containerModeFile = pkgs.writeText "clara-container-mode" ''
        # Written by the NixOS activation script. Do not edit manually.
        backend=${cfg.container.backend}
        container_name=${containerName}
        exec_user=${cfg.user}
        clara_bin=${containerDataDir}/current-package/bin/clara
      '';

      # Default: /var/lib/clara/workspace → /data/workspace.
      ...
      commonServiceConfig = {
        ...
        # so interactive users in the clara group can read/write them.
        UMask = "0007";
      ...
      // common.processEnvironment { inherit claraHome; };

      unitPath = common.processPath { inherit pkgs cfg; };

    in
    {
      options.services.clara-agent =
        common.sharedOptions {
          defaultPackage = clara-agent;
          defaultPackageText = lib.literalExpression "clara-agent.packages.\${system}.default";
        ...
        // (
          with lib;
          {
            ...
            stateDir = mkOption {
              type = types.str;
              default = "/var/lib/clara";
              description = "State directory. Contains .clara/ subdir (CLARA_HOME).";
            };
            ...
            addToSystemPackages = mkOption {
              ...
              default = false;
              description = ''
                Add the clara CLI to environment.systemPackages and export
                CLARA_HOME system-wide (via environment.variables) so interactive
            ...
            container = {
              ...
              hostUsers = mkOption {
                ...
                default = [ ];
                description = ''
                  Interactive users who get a ~/.clara symlink to the service
                  stateDir. These users are automatically added to the clara group.
                '';
                example = [ "sidbin" ];
              };
            };
      ...
      config = lib.mkIf cfg.enable (
        lib.mkMerge [
          ...
          (lib.mkIf (cfg.mcpServers != { }) {
            services.clara-agent.settings.mcp_servers = common.mcpServersToConfig cfg.mcpServers;
          })
          ...
          # ── Host CLI ──────────────────────────────────────────────────────
          # Add the clara CLI to system PATH and export CLARA_HOME system-wide
          # so interactive shells share state (sessions, skills, cron) with the
          # gateway service instead of creating a separate ~/.clara/.
          (lib.mkIf cfg.addToSystemPackages {
            environment.systemPackages = [ effectivePackage ];
            environment.variables.CLARA_HOME = claraHome;
          })
          ...
          # ── Assertions ─────────────────────────────────────────────────────
          {
            assertions =
              common.pluginNameAssertions {
                inherit cfg;
                optionPath = "services.clara-agent";
              }
              ++ common.workspaceFilesAssertions {
                inherit cfg;
                opt = options.services.clara-agent.workingDirectory;
                optionPath = "services.clara-agent";
              }
              ...
              ++ [
                {
                  ...
                  message = "services.clara-agent: backend.mode is not supported together with container.enable — the container runs the gateway only.";
                }
              ];
          }

          # ── Per-user profile for extraPackages ───────────────────────────
          # Wire extraPackages into the clara user's per-user profile so the
          ...
          (lib.mkIf
            (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != [ ])
            {
              warnings = [
                ''
                  services.clara-agent: container.enable is true and container.hostUsers
                  is set, but addToSystemPackages is false. Without a host-installed clara
                  binary, container routing will not work for interactive users.
                  Set addToSystemPackages = true or ensure clara is on PATH.
                ''
              ];
            }
          )
          ...
          # ── Directories ───────────────────────────────────────────────────
          {
            systemd.tmpfiles.rules = [
              ...
              "d ${claraHome}                  2770 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.stateDir}/home           0750 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.workingDirectory}        2770 ${cfg.user} ${cfg.group} - -"
            ]
            ++ map (d: "d ${claraHome}/${d} 2770 ${cfg.user} ${cfg.group} - -") common.stateSubdirs;
          }

          # ── Activation: link config + auth + documents ────────────────────
          {
            system.activationScripts."clara-agent-setup" =
              lib.stringAfter
                (
                ...
                ''
                  ...
                  chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${claraHome} ${cfg.stateDir}/home ${cfg.workingDirectory}
                  chmod 2770 ${cfg.stateDir} ${claraHome} ${cfg.workingDirectory}
                  ...
                  find ${claraHome} -maxdepth 1 \
                    \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
                    -exec chmod g+rw {} + 2>/dev/null || true
                  for _subdir in ${lib.concatStringsSep " " common.stateSubdirs}; do
                    mkdir -p "${claraHome}/$_subdir"
                    chown ${cfg.user}:${cfg.group} "${claraHome}/$_subdir"
                    chmod 2770 "${claraHome}/$_subdir"
                    find "${claraHome}/$_subdir" -type f \
                      -exec chmod g+rw {} + 2>/dev/null || true
                  done

                  ${common.mkStateScript {
                    inherit pkgs cfg claraHome;
                    workingDirectory = cfg.workingDirectory;
                  ...
                  chown -h ${cfg.user}:${cfg.group} ${claraHome}/plugins/nix-managed-* 2>/dev/null || true
                  ...
                  ${
                    if cfg.container.enable then
                      ''
                        install -o ${cfg.user} -g ${cfg.group} -m 0644 ${containerModeFile} ${claraHome}/.container-mode
                      ''
                    else
                      ''
                        rm -f ${claraHome}/.container-mode
                        ...
                        ${lib.concatStringsSep "\n" (
                          map (
                            ...
                            let
                              ...
                              symlinkPath = "${userHome}/.clara";
                            in
                            ''
                              if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${claraHome}" ]; then
                                rm -f "${symlinkPath}"
                                echo "clara-agent: removed symlink ${symlinkPath}"
                              fi
                            ''
                  ...
                  # Create ~/.clara -> stateDir/.clara for each hostUser so the
          ...
          (lib.mkIf (!cfg.container.enable) {
            systemd.services.clara-agent = {
              description = "Clara Agent Gateway";
              wantedBy = [ "multi-user.target" ];
              after = [ "network-online.target" ];
              wants = [ "network-online.target" ];

              # cfg.environment and cfg.environmentFiles are written to
              # $CLARA_HOME/.env by the activation script. load_clara_dotenv()
          ...
          (lib.mkIf (!cfg.container.enable && cfg.backend.mode != "none") {
            systemd.services.clara-backend = {
          ...
          (lib.mkIf cfg.container.enable {
            ...
            systemd.services.clara-agent = {
              description = "Clara Agent Gateway (container)";
              wantedBy = [ "multi-user.target" ];
              after = [
              ...
              preStart = ''
                ...
                if [ "$NEED_CREATE" = "true" ]; then
                  ...
                  CLARA_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
                  CLARA_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
                  ...
                  ${containerBin} create \
                    ...
                    --env CLARA_UID="$CLARA_UID" \
                    --env CLARA_GID="$CLARA_GID" \
                    --env CLARA_HOME=${containerDataDir}/.clara \
                    --env CLARA_MANAGED=true \
                    --env HOME=${containerHomeDir} \
                    ${lib.concatStringsSep " " cfg.container.extraOptions} \
                    ${cfg.container.image} \
                    ${containerDataDir}/current-package/bin/clara gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
