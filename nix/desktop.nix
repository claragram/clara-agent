# nix/desktop.nix — Clara Desktop (Electron) app build + wrapper
#
# `claraAgent` is the fully-built `.#default` package — it ships the
# `clara` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `CLARA_DESKTOP_CLARA` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Clara CLI").
# No reimplementation of the agent resolution in this wrapper.
{
  ...
  # CLARA_HOME and CLARA_MANAGED here, which gives the app the same state
...
let
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "clara-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "clara-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = claraNpmLib.buildNpmPackage {
    dirs = [
      "apps/desktop"
      "apps/shared"
    ];
    pname = "clara-desktop-renderer";
    ...
    buildPhase = ''
      ...
      pushd apps/desktop
        ...
        ${lib.getExe claraNpmLib.node-gyp} rebuild \
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/clara-desktop $out/bin
    cp -r ${renderer}/* $out/share/clara-desktop/
    ...
    substituteInPlace $out/share/clara-desktop/dist/electron-main.mjs \
      --replace-fail "process.resourcesPath" "'$out/share/clara-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # CLARA_DESKTOP_CLARA to the absolute path of the nix-built `clara`
    # binary so the desktop's resolver step 4 ("existing Clara CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/clara-desktop \
      --add-flags "$out/share/clara-desktop" \
      --set CLARA_DESKTOP_CLARA "${lib.getExe claraAgent}" \
    ...
    install -m 0644 ${../apps/desktop/assets/icon.png} \
      $out/share/icons/hicolor/1024x1024/apps/clara.png
    export PYTHONPATH=$(mktemp -d)
    cp ${../clara_cli/linux_desktop_entry.py} "$PYTHONPATH/linux_desktop_entry.py"
    export DESKTOP_EXEC="$out/bin/clara-desktop"
    export DESKTOP_ICON="$out/share/icons/hicolor/1024x1024/apps/clara.png"
    python3 -c 'import os; from linux_desktop_entry import render_desktop_entry; print(render_desktop_entry(os.environ["DESKTOP_EXEC"], os.environ["DESKTOP_ICON"]))' > $out/share/applications/clara.desktop
    runHook postInstall
  '';
