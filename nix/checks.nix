...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      clara-agent = self'.packages.default;
      claraVenv = clara-agent.claraVenv;
      ...
      evalHomeModule =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              home = {
                username = "clara-check";
                homeDirectory = "/home/clara-check";
                stateVersion = "24.11";
              };
            }
            { services.clara-agent = settings; }
      ...
      evalHomeSplit =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              programs.clara-agent = programs;
              services.clara-agent = services;
            }
          ];
        };

      # The option names that each module defines under
      # services.clara-agent. The internal names that the module system adds
      # are not in the list.
      moduleOptionNames =
        eval: lib.attrNames (lib.filterAttrs (n: _: !lib.hasPrefix "_" n) eval.options.services.clara-agent);
      ...
      configKeys = pkgs.runCommand "clara-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${claraVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "clara-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "clara-build-package" { } ''
          echo "PASS: package built at ${clara-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "clara-build-devshell" { } ''
          ...
          let
            enabled = evalHomeSplit {
              ...
              services = {
                ...
                environment.CLARA_TEST = "1";
                environmentFiles = [ "/run/secrets/clara-env" ];
                claraHomeFiles."SOUL.md" = "test soul";
            ...
            processes =
              if pkgs.stdenv.hostPlatform.isDarwin then
                ...
                }) (lib.filterAttrs (n: _: lib.hasPrefix "clara" n) cfg.launchd.agents)
              else
                lib.mapAttrs (_: unit: {
                  argv = [ unit.Service.ExecStart ];
                  env = unit.Service.Environment;
                }) (lib.filterAttrs (n: _: lib.hasPrefix "clara" n) cfg.systemd.user.services);
            ...
            activation = cfg.home.activation.claraAgentSetup.data;

            failures =
              lib.optional (names != [
                "clara-agent"
                "clara-backend"
              ]) "expected clara-agent + clara-backend processes, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/clara gateway" (argvOf "clara-agent")
              ) "gateway process does not run `clara gateway`: ${argvOf "clara-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/clara serve" (argvOf "clara-backend")
              ) "backend process does not run `clara serve`: ${argvOf "clara-backend"}"
              ++ lib.optional (
                !lib.hasInfix "--no-open" (argvOf "clara-backend")
              ) "backend must pass --no-open so a service never opens a browser"
              ++ lib.optional (
                lib.any (n: !lib.hasInfix "/home/clara-check/.clara" (envOf n)) names
              ) "gateway and backend must share one CLARA_HOME"
              ++ lib.optional (
                cfg.home.sessionVariables.CLARA_HOME or null != "/home/clara-check/.clara"
              ) "programs.clara-agent.enable must export CLARA_HOME for interactive shells"
              ++ lib.optional (
                !lib.hasInfix "clara-config-merge" activation
              ) "activation must deep-merge config.yaml, not overwrite it"
              ++ lib.optional (
                !lib.hasInfix "/home/clara-check/.clara/SOUL.md" activation
              ) "claraHomeFiles must install into CLARA_HOME"
              ++ lib.optional (
                !lib.hasInfix "/home/test-user/workspace/AGENTS.md" activation
              ) "documents must install into workingDirectory"
              # The CLI reads CLARA_MANAGED to name the rebuild command when
              # it refuses to write the configuration. A Home Manager install
              # has no nixos-rebuild command. Thus it must not report NixOS.
              ++ lib.optional (
                !lib.any (n: lib.hasInfix "CLARA_MANAGED=home-manager" (envOf n)) names
              ) "processes must report CLARA_MANAGED=home-manager"
              ++ lib.optional (
                !lib.hasInfix "clara-managed" activation
              ) "activation must write a .managed marker naming the managing system";
          in
          pkgs.runCommand "clara-home-manager-module" { } (
            if failures != [ ] then
        ...
        workspace-files-need-a-directory =
          let
            ...
            # the priority.
            sameAsDefault = "/home/clara-check";
            ...
            cases = [
              ...
              {
                name = "claraHomeFiles needs no directory";
                ok = accepts { claraHomeFiles."SOUL.md" = "x"; };
              }
              {
          ...
          in
          pkgs.runCommand "clara-workspace-files-need-a-directory" { } (
            if failed != [ ] then
        ...
        # ── The desktop application shares one CLARA_HOME ───────────────
        # `programs.enable` exports CLARA_HOME with home.sessionVariables,
        ...
        # no shell profile. Thus the desktop application would open ~/.clara
        # while the services use the CLARA_HOME of the module, and the user
        ...
        home-manager-desktop =
          let
            tokenFile = "/run/secrets/clara-desktop-token";
            ...
            enabled = evalHomeSplit {
              ...
              services = {
                enable = true;
                claraHome = "/home/clara-check/.clara-work";
            ...
            desktopPackages = builtins.filter (p: (p.pname or "") == "clara-desktop") cfg.home.packages;
            ...
            # second Clara runtime beside the services is the fault that
            # `programs.enable` plus a plain desktop package would give.
            agentPackages = builtins.filter (p: (p.pname or "") == "clara-agent") cfg.home.packages;
            ...
            backendScript =
              let
                argv =
                  if pkgs.stdenv.hostPlatform.isDarwin then
                    cfg.launchd.agents.clara-backend.config.ProgramArguments
                  else
                    [ cfg.systemd.user.services.clara-backend.Service.ExecStart ];
            ...
            failures =
              ...
              ) "programs.desktop.enable must install exactly one clara-desktop package, got ${toString (lib.length desktopPackages)}"
              ++ lib.optional (
                setValue "CLARA_HOME" != "/home/clara-check/.clara-work"
              ) "the launcher must carry CLARA_HOME: a GUI launcher reads no shell profile, so home.sessionVariables never reaches it (got: ${toString (setValue "CLARA_HOME")})"
              ++ lib.optional (
                setValue "CLARA_MANAGED" != "home-manager"
              ) "the launcher must report CLARA_MANAGED=home-manager while the services own the configuration (got: ${toString (setValue "CLARA_MANAGED")})"
              ++ lib.optional (
                lib.length agentPackages == 1
                && setValue "CLARA_DESKTOP_CLARA" != "${lib.head agentPackages}/bin/clara"
              ) "the launcher must pin the agent package that programs.enable installs, and not a second runtime: ${toString (setValue "CLARA_DESKTOP_CLARA")}"

              # ── The application reaches the backend of the service ──────
              ++ lib.optional (
                setValue "CLARA_DESKTOP_REMOTE_URL" != "http://127.0.0.1:9231"
              ) "the launcher must name the backend of the service, or the application starts a second one (got: ${toString (setValue "CLARA_DESKTOP_REMOTE_URL")})"
              ++ lib.optional (
                !lib.hasInfix "CLARA_DESKTOP_REMOTE_TOKEN" wrapper
              ) "the launcher must give a token with the URL: the desktop resolver throws when the URL is set alone"
              ++ lib.optional (
                !lib.hasInfix "CLARA_DASHBOARD_SESSION_TOKEN" backendScript
              ...
              ++ lib.optional (
                setValue "CLARA_DESKTOP_REMOTE_TOKEN" != null
              ) "the token must never be a --set value: makeWrapper writes it into the world-readable Nix store";
          in
          pkgs.runCommand "clara-home-manager-desktop" { } (
            if failures != [ ] then
              throw "Home Manager desktop check failed:\n${lib.concatMapStringsSep "\n" (f: "  - ${f}") failures}"
            else
              ''
                echo "PASS: the desktop launcher shares CLARA_HOME, the runtime and the backend of the service"
                mkdir -p $out
                echo "ok" > $out/result
              ''
          );
        ...
        home-manager-desktop-standalone =
          let
            ...
            failures =
              ...
              ++ lib.optional (
                !lib.hasInfix "--set CLARA_HOME" wrapper
              ) "the launcher must carry CLARA_HOME even with no services"
              ++ lib.optional (
                lib.hasInfix "CLARA_MANAGED" wrapper
              ) "the launcher must not claim a managed install when no activation writes one"
              ++ lib.optional (
                lib.hasInfix "CLARA_DESKTOP_REMOTE_URL" wrapper
              ) "the launcher must not name a backend when the services run none"
              ++ lib.optional (
                cfg.systemd.user.services ? clara-backend || cfg.launchd.agents ? clara-backend
              ) "programs.enable alone must start no service";
          in
          pkgs.runCommand "clara-home-manager-desktop-standalone" { } (
            if failures != [ ] then
        ...
        # command line. A silent removal thus leaves them with no `clara`
        ...
        home-manager-install-package-removed =
          let
            ...
            cases = [
              {
                value = true;
                expect = "programs.clara-agent.enable = true;";
              }
              {
                value = false;
                expect = "programs.clara-agent.enable = false;";
              }
            ];
          ...
          in
          pkgs.runCommand "clara-home-manager-install-package-removed" { } (
            if failures != [ ] then
        ...
        module-option-parity =
          ...
          in
          pkgs.runCommand "clara-module-option-parity" { } (
            if failures != [ ] then
        ...
        nixos-module =
          let
            ...
            units = lib.filterAttrs (n: _: lib.hasPrefix "clara" n) cfg.systemd.services;
            names = lib.attrNames units;
            execOf = name: units.${name}.serviceConfig.ExecStart;
            activation = cfg.system.activationScripts."clara-agent-setup".text;

            failures =
              lib.optional (names != [
                "clara-agent"
                "clara-backend"
              ]) "expected clara-agent + clara-backend units, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/clara gateway" (execOf "clara-agent")
              ) "gateway unit does not run `clara gateway`: ${execOf "clara-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/clara dashboard" (execOf "clara-backend")
              ) "backend unit does not run `clara dashboard`: ${execOf "clara-backend"}"
              ++ lib.optional (
                units.clara-agent.environment.CLARA_HOME != units.clara-backend.environment.CLARA_HOME
              ) "gateway and backend must share one CLARA_HOME"
              ++ lib.optional (
                !lib.hasInfix "/var/lib/clara/.clara/SOUL.md" activation
              ) "claraHomeFiles must install into CLARA_HOME";
          ...
          in
          pkgs.runCommand "clara-nixos-module" { } (
            if failures != [ ] then
        ...
        # polls and then execs clara, and the assertions reject a
        # configuration that cannot work.
        backend-bind-wait =
          let
            execOf =
              settings:
              (evalNixosModule ({ enable = true; } // settings)).config.systemd.services.clara-backend.serviceConfig.ExecStart;
            ...
            failures =
              ...
              lib.optional (!lib.hasInfix "bin/clara serve --host 127.0.0.1" direct)
                "without waitFor the backend must exec clara directly, got: ${direct}"
              ++ lib.optional (lib.hasInfix "clara-backend-launch" direct)
                "without waitFor the backend must not use the launcher"

              # The hostname mode polls the resolver, then binds the name.
              ++ lib.optional (!lib.hasInfix "clara-backend-launch" hostnameWait)
              ...
              ++ lib.optional (!lib.hasInfix "exec " hostnameScript)
                "the launcher must exec clara, so that it keeps the MainPID"
              ...
              ++ lib.optional (!lib.hasInfix "bin/clara dashboard" interfaceScript)
          ...
          in
          pkgs.runCommand "clara-backend-bind-wait" { } (
            if failures != [ ] then
        ...
        # $CLARA_HOME/.env. The important property is that a second run
        ...
        env-file-assembly =
          let
            envScript = (import ./moduleCommon.nix { inherit lib; }).mkEnvScript {
              inherit pkgs;
              environment = {
                CLARA_PUBLIC = "visible";
              };
            };
          in
          pkgs.runCommand "clara-env-file-assembly" { } ''
            set -e
            workdir=$(mktemp -d)
            ...
            grep -qx 'CLARA_PUBLIC=visible' "$workdir/.env" || \
        ...
        service-argv =
          let
            ...
            sentinel = "--clara-nix-argv-probe";
            probe = argv: lib.escapeShellArgs (argv ++ [ sentinel ]);
          in
          pkgs.runCommand "clara-service-argv" { } ''
            set -e
        ...
        package-contents = pkgs.runCommand "clara-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${clara-agent}/bin/clara || (echo "FAIL: clara binary missing"; exit 1)
          test -x ${clara-agent}/bin/clara-agent || (echo "FAIL: clara-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${clara-agent}/bin/clara --version 2>&1 | grep -qi "clara" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "clara-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in clara clara-agent clara-acp; do
            test -x ${clara-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "clara-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking clara --help ==="
          ${clara-agent}/bin/clara --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${clara-agent}/bin/clara --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "clara-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${clara-agent}/share/clara-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${clara-agent}/share/clara-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "CLARA_BUNDLED_SKILLS" ${clara-agent}/bin/clara || \
            (echo "FAIL: CLARA_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: CLARA_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${clara-agent}/share/clara-agent/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${clara-agent}/share/clara-agent/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "CLARA_OPTIONAL_SKILLS" ${clara-agent}/bin/clara || \
            (echo "FAIL: CLARA_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, CLARA_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "clara-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${clara-agent}/share/clara-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${clara-agent}/share/clara-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "CLARA_BUNDLED_PLUGINS" ${clara-agent}/bin/clara || \
            (echo "FAIL: CLARA_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: CLARA_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "clara-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${clara-agent}/share/clara-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${clara-agent}/share/clara-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${clara-agent}/share/clara-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "CLARA_BUNDLED_LOCALES" ${clara-agent}/bin/clara || \
            (echo "FAIL: CLARA_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: CLARA_BUNDLED_LOCALES set in wrapper"

          # locales/ is a bare data dir (no __init__.py), shipped via a
          # symlink + CLARA_BUNDLED_LOCALES (not via wheel data-files).
          # Verify the wrapper override resolves real strings.
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && CLARA_BUNDLED_LOCALES=${clara-agent}/share/clara-agent/locales \
            ${claraVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with CLARA_BUNDLED_LOCALES set"; exit 1)
        ...
        # CLARA_OPTIONAL_MCPS (not via wheel data-files).
        bundled-mcps = pkgs.runCommand "clara-bundled-mcps" { } ''
          set -e
          echo "=== Checking bundled optional-mcps ==="
          test -d ${clara-agent}/share/clara-agent/optional-mcps || (echo "FAIL: optional-mcps directory missing"; exit 1)
          echo "PASS: optional-mcps directory exists"

          MANIFEST_COUNT=$(find -L ${clara-agent}/share/clara-agent/optional-mcps -name "manifest.yaml" | wc -l)
          test "$MANIFEST_COUNT" -gt 0 || (echo "FAIL: no manifest.yaml files found"; exit 1)
          echo "PASS: $MANIFEST_COUNT catalog manifests found"

          grep -q "CLARA_OPTIONAL_MCPS" ${clara-agent}/bin/clara || \
            (echo "FAIL: CLARA_OPTIONAL_MCPS not in wrapper"; exit 1)
          echo "PASS: CLARA_OPTIONAL_MCPS set in wrapper"

          export HOME=$(mktemp -d)
          CATALOG=$(cd "$HOME" && ${clara-agent}/bin/clara mcp catalog 2>/dev/null || true)
          echo "catalog output: $CATALOG"
          test -n "$CATALOG" || (echo "FAIL: clara mcp catalog returned empty"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "clara-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${clara-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${clara-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "CLARA_TUI_DIR" ${clara-agent}/bin/clara || \
            (echo "FAIL: CLARA_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: CLARA_TUI_DIR set in wrapper"
        ...
        # Verify CLARA_NODE is set in wrapper and points to Node 26+
        # (Clara pins its toolchain to Node 26 everywhere)
        clara-node = pkgs.runCommand "clara-node-version" { } ''
          set -e
          echo "=== Checking CLARA_NODE in wrapper ==="
          grep -q "CLARA_NODE" ${clara-agent}/bin/clara || \
            (echo "FAIL: CLARA_NODE not set in wrapper"; exit 1)
          echo "PASS: CLARA_NODE present in wrapper"

          CLARA_NODE=$(sed -n "s/^export CLARA_NODE='\(.*\)'/\1/p" ${clara-agent}/bin/clara)
          test -x "$CLARA_NODE" || (echo "FAIL: CLARA_NODE=$CLARA_NODE not executable"; exit 1)
          echo "PASS: CLARA_NODE executable at $CLARA_NODE"

          NODE_MAJOR=$("$CLARA_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 26 || \
            (echo "FAIL: Node v$NODE_MAJOR < 26, Clara requires Node 26"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 26"

          echo "=== All CLARA_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify CLARA_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "clara-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(CLARA_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking CLARA_MANAGED guards ==="
          check_blocked "config set" ${clara-agent}/bin/clara config set model foo
          check_blocked "config edit" ${clara-agent}/bin/clara config edit
        ...
        extra-python-packages = let
          ...
          claraWithExtra = clara-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "clara-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${claraWithExtra}/bin/clara || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${claraWithExtra}/bin/clara || \
          ...
          if grep -q "PYTHONPATH" ${clara-agent}/bin/clara; then
        ...
        extra-dependency-groups = let
          claraWithGroups = clara-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "clara-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${claraWithGroups}"
          echo "venv: ${claraWithGroups.claraVenv}"
        ...
        messaging-variant = pkgs.runCommand "clara-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.claraVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "clara-config-roundtrip" {
          ...
          merge_and_load() {
            local clara_home="$1"
            export CLARA_HOME="$clara_home"
            ${configMergeScript} ${nixSettings} "$clara_home/config.yaml"
            ${claraVenv}/bin/python3 -c '
import json, sys
