...
{ ... }:
{
  perSystem =
    ...
    let
      ...
      claraNpmLib = self'.packages.default.passthru.claraNpmLib;
      ...
      claraAgentDevShellHook = self'.packages.default.passthru.devShellHook;
    in
    {
      devShells.default = pkgs.mkShell {
        packages = with pkgs; [
          (pkgs.runCommand "clara" { } ''
            mkdir -p $out/bin
            install -Dm755 ${../clara} $out/bin/clara
          '')
        ...
        shellHook = ''
          ${claraAgentDevShellHook}
          ${claraNpmLib.mkNpmDevShellHook npmPackageJsonPaths}
          ...
          export CLARA_PYTHON_SRC_ROOT=$(git rev-parse --show-toplevel)
          ...
          echo "Clara Agent dev shell in $CLARA_PYTHON_SRC_ROOT"
          echo "Ready. Run 'clara' or 'sandbox clara' to start."
        '';
      };
    };
}
