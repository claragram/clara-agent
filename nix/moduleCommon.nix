...
#
# `services.clara-agent` is the same option set on both modules. Both modules
...
#   homeManagerModules.nix  claraHome, programs.clara-agent (the CLI and
...
let
  ...
  deepConfigType = types.mkOptionType {
    name = "clara-config-attrs";
    description = "Clara YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    options = {
      ...
      auth = mkOption {
        ...
        description = ''
          ...
          (remote MCP servers). Tokens are stored in $CLARA_HOME/mcp-tokens/.
        '';
      };
  ...
  sharedOptions =
    ...
    {
      enable = lib.mkEnableOption "Clara Agent";
      ...
      package = mkOption {
        ...
        description = "The clara-agent package to use.";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          The Clara configuration, as an attribute set. The module joins the
          ...
          which includes the keys that `clara config set` and the settings
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          $CLARA_HOME/.env. Clara reads that file at each start, with
          load_clara_dotenv().

          Each activation writes .env again from the start. Thus a secret
          file cannot go into .env two times.
        '';
        example = literalExpression ''[ config.sops.secrets."clara/env".path ]'';
      };
      ...
      authFile = mkOption {
        ...
        description = ''
          ...
          does not exist. Thus a token that Clara refreshes at runtime stays
          after an activation.
        '';
      };
      ...
      documents = mkOption {
        ...
        description = ''
          ...
          Clara reads SOUL.md and memories/ from CLARA_HOME, so put those
          files in `claraHomeFiles`.
      ...
      claraHomeFiles = mkOption {
        type = documentsType;
        default = { };
        description = ''
          Files that the module installs into CLARA_HOME. Each key is a path
          relative to that directory, and the module makes the necessary
          subdirectories. Each value is a string or a path.

          Clara reads SOUL.md and the memory files from CLARA_HOME and not
          from the working directory. Declare those files here, or Clara
          does not load them.
        '';
        example = literalExpression ''
          {
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the clara plugins
          directory. Each package must contain a plugin.yaml and __init__.py
          at its root. Clara discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          clara_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as clara (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in clara-agent's
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `clara gateway`.";
      };
      ...
      # ── The backend: `clara serve` or `clara dashboard` ──────────────
      # `clara serve` and `clara dashboard` are the same entry point,
      # clara_cli.main:cmd_dashboard, with one flag of difference. serve runs
      # without a user interface. dashboard also serves the web application.
      # Both give the /api/ws and /api/pty sockets that Clara Desktop
      ...
      # controls an external gateway, with `clara gateway restart`. It does
      ...
      backend = {
        mode = mkOption {
          ...
          description = ''
            ...
            - "serve"     — the backend without a user interface. It gives
                            the /api/ws and /api/pty sockets that Clara
                            Desktop connects to.
        ...
        sessionTokenFile = mkOption {
          ...
          description = ''
            ...
            CLARA_DASHBOARD_SESSION_TOKEN. That token authorizes the /api
            routes and the /api/ws socket. Clara Desktop presents the same
          ...
          '';
          example = literalExpression ''config.sops.secrets."clara/desktop-token".path'';
        };
      };
    };
  ...
  # removal leaves them with no `clara` on the PATH and no message. The
  ...
  installPackageRemovedMessage =
    value:
    ''
      services.clara-agent.installPackage was removed. Clara now
      separates the installation from the services, which is the
      Home Manager convention:

        programs.clara-agent.enable = ${lib.boolToString (value != false)};  # the clara CLI, and CLARA_HOME for your shells
        programs.clara-agent.desktop.enable = true;  # the desktop application

      `services.clara-agent` keeps the state, the configuration and
  ...
  mkConfigFiles =
    ...
    let
      generated = pkgs.writeText "clara-config.yaml" (
  ...
  mkDocumentTree =
    { pkgs, documents }:
    pkgs.runCommand "clara-documents" { } (
      ''
        mkdir -p $out
      ''
  ...
  mkEnvScript =
    { pkgs, environment }:
    let
      base = pkgs.writeText "clara-env-base" (
        lib.concatStringsSep "\n" (lib.mapAttrsToList (k: v: "${k}=${v}") environment)
        + lib.optionalString (environment != { }) "\n"
      );
    in
    pkgs.writeShellScript "clara-env-merge" ''
      set -eu
      ...
      for file in "$@"; do
        if [ -r "$file" ]; then
          ...
          echo "clara-agent: WARNING cannot read environmentFile $file" >&2
        fi
      done
    '';
  ...
  mkStateScript =
    {
      ...
      # CLARA_MANAGED variable of the service. The value tells the shell
    ...
    in
    ''
      # Directories. The service units and Clara make most of these
      ...
      ${run}mkdir -p ${
        lib.escapeShellArgs (
          ...
          ++ map (d: "${claraHome}/${d}") stateDirs
        )
      }

      # config.yaml: merge the Nix settings into the file on disk. Clara
      ...
      ${
        if cfg.configFile != null then
          "${inst} -m ${modes.config} -D ${configFiles.effective} ${claraHome}/config.yaml"
        else
          ''
            ${run}${configFiles.mergeScript} ${configFiles.generated} ${claraHome}/config.yaml
            ${run}chmod ${modes.config} ${claraHome}/config.yaml
          ''
      }

      # The managed-mode marker. It makes an interactive shell also refuse to
      # change the configuration that Nix owns.
      ${inst} -m ${modes.managed} ${pkgs.writeText "clara-managed" managedSystem} ${claraHome}/.managed

      ${lib.optionalString (cfg.environment != { } || cfg.environmentFiles != [ ]) ''
        ${run}${envScript} ${claraHome}/.env ${modes.env} ${lib.escapeShellArgs cfg.environmentFiles}
        ${lib.optionalString (owner != null) "${run}chown ${owner} ${claraHome}/.env"}
      ''}

      ${lib.optionalString (cfg.authFile != null) (
        if cfg.authFileForceOverwrite then
          "${inst} -m ${modes.auth} ${cfg.authFile} ${claraHome}/auth.json"
        else
          ''
            if [ ! -e ${claraHome}/auth.json ]; then
              ${inst} -m ${modes.auth} ${cfg.authFile} ${claraHome}/auth.json
            fi
          ''
      )}
      ...
      ${installDocuments homeDocumentTree claraHome cfg.claraHomeFiles}
      ...
      ${run}find ${claraHome}/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
      ${lib.concatMapStringsSep "\n" (plugin: ''
        if [ ! -f ${plugin}/plugin.yaml ]; then
          echo "clara-agent: ERROR extraPlugins entry '${plugin}' has no plugin.yaml" >&2
          exit 1
        fi
        ${run}ln -sfn ${plugin} ${claraHome}/plugins/nix-managed-${lib.getName plugin}
      '') cfg.extraPlugins}
    '';
  ...
  gatewayArgv =
    cfg:
    [
      "${effectivePackage cfg}/bin/clara"
      "gateway"
    ]
    ++ cfg.extraArgs;
  ...
  #
  # `exec` on the last line keeps clara as the MainPID of the unit. No shell
  ...
  backendLauncher =
    ...
    pkgs.writeShellScript "clara-backend-launch" (
      builtins.replaceStrings [ "@HOST@" ] [ ''"$_target"'' ] ''
        ...
        ${lib.optionalString (cfg.backend.sessionTokenFile != null) ''
          ...
          if [ ! -r "$_token_file" ]; then
            echo "clara-backend: cannot read the session token file '$_token_file'. The unit stops." >&2
            echo "clara-backend: backend.sessionTokenFile must name a runtime path that this user can read." >&2
            exit 1
          fi

          CLARA_DASHBOARD_SESSION_TOKEN="$(${pkgs.coreutils}/bin/tr -d '\r\n' < "$_token_file")"
          export CLARA_DASHBOARD_SESSION_TOKEN

          if [ -z "$CLARA_DASHBOARD_SESSION_TOKEN" ]; then
            echo "clara-backend: the session token file '$_token_file' is empty. The unit stops." >&2
            exit 1
          fi
        ''}
        ${
          if cfg.backend.waitFor == null then
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "clara-backend: '$_target' did not resolve after ''${_timeout}s. The unit stops." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "clara-backend: waits for '$_target' to resolve..." >&2
                fi
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "clara-backend: interface '$_iface' had no IPv4 address after ''${_timeout}s. The unit stops." >&2
                  echo "clara-backend: a fallback address can expose the backend more widely than you intend." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "clara-backend: waits for an IPv4 address on '$_iface'..." >&2
                fi
                ${pkgs.coreutils}/bin/sleep 2
                _waited=$(( _waited + 2 ))
              done
        ...
        echo "clara-backend: binds to $_target:${toString cfg.backend.port} (from $_how)" >&2
  ...
  backendDescription =
    ...
    if cfg.backend.mode == "dashboard" then
      "Clara Agent web dashboard and desktop backend"
    else
      "Clara Agent backend for Clara Desktop";

  # The environment that each Clara process needs, from either module.
  #
  # managedSystem gives the value of CLARA_MANAGED. The CLI reads that
    ...
    [
      {
        ...
        message = ''
          ...
          To give Clara an identity and a memory, use
          ${optionPath}.claraHomeFiles instead. Those files go to
          CLARA_HOME. Clara reads SOUL.md and memories/ only from there.
        '';
      }
    ];
  ...
  # The subdirectories of CLARA_HOME that both modules make.
  stateSubdirs = [
    "cron"
    "sessions"
    "logs"
