...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # Clara derivation. This is deliberately a derivation environment
          # variable, not a devShell variable: ``nix develop -c uv build``
          # must remain blocked.
          (final: prev: {
            clara-agent = prev.clara-agent.overrideAttrs (_old: {
              CLARA_NIX_BUILD = "1";
            });
          })
        ]
  ...
  # $CLARA_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$CLARA_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        clara-agent = prev.clara-agent.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "clara-agent-env" {
    clara-agent = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "clara-agent-editable-env" {
    clara-agent = dependency-groups;
  };
}
