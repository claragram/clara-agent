# nix/web.nix — Clara Web Dashboard (Vite/React) frontend build
{ claraNpmLib, ... }:
claraNpmLib.buildNpmPackage {
  dirs = [
    "web"

    # @clara/shared ships as a file: workspace dep of web, so its source
  ...
  buildPhase = ''
    ...
    # outDir in vite.config.ts points to ../clara_cli/web_dist for the
