# nix/packages.nix — Clara Agent package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    {
    ...
    let
      ...
      minimal = pkgs.callPackage ./clara-agent.nix {
    ...
    in
    {
      packages = {
        ...
        tui = full.claraTui;
        web = full.claraWeb;
        desktop = full.claraDesktop;

        update-npm-lockfile = full.claraNpmLib.updateNpmLockfile;
      };
    };
}
