...
# node:26 image instead (Clara pins its toolchain to Node 26 everywhere).
...
# Do not write .pyc files at runtime: /opt/clara is immutable in the
...
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/clara/.playwright

# Install system dependencies in one layer, clear APT cache.
# tini was previously PID 1 to reap orphaned zombie processes (MCP stdio
# subprocesses, git, bun, etc.) that would otherwise accumulate when clara
# ran as PID 1. See #15012. Phase 2 of the s6-overlay supervision plan
# replaces tini with s6-overlay's /init (PID 1 = s6-svscan), which reaps
# zombies non-blockingly on SIGCHLD and additionally supervises the main
# clara process, the dashboard, and per-profile gateways.
RUN apt-get -o Acquire::Retries=3 update && \
    apt-get -o Acquire::Retries=3 install -y --no-install-recommends \
...
db.execute(\"INSERT INTO docs VALUES ('clara')\"); \
...
# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main clara process, the dashboard,
...
# 'Clara WebUI' catalog, NAS compose projects that preserve an old
...
# Non-root user for runtime; UID can be overridden via CLARA_UID at runtime
RUN useradd -u 10000 -m -d /opt/data clara
...
#
# ui-tui/packages/clara-ink/ is copied IN FULL (not just its manifests)
...
COPY ui-tui/packages/clara-ink/ ui-tui/packages/clara-ink/
# apps/shared/ is copied IN FULL because web/package.json references it as a
# `file:` workspace dependency (same pattern as clara-ink above).
...
# lock on the @clara/ink entry, tripped the TUI launcher's
...
#
# The [otlp] extra contains the SDK/exporter imported by Clara when Gateway
# Health export is enabled. Collector and observability-backend dependencies
# remain external and are not part of the Clara production image.
#
# The hindsight memory provider's client (hindsight-client) is baked in
# for the same reason: it lazy-installs into /opt/clara/.venv at first
...
# gives the non-root clara user read + traverse but no write; root retains
# write so the build steps below don't need chmod u+w dances.
COPY --link --chmod=a+rX,go-w . .

# ---------- Permissions ----------
# Link clara-agent itself (editable). Deps are already installed in the
# cached layer above; `--no-deps` makes this a fast egg-link creation with no
# resolution or downloads.
RUN uv pip install --no-cache-dir --no-deps -e "."

# Wire the exec shim and install-method stamp.  Files under /opt/clara are
# already root-owned (COPY, uv sync, npm install all run as root) and
# read-only for the clara user (go-w from the --chmod above).

USER root
RUN mkdir -p /opt/clara/bin && \
    cp /opt/clara/docker/clara-exec-shim.sh /opt/clara/bin/clara && \
    chmod 0755 /opt/clara /opt/clara/bin/clara && \
    printf 'docker\n' > /opt/clara/.install_method
# The ``.install_method`` stamp is baked next to the running code (the install
# tree), NOT into $CLARA_HOME. $CLARA_HOME (/opt/data) is a shared data
# volume that is commonly bind-mounted from the host and even shared with a
# host-side Desktop/CLI install; stamping it at boot used to clobber that
# host install's marker and wrongly block its ``clara update``. A code-scoped
# stamp is read first by detect_install_method() and is immune to the share.
# Start as root so the s6-overlay stage2 hook can usermod/groupmod and chown
# the data volume. Each supervised service then drops to the clara user via
# `s6-setuidgid clara` in its run script. If CLARA_UID is unset, services
# run as the default clara user (UID 10000).
...
# lives outside both /opt/clara (which operators sometimes bind-mount as a
# checkout) and /opt/data (the mutable CLARA_HOME volume).
# .dockerignore excludes .git, so `git rev-parse HEAD` from inside the
# container always returns nothing — meaning `clara dump` reports
# "(unknown)" and the startup banner drops its `· upstream <sha>` suffix.
# That makes support triage from container bug reports impossible:
# we can't tell which commit the user is actually running.
#
# Fix: write the commit SHA passed via the CLARA_GIT_SHA build-arg to
# /opt/clara/.clara_build_sha at build time, and have
# clara_cli/build_info.py read it at runtime.  Both `clara dump` and
...
ARG CLARA_GIT_SHA=
RUN set -eu; \
    if [ -n "${CLARA_GIT_SHA}" ]; then \
        printf '%s\n' "${CLARA_GIT_SHA}" > /opt/clara/.clara_build_sha; \
    fi; \
    mkdir -p /etc/clara; \
    CLARA_GIT_SHA="${CLARA_GIT_SHA}" python3 -c 'import json, os, pathlib, tomllib; project = tomllib.loads(pathlib.Path("/opt/clara/pyproject.toml").read_text(encoding="utf-8"))["project"]; marker = pathlib.Path("/etc/clara/image-provenance.json"); marker.write_text(json.dumps({"schema": 1, "deployment_kind": "image", "manager": "docker", "image": "workprise/clara-agent", "version": project["version"], "revision": os.environ.get("CLARA_GIT_SHA") or None}, sort_keys=True, separators=(",", ":")) + "\n", encoding="utf-8"); marker.chmod(0o444)'

# ---------- s6-overlay service wiring ----------
# Static services declared at build time: main-clara + dashboard.
...
# `exec clara`. Wired in as cont-init.d/01- so it
# runs before user services start.
#
# 02-reconcile-profiles re-creates per-profile gateway s6 service
# slots from $CLARA_HOME/profiles/<name>/ after a container restart
# (the /run/service/ scandir is tmpfs and wiped on restart). Phase 4.
RUN mkdir -p /etc/cont-init.d && \
    printf '#!/command/with-contenv sh\nexec /opt/clara/docker/stage2-hook.sh\n' \
        > /etc/cont-init.d/01-clara-setup && \
    chmod +x /etc/cont-init.d/01-clara-setup
COPY --chmod=0755 docker/cont-init.d/015-supervise-perms /etc/cont-init.d/015-supervise-perms
COPY --chmod=0755 docker/cont-init.d/02-reconcile-profiles /etc/cont-init.d/02-reconcile-profiles

# ---------- Runtime ----------
ENV CLARA_WEB_DIST=/opt/clara/clara_cli/web_dist
# Point the TUI launcher at the prebuilt bundle baked at build time (Layer 8:
# `ui-tui && npm run build`). This makes _make_tui_argv take the prebuilt-bundle
# fast path (`node --expose-gc /opt/clara/ui-tui/dist/entry.js`) and skip the
...
ENV CLARA_TUI_DIR=/opt/clara/ui-tui
ENV CLARA_HOME=/opt/data
ENV CLARA_WRITE_SAFE_ROOT=/opt/data
ENV CLARA_DISABLE_LAZY_INSTALLS=1
# The published image seals /opt/clara (root-owned, read-only) so a runtime
...
# is seeded + chowned to the clara user by docker/stage2-hook.sh and lives
...
ENV CLARA_LAZY_INSTALL_TARGET=/opt/data/lazy-packages

# `docker exec` privilege-drop shim. When operators run
# `docker exec <c> clara ...` they default to root, and any file the
# command writes under $CLARA_HOME (auth.json, .env, config.yaml) ends
# up root-owned and unreadable to the supervised gateway (UID 10000).
# The shim lives at /opt/clara/bin/clara, sits earliest on PATH, and
# transparently re-exec's the real venv binary via `s6-setuidgid clara`
# when invoked as root. Non-root callers (supervised processes,
# `--user clara`, etc.) hit the short-circuit path with no overhead.
# Recursion is impossible because the shim exec's the venv binary by
# absolute path (/opt/clara/.venv/bin/clara). See the shim source for
# the opt-out env var (CLARA_DOCKER_EXEC_AS_ROOT=1).
COPY --chmod=0755 docker/clara-exec-shim.sh /opt/clara/bin/clara
COPY --chmod=0755 docker/entrypoint-dispatch.sh /opt/clara/docker/entrypoint-dispatch.sh

# Pre-s6 entrypoint.sh did `source .venv/bin/activate` which exported
# the venv bin onto PATH; Architecture B's main-wrapper.sh does the
# same for the container's main process, but `docker exec` and our
# cont-init.d scripts don't pass through the wrapper. Expose the venv
# bin globally so `docker exec <container> clara ...` and any
# subprocess that doesn't activate the venv first still find clara.
#
# /opt/clara/bin is prepended ahead of the venv so the privilege-drop
# shim wins PATH resolution. The shim's last act is to exec the venv
# binary by absolute path, so this PATH ordering is transparent to
# every other consumer.
ENV PATH="/opt/clara/bin:/opt/clara/.venv/bin:/opt/data/.local/bin:${PATH}"
...
#
# main-wrapper.sh handles arg routing (bare-exec vs. clara
# subcommand vs. no-args), drops to the clara user via s6-setuidgid,
...
ENTRYPOINT [ "/opt/clara/docker/entrypoint-dispatch.sh" ]
CMD [ ]
