...
Config surface is deliberately env-only (``CLARA_STARTUP_WATCHDOG=0`` to
disable, ``CLARA_STARTUP_WATCHDOG_TIMEOUT_S`` to tune): the watchdog must be
...
ENV_STARTUP_WATCHDOG = "CLARA_STARTUP_WATCHDOG"
ENV_STARTUP_WATCHDOG_TIMEOUT_S = "CLARA_STARTUP_WATCHDOG_TIMEOUT_S"
...
# Module-level singleton: the arm sites (clara_cli.main / clara_cli.gateway
# / gateway.run.main / cli.py --gateway / scripts/clara-gateway) and the
...
def _process_clara_home() -> Path:
    """CLARA_HOME for process-level diagnostic files.

    Stdlib-only replica of ``clara_constants``' platform default — this
    module must not import application code (see module docstring). Hosted
    images always set ``CLARA_HOME`` explicitly.
    """
...
def get_startup_watchdog_dump_path(home: Optional[Path] = None) -> Path:
    """Return ``<CLARA_HOME>/logs/gateway-startup-watchdog.log``."""
...
def startup_watchdog_disabled() -> bool:
    """True when ``CLARA_STARTUP_WATCHDOG`` opts out explicitly."""
...
def arm_startup_watchdog(
    ...
    *,
    ...
    ``CLARA_STARTUP_WATCHDOG=0`` or when the thread could not be started.
    """
    global _handle
    try:
