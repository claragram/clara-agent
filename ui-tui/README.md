...
React + Ink terminal UI for Clara. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `CLARA_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `clara setup`, suspends Ink while it runs, opens a new session on success
...
- `commands/billing.ts` — `/billing`: manage Clara remote spending — buy credits, auto-reload, limits
...
- Input history is stored in `~/.clara/.clara_history` or under `CLARA_HOME`.
...
### Billing (`billing.ts`)
`/billing` — manage Clara remote spending — buy credits, auto-reload, limits
...
### Credits (`credits.ts`)
`/credits` — Clara credit balance and browser top-up

### Setup (`setup.ts`)
`/setup` — launches external `clara setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/clara-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external clara setup, suspends Ink while it runs
      ...
      slash/
        ...
        commands/
          billing.ts                /billing — manage Clara remote spending
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.clara/tui-parent.log
    ...
    types/
      clara-ink.d.ts            type declarations for @clara/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent ClaraCLI subprocess for slash commands
```
