...
// ~/.clara. To exercise the real persistence path we clear that gate, point
// CLARA_HOME at a temp dir, and re-import the module fresh (path + enabled
// flag are captured at module load).
const loadFresh = async (home: string) => {
  vi.resetModules()
  vi.stubEnv('VITEST', '')
  vi.stubEnv('CLARA_HOME', home)
...
describe('recordParentLifecycle', () => {
  ...
  beforeEach(() => {
    home = mkdtempSync(join(tmpdir(), 'clara-parentlog-'))
  })
