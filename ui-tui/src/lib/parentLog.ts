...
const logDir = join(process.env.CLARA_HOME?.trim() || join(homedir(), '.clara'), 'logs')
...
// a real ~/.clara (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('clara-tui: parent lifecycle log unavailable\n')
    }
  }
}
