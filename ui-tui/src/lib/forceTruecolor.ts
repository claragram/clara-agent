...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.CLARA_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Clara on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via CLARA_TUI_TRUECOLOR=1.
