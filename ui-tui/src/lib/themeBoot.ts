 ...
 * `clara-boot-background` / `clara-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports CLARA_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.clara matches get_clara_home()'s default.
const bootFilePath = () => join(process.env.CLARA_HOME ?? join(homedir(), '.clara'), 'tui-theme-boot.json')

// Never touch the user's real ~/.clara from test runs (the TS suite has no
// CLARA_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into CLARA_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.CLARA_TUI_THEME || env.CLARA_TUI_LIGHT) {
    return result
  }
  ...
  if (
    ...
    !env.CLARA_TUI_BACKGROUND
  ) {
    env.CLARA_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.CLARA_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.CLARA_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into CLARA_TUI_THEME.
