...
const ENV_KEYS = ['CLARA_AUTO_HEAPDUMP', 'CLARA_HEAPDUMP_DIR', 'CLARA_HEAPDUMP_MAX_BYTES'] as const
...
describe('performHeapDump auto opt-in gate (#21767)', () => {
  ...
  beforeEach(() => {
    ...
    dir = mkdtempSync(join(tmpdir(), 'clara-heapdump-test-'))
    process.env.CLARA_HEAPDUMP_DIR = dir
  })
  ...
  it('writes diagnostics only for auto-high without CLARA_AUTO_HEAPDUMP', async () => {
  ...
  it('writes diagnostics only for auto-critical without CLARA_AUTO_HEAPDUMP', async () => {
  ...
  it('writes both diagnostics and snapshot for auto-high when CLARA_AUTO_HEAPDUMP=1', async () => {
    process.env.CLARA_AUTO_HEAPDUMP = '1'
  ...
  it('accepts truthy spellings (true|yes|on, case-insensitive) as opt-in', async () => {
    for (const value of ['true', 'YES', 'On']) {
      process.env.CLARA_AUTO_HEAPDUMP = value
      const result = await performHeapDump('auto-high')
  ...
  it('writes both for manual triggers regardless of CLARA_AUTO_HEAPDUMP', async () => {
...
describe('heapdump retention guard (#21767)', () => {
  ...
  beforeEach(() => {
    savedDir = process.env.CLARA_HEAPDUMP_DIR
    savedMax = process.env.CLARA_HEAPDUMP_MAX_BYTES
    delete process.env.CLARA_AUTO_HEAPDUMP
    dir = mkdtempSync(join(tmpdir(), 'clara-heapdump-prune-'))
    process.env.CLARA_HEAPDUMP_DIR = dir
  })

  afterEach(() => {
    if (savedDir === undefined) {
      delete process.env.CLARA_HEAPDUMP_DIR
    } else {
      process.env.CLARA_HEAPDUMP_DIR = savedDir
    }

    if (savedMax === undefined) {
      delete process.env.CLARA_HEAPDUMP_MAX_BYTES
    } else {
      process.env.CLARA_HEAPDUMP_MAX_BYTES = savedMax
    }
  ...
  it('evicts oldest files when total bytes exceed the cap, retaining the newest', async () => {
    ...
    process.env.CLARA_HEAPDUMP_MAX_BYTES = String(2 * 1024)
