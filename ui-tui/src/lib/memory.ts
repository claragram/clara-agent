...
export interface HeapDumpResult {
  ...
  // the heavy snapshot because CLARA_AUTO_HEAPDUMP was not enabled (#21767).
  suppressed?: boolean
  success: boolean
}
...
export async function performHeapDump(trigger: MemoryTrigger = 'manual'): Promise<HeapDumpResult> {
  try {
    ...
    const dir = process.env.CLARA_HEAPDUMP_DIR?.trim() || join(homedir() || tmpdir(), '.clara', 'heapdumps')

    await mkdir(dir, { recursive: true })

    const base = `clara-${new Date().toISOString().replace(/[:.]/g, '-')}-${process.pid}-${trigger}`
    ...
    const autoEnabled = /^(?:1|true|yes|on)$/i.test((process.env.CLARA_AUTO_HEAPDUMP ?? '').trim())
...
async function pruneHeapdumps(dir: string): Promise<void> {
  const raw = process.env.CLARA_HEAPDUMP_MAX_BYTES?.trim()
