 ...
 * confirm button carries the recovery: Clara → `/topup`, other providers → their
...
export function billingDialogCopy(block: BillingBlock): BillingDialogCopy {
  if (block.is_clara) {
    return {
      cancelLabel: 'Dismiss',
      confirmLabel: 'Top up',
      detail: 'Your Clara credit balance is exhausted — top up to keep going.',
      title: 'Out of Clara credits'
    }
  }
