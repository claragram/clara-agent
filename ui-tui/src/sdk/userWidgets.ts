...
import { Box, Text } from '@clara/ink'
...
/**
 * User widget apps — Clara authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$CLARA_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.clara/plugins/`: files under CLARA_HOME execute
...
const widgetsDir = () => join(process.env.CLARA_HOME?.trim() || join(homedir(), '.clara'), 'tui-widgets')
 ...
 *  every change, so a widget Clara writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Clara) ever writes must hot-load too — a 10s poll here read
