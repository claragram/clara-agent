...
import { terminalBackgroundHex } from '@clara/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $CLARA_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see CLARA_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['CLARA_TUI_BACKGROUND', process.env.CLARA_TUI_BACKGROUND ?? '(unset)'],
            ['CLARA_TUI_THEME', process.env.CLARA_TUI_THEME ?? '(unset)'],
