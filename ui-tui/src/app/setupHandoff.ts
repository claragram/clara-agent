import type { RunExternalProcess } from '@clara/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`clara ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching clara: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`clara ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
