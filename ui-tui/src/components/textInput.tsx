import type { InputEvent, Key } from '@clara/ink'
import * as Ink from '@clara/ink'
 ...
 * but that accepts `key.meta` on macOS — and clara-ink reports Option as
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.CLARA_TUI_TERMUX_FAST_ECHO ?? '')
      .trim()
      .toLowerCase()
  ...
  useInput(
    (inp: string, k: Key, event: InputEvent) => {
      ...
      if (actionHome) {
        ...
        // ESC d, which clara-ink decodes as meta+'d'; without this branch it
