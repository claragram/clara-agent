#!/usr/bin/env python3
"""
SQLite State Store for Clara Agent.
...
# transitive dependency via clara_state_common). run_agent keeps its own
# predating copy — clara_state cannot import run_agent (circular) — guarded
...
from clara_startup_watchdog import report_startup_progress
from clara_cli.sqlite_runtime import (
    is_sqlite_wal_reset_vulnerable as _is_sqlite_wal_reset_vulnerable,
)
from typing import Any, Callable, Dict, Iterator, List, Optional, Set, Tuple, TypeVar, cast

from clara_state_common import (  # noqa: F401  (re-exported for back-compat)
    _BRANCH_CHILD_SQL,
...
from clara_state_portability import SessionPortabilityMixin
from clara_state_schema import SessionSchemaMixin
from clara_state_search import SessionSearchMixin
...
def _workspace_key_clause(key: str) -> Tuple[str, List[str]]:
    ...
    Used by ``clara -c``/``--resume`` to continue the most recent session in
...
DEFAULT_DB_PATH = get_clara_home() / "state.db"
...
#
# The ceilings above bound Clara's SQLite descriptors, which is only ever part
...
def _default_db_path() -> Path:
    ...
    freezes the developer's real ``~/.clara`` even when a test fixture later
    redirects ``CLARA_HOME`` — importing this module during collection was
    ...
    2. Otherwise resolve ``get_clara_home()`` fresh so a runtime
       ``CLARA_HOME`` redirect takes effect regardless of import order.
    """
...
# ~/.clara/state.db accumulated pytest fixture rows — sessions with
...
# FTS write corruption").  The hermetic conftest redirects CLARA_HOME per
# test, but any escape (a session-scoped fixture running before the autouse
# fixture, a subprocess child launched without CLARA_HOME, a stale worktree
# without the re-pin, or a developer shell that exports CLARA_HOME to the
...
_STATE_DB_GUARD_BYPASS_ENV = "CLARA_STATE_DB_GUARD_BYPASS"

#: Additional production roots to refuse (beyond the platform default
#: ``~/.clara``).  The test conftest injects the pre-sandbox production
#: root here so custom-``CLARA_HOME`` deployments are covered too.
_STATE_DB_GUARD_EXTRA_DENY_ROOTS: Tuple[Path, ...] = ()


def _real_platform_state_root() -> Optional[Path]:
    """Resolve the REAL platform-default Clara root for the guard.

    Deliberately avoids ``Path.home()`` / ``clara_constants``: tests
    routinely monkeypatch ``Path.home`` to a tempdir, and ``clara_state``
    ...
    try:
        if sys.platform == "win32":
            ...
            root = (
                Path(base) / "clara"
                if base
                else Path(os.path.expanduser("~")) / "AppData" / "Local" / "clara"
            )
        else:
            root = Path(os.path.expanduser("~")) / ".clara"
        return root.resolve()
    except Exception:
        return None
...
#: redirects ``CLARA_HOME`` to the per-session tmp isolation root.  Its
...
#: is OURS: it declares "this process tree is running under Clara test
#: isolation", and it inherits into subprocess children by default — so a
#: child that received the patched ``CLARA_HOME`` also received the marker,
#: and a child that resolves a production DB while carrying it is, by
#: definition, an isolation escape (#82770).
_TEST_ISOLATION_MARKER_ENV = "CLARA_TEST_ISOLATION"
...
def _has_pytest_ancestor() -> bool:
    ...
    ``CLARA_HOME`` redirect: that child aims at the production DB *and*
...
def _in_test_context() -> bool:
    ...
    so a real ``clara`` invocation pays for at most one walk.
    """
...
def _is_production_state_db(resolved: Path, root: Path) -> bool:
    """True when *resolved* is a DB file of the real Clara home *root*.
    ...
    under ``~/.clara/clara-agent/...``) so hermetic tests using unusual
...
def _ensure_test_isolation(db_path: Path) -> None:
    ...
    pytest and for hermetic (tmp ``CLARA_HOME``) paths.
    ...
    a rebuilt environment loses ``PYTEST_*`` and ``CLARA_HOME`` together,
    ...
    for root in _production_state_roots():
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                f"state.db at {resolved} (under real Clara root {root}). "
                "Tests must run against a temporary CLARA_HOME — pass an "
                "explicit tmp db_path or let the hermetic conftest redirect "
                "CLARA_HOME. If this test genuinely needs the live "
...
# clara_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def _apply_wal_size_limit(conn: sqlite3.Connection) -> None:
    ...
    3.0 GB ``state.db``: ``clara sessions optimize`` (FTS merge + VACUUM)
    ...
    ``clara_cli/kanban_db.py`` already bounds its WAL growth with
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``clara_cli.kanban_db.connect`` so
...
def _wal_reset_repair_hint() -> str:
    ...
    ``clara update`` can actually do for this install (#75153).
    """
    try:
        ...
        if method in {"git", "unknown"}:
            return f"Clara-managed installs can repair the embedded runtime with `{cmd}`"
        if method == "docker":
    ...
    return (
        ...
        "(or backports 3.50.7 / 3.44.6) and restart Clara"
    )
...
def _log_wal_reset_bug_once(
    ...
    *,
    ...
    # Check whether this is a Clara-managed install (uv-managed venv)
    ...
    logger.warning(
        ...
        "%s. See `clara doctor`. This warning fires once per "
...
def _log_journal_mode_upgrade_once(db_label: str, previous_mode: str) -> None:
    ...
    ``clara_cli/managed_uv._default_live_venv``, which treats a database
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see clara_cli/kanban_db.py) would
...
def apply_database_pragmas(
    ...
    * ``synchronous`` — durability level: ``OFF``/``NORMAL``/``FULL``/``EXTRA``
    ...
    """
    try:
        # Local import avoids a circular import with clara_cli.config.
...
def classify_persistence_error(exc_or_str) -> str:
    ...
    * ``"corrupt"`` — the database file itself is structurally damaged
      ...
      (``clara doctor`` / automatic schema surgery).
...
# threads inside ONE interpreter, yet a normal Clara host runs several
# independent processes against the same ``state.db``: the gateway service,
# the Desktop app's own ``clara serve`` backend, interactive CLI sessions,
...
# included because rollback-journal (DELETE) mode — Clara's fallback on
...
def _db_fingerprint(db_path: Path) -> "Optional[str]":
    ...
    RESERVED lock (see ``clara_cli.sqlite_safe_read`` rule 1). This function
    ...
    try:
        ...
        try:
            ...
            # Scaffold/embed installs ship clara_state without clara_cli. No
...
def _backup_db_file(db_path: Path) -> "Tuple[Optional[Path], Optional[str]]":
    ...
    connection's POSIX advisory locks (see ``clara_cli.sqlite_safe_read``).
...
def preflight_db_writability(
    ...
    *,
    ...
    - **Repairs** permissions with ``chmod u+rw`` when the file lives inside
      the Clara home tree (``get_clara_home()``) — the safe repair scope:
      Clara owns those files, and the OS makes ``chmod`` fail on files the
    ...
    to check). Shared by :class:`SessionDB` and ``clara_cli.kanban_db``.
    """
    raw = str(db_path)
    ...
    try:
        home: Optional[Path] = Path(get_clara_home()).resolve()
    ...
    def _ensure_writable(p: Path, *, is_dir: bool = False) -> None:
        ...
        raise sqlite3.OperationalError(
            ...
            f"user. Clara needs read-write access to open the database. "
    ...
    try:
        ...
        for fts_table in ("messages_fts", "messages_fts_trigram", "messages_fts_cjk"):
            try:
                ...
                # (clara_state.py:645-723). The supported degraded-runtime
                # path (SessionDB._is_fts5_unavailable_error + the
                # regression suite in tests/test_clara_state.py:600-632)
        ...
        probe_session_id = f"_clara_fts_health_probe_{time.time_ns()}"
        try:
...
def _live_writer_holds_db(db_path: Path) -> bool:
    ...
    bug and on NFS/SMB, Clara deliberately runs ``state.db`` in
        ...
        if not holding_lock:
            ...
            # later BEGIN EXCLUSIVE acquisition.
                ...
                report["error"] = (
                    ...
                    "Stop the gateway (clara gateway stop) and retry."
                )
                logger.error("state.db repair skipped: %s", report["error"])
            else:
...
# (``~/.clara/lib/libfts5_cjk.so``, built by ``native/fts5_cjk/build.sh``).
...
# next ``clara sessions optimize-storage`` on a capable host).
#
...
def fts5_cjk_so_path() -> Path:
    ...
    env = os.getenv("CLARA_FTS5_CJK_SO")
    if env:
        return Path(env).expanduser()
    return get_clara_home() / "lib" / "libfts5_cjk.so"


def _cjk_fts_config_enabled() -> bool:
    """config.yaml ``sessions.cjk_fts`` (default on), via its env bridge."""
    return os.getenv("CLARA_CJK_FTS", "1").strip().lower() not in (
        "0", "false", "off", "no",
    )
...
_STATE_DB_CORRUPT_MSG = (
    ...
    "Stop the gateway, then run `clara sessions recover --source <state.db> "
...
def divert_session_transcript_jsonl(session_id: str, messages) -> "Optional[Path]":
    """Append pending messages as JSON lines under CLARA_HOME/sessions.
...
# (see clara_cli/sqlite_safe_read.py for the module built around this rule).
...
#
# The registry itself lives in clara_state_registry.py — a bounded
# module owning acquisition, generation identity, refcounting,
# retirement, and teardown.  These re-exports keep the historical
# import path (``from clara_state import get_shared_session_db``)
# working for every call site and test that imports from here.
#
# Routing rules (see clara_state_registry for the full lifecycle):
...
from clara_state_registry import (  # noqa: F401  (re-export)
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    (scaffold/embed installs that ship clara_state without clara_cli). A
    ...
    try:
        ...
        logger.debug(
            "clara_cli.sqlite_safe_read unavailable; opening %s untracked "
    ...
    # patch clara_state.sqlite3.connect keep control of connection creation;
...
def is_zeroed_state_db(
    ...
    Prefer ``clara_cli.backup.is_zeroed_sqlite_file`` when available; this
    ...
    if size < 0:
        return False
    from clara_cli.sqlite_safe_read import has_live_connection, read_header_bytes_preopen
    ...
    with quarantine_cross_process_lock(path) as acquired:
        if not acquired:
            logger.error(
                ...
                "state-snapshots via `clara snapshot list` / "
                "`clara snapshot restore <id>`.",
                path,
            )
            return None
        return _do_quarantine()
...
# ── Read-only health/stats probes (clara doctor, dashboards) ──────────
...
_CLARA_CMDLINE_MARKERS = ("clara_cli.main", "clara_cli/main", "clara serve",
                           "clara-agent", "clara gateway", "clara chat")


def _looks_like_clara(cmdline: str) -> bool:
    """Heuristic: does this cmdline look like a Clara process?
    ...
    We only flag processes that look like Clara, not every system daemon.
    """
    lower = cmdline.lower()
    return any(marker in lower for marker in _CLARA_CMDLINE_MARKERS)
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    # With multiple clara processes (gateway + CLI sessions + worktree agents)
    ...
    # legitimately held for multi-second stretches by sibling Clara
    ...
    # `clara update` leaves mixed-version processes sharing the DB until
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        try:
            ...
            def _handle_quarantine_if_zeroed(already_locked: bool = False):
                if (
                    ...
                    msg = (
                        ...
                        f"Restore from {snaps} via `clara snapshot list` / "
                        f"`clara snapshot restore <id>` if available. "
            ...
            # NOTE: the v23 FTS optimization is OPT-IN (`clara db optimize`),
            ...
            # ``clara_state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `clara update` to rebuild the venv with a "
                ...
                if cjk_present:
                    ...
                    if live:
                        ...
                        logger.warning(
                            ...
                            "run `clara sessions optimize-storage` on a host "
        ...
        while True:
            ...
            try:
                ...
                if "locked" in err_msg or "busy" in err_msg:
                    ...
                    raise sqlite3.OperationalError(
                        f"database is locked (another Clara process held the "
    ...
    def _db_file_was_replaced(self) -> bool:
        ...
        if recorded_app:
            ...
            # replace. A copied Clara DB that minted its own id is nonzero.
    ...
    def _halt_db_corrupt(self, exc: BaseException) -> None:
        ...
        logger.error(
            ...
            "gateway and run `clara sessions recover --source %s "
            "--inspect-only`.",
            self.db_path,
            exc,
            self.db_path,
            ...
            try:
                ...
                for pid_str in os.listdir("/proc"):
                    ...
                    try:
                        ...
                        # so check whether this is a Clara process —
                        # only flag uninspectable holders that look like
                        # another Clara instance, not every system daemon.
                        cmdline = _read_proc_cmdline(pid)
                        if cmdline is not None and _looks_like_clara(cmdline):
    ...
    def _reap_inactive_orphan_desktop_holders(
        ...
        if not sys.platform.startswith("linux") or psutil is None:
            return []
        try:
            from clara_cli.dashboard_procs import _is_ephemeral_port_zero_backend
        except Exception:
            return []
    ...
    def close(self):
        ...
        if getattr(self, "_shared_registry_owned", False):
            from clara_state_registry import release
            ...
            if self._conn:
                if self._db_corrupt:
                    ...
                    logger.warning(
                        ...
                        "then run `clara sessions recover --source %s "
                        "--inspect-only`.",
                        self.db_path,
                        self._db_corrupt_reason,
                        self.db_path,
    ...
    #
    # `optimize_fts_storage()` (the `clara sessions optimize-storage`
    ...
    # ── Opt-in v23 FTS storage optimization (`clara sessions optimize-storage`) ──
    #
    ...
    def _own_profile_name(self) -> Optional[str]:
        ...
        try:
            ...
            root = get_default_clara_root().resolve()
    ...
    # offline repair path behind ``clara sessions repair-routing``.
    ...
    def set_session_yolo(self, session_id: str, enabled: bool) -> None:
        ...
        ``--yolo`` launch — survives ``clara --resume`` into a fresh
    ...
    # of truth: clara_state_common._RECOVERABLE_END_REASONS, interpolated
    ...
    def _rows_to_conversation(
        ...
        *,
        ...
        for row in rows:
            ...
            # that replay reasoning (OpenRouter, OpenAI, Clara) receive
        ...
        if repair_alternation and messages:
            # Lazy import: clara_state already depends on agent.* (see
    ...
    def get_conversation_root(self, session_id: str) -> str:
        ...
        identifier — used for Clara Portal ``conversation=`` usage tagging.
    ...
    def search_sessions(
        ...
        ``clara -c``/``--resume`` so the "last" session is the last one in
    ...
    def latest_conversation_boundary(
        ...
        uses — never the key alone.  ``X-Clara-Session-Key`` accepts any
    ...
    def delete_session_if_empty(
        ...
        and ``clara sessions list`` output. (Pattern ported from
    ...
    def retag_kanban_worker_sessions(self, workspaces_root: str) -> int:
        ...
        Workers used to spawn without ``CLARA_SESSION_SOURCE``, so their runs
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Clara, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Clara session.

        A Clara session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Clara session is already bound to any Telegram DM topic.
    ...
    def vacuum(self) -> int:
        ...
        with self._lock:
            ...
            # TRUNCATE: a manual `clara sessions vacuum` runs in a transient
    ...
    def reclaim_stale_running_handoffs(self, error: str) -> List[str]:
        ...
        Such a row is unrecoverable *and* poisoclara: ``request_handoff`` only
        ...
        try:
            ...
            # leaves poisoclara 'running' rows in place (sessions that can
