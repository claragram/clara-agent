# Clara Agent Security Policy

This document describes Clara Agent's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/claraship/clara-agent/security/advisories/new)
or **hey@claraship.com**. Do not open public issues for
...
- Environment details (`clara --version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Clara Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Clara Agent's documentation
...
Clara Agent supports two OS-level isolation postures. They address
...
Clara Agent supports this in two ways:

- **Clara Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Clara Agent's in-process heuristics
...
Clara Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Clara Agent
itself. Bugs in Clara Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Clara Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Clara Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Clara Agent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  clara-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Clara Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Clara Agent. Bugs in Clara
...
- Clara Agent includes supply-chain guards for MCP server
