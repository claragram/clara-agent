 ...
 *
 * The root ``package.json`` is installed by ``clara update`` on every user,
 ...
 *   workspace graph entirely. ``clara update`` and ``clara doctor --fix``
 ...
 *   ``clara tools`` → Browser Automation → Camofox, and only used at
 *   runtime when ``CAMOFOX_URL`` is set. Its postinstall fetches a ~300MB
 *   Firefox-fork binary, which silently blocked ``clara update`` for
 ...
 * fails — read the lazy-install guidance in the ``clara-agent-dev`` skill
...
test('camofox is not in root dependencies (must stay opt-in)', () => {
  ...
  assert.ok(
    ...
    'Camofox is a ~300MB binary-postinstall backend that must stay ' +
      ...
      'Camofox post_setup handler in clara_cli/tools_config.py so it ' +
      'only installs when the user explicitly selects Camofox via ' +
      '`clara tools` → Browser Automation → Camofox.'
  )
})
