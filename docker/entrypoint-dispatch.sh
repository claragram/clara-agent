...
if [ "$$" -eq 1 ]; then
    exec /init /opt/clara/docker/main-wrapper.sh "$@"
fi

echo "[clara] WARNING: container entrypoint is not PID 1; skipping s6-overlay /init and falling back to direct bootstrap. Supervised services are unavailable in this runtime, but the requested command will still run." >&2
...
/opt/clara/docker/stage2-hook.sh
exec /opt/clara/docker/main-wrapper.sh "$@"
