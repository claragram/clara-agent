...
# Clara WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: CLARA_TINI_SHIM_TARGET overrides the `/init` path (and
# CLARA_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${CLARA_TINI_SHIM_TARGET:-/init}"
WRAPPER="${CLARA_TINI_SHIM_WRAPPER:-/opt/clara/docker/main-wrapper.sh}"
