#!/bin/sh
# shellcheck shell=sh
# /opt/clara/docker/main-wrapper.sh — wraps the container's CMD with
...
# with-contenv before touching CLARA_HOME / PATH. On the non-PID-1
# fallback path the Dockerfile env is still intact, so we skip the
# re-exec and continue directly.
#
# Routing:
#   no args                       → exec `clara` (the default)
#   first arg is an executable    → exec it directly (sleep, bash, sh, …)
#   first arg is anything else    → exec `clara <args>` (subcommand passthrough)
#
# Drop to clara via s6-setuidgid, but skip it when already non-root.
set -e

if [ -z "${CLARA_MAIN_WRAPPER_ENV_READY:-}" ] && \
   [ -z "${CLARA_HOME:-}" ] && \
   [ -x /command/with-contenv ]; then
    export CLARA_MAIN_WRAPPER_ENV_READY=1
    exec /command/with-contenv sh "$0" "$@"
fi
unset CLARA_MAIN_WRAPPER_ENV_READY

drop() { [ "$(id -u)" = 0 ] && set -- s6-setuidgid clara "$@"; exec "$@"; }
...
# an arbitrary non-root, non-clara UID, the bootstrap was skipped and the
# baked image dirs (owned by the clara build UID) are unwritable, so fail
...
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u clara)" ]; then
    cat >&2 <<EOF
[clara] ERROR: container started with --user $cur_uid (an arbitrary, non-clara UID) — not supported.
...
# to the clara user's home before dropping privileges so libraries that
...
_clara_orig_cwd="${CLARA_ORIG_CWD:-$PWD}"
...
# the user's command so `clara chat` starts in the Docker -w
