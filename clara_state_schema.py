...
``clara_state.SessionDB``. It defines no ``__init__`` and no state of its
...
``SessionDB.__init__``. It must never import clara_state (cycle) — shared
module-level constants live in clara_state_common.
"""
...
# Moved methods logged under the "clara_state" logger before the split;
# keep that logger identity so log filtering/capture behavior is unchanged.
logger = logging.getLogger("clara_state")
...
def schema_read_probe_statements() -> tuple:
    ...
    ``clara_cli/web_server.py``) run these probes right after a read-only
    ...
    served an empty session list after `clara update` until the user's
...
class SessionSchemaMixin:
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._clara_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._clara_fts5_probe")
            return True
        except sqlite3.OperationalError as exc:
            ...
            if escalated:
                ...
                if foreign_holders:
                    logger.error(
                        ...
                        "`clara sessions optimize-storage` with the gateway stopped. "
                        "`clara doctor` reports this degraded state.",
                        attempts,
                        foreign_holders,
                    )
    ...
    def _parse_schema_columns(schema_sql: str) -> Dict[str, Dict[str, str]]:
        ...
        try:
            from clara_constants import get_clara_home
            cache_path = get_clara_home() / "cache" / "schema_columns.json"
    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 23:
                ...
                # optimization is available. `clara sessions optimize-storage`
        ...
        if fts5_available:
            ...
            # not yet opted into `clara db optimize`) must keep its EXISTING
