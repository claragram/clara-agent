...
(the updater, `clara serve`/dashboard, the Desktop app) currently discovers
...
- ``identify`` → pid, profile label, clara_home, code_sha/code_version
...
- POSIX: Unix domain socket at ``$CLARA_HOME/gateway.sock``. When the home
  path is too long for ``sun_path`` (~104 bytes on macOS/BSD), the socket is
  bound in the system temp dir and a pointer file
  ``$CLARA_HOME/gateway.sock.path`` records the real location; clients
  follow the pointer transparently.
- Windows: named pipe ``\\\\.\\pipe\\clara-gateway-<home-hash>`` served via
...
Consumers (``clara update --plan`` inventory, the post-update fleet version
...
def windows_pipe_name(home: Path) -> str:
    """Per-CLARA_HOME named pipe path (Windows transport)."""
    return rf"\\.\pipe\clara-gateway-{_home_hash(home)}"
...
def _fallback_socket_path(home: Path) -> Path:
    ...
    """
    name = f"clara-gw-{_home_hash(home)}.sock"
...
def _detect_supervisor() -> str:
    ...
    if sys.platform == "darwin" and (
        env.get("XPC_SERVICE_NAME", "").startswith("ai.clara")
        or env.get("LAUNCHD_SOCKET")
    ):
        return "launchd"
    if env.get("CLARA_DESKTOP_MANAGED"):
        return "desktop"
...
def build_identify_payload() -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "clara_home": record.get("clara_home"),
        "profile": _profile_label_for_home(record.get("clara_home") or ""),
...
class GatewayControlServer:
    ...
    its CLARA_HOME), ``stop()`` on shutdown. All failures are non-fatal —
...
# ---------------------------------------------------------------------------
# Client (__PROT_0_synchroclara__ — used by CLI/updater consumers)
# ---------------------------------------------------------------------------
