...
The gateway's ``shutdown_signal_handler`` runs __PROT_1_synchroclaraly__ inside the
...
the async helper, never in the __PROT_0_synchroclara__ probe.
"""
...
def snapshot_shutdown_context(received_signal: Any = None) -> Dict[str, Any]:
    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        ...
        if clara_home_str:
            takeover_path = Path(clara_home_str) / ".gateway-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(clara_home_str) / ".gateway-planned-stop.json"
...
def check_systemd_timing_alignment(
    ...
    upgraded clara-agent but never re-ran ``clara setup`` to regenerate
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../clara-gateway.service"
    ...
    # that's the common case for clara.
...
def parse_systemd_duration_to_us(raw: str) -> Optional[int]:
    ...
    Public: also consumed by clara_cli.gateway's restart-wait sizing.
    """
    if not raw:
        return None
    units = {
