...
2. An event-loop heartbeat file at ``<CLARA_HOME>/state/gateway.heartbeat`` so
...
def _process_clara_home() -> Path:
    """CLARA_HOME for process-level identity files (ignore profile overrides)."""
...
def get_loop_heartbeat_path(home: Optional[Path] = None) -> Path:
    """Return ``<CLARA_HOME>/state/gateway.heartbeat``."""
...
def get_loop_tick_socket_path(
    ...
    ``<CLARA_HOME>/state/gateway.loop-tick.<pid>.sock`` — PID-suffixed so a
...
async def loop_heartbeat_forever(
    ...
    *initiated* by the loop, so a frozen loop still lets the file age.
    ...
    wedged; see ``clara_cli.gateway.probe_gateway_loop_liveness`` for the
