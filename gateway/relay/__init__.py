"""Relay/connector support package for the Clara gateway.
...
def relay_connection_auth() -> tuple[Optional[str], Optional[str]]:
    ...
    Both come from enrollment (``clara gateway enroll`` writes them to
    ``~/.clara/.env``): ``GATEWAY_RELAY_ID`` identifies the enrolled instance,
...
def relay_endpoint() -> Optional[str]:
    ...
    ``CLARA_DASHBOARD_PUBLIC_URL``); a hosted/NAS container has the same var
...
def relay_wake_url() -> Optional[str]:
    ...
    sets it explicitly (or passes ``--wake-url`` to ``clara gateway enroll``).
...
def relay_display_name() -> Optional[str]:
    ...
    if not value:
        try:
            from clara_cli.skin_engine import get_active_skin  # late import: boot-safe
        ...
        # multi-agent scope it would prefix every reply "**Clara Agent:**" —
...
def _resolve_relay_identity_token() -> str:
    ...
    ``clara gateway enroll`` CLI. Three modes, in precedence order:

      1. **Generic OIDC client-credentials** (air-gapped / self-hosted-IdP, NO
         Clara Portal): when ``gateway.idp.token_url`` (or
      ...
      2. **Clara Portal** (default): ``resolve_clara_access_token()`` — existing
    ...
    if not token_url:
        # Mode 2 — Clara Portal (default, unchanged behaviour).
        from clara_cli.auth import resolve_clara_access_token

        return resolve_clara_access_token()
...
def self_provision_relay() -> bool:
    ...
    is already present, AND the agent can resolve its own Clara access token. In
    that case the runtime resolves the agent's own Clara access token (the same
    ``resolve_clara_access_token()`` the enroll CLI / dashboard register use),
    ...
    up. The creds live ONLY in process memory — never written to ``~/.clara/.env``.
    ...
    sets neither ``CLARA_MANAGED`` nor a ``.managed`` marker), so gating on it
    ...
    try:
        ...
        host = ""
    gateway_id = os.environ.get("GATEWAY_RELAY_ID", "").strip() or f"gw-{host or 'clara'}"
