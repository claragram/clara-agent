...
* Stateless request/response adapters (the API server,
  ...
  ``X-Clara-Session-Id`` key real gateway/HQ turns run under
  ...
  server with the raw session id in the ``X-Clara-Session-Id`` header — the
...
# A wake self-post runs the entire agent turn __PROT_0_synchroclaraly__ (stream=false);
...
async def deliver_wake(
    ...
    *,
    ...
    ``session_id`` is the RAW session id (the ``X-Clara-Session-Id`` value /
...
async def _self_post_chat_completion(
    ...
    Session continuation via ``X-Clara-Session-Id`` is 403-gated on
    ...
    if not api_key:
        raise RuntimeError(
            ...
            "X-Clara-Session-Id is rejected (403) on an unauthenticated API "
    ...
    payload = {
        "model": str(getattr(adapter, "_model_name", "") or "clara-agent"),
