...
Module-level run.py helpers a handler needs (``_clara_home``,
...
from clara_cli.config import atomic_config_write, cfg_get, clear_model_endpoint_credentials
from utils import (
    atomic_json_write,
...
def _model_switch_skew_guard() -> Optional[str]:
    ...
    return t(
        ...
        error=(
            ...
            f"crash — restart the gateway to load the new code: clara gateway restart"
        ),
    )
...
class GatewaySlashCommandsMixin:
    ...
    def _typed_command_prefix_for(self, platform) -> str:
        """Return the prefix users can always type to reach Clara commands.
    ...
    async def _handle_reset_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        #
        # _cleanup_agent_resources is __PROT_0_synchroclara__ and can block for a long time
        ...
        try:
            from clara_cli.tips import get_random_tip
    ...
    async def _handle_profile_command(self, event: MessageEvent) -> str:
        ...
        from clara_cli.slash_exec import CommandContext, execute_command
    ...
    async def _handle_kanban_command(self, event: MessageEvent) -> str:
        ...
        import re
        import shlex
        from clara_cli.kanban import run_slash
        ...
        if action in {"pause", "resume"}:
            ...
            if action == "pause":
                ...
                return (
                    ...
                    f"`clara gateway restart` to reset."
                )
            # action == "resume"
            if platform not in failed:
                return (
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        from gateway.run import _clara_home
        ...
        try:
            ...
            await asyncio.to_thread(
                atomic_json_write,
                _clara_home / ".restart_notify.json",
                notify_data,
                indent=None,
            )
        ...
        try:
            ...
            await asyncio.to_thread(
                atomic_json_write,
                _clara_home / ".restart_last_processed.json",
                dedup_data,
                indent=None,
            )
        except Exception as e:
    ...
    async def _handle_version_command(self, event: MessageEvent) -> str:
        """Handle /version — show the running Clara Agent version."""
        from clara_cli.slash_exec import CommandContext, execute_command
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        """
        from gateway.run import _clara_home, _load_gateway_config
        ...
        from clara_cli.providers import get_label
        ...
        # via the shared single-owner parser (clara_cli.model_switch).
        ...
        custom_provs = None
        excluded_provs = []
        config_path = (_command_profile_home or _clara_home) / "config.yaml"
        try:
        ...
        if not model_input and not explicit_provider:
            ...
            if has_picker:
                try:
                    ...
                    # __PROT_1_synchroclara__ urllib HTTP fetch on a stale cache) off the
                ...
                if providers:
                    ...
                    async def _on_model_selected_scoped(
                        ...
                        # can fall through to a __PROT_2_synchroclara__ models.dev HTTP fetch
                        ...
                        try:
                            from clara_cli.context_switch_guard import (
                        ...
                        from clara_cli.model_switch import format_model_for_display
                        ...
                        from clara_cli.model_switch import resolve_display_context_length_async
                        _sw_config_ctx = None
                        _sw_model_cfg = {}
                        try:
        ...
        # through to a __PROT_3_synchroclara__ models.dev HTTP fetch (requests.get, 15s
        ...
        async def _finish_switch() -> str:
            ...
            # Copilot, and Clara-enforced caps win over the raw models.dev entry.
            mi = result.model_info
            from clara_cli.model_switch import resolve_display_context_length_async
    ...
    async def _handle_codex_runtime_command(self, event: MessageEvent) -> str:
        ...
        Same surface as the CLI handler in cli.py:
            ...
            /codex-runtime auto             — Clara default runtime
        ...
        from clara_cli import codex_runtime_switch as crs
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        All resolution/persistence goes through clara_cli.personality —
        the single owner of personality state on every surface.
        """
        from gateway.run import _load_gateway_config
        from clara_cli.personality import (
        ...
        # Persist the selection only — clara_cli.personality never writes
        # agent.system_prompt (user-owned manual overlay). persist_personality
        # writes get_clara_home()/config.yaml, i.e. the routed profile under
    ...
    async def _handle_goal_command(self, event: "MessageEvent") -> str:
        ...
        if lower.startswith("draft"):
            ...
            try:
                import asyncio
                from clara_cli.goals import draft_contract
            ...
            from clara_cli.goals import parse_contract
    ...
    async def _handle_heartbeat_command(self, event: "MessageEvent") -> str:
        ...
        """
        from clara_cli.heartbeat import parse_interval, format_interval, MIN_INTERVAL_SECONDS
        ...
        return (
            ...
            "elapsed. Lives while the gateway runs — use `clara cron` for durable schedules."
        )
    ...
    async def _handle_loop_command(self, event: "MessageEvent") -> str:
        ...
        """
        try:
            from clara_cli.loops import dispatch_loop_command, goal_blocks_loop_tick
        except Exception as exc:
    ...
    async def _handle_diff_command(self, event: MessageEvent) -> str:
        ...
        Clara itself changed. ``--stat`` limits output to the summary.
    ...
    async def _handle_memory_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _gateway_config_home
        from clara_cli.write_approval_commands import handle_pending_subcommand
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        ...
        the pending JSON file under ``~/.clara/pending/skills/``. (Note this is
        the write-approval ``diff <id>``; the CLI also has an unrelated
        ``clara skills diff <name>`` that diffs a bundled skill vs stock.)
        """
        from gateway.run import _gateway_config_home
        from clara_cli.write_approval_commands import handle_pending_subcommand
        from tools import write_approval as wa
        ...
        # the real review surface. (Note: `clara skills diff <name>` is a
        ...
        if args and args[0].lower() == "diff" and len(out) > 3000:
            ...
            out = (out[:3000]
                   + "\n… (truncated — full diff in "
                     f"~/.clara/pending/skills/{pending_id}.json)")
        return out
    ...
    async def _handle_fast_command(self, event: MessageEvent) -> Optional[str]:
        ...
        from clara_cli.models import model_supports_fast_mode
    ...
    async def _handle_approvals_command(self, event: MessageEvent) -> str:
        ...
        from gateway.slash_access import policy_for_source
        from clara_cli.approval_mode import run_approval_mode_command
    ...
    async def _handle_busy_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        """Handle /busy — control what happens when messaging while Clara is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            ...
            if arg == "queue":
                behavior = "Messages will be queued for the next turn while Clara is busy."
            elif arg == "steer":
                behavior = "Messages will be steered into the current run (after the next tool call)."
            else:
                behavior = "Messages will interrupt the current run while Clara is busy."
            return EphemeralReply(
    ...
    async def _handle_save_command(self, event: MessageEvent) -> str:
        ...
        """
        from clara_cli.session_export import (
            SAVE_USAGE,
        ...
        if redact:
            from clara_cli.session_export_md import redact_session_data

            export_data = redact_session_data(export_data)

        import tempfile

        temp_dir = tempfile.mkdtemp(prefix="clara_save_")
    ...
    async def _handle_sessions_command(self, event: MessageEvent) -> str:
        ...
        from clara_cli.session_listing import (
    ...
    async def _handle_branch_command(self, event: MessageEvent) -> str:
        ...
        try:
            await self._session_db.create_session(
                ...
                # path (clara_state.py:1994-2009) that searches by the complete
    ...
    async def _handle_topup_command(self, event: MessageEvent) -> str:
        """Handle /topup -- show the Clara balance and hand off to the portal.
    ...
    async def _handle_usage_command(self, event: MessageEvent) -> str:
        ...
        # ── Clara credits magnitudes + monthly-grant % gauge ─────────────
        # Shared with the CLI / TUI /usage block via clara_credits_lines(): a single
        # auth-gate + portal-fetch + render path (which also honors the dev fixture).
        # Run off the event loop. The helper gates on "a Clara account is logged in"
        # — NOT the inference provider and NOT nested under `if provider:` — so a
        # Clara-credentialled user running inference elsewhere (or with none resident)
        ...
        try:
            ...
            credits_lines = await asyncio.to_thread(clara_credits_lines, markdown=True)
        except Exception:
    ...
    async def _handle_insights_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            # with no explicit path resolves ``get_clara_home()`` at call
    ...
    async def _handle_reload_skills_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            # get_clara_home()/skills, a contextvar override under multiplex.
    ...
    async def _handle_bundles_command(self, event: MessageEvent) -> str:
        ...
        if not bundles:
            return (
                ...
                "  `clara bundles create <name> --skill <s1> --skill <s2>`\n"
    ...
    async def _handle_approve_command(self, event: MessageEvent) -> Optional[str]:
        ...
        flow as the CLI's __PROT_4_synchroclara__ input() approval.
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        ...
        full log uploads should use ``clara debug share`` from the CLI.
        """
        import asyncio
        ...
        # profile's logs/config off ``get_clara_home()`` and uploads them to a
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update Clara Agent to the latest version.

        Spawns ``clara update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``clara update`` may trigger. Marker
        files are written so either the current gateway process or the next one
        can notify the user when the update finishes.
        """
        from gateway.run import _clara_home, _resolve_clara_bin
        import json
        import shutil
        import subprocess
        from datetime import datetime
        from clara_cli.config import is_managed, format_managed_message
        ...
        if is_managed():
            return f"✗ {format_managed_message('update Clara Agent')}"
        ...
        if not clara_cmd:
            return t("gateway.update.clara_cmd_not_found")
        ...
        # Spawn `clara update --gateway` detached so it survives gateway restart.
        ...
        # gateway can stream it to the messenger in near-real-time.
        #
        # Windows: no bash/setsid chain.  Run `clara update --gateway`
        ...
        try:
            if sys.platform == "win32":
                ...
                # than through clara_cmd (venv\Scripts\clara.exe): the shim
                ...
                clara_cmd_str = " ".join(shlex.quote(part) for part in clara_cmd)
                update_cmd = (
                    f"PYTHONUNBUFFERED=1 {clara_cmd_str} update --gateway"
