...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.clara to the active profile home."""
    ...
    if raw == "~/.clara":
        return clara_home
    if raw.startswith("~/.clara/"):
        return clara_home / raw.removeprefix("~/.clara/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return clara_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under CLARA_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from clara_constants import get_clara_home

    scripts_root = (get_clara_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.clara" or raw_text.startswith("~/.clara/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__clara_ignore__") is True
        ):
            return False, None
