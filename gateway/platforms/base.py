...
# Audio file extensions Clara recognizes for native audio delivery.
# Keep Telegram's narrower attachment/voice sets below separate: formats such
# as MPEG-2 Layer II are audio to Clara but unsupported by sendAudio/sendVoice.
_AUDIO_MIME_TYPES = {
    ".ogg": "audio/ogg",
    ".opus": "audio/opus",
    ".mp3": "audio/mpeg",
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Clara' DM-topic helper
    ...
    # Routed Clara profile for shared state.db namespaces (topic bindings
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Clara-created Telegram private-chat
    ...
    if (
        ...
        and raw_message.get("_clara_no_thread_response")
    ):
...
def build_auto_tts_output_path(platform) -> str:
    ...
    TTS tool's ``CLARA_SESSION_PLATFORM`` contextvar — that contextvar is
...
def gateway_trust_env() -> bool:
    ...
    """
    try:
        from clara_cli.config import load_config_readonly as _load_config
...
from clara_constants import get_default_clara_root, get_clara_dir, get_clara_home
...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.clara/.env manually."
)
...
IMAGE_CACHE_DIR = get_clara_dir("cache/images", "image_cache")


def _resolve_cache_dir(constant_name: str, new_subpath: str, old_name: str) -> Path:
    """Resolve fresh via get_clara_dir (active profile), unless a test has
    monkeypatched the constant away from its import-time default."""
    fresh = get_clara_dir(new_subpath, old_name)
        ...
        for attempt in range(retries + 1):
            try:
                async with client.stream(
                    ...
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; ClaraAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                ) as response:
...
AUDIO_CACHE_DIR = get_clara_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_clara_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_clara_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_clara_dir("cache/screenshots", "browser_screenshots")
...
_CLARA_ROOT = get_default_clara_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "CLARA_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "CLARA_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "CLARA_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "CLARA_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _CLARA_HOME / "image_cache",
    _CLARA_HOME / "audio_cache",
    _CLARA_HOME / "video_cache",
    _CLARA_HOME / "document_cache",
    _CLARA_HOME / "browser_screenshots",
    ...
    _CLARA_HOME / "cache" / "images",
    _CLARA_HOME / "cache" / "audio",
    _CLARA_HOME / "cache" / "videos",
    _CLARA_HOME / "cache" / "documents",
    _CLARA_HOME / "cache" / "screenshots",
)
...
def _profile_cache_roots() -> List[Path]:
    """Return per-profile canonical cache roots under the shared Clara root.

    Profile gateways write generated artifacts to
    ``<root>/profiles/<name>/cache/{images,audio,...}``. The static safe-roots
    list only covers the *active* CLARA_HOME's cache, so a gateway running at
    the root (e.g. ``CLARA_HOME=/opt/data``) while the model emits a
    ...
    is consulted (which otherwise wins when CLARA_HOME is symlinked under a
    denied prefix and $HOME is not that prefix). See issue #31733.
    """
    roots: List[Path] = []
    profiles_dir = _CLARA_ROOT / "profiles"
    try:
...
def _kanban_attachment_roots() -> List[Path]:
    ...
    home_override = os.environ.get("CLARA_KANBAN_HOME", "").strip()
    root = Path(home_override).expanduser() if home_override else _CLARA_ROOT
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Clara profile and shared Clara root both contain control
    ...
    # CLARA_HOME root. The set mirrors the canonical read guard in
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.clara
    ...
    for clara_root in (_CLARA_HOME, _CLARA_ROOT):
        for rel in _ROOT_CREDENTIAL_FILES:
            denied.append(clara_root / rel)
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Clara
    secrets (``~/.clara/.env``, ``auth.json``) are *separate, more-specific*
...
def _cache_dir_container_mounts() -> List[Tuple[Path, Path]]:
    """(host, container) pairs for the auto-mounted Clara cache dirs.

    The agent legitimately sees generated artifacts at ``/root/.clara/...``
...
def _translate_docker_container_media_path(candidate: Path, session_key: str = "") -> Optional[Path]:
    ...
    auto-mounted Clara cache dirs (``/root/.clara/...``), the session's
    ...
    # In-process gateways (Desktop backend, `clara serve`) may not have
    ...
    # are longer prefixes, so /root/.clara/... still translates to the host
    # cache — this only catches stray home writes like /root/out.png.
    if not any(c.as_posix() == "/root" for _, c in mounts):
        # /root/.clara/* that did NOT match a cache mount is the container's
        ...
        if not candidate.as_posix().startswith("/root/.clara"):
...
def validate_media_delivery_path(path: str, session_key: str = "") -> Optional[str]:
    ...
    ``CLARA_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Clara-managed cache, under an operator-allowlisted root
    (``CLARA_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # credential/secret stores under the Clara root (~/.clara/.env,
    ...
    # ``MEDIA:~/.clara/google_token.json``) remain rejected.
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?clara\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+clara[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    # into the ``CLARA_SESSION_ASYNC_DELIVERY`` contextvar at session-bind
    ...
    # Clara commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        # Cross-CLARA_HOME token takeover is armed by GatewayRunner only for
        ...
        self._busy_text_mode: str = (
            os.environ.get("CLARA_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "CLARA_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "CLARA_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: Clara sends
    ...
    def _acquire_platform_lock(self, scope: str, identity: str, resource_desc: str) -> bool:
        ...
        A live cross-CLARA_HOME holder may be replaced only when the runner
        ...
        if owner_profile:
            ...
            remedy = (
                ...
                f"(clara --profile {owner_profile} gateway stop)."
            )
        else:
            holder = f" (PID {owner_pid})" if owner_pid else ""
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        host filesystem path that would leak the Clara home layout.
        """
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        would leak the Clara home layout.
        """
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.clara/media/generated/stale.png"}
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        # pattern — set the guard __PROT_1_synchroclaraly__, not inside the task.)
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          CLARA_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          CLARA_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          CLARA_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("CLARA_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("CLARA_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("CLARA_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
        ...
        try:
            ...
            if response:
                ...
                if not is_ephemeral_response:
                    ...
                    # paths auto-detected above.  Run that __PROT_0_synchroclara__ DB/decode
                ...
                if (self._should_auto_tts_for_chat(event.source.chat_id)
                        ...
                        and not self._streaming_tts_turn_completed(
                            session_key,
                            getattr(interrupt_event, "_clara_run_generation", None),
                            event=event,
                        )):
                    try:
                        ...
                        if check_tts_requirements():
                            ...
                            # CLARA_SESSION_PLATFORM contextvar the tool would
            ...
            # not at the top of this task.  _clara_run_generation is set on
    ...
    def build_source(
        ...
        per-profile CLARA_HOME isolation.
        """
