...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *Clara Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to CLARA_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/clara), this function
    mirrors the bridge source to a writable CLARA_HOME location and returns that
    ...
    # Try CLARA_HOME location first
    clara_home = get_clara_home()
    clara_home_bridge = clara_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to CLARA_HOME if needed
    if clara_home_bridge.exists():
        return clara_home_bridge

    # Mirror the bridge source to CLARA_HOME
    try:
        clara_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            clara_home_bridge,
            dirs_exist_ok=False,
