...
There are two ways to add a platform to the Clara gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.clara/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Clara code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `clara gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `clara_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Clara core.
...
And add it to the `clara-gateway` composite:

```python
"clara-gateway": {
    "includes": [..., "clara-your-platform"]
}
```
...
## 12. Status Display (`clara_cli/status.py`)
...
## 13. Gateway Setup Wizard (`clara_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ clara_cli/ toolsets.py \
  --include="*.py" -l | sort -u
