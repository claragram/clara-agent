...
class QQAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # to ~/.clara/.update_response. Set here so the cross-adapter gateway
    ...
    async def _open_ws(self, gateway_url: str) -> None:
        ...
        # Honor WSL proxy env for QQ WebSocket. Clara upgrades overwrite this
    ...
    def _create_task(coro):
        ...
        ``_dispatch_payload`` __PROT_0_synchroclaraly__ outside of ``asyncio.run()``.
        """
        try:
    ...
    def _dispatch_payload(self, payload: Dict[str, Any]) -> None:
        """Route inbound WebSocket payloads (dispatch __PROT_1_synchroclaraly__, spawn async handlers)."""
        op = payload.get("op")
        t = payload.get("t")
        s = payload.get("s")
        d = payload.get("d")
    ...
    async def _default_interaction_dispatch(
        ...
        - ``update_prompt:<answer>`` →
          writes the answer to ``~/.clara/.update_response`` for the
          detached ``clara update --gateway`` process to consume.
    ...
    def _write_update_response(answer: str, operator: str = "") -> None:
        ...
        ``clara update --gateway`` watcher polls this file for a ``y``/``n``
    ...
    def _resolve_stt_config(self) -> Optional[Dict[str, str]]:
        ...
        # 2. QQ-specific env vars (set by `clara setup gateway` / `clara gateway`)
    ...
    async def send_update_prompt(
        ...
        ``gateway/run.py``'s ``clara update --gateway`` watcher. Button
        ...
        ``~/.clara/.update_response`` so the detached update process
        can read it.
        """
