...
Defenses 1 and 2 (the ``_CLARA_GATEWAY`` guard on ``clara gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.clara.gateway")``,
...
State lives in ``<CLARA_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_clara_home() / "gateway" / "restart_loop.json"
