...
def _normalize_multiplex_profile_allowlist(value: Any) -> Optional[List[str]]:
    ...
    from clara_cli.profiles import normalize_profile_name, validate_profile_name
...
# dashboard's pre-write mutation validation (clara_cli/web_server.py) so
...
class PlatformConfig:
    ...
    # "Clara is thinking…"). Platforms with textless indicators (Discord,
...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_clara_home() / "sessions")
    ...
    # gateway behaves exactly as before — single CLARA_HOME, no profile stamping.
    ...
    # advertise a route. CLARA_ROOM_LINK_URL remains the operator override.
    ...
    # reconnect doing __PROT_0_synchroclara__ socket I/O during a network blip — so a
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        # set to a recognized value. Hosted deployments (Clara Portal / Fly) stamp
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.clara/config.yaml (primary user-facing config)
    3. ~/.clara/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_clara_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through clara_cli.config.load_config, so without this a managed
            ...
            # (e.g. via `clara config set gateway.<key> ...`, which naturally
            ...
            if isinstance(gateway_section, dict):
                if "multiplex_profiles" in gateway_section and "multiplex_profiles" not in gw_data:
                    # gateway.multiplex_profiles written by `clara config set gateway.multiplex_profiles true`
            ...
            if not isinstance(streaming_cfg, dict) and isinstance(gateway_section, dict):
                ...
                # ``clara config set gateway.streaming.*``
