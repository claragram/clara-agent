...
- Clone: `git clone https://github.com/getitcheappro/image-size-patched.git`
- Requirements: Node.js 16 or newer and Corepack/Yarn 4.
- Install: run `corepack enable`, then `yarn install --immutable`.
- Validate: run `yarn lint`, `yarn test`, and `yarn build`.
- Environment variables: none. No `.env` file, database, API key, or external
  service is required.
- Local ports/URLs: none; this repository builds a library and does not start a
  web server.
- Troubleshooting: if Corepack cannot download Yarn, verify network access to
  ...
  GitHub and do not use `--ignore-scripts`, because the pinned source commit is
