...
batch (tracker: [#77056](https://github.com/claraprise/clara-agent/issues/77056)).
...
- **Two arms, one variable.** `baseline` and `fixes` runs differ ONLY by
  ...
  Clara home, same model, same tasks, same reps.
...
1. Create a dedicated Clara home with credentials for the models under test:
   ...
   integration at it; no Clara observability plugin needs to be enabled.
