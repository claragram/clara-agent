...
measured in the production session DB) through `clara chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    CLARA_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured Clara home with credentials for the
...
(tracker: claraship/clara-agent#77056).
"""
import json
import os
import re
...
""".strip(),
            ...
            env.update({
                ...
                "CLARA_HOME": str(HOME),
                "CLARA_NEMO_RELAY_PLUGINS_TOML": str(relay_config),
            })
            q = TASKS[name].replace("{WORK}", str(work))
            t0 = time.time()
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "clara_cli.main", "chat", "--query", q,
