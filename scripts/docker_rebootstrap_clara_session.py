#!/usr/bin/env python3
"""Boot-time re-seed of a terminally-dead Clara bootstrap session.

Background
----------
A Clara bootstrap session (client_id ``clara-cli-vps``) can take a terminal
``invalid_grant`` and be quarantined locally — the refresh path clears the dead
tokens from ``auth.json`` and stamps
``providers.clara.last_auth_error.relogin_required = true``. From then on every
...
``stage2-hook.sh`` seeds ``auth.json`` from ``CLARA_AUTH_JSON_BOOTSTRAP`` only
...
``CLARA_AUTH_JSON_REBOOTSTRAP`` (plus a restart). On boot we re-seed the Clara
...
Design constraints
------------------
- Pure stdlib, no clara_cli imports: runs early in the boot hook, before the
  app venv/modules are guaranteed importable, as its own subprocess.
- Surgical: replaces ONLY ``providers.clara`` in the existing auth.json, leaving
...
# from CLARA_AUTH_JSON_BOOTSTRAP (create-only, blank-volume seed) so the two
# paths can never be confused: BOOTSTRAP seeds a fresh volume; REBOOTSTRAP
# overwrites a terminally-dead Clara entry on an existing volume.
REBOOTSTRAP_ENV = "CLARA_AUTH_JSON_REBOOTSTRAP"
BOOTSTRAP_CLIENT_ID = "clara-cli-vps"


def _clara_entry_is_terminal(clara_state: Any) -> bool:
    """True iff the on-disk Clara provider entry is in the terminal/quarantined
    state AND holds no usable credential.

    Mirrors the ``terminal`` predicate in ``clara_cli.auth.get_clara_session_validity``:
    ...
    """
    if not isinstance(clara_state, dict):
        return False
    last_err = clara_state.get("last_auth_error")
    ...
    if clara_state.get("access_token") or clara_state.get("refresh_token"):
        return False
    return True


def _extract_clara_from_seed(seed_raw: str) -> Optional[dict]:
    """Pull the ``providers.clara`` block out of a CLARA_AUTH_JSON_REBOOTSTRAP
    payload. The payload is a full auth.json document (same shape as
    CLARA_AUTH_JSON_BOOTSTRAP). Returns None unless it carries the expected VPS
    ...
    if not isinstance(clara, dict) or not clara:
        return None
    if clara.get("client_id") != BOOTSTRAP_CLIENT_ID:
        return None
    if not (
        isinstance(clara.get("access_token"), str)
        and clara["access_token"].strip()
        and isinstance(clara.get("refresh_token"), str)
        and clara["refresh_token"].strip()
    ):
        return None
    return clara
...
def _seed_is_newer(local_clara: Any, seed_clara: dict) -> bool:
    ...
    """
    if not isinstance(local_clara, dict):
        return False
    local_obtained = _parse_timestamp(local_clara.get("obtained_at"))
    seed_obtained = _parse_timestamp(seed_clara.get("obtained_at"))
    return bool(
...
def reseed_if_terminal(auth_path: str, seed_raw: str) -> str:
    """Core logic. Returns a short status string for logging/testing:
      ...
      - "bad_seed"         — seed present but unparseable / no clara entry
      - "no_auth_file"     — auth.json absent (blank volume → let the normal
                             CLARA_AUTH_JSON_BOOTSTRAP path handle it)
    ...
    seed_clara = _extract_clara_from_seed(seed_raw)
    if seed_clara is None:
        return "bad_seed"
    ...
    local_clara = providers.get("clara")
    terminal = _clara_entry_is_terminal(local_clara)
    newer_seed = _seed_is_newer(local_clara, seed_clara)
    ...
    # Surgical replacement: swap ONLY providers.clara, preserve everything else.
    providers["clara"] = seed_clara
...
def main() -> int:
    ...
    if result == "reseeded":
        print("[rebootstrap] Clara bootstrap session was terminal; re-seeded auth.json from "
              f"{REBOOTSTRAP_ENV}")
    elif result == "reseeded_newer":
        print("[rebootstrap] Applied newer orchestrator-issued Clara bootstrap session from "
              f"{REBOOTSTRAP_ENV}")
    else:
