# Behavioral test for install.ps1's clara launcher staging (PR #92092,
...
#
# The test lifts the real Install-ClaraCommandLaunchers function from the
...
# the caller (Set-PathVariable passes $ClaraHome\bin -- the managed binary
...
$fn = $ast.Find({
    ...
    $n.Name -eq 'Install-ClaraCommandLaunchers'
}, $true)

if (-not $fn) {
    throw "Install-ClaraCommandLaunchers not found in $installPs1"
}
...
$caseRoot = [System.IO.Path]::GetFullPath((Join-Path $tempBase (
    'clara-cli-launcher-test-' + [guid]::NewGuid().ToString('N')
)))
...
try {
    $installRoot = Join-Path $caseRoot 'clara-agent'
    ...
    $missingThrew = $false
    try {
        Install-ClaraCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    } catch {
        $missingThrew = $_.Exception.Message -like '*required launcher not found*'
    }
    Assert-True $missingThrew 'missing clara.exe fails the launcher stage'
    ...
    $claraV1 = [byte[]](77, 90, 1)
    $claraV2 = [byte[]](77, 90, 2)
    $acp = [byte[]](77, 90, 3)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'clara.exe'), $claraV1)
    Set-Content -Path (Join-Path $installRoot 'venv\pyvenv.cfg') `
        -Value "home = X" -Encoding Ascii

    $staged = Install-ClaraCommandLaunchers -Root $installRoot -Destination $binDir
    Assert-True ($staged -eq $binDir) 'returns the destination it staged into'
    Assert-BytesEqual $claraV1 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'clara.exe'))) `
        'normal venv: exe copy lands in the destination'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'clara-acp.exe'))) `
        'optional ACP launcher may be absent'

    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'clara.exe'), $claraV2)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'clara-acp.exe'), $acp)
    Install-ClaraCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-BytesEqual $claraV2 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'clara.exe'))) `
        'installer refreshes an existing Clara launcher'
    Assert-BytesEqual $acp `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'clara-acp.exe'))) `
    ...
    Install-ClaraCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-True (Test-Path -LiteralPath (Join-Path $binDir 'clara.cmd')) `
        'relocatable venv: .cmd delegator staged'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'clara.exe'))) `
        'relocatable venv: stale exe copy removed'
    $cmdBody = [System.IO.File]::ReadAllText((Join-Path $binDir 'clara.cmd'))
    Assert-True ($cmdBody.Contains((Join-Path $scriptsDir 'clara.exe')) -and $cmdBody.Contains('%*')) `
