...
# tree generates llms.txt — the index every LLM (Clara included, via the
# clara-agent skill) reads to learn what Clara can do — and
...
_MCP_CATALOG_FILES = {"clara_cli/mcp_catalog.py"}
...
# Rust crates — currently just the Tauri bootstrap installer (Clara-Setup).
...
def _py_test_only(p: str) -> bool:
    ...
    Product jobs (Desktop E2E's ``clara serve`` backend, the Docker image)
