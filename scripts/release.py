...
VERSION_FILE = REPO_ROOT / "clara_cli" / "__init__.py"
...
LEGACY_AUTHOR_MAP = {
    ...
    "pi@clara.local": "Elektrofussel",  # PR #61675 salvage (honcho: defaultHost + private-range local URL detection)
    ...
    "75556242+webtecnica@users.noreply.github.com": "webtecnica",  # PR #63360 salvage (clara: restore inference-api base_url)
    ...
    "kavi@local.clara": "kavioavio",  # Issue #46544 / PR #47705 evolution (smart DENY exact-operation owner override)
    ...
    "sonxi@clara.local": "17324393074",  # PR #53196 salvage (tools_config: known_plugin_toolsets null guard; commit under unlinked local identity)
    ...
    "sunsky.lau@gmail.com": "liuhao1024",  # PR #56993 salvage (gateway: process-level CLARA_HOME for pid/lock/status identity files; #56986)
    ...
    "wyuebei@gmail.com": "wyuebei-cloud",  # PR #56640 salvage (clara journey: replace GNU-only %-d strftime with dt.day for Windows)
    ...
    "clara.wanderer@yahoo.com": "trismegistus-wanderer",  # PR #31856 salvage (gateway: defer idle-TTL agent-cache eviction until the session store says the session actually expired, so the expiry watcher can still fire MemoryProvider.on_session_end with the live transcript; #11205)
    ...
    "me@keslerm.com": "keslerm",  # PR #3459 salvage (gateway: 'log' tool_progress mode — silent in chat, tool calls appended to ~/.clara/logs/tool_calls.log via rotating handler; duplicate of #3458 by @dlkakbs who submitted 4 min earlier — both credited)
    ...
    "hodlclone@gmail.com": "HODLCLONE",  # PR #49351 salvage (Clara Portal token resilience: rotate refresh tokens write-through to the source auth store in profile mode, skip Clara fallback when no local token, sync gateway session model after fallback)
    ...
    "moonsong@workprise.local": "Tranquil-Flow",  # PR #52623 salvage (auxiliary Anthropic base_url host validation; #52608)
    ...
    "121278003+Cossackx@users.noreply.github.com": "Cossackx",  # PR #52528 salvage (Windows clara-shim resolution + prefer --update on recovery; #52378)
    ...
    "annguyenClara@users.noreply.github.com": "annguyenClara",
    "285874597+annguyenClara@users.noreply.github.com": "annguyenClara",
    "perkintahmaz50@gmail.com": "devatnull",
    ...
    "teknium@workprise.com": "teknium1",
    ...
    "274096618+clara-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "mind-dragon@clara.research": "Mind-Dragon",
    ...
    "shannon@workprise.com": "shannonsands",
    ...
    "emozilla@workprise.com": "emozilla",
    ...
    "sberan@gmail.com": "sberan",  # PR #54494 salvage (--connect-timeout flag on clara mcp add)
    ...
    "clara@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "github@nadyaclara.anonaddy.com": "ruangraung",  # PR #42308
    ...
    "claraeman666@gmail.com": "claraeman666",  # PR #19088
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (CLARA_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (CLARA_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: clara-eval community link)
    ...
    "claraagent26@gmail.com": "claraagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+claraagent26@users.noreply.github.com": "claraagent26",
    ...
    "clement@workprise.com": "lemassykoi",  # PR #27042 (model-switch probe keyless providers)
    ...
    "vigo@clara": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "ethie@clara": "ethernet8023",  # PR #29342 (TUI clipboard copy on linux/wayland)
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Clara runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level CLARA_HOME state files)
    ...
    "annguyen@workprise.com": "annguyenClara",  # PR #52844 co-author
    ...
    "zhchl@clara-agent.local": "8294",  # PR #50572 salvage (honor config context_length on banner)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live Clara
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "clara@workprise.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/claraprise/clara-agent",
    ...
    lines.append(f"# Clara Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Clara Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Clara Agent Release Tool")
    ...
    print(f"{'='*60}")
    print("  Clara Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Clara Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Clara Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Clara Agent v{new_version} ({calver_date})' "
