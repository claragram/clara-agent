...
# (%CLARA_HOME%\clara-setup.exe). That binary has no self-update path --
# copy_self_to_clara_home deliberately no-ops during --update -- so every
...
#
# This script lives in the repo checkout, so EVERY `clara update` refreshes
...
#     -InstallRoot <path>   repo checkout (CLARA_HOME\clara-agent)
#     -Branch <ref>         branch to update against
#     -DesktopPid <pid>     the Electron main process to wait out
#     [-RelaunchExe <path>] Clara.exe to start when done (omit = no relaunch)
#     [-NoUi]               headless (tests); default shows a progress window
#     [-NoMarkerCleanup]    leave .clara-update-in-progress in place (tests)
#
...
# .clara-update-result.json for the relaunched Desktop to surface, and
# relaunches the Desktop so the user is never left stranded.
#
# Marker: we claim CLARA_HOME\.clara-update-in-progress with OUR pid as
# step 0 (the wrapper cmd.exe pid the Desktop saw is useless -- it exits
# immediately), retaining CLARA_UPDATE_STARTED_AT from the Desktop hand-off.
# clara_cli/update_lock.py's ancestry rule lets our
# `clara update` child adopt the claim; electron/update-marker.ts parks a
...
# pass our foreground right on to the new Clara.exe pid.
try {
    Add-Type -Namespace ClaraHandoff -Name Win32 -MemberDefinition @'
...
$ClaraHome = if ($InstallRoot) { Split-Path -Parent $InstallRoot } else { $TempDir }
$MarkerPath = Join-Path $ClaraHome ".clara-update-in-progress"
$LogDir = Join-Path $ClaraHome "logs"
$LogPath = Join-Path $LogDir "desktop-update-handoff.log"
$ResultPath = Join-Path $ClaraHome ".clara-update-result.json"
$script:Ui = $null
$script:UiStage = "Clara will open once done."   # until the first gate; matches ui.html
...
function Stop-UiServer([switch]$LeaveWindow) {
    ...
    # profile plus any stale clara-update-ui-* leftovers from interrupted
    ...
    # update marker (.clara-update-in-progress) serialises hand-offs, so no
    # other run's profile can be in active use here.
    try {
        $profileDirs = @()
        if ($script:UiServer.Profile) { $profileDirs += $script:UiServer.Profile }
        Get-ChildItem -LiteralPath $TempDir -Directory -Filter "clara-update-ui-*" -ErrorAction SilentlyContinue |
...
function Publish-UiProgress([string]$Message) {
    ...
    # stderr remain __PROT_0_asynchroclaraly__ drained in Invoke-ClaraStep and are never
...
function Show-ProgressWindow {
    ...
    if ($htmlPath -and $browser) {
        ...
        if ($server) {
            try {
                ...
                $browserProfile = Join-Path $TempDir ("clara-update-ui-{0}" -f $PID)
                $browserArgs = @(
    ...
    try {
        ...
        try {
            $form.Activate()
            if ($script:Win32) { [ClaraHandoff.Win32]::SetForegroundWindow($form.Handle) | Out-Null }
        } catch {}
        [System.Windows.Forms.Application]::DoEvents()
...
function Show-ErrorFinale([string]$Message) {
    ...
    # log tail -- `clara debug share` uploads the real evidence and the
    ...
    try {
        ...
        $ui.Sub.Text = "Run `"clara debug share`" in a terminal to send a report."
        ...
        try {
            $ui.Form.Activate()
            if ($script:Win32) { [ClaraHandoff.Win32]::SetForegroundWindow($ui.Form.Handle) | Out-Null }
        } catch {}
...
function Start-DesktopRelaunch {
    ...
    # update it can remove the old Clara.exe before writing the replacement,
    ...
    # DO NOT spawn Clara.exe as our child: Electron/Chromium calls
    ...
    try {
        ...
        if ($r -and $r.ReturnValue -eq 0) {
            ...
            try {
                if ($script:Win32) {
                    [ClaraHandoff.Win32]::AllowSetForegroundWindow([int]$r.ProcessId) | Out-Null
                    ...
                    while ((Get-Date) -lt $deadline) {
                        ...
                        if ($hwnd -ne [System.IntPtr]::Zero) {
                            [ClaraHandoff.Win32]::ShowWindow($hwnd, 9) | Out-Null  # SW_RESTORE
                            [ClaraHandoff.Win32]::SetForegroundWindow($hwnd) | Out-Null
    ...
    if (-not $spawned) {
        ...
        # verify by watching for a fresh Clara process.
        try {
            ...
            while ((Get-Date) -lt $explorerDeadline) {
                ...
                if ($fresh.Count -gt 0) {
                    ...
                    try {
                        if ($script:Win32) {
                            [ClaraHandoff.Win32]::AllowSetForegroundWindow([int]$fresh[0].Id) | Out-Null
...
# closes it. `clara update` deliberately runs its build steps with stdout
# inherited (clara_cli/main.py, the tee-stderr runner), so the tree under a
...
if ($env:CLARA_UPDATE_PIPE_DRAIN_SECONDS) {
    $parsedGrace = 0
    if ([int]::TryParse($env:CLARA_UPDATE_PIPE_DRAIN_SECONDS, [ref]$parsedGrace) -and $parsedGrace -ge 0) {
...
if ($env:CLARA_UPDATE_STEP_IDLE_SECONDS) {
    $parsedIdle = 0
    if ([int]::TryParse($env:CLARA_UPDATE_STEP_IDLE_SECONDS, [ref]$parsedIdle) -and $parsedIdle -gt 0) {
        $script:StepIdleTimeoutSeconds = $parsedIdle
    }
}

# Silence on the pipes is NOT silence in the update. `clara update` captures
# the (very loud) Electron/vite build into logs/update.log instead of its own
# stdout (clara_cli/update_cmd.py, the update-log tee), so a real update is
...
if ($env:CLARA_UPDATE_PROGRESS_LOG) {
    $script:StepProgressLogPath = $env:CLARA_UPDATE_PROGRESS_LOG
}
...
public static class ClaraUpdateJob {
...
function Invoke-ClaraStep([string]$Exe, [string[]]$ClaraArgs, [string]$Tag) {
    # The window does not stream child output, so no line-pump: both pipes
    # drain __PROT_1_asynchroclaraly__ (no deadlock however chatty the child) while a small
    ...
    # afterwards, where `clara debug share` picks it up.
    #
    ...
    # .clara-update-result.json, clearing .clara-update-in-progress,
    ...
    # Clara" until the user killed something by hand. Losing the tail of a
    ...
    $arguments = ($ClaraArgs | ForEach-Object { '"{0}"' -f ($_ -replace '"', '\"') }) -join ' '
    ...
    try {
        ...
        $started = [ClaraUpdateJob]::StartAssigned($Exe, $arguments)
    } finally {
    ...
    while ($true) {
        ...
        if ($proc.HasExited) {
            ...
            # Quiet pipes are how a healthy `clara update` looks for 40+
            ...
            if ($currentLogStamp -ne $progressLogStamp) {
                ...
                $stalled = [ClaraUpdateJob]::TerminateAndWait($job, 124, 10000)
                if (-not $stalled) {
...
# the page can be QA'd in any browser; CLARA_SELFTEST_FAIL=1 exercises the
# error state, CLARA_SELFTEST_HOLD_SECONDS delays the terminal event.
if ($SelfTestUi) {
    ...
    $hold = 6
    if ($env:CLARA_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:CLARA_SELFTEST_HOLD_SECONDS }
    Publish-UiProgress "Testing quiet update"
    Start-Sleep -Seconds $hold
    if ($env:CLARA_SELFTEST_FAIL) {
...
# -SelfTestPipeDrain: prove Invoke-ClaraStep survives a leaked pipe ------
# The #90455 deadlock needs no update, no checkout and no Clara install to
...
#            update log, the shape of every real `clara update` build (output
...
if ($SelfTestPipeDrain) {
    ...
    if ($env:CLARA_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:CLARA_SELFTEST_HOLD_SECONDS }
    $floodKb = 8192
    if ($env:CLARA_SELFTEST_FLOOD_KB) { $floodKb = [int]$env:CLARA_SELFTEST_FLOOD_KB }
    # $PSHOME is this interpreter's own directory -- no hardcoded system path.
    $powershell = Join-Path $PSHOME "powershell.exe"
    $stamp = [Guid]::NewGuid().ToString("N")
    $childPs1 = Join-Path $TempDir "clara-pipe-drain-$stamp.ps1"
    $floodPs1 = Join-Path $TempDir "clara-pipe-flood-$stamp.ps1"
    $pidFile = Join-Path $TempDir "clara-pipe-drain-$stamp.pid"
    $stallPs1 = Join-Path $TempDir "clara-step-stall-$stamp.ps1"
    $stallPidFile = Join-Path $TempDir "clara-step-stall-$stamp.pid"
    $stallGrandchildPidFile = Join-Path $TempDir "clara-step-stall-grandchild-$stamp.pid"
    $logStallPs1 = Join-Path $TempDir "clara-step-logstall-$stamp.ps1"
    $logStallProgress = Join-Path $TempDir "clara-step-logstall-$stamp.update.log"
...
'@
    ...
    # merely loud. `clara update` is this shape -- the Electron/vite build
...
'@
    ...
    $res = Invoke-ClaraStep $powershell @(
    ...
    $flood = Invoke-ClaraStep $powershell @(
    ...
    $stall = Invoke-ClaraStep $powershell @(
    ...
    try {
        $logstall = Invoke-ClaraStep $powershell @(
    ...
    if ($stallAlive) { $problems += "stalled child pid $stallPid remained alive after Invoke-ClaraStep returned" }
    if ($stallGrandchildAlive) { $problems += "stalled descendant pid $stallGrandchildPid remained alive after Invoke-ClaraStep returned" }
...
try {
    ...
    try {
        ...
        $startedAt = 0L
        $hasStartedAt = [int64]::TryParse($env:CLARA_UPDATE_STARTED_AT, [ref]$startedAt)
    ...
    Publish-UiProgress "Waiting for Clara to close"
    if ($DesktopPid -gt 0) {
        ...
        if (Get-Process -Id $DesktopPid -ErrorAction SilentlyContinue) {
            ...
            $finalCode = 4
            $finalMsg = "Update aborted: the Clara window (pid $DesktopPid) did not exit within 30s. Nothing was changed. Close Clara fully and try again."
    ...
    Publish-UiProgress "Preparing Clara files"
    $shim = Join-Path $InstallRoot "venv\Scripts\clara.exe"
    if (Test-Path -LiteralPath $shim) {
        ...
        if (-not $unlocked) {
            ...
            $finalCode = 5
            $finalMsg = "Update aborted: another process is still holding the Clara install open (venv\Scripts\clara.exe locked after 20s). Nothing was changed. Close other Clara windows/terminals and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    ...
    # --force skips only the clara.exe shim guard, which step 2 just PROVED
    ...
    #
    # DRIVE THE UPDATE THROUGH venv\Scripts\python.exe, NOT venv\Scripts\clara.exe.
    # `uv pip install -e .` has to replace the console-script shims, so
    # _quarantine_running_clara_exe must first rename the running clara.exe
    # out of the way. On Windows that rename fails whenever ANY child process
    # spawned from that clara.exe is still alive: a child inherits a handle on
    # the parent image, and the resulting sharing violation is indistinguishable
    # from a user leaving a second Clara window open. It is the inherited
    ...
    # install whose Start Menu shortcut points at a Clara.exe that no longer
    ...
    #
    # Running the same code as `python.exe -m clara_cli.main update` puts the
    ...
    if (-not (Test-Path -LiteralPath $pythonExe)) {
        $finalCode = 3
        $finalMsg = "Update aborted: $pythonExe is missing. The install needs repair (run the Clara installer or `clara doctor`)."
        Write-HandoffLog $finalMsg
        exit $finalCode
    }
    $updateArgs = @("-m", "clara_cli.main", "update", "--yes", "--gateway", "--force", "--branch", $Branch)
    ...
    # collides with the "close all Clara windows" sentinel.
    try {
        $updateHelp = & $pythonExe -m clara_cli.main update --help 2>$null | Out-String
        if ($updateHelp -match "--keep-stash") {
            $updateArgs += "--keep-stash"
        } else {
            Write-HandoffLog "installed clara predates --keep-stash; running without it"
        }
    } catch {
    ...
    $res = Invoke-ClaraStep $pythonExe $updateArgs "update"
    Write-HandoffLog "clara update exit code: $($res.Code)"

    $retryPolicyPath = Join-Path $PSScriptRoot "retry-policy.ps1"
    if (Test-Path -LiteralPath $retryPolicyPath) {
        . $retryPolicyPath
        $shouldRetry = Test-ClaraUpdateShouldRetry -ExitCode $res.Code -InstallRoot $InstallRoot
    } else {
    ...
    # `clara update` treats a Desktop GUI build failure as NON-fatal (prints
    ...
    if ($res.Code -eq 0 -and $res.Output -match "Desktop build failed") {
        Write-HandoffLog "clara update reported a desktop build failure (non-fatal there, fatal here); retrying build"
        Publish-UiProgress "Rebuilding Desktop"
        $rebuild = Invoke-ClaraStep $pythonExe @("-m", "clara_cli.main", "desktop", "--force-build", "--build-only") "rebuild"
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed) {
        ...
        $finalCode = 6
        $finalMsg = "Code and dependencies updated, but the Desktop app REBUILD FAILED - you are running the previous build. Run `clara desktop --force-build` from a terminal to retry."
    } else {
        $finalCode = $res.Code
        $finalMsg = "Update failed (exit $($res.Code)). Run `clara debug share` in a terminal to send a report."
    }
    exit $finalCode
} finally {
    ...
    #   3. only then the terminal UI state — done means "Clara is back",
    ...
    if (-not $script:TreeSafeToFinalize) {
        ...
        $finalCode = 7
        $finalMsg = "Update recovery could not stop every updater process. Clara was not restarted to avoid overlapping the active install. Wait for it to finish or restart Windows, then reopen Clara."
        ...
        if ($finalCode -ne 0) {
            ...
            Publish-UiProgress "Opening Clara"
            ...
            if (-not $cameBack -and $RelaunchExe) {
                ...
                $finalMsg = "Update complete. Reopen Clara to finish (it could not restart itself)."
