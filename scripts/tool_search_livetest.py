#!/usr/bin/env python3
"""Live test harness for Clara Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any clara imports.
ORIGINAL_HOME = os.environ.get("CLARA_HOME")
ORIGINAL_AUTH = Path.home() / ".clara" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.clara/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.clara/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="clara_ts_live_"))
    clara_home = home_dir / ".clara"
    clara_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, clara_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".clara" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, clara_home / ".env")
        ...
        load_clara_dotenv(clara_home=str(Path.home() / ".clara"))
    ...
    (clara_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return clara_home
...
def reset_module_state():
    """Drop cached modules so the new CLARA_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "clara_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["CLARA_HOME"] = str(home)
...
def main():
    ...
    # Restore original CLARA_HOME
    if ORIGINAL_HOME is not None:
        os.environ["CLARA_HOME"] = ORIGINAL_HOME
    else:
