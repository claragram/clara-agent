"""Run a real Clara CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_clara_executable(clara_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "clara",
        Path(".venv") / "Scripts" / "clara.exe",
    ):
        candidate = clara_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("clara")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "Clara executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="Clara source checkout containing .venv/bin/clara",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated CLARA_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["clara.client.active"] != [
        {
        ...
        raise AssertionError(
            f"Unexpected client-active counter: {by_name['clara.client.active']}"
        )
    [model] = by_name["clara.model_route.count"]
    expected_model = {
        "name": "clara.model_route.count",
        "dimensions": {
            "model": MODEL_CANARY,
            "provider": "custom",
        },
    ...
    if model != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['clara.model_route.count']}"
        )
    expected_start = {
    ...
    if by_name["clara.task_run.started"] != [expected_start]:
        raise AssertionError(
            f"Unexpected task start: {by_name['clara.task_run.started']}"
        )
    [terminal] = by_name["clara.task_run.finished"]
    ...
    [tool] = by_name["clara.tool_call.count"]
    ...
    lifecycle = by_name["clara.skill.lifecycle.count"]
    expected_actions = {
        "archived",
        "created",
        "edited",
    ...
    loads = by_name["clara.skill.load.count"]
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The Clara development environment requires jsonschema"
        ) from exc
    ...
    if metrics["clara.client.active"] != [
        {
            "name": "clara.client.active",
            "type": "counter",
            "dimensions": {},
        ...
        raise AssertionError(
            f"Unexpected client-active metric: {metrics['clara.client.active']}"
        )
    [model] = metrics["clara.model_route.count"]
    if model["dimensions"] != {
        "model": MODEL_CANARY,
        "provider": "custom",
    } or model["value"] != 2:
        raise AssertionError(
            f"Unexpected model metric: {metrics['clara.model_route.count']}"
        )
    [terminal] = metrics["clara.task_run.finished"]
    ...
    [tool] = metrics["clara.tool_call.count"]
    if (
        tool["dimensions"]
        != {
            "approval_outcome": "not_required",
    ...
    lifecycle = metrics["clara.skill.lifecycle.count"]
    ...
    loads = metrics["clara.skill.load.count"]
    if {
        (
...
def main() -> int:
    args = _arguments()
    clara_repo = args.clara_repo.resolve()
    relay_python = args.relay_python.resolve() if args.relay_python else None
    clara = _resolve_clara_executable(clara_repo)
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="clara-relay-shared-metrics-"))
    home = root / "clara-home"
    ...
    (root / "clara.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "clara.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"Clara exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("Clara model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("Clara did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("Clara did not print the mock model response")
    ...
    skill_result = subprocess.run(
        [
            ...
            "-c",
            "\n".join([
                "from clara_cli.observability import relay_shared_metrics",
    ...
    package_paths, packages = _validate_packages(
        ...
        / "observability"
        / "schemas"
        / "clara.shared_metrics.v2.schema.json",
    )

    print("Clara -> NeMo Relay shared-metrics smoke test passed")
