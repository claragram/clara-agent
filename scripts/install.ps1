# ============================================================================
# Clara Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$ClaraHome = $(if ($env:CLARA_HOME) { $env:CLARA_HOME } else { "$env:LOCALAPPDATA\clara" }),
    [string]$InstallDir = $(if ($env:CLARA_HOME) { "$env:CLARA_HOME\clara-agent" } else { "$env:LOCALAPPDATA\clara\clara-agent" }),
    ...
    # builds apps/desktop into a launchable Clara.exe.
    #
    # Why opt-in:
    #   * Clara-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Clara.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Clara.exe
    ...
    #     `clara desktop` already builds on demand.
...
# 5.1's progress UI repaints __PROT_0_synchroclaraly__ on every received byte, which
...
# CLARA_HOME and InstallDir -- in that short form:
...
function ConvertTo-LongPath {
    ...
    try {
        if (-not ([System.Management.Automation.PSTypeName]'ClaraInstall.LongPath').Type) {
            Add-Type -Namespace 'ClaraInstall' -Name 'LongPath' -MemberDefinition @'
...
'@
        ...
        $length = [ClaraInstall.LongPath]::GetLongPathNameW($Path, $buffer, $buffer.Capacity)
...
# long. An explicitly passed -ClaraHome / -InstallDir is normalized in place
...
if ($PSBoundParameters.ContainsKey('ClaraHome')) {
    $ClaraHome = ConvertTo-LongPath $ClaraHome
} else {
    $ClaraHome = ConvertTo-LongPath $(
        if ($env:CLARA_HOME) { $env:CLARA_HOME } else { "$env:LOCALAPPDATA\clara" }
    )
}
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:CLARA_HOME) { "$env:CLARA_HOME\clara-agent" } else { "$env:LOCALAPPDATA\clara\clara-agent" }
    )
}
if ($script:NormalizedProfilePaths) {
    ...
    Write-PathDiag "resolved install paths: ClaraHome=$ClaraHome InstallDir=$InstallDir"
}
...
$RepoUrlSsh = "git@github.com:claraship/clara-agent.git"
$RepoUrlHttps = "https://github.com/claraship/clara-agent.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by Claraship.              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $ClaraHome)) {
        New-Item -ItemType Directory -Force -Path $ClaraHome | Out-Null
    }
    $envFile = Join-Path $ClaraHome ".env"
...
function Install-AgentBrowser {
    ...
    # which every consumer (tools/browser_tool.py, `clara update`'s npx
    ...
    $prefixDir = Join-Path $ClaraHome "node"
...
function Install-Uv {
    # Clara owns its own uv at $ClaraHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (clara_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `clara update` stay in sync.
    $managedUv = Join-Path $ClaraHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $ClaraHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $ClaraHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $ClaraHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $ClaraHome "bin"
        ...
        # (clara_cli/managed_uv.py looks only at $ClaraHome\bin\uv.exe).
...
# Put the Clara-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone clara-setup.exe run, a user typing `npm`) silently
...
# Upgrade the Clara-managed Node tree's bundled npm into $NpmRange when
...
function Update-ManagedNpm {
    ...
    if (Test-ManagedNodeInUse $NodeDir) {
        Write-Warn "Clara-managed Node.js is in use by a running app; skipping the bundled npm upgrade (applies on a later update with the app closed)."
        return $false
    }

    Write-Info "Upgrading bundled npm to satisfy $range ..."

    $tmpCwd = Join-Path $env:TEMP ("clara-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function Resolve-UvCmd {
    ...
    # sibling process and CLARA_HOME wasn't propagated).
...
function Initialize-ManagedPythonEnvironment {
    # Python used by Clara belongs to the checkout, never to another
    # application or a user-level uv configuration. Keep this aligned with
    # clara_cli.managed_uv.managed_python_env(), which owns the update path.
    foreach ($name in @(
    ...
    $managedRoot = Join-Path $InstallDir ".clara-runtime\python"
...
function Resolve-AvailablePythonVersion {
    # Return the path and minor version of the first Clara-managed interpreter
    ...
    #
    # Under Clara-Setup.exe each stage runs in a fresh powershell.exe. The
    ...
    foreach ($ver in $candidates) {
        ...
        try {
            ...
            throw "Failed to resolve Clara-managed Python $ver`: $_"
        } finally {
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun Clara setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which Clara uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\clara\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Clara needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Clara copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\clara\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``CLARA_GIT_BASH_PATH`` (User scope) so Clara can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a Clara-managed PortableGit install instead..."
    }

    # Download PortableGit into $ClaraHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $ClaraHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Clara features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Clara needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``CLARA_GIT_BASH_PATH`` (User env scope) so Clara can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $ClaraHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $ClaraHome\git\usr\bin\bash.exe
    $candidates += "$ClaraHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$ClaraHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("CLARA_GIT_BASH_PATH", $candidate, "User")
            $env:CLARA_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set CLARA_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Clara may not find Git Bash."
    Write-Info "If needed, set CLARA_GIT_BASH_PATH manually to your bash.exe path."
}
...
# range used to provision the Clara-managed tree. Keeping this probe separate
...
function Test-SystemNodeReady {
    ...
    if (Test-NodeVersionOk $version) {
        ...
        Write-Warn "Node.js $version is unsupported (Clara requires Node 22.22+, 24.11+, or 26+)"
        return $false
    }
    ...
    if ($npmVersion) {
        Write-Warn "Node.js $version uses npm $npmVersion, which does not satisfy Clara requirement $npmRange"
    } else {
...
function Test-Node {
    ...
    Write-Info "Using a Clara-managed Node.js installation instead..."

    # Prefer a Clara-managed Node from a previous run over a too-old system one.
    $managedNode = "$ClaraHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$ClaraHome\node;$env:Path"
        Set-ManagedNodeFirstOnUserPath "$ClaraHome\node"
        Write-Success "Node.js $version found (Clara-managed)"
        ...
        Update-ManagedNpm "$ClaraHome\node" | Out-Null
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Clara-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $ClaraHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $ClaraHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\clara-node-extract"
            ...
            if ($extractedDir) {
                ...
                Get-ChildItem "$ClaraHome" -Directory -Filter "node.old-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                Get-ChildItem "$ClaraHome" -Directory -Filter "node.new-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                ...
                if (Test-Path "$ClaraHome\node") {
                    try {
                        Rename-Item "$ClaraHome\node" $backup -ErrorAction Stop
                    } catch {
                        Write-Warn "Clara-managed Node.js is in use by a running app; deferring its upgrade. Close the app and re-run the update."
                ...
                $version = & "$ClaraHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $ClaraHome\node\ (portable, user-scoped)"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\clara-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `clara
                    # update` path (#4735).
                    ...
                    $stashName = "clara-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. clara-setup.exe bakes its build-time commit
                    ...
                    # ff-only pull cannot succeed -- mirror ``clara update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if Clara behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running clara processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/claraship/clara-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/claraship/clara-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/claraship/clara-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\clara-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\clara-agent-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `clara update` (see the notes at the shared clone-path
    ...
    # next `clara update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Clara-Setup.exe
    ...
    if (-not $resolvedPython) {
        throw "Clara-managed Python is unavailable. Run install.ps1 -Stage python first."
    }
    ...
    if (Test-Path -LiteralPath "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running clara process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running clara processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Clara_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (clara.exe) plus its child tree.
            & taskkill /F /T /IM clara.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM clara.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m clara_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not clara.exe.
            ...
            # Clara children re-exec through .clara-runtime, so killing only
            ...
            # user (see clara_cli/gateway_windows.py), so the installer always
        ...
        try {
            ...
            throw (
                ...
                "A process still has the install directory open (often a non-Clara " +
    ...
    $env:UV_PYTHON = $venvPythonExe
        ...
        # the task fires normally on next logon and `clara update` / the
...
function Restore-VenvBackup {
    ...
    # venv so Clara (and the venv-blocker probe) stay usable (#83149).
    $backupName = Get-PendingVenvBackup
    if (-not $backupName) { return }
    try {
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `clara.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install clara-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and clara.exe broken with
    ...
    # here, not 30 seconds later when the user runs `clara`.
    if (-not $NoVenv) {
        $venvPython = "$InstallDir\venv\Scripts\python.exe"
        if (-not (Test-Path $venvPython)) {
            throw "Install reported success but $venvPython does not exist. The dependency sync likely landed in a sibling .venv\ directory. Re-run the installer; if it persists, close Clara processes and preserve existing venv directories before retrying. Do not delete venv in place."
        }
        ...
        if ($importExitCode -ne 0) {
            ...
            $hint = if (Test-Path $sibling) {
                "Detected sibling .venv\ at $sibling -- uv synced there instead of venv\. Close Clara processes, preserve the existing venv, and rerun the installer so the transactional recovery path can move directories safely."
            } else {
    ...
    Complete-VenvTransaction
        ...
        # a failed update leaves Clara and its blocker probe usable.
        Restore-VenvBackup
        Pop-Location
        throw
    }
    ...
    if (-not $NoVenv) {
        # uv on Windows can register clara.exe in dist-info/RECORD but fail to
        ...
        # `clara` command while clara-agent.exe / clara-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m clara_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `clara dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\clara_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `clara dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `clara dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: clara_cli/web_server.py"
        }
    }
...
function Install-ClaraCommandLaunchers {
    param(
        [Parameter(Mandatory=$true)] [string]$Root,
        [Parameter(Mandatory=$true)] [string]$Destination
    )

    # Expose ONLY the clara launchers on PATH -- never the whole
    # venv\Scripts directory, which contains python.exe / pip.exe and
    # silently hijacks the `python` command in every terminal (#83797).
    # Requiring clara.exe before creating the destination keeps the PATH
    # stage from reporting success with an unusable command (PR #92092).
    $scriptsDir = Join-Path $Root "venv\Scripts"
    $requiredSource = Join-Path $scriptsDir "clara.exe"
    if (-not (Test-Path -LiteralPath $requiredSource -PathType Leaf)) {
        throw "Cannot set up the clara command: required launcher not found: $requiredSource"
    }

    New-Item -ItemType Directory -Force -Path $Destination | Out-Null

    # Launcher form depends on the venv (keep in lockstep with
    # clara_cli/_install_repair.py): a normal venv's exe trampoline
    ...
    foreach ($launcher in @("clara", "clara-acp")) {
    ...
    $requiredExe = Join-Path $Destination "clara.exe"
    $requiredCmd = Join-Path $Destination "clara.cmd"
    if (-not ((Test-Path -LiteralPath $requiredExe -PathType Leaf) -or
              (Test-Path -LiteralPath $requiredCmd -PathType Leaf))) {
        throw "Cannot set up the clara command: launcher was not installed: $requiredExe"
    }
    return $Destination
}

function Set-PathVariable {
    Write-Info "Setting up clara command..."
    
    if ($NoVenv) {
        $claraBin = "$InstallDir"
    } else {
        # $ClaraHome\bin is the managed binary dir (shared with the managed
        # uv), OUTSIDE the git checkout: `clara update`'s autostash
        ...
        # installer staged under clara-agent\bin. No git operation can ever
        # touch this dir. Staging and verification live in
        # Install-ClaraCommandLaunchers, which throws BEFORE any PATH
        # mutation when the launchers cannot be staged.
        $claraBin = "$ClaraHome\bin"
        Install-ClaraCommandLaunchers -Root $InstallDir -Destination $claraBin | Out-Null
    }
    ...
    #   clara-agent\bin -- lived inside the git checkout, where the update
    #                       autostash could sweep the launchers off disk
    # The clara-agent\bin FILES are left in place on purpose: editor/ACP
    ...
    if (-not $NoVenv) {
        ...
        if ($cleaned.Count -ne $items.Count) {
            ...
            Write-Info "Removed legacy launcher entries from user PATH (kept clara via $claraBin)"
        }
    }
    
    if ($currentPath -notlike "*$claraBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$claraBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $claraBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set CLARA_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\clara instead
    # of the Unix default ~/.clara
    $currentClaraHome = [Environment]::GetEnvironmentVariable("CLARA_HOME", "User")
    if (-not $currentClaraHome -or $currentClaraHome -ne $ClaraHome) {
        [Environment]::SetEnvironmentVariable("CLARA_HOME", $ClaraHome, "User")
        Write-Success "Set CLARA_HOME=$ClaraHome"
    }
    $env:CLARA_HOME = $ClaraHome
    
    # Update current session
    $env:Path = "$claraBin;$env:Path"
    
    Write-Success "clara command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.clara-bootstrap-complete which tells the Clara
    ...
    $markerPath = Join-Path $InstallDir ".clara-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the CLARA_HOME directory structure ($ClaraHome, default %LOCALAPPDATA%\clara)
    New-Item -ItemType Directory -Force -Path "$ClaraHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ClaraHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ClaraHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ClaraHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ClaraHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ClaraHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ClaraHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ClaraHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ClaraHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$ClaraHome\.env"
    ...
    # Create config.yaml
    $configPath = "$ClaraHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Clara's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$ClaraHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in clara_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $ClaraHome"
    
    # Seed bundled skills into $ClaraHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $ClaraHome\skills ..."
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            Write-Success "Skills synced to $ClaraHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$ClaraHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $ClaraHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Clara-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'clara setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\clara-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\clara-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\clara-npm-tui-$(Get-Random).log"
...
# failure is non-fatal (browser_exec can still run via uvx, and `clara tools`
...
function Install-BrowserUseCli {
    ...
    $managedBin = Join-Path $ClaraHome "bin"
    $managedBu = Join-Path $managedBin "browser-use.exe"
    # MANAGED-FIRST: only Clara' managed copy short-circuits. A browser-use
    ...
    $ErrorActionPreference = "Continue"
    try {
        # UV_TOOL_BIN_DIR keeps the binary inside Clara' managed bin dir,
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Info "Install later with: uv tool install browser-use  (or via 'clara tools')"
        }
    } catch {
...
# Provision it at install time so enabling the tool later -- via `clara
...
# paths still lazy-install via install_cua_driver() (clara_cli/tools_config)
...
function Install-CuaDriver {
    ...
    $ErrorActionPreference = "Continue"
    try {
        # Same upstream installer `clara computer-use install` runs. Bounded
        ...
        # Clara' _CUA_INSTALLER_TIMEOUT (660s).
        ...
        if (Wait-Job $job -Timeout 660) {
            ...
            if ($installedCuaDriver -and (Test-CuaDriverRuntimeContract -DriverPath $installedCuaDriver.Source)) {
                Write-Success "Computer Use driver installed (enable via 'clara tools' -> Computer Use)"
            } else {
                Write-Warn "Computer Use driver install did not produce a compatible runtime -- repair it before enabling the tool."
                Write-Info "Install later with: clara computer-use install"
            }
        } else {
...
# the final `electron` -> `Clara` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Clara.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_clara_desktop command
    # resolves apps/desktop/release/win-unpacked/Clara.exe directly,
    ...
    # enforces the supported Node lines and prepends the Clara-managed Node to
    ...
    # The --dir mode produces an unpacked Clara.exe in
    ...
    # are NOT trying to work around). The Clara icon + product name are
    # stamped onto Clara.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\clara-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Clara.exe",
        "$desktopDir\release\win-arm64-unpacked\Clara.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Clara.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Clara icon + identity are stamped onto Clara.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, clara-agent#38216).
    ...
    #    Clara.exe. We deliberately do NOT point them at `clara desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Clara.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Clara.lnk')
        )
        ...
        # with the Clara icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.clara/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'clara setup'."
        return
    }
    ...
    # Run clara setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m clara_cli.main setup
    } else {
        python -m clara_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $claraCmd = "$InstallDir\venv\Scripts\clara.exe"
    if (-not (Test-Path $claraCmd)) {
        $claraCmd = "clara"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$ClaraHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'clara whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: clara gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$ClaraHome\logs\gateway.log"
            Start-Process -FilePath $claraCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$ClaraHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: clara gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: clara gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$ClaraHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$ClaraHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ClaraHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ClaraHome\clara-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   clara              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   clara setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   clara config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   clara config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   clara gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   clara update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Clara repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Clara-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Clara to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://agent.claraship.com/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
