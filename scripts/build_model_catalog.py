#!/usr/bin/env python3
"""Build the Clara Model Catalog — a centralized JSON manifest of curated models.

This script reads the in-repo hardcoded curated lists (``OPENROUTER_MODELS``,
``_PROVIDER_MODELS["clara"]``) and writes them to a JSON manifest that the
Clara CLI fetches at runtime. Publishing the catalog through the docs site
lets maintainers update model lists without shipping a Clara release.
...
# Ensure CLARA_HOME is set for imports that touch it at module level.
os.environ.setdefault("CLARA_HOME", os.path.join(os.path.expanduser("~"), ".clara"))

from clara_cli.models import (  # noqa: E402
    OPENROUTER_MODELS,
...
def build_catalog() -> dict:
    return {
        ...
        "metadata": {
            "source": "clara-agent repo",
            "docs": "https://agent.claraprise.com/docs/reference/model-catalog",
        },
        "providers": {
            "openrouter": {
                "metadata": {
                    ...
                    "note": (
                        ...
                        'The entry labeled "default": true is the model Clara '
            ...
            "clara": {
                "metadata": {
                    ...
                    "note": (
                        ...
                        "(partition_clara_models_by_tier), not this manifest. "
                        'The entry labeled "default": true is the model Clara '
                        "silently lands on when the user never picked one."
                    ),
                },
                "models": [
                    _clara_entry(mid)
                    for mid in _PROVIDER_MODELS.get("clara", [])
                ],
            },
        },
    }
