...
unprivileged `clara` user, giving the layout most people have —
$CLARA_HOME/clara-agent plus a ~/.local/bin launcher. Pass --root for the FHS
...
Environment:
  CLARA_DEV_SANDBOX_DIR    Sandbox directory name, relative to the repo root
                            (default: .clara-sandbox).

Examples:
  # create a sandbox, install this branch as `main`, and then drop to a shell,
  # skipping `clara setup` & the browser tools for speed.
...
INSTALL_REF=""
UPSTREAM_URL="${CLARA_DEV_SANDBOX_UPSTREAM:-https://github.com/claraprise/clara-agent.git}"
...
GIT_ROOT="${CLARA_SANDBOX_SOURCE_ROOT:-$(git rev-parse --show-toplevel)}"
...
SANDBOX_DIR_NAME="${CLARA_DEV_SANDBOX_DIR:-.clara-sandbox}"
...
if [ "$PERSISTENT" = true ]; then
  ...
  SANDBOX_ROOT="$(mktemp -d -t clara-sandbox.XXXXXX)"
...
if [ -n "$INSTALL_REF" ]; then
  ...
  UPSTREAM_REPO="$(mktemp -d -t clara-sandbox-upstream.XXXXXX)"
...
if [ "$INSTALL_SHORTCUT" = true ]; then
  mkdir -p "$SANDBOX_ROOT/root/http/agent.claraprise.com"
  if [ -n "$INSTALL_REF" ]; then
    git -C "$UPSTREAM_REPO" show "$UPSTREAM_COMMIT:scripts/install.sh" \
      > "$SANDBOX_ROOT/root/http/agent.claraprise.com/install.sh"
  else
    cp -a "$INSTALLER_PATH" "$SANDBOX_ROOT/root/http/agent.claraprise.com/install.sh"
  fi
  set -- bash -c '
    set +e
    curl -fsSL https://agent.claraprise.com/install.sh | bash -s -- "$@"
    install_status=$?
    if [ "$install_status" -eq 0 ] && [ -f /work/promote-main ]; then
      next_main=$(cat /work/promote-main)
      if git --git-dir=/work/repos/clara-agent.git update-ref refs/heads/main "$next_main"; then
...
# A user-level install writes the `clara` launcher to ~/.local/bin and the
# checkout to $CLARA_HOME; both live under the sandbox HOME, which is bound
...
SOURCE_REF="$COMMIT"
SNAPSHOT_REPO=""
FAKE_REPO="$SANDBOX_ROOT/root/repos/clara-agent.git"
git -C "$SANDBOX_ROOT/root/repos" init --bare -q clara-agent.git
if [ -n "$INSTALL_REF" ]; then
...
if [ -n "$(git -C "$GIT_ROOT" status --porcelain)" ]; then
  ...
  SNAPSHOT_REPO="$(mktemp -d -t clara-sandbox-snapshot.XXXXXX)"
  git -C "$SNAPSHOT_REPO" init -q
  git -C "$SNAPSHOT_REPO" fetch -q "$GIT_ROOT" "$COMMIT"
  git -C "$SNAPSHOT_REPO" config user.name 'Clara sandbox'
...
if [ -n "$INSTALL_REF" ]; then
  git --git-dir="$FAKE_REPO" fetch -q --force "$SOURCE_REPO" \
    "$SOURCE_REF:refs/clara-sandbox/next"
