...
CLARA_HOME so the operator's own telemetry state is untouched.
...
STAGING = "https://telemetry.staging-workprise.com/v1/telemetry"


def main() -> int:
    scratch = Path(tempfile.mkdtemp(prefix="clara-telemetry-e2e-"))
    os.environ["CLARA_HOME"] = str(scratch)
    ...
    from clara_cli.observability.shared_metrics_sender import SharedMetricsSender
    ...
    for index, metric_count in ((0, 3), (1, 140)):
        ...
        payload = {
            "schema_version": "clara.shared_metrics.v2",
    ...
    print(f"scratch CLARA_HOME : {scratch}")
    ...
    print("  aws s3 ls --recursive "
          "s3://clara-agent-telemetry-staging-767397871023-us-west-2-an/raw/ "
          "| tail -20")
    return 0
