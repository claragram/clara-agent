...
When Clara runs in TUI mode, the gateway child process communicates with
...
# ``get_clara_home() / "plugins"`` (user) and ``./.clara/plugins/`` (project,
# gated behind ``CLARA_ENABLE_PROJECT_PLUGINS``) — see
# ``clara_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``clara_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import clara_constants (this script
    ...
    # plugins in get_clara_home()/plugins/ and project plugins in
    # ./.clara/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_clara_home() / "plugins"]
    if os.environ.get("CLARA_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".clara" / "plugins")
