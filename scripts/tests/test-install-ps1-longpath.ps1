...
function Invoke-Normalization {
    ...
    try {
        ...
        # its diagnostics are all `[clara] `-prefixed, so the two separate
        ...
        $stderr = ($raw | Where-Object { $_ -like '`[clara`]*' }) -join "`n"
        $stdout = ($raw | Where-Object { $_ -notlike '`[clara`]*' }) -join "`n"
    } finally {
    ...
    return @{
        ...
        ClaraHome = $(if ($paths) { $paths.clara_home } else { $null })
...
$expectedInstallDir = "$($longRoot)${sep}AppData${sep}Local" + '\clara\clara-agent'
...
$result = Invoke-Normalization -Environment @{ TEMP = $shortTemp; TMP = $shortTemp } `
    -ExtraArgs @('-InstallDir', (Join-Path $shortProfile 'custom-clara'))
Assert-Equal -Expected (Join-Path $longRoot 'custom-clara') -Actual $result.InstallDir -Label "explicit -InstallDir keeps the caller's directory, on the long root"
