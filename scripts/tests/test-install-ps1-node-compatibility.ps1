...
# changing PATH, installing software, or touching the user's Clara home.

$repoRoot = Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $MyInvocation.MyCommand.Path))
$installScript = Join-Path $repoRoot 'scripts\install.ps1'
$testRoot = Join-Path $env:TEMP ("clara-node-compatibility-test-" + [Guid]::NewGuid().ToString('N'))
$ClaraHome = Join-Path $testRoot 'home'
$InstallDir = Join-Path $testRoot 'missing-checkout'
. $installScript -ClaraHome $ClaraHome -InstallDir $InstallDir
