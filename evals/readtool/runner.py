"""Run the read-tool eval through the REAL Clara AIAgent.

For each task: fresh temp CLARA_HOME, fresh fixture workspace, real
...
Usage:
  python3 evals/readtool/runner.py --model anthropic/claude-opus-4.8 \\
      --provider clara --reps 3 --label baseline
...
def run_task(task, model: str, provider: str, timeout_mult: float,
    ...
    clara_home = Path(tempfile.mkdtemp(prefix="readtool-home-")) / ".clara"
    ...
    try:
        ...
        shutil.rmtree(clara_home.parent, ignore_errors=True)
    return result
...
def main() -> int:
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise SystemExit(
            ...
            "source ~/.clara/.env; set +a  — then relaunch."
        )
