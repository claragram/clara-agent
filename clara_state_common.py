...
Extracted verbatim from clara_state.py so the SessionDB mixin modules
(clara_state_search / clara_state_schema / clara_state_portability) can
reference them without importing clara_state (which would be a cycle).
clara_state re-imports every name here for backward compatibility.
"""
...
# optimized via ``clara sessions optimize-storage``. A legacy DB sits at
...
--
-- A host-declared conversation key (X-Clara-Session-Key / build_session_key)
...
# `clara sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `clara db optimize`. This is
...
# service, the Desktop app's `clara serve` backend, interactive CLI sessions,
...
#
# Semantics mirror `clara_state._cross_process_repair_lock` (the schema-
...
# clara_state) because the search/schema mixins cannot import clara_state
# (cycle).
#
...
def _describe_lock_holder(record) -> str:
    ...
    if not isinstance(record, dict) or "pid" not in record:
        return "unknown (no holder record; pre-fix writer or non-Clara)"
    pid = record.get("pid")
