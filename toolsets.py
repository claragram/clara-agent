...
_CLARA_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (CLARA_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "or XAI_API_KEY). Off by default; enable in `clara tools` → "
            "X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (CLARA_KANBAN_TASK env "
    ...
    # Coding posture (base Clara — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in clara_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Clara toolsets (CLI + messaging platforms)
    #
    ...
    # the `clara send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "clara-cli": {
        ...
        "tools": _CLARA_CORE_TOOLS,
        "includes": []
    },

    "clara-cron": {
        # Mirrors clara-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `clara tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as clara-cli; gated by `clara tools`",
        "tools": _CLARA_CORE_TOOLS,
        "includes": []
    },
    ...
    "clara-email": {
        "description": "Email bot toolset - interact with Clara via email (IMAP/SMTP)",
        "tools": _CLARA_CORE_TOOLS,
        "includes": []
    },
    ...
    "clara-sms": {
        "description": "SMS bot toolset - interact with Clara via SMS (Twilio)",
        "tools": _CLARA_CORE_TOOLS,
        "includes": []
    },
    ...
    "clara-webhook": {
        ...
        "tools": _CLARA_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "clara-gateway": {
        ...
        "tools": [],
        "includes": ["clara-telegram", "clara-discord", "clara-whatsapp", "clara-slack", "clara-signal", "clara-bluebubbles", "clara-homeassistant", "clara-email", "clara-sms", "clara-mattermost", "clara-matrix", "clara-dingtalk", "clara-feishu", "clara-wecom", "clara-wecom-callback", "clara-weixin", "clara-qqbot", "clara-webhook", "clara-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``clara-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_CLARA_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``clara-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (clara-<name>).
        # Gives them _CLARA_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("clara-"):
            platform_name = name[len("clara-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_CLARA_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
