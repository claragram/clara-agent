# Clara Agent - Development Guide

Instructions for AI coding assistants and developers working on the clara-agent codebase.

**Never give up on the right solution.**

## What Clara Is

Clara is a personal AI agent that runs the same agent core across a CLI, a
...
Read the balance right: Clara ships a **lot** — most merges are bug fixes to
...
- **Expand reach at the edges.** New platform adapters, channels, providers,
  ...
  (`clara tools`, `clara setup`, auto-install) rather than bolting on a raw
  env var.
...
- **E2E validation, not just green unit mocks.** For anything touching
  ...
  against a temp `CLARA_HOME`. Mocks hide integration bugs.
...
- **New `CLARA_*` env vars for non-secret config.** `.env` is for secrets
...
- **Outbound telemetry / usage attribution without opt-in gating.** No new
  ...
  generic user-facing opt-in (config gate + setup prompt + `clara tools`
...
- **Third-party products / other people's projects integrated into the core
  ...
  **standalone plugin repo** users install into `~/.clara/plugins/` (or via a
  pip entry point), and promote them in the Workprise Discord
...
2. **CLI command + skill** — manages config/state/infra expressible as shell
   commands. The agent runs `clara <subcommand>` guided by a skill. Zero
   model-tool footprint. Default choice for subscriptions, scheduled tasks,
   service setup. Examples: `clara webhook`, `clara cron`, `clara tools`.
...
4. **Plugin** — third-party/niche/user-specific capability that doesn't ship in
   core. Lives in `~/.clara/plugins/` or a pip package, discovered at runtime.
...
one behind a plain URL + token, or Clara Cloud. Only the first two are spawned
by us and carry `CLARA_DESKTOP=1`. Every env-keyed GUI gate is therefore a
...
- **The toolset is the surface gate.** Keep the tools off `_CLARA_CORE_TOOLS`
...
- **Ask which identity you actually mean.** `CLARA_DESKTOP=1` legitimately
  ...
  and the embedded terminal pane (`clara --tui` against that same backend) is
...
`$HOME/.clara/clara-agent/venv` (for worktrees that share a venv with the
main checkout).
...
├── toolsets.py           # Toolset definitions, _CLARA_CORE_TOOLS list
├── cli.py                # ClaraCLI class — interactive CLI orchestrator (~11k LOC)
├── clara_state.py       # SessionDB — SQLite session store (FTS5 search)
├── clara_constants.py   # get_clara_home(), display_clara_home() — profile-aware paths
├── clara_logging.py     # setup_logging() — agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # Agent internals (provider adapters, memory, caching, compression, etc.)
├── clara_cli/           # CLI subcommands, setup wizard, plugins loader, skin engine
...
│   ├── clara-achievements/  # Gamified achievement tracking
...
├── ui-tui/               # Ink (React) terminal UI — `clara --tui`
...
**User config:** `~/.clara/config.yaml` (settings), `~/.clara/.env` (API keys only).
**Logs:** `~/.clara/logs/` — `agent.log` (INFO+), `errors.log` (WARNING+),
`gateway.log` when running the gateway. Profile-aware via `get_clara_home()`.
Browse with `clara logs [--follow] [--level ...] [--session ...]`.

## TypeScript Style

Applies to TypeScript across Clara: desktop, TUI, website, and future TS packages.
...
- **Skin engine** (`clara_cli/skin_engine.py`) — data-driven CLI theming; initialized from `display.skin` config key at startup; skins customize banner colors, spinner faces/verbs/wings, tool prefix, response box, branding text
- `process_command()` is a method on `ClaraCLI` — dispatches on canonical command name resolved via `resolve_command()` from the central registry
- Skill slash commands: `agent/skill_commands.py` scans `~/.clara/skills/`, injects as **user message** (not system prompt) to preserve prompt caching

### Slash Command Registry (`clara_cli/commands.py`)
...
- **Slack** — `slack_subcommand_map()` generates `/clara` subcommand routing
- **Autocomplete** — `COMMANDS` flat dict feeds `SlashCommandCompleter`
- **CLI help** — `COMMANDS_BY_CATEGORY` dict feeds `show_help()`

### Adding a Slash Command

1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `clara_cli/commands.py`:
```python
CommandDef("mycommand", "Description of what it does", "Session",
           aliases=("mc",), args_hint="[arg]"),
```
2. Add handler in `ClaraCLI.process_command()` in `cli.py`:
```python
...
The TUI is a full replacement for the classic (prompt_toolkit) CLI, activated via `clara --tui` or `CLARA_TUI=1`.
...
npm run dev       # watch mode (rebuilds clara-ink + tsx --watch)
npm start         # production
npm run build     # full build (clara-ink + tsc)
...
### TUI in the Dashboard (`clara dashboard` → `/chat`)

The dashboard embeds the real `clara --tui` — **not** a rewrite.  See `clara_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `clara_cli/web_server.py`.

- Browser loads `web/src/pages/ChatPage.tsx`, which mounts xterm.js's `Terminal` with the WebGL renderer, `@xterm/addon-fit` for container-driven resize, and `@xterm/addon-unicode11` for modern wide-character widths.
- `/api/pty?token=…` upgrades to a WebSocket; auth uses the same ephemeral `_SESSION_TOKEN` as REST, via query param (browsers can't set `Authorization` on WS upgrade).
- The server spawns whatever `clara --tui` would spawn, through `ptyprocess` (POSIX PTY — WSL works, native Windows does not).
- Frames: raw PTY bytes each direction; resize via `\x1b[RESIZE:<cols>;<rows>]` intercepted on the server and applied with `TIOCSWINSZ`.

**Do not re-implement the primary chat experience in React.** The main transcript, composer/input flow (including slash-command behavior), and PTY-backed terminal belong to the embedded `clara --tui` — anything new you add to Ink shows up in the dashboard automatically. If you find yourself rebuilding the transcript or composer for the dashboard, stop and extend Ink instead.

**Structured React UI around the TUI is allowed when it is not a second chat surface.** Sidebar widgets, inspectors, summaries, status panels, and similar supporting views (e.g. `ChatSidebar`, `ModelPickerDialog`, `ToolCall`) are fine when they complement the embedded TUI rather than replacing the transcript / composer / terminal. Keep their state independent of the PTY child's session and surface their failures non-destructively so the terminal pane keeps working unimpaired.

### Electron Desktop Chat App (`apps/desktop/`)

A **separate** chat surface from both the classic CLI and the dashboard's embedded TUI. It is an Electron + React + nanostore renderer (`@assistant-ui/react`) that talks to a `tui_gateway` backend over JSON-RPC (`requestGateway(method, params)`). The WebSocket/JSON-RPC transport lives in the framework-agnostic `apps/shared` package (`@clara/shared` — `JsonRpcGatewayClient` + WS URL helpers), which the web dashboard (`web/`) also consumes; **desktop has no build/runtime dependency on the dashboard frontend** — it spawns a headless `clara serve` backend server (the same gateway `dashboard` serves, minus the browser UI entirely: `serve` sets `headless_backend=True`, so `cmd_dashboard` skips `_build_web_ui` AND exports `CLARA_SERVE_HEADLESS=1` so `mount_spa()` disables the SPA even if a stray `web_dist/` exists — only the JSON-RPC/WS/API surface is reachable). `dashboard` and `serve` share `cmd_dashboard`/`start_server` but are independent surfaces — neither launches the other. The one exception is a backward-compat *fallback*: `serve` is newer, so the desktop spawn (`electron/backend-command.ts` + `backendSupportsServe()` in `electron/main.ts`) detects whether the resolved runtime registers `serve` and, only when it does not (an older managed install / PATH `clara` the app hasn't updated yet), rewrites the argv to the legacy `dashboard --no-open`. Without that, a new app against an un-upgraded runtime would crash on an unknown subcommand and brick every mid-upgrade user. It does NOT embed `clara --tui` — it has its own composer, transcript, and slash-command pipeline. For scoped Desktop architecture, state, resolver, transport, and testing rules, read `apps/desktop/AGENTS.md`.
...
- **The renderer curates via `apps/desktop/src/lib/desktop-slash-commands.ts`.** This is the load-bearing file. It holds `DESKTOP_COMMAND_SPECS` (the built-ins and their Desktop surfaces) plus `NO_DESKTOP_SURFACE` block-lists for terminal-only / messaging-only / picker-owned / settings-owned / advanced commands that should NOT clutter the desktop popover.
  ...
  - `isDesktopSlashExtensionCommand(name)` — true when the command is NOT a known Clara built-in (i.e. a skill or user quick command). Both suggestion and catalog-filter paths allow extensions through so skill commands surface in the palette. (Added when fixing "skill commands missing from the desktop slash palette" — the curated allow-list was silently dropping every skill/quick command from completions even though they executed fine when typed.)
...
be core tools. For custom or local-only tools, do **not** edit Clara core.
Use the plugin route instead: create `~/.clara/plugins/<name>/plugin.yaml`
and `~/.clara/plugins/<name>/__init__.py`, then register tools with
`ctx.register_tool(...)`. Plugin toolsets are discovered automatically and can be
enabled or disabled without touching `tools/` or `toolsets.py`.

Use the built-in route below only when the user is explicitly contributing a new
core Clara tool that should ship in the base system.
...
**2. Add to `toolsets.py`** — either `_CLARA_CORE_TOOLS` (all platforms) or a new toolset. **This step is required:** auto-discovery imports the tool and registers its schema, but the tool is only *exposed to an agent* if its name appears in a toolset. `_CLARA_CORE_TOOLS` is not dead code — it's the default bundle every platform's base toolset inherits from.

Auto-discovery: any `tools/*.py` file with a top-level `registry.register()` call is imported automatically — no manual import list to maintain. Wiring into a toolset is still a deliberate, manual step.

The registry handles schema collection, dispatch, availability checking, and error wrapping. All handlers MUST return a JSON string.

**Path references in tool schemas**: If the schema description mentions file paths (e.g. default output directories), use `display_clara_home()` to make them profile-aware. The schema is generated at import time, which is after `_apply_profile_override()` sets `CLARA_HOME`.

**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_clara_home()` for the base directory — never `Path.home() / ".clara"`. This ensures each profile gets its own state.
...
### config.yaml options:
1. Add to `DEFAULT_CONFIG` in `clara_cli/config.py`
...
1. Add to `OPTIONAL_ENV_VARS` in `clara_cli/config.py` with metadata:
```python
"NEW_API_KEY": {
...
| `load_config()` | `clara tools`, `clara setup`, most CLI subcommands | `clara_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
The skin engine (`clara_cli/skin_engine.py`) provides data-driven CLI visual customization. Skins are **pure data** — no code changes needed to add a new skin.

### Architecture

```
clara_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
~/.clara/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
- `default` — Classic Clara gold/kawaii (the current look)
...
Add to `_BUILTIN_SKINS` dict in `clara_cli/skin_engine.py`:
...
Users create `~/.clara/skins/<name>.yaml`:
...
Clara has two plugin surfaces. Both live under `plugins/` in the repo so
repo-shipped plugins can be discovered alongside user-installed ones in
`~/.clara/plugins/` and pip-installed entry points.

### General plugins (`clara_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.clara/plugins/`, `./.clara/plugins/`,
...
- Register CLI subcommands via `ctx.register_cli_command(...)` — the
  plugin's argparse tree is wired into `clara` at startup so
  `clara <pluginname> <subcmd>` works with no change to `main.py`
...
bundled, `$CLARA_HOME/plugins/`, `./.clara/plugins/` (opt-in via
`CLARA_ENABLE_PROJECT_PLUGINS`), and `clara_agent.memory_providers` entry
...
`post_setup(clara_home, config)` for setup-wizard integration.

**CLI commands via `plugins/memory/<name>/cli.py`:** if a memory plugin
defines `register_cli(subparser)`, `discover_plugin_cli_commands()` finds
it at argparse setup time and wires it into `clara <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `clara --help`.

**Rule (Teknium, May 2026):** plugins MUST NOT modify core files
(`run_agent.py`, `cli.py`, `gateway/run.py`, `clara_cli/main.py`, etc.).
...
into `~/.clara/plugins/` (or via pip entry points) — they implement
the same `MemoryProvider` ABC, register through the same discovery
path, and integrate via `clara memory setup` / `post_setup()` without
...
`~/.clara/plugins/` (or via pip entry points). They register through
...
don't own. Promote standalone plugins in the Workprise Discord
...
2. User: `$CLARA_HOME/plugins/model-providers/<name>/`
...
The desktop "Bots" experience ships bundled in-tree. Each bot is a Clara
...
`ui_meta['clara-bots'].chat` and verified it per click; five hardening
...
- **`optional-skills/`** — heavier or niche skills shipped with the repo but
  ...
  `clara skills install official/<category>/<skill>`. Adapter lives in
...
`metadata.clara.tags`, `metadata.clara.category`,
`metadata.clara.related_skills`, `metadata.clara.config` (config.yaml
settings the skill needs — stored under `skills.config.<key>`, prompted
during setup, injected at load time).

Top-level `tags:` and `category:` are also accepted and mirrored from
`metadata.clara.*` by the loader.
...
2. **Tools referenced in SKILL.md prose must be native Clara tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "Clara Agent" is the secondary collaborator. If the
   contributor's commit shows "Clara Agent" as author (because they
   used Clara to draft the skill), replace it with their actual name
...
lives in the `clara-agent-dev` skill at
...
`"messaging"`); `_CLARA_CORE_TOOLS` is the default bundle most
...
Enable/disable per platform via `clara tools` (the curses UI) or the
...
Clara returns a delegation id immediately and the result re-enters the
...
go to `~/.clara/skills/.archive/` and are restorable.

- **Core:** `agent/curator.py` (review loop, auto-transitions, LLM review
  prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots).
- **CLI:** `clara_cli/curator.py` wires `clara curator <verb>` where
  verbs are: `status`, `run`, `pause`, `resume`, `pin`, `unpin`,
  `archive`, `restore`, `prune`, `backup`, `rollback`.
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.clara/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
schedule jobs via the `cronjob` tool; users via `clara cron <verb>`
...
- File lock at `~/.clara/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
collaborate on shared tasks. Users drive it via `clara kanban <verb>`;
workers spawned by the dispatcher drive it via a dedicated `kanban_*`
toolset so their schema footprint is zero when they're not inside a
kanban task.

- **CLI:** `clara_cli/kanban.py` wires `clara kanban` with verbs
...
- **Plugin assets:** `plugins/kanban/dashboard/` (web UI) +
  `plugins/kanban/systemd/` (`clara-kanban-dispatcher.service` for
  standalone dispatcher deployment).

Isolation model:
- **Board** is the hard boundary — workers are spawned with
  `CLARA_KANBAN_BOARD` pinned in their env so they can't see other
  boards.
...
- **Plan** (`clara_cli/update_inventory.py`, `clara update --plan`):
...
- **Snapshot** (`clara_cli/backup.py`): pre-update quick snapshot for
...
- **Restart-per-kind**: systemd and launchd restarts are FLEET-WIDE
  (every `clara-gateway*` unit / `ai.clara.gateway*` LaunchAgent),
...
- **Report**: every run writes a machine-readable receipt to
  `~/.clara/logs/update_receipts/` (`latest.json` pointer; steps,
...
`clara serve` (control plane, desktop-spawned child) dies with the app
— by design. The messaging gateway (`gateway run`) SURVIVES the app: the
serve backend's `/api/gateway/*` endpoints spawn it detached
(`_spawn_clara_action` — `start_new_session` / `DETACHED_PROCESS`), so
...
Clara-Agent ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
in config.yaml (or `CLARA_BACKGROUND_NOTIFICATIONS` env var):
...
Clara supports **profiles** — multiple fully isolated instances, each with its own
`CLARA_HOME` directory (config, API keys, memory, sessions, skills, gateway, etc.).

The core mechanism: `_apply_profile_override()` in `clara_cli/main.py` sets
`CLARA_HOME` before any module imports. All `get_clara_home()` references
automatically scope to the active profile.

### Rules for profile-safe code

1. **Use `get_clara_home()` for all CLARA_HOME paths.** Import from `clara_constants`.
   NEVER hardcode `~/.clara` or `Path.home() / ".clara"` in code that reads/writes state.
   ```python
   # GOOD
   from clara_constants import get_clara_home
   config_path = get_clara_home() / "config.yaml"

   # BAD — breaks profiles
   config_path = Path.home() / ".clara" / "config.yaml"
   ```

2. **Use `display_clara_home()` for user-facing messages.** Import from `clara_constants`.
   This returns `~/.clara` for default or `~/.clara/profiles/<name>` for profiles.
   ```python
   # GOOD
   from clara_constants import display_clara_home
   print(f"Config saved to {display_clara_home()}/config.yaml")

   # BAD — shows wrong path for profiles
   print("Config saved to ~/.clara/config.yaml")
   ```

3. **Module-level constants are fine** — they cache `get_clara_home()` at import time,
   which is AFTER `_apply_profile_override()` sets the env var. Just use `get_clara_home()`,
   not `Path.home() / ".clara"`.

4. **Tests that mock `Path.home()` must also set `CLARA_HOME`** — since code now uses
   `get_clara_home()` (reads env var), not `Path.home() / ".clara"`:
   ```python
   with patch.object(Path, "home", return_value=tmp_path), \
        patch.dict(os.environ, {"CLARA_HOME": str(tmp_path / ".clara")}):
       ...
   ```
...
6. **Profile operations are HOME-anchored, not CLARA_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".clara" / "profiles"`, NOT `get_clara_home() / "profiles"`.
   This is intentional — it lets `clara -p coder profile list` see all profiles regardless
...
- Use the canonical matchers: `gateway.status.looks_like_gateway_command_line`
  (gateway run), `clara_cli.update_cmd._clara_holder_subcommand`
  (top-level subcommand of any Clara argv). Never hand-roll token scans.
...
### DO NOT hardcode `~/.clara` paths
Use `get_clara_home()` from `clara_constants` for code paths. Use `display_clara_home()`
for user-facing print/log messages. Hardcoding `~/.clara` breaks profiles — each profile
has its own `CLARA_HOME` directory. This was the source of 5 bugs fixed in PR #3575.

### All CLI menu-pickers MUST use curses.
Interactive menus must use `clara_cli/curses_ui.py`. See `clara_cli/tools_config.py` for an example.
...
canary ledger, clara#85796 / gateway-gateway#210). Violating any of them
...
with actual imports (not mocks) against a temp `CLARA_HOME`.

### Tests must not write to `~/.clara/`
The `_isolate_clara_home` autouse fixture in `tests/conftest.py` redirects `CLARA_HOME` to a temp dir. Never hardcode `~/.clara/` paths in tests.

**Profile tests**: When testing profile features, also mock `Path.home()` so that
`_get_profiles_root()` and `_get_default_clara_home()` resolve within the temp dir.
Use the pattern from `tests/clara_cli/test_profiles.py`:
```python
@pytest.fixture
...
subprocess (`--file-retries`, default 1; `CLARA_TEST_FILE_RETRIES=0` to
...
| HOME / `~/.clara/` | Your real config+auth.json                  | Temp dir per test                         |
...
Clara supports Linux, macOS and native Windows, and plenty of its behaviour
...
`tests/clara_cli/test_venv_holder_windows_live.py` on a real
