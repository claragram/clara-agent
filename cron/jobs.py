...
Jobs are stored in ~/.clara/cron/jobs.json
Output is saved to ~/.clara/cron/output/{job_id}/{timestamp}.md
"""
...
# store under its own CLARA_HOME, and a profile-scoped gateway runs that
# profile's jobs under that same CLARA_HOME — so a job authored in profile
# `coder` lives in `~/.clara/profiles/coder/cron/jobs.json` and executes with
# `coder`'s `.env`, `config.yaml`, and skills. We deliberately anchor on
# `get_clara_home()` (the active profile home), NOT `get_default_clara_root()`
# (the shared root). Anchoring at the root would funnel every profile's jobs
# into one shared `jobs.json` and run them under whatever CLARA_HOME the
...
# `_get_clara_home()` / `_get_lock_paths()` resolution in cron/scheduler.py.
CLARA_DIR = get_clara_home().resolve()
...
CRON_DIR = CLARA_DIR / "cron"
JOBS_FILE = CRON_DIR / "jobs.json"
# Heartbeat file the in-process ticker touches on every loop iteration. The
# gateway process and the (separate) ``clara cron status`` process share it
...
# threshold in `clara cron status` (clara_cli/cron.py), so the two never
# drift apart.
...
def _current_cron_store() -> _CronStorePaths:
    ...
    3. the ACTIVE profile home, resolved fresh via get_clara_home()
       (context-local override, then the CLARA_HOME env var) — so a test
       or embedder that re-points CLARA_HOME after this module was
...
# the cron inactivity timeout is disabled (CLARA_CRON_TIMEOUT=0 → unlimited),
...
# TTL only recovers a claim left by a tick that DIED mid-run. CLARA_CRON_TIMEOUT
...
def _oneshot_run_claim_ttl_seconds() -> float:
    ...
    Derived from ``CLARA_CRON_TIMEOUT`` (the cron inactivity timeout the
...
def _jobs_lock():
    ...
    and standalone ``clara`` CLI invocations, which previously shared no lock
...
def _preserve_file_ownership(path: Path, before: Optional[os.stat_result]) -> None:
    ...
    command (``docker exec clara clara cron create ...`` — ``docker exec``
...
def _is_named_profile_path(path: Path) -> bool:
    ...
    Named profiles live under ``<clara_home>/profiles/<name>/``.  The
    default profile lives at ``<clara_home>`` directly (no ``profiles``
    parent), as do custom ``CLARA_HOME`` paths outside ``~/.clara``.
...
def _ensure_cron_dir(cron_dir: Path) -> None:
    ...
    Default and custom Clara homes keep ``parents=True`` so first-run
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    if 'T' in schedule or re.match(r'^\d{4}-\d{2}-\d{2}', schedule):
        try:
            ...
            #
            # Anchor to the CONFIGURED Clara timezone, not the server's local
            # timezone. The due-check (`get_due_jobs`) compares `next_run_at`
            # against `clara_time.now()`, which uses the configured zone. If a
            ...
            if dt.tzinfo is None:
                clara_tz = _clara_now().tzinfo
                dt = dt.replace(tzinfo=clara_tz)
            return {
                "kind": "once",
                "run_at": dt.isoformat(),
    ...
    if schedule_lower.startswith("in "):
        ...
        run_at = _clara_now() + timedelta(minutes=minutes)
        return {
            "kind": "once",
...
def _ensure_aware(dt: datetime) -> datetime:
    """Return a timezone-aware datetime in Clara configured timezone.

    Backward compatibility:
    - Older stored timestamps may be naive.
    - Naive values are interpreted as *system-local wall time* (the timezone
      `datetime.now()` used when they were created), then converted to the
      configured Clara timezone.

    This preserves relative ordering for legacy naive timestamps across
    timezone changes and avoids false not-due results.
    """
    target_tz = _clara_now().tzinfo
    if dt.tzinfo is None:
...
def _stored_wall_clock_is_future(stored: datetime, current: datetime) -> bool:
    ...
    Cron schedules express local wall-clock intent. If Clara/system local time
...
def _record_persisted_error_recovery(job: Dict[str, Any], previous_next_run: str) -> None:
    ...
    entry = {
        ...
        "rearmed_at": _clara_now().isoformat(),
    }
    ...
    if kind == "once":
        ...
        if not _ensure_croniter():
            logger.warning(
                ...
                "reinstall clara-agent or run 'pip install croniter' in your "
                "runtime env.",
                expr,
            )
            return None
...
# =============================================================================
# Ticker heartbeat (liveness signal for `clara cron status`)
# =============================================================================
...
def _atomic_write_epoch(path: Path) -> None:
    ...
    in another process (``clara cron status``) never sees a torn/truncated
...
def record_ticker_heartbeat(success: bool = False) -> None:
    ...
    `clara cron status` can tell a thread that is merely *alive and looping*
...
def get_ticker_heartbeat_age() -> Optional[float]:
    ...
    ``clara cron status`` must report per-profile liveness (#69377).
    """
    store = _current_cron_store()
...
def record_ticker_error(message: str) -> None:
    ...
    The ticker thread lives inside the gateway process; ``clara cron
    ...
    try:
        for _attempt in range(5):
            ...
            try:
                with os.fdopen(fd, "w", encoding="utf-8") as f:
                    json.dump(
                        {"jobs": jobs, "updated_at": _clara_now().isoformat()},
                        f,
                        indent=2,
                        ensure_ascii=False,
                    )
...
def _resolve_default_model_snapshot() -> Optional[str]:
    ...
    try:
        ...
        cfg_path = get_clara_home() / "config.yaml"
...
def _normalize_reasoning_effort(value: Any) -> Optional[str]:
    ...
    every other effort surface uses (``clara_constants.parse_reasoning_effort``)
...
def create_job(
    ...
    Args:
        ...
        script: Optional path to a script whose stdout feeds the job. With
                ...
                ~/.clara/scripts/; ``.sh`` / ``.bash`` files run via bash,
        ...
        monitor_script: Optional path to a cheap monitor source script (same
                ...
                ~/.clara/scripts/, .sh/.bash via bash, else Python). Each
        ...
        reasoning_effort: Optional per-job reasoning effort pin. One of the
                canonical Clara levels (none|minimal|low|medium|high|xhigh|
    ...
    # covered, not just `clara cron create`.
...
def pause_job(job_id: str, reason: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job["id"],
        {
            "enabled": False,
            "state": "paused",
            "paused_at": _clara_now().isoformat(),
...
def trigger_job(
    ...
    if is_terminal_job(job):
        ...
        raise ValueError(
            ...
            f"Create a new occurrence with 'clara cron resume {name} "
            "--run-now' or '--at <ISO-8601>'."
        )
    manual_run_at = _clara_now().isoformat()
    return update_job(
        job["id"],
        {
            "enabled": True,
...
def _write_wedged_oneshot_diagnostic(job: Dict[str, Any]) -> None:
    ...
    try:
        ...
        text = (
            ...
            f"- removed at: {_clara_now().isoformat()}\n\n"
...
def _machine_id() -> str:
    ...
    Uses ``CLARA_MACHINE_ID`` if set, else hostname + pid. The CAS correctness
    comes from the file lock + the fresh-claim check, not from this value.
    """
    explicit = os.getenv("CLARA_MACHINE_ID", "").strip()
    if explicit:
        return explicit
    try:
        import socket
...
def _get_due_jobs_locked() -> List[Dict[str, Any]]:
    ...
    # (derived from CLARA_CRON_TIMEOUT). See _oneshot_run_claim_ttl_seconds.
    ...
    for job in jobs:
        ...
        try:
            ...
            if next_run_dt <= now:
                ...
                # desktop both run in-process 60s tickers on one CLARA_HOME)
                ...
                # (_oneshot_run_claim_ttl_seconds, derived from CLARA_CRON_TIMEOUT),
                ...
                # dispatch timing on the job record so `clara cron list` /
                # `clara cron status` (separate CLI processes) can show a
...
# execution. Unlike the quick-snapshot store (`clara_cli.backup`, capped at 20)
...
def _prune_job_output(job_output_dir: Path, keep: int) -> int:
    ...
    Mirrors the quick-snapshot retention in ``clara_cli.backup._prune_quick_snapshots``:
...
def save_job_output(job_id: str, output: str):
    ...
    timestamp = _clara_now().strftime("%Y-%m-%d_%H-%M-%S")
...
def _canonical_skill_ref(raw: Any) -> str:
    ...
    A job may store an absolute path under ``CLARA_HOME/skills`` or an
