...
This is an additive Clara extension surfaced under ACP ``_meta.clara`` so
...
compression rotates the internal Clara head, ``build_session_provenance`` lets
...
def build_session_provenance(
    ...
    *,
    ...
    """Build ``_meta.clara.sessionProvenance`` for an ACP session.

    Args:
        db: A ``SessionDB`` (must expose ``get_session``).
        acp_session_id: The stable ACP/editor-facing session handle.
        current_clara_session_id: The live internal Clara DB session id
            (``state.agent.session_id``).
        previous_clara_session_id: The internal id from before the most recent
            turn, when known. Supplied by ``prompt()`` to flag a rotation.

    Returns:
        A dict suitable for ``{"clara": {"sessionProvenance": <dict>}}`` under
        ACP ``_meta``, or ``None`` if the session can't be read.
    """
    try:
        row = db.get_session(current_clara_session_id)
    except Exception:
        return None
    if not row:
        return None
    ...
    root_id = current_clara_session_id
    compression_depth = 0
    cursor_parent = parent_id
    seen = {current_clara_session_id}
    ...
    rotated = bool(
        ...
        and previous_clara_session_id != current_clara_session_id
    )

    provenance: Dict[str, Any] = {
        "acpSessionId": acp_session_id,
        "currentClaraSessionId": current_clara_session_id,
        "rootClaraSessionId": root_id,
        "parentClaraSessionId": parent_id,
        "sessionKind": "continuation" if is_continuation else "root",
        "compressionDepth": compression_depth,
    }
    if previous_clara_session_id:
        provenance["previousClaraSessionId"] = previous_clara_session_id
    if rotated:
...
def session_provenance_meta(
    ...
    *,
    ...
    """Return a ready ``_meta`` payload: ``{"clara": {"sessionProvenance": ...}}``."""
    ...
    if prov is None:
        return None
    return {"clara": {"sessionProvenance": prov}}
