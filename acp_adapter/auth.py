"""ACP auth helpers — detect and advertise Clara authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "clara-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Clara runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Clara can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Clara.
    ...
    not have Clara provider credentials configured yet, so Clara always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Clara using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Clara provider",
            description=(
                "Open Clara' interactive model/provider setup in a terminal. "
                "Use this when Clara has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
