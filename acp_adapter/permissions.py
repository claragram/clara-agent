"""ACP permission bridging for Clara dangerous-command approvals."""
...
# Maps ACP permission option ids to Clara approval result strings.
...
def _build_permission_options(
    *, allow_permanent: bool, allow_session: bool = True,
    ...
    """Return ACP options that match Clara approval semantics."""
    ...
    # Smart DENY override — the editor must not offer a scope Clara
    ...
    if not once_only:
        options.append(PermissionOption(
            ...
            # hint while keeping Clara semantics in the option id.
            kind="allow_always",
...
def _map_outcome_to_clara(outcome: object, *, allowed_option_ids: set[str]) -> str:
    """Map an ACP permission outcome into Clara approval strings."""
    ...
    if option_id not in allowed_option_ids:
        ...
        return "deny"
    return _OPTION_ID_TO_CLARA.get(option_id, "deny")
...
def make_approval_callback(
    ...
    """
    Return a Clara-compatible approval callback that bridges to ACP.
