"""CLI entry point for the clara-agent ACP adapter.

Loads environment variables from ``~/.clara/.env``, configures logging
...
try:
    ...
    # shadowing Clara's own modules — ``clara acp`` can be started from any
...
def _load_env() -> None:
    """Load .env from CLARA_HOME (default ``~/.clara``)."""
    from clara_cli.env_loader import load_clara_dotenv

    clara_home = get_clara_home()
    loaded = load_clara_dotenv(clara_home=clara_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", clara_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="clara-acp",
        description="Run Clara Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Clara version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Clara provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.clara/node/ "
...
def _run_check() -> None:
    ...
    from acp_adapter.server import ClaraACPAgent  # noqa: F401

    print("Clara ACP check OK")


def _run_setup() -> None:
    from clara_cli.main import main as clara_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "clara", "model"]
        clara_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting clara-agent ACP adapter")
    ...
    import acp
    from .server import ClaraACPAgent
    ...
    if os.environ.get("CLARA_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
