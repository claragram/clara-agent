...
# 5. Symlinks the 'clara' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Clara Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/clara-uv-install.$$.log")"
...
if is_termux; then
    # Clara currently declares requires-python >=3.11,<3.14. Termux can expose
    ...
    if [ -z "${PYTHON_PATH:-}" ]; then
        if command -v python >/dev/null 2>&1; then
            ...
            echo -e "${RED}✗${NC} Termux Python $PYTHON_FOUND_VERSION is not supported; Clara requires Python >=3.11,<3.14"
...
# ============================================================================
# PATH setup — symlink clara into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up clara command..."

CLARA_BIN="$SCRIPT_DIR/venv/bin/clara"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$CLARA_BIN" "$COMMAND_LINK_DIR/clara"
echo -e "${GREEN}✓${NC} Symlinked clara → $COMMAND_LINK_DISPLAY_DIR/clara"
...
# ============================================================================
# Seed bundled skills into ~/.clara/skills/
# ============================================================================

CLARA_SKILLS_DIR="${CLARA_HOME:-$HOME/.clara}/skills"
mkdir -p "$CLARA_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.clara/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$CLARA_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  clara status        # Check configuration"
if is_termux; then
    echo "  clara gateway       # Run gateway in foreground"
else
    echo "  clara gateway install # Install gateway service (messaging + cron)"
fi
echo "  clara cron list     # View scheduled jobs"
echo "  clara doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m clara_cli.main setup
fi
