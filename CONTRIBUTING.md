# Contributing to Clara Agent

Thank you for contributing to Clara Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Clara to work everywhere.
...
- **Search both open *and* merged PRs and issues** for your topic or error symptom — the duplicate-check in the PR template fires at review time, after you've already done the work:
  ```bash
  gh search issues --repo claraship/clara-agent "<your terms>"
  gh search prs --repo claraship/clara-agent --state all "<your terms>"
  ```
  Or use the web UI: [issues](https://github.com/claraship/clara-agent/issues?q=) · [PRs (all states)](https://github.com/claraship/clara-agent/pulls?q=is%3Apr).
...
Related: #38284 covers the agent-side analog — Clara itself checking existing issues and PRs before deep self-troubleshooting. This section is the human-contributor complement.
...
Bundled skills (in `skills/`) ship with every Clara install. They should be **broadly useful to most users**:
...
If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Claraship Discord](https://discord.gg/j9vzaxnsg). Users can install it with `clara skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.clara/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(clara_home, config)` for setup-wizard integration
...
- Integrate with `clara memory setup` via `post_setup()` — no need to touch core code
...
The reason is maintenance load, not quality. Every external product absorbed into the core tree becomes ours to keep working against a fast-moving codebase, for a backend we don't own and can't control. Clara ships a lot and the core moves quickly; coupling third-party products into it creates an open-ended burden on the maintainers.

Publish these as a **standalone plugin repo** instead:

- Implement the relevant ABC and use the existing plugin discovery path (`~/.clara/plugins/`, project `.clara/plugins/`, or a pip entry point) — see [Build a Clara Plugin](https://agent.claraship.com/docs/guides/build-a-clara-plugin)
- Register lifecycle hooks (`pre_tool_call`, `post_tool_call`, `pre_llm_call`, `post_llm_call`, `on_session_start`, `on_session_end`), tools (`ctx.register_tool`), and CLI subcommands (`ctx.register_cli_command`) through the surface we already expose — no core changes needed
- If your plugin needs a capability the framework doesn't expose, that's a feature request to **widen the generic plugin surface** (a new hook or `ctx` method) — never special-case your plugin in core
- Promote it in the [Claraship Discord](https://discord.gg/j9vzaxnsg) `#plugins-skills-and-skins` channel so users can find and install it
...
The installer creates the Clara venv, wires the `clara` command, stamps the
install method for `clara update`, and clones the full git project into
`$CLARA_HOME/clara-agent` (usually `~/.clara/clara-agent`). That keeps your
...
Use this only if you intentionally do not want Clara' managed install layout
(for example, a throwaway clone inside a container or CI job). If you install
this way, make sure you run the `clara` entrypoint from this venv; running the
system `python3 -m clara_cli.main` can pick up unrelated system Python
packages.
...
```bash
mkdir -p ~/.clara/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.clara/config.yaml
touch ~/.clara/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.clara/.env
```

### Run

```bash
# The standard installer already put `clara` on PATH.
clara doctor
clara chat -q "Hello"
```

If you used the manual clone fallback, run `./clara` from the checkout or
symlink this clone's venv explicitly:

```bash
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/clara" ~/.local/bin/clara
```
...
├── cli.py                    # ClaraCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (clara-cli, clara-telegram, etc.)
├── clara_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── clara_cli/               # CLI command implementations
│   ├── main.py                   # Entry point, argument parsing, command dispatch
│   ├── config.py                 # Config management, migration, env var definitions
│   ├── setup.py                  # Interactive setup wizard
│   ├── auth.py                   # Provider resolution, OAuth, Clara Portal
...
├── tests/                    # Test suite
├── website/                  # Documentation site (agent.claraship.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.clara/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.clara/`)

| Path | Purpose |
|------|---------|
| `~/.clara/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.clara/.env` | API keys and secrets |
| `~/.clara/auth.json` | OAuth credentials (Clara Portal) |
| `~/.clara/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.clara/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.clara/state.db` | SQLite session database |
| `~/.clara/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.clara/cron/` | Scheduled job data |
| `~/.clara/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`clara_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.clara/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
- **Ephemeral injection**: System prompts and prefill messages are injected at API call time, never persisted to the database or logs.
- **Provider abstraction**: The agent works with any OpenAI-compatible API. Provider resolution happens at init time (Clara Portal OAuth, OpenRouter API key, or custom endpoint).
...
(for example `_CLARA_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.clara`:
...
The user may skip setup and keep loading the skill. Clara only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `clara setup` or update `~/.clara/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native Clara tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Clara Agent" is the secondary collaborator. If the contributor's commit shows "Clara Agent" as author because they used Clara to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Clara tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `clara --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Clara uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.clara/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `clara_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `clara_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Clara runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the clara wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `clara_cli/gateway.py::_scan_gateway_pids` for the pattern.
   ```
...
9. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `clara_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
15. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `clara_cli/gateway_windows.py::
...
Clara has terminal access. Security matters.
...
2. **Test manually**: Run `clara` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/claraship/clara-agent/issues)
...
- **Discord**: [discord.gg/j9vzaxnsg](https://discord.gg/j9vzaxnsg) — for questions, showcasing projects, and sharing skills
