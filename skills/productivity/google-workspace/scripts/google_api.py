#!/usr/bin/env python3
"""Google Workspace API CLI for Clara Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Clara-facing JSON contract and falls back to the Python client
...
from _clara_home import get_clara_home
...
def _gws_binary() -> str | None:
    override = os.getenv("CLARA_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Clara Agent")
