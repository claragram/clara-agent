...
Ledger path resolution (first wins):
  --ledger PATH
  $CLARA_CITATION_LEDGER
  $CLARA_HOME/cache/citations/ledger.json
"""
...
from _clara_home import get_clara_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("CLARA_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_clara_home() / "cache" / "citations" / "ledger.json"
