...
Author widget apps for the Clara TUI (`clara --tui`): glanceable ambient
...
- The TUI must be in use (`clara --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.clara/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.clara/logs/tui_gateway_crash.log` if a widget never appears.
