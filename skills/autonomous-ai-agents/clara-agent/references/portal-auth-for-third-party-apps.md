# Clara Portal — authenticating third-party apps against the subscription

Recurring user question: "Can app X (Karakeep, OpenWebUI, LibreChat, OpenViking,
LangChain pipeline, n8n flow, etc.) use my Clara Portal subscription without me
...
## Layer 1 — Is this thing a Clara plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Clara process**. Its LLM calls go through Clara's already-configured provider. | Already uses Portal if user's Clara is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
| **Karakeep, n8n, LibreChat, OpenWebUI, any self-hosted app** | Different process, often different machine. Makes its own HTTPS calls to `inference-api.claraprise.com`. | Layer 2/3 below. |

**Pitfall to avoid**: do not pitch "OAuth into Portal" as the solution for a
plugin that already runs inside Clara. That LLM call is already authenticated
via Clara's provider config. The plugin's own server auth (e.g.
...
Portal at `https://inference-api.claraprise.com/v1` is an OpenAI-compatible
inference endpoint. It accepts **bearer-token authentication only**: either

1. **A static API key** from `portal.claraprise.com → API Keys`, or
...
"Sign in with Clara Portal" SSO that third-party apps can register as clients
against. There is no shared cookie or session that browser-Portal-login
extends to other apps on the same machine.

What Clara Agent has that *feels* like OAuth — `clara login --provider clara`
opening a browser, user signs in, token lands in `~/.clara/auth.json` — is a
**Clara-specific browser flow**. Under the hood it produces a credential
Clara uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Clara's existing Portal credential out of `~/.clara/auth.json`.
2. Expose a local OpenAI-compatible endpoint at `http://localhost:NNNN/v1`.
3. Forward incoming requests to `inference-api.claraprise.com/v1` with that
   bearer attached.
...
rides on the credential Clara already holds.

Where this could live in Clara:
...
Only when the consumer is *another first-party Clara thing* (a future SDK, a
Clara-branded extension, a Discord-bot integration that needs per-user
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Clara itself.
...
1. First decide: Clara plugin (runs inside Clara) or separate app? If plugin,
   it already uses Portal via Clara's provider config — done.
2. If separate app: today, paste the static API key from Portal → API Keys.
   Base URL `https://inference-api.claraprise.com/v1`. Rate limits are
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Clara agent itself") is fully supported. Clara treats agent
...
pitch in the Tool Gateway announcement is specifically about Clara Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Clara is configured to use Portal as its provider. It is
