...
Full reference: https://agent.claraship.com/docs/user-guide/configuration
...
`clara config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `clara tools` (interactive) or `clara tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_CLARA_CORE_TOOLS` is the default bundle most platforms inherit).
