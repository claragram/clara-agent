...
Write plugins for the Clara desktop app: statusbar items, layout panes,
...
system (`~/.clara/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$CLARA_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$CLARA_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
...
- The Clara desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$CLARA_HOME/desktop-plugins/` (usually
  `~/.clara/desktop-plugins/`).

## How to Run

1. Create `$CLARA_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.clara/...` by default, or `~/.clara/profiles/<profile>/...` under a
...
The ONLY import surface is `@clara/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from Clara
  ...
  (`clara://index-network/intent/1`), a hash path (`/index-network/intent/1`),
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.clara/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@clara/plugin-sdk`, `react`, and
