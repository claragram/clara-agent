...
Full docs: https://agent.claraship.com/docs/integrations/providers
...
| clara | OAuth device code | `clara auth add clara` (or `CLARA_API_KEY`) |
| openai-codex | OAuth | `clara auth add openai-codex` |
| qwen-oauth | OAuth | `clara auth add qwen-oauth` |
| minimax-oauth | OAuth | `clara auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`clara auth`).
Fallback chain when the primary fails: `clara fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`clara_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   clara config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
