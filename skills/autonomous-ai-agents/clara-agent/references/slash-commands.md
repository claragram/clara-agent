...
Registry of record: `clara_cli/commands.py` (`COMMAND_REGISTRY`) — every
...
/snapshot [sub]          Create/restore Clara config+state snapshots (CLI)
...
/subscription (/upgrade) Clara plan info (CLI)
/topup                   Clara balance / billing
/copy [N]                Copy last response to clipboard (CLI)
/paste                   Attach clipboard image (CLI)
/image <path>            Attach a local image file (CLI)
/update                  Update Clara to latest
/version (/v)            Show version
/debug [clara|local]      Upload debug report, get shareable links
```
