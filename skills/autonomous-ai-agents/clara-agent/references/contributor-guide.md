...
For occasional contributors and PR authors. Full developer docs: https://agent.claraprise.com/docs/developer-guide/
...
Config: `~/.clara/config.yaml` (settings), `~/.clara/.env` (API keys) — both under `$CLARA_HOME` when it is set.
...
`_CLARA_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_clara_home()` for paths,
never hardcode `~/.clara`. For custom/local-only tools, write a plugin in
`~/.clara/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `clara_cli/commands.py`
...
- Tests auto-redirect `CLARA_HOME` to temp dirs — never touch real `~/.clara/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/clara_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_clara_home()` from `clara_constants` for all paths (profile-safe)
