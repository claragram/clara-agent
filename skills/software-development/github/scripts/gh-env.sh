#!/usr/bin/env bash
# GitHub environment detection helper for Clara Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _clara_env="${CLARA_HOME:-$HOME/.clara}/.env"; [ -f "$_clara_env" ] && grep -q "^GITHUB_TOKEN=" "$_clara_env" 2>/dev/null; then
