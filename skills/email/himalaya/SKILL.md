...
This skill is separate from the Clara Email gateway adapter. The gateway
adapter lets people email the agent and uses Clara' built-in IMAP/SMTP
...
To reply non-interactively from Clara, read the original message, compose a reply, and pipe it:
...
**Non-interactive (use this from Clara)** — pipe the message via stdin:
