...
work on a file several Clara processes share (gateway service, the Desktop
app's `clara serve` backend, CLI sessions, the TUI slash worker):

* `clara_state_common.fts_rebuild_admission` — full structural FTS rebuilds
* `clara_state._cross_process_repair_lock`   — writable_schema surgery / VACUUM
...
from clara_state import SessionDB, repair_state_db_schema
...
def test_repair_lock_fails_closed_when_lock_file_is_unopenable(tmp_path):
    ...
    with clara_state._cross_process_repair_lock(db_path) as holding:
