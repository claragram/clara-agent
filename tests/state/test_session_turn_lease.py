...
import clara_state
from clara_state import SessionDB, SessionTurnLeaseLostError
    ...
    def flaky_acquire(*args, **kwargs):
        ...
        if attempts["n"] == 1:
            raise sqlite3.OperationalError(
                "database is locked (another Clara process held the "
...
def test_non_expired_turn_lease_from_dead_pid_is_reclaimed(
    ...
    monkeypatch.setattr(
        clara_state, "psutil", SimpleNamespace(pid_exists=pid_exists)
    )
