"""
Tests for timezone support (clara_time module + integration points).
...
def _reset_clara_time_cache():
    """Reset the clara_time module cache."""
    clara_time.reset_cache()


# =========================================================================
# clara_time.now() — core helper
# =========================================================================

class TestClaraTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_clara_time_cache()
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["CLARA_TIMEZONE"] = "Asia/Kolkata"
        result = clara_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["CLARA_TIMEZONE"] = "Europe/London"
        tz = clara_time.get_timezone()
        assert isinstance(tz, ZoneInfo)
        assert str(tz) == "Europe/London"

    def test_cache_isolated_by_active_profile_config(self, tmp_path, monkeypatch):
        """Switching CLARA_HOME must not reuse another profile's timezone."""
        ...
        monkeypatch.delenv("CLARA_TIMEZONE", raising=False)

        monkeypatch.setenv("CLARA_HOME", str(first_home))
        assert str(clara_time.get_timezone()) == "Asia/Tokyo"

        # Multiplexed profile runtime scopes switch CLARA_HOME in one process.
        monkeypatch.setenv("CLARA_HOME", str(second_home))
        assert str(clara_time.get_timezone()) == "America/New_York"
    ...
    def test_concurrent_profile_resolution_never_mixes_zones(
        ...
        def worker(key: str) -> None:
            barrier.wait()
            for _ in range(200):
                token = set_clara_home_override(str(homes[key]))
                try:
                    tz = clara_time.get_timezone()
                    if str(tz) != zones[key]:
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When CLARA_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) CLARA_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("CLARA_TIMEZONE=" + os.environ.get("CLARA_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "CLARA_TIMEZONE=NOT_SET" in result["output"], (
            "CLARA_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When CLARA_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("CLARA_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=clara_tz) which shifted
        absolute time when system-local tz != Clara tz.  The fix interprets
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Clara tz
        ...
        # Use a Clara timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _clara_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["CLARA_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_clara_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Clara timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["CLARA_TIMEZONE"] = "US/Eastern"
        _reset_clara_time_cache()
