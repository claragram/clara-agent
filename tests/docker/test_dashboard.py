"""Harness: dashboard opt-in via CLARA_DASHBOARD.

Today (tini): dashboard starts once when CLARA_DASHBOARD=1; if it crashes
...
Every ``docker exec`` here runs as the unprivileged ``clara`` user
...
def test_dashboard_not_running_by_default(
    ...
    """Without CLARA_DASHBOARD, no dashboard process should be running."""
    start_container(built_image, container_name, cmd="sleep 60")
    r = docker_exec(container_name, "pgrep", "-f", "clara dashboard")
    # pgrep exits non-zero when no match found
    assert r.returncode != 0, (
        "Dashboard should not be running without CLARA_DASHBOARD"
    )
...
# whenever `CLARA_DASHBOARD_HOST` was non-loopback, silently disabling
...
def test_dashboard_oauth_gate_engages_on_non_loopback_bind(
    ...
    when a ``DashboardAuthProvider`` is registered (the bundled clara
    provider activates whenever ``CLARA_DASHBOARD_OAUTH_CLIENT_ID`` is
    set).
    ...
    1. ``/api/auth/providers`` (publicly reachable through the gate so
       the login page can bootstrap) returns 200 with ``clara`` in the
    ...
    start_container(
        ...
        "CLARA_DASHBOARD=1",
        "CLARA_DASHBOARD_HOST=0.0.0.0",
        "CLARA_DASHBOARD_OAUTH_CLIENT_ID=agent:test-instance",
        cmd="sleep 120",
    )
    ...
    assert "clara" in provider_names, (
        "Bundled dashboard_auth/clara provider should register when "
        f"CLARA_DASHBOARD_OAUTH_CLIENT_ID is set. Got: {payload!r}"
    )
    ...
    assert status_code == 401, (
        ...
        "when a provider is registered and CLARA_DASHBOARD_INSECURE "
...
def test_dashboard_insecure_env_var_no_longer_bypasses_gate(
    ...
    """``CLARA_DASHBOARD_INSECURE=1`` NO LONGER disables the auth gate
