...
Override the image with ``CLARA_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("CLARA_TEST_IMAGE", "clara-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``CLARA_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("CLARA_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged clara user
# ---------------------------------------------------------------------------
#
# Background: every Clara runtime path inside the container drops to UID
# 10000 (the ``clara`` user) via ``s6-setuidgid clara``. ``docker exec``
...
# fails (PermissionError swallowed) for clara, so a test that ran as root
...
# helpers below run every probe under ``-u clara`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: clara).
