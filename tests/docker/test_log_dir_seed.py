"""Runtime smoke test for Docker $CLARA_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the clara user after container boot.
...
registered later runs its log service as the dropped clara user and
...
def test_logs_gateways_seeded_and_clara_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by clara after boot."""
    ...
    assert "logs=clara" in r.stdout, (
        f"logs/ not owned by clara: {r.stdout}"
    )
    assert "gateways=clara" in r.stdout, (
        f"logs/gateways/ not owned by clara: {r.stdout}"
    )
