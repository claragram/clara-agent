...
Phase 4 wires `clara -p <profile> gateway start/stop` through the s6
...
def _wait_for_want_state(container_name: str, want_up: bool, timeout: float = 15.0) -> None:
    ...
    s6 state transitions are __PROT_0_asynchroclara__; fixed two-second sleeps flaked
    on loaded CI hosts.
    """
...
def test_profile_create_then_gateway_start(
    ...
    r = _sh(container_name, f"clara profile create {PROFILE}")
    ...
    r = _sh(container_name, f"clara -p {PROFILE} gateway start", timeout=60)
    ...
    r = _sh(container_name, f"clara -p {PROFILE} gateway stop", timeout=30)
    assert r.returncode == 0
