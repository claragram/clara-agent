...
def progress(tmp_path):
    ...
    status = tmp_path / "clara-update-status"
...
# Stands in for `clara update`, and reports the stage that was on screen
...
n="$(cat "$CLARA_TEST_CALLS" 2>/dev/null || echo 0)"; n=$((n + 1))
printf '%s' "$n" > "$CLARA_TEST_CALLS"
for f in "$TMPDIR"/clara-update-status.[0-9]*; do
  case "$f" in *.tmp) continue ;; esac
  cp "$f" "$CLARA_TEST_CAPTURE.$n" 2>/dev/null
done
exit "$(cat "$CLARA_TEST_EXITS.$n" 2>/dev/null || echo 0)"
"""
...
def _run_handoff(tmp_path, exits: dict[int, int]) -> list[dict]:
    ...
    install_root = tmp_path / "clara-agent"
    (install_root / "venv" / "bin").mkdir(parents=True)
    clara = install_root / "venv" / "bin" / "clara"
    clara.write_text(FAKE_CLARA)
    clara.chmod(0o755)
    ...
    env = {
        **os.environ,
        "TMPDIR": str(tmp_path),
        "CLARA_TEST_CAPTURE": str(capture),
        "CLARA_TEST_CALLS": str(calls),
        "CLARA_TEST_EXITS": str(tmp_path / "exits"),
    }
    ...
    result = tmp_path / ".clara-update-result.json"
...
def test_update_gate_publishes_its_stage_before_running(tmp_path):
    """`clara update` is the longest wait in the hand-off and the one the
