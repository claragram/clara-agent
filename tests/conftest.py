"""Shared fixtures for the clara-agent test suite.
...
2. **Isolated CLARA_HOME.** CLARA_HOME points to a per-test tempdir so
   code reading ``~/.clara/*`` via ``get_clara_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".clara"`` instead of the canonical
   ``get_clara_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No CLARA_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox CLARA_HOME before ANY test module is imported ──────────────────
# `clara_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
# `get_clara_home()` and attaches rotating file handlers to the ROOT logger.
...
# real `~/.clara/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes CLARA_HOME, but fixtures run
...
# the REAL Clara root — capture it BEFORE the sandbox rewires CLARA_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.clara (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("CLARA_KANBAN_HOME", "").strip()
_PRE_SANDBOX_CLARA_HOME = os.environ.get("CLARA_HOME", "")


def _clara_home_points_at_production(value: str) -> bool:
    """True when a pre-set CLARA_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    CLARA_HOME=~/.clara``) hand pytest the PRODUCTION home. Historically
    the session sandbox below honored any pre-set value, so collection-time
    imports (logging handlers, ``clara_state.DEFAULT_DB_PATH``) froze paths
    inside the real ``~/.clara`` — the escape vector that landed pytest
    ...
    CLARA_HOME is honored now.
    """
    if not value:
        return True
    try:
        resolved = Path(value).expanduser().resolve()
        real_root = (Path.home() / ".clara").resolve()
    except Exception:
        return True
...
if _clara_home_points_at_production(os.environ.get("CLARA_HOME", "")):
    _SESSION_CLARA_HOME = tempfile.mkdtemp(prefix="clara-test-home-")
    os.environ["CLARA_HOME"] = _SESSION_CLARA_HOME
    atexit.register(shutil.rmtree, _SESSION_CLARA_HOME, True)
...
# like a real CLI") — which used to disarm clara_state's live-DB guard in
# the child at the same moment the child lost the CLARA_HOME redirect.
# CLARA_TEST_ISOLATION is OUR marker: exported here (before any test module
# imports), inherited by every child by default, and honored by
# clara_state._running_under_pytest() as a test-context signal. A child
...
# open a real DB must export CLARA_STATE_DB_GUARD_BYPASS=1 in that child's
# env instead of stripping markers.
os.environ["CLARA_TEST_ISOLATION"] = os.environ.get("CLARA_HOME", "") or "1"

#: CLARA_HOME as it stood when conftest was imported - i.e. before any test
...
CLARA_HOME_AT_CONFTEST_IMPORT = os.environ.get("CLARA_HOME", "")
...
# CLARA_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_CLARA_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``clara_cli.voice.speak_text`` — real synthesis,
    ...
    # any test run launched FROM a Clara agent session inherits them and
    # clara_constants home-resolution helpers prefer them over monkeypatched
    ...
    "CLARA_MAX_ITERATIONS",
    "CLARA_SESSION_PLATFORM",
    "CLARA_SESSION_CHAT_ID",
    "CLARA_SESSION_CHAT_NAME",
    "CLARA_SESSION_CHAT_TYPE",
    "CLARA_SESSION_THREAD_ID",
    "CLARA_SESSION_SOURCE",
    "CLARA_SESSION_KEY",
    "CLARA_GATEWAY_SESSION",
    "CLARA_CRON_SESSION",
    "_CLARA_GATEWAY",
    "CLARA_PLATFORM",
    "CLARA_MODEL",
    "CLARA_INFERENCE_MODEL",
    "CLARA_INFERENCE_PROVIDER",
    "CLARA_TUI_PROVIDER",
    "CLARA_MANAGED",
    "CLARA_MANAGED_DIR",
    "CLARA_DEV",
    ...
    "CLARA_TIMEZONE",
    "CLARA_REDACT_SECRETS",
    "CLARA_BACKGROUND_NOTIFICATIONS",
    "CLARA_EXEC_ASK",
    "CLARA_HOME_MODE",
    "CLARA_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.clara/kanban.db instead of the per-test CLARA_HOME.
    "CLARA_KANBAN_DB",
    "CLARA_KANBAN_BOARD",
    "CLARA_KANBAN_HOME",
    "CLARA_KANBAN_WORKSPACES_ROOT",
    "CLARA_KANBAN_LOGS_ROOT",
    "CLARA_KANBAN_TASK",
    "CLARA_KANBAN_WORKSPACE",
    "CLARA_KANBAN_RUN_ID",
    "CLARA_KANBAN_CLAIM_LOCK",
    "CLARA_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    # otherwise-unrelated config tests away from the default "clara" host.
    "CLARA_HONCHO_HOST",
    # Dashboard OAuth auth gate (PR #30156). When set, the bundled
    # dashboard-auth `clara` plugin auto-registers itself on plugin discovery,
    ...
    "CLARA_DASHBOARD_OAUTH_CLIENT_ID",
    "CLARA_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and CLARA_HOME to per-test tempdirs so code that
    reads ``~/.clara/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral CLARA_* vars that could change test semantics.
    for name in _CLARA_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("CLARA_HONCHO_HOST", "clara")

    # 3. Redirect CLARA_HOME to a per-test tempdir. Code that reads
    #    ``~/.clara/*`` via ``get_clara_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.clara/*`` via
    #    ``Path.home() / ".clara"`` instead of ``get_clara_home()``
    #    is a bug to fix at the callsite.
    fake_clara_home = tmp_path / "clara_test"
    fake_clara_home.mkdir()
    (fake_clara_home / "sessions").mkdir()
    (fake_clara_home / "cron").mkdir()
    (fake_clara_home / "memories").mkdir()
    (fake_clara_home / "skills").mkdir()
    monkeypatch.setenv("CLARA_HOME", str(fake_clara_home))
    ...
    # clara_state's live-DB guard stays armed in them even when the test
    # strips pytest's own PYTEST_* vars from the child env.
    monkeypatch.setenv("CLARA_TEST_ISOLATION", str(fake_clara_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("CLARA_STATE_DB_GUARD_BYPASS", raising=False)

    # 3b. clara_state computes ``DEFAULT_DB_PATH = get_clara_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from clara_state import ...``) that
    ...
    clara_state_mod = sys.modules.get("clara_state")
    if clara_state_mod is not None and hasattr(clara_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            clara_state_mod, "DEFAULT_DB_PATH", fake_clara_home / "state.db"
        )
    ...
    monkeypatch.setenv("CLARA_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.clara/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import clara_cli.plugins as _plugins_mod
        ...
        # previous test's tmp_path CLARA_HOME can't leak forward. Paths
...
@pytest.fixture(autouse=True)
def _isolate_clara_home(_hermetic_environment):
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from clara_cli import kanban_db as _kb_mod
    except Exception:
        return
...
# invocation), kanban writes silently pollute the real ~/.clara. This autouse
...
# allow-list because test-level fixtures legitimately move CLARA_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session CLARA_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``CLARA_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) Clara root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_CLARA_HOME and not _clara_home_points_at_production(
        _PRE_SANDBOX_CLARA_HOME
    ):
        # CLARA_HOME was genuinely set to a CUSTOM root before the sandbox
        ...
        return get_default_clara_root().resolve()
    # No pre-existing CLARA_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".clara").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.clara`` captured at import time. Hermetic tests that legitimately
    move CLARA_HOME to sibling tempdirs are unaffected.

    Only patches when ``clara_cli.kanban_db`` is *already imported* — a
    ...
    """
    _kdb = sys.modules.get("clara_cli.kanban_db")
    if _kdb is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import clara_cli.kanban_db`
...
# ``clara_state._ensure_test_isolation`` (the single choke point every
# ``SessionDB()`` construction goes through) refuses, under pytest, any DB
# path that resolves inside the REAL Clara root. This fixture wires the
# test-side knobs:
...
#     installs where CLARA_HOME is not ~/.clara) into the guard's
...
# so subprocess children that import clara_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("clara_state")
    ...
    if _PRE_SANDBOX_CLARA_HOME and not _clara_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_CLARA_HOME).expanduser().resolve()
        )
    monkeypatch.setattr(
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local Clara home override redirects every later
    # ``get_clara_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from clara_constants import get_clara_home_override, set_clara_home_override

        if get_clara_home_override() is not None:
            set_clara_home_override(None)
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal clara config dict suitable for unit tests."""
    return {
...
def _ensure_current_event_loop(request):
    ...
    Python 3.11+ no longer guarantees a current loop for plain __PROT_0_synchroclara__ tests.
...
# environment and finds the developer's live ``clara-gateway`` process
...
# correlated with a ``tests/clara_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> clara-gateway``
...
def _wal_is_usable() -> bool:
    """True when Clara will actually put a database into WAL mode here.

    Clara refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running Clara can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a Clara managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``clara_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_clara_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_clara_home`` fixture
    redirects ``CLARA_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.clara/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``clara_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["CLARA_VOICE_TTS"] = "1"``. The
...
#      ``clara_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``clara_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``clara_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``clara_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
# OS gating
#
# Clara runs on Linux, macOS and native Windows, and a lot of its behaviour
# genuinely differs per host: PTY vs pywinpty, taskkill vs SIGTERM, launchd
# vs systemd, Keychain vs libsecret, ``%LOCALAPPDATA%`` vs ``~/.clara``.
#
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where Clara falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — Clara uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart clara-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting clara/python patterns are also blocked.
    """
    ...
    def _matches_clara_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _CLARA_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_clara_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch clara-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m clara_cli.main".
                if (
                    "clara" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live clara-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting clara/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `clara update` (or the
        # equivalent `python -m clara_cli.main update`).  These commands
        ...
        if "update" in low and (
            # clara update / clara update --gateway / setsid bash -c ... clara update
            ("clara" in low and "update" in low.split())
            or
            # python -m clara_cli.main update --gateway
            ("clara_cli" in low and "update" in low.split())
            or
            # venv/bin/clara update  (absolute path variant used in tests)
            (".venv/bin/clara" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`clara update` against the real checkout, fetching "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_CLARA_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import clara_cli.voice as _voice
    except Exception:
