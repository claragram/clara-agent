...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.clara/clara-agent/utils.py)
...
2. ``clara update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``clara_cli/main.py`` (the ``clara update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``clara_constants`` after the
