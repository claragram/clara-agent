"""Tests for clara_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Clara entry point
(clara, clara-agent, clara-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Clara entry point imports clara_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported clara_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("clara_bootstrap", None)
    import clara_bootstrap  # noqa: WPS433
    return clara_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Clara CLI
...
class TestEntryPointsImportBootstrap:
    """Every Clara entry point must import clara_bootstrap as its
    ...
    # Entry points that invoke Clara as a process.  Each one must
    # import clara_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "clara_cli/main.py",   # clara CLI (console_script)
        "run_agent.py",          # clara-agent (console_script)
        "acp_adapter/entry.py",  # clara-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import clara_bootstrap' and that
        ...
        half-finished ``clara update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``clara_bootstrap`` as a top-level module) leaves clara
        ...
        """
        # Resolve relative to the clara-agent repo root.  Tests live
        # at tests/test_clara_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # clara start even when clara_bootstrap hasn't been
        ...
        assert first_import_name == "clara_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'clara_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import clara_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Clara's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("CLARA_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["CLARA_PYTHON_SRC_ROOT"] = env
            elif "CLARA_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["CLARA_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/clara")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("CLARA_PYTHON_SRC_ROOT", None)
            else:
                os.environ["CLARA_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/clara", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Clara root.  The guard must relocate Clara to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/clara"])
        assert result[0] == "/opt/clara"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Clara root.
        assert result.index("/opt/clara") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["CLARA_PYTHON_SRC_ROOT"] = "/env/clara"
            hb.harden_import_path()
            assert sys.path[0] == "/env/clara"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("CLARA_PYTHON_SRC_ROOT", None)
