...
Tests cover:
- managed_clara_tools_enabled() subscription-based gate
...
from clara_cli.clara_account import ClaraPaidServiceAccessInfo, ClaraPortalAccountInfo
...
# managed_clara_tools_enabled
# ---------------------------------------------------------------------------
class TestManagedClaraToolsEnabled:
    """Subscription-based gate: True for paid Clara subscribers."""

    def test_disabled_when_not_logged_in(self, monkeypatch):
        monkeypatch.setattr(
            "clara_cli.clara_account.get_clara_portal_account_info",
            lambda: ClaraPortalAccountInfo(logged_in=False, source="none", fresh=False),
        )
        assert managed_clara_tools_enabled() is False
...
class TestClaraToolGatewayUnavailableMessage:
    def test_uses_entitlement_reason_for_logged_in_user(self, monkeypatch):
        monkeypatch.setattr(
            "clara_cli.clara_account.get_clara_portal_account_info",
            lambda force_fresh=False: ClaraPortalAccountInfo(
                logged_in=True,
                source="account_api",
                fresh=True,
                paid_service_access=False,
                portal_base_url="https://portal.example.test",
                paid_service_access_info=ClaraPaidServiceAccessInfo(
                    allowed=False,
        ...
        message = clara_tool_gateway_unavailable_message("managed image generation")
...
class TestResolveModalBackendState:
    ...
    @staticmethod
    def _resolve(monkeypatch, mode, *, has_direct, managed_ready, clara_enabled=False):
        """Helper to call resolve_modal_backend_state with feature flag control."""
        monkeypatch.setattr(
            "tools.tool_backend_helpers.managed_clara_tools_enabled",
            lambda: clara_enabled,
        )
    ...
    def test_auto_prefers_managed_when_available(self, monkeypatch):
        result = self._resolve(monkeypatch, "auto", has_direct=True, managed_ready=True, clara_enabled=True)
    ...
    def test_direct_selects_direct_when_available(self, monkeypatch):
        result = self._resolve(monkeypatch, "direct", has_direct=True, managed_ready=True, clara_enabled=True)
        assert result["selected_backend"] == "direct"

    def test_direct_none_when_no_credentials(self, monkeypatch):
        result = self._resolve(monkeypatch, "direct", has_direct=False, managed_ready=True, clara_enabled=True)
        assert result["selected_backend"] is None

    # --- managed mode ---


    def test_managed_blocked_when_clara_disabled(self, monkeypatch):
        result = self._resolve(monkeypatch, "managed", has_direct=True, managed_ready=True, clara_enabled=False)
        assert result["selected_backend"] is None
