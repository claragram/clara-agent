...
def _restore_tool_modules():
    original_clara_home = os.environ.get("CLARA_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_clara_home is None:
            os.environ.pop("CLARA_HOME", None)
        else:
            os.environ["CLARA_HOME"] = original_clara_home
        _reset_modules(("tools", "clara_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    clara_cli = types.ModuleType("clara_cli")
    clara_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["clara_cli"] = clara_cli
    clara_home = tmp_path / "clara-home"
    os.environ["CLARA_HOME"] = str(clara_home)
    sys.modules["clara_cli.config"] = types.SimpleNamespace(
        get_clara_home=lambda: clara_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="clara-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": clara_home / "modal_snapshots.json",
