...
def clean_env(monkeypatch):
    ...
    monkeypatch.delenv("CLARA_LOCAL_STT_COMMAND", raising=False)
    monkeypatch.delenv("CLARA_LOCAL_STT_LANGUAGE", raising=False)
...
class TestTranscribeLocalCommand:
    def test_command_provider_uses_sanitized_child_env(self, monkeypatch):
        """Salvage of #56332: command STT must not inherit Clara secrets."""
    ...
    def test_command_fallback_with_template(self, monkeypatch, sample_ogg, tmp_path):
        ...
        monkeypatch.setenv("CLARA_LOCAL_STT_LANGUAGE", "en")
...
def mock_xai_http_module():
    ...
    fake_module = MagicMock()
    fake_module.clara_xai_user_agent = MagicMock(return_value="clara-xai/test")
...
class TestTranscribeXAI:
    ...
    def test_sends_language_and_format(self, monkeypatch, sample_ogg, mock_xai_http_module):
        ...
        monkeypatch.setenv("CLARA_LOCAL_STT_LANGUAGE", "fr")
...
class TestExplicitOpenaiSelectionError:
    ...
    with the ``clara tools`` remediation for managed-Clara users), the old
    ...
    def test_get_provider_openai_none_not_generic_when_managed_route_down(
        ...
        monkeypatch.setattr(
            "tools.transcription_tools.managed_clara_tools_enabled", lambda: True
        )
        monkeypatch.setattr(
    ...
    def test_dispatch_returns_selection_specific_error(self, monkeypatch):
        ...
        its clara tools remediation instead of the all-provider install
        hint."""
        self._no_openai_credentials(monkeypatch)
        monkeypatch.setattr(
            "tools.transcription_tools.managed_clara_tools_enabled", lambda: True
        ...
        with patch("tools.transcription_tools._HAS_OPENAI", True), \
             patch("tools.transcription_tools._HAS_FASTER_WHISPER", False), \
             patch(
                 "tools.transcription_tools.clara_tool_gateway_unavailable_message",
                 lambda what: f"managed route down for {what}; run `clara tools`",
             ):
        ...
        assert "clara tools" in result["error"]
