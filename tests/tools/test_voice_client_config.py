...
Real config files in a temp CLARA_HOME, real resolution chains (no mocked
...
def voice_home(tmp_path, monkeypatch):
    """Isolated CLARA_HOME + reloaded config modules; yields a config writer."""
    home = tmp_path / ".clara"
    home.mkdir()
    ...
    for var in (
        ...
        "DEEPINFRA_API_KEY", "CLARA_LOCAL_STT_LANGUAGE",
    ):
