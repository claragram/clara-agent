...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad CLARA_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: CLARA_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_clara_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "CLARA_HOME": "/h", "CLARA_PROFILE": "p",
        "CLARA_CONFIG": "/c.yaml", "CLARA_ENV": "/e",
        "CLARA_DELEGATED_CHILD_CONTEXT": "1",
        # other CLARA_* → dropped (broad prefix removed)
        "CLARA_BASE_URL": "https://x", "CLARA_INTERACTIVE": "1",
        "CLARA_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in (
        ...
        "CLARA_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "CLARA_BASE_URL", "CLARA_INTERACTIVE", "CLARA_KANBAN_DB",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped CLARA_* vars."""
    import logging
