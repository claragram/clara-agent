...
@pytest.fixture(autouse=True)
def _enable_managed_clara_tools(monkeypatch):
    """Patch the source modules so managed_clara_tools_enabled() returns True
    even after tool modules are dynamically reloaded."""
    monkeypatch.setattr(
        "clara_cli.clara_account.get_clara_portal_account_info",
        lambda: ClaraPortalAccountInfo(
            logged_in=True,
            source="jwt",
            fresh=False,
...
def test_managed_fal_submit_uses_gateway_origin_and_clara_token(monkeypatch):
    captured = {}
    _install_fake_tools_package()
    _install_fake_fal_client(captured)
    monkeypatch.delenv("FAL_KEY", raising=False)
    monkeypatch.setenv("FAL_QUEUE_GATEWAY_URL", "http://127.0.0.1:3009")
    monkeypatch.setenv("TOOL_GATEWAY_USER_TOKEN", "clara-token")
    ...
    assert captured["client_key"] == "clara-token"
...
def test_openai_tts_uses_managed_audio_gateway_when_direct_key_absent(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("TOOL_GATEWAY_DOMAIN", "claraship.com")
    monkeypatch.setenv("TOOL_GATEWAY_USER_TOKEN", "clara-token")
    ...
    assert captured["api_key"] == "clara-token"
    assert captured["base_url"] == "https://openai-audio-gateway.claraship.com/v1"
...
def test_transcription_uses_model_specific_response_formats(monkeypatch, tmp_path):
    ...
    # The managed audio route is the stored "clara" selection (strict model);
    ...
    transcription_tools._load_stt_config = lambda: {"provider": "clara"}
    ...
    assert whisper_capture["base_url"] == "https://openai-audio-gateway.claraship.com/v1"
