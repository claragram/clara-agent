...
was gated on ``[ -f "$CLARA_HOME/.env" ]`` while the first-boot seed that
...
def _run_keygen(
    ...
    script = (
        ...
        f"{env_setup}"
        f'CLARA_HOME="{home}"\n'
        # In tests we run unprivileged; as_clara is a passthrough then.
        'as_clara() { "$@"; }\n'
...
def test_keygen_skips_when_container_env_provides_key(
    ...
    Clara loads $CLARA_HOME/.env with override=True, so a key generated
...
def test_dockerignore_keeps_env_example_template() -> None:
    """The first-boot seed copies /opt/clara/.env.example -> $CLARA_HOME/.env.
    ...
    assert verdict == "included", (
        ...
        "seeds $CLARA_HOME/.env from it on first boot"
    )
