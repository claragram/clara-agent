...
def test_output_path_rejects_clara_oauth_store(tmp_path, monkeypatch):
    """TTS output_path must not bypass the shared protected-file write guard."""
    import agent.file_safety as file_safety

    clara_home = tmp_path / "clara-home"
    clara_home.mkdir()
    monkeypatch.setattr(file_safety, "_clara_home_path", lambda: clara_home)
    monkeypatch.setattr(file_safety, "_clara_root_path", lambda: clara_home)

    target = clara_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
...
def test_output_path_rejects_mcp_token_directory(tmp_path, monkeypatch):
    ...
    clara_home = tmp_path / "clara-home"
    token_dir = clara_home / "mcp-tokens"
