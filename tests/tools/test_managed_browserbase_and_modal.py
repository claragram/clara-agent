...
def _enable_managed_clara_tools(monkeypatch):
    """Ensure managed_clara_tools_enabled() returns True even after module reloads.
    ...
    clara_cli.clara_account — so the function body returns True.
    """
    monkeypatch.setattr(
...
def _install_fake_tools_package():
    ...
    # clara_cli.auth (imported transitively by clara_account /
...
def test_browser_use_explicit_local_mode_stays_local_even_when_managed_gateway_is_ready(tmp_path):
    ...
    env.update({
        ...
        "TOOL_GATEWAY_USER_TOKEN": "clara-token",
