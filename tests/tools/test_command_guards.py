...
def _mode_manual(monkeypatch):
    ...
    The test conftest redirects CLARA_HOME to an empty tempdir, so the
...
def _clean_state():
    ...
    saved = {}
    for k in ("CLARA_INTERACTIVE", "CLARA_GATEWAY_SESSION", "CLARA_EXEC_ASK", "CLARA_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["CLARA_INTERACTIVE"] = "1"
        session_key = os.getenv("CLARA_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No CLARA_INTERACTIVE or CLARA_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
...
class TestGatewayApprovalAllowPermanent:
    ...
    def _capture_gateway_payload(self, command, session_key):
        ...
        def notify(data):
            ...
            # The notify fires __PROT_0_synchroclaraly__ before _await_gateway_decision
