...
class TestSnapshotEndToEnd:
    ...
    def test_exported_env_changes_persist_between_commands(self, tmp_path):
        ...
        try:
            first = env.execute(
                'export CLARA_STICKY_ENV_PROBE="sticky"; '
                'export PATH="/tmp/clara-session-bin:$PATH"; '
                'echo "first=$CLARA_STICKY_ENV_PROBE"'
            )
            second = env.execute(
                'echo "second=$CLARA_STICKY_ENV_PROBE"; echo "PATH=$PATH"'
            )
        finally:
            env.cleanup()
    ...
    def test_snapshot_picks_up_init_file_exports(self, tmp_path, monkeypatch):
        ...
        with patch(
            ...
            try:
                result = env.execute(
                    'echo "PROBE=$CLARA_SHELL_INIT_PROBE"; echo "PATH=$PATH"'
                )
            finally:
                env.cleanup()
