...
A package-manager install (NixOS, and anything else shipping Clara from a
...
def _missing_and_installable(monkeypatch):
    ...
    depend on the host's ~/.clara/config.yaml (a local
...
def test_nixos_install_fails_fast_without_touching_the_installer(monkeypatch):
    monkeypatch.setattr("clara_cli.config.get_managed_system", lambda: "nixos")
...
def test_durable_install_target_overrides_the_guard(monkeypatch, tmp_path):
    """The container deployment sets CLARA_MANAGED *and* a writable target.

    Dockerfile sets CLARA_LAZY_INSTALL_TARGET and the NixOS container module
    passes CLARA_MANAGED=true; blocking there would break that deployment.
    """
    monkeypatch.setattr("clara_cli.config.get_managed_system", lambda: "nixos")
...
def test_unreadable_config_fails_open(monkeypatch):
    ...
    monkeypatch.setattr("clara_cli.config.get_managed_system", _raise)
