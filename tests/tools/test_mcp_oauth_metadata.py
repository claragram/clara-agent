...
Covers:
- :class:`ClaraTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.ClaraMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _CLARA_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# ClaraTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = ClaraTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("CLARA_HOME", str(tmp_path))
        storage = ClaraTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (ClaraMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: ClaraTokenStorage, **context_attrs):
    ...
    provider = _CLARA_PROVIDER_CLS.__new__(_CLARA_PROVIDER_CLS)
    provider._clara_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CLARA_HOME", str(tmp_path))
        storage = ClaraTokenStorage("mgr-srv")
        ...
        with patch.object(
            _CLARA_PROVIDER_CLS.__bases__[0], "_initialize", new=AsyncMock()
        ):
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CLARA_HOME", str(tmp_path))
        storage = ClaraTokenStorage("flow-srv")
        ...
        with patch.object(
            _CLARA_PROVIDER_CLS.__bases__[0],
            "async_auth_flow",
