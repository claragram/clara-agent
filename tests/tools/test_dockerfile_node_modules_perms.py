...
Older images made /opt/clara/ui-tui and /opt/clara/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV CLARA_TUI_DIR=/opt/clara/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R clara:clara /opt/clara/ui-tui" not in text
    assert "chown -R clara:clara /opt/clara/node_modules" not in text
