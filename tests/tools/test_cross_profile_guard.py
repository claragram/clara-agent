...
def fake_clara(tmp_path, monkeypatch):
    """Build a two-profile Clara layout and point CLARA_HOME at
    the clara-security profile (matching the original-incident shape).
    """
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_clara):
        from tools.file_tools import write_file_tool
        target = fake_clara["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_allowed_guard_retired(self, fake_clara):
        ...
        from tools.file_tools import write_file_tool
        target = fake_clara["root"] / "skills" / "shared-skill" / "SKILL.md"
    ...
    def test_non_clara_path_unaffected(self, fake_clara, tmp_path):
        from tools.file_tools import write_file_tool
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_allowed_guard_retired(self, fake_clara):
    ...
    def test_cross_profile_patch_bypass(self, fake_clara):
        from tools.file_tools import patch_tool
        target = fake_clara["root"] / "skills" / "shared-skill" / "SKILL.md"
        result_json = patch_tool(
            mode="replace",
    ...
    def test_v4a_patch_writes_through_guard_retired(self, fake_clara):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        self, fake_clara, monkeypatch
    ):
        """The original incident shape — model expects 'foo' in active
        profile, but 'foo' lives in default. Error must point at default."""
        self._make_skill_in_profile(fake_clara["root"], "default-only-skill")

        # Re-import the module so SKILLS_DIR picks up CLARA_HOME (set in
        ...
        assert "not found in active profile 'clara-security'" in err
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.clara/profiles/<name>/."""
        # Don't set CLARA_HOME — falls back to default.
        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_clara_home_path", lambda: tmp_path / "fake")
        monkeypatch.setattr(fs, "_clara_root_path", lambda: tmp_path / "fake")
    ...
    def test_named_profile_line_in_prompt_text(self, fake_clara):
        """When active profile is 'clara-security', the prompt warns
        ...
        assert "Active Clara profile" in src
        assert "cross_profile=True" not in src  # guard retired
        assert "~/.clara/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active Clara profile: default" in src
        assert "Active Clara profile: {active_profile}" in src
