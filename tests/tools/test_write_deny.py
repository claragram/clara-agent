...
class TestWriteDenyExactPaths:
    ...
    def test_clara_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.clara/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "clara_root"
        ...
        # Sanity check: CLARA_HOME does point to the profile dir, not the root.
        from clara_constants import get_clara_home, get_default_clara_root
...
class TestWriteAllowed:
    ...
    def test_clara_control_files_requested_writable(self):
