...
Clara state stays profile-scoped through CLARA_HOME. Host subprocesses should
...
See: https://github.com/claraprise/clara-agent/issues/25114
See: https://github.com/claraprise/clara-agent/issues/36144
See: https://github.com/claraprise/clara-agent/issues/29015
"""
...
class TestGetSubprocessHome:
    """Unit tests for clara_constants.get_subprocess_home()."""

    def _host_mode(self, monkeypatch):
        monkeypatch.setattr(clara_constants, "is_container", lambda: False)
        monkeypatch.delenv("TERMINAL_HOME_MODE", raising=False)
        monkeypatch.delenv("CLARA_REAL_HOME", raising=False)
    ...
    def test_host_auto_keeps_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        clara_home = real_home / ".clara" / "profiles" / "coder"
        profile_home = clara_home / "home"
    ...
    def test_real_mode_repairs_parent_home_already_pointing_at_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CLARA_REAL_HOME", str(real_home))

        from clara_constants import get_subprocess_home, get_real_home
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CLARA_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("CLARA_HOME", str(base / "beta"))
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_host_auto_preserves_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["CLARA_REAL_HOME"] == str(real_home)
    ...
    def test_profile_mode_injects_profile_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(clara_home / "home")
        assert result["CLARA_REAL_HOME"] == str(real_home)
    ...
    def test_no_injection_when_clara_home_unset(self, monkeypatch):
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from clara_cli.profiles import _PROFILE_DIRS
