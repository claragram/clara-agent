"""Guard: Clara-owned subprocesses must not resolve managed runtimes by bare PATH.

Clara installs runtimes for itself — ``uv`` at ``$CLARA_HOME/bin/uv``, Node at
``$CLARA_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in Clara's own
...
* when a system copy also exists, the one Clara does not own wins — which is
...
``iter_clara_node_dirs()``, ``resolve_uv()``, or ``ensure_uv()``. This test is
...
# Runtimes Clara provisions into CLARA_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not Clara-owned subprocess code: plugins ship their own
...
def _source_files() -> list[Path]:
    ...
    # (clara_agent-<version>/) gets created and deleted concurrently, and
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for a Clara-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$CLARA_HOME/bin (uv) and $CLARA_HOME/node are not on an "
        ...
        "  node/npm -> clara_constants.find_node_executable()\n"
        "  PATH env -> clara_constants.iter_clara_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(clara_constants, helper))


def test_managed_uv_helpers_exist():
    from clara_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv
