...
``Invoke-ClaraStep``, which starts the step with ``RedirectStandardOutput`` /
...
last of them to close it. ``clara update`` deliberately runs build steps with
stdout inherited (the tee-stderr runner in ``clara_cli/main.py``), so the
...
closes and ``Invoke-ClaraStep`` blocks for the life of the gateway.

Everything the hand-off owes the Desktop is downstream of that call:
``.clara-update-result.json`` is never written, ``.clara-update-in-progress``
is never cleared, and the Desktop is never relaunched -- so the app sits on
"Updating Clara" until the user kills the gateway by hand, and the stale marker
...
step writing to both pipes took 18.3s vs 0.29s. ``clara update`` is exactly this
...
class TestIdleWatchdogCountsUpdateLogGrowth:
    ...
    Real updates are stdout-silent for 40+ minutes: ``clara update`` captures
    the (very loud) Electron/vite build into ``logs/update.log`` — NOT the
    child's stdout (``clara_cli/update_cmd.py``, the update-log tee) — so the
    ...
    the ``CLARA_UPDATE_STEP_IDLE_SECONDS`` override each fails a test here.
    """
...
def test_update_step_survives_pipe_leak_flood_and_live_child_stall(
    ...
    *leak* -- a step that spawns a grandchild with ``UseShellExecute = $false``
    ...
    asserts the grandchild was **still alive** when ``Invoke-ClaraStep``
    ...
    *logstall* -- a step that is silent on its pipes but appends to the
    ...
    This is the shape of every real ``clara update`` build: output streams to
    ...
    env = {
        **os.environ,
        ...
        "CLARA_UPDATE_PIPE_DRAIN_SECONDS": "3",
        "CLARA_UPDATE_STEP_IDLE_SECONDS": "3",
        "CLARA_SELFTEST_HOLD_SECONDS": "45",
    }
    ...
    assert "PIPE-DRAIN SELF-TEST: PASS" in result.stdout, (
        ...
        "Clara' forever) or metered a chatty step (backpressure on the running "
