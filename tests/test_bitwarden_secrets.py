...
binary download are exercised manually by `clara secrets bitwarden
...
def clara_home(tmp_path, monkeypatch):
    """Point Clara at an isolated home directory."""
    home = tmp_path / ".clara"
    home.mkdir()
    monkeypatch.setenv("CLARA_HOME", str(home))
    # Some modules cache get_clara_home; clear if needed.
    import clara_constants
    if hasattr(clara_constants, "_CLARA_HOME_CACHE"):
        clara_constants._CLARA_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(clara_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from clara_cli.env_loader import _apply_external_secret_sources
