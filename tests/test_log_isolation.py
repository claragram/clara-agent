"""The test suite must never write into the operator's real Clara logs.

`clara_cli/main.py` calls `setup_logging()` at module scope, which resolves
...
the whole pytest session's logging to `<CLARA_HOME>/logs/agent.log`.

If CLARA_HOME is not already sandboxed at that moment, that is the
...
`tests/conftest.py` sets CLARA_HOME at module scope for that reason - this
...
def _all_file_destinations() -> list[str]:
    ...
    try:
        ...
        listener = getattr(clara_logging, "_queue_listener", None)
        if listener is not None:
            collect(getattr(listener, "handlers", ()))
    except Exception:
        pass
...
class TestLogIsolation:
    def test_clara_home_is_sandboxed_before_imports(self):
        # Deliberately NOT os.environ: by test time the per-test `_isolate_env`
        # fixture has sandboxed CLARA_HOME, so reading it here would pass even
        # with the conftest block deleted. Assert the value captured at conftest
        # import, which is the moment that actually matters.
        from tests.conftest import CLARA_HOME_AT_CONFTEST_IMPORT as home

        assert home, "conftest must set CLARA_HOME before test modules import"
        assert Path(home).resolve() != _real_clara_home().resolve(), (
            f"CLARA_HOME pointed at the operator's real home ({home}) when "
    ...
    def test_importing_the_cli_does_not_target_the_real_logs(self):
        pytest.importorskip("clara_cli.main")

        real_logs = str(_real_clara_home() / "logs")
        offenders = [p for p in _all_file_destinations() if p.startswith(real_logs)]

        assert offenders == [], (
            "the test session is writing into the operator's real Clara logs:\n  "
            + "\n  ".join(offenders)
        )
