"""display_clara_home() renders POSIX separators on every platform.

Maintainer catch (#95681 arc): on Windows with a custom CLARA_HOME under
the user profile (e.g. AppData/Local/clara), ``"~/" +
str(home.relative_to(Path.home()))`` produced the mixed-separator chimera
``~/AppData\\Local\\clara`` — which then leaked into every consumer that
appends sub-paths (the skill_manage schema showed the agent
``~/AppData\\Local\\clara/skills/``). The ``~/`` shorthand implies POSIX
...
class TestDisplayClaraHomePosix(unittest.TestCase):
    def test_nested_home_renders_forward_slashes(self):
        ...
        import clara_constants as hc

        fake_userhome = Path.home()
        nested = fake_userhome / "AppData" / "Local" / "clara"
        with patch.object(hc, "get_clara_home", return_value=nested):
            out = hc.display_clara_home()
        self.assertEqual(out, "~/AppData/Local/clara")
        self.assertNotIn("\\", out)

    def test_default_home_unchanged(self):
        import clara_constants as hc

        with patch.object(hc, "get_clara_home",
                          return_value=Path.home() / ".clara"):
            out = hc.display_clara_home()
        self.assertEqual(out, "~/.clara")

    def test_outside_home_falls_back_to_absolute(self):
        import clara_constants as hc

        outside = Path("/opt/clara-custom") if os.name != "nt" else Path("C:/opt/clara-custom")
        with patch.object(hc, "get_clara_home", return_value=outside):
            out = hc.display_clara_home()
