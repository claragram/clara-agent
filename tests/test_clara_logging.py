"""Tests for clara_logging — centralized logging setup."""
import io
import logging
import os
import stat
...
import clara_logging
# Use whatever RotatingFileHandler class clara_logging actually resolved so
# the autouse fixture's isinstance checks (which strip rotating handlers
# between tests) match the real handlers on every platform. clara_logging
...
from clara_logging import RotatingFileHandler
...
def _reset_logging_state():
    ...
    """
    clara_logging._logging_initialized = False
    ...
    clara_logging._reset_queued_handlers()
    ...
    clara_logging._install_session_record_factory()
    yield
    ...
    clara_logging._logging_initialized = False
    clara_logging.clear_session_context()


@pytest.fixture
def clara_home(tmp_path, monkeypatch):
    """Provide an isolated CLARA_HOME for logging tests.

    Uses the same tmp_path as the autouse _isolate_clara_home from conftest,
    reading it back from the env var to avoid double-mkdir conflicts.
    """
    home = Path(os.environ["CLARA_HOME"])
    return home
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, clara_home):
        log_dir = clara_logging.setup_logging(clara_home=clara_home)
        assert log_dir == clara_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, clara_home):
        clara_logging.setup_logging(clara_home=clara_home)
        root = logging.getLogger()

        agent_handlers = [
            h for h in clara_logging.rotating_file_handlers()
    ...
    def test_idempotent_no_duplicate_handlers(self, clara_home):
        clara_logging.setup_logging(clara_home=clara_home)
        clara_logging.setup_logging(clara_home=clara_home)  # second call — should be no-op
    ...
    def test_writes_to_agent_log(self, clara_home):
        clara_logging.setup_logging(clara_home=clara_home)

        test_logger = logging.getLogger("test_clara_logging.write_test")
        test_logger.info("test message for agent.log")

        # Flush handlers
        clara_logging.flush_log_queue()

        agent_log = clara_home / "logs" / "agent.log"
        assert agent_log.exists()
        content = agent_log.read_text()
        assert "test message for agent.log" in content

    def test_profile_routing_follows_context_home(self, clara_home, tmp_path):
        ...
        profile_home.mkdir()
        clara_logging.setup_logging(clara_home=clara_home)
        assert clara_logging.enable_profile_log_routing(
            [clara_home, profile_home]
        ) is True
        ...
        assert "profile-routed cron record" not in (
            clara_home / "logs" / "agent.log"
        ).read_text()




    def test_explicit_params_override_config(self, clara_home):
        """Explicit function params take precedence over config.yaml."""
        import yaml
        config = {"logging": {"level": "DEBUG"}}
        (clara_home / "config.yaml").write_text(yaml.dump(config))

        clara_logging.setup_logging(clara_home=clara_home, log_level="WARNING")
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, clara_home):
        clara_logging.setup_logging(clara_home=clara_home, mode="gateway")
        root = logging.getLogger()
    ...
    def test_gateway_log_not_created_in_cli_mode(self, clara_home):
        clara_logging.setup_logging(clara_home=clara_home, mode="cli")
        root = logging.getLogger()
    ...
    def test_gateway_log_receives_gateway_records(self, clara_home):
        ...
        gw_log = clara_home / "logs" / "gateway.log"
        assert gw_log.exists()
        assert "telegram connected" in gw_log.read_text()

    def test_gateway_log_rejects_non_gateway_records(self, clara_home):
...
class TestGuiMode:
    ...
    def test_gui_log_created(self, clara_home):
        clara_logging.setup_logging(clara_home=clara_home, mode="gui")
        root = logging.getLogger()
    ...
    def test_gui_log_receives_only_gui_components(self, clara_home):
        clara_logging.setup_logging(clara_home=clara_home, mode="gui")

        logging.getLogger("clara_cli.web_server").info("dashboard online")
        logging.getLogger("tui_gateway.ws").info("ws connected")
        logging.getLogger("gateway.run").info("gateway event")

        clara_logging.flush_log_queue()

        gui_log = clara_home / "logs" / "gui.log"
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, clara_home):
        """When session context is set, log lines include [session_id]."""
        clara_logging.setup_logging(clara_home=clara_home)
        clara_logging.set_session_context("abc123")
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = clara_logging._ComponentFilter(("gateway",))
...
class TestSetupVerboseLogging:
    ...
    def test_adds_stream_handler(self, clara_home):
        clara_logging.setup_logging(clara_home=clara_home)
        clara_logging.setup_verbose_logging()
        ...
        verbose_handlers = [
            ...
            and getattr(h, "_clara_verbose", False)
        ]
...
class TestAddRotatingHandler:
    ...
    def test_no_duplicate_for_same_path(self, tmp_path):
        ...
        clara_logging._add_rotating_handler(
            logger, log_path,
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        handlers = [h for h in clara_logging.rotating_file_handlers() if isinstance(h, RotatingFileHandler)]
        ...
        clara_logging.set_session_context("factory_test")
    ...
    def test_managed_mode_initial_open_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("clara_cli.config.is_managed", return_value=True):
                clara_logging._add_rotating_handler(
                    logger, log_path,
                    level=logging.INFO, max_bytes=1024, backup_count=1,
                    formatter=formatter,
...
class TestWindowsConcurrentLogLockTimeout:
    ...
    def _make_logger_and_handler(self, log_path: Path):
        ...
        handler = clara_logging._ManagedRotatingFileHandler(
    ...
    def test_helper_only_matches_windows_concurrent_lock_timeout(self):
        ...
        assert clara_logging._is_windows_concurrent_log_lock_timeout(
            RuntimeError("Cannot acquire lock after 20 attempts")
        )
        assert not clara_logging._is_windows_concurrent_log_lock_timeout(
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, clara_home):
        level, max_size, backup = clara_logging._read_logging_config()
        assert level is None
        assert max_size is None
        assert backup is None

    def test_reads_logging_section(self, clara_home):
        import yaml
...
class TestExternalRotationRecovery:
    ...
    def _make_handler(self, log_path: Path) -> clara_logging._ManagedRotatingFileHandler:
        handler = clara_logging._ManagedRotatingFileHandler(
    ...
    def _emit(self, handler: logging.Handler, msg: str) -> None:
        ...
        # Match the record factory that clara_logging installs at import time.
        record.session_tag = ""
        handler.emit(record)
        clara_logging.flush_log_queue()
    ...
    def test_gateway_log_attached_after_external_rotation_then_re_setup(
        ...
        """
        clara_logging.setup_logging(clara_home=clara_home, mode="gateway")
        gw_path = clara_home / "logs" / "gateway.log"
        rotated = clara_home / "logs" / "gateway.log.1"
...
class TestSafeStderr:
    ...
    def test_wraps_non_utf8_stderr(self, monkeypatch):
        ...
        result = clara_logging._safe_stderr()
...
class TestAsyncQueueLogging:
    ...
    def test_file_handlers_not_on_root(self, clara_home):
        clara_logging.setup_logging(clara_home=clara_home)
        root = logging.getLogger()
        ...
        queue_handlers = [
            h for h in root.handlers if getattr(h, "_clara_queue", False)
        ]
        assert len(queue_handlers) == 1
        # The real file handlers are discoverable via the accessor.
        assert any(
            "agent.log" in getattr(h, "baseFilename", "")
            for h in clara_logging.rotating_file_handlers()
        )
