...
- ``clara verify`` records into the evidence ledger (pass and fail),
- a passing run satisfies the verify-on-stop guard,
- the verify-on-stop nudge names ``clara verify --json`` when the workspace
...
def _workspace(tmp_path, *, scripts=None, manifest_recipe=None):
    ...
    if manifest_recipe is not None:
        clara_dir = project / ".clara"
        clara_dir.mkdir()
        (clara_dir / "environment.json").write_text(
...
def test_record_verify_run_marks_workspace_passed(clara_home):
    project = _workspace(clara_home)
    ...
    assert status["evidence"]["canonical_command"] == "clara verify"


def test_record_verify_run_records_failure(clara_home):
    project = _workspace(clara_home)
...
def test_cli_passing_run_writes_ledger_evidence(clara_home, capsys):
    project = _workspace(clara_home, manifest_recipe={"name": "Fake", "test": ["echo ok"]})
...
def test_cli_failing_run_writes_failed_evidence(clara_home, capsys):
    project = _workspace(clara_home, manifest_recipe={"name": "Fake", "test": ["false"]})
    code = run_verify_command(make_args(project))
    assert code == 1
    status = verification_status(session_id=None, cwd=project)
    assert status["status"] == "failed"
...
def test_cli_partial_run_records_targeted_scope(clara_home, capsys):
...
def test_cli_run_uses_clara_session_id_env(clara_home, capsys, monkeypatch):
    monkeypatch.setenv("CLARA_SESSION_ID", "sess-42")
    project = _workspace(clara_home, manifest_recipe={"name": "Fake", "test": ["echo ok"]})
...
# ---------------------------------------------------------------------------
# closed loop: edit -> stop guard nudge -> clara verify -> guard satisfied
# ---------------------------------------------------------------------------


def test_passing_verify_run_satisfies_stop_guard(clara_home, capsys):
    project = _workspace(clara_home, manifest_recipe={"name": "Fake", "test": ["echo ok"]})
...
# ---------------------------------------------------------------------------
# nudge wording: recipe-aware `clara verify --json` suggestion
# ---------------------------------------------------------------------------


def test_nudge_mentions_clara_verify_when_recipe_has_start(clara_home):
    project = _workspace(clara_home, scripts={"test": "vitest", "dev": "vite"})
    changed = str(project / "src" / "app.ts")
    ...
    assert "clara verify --json" in nudge
    # The cheap verify commands are still listed first.
    assert "npm run test" in nudge


def test_nudge_mentions_clara_verify_when_manifest_exists(clara_home):
    # No start script, but a saved .clara/environment.json qualifies.
    project = _workspace(
        clara_home,
        scripts={"test": "vitest"},
...
def test_nudge_keeps_plain_wording_without_recipe_start(clara_home):
    # Verify commands but no start script and no manifest: today's wording.
    project = _workspace(clara_home, scripts={"test": "vitest"})
    changed = str(project / "src" / "app.ts")
    mark_workspace_edited(session_id="s1", cwd=project, paths=[changed])
    nudge = build_verify_on_stop_nudge(session_id="s1", changed_paths=[changed])
    assert nudge is not None
    assert "clara verify" not in nudge


def test_nudge_recipe_detection_failure_is_silent(clara_home, monkeypatch):
...
def test_detect_path_merges_project_facts_commands(clara_home, capsys):
    project = _workspace(clara_home)  # package.json with no scripts
...
def test_manifest_recipe_is_not_merged(clara_home, capsys):
...
def test_merge_skips_commands_recipe_already_has(clara_home, capsys):
    project = _workspace(clara_home, scripts={"test": "vitest"})
    code = run_verify_command(make_args(project, detect_only=True))
    assert code == 0
    payload = json.loads(capsys.readouterr().out)
