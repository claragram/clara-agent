"""Global emergency stop (`clara pause` / `clara resume`) — agent/estop.py.
...
already in flight is never touched. Removing the sentinel (`clara resume`)
...
def clara_home(tmp_path, monkeypatch):
    """Point CLARA_HOME at a temp dir and reset estop module log state."""
...
def test_engage_creates_sentinel_and_is_engaged(clara_home):
    assert estop.is_engaged() is False
    estop.engage()
    assert (clara_home / "ESTOP").exists()
    assert estop.is_engaged() is True


def test_disengage_removes_sentinel(clara_home):
    estop.engage()
    assert estop.disengage() is True
    assert not (clara_home / "ESTOP").exists()
    assert estop.is_engaged() is False
    # Disengaging when not engaged is a no-op that reports False.
    assert estop.disengage() is False


def test_reason_and_timestamp_stored(clara_home):
    ...
    raw = json.loads((clara_home / "ESTOP").read_text(encoding="utf-8"))
    assert raw["reason"] == "runaway cron fan-out"


def test_get_state_none_when_disengaged(clara_home):
    assert estop.get_state() is None


def test_corrupt_sentinel_still_engages(clara_home):
    """A hand-touched/corrupt ESTOP file must still pause (fail safe)."""
    (clara_home / "ESTOP").write_text("not json", encoding="utf-8")
    assert estop.is_engaged() is True
    state = estop.get_state()
    assert state is not None
...
def test_paused_reply_none_when_disengaged(clara_home):
    assert estop.paused_reply() is None


def test_paused_reply_surfaces_reason_and_resume_hint(clara_home):
    ...
    assert "clara resume" in notice


def test_paused_reply_without_reason(clara_home):
    estop.engage()
    notice = estop.paused_reply()
    assert notice is not None
    assert "paused" in notice.lower()
...
def test_check_paused_logs_once_per_engagement(clara_home, caplog):
...
def test_cron_tick_skips_dispatch_when_engaged(clara_home, monkeypatch):
...
def test_cron_tick_resumes_after_disengage(clara_home, monkeypatch):
    from cron import scheduler
...
def test_kanban_dispatch_blocked_when_engaged(clara_home):
...
@pytest.mark.asyncio
async def test_gateway_new_turn_gets_paused_reply(clara_home):
...
@pytest.mark.asyncio
async def test_gateway_internal_events_bypass_estop(clara_home):
...
# ── CLI: clara pause / clara resume ───────────────────────────────────────


def test_cli_pause_engages_with_reason(clara_home, capsys):
    from clara_cli.subcommands.pause import cmd_pause
...
def test_cli_pause_idempotent(clara_home, capsys):
    from clara_cli.subcommands.pause import cmd_pause
...
def test_cli_resume_disengages(clara_home, capsys):
    from clara_cli.subcommands.pause import cmd_pause, cmd_resume
...
def test_cli_resume_when_not_paused(clara_home, capsys):
    from clara_cli.subcommands.pause import cmd_resume
...
def test_builtin_subcommands_include_pause_resume():
    from clara_cli.main import _BUILTIN_SUBCOMMANDS

    assert "pause" in _BUILTIN_SUBCOMMANDS
    assert "resume" in _BUILTIN_SUBCOMMANDS


# ── clara status surfacing ─────────────────────────────────────────────────


def test_status_line_when_paused(clara_home):
    from clara_cli.status import _estop_status_line
...
def test_is_engaged_fails_safe_on_stat_error(clara_home, monkeypatch):
    """A stat failure must report ENGAGED (fail safe) — the pause has to
    hold even when CLARA_HOME is misbehaving, matching the module's
...
@pytest.mark.asyncio
async def test_gateway_slash_commands_bypass_estop(clara_home):
    ...
    try:
        ...
        return
    assert reply is None or "clara is paused" not in (reply or "").lower()
...
@pytest.mark.asyncio
async def test_gateway_pause_command_engages_and_resumes(clara_home):
    from gateway.run import GatewayRunner
...
def test_pause_command_registered_for_gateway():
    from clara_cli.commands import GATEWAY_KNOWN_COMMANDS, resolve_command
...
def test_profile_gateway_honors_canonical_root_estop(tmp_path, monkeypatch):
    """fleet-analyst-class: CLARA_HOME is a profile dir; pause lives at root.

    A process launched with CLARA_HOME=~/.clara/profiles/fleet-analyst must
    still treat ~/.clara/ESTOP as engaged. Otherwise `clara pause` is not
