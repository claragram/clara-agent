...
The gateway runs one asyncio loop for every session; SessionDB is __PROT_0_synchroclara__,
...
async def test_offload_goes_through_to_thread(monkeypatch):
    ...
    monkeypatch.setattr(clara_state.asyncio, "to_thread", _spy)
...
async def test_concurrent_claim_handoff_single_winner(tmp_path):
    db = AsyncSessionDB(clara_state.SessionDB(db_path=tmp_path / "state.db"))
    sid = "s-handoff"
