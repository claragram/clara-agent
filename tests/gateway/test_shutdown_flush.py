...
def test_recover_closes_owned_db_when_unexpected_exception_escapes(
    ...
    db = InterruptingDB()
    monkeypatch.setattr("clara_state.get_shared_session_db", lambda: db)
    monkeypatch.setattr(
        "clara_state.release_or_close", lambda _: setattr(db, "released", True)
    )
...
def test_get_flush_dir_uses_get_clara_home(tmp_path, monkeypatch):
    """Flush dir must use get_clara_home(), not hardcoded Path.home()."""
    import gateway.shutdown_flush as mod

    captured = {}

    def fake_get_clara_home():
        from pathlib import Path
        captured["called"] = True
        return tmp_path

    monkeypatch.setattr(
        "clara_constants.get_clara_home", fake_get_clara_home
    )
