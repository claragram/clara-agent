...
(``GatewayRunner._handoff_watcher``) polled the __PROT_0_synchroclara__, blocking
...
reverting any ``await self._session_db.<call>(...)`` back to a direct __PROT_1_synchroclara__
...
async def test_watcher_wraps_calls_via_asyncio_to_thread(monkeypatch):
    ...
    clara_state) and records which SessionDB callables were handed to it.
    ...
    wrapped = []
    real_to_thread = clara_state.asyncio.to_thread

    async def _spy_to_thread(func, *args, **kwargs):
        wrapped.append(getattr(func, "__name__", repr(func)))
        return await real_to_thread(func, *args, **kwargs)

    monkeypatch.setattr(clara_state.asyncio, "to_thread", _spy_to_thread)
