...
``clara_cli/setup.py::_setup_slack`` before the Slack adapter migrated to a
bundled plugin (#41112). ``interactive_setup`` lazy-imports its CLI helpers
from ``clara_cli.config`` (get_env_value / save_env_value / remove_env_value)
and ``clara_cli.cli_output`` (prompt / prompt_yes_no / print_*), so we patch
those source modules.
"""
import clara_cli.config as config_mod
import clara_cli.cli_output as cli_output_mod
...
def _patch_setup_io(monkeypatch, prompts, saved, removed, existing):
    ...
    # Manifest writing reaches out to clara_cli.slack_cli + filesystem; stub it.
    import clara_cli.slack_cli as slack_cli_mod
