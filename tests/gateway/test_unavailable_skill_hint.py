...
* ``disabled`` membership is checked by the declared name, because that
  is what :func:`clara_cli.skills_config.save_disabled_skills` stores.
"""
...
def tmp_skills(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Isolated skills dir + CLARA_HOME so the real user config is untouched."""
    home = tmp_path / ".clara"
    home.mkdir()
    (home / "skills").mkdir()
...
def test_frontmatter_slug_matched_even_when_dir_name_differs(
    ...
    # Config disables by declared name (matches what `clara skills config` writes).
    monkeypatch.setattr(
    ...
    assert "clara skills config" in msg
