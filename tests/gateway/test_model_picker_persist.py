...
closure the PR changed, against a real temp ``CLARA_HOME``.
"""
...
def _fake_switch_result():
    ...
    from clara_cli.model_switch import ModelSwitchResult
...
def _stub_picker_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(
        "clara_cli.model_switch.list_picker_providers",
    ...
    monkeypatch.setattr(
        "clara_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(gateway_run, "_clara_home", clara_home)
    _stub_picker_dependencies(monkeypatch)
    # save_config writes to ``get_clara_home() / config.yaml`` — point it here.
    monkeypatch.setattr("clara_constants.get_clara_home", lambda: clara_home)
    monkeypatch.setattr("clara_cli.config.get_clara_home", lambda: clara_home)
    return cfg_path
...
async def test_multiplex_picker_global_persists_only_named_profile(
    ...
    monkeypatch.setattr(gateway_run, "_clara_home", default_home)
    _stub_picker_dependencies(monkeypatch)
