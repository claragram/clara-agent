...
def _mention_entity(text, mention="@clara_bot"):
...
def test_observed_group_context_uses_shared_source_and_prompt_for_later_mentions():
    async def _run():
        ...
        text = "@clara_bot what did Alice say?"
        msg = _group_message(
            text,
            from_user_id=222,
...
def test_observed_group_context_preserves_slash_command_text_for_dispatch():
    ...
    event = MessageEvent(
        ...
        raw_message=_group_message(
            "/new@clara_bot",
            entities=[_bot_command_entity("/new@clara_bot", "/new@clara_bot")],
        ),
    )

    attributed = adapter._apply_telegram_group_observe_attribution(event)

    assert attributed.text == "/new@clara_bot"
...
def test_group_messages_can_require_direct_trigger_via_config():
    ...
    assert adapter._should_process_message(_group_message("hi @clara_bot", entities=[_mention_entity("hi @clara_bot")])) is True
    ...
    assert adapter._should_process_message(
        _group_message(
            "/status@clara_bot",
            entities=[_bot_command_entity("/status@clara_bot", "/status@clara_bot")],
        ),
        is_command=True,
    ) is True
...
def test_raw_bot_mention_fallback_does_not_match_email_or_substring():
    adapter = _make_adapter(require_mention=True, bot_username="clara_bot")

    assert adapter._should_process_message(_group_message("email ops@clara_bot.example")) is False
    assert adapter._should_process_message(_group_message("prefix@clara_bot hi")) is False
    assert adapter._should_process_message(_group_message("hi @clara_bot")) is True
...
def test_guest_mode_allows_only_direct_mentions_outside_allowed_chats():
    ...
    mentioned = _group_message(
        "hi @clara_bot",
        chat_id=-201,
        entities=[_mention_entity("hi @clara_bot")],
    )
...
def test_allowed_topics_drop_other_forum_topics_before_other_gates():
    ...
    assert adapter._should_process_message(
        _group_message("hi @clara_bot", chat_id=-100, thread_id=11, entities=[_mention_entity("hi @clara_bot")])
    ) is False
...
def test_config_bridges_telegram_group_settings(monkeypatch, tmp_path):
    ...
    # from a developer's real ~/.clara/.env, defeating the env-over-YAML bridge
...
class _IdentityBot:
    ...
    def __init__(self, bot_id=999, cached="clara_bot", server=None):
        self.id = bot_id
        self._cached = cached
...
def test_bot_never_adopts_another_accounts_username():
    ...
    adapter = _make_adapter(require_mention=True)
    adapter._bot = _IdentityBot(cached="clara_bot")
    ...
    assert adapter._current_bot_username() == "clara_bot"
