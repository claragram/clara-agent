"""Unit tests for the generic-OIDC / Clara-Portal caller-identity token resolver.

Covers gateway.relay._resolve_relay_identity_token() — the canonical resolver
shared by the runtime self-provision path and the `clara gateway enroll` CLI.
...
Three modes:
  ...
  2. Clara Portal (resolve_clara_access_token) otherwise — the default.

The HTTP calls and the Clara resolver are monkeypatched; these prove the mode
