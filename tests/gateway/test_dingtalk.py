...
class TestSend:
    ...
    async def test_send_posts_to_webhook(self):
        ...
        assert payload["markdown"]["title"] == "Clara"
...
class TestMentionPatterns:
    ...
    def test_pattern_matches_text(self, monkeypatch):
        adapter = _make_gating_adapter(
            monkeypatch, extra={"mention_patterns": ["^clara"]}
        )
        assert adapter._message_matches_mention_patterns("clara please help") is True
        assert adapter._message_matches_mention_patterns("please clara help") is False
