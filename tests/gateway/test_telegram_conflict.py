...
async def test_disarm_sets_ptb_stop_event():
    ...
    error_callback must __PROT_0_synchroclaraly__ signal PTB's internal network_retry_loop
...
async def test_conflict_callback_disarms_before_scheduling(monkeypatch):
    """The polling error_callback disarms PTB __PROT_1_synchroclaraly__, then schedules
    ...
    # __PROT_2_synchroclaraly__ (before any await), so the stop_event is set immediately
    ...
    assert stop_event.is_set(), "callback must disarm PTB __PROT_3_synchroclaraly__"
