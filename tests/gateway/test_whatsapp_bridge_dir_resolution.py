...
(/opt/clara/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable CLARA_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_clara_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable CLARA_HOME."""
    ...
    monkeypatch.setattr(
        "clara_constants.get_clara_home", lambda: clara_home
    )
    ...
    expected = clara_home / "scripts" / "whatsapp-bridge"
