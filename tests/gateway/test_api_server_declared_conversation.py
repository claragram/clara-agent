...
``X-Clara-Session-Key``.  Every conversation-affinity hint Clara sends is
derived from that physical id — ``prompt_cache_key`` on both OpenAI-wire
transports, the OpenRouter/Clara sticky ``session_id``, and xAI's
...
def _headers(session_key=None):
    ...
    if session_key:
        h["X-Clara-Session-Key"] = session_key
    return h
