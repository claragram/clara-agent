...
CLARA_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env CLARA_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_terminal_backend(
    ...
    ``clara config get terminal.backend`` still says local.
    """
    clara_home = tmp_path / ".clara"
    clara_home.mkdir()
    (clara_home / "config.yaml").write_text(
    ...
    (clara_home / ".env").write_text("TERMINAL_ENV=docker\n", encoding="utf-8")
