...
Proves, on windows-latest:
  1. `GatewayControlServer` binds ``\\\\.\\pipe\\clara-gateway-<hash>`` via
...
os.environ["CLARA_HOME"] = sys.argv[2]
...
def test_named_pipe_identify_status_and_fleet_consumer(live_server, monkeypatch):
    ...
    assert ident["kind"] == "clara-gateway"
    ...
    monkeypatch.setattr("clara_cli.profiles._get_default_clara_home", lambda: home)
    monkeypatch.setattr(
        "clara_cli.profiles._get_profiles_root", lambda: home / "no-profiles"
    )
...
def test_pipe_gone_after_kill_falls_back(live_server, monkeypatch):
    ...
    (home / "gateway_state.json").write_text(
        json.dumps(
            {"pid": os.getpid(), "code_sha": "OLD", "kind": "clara-gateway"}
        ),
        encoding="utf-8",
    )
    monkeypatch.setattr(
