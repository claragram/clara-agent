...
import pytest
from clara_cli.config import DEFAULT_CONFIG, load_config, save_config
...
class TestNonInteractiveSetup:
    ...
    def test_chat_first_run_headless_skips_setup_prompt(self, capsys):
        """Bare `clara` should not prompt for input when no provider exists and stdin is headless."""
        from clara_cli.main import cmd_chat

        args = _make_chat_args()

        with (
            patch("clara_cli.main._has_any_provider_configured", return_value=False),
            patch("clara_cli.main.cmd_setup") as mock_setup,
        ...
        out = capsys.readouterr().out
        assert "clara config set model.provider custom" in out
