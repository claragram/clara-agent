"""Regression tests for the OAuth dispatcher in clara_cli.web_server.
...
Clara must not mint subscription OAuth tokens from an unattended HTTP
...
from clara_cli.web_server import _SESSION_TOKEN, app

client = TestClient(app)
HEADERS = {"X-Clara-Session-Token": _SESSION_TOKEN}
...
def _fake_clara_device_data():
    return {
        "device_code": "device-code",
        "user_code": "CLARA-1234",
        "verification_uri": "https://portal.claraship.com/device",
        "verification_uri_complete": (
            "https://portal.claraship.com/device?user_code=CLARA-1234"
        ),
        "expires_in": 600,
        "interval": 5,
    }
...
def _invoke_scope_refusal():
    request = httpx.Request("POST", "https://portal.claraship.com/oauth/device/code")
...
def test_minimax_login_does_not_launch_anthropic_flow():
    ...
    with patch(
        "clara_cli.auth._minimax_request_user_code",
        return_value=fake_user_code_resp,
    ), patch(
        "clara_cli.auth._minimax_pkce_pair",
        return_value=("verifier-stub", "challenge-stub", "stub-state"),
    ), patch(
        "clara_cli.web_server._minimax_poller",
        return_value=None,
    ):
        resp = client.post(
...
def test_oauth_provider_status_uses_profile_query(tmp_path, monkeypatch):
    from clara_cli import web_server as ws
    ...
    def fake_status():
        observed_homes.append(get_clara_home())
    ...
    fake_catalog = ({
        ...
        "name": "Fake OAuth",
        "flow": "pkce",
        "cli_command": "clara auth add fake-oauth",
...
def test_codex_dashboard_start_rewords_device_authorization_error(monkeypatch):
    ...
    try:
        ...
        assert "clara auth" not in detail
    finally:
...
def test_clara_dashboard_poller_preserves_effective_scope_when_token_omits_scope(monkeypatch):
    from clara_cli import auth as auth_mod
    from clara_cli import web_server as ws

    session_id = "clara-effective-scope-test"
    ws._oauth_sessions[session_id] = {
        "session_id": session_id,
        "provider": "clara",
        "flow": "device_code",
    ...
    def fake_refresh_clara_oauth_from_state(state, **kwargs):
    ...
    monkeypatch.setattr(
        ...
        fake_refresh_clara_oauth_from_state,
    )
    monkeypatch.setattr(auth_mod, "persist_clara_credentials", lambda state: None)

    try:
        ws._clara_poller(session_id)
        assert captured_state["scope"] == auth_mod.DEFAULT_CLARA_SCOPE
...
def test_anthropic_dashboard_oauth_is_removed_and_external():
    ...
    assert providers["anthropic"]["cli_command"] == "clara auth add anthropic"
...
def test_accounts_offers_every_oauth_provider_from_catalog():
    ...
    `clara model` universe) must be offered by /api/providers/oauth. This keeps
    ...
    for d in provider_catalog():
        if d.tab == "accounts":
            assert d.slug in offered, (
                f"{d.slug} is an accounts-tab provider in `clara model` but is "
...
def test_oauth_catalog_marks_external_providers_not_disconnectable():
    """External CLI credentials are visible in Accounts but cannot be removed by Clara."""
    resp = client.get("/api/providers/oauth", headers=HEADERS)
    assert resp.status_code == 200, resp.text
    providers = {p["id"]: p for p in resp.json()["providers"]}
...
def test_xai_dashboard_poller_seeds_single_entry_and_clears_suppression(tmp_path, monkeypatch):
    ...
    ``device_code`` suppression left by a prior ``clara auth remove
    xai-oauth``.
    """
    from clara_cli import auth as auth_mod
    from clara_cli import web_server as ws
    ...
    # Prior `clara auth remove xai-oauth` left the source suppressed.
...
def test_status_falls_through_to_generic_dispatcher_for_catalog_only_provider():
    ...
    logged-out; now they dispatch to clara_cli.auth.get_auth_status (the
    ...
    with patch("clara_cli.auth.get_auth_status", return_value=fake_status):
