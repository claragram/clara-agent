...
def _stage(home_root, name):
    # Machine-scoped models dir (the shared root — tmp CLARA_HOME IS the root here).
...
def test_not_ours_returns_none(clara_home):
    from clara_cli.local_runtime.capabilities import managed_model_supports_vision

    assert managed_model_supports_vision("gpt-4o") is None


def test_catalog_vision_model_with_projector_on_disk(clara_home):
    ...
    _stage(clara_home, variant.model_id)
    adir = assets_dir()
...
def test_catalog_vision_model_missing_projector_is_blind(clara_home):
...
def test_lookup_chain_consults_managed_runtime(clara_home, monkeypatch):
    ...
    monkeypatch.setattr(
        "clara_cli.local_runtime.capabilities.managed_model_supports_vision",
        lambda mid: True)
...
def test_webp_transcodes_to_png_for_managed_provider(clara_home, monkeypatch, tmp_path):
...
def test_webp_passes_through_for_cloud_providers(clara_home, monkeypatch, tmp_path):
...
def test_vision_analyze_normalization_narrows_for_managed(clara_home, monkeypatch, tmp_path):
