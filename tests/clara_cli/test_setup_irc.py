"""Tests for IRC gateway configuration via `clara setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Clara install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Clara install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#clara")
            monkeypatch.setenv("IRC_NICKNAME", "clara-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `clara setup gateway` experience with IRC present."""
