...
import clara_cli.doctor as doctor
import clara_cli.gateway as gateway_cli
from clara_cli import doctor as doctor_mod
from clara_cli.doctor import _has_provider_env_config
...
class TestDoctorPlatformHints:
    ...
    def test_sqlite_upgrade_hint_recreates_docker_containers(self, monkeypatch):
        ...
        assert "docker pull workprise/clara-agent:latest" in hint
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `clara doctor` crashed on Windows
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "CLARA_HOME", clara_home)
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("CLARA_KANBAN_TASK", "probe")
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_clara_home(self, tmp_path, provider="", memory_config=None):
        """Create a minimal CLARA_HOME with config.yaml."""
        home = tmp_path / ".clara"
    ...
    def _run_doctor_and_capture(
        ...
        *,
        ...
        home = self._make_clara_home(tmp_path, provider, memory_config)
        ...
        monkeypatch.setattr(doctor_mod, "CLARA_HOME", home)
        ...
        try:
            from clara_cli import auth as _auth_mod
            monkeypatch.setattr(_auth_mod, "get_clara_auth_status_local", lambda: {})
...
def test_run_doctor_accepts_clara_provider_ids_that_catalog_aliases(
...
def _doctor_env_for_agent_browser(monkeypatch, tmp_path):
    ...
    branch in run_doctor (clara_cli/doctor.py ~1557-1605)."""
    home = tmp_path / ".clara"
    home.mkdir(parents=True, exist_ok=True)
    (home / "config.yaml").write_text("memory: {}\n", encoding="utf-8")
    project = tmp_path / "project"
    ...
    try:
        from clara_cli import auth as _auth_mod
        monkeypatch.setattr(_auth_mod, "get_clara_auth_status", lambda: {})
        monkeypatch.setattr(_auth_mod, "get_codex_auth_status", lambda: {})
        monkeypatch.setattr(_auth_mod, "get_xai_oauth_auth_status", lambda: {})
    except Exception:
        pass
...
def test_run_doctor_fix_warms_npx_cache_when_agent_browser_resolves_via_npx(
    ...
    """`clara doctor --fix` must actually call warm_agent_browser_npx_cache()
...
class TestGitHubTokenCheck:
    ...
    def _isolate_home(monkeypatch, home):
        """Point doctor at the temp CLARA_HOME.

        ``run_doctor`` reads the module-level ``CLARA_HOME`` constant (cached
        at import time), NOT the env var — so ``setenv("CLARA_HOME")`` alone
        leaves doctor probing the REAL ~/.clara. On a dev machine with a
...
def _run_doctor_with_healthy_oauth_fallback(
    ...
    *,
    ...
    monkeypatch.setattr(_auth_mod, "get_clara_auth_status_local", lambda: {"logged_in": True})
    monkeypatch.setattr(_auth_mod, "get_codex_auth_status", lambda: {})
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from clara_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("clara_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "clara_cli.doctor", raising=False)
        from clara_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestDoctorXaiOAuthStatus:
    ...
    failure (or runtime exception) cannot silence the Clara / Codex / Gemini /
    ...
    def _run(self, monkeypatch, tmp_path, *, xai_auth_fn) -> str:
        ...
        from clara_cli import auth as _auth_mod
        monkeypatch.setattr(_auth_mod, "get_clara_auth_status_local", lambda: {"logged_in": False})
    ...
    def test_import_failure_does_not_affect_other_providers(self, monkeypatch, tmp_path):
        """Clara / Codex / Gemini / MiniMax rows must survive an xAI import failure."""
        home = tmp_path / ".clara"
        home.mkdir(parents=True, exist_ok=True)
        (home / "config.yaml").write_text("memory: {}\n", encoding="utf-8")
        project = tmp_path / "project"
        ...
        with contextlib.redirect_stdout(buf):
            doctor_mod.run_doctor(Namespace(fix=False))
        out = buf.getvalue()
        assert "Clara Portal auth" in out
...
class TestDoctorStaleMaxIterationsDrift:
    """Regression for #17534: a stale CLARA_MAX_ITERATIONS in .env shadows
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        if ghost is not None:
            env_lines.append(f"CLARA_MAX_ITERATIONS={ghost}\n")
        (clara_home / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "CLARA_HOME", clara_home)
        monkeypatch.setattr(doctor_mod, "get_clara_home", lambda: clara_home)
        ...
        if os_environ_value is not None:
            ...
            monkeypatch.setenv("CLARA_MAX_ITERATIONS", str(os_environ_value))
        else:
            monkeypatch.delenv("CLARA_MAX_ITERATIONS", raising=False)
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        assert "Removed stale CLARA_MAX_ITERATIONS" in out
        env_after = (clara_home / ".env").read_text(encoding="utf-8")
        assert "CLARA_MAX_ITERATIONS" not in env_after
...
class TestDoctorDeprecatedConfigAndEnv:
    ...
    def test_clara_tool_progress_warning_says_unsupported_since_floor(self):
        """CLARA_TOOL_PROGRESS lost its last consumer (the retired v3→4
        ...
        findings = dict(
            doctor_mod.collect_deprecated_env_vars({"CLARA_TOOL_PROGRESS": "true"})
        )
        assert "ignored/unsupported since config floor v12" in findings["CLARA_TOOL_PROGRESS"]
        # The MODE variant is still read by the gateway fallback → keeps the
        # plain deprecation wording.
        mode = dict(
            doctor_mod.collect_deprecated_env_vars({"CLARA_TOOL_PROGRESS_MODE": "all"})
        )
        assert mode["CLARA_TOOL_PROGRESS_MODE"] == "display.tool_progress in config.yaml"

    def _run_doctor_with_config(self, monkeypatch, tmp_path, *, config_yaml: str, env_text: str = ""):
        clara_home = tmp_path / ".clara"
        clara_home.mkdir(parents=True)
        (clara_home / "config.yaml").write_text(config_yaml, encoding="utf-8")
        env_body = env_text if env_text else "OPENAI_API_KEY=sk-test\n"
        (clara_home / ".env").write_text(env_body, encoding="utf-8")
    ...
    def test_report_does_not_count_as_blocking_issue(self, monkeypatch, tmp_path, capsys):
        ...
        findings = doctor_mod.report_deprecated_config_and_env(
            ...
            {"CLARA_TOOL_PROGRESS_MODE": "verbose"},
        )
        out = capsys.readouterr().out
        assert len(findings) == 2
        assert "Deprecated: delegation.max_async_children" in out
        assert "Deprecated: CLARA_TOOL_PROGRESS_MODE" in out
...
class TestMacOSTCCGrants:
    ...
    def test_warns_on_cdhash_pinned_dr(self, monkeypatch, capsys, tmp_path):
        ...
        monkeypatch.setattr(
            doctor_mod,
            "_macos_desktop_dr",
            lambda app: 'designated => identifier "com.workprise.clara" and cdhash H"97e692f3890f781fa0ad5ad6cb9d769cfaf42628"',
        )
        doctor_mod.check_macos_tcc_grants()
        out = capsys.readouterr().out
        assert "TCC grants will reset after every update" in out
        assert "cdhash-pinned" in out
        assert "clara update" in out
    ...
    def test_ok_and_repair_info_on_identifier_dr(self, monkeypatch, capsys, tmp_path):
        ...
        monkeypatch.setattr(
            doctor_mod,
            "_macos_desktop_dr",
            lambda app: 'designated => identifier "com.workprise.clara"',
        )
        doctor_mod.check_macos_tcc_grants()
        out = capsys.readouterr().out
        ...
        assert "tccutil reset ScreenCapture com.workprise.clara" in out
        assert "toggle" in out
        assert "relaunch" in out

    def test_ok_on_certificate_anchored_dr(self, monkeypatch, capsys, tmp_path):
        """A cert-anchored DR (clara desktop --setup-tcc-identity, or a
        ...
        monkeypatch.setattr(
            doctor_mod,
            "_macos_desktop_dr",
            lambda app: 'designated => identifier "com.workprise.clara" and certificate root = H"aabbcc"',
        )
        doctor_mod.check_macos_tcc_grants()
        out = capsys.readouterr().out
        assert "TCC signing identity is stable" in out
