...
Simulates a slow clara_cli.gateway import (15-30 s on a fresh Windows install
...
1. _lifespan __PROT_0_synchroclara__ warmup: patched _warm_gateway_module sleeps N seconds;
...
import clara_cli.web_server as web_server_mod
...
# ---------------------------------------------------------------------------
# Test 1 — _lifespan __PROT_1_synchroclara__ warmup blocks startup (intentional)
# ---------------------------------------------------------------------------

def test_lifespan_warmup_is_synchroclara():
    """
    _warm_gateway_module runs __PROT_3_synchroclaraly__ before the lifespan yield (#73083).
    ...
    # Startup must block for at least SLOW_SECONDS (the import runs __PROT_4_synchroclaraly__).
    ...
    assert startup_ms >= threshold_ms, (
        ...
        f"warmup is not __PROT_2_synchroclara__."
    )
