"""Tests for clara_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
def isolated_home(tmp_path, monkeypatch):
    """Isolate CLARA_HOME + reset any module-level catalog cache per test."""
    home = tmp_path / ".clara"
    home.mkdir()
    ...
    import importlib
    from clara_cli import model_catalog
...
def _valid_manifest() -> dict:
    return {
        ...
        "providers": {
            ...
            "clara": {
                "metadata": {"display_name": "Clara Portal"},
                "models": [
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from clara_cli.model_catalog import _validate_manifest
...
class TestFallbackChain:
    ...
    PRIMARY = "https://agent.claraship.com/docs/api/model-catalog.json"
    FALLBACK = (
        "https://raw.githubusercontent.com/claraship/clara-agent"
...
class TestCuratedAccessors:
    ...
    def test_clara_returns_none_when_catalog_empty(self, isolated_home):
        from clara_cli import model_catalog
        with patch.object(model_catalog, "_fetch_manifest", return_value=None):
            assert model_catalog.get_curated_clara_models() is None
...
class TestDefaultModelFromCache:
    ...
    def _manifest_with_default(self) -> dict:
        ...
        m["providers"]["clara"]["models"][1]["default"] = True  # kimi-k2.6
        return m
    ...
    def test_reads_label_from_disk_cache(self, isolated_home):
        ...
        with patch.object(model_catalog, "_fetch_manifest") as fetch:
            ...
            assert (
                model_catalog.get_default_model_from_cache("clara")
    ...
    def test_shipped_manifest_labels_glm52_default(self, isolated_home):
        ...
        import clara_cli.model_catalog as model_catalog
        from clara_cli.models import PREFERRED_SILENT_DEFAULT_MODEL

        repo_root = Path(model_catalog.__file__).resolve().parent.parent
        manifest = json.loads(
            (repo_root / "website" / "static" / "api" / "model-catalog.json").read_text()
        )
        for provider in ("openrouter", "clara"):
...
class TestRefreshCadence:
    def test_default_ttl_is_twenty_minutes_and_legacy_hours_honoured(self):
        ...
        with patch("clara_cli.config.load_config", return_value={"model_catalog": {"ttl_minutes": 20}}):
        ...
        with patch("clara_cli.config.load_config", return_value={"model_catalog": {"ttl_minutes": 20, "ttl_hours": 3}}):
    ...
    def test_refresh_catalogs_forces_every_source(self):
        ...
        with patch.object(model_catalog, "_load_catalog_config", return_value={
            "enabled": True, "url": "http://master", "ttl_hours": 1.0, "providers": {},
             ...
             patch("clara_cli.models.fetch_openrouter_models") as orm, \
             patch("clara_cli.models.fetch_clara_recommended_models") as clara:
        ...
        clara.assert_called_once_with(force_refresh=True)


class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in clara_cli.models."""



    def test_picker_clara_row_uses_curated_list(self, tmp_path, monkeypatch):
        """The /model picker surfaces the curated ``_PROVIDER_MODELS["clara"]``
        list in curated order — matching the ``clara model`` CLI — not the live
        ...
        # ``CLARA_HOME / auth.json`` then resolves to the same path the
        # seat-belt thinks is the "real" user store. Use the autouse
        # ``_hermetic_environment`` CLARA_HOME directly instead.
        import importlib
        from clara_cli import model_catalog
        from clara_cli.models import get_curated_clara_model_ids
        importlib.reload(model_catalog)
        try:
            from clara_cli.model_switch import list_picker_providers

            active_home = Path(os.environ["CLARA_HOME"])
            (active_home / "auth.json").write_text(
                json.dumps(
                    {
                        "providers": {"clara": {"access_token": "fake"}},
                        "credential_pool": {},
                    }
                )
            )
            ...
            # (``curated["clara"] = get_curated_clara_model_ids()``), so the test
            # stays an invariant — it can't rot as the curated/manifest list grows.
            with patch.object(
                model_catalog, "_fetch_manifest", return_value=_valid_manifest()
            ), patch("clara_cli.models.check_clara_free_tier", return_value=False), patch(
                "clara_cli.models.union_with_portal_free_recommendations",
                side_effect=lambda ids, *a, **k: (ids, {}),
            ), patch(
                "clara_cli.models.union_with_portal_paid_recommendations",
                side_effect=lambda ids, *a, **k: (ids, {}),
            ):
                expected = get_curated_clara_model_ids()
                picker = list_picker_providers(
                    current_provider="clara", max_models=99
                )
        finally:
            model_catalog.reset_cache()

        clara_row = next((r for r in picker if r["slug"] == "clara"), None)
        assert clara_row is not None, "clara row must appear when authed"
        assert clara_row["models"] == expected
    ...
    def test_picker_max_models_cap_semantics(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            with patch.object(
                ...
                expected = get_curated_clara_model_ids()
                full = list_picker_providers(current_provider="clara", max_models=None)
                one = list_picker_providers(current_provider="clara", max_models=1)
                ...
                zero = list_authenticated_providers(
                    current_provider="clara", max_models=0
                )
        finally:
            model_catalog.reset_cache()

        def _clara(rows):
            return next((r for r in rows if r["slug"] == "clara"), None)

        # Only meaningful when the curated list actually exceeds 1 entry.
        assert len(expected) > 1, "test needs a multi-model curated clara list"

        full_row = _clara(full)
        assert full_row is not None and full_row["models"] == expected

        one_row = _clara(one)
        assert one_row is not None and one_row["models"] == expected[:1]

        zero_row = _clara(zero)
...
#
# History: qwen/qwen3.6-plus was added to _PROVIDER_MODELS["clara"] in commit
...
class TestManifestMatchesInRepoLists:
    ...
    def test_in_repo_lists_match_manifest(self):
        ...
        assert self._strip_volatile(actual) == self._strip_volatile(expected), (
            ...
            "_PROVIDER_MODELS['clara'] / OPENROUTER_MODELS. "
