...
def test_session_cookies_use_host_prefix_on_https_direct():
    ...
    provider = next(c for c in cookies if c.startswith(f"__Host-{SESSION_PROVIDER_COOKIE}=clara"))
...
def test_session_cookies_use_secure_prefix_when_proxied():
    """HTTPS + /clara prefix → __Secure- prefix (__Host- forbids
    Path != "/"; __Secure- keeps the Secure-required hardening)."""
    client = TestClient(_build_app(use_https=True, prefix="/clara"))
    r = client.get("/set")
    cookies = r.headers.get_list("set-cookie")
    at = next(c for c in cookies if c.startswith(f"__Secure-{SESSION_AT_COOKIE}="))
    assert "Path=/clara" in at
    assert "Secure" in at
...
def test_clear_session_cookies_prefixed_deletions_carry_secure():
    ...
    so an insecure deletion for __Host-clara_session_at is silently
