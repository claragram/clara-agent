"""Tests for clara_cli.update_inventory — the plan phase (#91277 Phase 2)."""
...
def fleet(monkeypatch, tmp_path):
    ...
    import re
    monkeypatch.setattr("clara_cli.profiles._get_default_clara_home", lambda: default_home)
    monkeypatch.setattr("clara_cli.profiles._get_profiles_root", lambda: default_home / "profiles")
    monkeypatch.setattr("clara_cli.profiles._PROFILE_ID_RE", re.compile(r"^[a-z0-9][a-z0-9_-]*$"), raising=False)
    monkeypatch.setattr("gateway.status._pid_exists", lambda pid: pid in (100, 200))
    monkeypatch.setattr("clara_cli.gateway._get_service_pids", lambda all_profiles=False: {100})
    monkeypatch.setattr("clara_cli.gateway.supports_systemd_services", lambda: True)
    monkeypatch.setattr("clara_cli.gateway.find_profile_gateway_processes", lambda exclude_pids=None: [])
    monkeypatch.setattr(
        "clara_cli.build_info.get_code_identity",
        lambda refresh=False: {"sha": "a" * 40, "short_sha": "a" * 8, "version": "1.0", "source": "git"},
    )
    monkeypatch.setattr("clara_cli.config.detect_install_method", lambda *a, **k: "git")
    monkeypatch.setattr("clara_cli.config.get_managed_system", lambda: None)
    return tmp_path
...
class TestCollectInventory:
    def test_two_profile_fleet(self, fleet):
        ...
        from clara_cli.update_inventory import describe_restart_mechanism

        assert "clara -p work gateway restart" in describe_restart_mechanism(
    ...
    def test_docker_install_not_updatable_in_place(self, fleet, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda m: "docker pull claraship/clara-agent:latest",
        )
        plan = ui.collect_runtime_inventory()
    ...
    def test_pid_file_fallback_covers_unstamped_profiles(self, fleet, monkeypatch):
        ...
        from clara_cli.gateway import ProfileGatewayProcess
    ...
    def test_never_raises_when_everything_fails(self, monkeypatch):
        ...
        for target in (
            "clara_cli.config.detect_install_method",
            "clara_cli.build_info.get_code_identity",
            "clara_cli.profiles._get_default_clara_home",
...
class TestReceiptIntegration:
    def test_plan_recorded_into_active_receipt(self, fleet, monkeypatch, tmp_path):
        ...
        home.mkdir()
        monkeypatch.setattr("clara_cli.config.get_clara_home", lambda: home, raising=False)
        ur._current = None
