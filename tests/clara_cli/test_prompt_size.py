"""Tests for the ``clara prompt-size`` diagnostic (issue #34667)."""
...
from clara_cli.prompt_size import (
    _SKILLS_BLOCK_RE,
...
def _seed_memory(clara_home, memory_text="", user_text=""):
    mem_dir = clara_home / "memories"
...
def _seed_skill(clara_home, name, description):
    skill_dir = clara_home / "skills" / "demo" / name
...
def test_runs_offline_without_credentials(isolated_home, monkeypatch):
    ...
    for var in ("OPENROUTER_API_KEY", "OPENAI_API_KEY", "CLARA_API_KEY",
                "ANTHROPIC_API_KEY"):
