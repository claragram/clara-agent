"""Regression tests for the Clara Portal env-override bypassing the host
allowlist, mirroring the existing CLARA_INFERENCE_BASE_URL /
_ALLOWED_CLARA_INFERENCE_HOSTS treatment.
...
``CLARA_PORTAL_BASE_URL=https://portal.staging-workprise.com`` in its
...
Before this fix, ``resolve_clara_access_token`` / ``resolve_clara_runtime_
...
``_CLARA_PORTAL_ALLOWED_HOSTS``, which only recognised the production host.
...
``_quarantine_clara_oauth_state`` and wiped the entire credential pool.

The correct fix (mirroring ``_clara_inference_env_override()``): the env
...
from clara_cli.auth import (
    ...
    _CLARA_PORTAL_ALLOWED_HOSTS,
    _clara_portal_env_override,
)


class TestPortalEnvOverrideHelper:
    def test_none_when_unset(self, monkeypatch):
        monkeypatch.delenv("CLARA_PORTAL_BASE_URL", raising=False)
        monkeypatch.delenv("CLARA_PORTAL_BASE_URL", raising=False)
        assert _clara_portal_env_override() is None
    ...
    def test_env_override_not_gated_by_allowlist(self, monkeypatch):
        ...
        monkeypatch.setenv(
            "CLARA_PORTAL_BASE_URL", "https://portal.staging-workprise.com"
        )
        assert "portal.staging-workprise.com" not in _CLARA_PORTAL_ALLOWED_HOSTS
        assert (
            _clara_portal_env_override() == "https://portal.staging-workprise.com"
        )


class TestResolveAccessTokenEnvOverrideWins:
    """End-to-end: resolve_clara_access_token must use the env override for
    ...
    def test_env_override_wins_even_with_staging_state_stored(
        ...
        a prior CLARA_AUTH_JSON_BOOTSTRAP seed), and the env var is set to
        ...
        staging_portal = "https://portal.staging-workprise.com"
    ...
    def test_no_env_no_staging_state_prod_url_used_unmodified(
        ...
        monkeypatch.delenv("CLARA_PORTAL_BASE_URL", raising=False)
        monkeypatch.delenv("CLARA_PORTAL_BASE_URL", raising=False)
        self._write_auth_file(tmp_path, stored_portal_url=DEFAULT_CLARA_PORTAL_URL)

        seen_portal_urls, records = self._run_and_capture(monkeypatch, auth)

        assert seen_portal_urls == [DEFAULT_CLARA_PORTAL_URL]
