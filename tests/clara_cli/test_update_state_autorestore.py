...
second Clara holder the updater's drain did not stop — survives the copy and is
...
def test_torn_restore_is_what_the_guard_prevents(live_db_with_hot_wal, snapshot_db):
    ...
    """
    from clara_cli.backup import verify_sqlite_integrity
...
def test_post_update_guard_covers_sibling_profiles(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr(update_cmd, "get_clara_home", lambda: root_home)
    monkeypatch.setattr(
        "clara_cli.backup._sibling_profile_homes",
