...
def test_startup_route_uses_configured_clara_provider(monkeypatch):
    monkeypatch.setattr(model_switch, "DIRECT_ALIASES", {})
    route = model_switch.resolve_startup_model_route(
        "clara/deepseek-v4-pro",
        user_providers={"clara": {"base_url": "https://inference.example/v1"}},
    )
    assert route == model_switch.StartupModelRoute("deepseek-v4-pro", "clara", "")
...
def test_startup_route_aggregator_native_slug_stays_on_aggregator(monkeypatch):
    ...
    monkeypatch.setattr(model_switch, "DIRECT_ALIASES", {})
    monkeypatch.setattr(
        "clara_cli.models._find_openrouter_slug",
