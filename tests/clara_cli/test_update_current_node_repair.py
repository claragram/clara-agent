...
A previous ``clara update`` whose npm install failed printed "Fix npm and
re-run `clara update`" — but re-running hit the "Already up to date!" early
...
def test_current_checkout_repairs_failed_node_deps(capsys):
    ...
    assert "Fix npm and re-run `clara update`." in out
