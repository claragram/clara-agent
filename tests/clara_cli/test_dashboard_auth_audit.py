...
Profile-aware location: ``$CLARA_HOME/logs/dashboard-auth.log``.
...
from clara_cli.dashboard_auth.audit import audit_log, AuditEvent


@pytest.fixture
def profile_home(tmp_path, monkeypatch):
    """Redirect $CLARA_HOME and ~ to a tmp dir for the duration of the test."""
    home = tmp_path / ".clara"
    home.mkdir()
...
def test_audit_writes_jsonlines(profile_home):
    audit_log(AuditEvent.LOGIN_START, provider="clara", ip="1.2.3.4")
    audit_log(
        AuditEvent.LOGIN_SUCCESS,
        provider="clara", user_id="u1",
...
def test_audit_redacts_token_like_fields(profile_home):
    audit_log(
        AuditEvent.LOGIN_SUCCESS,
        provider="clara", access_token="should-not-appear",
