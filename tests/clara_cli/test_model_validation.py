"""Tests for provider-aware `/model` validation in clara_cli.models."""
...
class TestFetchApiModels:
    ...
    def test_probe_api_models_tries_v1_fallback(self):
        ...
        with patch("clara_cli.models._urlopen_model_catalog_request", side_effect=_fake_urlopen):
    ...
    def test_probe_api_models_uses_copilot_catalog(self):
        ...
        with patch("clara_cli.models._urlopen_model_catalog_request", return_value=_Resp()) as mock_urlopen:
...
class TestValidateApiFallback:
    ...
    def test_fetch_lmstudio_models_filters_embedding_type(self):
        ...
        with patch("clara_cli.models._urlopen_model_catalog_request", return_value=mock_resp):
    ...
    def test_validate_lmstudio_distinguishes_auth_failure(self):
        ...
        with patch("clara_cli.models._urlopen_model_catalog_request", side_effect=http_error):
...
class TestValidateCodexAutoCorrection:
    ...
    def test_missing_dash_auto_corrects(self):
        ...
        with patch("clara_cli.models.provider_model_ids", return_value=codex_models):
...
class TestValidateCodex900kVariants:
    """`-900k` is a Clara picker convention: valid variants come from the
    ...
    def test_catalog_listed_variant_accepted(self):
        with patch("clara_cli.models.provider_model_ids", return_value=self._CATALOG):
    ...
    def test_valid_variant_missing_from_catalog_still_accepted(self):
        ...
        with patch("clara_cli.models.provider_model_ids", return_value=["gpt-5.6-sol"]):
            result = validate_requested_model("gpt-5.6-sol-900k", "openai-codex")
        assert result["accepted"] is True
...
class TestProbeApiModelsUserAgent:
    """Probing custom /v1/models must send a Clara User-Agent.
    ...
    def test_probe_sends_clara_user_agent(self):
        ...
        assert ua.startswith("clara-cli/"), (
            f"User-Agent must advertise clara-cli, got {ua!r}"
        )
    ...
    def test_probe_user_agent_sent_without_api_key(self):
        ...
        assert ua and ua.startswith("clara-cli/")
...
class TestValidateOpenRouterVariantSuffixes:
    ...
    def test_static_catalog_fallback_accepts_variant(self):
        ...
        with patch("clara_cli.models.fetch_api_models", return_value=None), \
             patch(
                 "clara_cli.models.provider_model_ids",
...
class TestValidateRequestedModelClaraPortalRecommendations:
    """Regression tests for issue #71312: the Clara Telegram picker (and any
    other messaging-platform /model validation, since they all share
    validate_requested_model()) rejected models that are live Clara Portal
    recommendations (/api/clara/recommended-models) but not yet in the
    hardcoded curated catalog -- even though `clara chat` already accepts
    ...
    def _validate_clara(self, model, api_models=None, portal_payload=None, portal_raises=False):
        api_models = api_models if api_models is not None else ["inclusionai/ling-2.6-flash"]
        probe_payload = {
            "models": api_models,
            "probed_url": "https://portal.claraship.com/v1/models",
            "resolved_base_url": "https://portal.claraship.com/v1",
            "suggested_base_url": None,
            "used_fallback": False,
        }
        ...
        with patch("clara_cli.models.fetch_api_models", return_value=api_models), \
             ...
             patch("clara_cli.models._resolve_clara_portal_url", return_value="https://portal.claraship.com"), \
    ...
    def test_free_portal_recommendation_accepted(self):
        ...
        result = self._validate_clara("inclusionai/ling-3.0-flash:free")
        assert result["accepted"] is True
        assert result["persist"] is True
        assert "Portal recommendation" in (result["message"] or "")

    def test_paid_portal_recommendation_accepted(self):
        result = self._validate_clara("inclusionai/ling-3.0-pro")
        assert result["accepted"] is True
    ...
    def test_model_absent_from_portal_and_catalog_still_rejected(self):
        ...
        result = self._validate_clara("totally-made-up-model-xyz")
        assert result["accepted"] is False
    ...
    def test_non_string_model_name_entries_ignored(self):
        ...
        assert self._validate_clara("5", portal_payload=payload)["accepted"] is False
        result = self._validate_clara(
            "inclusionai/ling-3.0-flash:free", portal_payload=payload
        )
        assert result["accepted"] is True

    def test_non_clara_provider_does_not_consult_portal_feed(self):
        """This fallback tier is Clara-specific; a non-Clara provider must
        not have its rejection changed by (or trigger a call to) the Clara
        Portal feed."""
        probe_payload = {
        ...
        with patch("clara_cli.models.fetch_api_models", return_value=["some/other-model"]), \
             patch("clara_cli.models.probe_api_models", return_value=probe_payload), \
             patch("clara_cli.models.fetch_clara_recommended_models") as mock_portal, \
             patch("clara_cli.models._model_in_provider_catalog", return_value=False):
    ...
    def test_curated_catalog_hit_short_circuits_before_portal_check(self):
        ...
        with patch("clara_cli.models.fetch_api_models", return_value=api_models), \
             patch("clara_cli.models.probe_api_models", return_value=probe_payload), \
             patch("clara_cli.models._model_in_provider_catalog", return_value=True), \
             patch("clara_cli.models.fetch_clara_recommended_models") as mock_portal:
            result = validate_requested_model("inclusionai/ling-2.6-flash", "clara")
        mock_portal.assert_not_called()
        assert result["accepted"] is True
