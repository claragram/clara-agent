...
class TestWsAuthOkGated:
    ...
    def test_ticket_subprotocol_is_single_use_and_selects_only_the_public_protocol(self, gated_app):
        ...
        assert web_server._ws_auth_ok(ws_one) is True
        assert ws_one._clara_auth_identity == {
            "user_id": "subprotocol-user",
            "provider": "stub",
        }
        assert ws_one._clara_ws_subprotocol == web_server._GATEWAY_WS_PROTOCOL
        assert ticket not in ws_one._clara_ws_subprotocol
        assert web_server._ws_auth_ok(ws_two) is False
    ...
    def test_ticket_subprotocol_rejects_missing_public_protocol_or_ambiguous_tickets(self, gated_app):
        ...
        missing_public = _fake_ws(
            query={},
            path="/api/ws",
            protocols=(f"clara-gateway-ticket.{first}",),
        )
        ...
        ambiguous = _fake_ws(
            ...
            protocols=(
                "clara-gateway-v1",
                f"clara-gateway-ticket.{first}",
                f"clara-gateway-ticket.{second}",
            ),
        )
    ...
    def test_rejection_audit_logs(self, gated_app, tmp_path, monkeypatch):
        ...
        from clara_cli.dashboard_auth import audit as audit_mod
        ...
        # The audit module may write __PROT_1_asynchroclaraly__ through stdlib logging,
        # but flush is __PROT_0_synchroclara__. If the file doesn't exist yet, the
...
class TestWsHostOriginGuardOrigins:
    ...
    def test_explicit_non_loopback_file_origin_allowed(self, insecure_explicit_host_app):
        """Packaged Clara Desktop also uses file:// when connecting to a
