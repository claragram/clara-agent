"""ensure_clara_home is memoized per home path (perf: it runs on every
...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".clara"
    second = tmp_path / "b" / ".clara"

    monkeypatch.setenv("CLARA_HOME", str(first))
    cfg.ensure_clara_home()

    # Profile switch: CLARA_HOME moves → the new path is ensured too.
    monkeypatch.setenv("CLARA_HOME", str(second))
    cfg.ensure_clara_home()
