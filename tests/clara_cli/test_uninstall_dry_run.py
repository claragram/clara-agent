...
def test_dry_run_prints_plan_without_mutating(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(uninstall, "get_clara_home", lambda: clara_home)
    monkeypatch.setattr(uninstall, "_is_default_clara_home", lambda home: False)
    ...
    assert str(clara_home) in output
    assert project_root.exists()
    assert clara_home.exists()
