...
from clara_cli.subcommands.dashboard import build_dashboard_parser
...
def test_config_get_unset_subcommands_parse():
    """`clara config get/unset` parse key args (and --json for get)."""
    parser = argparse.ArgumentParser(prog="clara")
...
# ── deprecated `clara login` fails gracefully, not with argparse error ────
#
# `clara login` is a removed command; its handler (`login_command` in
# `clara_cli/auth.py`) prints a deprecation notice pointing at `clara auth` /
# `clara model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `clara --help`.
