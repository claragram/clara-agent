...
def test_termux_skips_bundled_skill_sync_when_stamp_fresh(monkeypatch, tmp_path, main_mod):
    ...
    monkeypatch.setattr(main_mod, "get_clara_home", lambda: tmp_path)
...
def test_oneshot_subprocess_exits_without_teardown_abort():
    program = textwrap.dedent(
        ...
        from clara_cli.main import _exit_after_oneshot
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """clara -z bypasses ClaraCLI, but recall still needs SessionDB."""
    from clara_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "clara_state", mod("clara_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "clara_cli.config",
        mod("clara_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "clara_cli.models",
        mod("clara_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
    ...
    monkeypatch.setitem(
        sys.modules,
        "clara_cli.tools_config",
        mod("clara_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["CLARA_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="clara/clara-test", provider="clara", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["CLARA_MODEL"] == "clara/clara-test"
    assert env["CLARA_INFERENCE_MODEL"] == "clara/clara-test"
    assert env["CLARA_TUI_PROVIDER"] == "clara"
    assert env["CLARA_INFERENCE_PROVIDER"] == "clara"
    assert env["CLARA_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["CLARA_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("clara-tui-active-session-")
...
def test_make_tui_argv_dev_prebuilds_clara_ink(monkeypatch, main_mod, tmp_path):
    ...
    monkeypatch.delenv("CLARA_TUI_DIR", raising=False)
