"""Tests for the post-pull HEAD-movement gate in ``clara update``.
...
def _patch_update_deps(monkeypatch, tmp_path, run_side_effect):
    """Patch the clara_cli.main helpers ``_cmd_update_impl`` touches.

    ``_m()`` in update_cmd.py lazily returns clara_cli.main, so patching
    attributes on that module is the canonical test surface (matches
    tests/clara_cli/test_cmd_update.py).
    """
    ...
    monkeypatch.setattr(
        clara_main, "_resolve_update_branch", lambda args: "main"
    )
    ...
    monkeypatch.setattr(
        clara_main, "_run_pre_update_backup", lambda *a, **k: None
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(clara_main, "_finish_dashboard_update_cleanup", lambda *a: None)
    ...
    monkeypatch.setattr(
        clara_gateway, "find_gateway_pids", lambda all_profiles=False: []
    )
    monkeypatch.setattr(
        clara_gateway, "supports_systemd_services", lambda: False
    )
    monkeypatch.setattr(
...
def test_update_success_when_head_moves(monkeypatch, tmp_path, capsys):
    ...
    clara_main.cmd_update(args)  # completes normally (no SystemExit)
