"""Regression test: `clara dashboard --tui` must not hard-crash.

Older Clara desktop app shells (<= 0.15.x) spawn the backend as::

    clara dashboard --no-open --tui --host 127.0.0.1 --port <PORT>
...
died before it became ready and the desktop GUI showed only "Clara couldn't
...
def _run_cli(args, timeout=60):
    """Invoke the real clara_cli.main parser in a subprocess.
