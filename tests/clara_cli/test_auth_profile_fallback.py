...
When ``CLARA_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.clara/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.clara            (has its own auth.json fixture)
    * Profile     -> tmp_path/.clara/profiles/coder   (active, CLARA_HOME points here)
...
def test_provider_auth_state_falls_back_to_global_when_profile_has_none(profile_env):
    ...
    _write(profile_env["global"] / "auth.json", _make_auth_store(providers={
        "clara": {"access_token": "clara-global", "refresh_token": "rt-global"},
    }))
    _write(profile_env["profile"] / "auth.json", _make_auth_store(providers={}))

    state = get_provider_auth_state("clara")
    assert state is not None
    assert state["access_token"] == "clara-global"
...
def test_provider_auth_state_returns_none_when_neither_has_it(profile_env):
    ...
    assert get_provider_auth_state("clara") is None


# ---------------------------------------------------------------------------
# _load_provider_state — internal global fallback (issue #18594 follow-up)
#
# Several runtime helpers (notably ``resolve_clara_runtime_credentials`` and
# ``resolve_clara_access_token``) call ``_load_provider_state`` directly with
# a profile-loaded auth store rather than going through
# ``get_provider_auth_state``. Without the fallback wired into
# ``_load_provider_state`` itself, those helpers raise ``"Clara is not
# logged into Clara Portal"`` even though the user has a valid global Clara
...
def test_auth_lock_reentrancy_is_scoped_after_profile_context_switch(profile_env):
    ...
    with auth._auth_store_lock():
        ...
        token = set_clara_home_override(profile_b)
        try:
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (CLARA_HOME != ~/.clara, no profiles)."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    clara_home = tmp_path / "classic"
    clara_home.mkdir()
