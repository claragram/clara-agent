...
def _write_auth_store(tmp_path, payload: dict) -> None:
    ...
    (clara_home / "auth.json").write_text(json.dumps(payload, indent=2))
...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    from clara_cli.auth_commands import auth_add_command
    ...
    payload = json.loads((tmp_path / "clara" / "auth.json").read_text())
...
def test_auth_add_configured_provider_uses_canonical_pool_key(tmp_path, monkeypatch):
    ...
    payload = json.loads((clara_home / "auth.json").read_text(encoding="utf-8"))
...
def test_auth_add_migrates_display_name_derived_legacy_pool_key(
    ...
    with patch("clara_cli.models.clear_provider_models_cache") as clear_cache:
        auth_add_command(_Args())
...
def test_auth_list_includes_non_registry_configured_provider(
    ...
    from clara_cli.auth_commands import auth_list_command
...
def test_interactive_auth_add_accepts_non_registry_configured_provider(
    ...
    from clara_cli import auth_commands
...
def test_auth_add_clara_oauth_persists_pool_entry(tmp_path, monkeypatch):
    monkeypatch.setenv("CLARA_HOME", str(tmp_path / "clara"))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})
    token = _jwt_with_email("clara@example.com")
    monkeypatch.setattr(
        "clara_cli.auth._clara_device_code_login",
        lambda **kwargs: {
    ...
    entries = payload["credential_pool"]["clara"]
    device_code_entries = [
    ...
    # `clara auth add clara` must also populate providers.clara so the
    # 401-recovery path (resolve_clara_runtime_credentials) can refresh an
    # invoke JWT when the token expires. If this mirror is missing, recovery
    # raises "Clara is not logged into Clara Portal" and the agent dies.
...
def test_auth_add_clara_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`clara auth add clara --type oauth --label <name>` must preserve the
    custom label end-to-end — it was silently dropped in the first cut of the
    persist_clara_credentials helper because `--label` wasn't threaded through.
    """
    monkeypatch.setenv("CLARA_HOME", str(tmp_path / "clara"))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})
    token = _jwt_with_email("clara@example.com")
    ...
    pool_entry = payload["credential_pool"]["clara"][0]
    assert pool_entry["source"] == "device_code"
    assert pool_entry["label"] == "my-clara"

    # … and survives in providers.clara so a subsequent load_pool() re-seeds
    # it without reverting to the auto-derived fingerprint.
    assert payload["providers"]["clara"]["label"] == "my-clara"


def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``clara auth add openai-codex`` runs for different ChatGPT
    ...
    second independent one. ``clara auth list`` showed two labels sharing
    ...
    monkeypatch.setattr("clara_cli.auth._codex_device_code_login", lambda: next(logins))
...
def test_codex_auth_status_reports_pool_only_credential(tmp_path, monkeypatch):
    ...
    from clara_cli.auth import get_codex_auth_status
...
def test_codex_runtime_pool_only_rate_limit_is_not_missing_auth(tmp_path, monkeypatch):
    ...
    from clara_cli.auth import AuthError, CODEX_RATE_LIMITED_CODE, resolve_codex_runtime_credentials
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    """clara auth add xai-oauth must set active_provider and write a pool entry.
    ...
    monkeypatch.setattr(
        "clara_cli.auth._xai_oauth_device_code_login",
        lambda **kwargs: {
            "tokens": {
...
def test_auth_add_xai_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``clara auth add xai-oauth`` runs must produce independent pool entries.
...
def test_auth_remove_reindexes_priorities(tmp_path, monkeypatch):
    ...
    from clara_cli.auth_commands import auth_remove_command
...
def test_auth_remove_codex_migrates_legacy_dict_suppression(tmp_path, monkeypatch):
    ...
    payload = json.loads((tmp_path / "clara" / "auth.json").read_text(encoding="utf-8"))
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`clara logout --provider openai-codex` must still clear model.provider.
    ...
    config_text = (clara_home / "config.yaml").read_text()
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from clara_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
# =============================================================================
# Unified credential-source stickiness — every source Clara reads from has a
...
def test_seed_from_singletons_respects_clara_pkce_suppression(tmp_path, monkeypatch):
    """anthropic clara_pkce must not re-seed from ~/.clara/.anthropic_oauth.json when suppressed."""
    clara_home = tmp_path / "clara"
    clara_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("CLARA_HOME", str(clara_home))

    import yaml
    (clara_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    (clara_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
        "suppressed_sources": {"anthropic": ["clara_pkce"]},
    }))

    # Stub the readers so only clara_pkce is "available"; claude_code returns None
    import agent.anthropic_adapter as aa
    monkeypatch.setattr(aa, "read_clara_oauth_credentials", lambda: {
    ...
    # clara_pkce suppressed, claude_code returns None → nothing should be seeded
    assert entries == []
    assert "clara_pkce" not in active
...
def test_credential_sources_registry_has_expected_steps():
    ...
    required = {
        ...
        "~/.clara/.anthropic_oauth.json",
        "auth.json providers.clara",
...
def test_auth_remove_copilot_suppresses_all_variants(tmp_path, monkeypatch):
    ...
    from types import SimpleNamespace
    from clara_cli.auth import is_source_suppressed
    from clara_cli.auth_commands import auth_remove_command

    with patch(
        "clara_cli.copilot_auth.resolve_copilot_token",
        return_value=("ghp_fake", "gh"),
    ), patch(
        "clara_cli.copilot_auth.get_copilot_api_token",
