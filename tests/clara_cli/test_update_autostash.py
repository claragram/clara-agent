...
from clara_cli import config as clara_config
from clara_cli import main as clara_main
...
def _patch_gateway_discovery():
    ...
    ``_purge_stale_clara_modules`` must also be stubbed: it evicts
    ``clara_cli.gateway`` from ``sys.modules`` mid-update, and the restart
    phase's fresh ``from clara_cli.gateway import ...`` then loads an
    ...
    with patch("clara_cli.gateway.find_gateway_pids", return_value=[]), \
         ...
         patch("clara_cli.update_inventory.collect_runtime_inventory", return_value=None), \
         patch("clara_cli.update_inventory.report_unaccounted_runtimes", return_value=False), \
         patch.object(clara_main, "_fleet_probe_expected_runtimes", lambda *a, **kw: False), \
         patch.object(clara_main, "_purge_stale_clara_modules", lambda *a, **kw: None), \
         patch("clara_cli.update_receipt.collect_fleet_versions", return_value=[]):
        yield
...
def _setup_update_mocks(monkeypatch, tmp_path):
    ...
    (tmp_path / ".git").mkdir()
    monkeypatch.setattr(clara_main, "PROJECT_ROOT", tmp_path)
    monkeypatch.setattr(clara_main, "_stash_local_changes_if_needed", lambda *a, **kw: None)
    monkeypatch.setattr(clara_main, "_restore_stashed_changes", lambda *a, **kw: True)
    monkeypatch.setattr(clara_config, "get_missing_env_vars", lambda required_only=True: [])
    monkeypatch.setattr(clara_config, "get_missing_config_fields", lambda: [])
    monkeypatch.setattr(clara_config, "check_config_version", lambda: (5, 5))
    monkeypatch.setattr(clara_config, "migrate_config", lambda **kw: {"env_added": [], "config_added": []})
    monkeypatch.setattr(clara_main, "_upgrade_pip_before_lazy_refresh", lambda *a, **kw: None)
    monkeypatch.setattr(clara_main, "_refresh_active_lazy_features", lambda *a, **kw: True)
...
def test_refresh_active_memory_provider_dependencies_reinstalls_active_provider(monkeypatch):
    ...
    monkeypatch.setattr(
        "clara_cli.memory_setup._install_dependencies",
        lambda provider_name, force=False: recorded.append((provider_name, force)),
    )

    clara_main._refresh_active_memory_provider_dependencies()
...
def test_reload_updated_runtime_modules_restores_new_clara_constants_symbol(monkeypatch):
    """A pre-pull module object missing a new helper is repaired by reload."""
    import clara_constants

    monkeypatch.delattr(clara_constants, "apply_subprocess_home_env", raising=False)
    assert not hasattr(clara_constants, "apply_subprocess_home_env")

    clara_main._reload_updated_runtime_modules()

    assert callable(clara_constants.apply_subprocess_home_env)
...
def _make_update_side_effect(
    ...
    ``refs/clara-update-backups/orphan-<branch>-*`` (oldest first) so the
...
def test_cmd_update_skips_stash_restore_when_reset_fails(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        clara_main, "_stash_local_changes_if_needed",
        lambda *a, **kw: "abc123deadbeef",
    )
    restore_calls = []
    monkeypatch.setattr(
        clara_main, "_restore_stashed_changes",
        lambda *a, **kw: restore_calls.append(1) or True,
    )

    side_effect, _ = _make_update_side_effect(ff_only_fails=True, reset_fails=True)
    monkeypatch.setattr(clara_main.subprocess, "run", side_effect)

    with pytest.raises(SystemExit, match="1"):
        clara_main.cmd_update(SimpleNamespace())
...
def test_cmd_update_orphan_history_backs_up_before_reset(monkeypatch, tmp_path, capsys):
    ...
    ``refs/clara-update-backups/orphan-*`` ref before the reset proceeds."""
    _setup_update_mocks(monkeypatch, tmp_path)
    ...
    assert ref_name.startswith("refs/clara-update-backups/orphan-main-")
    ...
    assert f"expires after {clara_main._ORPHAN_RESCUE_REF_MAX_AGE_DAYS} days" in out
...
def test_cmd_update_orphan_rescue_refs_pruned_beyond_keep_limit(monkeypatch, tmp_path, capsys):
    ...
    total = clara_main._ORPHAN_RESCUE_REFS_TO_KEEP + 2
    stale_refs = [
        "refs/clara-update-backups/orphan-main-"
    ...
    assert len(delete_calls) == total - clara_main._ORPHAN_RESCUE_REFS_TO_KEEP
    deleted_refs = {c[c.index("-d") + 1] for c in delete_calls}
    assert deleted_refs == set(stale_refs[: total - clara_main._ORPHAN_RESCUE_REFS_TO_KEEP])
...
def test_cmd_update_orphan_rescue_refs_expired_by_age(monkeypatch, tmp_path, capsys):
    ...
    now = datetime.now(timezone.utc)
    old = now - timedelta(days=clara_main._ORPHAN_RESCUE_REF_MAX_AGE_DAYS + 5)
...
def test_prune_orphan_rescue_refs_leaves_unparseable_names_alone():
    ...
    weird = "refs/clara-update-backups/orphan-main-not-a-timestamp-xyz"
    calls = []
    ...
    with mock_patch.object(clara_main.subprocess, "run", side_effect=fake_run):
        clara_main._prune_orphan_rescue_refs(["git"], ".", "main")
...
def _setup_setting_test(monkeypatch, tmp_path, mode):
    ...
    monkeypatch.setattr(
        clara_main, "_discard_stashed_changes",
...
def _setup_keep_stash_test(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        clara_main, "_park_stashed_changes",
    ...
    monkeypatch.setattr(
        "clara_cli.gateway.find_gateway_pids", lambda **kw: [], raising=False
    )
...
def test_update_keep_stash_parks_instead_of_restoring(monkeypatch, tmp_path):
    ...
    clara_main.cmd_update(SimpleNamespace(yes=True, keep_stash=True))
...
def test_update_without_keep_stash_still_restores(monkeypatch, tmp_path):
    ...
    clara_main.cmd_update(SimpleNamespace(yes=True, keep_stash=False))
...
def test_bootstrap_marker_not_autostashed_by_update(tmp_path):
    ...
    ``clara update``'s ``git stash push --include-untracked`` does not sweep it
    ...
    repo_gitignore = Path(clara_main.__file__).resolve().parents[1] / ".gitignore"
    ...
    marker = tmp_path / ".clara-bootstrap-complete"
    marker.write_text("")

    # Exact flags used by clara update (clara_cli/main.py).
    git("stash", "push", "--include-untracked", "-m", "clara-update-autostash")

    assert marker.exists(), (
        ".clara-bootstrap-complete was swept into the update autostash — it must "
    ...
    assert ".clara-bootstrap-complete" not in status
...
def test_restore_rejects_invalid_python_and_keeps_clean_updated_tree(
    ...
    assert "made the Clara agent unexecutable" in output
...
def test_restore_allows_preexisting_import_time_failure(monkeypatch, tmp_path):
    ...
    assert clara_main._restore_stashed_changes(
        ["git"], tmp_path, stash_ref, prompt_user=False
    )
...
def test_prune_orphan_rescue_refs_with_real_git_unpins_objects(tmp_path):
    ...
    old_ref = "refs/clara-update-backups/orphan-main-20200101-000000-" + snap_sha[:12]
    ...
    clara_main._prune_orphan_rescue_refs(["git"], tmp_path, "main")
    remaining = git("for-each-ref", "refs/clara-update-backups/").stdout
