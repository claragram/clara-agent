...
the SAME provider universe as ``clara model`` (the CLI/TUI picker).
...
def test_every_clara_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `clara model` universe."""
    ...
    assert not missing, (
        "providers shown in `clara model` but not configurable in the desktop "
