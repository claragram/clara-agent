...
def _complete_stub_login(client) -> None:
    ...
    client's cookie jar holds ``clara_session_at`` / ``clara_session_rt``
...
class _UnreachableProvider(StubAuthProvider):
    ...
    Models the real-world bug — a self-hosted-OIDC session hits the ``clara``
    provider first, which tries to reach Clara Portal's JWKS; if that's
    unreachable ``clara`` raises ProviderError. The gate must keep trying the
    ...
    def verify_session(self, *, access_token: str):
        from clara_cli.dashboard_auth.base import ProviderError
...
def _mint_stub_at(stub: StubAuthProvider) -> str:
    ...
    state = dict(
        seg.split("=", 1)
        for seg in ls.cookie_payload["clara_session_pkce"].split(";")
        if "=" in seg
    )["state"]
    verifier = dict(
        seg.split("=", 1)
