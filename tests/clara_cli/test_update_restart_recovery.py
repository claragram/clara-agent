...
def test_abort_recovery_hands_managed_profiles_to_a_fresh_process(monkeypatch):
    ...
    assert argv[1:4] == ["-m", "clara_cli.update_restart_recovery", "--stdin"]
    ...
    assert kwargs["env"]["CLARA_UPDATE_RESTART_RECOVERY"] == "1"
...
def test_service_matching_is_exact_for_overlapping_profile_names():
    assert update_cmd._gateway_service_matches_profile(
        "foo", "clara-gateway-foo.service"
    )
    assert not update_cmd._gateway_service_matches_profile(
        "foo", "clara-gateway-foobar.service"
    )
    assert update_cmd._gateway_service_matches_profile(
        "default", "ai.clara.gateway"
    )
    assert not update_cmd._gateway_service_matches_profile(
        "default", "ai.clara.gateway-foo"
    )


def test_recovery_child_restarts_each_profile_with_a_fresh_main(monkeypatch):
    recovery = importlib.import_module("clara_cli.update_restart_recovery")
    calls = []
    ...
    assert [call[0] for call in calls] == [
        [sys.executable, "-m", "clara_cli.main", "-p", "coder", "gateway", "restart"],
        [sys.executable, "-m", "clara_cli.main", "-p", "default", "gateway", "restart"],
    ]
    for _, kwargs in calls:
        ...
        assert kwargs["check"] is False
        assert kwargs["env"]["CLARA_UPDATE_RESTART_RECOVERY"] == "1"
        assert "_CLARA_GATEWAY" not in kwargs["env"]
...
def test_recovery_child_verifies_systemd_profiles_via_is_active(monkeypatch):
    ...
    def fake_run(argv, **kwargs):
        ...
        if argv[0].endswith("systemctl"):
            unit = argv[-1]
            active = unit == "clara-gateway.service"
...
def test_recovery_module_end_to_end_in_a_real_fresh_process(tmp_path):
    ...
    grandchild ``clara_cli.main … gateway restart`` invocations (recording
    ...
    shim = textwrap.dedent(
        ...
        def _shim_run(argv, *args, **kwargs):
            argv_list = list(argv)
            if "clara_cli.main" in argv_list:
    ...
    env["_CLARA_GATEWAY"] = "1"  # must be scrubbed before the grandchild runs
