"""Tests for the Speech-to-Text category in `clara tools` (tools_config).
...
from clara_cli.tools_config import (  # noqa: E402
...
class TestSttCategory:
    ...
    def test_managed_row_shares_tts_coverage_category(self):
        from clara_cli.clara_subscription import MANAGED_FEATURE_COVERAGE_CATEGORY

        managed = [p for p in _stt_cat()["providers"] if p.get("managed_clara_feature")]
        assert managed, "expected a Clara Subscription row"
        for p in managed:
            assert p["managed_clara_feature"] == "stt"
    ...
    def test_apply_provider_selection_stt(self):
        ...
        with patch(
            ...
            feats.return_value = MagicMock(
                clara_auth_present=False, account_info=None
            )
...
class TestModelPicker:
    ...
    def test_configure_stt_model_defaults_to_current(self):
        ...
        with patch(
            "clara_cli.tools_config._prompt_choice", return_value=0
        ) as pc:
...
class TestPostSetup:
    def test_faster_whisper_in_post_setup_ready(self):
        from clara_cli.tools_config import _POST_SETUP_READY
