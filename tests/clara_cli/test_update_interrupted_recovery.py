...
``clara update`` killed mid-install (Ctrl-C, terminal close, WSL OOM) gets
...
def _stub_install_env(monkeypatch, m, seen):
    ...
    monkeypatch.setattr("clara_cli.managed_uv.ensure_uv", lambda: None)
    # The install executor moved to clara_cli._install_repair (shared between
...
def test_recovery_self_lock_does_not_clear_core_marker_via_import_probes(
    ...
    monkeypatch.setattr(m, "_clara_exe_shims", lambda d: [shim])
    monkeypatch.setattr(
        m,
