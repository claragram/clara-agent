...
def container_env(tmp_path, monkeypatch):
    """Set up a fake CLARA_HOME with .container-mode file."""
    clara_home = tmp_path / ".clara"
    clara_home.mkdir()
    monkeypatch.setenv("CLARA_HOME", str(clara_home))
    monkeypatch.delenv("CLARA_DEV", raising=False)

    container_mode = clara_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=clara-agent\n"
        "exec_user=clara\n"
        "clara_bin=/data/current-package/bin/clara\n"
    )
    return clara_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("clara_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "clara-agent"
    assert info["exec_user"] == "clara"
    assert info["clara_bin"] == "/data/current-package/bin/clara"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from clara_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "clara"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "clara-agent" in cmd
    assert "/data/current-package/bin/clara" in cmd
    assert "chat" in cmd
