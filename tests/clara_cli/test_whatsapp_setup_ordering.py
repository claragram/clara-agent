...
Before the fix, ``clara whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``clara gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def _env_value(clara_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from clara_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("clara_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``clara whatsapp`` with an existing paired session and
