"""Tests for clara_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import clara_cli.gui_uninstall as gu


def _make_agent(clara_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = clara_home / "clara-agent"
    (agent_root / "clara_cli").mkdir(parents=True)
    (agent_root / "clara_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(clara_home: Path) -> None:
    """Create the source-built GUI artifacts a `clara desktop` run produces."""
    desktop = clara_home / "clara-agent" / "apps" / "desktop"
    ...
    (clara_home / "clara-agent" / "node_modules").mkdir(parents=True)
    (clara_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(clara_home: Path) -> None:
    (clara_home / "config.yaml").write_text("x: 1\n")
    (clara_home / ".env").write_text("KEY=secret\n")
    (clara_home / "sessions").mkdir()
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    _make_agent(clara_home)
    _make_gui_build(clara_home)
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    summary = gu.gui_install_summary(clara_home)
    ...
    assert summary["clara_home"] == str(clara_home)
...
def test_linux_discovery_includes_launcher_entry(tmp_path, monkeypatch):
    """The launcher entry that `clara desktop` installs is removable."""
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    clara_home = tmp_path / ".clara"
    _make_agent(clara_home)
    icon = lde.icon_path(clara_home / "clara-agent")
    icon.parent.mkdir(parents=True, exist_ok=True)
    icon.write_bytes(b"\x89PNG")
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    removed = gu.uninstall_gui(clara_home)
    ...
    assert lde.icon_path(clara_home / "clara-agent").exists()
    # The agent itself survives a GUI uninstall.
    assert (clara_home / "clara-agent" / "clara_cli").is_dir()
...
def test_uninstall_skips_cache_refresh_when_no_launcher_entry(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(tmp_path / ".clara")
...
def test_uninstall_args_namespace_mode_mapping():
    ...
    import clara_cli.uninstall as uninstall
