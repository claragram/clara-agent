...
import clara_cli.inventory as inv
import clara_cli.models as models_mod


def _patch_catalog(monkeypatch, caps_by_model, *, provider="clara"):
    """Point the Clara/OpenRouter catalog readers at a fixed capability map."""
    monkeypatch.setattr(models_mod, "model_supports_fast_mode", lambda model: False)
    monkeypatch.setattr(models_mod, "warm_clara_reasoning_caps_async", lambda: None)
...
def test_optional_reasoning_route_can_disable(monkeypatch):
    ...
    rows = [{"slug": "clara", "models": ["deepseek/deepseek-v4-pro"]}]
...
def test_non_reasoning_route_offers_no_reasoning_controls(monkeypatch):
    ...
    rows = [{"slug": "clara", "models": ["moonshotai/kimi-k3-instruct"]}]
    inv._apply_capabilities(rows)
...
def test_reasoning_mandatory_route_cannot_disable(monkeypatch):
    ...
    rows = [{"slug": "clara", "models": ["z-ai/glm-5.3"]}]
    inv._apply_capabilities(rows)
...
def test_unlisted_model_states_no_restriction(monkeypatch):
    ...
    """
    _patch_catalog(monkeypatch, {})
    rows = [{"slug": "clara", "models": ["mystery/model"]}]
    inv._apply_capabilities(rows)
...
def test_catalog_failure_never_breaks_the_picker(monkeypatch):
    ...
    monkeypatch.setattr(models_mod, "clara_model_reasoning_capabilities", _boom)
    rows = [{"slug": "clara", "models": ["deepseek/deepseek-v4-pro"]}]
    inv._apply_capabilities(rows)
