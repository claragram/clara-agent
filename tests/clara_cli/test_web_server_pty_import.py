"""Test the platform-branched PTY bridge import in clara_cli.web_server.
...
def test_web_server_uses_posix_pty_bridge_on_posix():
    ...
    from clara_cli.pty_bridge import PtyBridge as PosixBridge
    from clara_cli.pty_bridge import PtyUnavailableError as PosixErr
...
def test_pty_bridge_import_block_is_platform_branched():
    ...
    #       try:
    #           from clara_cli.win_pty_bridge import WinPtyBridge as PtyBridge, ...
    #       except ImportError:
    #           PtyBridge = None
    #           ...
    #   else:
    ...
    assert "from clara_cli.win_pty_bridge import" in src
    assert "from clara_cli.pty_bridge import" in src
