"""Regression tests for Clara OAuth refresh and inference JWT interactions."""
...
from clara_cli.auth import AuthError, get_provider_auth_state, resolve_clara_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        import ssl
        from clara_cli.auth import _resolve_verify
...
def _setup_clara_auth(
    ...
    *,
    ...
    (clara_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_resolve_clara_runtime_credentials_prefers_invoke_jwt_and_mirrors(
    tmp_path,
    monkeypatch,
):
    ...
    assert creds["source"] == auth_mod.CLARA_AUTH_PATH_INVOKE_JWT
    assert creds["auth_path"] == auth_mod.CLARA_AUTH_PATH_INVOKE_JWT

    payload = json.loads((clara_home / "auth.json").read_text())
    singleton = payload["providers"]["clara"]
    assert singleton["agent_key"] == token
    assert datetime.fromisoformat(singleton["agent_key_expires_at"]).timestamp() > time.time() + 300

    pool_entries = payload["credential_pool"]["clara"]
    assert len(pool_entries) == 1
    assert pool_entries[0]["agent_key"] == token
    assert pool_entries[0]["source"] == auth_mod.CLARA_DEVICE_CODE_SOURCE


def test_resolve_clara_runtime_credentials_invoke_jwt_is_idempotent(
    tmp_path,
    monkeypatch,
):
    import clara_cli.auth as auth_mod
    ...
    auth_store = {
        ...
        "providers": {
            "clara": {
                "portal_base_url": "https://portal.claraship.com",
                "inference_base_url": "https://inference-api.claraship.com/v1",
                "client_id": "clara-cli",
                "token_type": "Bearer",
                "scope": auth_mod.DEFAULT_CLARA_SCOPE,
                "access_token": token,
    ...
    auth_path = clara_home / "auth.json"
    ...
    monkeypatch.setattr(auth_mod, "_write_shared_clara_state", _unexpected_shared_write)
    monkeypatch.setattr(
        auth_mod,
    ...
    assert (
        payload["providers"]["clara"]["agent_key_obtained_at"]
        == original_obtained_at
    )


def test_resolve_clara_runtime_credentials_reauths_when_invoke_scope_missing(
    tmp_path,
    monkeypatch,
):
    import clara_cli.auth as auth_mod
    ...
    payload = json.loads((clara_home / "auth.json").read_text())
    assert payload["providers"]["clara"]["agent_key"] is None
    assert "credential_pool" not in payload or not payload["credential_pool"].get("clara")
...
def test_removed_legacy_session_env_var_does_not_change_jwt_auth(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CLARA_HOME", str(clara_home))
    monkeypatch.setenv("CLARA_AGENT_USE_LEGACY_SESSION_KEYS", "true")

    creds = auth_mod.resolve_clara_runtime_credentials()

    assert creds["api_key"] == token
    payload = json.loads((clara_home / "auth.json").read_text())
    assert payload["providers"]["clara"]["agent_key"] == token
    ...
    result = auth_mod._clara_device_code_login(
        portal_base_url="https://portal.example.com",
        inference_base_url="https://inference.example.com/v1",
        open_browser=False,
        timeout_seconds=1,
    ...
    assert requested_scopes == [auth_mod.DEFAULT_CLARA_SCOPE]
    assert result["agent_key"] == login_token


def test_clara_inference_auth_logs_do_not_include_secret_values(
    tmp_path,
    monkeypatch,
    caplog,
):
    ...
    caplog.set_level(logging.DEBUG, logger="clara_cli.auth")
    auth_mod.resolve_clara_runtime_credentials(
        force_refresh=True,
    )
...
def test_get_clara_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    """get_clara_auth_status() should find Clara credentials in the pool
    even when the auth store has no Clara provider entry — this is the
    case when login happened via the dashboard device-code flow which
    saves to the pool only.
    """
    from clara_cli.auth import get_clara_auth_status

    clara_home = tmp_path / "clara"
    clara_home.mkdir(parents=True, exist_ok=True)
    # Empty auth store — no Clara provider entry
    (clara_home / "auth.json").write_text(json.dumps({
        "version": 1, "providers": {},
    }))
    monkeypatch.setenv("CLARA_HOME", str(clara_home))

    # Seed the credential pool with a Clara entry
    from agent.credential_pool import PooledCredential, load_pool
    pool = load_pool("clara")
    token = _invoke_jwt(seconds=3600)
    expires_at = _future_iso(3600)
    entry = PooledCredential.from_dict("clara", {
        "access_token": token,
    ...
    status = get_clara_auth_status()
    assert status["logged_in"] is True
    assert "example.com" in str(status.get("portal_base_url", ""))


def test_get_clara_auth_status_empty_returns_not_logged_in(tmp_path, monkeypatch):
    """get_clara_auth_status() returns logged_in=False when both pool
...
# =============================================================================
# _login_clara: "Skip (keep current)" must preserve prior provider + model
# =============================================================================


class TestLoginClaraSkipKeepsCurrent:
    """When a user runs `clara model` → Clara Portal → Skip (keep current) after
    ...
    unconditionally after login, which flipped model.provider to "clara" while
    ...
    def _setup_home_with_openrouter(self, tmp_path, monkeypatch):
        ...
        return clara_home, config_path, auth_path

    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        """Patch OAuth + model-list + prompt so _login_clara doesn't hit network."""
        import clara_cli.auth as auth_mod
        import clara_cli.models as models_mod
        import clara_cli.clara_subscription as ns

        fake_auth_state = {
            "access_token": "fake-clara-token",
            "agent_key": "fake-agent-key",
            "inference_base_url": "https://inference-api.claraship.com",
            "portal_base_url": "https://portal.claraship.com",
        ...
        def _check_clara_free_tier(**kwargs):
            free_tier_calls.append(kwargs)
            return None

        monkeypatch.setattr(models_mod, "check_clara_free_tier", _check_clara_free_tier)
        monkeypatch.setattr(
            models_mod, "partition_clara_models_by_tier",
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        """User picks Skip → config.yaml untouched, Clara creds still saved."""
        import argparse
        import yaml
        from clara_cli.auth import PROVIDER_REGISTRY, _login_clara

        clara_home, config_path, auth_path = self._setup_home_with_openrouter(
        ...
        _login_clara(args, PROVIDER_REGISTRY["clara"])
        ...
        # auth.json: active_provider restored to openrouter, but Clara creds saved
        auth_after = json.loads(auth_path.read_text())
        assert auth_after["active_provider"] == "openrouter"
        assert "clara" in auth_after["providers"]
        assert auth_after["providers"]["clara"]["access_token"] == "fake-clara-token"
        # Existing openrouter creds still intact
        assert auth_after["providers"]["openrouter"]["api_key"] == "sk-or-fake"

    def test_picking_model_switches_to_clara(self, tmp_path, monkeypatch):
        """User picks a Clara model → provider flips to clara with that model."""
        import argparse
        import yaml
        from clara_cli.auth import PROVIDER_REGISTRY, _login_clara
    ...
    def test_skip_with_no_prior_active_provider_clears_it(self, tmp_path, monkeypatch):
        ...
        instead of leaving it as clara."""
        import argparse
        import yaml
        from clara_cli.auth import PROVIDER_REGISTRY, _login_clara
        ...
        auth_path = clara_home / "auth.json"
        auth_after = json.loads(auth_path.read_text())
        # active_provider should NOT be set to "clara" after Skip
        assert auth_after.get("active_provider") in {None, ""}
        # But Clara creds are still saved
        assert "clara" in auth_after.get("providers", {})


# =============================================================================
# persist_clara_credentials: shared helper for CLI + web dashboard login paths
# =============================================================================


def _full_state_fixture() -> dict:
    """Shape of the dict returned by _clara_device_code_login /
    refresh_clara_oauth_from_state. Used as helper input."""
    token = _invoke_jwt(seconds=3600)
    expires_at = _future_iso(3600)
    return {
        "portal_base_url": "https://portal.example.com",
...
def test_persist_clara_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    """Helper must populate BOTH credential_pool.clara AND providers.clara.

    Regression guard: before this helper existed, `clara auth add clara`
    wrote only the pool. After the Clara agent_key's 24h TTL expired, the
    401-recovery path in run_agent.py called resolve_clara_runtime_credentials
    which reads providers.clara, found it empty, raised AuthError, and the
    agent failed with "Non-retryable client error". Both stores must stay
    in sync at write time.
    """
    from clara_cli.auth import persist_clara_credentials, CLARA_DEVICE_CODE_SOURCE
    ...
    entry = persist_clara_credentials(state)

    assert entry is not None
    assert entry.provider == "clara"
    assert entry.source == CLARA_DEVICE_CODE_SOURCE

    payload = json.loads((clara_home / "auth.json").read_text())

    # providers.clara populated with the full state (new behaviour)
    singleton = payload["providers"]["clara"]
    ...
    # credential_pool.clara has exactly one canonical device_code entry
    pool_entries = payload["credential_pool"]["clara"]
    assert len(pool_entries) == 1, pool_entries
    pool_entry = pool_entries[0]
    assert pool_entry["source"] == CLARA_DEVICE_CODE_SOURCE
    assert pool_entry["agent_key"] == state["agent_key"]
    assert pool_entry["inference_base_url"] == "https://inference.example.com/v1"


def test_persist_clara_credentials_idempotent_no_duplicate_pool_entries(tmp_path, monkeypatch):
    ...
    ``load_pool()``. The helper now writes providers.clara and lets seeding
    ...
    persist_clara_credentials(first)
    ...
    persist_clara_credentials(second)

    payload = json.loads((clara_home / "auth.json").read_text())

    # providers.clara reflects the latest write (singleton semantics)
    assert payload["providers"]["clara"]["access_token"] == second_token
    assert payload["providers"]["clara"]["agent_key"] == second_token

    # credential_pool.clara has exactly one entry, carrying the latest agent_key
    pool_entries = payload["credential_pool"]["clara"]
    assert len(pool_entries) == 1, pool_entries
    assert pool_entries[0]["source"] == CLARA_DEVICE_CODE_SOURCE
...
def test_persist_clara_credentials_no_label_uses_auto_derived(tmp_path, monkeypatch):
    ...
    entry = persist_clara_credentials(_full_state_fixture())
    assert entry is not None
    ...
    # No "label" key embedded in providers.clara when the caller didn't supply one.
    payload = json.loads((clara_home / "auth.json").read_text())
    assert "label" not in payload["providers"]["clara"]


def test_refresh_token_reuse_detection_surfaces_actionable_message():
    """Regression for #15099.

    When the Clara Portal server returns ``invalid_grant`` with
    ``error_description`` containing "reuse detected", Clara must surface an
    ...
    re-authenticate" string led users to report this as a Clara persistence
    bug when the true cause is external RT consumption (monitoring scripts,
    custom self-heal hooks).
    """
    from clara_cli.auth import _refresh_access_token
    ...
    assert "clara auth add clara" in message.lower()
...
def test_refresh_token_exchange_sends_refresh_token_header():
    """Clara refresh tokens must be sent in a header so sandbox proxies can
    ...
    assert client.kwargs["headers"]["x-clara-refresh-token"] == "refresh-1"
...
# =============================================================================
# Shared Clara token store — cross-profile persistence (Codex-style auto-import)
# =============================================================================


@pytest.fixture
def shared_store_env(tmp_path, monkeypatch):
    """Redirect CLARA_SHARED_AUTH_DIR to a tmp_path.

    Required for every test that exercises the shared Clara store — the
    ...
    monkeypatch.setenv("CLARA_SHARED_AUTH_DIR", str(shared_dir))
    return shared_dir


def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    """Without CLARA_SHARED_AUTH_DIR override, the seat belt must trip.
    ...
    writing to the user's real ``~/.clara/shared/`` across CI runs.
    """
    from clara_cli.auth import _clara_shared_store_path

    monkeypatch.delenv("CLARA_SHARED_AUTH_DIR", raising=False)

    with pytest.raises(RuntimeError, match="shared Clara auth store"):
        _clara_shared_store_path()
...
def test_shared_store_write_and_read_roundtrip(shared_store_env):
    ...
    loaded = _read_shared_clara_state()
...
def test_persist_clara_credentials_mirrors_to_shared_store(
    tmp_path, monkeypatch, shared_store_env,
):
    """persist_clara_credentials must populate BOTH per-profile auth.json
    AND the shared store, so a future profile's `clara auth add clara
    --type oauth` can one-tap import instead of redoing device-code.
    """
    from clara_cli.auth import (
        _clara_shared_store_path,
        _read_shared_clara_state,
        persist_clara_credentials,
    )

    clara_home = tmp_path / "clara"
    clara_home.mkdir(parents=True, exist_ok=True)
    (clara_home / "auth.json").write_text(
        json.dumps({"version": 1, "providers": {}})
    )
    monkeypatch.setenv("CLARA_HOME", str(clara_home))

    persist_clara_credentials(_full_state_fixture())

    # Per-profile auth.json populated
    payload = json.loads((clara_home / "auth.json").read_text())
    assert "clara" in payload.get("providers", {})
    ...
    assert str(_clara_shared_store_path()).startswith(str(shared_store_env))
...
def test_try_import_shared_rehydrates_on_success(shared_store_env, monkeypatch):
    ...
    every field persist_clara_credentials() needs.
    """
    from clara_cli import auth as auth_mod

    auth_mod._write_shared_clara_state(_full_state_fixture())
    ...
    monkeypatch.setattr(auth_mod, "refresh_clara_oauth_from_state", _fake_refresh)

    result = auth_mod._try_import_shared_clara_state()
    ...
    assert result["client_id"] == "clara-cli"
...
class TestStalePortalBaseUrlMigration:
    """_migrate_stale_clara_portal_url auto-corrects stale portal_base_url on load."""

    def test_migrates_stale_portal_url_on_load(self, tmp_path, monkeypatch):
        from clara_cli.auth import _load_auth_store, DEFAULT_CLARA_PORTAL_URL
        ...
        auth_file.write_text(json.dumps({
            ...
            "providers": {
                "clara": {
                    "portal_base_url": "https://api.claraship.com",
        ...
        clara = store["providers"]["clara"]
        assert clara["portal_base_url"] == DEFAULT_CLARA_PORTAL_URL
    ...
    def test_runtime_credentials_rejects_http_for_production_portal(
        ...
        auth_mod.resolve_clara_runtime_credentials()
        assert refresh_calls == [auth_mod.DEFAULT_CLARA_PORTAL_URL]
...
class TestClaraDeviceAuthTimeoutMessage:
    def test_timeout_message_mentions_captcha_login_and_retry(self):
        ...
        msg = _clara_device_auth_timeout_message("https://portal.claraship.com")
        assert "CAPTCHA" in msg
        assert "clara portal" in msg
        assert "https://portal.claraship.com/login" in msg
    ...
    def test_timeout_message_falls_back_to_default_portal(self):
        from clara_cli.auth import (
            ...
            _clara_device_auth_timeout_message,
        )

        msg = _clara_device_auth_timeout_message("")
        assert f"{DEFAULT_CLARA_PORTAL_URL.rstrip('/')}/login" in msg
...
def test_poll_for_token_timeout_raises_actionable_message():
    ...
    so both the CLI login and the dashboard poller (web_server._clara_poller,
...
def test_clara_device_code_login_timeout_raises_actionable_message(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: {
            "device_code": "device",
            "user_code": "SMCL-97YT",
            "verification_uri": "https://portal.claraship.com/manage-subscription",
            "verification_uri_complete": (
                "https://portal.claraship.com/manage-subscription"
                "?user_code=SMCL-97YT"
            ),
            "expires_in": 600,
            "interval": 1,
    ...
    def _timeout(**kwargs):
        raise TimeoutError(
            auth_mod._clara_device_auth_timeout_message(
