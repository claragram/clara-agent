...
import clara_cli.gateway as gateway


_BREAKAWAY_MARKER = "_CLARA_GATEWAY_BREAKAWAY"
...
def _install_fake_gateway_run(monkeypatch, start_gateway):
    ...
    # /clara-gateway.service"`` and runs ``systemctl --user daemon-reload``
    # — both target the *real* user environment because the conftest only
    # sandboxes ``CLARA_HOME``, not ``HOME``. Tests that drive
    ...
    # unit (baking in the test's pytest-tmp ``CLARA_HOME`` value, which
...
def _run_native_windows_gateway_start_diag(
    ...
    script = textwrap.dedent(
        ...
        async def start_gateway(*, replace, verbosity):
            assert "_CLARA_GATEWAY_BREAKAWAY" not in os.environ
            return True
        ...
        diag_path = pathlib.Path(os.environ["CLARA_HOME"]) / "logs" / "gateway-exit-diag.log"
    ...
    env.update(
        {
            "CLARA_HOME": str(tmp_path),
            "CLARA_GATEWAY_DETACHED": "1",
            "CLARA_GATEWAY_EXIT_DIAG": "1",
            "CLARA_GATEWAY_MAX_STARTS": "0",
...
def test_gateway_run_subprocess_preserves_daemon_exit_codes(
    ...
    script = textwrap.dedent(
        ...
        outcome = os.environ["CLARA_TEST_GATEWAY_OUTCOME"]
    ...
    env = {
        **os.environ,
        "CLARA_HOME": str(tmp_path),
        "CLARA_GATEWAY_EXIT_DIAG": "0",
        "CLARA_TEST_GATEWAY_OUTCOME": outcome,
...
def _clear_supervisor_markers(monkeypatch):
    ...
    monkeypatch.delenv("CLARA_S6_SUPERVISED_CHILD", raising=False)
...
def test_s6_runtime_snapshot_reports_supervised_service(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("clara_constants.is_container", lambda: True)
    monkeypatch.setattr("clara_cli.service_manager.detect_service_manager", lambda: "s6")
    monkeypatch.setattr("clara_cli.service_manager.get_service_manager", lambda: FakeS6Manager())
...
def test_spawn_detached_gateway_timestamps_stderr(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway, "get_clara_home", lambda: tmp_path)
...
def test_systemd_install_checks_linger_status(monkeypatch, tmp_path, capsys):
    unit_path = tmp_path / "systemd" / "user" / "clara-gateway.service"

    monkeypatch.setattr(gateway, "get_systemd_unit_path", lambda system=False: unit_path)
    # Synthetic unit with a non-temp home: the real generator bakes the
    # hermetic test CLARA_HOME (a tmp dir), which the temp-home write
    # guard correctly refuses.
    monkeypatch.setattr(
        gateway,
        "generate_systemd_unit",
        lambda system=False, run_as_user=None: (
            '[Service]\nEnvironment="CLARA_HOME=/home/alice/.clara"\n'
        ),
    )
...
def test_gateway_install_noninteractive_skips_legacy_unit_prompt(monkeypatch, tmp_path):
    ...
    """
    monkeypatch.setattr(gateway, "has_legacy_clara_units", lambda: True)
    ...
    monkeypatch.setattr(gateway, "remove_legacy_clara_units", lambda interactive=False: calls.append(("remove_legacy",)))
    monkeypatch.setattr(gateway, "print_legacy_unit_warning", lambda: None)

    fake_path = tmp_path / "clara-gateway.service"
...
class TestWaitForGatewayExit:
    ...
    def test_kill_gateway_processes_force_uses_helper(self, monkeypatch):
        ...
        monkeypatch.setattr(
            gateway, "_capture_gateway_argv", lambda pid: ["python", "-m", "clara_cli.main", "gateway", "run"]
        )
        monkeypatch.setattr(
            gateway,
            "terminate_pid",
    ...
    def test_kill_gateway_processes_force_refuses_recycled_pid(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda pid: None if pid == 11 else ["python", "-m", "clara_cli.main", "gateway", "run"],
        )
        monkeypatch.setattr(
            gateway,
            "terminate_pid",
...
class TestReapUnsupervisedGatewayOrphansMacOS:
    ...
    gateway every time Clara Desktop opens (``clara serve`` calls
...
class TestReapUnsupervisedGatewayOrphansWindows:
    ...
    Scheduled-Task-supervised gateway every time Clara Desktop opens
    (``clara serve`` calls ``_reap_unsupervised_gateway_orphans`` during
...
class TestReaperCandidateIsSupervisorOwned:
    ...
    def test_windows_scheduled_task_gateway_spared_without_pidfile(self, monkeypatch):
        ...
        gateway_pid = 52615
        bootstrap_pid = 52616   # Task-launched `clara gateway run` bootstrap
        ...
        bootstrap = SimpleNamespace(
            pid=bootstrap_pid, parent=lambda: services, name=lambda: "clara-gateway.exe"
        )
        gw = SimpleNamespace(
            pid=gateway_pid, parent=lambda: bootstrap, name=lambda: "clara-gateway.exe"
        )
        ...
        orphan = SimpleNamespace(pid=orphan_pid, parent=lambda: None, name=lambda: "clara-gateway.exe")
    ...
    def test_windows_backstop_fails_open_when_bootstrap_exited(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway, "is_windows", lambda: True)
        stranded = SimpleNamespace(pid=4242, parent=lambda: None, name=lambda: "clara-gateway.exe")
...
def test_module_has_logger():
    ...
    assert gateway.logger.name == "clara_cli.gateway"
...
class TestWindowsScheduledTaskSupervisorGuard:
    ...
    def test_running_task_skips_reap(self, monkeypatch):
        """Clara_Gateway_* is Running => reaper returns False, kills nothing."""
        monkeypatch.setattr(gateway, "is_windows", lambda: True)
        monkeypatch.setattr(gateway, "is_macos", lambda: False)
        monkeypatch.setattr(gateway, "supports_systemd_services", lambda: False)
        ...
        # hardcoded "ClaraGateway" would leave the guard dormant on every
        # standard install (task name is Clara_Gateway / Clara_Gateway_<p>).
        import clara_cli.gateway_windows as gateway_windows

        monkeypatch.setattr(
            gateway_windows, "get_task_name", lambda: "Clara_Gateway_testprof"
        )
        queried = []
        ...
        assert result is False
        assert killed_pids == []
        assert queried == ["Clara_Gateway_testprof"]
    ...
    def test_windows_scheduled_task_running_returns_false_off_windows(self, monkeypatch):
        ...
        assert gateway._windows_scheduled_task_running("ClaraGateway") is False
        assert gateway._windows_scheduled_task_supervises("ClaraGateway") is False
        assert gateway._windows_scheduled_task_state("ClaraGateway") is None
    ...
    def test_supervises_ready_and_queued_but_not_disabled(self, monkeypatch):
        ...
        for state, expected in states.items():
            ...
            assert gateway._windows_scheduled_task_supervises("Clara_Gateway") is expected, state
            assert gateway._windows_scheduled_task_running("Clara_Gateway") is (state == "Running")
...
def test_find_windows_gateway_services_rejects_transitional_ancestor(monkeypatch):
    ...
    class FakeService:
        def as_dict(self):
            return {"name": "ClaraGateway", "pid": 100, "status": "stop_pending"}
