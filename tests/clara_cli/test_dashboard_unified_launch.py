...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        monkeypatch.setattr(
            "clara_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
        ...
        # CLARA_HOME.  For a standard install (CLARA_HOME unset) that root is
        # the platform-native default (~/.clara), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from clara_constants import get_default_clara_root
        assert env.get("CLARA_HOME") == str(get_default_clara_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (CLARA_DESKTOP=1) must NOT
