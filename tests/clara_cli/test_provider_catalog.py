"""Tests for the unified provider catalog (clara_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``clara model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from clara_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
