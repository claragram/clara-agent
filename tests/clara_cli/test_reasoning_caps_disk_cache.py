...
"unknown" — and on that answer the Clara profile drops a "thinking off" disable
rather than risk a 400. A short-lived process (``clara -p``, a cron job, a
...
def test_fetched_catalog_answers_a_later_process_offline(
    ...
    caps = models_mod.clara_model_reasoning_capabilities("deepseek/deepseek-v4-pro")
    assert caps["mandatory"] is False
    mandatory = models_mod.clara_model_reasoning_capabilities(
...
def test_mirror_is_keyed_by_catalog_url(cold_process, offline, monkeypatch):
    ...
    models_mod.clara_model_reasoning_capabilities(
        "deepseek/deepseek-v4-pro", allow_fetch=True
    )
...
def test_staging_portal_does_not_read_productions_mirror(
    ...
    cold_process()
    monkeypatch.setenv("CLARA_INFERENCE_BASE_URL", "https://staging.claraship.com")
    monkeypatch.setattr(models_mod, "_urlopen_model_catalog_request", offline)
...
def test_stale_copy_is_still_served(cold_process, offline, monkeypatch):
    ...
    """
    url = models_mod.clara_catalog_url()
...
def test_unreadable_mirror_degrades_to_unknown(cold_process, offline, monkeypatch):
    ...
    monkeypatch.setattr(models_mod, "_urlopen_model_catalog_request", offline)
    assert models_mod.clara_model_reasoning_capabilities("deepseek/deepseek-v4-pro") is None
...
def test_pricing_fetch_seeds_the_mirror(cold_process, offline, monkeypatch):
    ...
    models_mod.fetch_models_with_pricing(
        base_url="https://inference-api.claraship.com"
    )
...
def test_a_missing_mirror_is_looked_for_once_per_process(cold_process, monkeypatch):
    ...
    """
    counts = {"url": 0, "read": 0}
    real_url = models_mod.clara_catalog_url
    ...
    monkeypatch.setattr(models_mod, "clara_catalog_url", _counting_url)
    monkeypatch.setattr(models_mod, "_read_reasoning_caps_disk", _counting_read)
    for _ in range(5):
        models_mod.clara_model_reasoning_capabilities("deepseek/deepseek-v4-pro")
