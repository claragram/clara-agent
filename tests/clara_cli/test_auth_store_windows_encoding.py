...
``_load_auth_store`` and the Codex/Clara shared-store readers previously called
...
def clara_home(tmp_path, monkeypatch):
    """Point CLARA_HOME at a tmp dir so we never touch the real auth store.

    Required because ``_auth_file_path()`` has a seat belt that refuses to
    resolve to the real user's ~/.clara/auth.json under pytest.
    """
...
class TestAuthStoreEncodingRoundTrip:
    def test_load_reads_utf8_with_non_ascii_label(self, clara_home, windows_default_encoding):
    ...
    def test_load_does_not_corrupt_store_on_non_ascii(self, clara_home, windows_default_encoding):
        ...
        assert not (clara_home / "auth.json.corrupt").exists()
    ...
    def test_load_handles_utf8_with_bom(self, clara_home):
...
class TestExplicitEncodingPassed:
    ...
    def test_load_auth_store_passes_utf8_encoding(self, clara_home):
        auth_path = clara_home / "auth.json"
...
# --- sibling readers of the same ~/.clara/auth.json in other modules -------
#
# _load_auth_store lives in clara_cli/auth.py, but several other modules read
# the same ~/.clara/auth.json directly. They had the same UTF-8-vs-cp1252
# asymmetry on Windows — these tests pin the sibling reads too.

class TestAuthJsonSiblingReaders:
    def test_has_xai_credentials_reads_non_ascii_store(self, clara_home, windows_default_encoding, monkeypatch):
        ...
        _write_utf8(clara_home / "auth.json", store)

        from tools.xai_http import has_xai_credentials

        assert has_xai_credentials() is True

    def test_auxiliary_clara_provider_reads_non_ascii_store(self, clara_home, windows_default_encoding, monkeypatch):
        """agent/auxiliary_client's Clara-provider lookup reads the same store.

        The lookup returns None on any read failure, silently disabling Clara as
        ...
        store = {
            ...
            "providers": {"clara": {"agent_key": "k", "label": "工作账号"}},
        }
        _write_utf8(clara_home / "auth.json", store)
        ...
        # CLARA_HOME env from the fixture doesn't reach it — point it at
        # the tmp store explicitly.
        monkeypatch.setattr(aux, "_AUTH_JSON_PATH", clara_home / "auth.json")

        # _read_clara_auth consults the credential pool FIRST and returns early
        ...
        provider = aux._read_clara_auth()
        assert provider is not None
        assert provider.get("agent_key") == "k"

    def test_read_shared_clara_state_reads_non_ascii_store(
        self, tmp_path, monkeypatch, windows_default_encoding
    ):
        """clara_cli.auth._read_shared_clara_state must read a non-ASCII store.

        The shared Clara store (``clara_auth.json``) is written as UTF-8. A
        ...
        _write_utf8(shared_dir / "clara_auth.json", payload)

        provider = auth._read_shared_clara_state()
        assert provider is not None
    ...
    def test_has_any_provider_configured_reads_non_ascii_auth_store(
        self, clara_home, monkeypatch, windows_default_encoding
    ):
        """clara_cli.main._has_any_provider_configured reads auth.json.
        ...
        # clara_cli.auth; patch it on the source module so the local import
        # sees the fake.
    ...
    def test_managed_tool_gateway_reads_non_ascii_clara_state(
        self, clara_home, windows_default_encoding
    ):
        """tools.managed_tool_gateway._read_clara_provider_state reads auth.json.

        The Clara provider entry can carry a non-ASCII label. Under the
        ...
        from tools.managed_tool_gateway import _read_clara_provider_state

        clara = _read_clara_provider_state()
        assert clara is not None
        assert clara.get("agent_key") == "k"
        # The non-ASCII label round-trips intact.
        assert clara.get("label") == "工作账号"
