"""Tests for the `clara proxy` subcommand and its upstream adapters."""
...
from clara_cli.proxy.adapters import ADAPTERS, get_adapter
from clara_cli.proxy.adapters.base import UpstreamAdapter, UpstreamCredential
from clara_cli.proxy.adapters.clara_portal import ClaraPortalAdapter
from clara_cli.proxy.adapters.xai import XAIGrokAdapter
...
def _write_auth_store(clara_home: Path, clara_state: Dict[str, Any]) -> Path:
    """Write an auth.json with the given clara state into a hermetic CLARA_HOME."""
    auth_path = clara_home / "auth.json"
    auth_path.write_text(json.dumps({
        "version": 1,
        "providers": {"clara": clara_state},
    }))
    return auth_path
...
def test_clara_adapter_concurrent_refresh_serialized(tmp_path, monkeypatch):
    ...
    with patch(
        "clara_cli.proxy.adapters.clara_portal.resolve_clara_runtime_credentials",
...
def _write_xai_pool_entry(
    ...
    *,
    ...
    """Write an xai-oauth pool entry into a hermetic CLARA_HOME."""
    auth_path = clara_home / "auth.json"
    auth_path.write_text(json.dumps({
        "version": 1,
        "providers": {},
...
def test_xai_adapter_retry_rotates_pool_entry_on_429(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("clara_cli.auth.refresh_xai_oauth_pure", _refresh_must_not_run)
...
from clara_cli.proxy.server import create_app  # noqa: E402
