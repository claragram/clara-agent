"""Regression tests for #34107 — Docker UID/GID handling in ensure_clara_home.

When Clara runs in Docker with ``CLARA_UID=1000`` / ``CLARA_GID=911``,
the entrypoint chowns the top-level ``CLARA_HOME`` once at startup. But
...
The fix is a ``_chown_to_clara_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_clara_uid_gid
# ---------------------------------------------------------------------------


class TestResolveClaraUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("CLARA_UID", "1000")
        monkeypatch.setenv("CLARA_GID", "911")
        from clara_cli.config import _resolve_clara_uid_gid
...
class TestChownToClaraUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_clara_uid(d)  # must not raise
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("CLARA_UID", raising=False)
        monkeypatch.delenv("CLARA_GID", raising=False)
        from clara_cli import config as cfg
