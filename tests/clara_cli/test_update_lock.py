"""Cross-process update mutual exclusion (``clara_cli.update_lock``).

Three surfaces can start an update of one install tree: a terminal ``clara
...
dashboard-spawned ``clara update`` was mid-``npm install``, which then failed
...
@pytest.fixture
def marker(tmp_path):
    return tmp_path / ".clara-update-in-progress"


def test_marker_path_follows_process_clara_home(tmp_path, monkeypatch):
    """The lock must land where the Rust updater and Electron gate look.

    All three resolve the *process* CLARA_HOME; a profile-scoped path would
    put the lock somewhere the other two owners never read.
    """
    monkeypatch.setenv("CLARA_HOME", str(tmp_path))
    assert update_marker_path() == tmp_path / ".clara-update-in-progress"
...
class TestHandoffFromOrchestratingUpdater:
    """The Tauri updater holds the marker, then spawns ``clara update``.

    The regression: the child saw its own parent's live marker and exited 2,
    so every GUI update failed with "Clara is still running" and retrying
...
class TestAncestryHandoff:
    ...
    ``clara-setup`` under ``~/.clara`` is only refreshed by a full installer
    ...
    def _liveness_pinned_true(self, monkeypatch):
        monkeypatch.setattr("clara_cli.update_lock._pid_alive", lambda pid: True)
