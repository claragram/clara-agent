...
def _summary_output(capsys, provider_ready: bool):
    ...
    with patch("clara_cli.auth.resolve_provider", resolver), \
         patch.object(setup_mod, "get_clara_subscription_features") as feats:
...
def test_summary_warns_when_no_provider(capsys):
    ...
    assert "clara model" in out
    assert "clara setup --portal" in out
