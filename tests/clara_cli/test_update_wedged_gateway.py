...
still running after 180.0s — restart may fail") and could deadlock `clara
...
def tmp_path():
    ...
    The loop-tick witness tests bind real UNIX sockets under CLARA_HOME.
...
def _start_freezeable_producer(tmp_path, block_s, errors, write_stall_s=1.5):
    ...
    - ``state["trigger"]`` — freezes the gateway loop __PROT_2_synchroclaraly__ for
    ...
    async def freeze_gate() -> None:
        while True:
            ...
            freeze_evt.clear()
            # __PROT_0_Synchroclara__ sleep: freezes the entire loop for block_s.
            time.sleep(block_s)
...
def _launchd_harness(monkeypatch, tmp_path, pid):
    ...
    monkeypatch.setattr(
        "gateway.shutdown_watchdog._process_clara_home", lambda: tmp_path
    )
    return events
...
class TestLoopTickWitness:
    ...
    def test_stalled_heartbeat_write_never_escalates_a_running_loop(
        ...
        try:
            ...
            events = []
            monkeypatch.setattr(
                gateway_cli, "get_launchd_label", lambda: "ai.clara.gateway"
            )
            monkeypatch.setattr(gateway_cli, "_launchd_domain", lambda: "gui/501")
            monkeypatch.setattr(gateway_cli, "_get_restart_drain_timeout", lambda: 180.0)
            monkeypatch.setattr(
    ...
    def test_single_silent_probe_is_not_destructive(self, tmp_path, monkeypatch):
        ...
        def flaky_probe(_pid, _home, timeout=1.0):
            ...
            # First sample misses (transient __PROT_1_synchroclara__ stall), the loop
...
def test_default_probe_budget_stays_inside_query_tier():
    ...
    assert worst_case <= 5.0, (
        ...
        "subprocess-timeout doc reference in clara_cli/gateway.py"
    )
