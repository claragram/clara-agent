...
The bug class: on Windows, when `clara.exe`/sibling shims could not be
...
import clara_cli._install_repair as ir
import clara_cli.update_cmd as update_cmd


def _make_shims(scripts_dir: Path, names=("clara", "clara-gateway")) -> list[Path]:
...
def test_strict_quarantine_refuses_before_install(windows, tmp_path, monkeypatch):
    ...
    # clara.exe cannot be renamed; clara-gateway.exe can
    real_rename = Path.rename

    def deny_clara(self, target):
        if self.name == "clara.exe":
            raise PermissionError(13, "held open")
        return real_rename(self, target)

    monkeypatch.setattr(Path, "rename", deny_clara)
    monkeypatch.setattr(cli_main, "_clara_exe_shims", lambda d: shims)
    ...
    assert install_ran == []
    assert "clara.exe" in exc_info.value.failed_shims
    # The successful rename (clara-gateway.exe) was rolled back.
    assert (scripts / "clara-gateway.exe").exists()
    assert not list(scripts.glob("clara-gateway.exe.old.*"))


def test_non_strict_keeps_warn_and_try(windows, tmp_path, monkeypatch):
    scripts = tmp_path / "venv" / "Scripts"
    shims = _make_shims(scripts, names=("clara",))
    monkeypatch.setattr(
        Path, "rename",
...
def test_recovery_install_cmd_fail_closed(windows, tmp_path, monkeypatch):
    ...
    _make_shims(scripts, names=("clara",))
