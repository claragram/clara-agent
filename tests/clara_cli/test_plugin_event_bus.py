...
Covers:
  ...
  - Namespace spoofing (clara:, foreign, already-colon'd) is rejected
  - Non-blocking bounded delivery for __PROT_0_synchroclara__ subscribers
  ...
  - `clara plugins show` output includes emits/listens
"""
...
def test_emit_rejects_namespaced_names(bad_event):
    ...
    for name in (bad_event, f"b:{bad_event}", "clara:x", "a:x", "b:x"):
...
def test_subscribe_is_unrestricted():
    """Any plugin may subscribe to any event, including clara: and foreign."""
    manager = _fresh_manager()
    ctx_a = _make_ctx(manager, "plugin_a", key="a")
    got = []
    # None of these raise — only emit is namespace-gated.
    ctx_a.subscribe("clara:core_event", lambda **p: got.append("clara"))
    ctx_a.subscribe("b:ping", lambda **p: got.append("b"))
    assert "clara:core_event" in manager._subscriptions
    assert "b:ping" in manager._subscriptions
