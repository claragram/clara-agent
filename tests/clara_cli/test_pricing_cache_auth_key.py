...
Clara ``/v1/models`` answers each caller with the catalog their org may reach,
...
from clara_cli.models import fetch_models_with_pricing, peek_cached_pricing
...
class TestClaraCatalogExpiry:
    """A Clara catalog reflects the org's policy, which an admin can change while
    ...
    def test_entry_expires_so_a_policy_change_is_picked_up(self, catalog, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: now + _CLARA_CATALOG_TTL_SECONDS + 1,
        )
        fetch_models_with_pricing(
            api_key="sk-test", base_url=BASE,
            cache_ttl_seconds=_CLARA_CATALOG_TTL_SECONDS,
