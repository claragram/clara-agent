"""Every Clara model list is narrowed to the org's policy before it is shown.
...
def policy(monkeypatch):
    ...
    monkeypatch.setattr(models_mod, "clara_policy_allowed_ids", lambda **_k: ALLOWED)
    return ALLOWED
...
def no_policy(monkeypatch):
    ...
    monkeypatch.setattr(models_mod, "clara_policy_allowed_ids", lambda **_k: None)


class TestLoginClara:
    ...
    def _run(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(models_mod, "get_curated_clara_model_ids", lambda: list(CURATED))
        monkeypatch.setattr(models_mod, "get_pricing_for_provider", lambda _p: {})
        monkeypatch.setattr(models_mod, "check_clara_free_tier", lambda **_k: None)
        monkeypatch.setattr(
            models_mod,
        ...
        auth_mod._login_clara(args, auth_mod.PROVIDER_REGISTRY["clara"])
        return seen
...
class TestModelSwitchPicker:
    """The ``/model`` picker's clara branch (``list_authenticated_providers``)."""

    def _rows(self, monkeypatch):
        import clara_cli.auth as auth_mod
        import clara_cli.model_switch as ms

        monkeypatch.setattr(
            auth_mod,
            "_load_auth_store",
            lambda *a, **k: {"providers": {"clara": {"access_token": "tok"}}},
        )
        monkeypatch.setattr(models_mod, "get_curated_clara_model_ids", lambda: list(CURATED))
        monkeypatch.setattr(models_mod, "get_pricing_for_provider", lambda _p: {})
        monkeypatch.setattr(models_mod, "check_clara_free_tier", lambda **_k: None)
    ...
    def test_hidden_model_is_filtered(self, monkeypatch, policy):
        ...
        assert row is not None, "clara row should be listed"
...
class TestRecommendedDefaultEndpoint:
    ...
    def _call(self, monkeypatch):
        ...
        return get_recommended_default_model(provider="clara")
...
class TestAuxiliaryFastModel:
    ...
    def _pick(self, monkeypatch, *, catalog):
        ...
        monkeypatch.setattr(
            models_mod, "_resolve_clara_pricing_credentials",
            lambda: ("sk-clara", "https://inference.example.com"),
        )
        monkeypatch.setattr(models_mod, "fetch_models_with_pricing", _fake_fetch)
        picked = aux._fast_model_from_catalog("clara")
        return picked, seen

    def test_reads_the_catalog_with_clara_oauth_credentials(self, monkeypatch, no_policy):
        """The api-key resolver raises for OAuth providers."""
        _, seen = self._pick(monkeypatch, catalog=["vendor/haiku-fast"])
        assert seen["api_key"] == "sk-clara"

    def test_hidden_model_is_not_selected(self, monkeypatch, policy):
        import agent.auxiliary_client as aux

        monkeypatch.setattr(
            models_mod, "clara_policy_allowed_ids", lambda **_k: {"vendor/allowed"}
        )
...
class TestClaraPrefetch:
    """The clara disk-cache entry is write-only, so prefetching it is a round
    trip for nothing."""

    def test_clara_is_not_collected_for_prefetch(self, monkeypatch):
        import clara_cli.auth as auth_mod
        import clara_cli.model_switch as ms
        ...
        slugs = ms._collect_authed_provider_slugs({}, {"clara": list(CURATED)}, [])
        assert "clara" not in slugs


class TestPolicyNoticeIsShown:

    def test_login_prints_it(self, monkeypatch, tmp_path, policy, capsys):
        import clara_cli.clara_account as account_mod

        monkeypatch.setattr(account_mod, "clara_policy_present", lambda: True)
        TestLoginClara()._run(monkeypatch, tmp_path)
    ...
    def test_login_silent_for_an_ungoverned_org(
        ...
        monkeypatch.setattr(account_mod, "clara_policy_present", lambda: False)
        TestLoginClara()._run(monkeypatch, tmp_path)
...
class TestAuxFallbackRespectsPolicy:
    ...
    def test_blocked_recommendation_is_not_used(self, monkeypatch):
        ...
        assert aux._get_aux_model_for_provider("clara", prefer_fast=True) == ""
    ...
    def test_allowed_recommendation_still_used(self, monkeypatch):
        ...
        assert (
            aux._get_aux_model_for_provider("clara", prefer_fast=True)
            == "vendor/ok-haiku"
        )
...
def test_titling_seeds_the_shared_catalog_entry_like_the_pickers(monkeypatch):
    ...
    without the Clara-only arguments costs the picker its sale chrome and leaves
    ...
    monkeypatch.setattr(models_mod, "fetch_models_with_pricing", _fake_fetch)
    aux._fast_model_from_catalog("clara")

    assert seen.get("include_sale_original") is True
    assert seen.get("cache_ttl_seconds") == models_mod._CLARA_CATALOG_TTL_SECONDS
