"""Tests for the clara_cli models module."""
...
from clara_cli.models import (
    ...
    is_clara_free_tier, partition_clara_models_by_tier,
    check_clara_free_tier, _FREE_TIER_CACHE_TTL,
    union_with_portal_free_recommendations,
    union_with_portal_paid_recommendations,
)
import clara_cli.models as _models_mod
...
class TestModelIds:
    def test_returns_non_empty_list(self):
        with patch("clara_cli.models.fetch_openrouter_models", return_value=LIVE_OPENROUTER_MODELS):
            ids = model_ids()
...
class TestFetchOpenRouterModels:
    ...
    def test_falls_back_to_static_snapshot_on_fetch_failure(self, monkeypatch):
        ...
        with patch("clara_cli.model_catalog.get_curated_openrouter_models", return_value=None), \
             patch("clara_cli.models._urlopen_model_catalog_request", side_effect=OSError("boom")):
    ...
    def test_filters_out_models_without_tool_support(self, monkeypatch):
        ...
        clara-agent is tool-calling-first — surfacing a non-tool model leads to
        ...
        monkeypatch.setattr(_models_mod, "_openrouter_catalog_cache", None)
        with (
            patch("clara_cli.model_catalog.get_curated_openrouter_models", return_value=[]),
            patch("clara_cli.models._urlopen_model_catalog_request", return_value=_Resp()),
        ):
            models = fetch_openrouter_models(force_refresh=True)
...
class TestOpenRouterToolSupportHelper:
    ...
    def test_tools_in_supported_parameters(self):
        from clara_cli.models import _openrouter_model_supports_tools
...
class TestIsClaraFreeTier:
    """Tests for is_clara_free_tier — account tier detection."""

    def test_paid_service_access_allowed_true_is_not_free(self):
        assert is_clara_free_tier({"paid_service_access": {"allowed": True}}) is False


    def test_empty_subscription_not_free(self):
        """Empty subscription dict defaults to not-free (don't block users)."""
        assert is_clara_free_tier({"subscription": {}}) is False


    def test_empty_response_not_free(self):
        """Completely empty response defaults to not-free."""
        assert is_clara_free_tier({}) is False


class TestPartitionClaraModelsByTier:
    """Tests for partition_clara_models_by_tier — free vs paid tier model split."""
    ...
    def test_paid_tier_all_selectable(self):
        ...
        sel, unav = partition_clara_models_by_tier(models, pricing, free_tier=False)
        assert sel == models
        assert unav == []
    ...
    def test_all_paid_models(self):
        ...
        sel, unav = partition_clara_models_by_tier(models, pricing, free_tier=True)
        assert sel == []
        assert unav == models
...
class TestUnionWithPortalFreeRecommendations:
    ...
    def test_adds_portal_free_model_missing_from_curated(self):
        ...
        with patch(
            "clara_cli.models.fetch_clara_recommended_models",
...
class TestCheckClaraFreeTierCache:
    """Tests for the TTL cache on check_clara_free_tier()."""
    ...
    @patch("clara_cli.clara_account.get_clara_portal_account_info")
    def test_result_is_cached(self, mock_account):
        """Second call within TTL returns cached result without account lookup."""
        mock_account.return_value = ClaraPortalAccountInfo(
            logged_in=True,
            source="jwt",
            fresh=False,
            paid_service_access=False,
        )
        result1 = check_clara_free_tier()
        result2 = check_clara_free_tier()
    ...
    def test_force_fresh_bypasses_cache(self, mock_account):
        ...
        assert check_clara_free_tier() is False
        assert check_clara_free_tier(force_fresh=True) is False
...
class TestClaraRecommendedModels:
    """Tests for fetch_clara_recommended_models + get_clara_recommended_aux_model."""
    ...
    def setup_method(self):
        _models_mod._clara_recommended_cache.clear()
    ...
    def test_fetch_caches_per_portal_url(self):
        from clara_cli.models import fetch_clara_recommended_models
        mock_cm = self._mock_urlopen(self._SAMPLE_PAYLOAD)
        with patch("clara_cli.models._urlopen_model_catalog_request", return_value=mock_cm) as mock_urlopen:
            a = fetch_clara_recommended_models("https://portal.example.com")
            b = fetch_clara_recommended_models("https://portal.example.com")
    ...
    def test_paid_tier_prefers_paid_recommendation(self):
        ...
        with patch("clara_cli.models.fetch_clara_recommended_models", return_value=payload):
            text = get_clara_recommended_aux_model(vision=False, free_tier=False)
            vision = get_clara_recommended_aux_model(vision=True, free_tier=False)
    ...
    def test_tier_detection_error_defaults_to_paid(self):
        ...
        with (
            patch("clara_cli.models.fetch_clara_recommended_models", return_value=payload),
            patch("clara_cli.models.check_clara_free_tier", side_effect=RuntimeError("boom")),
        ):
            assert get_clara_recommended_aux_model(vision=False) == "paid-model"
...
class TestCodexSoftAcceptPlausibilityGate:
    ...
    def test_unrelated_name_rejected_on_openai_codex(self):
        from clara_cli.models import validate_requested_model
...
class TestFormatPricePerMtok:
    ...
    def test_standard_prices_keep_two_decimals(self):
        from clara_cli.models import _format_price_per_mtok
    ...
    def test_clara_list_includes_sonnet_5(self):
        from clara_cli.models import _PROVIDER_MODELS
        assert "anthropic/claude-sonnet-5" in _PROVIDER_MODELS["clara"]
...
class TestLocalOllamaModelDiscovery:
    ...
    def test_native_tags_cache_expires(self, monkeypatch):
        from clara_cli.models import fetch_ollama_local_models
        ...
        try:
            ...
            monkeypatch.setattr("clara_cli.models.time.monotonic", lambda: 301.0)
    ...
    def test_runtime_error_from_config_load_does_not_escape_ollama_helpers(self):
        ...
        from clara_cli.models import _get_ollama_base_url, should_use_ollama_native_catalog

        with patch("clara_cli.config.load_config", side_effect=RuntimeError("bad home")), patch(
            "clara_cli.models.probe_ollama_local_models",
            return_value=None,
        ):
    ...
    def test_probe_ollama_models_malformed_base_url_returns_none(self):
        ...
        from clara_cli.models import probe_ollama_local_models
    ...
    def test_fetch_ollama_models_preserves_probe_failure(self):
        ...
        with patch("clara_cli.models.probe_ollama_local_models", return_value=None):
    ...
    def test_ollama_port_detection_requires_working_api_tags(self):
        ...
        with patch("clara_cli.models.probe_ollama_local_models", return_value=["qwen3:1.7b"]):
    ...
    def test_provider_model_ids_ollama_cloud_config_uses_generic_catalog(self):
        ...
        ), patch("clara_cli.models.fetch_ollama_local_models") as fetch_local, patch(
            "clara_cli.models.fetch_api_models",
    ...
    def test_native_ollama_catalog_uses_configured_key_env(self, monkeypatch):
        from clara_cli.models import _get_ollama_request_headers
    ...
    def test_provider_model_ids_ignores_active_non_ollama_custom_endpoint(self):
        ...
        with patch(
            ...
            "clara_cli.models.fetch_ollama_local_models",
            return_value=["qwen3:1.7b"],
        ) as fetch_local:
            assert provider_model_ids("ollama", force_refresh=True) == ["qwen3:1.7b"]
    ...
    def test_ollama_cache_fingerprint_does_not_probe_custom_endpoint(self):
        from clara_cli.models import _credential_fingerprint
        ...
        ), patch("clara_cli.models.probe_ollama_local_models") as probe_ollama:
    ...
    def test_clear_provider_models_cache_clears_ollama_native_tags_cache(self):
        import clara_cli.models as models
    ...
    def test_picker_user_provider_row_discovers_ollama_api_tags(self):
        ...
        from clara_cli.model_switch import list_authenticated_providers
    ...
    def test_picker_user_provider_verifies_ambiguous_ollama_port(self):
        ...
        with patch("clara_cli.config.load_config", return_value={"providers": {}}), patch(
            "clara_cli.models.should_use_ollama_native_catalog",
            return_value=True,
        ), patch(
            "clara_cli.models.fetch_ollama_local_models",
            return_value=["qwen3:1.7b"],
        ), patch("clara_cli.models.fetch_api_models", return_value=[]) as fetch_api:
            rows = list_authenticated_providers(
    ...
    def test_named_custom_model_flow_discovers_ollama_api_tags(self):
        ...
        from clara_cli.main import _model_flow_named_custom
        ...
        try:
            with patch("clara_cli.config.load_config", return_value=config), patch(
                "clara_cli.config.save_config"
            ), patch("clara_cli.auth._save_model_choice"), patch(
                "clara_cli.auth.deactivate_provider"
            ), patch("clara_cli.main._save_custom_provider"), patch(
                "clara_cli.curses_ui.curses_radiolist",
    ...
    def test_model_validation_ollama_cloud_config_does_not_use_local_tags(self):
        ...
        ), patch("clara_cli.models.probe_ollama_local_models") as probe_ollama, patch(
            "clara_cli.models.probe_api_models",
            return_value={
                "models": ["qwen3:1.7b"],
    ...
    def test_switch_model_on_current_ollama_custom_endpoint_keeps_base_url(self):
        ...
        from clara_cli.model_switch import switch_model
        ...
        try:
            with patch(
                ...
                "clara_cli.model_switch.get_model_info",
                return_value=None,
            ):
                result = switch_model(
    ...
    def test_switch_model_on_non_ollama_custom_endpoint_still_resolves_runtime(self):
        ...
        with patch(
            ...
            "clara_cli.models.validate_requested_model",
            return_value={
                "accepted": True,
                "persist": True,
                "recognized": True,
    ...
    def test_ollama_root_matching_is_case_insensitive_for_hostnames(self):
        from clara_cli.models import _same_ollama_native_root
    ...
    def test_ollama_host_environment_forms_are_normalized(self, monkeypatch):
        from clara_cli.models import _get_ollama_base_url, _root_for_ollama_native_api
    ...
    def test_ollama_failed_probe_is_cached_briefly(self):
        ...
        with patch(
            "clara_cli.models._urlopen_model_catalog_request",
    ...
    def test_switch_model_direct_ollama_alias_preserves_matching_origin_api_key(self):
        import clara_cli.model_switch as model_switch
        ...
        try:
            with patch.object(model_switch, "get_model_info", return_value=None), patch(
                "clara_cli.models._get_provider_config_dict",
