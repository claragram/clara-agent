...
These guard the contract:
  ...
  * OpenRouter and Clara Portal are NEVER merged — they keep their curated
    (OpenRouter) or live-Portal (Clara) semantics.
...
appear in ``/model`` without a Clara release.
"""
...
class TestProviderModelIdsPreferred:
    ...
    def test_k3_live_discovery_is_scoped_to_kimi_coding_endpoint(self):
        ...
        with patch("clara_cli.urllib_security.open_credentialed_url", side_effect=fake_open):
            with patch(
    ...
    def test_kimi_setup_flow_uses_same_coding_plan_catalog(self):
        ...
        from clara_cli.model_setup_flows import _model_flow_kimi
        ...
        with (
            patch("clara_cli.main._prompt_api_key", return_value=("sk-kimi-test", False)),
            patch("clara_cli.auth._prompt_model_selection", side_effect=fake_select),
            patch("clara_cli.config.get_env_value", return_value=""),
...
class TestOpenRouterAndClaraUnchanged:
    """Per Teknium: openrouter and clara are NEVER merged with models.dev."""


    def test_openrouter_does_not_call_merge(self):
        """openrouter takes its own live path — merge helper must NOT run."""
        with patch(
            "clara_cli.models._merge_with_models_dev",
