"""Tests for `clara memory setup [provider]` routing.
...
``clara memory setup honcho``) without the interactive picker — which matters
because the per-provider ``clara <provider>`` subcommand is only registered
...
class TestMemorySetupProviderRouting:
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "clara memory setup" in out
...
class TestInstallDependenciesRunner:
    ...
    def _run_with_missing_dep(self, tmp_path, which_side_effect, run_behavior=None):
        ...
        # The hermetic conftest sets CLARA_DISABLE_LAZY_INSTALLS=1 so no test
        ...
        with _patch.dict(os.environ, {"CLARA_DISABLE_LAZY_INSTALLS": "0"}), \
             patch("plugins.memory.find_provider_dir", return_value=tmp_path), \
             patch("clara_cli.tools_config.shutil.which", side_effect=which_side_effect), \
             patch("clara_cli.tools_config.subprocess.run", fake_run):
