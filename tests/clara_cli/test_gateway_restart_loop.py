...
Covers:
- Defense 1: gateway stop/restart refuse when _CLARA_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "clara gateway stop",
        "clara gateway uninstall",
        "clara  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "CLARA GATEWAY RESTART",           # uppercase
    ])
    def test_clara_gateway_commands(self, text):
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.clara.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.clara/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l clara-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.clara.gateway.restart-once.plist",
        "launchctl bootout gui/501/ai.clara.gateway",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.clara.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.clara/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_label_defined_before_verb_is_caught(self):
        ...
        cmd = (
            "uid=$(id -u); for item in 'ai.clara.gateway-apollo:/a.plist' "
            "'ai.clara.gateway-cronus:/c.plist' 'ai.clara.gateway-plutus:/p.plist' "
            "'ai.clara.gateway:/Users/botuser/Library/LaunchAgents/ai.clara.gateway.plist'; "
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated clara note on the next line"
        )
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `launchctl kick"start" ... ai.clara.gateway` executes exactly as
        ...
        'launchctl kick"start" -k gui/501/ai.clara.gateway',
        "launchctl kick'start' -k gui/501/ai.clara.gateway",
        "launchctl kick\\start -k gui/501/ai.clara.gateway",
        'launchctl "kickstart" -k gui/501/ai.clara.gateway',
        # Splices on the newer/legacy unload spellings this PR added.
        'launchctl boot"out" gui/501/ai.clara.gateway',
        "launchctl dis\\able gui/501/ai.clara.gateway",
        # The gateway identifier itself can be spliced just as easily.
        'launchctl bootout gui/501/ai.clara."gateway"',
        # Same class on the systemctl and clara-CLI branches.
        'systemctl re"start" clara-gateway',
        'clara gateway re"start"',
    ])
    ...
    def test_spliced_verb_inside_shell_c_payload_is_blocked(self):
        ...
        command = 'sh -c \'launchctl kick"start" -k gui/501/ai.clara.gateway\''
    ...
    @pytest.mark.parametrize("text", [
        ...
        'launchctl kick"start" -k gui/501/ai.clara.update-checker',
        'systemctl re"start" clara-meta.service',
    ...
    @pytest.mark.parametrize("text", [
        ...
        "clara cron list",
        "clara update",
        "clara config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `clara gateway start` is benign — starting a gateway from inside a
        ...
        "clara gateway start",
        "clara gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway clara
        # services must not be falsely blocked (the old `.*clara` matched any
        # clara token).
        "launchctl unload ai.clara.update-checker.plist",
        "launchctl restart ai.clara.daemon",
        ...
        "systemctl restart clara-meta.service",
        "systemctl restart clara-cron-helper",
        ...
        "echo after the clara gateway restarted cleanly",
        "the clara gateway stopped responding, please investigate",
        # #92372 Branch D: `p?kill` without a leading \b matched the "kill"
        # tail of "skill".
        "clara skill view gateway-notes && echo clara gateway docs",
        # #77173/#77536: a file path with embedded spaces containing the
        # lifecycle words must not match — `clara` is a path component
        # there, not a command.
        "cat '/docs/clara gateway restart-notes.md'",
        "less /home/user/notes/clara gateway restart runbook.txt",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "clara gateway restart",
        "clara gateway restart; echo done",
        "clara gateway stop && echo stopped",
        # #77173 command-position anchor must not weaken separator/subshell
        # forms either.
        "true;clara gateway restart",
        "true && clara gateway stop",
        "echo $(clara gateway restart)",
        "echo `clara gateway restart`",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'import subprocess\nsubprocess.run(["launchctl", "bootout", "gui/501/ai.clara.gateway"])',
        'subprocess.run(["clara", "gateway", "restart"])',
        'os.system("launchctl kickstart -k gui/501/ai.clara.gateway")',
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'data = ["clara", "notes"]  # unrelated list',
    ])
    ...
    def test_inert_heredoc_body_prose_not_blocked(self):
        ...
        text = (
            ...
            "If the box is wedged, a human can run: clara gateway restart\n"
            "EOF"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
...
class TestProfileFlagGatewayLifecycle:
    """#78028: `clara -p <profile> gateway restart|stop` bypasses Branch A's
    ...
    def _pin_profile_identity(self, monkeypatch):
        # The ambient test env may carry CLARA_HOME/CLARA_PROFILE; pin the
        # profile identity explicitly so every assertion is deterministic.
        monkeypatch.setenv("CLARA_PROFILE", "zeus")
        monkeypatch.delenv("CLARA_PROFILE_NAME", raising=False)

    @pytest.mark.parametrize("text", [
        "clara -p zeus gateway stop",
        "clara -p zeus gateway restart",
        "clara --profile zeus gateway restart",
        "clara --profile zeus gateway stop",
        "clara --profile=zeus gateway restart",
        # Global flags before/after the selector must not hide the shape.
        "clara -v -p zeus gateway restart",
        "clara -p zeus -v gateway restart",
        "clara --debug --profile zeus gateway stop",
    ...
    @pytest.mark.parametrize("text", [
        "clara -p venus gateway stop",
        "clara -p venus gateway restart",
        "clara --profile venus gateway restart",
        "clara --profile=venus gateway stop",
        "clara -p venus -v gateway restart",
    ])
    def test_sibling_allowed(self, text):
        assert not _contains_gateway_lifecycle_command(text), f"Should allow: {text!r}"

    @pytest.mark.parametrize("text", [
        "clara -p zeus gateway start",
        "clara -p zeus gateway start --all",
    ])
    ...
    def test_adjacent_form_still_blocked(self):
        ...
        assert _contains_gateway_lifecycle_command("clara gateway restart")
        assert _contains_gateway_lifecycle_command("clara gateway stop")

    def test_clara_home_derived_profile(self, monkeypatch):
        # Without CLARA_PROFILE the guard falls back to the CLARA_HOME-
        # derived profile identity (get_active_profile_name) — the signal the
        # gateway process itself carries.
        monkeypatch.delenv("CLARA_PROFILE", raising=False)
        monkeypatch.delenv("CLARA_PROFILE_NAME", raising=False)
        import clara_cli.profiles as profiles_mod

        monkeypatch.setattr(profiles_mod, "get_active_profile_name", lambda: "zeus")
        assert _contains_gateway_lifecycle_command("clara -p zeus gateway restart")
        assert not _contains_gateway_lifecycle_command("clara -p venus gateway restart")
    ...
    def test_no_profile_context_conservative_allow(self, monkeypatch):
        ...
        assert not _contains_gateway_lifecycle_command("clara -p zeus gateway restart")
        assert _contains_gateway_lifecycle_command("clara gateway restart")
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_clara_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade clara then run clara gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_clara_gateway_once.sh). The script must live under
        # CLARA_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("CLARA_HOME", str(tmp_path / ".clara"))
        scripts_dir = tmp_path / ".clara" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_CLARA_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _CLARA_GATEWAY=1.

    Issue #37453: systemctl --user restart clara-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart clara-gateway",
        "systemctl --user restart clara-gateway",
        "systemctl stop clara-gateway.service",
        "clara gateway restart",
        "clara gateway uninstall",
        "launchctl kickstart gui/501/ai.clara.gateway",
        "launchctl bootout gui/501/ai.clara.gateway",
        # #62891 exact reported shape and its bootstrap sibling.
        "launchctl submit -l ai.clara.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.clara/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.clara.gateway.restart-once.plist",
        "pkill -f clara.*gateway",
    ])
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart clara-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.clara.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-clara label: label-independent detection is the point
        # (#62891 second reproduction used `ai.clara.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.clara.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_cli_agent_session_not_blocked_by_inherited_env(
        ...
        """#92560: CLI/TUI agent sessions inherit _CLARA_GATEWAY=1 from the
        ...
        # Simulate a CLI agent session: _CLARA_GATEWAY=1 is in the
        ...
        self._patch_env(monkeypatch, _FakeEnv(), inside_gateway=False)
        monkeypatch.setenv("_CLARA_GATEWAY", "1")
        monkeypatch.setattr(
            tt, "_check_all_guards", lambda cmd, env, **kwargs: {"approved": True}
        )

        result = json.loads(tt.terminal_tool(command="clara gateway restart"))

        assert result["exit_code"] == 0
        assert calls == ["clara gateway restart"]
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.clara.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.clara.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-clara systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run clara gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("clara gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfeclara gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("", str(script))


    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under CLARA_HOME/scripts (the
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("clara gateway restart")\n', encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/clara\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 clara gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_clara_home()` falls back to Path.home(), which raises
        RuntimeError when neither CLARA_HOME nor HOME resolves
...
class TestRelativePathDoesNotDisableDataExemption:
    ...
    @pytest.mark.parametrize("command", [
        "grep -r 'systemctl restart clara-gateway' .",
        "grep -rn 'clara gateway restart' ./logs",
        "rg 'clara gateway restart' ../archive",
        "grep -c 'systemctl stop clara-gateway' ./var/log/syslog",
        "sqlite3 ./stats.db \"SELECT restart_reason FROM clara_gateway_restarts\"",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 ./db ".shell clara gateway restart"',
        'sqlite3 ./db ".system systemctl restart clara-gateway"',
        'psql ./x -c "\\! systemctl restart clara-gateway"',
        "grep -r 'clara gateway restart' . | sh",
        "grep -r 'clara gateway restart' ./logs | bash",
        "grep -r 'clara gateway restart' . | sudo sh",
        "grep -r 'x' . ; clara gateway restart",
        "grep -r 'x' . && systemctl restart clara-gateway",
        'grep -r "$(clara gateway restart)" .',
        "rg 'x' ./logs | xargs systemctl restart clara-gateway",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell clara gateway restart"',
        'sqlite3 db ".system systemctl restart clara-gateway"',
        'psql -c "\\! systemctl restart clara-gateway"',
    ])
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    clara_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run clara gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CLARA_HOME", str(tmp_path / ".clara"))
        (tmp_path / ".clara").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run clara gateway restart nightly",
        ))
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "'systemctl stop clara-gateway'\"",
        # grep/rg pattern arguments hunting for the lifecycle string.
        "grep -c 'systemctl restart clara-gateway' /var/log/syslog",
        "rg 'clara gateway restart' /home/user/.clara/logs/",
        "journalctl -u clara-gateway --grep 'systemctl restart clara-gateway'",
        # SQL with stop/restart column/value words but no command shape.
        'sqlite3 stats.db "SELECT stop_time, restart_reason FROM '
        'clara_gateway_restarts"',
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell clara gateway restart"',
        'psql -c "\\! systemctl restart clara-gateway"',
        "grep 'systemctl restart clara-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart clara-gateway",
        'grep "$(systemctl restart clara-gateway)" f',
        "grep 'restart' log; systemctl restart clara-gateway",
        'sqlite3 db "SELECT 1"; clara gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "clara gateway restart",
        "sudo systemctl stop clara-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart clara-gateway%'\""
        )
