...
def test_resolve_rejects_directory_without_manifest(tmp_path: Path) -> None:
    ...
    from clara_cli.plugin_dev import resolve_plugin_path
...
def test_doctor_default_target_does_not_copy_cwd(
    ...
    """``clara plugins doctor`` with no argument defaults to ``.``.

    Before the manifest guard, that copied the whole working directory into
    a temporary CLARA_HOME — running it from ``$HOME`` copied the home
    directory, cloud-storage placeholders included.
    """
    import os
    import shutil

    from clara_cli import plugin_dev
...
def test_resolve_prefers_installed_id_over_unrelated_local_dir(
    ...
    installed = clara_home / "plugins" / "sample"
    ...
    monkeypatch.chdir(workdir)
    monkeypatch.setattr(plugin_dev, "get_clara_home", lambda: clara_home)
...
def test_doctor_removes_temp_home_when_staging_copy_fails(
    ...
    """A copy failure (e.g. ENOSPC) must not strand a clara-plugin-doctor-* dir."""
    import errno
    import shutil
    import tempfile
    ...
    # exact window where a stranded clara-plugin-doctor-* dir was observed.
    leftovers = list(scratch.glob("clara-plugin-doctor-*"))
