...
- Bare `clara setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `clara setup --quick` runs the narrower "fill in missing items" flow.
- `clara setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("clara_cli.setup.ensure_clara_home", {}),
        ("clara_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("clara_cli.config.is_managed", {"return_value": False}),
        ("clara_cli.setup.load_config", {"return_value": {}}),
        ("clara_cli.setup.save_config", {}),
        ("clara_cli.setup.get_env_value", {"return_value": None}),
        ("clara_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("clara_cli.setup._print_setup_summary", {}),
        ("clara_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("clara_cli.setup.load_config", {"return_value": {}}),
        ("clara_cli.setup.save_config", {}),
        ("clara_cli.auth.get_active_provider", {"return_value": None}),
        ("clara_cli.setup.get_env_value", {"return_value": None}),
        ("clara_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `clara setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="clara_cli.setup.prompt_choice",
                quick="clara_cli.setup._run_quick_setup",
                model="clara_cli.setup.setup_model_provider",
                terminal="clara_cli.setup.setup_terminal_backend",
                agent="clara_cli.setup.setup_agent_settings",
                gateway="clara_cli.setup.setup_gateway",
                tools="clara_cli.setup.setup_tools",
            )
...
class TestQuickFlag:
    ...
    def test_quick_flag_runs_quick_setup_only(self, existing_install):
        ...
        with ExitStack() as stack:
            ...
            from clara_cli.setup import run_setup_wizard
            from clara_cli import setup as setup_mod
...
class TestFreshInstall:
    ...
    def test_reconfigure_on_fresh_install_falls_through(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("clara_cli.setup.prompt_choice", {"return_value": 0}),
                first="clara_cli.setup._run_first_time_quick_setup",
            )
            from clara_cli.setup import run_setup_wizard
            from clara_cli import setup as setup_mod
    ...
    def test_blank_slate_runs_inside_navigation_step(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("clara_cli.setup.prompt_choice", {"return_value": 2}),
                blank="clara_cli.setup._run_blank_slate_setup",
            )
            from clara_cli import setup as setup_mod
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "clara_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["clara", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
