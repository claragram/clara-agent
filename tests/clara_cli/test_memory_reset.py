"""Tests for the `clara memory reset` CLI command.
...
- Profile-scoped reset (uses CLARA_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake CLARA_HOME with memory files."""
    clara_home = tmp_path / ".clara"
    memories = clara_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("CLARA_HOME", str(clara_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nClara repo is at ~/.clara/clara-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `clara memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        clara_home = tmp_path / ".clara"
        (clara_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("CLARA_HOME", str(clara_home))
