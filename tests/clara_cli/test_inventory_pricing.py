...
columns + Free/Pro badges and gate paid models on free Clara accounts, the
same way the `clara model` CLI picker does.
"""
...
def _patch_pricing(monkeypatch, *, free_tier, pricing, unavailable=None):
    ...
    monkeypatch.setattr(models_mod, "check_clara_free_tier", lambda *, force_fresh=False: free_tier)
    monkeypatch.setattr(
...
def test_apply_pricing_free_models_get_flat_100_percent_sale(monkeypatch):
    ...
    rows = [{"slug": "clara", "models": ["a/free", "b/natively-free"]}]
    inv._apply_pricing(rows)
