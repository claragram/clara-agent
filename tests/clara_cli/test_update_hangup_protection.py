"""Tests for SIGHUP protection and stdout mirroring in ``clara update``.

Covers ``_UpdateOutputStream``, ``_install_hangup_protection``, and
``_finalize_update_output`` in ``clara_cli/main.py``.  These exist so
that ``clara update`` survives a terminal disconnect mid-install
...
def test_update_completion_includes_bounded_action_identity(monkeypatch, capsys):
    monkeypatch.setenv("CLARA_ACTION_ID", "a" * 32)
    ...
    monkeypatch.setattr("clara_cli.update_cmd._branch_head_suffix", lambda: "")

    _print_update_completion("✓ Update complete!")

    assert capsys.readouterr().out.splitlines() == [
        "✓ Update complete!",
        f"=== clara-update completed {'a' * 32} ===",
    ]


def test_update_completion_rejects_untrusted_action_identity(monkeypatch, capsys):
    monkeypatch.setenv("CLARA_ACTION_ID", "not-safe\nforged")
    monkeypatch.setattr("clara_cli.update_cmd._branch_head_suffix", lambda: "")
...
class TestInstallHangupProtection:
    ...
    def test_wraps_stdout_and_stderr_with_mirror(self, tmp_path, monkeypatch):
        ...
        if hasattr(_cfg, "_CLARA_HOME_CACHE"):
            _cfg._CLARA_HOME_CACHE = None  # type: ignore[attr-defined]
        ...
        try:
            ...
            assert "clara update started" in contents
        finally:
    ...
    def test_non_fatal_if_log_setup_fails(self, monkeypatch):
        """If get_clara_home() raises, stdio must be left untouched but SIGHUP still handled."""
        prev_out, prev_err = sys.stdout, sys.stderr
        ...
        monkeypatch.setattr(
            "clara_cli.config.get_clara_home", _boom, raising=True
        )
