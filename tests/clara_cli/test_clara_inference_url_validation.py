"""Regression tests for Clara Portal inference_base_url host-allowlist validation.
...
client. ``_validate_clara_inference_url_from_network()`` blocks any URL
...
4. The env-var override path (``CLARA_INFERENCE_BASE_URL``) is NOT
...
from clara_cli.auth import (
    DEFAULT_CLARA_INFERENCE_URL,
    _ALLOWED_CLARA_INFERENCE_HOSTS,
...
class TestValidatorRules:
    ...
    def test_attacker_host_rejected(self, caplog):
        with caplog.at_level(logging.WARNING, logger="clara_cli.auth"):
            assert (
                _validate_clara_inference_url_from_network("https://attacker.com/v1")
                is None
            )
    ...
    def test_default_inference_url_is_in_allowlist(self):
        """Sanity check: DEFAULT_CLARA_INFERENCE_URL must itself validate.

        If anyone retargets the default away from
        ``inference-api.claraprise.com``, they MUST update the allowlist
        in the same change — otherwise the allowlist would reject the
        Portal's own legitimate default and break every install.
        """
        assert (
            _validate_clara_inference_url_from_network(DEFAULT_CLARA_INFERENCE_URL)
            == DEFAULT_CLARA_INFERENCE_URL.rstrip("/")
        )
...
class TestCallSiteWiring:
    ...
    ``_validate_clara_inference_url_from_network`` with the un-validated
    ``_optional_base_url`` again, the test catches it.

    Each site lives inside ``resolve_clara_runtime_credentials`` and one
    ...
    def test_no_unvalidated_inference_base_url_assignments_remain(self):
        ...
        for needle in (
            ...
            assert needle not in source, (
                ...
                f"Use _validate_clara_inference_url_from_network() instead."
            )
    ...
    def test_validator_wired_at_all_known_call_sites(self):
        ...
        refresh_count = source.count(
            '_validate_clara_inference_url_from_network(refreshed.get("inference_base_url"))'
        )
        mint_count = source.count(
            '_validate_clara_inference_url_from_network(mint_payload.get("inference_base_url"))'
        )
    ...
    def test_proxy_adapter_also_validates(self):
        """The Clara proxy adapter applies the validator as defense-in-depth
        ...
        boundary."""
        from pathlib import Path
        import clara_cli.proxy.adapters.clara_portal as _clara_adapter
        source = Path(_clara_adapter.__file__).read_text(encoding="utf-8")
        assert "_validate_clara_inference_url_from_network" in source


class TestEnvOverrideNotGated:
    """The documented dev/staging env-var override must keep working.

    ``CLARA_INFERENCE_BASE_URL`` is read by ``resolve_clara_runtime_credentials``
    ...
    def test_env_override_path_does_not_call_validator(self):
        """In resolve_clara_runtime_credentials, the env override is
        ...
        for line in source.splitlines():
            if "CLARA_INFERENCE_BASE_URL" in line and "os.getenv" in line:
                assert "_validate_clara_inference_url_from_network" not in line, (
...
class TestHealsPoisonedStoredValue:
    ...
    stale ``stg-inference-api.claraprise.com`` persisted before the
    ...
    staging endpoint (real incident: opus-4.8 routed to clara, clara pinned
    ...
    def test_refresh_resets_rejected_url_to_default(self, monkeypatch):
        ...
        poisoned = "https://stg-inference-api.claraprise.com/v1"
        state = {
            "access_token": "tok",
            "refresh_token": "rtok",
            "client_id": "clara-cli",
            "portal_base_url": auth.DEFAULT_CLARA_PORTAL_URL,
        ...
        monkeypatch.setattr(auth, "_clara_invoke_jwt_status", lambda *a, **k: "needs_refresh")
        monkeypatch.setattr(
            auth,
        ...
        monkeypatch.setattr(auth, "_assert_clara_inference_jwt_usable", lambda *a, **k: None)
        monkeypatch.setattr(auth, "_select_clara_invoke_jwt", lambda *a, **k: None)

        result = auth.refresh_clara_oauth_from_state(state, force_refresh=True)

        assert result["inference_base_url"] == auth.DEFAULT_CLARA_INFERENCE_URL, (
...
class TestEnvOverrideWins:
    """``CLARA_INFERENCE_BASE_URL`` must win over the stored value for the
    ...
    STAGING = "https://stg-inference-api.claraprise.com/v1"

    def _patch_no_refresh(self, monkeypatch, auth, state):
        import contextlib

        # No refresh fires: the stored access token is a usable invoke JWT.
        monkeypatch.setattr(auth, "_clara_invoke_jwt_status", lambda *a, **k: None)
        monkeypatch.setattr(
        ...
        monkeypatch.setattr(auth, "_write_shared_clara_state", lambda *a, **k: None)
        monkeypatch.setattr(auth, "_sync_clara_pool_from_auth_store", lambda *a, **k: None)
    ...
    def test_no_refresh_env_override_not_persisted(self, monkeypatch):
        ...
        state = self._base_state(auth, auth.DEFAULT_CLARA_INFERENCE_URL)
        self._patch_no_refresh(monkeypatch, auth, state)
        monkeypatch.setenv("CLARA_INFERENCE_BASE_URL", self.STAGING)

        auth.resolve_clara_runtime_credentials()

        assert state["inference_base_url"] == auth.DEFAULT_CLARA_INFERENCE_URL, (
    ...
    def test_no_refresh_heals_poisoned_stored_without_env(self, monkeypatch):
        ...
        self._patch_no_refresh(monkeypatch, auth, state)
        monkeypatch.delenv("CLARA_INFERENCE_BASE_URL", raising=False)

        result = auth.resolve_clara_runtime_credentials()
        assert result["base_url"] == auth.DEFAULT_CLARA_INFERENCE_URL, (
...
class TestProxyAdapterEnvOverride:
    """The Clara proxy adapter is the second chokepoint: it re-validates the
    base_url returned by resolve_clara_runtime_credentials() against the prod
    allowlist. That re-validation must not clobber a legitimate
    CLARA_INFERENCE_BASE_URL staging override.
    """
    ...
    def test_proxy_adapter_consults_env_override(self):
        ...
        source = Path(_clara_adapter.__file__).read_text(encoding="utf-8")
        assert "_clara_inference_env_override()" in source, (
