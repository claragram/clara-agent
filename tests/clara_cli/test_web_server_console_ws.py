"""Dashboard Clara Console websocket tests."""
...
@pytest.fixture
def console_client(monkeypatch, _isolate_clara_home):
...
def test_console_ws_cancel_returns_to_prompt(console_client, monkeypatch):
    from clara_cli.console_engine import ConsoleResult, ClaraConsoleEngine

    def slow_execute(self, line: str, *, confirmed: bool = False):
        time.sleep(0.2)
        return ConsoleResult("ok", output="late", command=line)

    monkeypatch.setattr(ClaraConsoleEngine, "execute", slow_execute)
