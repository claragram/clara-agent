...
Covers:
 - Normal path (tokens already in Clara auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Clara
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `clara auth openai-codex`. The old
...
@pytest.fixture()
def clara_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Clara auth store (no Codex CLI needed)."""
    ...
    for var in [
        ...
        "CLARA_API_KEY", "DEEPSEEK_API_KEY",
    ]:
...
def test_normal_path_still_works(clara_auth_only_env):
    """openai-codex appears when tokens are already in Clara auth store."""
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Clara
    auth store."""
    clara_home = tmp_path / ".clara"
    clara_home.mkdir()
...
def test_no_codex_when_no_credentials(tmp_path, monkeypatch):
    ...
    for var in [
        "OPENROUTER_API_KEY", "OPENAI_API_KEY", "ANTHROPIC_API_KEY",
        "CLARA_API_KEY", "DEEPSEEK_API_KEY", "COPILOT_GITHUB_TOKEN",
