"""Behavior tests for clara_cli.inventory.
...
def _clara_row(model: str = "openai/gpt-5.5") -> dict:
    return {
        "slug": "clara",
        "name": "Clara",
        "models": [model],
...
def test_list_authenticated_providers_force_fresh_is_keyword_only():
    """``force_fresh_clara_tier`` must be keyword-only on the public listing API.
    ...
    param = sig.parameters["force_fresh_clara_tier"]
...
def test_explicit_only_filters_ambient_credentials_but_keeps_current_and_custom_rows():
    rows = [
        ...
        {"slug": "copilot", "name": "Copilot", "models": ["gpt-5.4"],
         "total_models": 1, "is_current": False, "is_user_defined": False,
         "source": "clara"},
        {"slug": "clara", "name": "Clara", "models": ["anthropic/claude-sonnet-5"],
         "total_models": 1, "is_current": False, "is_user_defined": False,
         "source": "clara"},
...
def test_explicit_only_keeps_anthropic_row_with_oauth_credentials():
    ...
    Claude Code (~/.claude/.credentials.json) and Clara' own device flow
    ...
    with (
        ...
        patch(
            "clara_cli.inventory._anthropic_oauth_credentials_present",
            return_value=True,
        ),
    ):
        payload = build_models_payload(ctx, explicit_only=True)
...
def test_anthropic_oauth_presence_accepts_pool_only_oauth_entry():
    ...
    """
    from clara_cli.inventory import _anthropic_oauth_credentials_present

    with (
        patch(
            "agent.anthropic_adapter.read_clara_oauth_credentials",
            return_value=None,
        ),
        patch(
        ...
        patch(
            "clara_cli.auth.read_credential_pool",
            return_value=[
...
def test_user_defined_rows_carry_alias_set_for_gui_current_match():
    ...
    assert "aliases" not in by_slug["clara"]
...
def test_build_models_payload_forwards_refresh_flag():
    ...
    with patch("clara_cli.model_switch.list_authenticated_providers", side_effect=_capture):
...
def test_list_authenticated_providers_refresh_busts_cache():
    ...
    from clara_cli import model_switch

    with patch("clara_cli.models.clear_provider_models_cache") as clear:
