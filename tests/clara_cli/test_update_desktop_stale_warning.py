...
#88251: ``clara update`` treated a failed desktop pack as non-fatal, printed
...
def desktop_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "clara_constants.with_clara_node_path", lambda: {}, raising=False
    )
    monkeypatch.setattr(
        "clara_constants.display_clara_home", lambda: str(tmp_path), raising=False
    )
...
def test_gateway_exit_code_file_tracks_desktop_rebuild(tmp_path, monkeypatch):
    monkeypatch.setattr(update_cmd, "get_clara_home", lambda: tmp_path)
