...
2. Spawn: _default_spawn sets the CLARA_KANBAN_GOAL_MODE env vars only
...
# ---------------------------------------------------------------------------
# CLI judge gate tests (clara kanban complete bypass fix)
# ---------------------------------------------------------------------------

class TestCLIJudgeGate:
    """clara kanban complete must apply the same goal_mode judge gate as the
    ...
    def _run(self, monkeypatch, *, goal_mode=True, judge_available=True,
        ...
        from clara_cli.kanban import _cmd_complete
        ...
        monkeypatch.setattr("clara_cli.kanban.kb.get_task", lambda conn, tid: fake_task)
        monkeypatch.setattr("clara_cli.kanban.kb.complete_task", fake_complete_task)
        monkeypatch.setattr("clara_cli.kanban.kb.connect_closing", fake_connect_closing)
        monkeypatch.setattr("clara_cli.kanban._worker_run_id_for", lambda _: None)
