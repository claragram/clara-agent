"""Tests for `clara curator run` CLI behavior."""
...
def test_run_defaults_to_synchroclara(monkeypatch, capsys):
    import agent.curator as curator_state
    import clara_cli.curator as curator_cli
    ...
    assert calls[0]["__PROT_1_synchroclara__"] is True
    assert calls[0]["dry_run"] is False
    assert "background" not in capsys.readouterr().out


def test_dry_run_default_reports_synchroclara_wording(monkeypatch, capsys):
    import agent.curator as curator_state
    import clara_cli.curator as curator_cli
    ...
    assert "Read the report with `clara curator status`" in out
