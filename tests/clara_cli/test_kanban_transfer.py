"""Tests for kanban board export / import (``clara_cli.kanban_transfer``).
...
from clara_cli import kanban_transfer as kt
from clara_cli.archive_safe import normalize_archive_parts, safe_extract_targz
...
def kanban_root(tmp_path, monkeypatch):
    ...
    def _use(name: str) -> Path:
        ...
        monkeypatch.setenv("CLARA_KANBAN_HOME", str(root))
        for var in ("CLARA_KANBAN_DB", "CLARA_KANBAN_WORKSPACES_ROOT",
                    "CLARA_KANBAN_ATTACHMENTS_ROOT", "CLARA_KANBAN_BOARD"):
...
def test_import_rejects_a_future_format_version(kanban_root, tmp_path):
    ...
    kanban_root("target")
    with pytest.raises(ValueError, match="newer than this Clara"):
