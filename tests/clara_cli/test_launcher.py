"""Tests for the top-level `./clara` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./clara` should use `clara_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "clara"
    called = []

    fake_main_module = types.ModuleType("clara_cli.main")

    def fake_main():
        called.append("clara_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "clara_cli.main", fake_main_module)
