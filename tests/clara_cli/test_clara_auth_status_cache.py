"""Tests for the get_clara_auth_status() process-level cache.
...
against portal.claraprise.com during one render. The cache is keyed
...
def _seed_auth_file(tmp_path):
    """Drop a placeholder auth.json into the test CLARA_HOME.
...
def test_get_clara_auth_status_caches_consecutive_calls(tmp_path, monkeypatch):
    ...
    auth_mod.invalidate_clara_auth_status_cache()
    ...
    with patch.object(auth_mod, "_compute_clara_auth_status", side_effect=fake_compute):
        first = auth_mod.get_clara_auth_status()
        second = auth_mod.get_clara_auth_status()
        third = auth_mod.get_clara_auth_status()

    assert call_count["n"] == 1, (
        f"_compute_clara_auth_status was called {call_count['n']}× — "
        "cache is not deduplicating within TTL."
    )
    # Each call returns a copy so callers can't mutate the cached dict.
    assert first == second == third
    first["mutated"] = True
    assert "mutated" not in auth_mod.get_clara_auth_status()

    auth_mod.invalidate_clara_auth_status_cache()


def test_get_clara_auth_status_caches_failure_path(tmp_path, monkeypatch):
    """Logged-out snapshots are cached too — that's where the cost was.

    Teknium's case: ~31 cache misses per `clara tools` "All Platforms"
    ...
    with patch.object(auth_mod, "_compute_clara_auth_status", side_effect=fake_compute):
        for _ in range(10):
            auth_mod.get_clara_auth_status()
