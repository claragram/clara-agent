...
from clara_cli.auth import (
    CLARA_BILLING_MANAGE_SCOPE,
    clara_token_has_billing_scope,
    step_up_clara_billing_scope,
)
...
def _stub_persist(monkeypatch):
    ...
    monkeypatch.setattr(auth, "_write_shared_clara_state", lambda *a, **kw: None)
    monkeypatch.setattr(auth, "_sync_clara_pool_from_auth_store", lambda: None)
...
def test_step_up_requests_billing_scope_and_reuses_prior_urls(monkeypatch, _stub_persist):
    ...
    monkeypatch.setattr(auth, "_clara_device_code_login", _fake_login)

    granted = step_up_clara_billing_scope()
    assert granted is True
    # Requested scope must include billing:manage, preserving prior scopes.
    assert CLARA_BILLING_MANAGE_SCOPE in captured["scope"].split()
    ...
    assert captured["client_id"] == "clara-cli"
...
def test_device_login_fires_on_verification_before_polling(monkeypatch):
    ...
    try:
        auth._clara_device_code_login(open_browser=False, on_verification=_cb)
    except Exception:
        pass
