"""``clara`` must survive git operations on the checkout (launcher layout).

The Windows ``clara`` command is a launcher derived from the venv console
script. Its canonical home is the managed binary dir ``CLARA_HOME\\bin`` —
OUTSIDE the git checkout — because the earlier in-checkout home
(``clara-agent\\bin``) was swept by ``clara update``'s autostash
(``git stash push --include-untracked``) and, with the desktop updater's
``--keep-stash``, never restored: ``clara`` stopped resolving in every new
...
existing install's PATH to the canonical layout from the ``clara update``
tail. Platform verdict, PATH values, and registry I/O are injected
parameters (same pattern as ``clara_constants.venv_bin_dir``), so these
...
from clara_cli._install_repair import (
...
def _make_managed(tmp_path, monkeypatch, *, relocatable: bool = False):
    """Fake managed layout: CLARA_HOME/clara-agent/venv/Scripts + launchers."""
    home = tmp_path / "clara"
    root = home / "clara-agent"
...
def test_source_checkout_untouched(tmp_path, monkeypatch):
    """A checkout NOT under CLARA_HOME gains nothing anywhere."""
    home = tmp_path / "clara-home"
    home.mkdir()
    monkeypatch.setenv("CLARA_HOME", str(home))
    root = tmp_path / "src" / "clara-agent"
    scripts = root / "venv" / "Scripts"
    scripts.mkdir(parents=True)
    for name in _WINDOWS_BIN_LAUNCHERS:
...
def test_profile_session_still_heals_the_shared_bin(tmp_path, monkeypatch):
    """Under ``clara -p <name>`` CLARA_HOME points inside profiles/<name>;
...
# ---------------------------------------------------------------------------
# migrate_windows_bin_path — the `clara update` tail migration
# ---------------------------------------------------------------------------
...
def test_migration_moves_path_to_home_bin_and_strips_legacy(managed_install):
    ...
    (root / "bin").mkdir()
    (root / "bin" / "clara.exe").write_bytes(b"legacy copy")
    ...
    assert (root / "bin" / "clara.exe").read_bytes() == b"legacy copy"
...
def test_normalize_windows_path_equivalences():
    assert (
        _normalize_windows_path(r"C:\Users\Me\AppData\Local\clara\bin")
        == _normalize_windows_path("c:/users/me/appdata/local/CLARA/BIN/")
    )
...
def test_repo_gitignores_the_legacy_bin_dir():
    ...
    launchers at ``<checkout>/bin``. ``clara update`` autostashes with
    ...
    result = subprocess.run(
        ["git", "-C", str(repo_root), "check-ignore", "-q", "bin/clara.exe"],
        capture_output=True,
    )
    assert result.returncode == 0, (
        "bin/clara.exe is not gitignored — clara update's autostash "
