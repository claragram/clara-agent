"""Tests for Bug #12905 fix — stale OAuth token detection in clara model flow.

Bug 3: `clara model` with `provider=anthropic` skips OAuth re-authentication
when a stale ANTHROPIC_TOKEN exists in ~/.clara/.env but no valid
...
class TestStaleOAuthTokenDetection:
    ...
    def test_stale_oauth_token_triggers_reauth(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr("clara_cli.secret_prompt.masked_secret_prompt", lambda _: "")

        from clara_cli.main import _model_flow_anthropic
        cfg = {}
