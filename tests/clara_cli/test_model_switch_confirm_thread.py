...
def _patch_deps(monkeypatch, printed):
    ...
    monkeypatch.setattr(
        "clara_cli.model_switch.switch_model", lambda **_: _make_result()
    )
    monkeypatch.setattr(
        "clara_cli.model_switch.resolve_display_context_length", lambda *a, **k: None
    )
    return cli_mod
...
def test_confirm_stays_synchroclara_without_app(monkeypatch):
