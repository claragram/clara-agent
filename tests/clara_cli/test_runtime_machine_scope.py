...
def profile_home(tmp_path, monkeypatch):
    """A NAMED-profile CLARA_HOME under <root>/profiles/<name>."""
    root = tmp_path / ".clara"
    profile = root / "profiles" / "coder"
    profile.mkdir(parents=True)
    monkeypatch.setenv("CLARA_HOME", str(profile))
    # clara_constants memoizes root resolution per (native, env) pair;
...
def test_models_and_runtimes_resolve_to_the_shared_root(profile_home):
    ...
    import clara_cli.local_runtime.binaries as binaries
    import clara_cli.local_runtime.bootstrap as bootstrap
...
def test_all_runtime_state_follows_runtimes_root(profile_home):
    ...
    root, profile = profile_home
    from clara_cli.local_runtime.growth import window_overrides_path
...
def test_default_profile_paths_unchanged(tmp_path, monkeypatch):
    """CLARA_HOME at the root itself (default profile) resolves exactly
