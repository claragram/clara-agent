"""Tests for clara_cli.cron command handling."""
...
from clara_cli import cron as cron_cli
...
class TestUnverifiedDeliveryVisibility:
    ...
    state must be visible in ``clara cron list`` and ``clara cron doctor``,
...
class TestCronListStatusRendering:
    ...
    def test_delivery_failed_is_not_green_ok(self, tmp_cron_dir, capsys, monkeypatch):
        monkeypatch.setattr("clara_cli.gateway.find_gateway_pids", lambda: [1])
        # capsys is not a tty, so force colors on to check the paint itself.
        monkeypatch.setattr("clara_cli.colors.should_use_color", lambda: True)
        create_job(prompt="Daily digest", schedule="every 1h")
        jobs = load_jobs()
...
class TestGatewayNotRunningWarning:
    ...
    def test_list_warns_when_gateway_absent(self, tmp_cron_dir, capsys, monkeypatch):
        ...
        monkeypatch.setattr("clara_cli.gateway.find_gateway_pids", lambda: [])
        cron_command(Namespace(cron_command="list", all=True))
        out = capsys.readouterr().out
...
class TestExternalCronProviderStatus:
    ...
    def test_status_reports_provider_not_ticker_for_chronos(
        ...
        monkeypatch.setattr(
            "clara_cli.cron._active_cron_provider_name", lambda: "chronos"
        )
...
class TestCronRunBackgroundDispatch:
    """`clara cron run` must not report 'failed' when the run was dispatched
    ...
    instead, and leave __PROT_0_synchroclara__ runs unchanged.
    """
    ...
    def test_sync_run_failure_still_reported(self, monkeypatch, capsys):
        # A genuine __PROT_1_synchroclara__ failure must keep reporting 'failed' — only
