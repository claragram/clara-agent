"""Tests for clara_cli.web_server and related config utilities."""
...
@pytest.fixture
def _install_example_plugin(_isolate_clara_home):
    """Drop the example-dashboard fixture into the per-test CLARA_HOME
    ...
    ``CLARA_BUNDLED_PLUGINS`` override because the bundled dir is
    resolved per-call (other tests in the suite implicitly rely on the
    real bundled plugins — kanban, clara-achievements, model providers
    ...
    user_plugins_dir = get_clara_home() / "plugins"
    ...
    # plugin in exactly as a real operator would with `clara plugins
    # enable example`.
    ...
    #      and start serving requests against a torn-down CLARA_HOME.
    app = web_server.app
...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known Clara vars not present in .env."""
        env_file = tmp_path / ".env"
...
class TestSessionTokenInjection:
    """The desktop shell mints CLARA_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        import clara_cli.web_server as ws

        original_app = ws.app
        original_token = ws._SESSION_TOKEN
        monkeypatch.setenv("CLARA_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
    ...
    def test_falls_back_to_random_token(self, monkeypatch):
        ...
        monkeypatch.delenv("CLARA_DASHBOARD_SESSION_TOKEN", raising=False)
        with patch.object(
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_clara_home):
        """Create a TestClient and isolate the state DB under the test CLARA_HOME."""
        try:
            from starlette.testclient import TestClient
        except ImportError:
        ...
        import clara_state
        from clara_constants import get_clara_home
        from clara_cli.web_server import app, _SESSION_HEADER_NAME, _SESSION_TOKEN

        monkeypatch.setattr(clara_state, "DEFAULT_DB_PATH", get_clara_home() / "state.db")
    ...
    def test_profiles_sidebar_heals_stale_schema_store(self):
        ...
        array — the desktop rendered "No sessions yet" after `clara update`
    ...
    def test_startup_eager_reconcile_heals_stale_store(self):
        ...
        #79531/#80037: after `clara update` an old-schema state.db used to
    ...
    def test_startup_eager_reconcile_never_raises(self, monkeypatch):
        ...
        monkeypatch.setattr(clara_state, "SessionDB", boom)
    ...
    def test_heal_gives_up_when_reconcile_cannot_fix_the_store(self, monkeypatch):
        ...
        original_init = clara_state.SessionDB.__init__
        ...
        # the class on clara_state covers every open the helper makes.
        monkeypatch.setattr(clara_state.SessionDB, "__init__", counting_init)
    ...
    def test_generic_corruption_does_not_trigger_writable_heal(
        ...
        monkeypatch.setattr(clara_state, "SessionDB", corrupt_open)
    ...
    def test_decode_error_triggers_writable_heal(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(clara_state, "SessionDB", scripted_open)
    ...
    def test_messaging_platforms_profile_scopes_gateway_reads(self, monkeypatch):
        ...
        CLARA_HOME contextvar override (#56986), so /api/messaging/platforms
    ...
    def test_declared_surface_put_writes_config_and_secret(self):
        ...
        config_path = get_clara_home() / "hindsight" / "config.json"
    ...
    def test_put_memory_provider_config_writes_config_and_secret(self):
        from clara_constants import get_clara_home
        from clara_cli.config import load_config, load_env
    ...
    def _isolate_honcho_config(self):
        # Honcho tests write the suite-wide CLARA_HOME honcho.json; snapshot and
        # restore it so provider status/config state never leaks across tests.
        from clara_constants import get_clara_home

        path = get_clara_home() / "honcho.json"
    ...
    def test_put_honcho_writes_host_block_root_and_secret(self, monkeypatch, tmp_path):
        ...
        cfg = json.loads((get_clara_home() / "honcho.json").read_text(encoding="utf-8"))
        # baseUrl is root-scoped; the rest live in the active host block.
        assert cfg["baseUrl"] == "https://honcho.example.dev"
        assert cfg["hosts"]["clara"]["workspace"] == "myws"
        assert cfg["hosts"]["clara"]["peerName"] == "eri"
        assert cfg["hosts"]["clara"]["environment"] == "local"
        assert cfg["hosts"]["clara"]["sessionStrategy"] == "per-repo"
        # The key lands where the client reads first; GET keeps it write-only.
        assert cfg["hosts"]["clara"]["apiKey"] == "hch-test-key"
    ...
    def test_get_media_requires_auth(self):
        from clara_cli.web_server import _SESSION_HEADER_NAME
    ...
    def test_update_clara_returns_docker_guidance_without_spawning(self, monkeypatch):
        import clara_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn clara update")
        ...
        # method through clara_cli.config directly.
        monkeypatch.setattr(
            "clara_cli.config.detect_install_method", lambda *_a, **_k: "docker"
        )
        monkeypatch.setattr(web_server, "detect_install_method", lambda _root: "docker")
        monkeypatch.setattr(web_server, "_spawn_clara_action", fail_spawn)
        web_server._ACTION_PROCS.pop("clara-update", None)
        web_server._ACTION_RESULTS.pop("clara-update", None)

        resp = self.client.post("/api/clara/update")
        ...
        assert "docker pull claraship/clara-agent:latest" in data["message"]
        ...
        assert any("docker pull claraship/clara-agent:latest" in line for line in status_data["lines"])
    ...
    def test_update_clara_returns_apt_guidance_without_spawning(self, monkeypatch):
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("APT-managed update guard should not spawn clara update")

        monkeypatch.setattr(web_server, "_dashboard_local_update_managed_externally", lambda: False)
        # The shared admission gate (#91277 Phase 3) resolves the install
        # method through clara_cli.config directly, so patch it there (the
        # web_server module alias only feeds the /update/check endpoint).
        monkeypatch.setattr(
            "clara_cli.config.detect_install_method", lambda *_a, **_k: "apt"
        )
        ...
        assert data["update_command"] == "pkg upgrade clara-agent"
        assert spawned is False

        check = self.client.get("/api/clara/update/check")
        assert check.status_code == 200
        check_data = check.json()
        assert check_data["install_method"] == "apt"
        assert check_data["can_apply"] is False
        assert check_data["update_command"] == "pkg upgrade clara-agent"
    ...
    def test_update_status_recovers_completed_result_after_dashboard_restart(self, monkeypatch, tmp_path):
        ...
        action_id = "c" * 32
        (tmp_path / "clara-update.log").write_text(
            "=== clara-update started 2026-08-17 11:19:34 ===\n"
            "pulling updates...\n",
            encoding="utf-8",
        )
        (tmp_path / "update.log").write_text(
            "=== clara update started 2026-08-17T11:19:35 ===\n"
            "✓ Update complete!\n"
            f"=== clara-update completed {action_id} ===\n",
            encoding="utf-8",
        )
        ...
        status = self.client.get("/api/actions/clara-update/status?lines=2000")
        ...
        assert f"=== clara-update completed {action_id} ===" in data["lines"]

    def test_update_clara_spawns_with_action_id(self, monkeypatch):
        import clara_cli.web_server as web_server
        ...
        monkeypatch.setattr(web_server, "_spawn_clara_action", fake_spawn)
        web_server._ACTION_PROCS.pop("clara-update", None)
        web_server._ACTION_RESULTS.pop("clara-update", None)
        ...
        assert calls == [
            (["update"], "clara-update", {"CLARA_ACTION_ID": "a" * 32})
        ]

    def test_update_clara_reuses_running_action(self, monkeypatch):
        import clara_cli.web_server as web_server
        ...
        web_server._ACTION_PROCS["clara-update"] = Proc()
        web_server._ACTION_IDS["clara-update"] = "b" * 32

        try:
            resp = self.client.post("/api/clara/update")
        finally:
            web_server._ACTION_PROCS.pop("clara-update", None)
            web_server._ACTION_IDS.pop("clara-update", None)
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        ...
        tests/clara_cli/test_setup_hidden_env.py.
        """
        from clara_cli.web_server import (
    ...
    def test_model_set_maps_unknown_vendor_to_aggregator(self, monkeypatch):
        ...
        a Clara provider — keep the user's aggregator instead of writing a
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from clara_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from clara_cli.config import save_env_value
        from clara_cli.web_server import _SESSION_TOKEN
    ...
    def test_telegram_onboarding_apply_reports_restart_failure_after_save(
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "suggested_username": "clara_pair_restart_fails_bot",
                    "deep_link": "https://t.me/newbot/ClaraSetupBot/clara_pair_restart_fails_bot",
                    "qr_payload": "https://t.me/newbot/ClaraSetupBot/clara_pair_restart_fails_bot",
            ...
            return {
                "status": "ready",
                "bot_username": "clara_pair_restart_fails_bot",
        ...
        monkeypatch.setattr(ws, "_spawn_clara_action", fail_spawn_action)
    ...
    def test_unauthenticated_api_blocked(self):
        ...
        from starlette.testclient import TestClient
        from clara_cli.web_server import app
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from clara_cli.web_server import _parse_model_ids
    ...
    def test_set_model_main_custom_persists_api_key_and_registers_provider(self):
        ...
        ``clara model`` custom flow. Regression for the desktop loop where a
    ...
    def test_deleting_the_active_custom_endpoint_clears_its_model_mirror(self):
        ...
        """
        from clara_cli.config import custom_endpoint_key_env, get_env_value, load_config
    ...
    def test_custom_endpoint_save_scopes_to_the_requested_profile(self):
        ...
        """
        from clara_cli import profiles as profiles_mod
        from clara_cli.config import custom_endpoint_key_env
        from clara_constants import get_clara_home

        default_home = get_clara_home()
        worker_home = profiles_mod.get_profile_dir("worker")
        worker_home.mkdir(parents=True)
    ...
    def test_custom_endpoint_save_leaves_a_hand_written_env_ref_alone(self, monkeypatch):
        ...
        from clara_cli.config import custom_endpoint_key_env, get_config_path, get_env_value
    ...
    def test_two_endpoints_on_one_host_keep_separate_credentials(self):
        ...
        the first's key.
        """
        from clara_cli.config import custom_endpoint_key_env, get_env_value
...
class TestBuildSchemaFromConfig:
    ...
    def test_overrides_applied(self):
        from clara_cli.web_server import CONFIG_SCHEMA
    ...
    def test_timezone_field_is_searchable_select(self):
        ...
        """
        from clara_cli.web_server import CONFIG_SCHEMA, _timezone_options
...
class TestConfigRoundTrip:
    ...
    def test_round_trip_preserves_schema_invisible_nested_keys(self):
        ...
        from clara_cli.config import load_config, read_raw_config, save_config
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_clara_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_profiles_create_builder_mcp_auth_is_profile_scoped(
        ...
        from clara_constants import get_clara_home
        import clara_cli.profiles as profiles_mod
    ...
    def test_toolsets_resolve_subscription_features_once(self, monkeypatch):
        import clara_cli.tools_config as tools_config
        from clara_cli.clara_subscription import ClaraSubscriptionFeatures

        calls = 0
        features = ClaraSubscriptionFeatures(
            subscribed=False,
            clara_auth_present=False,
            provider_is_clara=False,
            features={},
            account_info=None,
        )
    ...
    def test_get_toolset_config_reports_truthful_provider_status(self, monkeypatch):
        ...
        logged-out Clara Subscription rows, xAI TTS without Grok OAuth, and
        never-installed KittenTTS/Piper. The endpoint now reports the honest
        state so keyless ≠ ready.
        """
        import clara_cli.tools_config as tools_config
        from clara_cli.clara_account import ClaraPortalAccountInfo

        # Logged out of Clara Portal → managed subscription rows need sign-in.
        monkeypatch.setattr(
            "clara_cli.clara_subscription.get_clara_portal_account_info",
            lambda *a, **k: ClaraPortalAccountInfo(
        ...
        assert by_name["Clara Subscription"]["status"] == "needs_auth"
    ...
    def test_select_managed_clara_provider_reports_needs_clara_auth(self, monkeypatch):
        """Selecting a managed Clara row while logged out flags needs_clara_auth.
        ...
        (ensure_clara_portal_access) — so the row never activated and nothing
        told the user to sign in. The endpoint now reports the entitlement
        gap so the client can drive the existing Clara OAuth flow.
        """
        from clara_cli.clara_account import ClaraPortalAccountInfo
        ...
        resp = self.client.put(
            ...
            json={"provider": "Clara Subscription (Browser Use cloud)"},
        )
        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is True
        assert data["needs_clara_auth"] is True
        assert data["feature"] == "browser"
        # The selection is still persisted — activation is what's gated.
        # Managed rows store the single 'clara' provider string (the runtime
        # maps it to the Browser Use cloud through the Clara Tool Gateway).
        from clara_cli.config import load_config
        cfg = load_config()
        assert cfg["browser"]["cloud_provider"] == "clara"
    ...
    def test_terminal_ssh_probe_ready_when_configured(self, monkeypatch):
        ...
        config["terminal"]["ssh_user"] = "clara"
        save_config(config)
        ...
        assert "clara@devbox.example.com" in ssh["detail"]
...
class TestDesktopLoopbackAuthExemption:
    ...
    def test_exempt_with_desktop_env_and_session_token_on_loopback(self, monkeypatch):
        ...
        monkeypatch.setenv("CLARA_DASHBOARD_SESSION_TOKEN", "desktop-minted")
    ...
    def test_not_exempt_without_desktop_env(self, monkeypatch):
        ...
        monkeypatch.setenv("CLARA_DASHBOARD_SESSION_TOKEN", "tok")
    ...
    def test_not_exempt_without_any_credential(self, monkeypatch):
        ...
        # CLARA_DESKTOP=1 alone is not enough: a plain serve with the env var
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from clara_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from clara_cli.web_server import _denormalize_config_from_web
        from clara_cli.config import save_config
...
class TestStatusInstallId:
    ...
    Clara home (not the profile home), survives a fresh process-cache read,
    ...
    def test_status_reports_persistent_install_id(self, monkeypatch):
        ...
        id_file = get_default_clara_root() / "install_id"
    ...
    def test_all_profiles_of_one_install_share_the_id(self, monkeypatch, tmp_path):
        """CLARA_HOME=<root> and CLARA_HOME=<root>/profiles/<name> resolve to
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_non_dict(self):
        from clara_cli.web_server import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.clara/dashboard-themes/."""
    ...
    def test_ignores_transient_profile_override(self, tmp_path, monkeypatch):
        ...
        token = set_clara_home_override(str(other))
        try:
            results = web_server._discover_user_themes()
        finally:
...
class TestThemeBootstrapCSS:
    ...
    @staticmethod
    def _write_theme(clara_home, name="ocean"):
        themes_dir = clara_home / "dashboard-themes"
    ...
    def test_user_theme_renders_bundle_vars(self, tmp_path, monkeypatch):
        ...
        assert css.startswith('<style id="clara-theme-bootstrap">')
...
class TestDeleteSessionEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_clara_home):
        ...
        monkeypatch.setattr(
            clara_state, "DEFAULT_DB_PATH", get_clara_home() / "state.db"
        )
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``clara_cli/web_server.py``).
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in clara_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_clara_home):
        ...
        # Pin the SessionDB to the isolated CLARA_HOME so each test
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_clara_home, _install_example_plugin):
        ...
        installs it into the per-test ``CLARA_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into CLARA_HOME by the class-level
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface — kanban, clara-achievements, future
        plugins — must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (clara-achievements is loaded by default).
        resp = self.client.get("/api/plugins/clara-achievements/overview")
        assert resp.status_code == 401
...
class TestDashboardPluginManifestExtensions:
    ...
    def test_user_plugins_ignore_profile_home_override(self, tmp_path, monkeypatch):
        ...
        CLARA_HOME override scopes a request to another profile."""
        from clara_constants import (
            reset_clara_home_override,
            set_clara_home_override,
        )
    ...
    def test_user_plugins_found_under_profile_scoped_process(self, tmp_path, monkeypatch):
        ...
        sets CLARA_HOME=<root>/profiles/<name>) must still discover user
        plugins installed in the clara root's plugins/ directory."""
        root = tmp_path / "clara-root"
...
# or ``sh -c 'printf …'``) instead of the real ``clara --tui`` binary.  The
...
class TestPtyWebSocket:
    ...
    def test_tui_python_command_uses_child_path(self, tmp_path):
        ...
        import clara_cli.main as main_mod

        command = f"clara-review-python{Path(sys.executable).suffix}"
        ...
        env = {
            "CLARA_CWD": str(tmp_path),
            "CLARA_PYTHON": command,
            "PATH": str(bin_dir),
        }

        main_mod._apply_tui_python_env(env)

        assert env["CLARA_PYTHON"] == command
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from clara_cli.pty_bridge import PtyUnavailableError
        ...
        import clara_cli.web_server as ws_mod
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from clara_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    import clara_cli.main as cli_main
    import clara_cli.web_server as ws
    ...
    gateway_url = env.get("CLARA_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_clara_home, _install_example_plugin):
        ...
        the per-test ``CLARA_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_clara_home):
        import threading
        ...
        with self._client():
            assert called.wait(3.0), "expected cron tick under CLARA_DESKTOP=1"
...
class TestHeadlessServeTokenPage:
    """Headless `clara serve` must serve the Desktop token handshake page
    at `/` when the dashboard auth gate is off (#94227).

    The Electron renderer boots by fetching `/` and extracting
    ``window.__CLARA_SESSION_TOKEN__`` for WebSocket auth. Headless serve
    ...
    def test_root_serves_token_page_when_not_gated(self, monkeypatch):
        ...
        match = re.search(
            r'window\.__CLARA_SESSION_TOKEN__\s*=\s*("(?:\\.|[^"\\])*")',
            resp.text,
        )
        ...
        assert "window.__CLARA_AUTH_REQUIRED__=false" in resp.text
...
class TestHashedAssetCacheHeaders:
    ...
    def test_serve_css_is_immutable_and_keeps_prefix_rewrites(
        ...
        prefixed = client.get(
            ...
            headers={"X-Forwarded-Prefix": "/clara"},
        )
        assert prefixed.status_code == 200
        assert prefixed.headers["cache-control"] == self._IMMUTABLE
        assert "url(/clara/ds-assets/bg.png)" in prefixed.text
        assert "url(/clara/fonts-terminal/x.woff2)" in prefixed.text
