"""Tests for ``clara_cli.diagnostics_upload`` — the Clara-S3 upload client.
...
class TestRequestUploadUrl:
    def test_happy_path_posts_json_and_returns_dict(self):
        from clara_cli.diagnostics_upload import request_upload_url
        ...
        with patch(
            "clara_cli.diagnostics_upload.urllib.request.urlopen",
            return_value=resp,
        ) as urlopen:
    ...
    def test_base_url_env_override(self, monkeypatch):
        ...
        monkeypatch.setenv("CLARA_DIAGNOSTICS_BASE_URL", "https://staging.example.com")
        import clara_cli.diagnostics_upload as mod
        ...
        try:
            ...
            monkeypatch.delenv("CLARA_DIAGNOSTICS_BASE_URL", raising=False)
...
class TestPutBundle:
    def test_put_sends_exact_body_and_content_type(self):
        from clara_cli.diagnostics_upload import put_bundle
...
# ---------------------------------------------------------------------------
# share_to_clara (orchestration)
# ---------------------------------------------------------------------------

class TestShareToClara:
    def test_orchestrates_request_then_put(self):
        from clara_cli import diagnostics_upload as mod
        ...
        with patch.object(mod, "request_upload_url", return_value=info) as req, \
            ...
            result = mod.share_to_clara(blob)
    ...
    def test_share_succeeds_without_id_in_response(self):
        ...
        with patch.object(mod, "request_upload_url", return_value=info), \
             patch.object(mod, "put_bundle") as put:
            result = mod.share_to_clara(b"data")
        assert result == info
