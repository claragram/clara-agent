"""Tests for clara_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $CLARA_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Clara-managed node dir.
"""
...
def _make_clara_node(clara_home: Path) -> Path:
    """Create a fake $CLARA_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = clara_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    clara_home = fake_home / ".clara"
    _make_clara_node(clara_home)
    ...
    removed = uninstall.remove_node_symlinks(clara_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    clara_home = fake_home / ".clara"
    node_bin = _make_clara_node(clara_home)
