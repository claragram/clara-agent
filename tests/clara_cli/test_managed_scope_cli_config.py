...
clara_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `clara config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_clara_home = get_clara_home()`` at import time (module
    singleton), so monkeypatching CLARA_HOME after import doesn't move it.
    ...
    this only matters for tests that swap CLARA_HOME.)
    """
    import cli

    cli._clara_home = home
