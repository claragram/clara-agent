...
def clara_home(tmp_path, monkeypatch):
    ...
    # /etc/clara on the dev/CI box can't influence the test.
    monkeypatch.setenv("CLARA_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(clara_home, monkeypatch):
...
def test_env_expansion_in_user_config(clara_home, monkeypatch):
    from clara_cli.config import load_config, cfg_get
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_clara_dotenv(clara_home=str(home))
