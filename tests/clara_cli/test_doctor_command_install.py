"""Tests for the Command Installation check in clara doctor."""
...
import clara_cli.doctor as doctor_mod


def _setup_doctor_env(monkeypatch, tmp_path, venv_name="venv"):
    """Create a minimal CLARA_HOME + PROJECT_ROOT for doctor tests."""
    home = tmp_path / ".clara"
    ...
    clara_bin = venv_bin_dir / "clara"
    clara_bin.write_text("#!/usr/bin/env python\n# entry point\n")
...
class TestDoctorCommandInstallation:
    ...
    def test_fix_repairs_wrong_symlink(self, monkeypatch, tmp_path):
        home, project, clara_bin = _setup_doctor_env(monkeypatch, tmp_path)

        # Create a symlink pointing to wrong target
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "clara"
        wrong_target = tmp_path / "wrong_clara"
        ...
        assert cmd_link.resolve() == clara_bin.resolve()
