...
``clara update`` on macOS printed ``Update complete!`` and exited 0 while the
``ai.clara.gateway`` LaunchAgent sat deregistered for 36 minutes.  The restart
...
outcomes are __PROT_0_asynchroclara__ - the ``_request_gateway_self_restart`` branch returns
...
from clara_cli.update_cmd import _warn_incomplete_gateway_fleet_restart
...
class TestIncompleteFleetWarningIsPlatformCorrect:
    ...
    def test_linux_recovery_instructions_are_unchanged(self, monkeypatch, capsys):
        ...
        _warn_incomplete_gateway_fleet_restart(["clara-gateway.service"])
