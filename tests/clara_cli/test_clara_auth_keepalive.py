from clara_cli import clara_auth_keepalive as keepalive
...
def test_refresh_always_fires_before_expiry_for_observed_lifetimes():
    ...
    for lifetime in OBSERVED_LIFETIMES_SECONDS:
        tick = keepalive._tick_seconds(
            keepalive.CLARA_AUTH_KEEPALIVE_INTERVAL_SECONDS, lifetime
        )
        horizon = keepalive._refresh_horizon_seconds(
            tick, keepalive.CLARA_INVOKE_JWT_MIN_TTL_SECONDS
        )
...
def test_interval_precedence_and_disable(monkeypatch):
    def _config(section):
        monkeypatch.setattr(keepalive, "_clara_config", lambda: section)

    # An absent section leaves the module default in place.
    _config({})
    assert (
        keepalive._interval_seconds(None)
        == keepalive.CLARA_AUTH_KEEPALIVE_INTERVAL_SECONDS
    )

    _config({keepalive.CLARA_AUTH_KEEPALIVE_INTERVAL_CONFIG_KEY: 600})
    ...
    _config({keepalive.CLARA_AUTH_KEEPALIVE_INTERVAL_CONFIG_KEY: "not-a-number"})
    assert (
        keepalive._interval_seconds(None)
        == keepalive.CLARA_AUTH_KEEPALIVE_INTERVAL_SECONDS
    )
    ...
    _config({keepalive.CLARA_AUTH_KEEPALIVE_INTERVAL_CONFIG_KEY: 0})
    assert keepalive._interval_seconds(None) == 0
    assert keepalive.start_clara_auth_keepalive() is None
...
def test_keepalive_refreshes_stale_pool_entry(monkeypatch):
    ...
    assert keepalive.refresh_clara_auth_keepalive_once() is True
...
def test_keepalive_falls_back_to_singleton_state(monkeypatch):
    ...
    def _resolve_clara_runtime_credentials(**kwargs):
        calls.append(kwargs)
        return {
            "provider": "clara",
    ...
    monkeypatch.setattr(
        ...
        _resolve_clara_runtime_credentials,
    )

    assert keepalive.refresh_clara_auth_keepalive_once(timeout_seconds=15.0) is True
