...
def test_auto_detected_clara_auth_failure_falls_through_to_openrouter(monkeypatch):
    """When auto-detect picks Clara but credentials are revoked, fall through to OpenRouter."""
    from clara_cli.auth import AuthError
    ...
    # resolve_provider returns "clara" (stale active_provider in auth.json)
    ...
    # Clara credential resolution fails with revoked token
    monkeypatch.setattr(
        rp, "resolve_clara_runtime_credentials",
        lambda **kw: (_ for _ in ()).throw(
            AuthError("Refresh session has been revoked",
                      provider="clara", code="invalid_grant", relogin_required=True)
        ),
    )
...
class TestProviderEntryApiKeyEnvAlias:
    ...
    def test_snake_case_api_key_env_normalizes_to_key_env(self):
        from clara_cli.config import _normalize_custom_provider_entry
        entry = {
            "name": "vendor",
    ...
    def test_valid_fields_set_lists_key_env(self):
        ...
        from clara_cli.config import _VALID_CUSTOM_PROVIDER_FIELDS
...
def test_minimax_oauth_runtime_returns_anthropic_messages_mode(monkeypatch):
    ...
    from clara_cli.auth import MINIMAX_OAUTH_GLOBAL_INFERENCE
