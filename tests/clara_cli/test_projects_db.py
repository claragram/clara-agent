"""Tests for the per-profile Projects store (clara_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Clara Agent", folders=["/tmp/clara"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "clara-agent"
    assert proj.name == "Clara Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/clara"
    assert [f.path for f in proj.folders] == ["/tmp/clara"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "clara-agent").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' CLARA_HOME.
