...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (CLARA_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via CLARA_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in CLARA_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from clara_cli.providers import CLARA_OVERLAYS
        assert "bedrock" in CLARA_OVERLAYS
