...
``clara*.exe`` shims aside (``clara.exe.old.<ms>``) before invoking the
...
quarantined shims stayed renamed aside and ``clara`` vanished from PATH
...
def _make_scripts_dir(tmp_path: Path) -> Path:
    ...
    for name in ("clara", "clara-agent", "clara-acp", "clara-gateway"):
...
# ---------------------------------------------------------------------------
# clara_cli.main._run_quarantined_install
# ---------------------------------------------------------------------------
...
def test_main_noop_success_restores_shims(tmp_path):
    ...
    assert "clara.exe" in names, "clara.exe must be restored after a no-op install"
    assert "clara-acp.exe" in names
    assert "clara-gateway.exe" in names
    assert (scripts / "clara.exe").read_bytes() == b"MZ-old-clara"
...
def test_main_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    assert (scripts / "clara.exe").read_bytes() == b"MZ-new-clara"
...
# ---------------------------------------------------------------------------
# clara_cli._install_repair._run_install_cmd (the deferred-recovery path)
# ---------------------------------------------------------------------------
...
def test_repair_noop_success_restores_shims(tmp_path):
    ...
    names = _shim_names(scripts)
    assert "clara.exe" in names, "clara.exe must be restored after a no-op recovery install"
    assert (scripts / "clara.exe").read_bytes() == b"MZ-old-clara"
...
def test_repair_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    def fake_run(cmd, cwd=None, check=None, env=None):
        (scripts / "clara.exe").write_bytes(b"MZ-new-clara")
