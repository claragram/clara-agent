"""Tests for normalized Clara Portal account entitlement helpers."""
...
from clara_cli.clara_account import (
    ...
    reset_clara_portal_account_info_cache,
)
...
def test_fresh_account_payload_normalization(monkeypatch, payload, expected_paid):
    ...
    monkeypatch.setattr("clara_cli.auth.get_provider_auth_state", lambda provider: _state(token))
    monkeypatch.setattr("clara_cli.auth.resolve_clara_access_token", lambda: "fresh-token")
    monkeypatch.setattr("clara_cli.clara_account._fetch_clara_account_info", lambda *a, **kw: payload)

    info = get_clara_portal_account_info(force_fresh=True)

    assert isinstance(info, ClaraPortalAccountInfo)
...
def test_no_oauth_token_reports_inference_key_present(monkeypatch):
    monkeypatch.setattr("clara_cli.auth.get_provider_auth_state", lambda provider: {})
...
def test_member_spend_cap_exceeded_message(monkeypatch):
    ...
    msg = format_clara_portal_entitlement_message(info, capability="Clara model access")
