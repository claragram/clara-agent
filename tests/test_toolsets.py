...
class TestToolsetConsistency:
    ...
    def test_clara_platforms_share_core_tools(self):
        """All clara-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on clara-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["clara-cli", "clara-telegram", "clara-discord", "clara-whatsapp", "clara-slack", "clara-signal", "clara-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_clara_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("clara-whatsapp")
...
class TestResolveToolsetMemo:
    ...
    def test_second_resolution_is_cached(self, monkeypatch):
        ...
        first = resolve_toolset("clara-cli")
        second = resolve_toolset("clara-cli")
        ...
        assert (
            "clara-cli", True, registry_id, generation
    ...
    def test_memo_result_matches_fresh_resolution(self):
        ...
        toolsets_mod._resolve_toolset_memo.clear()
        first = resolve_toolset("clara-cli", include_registry=False)
        second = resolve_toolset("clara-cli", include_registry=False)
        assert first == second
