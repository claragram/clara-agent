...
def test_required_frontmatter_fields(p):
    ...
    clara = (fm.get("metadata") or {}).get("clara") or {}
    if not (clara.get("tags") or fm.get("tags")) and not _grandfathered(p, "tags"):
        pytest.fail(f"{_rel(p)}: no tags (metadata.clara.tags or top-level tags)")
...
def test_related_skills_resolve(p):
    ...
    clara = (fm.get("metadata") or {}).get("clara") or {}
    dangling = [
        rs for rs in (clara.get("related_skills") or []) if rs not in _all_names()
    ]
