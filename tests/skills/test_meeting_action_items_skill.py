...
def test_frontmatter_required_fields():
    ...
    clara = fm["metadata"]["clara"]
    assert clara["tags"]
    assert "related_skills" in clara
...
def test_author_credits_human_first():
    fm, _ = _frontmatter_and_body()
    assert not fm["author"].startswith("Clara Agent"), "human contributor must be credited first"
...
def test_related_skills_resolve_in_repo():
    ...
    for name in fm["metadata"]["clara"]["related_skills"]:
        hits = (
