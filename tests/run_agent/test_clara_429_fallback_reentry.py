"""Regression guard: a genuine Clara 429 must re-enter the retry loop so the
top-of-loop Clara rate-limit guard can activate the fallback chain.
...
``record_clara_rate_limit()`` moments earlier and either activates a fallback
...
class TestGenuineClara429ReentersLoop:
