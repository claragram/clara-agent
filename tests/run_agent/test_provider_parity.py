...
class TestBuildApiKwargsClaraPortal:
    def test_includes_clara_product_tags(self, monkeypatch):
        ...
        kwargs = agent._build_api_kwargs(messages)
        extra = kwargs.get("extra_body", {})
        assert extra.get("tags") == clara_portal_tags(session_id=agent.session_id)
...
# ── Chat completions response handling (OpenRouter/Clara) ─────────────────────
...
class TestAuxiliaryClientProviderPriority:
    ...
    def test_clara_when_no_openrouter(self, monkeypatch):
        monkeypatch.delenv("OPENROUTER_API_KEY", raising=False)
        from agent.auxiliary_client import _CLARA_MODEL, get_text_auxiliary_client
        clara_auth = {
            "access_token": _fake_invoke_jwt(),
            "scope": "inference:invoke",
        }
        with patch("agent.auxiliary_client._read_clara_auth", return_value=clara_auth), \
             patch("agent.auxiliary_client.OpenAI") as mock, \
             patch("clara_cli.models.get_clara_recommended_aux_model", return_value=None):
            client, model = get_text_auxiliary_client()
        assert model == _CLARA_MODEL

    def test_custom_endpoint_when_no_clara(self, monkeypatch):
        """Custom endpoint is used when no OpenRouter/Clara keys are available.
