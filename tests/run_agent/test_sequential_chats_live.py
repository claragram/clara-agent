...
AlexKucera reported on Discord (2026-04-16) that after ``clara update`` pulled
...
Opt-in — not part of default CI:
    CLARA_LIVE_TESTS=1 pytest tests/run_agent/test_sequential_chats_live.py -v

Requires ``OPENROUTER_API_KEY`` to be set (or sourced via ~/.clara/.env).
"""
...
# Load ~/.clara/.env so live runs pick up OPENROUTER_API_KEY without
