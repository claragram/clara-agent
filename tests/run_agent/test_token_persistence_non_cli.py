...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    clara_state = ModuleType("clara_state")
    clara_state.SessionDB = FakeSessionDB
    clara_state.get_shared_session_db = lambda db_path=None: sentinel_db
    monkeypatch.setitem(sys.modules, "clara_state", clara_state)
    ...
    result = json.loads(agent._invoke_tool(
        "session_search",
        {"query": "Clara", "detail": "full"},
        "task-id",
    ))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Clara"
...
def test_sequential_session_search_forwards_detail(monkeypatch):
    ...
    tool_call = SimpleNamespace(
        ...
        function=SimpleNamespace(
            name="session_search",
            arguments=json.dumps({"query": "Clara", "detail": "full"}),
        ),
    )
