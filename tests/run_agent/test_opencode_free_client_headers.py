...
@patch("run_agent.OpenAI")
def test_opencode_free_sends_clara_attribution(mock_openai):
    """Keyless requests still identify as Clara (attribution headers match
    ...
    headers = _zen_call_headers(mock_openai)
    assert headers.get("X-Title") == "Clara Agent"
    assert str(headers.get("User-Agent", "")).startswith("ClaraAgent/")
