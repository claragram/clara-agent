...
from clara_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(clara_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <clara_home>/plugins/<name> and opt it in."""
    plugin_dir = clara_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CLARA_HOME", str(tmp_path / "clara_empty"))
