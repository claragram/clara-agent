...
def test_registered_observable_metric_names_cover_snapshot_metrics(monkeypatch):
    ...
    gateway_snapshot = type("S", (), {"metrics": [
        _M("clara.gateway.up"), _M("clara.gateway.active_agents"),
        _M("clara.gateway.busy"), _M("clara.gateway.drainable"),
        _M("clara.gateway.restart_requested"),
        _M("clara.platform.up"), _M("clara.platform.degraded"),
    ]})()
    cron_snapshot = type("S", (), {"metrics": [
        _M("clara.cron.scheduler.heartbeat_age_seconds"),
        _M("clara.cron.scheduler.last_success_age_seconds"),
        _M("clara.cron.scheduler.catch_up_occurrences"),
        _M("clara.cron.jobs.enabled"), _M("clara.cron.jobs.running"),
        _M("clara.cron.jobs.overdue"),
    ]})()
...
def test_monitoring_docs_distinguish_relay_health_scope_and_terminal_flush():
    ...
    assert "Clara Agent-owned Relay transport health" in text
