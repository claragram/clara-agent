"""Tests for the bundled Clara dashboard-auth plugin.

Covers four shapes from Phase 4 of ``.clara/plans/2026-05-21-dashboard-oauth-auth.md``:
...
import plugins.dashboard_auth.clara as clara_plugin
...
def _patched_jwks(provider: clara_plugin.ClaraDashboardAuthProvider, rsa_keypair):
...
class TestConstruction:
    def test_protocol_compliance(self):
        assert_protocol_compliance(clara_plugin.ClaraDashboardAuthProvider)
    ...
    def test_name_and_display(self):
        ...
        assert p.name == "clara"
        assert p.display_name == "Workprise"
    ...
    def test_rejects_malformed_client_id(self):
        with pytest.raises(ValueError, match="agent:"):
            clara_plugin.ClaraDashboardAuthProvider(
                client_id="clara-dashboard", portal_url="https://x"
            )
...
class TestPluginRegister:
    def test_skips_when_client_id_missing(self, monkeypatch):
        monkeypatch.delenv("CLARA_DASHBOARD_OAUTH_CLIENT_ID", raising=False)
        monkeypatch.delenv("CLARA_DASHBOARD_PORTAL_URL", raising=False)
        ctx = MagicMock()
        clara_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_not_called()
        # Skip reason is surfaced for the gate's fail-closed message.
        assert "CLARA_DASHBOARD_OAUTH_CLIENT_ID" in clara_plugin.LAST_SKIP_REASON

    def test_registers_with_default_portal_url_when_only_client_id_set(
        self, monkeypatch
    ):
        """Phase 7 follow-up: CLARA_DASHBOARD_PORTAL_URL is optional —
        defaults to the production Clara Portal. The user shouldn't have
        to set it for the common production deployment path."""
        monkeypatch.setenv("CLARA_DASHBOARD_OAUTH_CLIENT_ID", "agent:inst1")
        monkeypatch.delenv("CLARA_DASHBOARD_PORTAL_URL", raising=False)
        ctx = MagicMock()
        clara_plugin.register(ctx)
        ...
        assert registered._portal_url == "https://portal.claraprise.com"
    ...
    def test_empty_portal_url_env_uses_default(self, monkeypatch):
        ...
        monkeypatch.setenv("CLARA_DASHBOARD_OAUTH_CLIENT_ID", "agent:inst1")
        monkeypatch.setenv("CLARA_DASHBOARD_PORTAL_URL", "")
        ctx = MagicMock()
        clara_plugin.register(ctx)
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
        assert registered._portal_url == "https://portal.claraprise.com"
...
class TestConfigYamlSource:
    ...
    canonical surface for these settings. ``CLARA_DASHBOARD_OAUTH_CLIENT_ID``
    and ``CLARA_DASHBOARD_PORTAL_URL`` are operator overrides that win when
    ...
    def patch_config(self, monkeypatch):
        """Yield a callable that replaces ``clara_cli.config.load_config``
        ...
        def _set(oauth_block: Dict[str, Any] | None) -> None:
            ...
            monkeypatch.setattr(
                "clara_cli.config.load_config", lambda: cfg
            )
    ...
    def test_env_overrides_config_client_id(self, patch_config, monkeypatch):
        ...
        CLARA_DASHBOARD_OAUTH_CLIENT_ID at deploy time and we MUST
        honour it even if a stale config.yaml ships in the image."""
        monkeypatch.setenv("CLARA_DASHBOARD_OAUTH_CLIENT_ID", "agent:from-env")
        patch_config({"client_id": "agent:from-config"})
        ctx = MagicMock()
        clara_plugin.register(ctx)
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
    ...
    def test_neither_source_skips_with_helpful_reason(
        ...
        assert "dashboard.oauth.client_id" in clara_plugin.LAST_SKIP_REASON, (
            f"skip reason omits the config.yaml surface — operators "
            f"won't know it exists. got: {clara_plugin.LAST_SKIP_REASON!r}"
        )
...
class TestStartLogin:
    @pytest.fixture
    def provider(self):
        return clara_plugin.ClaraDashboardAuthProvider(
            client_id="agent:inst1", portal_url="https://portal.example.com"
        )

    def test_returns_login_start(self, provider):
        result = provider.start_login(
            redirect_uri="https://clara.fly.dev/auth/callback"
        )
    ...
    def test_authorize_url_has_required_params(self, provider):
        ...
        assert params["redirect_uri"] == "https://clara.fly.dev/auth/callback"
    ...
    def test_code_verifier_in_cookie_payload_43_to_128_chars(self, provider):
        ...
        assert "clara_session_pkce" in result.cookie_payload
        pkce = result.cookie_payload["clara_session_pkce"]
    ...
    def test_two_calls_produce_different_state_and_verifier(self, provider):
        ...
        assert a.cookie_payload["clara_session_pkce"] != b.cookie_payload[
            "clara_session_pkce"
        ]
    ...
    def test_allows_http_with_arbitrary_host(self, provider):
        ...
        provider.start_login(redirect_uri="http://clara.fly.dev/auth/callback")
...
class TestCompleteLogin:
    ...
    def test_happy_path_returns_session(self, provider, rsa_keypair):
        ...
        with patch("plugins.dashboard_auth.clara.httpx.post", return_value=mock_resp):
    ...
    def test_network_error_raises_provider_error(self, provider):
        with patch(
            "plugins.dashboard_auth.clara.httpx.post",
...
class TestVerifySession:
    ...
    def test_jwks_client_sends_explicit_http_headers(self, provider):
        ...
        client_cls.assert_called_once_with(
            provider._jwks_url,
            cache_keys=True,
            lifespan=clara_plugin._JWKS_CACHE_SECONDS,
            headers={
    ...
    def test_verification_failure_message_surfaces_token_claims(
        ...
        config drift between CLARA_DASHBOARD_PORTAL_URL/CLIENT_ID and Portal."""
    ...
    def test_contract_version_missing_warns_but_succeeds(
        ...
        with caplog.at_level(logging.WARNING, logger="plugins.dashboard_auth.clara"):
...
class TestRefreshAndRevoke:
    ...
    def test_refresh_happy_path_returns_rotated_session(self, provider, rsa_keypair):
        ...
        with patch(
            "plugins.dashboard_auth.clara.httpx.post", return_value=mock_resp
        ) as mock_post:
        ...
        assert kwargs["headers"]["x-clara-refresh-token"] == "rt_old_value"
