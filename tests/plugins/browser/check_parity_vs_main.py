...
Run from the PR worktree:
    ...
    cd ~/.clara/clara-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.clara/clara-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.clara/clara-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a clara-agent checkout"
)
...
# Isolated CLARA_HOME for the config write.
