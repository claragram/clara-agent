...
class TestGenerate:
    ...
    def test_aspect_ratio_mapping(self):
        """Clara 'square' must map to Krea '1:1' in the wire payload."""
        from plugins.image_gen.krea import KreaImageGenProvider
...
# ---------------------------------------------------------------------------
# Managed Clara gateway path
# ---------------------------------------------------------------------------


def _managed_cfg(
    origin: str = "https://krea-gateway.example.com",
    token: str = "clara-tok-abc",
):
    ...
    return SimpleNamespace(
        ...
        clara_user_token=token,
        managed_mode=True,
    )


class TestManagedGateway:
    def test_managed_submit_uses_gateway_origin_and_clara_token(self, monkeypatch):
        """Managed mode submits to the gateway origin with the Clara token."""
