...
# worktree), ``MAIN_DIR`` falls back to a sibling ``clara-agent-main``
...
def _resolve_main_dir() -> Path:
    ...
    if (candidate / "tools" / "image_generation_tool.py").exists() and candidate != REPO_ROOT:
        return candidate
    sibling = REPO_ROOT.parent / "clara-agent-main"
...
assert (PR_DIR / "tools" / "image_generation_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a clara-agent checkout"
)
...
# Isolated CLARA_HOME so the config write is hermetic.
home = tempfile.mkdtemp()
os.environ["CLARA_HOME"] = home
...
def main() -> int:
    ...
    if MAIN_DIR == PR_DIR:
        print(
            ...
            "      Set up a sibling 'clara-agent-main' checkout pinned to "
