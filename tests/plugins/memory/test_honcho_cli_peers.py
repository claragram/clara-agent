"""Regression tests for #76414: `clara honcho peers` showed "(not set)"
for every non-default profile.

_all_profile_host_configs() built the per-profile host key inline as
f"{HOST}.{profile}" ("clara.work") while every other reader/writer —
profile_host_key(), resolve_active_host(), honcho status/enable/sync and
the runtime plugin — uses the underscore form ("clara_work"). The lookup
...
a real honcho.json (temp CLARA_HOME, no network).
"""
import io
import json
import sys
...
def honcho_home(tmp_path, monkeypatch):
    cfg = {
        "peerName": "alice",
        "hosts": {
            "clara": {"peerName": "alice", "aiPeer": "clara"},
            "clara_work": {"peerName": "alice", "aiPeer": "clara"},
            "clara_my_profile": {"peerName": "bob", "aiPeer": "clara"},
        },
    }
...
class TestAllProfileHostConfigs:
    def test_profile_host_keys_match_writer_form(self, honcho_home, monkeypatch):
        ...
        monkeypatch.setattr(
            "clara_cli.profiles.list_profiles",
        ...
        assert host == "clara_work"  # not "clara.work"
    ...
    def test_sanitized_profile_names_resolve(self, honcho_home, monkeypatch):
        ...
        resolve — profile_host_key maps 'my.profile' -> 'clara_my_profile';
    ...
    def test_legacy_dot_form_host_key_still_readable(self, honcho_home, monkeypatch):
        ...
        ("clara.work") must keep working — the README promises those keys
        ...
        del cfg["hosts"]["clara_work"]
        cfg["hosts"]["clara.work"] = {"peerName": "carol", "aiPeer": "clara"}
        path.write_text(json.dumps(cfg))
        monkeypatch.setattr(
            "clara_cli.profiles.list_profiles",
            lambda: [SimpleNamespace(name="default"), SimpleNamespace(name="work")],
...
class TestCmdPeers:
    def test_peers_shows_populated_identity_not_host_key_leak(
        ...
        raw malformed key 'clara.work' (or '(not set)')."""
        monkeypatch.setattr(
            "clara_cli.profiles.list_profiles",
            lambda: [SimpleNamespace(name="default"), SimpleNamespace(name="work")],
        )
        out = _peers_output(SimpleNamespace())
        assert "clara.work" not in out
        ...
        assert "alice" in work_line and "clara" in work_line
    ...
    def test_peers_falls_back_cleanly_when_block_missing(
        ...
        out = _peers_output(SimpleNamespace())
        assert "clara.new" not in out  # well-formed key, no dot-form leak
