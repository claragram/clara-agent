...
def _isolate_env(tmp_path, monkeypatch):
    """Ensure CLARA_HOME and RETAINDB vars are isolated."""
...
class TestRetainDBMemoryProvider:
    ...
    def test_initialize_creates_client_and_queue(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", clara_home=str(tmp_path / ".clara"))
...
class TestPrefetch:
    ...
    def _make_initialized_provider(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CLARA_HOME", str(clara_home))
        p = RetainDBMemoryProvider()
        p.initialize("test-session", clara_home=str(clara_home))
        return p
