...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "clara"
        assert config.workspace_id == "clara"
...
class TestFromGlobalConfig:
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"clara": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_host_block_overrides_root_and_env(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"clara": {"baseUrl": "http://host-block:9001"}},
        }))
    ...
    def test_base_url_full_precedence_chain(self, tmp_path):
        ...
        config_file = tmp_path / "config.json"
        layers = {
            "hosts": {"clara": {"baseUrl": "http://host:1"}},
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="clara-agent"
        ):
            result = config.resolve_session_name("/home/user/clara-agent/subdir")
        assert result == "clara-agent"


class TestResolveConfigPath:
    def test_prefers_clara_home_when_exists(self, tmp_path):
        clara_home = tmp_path / "clara"
        clara_home.mkdir()
        local_cfg = clara_home / "honcho.json"
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: CLARA_HOME points at ~/.clara/profiles/<name>, so
        # _get_default_clara_home() must resolve back to ~/.clara — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".clara"
        ...
        assert _get_default_clara_home() == default_home
        assert result == default_cfg


class TestResolveActiveHost:
    def test_profile_host_key_uses_honcho_safe_separator(self):
        assert profile_host_key("coder") == "clara_coder"
        assert profile_host_key("default") == "clara"


    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"CLARA_HONCHO_HOST": "clara.coder"}):
            assert resolve_active_host() == "clara.coder"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False), patch(
            ...
            os.environ.pop("CLARA_HONCHO_HOST", None)
            # Temporarily remove clara_cli.profiles to simulate import failure
            saved = sys.modules.get("clara_cli.profiles")
            sys.modules["clara_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "clara"
            finally:
                if saved is not None:
                    sys.modules["clara_cli.profiles"] = saved
                else:
                    sys.modules.pop("clara_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="clara_coder")
        assert config.host == "clara_coder"
        assert config.workspace_id == "clara"  # shared workspace
        assert config.ai_peer == "clara_coder"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"clara": {"enabled": True, "aiPeer": "clara"}},
        }))
...
class TestGetHonchoClient:
    ...
    def test_dot_form_legacy_host_key_keeps_local_api_key(self):
        """Regression for #37436: a legacy dot-form host block (clara.work)
        ...
        cfg = HonchoClientConfig(
            ...
            host="clara_work",
            workspace_id="clara",
            raw={"hosts": {"clara.work": {"apiKey": "explicit-local-key"}}},
        )
    ...
    def test_timeout_change_triggers_client_rebuild(self):
        ...
        cfg_yaml = get_clara_home() / "config.yaml"
    ...
    def test_managed_config_timeout_does_not_thrash_singleton(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CLARA_MANAGED_DIR", str(managed_dir))
...
class TestGetHonchoClientBaseUrlDoublePrefixFix:
    ...
    def test_lan_default_host_empty_key_uses_local_placeholder(self, tmp_path):
        ...
        with patch.dict(os.environ, {}, clear=True), \
             patch("clara_cli.profiles.get_active_profile_name", return_value="default"), \
