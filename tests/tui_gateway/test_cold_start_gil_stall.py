...
__PROT_0_synchroclara__ CPU-bound work ran on the loop thread during the window
between ``CLARA_BACKEND_READY`` and the first prompt. Three fixes:
...
class TestCopilotAuthSkipsGhCli:
    ...
    def test_no_env_vars_falls_back_to_gh_cli(self, monkeypatch):
        ...
        with patch(
            "clara_cli.copilot_auth._try_gh_cli_token",
