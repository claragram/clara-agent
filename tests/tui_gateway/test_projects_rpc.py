...
def test_record_repos_persists_and_shows_zero_session_repo(tmp_path):
    ...
    # Repo-first: a scanned repo with no clara sessions still surfaces.
...
def test_scan_time_is_not_treated_as_session_activity(tmp_path):
    ...
    in Clara outranked the ones they actually work in.
    """
...
def test_terminal_session_persists_its_launch_cwd():
    ...
    The user cd'd into that directory before running clara. Dropping it left
...
def test_desktop_agent_rebuild_preserves_workspace_provenance(
    ...
    session = {
        "agent": object(),
        "attached_images": [],
        "cwd": "/picked/repo" if explicit_cwd else "/opt/clara",
        "edit_snapshots": {},
...
def _serving_launch_profile(launch_home: Path):
    ...
    token = set_clara_home_override(launch_home)
...
def test_projects_without_a_profile_stay_on_the_launch_home(monkeypatch, tmp_path):
    ...
    assert not (Path(os.environ["CLARA_HOME"]) / "projects.db").exists()
