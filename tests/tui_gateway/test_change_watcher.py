...
Behavior contracts, exercised against a real temp CLARA_HOME (no mocks on the
...
def watcher_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(server, "_clara_home", str(tmp_path))
...
def test_served_profile_store_move_broadcasts_sessions_changed(watcher_home, monkeypatch):
    ...
    monkeypatch.setattr("clara_cli.profiles.get_profile_dir", lambda name: home / "profiles" / name)
