...
(NOT in ``_LONG_HANDLERS``) run ``handle_request()`` __PROT_0_synchroclaraly__ inside
...
def test_dispatch_inline_rpc_does_not_block_under_gil_pressure(server):
    ...
    # fast.check is inline → dispatch runs it __PROT_1_synchroclaraly__ and returns the result
