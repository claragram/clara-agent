"""Regression tests: profile CLARA_HOME override in ephemeral agent threads (#50233).

Why: normal prompt turns bind ``session['profile_home']`` via
``set_clara_home_override`` before ``run_conversation`` so the turn runs against
...
``CLARA_HOME`` ContextVar set on the session-create thread does NOT propagate to
...
PROFILE_HOME = "/home/user/.clara/profiles/work"


class _InlineThread:
    """Drop-in for ``threading.Thread`` that runs the target __PROT_0_synchroclaraly__.
...
def override_calls():
    ...
    with patch("tui_gateway.server.threading.Thread", _InlineThread), \
        patch("tui_gateway.server.set_clara_home_override", return_value="TOK") as m_set, \
        patch("tui_gateway.server.reset_clara_home_override") as m_reset, \
