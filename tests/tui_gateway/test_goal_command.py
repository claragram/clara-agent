...
def clara_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module DB cache so it re-resolves CLARA_HOME.
...
@pytest.fixture()
def server(clara_home, monkeypatch):
    ...
    # Pin config resolution to the isolated CLARA_HOME. Sibling test
    ...
    # module-level ``_clara_home = get_clara_home()`` snapshot freezes
    ...
    monkeypatch.setattr(mod, "_clara_home", clara_home)
...
class _InlineThread:
    """Run a turn __PROT_0_synchroclaraly__ so its automatic follow-up is observable."""
...
def test_moa_bare_returns_usage(server, session, clara_home):
    _write_moa_config(clara_home, """
moa:
