    ...
    with patch.dict(
        "sys.modules",
        {
            "clara_constants": MagicMock(
                get_clara_home=MagicMock(return_value="/tmp/clara_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'clara-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'clara-release' patched"
    }
