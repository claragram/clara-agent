...
class _RecordingDB:
    """Stand-in for ``clara_state.SessionDB`` that counts ``close()`` calls."""
...
def test_lazy_recall_open_is_owned_by_the_agent(monkeypatch):
    ...
    monkeypatch.setattr("clara_state.get_shared_session_db", _factory)
...
def build_env(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(server, "set_clara_home_override", lambda _home: None)
    monkeypatch.setattr(server, "reset_clara_home_override", lambda _tok: None)
