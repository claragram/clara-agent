"""Windows installer regression for Clara children outside the venv.

The venv sweep deliberately selects process roots by executable path so it
does not kill unrelated Python processes. A selected Clara process can spawn
...
def test_venv_sweep_stops_managed_runtime_children_but_not_unrelated_processes(
    ...
    runtime_dir = install_dir / ".clara-runtime" / "python" / "generation-test"
    ...
    # Keep the test away from real gateway tasks and real Clara launchers
    ...
    wrapper.write_text(
        ...
        f"-InstallDir '{install_dir}' -ClaraHome '{clara_home}'\n"
        "exit $LASTEXITCODE\n",
        encoding="utf-8",
    )
