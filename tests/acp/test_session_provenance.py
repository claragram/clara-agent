...
def test_root_session_no_compression(db):
    ...
    assert prov["currentClaraSessionId"] == "root1"
    assert prov["rootClaraSessionId"] == "root1"
    assert prov["parentClaraSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_clara_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentClaraSessionId"] == "old"
    assert prov["rootClaraSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousClaraSessionId"] == "old"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootClaraSessionId"] == "p"  # lineage root still walked
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"clara"}
    assert "sessionProvenance" in meta["clara"]
    assert meta["clara"]["sessionProvenance"]["currentClaraSessionId"] == "root1"
