...
class TestCacheFileReadBlocking:
    """Internal Clara cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        clara_home = tmp_path / ".clara"
        cache = clara_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._clara_home_path", return_value=clara_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Clara cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        clara_home = tmp_path / ".clara"
        hub = clara_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_clara_home(self, tmp_path):
        """The env basename guard does not depend on CLARA_HOME resolution."""
        clara_home = tmp_path / ".clara"
        clara_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        clara_home = tmp_path / ".clara"
        cache = clara_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
