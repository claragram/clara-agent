...
The official Codex endpoint must receive Clara' own harness identity, rather
...
These tests pin:
- the required Clara originator
- the versioned Clara User-Agent
...
class TestCodexCloudflareHeaders:
    ...
    def test_user_agent_advertises_clara_version(self):
...
class TestPrimaryClientWiring:
    ...
    def test_apply_client_headers_on_base_url_change(self):
        ...
        with patch("run_agent.OpenAI") as mock_openai:
            ...
            assert headers.get("originator") == "clara-agent"
            assert headers.get("ChatGPT-Account-ID") == "acct-rotation"
            assert headers.get("User-Agent") == f"ClaraAgent/{__version__}"
