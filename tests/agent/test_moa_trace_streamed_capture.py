...
path via ``clara chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp CLARA_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point CLARA_HOME at a temp dir and turn moa.save_traces on."""
    clara_home = tmp_path / ".clara"
    clara_home.mkdir()
    monkeypatch.setenv("CLARA_HOME", str(clara_home))

    # save_moa_turn reads config via clara_cli.config.load_config; stub it to
