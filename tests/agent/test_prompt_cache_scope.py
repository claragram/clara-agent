...
class TestPerResponseRunNonceIsolation:
    ...
    Clara Studio group chat builds ``gc_run_<room>_<profile>_<name>_<uuid4hex>``
    for every reply and destroys it when the reply completes
    (``groupRuntimeSessionId``), so every conversation-affinity hint Clara
    ...
    is an identity, and Clara' public session API lets a client choose one
    ...
    def test_parentless_rows_resolve_to_their_own_scope(self, db):
        ...
        an owner Clara was never told about must never be guessed. #96811
    ...
    @pytest.mark.parametrize("profile_name", ["openrouter", "clara"])
    def test_distinct_ids_keep_distinct_provider_sticky_keys(self, profile_name):
        """OpenRouter/Clara route by this key, and it isolates distinct ids.
