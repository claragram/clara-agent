...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate CLARA_HOME under tmp_path."""
    monkeypatch.setenv("CLARA_HOME", str(tmp_path / ".clara"))
    (tmp_path / ".clara").mkdir()

    # Force the constants/image cache helpers to re-read CLARA_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("clara_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_clara_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under CLARA_HOME — gateway cleanup
