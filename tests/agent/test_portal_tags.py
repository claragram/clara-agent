"""Tests for agent.portal_tags — Clara Portal request tag contract."""
...
def test_clara_portal_tags_contains_product_and_client():
    """Every Clara Portal request gets BOTH the product tag and the version tag."""
    from agent.portal_tags import clara_client_tag, clara_portal_tags

    tags = clara_portal_tags()
...
def test_ambient_context_set_none_clears():
    ...
    for empty in (None, ""):
        ...
        try:
            ...
            assert len(clara_portal_tags()) == 2
        finally:
...
def test_ambient_context_isolated_between_contexts():
    ...
    assert not any(t.startswith("conversation=") for t in clara_portal_tags())
...
def test_ambient_context_propagates_via_thread_context_helper():
    ...
    try:
        with ThreadPoolExecutor(max_workers=1) as ex:
            plain = ex.submit(clara_portal_tags).result()
            propagated = ex.submit(
                propagate_context_to_thread(clara_portal_tags)
            ).result()
    finally:
        reset_conversation_context(token)
...
def test_clara_sticky_key_matches_conversation_tag():
