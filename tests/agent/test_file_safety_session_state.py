...
manipulation. Clara keeps canonical conversation history in state.db and may
...
def fake_homes(tmp_path, monkeypatch):
    """Point CLARA_HOME at a temp profile dir.

    Uses the real env-var resolution chain (get_clara_home /
    get_default_clara_root) instead of monkeypatching private helpers —
    ...
    CLARA_HOME=<root>/profiles/<name> makes get_default_clara_root()
