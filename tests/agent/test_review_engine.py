...
def test_load_review_credentials_cfg_missing_section(monkeypatch):
    monkeypatch.setattr(
        "clara_cli.config.load_config_readonly", lambda: {"auxiliary": {}}
    )
    assert re_mod._load_review_credentials_cfg() is None
...
def test_collect_skills_from_preloaded_prompt_and_history():
    ...
    parent.ephemeral_system_prompt = (
        ...
        '"clara-agent-dev" skill preloaded. Treat its instructions as '
    ...
    msgs = [
        {"role": "assistant", "content": "", "tool_calls": [
            ...
            {"function": {"name": "skill_view",
                          "arguments": '{"name": "clara-agent-dev", '
    ...
    assert names == ["clara-agent-dev", "github-pr-workflow"]
...
def test_briefing_includes_loaded_skills_instruction():
    ...
    _, context = build_review_task(snap, "", ["clara-agent-dev", "xitter"])
    assert "clara-agent-dev, xitter" in context
...
def test_start_review_threads_loaded_skills_into_context(monkeypatch):
    ...
    parent = _fake_parent()
    parent.ephemeral_system_prompt = (
        'session with the "clara-agent-dev" skill preloaded.'
    )
    msgs = [
    ...
    assert result["status"] == "dispatched"
    assert "clara-agent-dev" in built["context"]
...
def test_format_dispatch_note_sync_fallback():
    ...
    assert "__PROT_0_synchroclaraly__" in note
