...
``clara_subprocess_env`` / ``_sanitize_subprocess_env``, which the factory
...
``CLARA_HOME`` or secret-scrub propagation.
...
SCAN_DIRS = ("agent", "clara_cli", "tools", "gateway", "cron", "tui_gateway")
SCAN_ROOT_FILES = ("cli.py", "clara_constants.py")
...
ALLOWED_RAW_SPAWN_ENV_FILES = {
    # THE owner module: _sanitize_subprocess_env / clara_subprocess_env /
