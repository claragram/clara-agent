...
def test_reconcile_absorbed_into_beats_everything_else(curator_env):
    ...
    out = curator_env._reconcile_classification(
        ...
        destinations={"clara-agent-dev"},
        absorbed_declarations={
            "pr-review-format": {"into": "clara-agent-dev", "declared": True},
        },
    )
...
# `clara curator status`). The full data has always been in REPORT.md on
...
# ---------------------------------------------------------------------------
# Pin hint — surfaces `clara curator pin <umbrella>` in the rename block so
