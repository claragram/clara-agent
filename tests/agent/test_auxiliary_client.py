...
from agent.auxiliary_client import (
    ...
    _refresh_clara_recommended_model,
...
def _clean_env(monkeypatch):
    ...
    for key in (
        ...
        "OPENAI_MODEL", "LLM_MODEL", "CLARA_INFERENCE_BASE_URL",
...
class TestResolveTaskProviderModel:
    ...
    def test_explicit_provider_moa_unwraps_to_aggregator(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "clara_cli.moa_config.resolve_moa_preset",
            lambda cfg, name: preset,
        )
        monkeypatch.setattr("clara_cli.config.load_config", lambda: {"moa": {}})
        monkeypatch.setattr("clara_cli.config.load_config_readonly", lambda: {"moa": {}})
...
class TestMoaAggregatorSharedResolution:
    ...
    Real-config tests: write an actual config.yaml under a temp CLARA_HOME
                    ...
                    "moa": {
                        ...
                        "presets": {
                            ...
                            "clara-mix": {
                                "enabled": True,
                                "reference_models": [
                                    {"provider": "clara", "model": "clara-4-70b"}
                                ],
                                "aggregator": {
                                    "provider": "clara",
                                    "model": "clara-4-405b",
...
class TestBuildCallKwargsMaxTokens:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("clara", "clara-4", "https://inference-api.claraship.com/v1", "max_tokens"),
        ],
    )
...
class TestClaraTagsScoping:
    def test_tags_injected_when_provider_is_clara(self, monkeypatch):
        import agent.auxiliary_client as aux

        monkeypatch.setattr(aux, "auxiliary_is_clara", False)
        ...
        assert kwargs["extra_body"]["tags"] == aux._clara_portal_tags()

    def test_tags_not_injected_for_gemini_when_main_is_clara(self, monkeypatch):
        import agent.auxiliary_client as aux

        monkeypatch.setattr(aux, "auxiliary_is_clara", True)
...
class TestResolveXaiOAuthForAux:
    def test_uses_pool_backed_credentials_without_singleton(self, tmp_path, monkeypatch):
        ...
        ``clara auth status`` already reports these as logged in; compression
        ...
        monkeypatch.setenv("CLARA_HOME", str(clara_home))
        monkeypatch.delenv("CLARA_XAI_BASE_URL", raising=False)
    ...
    def test_pool_backed_credentials_honor_base_url_env_override(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CLARA_HOME", str(clara_home))
        monkeypatch.setenv("CLARA_XAI_BASE_URL", "https://example.x.ai/v1/")
...
class TestResolveProviderClientUniversalModelFallback:
    ...
    provider was picked via ``clara model``, no per-task override is
    ...
    fallback chain (OpenRouter / Clara / etc.), so aux tasks billed
...
class TestOpenRouterPaidLaneGuard:
    ...
    def test_free_only_allows_builtin_default_model(self, monkeypatch):
        ...
        with patch("agent.auxiliary_client._select_pool_entry", return_value=(False, None)), \
             patch("clara_cli.config.load_config_readonly", return_value={"auxiliary": {"free_only": True}}), \
             patch("agent.auxiliary_client.OpenAI") as mock_openai:
            mock_client = MagicMock(name="openrouter_client")
            mock_openai.return_value = mock_client
            client, model = _try_openrouter()
    ...
    def test_free_only_skips_paid_configured_model(self, monkeypatch):
        ...
        monkeypatch.setenv("OPENROUTER_API_KEY", "or-key")
        with patch("agent.auxiliary_client._select_pool_entry", return_value=(False, None)), \
             patch("clara_cli.config.load_config_readonly",
    ...
    def test_paid_lane_warns_once(self, monkeypatch, caplog):
        ...
        monkeypatch.setenv("OPENROUTER_API_KEY", "or-key")
        with patch("agent.auxiliary_client._select_pool_entry", return_value=(False, None)), \
             patch("clara_cli.config.load_config_readonly", return_value=_paid_cfg), \
             patch("agent.auxiliary_client.OpenAI") as mock_openai:
            mock_client = MagicMock(name="openrouter_client")
            mock_openai.return_value = mock_client
...
class TestGetTextAuxiliaryClient:
    ...
    def test_codex_pool_entry_takes_priority_over_auth_store(self):
        ...
        with (
            ...
            patch("clara_cli.auth._read_codex_tokens", side_effect=AssertionError("legacy codex store should not run")),
        ):
            from agent.auxiliary_client import _build_codex_client
    ...
    def test_returns_none_when_nothing_available(self, monkeypatch):
        ...
        with patch("agent.auxiliary_client._read_clara_auth", return_value=None), \
    ...
    def test_custom_endpoint_uses_codex_wrapper_when_runtime_requests_responses_api(self):
        with patch("agent.auxiliary_client._resolve_custom_runtime",
             ...
             patch("agent.auxiliary_client._read_clara_auth", return_value=None), \
             patch("agent.auxiliary_client._resolve_clara_runtime_api", return_value=None), \
...
class TestAuxiliaryPoolAwareness:
    ...
    def test_try_clara_refreshes_stale_pool_entry(self):
        ...
        with (
            patch("agent.auxiliary_client.load_pool", return_value=pool),
            patch("agent.auxiliary_client.OpenAI") as mock_openai,
            patch("clara_cli.models.get_clara_recommended_aux_model", return_value=None),
        ):
            from agent.auxiliary_client import _try_clara

            client, model = _try_clara()

        assert pool.refreshed is True
        assert client is not None
        assert model == _CLARA_MODEL
    ...
    def test_call_llm_retries_clara_after_401(self):
        class _Auth401(Exception):
            status_code = 401

        stale_client = MagicMock()
        stale_client.base_url = "https://inference-api.claraship.com/v1"
        stale_client.chat.completions.create.side_effect = _Auth401("stale clara key")

        fresh_client = MagicMock()
        fresh_client.base_url = "https://inference-api.claraship.com/v1"
        fresh_client.chat.completions.create.return_value = {"ok": True}

        with (
            patch("agent.auxiliary_client._resolve_task_provider_model", return_value=("clara", "clara-model", None, None, None)),
            ...
            patch("agent.auxiliary_client._resolve_clara_runtime_api", return_value=("fresh-agent-key", "https://inference-api.claraship.com/v1")),
        ):
            result = call_llm(
                task="compression",
                messages=[{"role": "user", "content": "hi"}],
...
class TestIsModelNotFoundError:
    ...
    def test_clara_openrouter_catalog_404(self):
        """The exact incident error: a Portal-recommended model dropped from
        the Clara → OpenRouter catalog."""
        exc = Exception(
...
class TestRefreshClaraRecommendedModel:
    """_refresh_clara_recommended_model picks a fresh model after a stale 404."""
    ...
    def test_falls_back_to_default_when_portal_unavailable(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "clara_cli.models.get_clara_recommended_aux_model", _boom)
        out = _refresh_clara_recommended_model(
            vision=False, stale_model="some/dead-model")
        assert out == _CLARA_MODEL
    ...
    def test_returns_none_when_no_distinct_alternative(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "clara_cli.models.get_clara_recommended_aux_model",
            lambda **kw: _CLARA_MODEL,
        )
        out = _refresh_clara_recommended_model(
            vision=False, stale_model=_CLARA_MODEL)
        assert out is None
...
class TestGetProviderChain:
    ...
    def test_returns_four_entries(self):
        ...
        assert labels == ["openrouter", "clara", "local/custom", "api-key"]
...
class TestTryPaymentFallback:
    ...
    def test_skips_failed_provider(self):
        ...
        with patch("agent.auxiliary_client._try_openrouter", return_value=(None, None)), \
             patch("agent.auxiliary_client._try_clara", return_value=(mock_client, "clara-model")), \
             patch("agent.auxiliary_client._read_main_provider", return_value="openrouter"):
            client, model, label = _try_payment_fallback("openrouter", task="compression")
        assert client is mock_client
        assert model == "clara-model"
        assert label == "clara"
    ...
    def test_codex_not_in_fallback_chain(self):
        ...
        When OR/Clara/custom/api-key all fail, payment-fallback returns None —
        Codex is never tried with a guessed model.
        """
        with patch("agent.auxiliary_client._try_openrouter", return_value=(None, None)), \
             patch("agent.auxiliary_client._try_clara", return_value=(None, None)), \
...
class TestAuxiliaryFallbackLayering:
    ...
    def test_fallback_entry_openai_codex_uses_oauth_pool_without_inline_key(self):
        """Configured Codex fallback resolves through Clara auth / credential pool."""
...
def test_resolve_api_key_provider_skips_unconfigured_anthropic(monkeypatch):
    ...
    from clara_cli.auth import ProviderConfig
    ...
    monkeypatch.setattr("clara_cli.auth.PROVIDER_REGISTRY", fake_registry)
    monkeypatch.setattr(
...
class TestAuxClientNoSdkRetries:
    ...
    disabled so Clara owns the retry/timeout budget (issue #54465). The SDK
...
class TestAuxiliaryTaskExtraBody:
    def test_sync_call_merges_task_extra_body_from_config(self):
        ...
        with patch("clara_cli.config.load_config", return_value=config), patch("clara_cli.config.load_config_readonly", return_value=config), patch(
            "agent.auxiliary_client._get_cached_client",
    ...
    def test_moa_tasks_reject_task_level_reasoning_effort(self, moa_task, caplog):
        ...
        with patch("clara_cli.config.load_config", return_value=config), patch("clara_cli.config.load_config_readonly", return_value=config), \
             caplog.at_level(logging.WARNING, logger="agent.auxiliary_client"):
...
class TestAnthropicAuxiliaryReasoningTranslation:
    """Native Anthropic aux adapters must receive normalized Clara reasoning.

    MoA slot reasoning is carried through call_llm as a Clara
...
class TestAuxiliaryClientPoisonedCacheEviction:
    ...
    See https://github.com/claraship/clara-agent/issues/23432.
    """
...
class TestBuildCallKwargsToolDedup:
    ...
    regresses.  See: https://github.com/claraship/clara-agent/issues/18478
    """
...
class TestNvidiaBillingHeaders:
    ...
    def test_resolve_provider_client_cloud_adds_billing_origin_header(self, monkeypatch):
        ...
        assert headers["X-BILLING-INVOKE-ORIGIN"] == "ClaraAgent"
...
def test_pool_runtime_base_url_uses_clara_env_override(monkeypatch):
    entry = SimpleNamespace(
        provider="clara",
        runtime_base_url="https://inference-api.claraship.com/v1",
...
class TestAuxUnhealthyCache:
    ...
    def test_call_llm_marks_provider_unhealthy_on_402(self, monkeypatch):
        ...
        clara_client = MagicMock()
        clara_resp = MagicMock()
        clara_resp.choices = [MagicMock(message=MagicMock(content="ok"))]
        clara_client.chat.completions.create.return_value = clara_resp
        ...
        with patch("agent.auxiliary_client._get_cached_client",
             ...
             patch("agent.auxiliary_client._try_payment_fallback",
                    return_value=(clara_client, "n-model", "clara")), \
...
class TestCustomEndpointApiKeyInheritance:
    ...
    def test_inherits_main_api_key_when_aux_key_empty(self, monkeypatch):
        ...
        with patch("clara_cli.config.load_config", return_value=fake_config), patch("clara_cli.config.load_config_readonly", return_value=fake_config), \
    ...
    def test_runtime_override_key_is_used(self, monkeypatch):
        ...
        with patch.object(ac, "_RUNTIME_MAIN_API_KEY", "sk-runtime-key"), \
             ...
             patch("clara_cli.config.load_config", return_value={"model": {}}), patch("clara_cli.config.load_config_readonly", return_value={"model": {}}), \
             patch.object(ac, "_create_openai_client", side_effect=_capture_create):
            client, model = resolve_provider_client(
                "custom",
                model="test-model",
...
class TestSynchroclaraFallbackCachePlans:
    @staticmethod
...
class TestAsynchroclaraFallbackCachePlans:
    @pytest.mark.asyncio
...
class TestFastModelTier:
    ...
    def test_catalog_match_prefers_rolling_alias_over_pinned_id(self):
        ...
        with patch("clara_cli.models.fetch_models_with_pricing", return_value=catalog):
            assert ac._fast_model_from_catalog("clara") == "~openai/gpt-mini-latest"
    ...
    def test_catalog_match_skips_reasoning_batch_and_embedding_lookalikes(self):
        ...
        with patch("clara_cli.models.fetch_models_with_pricing", return_value=catalog):
            assert ac._fast_model_from_catalog("clara") == "google/gemini-3.6-flash"
    ...
    def test_catalog_match_skips_the_non_chat_siblings_of_a_chat_model(self):
        ...
        with patch("clara_cli.models.fetch_models_with_pricing", return_value=catalog):
            assert ac._fast_model_from_catalog("clara") == "openai/gpt-4o-mini"
    ...
    def test_catalog_match_takes_the_newest_of_a_family(self):
        ...
        with patch("clara_cli.models.fetch_models_with_pricing", return_value=catalog):
            assert ac._fast_model_from_catalog("clara") == "openai/gpt-10-mini"
    ...
    def test_catalog_fetch_is_authenticated(self):
        ...
        with patch(
            "clara_cli.auth.resolve_api_key_provider_credentials",
            return_value={"api_key": "sk-test", "base_url": "https://api.example.com/v1"},
        ), patch(
            "clara_cli.models.fetch_models_with_pricing", return_value={}
        ) as fetch:
    ...
    def test_fast_tier_is_opt_in(self):
        ...
        with patch.object(ac, "_fast_model_from_catalog") as spy:
            ac._get_aux_model_for_provider("clara")
