...
``~/.clara/.anthropic_oauth.json`` for ``clara_pkce`` ones.  Those singletons
...
def clara_home(tmp_path, monkeypatch):
    """Real on-disk CLARA_HOME so ``load_pool()`` re-reads what we persisted."""
    home = tmp_path / "clara"
    ...
    monkeypatch.setattr(
        "clara_cli.auth.is_provider_explicitly_configured", lambda pid: True
    )
    return home
...
def test_clara_oauth_writer_raises_instead_of_swallowing(clara_home, monkeypatch):
    ...
    with pytest.raises(CredentialPersistError):
        AA._write_clara_oauth_credentials(
            _ROTATED_ACCESS, _ROTATED_REFRESH, _EXPIRED_MS + 3_600_000
        )
...
def test_pool_claude_code_fails_closed_and_reload_cannot_resurrect(
    clara_home, claude_credentials, monkeypatch
):
    monkeypatch.setattr(AA, "refresh_anthropic_oauth_pure", _rotating_refresh)
    _break_durable_write(monkeypatch)
...
# ---------------------------------------------------------------------------
# Pool path: clara_pkce
# ---------------------------------------------------------------------------


def test_pool_clara_pkce_fails_closed_and_reload_cannot_resurrect(
    ...
    reloaded = [
        e for e in load_pool("anthropic").entries() if e.source == "clara_pkce"
    ]
    assert reloaded
    assert reloaded[0].refresh_token == _STALE_REFRESH
    assert reloaded[0].last_status == STATUS_DEAD
