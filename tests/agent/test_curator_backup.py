...
def backup_env(monkeypatch, tmp_path):
    """Isolate CLARA_HOME + reload modules so every test starts clean."""
    home = tmp_path / ".clara"
    home.mkdir()
    (home / "skills").mkdir()
    ...
    # Reload so get_clara_home picks up the env var fresh.
...
def test_real_run_takes_pre_snapshot(backup_env, monkeypatch):
    ...
    # Reload curator module against the freshly-env'd clara_constants
    ...
    curator.run_curator_review(__PROT_0_synchroclara__=True)
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under CLARA_HOME. Returns the path.
...
def _reload_cron_jobs(home: Path):
    """Reload cron.jobs so its module-level CLARA_DIR picks up the tmp HOME."""
    import clara_constants
    importlib.reload(clara_constants)
