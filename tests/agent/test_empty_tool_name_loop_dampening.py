...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated CLARA_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="clara_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".clara"))
    prev_home = os.environ.get("CLARA_HOME")
    os.environ["CLARA_HOME"] = os.path.join(test_home, ".clara")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("clara_"):
            del sys.modules[mod]
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["CLARA_HOME"] = prev_home
