"""Tests for the Clara-credits subscription % gauge in build_clara_credits_snapshot.
...
from clara_cli.clara_account import (
    ...
    ClaraPaidServiceAccessInfo,
    ClaraPortalSubscriptionInfo,
    _subscription_from_payload,
)
from agent.account_usage import build_clara_credits_snapshot, render_account_usage_lines
...
def _acct(**kwargs):
    ...
    kwargs.setdefault("portal_base_url", "https://portal.claraprise.com")
...
def test_gauge_present_with_monthly_credits():
    snap = build_clara_credits_snapshot(_acct(
        paid_service_access=True,
        subscription=ClaraPortalSubscriptionInfo(
...
def test_nan_remaining_no_window_no_nan_string():
    ...
    snap = build_clara_credits_snapshot(_acct(
        paid_service_access=True,
        subscription=ClaraPortalSubscriptionInfo(monthly_credits=220, credits_remaining=float("nan")),
        paid_service_access_info=ClaraPaidServiceAccessInfo(purchased_credits_remaining=5.0),
    ))
...
def test_failopen_none_and_logged_out():
    assert build_clara_credits_snapshot(None) is None
    assert build_clara_credits_snapshot(_acct(logged_in=False)) is None
