...
Prior to this change, aggregator users (OpenRouter / Clara Portal) had aux
...
class TestResolveVisionMainFirst:
    ...
    @staticmethod
    def _stub_clara_portal(seen: dict):
        """Stub the Clara network boundary, keeping the resolution chain real.

        Returns a ``_try_clara`` replacement that answers with the Portal's
        tier-aware slots: a vision model for ``vision=True``, the text chat
        default otherwise.
        """
        clara_client = MagicMock()
        clara_client.api_key = "jwt-test"
        clara_client.base_url = "https://inference-api.claraprise.com/v1"
        ...
        return clara_client, fake_try_clara

    def test_clara_main_vision_uses_portal_pick_not_text_chat_model(self):
        """Clara main → vision runs the Portal's vision slot, not the chat model.

        A Clara chat default is routinely text-only (e.g. a ``:free`` chat SKU).
        Letting it reach the vision lane means the image goes to a model that
        cannot accept one and the Portal 404s. Only the Clara network boundary
        ...
        """
        seen: dict = {}
        clara_client, fake_try_clara = self._stub_clara_portal(seen)

        with patch(
            "agent.auxiliary_client._read_main_provider", return_value="clara",
        ), patch(
            ...
            "agent.auxiliary_client._try_clara", side_effect=fake_try_clara,
        ):
            from agent.auxiliary_client import resolve_vision_provider_client

            provider, client, model = resolve_vision_provider_client()

        assert provider == "clara"
        assert client is clara_client
        assert seen["vision"] is True
        assert model == "stepfun/step-3.7-flash:free"

    def test_clara_main_vision_honours_explicit_vision_model(self):
        """An explicit auxiliary.vision.model still overrides the Portal pick."""
        seen: dict = {}
        _clara_client, fake_try_clara = self._stub_clara_portal(seen)
    ...
    def test_clara_explicit_vision_provider_also_skips_chat_model(self):
        """``auxiliary.vision.provider: clara`` takes the same Portal pick.
        ...
        with patch(
            ...
            "agent.auxiliary_client._resolve_task_provider_model",
            return_value=("clara", None, None, None, None),
        ), patch(
            "agent.auxiliary_client._try_clara", side_effect=fake_try_clara,
        ):
            from agent.auxiliary_client import resolve_vision_provider_client
    ...
    def test_clara_text_aux_still_uses_main_chat_model(self):
        """The vision carve-out must not leak into text aux resolution.

        Text auxiliary work on a Clara main deliberately keeps the user's chat
        ...
        with patch(
            ...
            _client, model = resolve_provider_client("clara")
    ...
    def test_copilot_vision_sets_vision_header(self, monkeypatch):
        ...
        with patch(
            ...
            "clara_cli.copilot_auth.copilot_request_headers",
...
class TestResolveVisionCustomProvider:
    ...
    it returns ``(None, None)`` and vision falls through to OpenRouter / Clara,
