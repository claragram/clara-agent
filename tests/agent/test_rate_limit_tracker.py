...
# ── Sample headers from Clara inference API ──────────────────────────────
...
class TestParseHeaders:
    def test_basic_parsing(self):
        state = parse_rate_limit_headers(CLARA_HEADERS, provider="clara")
        assert state is not None
        assert state.provider == "clara"
...
class TestAgentIntegration:
    ...
    def test_capture_rate_limits_from_headers(self):
        ...
        state = parse_rate_limit_headers(MockResponse.headers, provider="clara")
        assert state is not None
        assert state.requests_min.limit == 800
        assert state.tokens_hour.limit == 336000000
