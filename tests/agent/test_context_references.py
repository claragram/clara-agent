...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "Clara Tests")
...
def test_binary_reference_block_maps_host_attachment_to_container_path(tmp_path: Path, monkeypatch):
    ...
    attachments = clara_home / "attachments"
    ...
    assert result.expanded
    # Default container base for the docker backend is /root/.clara.
    assert "/root/.clara/attachments/archive.zip" in result.message
...
def test_binary_reference_block_keeps_host_path_on_local_backend(tmp_path: Path, monkeypatch):
    ...
    assert "/root/.clara/attachments/" not in result.message
...
async def test_blocks_canonical_read_denylist_credential_stores(tmp_path: Path, monkeypatch):
    ...
    a chat peer could otherwise attach `@file:~/.clara/auth.json` and read the
    ...
    oauth = clara_home / ".anthropic_oauth.json"
    oauth.write_text('{"access_token": "OAUTH-SECRET"}\n', encoding="utf-8")

    mcp_token = clara_home / "mcp-tokens" / "github.json"
    ...
    result = await preprocess_context_references_async(
        "inspect @file:.clara/auth.json and @file:.clara/.anthropic_oauth.json "
        "and @file:.clara/mcp-tokens/github.json and @file:project/.env",
        cwd=tmp_path,
...
@pytest.mark.parametrize(
    ...
    [
        "/tmp/plain.png",
        "/Users/me/Library/Application Support/Clara/composer-images/a.png",
