...
pytestmark = pytest.mark.skipif(
    os.environ.get("CLARA_RUN_SLOW_PET_TESTS") != "1",
    reason=(
        "pet generation image-processing suite is opt-in; run with "
        "CLARA_RUN_SLOW_PET_TESTS=1 scripts/run_tests.sh tests/agent/test_pet_generate.py"
    ),
)
...
def test_list_sprite_providers_marks_default(monkeypatch):
    ...
    registry = {"openai": _FakeImgProvider("openai"), "clara": _FakeImgProvider("clara")}
    ...
    # Listed in preference order: Clara Portal before OpenAI.
    assert [p["name"] for p in listed] == ["clara", "openai"]
