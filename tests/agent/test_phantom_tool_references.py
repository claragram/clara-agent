...
Covers:
  * CLARA_AGENT_HELP_GUIDANCE degrades to the docs-only variant when the
...
class TestClaraAgentHelpGuidance:
    def test_skill_variant_used_when_skill_view_present(self):
        from agent.prompt_builder import CLARA_AGENT_HELP_GUIDANCE
        assert "skill_view(name='clara-agent')" in CLARA_AGENT_HELP_GUIDANCE
    ...
    def test_no_skills_variant_has_no_skill_view_reference(self):
        ...
        assert "agent.claraship.com/docs" in CLARA_AGENT_HELP_GUIDANCE_NO_SKILLS
...
class TestEssentialSkillsUndisableable:
    def test_agent_side_reader_strips_essential(self, monkeypatch, tmp_path):
        ...
        assert "clara-agent" not in disabled
...
class TestEssentialOnlySync:
    def test_opted_out_sync_seeds_only_essential(self, monkeypatch, tmp_path):
        """A profile with .no-bundled-skills still gets the clara-agent skill."""
        ...
        for cat, name in [
            ("autonomous-ai-agents", "clara-agent"),
            ("media", "gif-search"),
        ]:
            d = bundled / cat / name
        ...
        monkeypatch.setattr(ss, "_clara_home", lambda: home)
        ...
        assert result["copied"] == ["clara-agent"]
        assert (home / "skills" / "autonomous-ai-agents" / "clara-agent" / "SKILL.md").exists()
