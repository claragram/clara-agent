...
def isolated_home(monkeypatch):
    """Temp CLARA_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="clara_test_31179_")
    clara_home = os.path.join(test_home, ".clara")
...
def _fresh_modules():
    """Drop cached clara modules so each test reloads against current env."""
    _drop_reload_targets()
