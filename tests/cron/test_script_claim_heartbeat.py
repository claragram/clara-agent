...
def test_cancel_event_terminates_script_process_tree(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(scheduler, "_get_clara_home", lambda: tmp_path)
...
def test_long_running_script_refreshes_owned_claim_in_profile_store(
    ...
    monkeypatch.setattr(jobs, "_clara_now", lambda: current_time[0])
    ...
    with (
        ...
        patch("clara_state.get_shared_session_db", return_value=MagicMock()),
    ):
