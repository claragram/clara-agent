...
clara/claude-fable-5), the job would silently inherit it on its next tick and
...
resolve_runtime_provider against a temp CLARA_HOME) and the create_job
...
class TestProviderDriftGuard:
    ...
    def test_b_unpinned_snapshot_differs_fails_closed(self, tmp_path):
        ...
        job = _base_job(provider_snapshot="openrouter")
        success, output, final_response, error, agent_constructed = \
            _run_with_current_provider(job, "clara", tmp_path)
        ...
        assert "clara" in blob
        assert "spend" in blob
        assert "clara cron edit pin-test --provider <provider> --model <model>" in blob
        ...
        assert "host running clara" in delivered
        assert "clara cron edit pin-test --provider <provider> --model <model>" in delivered
    ...
    def test_missing_model_guides_to_user_owned_cli(self, tmp_path, monkeypatch):
        ...
        assert agent_constructed is False
        assert error is not None
        assert "clara cron edit pin-test --model <name>" in error
...
class TestCreateJobSnapshot:
    ...
    def test_pinned_job_skips_snapshot(self, monkeypatch):
        ...
        with patch("clara_cli.runtime_provider.resolve_runtime_provider", resolver):
            job = jobs.create_job(
                prompt="do a thing", schedule="every 1 hour", provider="clara"
            )

        # Explicit provider → pinned → no snapshot needed, and resolution skipped.
        assert job["provider"] == "clara"
...
def _run_with_current_provider_and_model(
    ...
    *,
    ...
    fake_db = MagicMock()
    with patch("cron.scheduler._clara_home", tmp_path), \
         patch("cron.scheduler._get_clara_home", return_value=tmp_path), \
         patch("cron.scheduler._resolve_origin", return_value=None), \
         patch("clara_cli.env_loader.load_clara_dotenv"), \
         patch("clara_cli.env_loader.reset_secret_source_cache"), \
         patch("clara_state.get_shared_session_db", return_value=fake_db), \
