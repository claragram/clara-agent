...
the shared /blueprint command handler (clara_cli/blueprint_cmd.py), and
the docs generator. Uses an isolated CLARA_HOME for anything that touches the
cron job store.
"""
...
class TestRenderers:
    ...
    def test_deeplink_shape(self):
        ...
        assert url.startswith("clara://blueprint/morning-brief?")
