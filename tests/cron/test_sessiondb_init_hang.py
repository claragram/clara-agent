...
which is already bounded by CLARA_CRON_TIMEOUT — the worker thread submitted
...
timeout (CLARA_CRON_SESSION_DB_TIMEOUT, default 10s) so a hang there can
...
class TestSessionDbInitTimeout:
    def test_sessiondb_init_preserves_multiplex_profile_context(
        ...
        profile_token = set_clara_home_override(profile_home)
        try:
            with patch("cron.scheduler._clara_home", None), \
                 patch("cron.scheduler._resolve_origin", return_value=None), \
                 patch("clara_cli.env_loader.load_clara_dotenv"), \
                 patch("clara_cli.env_loader.reset_secret_source_cache"), \
                 patch("clara_state.get_shared_session_db", side_effect=make_session_db), \
                 patch(
            ...
            reset_clara_home_override(profile_token)
    ...
    def test_run_job_does_not_hang_when_sessiondb_init_wedges(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CLARA_CRON_SESSION_DB_TIMEOUT", "0.2")
        ...
        with patch("cron.scheduler._clara_home", tmp_path), \
             ...
             patch("clara_cli.env_loader.load_clara_dotenv"), \
             patch("clara_cli.env_loader.reset_secret_source_cache"), \
             patch("clara_state.get_shared_session_db"), \
             patch(
                 "clara_cli.runtime_provider.resolve_runtime_provider",
                 return_value=_RUNTIME,
             ), \
    ...
    def test_invalid_timeout_env_falls_back_to_default(self, tmp_path, monkeypatch, caplog):
        """A malformed CLARA_CRON_SESSION_DB_TIMEOUT logs a warning and still
        bounds the call (mirrors CLARA_CRON_TIMEOUT's own fallback)."""
        monkeypatch.setenv("CLARA_CRON_SESSION_DB_TIMEOUT", "not-a-number")
        fake_db = MagicMock()
        ...
        assert any(
            "CLARA_CRON_SESSION_DB_TIMEOUT" in rec.message
    ...
    def test_timeout_resolved_from_config_yaml(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("CLARA_CRON_SESSION_DB_TIMEOUT", raising=False)
    ...
    def test_late_session_db_result_is_closed(self, tmp_path, monkeypatch):
        ...
        try:
            with patch("cron.scheduler._clara_home", tmp_path), \
                 ...
                 patch("clara_cli.env_loader.load_clara_dotenv"), \
                 patch("clara_cli.env_loader.reset_secret_source_cache"), \
                 patch("clara_state.get_shared_session_db", side_effect=_hanging_then_capture), \
                 patch(
                     "clara_cli.runtime_provider.resolve_runtime_provider",
                     return_value={
                         "api_key": "test-key",
...
class TestSessionDbInitAfterEarlyReturns:
    ...
    def test_wake_gate_false_never_opens_session_db(self, tmp_path, monkeypatch):
        ...
        with patch("cron.scheduler._clara_home", tmp_path), \
             ...
             patch("clara_cli.env_loader.load_clara_dotenv"), \
             patch("clara_cli.env_loader.reset_secret_source_cache"), \
             patch("clara_state.get_shared_session_db") as mock_db_cls, \
             patch(
