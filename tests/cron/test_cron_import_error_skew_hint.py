...
Field incident: an interrupted `clara update` (pull done, restart never ran)
...
had no way to know the fix was one `clara gateway restart`. The failure
...
def test_import_error_without_skew_stays_a_plain_import_message(monkeypatch):
    ...
    assert "clara gateway restart" not in msg
