...
A one-shot ``clara cron run <job_id>`` used to background-dispatch the run
...
1. ``clara_cli.cron._job_action("run", ...)`` declares the channel stateless
   before invoking the cron API, so the background-dispatch path is gated off
   and the run executes __PROT_1_synchroclaraly__ to completion in the CLI process.
...
def _orphan_claimed_row(executions, job_id: str) -> str:
    ...
    Mirrors what a one-shot ``clara cron run`` leaves behind: a row stuck in
...
class TestOneShotCliRunIsSynchroclara:
    @pytest.fixture(autouse=True)
    ...
    def test_cli_run_declares_stateless_channel_before_dispatch(self, monkeypatch):
        """`clara cron run` must gate off async delivery so the run executes
        __PROT_2_synchroclaraly__ in the CLI process instead of on a doomed daemon thread."""
    ...
    def test_background_dispatch_refused_when_channel_stateless(self, monkeypatch):
        ...
        tool's background dispatcher must fall back to __PROT_0_synchroclara__ execution
        ...
        monkeypatch.setenv("CLARA_SESSION_KEY", "inherited-gateway-session")
