...
def cron_store(tmp_path, monkeypatch):
    ...
    (clara_home / "cron").mkdir(parents=True)
    monkeypatch.setenv("CLARA_HOME", str(clara_home))
    monkeypatch.setattr(J, "CLARA_DIR", clara_home)
    monkeypatch.setattr(J, "CRON_DIR", clara_home / "cron")
    monkeypatch.setattr(J, "JOBS_FILE", clara_home / "cron" / "jobs.json")
    monkeypatch.setattr(J, "OUTPUT_DIR", clara_home / "cron" / "output")
...
class TestCronRearmRespectsScheduleLegality:
    def test_weekday_job_errored_friday_does_not_fire_on_saturday(self, cron_store):
        ...
        with mock.patch.object(J, "_clara_now", lambda: now):
            due = J.get_due_jobs()
