...
Uses a throwaway CLARA_HOME so it never touches ~/.clara. No real names.
"""
...
def _fresh_home():
    """Point CLARA_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_clara_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["CLARA_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER CLARA_HOME is set.
...
# a different get_clara_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"CLARA_HOME (throwaway): {HOME}")
