...
def test_provider_flag_uses_named_custom_default_model(monkeypatch):
    ...
    monkeypatch.setitem(
        cli.CLI_CONFIG,
        "model",
        {"default": "tencent/hy3:free", "provider": "clara"},
    )
    config = {
        "model": {"default": "tencent/hy3:free", "provider": "clara"},
        "providers": {
            "gmk-lan": {
                "name": "GMK Local",
    ...
    monkeypatch.setattr("clara_cli.config.load_config", lambda: config)
    monkeypatch.setattr("clara_cli.runtime_provider.load_config", lambda: config)

    shell = cli.ClaraCLI(provider="gmk-lan", compact=True, max_turns=1)
...
def test_provider_flag_logs_when_custom_default_model_cannot_resolve(monkeypatch, caplog):
    ...
    monkeypatch.setattr(
        "clara_cli.runtime_provider._get_named_custom_provider",
        _boom,
    )
...
def test_clara_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    cli = _import_cli()
    calls = {"count": 0}

    def _unexpected_runtime_resolve(**kwargs):
        calls["count"] += 1
        raise AssertionError("resolve_runtime_provider should not be called in ClaraCLI.__init__")

    monkeypatch.setattr("clara_cli.runtime_provider.resolve_runtime_provider", _unexpected_runtime_resolve)
    monkeypatch.setattr("clara_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))

    shell = cli.ClaraCLI(model="gpt-5", compact=True, max_turns=1)
...
def test_runtime_resolution_failure_is_not_sticky(monkeypatch):
    ...
    monkeypatch.setattr("clara_cli.runtime_provider.resolve_runtime_provider", _runtime_resolve)
    monkeypatch.setattr("clara_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
...
def test_model_flow_clara_does_not_restore_stale_custom_api_key(tmp_path, monkeypatch):
    import yaml

    config_home = tmp_path / "clara"
    config_home.mkdir()
    monkeypatch.setenv("CLARA_HOME", str(config_home))
    ...
    monkeypatch.setattr(
        "clara_cli.auth.get_provider_auth_state",
        lambda provider: {
            "access_token": "clara-token",
    ...
    monkeypatch.setattr(
        "clara_cli.models.get_curated_clara_model_ids",
        lambda: [selected_model],
    )
    monkeypatch.setattr("clara_cli.models.get_pricing_for_provider", lambda provider: {})
    monkeypatch.setattr("clara_cli.models.check_clara_free_tier", lambda **kwargs: False)
    monkeypatch.setattr(
        "clara_cli.models.union_with_portal_paid_recommendations",
        lambda model_ids, pricing, portal_url: (model_ids, pricing),
    )
    monkeypatch.setattr(
        "clara_cli.auth._prompt_model_selection",
        lambda *args, **kwargs: selected_model,
    )
    monkeypatch.setattr(
        "clara_cli.clara_subscription.prompt_enable_tool_gateway",
        lambda config: None,
    )

    clara_main._model_flow_clara(stale_config, current_model="glm-5.2")

    config = yaml.safe_load(config_path.read_text()) or {}
    model = config.get("model")
    assert model["provider"] == "clara"
    assert model["default"] == selected_model
    assert model["base_url"] == "https://inference-api.claraprise.com/v1"
...
def test_codex_provider_uses_config_model(monkeypatch):
    ...
    monkeypatch.setattr(
        "clara_cli.codex_models.get_codex_model_ids",
        lambda access_token=None: ["gpt-5.2-codex"],
    )

    shell = cli.ClaraCLI(compact=True, max_turns=1)
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    ...
    saved_env = {}
    monkeypatch.setattr("clara_cli.config.save_env_value", lambda key, value: saved_env.__setitem__(key, value))
    monkeypatch.setattr("clara_cli.auth._save_model_choice", lambda model: saved_env.__setitem__("MODEL", model))
    monkeypatch.setattr("clara_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr("clara_cli.main._save_custom_provider", lambda *args, **kwargs: None)
    monkeypatch.setattr(
    ...
    monkeypatch.setattr("clara_cli.config.save_config", lambda cfg: None)
    ...
    monkeypatch.setattr("clara_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": next(answers))

    clara_main._model_flow_custom({})
...
def test_model_flow_custom_persists_selected_api_mode(monkeypatch):
    ...
    monkeypatch.setattr("clara_cli.auth._save_model_choice", lambda model: None)
    monkeypatch.setattr("clara_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr(
        "clara_cli.models.probe_api_models",
        lambda api_key, base_url: {
    ...
    saved_env = {}
    monkeypatch.setattr("clara_cli.config.load_config", lambda: saved_cfg)
    monkeypatch.setattr("clara_cli.config.save_config", lambda cfg: saved_cfg.update(cfg))
    monkeypatch.setattr(
        "clara_cli.config.save_env_value",
        lambda key, value: saved_env.__setitem__(key, value),
    )
    monkeypatch.setattr(
        "clara_cli.main._save_custom_provider",
    ...
    monkeypatch.setattr("builtins.input", lambda _prompt="": next(answers))
    monkeypatch.setattr("clara_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": "test-key")

    clara_main._model_flow_custom({"model": {"provider": "custom"}})
...
def test_cmd_model_forwards_clara_login_tls_options(monkeypatch):
    monkeypatch.setattr(clara_main, "_require_tty", lambda *a: None)
    monkeypatch.setattr(
        "clara_cli.config.load_config",
        lambda: {"model": {"default": "gpt-5", "provider": "clara"}},
    )
    monkeypatch.setattr("clara_cli.config.save_config", lambda cfg: None)
    monkeypatch.setattr("clara_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("clara_cli.config.save_env_value", lambda key, value: None)
    monkeypatch.setattr("clara_cli.auth.resolve_provider", lambda requested, **kwargs: "clara")
    monkeypatch.setattr("clara_cli.auth.get_provider_auth_state", lambda provider_id: None)
    monkeypatch.setattr(clara_main, "_prompt_provider_choice", lambda choices, **kwargs: 0)
    ...
    monkeypatch.setattr("clara_cli.auth._login_clara", _fake_login)

    clara_main.cmd_model(
        SimpleNamespace(
            portal_url="https://portal.claraprise.com",
            inference_url="https://inference.workprise.com/v1",
...
def test_auto_provider_name_localhost():
    from clara_cli.main import _auto_provider_name
...
def test_save_custom_provider_uses_provided_name(monkeypatch, tmp_path):
    ...
    import yaml
    from clara_cli.main import _save_custom_provider

    cfg_path = tmp_path / "config.yaml"
    cfg_path.write_text(yaml.dump({}))

    monkeypatch.setattr(
        "clara_cli.config.load_config", lambda: yaml.safe_load(cfg_path.read_text()) or {},
    )
    saved = {}
    def _save(cfg):
        saved.update(cfg)
    monkeypatch.setattr("clara_cli.config.save_config", _save)
...
def test_save_custom_provider_references_the_key_instead_of_inlining_it(monkeypatch, tmp_path):
    ...
    saved = {}
    monkeypatch.setattr("clara_cli.config.save_config", lambda cfg: saved.update(cfg))

    _save_custom_provider(
        "http://localhost:11434/v1",
        api_key="sk-secret",
        name="Ollama",
        key_env="CLARA_CUSTOM_LOCALHOST_11434_API_KEY",
    )

    entry = saved["custom_providers"][0]
    assert entry["key_env"] == "CLARA_CUSTOM_LOCALHOST_11434_API_KEY"
...
def test_custom_endpoint_key_env_is_a_valid_posix_name_for_ip_endpoints():
    ...
    from clara_cli.config import _ENV_VAR_NAME_RE, custom_endpoint_key_env
