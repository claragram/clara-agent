...
def _isolate(tmp_path, monkeypatch):
    """Isolate CLARA_HOME so tests don't touch real config."""
    home = tmp_path / ".clara"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal ClaraCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = ClaraCLI.__new__(ClaraCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["clara-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below Clara' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly Clara' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above Clara' minimum context length."""
