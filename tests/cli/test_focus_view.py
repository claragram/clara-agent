...
class TestStatusBarSegment:
    ...
    def test_text_renderer_includes_the_badge_at_every_width_tier(self, width):
        ...
        host = ClaraCLI.__new__(ClaraCLI)
        host.model = "opus"
        ...
        with patch.object(ClaraCLI, "_get_status_bar_snapshot", return_value=snapshot), \
             patch.object(ClaraCLI, "_is_session_yolo_active", return_value=False):
            text = ClaraCLI._build_status_bar_text(host, width=width)
