...
minimal ``ClaraCLI`` stub (pattern used elsewhere in tests/cli).
"""
...
def clara_home(tmp_path, monkeypatch):
    """Isolated CLARA_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".clara"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("CLARA_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves CLARA_HOME each test.
...
def _make_cli_with_goal(session_id: str, goal_text: str = "build a thing"):
    """Build a minimal ClaraCLI stub with an active goal wired in."""
...
class TestInterruptAutoPause:
    ...
    def test_interrupted_turn_is_resumable(self, clara_home):
        ...
        with patch("clara_cli.goals.judge_goal"):
...
class TestHealthyTurnStillRuns:
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, clara_home):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, clara_home):
        ...
        src = inspect.getsource(ClaraCLI.chat)
