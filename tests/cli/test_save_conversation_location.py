...
Regression: the old implementation wrote ``clara_conversation_<ts>.json``
to the current working directory (CWD). Users who ran /save expected the
file to be discoverable via ``clara sessions browse``, but CWD-resident
snapshots are not indexed in the state DB and are generally invisible.
The fix writes snapshots under ``~/.clara/sessions/saved/`` and prints
...
def clara_home(tmp_path, monkeypatch):
    ...
    # Clear any cached clara_home computation
    import clara_constants
    if hasattr(clara_constants, "_clara_home_cache"):
        clara_constants._clara_home_cache = None
    return home
...
def test_save_conversation_writes_under_clara_home(clara_home, tmp_path, monkeypatch, capsys):
    """Snapshot must land under ~/.clara/sessions/saved/, not CWD."""
    # Change CWD to a different directory to prove the file does NOT go there.
    work = tmp_path / "somewhere-else"
    work.mkdir()
    monkeypatch.chdir(work)

    # Import fresh to pick up the CLARA_HOME fixture
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "clara_constants"]:
    ...
    cli.ClaraCLI.save_conversation(stub, "/save json")

    # File must NOT be in CWD
    cwd_leak = list(work.glob("clara_conversation_*.json"))
    assert not cwd_leak, f"snapshot leaked to CWD: {cwd_leak}"

    # File MUST be under ~/.clara/sessions/saved/
    saved_dir = clara_home / "sessions" / "saved"
    assert saved_dir.is_dir(), "expected saved/ subdirectory to be created"
    files = list(saved_dir.glob("clara_conversation_*.json"))
    ...
    assert "clara --resume 20260101_120000_abc123" in out, out


def test_save_conversation_empty_history_does_nothing(clara_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "clara_constants"]:
        sys.modules.pop(mod, None)
    import cli
...
def test_save_conversation_bare_shows_usage(clara_home, capsys):
    ...
    cli.ClaraCLI.save_conversation(stub, "/save")
...
def test_save_conversation_bad_format_shows_usage(clara_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "clara_constants"]:
        sys.modules.pop(mod, None)
    import cli

    stub = _make_stub_cli([{"role": "user", "content": "hi"}])
    cli.ClaraCLI.save_conversation(stub, "/save pdf")
