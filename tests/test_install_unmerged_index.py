...
The ``clara update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_parking_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m clara_cli.main gateway run`` — image name
    ``pythonw``, not ``clara.exe`` — so the ``taskkill /IM clara.exe`` guard
    ...
    # The clara.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM clara.exe' in text
