...
#
# Installs a real, earlier Clara the way a user does, applies ONE update route,
# and requires the checkout to land on this commit with a working `clara`.
#
...
# Keep sandbox state out of the default .clara-sandbox so a run never clobbers
...
SANDBOX_DIR_NAME=".clara-sandbox-e2e-$ROUTE"
export CLARA_DEV_SANDBOX_DIR="$SANDBOX_DIR_NAME"

SANDBOX_ROOT="$REPO_ROOT/$SANDBOX_DIR_NAME"
INSTALL_DIR="/home/clara/.clara/clara-agent"   # user-level layout (sandbox default)
FAKE_REMOTE="/work/repos/clara-agent.git"
...
LOG_DIR="${CLARA_E2E_LOG_DIR:-$(mktemp -d -t clara-install-e2e-logs.XXXXXX)}"
...
# ── helpers ────────────────────────────────────────────────────────────────
# Does the INSTALLED clara accept FLAG on `clara update`?
#
...
# that silently rots. `clara update --help` is the same surface a user meets,
# and argparse prints every option it accepts.
update_supports() {
  local flag="$1"
  in_sandbox "clara update --help 2>&1" | grep -qF -- "$flag"
}
...
require_clara_works() {
  local when="$1" out
  out="$(in_sandbox "clara --version" 2>&1)" \
    || { printf '%s\n' "$out" >&2; fail "clara --version failed $when"; }
  printf '%s\n' "$out" | sed 's/^/    /'
  ok "clara runs $when"
}

# ── install the earlier Clara ─────────────────────────────────────────────
...
require_clara_works 'after install'

# ── apply exactly one update route ─────────────────────────────────────────
case "$ROUTE" in
  update)
    step 'ROUTE: clara update'
    ...
    # clara which it accepts; older ones read the prompt from stdin, so close it.
    if update_supports --yes; then
      update_cmd="clara update --yes"
    else
      update_cmd="clara update </dev/null"
    fi
    if ! in_sandbox "cd $INSTALL_DIR && $update_cmd"; then
      collect_sandbox_logs update
      fail "clara update failed ($update_cmd)"
    fi
    require_landed_on_target 'clara update'
    require_clara_works 'after clara update'
    ;;
  installer)
    ...
    require_clara_works 'after installer re-run'
    ;;
esac
