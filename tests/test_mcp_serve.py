"""
Tests for mcp_serve — Clara MCP server.
...
def _isolate_clara_home(tmp_path, monkeypatch):
    """Redirect CLARA_HOME to a temp directory."""
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking clara_state schema."""
...
class TestEventBridgePollE2E:
    ...
    def test_poll_picks_up_new_conversation_on_db_change(
        ...
        # _poll_once reads <CLARA_HOME>/state.db for its mtime gate; the autouse
        # fixture points CLARA_HOME at tmp_path.
        db_path = tmp_path / "state.db"
